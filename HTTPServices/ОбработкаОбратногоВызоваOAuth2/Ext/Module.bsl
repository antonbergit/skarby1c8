...
Функция oauth2callbackGET(Запрос)
	...
	Если Не ЗначениеЗаполнено(code) Тогда
		...
		400,
		"Bad Request",
		НСтр("ru='Ошибка авторизации: отсутствует параметр ""code""';uk='Помилка авторизації: відсутній параметр ""code""'"));
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(state) Тогда
		...
		400,
		"Bad Request",
		НСтр("ru='Ошибка авторизации: отсутствует параметр ""state""';uk='Помилка авторизації: відсутній параметр ""state""'"));
	КонецЕсли;
	...
	Если Не ПараметрыАвторизации.Свойство("id") Тогда
		...
		400,
		"Bad Request",
		НСтр("ru='Ошибка авторизации: отсутствует ключ ""id"" в параметре ""state""';uk='Помилка авторизації: відсутній ключ ""id"" у параметрі ""state""'"));
	КонецЕсли;
...
Процедура УстановитьРазделениеСеанса(ПараметрыАвторизации)
	...
	Если Не ПараметрыАвторизации.Свойство("zone") Тогда
		ВызватьИсключение НСтр("ru='Отсутствует ключ ""zone"" в параметре ""state""';uk='Відсутній ключ ""zone"" у параметрі ""state""'");
	КонецЕсли;
