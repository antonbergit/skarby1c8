...
Процедура ИмпортироватьСправочникиСервер(ПараметрыИмпорта, ТекстСообщения)
	...
	Если Не Соун1.Открыта() Или Не СоунВ.Открыта() Тогда
		ТекстСообщения = НСтр("ru='Не удалось открыть дбф файлы! Загрузка классификаторов отменена! ';uk='Не удалось открыть дбф файлы! Загрузка классификаторов отменена! '");
		Возврат;
	КонецЕсли;
...
"%1';uk='Ошибка при импорте справочника ""Виды налоговых органов"":"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
					 
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Импорт прерван.';uk='Импорт прерван.'"));
			Возврат;
		КонецПопытки;
...
Процедура КаталогСоун1НачалоВыбораЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если НЕ КонтекстЭДОКлиент.ВыбранКорректныйФайл(ВыбранноеИмяФайлаСоун1, "SOUN1.DBF") Тогда
		...
		АдресСоун1 = "";
		ПоказатьПредупреждение(, НСтр("ru='Выбранный файл не является файлом Soun1.dbf';uk='Выбранный файл не является файлом Soun1.dbf'"));
	КонецЕсли;
...
Процедура КаталогСоунВНачалоВыбораЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Не КонтекстЭДОКлиент.ВыбранКорректныйФайл(ВыбранноеИмяФайлаСоунВ, "SOUN_V.DBF") Тогда
		...
		АдресСоунВ = "";
		ПоказатьПредупреждение(, НСтр("ru='Выбранный файл не является файлом Soun_v.dbf';uk='Выбранный файл не является файлом Soun_v.dbf'"));
	КонецЕсли;
	УстановитьДоступностьКнопкиИмпортировать();
