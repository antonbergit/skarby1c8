...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый Тогда
		ТекстПредупреждения = НСтр("ru='Копирование входящих сообщений запрещено!';uk='Копирование входящих сообщений запрещено!'");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Функция ПолучитьВложениеНаСервере(ИмяФайла)
	...
	Если СтрВложения.Количество() = 0 Тогда
		Результат.ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Вложение с именем  %1 не обнаружено.';uk='Вложение с именем  %1 не обнаружено.'"), Символ(34) + ИмяФайла + Символ(34));
		Возврат Результат;
	КонецЕсли;
...
Процедура ПрорисоватьПанельПриема()
	...
	Если ЭтоТребованиеФНС Тогда
		...
		Элементы.ЗаголовокНеотправленныхСообщений.Заголовок = 
			НСтр("ru='Оператору не отправлено извещение о получении документа';uk='Оператору не отправлено извещение о получении документа'");
...
Процедура УправлениеЭУ(КонтекстЭДОСервер)
	...
	Если ЭтоТребованиеОПредставленииДокументов Тогда
		...
		Элементы.ИнформацияСроки.Заголовок = 
			НСтр("ru='Обратите внимание на срок представления документов, указанный в приложенном файле';uk='Обратите внимание на срок представления документов, указанный в приложенном файле'");
	КонецЕсли;
		
	Если ЭтоТребованиеОПредставленииПоясненийКДекларацииНДС Тогда
		
		Элементы.КомандаСоздатьОтвет.Заголовок = 
			НСтр("ru='Подготовить пояснения';uk='Подготовить пояснения'");
		Элементы.ИнформацияСроки.Заголовок = 
			НСтр("ru='Ответ на требование должен быть направлен в течение 5 дней с момента получения';uk='Ответ на требование должен быть направлен в течение 5 дней с момента получения'");
