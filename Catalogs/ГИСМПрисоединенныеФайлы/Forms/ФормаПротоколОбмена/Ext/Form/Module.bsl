...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		ВызватьИсключение НСтр("ru='Протокол обмена может быть открыт только в контексте документа.';uk='Протокол обмена может быть открыт только в контексте документа.'");
	КонецЕсли;
...
Процедура ДеревоФайловВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ДанныеСтроки.Файл) Тогда
		...
		ПараметрыФормы.Вставить("Заголовок", СтрШаблон(НСтр("ru='Сообщения операции: %1';uk='Сообщения операции: %1'"), ДанныеСтроки.Операция));
...
Процедура ПоказатьСообщенияXML(Команда)
	...
	Иначе
		...
		ПоказатьПредупреждение(,НСтр("ru='По данной операции еще не создано сообщений';uk='По данной операции еще не создано сообщений'"));
...
Процедура ПодтвердитьПоступление_ПослеПолученияДокументовКПодтверждению(ДокументыКПодтверждению, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru='В форме протокола обмена подтвержение получения по нескольким документам не поддерживается.';uk='В форме протокола обмена подтвержение получения по нескольким документам не поддерживается.'");
...
#КонецОбласти
	...
	ПредставлениеЭлемента = НСтр("ru='Представление даты ""<не передано>""';uk='Представление даты ""<не передано>""'");
	...
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<не передано>';uk='<не передано>'"));
...
Функция ПредставлениеОперации(Операция, ДокументСсылка, ВыборкаПоФайлам = Неопределено)
	...
	Если Операция = Перечисления.ОперацииОбменаГИСМ.ПередачаДанных Тогда
		...
		Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.МаркировкаТоваровГИСМ") Тогда
			...
			Если ВыборкаПоФайлам = Неопределено Тогда
				Возврат НСтр("ru='Передача сведений о маркированной продукции';uk='Передача сведений о маркированной продукции'");
			Иначе
				Возврат СтрШаблон(НСтр("ru='Передача сведений о маркированной продукции, Версия %1';uk='Передача сведений о маркированной продукции, Версия %1'"), ВыборкаПоФайлам.Версия);
			КонецЕсли;
			
		ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.УведомлениеОбОтгрузкеМаркированныхТоваровГИСМ") Тогда
			
			Если ВыборкаПоФайлам = Неопределено Тогда
				Возврат НСтр("ru='Передача уведомления об отгрузке маркированной продукции';uk='Передача уведомления об отгрузке маркированной продукции'");
			Иначе
				Возврат СтрШаблон(НСтр("ru='Передача уведомления об отгрузке маркированной продукции, Версия %1';uk='Передача уведомления об отгрузке маркированной продукции, Версия %1'"), ВыборкаПоФайлам.Версия);
			КонецЕсли;
			
		ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаявкаНаВыпускКиЗГИСМ") Тогда
			
			Если ВыборкаПоФайлам = Неопределено Тогда
				Возврат НСтр("ru='Передача заявки на выпуск КиЗ';uk='Передача заявки на выпуск КиЗ'");
			Иначе
				Возврат СтрШаблон(НСтр("ru='Передача заявки на выпуск КиЗ, Версия %1';uk='Передача заявки на выпуск КиЗ, Версия %1'"), ВыборкаПоФайлам.Версия);
			КонецЕсли;
		...
		Иначе
			...
			Если ВыборкаПоФайлам = Неопределено Тогда
				Возврат СтрШаблон(НСтр("ru='Передача документа ""%1""';uk='Передача документа ""%1""'"), ПредставлениеОбъекта);
			Иначе
				Возврат СтрШаблон(НСтр("ru='Передача документа ""%1"", Версия %2';uk='Передача документа ""%1"", Версия %2'"), ПредставлениеОбъекта, ВыборкаПоФайлам.Версия);
			КонецЕсли;
	...
	ИначеЕсли Операция = Перечисления.ОперацииОбменаГИСМ.ПолучениеТоваров Тогда
		...
		Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.УведомлениеОПоступленииМаркированныхТоваровГИСМ") Тогда
			Возврат НСтр("ru='Уведомление о поступлении маркированных товаров';uk='Уведомление о поступлении маркированных товаров'");
		КонецЕсли;
		
		Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаявкаНаВыпускКиЗГИСМ") Тогда
			Возврат НСтр("ru='Уведомление о выпуске КиЗ эмитентом';uk='Уведомление о выпуске КиЗ эмитентом'");
		КонецЕсли;
		
	ИначеЕсли Операция = Перечисления.ОперацииОбменаГИСМ.ПолучениеНовогоСтатуса Тогда
		
		Возврат СтрШаблон(НСтр("ru='Обновлено состояние: %1';uk='Обновлено состояние: %1'"), ВыборкаПоФайлам.СтатусОбработкиЭмитентом);
...
Процедура ЗаполнитьПоОснованиюДокументаУведомлениеОСписанииКиЗ(Основание)
	...
	|		ПО СтатусыИнформированияГИСМ.Документ = ТаблицаДокументы.Ссылка
	|ГДЕ
	|	ТаблицаДокументы.Основание = &Основание
	|
	|УПОРЯДОЧИТЬ ПО
...
Процедура ОбработатьПередачуДанных(ВыборкаПоФайлам, СтрокаВторогоУровня, ПередачаДанных)
	...
	Если ИнтеграцияГИСМ.ОперацииПередачиДанных().Найти(ВыборкаПоФайлам.Операция) <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(СтрокаТЧ.ОтветНаЗапросКвитанции) Тогда
				...
				Если СтрокаТЧ.ОтветНаЗапросСостояниеОбработки = Перечисления.СтатусыОбработкиСообщенийГИСМ.Принято Тогда
					...
					Если СтатусЗапросаКвитанции = Перечисления.СтатусыОбработкиСообщенийГИСМ.Принято Тогда
						...
						СтрокаВторогоУровня.Состояние = НСтр("ru='Квитанция получена: Документ принят в обработку';uk='Квитанция получена: Документ принят в обработку'");
						
					ИначеЕсли СтатусЗапросаКвитанции = Перечисления.СтатусыОбработкиСообщенийГИСМ.Отклонено Тогда
						
						СтрокаВторогоУровня.Состояние = НСтр("ru='Квитанция получена: Документ отклонен';uk='Квитанция получена: Документ отклонен'");
					...
					ИначеЕсли СтатусЗапросаКвитанции = Перечисления.СтатусыОбработкиСообщенийГИСМ.Обрабатывается Тогда
						...
						СтрокаВторогоУровня.Состояние = НСтр("ru='Ожидается получение квитанции';uk='Ожидается получение квитанции'");
					...
					ИначеЕсли СтатусЗапросаКвитанции = Перечисления.СтатусыОбработкиСообщенийГИСМ.НаМодерации Тогда
						...
						СтрокаВторогоУровня.Состояние = НСтр("ru='Ожидается утвержение в ФНС';uk='Ожидается утвержение в ФНС'");
						СтрокаВторогоУровня.ИндексКартинки = 3;
						
					Иначе
						
						СтрокаВторогоУровня.Состояние = НСтр("ru='Ошибка обработки';uk='Помилка обробки'");
						СтрокаВторогоУровня.УсловноеОформление = "УсловноеОформлениеОшибка";
				...
				ИначеЕсли СтрокаТЧ.ОтветНаЗапросСостояниеОбработки = Перечисления.СтатусыОбработкиСообщенийГИСМ.Отклонено Тогда
					...
					СтрокаВторогоУровня.Состояние = НСтр("ru='Документ отклонен';uk='Документ отклонен'");
					СтрокаВторогоУровня.УсловноеОформление = "УсловноеОформлениеОшибка";
					СтрокаВторогоУровня.ИндексКартинки = 4;
		...
		ИначеЕсли ЗначениеЗаполнено(ВыборкаПоФайлам.Ссылка) Тогда
			...
			СтрокаВторогоУровня.Состояние = НСтр("ru='Ожидается передача данных';uk='Ожидается передача данных'");
			СтрокаВторогоУровня.ИндексКартинки = 3;
