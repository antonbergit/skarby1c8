...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВходВПрограммуРазрешен
	   ...
	   И СрокДействия <= НачалоДня(ТекущаяДатаСеанса()) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Ограничение должно быть до завтра или более.';uk='Обмеження повинне бути до завтра або більш.'"),, "ВходВПрограммуРазрешен",, Отказ);
	КонецЕсли;
		...
		Для Каждого Строка Из ЭлементыДерева Цикл
			...
			Если Строка.ЭтоНесуществующаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					...
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Роль ""%1"" в строке %%1 не найдена в метаданных.';uk='Роль ""%1"" у рядку %%1 не знайдена в метаданих.'"), Строка.Синоним));
			КонецЕсли;
...
"позначку на видалення із цього зовнішнього користувача.'"));
		Возврат;
	КонецЕсли;
...
" %1.'"),
	...
	Если ДействияВФорме.СвойстваЭлемента = "Редактирование" Тогда
		Рекомендация = Символы.ПС
			+ НСтр("ru='Нажмите ""Записать"", чтобы устранить проблему и не выводить это предупреждение.';uk='Натисніть ""Записати"", щоб усунути проблему й не виводити це попередження.'");
