...
"флажок ""Показывать пользователей нижестоящих групп"".';uk='Для перетаскування користувача в групи необхідно відключити"
"прапорець ""Показувати користувачів нижчестоящих груп"".'"));
		Возврат;
	КонецЕсли;
	...
	Если Строка = ХранимыеПараметры.ГруппаВсеПользователи
		...
		СообщениеПользователю = Новый Структура("Сообщение, ЕстьОшибки, Пользователи",
			НСтр("ru='Из групп с типом участников ""Все пользователи заданного типа"" исключение пользователей невозможно.';uk='З груп з типом учасників ""Всі користувачі заданого типу"" виключення користувачів неможливо.'"),
			Истина,
			Неопределено);
	Иначе
		...
		Если ГруппаПомеченаНаУдаление Тогда
			ШаблонДействия = ?(Перемещение, НСтр("ru='Группа ""%1"" помечена на удаление. %2';uk='Група ""%1"" позначена на видалення. %2'"), 
				НСтр("ru='Группа ""%1"" помечена на удаление. %2';uk='Група ""%1"" позначена на видалення. %2'"));
		...
		Если КоличествоПользователей = 1 Тогда
			Если ДействиеИсключитьПользователя Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Исключить пользователя ""%1"" из группы ""%2""?';uk='Виключити користувача ""%1"" із групи ""%2""?'"),
		...
		Иначе
			Если ДействиеИсключитьПользователя Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Исключить пользователей (%1) из группы ""%2""?';uk='Виключити користувачів (%1) із групи ""%2""?'"),
...
"так как в число ее участников входят все пользователи выбранных видов.';uk='Неможливо додати підгрупу до групи ""%1"", "
"тому що в число її учасників входять усі користувачі обраних видів.'"),
