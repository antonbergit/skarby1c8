...
Процедура ВариантыДополнительногоОтчетаПередУдалением(Элемент, Отказ)
	...
	Если НЕ Вариант.Пользовательский Тогда
		ПоказатьПредупреждение(, НСтр("ru='Пометка на удаление предопределенного варианта отчета запрещена.';uk='Позначка на видалення визначеного варіанта звіту заборонена.'"));
		Возврат;
	КонецЕсли;
	
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Пометить ""%1"" на удаление?';uk='Позначити ""%1"" на видалення?'"), Вариант.Наименование);
...
Процедура НастроитьВидимость(Команда)
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru='Для настройки видимости команд печати обработку необходимо записать. Продолжить?';uk='Для налаштування видимості команд друку обробку необхідно записати. Продовжити?'");
...
Процедура ПродолжитьЗаписьНаКлиенте(Результат, ЗакрытьПослеЗаписи)  Экспорт
	...
	Если ПараметрыЗаписи.РегистрацияОбработки Тогда
		ОбновитьПовторноИспользуемыеЗначения();
		ТекстОповещения = НСтр("ru='Для применения изменений в открытых окнах необходимо их закрыть и открыть заново.';uk='Для застосування змін до відкритих вікон необхідно їх закрити і відкрити знову.'");
...
"3. ""[Open]"" - отменить загрузку и показать список конфликтующих отчетов.';uk='Внутрішнє ім`я звіту ""[Name]"""
...
"3. ""[Open]"" - скасувати завантаження і показати список конфліктуючих звітів.'");
		Иначе
...
"3. ""[Open]"" - отменить загрузку и показать список конфликтующих обработок.';uk='Внутрішнє ім`я обробки ""[Name]"""
...
"3. ""[Open]"" - скасувати завантаження і показати список конфліктуючих обробок.'");
		КонецЕсли;
...
"3. ""[Open]"" - открыть карточку конфликтующего отчета.';uk='Внутрішнє ім`я звіту ""[Name]"""
"вже зайнято існуючим додатковим звітом [List]."
""
"Виберіть:"
"1. ""[Continue]"" - завантажити новий звіт в режимі налагодження."
"2. ""[Disable]"" - завантажити новий звіт, відключивши публікацію конфліктуючого звіту."
"3. ""[Open]"" - відкрити картку конфліктуючого звіту.'");
...
"3. ""[Open]"" - открыть карточку конфликтующей обработки.';uk='Внутрішнє ім`я обробки ""[Name]"""
...
"профиля безопасности %1, в котором будут разрешены только следующие операции:';uk='Додатковий звіт або обробка буде підключатися до програми "
"з використанням ""персонального"""
"профілю безпеки %1, в якому будуть дозволені тільки "
"наступні операції:'"),
				БезопасныйРежим);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 не является корректным режимом подключения для дополнительных отчетов и обработок, требующих разрешений на использование профилей безопасности!';uk='%1 не є коректним режимом підключення для "
"додаткових звітів і обробок, що потребують дозволів на "
			...
			Если Не ПустаяСтрока(ЭлементКоманда.ЗаменяемыеКоманды) Тогда
				...
				Если КоличествоЗаменяемых > 0 Тогда
					Если КоличествоЗаменяемых = 1 Тогда
						ШаблонКомментария = НСтр("ru='Заменяет стандартную команду печати %1';uk='Заміняє стандартну команду друку %1'");
					Иначе
						ШаблонКомментария = НСтр("ru='Заменяет стандартные команды печати: %1';uk='Заміняє стандартні команди друку: %1'");
					КонецЕсли;
