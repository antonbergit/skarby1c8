...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Справочник не предназначен для ручного заполнения!';uk='Довідник не призначений для ручного заповнення!'"));
		Отказ = Истина;
		Возврат;
...
Процедура ВыгрузитьПротокол(Команда)
	...
	ПолучитьФайл(Адрес, НСтр("ru='Протокол';uk='Протокол'") + "_" + Объект.ИдентификаторОтправкиНаСервере + ".html");
...
Процедура ОбновитьФорму()
	...
	Иначе
		...
		ИначеЕсли Объект.СтатусОтправки = Перечисления.СтатусыОтправки.ПринятЕстьОшибки Тогда
			...
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ru='Квитанция о сдаче отчета получена, протокол ошибок получен';uk='Квитанция о сдаче отчета получена, протокол ошибок получен'");
			Элементы.НадписьСтатусОтправки.ЦветТекста = ЦветСиний;
			Элементы.НадписьСтатусОтправки.ГиперСсылка = Истина;
		ИначеЕсли Объект.СтатусОтправки = Перечисления.СтатусыОтправки.Сдан Тогда
			Элементы.КартинкаСтатусОтправки.Картинка = БиблиотекаКартинок.ПисьмоПодтверждениеПолучено;
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ru='Квитанция о сдаче отчета получена';uk='Квитанция о сдаче отчета получена'");
			Элементы.НадписьСтатусОтправки.ЦветТекста = ЦветСиний;
			Элементы.НадписьСтатусОтправки.ГиперСсылка = Истина;
		КонецЕсли;
	КонецЕсли;
