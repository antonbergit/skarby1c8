...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьДанныеБанка(МФОБанка, КоррСчетБанка, Объект.Банк, Объект.Владелец);
...
Процедура МФОБанкаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ПустаяСтрока(МФОБанка) Тогда
...
// Процедура - обработчик события ОкончаниеВводаТекста поля МФОБанка.
//
&НаКлиенте
Процедура МФОБанкаОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Если ВебКлиент Тогда
		...
		Если СтрДлина(Текст) > 6 Тогда
			...
			Сообщение.Текст = НСтр("ru='Введенное значение превышает допустимую длину МФО 6 символов!';uk='Введене значення перевищує допустиму довжину МФО 6 символів!'");
			Сообщение.Сообщить();
...
КонецПроцедуры // МФОБанкаОкончаниеВводаТекста()
...
// Процедура - обработчик события ОбработкаВыбора поля МФОБанкаРасчетов.
//
&НаКлиенте
...
// Процедура - обработчик события ОкончаниеВводаТекста поля МФОБанкаДляРасчетов
//
&НаКлиенте
Процедура МФОБанкаДляРасчетовОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка)
...
КонецПроцедуры // МФОБанкаДляРасчетовОкончаниеВводаТекста()
...
Процедура МФОБанкаДляРасчетовОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ПустаяСтрока(МФОБанкаДляРасчетов) Тогда
		
		ЭтаФорма.ТекущийЭлемент = Элементы.МФОБанкаДляРасчетов;
...
Процедура ИспользуетсяБанкДляРасчетовПриИзменении(Элемент)
	...
	Элементы.МФОБанкаДляРасчетов.Видимость      = ИспользуетсяБанкДляРасчетов;
...
Процедура НомерСчетаОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Если ВебКлиент Тогда
		...
		Если СтрДлина(Текст) > 20 Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Введенное значение превышает допустимую длину номера счета 20 символов!';uk='Введене значення перевищує припустиму довжину номера рахунку 20 символів!'");
			Сообщение.Сообщить();
...
Процедура ОткрытьФормуВыбораБанка(ЭтоБанк, СписокНайденныхБанков = Неопределено)
	...
	ОткрытьФорму("Справочник.Банки.ФормаВыбора", ПараметрыФормы, ?(ЭтоБанк, Элементы.МФОБанка, Элементы.МФОБанкаДляРасчетов));
	
КонецПроцедуры // ОткрытьФормуВыбораБанка()

&НаСервереБезКонтекста
Функция ДеятельностьБанкаПрекращена(МФО)
...
Функция НайтиБанки(ТекстДляПоиска, Поле, Валютный = Ложь)
	...
	ЭтоБанк = (Поле = "МФОБанка") ИЛИ (Поле = "КоррСчетБанка");
	...
	Если ПустаяСтрока(ТекстДляПоиска) Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Поле ""%1"" заполнено не корректно.';uk='Поле ""%1"" заповнене не коректно.'"), 
			?(СтрНайти(Поле, "МФО") > 0, "МФО", "Корр. счет")
			);
	...
	Если СтрНайти(Поле, "МФО") = 1 Тогда
	...
	ИначеЕсли ОбластьПоиска = "Код" И СписокНайденныхБанков.Количество() = 1 Тогда
		...
		Если Поле = "МФОБанкаДляРасчетов" Тогда
...
//
&НаСервереБезКонтекста
Процедура ЗаполнитьДанныеБанка(МФОБанка, КоррСчетБанка, Знач Банк, Знач ВладелецСчета)

	ЗаполнитьМФОиКоррСчет(Банк, МФОБанка, КоррСчетБанка);
...
Процедура ПроверитьВозможностьИзменений(Отказ)
	...
	Если Выборка.Следующий() Тогда
		ТекстСообщения = НСтр("ru='В базе присутствуют эквайринговые терминалы, в которых выбран текущий банковский счет. Изменение валюты расчетов запрещено.';uk='У базі присутні еквайрингові термінали, у яких обраний поточний банківський рахунок. Зміна валюти розрахунків заборонена.'");
...
Процедура Подключаемый_ВыполнитьНазначаемуюКоманду(Команда)
	Если НЕ ДополнительныеОтчетыИОбработкиКлиент.ВыполнитьНазначаемуюКомандуНаКлиенте(ЭтаФорма, Команда.Имя) Тогда
		...
		ДополнительныеОтчетыИОбработкиКлиент.l(ЭтаФорма, РезультатВыполнения);
	КонецЕсли;
КонецПроцедуры
