...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОтказИзменитьСчетУчета(Ссылка) Тогда
		Элементы.ГруппаСчетовУчета.Подсказка = НСтр("ru='В базе есть движения по этому банковскому счету! Изменение счета учета запрещено!';uk='У базі є рухи по цьому банківському рахункові! Зміна рахунку обліку заборонена!'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ВладелецОрганизация Тогда
		Отказ = Истина;
		ПоказатьПредупреждение(, НСтр("ru='Счета учетов редактируются только для банковских счетов организаций!';uk='Рахунки обліків редагуються тільки для банківських рахунків організацій!'"));
	КонецЕсли;
