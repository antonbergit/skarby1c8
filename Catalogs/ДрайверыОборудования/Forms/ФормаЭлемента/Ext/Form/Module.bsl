...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ИдентификаторОбъекта.ПодсказкаВвода = ?(ПоставляемыйВСоставеКонфигурации, НСтр("ru='<Не указан>';uk='<Не вказано>'"), 
		НСтр("ru='<ProgID компоненты не введен>';uk='<ProgID компоненти не введено>'"));
	Элементы.ИмяМакетаДрайвера.ПодсказкаВвода = ?(ПоставляемыйВСоставеКонфигурации, НСтр("ru='<Не указан>';uk='<Не вказано>'"), "");
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПустаяСтрока(Объект.ТипОборудования) Тогда 
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Тип оборудования не указан.';uk='Тип обладнання не зазначений.'")); 
		Возврат;
	КонецЕсли;
...
"Записать данные и продолжить?';uk='Продовження операції можливо тільки після запису даних."
"Записати дані й продовжити?'");
...
Процедура ВыгрузитьФайлДрайвераКоманда(Команда)
	...
	Если ПоставляемыйВСоставеКонфигурации Тогда
		...
		Если ПустаяСтрока(Объект.ИмяМакетаДрайвера) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Имя макета драйвера не указано.';uk='Ім`я макета драйвера не зазначене.'"));
			Возврат;
		Иначе
	...
	Иначе 
		...
		Если ПустаяСтрока(Объект.ИмяФайлаДрайвера) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Файл драйвера не загружен.';uk='Файл драйвера не завантажений.'"));
			Возврат;
		КонецЕсли;
...
Функция ПолучитьОписаниеРевизииИнтерфейса(РевизияИнтерфейса)
	...
	Если НоваяАрхитектура И РевизияИнтерфейса > 1000 Тогда
		Результат = Символы.НПП + НСтр("ru='(Версия требований к разработке драйверов %Версия%)';uk='(Версия требований к разработке драйверов %Версия%)'");
...
Процедура ОбновитьТекущийСтатусДрайвера()
	...
	Если НоваяАрхитектура И ИнтеграционныйКомпонент Тогда
		ТекущийСтатусДрайвера = НСтр("ru='Установлен интеграционный компонент.';uk='Установлен интеграционный компонент.'") + Символы.НПП;
		ТекущийСтатусДрайвера = ТекущийСтатусДрайвера + ?(ОсновнойДрайверУстановлен, НСтр("ru='Установлена основная поставка драйвера.';uk='Встановлена основна поставка драйвера.'"),
																					 НСтр("ru='Основная поставка драйвера не установлена.';uk='Основна поставка драйвера не встановлена.'")); 
	Иначе
		ТекущийСтатусДрайвера = НСтр("ru='Установлен на текущем компьютере.';uk='Встановлено на поточному комп`ютері.'");
	КонецЕсли;
...
Процедура ПолучениеОбъектаДрайвераЗавершение(ОбъектДрайвера, Параметры) Экспорт
	...
	ИначеЕсли ПустаяСтрока(Объект.ИдентификаторОбъекта) И ПоставляемыйВСоставеКонфигурации Тогда
		ТекущийСтатусДрайвера = НСтр("ru='Установка драйвера не требуется.';uk='Установка драйвера не потрібна.'");
	Иначе
...
Процедура ОбновитьСостояниеЭлементов();
	...
	Если НЕ ПустаяСтрока(ИмяФайлДрайвераНаФорме) Или ПоставляемыйВСоставеКонфигурации Тогда
		Если ПустаяСтрока(Объект.ИдентификаторОбъекта) Тогда
			ДополнительнаяИнформация = НСтр("ru='Не указан ProgID компоненты или установка драйвера не требуется.';uk='Не зазначений ProgID компоненти або установка драйвера не потрібна.'");
		...
		Иначе
			ДополнительнаяИнформация = НСтр("ru='Драйвер поставляется в виде архива.';uk='Драйвер поставляется в виде архива.'")
				+ ?(ПустаяСтрока(Объект.ВерсияДрайвера), "", Символы.ПС + НСтр("ru='Версия драйвера в архиве:';uk='Версия драйвера в архиве:'")
