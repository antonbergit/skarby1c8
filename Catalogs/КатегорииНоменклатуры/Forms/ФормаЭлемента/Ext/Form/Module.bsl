...
Процедура ДобавитьДопРеквизит(ВладелецНабораЭлементов)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ТекстВопроса = НСтр("ru='Перед началом управления свойствами необходимо записать объект. Записать?';uk='Перед початком керування властивостями необхідно записати об`єкт. Записати?'");
		Ответ = Неопределено;
