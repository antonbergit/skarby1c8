...
Процедура ЗаполнитьДеревоОперандов(ИсключитьСсылку, ЭтоФормированиеЦен, ВключаяЦеныНоменклатуры, ВключаяЦеныКонтрагентов)
	...
	|
	|Итоги Выбор Когда ЭтоВидЦенНоменклатуры = Истина Тогда "+НСтр("ru='""ЦЕНЫ НОМЕНКЛАТУРЫ""';uk='""ЦІНИ НОМЕНКЛАТУРИ""'")+" Иначе "+НСтр("ru='""ЦЕНЫ КОНТРАГЕНТОВ""';uk='""ЦІНИ КОНТРАГЕНТІВ""'")+" Конец КАК Представление, 1 КАК Картинка 
	...
	Если ЭтоФормированиеЦен Тогда
		...
		НоваяГруппа.Представление	= НСтр("ru='РАСЧЕТНЫЕ ЗНАЧЕНИЯ';uk='РОЗРАХУНКОВІ ЗНАЧЕННЯ'");
		
		ДобавитьРасчетныйОперанд(НоваяГруппа, "ТекущееЗначение",		НСтр("ru='Текущее значение';uk='Поточне значення'"));
		ДобавитьРасчетныйОперанд(НоваяГруппа, "ПоследняяЦенаВПриходе",	НСтр("ru='Последняя цена поступления';uk='Остання ціна надходження'"));
		ДобавитьРасчетныйОперанд(НоваяГруппа, "ПоследняяЦенаВРасходе",	НСтр("ru='Последняя цена реализации';uk='Остання ціна реалізації'"));
		
		Если ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций") Тогда
			
			Если Справочники.Валюты.НайтиПоКоду("840") <> Неопределено Тогда
				
				ДобавитьРасчетныйОперанд(НоваяГруппа, "КурсДоллара", 	НСтр("ru='Текущий курс доллара';uk='Поточний курс долара'"));
				
			КонецЕсли;
			
			Если Справочники.Валюты.НайтиПоКоду("978") <> Неопределено Тогда
				
				ДобавитьРасчетныйОперанд(НоваяГруппа, "КурсЕвро", 		НСтр("ru='Текущий курс евро';uk='Поточний курс євро'"));
				
			КонецЕсли;
			
		КонецЕсли;
		
		ДобавитьРасчетныйОперанд(НоваяГруппа, "Себестоимость", 			НСтр("ru='Себестоимость';uk='Собівартість'"));
		
		Если Константы.ВалютаУчета.Получить() <> Константы.НациональнаяВалюта.Получить() Тогда
			
			ДобавитьРасчетныйОперанд(НоваяГруппа, "СебестоимостьНацВалюта", НСтр("ru='Себестоимость (нац. валюта)';uk='Собівартість (нац. валюта)'"));
	...
	Если ВключаяЦеныНоменклатуры = Ложь Тогда
		...
		НайденнаяСтрока = ДеревоРезультата.Строки.Найти(НСтр("ru='ЦЕНЫ НОМЕНКЛАТУРЫ';uk='ЦІНИ НОМЕНКЛАТУРИ'"), "Представление", Ложь);
	...
	Если ВключаяЦеныКонтрагентов = Ложь Тогда
		...
		НайденнаяСтрока = ДеревоРезультата.Строки.Найти(НСтр("ru='ЦЕНЫ КОНТРАГЕНТОВ';uk='ЦІНИ КОНТРАГЕНТІВ'"), "Представление", Ложь);
		Если НайденнаяСтрока <> Неопределено Тогда
...
Процедура ЗаполнитьДеревоОператоров()
	...
	ГруппаСтрок.Наименование	= НСтр("ru='АРИФМЕТИЧЕСКИЕ ОПЕРАТОРЫ';uk='АРИФМЕТИЧНІ ОПЕРАТОРИ'");
	ГруппаСтрок.Картинка		= 1;
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ru='Сложение ""+""';uk='Додавання ""+""'");
	НоваяСтрока.Оператор		= " + ";
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ru='Вычитание ""-""';uk='Вирахування ""-""'");
	НоваяСтрока.Оператор		= " - ";
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ru='Умножение ""*""';uk='Множення ""*""'");
	НоваяСтрока.Оператор		= " * ";
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ru='Деление ""/""';uk='Ділення ""/""'");
	НоваяСтрока.Оператор		= " / ";
	
	ГруппаСтрок 				= ДеревоОператоров.Строки.Добавить();
	ГруппаСтрок.Наименование	= НСтр("ru='ПРОЧЕЕ';uk='ІНШЕ'");
	ГруппаСтрок.Картинка		= 1;
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ru='Процент ""1%""';uk='Відсоток ""1%""'");
	НоваяСтрока.Оператор		= " %1";
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ru='Процент ""5%""';uk='Відсоток ""5%""'");
	НоваяСтрока.Оператор		= " %5";
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ru='Процент ""20%""';uk='Відсоток ""20%""'");
	НоваяСтрока.Оператор		= " %20";
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ru='Процент ""50%""';uk='Відсоток ""50%""'");
	НоваяСтрока.Оператор		= " %50";
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ru='Открывающая скобка ""(""';uk='Відкриваюча дужка ""(""'");
	НоваяСтрока.Оператор		= " (";
	
	НоваяСтрока 				= ГруппаСтрок.Строки.Добавить();
	НоваяСтрока.Наименование	= НСтр("ru='Закрывающая скобка "")""';uk='Закриваюча дужка "")""'");
	НоваяСтрока.Оператор		= ") ";
...
Процедура ОператорыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если СокрЛП(ТекстВставки) = "%1"
		...
		Если ДанныеОперанда = Неопределено 
			...
			ТекстСообщения = НСтр("ru='Укажите вид цен, от которого необходимо вычислить процент';uk='Вкажіть вид цін, від якого необхідно обчислити відсоток'");
...
Процедура ФормулаПриИзменении(Элемент)
	...
	Если Найти(Формула, ",") > 0 Тогда
		...
		ТекстСообщения = НСтр("ru='Для указания дробной части необходимо использовать точку, а не запятую.';uk='Для вказівки дробової частини необхідно використовувати точку, а не кому.'");
...
Процедура ПроверитьФормулу(Команда)
	...
	Если Ошибки = Неопределено Тогда
		...
		ТекстПредупреждения = НСтр("ru='Формула пригодна для расчетов.';uk='Формула придатна для розрахунків.'");
		ПоказатьПредупреждение(, ТекстПредупреждения, , НСтр("ru='Проверка формулы';uk='Перевірка формули'"));
