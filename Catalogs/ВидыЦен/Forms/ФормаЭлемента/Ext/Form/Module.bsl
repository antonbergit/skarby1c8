...
"Загрузка данных завершена.';uk='Ціни номенклатури."
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(ТекущийОбъект.Ссылка) Тогда
		...
		Если НЕ РезультатВыполненияЗапроса.Пустой()
			...
			ТекстСообщения = НСтр("ru='Текущий вид цен используется в розничных структурных единицах, по этому для него можно использовать только национальную валюту.';uk='Поточний вид цін використовується в роздрібних структурних одиницях, по цьому для нього можна використовувати тільки національну валюту.'");
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ КэшЗначений.РучнаяКорректировкаФлага Тогда
		
		Если Объект.ТипВидаЦен = КэшЗначений.ДинамическийПроцент
			Тогда
...
"Продолжить?';uk='Увага!"
"Всі ціни по цьому стовпчикові прайс-листа будуть видалені. "
"Продовжити?'");
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru='Операция имеет смыл только для записанных объектов';uk='Операція має сенс тільки для записаних об`єктів'");
...
"Записать и продолжить?';uk='Для розрахунків цін необхідно записати поточний об`єкт."
"Записати й продовжити?'");
...
Процедура РассчитыватьАвтоматическиПриИзменении(Элемент)
	...
	Если НЕ Объект.РассчитыватьАвтоматически Тогда
...
Процедура ЗагрузитьЦеныИзВнешнегоИсточника(Команда)
	...
	Если Объект.ТипВидаЦен <> КэшЗначений.Статический Тогда
		...
		ТекстСообщения = НСтр("ru='Загрузка предназначена только для статических видов цен.';uk='Завантаження призначене тільки для статичних видів цін.'");
		ПоказатьПредупреждение(Неопределено, ТекстСообщения, 15, НСтр("ru='Загрузить цены из внешнего источника';uk='Завантажити ціни із зовнішнього джерела'"));
		Возврат;
