...
"так як її профіль позначений на видалення.'"));
	КонецЕсли;
		...
		Если Объект.Ссылка = Справочники.ГруппыДоступа.Администраторы
		   И ТипЗнч(ТекущаяСтрока.Пользователь) <> Тип("СправочникСсылка.Пользователи") Тогда
			...
			Если ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ru='Внешний пользователь ""%2"" недопустим в предопределенной группе доступа Администраторы.';uk='Зовнішній користувач ""%2"" неприпустимий у визначеній групі "
"доступу Адміністратори.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Внешний пользователь ""%2"" в строке %1 недопустим в предопределенной группе доступа Администраторы.';uk='Зовнішній користувач ""%2"" в рядку %1 неприпустимий у "
"визначеній групі доступу Адміністратори.'");
			...
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				...
				ТекстДляНесколькихОшибок = НСтр("ru='Группа пользователей ""%2"" в строке %1 недопустима в предопределенной группе доступа Администраторы.';uk='Група користувачів ""%2"" в рядку %1 неприпустима в "
"наперед визначеній групі доступу Адміністратори.'");
			Иначе
				...
				ТекстДляНесколькихОшибок = НСтр("ru='Группа внешних пользователей ""%2"" в строке %1 недопустима в предопределенной группе доступа Администраторы.';uk='Група зовнішніх користувачів ""%2"" в рядку %1 неприпустима в "
"наперед визначеній групі доступу Адміністратори.'");
			КонецЕсли;
		...
		Если НедопустимыеПользователи.Найти(ТекущаяСтрока.Пользователь) <> Неопределено Тогда
			...
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстДляОднойОшибки      = НСтр("ru='Группа пользователей ""%2"" недопустима для указанного типа участников.';uk='Група користувачів ""%2"" неприпустима для зазначеного типу "
"учасників.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Группа пользователей ""%2"" в строке %1 недопустима для указанного типа участников.';uk='Група користувачів ""%2"" в рядку %1 неприпустима для зазначеного "
"типу учасників.'");
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
...
"Їх не можна видалити в цьому списку.'"));
		Иначе
			ПоказатьПредупреждение(,
...
Процедура ВыбратьПодобратьПользователейЗавершение(ВыборИПодборВнешнихПользователей, Подобрать) Экспорт
	...
	Если ВыборИПодборВнешнихПользователей Тогда
		...
		Если Не ИспользоватьВнешнихПользователей Тогда
			ПоказатьПредупреждение(, НСтр("ru='Ведение внешних пользователей отключено в настройках программы.';uk='Ведення зовнішніх користувачів відключено в налаштуваннях програми.'"));
