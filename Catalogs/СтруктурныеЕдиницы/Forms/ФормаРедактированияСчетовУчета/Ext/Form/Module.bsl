...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОтказИзменитьСчетУчета(Ссылка) Тогда
		Элементы.ГруппаСчетовУчета.Подсказка = НСтр("ru='В базе есть движения по этой розничной точке! Изменение счета учета запрещено!';uk='У базі є рухи по цій роздрібній точці! Зміна рахунку обліку заборонена!'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ЭтоРозницаСуммовойУчет Тогда
		Отказ = Истина;
		ПоказатьПредупреждение(, НСтр("ru='Счета учетов редактируются только для розницы с суммовым учетом!';uk='Рахунки обліків редагуються тільки для роздробу із сумовим обліком!'"));
	КонецЕсли;
