...
"нажмите кнопку Отключить или программно очистите константу Главный узел.';uk='Довідник ""%1"" не може використовуватися"
"в інформаційній базі з непідтвердженим скасуванням головного вузла."
""
"Для відновлення зв`язку з головним вузлом запустіть 1С:Підприємство й"
"натисніть кнопку Відновити або програмно встановіть головний вузол,"
"збережений у константі Головний вузол."
""
"Для підтвердження скасування зв`язку з головним вузлом запустіть 1С:Підприємство й"
"натисніть кнопку Відключити або програмно очистіть константу Головний вузол.'"),
...
"расширения для разделенных пользователей не подключены.';uk='Неможливо відновити ідентифікатори об`єктів розширень"
"у сеансі неподіленого користувача, оскільки"
...
"не найден в справочнике ""%2"".';uk='Ідентифікатор ""%1"""
...
"""%3"".'"),
				...
				Иначе
					ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Идентификатору ""%1"""
"найденному в справочнике ""%2"","
"соответствует удаленный объект метаданных.';uk='Ідентифікатору ""%1"""
"знайденому в довіднику ""%2"","
"відповідає видалений об`єкт метаданих.'"),
...
"полное имя которого отличается от заданного в идентификаторе.';uk='Ідентифікатору ""%1"""
"знайденому в довіднику ""%2"","
...
"но значение реквизита ""Пометка удаления"" установлено Истина.';uk='Ідентифікатор ""%1"""
"знайдений у довіднику ""%2"","
"але значення реквізиту ""Позначка видалення"" установлене Істина.'"),
				Строка(Свойства.Ссылка), НазваниеСправочника);
...
"""%1""."
""
"Допустимы только типы объектов метаданных перечисленные в комментарии к функции.';uk='Об`єкт метаданих не підтримується:"
"""%1""."
""
"Припустимі тільки типи об`єктів метаданих перераховані в коментарі до функції.'"),
				ПолноеИмяОбъектаМетаданных);
			ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + ОписаниеОшибки;
		КонецЕсли;
...
"регистре сведений ""Идентификаторы объектов версий расширений"".';uk='Для об`єкта метаданих ""%1"""
"не знайдений ідентифікатор у довіднику ""Ідентифікатори об`єктів метаданих"" і"
"регістрі відомостей ""Ідентифікатори об`єктів версій розширень"".'"),
			НСтр("ru='Для объекта метаданных ""%1"""
"не найден идентификатор в справочнике ""Идентификаторы объектов метаданных"".';uk='Для об`єкта метаданих ""%1"""
"не знайдений ідентифікатор у довіднику ""Ідентифікатори об`єктів метаданих"".'"));
...
"регістрі відомостей ""Ідентифікатори об`єктів версій розширень"".'"),
			НСтр("ru='Для объекта метаданных ""%1"""
"найдено несколько идентификаторов в справочнике ""Идентификаторы объектов метаданных"".';uk='Для об`єкта метаданих ""%1"""
...
"которому соответствует удаленный объект метаданных.';uk='Для об`єкта метаданих ""%1"""
"знайдений ідентифікатор у довіднику ""%2"","
"якому відповідає видалений об`єкт метаданих.'"),
				ПолноеИмяОбъектаМетаданных, НазваниеСправочника);
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Для объекта метаданных ""%1"""
"найден идентификатор в справочнике ""%2"","
"который соответствует другому объекту метаданных ""%3"".';uk='Для об`єкта метаданих ""%1"""
"знайдений ідентифікатор у довіднику ""%2"","
...
"реквизита ""Пометка удаления"" установлено Ложь недопустимо.';uk='Видалення ідентифікаторів об`єктів, у яких значення"
"реквізиту ""Позначка видалення"" встановлена Ложь неприпустимо.'"));
	КонецЕсли;
		...
		Если Не ТолькоПроверка Тогда
			Если ЗначениеЗаполнено(СписокКритичныхИзменений) Тогда
				ЗаписьЖурналаРегистрации(?(ОбъектыРасширений,
						НСтр("ru='Идентификаторы объектов расширений.Выполнены критичные изменения';uk='Ідентифікатори об`єктів розширень.Виконані критичні зміни'",
...
"Не слід називати підсистеми однаково, або використовувати новішу версію платформи.'"),
				ПолноеИмя,
				ОбъектМетаданных.ПолноеИмя());
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при поиске объекта метаданных по полному имени (при поиске ""%1"" был найден ""%2"").';uk='Помилка при пошуку об`єкту метаданих по повному імені (при пошуку ""%1"" був знайдений ""%2"").'"),
				ПолноеИмя,
				ОбъектМетаданных.ПолноеИмя());
		КонецЕсли;
	КонецЕсли;
...
"возможно только автоматически при обновлении данных справочника.';uk='Створення нового ідентифікатора об`єкта"
...
"установлено только автоматически при обновлении данных справочника.';uk='При зміні ідентифікатора об`єкта зазначене"
"повне ім`я ""%1"", яке може бути"
"встановлене тільки автоматично при оновленні даних довідника.'"),
				Объект.ПолноеИмя));
		Иначе
			...
			Если ЗначениеЗаполнено(НазваниеСправочника) Тогда
				ВызватьИсключениеПоОшибке(ОбъектыРасширений, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='При изменении идентификатора объекта указано"
"полное имя ""%1"","
"которое уже используется в справочнике ""%2"".';uk='При зміні ідентифікатора об`єкта зазначене"
"повне ім`я ""%1"","
...
"не может использоваться в неразделенном режиме.';uk='Довідник ""Ідентифікатори об`єктів розширень"""
"не може використовуватися в нерозділеному режимі.'");
...
"Подробнее см. ошибки при замене идентификатора в журнале регистрации.';uk='Не вдалося виконати заміну дублів ідентифікаторів об`єктів метаданих."
...
"%2';uk='При записі об`єкту ""%1"" виникла помилка:"
"%2'"),
...
"%3';uk='При записі рухів об`єкту ""%1"" в ""%2"" виникла помилка:"
"%3'"),
								ПолучитьНавигационнуюСсылку(Параметры.Объект.Ссылка),
...
"%3';uk='При записі по реєстраторові ""%1"" в ""%2"" виникла помилка:"
"%3'"),
...
"%2';uk='При видаленні запису ""%1"" виникла помилка:"
"%2'"),
...
"%2';uk='При додаванні запису ""%1"" виникла помилка:"
"%2'"),
							ПолучитьНавигационнуюСсылку(МенеджерРегистра.СоздатьКлючЗаписи(СтруктураИзмерений)),
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
						ВызватьИсключение;
...
Процедура СообщитьОбОшибке(Знач Описание, ОбъектыРасширений)
	...
	ЗаписьЖурналаРегистрации(
		?(ОбъектыРасширений,
			НСтр("ru='Идентификаторы объектов расширений.Замена идентификатора';uk='Ідентифікатори об`єктів розширень.Заміна ідентифікатора'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			НСтр("ru='Идентификаторы объектов метаданных.Замена идентификатора';uk='Ідентифікатори об`єктів метаданних.Заміна ідентифікатора'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
