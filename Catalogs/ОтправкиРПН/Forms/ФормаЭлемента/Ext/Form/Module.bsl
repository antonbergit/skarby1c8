...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Сообщение.Текст = НСтр("ru='Справочник не предназначен для ручного заполнения!';uk='Довідник не призначений для ручного заповнення!'");
		Сообщение.Сообщить();
		Отказ = Истина;
		Возврат;
	...
	ДатаЗакрытия = НСтр("ru='<не завершена>';uk='<не завершена>'");
	ДатаОбновления = НСтр("ru='<не определено>';uk='<не визначено>'");
	ОбновитьФорму();
...
Процедура ВыгрузитьПротокол(Команда)
	...
	ПолучитьФайл(Адрес, НСтр("ru='Протокол';uk='Протокол'") + ИмяФайлаПакетаДоТочки + ".html");
...
Процедура ОбновитьФорму()
	...
	ДатаОбновления = ?(Объект.ДатаПолученияРезультата = ПустаяДата, НСтр("ru='<не определено>';uk='<не визначено>'"), Объект.ДатаПолученияРезультата);
	...
	Если Объект.СтатусОтправки = Перечисления.СтатусыОтправки.Отправлен Тогда
		Если Объект.СтатусПроверкиНаПортале = Перечисления.СтатусыОтправки.Сдан Тогда
			...
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ru='Промежуточный протокол обработки порталом получен, отчет ждет доставки в управление';uk='Промежуточный протокол обработки порталом получен, отчет ждет доставки в управление'");
		...
		ИначеЕсли ПротоколЗаполнен(Объект.Ссылка) Тогда
			...
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ru='Промежуточный протокол обработки порталом получен';uk='Промежуточный протокол обработки порталом получен'");
			Элементы.НадписьСтатусОтправки.ГиперСсылка = Истина;
		Иначе
			Элементы.НадписьСтатусОтправки.ЦветТекста = ЦветСерый;
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ru='Протокол обработки пакета отсутствует';uk='Протокол обработки пакета отсутствует'");
	...
	Иначе
		...
		Если Объект.СтатусОтправки = Перечисления.СтатусыОтправки.НеПринят Тогда
			...
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ru='Протокол ошибок получен';uk='Протокол помилок отриманий'");
			Элементы.НадписьСтатусОтправки.ЦветТекста = ЦветСиний;
			Элементы.НадписьСтатусОтправки.ГиперСсылка = Истина;
		...
		ИначеЕсли Объект.СтатусОтправки = Перечисления.СтатусыОтправки.Сдан Тогда
			Элементы.КартинкаСтатусОтправки.Картинка = БиблиотекаКартинок.ПисьмоПодтверждениеПолучено;
			Элементы.НадписьСтатусОтправки.Заголовок = НСтр("ru='Протокол о сдаче отчета получен';uk='Протокол про здачу звіту отриманий'");
			Элементы.НадписьСтатусОтправки.ЦветТекста = ЦветСиний;
			Элементы.НадписьСтатусОтправки.ГиперСсылка = Истина;
		КонецЕсли;
	КонецЕсли;
