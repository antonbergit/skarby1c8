...
Процедура ПриОткрытии(Отказ)
	...
	#Если ВебКлиент Тогда
		ПоказатьПредупреждение(, НСтр("ru='В Веб-клиенте экспорт папок не поддерживается.';uk='У Веб-клієнті експорт папок не підтримується.'"));
		Отказ = Истина;
		Возврат;
	#КонецЕсли
...
Процедура ПапкаДляЭкспортаОткрытие(Элемент, СтандартнаяОбработка)
	...
	Если Не ПустаяСтрока(Элемент.ТекстРедактирования) Тогда
		...
		Если Не Файл.Существует() Тогда
			ТекстПредупреждения = НСтр("ru='Невозможно открыть папку, так как она не существует.';uk='Неможливо відкрити папку, оскільки вона не існує.'");
...
"Выберите другую папку.';uk='Папка ""%1"" не знайдена."
"Виберіть іншу папку.'"),
			ПапкаДляЭкспорта));
		Возврат;
	КонецЕсли;
...
"%3';uk='Не вдалося створити підпапку ""%1"" в папці ""%2"" з причини:"
"%3'"),
				Строка(ЧтоСохраняем),
				ПапкаДляЭкспорта,
...
"в папку на диске ""%2"".';uk='Успішно завершений експорт папки ""%1"""
"у папку на диску ""%2"".'"),
