...
Процедура ЗаполнитьТаблицуВалют()
	...
	Для Каждого ЗаписьКВ Из КлассификаторТаблица Цикл
		...
		Если НЕ СозданныеВалюты.Найти(ЗаписьКВ.Code)=Неопределено Тогда
			Продолжить;
		КонецЕсли; 
		...
		НоваяСтрока.СтраныИТерритории = ЗаписьКВ.Description;
		НоваяСтрока.Загружается = ЗаписьКВ.RBCLoading;
		...
		НоваяСтрока.Ранг = ЗаписьКВ.Rank;
		НоваяСтрока.Распространенная = ЗаписьКВ.Ranked;
		НоваяСтрока.СимвольноеПредставление = ЗаписьКВ.CurrencySymbol;
...
Функция ЗаписатьВыбранныеСтроки(Знач ВыбранныеСтроки)
	...
	Для каждого НомерСтроки Из ВыбранныеСтроки Цикл
		...
		ВалютаОбъект.ПараметрыПрописиНаУкраинском = ТекущиеДанные.ПараметрыПрописиНаУкраинском;
