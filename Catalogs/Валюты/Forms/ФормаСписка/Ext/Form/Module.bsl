...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Пользователи.РолиДоступны("ДобавлениеИзменениеКурсовВалют") Тогда
		// Элементы.ФормаПодборИзКВ.Видимость = Ложь; // УНФ
...
Процедура ЗагрузитьКурсыВалютВФонеНаСервере()
	...
	ПараметрыЗагрузки.Вставить("Валюты", РезультатЗапроса.Выгрузить());
	ПараметрыЗагрузки.Вставить("НачалоПериодаЗагрузки", ДобавитьМесяц(НачалоМесяца(ТекущаяДатаСеанса()), -3));
	ПараметрыЗагрузки.Вставить("ОкончаниеПериодаЗагрузки", КонецДня(ТекущаяДатаСеанса()) + 1);
	
	Результат = УправлениеНебольшойФирмойСервер.ЗапуститьВыполнениеВФоне(
	УникальныйИдентификатор,
	"Справочники.Валюты.ЗагрузитьКурсыВалют",
	ПараметрыЗагрузки,,,
	Ложь);
