...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Отказ Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Курс валюты можно связать только с курсом независимой валюты.';uk='Курс валюти можна зв`язати тільки з курсом незалежної валюти.'"));
	КонецЕсли;
	...
	Если Не ЭтоНовый()
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Валюта не может быть подчиненной, так как она является основной для других валют.';uk='Валюта не може бути підлеглою, тому що вона є основною для інших валют.'"));
		Отказ = Истина;
	КонецЕсли;
