...
"Отклонить?';uk='При отмене действия соглашения необходимо отклонить приглашение."
"Отклонить?'");
...
Процедура ШифроватьДанныеПакетаЭДПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.СертификатОрганизацииДляРасшифровки)
		...
		ТекстВопроса = НСтр("ru='Настройки шифрования будут очищены. Продолжить?';uk='Настройки шифрования будут очищены. Продолжить?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	Иначе
...
Процедура ПроверятьСертификатыПодписейПриИзменении(Элемент)
	...
	Если Объект.СертификатыПодписейКонтрагента.Количество() <> 0 Тогда
		ТекстВопроса = НСтр("ru='Настройки проверка сертификатов подписи контрагентов будут очищены. Продолжить?';uk='Настройки проверка сертификатов подписи контрагентов будут очищены. Продолжить?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	Иначе
		ВыполнитьОбработкуОповещения(ОписаниеОповещения);
	КонецЕсли;
...
Процедура ДоговорКонтрагентаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Исключение
		...
		ВидОперации = НСтр("ru='Открытие формы выбора договора';uk='Открытие формы выбора договора'");
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = НСтр("ru='Ошибка открытия формы выбора договора';uk='Ошибка открытия формы выбора договора'");
...
Процедура ОтправитьПриглашение(Команда)
	...
	Если Модифицированность Тогда
		ТекстВопроса = НСтр("ru='В текущую настройку ЭДО внесены изменения. Записать?';uk='В текущую настройку ЭДО внесены изменения. Записать?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	Иначе
		ВыполнитьОбработкуОповещения(ОписаниеОповещения);
	КонецЕсли;
...
Процедура ТестНастроек(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru='В текущую настройку ЭДО внесены изменения. Продолжить выполнение теста?';uk='В текущую настройку ЭДО внесены изменения. Продолжить выполнение теста?'");
...
"Продолжить?';uk='Изменения расширенного режима будут очищены."
"Продолжить?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	Иначе
...
"""индивидуальное"" - в расширенном режиме настроек.';uk='Изменение табличной части ""Электронные документы"" возможно:"
"""групповое"" - в Профиле настроек ЭДО;"
...
Процедура ИсходящиеДокументыВерсияФорматаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.АктИсполнитель") Тогда
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ФормаВключитьРасширенныйРежимНастройкиСоглашения.Заголовок = НСтр("ru='Включить расширенный режим настроек ЭДО';uk='Включить расширенный режим настроек ЭДО'");
	Если Объект.РасширенныйРежимНастройкиСоглашения Тогда
		Элементы.ФормаВключитьРасширенныйРежимНастройкиСоглашения.Заголовок = НСтр("ru='Выключить расширенный режим настроек ЭДО';uk='Выключить расширенный режим настроек ЭДО'");
	КонецЕсли;
...
Процедура ОбновитьВходящийКаталога(Форма)
	...
	ШаблонЗаголовка = НСтр("ru='Полный путь: %1';uk='Полный путь: %1'");
...
#Если НЕ ВебКлиент Тогда
	...
	ДиалогКаталога.Заголовок = НСтр("ru='Укажите каталог для обмена ЭД';uk='Укажите каталог для обмена ЭД'");
...
"уже используется в настройке ЭДО между контрагентом %2 и организацией %3';uk='Идентификатор контрагента %1"
...
"%1';uk='Проверка доступа к каталогам обмена."
"%1'");
	Попытка
...
Процедура ТестНастроекПродолжить(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	Если ЧерезОператораЭДО Тогда
		Состояние(НСтр("ru='Тест настроек.';uk='Тест настроек.'"),
			,
			НСтр("ru='Выполняется тестирование обмена ЭД через сервис ЭДО. Подождите...';uk='Выполняется тестирование обмена ЭД через сервис ЭДО. Подождите...'"));
	КонецЕсли;
...
"В профиле нет доступных сертификатов. Тест не выполнен.';uk='Тест обмена по профилю: %1."
...
Процедура ДействияПослеТестаСвязиЭДО(РезультатВыполнения, Контекст = Неопределено) Экспорт
	...
	Если ТипЗнч(РезультатВыполнения) = Тип("Структура") Тогда
		...
		Если РезультатВыполнения.Успех = Истина Тогда
			ШаблонСообщения = НСтр("ru='Тест обмена по профилю: %1"
"Пройден успешно.';uk='Тест обмена по профилю: %1"
"Пройден успешно.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Контекст.ПрофильНастроекЭДО);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
...
"Тест не выполнен.';uk='Тест обмена по профилю %1. В профиле нет доступных сертификатов."
"Тест не выполнен.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%1", ПрофильЭДО);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	Иначе
		ОбменСКонтрагентамиСлужебныйКлиент.ТестНастроекСПроверкойСертификата(Сертификат, ЭтаФорма, , ДляАвторизации);
	КонецЕсли;
...
"одновременно указывать сертификаты шифрования для организации и контрагента.';uk='Для корректной работы шифрования необходимо"
