	...
	Иначе
		...
		Исключение
			...
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				НСтр("ru='открытие формы соглашения';uk='открытие формы соглашения'"), ТекстОшибки, ТекстСообщения);
		КонецПопытки;
	КонецЕсли;
...
Процедура ПолучательОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.ИдентификаторКонтрагента) Тогда
		Если ВыбранноеЗначение <> Объект.Контрагент Тогда
			ТекстВопроса = НСтр("ru='Была изменена организация получатель. Изменить идентификатор получателя?';uk='Была изменена организация получатель. Изменить идентификатор получателя?'");
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
		КонецЕсли;
	Иначе
		ВыполнитьОбработкуОповещения(ОписаниеОповещения, КодВозвратаДиалога.Да);
...
"%5."
"';uk='По виду электронных документов %1 %2 "
"уже существует действующее соглашение между участниками %3 - %4:"
"%5."
"'");
...
"Записать соглашение?';uk='Внешние сертификаты можно выбирать только в записанном соглашении."
"Записать соглашение?'");
...
Процедура ИзменитьНадписьНаправлениеДокументов()
	...
	ШаблонСообщения = НСтр("ru='От кого: %1, кому: %2';uk='От кого: %1, кому: %2'");
...
Процедура ОбновитьЗаголовокФормы()
	...
	ТекстЗаголовка = НСтр("ru='Настройка ЭДО между организациями (создание)';uk='Настройка ЭДО между организациями (создание)'");
