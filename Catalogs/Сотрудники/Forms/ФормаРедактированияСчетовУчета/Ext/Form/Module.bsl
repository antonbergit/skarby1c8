...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОтказИзменитьСчетРасчетовСПерсоналом(Ссылка) Тогда
		Элементы.СПерсоналом.Подсказка = НСтр("ru='В базе есть движения по этому сотруднику! Изменение счетов учета по расчетам с персоналом запрещено!';uk='У базі є рухи по цьому співробітникові! Зміна рахунків обліку по розрахунках з персоналом заборонена!'");
		Элементы.СПерсоналом.Доступность = Ложь;
	КонецЕсли;

	Если ОтказИзменитьСчетРасчетовСПодотчетниками(Ссылка) Тогда
		Элементы.СПодотчетником.Подсказка = НСтр("ru='В базе есть движения по этому подотчетнику! Изменение счетов учета по расчетам с подотчетниками запрещено!';uk='У базі є рухи по цьому подотчетнику! Зміна рахунків обліку по розрахунках з подотчетниками заборонена!'");
