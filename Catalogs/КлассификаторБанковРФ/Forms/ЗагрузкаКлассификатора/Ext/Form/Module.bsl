...
Процедура ПутьКДискуИТСНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбораКаталога.Заголовок = НСтр("ru='Укажите путь к диску ИТС';uk='Вкажіть шлях до диска ІТС'");
	...
	Если НЕ ФайлДанных.Существует() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='В указанном каталоге не обнаружены данные классификатора. Необходимо указать путь к диску 1С:ИТС, на котором содержится база ""Гарант. Налоги, бухучет, предпринимательство.""';uk='У зазначеному каталозі не виявлені дані класифікатора. Необхідно вказати шлях до диска 1С:ІТС, на якому міститься база ""Гарант. Податки, бухоблік, підприємництво.""'"),
			,
			"ПутьКДискуИТС");
	КонецЕсли;
	...
	Иначе
		...
		Если ВариантЗагрузки = "ИТС" Тогда
			Если НЕ ЗначениеЗаполнено(ПутьКДискуИТС) И Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='При работе не в ОС Windows необходимо явно указать путь к диску';uk='При роботі не в ОС Windows необхідно явно вказати шлях до диска'"),
					,
					"ПутьКДискуИТС");
				Возврат;
			КонецЕсли;
...
Процедура ПриЗавершенииОперацииЗагрузки(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(НСтр("ru='Загрузка классификатора банков';uk='Завантаження класифікатора банків'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
"%1"
"Подробности см. в журнале регистрации.';uk='Завантаження класифікатора банків перерване через:"
"%1"
...
Процедура ЗагрузитьРезультат(АдресХранилища)
	...
	Если ВариантЗагрузки = "ИТС" Тогда
		Источник = НСтр("ru='Диск ИТС';uk='Диск ІТС'");
	Иначе
		Источник = НСтр("ru='Сайт РБК';uk='Сайт РБК'");
	КонецЕсли;
	
	ПараметрыЗагрузкиКлассификатора = ПолучитьИзВременногоХранилища(АдресХранилища);
	
	ИмяСобытия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Загрузка классификатора банков. %1.';uk='Завантаження класифікатора банків. %1.'"), Источник);
...
"Для установки требуется диск 1С:ИТС, на котором содержится база ""Гарант. Налоги, бухучет, предпринимательство.""';uk='На диску 1С:ІТС не виявлені дані класифікатора БІК РФ. "
"Для встановлення потрібно диск 1С:ІТС, на якому міститься база ""Гарант. Податки, бухоблік, підприємництво"".'");
...
// Параметры:
	...
	Иначе
		ПараметрыЗагрузкиФайловИТС.Вставить("ТекстСообщения", НСтр("ru='На диске ИТС не обнаружен файл обработки подготовки данных классификатора БИК РФ.';uk='На диску ІТС не виявлений файл обробки підготовки даних класифікатора БІК РФ.'"));
	КонецЕсли;
...
Процедура УстановитьИзмененияВИнтерфейсе()
	...
	Если Элементы.СтраницыФормы.ТекущаяСтраница = Элементы.СтраницаВыборИсточника
		...
		Элементы.ФормаКнопкаДалее.Заголовок = НСтр("ru='Загрузить';uk='Завантажити'");
	...
	Иначе
		...
		Элементы.ФормаКнопкаДалее.Заголовок = НСтр("ru='Закрыть';uk='Закрити'");
...
Функция ПолучитьДанныеРБКНаСервере(ПараметрыЗагрузкиФайловРБК)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Загрузка классификатора банков РФ';uk='Завантаження класифікатора банків РФ'");
...
Функция ТекстЗагрузкаЗапрещена()
	Возврат НСтр("ru='Загрузка классификатора банков в разделенном режиме запрещена.';uk='Завантаження класифікатора банків в розділеному режимі заборонене.'");
КонецФункции
