...
Процедура ЗаполнитьРегистрациюВИФНС()
	...
	Если ИспользоватьИФНСТорговойТочки Тогда
		...
		Иначе
			...
			НоваяРегистрация.Наименование = НСтр("ru='Регистрация к торговой точке';uk='Регистрация к торговой точке'") + Наименование;
			НоваяРегистрация.Код = КодИФНС;
			НоваяРегистрация.КодПоОКТМО = КодПоОКТМО;
			НоваяРегистрация.КодПоЕДРПОУ  =ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Владелец,"КПП");
	...
	Иначе
		//РегистрацияВИФНС = СведенияПоОрганизациям.ПолучитьРегистрациюВИФНС(Владелец);
	КонецЕсли;
