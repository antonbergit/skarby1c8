...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		//Объект.Владелец = ПолучениеУчетныхДанныхПовтИсп.ПолучитьТекущуюОрганизацию();
	...
	Элементы.КИ_0.ПодчиненныеЭлементы.ПредставлениеКИ_0.Заголовок = НСтр("ru='Адрес торговой точки';uk='Адрес торговой точки'");
...
Процедура ИзменитьКодВидаТорговойДеятельности(Команда)
	...
	СписокКодов.ПоказатьВыборЭлемента(Оповещние,НСтр("ru='Выберите код вида торговой деятельности';uk='Выберите код вида торговой деятельности'"));
...
Процедура УстановитьВидимостьИДоступностьЭлементов()
	...
	Если Объект.ЛьготаПрименяется Тогда
		...
		ИсчисленнаяСуммаСоЛьготой = Новый ФорматированнаяСтрока(
			НСтр("ru='Исчисленная сумма : ';uk='Исчисленная сумма : '"),
			Новый ФорматированнаяСтрока(Строка(Объект.ИсчисленнаяСумма)+ " р.", Новый Шрифт("",,,,,Истина,,)),
			НСтр("ru=' Применяется льгота. Сумма платежей 0 р.';uk=' Применяется льгота. Сумма платежей 0 р.'"));
...
"Подключиться сейчас?';uk='Для автоматичного створення податкового органа"
"в довіднику «Контрагенти» необхідно підключитися до Інтернет-підтримки"
"користувачів. Дані по податковому органу придадуться при сплаті податків."
"Підключитися зараз?'");
...
Процедура РассчитатьТорговыйСбор()
	...
	Если Объект.КодОбъектаОсуществленияТорговли = "08" Тогда
		...
		Объект.РасшифровкаРасчета = НСтр("ru='Ставка за объект составляет';uk='Ставка за объект составляет'") + Объект.ИсчисленнаяСумма + " р. ";
		
	ИначеЕсли Объект.КодОбъектаОсуществленияТорговли = "07" Тогда
		
		Объект.ИсчисленнаяСумма = ДанныеТарифа.СтавкаДо50квм;
		
		Объект.РасшифровкаРасчета = НСтр("ru='Исчисленная сумма при развозной и разносной торговле составляет';uk='Исчисленная сумма при развозной и разносной торговле составляет'") + Объект.ИсчисленнаяСумма + " р. ";
	...
	Иначе
		...
		Если Объект.ПлощадьТорговогоЗала > 50 И ТребуетсяПлощадьТороговогоЗала(Объект.КодОбъектаОсуществленияТорговли) Тогда
			...
			Объект.РасшифровкаРасчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Исчисленная сумма %1 р. состоит из %2 р. (ставка за объект) + %3 кв.м. (превышение 50 кв. м. площади) * %4 р. (ставка за превышение)';uk='Исчисленная сумма %1 р. состоит из %2 р. (ставка за объект) + %3 кв.м. (превышение 50 кв. м. площади) * %4 р. (ставка за превышение)'"),
		...
		Иначе 
			Объект.РасшифровкаРасчета = НСтр("ru='Ставка за объект составляет ';uk='Ставка за объект составляет '") + Объект.СтавкаЗаОбъект + " р. ";
		КонецЕсли;
	...
	Если Объект.ЛьготаПрименяется Тогда
		...
		Объект.РасшифровкаРасчета = Объект.РасшифровкаРасчета + НСтр("ru=' Применяется льгота. Итоговая ставка равна нулю.';uk=' Применяется льгота. Итоговая ставка равна нулю.'");
	Иначе
