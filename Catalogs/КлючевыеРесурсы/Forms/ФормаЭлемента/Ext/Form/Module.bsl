...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если Переключатель = "Год" Тогда
		...
		Если НомерМесяца < 1 ИЛИ НомерМесяца > 12 ИЛИ НомерДня < 1 ИЛИ НомерДня > 31 ИЛИ НомерДня > КоличествоДнейВМесяцеНаКлиенте(НомерМесяца, Год(ДатаГрафика)) Тогда
			ПоказатьПредупреждение(Неопределено,НСтр("ru='Вначале необходимо выделить день установки графика!';uk='Спочатку необхідно виділити день встановлення графіка!'"));
			Возврат;
		КонецЕсли;
...
Процедура УстановитьГрафик(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Текст = НСтр("ru='Установка графика возможна только после записи ресурса предприятия! Ресурс будет записан.';uk='Установка графіка можлива тільки після запису ресурсу підприємства! Ресурс буде записаний.'");
...
Функция ПолучитьТекущийГрафикНаДату(Дата)
	...
	Если НЕ ЗначениеЗаполнено(ГрафикРаботы) Тогда
		ГрафикРаботы = НСтр("ru='<Не установлен>';uk='<Не встановлений>'");
...
Процедура ОтметитьВыбранноеКакНеРабочееГод()
	...
	Если ЕстьПодходящиеДни
	   И ТекГрафик = Неопределено Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ru='Установите вначале график для выбранных дней!';uk='Встановіть спочатку графік для обраних днів!'"));
	КонецЕсли;
...
Процедура ОтметитьВыбранноеКакНеРабочееДень()
	...
	Если ТекГрафик = Неопределено Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ru='Установите вначале график для этого дня!';uk='Встановіть спочатку графік для цього дня!'"));
		Возврат;
	КонецЕсли;
