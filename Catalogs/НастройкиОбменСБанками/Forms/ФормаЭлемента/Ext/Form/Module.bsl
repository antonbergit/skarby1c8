...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Объект.ПрограммаБанка = Перечисления.ПрограммыБанка.СбербанкОнлайн Тогда
		...
		Исключение
			...
			Операция = НСтр("ru='Открытие настройки обмена с сервисом 1С:ДиректБанк.';uk='Открытие настройки обмена с сервисом 1С:ДиректБанк.'");
		...
		Если ТипЗнч(ДвоичныеДанныеКомпоненты) = Тип("ДвоичныеДанные") Тогда
			Элементы.ИнформацияОКомпонентеСбербанка.Заголовок = НСтр("ru='Обновить из файла...';uk='Обновить из файла...'");
			ВерсияКомпонентыСбербанк = ?(ЗначениеЗаполнено(Объект.ВерсияВнешнейКомпоненты), Объект.ВерсияВнешнейКомпоненты,
				НСтр("ru='<Неизвестна>';uk='<Неизвестна>'"));
			НазваниеМодуля = ?(ЗначениеЗаполнено(Объект.ИмяВнешнегоМодуля), Объект.ИмяВнешнегоМодуля,
				НСтр("ru='<Неизвестно>';uk='<Неизвестно>'"));
			ПредставлениеКомпоненты = НСтр("ru='%1. Версия %2';uk='%1. Версия %2'");
		...
		Иначе
			...
			Элементы.ИнформацияОКомпонентеСбербанка.Заголовок = НСтр("ru='Не загружен. Загрузить из файла...';uk='Не загружен. Загрузить из файла...'");
		КонецЕсли;
		Элементы.ГруппаВнешнегоМодуля.Видимость = Ложь;
...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	Если Объект.СертификатыПодписейОрганизации.Количество() И Не ТекущаяОрганизация = Объект.Организация
		...
		ТекстВопроса = НСтр("ru='После изменения организации список сертификатов будет очищен.';uk='После изменения организации список сертификатов будет очищен.'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(Истина, НСтр("ru='Сменить организацию';uk='Сменить организацию'"));
		Кнопки.Добавить(Ложь, НСтр("ru='Отменить изменение';uk='Скасувати зміну'"));
		ПоказатьВопрос(Оповещение, ТекстВопроса, Кнопки, , Истина, НСтр("ru='Изменение организации';uk='Изменение организации'"));
		Возврат;
...
Процедура ЗагрузитьСертификат(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		ТекстСообщения = НСтр("ru='Необходимо выбрать организацию';uk='Необходимо выбрать организацию'");
	...
	Если (Объект.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.ОбменЧерезДопОбработку")
		...
		ТекстСообщения = НСтр("ru='Необходимо загрузить внешний модуль';uk='Необходимо загрузить внешний модуль'");
...
Процедура ТестНастроек(Команда)
	...
	Если Модифицированность ИЛИ НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ТекстВопроса = НСтр("ru='Необходимо сохранить текущую настройку обмена. Продолжить выполнение теста?';uk='Необходимо сохранить текущую настройку обмена. Продолжить выполнение теста?'");
		СписокКнопок = Новый СписокЗначений();
		СписокКнопок.Добавить(Истина, НСтр("ru='Сохранить и выполнить тест';uk='Сохранить и выполнить тест'"));
		СписокКнопок.Добавить(Ложь, НСтр("ru='Отменить тест';uk='Отменить тест'"));
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, СписокКнопок, , Истина, НСтр("ru='Тест настроек';uk='Тест настроек'"));
	Иначе
...
"Удалите программу ""VpnKey-TLS для 1С:Предприятия 8"" и перезагрузите компьютер.';uk='На компьютере уже установлен внешний модуль другой версии."
...
Процедура ПослеВыводаПредупрежденияЗагрузитьВнешнийМодульСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	#Иначе
		...
		Фильтр = НСтр("ru='Файл внешнего модуля';uk='Файл зовнішнього модуля'") + "(*.zip)|*.zip";
...
"Файл внешнего модуля должен иметь расширение ""zip"" или ""epf""'");
	...
	Если РасширениеФайла = "ZIP" Тогда // внешняя компонента
		...
		Шаблон = НСтр("ru='%1. Версия %2.';uk='%1. Версия %2.'");
	...
	Иначе // внешняя обработка
		...
		#Иначе
			...
			Исключение
				...
				ТекстСообщения = НСтр("ru='Не удалось подключить выбранный внешний модуль';uk='Не удалось подключить выбранный внешний модуль'");
		...
		Элементы.ЗагрузитьВнешнийМодульИзФайла.Заголовок = НСтр("ru='Обновить из файла...';uk='Обновить из файла...'");
...
Процедура ПослеТестаНастройкиОбмена(Результат, Параметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Отмена ИЛИ Результат = Неопределено Тогда
		ПоказатьОповещениеПользователя(НСтр("ru='Тест не завершен.';uk='Тест не завершен.'"));
	...
	ИначеЕсли Результат = Истина ИЛИ (ТипЗнч(Результат) = Тип("Структура") И Результат.Успех) Тогда
		ПоказатьОповещениеПользователя(НСтр("ru='Тестирование выполнено успешно.';uk='Тестирование выполнено успешно.'"));
	Иначе
		Если ТипЗнч(Результат) = Тип("Структура") И Результат.Свойство("МассивСообщений") Тогда
			...
			КонецЦикла
		КонецЕсли;
		ПоказатьОповещениеПользователя(НСтр("ru='Тест не пройден.';uk='Тест не пройден.'"));
	КонецЕсли;
...
Процедура ПоместитьВХранилищеСертификат(Знач ДвоичныеДанные, Знач УникальныйИдентификатор, АдресСертификатаСбербанка, ПредставлениеСертификата = Неопределено)
	...
	Иначе
		...
		Исключение
			...
			Исключение
				ТекстСообщения = НСтр("ru='Не удалось прочитать файл сертификата, операция прервана.';uk='Не удалось прочитать файл сертификата, операция прервана.'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
				АдресСертификатаСбербанка = ПоместитьВоВременноеХранилище(Неопределено, УникальныйИдентификатор);
				Возврат;
			КонецПопытки;
...
"Записать?';uk='Загружать сертификаты можно только в записанной настройке обмена."
"Записать?'");
