...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НаименованиеЗадания = НСтр("ru='1С:ДиректБанк: Обновление списка банков, поддерживающих обмен.';uk='1С:ДиректБанк: Обновление списка банков, поддерживающих обмен.'");
...
"Обратитесь к администратору';uk='Нет прав настройки прямого обмена с банком."
...
Процедура ПриОткрытии(Отказ)
	...
	Если СпособСозданияНастройки = 1 И ЗначениеЗаполнено(АдресФайлаНастроекОбмена) Тогда
		...
		УчастникиОбменаИзФайла = УчастникиОбменаИзФайла(АдресФайлаНастроекОбмена);
...
"Продолжить закрытие программы?';uk='Подключение к сервису 1С:ДиректБанк не завершено."
"Продолжить закрытие программы?'");
		Иначе
			ТекстПредупреждения = НСтр("ru='Подключение к сервису 1С:ДиректБанк не завершено."
"Закрыть помощник подключения?';uk='Подключение к сервису 1С:ДиректБанк не завершено."
...
СтандартнаяОбработка = Ложь;
	...
	#Иначе
		Режим = РежимДиалогаВыбораФайла.Открытие;
		ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(Режим);
		Фильтр = НСтр("ru='Внешний модуль(*.zip;*.epf)|*.zip;*.epf';uk='Внешний модуль(*.zip;*.epf)|*.zip;*.epf'");
		ДиалогОткрытияФайла.Фильтр = Фильтр;
		ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
		ДиалогОткрытияФайла.Заголовок = НСтр("ru='Выберите файл внешнего модуля банка';uk='Виберіть файл зовнішнього модуля банку'");
...
"Файл внешнего модуля должен иметь расширение ""zip"" или ""epf""';uk='Указан неверный файл."
...
"Файл настроек содержит настройки для организации: %2';uk='Указан неверный файл."
"Требуются настройки для организации: %1"
...
"Файл настроек содержит настройки для банка: %2';uk='Указан неверный файл."
"Требуются настройки для банка: %1"
...
Процедура ОтправитьТестовыйЗапросПослеУстановкиПодписиСбербанк(ПодписьВерна, ДополнительныеПараметры) Экспорт
	...
	Если Не ПодписьВерна Тогда
		ОбменСБанкамиСлужебныйКлиент.СообщитьПользователю(НСтр("ru='Установленная подпись неверна.';uk='Встановлений підпис невірний.'"), УникальныйИдентификатор);
		ОбработатьРезультатТестаНастройки(Ложь);
		Возврат;
	КонецЕсли;
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка инициализации внешнего модуля."
"Код ошибки: ДО-%1"
"%2'");
			ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
			ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка завершения инициализации внешнего модуля."
"Код ошибки: ДО-%1"
"%2'");
		ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
...
"Модуль якої версії потрібно тепер використовувати? '");
	...
	Кнопки.Добавить(КодВозвратаДиалога.Отмена);
	ЗаголовокВопроса = НСтр("ru='Выбор версии модуля';uk='Выбор версии модуля'");
...
"Требуются настройки для организации: %1"
"Получены настройки для организации: %2';uk='Указана неверная учетная запись."
"Требуются настройки для организации: %1"
...
"Требуются настройки для банка: %1"
"Получены настройки для банка: %2';uk='Подключено неверное аппаратное устройство."
"Требуются настройки для банка: %1"
...
"Обратитесь к администратору.';uk='Внешний модуль не удалось подключить."
"Обратитесь к администратору.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ОбработатьРезультатПолученияНастроекССервераБанка(Ложь);
		Возврат;
...
Процедура ОбработатьОшибкуПолученияНастроекИзБанкаВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ru='При получении настроек обмена из банка произошла ошибка.';uk='При получении настроек обмена из банка произошла ошибка.'");
	ВидОперации = НСтр("ru='Получение настроек обмена с сервера банка.';uk='Получение настроек обмена с сервера банка.'");
...
Функция ДобавитьСертификатВНастройкуОбменаВК(Знач НастройкаОбмена, Знач СертификатBase64, Знач СтруктураСертификата)
	...
	Исключение
		Операция = НСтр("ru='Добавление сертификата в настройку обмена';uk='Добавление сертификата в настройку обмена'");
		ШаблонСообщения = НСтр("ru='При добавлении сертификата в настройку обмена произошла ошибка."
"%1';uk='При добавлении сертификата в настройку обмена произошла ошибка."
"%1'");
...
&НаСервереБезКонтекста
Функция УчастникиОбменаИзФайла(Знач АдресФайла)
...
"Обратитесь в свой банк.';uk='В файле настроек нет информации об организации."
"Обратитесь в свой банк.'");
	...
	Если Не ЗначениеЗаполнено(СсылкаНаОрганизацию) Тогда
		ТекстСообщения = НСтр("ru='Файл содержит настройки для неизвестной организации: %1%2%3';uk='Файл содержит настройки для неизвестной организации: %1%2%3'");
		Если СтруктураПоискаОрганизации.Свойство("ИНН") Тогда
			ТекстИНН = Символы.ПС + НСтр("ru='ИНН: %1';uk='ИНН: %1'");
			ТекстИНН = СтрШаблон(ТекстИНН, СтруктураПоискаОрганизации.ИНН);
		Иначе
			ТекстИНН = "";
		КонецЕсли;
		Если СтруктураПоискаОрганизации.Свойство("КПП") Тогда
			ТекстКПП = Символы.ПС + НСтр("ru='КПП: %1';uk='КПП: %1'");
			ТекстКПП = СтрШаблон(ТекстКПП, СтруктураПоискаОрганизации.КПП);
		Иначе
			ТекстКПП = "";
		КонецЕсли;
		Если СтруктураПоискаОрганизации.Свойство("Наименование") Тогда
			ТекстНаименование = Символы.ПС + НСтр("ru='Наименование: %1';uk='Наименование: %1'");
	...
	Если НЕ ЗначениеЗаполнено(СсылкаНаБанк) Тогда
		ТекстСообщения = НСтр("ru='Файл содержит настройки для неизвестного банка: %1%2';uk='Файл содержит настройки для неизвестного банка: %1%2'");
		Если СтруктураПоискаБанка.Свойство("Код") Тогда
			ТекстБИК = Символы.ПС + НСтр("ru='БИК: %1';uk='БИК: %1'");
...
Процедура ВывестиСсылкуНаСайтБанка(ИнформацияНаСайтеБанка, АдресСайта)
	...
	ИнформацияНаСайтеБанка.Заголовок = Новый ФорматированнаяСтрока(
		НСтр("ru='Подробную информацию о порядке и условиях подключения на стороне банка можно получить';uk='Подробную информацию о порядке и условиях подключения на стороне банка можно получить'"), " ",
		Новый ФорматированнаяСтрока(НСтр("ru='здесь.';uk='здесь.'"), , , , АдресСайта));
...
Процедура ВключитьНастройкуОбмена(Знач НастройкаОбмена, Успех)
	...
	Исключение
		...
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВидОперации = НСтр("ru='Включение настройки.';uk='Включение настройки.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
