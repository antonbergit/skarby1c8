...
Процедура СформироватьСтрокуОКУН()
	...
	Если Объект.ЗначенияОКУН.Количество() = 0 Тогда
		Элементы.СсылкаЗаполненияОКУН.Заголовок = (НСтр("ru='<<Заполнить ОКУН>>';uk='<<Заповнити ЗКПН>>'"));
		Возврат;
	КонецЕсли;
	...
	РезСтрока = НСтр("ru='ОКУН: ';uk='ЗКПН:'");
	...
	Иначе
		РезСтрока = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Выбрано %1 кода ОКУН';uk='Обране %1 коду ЗКПН'"),
				Выборка.Количество() );
...
Процедура Декорация8ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ПустаяСтрока(Объект.КодНалоговогоОрганаПолучателя) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Заполните код ИФНС и мы тогда сможем определить ссылку для поиска';uk='Заповніть код ІФПС і ми тоді зможемо визначити посилання для пошуку'"),
			,
