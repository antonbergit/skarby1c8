...
Функция ПроверитьУмещение()
	...
	Если НЕ (ПолеТабличногоДокумента.ОбластьПечати.Лево = 0 И ПолеТабличногоДокумента.ОбластьПечати.Право = 0) Тогда
		...
		Если КоличествоПоГоризонтали <> МассивТаблиц.Количество() Тогда
			ТекстСообщения = НСтр("ru='Максимальное количество по горизонтали: ';uk='Максимальна кількість по горизонталі:'") + МассивТаблиц.Количество();
	...
	Если НЕ (ПолеТабличногоДокумента.ОбластьПечати.Верх = 0 И ПолеТабличногоДокумента.ОбластьПечати.Низ = 0) Тогда
		...
		Если КоличествоПоВертикали <> МассивТаблиц.Количество() Тогда
			ТекстСообщения = НСтр("ru='Максимальное количество по вертикали: ';uk='Максимальна кількість по вертикалі:'") + МассивТаблиц.Количество();
...
Процедура ЭтикеткаПоУмолчанию(Команда)
	...
	ПоказатьВопрос(Оповещение,НСтр("ru='Редактируемый шаблон будет заменен на шаблон по умолчанию, продолжить?';uk='Шаблон, що редагується, буде замінений на шаблон за замовчуванням, продовжити?'"), РежимДиалогаВопрос.ДаНет);
	...
	Если СохранятьИзменения Тогда
		Если ПолеТабличногоДокумента.ОбластьПечати = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не установлена область печати';uk='Не установлена область печати'"),
				ЭтаФорма,
