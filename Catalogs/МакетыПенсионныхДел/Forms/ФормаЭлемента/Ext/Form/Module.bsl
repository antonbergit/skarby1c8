	...
	Для Каждого СтрокаТаблицы Из ТаблицаДокументов Цикл
		Если Не СтрокаТаблицы.ФайлыВыбраны И ЗначениеЗаполнено(СтрокаТаблицы.Документ) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ru='Добавьте файлы для документа ""%1""';uk='Добавьте файлы для документа ""%1""'"), СтрокаТаблицы.Документ),, 
		...
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.Документ) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ru='Заполните вид документа в строке %1';uk='Заполните вид документа в строке %1'"), ТаблицаДокументов.Индекс(СтрокаТаблицы) + 1),,
...
"%1.';uk='Не удалось получить текст вложения с именем"
"%1.'"), ТекстСообщения);
...
Функция ПредставлениеФайлов(Файлы)
	...
	Если ЗначениеЗаполнено(Файлы) Тогда
		...
		Если КоличествоСтраниц = 1 Тогда
			ШаблонПредставления = НСтр("ru='Страница № %1 (%2)';uk='Страница № %1 (%2)'");
		Иначе
			ШаблонПредставления = НСтр("ru='Страницы № %1 (%2)';uk='Страницы № %1 (%2)'");
		КонецЕсли;
...
Функция ПредставлениеФайловПоУмолчанию()
	...
	Возврат НСтр("ru='Выберите файл';uk='Виберіть файл'");
...
Функция ПроверитьЗаполнениеПоляОтправитель()
	...
	Если Не ЗначениеЗаполнено(Объект.Отправитель) Тогда
		ОчиститьСообщения();
		ТекстСообщения = НСтр("ru='Заполните поле ""От кого""';uk='Заполните поле ""От кого""'");
	...
	Если ЗначениеЗаполнено(Объект.Отправитель) И Не ЗначениеЗаполнено(УчетнаяЗаписьОтправителя(Объект.Отправитель)) Тогда
		ОчиститьСообщения();
		ТекстСообщения = НСтр("ru='Организация не подключена к 1С-Отчетности';uk='Организация не подключена к 1С-Отчетности'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "Объект.Отправитель");
		ПолеЗаполнено = Ложь;	
	КонецЕсли;
...
"Требования к документам определяются выбранным органом ПФР.';uk='Заполните поле ""Кому""."
...
Функция ПроверитьЗаполнениеПоляСотрудник()
	...
	Если Не ЗначениеЗаполнено(Объект.Сотрудник) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Заполните поле ""Сотрудник""';uk='Заполните поле ""Сотрудник""'"),, 
			"Объект.Сотрудник");
		ПолеЗаполнено = Ложь;	
	КонецЕсли;
	...
	Если ПроверитьЗаполнениеПоляОтправитель() И ПроверитьЗаполнениеПоляПолучатель() Тогда
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(Документ) Тогда
				ОчиститьСообщения();
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Поле ""Документ"" не заполнено';uk='Поле ""Документ"" не заполнено'"),, 
		...
		Иначе
			...
			ОперацииСФайламиЭДКОКлиент.ДобавитьФайлы(Оповещение, УникальныйИдентификатор,
				СтрШаблон(НСтр("ru='Выберите файлы документа <%1> ';uk='Выберите файлы документа <%1> '"), Документ), ПараметрыФормы);		
		КонецЕсли;
	КонецЕсли;
