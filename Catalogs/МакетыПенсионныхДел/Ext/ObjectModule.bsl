...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого СтрокаТаблицы Из ЭлектронныеДокументы Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.Документ) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ru='Заполните вид документа в строке %1';uk='Заполните вид документа в строке %1'"), ЭлектронныеДокументы.Индекс(СтрокаТаблицы) + 1),,
	...
	Если Не ЕстьРеестр И ЭлектронныеДокументы.Количество() <> 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Добавьте реестр направляемых документов';uk='Добавьте реестр направляемых документов'"),,
			"РеестрДокументов",,
			Отказ);
	КонецЕсли;
	...
	Если СтрДлина(СокрЛП(СтрЗаменить(ОрганПФРПоМестуНазначенияПенсии, "-", ""))) <> 6 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Код органа ПФР по месту назначения пенсии должен состоят из 6 цифр';uk='Код органа ПФР по месту назначения пенсии должен состоят из 6 цифр'"),
			Ссылка, 
	...
	Если Не ЗначениеЗаполнено(СокрЛП(СтрЗаменить(СтраховойНомерПФР, "-", ""))) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Заполните СНИЛС';uk='Заполните СНИЛС'"),, "СтраховойНомерПФР",, Отказ);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(АдресРегистрации) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Заполните адрес регистрации';uk='Заполните адрес регистрации'"),, "АдресРегистрации",, Отказ);
	КонецЕсли;
...
""
"Адрес регистрации: %3"
"Контактный телефон: %4';uk='Направляем Вам макет пенсионного дела сотрудника:"
"%1 (%2)"
""
"Адрес регистрации: %3"
