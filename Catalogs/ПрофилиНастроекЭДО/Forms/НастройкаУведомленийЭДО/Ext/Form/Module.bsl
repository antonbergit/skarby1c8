...
Функция ЕстьВключенныеСобытия()
	...
	Если Не Результат Тогда
		...
		ТекстСообщения = НСтр("ru='Нет событий для уведомления';uk='Нет событий для уведомления'");
...
Функция ТекстКодаОшибки(КодСостояния)
	...
	Если КодСостояния = 405 Тогда
		ТекстОшибки = НСтр("ru='Указанный клиентом метод нельзя применить к текущему ресурсу';uk='Указанный клиентом метод нельзя применить к текущему ресурсу'");
	КонецЕсли;
...
Функция СохранитьПараметрыПодписок(РасшифрованныйМаркер, ПараметрыУведомления)
	...
	Исключение
		...
		ВидОперации = НСтр("ru='Изменение свойств подписки ЭДО';uk='Изменение свойств подписки ЭДО'");
	...
	Иначе
		...
		ШаблонСообщения = НСтр("ru='Ошибка при сохранении настройки: %1';uk='Ошибка при сохранении настройки: %1'");
		ТекстОшибки = ТекстКодаОшибки(Ответ.КодСостояния);
		
		ШаблонСообщения = НСтр("ru='Ошибка выполнения команды сервиса. Код ошибки: %1. %2';uk='Ошибка выполнения команды сервиса. Код ошибки: %1. %2'");
