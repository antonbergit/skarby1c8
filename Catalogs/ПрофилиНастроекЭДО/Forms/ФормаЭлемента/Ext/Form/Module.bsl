...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НЕ Объект.ПометкаУдаления И Не ПропускатьПроверкуНастроекЭДО И ОтсутствуютПодчиненныеНастройкиЭДО() Тогда
		ТекстВопроса = НСтр("ru='Подключить контрагента к обмену электронными документами?';uk='Подключить контрагента к обмену электронными документами?'");
		...
		Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
			...
			Если ТипВыбранногоЗначения = Тип("СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования") Тогда
				...
				Если МассивСтрок.Количество() > 0 Тогда
					ТекстПредупреждения = НСтр("ru='Выбранный сертификат уже зарегистрирован в профиле настроек ЭДО.';uk='Выбранный сертификат уже зарегистрирован в профиле настроек ЭДО.'");
...
Процедура НадписьЛичныйКабинетУчастникаОбменаЭДНажатие(Элемент)
	...
	Если Объект.СертификатыПодписейОрганизации.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Для входа в личный кабинет должен быть зарегистрирован хотя бы один сертификат';uk='Для входа в личный кабинет должен быть зарегистрирован хотя бы один сертификат'");
		ПоказатьПредупреждение(, ТекстПредупреждения, 30);
		Возврат;
	КонецЕсли;
...
"Записать?';uk='Выполнить действие можно только в записанном профиле настроек ЭДО."
"Записать?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	Иначе
...
Процедура ТекстПриглашенийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЗаголовокФормы = НСтр("ru='Шаблон текста для приглашений контрагентов';uk='Шаблон текста для приглашений контрагентов'");
...
"Продолжить?';uk='Изменения ""Профиля настроек ЭДО"" будут применены для связанных ""Настроек ЭДО"" с отключенным расширенным режимом."
"Продолжить?'");
...
"Продолжить?'");
		ПараметрыОповещения = Новый Структура;
		ПараметрыОповещения.Вставить("ИспользоватьУПД", Объект.ИспользоватьУКД);
		ОписаниеОповещения = Новый ОписаниеОповещения("РазрешитьРедактированиеИсходящиеДокументыЗавершить", ЭтотОбъект, ПараметрыОповещения);
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	Иначе
		ИзменитьФлагУПДУКД();
...
Процедура ТестПрофиляНастроек(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru='В текущий профиль настроек ЭДО внесены изменения. Продолжить выполнение теста?';uk='В текущий профиль настроек ЭДО внесены изменения. Продолжить выполнение теста?'");
		СписокКнопок = Новый СписокЗначений();
		СписокКнопок.Добавить("Выполнить", НСтр("ru='Сохранить и выполнить тест.';uk='Сохранить и выполнить тест.'"));
		СписокКнопок.Добавить("Отменить", НСтр("ru='Отменить тест.';uk='Отменить тест.'"));
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, СписокКнопок, , "Выполнить", НСтр("ru='Тест настроек';uk='Тест настроек'"));
	Иначе
...
"Продолжить?';uk='Операция добавления сертификата в профиль настроек ЭДО займет несколько минут."
"Продолжить?'");
...
Процедура НастройкаУведомлений(Команда)
	...
	ПредставлениеОперации = НСтр("ru='Настройка уведомлений о событиях ЭДО';uk='Настройка уведомлений о событиях ЭДО'");
...
Процедура ИсходящиеДокументыПриИзменении(Элемент)
	...
	Если Элемент.ТекущиеДанные.ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.ПроизвольныйЭД")
		...
		Если Элемент.ТекущийЭлемент.Имя = "ИсходящиеДокументыИспользоватьЭП" И НЕ Элемент.ТекущиеДанные.ИспользоватьЭП Тогда
			ШаблонСообщения = НСтр("ru='Отправка документа %1 возможна только с использованием электронной подписи.';uk='Отправка документа %1 возможна только с использованием электронной подписи.'");
...
Процедура ИсходящиеДокументыВерсияФорматаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.АктНаПередачуПрав") Тогда
...
Процедура ТестПрофиляНастроекПродолжить(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	Если ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(Объект.СпособОбменаЭД) Тогда
		Если Объект.СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезЭлектроннуюПочту") Тогда
			Состояние(НСтр("ru='Тест настроек.';uk='Тест настроек.'"),
			,
...
"%1';uk='Тест. Проверка доступа к общему каталогу для обмена ЭД."
"%1'");
			Попытка
...
"%1';uk='Тест. Проверка доступа к серверу FTP для обмена ЭД."
"%1'");
			Попытка
...
"%2'");
	...
	Иначе
		...
		Состояние(НСтр("ru='Тест настроек.';uk='Тест настроек.'"),
			,
			НСтр("ru='Выполняется тестирование связи с сервисом ЭДО. Подождите...';uk='Выполняется тестирование связи с сервисом ЭДО. Подождите...'"));
...
Процедура ПослеПолученияОтпечатковПроверитьСертификаты(Отпечатки, Параметры = Неопределено) Экспорт
	...
	Иначе
		ТекстСообщения = НСтр("ru='Нет доступных сертификатов. Тест не выполнен.';uk='Нет доступных сертификатов. Тест не выполнен.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ПослеПолученияОтпечатковПроверитьСвязьЭДО(Отпечатки, Параметры = Неопределено) Экспорт
	...
	ШаблонОперация = НСтр("ru='Тест аутентификации: %1';uk='Тест аутентификации: %1'");
...
Процедура ДействияПослеТестаСвязиЭДО(РезультатВыполнения, Контекст = Неопределено) Экспорт
	...
	Если ТипЗнч(РезультатВыполнения) = Тип("Структура") Тогда
		Если РезультатВыполнения.Успех = Истина Тогда
			ТекстСообщения = НСтр("ru='Тест профиля настроек выполнен успешно.';uk='Тест профиля настроек выполнен успешно.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПерейтиВЛичныйКабинетОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("СоответствиеПрофилейИПараметровСертификатов", СоответствиеПрофилейИПараметровСертификатов)
		...
		ТекстСообщения = НСтр("ru='Нет доступных сертификатов, среди зарегистрированных по данному профилю настроек ЭДО.';uk='Нет доступных сертификатов, среди зарегистрированных по данному профилю настроек ЭДО.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,
			,
...
Процедура ЗарегистрироватьСертификатТакском()
	...
	ОбменСКонтрагентамиСлужебныйКлиент.ТестНастроекСПроверкойСертификата(ДобавляемыйСертификат, ЭтаФорма,
		ОбработкаОповещения, Ложь, Истина, Истина);
...
"Необходимо повторить добавление сертификата.';uk='Ошибка получения сертификата оператора ЭДО."
...
Процедура СформироватьИОтправитьРегистрационныйПакет1СЭДО(РеквизитыПакета, ЗашифрованныеДвоичныеДанныеОператораЭДО, ПодписанныеДвоичныеДанныеДляОператораЭДО,
	...
	ТекстЗаголовкаСообщения = НСтр("ru='Отправка заявления на изменение данных на сервисе 1С-ЭДО';uk='Отправка заявления на изменение данных на сервисе 1С-ЭДО'");
...
Процедура ОткрытьФормуВыбораСертификатаЭП()
	...
	Если Объект.ПометкаУдаления Тогда
		ТекстСообщения = НСтр("ru='Для выполнения действия необходимо снять пометку удаления.';uk='Для выполнения действия необходимо снять пометку удаления.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
"необходимо получить идентификатор участника обмена ЭД';uk='Перед добавлением новых сертификатов в профиль настроек ЭДО"
...
Процедура УстановитьНастройкуУведомлений()
	...
	Если УведомлятьОСобытиях Тогда
		ЗаголовокШаблон = НСтр("ru='Email для уведомлений: %1';uk='Email для уведомлений: %1'");
	Иначе
		ЗаголовокШаблон = НСтр("ru='Уведомления не подключены';uk='Уведомления не подключены'");
	КонецЕсли;
	...
	ИначеЕсли УведомлятьОСобытиях Тогда
		АдресУведомлений = НСтр("ru='<не указан>';uk='<не указан>'");
	КонецЕсли;
...
Процедура ЗаполнитьПредставлениеРегламентаЭДО(ТекСтрока)
		...
		Если ТекСтрока.ИспользоватьЭП Тогда
			Подпись = НСтр("ru='Подпись';uk='Підпис'");
		Иначе
			Подпись = НСтр("ru='Без подписи';uk='Без подписи'");
		КонецЕсли;
		
		ОтветнаяПодпись = "";
		Если ТекСтрока.ТребуетсяОтветнаяПодпись Тогда
			ОтветнаяПодпись = НСтр("ru='ответная подпись';uk='ответная подпись'");
		КонецЕсли;
		
		ИзвещениеОПолучении = "";
		Если ТекСтрока.ТребуетсяИзвещениеОПолучении Тогда
			ИзвещениеОПолучении = НСтр("ru='извещение о получении';uk='извещение о получении'");
		КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = НСтр("ru='ФНС 5.01 (с 2016г.)';uk='ФНС 5.01 (с 2016г.)'");
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='ФНС 5.01 (с 01.01.2016)';uk='ФНС 5.01 (с 01.01.2016)'"));
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ИсходящиеДокументы.ВерсияФормата");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = НСтр("ru='ФНС 5.01';uk='ФНС 5.01'");
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='ФНС 5.01 (до 31.12.2016)';uk='ФНС 5.01 (до 31.12.2016)'"));
