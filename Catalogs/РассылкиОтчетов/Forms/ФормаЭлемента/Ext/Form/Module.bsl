...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Автор) Тогда
			...
			ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("ru='Рассылка отчетов. Открытие формы элемента';uk='Розсилання звітів. Відкриття форми елемента'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
"Пользователь ""%1"" (%2) не может быть автором.';uk='Помилка заповнення автора розсилання:"
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(Объект.Наименование) Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ru='Не введено наименование';uk='Не введене найменування'");
	...
	Если Объект.ИспользоватьЭлектроннуюПочту И Не Объект.Личная Тогда
		Если Не ЗначениеЗаполнено(ТипПолучателейРассылки) Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("ru='Не выбран тип получателей';uk='Не обраний тип одержувачів'");
	...
	Если Объект.Подготовлена Тогда
		Если Объект.Отчеты.Количество() = 0 Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("ru='Не выбрано ни одного отчета';uk='Не обране жодного звіту'");
		...
		Если Объект.ИспользоватьСетевойКаталог Тогда
			Если Не ЗначениеЗаполнено(Объект.СетевойКаталогWindows) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ru='Не введен сетевой каталог Windows';uk='Не введений мережний каталог Windows'");
			...
			Если Не ЗначениеЗаполнено(Объект.СетевойКаталогLinux) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ru='Не введен сетевой каталог Linux';uk='Не введений мережний каталог Linux'");
		...
		Если Объект.ИспользоватьЭлектроннуюПочту Тогда
			...
			Иначе
				...
				Если Не ЗначениеЗаполнено(Объект.ВидПочтовогоАдресаПолучателей) Тогда
					Отказ = Истина;
					ТекстСообщения = НСтр("ru='Не выбран тип почтового адреса получателей';uk='Не обраний тип поштової адреси одержувачів'");
			...
			Если Не ЗначениеЗаполнено(Объект.УчетнаяЗапись) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ru='Не выбрана учетная запись для отправки';uk='Не обраний обліковий запис для відправлення'");
	...
	Для Каждого СтрокаОтчеты Из Объект.Отчеты Цикл
		...
		Если СтрокаОтчеты.СКД Тогда
			...
			Если Не ЗаполненыВсеОбязательныеНастройки Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru=""Для отчета '%1' заполнены не все настройки, обязательные для заполнения. Необходимо заполнить все обязательные настройки или снять флажок 'Подготовлена'."";uk=""Для звіту '%1' заповнені не всі налаштування, обов`язкові для заповнення. Необхідно заповнити всі обов`язкові налаштування або зняти прапорець 'Підготовлена'."""),
		...
		Если ПроверитьПерсонализированные Тогда
			...
			Если ОтчетПерсонализирован Тогда
				Отказ = Истина;
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru=""Для отчета '%1' задан отбор по получателям рассылки. Необходимо отключить отбор по получателям или изменить вид рассылки на 'Свой отчет для каждого получателя'."";uk=""Для звіту '%1' заданий відбір по одержувачах розсилання. Необхідно відключити відбір по одержувачах або змінити вид розсилання на 'Свій звіт для кожного одержувача'."""),
					Строка(СтрокаОтчеты.Отчет));
				Поле = "Отчеты["+ Формат(ТекущийОбъект.Отчеты.Индекс(ОбъектСтрокаОтчеты), "ЧН=0; ЧГ=0") +"].Представление";
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ТекущийОбъект, Поле);
			КонецЕсли;
...
"Не добавлено из-за ошибок: %3';uk='Додане звітів: %1 з %2,"
...
"%2'"),
		...
		ПараметрыПредупреждения.Заголовок = НСтр("ru='Ошибки добавления отчетов в рассылку';uk='Помилки додавання звітів у розсилання'");
	...
	Иначе
		...
		Если СтруктураВыбора.Успешно.Количество = 0 Тогда
			...
			ТекстОповещения = НСтр("ru='Все выбранные отчеты уже включены в рассылку';uk='Всі обрані звіти вже включені в розсилання'");
		Иначе
			Если СтруктураВыбора.Выбрано.Количество = 1 Тогда
				ЗаголовокОповещения = НСтр("ru='Отчет включен в рассылку';uk='Звіт включений у розсилання'");
			Иначе
				ЗаголовокОповещения = НСтр("ru='Отчеты включены в рассылку';uk='Звіти включені в розсилання'");
			КонецЕсли;
...
Процедура ПолучателиРассылкиНажатие(Элемент, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.ТипПолучателейРассылки) Тогда
		ТекстОшибки = НСтр("ru='Для ввода получателей необходимо выбрать их тип';uk='Для введення одержувачів необхідно вибрати їхній тип'");
...
Процедура КомандаВыполнитьСейчас(Команда)
	Если Не Объект.Подготовлена Тогда
		ПоказатьПредупреждение(, НСтр("ru='Рассылка не подготовлена';uk='Розсилання не підготовлене'"));
		Возврат;
	КонецЕсли;
	ПараметрыЗаписи = Новый Структура;
...
Процедура ИспользоватьПолучателяРассылкиВНастройкеОтчета(Команда)
	...
	Если Не Объект.Персонализирована Тогда
		...
		СообщениеТекст = НСтр("ru='Использовать получателя в параметрах возможно только для вида рассылки ""%1""';uk='Використовувати одержувача в параметрах можливо тільки для виду розсилання ""%1""'");
	...
	Если СКД Тогда
		...
		Если ИдентификаторНастройки = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ru='Выберите настройку отчета';uk='Виберіть налаштування звіту'"));
			Возврат;
		КонецЕсли;
		...
		Если СтрокаНастроек = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ru='Настройка отчета не найдена';uk='Налаштування звіту не знайдене'"));
			Возврат;
		КонецЕсли;
		...
		Если ЭтоЭлементОтбора Тогда
			...
			Иначе
				...
				КонецЕсли;
				Если ПолеКД = Неопределено Тогда
					ПоказатьПредупреждение(, НСтр("ru='Для настройки отчета не найдено описание доступного поля';uk='Для налаштування звіту не знайдений опис доступного поля'"));
					Возврат;
				КонецЕсли;
...
"Необходимо выбрать другой тип получателей или другую настройку.';uk='Тип ""%1"" не підходить по типу до обраного налаштування."
"Необхідно вибрати інший тип одержувачів або інше налаштування.'");
...
Процедура ПроверитьРассылкуПослеВыбораПолучателей(РезультатВыбора, ПараметрыДоставки) Экспорт
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		ПараметрыДоставки.Вставить("ШаблонТемы",    НСтр("ru='Тестовое сообщение 1С:Предприятие';uk='Тестове повідомлення 1С:Підприємство'"));
		ПараметрыДоставки.Вставить("ШаблонТекста",  НСтр("ru='Это сообщение отправлено системой рассылок 1С:Предприятие.';uk='Це повідомлення відправлене системою розсилань 1С:Підприємство.'") + Символы.ПС + Кэш.ЗаголовокСистемы);
...
"Нажмите ""Продолжить"", чтобы включить флажок ""Подготовлена"" и записать рассылку.';uk='Перед перевіркою розсилка має бути підготовлена."
...
"Нажмите ""Продолжить"", чтобы записать рассылку.';uk='Перед перевіркою розсилання повинна бути записана."
"Натисніть ""Продовжити"", щоб записати розсилання.'");
		КонецЕсли;
	...
	Если Представление.МаксимумПревышен
		...
		Если ПредставлениеОтключенных.Всего > 0 Тогда
			ШаблонОкончания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='(всего %1, исключая %2)';uk='(усього %1, крім %2)'"),
				Представление.Всего,
...
Функция ПолучателиУказаны(Получатели)
	Для Каждого СтрокаТаблицы Из Получатели Цикл
		...
		КонецЕсли;
	КонецЦикла;
	ТекстСообщения = НСтр("ru='Не выбрано ни одного получателя';uk='Не обране жодного одержувача'");
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПолучателиРассылки");
	Возврат Ложь;
КонецФункции
...
Функция ПроверитьСпособДоставки(Рассылка, Знач ПараметрыДоставки)
	...
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("ru='Рассылка отчетов. Проверка способа доставки';uk='Розсилання звітів. Перевірка способу доставки'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Процедура ЗаполнитьСведенияТаблицыОтчеты()
	...
	Для Каждого СтрокаОтчеты Из Объект.Отчеты Цикл
		...
		Если ИнформацияОбОтчете = Неопределено Тогда
			...
			СтрокаОтчеты.Представление = НСтр("ru='<Недостаточно прав для работы с отчетом>';uk='<Недостатньо прав для роботи зі звітом>'");
		Иначе
...
Функция ИнициализироватьОтчет(СтрокаОтчеты, ДобавитьДежурнуюФразу, ПользовательскиеНастройки, Интерактивно = Истина)
	// Параметры журнала
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("ru='Рассылка отчетов. Инициализация отчета';uk='Розсилання звітів. Ініціалізація звіту'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
"%2';uk='Звіт ""%1"" був видалений через помилку ініціалізації:"
"%2'"),
...
Процедура ДобавитьНастройкиОтчетов(ПрисоединяемыеОтчеты)
	...
	Для Каждого СтрокаОтчетыПараметры Из ПрисоединяемыеОтчеты Цикл
		...
		Если Не ЭтоНовый Тогда
			Если Найденные.Количество() > 0 Тогда
				ШаблонСтрокиСообщения = НСтр("ru='Для отчета ""%1"" загружены новые пользовательские настройки.';uk='Для звіту ""%1"" завантажені нові користувацькі налаштування.'");
			Иначе
