...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ПоказатьПредупреждение(, НСтр("ru='Требуется указать хотя бы одного получателя';uk='Потрібно вказати хоча б одного одержувача'"));
...
Процедура ОткрытьФормаДобавленияПолучателей(ЭтоПодбор)
	...
	Если ЭтоПодбор Тогда
		...
		ПараметрыФормыВыбора.Вставить("ЗаголовокФормыПодбора", НСтр("ru='Подбор получателей рассылки';uk='Підбір одержувачів розсилання'"));
...
Процедура ДобавитьПеретащитьПолучателя(ПолучательИлиНаборПолучателей)
	...
	Если МассивНовыхСтрок.Количество() > 0 Тогда
		Если МассивНовыхСтрок.Количество() = 1 Тогда
			ЗаголовокОповещения = НСтр("ru='Получатель добавлен в рассылку';uk='Одержувач доданий у розсилання'");
		Иначе
			ЗаголовокОповещения = НСтр("ru='Получатели добавлены в рассылку';uk='Одержувачі додані в розсилання'");
		КонецЕсли;
