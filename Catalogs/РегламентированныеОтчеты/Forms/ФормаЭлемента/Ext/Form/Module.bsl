...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПустаяСтрока(Объект.ИсточникОтчета) Тогда
		ПоказатьПредупреждение(,НСтр("ru='Укажите наименование объекта.';uk='Укажите наименование объекта.'"));
	...
	Если Объект.ВнешнийОтчетИспользовать И НЕ ВнешнийОтчетЗарегистрирован Тогда
		ПоказатьПредупреждение(,НСтр("ru='Внешний отчет отсутствует. Загрузите внешний отчет или установите признак ""Использовать"" в положение ""Объект"".';uk='Внешний отчет отсутствует. Загрузите внешний отчет или установите признак ""Использовать"" в положение ""Объект"".'"));
		Модифицированность = Истина;
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Функция ОпределитьТипОтчета(ФайлВнешнегоОтчета, Расширение = "", НеВыводитьСообщения = Ложь) Экспорт
	...
	Если НЕ ПустаяСтрока(ФайлВнешнегоОтчета) Тогда
		...
		Если ТипОтчета = 0 Тогда
			...
			Если НЕ НеВыводитьСообщения Тогда
				...
				НСтр("ru='Не найден отчет ""%1"".';uk='Не найден отчет ""%1"".'"), ФайлВнешнегоОтчета);
	...
	Иначе
		...
		Если НЕ НеВыводитьСообщения Тогда
			...
			Сообщение.Текст = НСтр("ru='Не выбран файл внешнего отчета.';uk='Не выбран файл внешнего отчета.'");
...
""
"Продолжить действие (не рекомендуется)?';uk='Внимание!"
""
"Отчет не предназначен для использования с текущей конфигурацией."
""
