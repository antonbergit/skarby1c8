...
Функция ПолучитьОтчет(НаименованиеОтчета = Неопределено)
	...
	Если ПравоДоступаКОтчету = Ложь Тогда
		...
		Сообщение.Текст = НСтр("ru='Недостаточно прав.';uk='Недостаточно прав.'");
		Сообщение.Сообщить();
	...
	ИначеЕсли ПравоДоступаКОтчету = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Отчет не найден.';uk='Отчет не найден.'");
		Сообщение.Сообщить();
	...
	Если ТекОтчет = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не удалось получить отчет.';uk='Не удалось получить отчет.'");
		Сообщение.Сообщить();
...
Функция УстановленаСортировкаПоКоду()
	...
	Если НЕ СортировкаПоКоду Тогда
		...
		ПоказатьПредупреждение(,НСтр("ru='Для изменения порядка следования необходимо установить сортировку по коду.';uk='Для изменения порядка следования необходимо установить сортировку по коду.'"));
...
Функция СоздатьНовыйОтчет()
	...
	Если Элементы.Список.ТекущаяСтрока = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Выберите отчет.';uk='Выберите отчет.'");
		Сообщение.Сообщить();
...
Процедура ПереместитьЭлементВыполнить(СписокРеквизит, СписокЭлемент, Вверх)
	...
	Иначе
		ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1';uk='%1'"), СтрОшибка));
	КонецЕсли;
...
Функция ПроверитьСписокПередОперацией(СписокРеквизит, СписокЭлемент, УстановленныеОтборы)
	...
	Для Каждого ЭлементГруппировки Из ЭлементыГруппировки Цикл
		Если ЭлементГруппировки.Использование Тогда
			ПоказатьПредупреждение(,НСтр("ru='Изменение порядка элементов невозможно при установленной группировке.';uk='Изменение порядка элементов невозможно при установленной группировке.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
