...
"(Да - добавить из классификатора; Нет - добавить самостоятельно; Отмена - отменить действияе)';uk='Є можливість додати одиницю виміру із класифікатора. Додати із класифікатора?"
"(Так - додати із класифікатора; Ні - додати самостійно; Скасування - скасувати дію)'");
...
Процедура ДобавитьЕдиницуИзмеренияИзКлассификатора()
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ДобавитьЕдиницуИзмеренияИзКлассификатораЗавершение", ЭтотОбъект);
...
&НаКлиенте
Процедура ДобавитьЕдиницуИзмеренияИзКлассификатораЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
