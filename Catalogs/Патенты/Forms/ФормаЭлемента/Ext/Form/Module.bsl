...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	Объект.Наименование = НСтр("ru='Патент';uk='Патент'") + ?(ПустаяСтрока(Объект.НомерПатента),""," №"+ Объект.НомерПатента)+
		 НСтр("ru=', срок действия:';uk=', срок действия:'") + Формат(Объект.ДатаНачала,"ДЛФ=D") + " - " + Формат(Объект.ДатаОкончания,"ДЛФ=D");
КонецПроцедуры
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	Если НЕ ЗначениеЗаполнено(Объект.НомерПатента) Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, , НСтр("ru='Номер патента';uk='Номер патенту'"));
	...
	Если СуммаНалога > 0  И НЕ ЗначениеЗаполнено(Объект.КБК) Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, , НСтр("ru='КБК';uk='КБК'"));
	...
	Если Объект.СуммаВторогоПлатежа > 0 И Объект.СуммаПервогоПлатежа > 0
		...
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, "КОРРЕКТНОСТЬ", НСтр("ru='Дата первого платежа';uk='Дата первого платежа'"));
...
Процедура УстановитьПредставлениеОплаты(Форма);
	...
	Если Форма.ОдинПлатеж Тогда
		...
		Элементы.СуммаПервогоПлатежа.Заголовок  	= Нстр("ru='Сумма налога';uk='Сума податку'");
	Иначе
		...
		Элементы.СуммаПервогоПлатежа.Заголовок  	= Нстр("ru='1/3 налога';uk='1/3 налога'");
	КонецЕсли;
...
Процедура СоздатьЗадачиПоОплате() Экспорт
	...
	ОбъектПерваяЗадача.ПредставлениеВКалендаре 			= ?(ОдинПлатеж, НСтр("ru='Уплатить налог с патента ';uk='Уплатить налог с патента '"), НСтр("ru='Уплатить 1/3 налога с патента ';uk='Уплатить 1/3 налога с патента '")) + ?(ПустаяСтрока(Объект.НомерПатента),"","№"+ Объект.НомерПатента)+ " (" + Формат(Объект.ДатаНачала,"ДЛФ=D") + "-" + Формат(Объект.ДатаПервогоПлатежа,"ДЛФ=D")+ ")";
	...
	Иначе
		...
		ОбъектВтораяЗадача.ПредставлениеВКалендаре 			= НСтр("ru='Уплатить 2/3 налога с патента ';uk='Уплатить 2/3 налога с патента '")+ ?(ПустаяСтрока(Объект.НомерПатента),"","№"+ Объект.НомерПатента)+ " (" + Формат(Объект.ДатаНачала,"ДЛФ=D") + "-" + Формат(Объект.ДатаВторогоПлатежа,"ДЛФ=D")+ ")";
