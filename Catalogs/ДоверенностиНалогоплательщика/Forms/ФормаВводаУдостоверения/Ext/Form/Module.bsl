...
Функция СохранениеВозможно()
	...
	Если НЕ ЗначениеЗаполнено(ДокументВид) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не задан вид документа.';uk='Не задан вид документа.'"), , "ДокументВид", , Отказ);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(СокрЛП(ДокументСерия) + СокрЛП(ДокументНомер)) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заданы серия и номер документа.';uk='Не заданы серия и номер документа.'"), , "ДокументСерия", , Отказ);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ДокументДатаВыдачи) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не задана дата выдачи документа.';uk='Не задана дата выдачи документа.'"), , "ДокументДатаВыдачи", , Отказ);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ДокументКемВыдан) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не задано наименование органа, выдавшего документ.';uk='Не задано наименование органа, выдавшего документ.'"), , "ДокументКемВыдан", , Отказ);
	КонецЕсли;
