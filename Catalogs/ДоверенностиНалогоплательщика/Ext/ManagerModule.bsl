...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Для каждого СтрокаМассиваОбъектов Из МассивОбъектов Цикл
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ДоверенностьНалогоплательщика", НСтр("ru='Доверенность налогоплательщика';uk='Доверенность налогоплательщика'"), СформироватьДоверенность(СтрокаМассиваОбъектов));
	КонецЦикла; 	
...
Функция СформироватьДоверенность(Знач Доверенность) Экспорт
	...
	Шапка.Параметры.ДатаВыдачи = Формат(Доверенность.ДатаВыдачи, "ДФ='«dd» MMMM yyyy'")+НСтр("ru=' года.';uk=' года.'");
	...
	Если Доверенность.ДатаОкончания = ДобавитьМесяц(Доверенность.ДатаВыдачи, 36) Тогда
		СрокВыдачи.Параметры.СрокВыдачи = НСтр("ru='сроком на три года.';uk='сроком на три года.'");
	ИначеЕсли Доверенность.ДатаОкончания = ДобавитьМесяц(Доверенность.ДатаВыдачи, 24) Тогда
		СрокВыдачи.Параметры.СрокВыдачи = НСтр("ru='сроком на два года.';uk='сроком на два года.'");
	ИначеЕсли Доверенность.ДатаОкончания = ДобавитьМесяц(Доверенность.ДатаВыдачи, 12) Тогда
		СрокВыдачи.Параметры.СрокВыдачи = НСтр("ru='сроком на год.';uk='сроком на год.'");
	ИначеЕсли Не ЗначениеЗаполнено(Доверенность.ДатаОкончания) Тогда 
		СрокВыдачи.Параметры.СрокВыдачи = НСтр("ru='.';uk='.'");
	Иначе
		СрокВыдачи.Параметры.СрокВыдачи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='сроком на период с %1 г. по %2 г.';uk='сроком на период с %1 г. по %2 г.'"),
	...
	Если ЗначениеЗаполнено(УдЛичнПредстФЛ) Тогда
		Подвал.Параметры.ДанныеУдостоверения_Представитель = "серия "+УдЛичнПредстФЛ.СерДок+", № " +УдЛичнПредстФЛ.НомДок+", выдан "+ УдЛичнПредстФЛ.ВыдДок + " "+Формат(УдЛичнПредстФЛ.ДатаДок, "ДФ='«dd» MMMM yyyy'")+НСтр("ru=' года.';uk=' года.'");
	КонецЕсли;
