...
Процедура ЮридическийАдресНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПредставлениеАдресаНачалоВыбора(ЭтотОбъект, Элемент, ДанныеВыбора, СтандартнаяОбработка,
		"ЮридическийАдрес", НСтр("ru='Юридический адрес организации';uk='Юридична адреса організації'"));
...
Процедура ФактическийАдресНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПредставлениеАдресаНачалоВыбора(ЭтотОбъект, Элемент, ДанныеВыбора, СтандартнаяОбработка,
		"ФактическийАдрес", НСтр("ru='Фактический адрес организации';uk='Фактична адреса організації'"));
...
Процедура ТелефонНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПредставлениеТелефонаНачалоВыбора(ЭтотОбъект, Элемент, ДанныеВыбора, СтандартнаяОбработка,
		"Телефон", НСтр("ru='Телефон организации';uk='Телефон організації'"));
	...
	Если Не ЗначениеЗаполнено(ГражданствоОКСМКодАльфа3) Тогда
		Если ЗначениеЗаполнено(Гражданство) Тогда
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='У страны ""%1"" не указан трехзначный буквенный код альфа-3 по ОКСМ.';uk='У країни ""%1"" не зазначений тризначний буквений код альфа-3 по ЗККМ.'"),
				Гражданство));
		КонецЕсли;
...
Процедура СтраховойНомерПФРПриИзменении(Элемент)
	...
	СтраховойНомерПФ = ТолькоЦифры(СтраховойНомерПФ);
...
"Обратитесь к администратору.';uk='Програма %1 ще не додана в список використовуваних програм."
"Зверніться до адміністратора.'"),
		...
		ИначеЕсли ЭтоПодчиненныйУзелРИБ Тогда
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Программа %1 еще не добавлена в список используемых программ."
"Выполните добавление в главном узле информационной базы.';uk='Програма %1 ще не додана в список використовуваних програм."
"Виконайте додавання в головному вузлі інформаційної бази.'"),
					Представление));
		Иначе
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить("Добавить",    НСтр("ru='Добавить';uk='Додати'"));
			Кнопки.Добавить("НеДобавлять", НСтр("ru='Не добавлять';uk='Не додавати'"));
			ПоказатьВопрос(
				Новый ОписаниеОповещения("ПрограммаОбработкаВыбораПродолжение", ЭтотОбъект, ВыбранноеЗначение),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Программа %1 еще не добавлена в список используемых программ."
"Добавить?';uk='Програма %1 ще не додана в список використовуваних програм."
"Додати?'"),
					Представление),
				Кнопки,, "НеДобавлять");
		КонецЕсли;
...
"используется в дальнейшем для создания электронных подписей.';uk='Ключ електронного підпису - це секретна інформація, яка зберігається"
"на комп`ютер, флешку, дискету або інший носій інформації й"
"використовується надалі для створення електронних підписів.'"));
...
"на выпуск сертификата и требуется для выпуска сертификата.';uk='Запит на сертифікат - це не секретна інформація, яка створюється"
"на основі ключа електронного підпису, відправляється разом із заявою"
"на випуск сертифіката й потрібна для випуску сертифіката.'"));
	КонецЕсли;
...
"5. Копия страхового свидетельства обязательного пенсионного страхования (СНИЛС) владельца сертификата.';uk='Комплект документів:"
"1. Заява на випуск сертифіката (підготовлене на попередньому кроці)."
"2. Копія свідоцтва про постановку на облік у податковому органі (ІПН)."
"3. Копія свідоцтва про державну реєстрацію індивідуального підприємця (ОДРН)."
"4. Копія документа, що засвідчує особистість власника сертифіката."
"5. Копія страхового свідчення обов`язкового пенсійного страхування (СНІОР) власника сертифіката.'");
	Иначе
		ТекстПредупреждения =
			НСтр("ru='Комплект документов:"
"1. Заявление на выпуск сертификата (подготовленное на предыдущем шаге)."
...
""
"* Копии документов заверяются подписью руководителя и печатью организации.';uk='Комплект документів:"
"1. Заява на випуск сертифіката (підготовлене на попередньому кроці)."
"2. Копія* свідоцтва про постановку на облік у податковому органі (ІПН)."
"3. Копія* свідоцтва про державну реєстрацію юридичної особи (ОДРН)."
"4. Копія* документа, що засвідчує особистість представника організації, зазначеного для випуску сертифіката."
"5. Копія* страхового свідоцтва обов`язкового пенсійного страхування (СНІОР) представника організації,"
"   зазначеного для випуску сертифіката."
"6. Копія* документа, що підтверджує повноваження керівника, що підписав заяву (протокол зборів"
"   засновників, рішення власника, устав) або актуальна виписка з ЄДРЮО, завірена податковим органом."
""
"* Копії документів засвідчуються підписом керівника й печаткою організації.'");
	КонецЕсли;
...
Процедура ВыгрузитьКорневойСертификат(Команда)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.СохранитьСертификат(Неопределено,
		АдресКорневогоСертификата, НСтр("ru='Корневой сертификат ООО НПЦ 1С';uk='Кореневий сертифікат ТОВ НВЦ 1С'"));
...
Процедура НапечататьДокументы(Команда)
	...
	НазваниеПечатнойФормы = НСтр("ru='Заявление на выпуск сертификата';uk='Заява на випуск сертифіката'");
...
Процедура ПерейтиНаСтраницуОтправкаЗаявления()
	...
	Если Не ЗначениеЗаполнено(Объект.Программа) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Поле ""Программа электронной подписи"" не заполнено';uk='Поле ""Програма електронному підпису"" не заповнене'"), , "Программа");
...
Процедура ПерейтиНаСтраницуОтправкаЗаявленияПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		...
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru='Создание ключа электронной подписи';uk='Створення ключа електронного підпису'"), "", Результат, Новый Структура);
		Возврат;
	КонецЕсли;
...
Процедура ПерейтиНаСтраницуОжиданиеОбработкиЗаявления()
	...
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить("Отправить",   НСтр("ru='Отправить';uk='Відправити'"));
	Кнопки.Добавить("НеОтправить", НСтр("ru='Не отправлять';uk='Не відправляти'"));
	
	Текст = "";
	Если Не ДокументыПечатались Тогда
		Текст =
			НСтр("ru='Документы еще не печатались."
"Заявление не будет принято пока не будут получены печатные документы.';uk='Документи ще не друкувалися."
"Заява не буде прийнята поки не будуть отримані друковані документи.'");
	КонецЕсли;
...
"Заявление может обрабатываться дольше обычного.';uk='Не зазначений ІПН обслуговуючої організації."
"Заява може оброблятися довше звичайного.'");
		КонецЕсли;
...
"Заявление может обрабатываться дольше обычного.';uk='Не указан ИНН или КПП обслуживающей организации."
...
"Библиотека интернет поддержки пользователей не внедрена в конфигурацию.';uk='Невозможно подключиться к порталу интернет-поддержки по причине:"
...
Функция ПерейтиНаСтраницуОжиданиеОбработкиЗаявленияНаСервере(ОписаниеОшибки)
	...
	СостояниеОбработкиЗаявления = НСтр("ru='Заявление принято для обработки.';uk='Заява прийнята для обробки.'");
...
Процедура ОбновитьДатаОбновленияСостоянияВЗаголовке(Форма)
	...
	Форма.Элементы.СостояниеОбработкиЗаявления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Состояние обработки заявления (на %1)';uk='Стан обробки заяви (на %1)'"),
...
Процедура ПерейтиНаСтраницуУстановкаСертификата()
	...
	Если Результат <> Истина Тогда
		ПослеЗаписи(НСтр("ru='Заявление отклонено.';uk='Заява відхилена.'"));
		Закрыть();
		Возврат;
	КонецЕсли;
...
Процедура ПерейтиНаСтраницуУстановкаСертификатаПослеУстановки(Установлен, Неопределен) Экспорт
	...
	Если Установлен Тогда
		...
		ПослеЗаписи(НСтр("ru='Заявление исполнено.';uk='Заява виконана.'"));
		Закрыть();
	Иначе
		ПослеЗаписи();
		ЗаписатьЗаявление();
...
Процедура ОбновитьДатаУстановкиСертификатаВЗаголовке(Форма)
	...
	Форма.Элементы.ОшибкаУстановкиСертификата.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Ошибка установки сертификата (на %1)';uk='Помилка установки сертифіката (на %1)'"),
...
Процедура ЗакрытьПомощникПослеУстановкиСертификатаЗавершение(Установлен, Неопределен) Экспорт
	...
	ПослеЗаписи(НСтр("ru='Сертификат установлен.';uk='Сертифікат встановлений.'"));
	Закрыть();
...
Процедура ПриИзмененииОрганизацииНаСервере(Загрузка = Ложь)
	...
	Если ЭтоИндивидуальныйПредприниматель Тогда
		...
		Элементы.ОГРН.Заголовок = НСтр("ru='ОГРНИП';uk='ОДРНІП'");
	Иначе
...
Функция ОрганизацияЗаполнена()
	...
	Если ЗначениеЗаполнено(МФО) Тогда
		ТекстСообщения = "";
		БИКСоответствуетТребованиям(МФО, ТекстСообщения);
		Если ЗначениеЗаполнено(ТекстСообщения) Тогда
	...
	Попытка
		...
		Если ПодробныйИтог.Результат <> "Корректный" Тогда
			...
			Если Не ЗначениеЗаполнено(Сообщение) Тогда
				Сообщение = НСтр("ru='Адрес не заполнен';uk='Адреса не заповнена'");
			КонецЕсли;
		КонецЕсли;
	Исключение
...
Функция РасчетныйСчетСоответствуетТребованиям(РасчетныйСчет, ТекстСообщения)
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Значение) Тогда
		ТекстСообщения = НСтр("ru='Расчетный счет должен состоять только из цифр.';uk='Розрахунковий рахунок повинен полягати тільки із цифр.'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрДлина(Значение) <> 20 Тогда
		ТекстСообщения = НСтр("ru='Расчетный счет должен состоять из 20 цифр.';uk='Розрахунковий рахунок повинен складатися з 20 цифр.'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСКонтрагентами") Тогда
		...
		Если Не МодульРегламентированныеДанныеКлиентСервер.КонтрольныйКлючЛицевогоСчетаСоответствуетТребованиям(Значение, МФО, ЭтоБанк) Тогда
			ТекстСообщения = НСтр("ru='Контрольное число счета не совпадает с рассчитанным с учетом БИК';uk='Контрольное число счета не совпадает с рассчитанным с учетом БИК'");
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Функция БИКСоответствуетТребованиям(БИК, ТекстСообщения)
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Значение) Тогда
		ТекстСообщения = НСтр("ru='БИК должен состоять только из цифр.';uk='БИК должен состоять только из цифр.'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрДлина(Значение) <> 9 Тогда
		ТекстСообщения = НСтр("ru='БИК должен состоять из 9 цифр.';uk='БИК должен состоять из 9 цифр.'");
		Возврат Ложь;
	КонецЕсли;
...
Функция КорреспондентскийСчетСоответствуетТребованиям(КорреспондентскийСчет, ТекстСообщения)
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Значение) Тогда
		ТекстСообщения = НСтр("ru='Корреспондентский счет должен состоять только из цифр.';uk='Кореспондентський рахунок повинен полягати тільки із цифр.'");
		Возврат Ложь;
	КонецЕсли;

	Если СтрДлина(Значение) <> 20 Тогда
		ТекстСообщения = НСтр("ru='Корреспондентский счет должен состоять из 20 цифр.';uk='Кореспондентський рахунок повинен складатися з 20 цифр.'");
		Возврат Ложь;
	КонецЕсли;
...
Функция АдресСоответствуетТребованиям(Знач АдресXML, ТекстСообщения)
	...
	Если Не АдресСтруктура.Свойство("КодРегиона") Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru='Это не российский адрес';uk='Це не російська адреса'");
		Возврат Ложь;
	КонецЕсли;
	
	// Проверка, что указан регион.
	Если Не АдресСтруктура.Свойство("Регион") Или Не ЗначениеЗаполнено(АдресСтруктура.Регион) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru='Не указан регион';uk='Не зазначений регіон'");
		Возврат Ложь;
	КонецЕсли;
	
	// Проверка, что регион указан правильно - код региона определен.
	Если Не ЗначениеЗаполнено(АдресСтруктура.КодРегиона) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru='Некорректный регион (код региона не определен)';uk='Некоректний регіон (код регіону не визначений)'");
		Возврат Ложь; 
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(НаселенныйПунктПолностью) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru='Не указан населенный пункт';uk='Не зазначений населений пункт'");
		Возврат Ложь;
	КонецЕсли;
...
Функция ТелефонСоответствуетТребованиям(Знач ТелефонXML, ТекстСообщения)
	...
	Если СтрЗаменить(ТелефонСтруктура.КодСтраны, "+", "") <> "7" Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru='Код страны не российский (должен быть ""7"")';uk='Код страны не российский (должен быть ""7"")'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(НомерТелефонаБезКодаСтраны) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru='Не заполнен номер телефона';uk='Не заповнений номер телефону'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрДлина(ТолькоЦифры(НомерТелефонаБезКодаСтраны)) <> 10 Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru='Номер телефона с кодом города должен состоять из 10-и цифр';uk='Номер телефону з кодом міста повинен складатися з 10-і цифр'");
		Возврат Ложь;
	КонецЕсли;
		...
		Если Не ЭтоИндивидуальныйПредприниматель Тогда
			Если ЗначениеЗаполнено(Реквизиты.Директор) Тогда
				Элементы.Сотрудник.СписокВыбора.Добавить(Реквизиты.Директор, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Директор: %1';uk='Директор: %1'"), Реквизиты.Директор));
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(Реквизиты.ГлавныйБухгалтер) Тогда
				Элементы.Сотрудник.СписокВыбора.Добавить(Реквизиты.ГлавныйБухгалтер, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Главный бухгалтер: %1';uk='Головний бухгалтер: %1'"), Реквизиты.ГлавныйБухгалтер));
			КонецЕсли;
		КонецЕсли;
		
		Если ЭтоИндивидуальныйПредприниматель Тогда
			Элементы.Сотрудник.Заголовок = НСтр("ru='Индивидуальный предприниматель';uk='Індивідуальний підприємець'");
		Иначе
			Элементы.Сотрудник.Заголовок = НСтр("ru='Сотрудник';uk='Співробітник'");
		КонецЕсли;
...
Процедура ПриИзмененииВидаДокументаНаСервере()
	...
	Если ДокументВид = "21" Тогда
		Элементы.ДокументНомер.Заголовок = НСтр("ru='Серия и номер';uk='Серія й номер'");
		Элементы.ДокументНомер.Маска = "99 99 999999";
	Иначе
		Элементы.ДокументНомер.Заголовок = НСтр("ru='Номер';uk='Номер'");
	...
	Если ПроверитьРеквизит(Отказ, "СтраховойНомерПФР") Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСКонтрагентами") Тогда
			...
			МодульРегламентированныеДанныеКлиентСервер.СтраховойНомерПФРСоответствуетТребованиям(
				СтраховойНомерПФ, ТекстСообщения);
		КонецЕсли;
		Если ЗначениеЗаполнено(ТекстСообщения) Тогда
...
Функция НомерПаспортаРФСоответствуетТребованиям(Знач НомерПаспортаРФ, ТекстСообщения)
	...
	Если ПустаяСтрока(СтрокаЦифр) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru='Номер паспорта не заполнен';uk='Номер паспорта не заповнений'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрДлина(СтрокаЦифр) < 10 Тогда
		ТекстСообщения  =  ТекстСообщения + НСтр("ru='Номер паспорта задан неполностью';uk='Номер паспорта заданий неповністю'");
		Возврат Ложь;
	КонецЕсли;
	
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрокаЦифр) Тогда
		Результат = Ложь;
		ТекстСообщения = ТекстСообщения + НСтр("ru='Номер паспорта должен состоять только из цифр.';uk='Номер паспорта повинен полягати тільки із цифр.'");
	КонецЕсли;
...
Функция ПроверитьРеквизит(Отказ, ИмяРеквизита)
	...
	Если Не ЗначениеЗаполнено(ЭтотОбъект[ИмяРеквизита]) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Поле ""%1"" не заполнено';uk='Поле ""%1"" не заповнене'"), ЗаголовокПоля), , ИмяРеквизита, , Отказ);
...
Функция ПодготовленныйДокумент()
	...
	Макет.Параметры.УдостоверениеЛичности = ДокументВидПредставление + " " + ДокументНомерПредставление + " "
		+ НСтр("ru='от';uk='від'") + " " + ДокументДатаВыдачиПредставление + " "
		+ НСтр("ru='выданный';uk='виданий'") + " " + ДокументКемВыдан;
...
"%1';uk='Не вдалося відправити заяву через:"
"%1'");
...
"%1';uk='Не удалось подключиться к порталу интернет-поддержки по причине:"
"%1'"),
	...
	Если Не ЗначениеЗаполнено(Ответ) Тогда
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОписанияОшибки,
			НСтр("ru='Сервер вернул пустой ответ.';uk='Сервер повернув порожню відповідь.'"));
		Возврат Ложь;
	КонецЕсли;
...
""
"Попробуйте отправить заявление еще раз.';uk='Не вдалося перевірити договір на ІТС (its.1c.ru) через:"
"Квиток, отриманий для користувача %1, невірний або застарів."
""
"Спробуйте відправити заяву ще раз.'"),
...
""
"После заключения договора отправьте заявление еще раз.';uk='Заява не прийнята через:"
"Користувач %1 не має діючого договору на ІТС (its.1c.ru)."
"Для укладання договору зверніться в обслуговуючу організацію."
""
"Після укладання договору відправте заяву ще раз.'"),
			ПредставлениеПользователяПорталаИнтернетПоддержки());
	Иначе
...
Функция ЗаявлениеXML(Билет)
	...
	ЗаписьXML.ЗаписатьАтрибут("ВерсПрог", "1С");
		...
		ЗаписьXML.ЗаписатьАтрибут("СНИЛС", ПредставлениеРеквизитаСтраховойНомерПФР(СтраховойНомерПФ));
...
"Удалите его и создайте новое.';uk='Не вдалося прочитати раніше створений запит на сертифікат."
"Заява не може бути відправлена."
"Видаліть її й створіть нову.'");
...
"%1';uk='Не вдалося оновити стан через:"
"%1'");
	...
	Если КодРезультата = "0" Тогда
		...
		Иначе
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОписанияОшибки,
				НСтр("ru='Сервер вернул пустые данные.';uk='Сервер повернув порожні дані.'"));
		КонецЕсли;
		
	ИначеЕсли КодРезультата = "1" Тогда
		ОписаниеОшибки = НСтр("ru='Заявление еще не обработано, попробуйте позже.';uk='Заява ще не оброблена, спробуйте пізніше.'");
	Иначе
		УзелDOM = ПостроительDOM.ПолучитьЭлементыПоИмени("errorMessage");
		Если УзелDOM.Количество() > 0 Тогда
			ОписаниеОшибки = УзелDOM[0].ТекстовоеСодержимое;
		Иначе
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сервер вернул код ошибки: %1';uk='Сервер повернув код помилки: %1'"),
	...
	Попытка
		...
		Если Не ФайлНайден Тогда
			ВызватьИсключение НСтр("ru='Неверный формат данных.';uk='Невірний формат даних.'");
		КонецЕсли;
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось разобрать ответ сервера по причине:"
"%1';uk='Не вдалося розібрати відповідь сервера через:"
"%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	КонецПопытки;
...
Процедура УстановитьСертификатПослеСозданияМенеджераКриптографии(Менеджер, Контекст) Экспорт
	...
	Если ТипЗнч(Менеджер) <> Тип("МенеджерКриптографии") Тогда
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru='Установка сертификата на компьютер';uk='Установка сертифіката на комп`ютер'"), "", Менеджер, Новый Структура);
...
"установить расширение для веб-клиента 1С:Предприятия.';uk='Для встановлення сертифіката на комп`ютер потрібно"
"встановити розширення для веб-клієнта 1С:Підприємства.'"),
			НСтр("ru='Для установки сертификата на компьютер требуется"
"установить дополнительную внешнюю компоненту.';uk='Для встановлення сертифіката на комп`ютер потрібно"
"встановити додаткову зовнішню компоненту.'"));
	КонецЕсли;
...
Процедура УстановитьСертификатПриОшибкеСозданияОбъектаКриптографииПослеПодключенияРасширенияРаботыСФайлами(Подключено, Контекст) Экспорт
	...
	Если Не Подключено Тогда
		ОшибкаУстановкиСертификата = НСтр("ru='Не установлено расширение для работы с файлами.';uk='Не встановлене розширення для роботи з файлами.'");
	Иначе
...
Процедура УстановитьСертификатПриОшибкеСозданияОбъектаКриптографииПослеПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
	...
	Если Не Подключено Тогда
		ОшибкаУстановкиСертификата = НСтр("ru='Не установлено расширение для работы с криптографией.';uk='Не встановлене розширення для роботи із криптографією.'");
	Иначе
		ОшибкаУстановкиСертификата = НСтр("ru='Не установлена дополнительная внешняя компонента.';uk='Не встановлений додатковий зовнішній компонент.'");
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПолученияРазрешений(РазрешенияПолучены, Контекст) Экспорт
	...
	Если Не РазрешенияПолучены Тогда
		ОшибкаУстановкиСертификата =
			НСтр("ru='Сохранение сертификатов во временную папку отменено пользователем.';uk='Збереження сертифікатів у тимчасову папку скасоване користувачем.'");
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ложь);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Если ПолученныеФайлы = Неопределено
	 Или ПолученныеФайлы.Количество() <> 2 Тогда
		...
		ОшибкаУстановкиСертификата = НСтр("ru='Сертификаты не были сохранены во временную папку.';uk='Сертифікати не були збережені в тимчасову папку.'");
...
"Имя контейнера: ""%1""."
"Путь к контейнеру: ""%2"".';uk='Не вдалося знайти контейнер ключа на комп`ютері."
"Ім`я контейнера: ""%1""."
"Шлях до контейнера: ""%2"".'"),
			КонтейнерКлючаИмя,
			КонтейнерКлючаПуть);
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ложь);
		Возврат;
...
Процедура УстановитьСертификатПослеВызоваУстановитьСертификатВКонтейнерИХранилище(Результат, ПараметрыВызова, Контекст) Экспорт
	...
	Если Результат <> Истина И Не ЗначениеЗаполнено(ОшибкаУстановкиСертификата) Тогда
		ОшибкаУстановкиСертификата = НСтр("ru='Не удалось установить личный сертификат.';uk='Не вдалося встановити особистий сертифікат.'");
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПоискаСертификата(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("СертификатКриптографии") Тогда
		Если Результат.Свойство("СертификатНеНайден") Тогда
			ОшибкаПоиска =
				НСтр("ru='Не удалось найти личный сертификат, установленный на компьютер.';uk='Не вдалося знайти особистий сертифікат, встановлений на комп`ютер.'");
		Иначе
...
"%1';uk='Не вдалося знайти особистий сертифікат, встановлений на комп`ютер через:"
"%1'"),
...
Процедура УстановитьСертификатПослеВызоваИмпортироватьСертификат(Результат, ПараметрыВызова, Контекст) Экспорт
	...
	Если Результат <> Истина И Не ЗначениеЗаполнено(ОшибкаУстановкиСертификата) Тогда
		ОшибкаУстановкиСертификата = НСтр("ru='Не удалось установить корневой сертификат.';uk='Не вдалося встановити кореневий сертифікат.'");
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПоискаКорневогоСертификата(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("СертификатКриптографии") Тогда
		Если Результат.Свойство("СертификатНеНайден") Тогда
			ОшибкаПоиска =
				НСтр("ru='Не удалось найти корневой сертификат, установленный на компьютер.';uk='Не вдалося знайти кореневий сертифікат, встановлений на комп`ютер.'");
		Иначе
			ОшибкаПоиска = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось найти корневой сертификат, установленный на компьютер по причине:"
"%1';uk='Не вдалося знайти кореневий сертифікат, встановлений на комп`ютер через:"
"%1'"),
				Результат.ОписаниеОшибки);
		КонецЕсли;
		ОшибкаУстановкиСертификата = СокрЛ(ОшибкаУстановкиСертификата + Символы.ПС + Символы.ПС) + ОшибкаПоиска;
...
Процедура ПредставлениеАдресаНачалоВыбора(Форма, Элемент, ДанныеВыбора, СтандартнаяОбработка, ИмяРеквизита, ЗаголовокФормы)
	...
	ВидКонтактнойИнформации.Вставить("АдресТолькоУкраинский",        Истина);
...
Функция ИмяРегионаРФПоРекомендациямДляСКПЭП(КодРегиона)
	...
	Если Не СтрНачинаетсяС(Строка, КодРегиона + " ") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Регион РФ с кодом ""%1"" не существует.';uk='Регіон РФ із кодом ""%1"" не існує.'"),
			КодРегиона);
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Строка) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для региона РФ с кодом ""%1"" имя, рекомендованное для СКПЭП, еще не назначено.';uk='Для регіону РФ із кодом ""%1"" ім`я, рекомендоване для СКПЕП, ще не призначене.'"),
			КодРегиона);
	КонецЕсли;
...
Процедура ЗагрузитьЗаявление()
	...
	ИначеЕсли ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.Отклонено Тогда
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ru='Заявление, по которому не удалось получить сертификат';uk='Заява, по якій не вдалося одержати сертифікат'");
		Элементы.ФормаДалее.Видимость = Ложь;
		Элементы.ФормаЗакрыть.Заголовок = НСтр("ru='Закрыть';uk='Закрити'");
	...
	ИначеЕсли ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.Исполнено Тогда
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ru='Заявление, по которому был получен сертификат';uk='Заява, по якій був отриманий сертифікат'");
...
Процедура ЗаполнитьРеквизитыЗаявления(Все = Ложь)
	...
	Если Все Тогда
		...
		Строка.Реквизит = НСтр("ru='Программа электронной подписи';uk='Програма електронного підпису'");
		Строка.Значение = ПредставлениеПрограммы;
	КонецЕсли;
	
	// Заполнение сведений об организации.
	ЗаполнитьРеквизит(НСтр("ru='Сведения об организации';uk='Відомості про організацію'"),, Истина);
	...
	ЗаполнитьРеквизит(НСтр("ru='Сведения о владельце сертификата';uk='Відомості про власника сертифіката'"),, Истина);
	...
	ЗаполнитьРеквизит("СтраховойНомерПФР", ПредставлениеРеквизитаСтраховойНомерПФР(СтраховойНомерПФ));
	...
	Строка = РеквизитыЗаявления.Добавить();
	Строка.Реквизит = НСтр("ru='Документ, удостоверяющий личность';uk='Документ, що засвідчує особу'");
	Строка.Значение = Элементы.ДокументВид.СписокВыбора.НайтиПоЗначению(ДокументВид).Представление;
	
	Строка = РеквизитыЗаявления.Добавить();
	Если ДокументВид = "21" Тогда
		Строка.Реквизит = НСтр("ru='Серия и номер';uk='Серія й номер'");
		Строка.Значение = ПредставлениеРеквизитаНомерПаспортаРФ(ДокументНомер);
	Иначе
		Строка.Реквизит = НСтр("ru='Номер';uk='Номер'");
	...
	Если Не ЭтоИндивидуальныйПредприниматель Тогда
		...
		ЗаполнитьРеквизит(НСтр("ru='Сведения о руководителе';uk='Відомості про керівника'"),, Истина);
		
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("ru='ФИО руководителя';uk='ПІБ керівника'");
		Строка.Значение = ДокументыРуководитель;
		
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("ru='Должность руководителя';uk='Посада керівника'");
		Строка.Значение = ДокументыРуководительДолжность;
		
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("ru='Основание действий руководителя';uk='Підстава дій керівника'");
		Строка.Значение = ДокументыРуководительОснование;
	КонецЕсли;
	
	// Заполнение сведений о печатных документах.
	ЗаполнитьРеквизит(НСтр("ru='Сведения об обслуживающей организации';uk='Відомості про обслуговуючу організацію'"),, Истина);
	
	Строка = РеквизитыЗаявления.Добавить();
	Строка.Реквизит = НСтр("ru='Наименование партнера';uk='Найменування партнера'");
	Строка.Значение = ДокументыПартнер;
	
	Строка = РеквизитыЗаявления.Добавить();
	Строка.Реквизит = НСтр("ru='ИНН партнера';uk='ІПН партнера'");
	Строка.Значение = ДокументыПартнерИНН;
	
	Если Не ДокументыПартнерЭтоИП Тогда
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("ru='КПП партнера';uk='КПП партнера'");
...
Функция ПредставлениеРеквизитаПол(Пол)
	...
	Если Пол = "Мужской" Тогда
		Возврат НСтр("ru='Мужской';uk='Чоловічий'");
		
	ИначеЕсли Пол = "Женский" Тогда
		Возврат НСтр("ru='Женский';uk='Жіночий'");
	КонецЕсли;
...
Процедура УстановитьСоглашение()
	...
	ТекстHTML = СтрЗаменить(ТекстHTML, "%1",
		...
		НСтр("ru='ООО «Научно-производственный центр ""1С""»';uk='ТОВ «Науково-виробничий центр ""1С""»'"));
	
	ТекстHTML = СтрЗаменить(ТекстHTML, "%2",
		// Адрес регламента удостоверяющего центра.
		НСтр("ru='http://ca.1c.ru/reglament.pdf';uk='http://ca.1c.ru/reglament.pdf'"));
	
	ТекстHTML = СтрЗаменить(ТекстHTML, "%3",
		// Сеть доверенных удостоверяющих центров.
		НСтр("ru='уполномоченного федерального органа в области использования электронной подписи';uk='уповноваженого федерального органа в області використання електронного підпису'"));
...
Процедура ПослеВыбораПрограммыПослеСозданияМенеджераКриптографии(Менеджер, Контекст) Экспорт
	...
	Если ТипЗнч(Менеджер) <> Тип("МенеджерКриптографии") Тогда
		...
		Если Контекст.ПоказатьОшибку Тогда
			ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
				НСтр("ru='Выбор программы электронной подписи';uk='Вибір програми електронному підпису'"), "", Менеджер, Новый Структура);
		КонецЕсли;
...
Процедура ЗаписатьЗаявление()
	...
	Если ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.НеПодготовлено Тогда
		...
		ТекущийОбъект.Наименование = Фамилия + " " + Имя + " " + Отчество + " (" + НСтр("ru='Заявление на новый сертификат';uk='Заява на новий сертифікат'") + ")";
		// Организация.
	...
	ИначеЕсли ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.Отклонено Тогда
		Если ЭтоНовоеСостояние Тогда
			ОбновитьЗначение(Содержание, "ДатаОбновленияСостояния");
			ОбновитьЗначение(Содержание, "СостояниеОбработкиЗаявления");
			НовоеНаименование = Фамилия + " " + Имя + " " + Отчество + " (" + НСтр("ru='Удалить';uk='Видалити'") + ")";
...
Процедура ПослеЗаписи(ТекстОповещения = "")
	...
	Если Не ЗначениеЗаполнено(ТекстОповещения) Тогда
		ТекстОповещения = НСтр("ru='Заявление сохранено.';uk='Заява збережена.'");
	КонецЕсли;
...
"требуется установить расширение для веб-клиента 1С:Предприятия.';uk='Для створення ключа електронного підпису й запиту на сертифікат"
"потрібно встановити розширення для веб-клієнта 1С:Підприємства.'"),
			НСтр("ru='Для создания ключа электронной подписи и запроса на сертификат"
"требуется установить дополнительную внешнюю компоненту.';uk='Для створення ключа електронного підпису й запиту на сертифікат"
"потрібно встановити додаткову зовнішню компоненту.'"));
	КонецЕсли;
...
Процедура СоздатьКлючИЗапросНаСертификатПослеПроверкиСуществованияКонтейнераКлючей(Существует, Контекст) Экспорт
	...
	Если Существует Тогда
		ПоказатьОповещениеПользователя(НСтр("ru='Используется ранее созданный контейнер ключа:';uk='Використовується раніше створений контейнер ключа:'"),,
			КонтейнерКлючаПуть);
...
"%1';uk='Не вдалося виконати операцію через:"
" %1'"),
		КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
	
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ru='Создание ключа электронной подписи';uk='Створення ключа електронного підпису'"), "", Ошибка, Новый Структура);
...
"требуются права администратора операционной системы.';uk='Ключ електронного підпису не створений."
""
"Слід враховувати, що для створення ключа за допомогою програми %1,"
"потрібні права адміністратора операційної системи.'"),
				Строки[0].Представление));
		Иначе
			ПоказатьПредупреждение(,
				НСтр("ru='Ключ электронной подписи не создан.';uk='Ключ електронного підпису не створений.'"));
		КонецЕсли;
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ложь);
		Возврат;
	КонецЕсли;
	...
	ПоказатьОповещениеПользователя(НСтр("ru='Создан новый контейнер ключа:';uk='Створений новий контейнер ключа:'"),, КонтейнерКлючаПуть);
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиВызоваСоздатьЗапросНаСертификат(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке,
		НСтр("ru='Не удалось выполнить операцию по причине:';uk='Не вдалося виконати операцію через:'"));
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиВызоваПолучитьОткрытыйКлюч(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке,
		НСтр("ru='Не удалось получить открытую часть ключа по причине:';uk='Не вдалося одержати відкриту частину ключа через:'"));
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиВызоваВычислитьИдентификаторКлючаСубъекта(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке,
		НСтр("ru='Не удалось вычислить идентификатор ключа субъекта по причине:';uk='Не вдалося обчислити ідентифікатор ключа суб`єкта через:'"));
...
Процедура СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке = Неопределено, ЗаголовокОшибки = "")
	...
	Если ИнформацияОбОшибке <> Неопределено Тогда
		...
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru='Создание запроса на сертификат';uk='Створення запиту на сертифікат'"), "", Ошибка, Новый Структура);
		
	ИначеЕсли Не Контекст.ЗапросСоздан Тогда
		ПоказатьПредупреждение(, НСтр("ru='Запрос на сертификат не создан.';uk='Запит на сертифікат не створений.'"));
	КонецЕсли;
...
Процедура ПолучитьИмяНовогоКонтейнераКлюча(Оповещение)
	...
	Если ЭтоИндивидуальныйПредприниматель Тогда
		...
		Субъект = ПодготовитьСтрокуДляИмениКонтейнера(Фамилия) + " "
			...
			+ НСтр("ru='Индивидуальный предприниматель';uk='Індивідуальний підприємець'");
	Иначе
...
Функция ОписаниеЗапросаНаКвалифицированныйСертификат(Поля)
	...
	Поля.Вставить("SNILS", СтраховойНомерПФ);
...
Процедура СоздатьОбъектКриптографииПослеПодключенияВнешнейКомпоненты(Подключена, Контекст) Экспорт
	...
	Иначе
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("Установить",      НСтр("ru='Установить';uk='Встановити'"));
		Кнопки.Добавить("НеУстанавливать", НСтр("ru='Не устанавливать';uk='Не встановлювати'"));
...
"%1';uk='Не вдалося встановити додаткову зовнішню компоненту через:"
"%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
	...
	Если Не Подключена Тогда
		ПоказатьПредупреждение(
			Новый ОписаниеОповещения("СоздатьОбъектКриптографииПослеПредупрежденияОбОшибке",
				ЭтотОбъект, Контекст),
			НСтр("ru='Не удалось подключить дополнительную внешнюю компоненту.';uk='Не вдалося підключити додаткову зовнішню компоненту.'"));
	Иначе
		СоздатьОбъектКриптографииПродолжение(Контекст);
	КонецЕсли;
...
"%1';uk='Не вдалося створити об`єкт зовнішньої компоненти для роботи із криптографією через:"
"%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
		ВыполнитьОбработкуОповещения(Контекст.ОбработкаПродолжения, Ложь);
		Возврат;
...
"%1';uk='Не вдалося створити об`єкт зовнішньої компоненти для роботи із двійковими даними через:"
"%1'"),
				КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
			ВыполнитьОбработкуОповещения(Контекст.ОбработкаПродолжения, Ложь);
			Возврат;
...
"%1';uk='Програма електронного підпису не доступна через зовнішню компоненту через:"
"%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
