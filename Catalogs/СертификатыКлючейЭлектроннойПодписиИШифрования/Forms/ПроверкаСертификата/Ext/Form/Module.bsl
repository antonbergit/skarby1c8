...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПроверкаПриВыборе Тогда
		Элементы.ФормаПроверить.Заголовок = НСтр("ru='Проверить и продолжить';uk='Перевірити й продовжити'");
...
"при этом дополнительных проверок не указано.';uk='Для перевірки сертифіката відключені стандартні перевірки,"
...
Процедура ПоказатьПредупреждениеОНевозможностиПродолжения()
	...
	ПоказатьПредупреждение(,
		НСтр("ru='Не удалось продолжить, т.к. пройдены не все требуемые проверки.';uk='Не вдалося продовжити, тому що пройдені не всі необхідні перевірки.'"));
...
Процедура ПроверитьНаСторонеКлиентаПослеИнициализацииСертификата(СертификатКриптографии, Контекст) Экспорт
	...
	Если Не Элементы.ГруппаЗаконныйСертификат.Видимость
	 Или Контекст.СертификатКриптографии.Субъект.Свойство("SN") Тогда
		...
		ОписаниеОшибки = "";
	Иначе
		ОписаниеОшибки = НСтр("ru='В описании субъекта сертификата не найдено поле ""SN"".';uk='В описі суб`єкта сертифіката не знайдене поле ""SN"".'");
	КонецЕсли;
...
Процедура ПроверитьНаСторонеКлиентаПослеПоискаСертификата(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("СертификатКриптографии") Тогда
		ОписаниеОшибки = Результат.ОписаниеОшибки + Символы.ПС + Символы.ПС
			+ НСтр("ru='Проверка подписания, созданной подписи и расшифровки не могут быть выполнены.';uk='Перевірка підписання, створеного підпису й розшифрування не можуть бути виконана.'");
	Иначе
		ОписаниеОшибки = "";
	КонецЕсли;
...
Процедура ПроверитьНаСторонеКлиентаПослеПоискаСертификатаВМоделиСервиса(Результат, Контекст) Экспорт
	...
	ИначеЕсли Результат.Сертификат = Неопределено Тогда
		ОписаниеОшибки = НСтр("ru='Сертификат не найден в хранилище сертификатов.';uk='Сертификат не найден в хранилище сертификатов.'") + Символы.ПС
			+ НСтр("ru='Проверка подписания, созданной подписи и расшифровки не могут быть выполнены.';uk='Перевірка підписання, створеного підпису й розшифрування не можуть бути виконана.'");
	Иначе
		ОписаниеОшибки = "";
	КонецЕсли;
...
Процедура ПроверитьНаСторонеКлиентаПослеПроверкиСертификата(Результат, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ru='Программа для использования закрытого ключа не указана в сертификате.';uk='Програма для використання закритого ключа не зазначена в сертифікаті.'");
...
"расшифровки не могут быть выполнены.';uk='Перевірка підписання, створеного підпису, шифрування й"
"розшифрування не можуть бути виконані.'");
	КонецЕсли;
