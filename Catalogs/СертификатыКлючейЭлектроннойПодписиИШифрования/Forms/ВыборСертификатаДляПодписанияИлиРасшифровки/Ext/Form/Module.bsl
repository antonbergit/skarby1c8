...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДобавлениеВСписок Тогда
		...
		Элементы.ПоказыватьВсе.Подсказка =
			НСтр("ru='Показать все сертификаты без отбора (например, включая добавленные и просроченные)';uk='Показати всі сертифікати без відбору (наприклад, включаючи додані й прострочені)'");
	КонецЕсли;
	...
	Если ДляШифрованияИРасшифровки = Истина Тогда
		Если Параметры.ДобавлениеВСписок Тогда
			Заголовок = НСтр("ru='Добавление сертификата для шифрования и расшифровки данных';uk='Додавання сертифіката для шифрування й розшифрування даних'");
		Иначе
			Заголовок = НСтр("ru='Выбор сертификата для шифрования и расшифровки данных';uk='Вибір сертифіката для шифрування й розшифрування даних'");
		КонецЕсли;
	...
	ИначеЕсли ЭлектроннаяПодпись.ИспользоватьШифрование() Тогда
		Заголовок = НСтр("ru='Добавление сертификата для подписания и шифрования данных';uk='Додавання сертифіката для підписання й шифрування даних'");
	Иначе
		Заголовок = НСтр("ru='Добавление сертификата для подписания данных';uk='Додавання сертифіката для підписання даних'");
	КонецЕсли;
	...
	Если СоздаватьЭлектронныеПодписиНаСервере Тогда
		Элементы.ГруппаСертификаты.Заголовок =
			НСтр("ru='Личные сертификаты на компьютере и сервере';uk='Особисті сертифікати на комп`ютері й сервері'");
	КонецЕсли;
...
Процедура ВыбратьПослеПроверкиСертификата(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Если ДляШифрованияИРасшифровки = Истина Тогда
			ЗаголовокФормы = НСтр("ru='Проверка шифрования и расшифровки';uk='Перевірка шифрування й розшифрування'");
		Иначе
...
Процедура ПерейтиКВыборуТекущегоСертификата(Оповещение)
	...
	Если Элементы.Сертификаты.ТекущиеДанные = Неопределено Тогда
		Результат.ОписаниеОшибки = НСтр("ru='Выделите сертификат, который будет использоваться.';uk='Виділіть сертифікат, який буде використовуватися.'");
...
"Откройте заявление на выпуск сертификата и выполните требуемые шаги.';uk='Для цього сертифіката заява на випуск ще не виконана."
"Відкрийте заяву на випуск сертифіката й виконайте необхідні кроки.'");
		ВыполнитьОбработкуОповещения(Оповещение, Результат);
		Возврат;
	КонецЕсли;
	...
	Если СертификатНаСервере Тогда
		...
		Иначе
			Результат.ОписаниеОшибки = НСтр("ru='Сертификат не найден на сервере (возможно удален).';uk='Сертифікат не знайдений на сервері (можливо видалений).'");
			Результат.ОбновитьСписокСертификатов = Истина;
			ВыполнитьОбработкуОповещения(Оповещение, Результат);
		КонецЕсли;
		Возврат;
...
Процедура ПерейтиКВыборуТекущегоСертификатаПослеПоискаСертификата(РезультатПоиска, Контекст) Экспорт
	...
	Если ТипЗнч(РезультатПоиска) <> Тип("СертификатКриптографии") Тогда
		Если РезультатПоиска.Свойство("СертификатНеНайден") Тогда
			Контекст.Результат.ОписаниеОшибки = НСтр("ru='Сертификат не найден на компьютере (возможно удален).';uk='Сертифікат не знайдений на комп`ютері (можливо видалений).'");
		Иначе
...
Процедура ПроверитьСертификатПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии")
		...
		Если ДляШифрованияИРасшифровки = Истина Тогда
			Результат.Вставить("ЗаголовокОшибки", НСтр("ru='Не удалось пройти проверку шифрования по причине:';uk='Не вдалося пройти перевірку шифрування через:'"));
		Иначе
			Результат.Вставить("ЗаголовокОшибки", НСтр("ru='Не удалось пройти проверку подписания по причине:';uk='Не вдалося пройти перевірку підписання через:'"));
		КонецЕсли;
...
Функция ПроверитьСертификатИЗаписатьВСправочник(Знач ЗначениеПароля, ОшибкаНаСервере)
	...
	Если ЭлектроннаяПодписьСлужебный.МенеджерКриптографии("", Ложь, ОшибкаНаСервере) = Неопределено
	   И ОшибкаНаСервере.Общая Тогда
		...
		Если ДляШифрованияИРасшифровки = Истина Тогда
			ОшибкаНаСервере.Вставить("ЗаголовокОшибки", НСтр("ru='Не удалось пройти проверку шифрования по причине:';uk='Не вдалося пройти перевірку шифрування через:'"));
		Иначе
			ОшибкаНаСервере.Вставить("ЗаголовокОшибки", НСтр("ru='Не удалось пройти проверку подписания по причине:';uk='Не вдалося пройти перевірку підписання через:'"));
		КонецЕсли;
		Возврат Ложь;
	КонецЕсли;
...
"%2';uk='Не вдалося пройти перевірку шифрування за допомогою програми %1 через:"
"%2'"),
...
"%2';uk='Не вдалося пройти перевірку розшифрування за допомогою програми %1 через:"
"%2'"),
		ОписаниеПрограммы.Наименование,
		ПредставлениеОшибки);
...
"%2';uk='Не вдалося пройти перевірку підписання за допомогою програми %1 через:"
"%2'"),
		ОписаниеПрограммы.Наименование,
		ПредставлениеОшибки);
