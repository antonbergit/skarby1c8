		...
		Для каждого Строка Из Строки Цикл
			...
			Если ТипЗнч(ДанныеСертификата) <> Тип("ДвоичныеДанные") Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Данные сертификата ""%1"" не найдены в справочнике';uk='Дані сертифіката ""%1"" не знайдені в довіднику'"), Выборка.Представление);
			КонецЕсли;
			Попытка
...
"%2';uk='Дані сертифіката ""%1"" у довіднику не коректні через:"
" %2'"),
...
"Выберите другой сертификат.';uk='В обраного особистого сертифіката минув термін дії."
...
"Выберите другой сертификат.';uk='В обраного особистого сертифіката не зазначена програма для закритого ключа."
"Виберіть інший сертифікат.'"));
			ПоказатьОшибку(Контекст.ОшибкаНаКлиенте, Контекст.ОшибкаНаСервере);
			ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ложь);
			Возврат;
...
Функция СвойстваСертификатов(Знач Ссылки, Знач ИдентификаторФормы)
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеСертификата = Выборка.ДанныеСертификата.Получить();
		Если ТипЗнч(ДанныеСертификата) <> Тип("ДвоичныеДанные") Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Данные сертификата ""%1"" не найдены в справочнике';uk='Дані сертифіката ""%1"" не знайдені в довіднику'"),
...
Процедура ЗаписатьСертификатыШифрованияНаСервере(ОписаниеОбъектов, АдресСертификатов, ИдентификаторФормы, Ошибка)
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ОтменитьТранзакцию();
		Ошибка.Вставить("ОписаниеОшибки", НСтр("ru='При записи сертификатов шифрования возникла ошибка:';uk='При записі сертифікатів шифрування виникла помилка:'")
