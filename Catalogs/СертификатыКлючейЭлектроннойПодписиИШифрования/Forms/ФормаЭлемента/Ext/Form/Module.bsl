...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого КлючИЗначение Из ПараметрыРеквизитов Цикл
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Поле %1 не заполнено.';uk='Поле %1 не заповнене.'"),
...
Процедура СертификатОтозван(Команда)
	...
	Если Объект.Отозван Тогда
		ПоказатьПредупреждение(, НСтр("ru='После записи отменить отзыв будет невозможно.';uk='Після запису скасувати відкликання буде неможливо.'"));
	КонецЕсли;
