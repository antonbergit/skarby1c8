	...
	Если НЕ Параметры.Свойство("Рекурсия")
		...
		Если НЕ (Параметры.Свойство("НеИспользоватьКлассификатор") И Параметры.НеИспользоватьКлассификатор = Истина) Тогда
			...
			// Дополним стандартный список базовой единицей измерения номенклатуры по классификатору.
				ПредставлениеЕдиницыИзмерения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 (ед. хранения)';uk='%1 (од. зберігання)'"),
			...
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнена номенклатура!';uk='Не заповнена номенклатура!'"));
			КонецЕсли;
		КонецЕсли;
