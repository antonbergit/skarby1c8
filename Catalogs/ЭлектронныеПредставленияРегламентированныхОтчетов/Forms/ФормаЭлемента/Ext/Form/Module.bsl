...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если СвойстваФайлов <> Неопределено Тогда
		...
		Если КонтекстЭДОСервер = Неопределено Тогда 
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Электронное представление не записано:%1.';uk='Электронное представление не записано:%1.'"),
...
Функция ПроверитьФайлыВыгрузкиУведомленияОКонтролируемыхСделках(ФайлыОтчетов)
	...
	Для каждого ФайлОтчета Из ФайлыОтчетов Цикл
		...
		Если РазмерФайла(ВремФайл) > МаксимальныйРазмер Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Размер файла %2 превышает максимально допустимый (%1 Мб). Загрузка файлов будет прервана';uk='Размер файла %2 превышает максимально допустимый (%1 Мб). Загрузка файлов будет прервана'"), 
			...
			Пока ФайлXML.ПрочитатьАтрибут() Цикл
				...
				Если ФайлXML.Имя = "ВерсФорм" Тогда
					...
					Если НоваяСтрокаТаблицы.ВерсФорм <> "5.02" И ФайлыОтчетов.Количество() > 1 Тогда
						...
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Файлы с версией, отличной от 5.02, не могут быть загружены комплектом';uk='Файлы с версией, отличной от 5.02, не могут быть загружены комплектом'"));
						Возврат Ложь;
	...
	Если ТаблицаПроверкиИдФайлИсх.Количество() > 1 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='В файлах различаются значения в полях ИдФайлИсх, ИдФайлПерв. Загрузка файлов будет прервана';uk='В файлах различаются значения в полях ИдФайлИсх, ИдФайлПерв. Загрузка файлов будет прервана'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если КоличествоИзТэгаКолФайл <> КоличествоВыбранныхФайлов Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='В описании к файлам число файлов комплекта не соответствует числу выбранных файлов. Загрузка файлов будет прервана';uk='В описании к файлам число файлов комплекта не соответствует числу выбранных файлов. Загрузка файлов будет прервана'"));
		Возврат Ложь;
	КонецЕсли;
	
	ТаблицаПроверкиКолФайл.Свернуть("КолФайл");
	Если ТаблицаПроверкиКолФайл.Количество() > 1 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='В файлах различается значение в поле КолФайл. Загрузка файлов будет прервана';uk='В файлах различается значение в поле КолФайл. Загрузка файлов будет прервана'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Для сч = 0 По КоличествоФайлов - 1 Цикл
		...
		Если НомерТекущегоФайла <> сч + 1 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не все файлы комплекта выбраны. Загрузка файлов будет прервана';uk='Не все файлы комплекта выбраны. Загрузка файлов будет прервана'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция ПроанализироватьФайлВыгрузкиФСРАР(ОсновнойФайл)
	...
	Если ВидОтправляемогоДокумента = Неопределено Тогда
		РегламентированнаяОтчетностьКлиентСервер.СообщитьОбОшибке(НСтр("ru='Не удалось определить тип файла выгрузки!';uk='Не удалось определить тип файла выгрузки!'"), Ложь);
		Возврат Ложь;
	КонецЕсли;
...
Функция ПроанализироватьФайлВыгрузкиРПН(ОсновнойФайл)
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось определить тип файла выгрузки!';uk='Не удалось определить тип файла выгрузки!'"));
		Возврат Ложь;
	КонецЕсли;
	ПараметрыИмпорта.Вставить("ВидОтчета", ВидОтправляемогоДокумента);
		...
		Для каждого УзелORG_INFO Из СписокУзловORG_INFO Цикл
			...
			Если ЗначениеЗаполнено(ИННВТекущемУзле) Тогда
				...
				ИначеЕсли ИННВПервомУзел <> ИННВТекущемУзле Тогда
					...
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						НСтр("ru='Отправка файлов, содержащих отчетность по нескольким организациям, не поддерживается';uk='Отправка файлов, содержащих отчетность по нескольким организациям, не поддерживается'"));
					Возврат Ложь;
				КонецЕсли;
			КонецЕсли;
	...
	Если Расширение = "xml" Тогда
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Файл ""%1"" не является файлом выгрузки в формате XML.';uk='Файл ""%1"" не является файлом выгрузки в формате XML.'"), КороткоеИмяФайла);
	...
	Иначе
		...
		Для Каждого Стр Из СвойстваФайлов Цикл
			Если Стр.ТипФайлаОтчетности <> Перечисления.ТипыСодержимогоТранспортногоКонтейнера.СведенияПриложениеПФР Тогда
				...
				НовСтр.СсылкаПоказать = НСтр("ru='Открыть';uk='Відкрити'");
			Иначе
			...
			Если Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.НДС")
				...
				Если ОпределитьТипФайлаНДС(ВыбранныйФайл.Имя) = Неопределено
					...
					ТекстСообщения = СтрШаблон(
						НСтр("ru='Файл ""%1"" не может быть отправлен в составе реестра НДС.';uk='Файл ""%1"" не может быть отправлен в составе реестра НДС.'"),
						ВыбранныйФайл.Имя);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ФайлыКомплектаПередНачаломДобавления(Отказ, Копирование, ЭтоДополнительныеФайлы)
	...
	Если Копирование Тогда
		ПоказатьПредупреждение(, НСтр("ru='Копирование запрещено!';uk='Копирование запрещено!'"));
		Возврат;
	КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	ИначеЕсли Форма.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.ПрочаяОтчетностьФНС") Тогда
		...
		Пока ИндексФайла >= 1 Цикл
			СвойстваФайла = Форма.СвойстваФайлов[ИндексФайла];
			ТекстСообщения = СтрШаблон(
				НСтр("ru='Файл ""%1"" не может быть отправлен в составе комплекта.';uk='Файл ""%1"" не может быть отправлен в составе комплекта.'"),
				СвойстваФайла.ИмяФайла);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Функция ПолучитьНаименование(ВидОтчета, НачалоПериода, КонецПериода, Организация) Экспорт
	...
	Если ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.РеестрСведенийФСС")
		...
		ШаблонНаименования = НСтр("ru='%1 (%2)';uk='%1 (%2)'");
	Иначе
		ШаблонНаименования = НСтр("ru='%1 за %3 (%2)';uk='%1 за %3 (%2)'");
	КонецЕсли;
...
Процедура ДополнительныеФайлыПФРДобавитьНаСервере(Номер, ИмяФайла)
	...
	ДополнительныйФайлПФРУдалить.Заголовок = НСтр("ru='Удалить';uk='Видалити'");
...
Процедура ВыгрузитьПакетПослеСохраненияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не Результат Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка при выгрузке пакета';uk='Ошибка при выгрузке пакета'"));
	КонецЕсли;
