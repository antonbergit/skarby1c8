...
Процедура ПроверитьДублиДисконтныхКарт(Форма)
	...
	Если КоличествоЭлементовДублей > 0 Тогда
		...
		Если КоличествоЭлементовДублей = 1 Тогда
			...
			СтруктураПараметровСообщенияОДублях.Вставить("КоличествоЭлементовДублей", НСтр("ru='одна';uk='одна'"));
			СтруктураПараметровСообщенияОДублях.Вставить("Склонение", НСтр("ru='карта';uk='карта'"));
		...
		ИначеЕсли КоличествоЭлементовДублей < 5 Тогда
			...
			СтруктураПараметровСообщенияОДублях.Вставить("Склонение", НСтр("ru='карты';uk='картки'"));
			
		Иначе
			
			СтруктураПараметровСообщенияОДублях.Вставить("КоличествоЭлементовДублей", КоличествоЭлементовДублей);
			СтруктураПараметровСообщенияОДублях.Вставить("Склонение", НСтр("ru='карт';uk='карт'"));
			
		КонецЕсли;	
		
		ТекстНадписиОДублях = НСтр("ru='С таким кодом есть [КоличествоЭлементовДублей] [Склонение]';uk='З таким кодом є [КоличествоЭлементовДублей] [Склонение]'");
	...
	Если ПолученныйТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.МагнитныйКод") Тогда
		...
		Иначе
			...
			ИначеЕсли Результат.Количество() = 0 Тогда
				...
				КонецЕсли;
			Иначе
				ВыбранноеЗначение = Результат.ВыбратьЭлемент(НСтр("ru='Выбор кода магнитной карты';uk='Вибір коду магнітної картки'"));
