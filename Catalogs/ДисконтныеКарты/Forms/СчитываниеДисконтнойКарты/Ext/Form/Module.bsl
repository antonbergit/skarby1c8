...
"(считывателя магнитных карт) или введите код вручную';uk='Зчитайте дисконтну картку за допомогою сканера штрихкода"
"(зчитувача магнітних карток) або введіть код вручну'");
...
"магнитных карт или введите магнитный код вручную';uk='Зчитайте дисконтну картку за допомогою зчитувача"
"магнітних карток або введіть магнітний код вручну'");
...
"штрихкода или введите штрихкод вручную';uk='Зчитайте дисконтну картку за допомогою сканера"
"штрихкода або введіть штрихкод вручну'");
		КонецЕсли;
	Иначе
		Если Не ЗначениеЗаполнено(ОсновнойТипКода) Тогда
			Текст = НСтр("ru='Считайте дисконтную карту при помощи сканера штрихкода"
"(считывателя магнитных карт)';uk='Зчитайте дисконтну картку за допомогою сканера штрихкода"
"(зчитувача магнітних карток)'");
		ИначеЕсли ОсновнойТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.МагнитныйКод") Тогда
			Текст = НСтр("ru='Считайте дисконтную карту при помощи считывателя"
"магнитных карт';uk='Зчитайте дисконтну картку за допомогою зчитувача"
"магнітних карток'");
		ИначеЕсли ОсновнойТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
			Текст = НСтр("ru='Считайте дисконтную карту при помощи сканера"
"штрихкода';uk='Зчитайте дисконтну картку за допомогою сканера"
"штрихкода'");
		КонецЕсли;
	КонецЕсли;
...
Процедура СформироватьЗаголовокФормы()
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.Страницы.ПодчиненныеЭлементы.ГруппаСчитываниеДисконтнойКарты Тогда
		...
		ЭтаФорма.Заголовок = НСтр("ru='Считывание дисконтной карты';uk='Зчитування дисконтної картки'");
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.Страницы.ПодчиненныеЭлементы.ГруппаВыборДисконтнойКарты Тогда
		ЭтаФорма.АвтоЗаголовок = Ложь;
		ТекущиеДанные = Элементы.НайденныеДисконтныеКарты.ТекущиеДанные;
		Если ТекущиеДанные <> Неопределено Тогда
			Если ЗначениеЗаполнено(ТекущиеДанные.Ссылка) Тогда
				ЭтаФорма.Заголовок = НСтр("ru='Выбор дисконтной карты';uk='Вибір дисконтної картки'");;
			Иначе
				ЭтаФорма.Заголовок = НСтр("ru='Выбор вида новой дисконтной карты';uk='Вибір вида нової дисконтної картки'");;
			КонецЕсли;
		Иначе
			Если НайденныеДисконтныеКарты.Количество() > 0 Тогда
				...
				КонецЕсли;
			Иначе
				ЭтаФорма.Заголовок = НСтр("ru='Выбор дисконтной карты \ вида новой дисконтной карты';uk='Вибір дисконтної картки \ вида нової дисконтної картки'");;
			КонецЕсли;
		КонецЕсли;			
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.Страницы.ПодчиненныеЭлементы.ГруппаСозданиеДисконтнойКарты Тогда
...
"Выберите подходящую карту.';uk='Виявлено декілька дисконтних карток із магнітним кодом ""%1""."
"Виберіть підходящу картку.'");
		Иначе
			Текст = НСтр("ru='Обнаружено несколько дисконтных карт со штрихкодом ""%1""."
"Выберите подходящую карту.';uk='Виявлено декілька дисконтних карток зі штрихкодом ""%1""."
"Виберіть підходящу картку.'");
		КонецЕсли;
...
Функция ОбработатьПолученныйКодНаСервере(Данные, ТипКодаКарты, Предобработка, ЕстьНайденныеКарты = Ложь)
	...
	Для Каждого СтрокаТЧ Из ДисконтныеКарты.ЗарегистрированныеДисконтныеКарты Цикл
		...
		НоваяСтрока.Наименование = Строка(СтрокаТЧ.Ссылка) + ?(ЗначениеЗаполнено(СтрокаТЧ.Контрагент) И ЗначениеЗаполнено(СтрокаТЧ.Ссылка), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru=' Клиент: %1';uk=' Клієнт: %1'"), Строка(СтрокаТЧ.Контрагент)), "");
	...
	Если ДисконтныеКарты.ЗарегистрированныеДисконтныеКарты.Количество() = 0 Тогда
		Для Каждого СтрокаТЧ Из ДисконтныеКарты.НеЗарегистрированныеДисконтныеКарты Цикл
			...
			НоваяСтрока.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Зарегистрировать новую карту: %1';uk='Зареєструвати нову картку: %1'"), Строка(СтрокаТЧ.ВидКарты))+?(СтрокаТЧ.ЭтоИменнаяКарта, " (Именная, ", " (")+СтрокаТЧ.ТипКарты+")";
...
Процедура ОбработатьПолученныйКодНаКлиенте(Данные, ПолученныйТипКода, Предобработка)
	...
	Если Не Результат Тогда
		...
		Если ТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
			ТекстСообщения = НСтр("ru='Карта со штрихкодом ""%1"" не зарегистрирована и нет ни одного подходящего вида дисконтных карт.';uk='Карта зі штрихкодом ""%1"" не зареєстрована й немає жодного підходящого виду дисконтних карток.'");
		Иначе
			ТекстСообщения = НСтр("ru='Карта с магнитным кодом ""%1"" не зарегистрирована и нет ни одного подходящего вида дисконтных карт.';uk='Карта з магнітним кодом ""%1"" не зареєстрована й немає жодного підходящого виду дисконтних карток.'");
		КонецЕсли;
...
"Выберите подходящую карту.';uk='Виявлено декілька дисконтних карток із кодом ""%1""."
"Виберіть підходящу картку.'");
			НадписьВыборДисконтнойКарты = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, КодКарты);
		Иначе // Только виды карт для регистрации новой карты.
			Если ТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
				Текст = НСтр("ru='Карта со штрихкодом ""%1"" не зарегистрирована."
"Выберите подходящий вид карты для регистрации новой дисконтной карты.';uk='Картка зі штрихкодом ""%1"" не зареєстрована."
"Виберіть підходящий вид картки для реєстрації нової дисконтної картки.'");
			Иначе
				Текст = НСтр("ru='Карта с магнитным кодом ""%1"" не зарегистрирована."
"Выберите подходящий вид карты для регистрации новой дисконтной карты.';uk='Картка з магнітним кодом ""%1"" не зареєстрована."
"Виберіть підходящий вид картки для реєстрації нової дисконтної картки.'");			   
			КонецЕсли;				   
			НадписьВыборДисконтнойКарты = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, КодКарты);
		КонецЕсли;
