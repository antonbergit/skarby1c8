...
"Каждую категорию разрешено вводить только один раз.';uk='Категорія [%КатегорияНовостей%] повторюється декілька раз."
...
Процедура ПередЗаписью(Отказ)
	...
	Если ЭтотОбъект.ЗагруженоССервера = Истина Тогда
		...
		Если (ЭтотОбъект.ПометкаУдаления = Истина)
			...
			Иначе
				ТекстСообщения = НСтр("ru='Нельзя помечать на удаление ленты новостей, загруженные автоматически с сервера новостей.';uk='Не можна позначати на видалення стрічки новин, завантажені автоматично із сервера новин.'");
				Сообщение = Новый СообщениеПользователю();
