...
Процедура ДекорацияТребуетсяОбновлениеССервераОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ВРег(НавигационнаяСсылка) = ВРег("Update") Тогда
		...
		ПоказатьОповещениеПользователя(
			...
			,
			НСтр("ru='Обновление данных, а также последующая оптимизация загруженных данных завершены';uk='Оновлення даних, а також наступна оптимізація завантажених даних завершені'"));
	...
	Если Запись.Выбран() Тогда
		Если Запись.ТекущаяВерсияНаКлиенте >= Запись.ТекущаяВерсияНаСервере Тогда
			ТекстНадписи = СтрШаблон(
				НСтр("ru='Данные актуальны и соответствуют данным с сервера от %1.';uk='Данные актуальны и соответствуют данным с сервера от %1.'"),
...
"а на сервере уже версия от %2.';uk='Данные никогда не обновлялись с сервера,"
"а на сервере уже версия от %2.'");
			Иначе
				ТекстНадписи = НСтр("ru='Последний раз данные обновлялись с сервера %1,"
"а на сервере уже версия от %2.';uk='Последний раз данные обновлялись с сервера %1,"
"а на сервере уже версия от %2.'");
			КонецЕсли;
			ТекстНадписи = СтрШаблон(
				ТекстНадписи,
			...
			Иначе
				Элементы.ДекорацияТребуетсяОбновлениеССервера.Заголовок   = Новый ФорматированнаяСтрока(
					...
					Новый ФорматированнаяСтрока(
						НСтр("ru='Проверить обновления';uk='Перевірити оновлення'"),
						,
