...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Иначе
			ЭтаФорма.ТолькоПросмотр = Истина;
		КонецЕсли;
		Элементы.ЗаголовокДоступныеКатегорииНовостей.Заголовок = НСтр("ru='Настройки отборов по этой ленте новостей (действуют на все области данных):';uk='Налаштування відборів по цій стрічці новин (діють на всі області даних):'");
	Иначе
		Если (ПравоДоступа("Изменение", Метаданные.Справочники.ЛентыНовостей) = Истина) Тогда
			ЭтаФорма.ТолькоПросмотр = Ложь;
		Иначе
			ЭтаФорма.ТолькоПросмотр = Истина;
		КонецЕсли;
		Элементы.ЗаголовокДоступныеКатегорииНовостей.Заголовок = НСтр("ru='Настройки отборов по этой ленте новостей:';uk='Налаштування відборів по цій стрічці новин:'");
	КонецЕсли;
	...
	Если (Объект.ЗагруженоССервера = Истина) Тогда
		...
		Иначе
			Элементы.ГруппаФайлНовостей.Заголовок = НСтр("ru='Получение файла новостей (эти поля перезаполняются автоматически при изменениях на сервере 1С)';uk='Одержання файлу новин (ці поля перезаповнюються автоматично при змінах на сервері 1С)'");
		КонецЕсли;
...
Процедура ДоступныеКатегорииНовостейПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Объект.ЗагруженоССервера = Истина Тогда
		...
		Сообщение.Текст = НСтр("ru='Нельзя добавлять категории для ленты новостей, загруженной с сервера.';uk='Не можна додавати категорії для стрічки новин, завантаженої із сервера.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура ДоступныеКатегорииНовостейПередУдалением(Элемент, Отказ)
	...
	Если Объект.ЗагруженоССервера = Истина Тогда
		Отказ = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Нельзя удалять категории для ленты новостей, загруженной с сервера.';uk='Не можна видаляти категорії для стрічки новин, завантаженої із сервера.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура КомандаПодборКатегорий(Команда)
	...
	Если Объект.ЗагруженоССервера = Истина Тогда
		Отказ = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Нельзя изменять категории для ленты новостей, загруженной с сервера.';uk='Не можна змінювати категорії для стрічки новин, завантаженої із сервера.'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьИнформационныеСтроки()
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Элементы.ДекорацияВидимостьПоУмолчанию.Заголовок   = НСтр("ru='без исключений. В модели сервиса нельзя настроить видимость ленты новостей по пользователям.';uk='без виключень. У моделі сервісу не можна настроїти видимість стрічки новин по користувачах.'");
		Элементы.ДекорацияВидимостьПоУмолчанию.Гиперссылка = Ложь;
	Иначе
		Элементы.ДекорацияВидимостьПоУмолчанию.Гиперссылка = Истина;
		Если Объект.ИсключенияВидимости.Количество() = 0 Тогда
			Если Объект.ВидимостьПоУмолчанию = Истина Тогда
				Элементы.ДекорацияВидимостьПоУмолчанию.Заголовок = НСтр("ru='без исключений';uk='без виключень'");
			Иначе
				Элементы.ДекорацияВидимостьПоУмолчанию.Заголовок = НСтр("ru='без исключений. Никто не сможет выбрать эту ленту новостей для получения новостей.';uk='без виключень. Ніхто не зможе вибрати цю стрічку новин для одержання новин.'");
		...
		Иначе // > 3
			...
			СписокИсключений = СтрЗаменить(СписокИсключений, "##", ", ");
			СписокИсключений = СтрЗаменить(СписокИсключений, "#", "");
			ТекстСообщения = НСтр("ru='%СписокИсключений% и еще %Объект.ИсключенияВидимости.Количество()-2% пользователей';uk='%СписокИсключений% і ще %Объект.ИсключенияВидимости.Количество()-2% користувачів'");
	...
	Если Объект.Ссылка.Пустая() Тогда
		Элементы.ДекорацияСтатистика.Заголовок = НСтр("ru='Необходимо вначале записать ленту новостей, чтобы получить по ней статистику.';uk='Необхідно спочатку записати стрічку новин, щоб одержати по ній статистику.'");
	Иначе
...
"';uk='и среди них нет ни одной помеченной на удаление"
"'");
...
"';uk='все они помечены на удаление (а значит не видны в списке новостей)"
"'");
				Иначе
					ТекстСтатистики = СтрШаблон(
						НСтр("ru=', из них помечено на удаление %1"
"';uk=', из них помечено на удаление %1"
"'"),
...
"';uk='Нет ни одной загруженной новости"
"'");
			Иначе
				ТекстСтатистики = СтрШаблон(
					НСтр("ru='Всего новостей в этой ленте новостей: %1';uk='Всего новостей в этой ленте новостей: %1'"),
					КоличествоНовостейВсего);
				Если КоличествоНовостейПомеченныхНаУдаление = 0 Тогда
					ТекстСтатистики = ТекстСтатистики + " " + НСтр("ru='и среди них нет ни одной помеченной на удаление"
"';uk='и среди них нет ни одной помеченной на удаление"
...
"'"),
			...
			Если ДатаПоследнейУспешнойЗагрузки = '00010101' Тогда
				...
				Иначе
					ТекстСтатистики = ТекстСтатистики
						+ СтрШаблон(
							НСтр("ru='Новости по этой ленте новостей еще ни разу не были успешно загружены (было %1 неудачных попыток загрузки новостей)';uk='Новости по этой ленте новостей еще ни разу не были успешно загружены (было %1 неудачных попыток загрузки новостей)'"),
			...
			Иначе
				Если КоличествоНеудачПриЗагрузкеНовостей = 0 Тогда
					ТекстСтатистики = ТекстСтатистики
						+ СтрШаблон(
							НСтр("ru='Новости по этой ленте новостей последний раз были успешно загружены %1 (по местному времени)';uk='Новости по этой ленте новостей последний раз были успешно загружены %1 (по местному времени)'"),
							Формат(МестноеВремя(ДатаПоследнейУспешнойЗагрузки), "ДЛФ=DT"));
				Иначе
					ТекстСтатистики = ТекстСтатистики
						+ СтрШаблон(
							НСтр("ru='Новости по этой ленте новостей последний раз были успешно загружены %1 (по местному времени), после этого было %2 неудачных попыток загрузки новостей.';uk='Новости по этой ленте новостей последний раз были успешно загружены %1 (по местному времени), после этого было %2 неудачных попыток загрузки новостей.'"),
							Формат(МестноеВремя(ДатаПоследнейУспешнойЗагрузки), "ДЛФ=DT"),
							КоличествоНеудачПриЗагрузкеНовостей);
				КонецЕсли;
			КонецЕсли;
