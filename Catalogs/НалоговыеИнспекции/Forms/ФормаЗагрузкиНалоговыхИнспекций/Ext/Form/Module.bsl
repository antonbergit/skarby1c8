...
Процедура ФайлДанныхНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ФайлДанных = Элемент.ТекстРедактирования;
	
	Если РасширениеРаботыСФайламиПодключено Тогда
		ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
		ДиалогОткрытияФайла.Фильтр = НСтр("ru='Файл данных (*.xml)|*.xml|Архивный файл данных (*.aml)|*.aml';uk='Файл даних (*.xml)|*.xml|Архивний файл даних (*.aml)|*.aml'");
		ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
		ДиалогОткрытияФайла.Заголовок = НСтр("ru='Выберите xml файл с данными о налоговых инспекциях';uk='Виберіть xml файл із даними про податкові інспекції'");
	    ДиалогОткрытияФайла.Расширение = "xml";
		ДиалогОткрытияФайла.Каталог = ФайлДанных;
		ДиалогОткрытияФайла.ПолноеИмяФайла = ДанныеВыбора;
		ДиалогОткрытияФайла.Показать(Новый ОписаниеОповещения("ФайлДанныхНачалоВыбораЗавершение", ЭтотОбъект, Новый Структура("ДиалогОткрытияФайла", ДиалогОткрытияФайла)));
	Иначе
...
Процедура ЗагрузитьДанныеОНалоговыхИнспекциях()
	...
	Если не ФайлСписокГНИ.Существует() Тогда
		Сообщить(СтрШаблон(НСтр("ru='Файл ¤1¤ не найден!""';uk='Файл ¤1¤ не знайдений!""'"), ФайлДанных));
		фОшибка = 1;
		Возврат;
	КонецЕсли;
...
Процедура ПриОткрытииЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если НЕ РасширениеРаботыСФайламиПодключено Тогда
		Элементы.ФайлДанных.Видимость = Ложь;
		Элементы.НеОбновлять.Видимость = Ложь;
		Элементы.ПредупреждениеЗагрузка.Видимость = Истина;
	Иначе
		Элементы.ФайлДанных.Видимость = Истина;
		Элементы.НеОбновлять.Видимость = Истина;
