...
Процедура ЗаполнитьИнформационныйТекстРезультатаЗаполнения(Форма)
	...
	Если Форма.РучноеРедактирование Тогда
		ИнформационныйТекст = НСтр("ru='График работы на текущий год изменен вручную. Нажмите «Заполнить по шаблону» чтобы вернуться к автоматическому заполнению.';uk='Графік роботи на поточний рік змінений вручну. Натисніть «Заповнити "
"за шаблоном» щоб повернутися до автоматичного заповнення.'");
	...
	Иначе
		Если Форма.РезультатЗаполненПоШаблону Тогда
			Если ЗначениеЗаполнено(Форма.Объект.ПроизводственныйКалендарь) Тогда
				ИнформационныйТекст = НСтр("ru='График работы автоматически обновляется при изменении производственного календаря за текущий год.';uk='Графік роботи автоматично оновлюється при зміні "
"виробничого календаря за поточний рік.'");
...
Процедура ЗаполнитьИнформационныйТекстРучногоРедактирования(Форма)
	...
	Если Форма.РучноеРедактирование Тогда
		ИнформационнаяКартинка = БиблиотекаКартинок.Предупреждение;
		ИнформационныйТекст = НСтр("ru='График работы на текущий год изменен вручную. Изменения выделены в результатах заполнения.';uk='Графік роботи на поточний рік змінений вручну. Зміни виділені "
"в результатах заповнення.'");
	КонецЕсли;
