...
Функция ЗапросКорректный()
	...
	Попытка
		...
		Если ПостроительЗапроса.Измерения.Количество() > 0 Тогда
			...
			Сообщение.Текст = НСтр("ru='Запрос не должен содержать итоги!';uk='Запит не повинен містити підсумки!'");
			Сообщение.Сообщить();
			Возврат Ложь;
		КонецЕсли; 
		...
		Если ПостроительЗапроса.ВыбранныеПоля.Количество() > 1 Тогда
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ru='Запрос должен содержать не более одного показателя!';uk='Запит повинен містити не більш одного показника!'");
			Сообщение.Сообщить();
			Возврат Ложь;
		КонецЕсли;
...
"Измените наименование параметра';uk='Параметр розрахунку з таким найменуванням вже існує!"
"Змініть найменування параметра'");
...
Функция ПроверкаНаВыборПоказателя()
	...
	Сообщение = Новый СообщениеПользователю();
	Сообщение.Текст = НСтр("ru='Не выбран показатель!';uk='Не обраний показник!'");
	Сообщение.Сообщить();
...
"Очистить запрос и вернуться к исходному?';uk='Запит містить помилку!"
"Очистити запит і повернутися до початкового? '");
...
Процедура ЗадаватьЗначениеПриРасчетеЗППриИзменении(Элемент)
	...
	Если Объект.ЗадаватьЗначениеПриРасчетеЗП Тогда
		...
		ТекстСообщения = НСтр("ru='После установки флага все реквизиты будут очищены! Продолжить?';uk='Після установки прапорця всі реквізити будуть очищені! Продовжити?'");
...
Процедура ИнициализироватьЗаполнениеПоШаблону(ИмяШаблона)
	...
	ТекстВопроса = НСтр("ru='Параметр расчета будет полностью перезаполнен! Продолжить выполнение операции?';uk='Параметр розрахунків буде повністю перезаповнений! Продовжити виконання операції?'");
