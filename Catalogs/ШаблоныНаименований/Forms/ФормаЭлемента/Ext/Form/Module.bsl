...
Процедура ДобавитьБлокВШаблон(БлокТекст = Неопределено, ПараметрИдентификатор = Неопределено)
	...
	Если ПараметрИдентификатор <> Неопределено Тогда
		Параметр = "[" + ЗаголовокСвойства(Реквизиты[ПараметрИдентификатор].ИдентификаторПредставление) + "]";
	Иначе
		Параметр = "";
	КонецЕсли;
	...
	Иначе
		#Если ВебКлиент Тогда
			Объект.ШаблонДляПредставления = Текст + ТекстДляВставки;
		#Иначе
...
&НаСервере
Функция ЗаголовокСвойства(Свойство)
	
	Возврат ?(ТипЗнч(Свойство) = Тип("ПланВидовХарактеристикСсылка.ДополнительныеРеквизитыИСведения"), Свойство.Наименование, Свойство);
...
Процедура ОбновитьПример()
	...
	ОбновитьШаблонДляФормирования(Реквизиты, Объект.ШаблонДляПредставления, Объект.ШаблонДляФормирования);
