...
Функция ПроверкаЗаполнения()
	...
	Если НЕ ЗначениеЗаполнено(РольУчастника) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указана роль участника сделки.';uk='Не указана роль участника сделки.'"));
		ЕстьОшибки = Истина;
	КонецЕсли;
	...
	Если ЯвляетсяЮрЛицом Тогда
		...
		Если НЕ ЗначениеЗаполнено(ЮрЛицоНаименование) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указано наименование организации.';uk='Не указано наименование организации.'"));
			ЕстьОшибки = Истина;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ЮрЛицоИНН) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указан ИНН организации.';uk='Не указан ИНН организации.'"));
			ЕстьОшибки = Истина;
		ИначеЕсли СтрДлина(СокрЛП(ЮрЛицоИНН)) <> 10 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Некорректная длина ИНН организации.';uk='Некорректная длина ИНН организации.'"));
			ЕстьОшибки = Истина;
		КонецЕсли;
	...
	Иначе
		...
		Если НЕ ЗначениеЗаполнено(ФизЛицоФамилия) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указана фамилия физического лица.';uk='Не указана фамилия физического лица.'"));
			ЕстьОшибки = Истина;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ФизЛицоИмя) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указано имя физического лица.';uk='Не указано имя физического лица.'"));
			ЕстьОшибки = Истина;
		КонецЕсли;
		
		ИННОбязательно = ОбязательноЗаполнениеИННДляИП(ВидДокумента);
		Если ИННОбязательно И НЕ ЗначениеЗаполнено(ФизЛицоИНН) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указан ИНН физического лица.';uk='Не указан ИНН физического лица.'"));
			ЕстьОшибки = Истина;
		ИначеЕсли ИННОбязательно И СтрДлина(СокрЛП(ФизЛицоИНН)) <> 12 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Некорректная длина ИНН физического лица.';uk='Некорректная длина ИНН физического лица.'"));
			ЕстьОшибки = Истина;
		КонецЕсли;
...
Процедура УправлениеЭУ(Форма)
	...
	Если Форма.ЯвляетсяЮрЛицом Тогда
		...
		КонтрагентПодсказка = "При выборе контрагента Наименование и ИНН заполнятся автоматически";
	Иначе
...
Процедура ЗаполнитьДаннымиКонтрагентаНаСервере()
	Если ЗначениеЗаполнено(Контрагент) Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			ЯвляетсяЮрЛицом = (Выборка.ВидКонтрагента = Перечисления.ВидыКонтрагентов.ЮридическоеЛицо);
