...
Процедура ЗаполнитьВычисляемыеПоляСтрокиТаблицыФайлов(СтрокаТаблицы)
	...
	НСтр("ru='%1 (%2)';uk='%1 (%2)'"), СтрокаТаблицы.ИмяФайла, ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.ТекстовоеПредставлениеРазмераФайла(СтрокаТаблицы.Размер));
...
Процедура ТаблицаФайловПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = НСтр("ru='Удалить выделенные файлы изображений?';uk='Удалить выделенные файлы изображений?'");
...
"Изменения в текущем документе не сохранятся.';uk='Пропустить подготовку текущего документа и перейти к следующему?"
