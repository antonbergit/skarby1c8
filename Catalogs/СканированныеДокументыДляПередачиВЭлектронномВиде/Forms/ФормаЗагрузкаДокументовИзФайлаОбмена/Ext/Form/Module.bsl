...
Процедура Загрузить(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ОрганизацияСсылка) Тогда
		ТекстСообщения = НСтр("ru='Для загрузки необходимо указать организацию.';uk='Для загрузки необходимо указать организацию.'");
	...
	Если СтрокиВыбранныхДокументов.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ru='Для загрузки необходимо выбрать хотя бы один документ.';uk='Для загрузки необходимо выбрать хотя бы один документ.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
...
&НаСервере
Функция ОпределитьОрганизациюПоИНН(ИНН)
	
	КонтекстЭДОСервер = ДокументооборотСКОВызовСервера.ПолучитьОбработкуЭДО();
	Возврат КонтекстЭДОСервер.ОпределитьОрганизациюПоИНН(ИНН);
...
Функция ЗаполнитьТаблицуЗагруженныеДокументы(ДеревоXML)
	...
	ОрганизацияСсылка = ОпределитьОрганизациюПоИНН(ИНН);
	
	Если НЕ ЗначениеЗаполнено(ОрганизацияСсылка) Тогда
		
		Если РегламентированнаяОтчетностьВызовСервера.ИспользуетсяОднаОрганизация() Тогда
			
			РегламентированнаяОтчетностьКлиентСервер.СообщитьПользователю("Пакет обмена предназначен для организации с реквизитами ИНН: " + ИНН + "
		...
		Иначе
			РегламентированнаяОтчетностьКлиентСервер.СообщитьПользователю("В справочнике организаций не обнаружено элемента с реквизитами ИНН: " + ИНН, ИдентификаторФормыВладельца);	
		КонецЕсли;
		
	КонецЕсли;

	ПредставлениеОрганизация = ЗначениеСтрокиУзла(УзелОрганизация, "Наименование") + " " + ИНН;
	...
	Если УзелУчастники <> Неопределено Тогда
		Для каждого УзелУчастник Из МассивСтрокУзла(УзелУчастники, "Участник") Цикл
			...
			Если ЕстьСтрокаУзла(УзелУчастник, "ЮридическоеЛицо") Тогда
				...
				ПредставлениеУчастника = НаименованиеОрганизации + ПрибавляемаяСтрока(ИНН);
...
Функция ПредставлениеУчастникаПоИдентификатору(ИдентификаторУчастника)
	...
	ИНН = СтрокаРеквизитов.ИНН;
	
	Если СтрокаРеквизитов.ЯвляетсяЮрЛицом Тогда
		//это организация
		ПредставлениеУчастника = СтрокаРеквизитов.НаименованиеОрганизации + ПрибавляемаяСтрока(ИНН);
	Иначе
		//это ИП
		ПредставлениеУчастника = СтрокаРеквизитов.ФамилияИП + ПрибавляемаяСтрока(СтрокаРеквизитов.ИмяИП) + ПрибавляемаяСтрока(СтрокаРеквизитов.ОтчествоИП) + ПрибавляемаяСтрока(ИНН);
	КонецЕсли;
