...
Функция ЗагрузитьИзВнешнегоИсточникаПоСтруктуреРезультатаВыбора(СтруктураРезультата) Экспорт
	...
	Исключение
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось извлечь файлы из файла обмена';uk='Не удалось извлечь файлы из файла обмена'"));
	...
	Для каждого СтрокаТЗЗагруженныеДокументы Из ТЗЗагруженныеДокументы Цикл
		...
		Для каждого Участник Из МассивУчастниковДокумента Цикл
			...
			Если СтрокаРеквизитов = Неопределено Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось определить реквизиты одного из участников документа';uk='Не удалось определить реквизиты одного из участников документа'") + ":
		...
		Исключение
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось записать элемент справочника';uk='Не удалось записать элемент справочника'") + ":
			|" + ОписаниеОшибки());
		...
		Для каждого СтрокаТЗФайлыДокументов Из МассивФайловДокумента Цикл
			...
			Если НЕ ЗагружаемыйФайл.Существует() Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='В пакете обмена не обнаружен файл';uk='В пакете обмена не обнаружен файл'") + "
	...
	Пока ВыборкаДокумент.Следующий() Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Электронный документооборот с контролирующими органами. Сканированные документы для передачи в электронном виде';uk='Электронный документооборот с контролирующими органами. Сканированные документы для передачи в электронном виде'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
		...
		Пока ВыборкаДокумент.Следующий() Цикл
			...
			Пока ВыборкаУчастник.Следующий() Цикл
				...
				Если 
					...
					И 
					(ЮрЛицоИНН = ВыборкаУчастник.ЮрЛицоИНН))
					ИЛИ
