...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если ФОУчетАлкогольнойПродукции Тогда
			Объект.ИмпортнаяАлкогольнаяПродукция = ЗначениеЗаполнено(Объект.СтранаПроисхождения) 
				И Объект.СтранаПроисхождения <> Справочники.СтраныМира.Украина;
		КонецЕсли;
	...
	ЭтоДобавлениеКартинкиИзНоменклатуры = Ложь;
	...
	// Гунько 2021-03-23
	Если Объект.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Операция") Тогда
		Если НЕ ЗначениеЗаполнено(Объект.гвТипСклада) Тогда
			Элементы.гвТипСклада.АвтоОтметкаНезаполненного = Истина;
		КонецЕсли;
	Иначе
		Элементы.гвТипСклада.Доступность = Ложь;
	КонецЕсли;
	// ...Гунько 2021-03-23
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_ПрисоединенныйФайл" Тогда
		...
		Если ЭтоДобавлениеКартинкиИзНоменклатуры Тогда
...
Процедура СтранаПроисхожденияПриИзменении(Элемент)
	...
	Если ФОУчетАлкогольнойПродукции Тогда
		Если Объект.СтранаПроисхождения = ПредопределенноеЗначение("Справочник.СтраныМира.Украина")
...
Процедура СтавкаНДСПриИзменении(Элемент)
	Если УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеСтавкиНДС(Объект.СтавкаНДС) <> 0 Тогда
		Объект.ЛьготаНДС = Неопределено;
		Объект.КодЛьготы = Неопределено;
	КонецЕсли;
	УправлениеФормой();
КонецПроцедуры
...
Процедура ИзменитьИзображение(Команда)
	...
	Если ДанныеФайлаДляИзменения = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ru='Изображение не доступно для редактирования';uk='Зображення не доступне для редагування'");
...
Процедура УправлениеФормой()
	...
	Если Объект.ТипНоменклатуры <> ТипНоменклатуры Тогда
		...
		// Гунько 2021-03-23
		Если Объект.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Операция") Тогда
			Элементы.гвТипСклада.Доступность = Истина;
			Если НЕ ЗначениеЗаполнено(Объект.гвТипСклада) Тогда
				Элементы.гвТипСклада.АвтоОтметкаНезаполненного = Истина;
			КонецЕсли;
		Иначе
	...
	Элементы.ГруппаПодакцизныйТовар.Видимость = ?((НЕ ЗначениеЗаполнено(Объект.ТипНоменклатуры))
											ИЛИ Объект.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Запас"), Истина, Ложь);
											
	Элементы.СтатьяДекларацииПоАкцизномуНалогу.Видимость = Объект.ПодакцизныйТовар;
											
	ЭтоНовый = (Объект.Ссылка.Пустая());
	Элементы.НоменклатураГТД.Доступность = НЕ ЭтоНовый;
...
Процедура СозданиеЦеныПоказатьВопрос()
	...
	ТекстВопроса = НСтр("ru='Перед установлением цены необходимо записать объект. Записать?';uk='Перед встановленням ціни необхідно записати об`єкт. Записати?'");
	Ответ = Неопределено;
...
Процедура ДобавитьИзображениеНаКлиентеФрагмент()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ЭтоДобавлениеКартинкиИзНоменклатуры = Истина;
...
Процедура ДобавитьИзображениеНаСервере(Источник)
	...
	Если ТипЗнч(Источник) = Тип("Массив") Тогда
		Для каждого ДобавленныйФайл ИЗ Источник Цикл
	...
	Иначе
		...
		ДвоичныеДанныеКартинки = УправлениеНебольшойФирмойСервер.СсылкаНаДвоичныеДанныеФайла(Источник, УникальныйИдентификатор);
...
Процедура ИзображениеВидимостьКоманднойПанели()
	...
	Если ЗначениеЗаполнено(Объект.ФайлКартинки) Тогда
		Элементы.АдресКартинкиКонтекстноеМенюУстановитьИзображениеОсновным.Видимость = Истина;
	...
	Иначе
		Элементы.АдресКартинкиКонтекстноеМенюУстановитьИзображениеОсновным.Видимость = Ложь;
...
Процедура ЗаполнитьАктуальныеЦены(Данные)
	...
	Для каждого Цена Из Цены Цикл
		...
		Если Цена.ВидЦен.ВалютаЦены <> НациональнаяВалюта Тогда
			Цена.ЦенаВНациональнойВалюте = ПересчитатьПоВыбраннойВалюте(Цена.ВидЦен.ВалютаЦены, ТекущаяДата(), Цена.Цена, НациональнаяВалюта);
		Иначе
...
Процедура ИнициализироватьДиаграммуЦеныПустую()
	...
	Если ЗначениеЗаполнено(ДиаграммаЦеныПериодЗаголовок) Тогда
		ДиаграммаЗаголовок = НСтр("ru='Нет данных за выбранный период.';uk='Немає даних за обраний період.'");
	Иначе
		ДиаграммаЗаголовок = НСтр("ru='Цены не установлены.';uk='Ціни не встановлені.'");
	КонецЕсли;
	...
	Точка1 = ДиаграммаЦены.Точки.Добавить(ТекущаяДата());
	Точка2 = ДиаграммаЦены.Точки.Добавить(ТекущаяДата() - 86400);
	Точка3 = ДиаграммаЦены.Точки.Добавить(ТекущаяДата() - 86400*2);
	Точка4 = ДиаграммаЦены.Точки.Добавить(ТекущаяДата() - 86400*3);
...
Процедура ИнициализироватьДиаграммуПродажиПустую()
	...
	Если ЗначениеЗаполнено(ДиаграммаПродажиПериодЗаголовок) Тогда
		ДиаграммаЗаголовок = НСтр("ru='Нет данных за выбранный период.';uk='Немає даних за обраний період.'");
	Иначе
		ДиаграммаЗаголовок = НСтр("ru='Отсутствуют данные по продажам.';uk='Відсутні дані по продажах.'");
	КонецЕсли;
	...
	Точка1 = ДиаграммаПродажи.Точки.Добавить(ТекущаяДата());
	Точка2 = ДиаграммаПродажи.Точки.Добавить(ТекущаяДата() - 86400);
	Точка3 = ДиаграммаПродажи.Точки.Добавить(ТекущаяДата() - 86400*2);
	Точка4 = ДиаграммаПродажи.Точки.Добавить(ТекущаяДата() - 86400*3);
	...
	Точка1 = ДиаграммаПродажиКоличество.Точки.Добавить(ТекущаяДата());
	Точка2 = ДиаграммаПродажиКоличество.Точки.Добавить(ТекущаяДата() - 86400);
	Точка3 = ДиаграммаПродажиКоличество.Точки.Добавить(ТекущаяДата() - 86400*2);
	Точка4 = ДиаграммаПродажиКоличество.Точки.Добавить(ТекущаяДата() - 86400*3);
