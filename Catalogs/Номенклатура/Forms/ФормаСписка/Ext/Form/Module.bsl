...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ПоказатьВопрос(Оповещение,
		НСтр("ru='В корзине остались несохраненные товары. Продолжить закрытие формы?';uk='У кошику залишилися незбережені товари. Продовжити закриття форми?'"),
		РежимДиалогаВопрос.ОКОтмена,
		0,
		КодВозвратаДиалога.ОК,
		НСтр("ru='Подбор товаров';uk='Підбір товарів'")
	);
...
"Загрузка данных завершена.';uk='Номенклатура."
"Завантаження даних завершене.'"));
...
Процедура ФоновоеЗаданиеВыполнено(Прогресс)
	...
	Исключение
		...
		ВызватьИсключение Нстр("ru='Ошибка';uk='Помилка'") + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура ФоновоеЗаданиеПроверитьНаКлиенте()
	...
	Иначе
		...
		ИначеЕсли ТипЗнч(Прогресс) = Тип("Структура")
			...
			МассивСтрок.Добавить(НСтр("ru='Пожалуйста, подождите...';uk='Будь ласка, зачекайте...'"));
...
Процедура УстановитьОтборЦенСервер(ЦенаОт, ЦенаДо)
	...
	Элементы.ЦеныОтбор.Заголовок = НСтр("ru='Цены';uk='Ціни'")+
		?(ЦенаОт<>0, НСтр("ru=' от ';uk=' від '")+ЦенаОт,"")+
		?(ЦенаДо<>0, НСтр("ru=' до ';uk=' до '")+ЦенаДо,"")
		;	
...
Процедура УстановитьЗаголовокОтборТипыНоменклатуры(ОтборТипНоменклатуры)
	...
	СтрокаТипов = НСтр("ru='Тип:';uk='Тип:'");
...
Процедура ПересчитатьЦеныСуммыКорзиныВВалюту(НоваяВалюта)
	...
	КурсВалюты = УправлениеНебольшойФирмойСервер.ПолучитьКурсыВалют(ВалютаПодбора,НоваяВалюта, ТекущаяДата());
...
Процедура ДобавитьНоменклатуруВКорзину(СтруктураВыбора)
	...
	Если НЕ ИспользоватьЦеныВСписке Тогда
		...
		СтруктураВыбора.Вставить("ДатаОбработки",	ТекущаяДата());
		...
		КонецЕсли;
		СтрокаКорзины.Цена = УправлениеНебольшойФирмойСервер.ПолучитьЦенуНоменклатурыПоВидуЦен(СтруктураВыбора);
	КонецЕсли;
	...
	ПоказатьОповещениеПользователя(НСтр("ru='Подбор товаров';uk='Підбір товарів'")
		,
		,
...
Процедура ПометитьНаУдалениеНоменклатуру()
	...
	ПоказатьОповещениеПользователя(
		СтрШаблон(НСтр("ru='Пометка удаления %1';uk='Позначка видалення %1'"), ?(НовоеЗначениеПометкиУдаления, НСтр("ru='установлена';uk='встановлена'"), НСтр("ru='снята';uk='знята'"))),
...
Процедура УстановитьПометкуУдаленияГруппы(Команда)
	...
	ПоказатьОповещениеПользователя(
		СтрШаблон(НСтр("ru='Пометка удаления %1';uk='Позначка видалення %1'"), ?(ТекущаяСтрокаДерева.ПометкаУдаления, НСтр("ru='установлена';uk='встановлена'"), НСтр("ru='снята';uk='знята'"))),
...
Процедура ОбновитьНадписьПодобраноТоваров() Экспорт
	...
	Если Корзина.Количество()=0 Тогда
		НадписьПодобраноТоваров = НСтр("ru='перетащите товары в корзину';uk='перетягніть товари в кошик'");
	Иначе	
...
Процедура ОбработатьПодготовленныеДанные(РезультатЗагрузки)
	...
	Описание = НСтр("ru='Подсистема ЗагрузкаДанныхИзВнешнегоИсточника: Выполнение серверного метода загрузки результата';uk='Підсистема ЗагрузкаДанныхИзВнешнегоИсточника: Виконання серверного методу завантаження результату'");
...
&НаСервере
Функция ОбработкаПолученияДанныхСписка(Источник, СтрокаТаблицы, Поле)
	
	// Обработчик для программной колонки "ЦенаДляТекущегоПользователя"
	Если Поле.Имя = "ЦенаДляТекущегоПользователя" Тогда
		ТекущийПользователь = Пользователи.ТекущийПользователь();
		Номенклатура = СтрокаТаблицы.Поле("Номенклатура");
		СтрокаТаблицы.Поле(Поле.Имя) = ОбработкаСпециальныхЦен.ПолучитьЦенуДляПользователя(Номенклатура, ТекущийПользователь);
	КонецЕсли;
