...
Процедура ДобавитьСерийныйНомерНаСервере()
	...
	//МаксимальныйНомерИзСправочника = Справочники.СерийныеНомера.ВычислитьМаксимальныйНомерСерии(Объект.Владелец, ШаблонСерийногоНомера);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если СокрЛП(Объект.Наименование)="" Тогда
		...
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru='Серийный номер не заполнен!';uk='Серійний номер не заповнений!'");
		Сообщение.Сообщить();
	КонецЕсли; 
