...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Отбор") И Параметры.Отбор.Свойство("Владелец") Тогда
		...
		Если НЕ ЗначениеЗаполнено(НоменклатураВладелец)
			...
			АвтоЗаголовок = Ложь;
			Заголовок = НСтр("ru='Серийные номера хранятся только для запасов';uk='Серійні номери зберігаються тільки для запасів'");
...
"Установите флаг ""Использовать серийные номера"" в карточке номенклатуры';uk='Для номенклатури не ведеться облік за серійними номерами!"
"Встановіть прапорець ""Використовувати серійні номери"" в картці номенклатури '");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Функция СоздатьСерйныйНомер(СерийныйНомерСтрока, НоменклатураВладелец)
	...
	Попытка
		...
		СтрокаСообщения = НСтр("ru='Создан серийный номер: %1%';uk='Створений серійний номер: %1%'");
...
Функция ПолучитьСерийныйНомерПоШтрихкоду(ДанныеШтрихкода, НоменклатураВладелец)
	...
	Иначе	
		СтрокаСообщения = НСтр("ru='Введенный штрихкод %1% привязан к другой номенклатуре (серийному номеру): %2%';uk='Введений штрихкод %1% прив`язаний до іншої номенклатури (серійному номеру): %2%'");
