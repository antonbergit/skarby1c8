	   ...
	   И ВводДоступен() Тогда
		Если ИмяСобытия = "TracksData" Тогда
			...
			Если Параметр[1][3] = Неопределено
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось распознать ни одного поля. Возможно, поля шаблона настроены неверно.';uk='Не вдалося розпізнати жодного поля. Можливо, поля шаблону налаштовані невірно.'"));
			Иначе
				...
				Иначе
					ТекстСообщения = НСтр("ru='Карта соответствует шаблону и содержит следующие поля:';uk='Карта відповідає шаблону й містить наступні поля:'")+Символы.ПС+Символы.ПС;
					Итератор = 1;
					Для каждого текПоле Из НайденШаблон.ДанныеДорожек Цикл
						...
						Итератор = Итератор + 1;
					КонецЦикла;
					ПоказатьПредупреждение(,ТекстСообщения, , НСтр("ru='Результат расшифровки кода карты';uk='Результат розшифрування коду картки'"));
				КонецЕсли;
			КонецЕсли;
