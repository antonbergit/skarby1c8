...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если СчетчикПолей = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не добавлено ни одного поля ни в одной из доступных дорожек.';uk='Не додане жодного поля в жодній з доступних доріжок.'"), , , , Отказ);
	КонецЕсли;
...
Процедура ВзятьПрефикс1(Команда)
	...
	Иначе
		ОчиститьСообщения();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Выделите мышкой участок кода.';uk='Виділіть мишкою ділянку коду.'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ОпределитьКоординатыПоля(Оповещение, ДанныеДорожки, Префикс, Суффикс, Разделитель, НКол, ККол)
	...
	Если НЕ ПустаяСтрока(Префикс)
		...
		Если НКол < 1 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Выделенный участок кода не должен пересекаться с суффиксом, префиксом или разделителем блоков.';uk='Виділена ділянка коду не повинна перетинатися із суфіксом, префіксом або роздільником блоків.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
				...
				Для каждого текПоле Из Объект["ПоляДорожки"+Строка(ВремИндекс)] Цикл
					Если текПоле.Поле = Поле Тогда
						...
						Если СчетчикДублей > 1 Тогда
							СтрСообщение = НСтр("ru='Дорожка %1, строка %2: Поле должно быть уникальным!';uk='Доріжка %1, рядок %2: Поле повинне бути унікальним!'");
