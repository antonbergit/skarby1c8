...
Функция СформироватьНаименование(НомерДоговора, ДатаДоговора, ВалютаРасчетов)
	...
	ТекстНаименования = НСтр("ru='№ %НомерДоговора% от %ДатаДоговора% (%ВалютаРасчетов%)';uk='№ %НомерДоговора% від %ДатаДоговора% (%ВалютаРасчетов%)'");
...
&НаСервере
Процедура УстановитьВидДоговораПоГК()
	
	Если Объект.ВидДоговора = Перечисления.ВидыДоговоров.Бартерный Тогда
		Объект.ВидДоговораПоГК = Справочники.ВидыДоговоровПоГК.Бартер;
	ИначеЕсли Объект.ВидДоговора = Перечисления.ВидыДоговоров.СКомиссионером
		  ИЛИ Объект.ВидДоговора = Перечисления.ВидыДоговоров.СКомитентом Тогда
...
Процедура УстановитьЗаголовокЦенообразование(Форма)
	...
	Если ЗначениеЗаполнено(Объект.ВидЦен) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ru='Вид цен ';uk='Вид цен '") + Строка(Объект.ВидЦен));
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Объект.ВидСкидкиНаценки) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ru='Вид скидки, наценки ';uk='Вид скидки, наценки '") + Строка(Объект.ВидСкидкиНаценки));
	КонецЕсли;
...
Процедура УстановитьЗаголовокБиллинг(Форма)
	...
	Если Объект.ЭтоДоговорОбслуживания И ЗначениеЗаполнено(Объект.ДоговорОбслуживанияТарифныйПлан) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ru='Тарифный план ';uk='Тарифный план '") + Строка(Объект.ДоговорОбслуживанияТарифныйПлан));
	КонецЕсли;
...
Процедура УстановитьЗаголовокОбменДанными(Форма)
	...
	Если ЗначениеЗаполнено(Объект.ВидДоговора) Тогда
		ДинамическиеПараметры.Добавить(НСтр("ru='договор ';uk='договор '") + Строка(Объект.ВидДоговора));
	КонецЕсли;
...
Процедура ПроверитьВозможностьИзменений(Отказ)
	...
	Если Выборка.Следующий() Тогда
		ТекстСообщения = НСтр("ru='В базе присутствуют эквайринговые терминалы, в которых выбран текущий договор. Изменение валюты расчетов запрещено.';uk='У базі присутні еквайрингові термінали, у яких обраний поточний договір. Зміна валюти розрахунків заборонена.'");
...
Процедура СоздатьНаправлениеДеятельностиВопрос()
	...
	Если Модифицированность Тогда
		ТекстВопроса = НСтр("ru='Перед созданием направления действия необходимо записать объект. Записать?';uk='Перед створенням напрямку дії необхідно записати об`єкт. Записати?'");
...
Процедура ЗаполнитьПолучателейРассылки()
	...
	Если НЕ ЕстьАдресаты Тогда
		
		Элементы.ПолучателиРассылки.Видимость = Ложь;
		Элементы.ДекорацияНетПолучателей.Видимость = Истина;
		
	Иначе
		
		Элементы.ПолучателиРассылки.Видимость = Истина;
		Элементы.ДекорацияНетПолучателей.Видимость = Ложь;
