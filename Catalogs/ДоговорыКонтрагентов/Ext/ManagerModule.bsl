...
Функция ДоговорСоответствуетУсловиямДокумента(ТекстСообщения, Договор, Организация, Контрагент, СписокВидовДоговора) Экспорт
	...
	ТекстСообщения = НСтр("ru='Реквизиты договора не соответствуют условиям документа:';uk='Реквізити договору не відповідають умовам документа:'");
	
	Если НеСоответствуетОрганизация Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru='"
" - Не совпадает организация';uk='"
" - Не збігається організація'");
	КонецЕсли;
	
	Если НеСоответствуетВидДоговора Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru='"
" - Не совпадает вид договора';uk='"
" - Не збігається вид договору'");
	КонецЕсли;
...
Функция ПолучитьСписокВидовДоговораДляДокумента(Документ, ВидОперации = Неопределено, ИмяТабличнойЧасти = "") Экспорт
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.НалоговаяНакладная")
		ИЛИ ТипЗнч(Документ) = Тип("ДокументСсылка.Приложение1КНалоговойНакладной")
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='Форма договора (HTML)';uk='Форма договору (HTML)'");
	...
	КомандаПечати.Представление = НСтр("ru='Форма договора (Microsoft word)';uk='Форма договору (Microsoft word)'");
	КомандаПечати.СписокФорм = "";
	КомандаПечати.ФункциональныеОпции = "ДоговорыКонтрагентовТехническиеКомандыПечати";
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ТарифныйПланДоговораОбслуживания";
	КомандаПечати.Представление = НСтр("ru='Тарифный план';uk='Тарифний план'");
	КомандаПечати.СписокФорм = "ФормаЭлемента,ФормаСписка,ФормаВыбора,ФормаВыбораСКонтрагентом";
...
Функция ПечатьТарифныйПланДоговораОбслуживания(МассивОбъектов, ОбъектыПечати, ИмяМакета) Экспорт
	...
	Для каждого Эл Из МассивОбъектов Цикл
		...
		ТекстСообщения = СтрШаблон(
			НСтр("ru='Договор %1 не является договором обслуживания!';uk='Договір %1 не є договіром обслуговування'"),
			Эл.Ссылка
		);
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбластьМакета.Параметры.ТекстЗаголовка = НСтр("ru='к договору № ';uk='до договору № '")
												+ Выборка.НомерДоговора
												+ НСтр("ru=' от ';uk=' від '")
												+ Формат(Выборка.ДатаДоговора, "Л="+КодЯзыкаПечать+"; ДЛФ=DD");
