...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.НастройкаУчетнойЗаписиЗаголовок.Заголовок = ?(КонтекстныйРежим,
		НСтр("ru='Для отправки писем необходимо настроить учетную запись электронной почты';uk='Для відправки листів необхідно настроїти обліковий запис "
"електронної пошти'"),
	...
	Иначе
		Заголовок = НСтр("ru='Настройка учетной записи электронной почты';uk='Налаштування облікового запису електронної пошти'");
	КонецЕсли;
...
// УНФ
&НаКлиенте
Процедура ПодключитьGmail(Команда)
...
Процедура ПоказатьВопросПередЗакрытиемФормы()
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Введенные данные не записаны. Закрыть форму?';uk='Введені дані не записані. Закрити форму?'"), Кнопки,,
		КодВозвратаДиалога.Отмена, НСтр("ru='Настройка учетной записи';uk='Налаштування облікового запису'"));
КонецПроцедуры
...
Процедура НастроитьЭлементыТекущейСтраницы()
	...
	Иначе
		...
		ИначеЕсли ТекущаяСтраница = Элементы.НастройкаУчетнойЗаписи
			...
			Если КонтекстныйРежим Тогда
				ЗаголовокКнопкиДалее = НСтр("ru='Настроить';uk='Налаштувати'");
			Иначе
...
"%1 завершена.';uk='Налаштування параметрів облікового запису"
"%1 завершене.'"), АдресЭлектроннойПочты);
	КонецЕсли;
...
"Настройте параметры подключения вручную.';uk='Не вдалося визначити налаштування з`єднання. "
"Налаштуйте параметри підключення вручну.'");
...
"http://buh.ru/articles/documents/42429/#briefly_43166';uk='Див. також рекомендації по налаштуванню пошти Gmail:"
...
Процедура ПодобратьНастройкиУчетнойЗаписи()
	...
	НаименованиеЗадания = НСтр("ru='Поиск настроек почтового сервера';uk='Пошук налаштувань поштового серверу'");
...
Процедура ОбработатьРезультатЗапросаТокена(Результат, Параметры) Экспорт
	...
	УчетнаяЗаписьСсылка = ДобавитьУчетнуюЗаписьGoogleНаСервере(СеансовыеДанные);
