...
Процедура ОпределитьНастройкиУчетнойЗаписи(Параметры, АдресРезультата) Экспорт
	...
	Если Параметры.ДляОтправки Тогда
		НайденныйПрофильSMTP = ОпределитьНастройкиSMTP(
		Параметры.АдресЭлектроннойПочты,
		ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ПарольSMTP", Параметры.Пароль));
	КонецЕсли;
	
	Если Параметры.ДляПолучения Тогда
		НайденныйПрофильIMAP = ОпределитьНастройкиIMAP(Параметры.АдресЭлектроннойПочты, Параметры.Пароль);
	КонецЕсли;
	...
	Результат.Вставить("ДляПолучения", НайденныйПрофильIMAP <> Неопределено);
...
Функция ПроверитьПодключениеКСерверуИсходящейПочты(Профиль, АдресЭлектроннойПочты)
	...
	ИмяОтправителяПисем = НСтр("ru='1С:Предприятие';uk='1С:Підприємство'");
...
Функция ВариантыНастройкиПодключенияКСерверуIMAP(АдресЭлектроннойПочты)
	...
	// УНФ
	Для Каждого АвтоВариант Из ЭлектроннаяПочтаУНФ.ВариантыНастройкиПодключенияКСерверуIMAP(ИмяСервераВУчетнойЗаписи) Цикл
		ЗаполнитьЗначенияСвойств(Результат.Добавить(), АвтоВариант);
	КонецЦикла;
	// Конец УНФ
	...
	// Имя сервера с префиксом "imap-mail.", защищенное соединение.
	ВариантНастройки = Результат.Добавить();
	ВариантНастройки.СерверВходящейПочты = "imap-mail." + ИмяСервераВУчетнойЗаписи;
	ВариантНастройки.ПортСервераВходящейПочты = 993;
	ВариантНастройки.ИспользоватьЗащищенноеСоединениеДляВходящейПочты = Истина;
...
Функция ВариантыНастройкиПодключенияКСерверуSMTP(АдресЭлектроннойПочты)
	...
	// УНФ
	Для Каждого АвтоВариант Из ЭлектроннаяПочтаУНФ.ВариантыНастройкиПодключенияКСерверуSMTP(ИмяСервераВУчетнойЗаписи) Цикл
		ЗаполнитьЗначенияСвойств(Результат.Добавить(), АвтоВариант);
	КонецЦикла;
	// Конец УНФ
	...
	// Имя сервера с префиксом "smtp-mail.", защищенное (STARTTLS) соединение, порт 587.
	ВариантНастройки = Результат.Добавить();
	ВариантНастройки.СерверИсходящейПочты = "smtp-mail." + ИмяСервераВУчетнойЗаписи;
	ВариантНастройки.ПортСервераИсходящейПочты = 587;
	ВариантНастройки.ИспользоватьЗащищенноеСоединениеДляИсходящейПочты = Истина;
