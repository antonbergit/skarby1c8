...
Процедура Подключаемый_ЮрАдресНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Результат = УправлениеКонтактнойИнформациейКлиент.НачалоВыбора(ЭтаФорма, Элементы[ПредставлениеЮрАдреса], , СтандартнаяОбработка);
...
Функция НеЗаполненЮрАдрес()
	...
	Для Каждого Элемент Из ЭтаФорма.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов Цикл
		
		Если Элемент.Вид = Справочники.ВидыКонтактнойИнформации.ЮрАдресОрганизации Тогда
			
			Возврат Не ЗначениеЗаполнено(ЭтаФорма[Элемент.ИмяРеквизита]);
...
Функция ВидимостьГруппыРеквизитыОрганизации(ИменаРеквизитовДляЗаполнения)
	...
	Если ИменаРеквизитовДляЗаполнения.Найти("ИНН") = Неопределено
		И ИменаРеквизитовДляЗаполнения.Найти("КодПоЕДРПОУ") = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура НастроитьЭлементыФормы(ИменаРеквизитовДляЗаполнения)
	...
	Элементы.КодПоЕДРПОУ.Видимость = Объект.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо;
...
Процедура ДобавитьИзображение(ИмяРеквизитаСКартинкой)
	...
	НСтр("ru='Для выбора изображения необходимо записать объект. Записать?';uk='Для вибору зображення необхідно записати об`єкт. Записати?'"),
...
Функция НомерСчетаУказанНекорректно()
	...
	НСтр("ru='Требуется указать строго 20 цифр.';uk='Потрібно вказати строго 20 цифр.'"),,
	"НомерСчета");
