...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОтказИзменитьСчетЗатрат(ВидНачисленияУдержания) Тогда
		...
		Элементы.ГруппаСчетовУчета.Подсказка	= НСтр("ru='В базе есть движения по этому виду начисления (удержания)! Изменение счета учета запрещено!';uk='У базі є рухи по цьому виду нарахування (утримання)! Зміна рахунку обліку заборонена!'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтоНалог Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Для типа вида начисления Налог счета учета не редактируются!';uk='Для типу виду нарахування Податок рахунку обліку не редагуються!'"));
		Отказ = Истина;
