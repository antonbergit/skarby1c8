...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Назначение.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
			"Объект.Назначение",
			НСтр("ru='Не указан вид пользователей.';uk='Не зазначений вид користувачів.'"),
			"");
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ)
	...
	Иначе
		Если Родитель = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
			ВызватьИсключение
				НСтр("ru='Невозможно добавить подгруппу к предопределенной группе ""Все внешние пользователи"".';uk='Неможливо додати підгрупу до визначеної групи ""Всі зовнішні користувачі"".'");
			
		ИначеЕсли Родитель.ВсеОбъектыАвторизации Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Невозможно добавить подгруппу к группе ""%1"", "
"так как в число ее участников входят все пользователи.';uk='Неможливо додати підгрупу до групи ""%1"", "
"тому що в число її учасників входять усі користувачі.'"), Родитель);
		КонецЕсли;
		...
		Если ВсеОбъектыАвторизации Тогда
			...
			Если ОбщегоНазначенияКлиентСервер.СпискиЗначенийИдентичны(НазначениеВсеВнешниеПользователи, МассивНазначения) Тогда
				ВызватьИсключение
					НСтр("ru='Невозможно создать группу, совпадающую по назначению с предопределенной группой ""Все внешние пользователи"".';uk='Неможливо створити групу, що збігається по призначенню з визначеною групою ""Всі зовнішні користувачі"".'");
			КонецЕсли;
...
"в число участников которой входят все пользователи указанных видов.';uk='Вже існує група ""%1"","
"у число учасників якої входять всі користувачі зазначених видів.'"),
...
"группы внешних пользователей ""%1"".';uk='Вид учасників групи повинен бути як у вищої"
...
"так как у нее имеются подгруппы.';uk='Неможливо змінити вид учасників групи ""%1"","
"тому що в неї є підгрупи.'"),
					Наименование);
			КонецЕсли;
		...
		Если ЗначениеЗаполнено(Ссылка) Тогда
			...
			Если НЕ РезультатЗапроса.Пустой() Тогда
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Невозможно изменить вид участников группы ""%1"","
"так как у нее имеется подгруппа ""%2"" с другим назначением участников.';uk='Неможливо змінити вид учасників групи ""%1"","
"тому що в неї є підгрупа ""%2"" з іншим призначенням учасників.'"),
					Наименование,
					Выборка.СсылкаПредставление);
			КонецЕсли;
		КонецЕсли;
