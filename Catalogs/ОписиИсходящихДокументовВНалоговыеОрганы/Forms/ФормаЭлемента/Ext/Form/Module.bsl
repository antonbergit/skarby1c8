...
"Продолжить?';uk='Для продолжения необходимо сохранить введеную информацию."
"Продолжить?'");
	...
	Если НЕ РезультатЗаписи Тогда
		ПоказатьПредупреждение(, НСтр("ru='Во время сохранения произошли ошибки! Продолжение невозможно.';uk='Во время сохранения произошли ошибки! Продолжение невозможно.'"));
		Возврат;
	КонецЕсли;

	ТекстВопроса = НСтр("ru='Текущий ответ на требование будет разбит на несколько. Продолжить?';uk='Текущий ответ на требование будет разбит на несколько. Продолжить?'");
...
Процедура ДобавитьДокументы(Команда)
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнено поле ""Требование""';uk='Не заполнено поле ""Требование""'"), , "Объект.Требование");
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьДокументыНДСЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		Если ВидДокумента = Неопределено  Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Выбор отменен. Не удалось определить вид выбранного документа.';uk='Выбор отменен. Не удалось определить вид выбранного документа.'"));
			Возврат;
		КонецЕсли;
...
Функция ОтработатьРезультатДобавленияДокументовФайлаОбменаНаСервере(ПунктТребования, СтруктураВыбора)
	...
	Если НЕ ЗагрузитьФайлыИзФайлаОбменаВИБ(Объект.Ссылка, ПолноеИмяФайлаОбмена, МассивИменНовыхФайлов) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Произошла ошибка при загрузке документов из файла обмена.';uk='Произошла ошибка при загрузке документов из файла обмена.'"));
		Возврат Ложь;
	КонецЕсли; 
	...
	Для каждого СтруктураСвойств Из МассивРезультат Цикл
		...
		Если ТипЗнч(СсылкаНаОбъект) <> ТипСкан Тогда
			...
			Если ВидДокумента = Перечисления.ВидыПредставляемыхДокументов.КнигаПокупок 
				...
				ИЛИ ВидДокумента = Перечисления.ВидыПредставляемыхДокументов.ДополнительныйЛистКнигиПродаж Тогда
				
				МассивНДС.Добавить(СсылкаНаОбъект);
				
			ИначеЕсли ВидДокумента = Перечисления.ВидыПредставляемыхДокументов.АктПриемкиСдачиРабот
		...
		Для каждого ЭлементГруппы Из ГруппаДокументов.ПолучитьЭлементы() Цикл
			...
			Если ЭлементГруппы.КоличествоФайлов = 0 Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='У документа %1 отсутствуют прикрепленные файлы.';uk='У документа %1 отсутствуют прикрепленные файлы.'"), ЭлементГруппы.СоставноеПоле);
