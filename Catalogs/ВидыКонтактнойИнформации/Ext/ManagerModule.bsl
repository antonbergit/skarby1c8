...
Процедура ОбновитьВидПоляДругое(Параметры) Экспорт
	...
	Пока ВидКонтактнойИнформацииСсылка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось обработать вид контактной информации: %1 по причине: %2';uk='Не вдалося обробити вид контактної інформації : %1 з причини: %2'"),
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Процедуре ОбновитьВидПоляДругое не удалось обработать некоторые виды контактной информации (пропущены): %1';uk='Процедурі ОбновитьВидПоляДругое не вдалося обробити деякі види контактної інформації (пропущені): %1'"), 
				ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			Метаданные.Справочники.ВидыКонтактнойИнформации,,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Процедура ОбновитьВидПоляДругое обработала очередную порцию видов контактной информации: %1';uk='Процедура ОбновитьВидПоляДругое обробила чергову порцію видів контактної інформації: %1'"),
					ОбъектовОбработано));
	КонецЕсли;
