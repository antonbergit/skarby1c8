...
Функция СформироватьАвтоНаименованиеНаКлиенте()
	...
	Если Объект.СпособПредоставления = СпособПредоставленияПроцент Тогда
		...
		СтрокаНаименования = "" + Объект.ЗначениеСкидкиНаценки + НСтр("ru='%';uk='%'");
	...
	Если Объект.НоменклатураГруппыЦеновыеГруппы.Количество() > 0 И НЕ СуммаВДокументе Тогда
		СтрокаНаименования = СтрокаНаименования + НСтр("ru=', с уточнением';uk=', з уточненням'");
		Элементы.Наименование.СписокВыбора.Добавить(СтрокаНаименования);
	КонецЕсли;
	...
	Если (Объект.ПолучателиСкидкиКонтрагенты.Количество() > 0 И Объект.Назначение <> НазначениеРозница) 
		...
		СтрокаНаименования = СтрокаНаименования + НСтр("ru=', указаны получатели';uk=', зазначені одержувачі'");
		Элементы.Наименование.СписокВыбора.Добавить(СтрокаНаименования);
	КонецЕсли;
	
	Если Объект.ВремяПоДнямНедели.Количество() > 0 Тогда
		СтрокаНаименования = СтрокаНаименования + НСтр("ru=', по расписанию';uk=', за розкладом'");
		Элементы.Наименование.СписокВыбора.Добавить(СтрокаНаименования);
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.ДатаНачала > Объект.ДатаОкончания Тогда
		...
		Сообщение.Текст = НСтр("ru='Дата начала больше даты окончания!';uk='Дата початку більше дати закінчення!'");
...
Процедура УправлениеВидимостьюНаСервере()
	...
	Если Объект.ВариантОграниченияПоНоменклатуре = ВариантОграниченияПоНоменклатуре Тогда
		...
		Элементы.ДекорацияПодсказкаУточнения.Заголовок = НСтр("ru='Заполните уточнения, если требуется, чтобы у определённых товаров или групп товаров значение скидки отличалось от основного значения. Если список не заполнен, то для всех позиций номенклатуры используется основное значение скидки.';uk='Заповніть уточнення, якщо потрібно, щоб у певних товарів або груп товарів значення знижки відрізнялося від основного значення. Якщо список не заповнений, то для всіх позицій номенклатури використовується основне значення знижки.'");
	...
	ИначеЕсли Объект.ВариантОграниченияПоНоменклатуре = ВариантОграниченияПоКатегориям Тогда
		...
		Элементы.НоменклатураГруппыЦеновыеГруппыЗначениеУточнения.Заголовок = НСтр("ru='Категория номенклатуры';uk='Категорія номенклатури'");
		
		Элементы.ДекорацияПодсказкаУточнения.Заголовок = НСтр("ru='Заполните уточнения, если требуется, чтобы у товаров определённых категорий значение скидки отличалось от основного значения. Если список не заполнен, то для всех категорий используется основное значение скидки.';uk='Заповніть уточнення, якщо потрібно, щоб у товарів певних категорій значення знижки відрізнялося від основного значення. Якщо список не заповнений, то для всіх категорій використовується основне значення знижки.'");
	...
	ИначеЕсли Объект.ВариантОграниченияПоНоменклатуре = ВариантОграниченияПоЦеновымГруппам Тогда
		Элементы.НоменклатураГруппыЦеновыеГруппыЗначениеУточнения.Заголовок = НСтр("ru='Ценовая группа';uk='Цінова група'");
		...
		Элементы.ДекорацияПодсказкаУточнения.Заголовок = НСтр("ru='Заполните уточнения, если требуется, чтобы у товаров определённых ценовых групп значение скидки отличалось от основного значения. Если список не заполнен, то для всех ценовых групп используется основное значение скидки.';uk='Заповніть уточнення, якщо потрібно, щоб у товарів певних цінових груп значення знижки відрізнялося від основного значення. Якщо список не заповнений, то для всіх цінових груп використовується основне значення знижки.'");
...
Процедура ЗначениеСкидкиНаценкиПриИзменении(Элемент)
	...
	Если Объект.СпособПредоставления = СпособПредоставленияПроцент Тогда
		Если Объект.ЗначениеСкидкиНаценки > 100 Тогда
			ТекстСообщения = НСтр("ru='Процент скидки должен быть не более 100%';uk='Відсоток знижки повинен бути не більш 100%'");
...
Процедура НастройкаВидимостиПолучателейНаСервере()
	...
	Если Объект.Назначение = НазначениеРозница Тогда
		...
		Элементы.ДекорацияПодсказкаПолучатели.Заголовок = НСтр("ru='Заполните список складов, если скидка (наценка) будет предоставляться только на определённых складах. ';uk='Заповніть список складів, якщо знижка (націнка) буде надаватися тільки на певних складах. '")+
		НСтр("ru='Если список не заполнен, то скидка (наценка) будет предоставляться на всех складах.';uk='Якщо список не заповнений, то знижка (націнка) буде надаватися на всіх складах.'");
	ИначеЕсли Объект.Назначение = НазначениеОпт Тогда
		...
		Элементы.ДекорацияПодсказкаПолучатели.Заголовок = НСтр("ru='Заполните список контрагентов, если скидка (наценка) будет предоставляться только определённому списку контрагентов. ';uk='Заповніть список контрагентів, якщо знижка (націнка) буде надаватися тільки певному списку контрагентів. '")+
		НСтр("ru='Если список не заполнен, то скидка (наценка) будет предоставляться всем контрагентам.';uk='Якщо список не заповнений , то знижка (націнка) надаватиметься всім контрагентам.'");
	Иначе
		Элементы.ГруппаКонтрагенты.Видимость = Истина;
		Элементы.ГруппаСклады.Видимость = Истина;
		Элементы.ГруппаСкладыИКонтрагенты.ОтображениеСтраниц = ОтображениеСтраницФормы.ЗакладкиСверху;
		
		Элементы.ДекорацияПодсказкаПолучатели.Заголовок = НСтр("ru='Заполните список контрагентов или складов, если скидка (наценка) будет предоставляться только определённому списку контрагентов (опт) ';uk='Заповніть список контрагентів або складів, якщо знижка (націнка) буде надаватися тільки певному списку контрагентів (опт) '")+
		НСтр("ru='или на определённых складах (розница). Если список контрагентов (складов) не заполнен, то скидка (наценка) будет предоставляться всем контрагентам (на всех складах).';uk='або на певних складах (роздріб). Якщо список контрагентів (складів) не заповнений, то знижка (націнка) надаватиметься всім контрагентам (на всіх складах).'");
	КонецЕсли;
