...
"Можно использовать символы подстановки * и ?';uk='Полнотекстовый поиск включен."
"Можно использовать символы подстановки * и ?'");
	Иначе
		Элементы.СтрокаПоиска.Подсказка = НСтр("ru='Полнотекстовый поиск отключен."
"Доступен поиск только точных фраз';uk='Полнотекстовый поиск отключен."
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПустаяСтрока(ЭтаФорма.СтрокаПоиска) Тогда
		...
		Сообщение.Текст = НСтр("ru='Строка поиска пустая';uk='Рядок пошуку порожній'");
	...
	Если (ЭтаФорма.ПоискДатаОТ <> '00010101') И (ЭтаФорма.ПоискДатаДО <> '00010101') Тогда
		Если (ЭтаФорма.ПоискДатаОТ > ЭтаФорма.ПоискДатаДО) Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Неверно задан интервал поиска по датам: дата начала позже даты окончания';uk='Невірно заданий інтервал пошуку по датах: дата початку пізніше дати закінчення'");
