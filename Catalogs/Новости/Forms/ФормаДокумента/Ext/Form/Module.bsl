...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстСообщения = СтрШаблон(
		НСтр("ru='(все время указано для часового пояса %1 (%2))';uk='(все время указано для часового пояса %1 (%2))'"),
		ЧасовойПояс(),
	...
	Если (ЭтотОбъект.БезАвторизации = Истина)
		...
		Элементы.ДекорацияВозможностьИзмененияНовости.Заголовок =
			НСтр("ru='Для редактирования этой новости необходимо авторизоваться в новостном центре.';uk='Для редагування цієї новини необхідно авторизуватися в новинному центрі.'");
	...
	ИначеЕсли (ЭтотОбъект.БезАвторизации = Ложь)
		...
		Элементы.ДекорацияВозможностьИзмененияНовости.Заголовок =
			НСтр("ru='Новость была успешно опубликована. Так как новость была опубликована в каналах, по которым запрещена перепубликация (емейл, смс), то изменить ее уже нельзя, но можно скопировать эту новость и править уже ее.';uk='Новина була успішно опублікована. Тому що новина була опублікована в каналах, по яким заборонена перепублікація (емейл, смс), тому змінити її вже не можна, але можна скопіювати цю новину й правити вже її.'");
		Элементы.ДекорацияВозможностьИзмененияНовости.Видимость = Истина;
		ЭтотОбъект.ТолькоПросмотр = Истина;
	КонецЕсли;
...
"или роль РедактированиеНовостей (вместе с настройкой лент новостей, в которых можно редактировать новости).';uk='Немає прав на редагування новин."
"Повинні бути включені ролі АдминистраторСистемы й ПолныеПрава (тоді можна редагувати всі локальні стрічки новин)"
"або роль РедактированиеНовостей (разом з налаштуванням стрічок новин, у яких можна редагувати новини).'");
...
"Невозможно отредактировать новость.';uk='Не знайдене ні однієї дозволеної для редагування стрічки новин."
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для С=0 По Объект.Действия.Количество()-1 Цикл
		...
		Для ТекущийСимвол=4 По СтрДлина(ТекущаяСтрока.УИНДействия) Цикл
			...
			Если (Найти(РазрешенныеСимволы, Символ) = 0) Тогда
				Отказ = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Идентификатор действия должен начинаться с 1C: и состоять только из английских символов, цифр и знака подчеркивания';uk='Ідентифікатор дії повинен починатися з 1C: і полягати тільки з англійських символів, цифр і знаків підкреслення'");
				Сообщение.Поле  = "Объект.Действия[" + С + "].УИНДействия";
				Сообщение.Сообщить();
			КонецЕсли;
		КонецЦикла;
		...
		Если ПустаяСтрока(ТекущаяСтрока.Действие) Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Пустое действие';uk='Порожня дія'");
		...
		Иначе
			...
			Иначе
				Отказ = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Неправильное действие. Выберите значение действия из списка';uk='Неправильна дія. Виберіть значення дії зі списку'");
				Сообщение.Поле  = "Объект.Действия[" + С + "].Действие";
				Сообщение.Сообщить();
			КонецЕсли;
		КонецЕсли;
	...
	Для С=0 По Объект.ПараметрыДействий.Количество()-1 Цикл
		...
		Если НайденныеВладельцы.Количество() = 0 Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Параметр не привязан ни к одному действию (должно быть хотя бы одно действие с таким же идентификатором [%УИНДействия%]). Рекомендуется удалить строку.';uk='Параметр не прив`язаний до жодної дії (повинне бути хоча б одна дія з таким же ідентифікатором [%УИНДействия%]). Рекомендується видалити рядок.'");
			Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%УИНДействия%", ТекущаяСтрока.УИНДействия);
		...
		Иначе // =1
			...
			Если ТекущийВладелец.Действие = "Переход по навигационной ссылке" Тогда
				...
				Иначе
					Отказ = Истина;
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = НСтр("ru='У действия [%Действие%] параметр [%Параметр%] неправильный. Выберите значение из списка';uk='У дії [%Действие%] параметр [%Параметр%] неправильний. Виберіть значення зі списку'");
...
Процедура ПримерНовостиДля1СПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если ПродолжитьОбработку = Истина Тогда
		...
		Иначе
			СтандартнаяОбработка = Ложь;
			ТекстСообщения = НСтр("ru='Неизвестная ссылка: %НавигационнаяСсылка%';uk='Невідоме посилання: %НавигационнаяСсылка%'");
	...
	Если ОтменаРедактирования <> Истина Тогда
		...
		Если ТекущиеДанные <> Неопределено Тогда
			...
			Если ТекущиеДанные.ВерсияОТ > ТекущиеДанные.ВерсияДО Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Версия ОТ [ВерсияОТ] не должна быть больше, чем Версия ДО [ВерсияДО].';uk='Версія ОТ [ВерсияОТ] не повинна бути більше, ніж Версія ДО [ВерсияДО].'");
...
""
"А в поле Действие должно быть значение, выбранное из списка.';uk='Ідентифікатор гіперпосилання повинен починатися з 1C: (де C - англійська буква),"
"і складатися тільки з англійських букв, цифр і знаків підкреслення."
""
"А в поле Дія повинне бути значення, обране зі списку.'");
			ПоказатьПредупреждение(
				, // ОписаниеоповещенияОЗавершении
...
Процедура КомандаБуферОбмена_Копировать(Команда)
	...
	Если ВРег(Команда.Имя) = ВРег("КомандаБуферОбмена_Копировать_КатегорииПростые")
		...
		Если ЭлементФормы.ВыделенныеСтроки.Количество() > 0 Тогда
			...
			КонецЦикла;
			ТекстСообщения = НСтр("ru='Скопировано строк: %ТекущийБуферОбмена.Количество()%';uk='Скопійоване рядків: %ТекущийБуферОбмена.Количество()%'");
	...
	Если ВРег(Команда.Имя) = ВРег("КомандаБуферОбмена_Вставить_ТекущийБуферОбмена_КатегорииПростые")
		...
		Если ДанныеБуфераОбмена.Количество() > 0 Тогда
			Для каждого ТекущаяСкопированнаяСтрока Из ДанныеБуфераОбмена Цикл
				...
				КонецЕсли;
			КонецЦикла;
			ТекстСообщения = НСтр("ru='Вставлено строк: %ДанныеБуфераОбмена.Количество()%';uk='Вставлене рядків: %ДанныеБуфераОбмена.Количество()%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДанныеБуфераОбмена.Количество()%", ДанныеБуфераОбмена.Количество());
			ПоказатьОповещениеПользователя(ТекстСообщения, , , БиблиотекаКартинок.Информация);
		Иначе
			ПоказатьОповещениеПользователя(НСтр("ru='Нет данных в буфере обмена.';uk='Немає даних у буфері обміну.'"), , НСтр("ru='Вначале скопируйте строки.';uk='Спочатку скопіюйте рядки.'"), БиблиотекаКартинок.Внимание);
		КонецЕсли;
...
"Для проверки новости необходимо открыть форму вручную или исправить значения в колонках.';uk='Форма %ИмяФормыПроверкиНовости% не обнаружена."
"Возможно, для описания формы использованы не имена в метаданных, а другой текст,"
"	либо неправильно введены идентификаторы метаданных и / или формы."
...
"%Предупреждение%';uk='Розмір всіх бінарних даних: %РазмерБинарныхДанных% байт"
"Приблизний розмір новини (включаючи текст новини): %ОбщийРазмер% байт"
"%Попередження%'");
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц()
	...
	Иначе
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не удалось загрузить бинарные данные (не совпадает количество строк в форме и в объекте). При сохранении они будут утеряны. Рекомендуется закрыть / открыть форму новости без сохранения';uk='Не вдалося завантажити бінарні дані (не збігається кількість рядків у формі й в об`єкті). При збереженні вони будуть загублені. Рекомендується закрити / відкрити форму новини без збереження'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура ЗаполнитьДоступныеКатегорииДляЛентыНовостейСервер()
	...
	Если МассивКатегорий_Простые.Количество() = 0 Тогда
		...
		Элементы.ГруппаКатегорииПростые.Заголовок = НСтр("ru='Простые категории (недоступно, так как для этой ленты новостей не включены отборы по простым категориям)';uk='Прості категорії (недоступно, тому що для цієї стрічки новин не включені відбори по простих категоріях)'");
	Иначе
	...
	Если МассивКатегорий_ИнтервалыВерсий.Количество() = 0 Тогда
		...
		Элементы.ГруппаКатегорииИнтервалыВерсий.Заголовок = НСтр("ru='Привязка к версиям (недоступна, так как для этой ленты новостей не включены отборы по версиям)';uk='Прив`язка до версій (недоступна, тому що для цієї стрічки новин не включені відбори по версіях)'");
	Иначе
...
#Область ФорматированиеHTML_БлокиИРазрывы
	...
	НоваяКоманда.Заголовок                 = НСтр("ru='Разрыв строки <br/>';uk='Разрыв строки <br/>'");
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Горизонтальная линия <hr/>';uk='Горизонтальная линия <hr/>'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Абзац <p></p>';uk='Абзац <p></p>'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Блок для абзаца <div></div>';uk='Блок для абзаца <div></div>'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Блок для текста внутри абзаца <span></span>';uk='Блок для текста внутри абзаца <span></span>'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_ФорматШрифта
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Жирный шрифт (<b></b>)';uk='Жирный шрифт (<b></b>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Курсивный шрифт (<i></i>)';uk='Курсивный шрифт (<i></i>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Подчеркнутый шрифт (<u></u>)';uk='Подчеркнутый шрифт (<u></u>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Зачеркнутый шрифт (<s></s>)';uk='Зачеркнутый шрифт (<s></s>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Верхний индекс (<sup></sup>)';uk='Верхний индекс (<sup></sup>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Нижний индекс (<sub></sub>)';uk='Нижний индекс (<sub></sub>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_ФорматШрифта_Заголовки
	...
	Для С=1 По 6 Цикл
		...
		НоваяКоманда.Заголовок                 = СтрШаблон(
			НСтр("ru='Заголовок %1 уровня (<h%1></h%1>)';uk='Заголовок %1 уровня (<h%1></h%1>)'"),
			С);
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_ФорматАбзаца
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Выравнивание влево';uk='Вирівнювання вліво'"); // <p style="text-align: left;"></p>
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Выравнивание вправо';uk='Вирівнювання вправо'"); // <p style="text-align: right;"></p>
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Выравнивание по центру';uk='Вирівнювання по центру'"); // <p style="text-align: center;"></p>
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Выравнивание по ширине';uk='Вирівнювання по ширині'"); // <p style="text-align: justify;"></p>
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_СтилиТекста
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Длинная цитата (<blockquote></blockquote>)';uk='Длинная цитата (<blockquote></blockquote>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Короткая цитата (<q></q>)';uk='Короткая цитата (<q></q>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Источник цитаты (<cite></cite>)';uk='Источник цитаты (<cite></cite>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Ввод с клавиатуры (<kbd></kbd>)';uk='Ввод с клавиатуры (<kbd></kbd>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Программный код (<code></code>)';uk='Программный код (<code></code>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("ru='Акцентированный текст (<em></em>)';uk='Акцентированный текст (<em></em>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_БлокиИРазрывы
	...
	ГруппаКоманд.Заголовок = НСтр("ru='Блоки и разрывы';uk='Блоки и разрывы'");
...
#Область ФорматированиеHTML_ФорматШрифта
	...
	ГруппаКоманд.Вид = ВидГруппыФормы.ГруппаКнопок;
	ГруппаКоманд.Заголовок = НСтр("ru='Формат шрифта';uk='Формат шрифта'");
...
#Область ФорматированиеHTML_ФорматШрифта_Заголовки
	...
	ГруппаКоманд.Заголовок = НСтр("ru='Заголовки';uk='Заголовки'");
...
#Область ФорматированиеHTML_СтилиТекста
	...
	ГруппаКоманд.Вид = ВидГруппыФормы.ГруппаКнопок;
	ГруппаКоманд.Заголовок = НСтр("ru='Стили текста';uk='Стили текста'");
...
Процедура СоздатьКомандыБуфераОбмена(МассивБуферовОбмена)
	...
	Для каждого ТекущийБуферОбмена Из МассивБуферовОбмена Цикл
		...
		НоваяКоманда.Заголовок                 = НСтр("ru='Копировать';uk='Копіювати'");
		...
		НоваяКоманда.ИзменяетСохраняемыеДанные = Ложь;
		НоваяКоманда.Отображение               = ОтображениеКнопки.Картинка;
		НоваяКоманда.Подсказка                 = НСтр("ru='Управление сохраненными раньше буферами обмена';uk='Управление сохраненными раньше буферами обмена'");
...
Процедура СоздатьКомандыБуфераОбмена_СохраненныеБуферыОбмена(МассивБуферовОбмена)
	...
	Для каждого ТекущийБуферОбмена Из МассивБуферовОбмена Цикл
		...
		Для каждого ТекущийЭлементСписка Из ЭтотОбъект["СохраненныеБуферыОбмена_" + ТекущийБуферОбмена] Цикл
			...
			НоваяКоманда.Действие                  = "КомандаБуферОбмена_Вставить";
			НоваяКоманда.Заголовок                 = СтрШаблон(
				НСтр("ru='Вставить из %1';uk='Вставить из %1'"),
...
Процедура СоздатьПодменюБуфераОбмена(Подменю, БуферОбмена)
	...
	ГруппаКомандВставить.Заголовок   = "Вставить";
		...
		ГруппаКомандВставитьСохраненный.Заголовок = НСтр("ru='Вставить сохраненный';uk='Вставить сохраненный'");
...
Процедура УстановитьУсловноеОформление()
	...
	// 2. Заменить в поле Объект.Важность число (0, 1, 2) на текст ("Обычная", "Очень важная", "Важная").
		УсловияОтбора = Новый Соответствие;
		УсловияОтбора.Вставить(0, НСтр("ru='Обычная';uk='Звичайна'"));
		УсловияОтбора.Вставить(1, НСтр("ru='Очень важная';uk='Дуже важлива'"));
		УсловияОтбора.Вставить(2, НСтр("ru='Важная';uk='Важлива'"));
	...
	// 8. Поле "Единственный параметр".
		...
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Несколько параметров настраиваются в таблице [Параметры]';uk='Декілька параметрів настроюються в таблиці [Параметри]'"));
		Элемент.Оформление.УстановитьЗначениеПараметра("Доступность", Ложь);
...
"';uk='Текст для внешних источников:"
" - 10 = %ТекстВИ_10_Э% / %ТекстВИ_10_О%,"
" - 13 = %ТекстВИ_13_Э% / %ТекстВИ_13_О%,"
"Текст для 1С:"
" - 10 = %Текст1С_10_Э% / %Текст1С_10_О%,"
...
"'");
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru='Статистика по переводам строк:';uk='Статистика по перекладах рядків:'"),
		,
		ТекстПояснения,
