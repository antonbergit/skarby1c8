...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокВыбора.Добавить(1, НСтр("ru='Все, кроме внешних';uk='Всі, крім зовнішніх'"));
	...
	Если ПараметрыРазмещения.Варианты.Всего = РезультатВыполнения.Размещено Тогда
		Если ПараметрыРазмещения.Варианты.Всего = 1 Тогда
			Если ПараметрыРазмещения.Действие = "Перемещение" Тогда
				Шаблон = НСтр("ru='Успешно перемещен в ""%1"".';uk='Успішно переміщено в ""%1"".'");
			Иначе
				Шаблон = НСтр("ru='Успешно размещен в ""%1"".';uk='Успішно розміщено в ""%1"".'");
			КонецЕсли;
			Текст = ПараметрыРазмещения.Варианты.Представление;
			Ссылка = ПолучитьНавигационнуюСсылку(ПараметрыРазмещения.Варианты.Массив[0]);
		Иначе
			Если ПараметрыРазмещения.Действие = "Перемещение" Тогда
				Шаблон = НСтр("ru='Успешно перемещены в ""%1"".';uk='Успішно переміщено в ""%1"".'");
			Иначе
				Шаблон = НСтр("ru='Успешно размещены в ""%1"".';uk='Успішно розміщено в ""%1"".'");
			КонецЕсли;
	...
	Иначе
		...
		Если Не ПустаяСтрока(РезультатВыполнения.НеМогутРазмещаться) Тогда
			ТекстОшибок = ?(ТекстОшибок = "", "", ТекстОшибок + Символы.ПС + Символы.ПС)
				+ НСтр("ru='Не могут размещаться в командном интерфейсе:';uk='Не можуть розміщатися в командному інтерфейсі:'")
				+ Символы.ПС
		...
		Иначе
			Шаблон = НСтр("ru='Размещено вариантов отчетов: %1 из %2.';uk='Розміщене варіантів звітів: %1 з %2.'");
		КонецЕсли;
		...
		Для Каждого ВариантСсылка Из ПараметрыРазмещения.Варианты.Массив Цикл
			...
			ИначеЕсли ВариантСсылка.ПометкаУдаления Тогда
				НеМогутРазмещаться = ?(НеМогутРазмещаться = "", "", НеМогутРазмещаться + Символы.ПС)
					...
					+ Строка(ВариантСсылка)
					+ " ("
					+ НСтр("ru='помечен на удаление';uk='позначений на видалення'")
					+ ")";
				Продолжить;
			КонецЕсли;
...
Функция НачатьЗамер(Событие)
	...
	Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
		Комментарий = Комментарий
			...
			+ "; " + НСтр("ru='Включая подчиненные:';uk='Включаючи підлеглі:'") + " " + Строка(ВключаяПодчиненные);
	Иначе
