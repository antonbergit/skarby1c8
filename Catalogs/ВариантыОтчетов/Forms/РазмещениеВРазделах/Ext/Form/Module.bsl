...
Процедура Разместить(Команда)
	Если ПроверитьКоличествоВариантов() Тогда
		ЗаписатьНаСервере();
		ТекстОповещения = НСтр("ru='Изменены настройки вариантов отчетов (%1 шт.).';uk='Змінено налаштування варіантів звітів (%1 шт.).'");
...
Процедура ПерезаполнитьДерево(ТолькоСнятьФлажки)
	...
	Если КоличествоВариантов = 0 Тогда
		ТекстСообщения = НСтр("ru='Необходимо выбрать варианты отчетов';uk='Необхідно вибрати варіанти звітів'");
		...
		Для Каждого СтрокаТаблицы Из ОтфильтрованныеВарианты Цикл
			Если ТекущаяПричина <> СтрокаТаблицы.Причина Тогда
				...
				Если ТекущаяПричина = 1 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + НСтр("ru='Помеченные на удаление:';uk='Позначені на видалення:'");
