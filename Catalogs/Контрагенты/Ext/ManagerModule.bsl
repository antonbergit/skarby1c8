...
// ЕДРПОУ - ЕДРПОУ проверяемого контрагента, Тип - Строка(12)
// Ссылка - Сам проверяемый контрагент, Тип - СправочникСсылка.Контрагенты
Функция ПроверитьДублиСправочникаКонтрагентыПоИННЕДРПОУ(Знач ИНН, Знач ЕДРПОУ, ИсключаяСсылку = Неопределено, ПроверкаПриЗаписи = Ложь) Экспорт
	...
	Если ПроверкаПриЗаписи Тогда
		Дубли = ЕстьЗаписиВРегистреДублей(ИНН, ЕДРПОУ, ИсключаяСсылку);
	КонецЕсли;
	...
	Если Дубли.Количество() = 0 Тогда
		...
		Запрос.Текст = 	"ВЫБРАТЬ
		               	...
		               	|	И Контрагенты.КодПоЕДРПОУ = &ЕДРПОУ";
...
//На вход получает ИНН, ЕДРПОУ и ссылку на контрагента
Функция ЕстьЗаписиВРегистреДублей(ИНН, ЕДРПОУ, ИсключаяСсылку = Неопределено) Экспорт
	...
	|	И НаличиеДублейУКонтрагентов.ЕДРПОУ = &ЕДРПОУ
...
// ЕДРПОУ - ЕДРПОУ записываемого контрагента
// НужноУдалить:
//				Истина - удалить запись по переданному контрагенту
//				Ложь   - сделать запись по переданному контрагенту
Процедура ВыполнитьДвиженияПоРегиструДублей(Ссылка, ИНН = "", ЕДРПОУ = "", НужноУдалить) Экспорт
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточника(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных) Экспорт
	...
	ОписаниеТиповКолонка = Новый ОписаниеТипов("СправочникСсылка.Контрагенты");
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "ИНН", 				"ИНН", 				ОписаниеТиповСтрока25, ОписаниеТиповКолонка, "Контрагент", 1, , Истина);
	...
	ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "КодПоЕДРПОУ", "Код по ЕГРПОУ", ОписаниеТиповСтрока10, ОписаниеТиповСтрока10);
...
Процедура СопоставитьЗагружаемыеДанныеИзВнешнегоИсточника(ТаблицаСопоставленияДанных, НастройкиЗагрузкиДанных) Экспорт
	...
	Для каждого СтрокаТаблицыФормы Из ТаблицаСопоставленияДанных Цикл
		...
		// Контрагент по ИНН, Наименованию, Расчетному счету
		ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СопоставитьКонтрагента(СтрокаТаблицыФормы.Контрагент, СтрокаТаблицыФормы.ИНН, СтрокаТаблицыФормы.КонтрагентНаименование, СтрокаТаблицыФормы.РасчетныйСчет);
		...
		// КодПоЕДРПОУ
		ЗагрузкаДанныхИзВнешнегоИсточникаПереопределяемый.СкопироватьСтрокуВЗначениеСтроковогоТипа(СтрокаТаблицыФормы.КодПоЕДРПОУ, СтрокаТаблицыФормы.КодПоЕДРПОУ_ВходящиеДанные);
...
Процедура ОбработатьПодготовленныеДанные(СтруктураДанных, ФоновоеЗаданиеАдресХранилища = "") Экспорт
		...
		Для каждого СтрокаТаблицы Из ТаблицаСопоставленияДанных Цикл
			...
			Если ЗагрузкаВПриложениеВозможна И СогласованноеСостояниеСтроки Тогда
				...
				Иначе
					...
					ЭлементСправочника.СтранаРегистрации = Справочники.СтраныМира.Украина;
				...
				Если НЕ ПустаяСтрока(СтрокаТаблицы.ИНН) Тогда
					...
					Для каждого ЗначениеРазделителя Из Разделители Цикл
						...
						ПозицияРазделителя = Найти(СтрокаТаблицы.ИНН, ЗначениеРазделителя);
						...
						ИНН = Лев(СтрокаТаблицы.ИНН, ПозицияРазделителя - 1);
			...
			ТекстПрогресса		= СтрШаблон(НСтр("ru='Обработано %1 из %2 строк...';uk='Оброблене %1 з %2 рядків...'"), ИндексТекущейстроки, РазмерТаблицыДанных);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(Нстр("ru='Загрузка данных';uk='Завантаження даних'"), УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.Контрагенты, , ОписаниеОшибки());
		ОтменитьТранзакцию();
...
Процедура ЗаполнитьДеталиПрайсЛистаИерархияНоменклатуры(СтруктураПечати, Выборка, ВывестиНоменклатуруБезРодителя = Ложь)
	...
	Если ВывестиНоменклатуруБезРодителя Тогда
		...
		ЗначенияПараметров.ЦеноваяГруппа = НСтр("ru='<...>';uk='<...>'");
		ЗаполнитьПараметрыОбласти("ЦеноваяГруппа", ЗначенияПараметров, СтруктураПечати);
...
Функция СформироватьПрайсЛист(МассивОбъектов, ОбъектыПечати)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Справочник.Контрагенты.ПФ_MXL_ПрайсЛист");
	
	КодЯзыкаПечать = Локализация.ПолучитьЯзыкФормированияПечатныхФорм();	
	Макет.КодЯзыкаМакета = КодЯзыкаПечать;
	
	СтруктураОбластей = ЗаполнитьСтрутуруОбластейМакета(Макет);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='Контактная информация';uk='Контактна інформація'");
