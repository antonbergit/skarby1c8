...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//ЭтотОбъект.ИспользоватьСпаркРиски = ПолучитьФункциональнуюОпцию("ИспользоватьСпаркРиски");
...
"Подключиться сейчас?';uk='Для автоматичного заповнення реквізитів контрагентів"
"необхідно підключитися до інтернет-підтримки користувачів."
"Підключитися зараз?'");
...
Процедура ПредставлениеПроверкиДанныхОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если СтрНайти(НавигационнаяСсылкаФорматированнойСтроки, "ПоказатьДубли") > 0 Тогда
		...
		ПараметрыФормы.Вставить("КодПоЕДРПОУ", СокрЛП(Объект.КодПоЕДРПОУ));
...
Процедура ДобавитьПолеКонтактнойИнформацииКонтактногоЛица(Команда)
	...
	ДополнительныеПараметры.Вставить("МножественнаяФормаВладельца", НСтр("ru='контактных лиц';uk='контактних осіб'"));
...
Процедура УправлениеФормой()
	...
	Если Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ЮридическоеЛицо") Тогда
		...
		КонтрагентЗарегистрированВУкраине = Объект.СтранаРегистрации = ПредопределенноеЗначение("Справочник.СтраныМира.Украина");
		
		Объект.НеЯвляетсяРезидентом = Не КонтрагентЗарегистрированВУкраине;
		
		Элементы.СтранаРегистрации.Видимость				= Истина;
		Элементы.ФИО.Видимость								= Ложь;
		Элементы.ИНН.Видимость								= Истина;
		Элементы.КодПоЕДРПОУ.Видимость						= КонтрагентЗарегистрированВУкраине;
		...
		//	Элементы.РегистрационныйНомер.Заголовок			= НСтр("ru='ОГРН';uk='ОДРН'");
		//	Элементы.РегистрационныйНомер.Подсказка			= НСтр("ru='Основной государственный регистрационный номер юридического лица';uk='Основний державний реєстраційний номер юридичної особи'");
		//	Элементы.РегистрационныйНомер.ПодсказкаВвода	= НСтр("ru='13 цифр';uk='13 цифр'");
		//	Элементы.РегистрационныйНомер.ОграничениеТипа	= Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(13));
		//	Элементы.ИНН.Подсказка							= НСтр("ru='Идентификационный номер налогоплательщика';uk='Ідентифікаційний номер платника податків'");
		//Иначе
		//	Элементы.РегистрационныйНомер.Заголовок			= НСтр("ru='Рег. номер';uk='Реєстр. номер'");
		//	Элементы.РегистрационныйНомер.Подсказка			= НСтр("ru='Регистрационный номер, присвоенный иностранной организации в стране регистрации (инкорпорации)';uk='Реєстраційний номер, присвоєний іноземній організації в країні реєстрації (інкорпорації)'");
		//	Элементы.РегистрационныйНомер.ПодсказкаВвода	= НСтр("ru='Рег. номер в стране регистрации';uk='Реєстр. номер в країні реєстрації'");
	...
	ИначеЕсли Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ИндивидуальныйПредприниматель") Тогда
		...
		//Элементы.РегистрационныйНомер.Заголовок				= НСтр("ru='ОГРН ИП';uk='ОГРН ІП'");
		...
		//Элементы.РегистрационныйНомер.ПодсказкаВвода		= НСтр("ru='15 цифр';uk='15 цифр'");
		...
		Элементы.КодПоЕДРПОУ.ПодсказкаВвода					= НСтр("ru='12 цифр';uk='12 цифр'");
		Элементы.КодПоЕДРПОУ.ОграничениеТипа				= Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(12));
...
Процедура УстановитьЗаголовокФормы(Форма)
	...
	Если Объект.Покупатель Тогда
		ВидыОтношений.Добавить(НСтр("ru='Покупатель';uk='Покупець'"));
	КонецЕсли;
	
	Если Объект.Поставщик Тогда
		ВидыОтношений.Добавить(НСтр("ru='Поставщик';uk='Постачальник'"));
	КонецЕсли;
	
	Если Объект.ПрочиеОтношения Тогда
		ВидыОтношений.Добавить(НСтр("ru='Прочие отношения';uk='Інші відносини'"));
	КонецЕсли;
	
	Заголовок = Объект.Наименование + " (" + НСтр("ru='Контрагент';uk='Контрагент'");
...
Процедура ОбновитьЭлементыКонтактныхЛиц()
	...
	Для Каждого ДанныеКонтактногоЛица Из ДанныеКонтактныхЛиц Цикл
		...
		Если ИндексКонтакта > 0 Тогда
			...
			ПолеНаименование.ПодсказкаВвода = НСтр("ru='Имя Фамилия';uk='Ім`я Прізвище'");
		...
		Для Каждого ДанныеКИ Из ДанныеКонтактногоЛица.КонтактнаяИнформация Цикл
			...
			Если КонтактнаяИнформацияУНФ.ДляТипаКонтактнойИнформацииДоступенВводКомментария(ДанныеКИ.Тип) Тогда
				...
				ПолеКомментарий.ПодсказкаВвода = НСтр("ru='Прим.';uk='Прим.'");
	...
	Для Каждого ДанныеКонтактногоЛица Из ДанныеКонтактныхЛиц Цикл
		...
		Для Каждого СтрокаТаблицы Из ДанныеКонтактногоЛица.КонтактнаяИнформация Цикл
			Если Не ПустаяСтрока(СтрокаТаблицы.ЗначенияПолей) Или Не ПустаяСтрока(СтрокаТаблицы.Представление) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='ФИО контакта не заполнено.';uk='ПІБ контакту не заповнене.'"),,,ИмяРеквизита, Отказ);
				Прервать;
			КонецЕсли;
		КонецЦикла;
		...
		Для Каждого СтрокаТаблицы Из ДанныеКонтактногоЛица.КонтактнаяИнформация Цикл
			...
			Если СвойстваВида.ОбязательноеЗаполнение И ПустаяСтрока(СтрокаТаблицы.Представление)
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрШаблон(НСтр("ru='Вид контактной информации ""%1"" не заполнен.';uk='Вид контактної інформації ""%1"" не заповнений.'"), СвойстваВида.ПредставлениеВида),,, ИмяРеквизита);
...
"Продолжить?';uk='Можливість введення ""%1"" буде додано для всіх %2."
"Продовжити?'"), ВыбранныйЭлемент.Значение, ДополнительныеПараметры.МножественнаяФормаВладельца);
		ЗаголовокВопроса = НСтр("ru='Подтверждение добавления';uk='Підтвердження додавання'");
...
Процедура СформироватьПредставлениеПроверкиЕДРПОУ(Форма)
	...
	Если Не ПустаяСтрока(Объект.КодПоЕДРПОУ) Тогда
		РегламентированныеДанныеКлиентСервер.ЕДРПОУСоответствуетТребованиям(Объект.КодПоЕДРПОУ, ЭтоЮрЛицо(Объект.ВидКонтрагента), ОписаниеОшибки);
	КонецЕсли;
	Форма.ПредставлениеПроверкиЕДРПОУ = Новый ФорматированнаяСтрока(ОписаниеОшибки, , Форма.ЦветТекстаНекорректногоКонтрагента);
...
Процедура СформироватьПредставлениеПроверкиДублей(Форма)
	...
	Если ИННиЕДРПОУКорректны(Форма) И Не ПустаяСтрока(Объект.ИНН) Тогда
...
&НаСервереБезКонтекста
Функция ПолучитьДублиКонтрагентаСервер(ИНН, ЕДРПОУ, ИсключаяСсылку)
	
	Возврат Справочники.Контрагенты.ПроверитьДублиСправочникаКонтрагентыПоИННЕДРПОУ(ИНН, ЕДРПОУ, ИсключаяСсылку);
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ИННиЕДРПОУКорректны(Форма)
	
	Объект = Форма.Объект;
	ЭтоЮрЛицо = ЭтоЮрЛицо(Объект.ВидКонтрагента);
	
	Если ЭтоЮрЛицо Тогда
		Результат = Объект.ИННВведенКорректно И (Объект.ЕДРПОУВведенКорректно Или ПустаяСтрока(Объект.КодПоЕДРПОУ));
	Иначе
...
Процедура УстановитьРеквизитыПроверкиКонтрагентов(Форма)
	...
	Форма.РеквизитыПроверкиКонтрагентов.ЭтоИностранныйКонтрагент =
		(Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ЮридическоеЛицо")
		И Объект.СтранаРегистрации <> ПредопределенноеЗначение("Справочник.СтраныМира.Украина"));
...
Процедура УстановитьЗаголовокЮридическихДанных(Форма)
	...
	Если Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ЮридическоеЛицо") Тогда
		ДинамическиеПараметры.Добавить(НСтр("ru='Юр.лицо';uk='Юр.особа'"));
	ИначеЕсли Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ИндивидуальныйПредприниматель") Тогда
		ДинамическиеПараметры.Добавить(НСтр("ru='ИП';uk='ІП'"));
	ИначеЕсли Объект.ВидКонтрагента = ПредопределенноеЗначение("Перечисление.ВидыКонтрагентов.ФизическоеЛицо") Тогда
		ДинамическиеПараметры.Добавить(НСтр("ru='Физ.лицо';uk='Фіз.особа'"));
	КонецЕсли;
...
Процедура ПрочитатьДанныеПанелиДопИнформации()
	...
	Если ПросмотрПродаж Тогда
		...
		КомпонентыФС = Новый Массив;
		КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Продажи на';uk='Продажі на'") + " ", КрупныйШрифт));
		...
		КомпонентыФС = Новый Массив;
		КомпонентыФС.Добавить(НСтр("ru='Последняя продажа';uk='Останній продаж'") + " ");
	...
	КомпонентыФС = Новый Массив;
	КомпонентыФС.Добавить(НСтр("ru='Последнее событие';uk='Остання подія'") + " ");
	КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(Формат(Дата, "Л=ru_RU; ДЛФ=D; ДП=<нет>"), , , , НавСсылка));
