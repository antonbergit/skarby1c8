...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Если ПолучитьФункциональнуюОпцию("ИспользоватьСпаркРиски") Тогда
	//	// Команды1СПАРКРиски
...
Процедура ИерархияУстановитьПометкуУдаления(Команда)
	...
	ТекстОповещения = СтрШаблон(НСтр("ru='Пометка удаления %1';uk='Позначка видалення %1'"),
		?(ПометкаУдаления, НСтр("ru='установлена';uk='встановлена'"), НСтр("ru='снята';uk='знята'")));
...
Процедура ПрочитатьИерархию()
	...
	СтрокаДерева.ПредставлениеГруппы = НСтр("ru='<Все группы>';uk='<Всі групи>'");
	
	СтрокаДерева = ЭлементыКоллекции.Добавить();
	СтрокаДерева.ИндексПиктограммы = -1;
	СтрокаДерева.ГруппаКонтрагентов = "БезГруппы";
	СтрокаДерева.ПредставлениеГруппы = НСтр("ru='<Нет группы>';uk='<Без групи>'");
...
"Загрузка данных завершена.';uk='Контрагенти."
...
Процедура УстановитьУсловноеОформление()
	...
	Отбор.ВидСравнения 		= ВидСравненияКомпоновкиДанных.Равно;
	Отбор.Использование 	= Истина;
	Отбор.ЛевоеЗначение 	= Новый ПолеКомпоновкиДанных("ЕДРПОУВведенКорректно");
	Отбор.ПравоеЗначение 	= Ложь;
	
	ОформляемоеПоле = НовоеУсловноеОформление.Поля.Элементы.Добавить();
	ОформляемоеПоле.Поле			= Новый ПолеКомпоновкиДанных("КодПоЕДРПОУ");
	ОформляемоеПоле.Использование	= Истина;
...
Процедура УстановитьЗаголовокФормы(Форма)
	...
	Если Форма.ОтборПокупатель Тогда
		ВидыОтношений.Добавить(НСтр("ru='Покупатели';uk='Покупці'"));
	КонецЕсли;
	
	Если Форма.ОтборПоставщик Тогда
		ВидыОтношений.Добавить(НСтр("ru='Поставщики';uk='Постачальники'"));
	КонецЕсли;
	...
	Заголовок = НСтр("ru='Контрагенты';uk='Контрагенти'");
