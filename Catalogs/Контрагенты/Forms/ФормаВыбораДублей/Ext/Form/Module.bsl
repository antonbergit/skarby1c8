...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокДублей.Параметры.УстановитьЗначениеПараметра("КодПоЕДРПОУ", СокрЛП(Параметры.КодПоЕДРПОУ));
	
	Если НЕ Параметры.ЭтоЮрЛицо Тогда
		Элементы.СписокКодПоЕДРПОУ.Видимость = Ложь;
	КонецЕсли;
	
	Заголовок = ?(ЗначениеЗаполнено(Параметры.КодПоЕДРПОУ),
					НСтр("ru='Список дублей по ИНН и ЕГРПОУ';uk='Список дублів по ІПН і ЄДРПОУ'"),
					НСтр("ru='Список дублей по ИНН';uk='Список дублів по ІПН'"));
...
Процедура ОбработатьАктивизациюСтрокиСписка()
	...
	Элементы.ОткрытьДокументыПоКонтрагенту.Заголовок = СтрШаблон(НСтр("ru='Документы по контрагенту (%1)';uk='Документи по контрагенту (%1)'"),
