...
Процедура ПроверитьСогласованностьДисконтнойКарты(Ошибки)
	...
	Если Не ЗначениеЗаполнено(СтрокиКонтрагент) Тогда
		...
		НСтр("ru='Дисконтная карта не может быть указана в шаблоне без контрагента.';uk='Дисконтна картка не може бути зазначена в шаблону без контрагента.'"), "",,,
...
Процедура ДобавитьОшибкуНесогласованностиЗначений(Ошибки, ТекСтрока1, ТекСтрока2)
	...
	НСтр("ru='Значение поля ""%2"" не согласовано со значением поля ""%1"". Возможно, не установлен флажок для поля ""%1""';uk='Значення поля ""%2"" не погоджене зі значенням поля ""%1"". Можливо, не встановлений прапорець для поля ""%1""'"),
