...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если КоличествоТоваров > 0 Тогда
		Если Товары[КоличествоТоваров-1].Код > ТекущийОбъект.МаксимальныйКод И ТекущийОбъект.МаксимальныйКод <> 0 Тогда
			...
			НСтр("ru='В табличной части ""Товары"" найдены строки с кодом, превышающим максимальное допустимое значение: %1. Измените максимальный код или уменьшите количество товаров к выгрузке при помощи отбора.';uk='У табличній частині ""Товари"" знайдені рядки з кодом, що перевищують максимальне припустиме значення: %1. Змінить максимальний код або зменшить кількість товарів до вивантаження за допомогою відбору.'"), ТекущийОбъект.МаксимальныйКод),,"Товары",,Отказ);
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура МаксимальныйКодПриИзменении(Элемент)
	...
	Состояние(НСтр("ru='Выполняется обновление списка товаров...';uk='Виконується відновлення списку товарів...'"));
	ОбновитьСписокТоваровНаСервере();
...
Процедура ПрефиксВесовогоТовараПриИзменении(Элемент)
	...
	Если Объект.ПрефиксВесовогоТовара = 0 Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Префикс = 0, обычно, используется для штучного, а не весового товара. Рекомендуется устанавливать префикс > 0.';uk='Префикс = 0, обычно, используется для штучного, а не весового товара. Рекомендуется устанавливать префикс > 0.'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ТоварыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если НЕ ОтменаРедактирования Тогда
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			...
			+ ?(ЗначениеЗаполнено(НайденнаяСтрока.Партия), " " + НСтр("ru='с партией';uk='с партією'") + " """ + НайденнаяСтрока.Партия + """", "")
			+ ?(ЗначениеЗаполнено(НайденнаяСтрока.ЕдиницаИзмерения), " " + НСтр("ru='с единицей';uk='с одиницею'") + " """ + НайденнаяСтрока.ЕдиницаИзмерения + """", ""))+НСтр("ru='. Выполнен обмен местами.';uk='. Виконаний обмін місцями.'");
...
Процедура ТоварыПередУдалением(Элемент, Отказ)
	...
	Для каждого ВыделеннаяСтрока Из Элементы.Товары.ВыделенныеСтроки Цикл
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.Номенклатура) И ТекущиеДанные.Используется Тогда
			ПоказатьПредупреждение(, (НСтр("ru='Номенклатуре, соответствующей заданному отбору, после удаления будут заново назначены коды товаров.';uk='Номенклатурі, відповідної до заданого відбору, після видалення будуть заново призначені коди товарів.'")));
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура УдалитьСвободныеКодыТоваровВКонцеСписка(Команда)
	...
	Если Товары.Количество() > 0 Тогда
		...
		Иначе
			ПоказатьПредупреждение(Неопределено,НСтр("ru='Нет данных для удаления';uk='Немає даних для видалення'"));
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(Неопределено,НСтр("ru='Нет данных для удаления';uk='Немає даних для видалення'"));
	КонецЕсли;
...
Процедура ЗагрузитьНастройкиОтбораПоУмолчанию()
	...
	Иначе
		ВызватьИсключение НСтр("ru='Некорректный тип подключаемого оборудования';uk='Некоректний тип підключаємого обладнання'");
	КонецЕсли;
