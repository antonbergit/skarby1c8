...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(ДатаНачала) И ЗначениеЗаполнено(ДатаОкончания)
		И ДатаОкончания < ДатаНачала Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Дата окончания не может быть меньше даты начала.';uk='Дата закінчення не може бути менше дати початку.'"),,,,Отказ);
	КонецЕсли;
...
Процедура ОбновитьЭлементыНастроекЗаполнения()
	...
	Для Каждого ОписаниеПоля Из ОписаниеПолей Цикл
		...
		Если ОписаниеПоля.ТипЗначения.СодержитТип(Тип("Дата")) Тогда
			ВариантЗаполнения.СписокВыбора.Добавить("Смещение",		НСтр("ru='смещение даты';uk='зміщення дати'"));
			ВариантЗаполнения.СписокВыбора.Добавить("Указанный",	НСтр("ru='указанный';uk='зазначений'"));
		Иначе
			ВариантЗаполнения.СписокВыбора.Добавить("ИзОснования",	НСтр("ru='из основания';uk='з підстави'"));
			ВариантЗаполнения.СписокВыбора.Добавить("Указанный",	НСтр("ru='указанный';uk='зазначений'"));
		КонецЕсли;
...
Процедура ПривестиЭлементФормыКОписаниюПоля(Форма, ОписаниеПоля, ОчищатьЗначение = Истина)
	...
	//ОписаниеПоля.СписокПолейОснования.Количество() > 0 Или ОписаниеПоля.ВариантЗаполнения = "Смещение";
...
Процедура ЗаполнитьСписокСтандартныхИнтервалов(СписокВыбора)
	...
	СписокВыбора.Добавить("Сразу",		НСтр("ru='Дата срабатывания правила';uk='Дата спрацьовування правила'"));
	СписокВыбора.Добавить("ЧерезЧас",	НСтр("ru='Через час';uk='Через годину'"));
	СписокВыбора.Добавить("ЧерезДень",	НСтр("ru='Через день';uk='Через день'"));
	СписокВыбора.Добавить("ЧерезНеделю",НСтр("ru='Через неделю';uk='Через тиждень'"));
	СписокВыбора.Добавить("ЧерезМесяц",	НСтр("ru='Через месяц';uk='Через місяць'"));
...
Процедура УстановитьПараметрыВыбораШаблона()
	...
	Если Объект.ТипДействия = Перечисления.ТипыДействийРабочегоПроцесса.ОтправитьEmail Тогда
		ПараметрыВыбораШаблона.Добавить(Новый ПараметрВыбора("Отбор.СпособОтправки", Перечисления.ВидыКаналовСвязи.Email));
	ИначеЕсли Объект.ТипДействия = Перечисления.ТипыДействийРабочегоПроцесса.ОтправитьSMS Тогда
		ПараметрыВыбораШаблона.Добавить(Новый ПараметрВыбора("Отбор.СпособОтправки", Перечисления.ВидыКаналовСвязи.SMS));
	КонецЕсли;
...
Функция АвтоматическоеНаименованиеДействия(Форма)
	...
	Шаблон = НСтр("ru='%1 по основанию: %2';uk='%1 на підставі: %2'");
