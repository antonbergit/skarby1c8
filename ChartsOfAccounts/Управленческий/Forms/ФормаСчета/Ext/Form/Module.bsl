...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОтказИзменитьАналитикуПрочихДоходовИРасходов(Объект.Ссылка) Тогда
		Элементы.ГруппаАналитикаУчетаПрочихДоходовИРасходов.Подсказка = НСтр("ru='В базе есть движения по этому счету! Изменение аналитики не рекомендуется!';uk='У базі є рухи по цьому рахунку! Зміна аналітики не рекомендується!'");
	КонецЕсли;
...
Процедура УправлениеФормой()
	...
	Если Объект.ТипСчета = ПредопределенноеЗначение("Перечисление.ТипыСчетов.КосвенныеЗатраты") Тогда
		...
		Элементы.СчетЗакрытия.Подсказка = ?(
			...
			НСтр("ru='Счет автоматического закрытия при закрытии месяца и бюджетировании';uk='Рахунок автоматичного закриття при закритті місяця й бюджетуванні'"),
			НСтр("ru='Счет автоматического закрытия при закрытии месяца';uk='Рахунок автоматичного закриття при закритті місяця'")
		);
		Элементы.СпособРаспределения.Подсказка = НСтр("ru='Способ автоматического распределения на себестоимость выпущенной продукции при закрытии месяца';uk='Спосіб автоматичного розподілу на собівартість випущеної продукції при закритті місяця'"
		);
	ИначеЕсли Объект.ТипСчета = ПредопределенноеЗначение("Перечисление.ТипыСчетов.НезавершенноеПроизводство") Тогда
		...
		Элементы.СпособРаспределения.Подсказка = НСтр("ru='Способ автоматического распределения на себестоимость выпущенной продукции при закрытии месяца для нематериальных затрат';uk='Спосіб автоматичного розподілу на собівартість випущеної продукції при закритті місяця для нематеріальних витрат'"
		);
	ИначеЕсли (ТипСчета <>  ПредопределенноеЗначение("Перечисление.ТипыСчетов.ПрочиеДоходы")
		...
		Элементы.СпособРаспределения.Подсказка = ?(
			ИспользоватьБюджетирование,
			НСтр("ru='Способ автоматического распределения на финансовый результат при закрытии месяца и бюджетировании';uk='Спосіб автоматичного розподілу на фінансовий результат при закритті місяця й бюджетуванні'"),
			НСтр("ru='Способ автоматического распределения на финансовый результат при закрытии месяца';uk='Спосіб автоматичного розподілу на фінансовий результат при закритті місяця'")
		);
