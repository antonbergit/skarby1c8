...
Функция ПолучитьСписокДляВыбораДокументаОснования()
	...
	Если НЕ МобильноеПриложениеВызовСервера.НужноОграничитьФункциональность() Тогда
		...
		Если ПолучитьФункциональнуюОпцию("УчетРозничныхПродаж") Тогда
			СписокОснований.Добавить("Документ.ОтчетОРозничныхПродажах.ФормаВыбора", НСтр("ru='Отчет о розничных продажах';uk='Звіт про роздрібні продажі'"));
		КонецЕсли;
	Конецесли;
	// Конец МобильноеПриложение
...
Процедура УстановитьВидимостьЭлементовВЗависимостиОтВидаОперации(ПриСозданииНаСервере = Ложь)
	...
	Элементы.РозничнаяВыручкаРасшифровкаПлатежаСуммаНДС.Видимость = Ложь;
	Элементы.НалогообложениеНДС.Видимость = Ложь;
	Элементы.ДокументыПланирования.Заголовок = НСтр("ru='Планирование';uk='Планування'");
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеВКассу.РозничнаяВыручка") Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ПлатежныйКалендарь") Тогда
			Элементы.ДокументыПланирования.Заголовок = НСтр("ru='Планирование, НДС';uk='Планування, ПДВ'");
		Иначе
			Элементы.ДокументыПланирования.Заголовок = НСтр("ru='НДС';uk='ПДВ'");
		КонецЕсли;
