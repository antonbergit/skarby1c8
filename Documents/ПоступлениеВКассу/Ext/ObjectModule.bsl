...
Процедура ЗаполнитьПоПеремещениюДСПлан(ДокументОснование, Сумма = Неопределено) Экспорт
	...
	"ВЫБРАТЬ
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(&Ссылка) КАК Основание,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеВКассу.Прочее) КАК ВидОперации,
	|	&Ссылка КАК ДокументОснование,
	|	ТаблицаДокумента.Организация КАК Организация,
...
Процедура ЗаполнитьПоПриемуВРемонт(ДанныеЗаполнения) Экспорт
	...
	ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ОтПокупателя;
	
	спРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеЗаполнения, 
		"Договор, Договор.ВалютаРасчетов, Организация.КассаПоУмолчанию, Организация.СтавкаНДСПоУмолчанию, РемонтВыполнен, СуммаДокумента");
	
	Если ПолучитьФункциональнуюОпцию("ОграничиватьДоступНаУровнеЗаписей") Тогда
		ЭтотОбъект.Касса = Справочники.Кассы.ПолучитьКассуСУчетомПравДоступаНаУровнеЗаписей(ДанныеЗаполнения.Организация.КассаПоУмолчанию);
	Иначе
		ЭтотОбъект.Касса = спРеквизитов.ОрганизацияКассаПоУмолчанию;
	КонецЕсли;
...
Процедура УстановитьНомерОрдера() 
	...
	|	И ПоступлениеВКассу.Касса = &Касса	
	|	И ПоступлениеВКассу.ВалютаДенежныхСредств = &ВалютаДенежныхСредств	
	|	И ПоступлениеВКассу.СтруктурнаяЕдиница = &СтруктурнаяЕдиница
	|	И ПоступлениеВКассу.Дата >= &ДатаНачалаГода
	|	И ПоступлениеВКассу.Дата <= &ДатаКонцаГода";
