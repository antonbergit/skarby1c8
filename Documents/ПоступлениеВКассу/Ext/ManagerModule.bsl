...
Процедура СформироватьТаблицаДенежныеСредства(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПоступлениеДенежныхСредств", НСтр("ru='Поступление денежных средств';uk='Надходження коштів'"));
	Запрос.УстановитьПараметр("ПоступлениеРозничнойВыручки", НСтр("ru='Поступление розничной выручки';uk='Надходження роздрібного виторгу'"));
...
Процедура СформироватьТаблицаДенежныеСредстваВКассахККМ(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("РасходДенежныхСредств", НСтр("ru='Расход денежных средств из кассы ККМ';uk='Витрата коштів з каси ККМ'"));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ru='Курсовая разница';uk='Курсова різниця'"));
...
Процедура СформироватьТаблицаРасчетыСПоставщиками(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("ПогашениеАвансаПоставщику", НСтр("ru='Погашение аванса поставщику';uk='Погашення авансу постачальникові'"));
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ru='Курсовая разница';uk='Курсова різниця'"));
	Запрос.УстановитьПараметр("Содержание", НСтр("ru='Оприходование денежных средств с произвольного счета';uk='Оприбуткування коштів з довільного рахунку'"));
	Запрос.УстановитьПараметр("СодержаниеПокупкаВалюты", НСтр("ru='Покупка валюты';uk='Покупка валюти'"));
	Запрос.УстановитьПараметр("СодержаниеРозничнаяВыручка", НСтр("ru='Розничная выручка';uk='Роздрібний виторг'"));
	Запрос.УстановитьПараметр("СодержаниеСебестоимость", НСтр("ru='Себестоимость';uk='Собівартість'"));
	Запрос.УстановитьПараметр("СодержаниеНаценка", НСтр("ru='Наценка';uk='Націнка'"));
...
Процедура СформироватьТаблицаСуммовойУчетВРознице(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("РозничнаяВыручка", НСтр("ru='Розничная выручка';uk='Роздрібний виторг'"));
	Запрос.УстановитьПараметр("Себестоимость", НСтр("ru='Себестоимость';uk='Собівартість'"));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ru='Курсовая разница';uk='Курсова різниця'"));
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаНДСПродаж(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства)
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("Организация", ДокументСсылкаПоступлениеВКассу.Организация);
	
	Запрос.Текст =
	"ВЫБРАТЬ
	|	&Организация КАК Организация,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
	|	ЗНАЧЕНИЕ(Перечисление.СобытияНДСПродаж.ОплатаПокупателем) КАК СобытиеНДС,
	|	ТаблицаНДСПродаж.Договор КАК ДоговорКонтрагента,
	|	НАЧАЛОПЕРИОДА(ТаблицаНДСПродаж.Дата, ДЕНЬ) КАК Период,
	|	ТаблицаНДСПродаж.СуммаУчета КАК Сумма
	|ИЗ
	|	ВременнаяТаблицаРасшифровкаПлатежа КАК ТаблицаНДСПродаж
	|ГДЕ
	|	ТаблицаНДСПродаж.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеВКассу.ОтПокупателя)";
	
	РезультатЗапроса = Запрос.Выполнить();
	
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаНДСПродаж", РезультатЗапроса.Выгрузить());
	
КонецПроцедуры // СформироватьТаблицаНДСПродаж()
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства) Экспорт
	...
	СформироватьТаблицаНДСПродаж(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства);
	...
	Если ДокументСсылкаПоступлениеВКассу.УчитыватьВНУ Тогда
		СформироватьТаблицаКнигаДоходовРасходовПоЕН(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства);
	КонецЕсли;
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		|	ПоступлениеВКассу.ВыводитьНаПечатьСуммуНДС КАК ВыводитьНаПечатьСуммуНДС,
		|	ПоступлениеВКассу.НомерОрдера КАК НомерОрдера,
		...
		ОбластьМакета.Параметры.Счет = СокрЛП(ВыборкаДляПечати.КорСчет) + ?(ЗначениеЗаполнено(ВыборкаДляПечати.КорСчетАвансов), "," + СокрЛП(ВыборкаДляПечати.КорСчетАвансов), "");		

		ОбластьМакета.Параметры.Заполнить(ВыборкаДляПечати);
		ОбластьМакета.Параметры.НомерОрдера = НомерДокумента;
		
		СведенияОбОрганизации = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(ВыборкаДляПечати.Организация, ВыборкаДляПечати.ДатаДокумента, , , КодЯзыкаПечать);
		ОбластьМакета.Параметры.ПолноеНаименование = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОбОрганизации, "ПолноеНаименование,");
		
		//ОбластьМакета.Параметры.НомерДокумента = УправлениеНебольшойФирмойСервер.ПолучитьНомерНаПечать(ТекущийДокументОбъект);
		...
		Если ВыборкаДляПечати.ВыводитьНаПечатьСуммуНДС Тогда
			ОбластьМакета = Макет.ПолучитьОбласть("СекцияВТомЧисле");
			СуммаНДС = ТекущийДокумент.РасшифровкаПлатежа.Итог("СуммаНДС");
			Если СуммаНДС = 0 Тогда
				ОбластьМакета.Параметры.ВТомЧисле = "0,00"+ " " + СокрП(ВыборкаДляПечати.ВалютаДенежныхСредств);
			Иначе				
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='КО-1 (Приходный кассовый ордер)';uk='КО-1 (Прибутковий касовий ордер)'");
...
Процедура СформироватьТаблицаРасчетыСПрочимиКонтрагентами(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("Комментарий", НСтр("ru='Уменьшение долга контрагента';uk='Зменшення боргу контрагента'"));
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаПоступлениеВКассу);
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ru='Курсовая разница';uk='Курсова різниця'"));
...
Процедура СформироватьТаблицаРасчетыПоКредитамИЗаймам(ДокументСсылкаПоступлениеВКассу, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("УчетРасчетовПоКредитамИЗаймам", НСтр("ru='Поступление по кредиту';uk='Надходження по кредиту'"));
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаПоступлениеВКассу);
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ru='Курсовая разница';uk='Курсова різниця'"));
