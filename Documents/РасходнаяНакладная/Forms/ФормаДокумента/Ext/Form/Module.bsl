...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//ITF YANKO 2023-06-05...
	// Создаем параметры выбора.
	МассивПараметров = Новый Массив;
	НовыйПараметр = Новый ПараметрВыбора("Отбор.Владелец", ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Першоджерело (Контрагенты)"));
	МассивПараметров.Добавить(НовыйПараметр);
	
	// Устанавливаем связи параметров и параметры выбора для элемента формы.
	Элементы.ДжерелоЗвернення.ПараметрыВыбора = Новый ФиксированныйМассив(МассивПараметров); 
	//...ITF YANKO 2023-06-05
...
&НаСервере
Функция ПолучитьКомментарийКонтрагента(КонтрагентСсылка) Экспорт
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	Если КонтрагентПередИзменением <> Объект.Контрагент Тогда
		...
		Если Объект.Предоплата.Количество() > 0
			...
			ТекстВопроса = НСтр("ru='Зачет предоплаты будет очищен, продолжить?';uk='Залік передоплати буде очищений, продовжити?'");
	...
	Если ЗначениеЗаполнено(Контрагент) Тогда
		Комментарий = ПолучитьКомментарийКонтрагента(Контрагент);
		
		Если НЕ ПустаяСтрока(Комментарий)
		   И СтрНайти(Комментарий, "#") > 0 Тогда
			
			Предупреждение(
				"⚠ Проблемный клиент!" + Символы.ПС + Комментарий
			);			
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаказПриИзменении(Элемент)
	...
	Если Объект.Предоплата.Количество() > 0
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаказПриИзмененииЗавершение", ЭтотОбъект), НСтр("ru='Зачет предоплаты будет очищен, продолжить?';uk='Залік передоплати буде очищений, продовжити?'"), Режим, 0);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСписокДокументОснование()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьРемонты") Тогда
		СписокДокументОснование.Добавить("Документ.ПриемИПередачаВРемонт.ФормаВыбора", НСтр("ru='Прием в ремонт';uk='Прийом до ремонту'"));
	КонецЕсли;
	СписокДокументОснование.Добавить("Документ.ПриходнаяНакладная.ФормаВыбора", НСтр("ru='Приходная накладная';uk='Прибуткова накладна'"));
	СписокДокументОснование.Добавить("Документ.СчетНаОплату.ФормаВыбора", НСтр("ru='Счет на оплату';uk='Рахунок на оплату'"));
	СписокДокументОснование.Добавить("Документ.ПеремещениеЗапасов.ФормаВыбора", НСтр("ru='Перемещение запасов';uk='Переміщення запасів'"));
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодсистемуПроизводство") Тогда
		СписокДокументОснование.Добавить("Документ.СборкаЗапасов.ФормаВыбора", НСтр("ru='Производство';uk='Виробництво'"));
	КонецЕсли;
...
Процедура ЗапасыСтранаПроисхожденияПриИзменении(Элемент)
	...
	Если КэшЗначений.УчетГТД Тогда
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТабличнойЧасти.СтранаПроисхождения)
			ИЛИ СтрокаТабличнойЧасти.СтранаПроисхождения = КэшЗначений.Украина Тогда
...
Процедура ЗаполнитьПоЗаказу(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоЗаказуЗавершение", ЭтотОбъект), НСтр("ru='Документ будет полностью перезаполнен по ""Заказу""! Продолжить выполнение операции?';uk='Документ буде повністю перезаповнений по ""Замовленню""! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура РедактироватьЗачетПредоплаты(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Укажите вначале контрагента!';uk='Вкажіть спочатку контрагента!'"));
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.Договор) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Укажите вначале договор контрагента!';uk='Вкажіть спочатку договір контрагента!'"));
		Возврат;
	КонецЕсли;
...
Процедура ПоискПоШтрихкоду(Команда)
	...
	ТекШтрихкод = "";
	ПоказатьВводЗначения(Новый ОписаниеОповещения("ПоискПоШтрихкодуЗавершение", ЭтотОбъект, Новый Структура("ТекШтрихкод", ТекШтрихкод)), ТекШтрихкод, НСтр("ru='Введите штрихкод';uk='Введіть штрихкод'"));
...
Процедура ИзменитьРезервЗаполнитьПоРезервам(Команда)
	...
	Если Объект.Запасы.Количество() = 0 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Табличная часть ""Запасы и услуги"" не заполнена!';uk='Таблична частина ""Запаси й послуги"" не заповнена!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Процедура ПередатьДанныеВЕГАИС(Команда)
	...
	Если Модифицированность ИЛИ НЕ Объект.Проведен Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Перед выполнение операции необходимо провести документ.';uk='Перед виконанням операції необхідно провести документ.'"));
		Возврат;
	КонецЕсли;
	
	Если Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходнаяНакладная.ВозвратПоставщику") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Передача накладной ЕГАИС поддерживается только для операции ""Возврат поставщику"".';uk='Передача накладної ЄДАІС підтримується тільки для операції ""Повернення постачальникові"".'"));
		Возврат;
	КонецЕсли;
...
Процедура ОповещениеПередачаДанныхВЕГАИС(ДанныеОтЕГАИС, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ДанныеОтЕГАИС = Неопределено Тогда
		Если НЕ ДанныеОтЕГАИС.Результат Тогда
			СтрокаСообщения = НСтр("ru='Ошибка передачи данных в ЕГАИС';uk='Помилка передачі даних в ЄДАІС'") 
...
Процедура ПодготовитьТТН(ДокументСсылка, Отказ)
	...
	Если ВыборкаДокумент.Следующий() Тогда
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаДокумент.Грузополучатель) Тогда
			Отказ = Истина;
			СтрокаОшибки = НСтр("ru='Не удалось определить соответствие контрагента классификатору ЕГАИС.';uk='Не вдалося визначити відповідність контрагента класифікаторові ЄДАІС.'");
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаДокумент.ОрганизацияЕГАИС) Тогда
			Отказ = Истина;
			СтрокаОшибки = НСтр("ru='Не удалось определить соответствие организации классификатору ЕГАИС.';uk='Не вдалося визначити відповідність організації класифікаторові ЄДАІС.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрокаОшибки,
				ДокументСсылка,
				"Объект.Организация");
		...
		Если КоличествоСтрок = 0 Тогда
			Отказ = Истина;
			СтрокаОшибки = НСтр("ru='Не удалось определить соответствие алкогольной продукции по классификатору ЕГАИС.';uk='Не вдалося визначити відповідність алкогольної продукції по класифікаторові ЄДАІС.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрокаОшибки,
				ДокументСсылка,
				"Объект.Запасы");
		...
		Пока ВыборкаТовары.Следующий() Цикл
			...
			Если Не ЗначениеЗаполнено(ВыборкаТовары.НомерСправкиА) Тогда
				Отказ = Истина;
				СтрокаОшибки = НСтр("ru='В строке %1 в справке Б не заполнен номер справки А.';uk='У рядку %1 в довідці Б не заповнений номер довідки А.'");
			...
			Если ЗначениеЗаполнено(ВыборкаТовары.СправкаБ) Тогда
				Если НЕ ЗначениеЗаполнено(ВыборкаТовары.АлкогольнаяПродукцияСправкиБ)
					...
					Отказ = Истина;
					СтрокаОшибки = НСтр("ru='В строке %1 не заполнена алкогольная продукция в справке Б.';uk='У рядку %1 не заповнена алкогольна продукція в довідці Б.'");
					СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаОшибки, ВыборкаТовары.НомерСтроки);
					ИмяПоляТоваров = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Объект.Запасы", ВыборкаТовары.НомерСтроки, "СправкаБ");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						СтрокаОшибки,
			...
			Иначе
				Отказ = Истина;
				СтрокаОшибки = НСтр("ru='В строке %1 не заполнена справка Б.';uk='У рядку %1 не заповнена довідка Б.'");
				СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаОшибки, ВыборкаТовары.НомерСтроки);
				ИмяПоляТоваров = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Объект.Запасы", ВыборкаТовары.НомерСтроки, "СправкаБ");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрокаОшибки,
...
Процедура ПересчитаемСуммуТабличнойЧастиПоФлагуАвторасчетНДС()
	...
	УчетНДССервер.ПересчитатьНДСсУчетомПогрешностиОкругления(Объект.Запасы, Объект, ПогрешностиОкругления, "Запасы и услуги");
...
Процедура ПересчитатьКурсКратностьВалютыРасчетов(СтруктураДанные)
	...
	Если Объект.Курс <> КурсНовый
		...
		ТекстВопроса = НСтр("ru = 'На дату документа у валюты расчетов (" + КурсВалютыСтрокой + ") был задан курс.
									|Установить курс расчетов (" + КурсНовыйВалютыСтрокой + ") в соответствии с курсом валюты?'; uk = 'На дату документа у валюти розрахунків (" + КурсВалютыСтрокой + ") був заданий курс.
									|Встановити курс розрахунків (" + КурсНовыйВалютыСтрокой + ") у відповідності з курсом валюти?'");
	...
	Если ТипЗнч(СтруктураЦеныИВалюта) = Тип("Структура") И СтруктураЦеныИВалюта.БылиВнесеныИзменения Тогда
		...
		Если ЗначениеЗаполнено(РезультатЗакрытия.ДисконтнаяКарта) И ЗначениеЗаполнено(РезультатЗакрытия.Контрагент) И Не Объект.Контрагент.Пустая() Тогда
			...
			Иначе // Выдадим сообщение и не будем менять данные о дисконтной карте.
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Дисконтная карта не считана. Владелец дисконтной карты не совпадает с контрагентом в документе.';uk='Дисконтна картка не лічена. Власник дисконтної картки не збігається з контрагентом у документі.'"),
				,
				"Контрагент",
				"Объект");
			КонецЕсли;
...
Процедура ОбработатьИзменениеУсловийДоговора(ДанныеДоговора, ДоговорПередИзменением)
	...
	Объект.ОснованиеПечати = Нстр("ru='Договор: ';uk='Договір: '") + Строка(Объект.Договор);
...
"Возможно, необходимо перезаполнить цены.';uk='Договір з контрагентом передбачає умови цін і знижок, "
"відмінні від установлених у документі! "
"Можливо, необхідно перезаповнити ціни.'") + Символы.ПС + Символы.ПС;
		КонецЕсли;
		
		ТекстПредупреждения = ТекстПредупреждения + НСтр("ru='Изменилась валюта расчетов по договору с контрагентом! "
"Необходимо проверить валюту документа!';uk='Змінилася валюта розрахунків за договором з контрагентом! "
"Необхідно перевірити валюту документа!'");
...
"Пересчитать документ в соответствии с договором?';uk='Договір з контрагентом передбачає умови цін і знижок, відмінні від установлених у документі! "
"Перерахувати документ відповідно до договору?'");
...
Процедура ЗаполнитьОснованиеПечатиНаКлиенте(ДоговорПередИзменением, ДанныеНовогоДоговора)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ОснованиеПечатиСсылка)
		...
		Объект.ОснованиеПечати = Нстр("ru='Договор: ';uk='Договір: '") + Строка(ДанныеНовогоДоговора.НаименованиеДоговора);
...
Процедура ВыбранаДисконтнаяКарта(ДисконтнаяКарта)
	...
	Если Объект.Контрагент.Пустая() И Не ВладелецДисконтнойКарты.Пустая() Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ru='Заполнен контрагент и считана дисконтная карта';uk='Заповнений контрагент і зчитана дисконтна картка'"),
			ПолучитьНавигационнуюСсылку(ДисконтнаяКарта),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В документе заполнен контрагент и считана дисконтная карта %1';uk='У документі заповнений контрагент і зчитана дисконтна картка %1'"), ДисконтнаяКарта),
			БиблиотекаКартинок.Информация32);
...
Процедура ПересчитатьПроцентСкидкиПриИзмененииДатыДокумента()
	...
	Если ПредПроцентСкидкиПоДисконтнойКарте <> НовПроцентСкидкиПоДисконтнойКарте Тогда
		...
		Если Объект.Запасы.Количество() > 0 Тогда
			Текст = НСтр("ru = 'Изминть % скидки по накопительной дисконтной карте с "+ПредПроцентСкидкиПоДисконтнойКарте+"% на "+НовПроцентСкидкиПоДисконтнойКарте+"% и перезаполнить скидки во всех строках?'uk='Змінити % знижки за накопичувальною дисконтною карткою з "+ПредПроцентСкидкиПоДисконтнойКарте+"% на "+НовПроцентСкидкиПоДисконтнойКарте+"% та перезаполнити знижки у всіх рядках?'");
		...
		Иначе
			Текст = НСтр("ru = 'Изминть % скидки по накопительной дисконтной карте с "+ПредПроцентСкидкиПоДисконтнойКарте+"% на "+НовПроцентСкидкиПоДисконтнойКарте+"%?';uk='Змінити % знижки за накопичувальною дисконтною карткою з "+ПредПроцентСкидкиПоДисконтнойКарте+"% на "+НовПроцентСкидкиПоДисконтнойКарте+"%?'");
...
Процедура ЗагрузкаИзФайлаЗапасы(Команда)
	...
	НастройкиЗагрузкиДанных.Вставить("Заголовок", НСтр("ru='Загрузка запасов из файла';uk='Завантаження запасів з файлу'"));
...
Процедура ОбработатьПодготовленныеДанные(РезультатЗагрузки)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(Нстр("ru='Загрузка данных';uk='Завантаження даних'"), УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.Номенклатура, , ОписаниеОшибки());
		ОтменитьТранзакцию();
