...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ОприходованиеЗапасов", НСтр("ru='Оприходование запасов';uk='Оприбуткування запасів'"));
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы.Количество() - 1 Цикл
		...
		Если КоличествоТребуетсяРезерв > 0 Тогда
			...
			Если Передача Тогда
				...
				СтрокаТаблицыПриход.СодержаниеПроводки = НСтр("ru='Передача запасов';uk='Передача запасів'"); 
...
Процедура СформироватьТаблицаРасчетыСПоставщиками(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("СторнированиеДолга", НСтр("ru='Сторнирование долга';uk='Сторнування боргу'"));
	Запрос.УстановитьПараметр("ВосстановлениеПредоплаты", НСтр("ru='Восстановление предоплаты';uk='Відновлення передоплати'"));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ru='Курсовая разница';uk='Курсова різниця'"));
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("СторнированиеПредоплаты", НСтр("ru='Сторнирование предоплаты';uk='Сторнування передоплати'"));
	Запрос.УстановитьПараметр("СторнированиеПоставки", НСтр("ru='Сторнирование поставки';uk='Сторнування поставки'"));
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаНДСПродаж(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("Организация", ДокументСсылкаРасходнаяНакладная.Организация);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТаблицаНДСПродаж.Период КАК Период,
	|	&Организация КАК Организация,
	|	СУММА(ТаблицаНДСПродаж.СуммаНДСЗакупкиПродажи) КАК СуммаНДС,
	...
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК ТаблицаНДСПродаж
	|ГДЕ
	|	ТаблицаНДСПродаж.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю)
	|
	|СГРУППИРОВАТЬ ПО
	...
	Для каждого СтрокаТаблицыФормы Из ТаблицаСопоставленияДанных Цикл
		...
		Если ПолучитьФункциональнуюОпцию("УчетГТД") Тогда
			...
			Если ЗначениеЗаполнено(СтрокаТаблицыФормы.СтранаПроисхождения)
				И СтрокаТаблицыФормы.СтранаПроисхождения <> Справочники.СтраныМира.Украина Тогда
...
//
Функция СформироватьАктОбОказанииУслуг(ТабличныйДокумент, ТекущийДокумент, Ошибки, КодЯзыкаПечать)
	...
	|	),
	|	РасходнаяНакладная.Контрагент.ВидКонтрагента КАК ПокупательЮрФизЛицо,
	...
	|	РасходнаяНакладная.Получил КАК ПредставительПокупателя,
	|	РасходнаяНакладная.Отпустил КАК ПредставительПоставщика,
	|	РасходнаяНакладная.ОтпустилДолжность КАК ДолжностьПредставителяПоставщика,
	|	РасходнаяНакладная.Контрагент.КонтактноеЛицо КАК РуководительПокупателя
	|ИЗ
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.РасходнаяНакладная.ПФ_MXL_АктОбОказанииУслуг");
...
Процедура СформироватьТОРГ12(ТабличныйДокумент, ТекущийДокумент, ИмяМакета, Ошибки, КодЯзыкаПечать)
	...
	|		Тогда РасходнаяНакладная.ОснованиеПечатиСсылка.НомерДоговора Иначе РасходнаяНакладная.ОснованиеПечатиСсылка.Номер Конец КАК ОснованиеНомер,
	|	Выбор Когда РасходнаяНакладная.ОснованиеПечатиСсылка ССЫЛКА Справочник.ДоговорыКонтрагентов 
	|		Тогда РасходнаяНакладная.ОснованиеПечатиСсылка.ДатаДоговора Иначе РасходнаяНакладная.ОснованиеПечатиСсылка.Дата Конец КАК ОснованиеДата,
	...
	Если Шапка.Организация = Шапка.Грузоотправитель Тогда
		ОбластьМакетаШапка.Параметры.ПредставлениеОрганизации = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОПоставщике, "ПолноеНаименование,ИНН,ФактическийАдрес,Телефоны,НомерСчета,Банк,МФО,КоррСчет");
	Иначе
		ОбластьМакетаШапка.Параметры.ПредставлениеОрганизации = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОГрузоотправителе, "ПолноеНаименование,ИНН,ФактическийАдрес,Телефоны,НомерСчета,Банк,МФО,КоррСчет");
	КонецЕсли;
	
	ПредставлениеГрузополучателя = "";
	Если ПустаяСтрока(Шапка.АдресДоставки) Тогда
		
		ПредставлениеГрузополучателя = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОГрузополучателе,"ПолноеНаименование,ИНН,ФактическийАдрес,Телефоны,НомерСчета,Банк,МФО,КоррСчет");
	...
	Иначе
		...
		ПредставлениеГрузополучателя = ПредставлениеГрузополучателя + УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОГрузополучателе,"Телефоны,НомерСчета,Банк,МФО,КоррСчет");
...
//
Процедура СформироватьНакладную(ТабличныйДокумент, ТекущийДокумент, ИсключаяУслуги, КодЯзыкаПечать)
	...
	|	РасходнаяНакладная.ДоверенностьНомер,
	|	РасходнаяНакладная.ДоверенностьДата,
	|	РасходнаяНакладная.ДоверенностьЛицо КАК Получил,
	|	ВЫРАЗИТЬ(РасходнаяНакладная.МестоСоставленияДокумента КАК СТРОКА(1000)) КАК МестоСоставленияДокумента
	|ИЗ
	|	Документ.РасходнаяНакладная КАК РасходнаяНакладная
	|ГДЕ
	...
	Пока ВыборкаСтрокЗапасы.Следующий() Цикл
		...
		Если ИсключаяУслуги И (ВыборкаСтрокЗапасы.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга ИЛИ ВыборкаСтрокЗапасы.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Работа) Тогда
			Продолжить;
		КонецЕсли; 
...
//
Процедура СформироватьТоварныйЧек(ТабличныйДокумент, ТекущийДокумент, КодЯзыкаПечать)
	...
	СтруктураЗаполненияСекции.Вставить("ТекстЗаголовка",  НСтр("ru='Товарный чек № ';uk='Товарний чек № '", КодЯзыкаПечать) + НомерДокумента + НСтр("ru=' от ';uk=' від '", КодЯзыкаПечать) + Формат(Шапка.ДатаДокумента, "Л="+КодЯзыкаПечать+"; ДЛФ=DD"));
	...
	ПредставлениеПоставщика = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОбОрганизации, "ПолноеНаименование,ИНН,КодПоЕДРПОУ,ЮридическийАдрес,Телефоны,");
	...
	Пока ВыборкаСтрокЗапасы.Следующий() Цикл
		...
		Иначе
			...
			//ПредставлениеЗапаса = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(ВыборкаСтрокЗапасы.Запас, ВыборкаСтрокЗапасы.Характеристика, ВыборкаСтрокЗапасы.Артикул);
			СтрокаСерийныеНомера = РаботаССерийнымиНомерами.СтрокаСерийныеНомераИзВыборки(ВыборкаСтрокСерийныеНомера, ВыборкаСтрокЗапасы.КлючСвязи);
			ПредставлениеЗапаса = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(ВыборкаСтрокЗапасы.Запас, 
				ВыборкаСтрокЗапасы.Характеристика, ВыборкаСтрокЗапасы.Артикул, СтрокаСерийныеНомера);
	...
	СтруктураЗаполненияСекции.Вставить("ИтоговаяСтрока", 
		НСтр("ru='Всего наименований ';uk='Всього найменувань '", КодЯзыкаПечать) + Строка(Количество) + НСтр("ru=', на сумму ';uk=', на суму '", КодЯзыкаПечать) + УправлениеНебольшойФирмойСервер.ФорматСумм(СуммаКПрописи, Шапка.ВалютаДокумента));
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		Если ИмяМакета = "Накладная" ИЛИ ИмяМакета = "НакладнаяСУслугами" Тогда
			...
			СформироватьНакладную(ТабличныйДокумент, ТекущийДокумент, ИсключаяУслуги, КодЯзыкаПечать);
			
		ИначеЕсли ИмяМакета = "АктОбОказанииУслуг" Тогда
			
			СформироватьАктОбОказанииУслуг(ТабличныйДокумент, ТекущийДокумент, Ошибки, КодЯзыкаПечать)
		...
		ИначеЕсли ИмяМакета = "ТОРГ12" ИЛИ ИмяМакета = "ТОРГ12СУслугами" Тогда
			...
			СформироватьТОРГ12(ТабличныйДокумент, ТекущийДокумент, ИмяМакета, Ошибки, КодЯзыкаПечать);
		...
		ИначеЕсли ИмяМакета = "ТоварныйЧек" Тогда
			...
			СформироватьТоварныйЧек(ТабличныйДокумент, ТекущийДокумент, КодЯзыкаПечать);
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ЭтоСчетНаОплату
		...
		Если ЭтоСчетНаОплату Тогда
			...
			ПредставлениеПФ = НСтр("ru='Счет на оплату';uk='Рахунок на оплату'");
			
		ИначеЕсли ЭтоСчетНаЧастичнуюОплату Тогда
			
			ИмяМакета = "СчетНаЧастичнуюОплату";
			ПредставлениеПФ = НСтр("ru='Счет на частичную оплату';uk='Рахунок на часткову оплату'");
			
		ИначеЕсли ЭтоСчетНаОплатуСФаксимильнойПодписью Тогда
			
			ИмяМакета = "СчетНаОплатуСФаксимильнойПодписью";
			ПредставлениеПФ = НСтр("ru='Счет на оплату (с факсимиле)';uk='Рахунок на оплату (з факсиміле)'");
			
		ИначеЕсли ЭтоСчетНаЧастичнуюОплатуСФаксимильнойПодписью Тогда
			
			ИмяМакета = "СчетНаЧастичнуюОплатуСФаксимильнойПодписью";
			ПредставлениеПФ = НСтр("ru='Счет на частичную оплату (с факсимиле)';uk='Рахунок на часткову оплату (з факсиміле)'")
...
"Документ %1."
"Для формирования печатной формы необходимо либо выбрать документом-основанием счет на оплату, либо указать в табличной части заказ покупателя.';uk='__________________"
"Документ %1."
"Для формування друкованої форми необхідно або вибрати документом-підставою рахунок на оплату, або вказати в табличній частині замовлення покупця.'");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	ЗначениеИдентификатора = "АктОбОказанииУслуг,ТОРГ12,ТОРГ12СУслугами,М15,ТТН,Накладная,НакладнаяСУслугами,%1,ГарантийныйТалон,БланкТоварногоНаполнения";
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства) Экспорт
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)
	|	КОНЕЦ КАК Партия,
	|	РасходнаяНакладнаяЗапасы.Ссылка.Заказ КАК ЗаказПокупателя,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(РасходнаяНакладнаяЗапасы.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
	|			ТОГДА РасходнаяНакладнаяЗапасы.Количество
	|		ИНАЧЕ РасходнаяНакладнаяЗапасы.Количество * РасходнаяНакладнаяЗапасы.ЕдиницаИзмерения.Коэффициент
	|	КОНЕЦ КАК Количество,
	|	ЕСТЬNULL(РасходнаяНакладнаяСерийныеНомера.СерийныйНомер, ЗНАЧЕНИЕ(Справочник.СерийныеНомера.ПустаяСсылка)) КАК СерийныйНомер
	|	ПОМЕСТИТЬ ВременнаяТаблицаРезервированиеТоваровПоСерийныхНомерах
	|ИЗ
	|	Документ.РасходнаяНакладная.Запасы КАК РасходнаяНакладнаяЗапасы
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РасходнаяНакладная.СерийныеНомера КАК РасходнаяНакладнаяСерийныеНомера
	|		ПО РасходнаяНакладнаяЗапасы.Ссылка = РасходнаяНакладнаяСерийныеНомера.Ссылка
	|			И РасходнаяНакладнаяЗапасы.КлючСвязи = РасходнаяНакладнаяСерийныеНомера.КлючСвязи
	|ГДЕ
	|	РасходнаяНакладнаяЗапасы.Ссылка = &Ссылка
	|	И РасходнаяНакладнаяЗапасы.гвЗарезервировано";
	// ...Гунько 2021-01-09
	...
	СформироватьТаблицаНДСПродаж(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства);
	...
	// Гунько 2021-01-09
	гвСформироватьТаблицаРезервированиеТоваровПоСерийныхНомерах(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства);
	// ...Гунько 2021-01-09
...
// Гунько 2021-01-03
Функция гвВыполнитьЗапросКонтроляОстатковИРезервированияСерийныхНомеров(ДокументОбъектРасходнаяНакладная)
	...
	|ГДЕ
	|	гвРезервированиеТоваровПоСерийныхНомерахОстатки.ЗаказПокупателя <> &ЗаказПокупателя";
	
	Запрос.УстановитьПараметр("Ссылка", ДокументОбъектРасходнаяНакладная.Ссылка);
	Запрос.УстановитьПараметр("ЗаказПокупателя", ДокументОбъектРасходнаяНакладная.Заказ);
	Запрос.УстановитьПараметр("Организация", ДокументОбъектРасходнаяНакладная.Организация);
	
	Возврат Запрос.Выполнить();	
		
КонецФункции

Процедура гвВыполнитьКонтрольРезервирования(ДокументОбъектРасходнаяНакладная, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	
	лРезультатЗапроса = гвВыполнитьЗапросКонтроляОстатковИРезервированияСерийныхНомеров(ДокументОбъектРасходнаяНакладная);
	
	// Серийные номера ранее зарезервированные другими заказами
	Если НЕ лРезультатЗапроса.Пустой() Тогда
		лВыборкаРезультат = лРезультатЗапроса.Выбрать();
...
Процедура гвСформироватьТаблицаРезервированиеТоваровПоСерийныхНомерах(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВременнаяТаблицаРезервированиеТоваровПоСерийныхНомерах.НомерСтроки,
	|	ВременнаяТаблицаРезервированиеТоваровПоСерийныхНомерах.ВидДвижения,
	|	ВременнаяТаблицаРезервированиеТоваровПоСерийныхНомерах.Период,
	|	ВременнаяТаблицаРезервированиеТоваровПоСерийныхНомерах.Организация,
	|	ВременнаяТаблицаРезервированиеТоваровПоСерийныхНомерах.Номенклатура,
	|	ВременнаяТаблицаРезервированиеТоваровПоСерийныхНомерах.Характеристика,
	|	ВременнаяТаблицаРезервированиеТоваровПоСерийныхНомерах.Партия,
	|	ВременнаяТаблицаРезервированиеТоваровПоСерийныхНомерах.ЗаказПокупателя,
	|	ВременнаяТаблицаРезервированиеТоваровПоСерийныхНомерах.Количество,
	|	ВременнаяТаблицаРезервированиеТоваровПоСерийныхНомерах.СерийныйНомер
	|ИЗ
	|	ВременнаяТаблицаРезервированиеТоваровПоСерийныхНомерах КАК ВременнаяТаблицаРезервированиеТоваровПоСерийныхНомерах";
	
	РезультатЗапроса = Запрос.Выполнить();
	
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаРезервированиеТоваровПоСерийныхНомерах", РезультатЗапроса.Выгрузить());
