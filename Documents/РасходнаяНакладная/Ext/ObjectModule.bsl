...
Процедура ЗаполнитьПоСтруктуре(ДанныеЗаполнения) Экспорт
	...
	Если ДанныеЗаполнения.Свойство("Основание")
		...
		Тогда
		ВызватьИсключение НСтр("ru='Нельзя ввести Расходную накладную на основании заказ-наряда!';uk='Не можна ввести Видаткову накладну на підставі замовлення-наряду!'");
	КонецЕсли;
...
Процедура ЗаполнитьПоПриходнойНакладной(ДанныеЗаполнения, Операция = "") Экспорт
	...
	Если Операция = "Продажа" Тогда
		...
		Иначе
			...
			НСтр("ru='Невозможен ввод операции ""Продажа покупателю"" на основании операции - ""%1""!';uk='Неможливе введення операції ""Продаж покупцеві"" на підставі операції - ""%1""!'"),
	...
	ИначеЕсли Операция = "Возврат" Тогда
		...
		Иначе
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Невозможен ввод операции ""Возврат"" на основании операции - ""%1""!';uk='Неможливе введення операції ""Повернення"" на підставі операції - ""%1""!'"),
			ДанныеЗаполнения.ВидОперации);
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьПоЗаказуПокупателя(ДанныеЗаполнения) Экспорт
	...
	|	ЗаказПокупателяЗапасы.гвСерийныйНомер.Наименование КАК СерийныйНомерНаименование,
	|	ЗаказПокупателяЗапасы.гвЗарезервировано
	// ...Гунько 2021-01-09
	|ИЗ
	...
	Если ТаблицаОстатков.Количество() > 0 Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			// Гунько 2021-01-09
			НоваяСтрока.СерийныеНомера = Выборка.СерийныйНомерНаименование;
			НоваяСтрока.КлючСвязи = Выборка.НомерСтроки;
			
			лНоваяСтрокаСерийныеНомера = СерийныеНомера.Добавить();
			лНоваяСтрокаСерийныеНомера.СерийныйНомер = Выборка.СерийныйНомер;
			лНоваяСтрокаСерийныеНомера.КлючСвязи = Выборка.НомерСтроки;			
			// ...Гунько 2021-01-09
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// Проверка блокировки изменения вида цен для пользователей со специальными ценами
	ПроверитьБлокировкуИзменениявидаЦен(Отказ);
	...
	Если АвторасчетНДС Тогда
		...
		УчетНДССервер.ПересчитатьНДСсУчетомПогрешностиОкругления(Запасы, ЭтотОбъект, ПогрешностиОкругления, "Запасы и услуги");
	КонецЕсли;
	...
	Для каждого Строка Из ТаблицаПредоплата Цикл
		...
		Если НайденнаяСтрокаЗапасы = Неопределено
		   ...
		   И Контрагент.ВестиРасчетыПоЗаказам Тогда
			ТекстСообщения = НСтр("ru='Нельзя зачесть аванс по заказу отличному от указанных в табличных частях ""Запасы"" или ""Расходы""!';uk='Не можна зарахувати аванс по замовленню відмінному від зазначених у табличних частинах ""Запаси"" або ""Витрати""!'");
	...
	Если Константы.ФункциональнаяОпцияРезервированиеЗапасов.Получить()
		...
		Для каждого СтрокаЗапасы Из Запасы Цикл
			...
			Если СтрокаЗапасы.Резерв > СтрокаЗапасы.Количество Тогда
				...
				ТекстСообщения = НСтр("ru='В строке №%Номер% табл. части ""Запасы и услуги"" количество отгружаемых позиций из резерва превышает общее количество запасов.';uk='У рядку №%Номер% табл. частини ""Запаси й послуги"" кількість позицій, що відвантажуються з резерву, перевищує загальну кількість запасів.'");
	...
	Если ЕстьРучныеСкидки ИЛИ ЕстьАвтоматическиеСкидки Тогда
		Для каждого СтрокаЗапасы Из Запасы Цикл
			...
			Если СтрокаЗапасы.ПроцентСкидкиНаценки <> 100 И ТекСуммаСкидки < ТекСумма
				...
				ТекстСообщения = НСтр("ru='Не заполнена колонка ""Сумма"" в строке %Номер% списка ""Запасы"".';uk='Не заповнений стовпчик ""Сума"" у рядку %Номер% списку ""Запаси"".'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", СтрокаЗапасы.НомерСтроки);
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
					ЭтотОбъект,
					ТекстСообщения,
		...
		Для Каждого Стр Из Запасы Цикл
			Если НЕ УправлениеНебольшойФирмойСервер.РазрешенаПродажаНоменклатурыПоДоговоруОбслуживания(Договор, Стр.Номенклатура, Стр.Характеристика) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Запрещено проводить незапланированные товары/услуги по текущему договору обслуживания!';uk='Заборонено проводити незаплановані товари/послуги за поточним договором обслуговування!'"),
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// Гунько 2020-04-14
	гвКонтрольДублейСерийныхНомеров(Отказ);
	//гвКонтрольСерийныеНомераПроданыРанее(Отказ);
	// ...Гунько 2020-04-14
	...
	// Гунько 2021-01-09
	УправлениеНебольшойФирмойСервер.гвОтразитьРезервированиеТоваровПоСерийныхНомерах(ДополнительныеСвойства, Движения, Отказ);
	// ...Гунько 2021-01-09
	...
	// Гунько 2021-01-09
	Документы.РасходнаяНакладная.гвВыполнитьКонтрольРезервирования(ЭтотОбъект, ДополнительныеСвойства, Отказ);
	// ...Гунько 2021-01-09
...
// Гунько 2020-04-14
Процедура гвКонтрольДублейСерийныхНомеров(Отказ)
	
	лТаблицаСерийныхНомеров = СерийныеНомера.Выгрузить(, "СерийныйНомер");
	лТаблицаСерийныхНомеров.Свернуть("СерийныйНомер");
	
	Если НЕ лТаблицаСерийныхНомеров.Количество() = СерийныеНомера.Количество() Тогда
		Отказ = Истина;
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, НСтр("ru='В документе есть дубли серийных номеров.';uk='В документі є дублі серіних номерів.'"),,, "СерийныеНомера", Отказ);
	КонецЕсли;	
	
КонецПроцедуры

Процедура гвКонтрольСерийныеНомераПроданыРанее(Отказ)
	...
	|ПОМЕСТИТЬ ВТ_ПродажиИВозвраты
	|ИЗ
	|	РегистрНакопления.Продажи КАК Продажи
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РасходнаяНакладная.СерийныеНомера КАК РасходнаяНакладнаяСерийныеНомера
	|		ПО Продажи.Документ = РасходнаяНакладнаяСерийныеНомера.Ссылка
	|ГДЕ
	|	РасходнаяНакладнаяСерийныеНомера.СерийныйНомер В(&МассивСерийныхНомеров)
	|	И НЕ РасходнаяНакладнаяСерийныеНомера.Ссылка = &Ссылка
	|	И Продажи.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Справочник.ХозяйственныеОперации.ПродажаПокупателю)
	...
	|ИЗ
	|	РегистрНакопления.Продажи КАК Продажи
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПриходнаяНакладная.СерийныеНомера КАК ПриходнаяНакладнаяСерийныеНомера
	|		ПО Продажи.Документ = ПриходнаяНакладнаяСерийныеНомера.Ссылка
	|ГДЕ
	|	ПриходнаяНакладнаяСерийныеНомера.СерийныйНомер В(&МассивСерийныхНомеров)
	|	И Продажи.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Справочник.ХозяйственныеОперации.ВозвратОтПокупателя)
	|	И Продажи.Активность
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ПродажиИВозвраты.СерийныйНомер КАК СерийныйНомер,
	|	СУММА(ВТ_ПродажиИВозвраты.Количество) КАК Количество
	|ПОМЕСТИТЬ ВТ_ПродажиИВозвратыСгрупированная
	|ИЗ
	|	ВТ_ПродажиИВозвраты КАК ВТ_ПродажиИВозвраты
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ПродажиИВозвраты.СерийныйНомер
	|
	|ИМЕЮЩИЕ
	|	СУММА(ВТ_ПродажиИВозвраты.Количество) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ВТ_ПродажиИВозвратыСгрупированная.СерийныйНомер) КАК СерийныйНомер,
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(РасходнаяНакладнаяСерийныеНомера.Ссылка) КАК Документ
	|ИЗ
	|	ВТ_ПродажиИВозвратыСгрупированная КАК ВТ_ПродажиИВозвратыСгрупированная
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РасходнаяНакладная.СерийныеНомера КАК РасходнаяНакладнаяСерийныеНомера
	|		ПО ВТ_ПродажиИВозвратыСгрупированная.СерийныйНомер = РасходнаяНакладнаяСерийныеНомера.СерийныйНомер
	|ГДЕ
	|	РасходнаяНакладнаяСерийныеНомера.Ссылка.Проведен
	|	И НЕ РасходнаяНакладнаяСерийныеНомера.Ссылка = &Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	РасходнаяНакладнаяСерийныеНомера.Ссылка.Дата УБЫВ";
	
	Запрос.УстановитьПараметр("МассивСерийныхНомеров", СерийныеНомера.ВыгрузитьКолонку("СерийныйНомер"));
	Запрос.УстановитьПараметр("Ссылка", Ссылка);
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		Выборка = РезультатЗапроса.Выбрать();
		лСтрокаСообщение = НСтр("ru='Следующие серийные номера уже проданы.';uk='Наступні серійні номери вже продані.'") + Символы.ПС;
		Если Выборка.Следующий() Тогда
			лСтрокаСообщение = лСтрокаСообщение + Выборка.Документ + " - " + Выборка.СерийныйНомер + Символы.ПС; 
		КонецЕсли;
		Отказ = Истина;
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, лСтрокаСообщение,,, "СерийныеНомера", Отказ);
	КонецЕсли;	
...
// Процедура проверяет, заблокирована ли возможность изменения вида цен для текущего пользователя
Процедура ПроверитьБлокировкуИзменениявидаЦен(Отказ)
	
	// Получаем текущего пользователя из системы
	...
	// Проверяем, заблокирована ли возможность изменения вида цен
	БлокировкаУстановлена = Ложь;
	Попытка
		БлокировкаУстановлена = ТекущийПользователь.БлокировкаИзменяемостиВидаЦен;
	Исключение
		// Если атрибута нет, то блокировки нет
		Возврат;
	КонецПопытки;
	...
	// Если это новый документ или ВидЦен не менялся, пропускаем проверку
	Если НЕ ЗначениеЗаполнено(Ссылка) Тогда
		Возврат;
	КонецЕсли;
	
	// Проверяем, изменилось ли значение ВидЦен
	СтарыеДанные = Ссылка.ПолучитьОбъект();
	
	Если СтарыеДанные.ВидЦен <> ВидЦен Тогда
		// Пользователь пытается изменить вид цен, но это запрещено
		СообщениеОбОшибке = НСтр("ru='Вам не разрешено изменять вид цен в документах. Обратитесь к администратору.';uk='Вам не дозволено змінювати вид цін у документах. Зверніться до адміністратора.'");
