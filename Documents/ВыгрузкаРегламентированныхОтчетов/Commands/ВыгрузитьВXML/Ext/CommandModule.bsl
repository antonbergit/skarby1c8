...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	Для Индекс = 0 ПО ПараметрКоманды.ВГраница() Цикл
		...
		Если мВыбраннаяФорма = Неопределено Тогда
			Сообщить(НСтр("ru='Выгрузка документа ';uk='Вивантаження документа '") + Документ + НСтр("ru=' невозможна - отчет ';uk=' неможливе - звіт '") + ИмяОтчета + НСтр("ru=' не поддерживает выгрузку по форме, действующей, в указанный период!';uk=' не підтримує вивантаження за формою, що діє, в зазначений період!'"));
		    Продолжить;
		КонецЕсли;
...
Функция ПолучитьПараметрыДокументов(МассивДокументов)
	...
	Для Индекс = 0 ПО МассивДокументов.ВГраница() Цикл
		...
		ВыгрузкаПечатьНН_ПоФормеНаДатуДокумента = Константы.ВыгрузкаПечатьНН_ПоФормеНаДатуДокумента.Получить(); 
