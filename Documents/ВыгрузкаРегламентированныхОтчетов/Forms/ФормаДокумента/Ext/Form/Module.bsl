		...
		Для каждого ВыгружаемоеПоле Из ТипОбъектаВыгрузки.Свойства Цикл
			...
			ИначеЕсли НЕ ЭтоЯчейкаТаблицы Тогда
				...
				Если Обязательное И НЕ ЗначениеЗаполнено(ЗначениеXML) Тогда
					...
					СтрокаОшибки.Графа 		= "";
					СтрокаОшибки.Описание 	= НСтр("ru = 'Показатель <" + Имя + "> должен быть заполнен!'");
				...
				ИначеЕсли ЗначениеЗаполнено(ЗначениеXML) Тогда		
					...
					Исключение	
						...
						СтрокаОшибки.ИмяЯчейки 	= Имя;
						СтрокаОшибки.Графа 		= "";
						СтрокаОшибки.Описание 	= НСтр("ru = 'Неверное значение показателя  <" + Имя + ">: " + ЗначениеXML + ". " + ОписаниеВозможнойОшибки + "'");
			...
			Иначе // это табличный показатель
				...
				Если Инд > ВыгружаемоеПоле.ВерхняяГраница + 1 Тогда
					...
					СтрокаОшибки.ИмяЯчейки 	= Имя;
					СтрокаОшибки.Графа 		= "";
					СтрокаОшибки.Описание 	= НСтр("ru = 'Превышено максимальное количество строк (" + (ВыгружаемоеПоле.ВерхняяГраница + 1) + ")!'");
				...
				Если ЕстьОшибка Тогда
					...
					Для каждого ПоказательГруппы Из Группа.Строки Цикл
						...
						Если ПоказательГруппы.Строки.Количество() = 0 Тогда
							...
							СтрокаОшибки.Графа 		= "";
							СтрокаОшибки.Описание 	= НСтр("ru = 'Не верно заполнена группа показателей: " + СписокРеквизитов + " (или ни один или несколько вместо одного)'");
						...
						Иначе
							...
							Для каждого ПоследовательныйРеквизит Из ПоказательГруппы.Строки Цикл
								...
								СтрокаОшибки.Графа 		= "";
								СтрокаОшибки.Описание 	= НСтр("ru = 'Не верно заполнена группа показателей: " + СписокРеквизитов + " (не заполнен ни один либо сразу несколько вместо одного)'");
