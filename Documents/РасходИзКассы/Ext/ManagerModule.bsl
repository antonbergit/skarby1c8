...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("СодержаниеВыдачаЗаймаСотруднику", НСтр("ru='Выдача займа сотруднику';uk='Видача позики співробітникові'"));
	Запрос.УстановитьПараметр("СодержаниеПеремещениеВКассуККМ", НСтр("ru='Перемещение денежных средств в кассу ККМ';uk='Переміщення грошових коштів у касу ККМ'"));
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства) Экспорт
	...
	Если ДокументСсылкаРасходИзКассы.УчитыватьВНУ Тогда
		СформироватьТаблицаКнигаДоходовРасходовПоЕН(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства);
	КонецЕсли;
	// Биллинг
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		Если ТекущийДокумент.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Поставщику Тогда
			СтрокаДебетСубСчет = "РасходИзКассы.Контрагент.СчетУчетаРасчетовСПоставщиком.Код";
		ИначеЕсли ТекущийДокумент.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Подотчетнику Тогда
			СтрокаДебетСубСчет = "РасходИзКассы.Подотчетник.СчетРасчетовСПодотчетниками.Код";
		ИначеЕсли ТекущийДокумент.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Налоги Тогда
			СтрокаДебетСубСчет = "РасходИзКассы.ВидНалога.СчетУчета.Код";
		ИначеЕсли ТекущийДокумент.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Покупателю Тогда
			СтрокаДебетСубСчет = "РасходИзКассы.Контрагент.СчетУчетаРасчетовСПокупателем.Код";
		ИначеЕсли ТекущийДокумент.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Прочее ИЛИ
			...
			СтрокаДебетСубСчет = "РасходИзКассы.Корреспонденция.Код";
		...
		ИначеЕсли ТекущийДокумент.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.ПеремещениеВКассуККМ Тогда
			СтрокаДебетСубСчет = "РасходИзКассы.КассаККМ.СчетУчета.Код";
		ИначеЕсли ТекущийДокумент.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.ЗарплатаСотруднику Тогда
			СтрокаДебетСубСчет = "РасходИзКассы.Подотчетник.СчетРасчетовСПерсоналом.Код";
		ИначеЕсли ТекущийДокумент.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Зарплата Тогда
			...
			Если РезультатЗапроса.Пустой() Тогда
				СтрокаДебетСубСчет = "НЕОПРЕДЕЛЕНО";
			Иначе
		...
		|	РасходИзКассы.НомерОрдера КАК НомерОрдера,
		...
		|	РасходИзКассы.Организация.КодПоЕДРПОУ КАК КодПоЕДРПОУ,
		|	РасходИзКассы.Организация.НаименованиеПолное КАК ПолноеНаименование,
		...
		|	РасходИзКассы.ПоДокументу КАК ПоДокументу,
		|	" + СтрокаДебетСубСчет + "   КАК ДебетСубСчет,
		...
		Если ВыборкаДляПечати.ВалютаДенежныхСредств = Константы.НациональнаяВалюта.Получить() Тогда
			ОбластьМакета.Параметры.СуммаПрописьюПолучил = "грн.      коп."; 
		Иначе
			ОбластьМакета.Параметры.СуммаПрописьюПолучил = "";
		КонецЕсли;
		ОбластьМакета.Параметры.НомерОрдера = ВыборкаДляПечати.НомерОрдера;
		...
		Руководители = УправлениеНебольшойФирмойСервер.ОтветственныеЛицаОрганизационнойЕдиницы(ВыборкаДляПечати.Организация, ТекущийДокумент.Ссылка.ПолучитьОбъект().МоментВремени());
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='КО-2 (Расходный кассовый ордер)';uk='КО-2 (Видатковий касовий ордер)'");
