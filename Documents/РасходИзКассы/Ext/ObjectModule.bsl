...
Процедура ЗаполнитьПоРасходДСПлан(ДокументСсылкаРасходДСПлан, Сумма = Неопределено) Экспорт
	...
	Если ДокументСсылкаРасходДСПлан.ТипДенежныхСредств = Перечисления.ТипыДенежныхСредств.Безналичные Тогда
		ВызватьИсключение НСтр("ru='Нельзя ввести расход денег. В заявке указан неверный способ оплаты (тип денежных средств)!';uk='Не можна ввести витрату грошей. У заявці зазначений невірний спосіб оплати (тип коштів)!'");
	КонецЕсли;
...
Процедура ЗаполнитьПоПеремещениеДСПлан(ДокументСсылкаПеремещениеДСПлан, Сумма = Неопределено) Экспорт
	...
	"ВЫБРАТЬ
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(&Ссылка) КАК Основание,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходИзКассы.Прочее) КАК ВидОперации,
	|	&Ссылка КАК ДокументОснование,
	|	ТаблицаДокумента.Организация КАК Организация,
...
Процедура ЗаполнитьПоНачислениеНалогов(ДокументСсылкаНачислениеНалогов) Экспорт
	...
	Если ДокументСсылкаНачислениеНалогов.ВидОперации <> Перечисления.ВидыОперацийНачислениеНалогов.Начисление Тогда
		ВызватьИсключение НСтр("ru='Расход из кассы можно ввести только на основании начисления налогов, а не возмещения.';uk='Витрату з каси можна ввести тільки на підставі нарахування податків, а не відшкодування.'");
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ ЗначениеЗаполнено(НомерОрдера) Тогда
...
Процедура УстановитьНомерОрдера() 
	...
	|	И РасходИзКассы.Касса = &Касса	
	|	И РасходИзКассы.ВалютаДенежныхСредств = &ВалютаДенежныхСредств		
	|	И РасходИзКассы.Дата >= &ДатаНачалаГода
	|	И РасходИзКассы.Дата <= &ДатаКонцаГода";
