...
Процедура СформироватьТаблицаЗапасыПринятые(ДокументСсылкаОтчетОРозничныхПродажах, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПриемЗапасовТоварыНаКомиссии", НСтр("ru='Реализация комиссионных товаров';uk='Реалізація комісійних товарів'"));
...
Процедура СформироватьТаблицаДенежныеСредстваВКассахККМ(ДокументСсылкаОтчетОРозничныхПродажах, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("ПолноеПроведение", СтруктураДополнительныеСвойства.ДляПроведения.ПолноеПроведение);
	Запрос.УстановитьПараметр("ПоступлениеДенежныхСредств", НСтр("ru='Поступление денежных средств в кассу ККМ';uk='Надходження грошових коштів у касу ККМ'"));
	Запрос.УстановитьПараметр("ОплатаПлатежнымиКартами", НСтр("ru='Оплата платежными картами';uk='Оплата платіжними картами'"));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ru='Курсовая разница';uk='Курсова різниця'"));
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаОтчетОРозничныхПродажах, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ОтражениеОплатыПлатежнымиКартами", НСтр("ru='Оплата платежными картами';uk='Оплата платіжними картами'"));
...
Процедура СформироватьТаблицаРозничныеПродажиПодакцизныхТоваров(ДокументСсылкаОтчетОРозничныхПродажах, СтруктураДополнительныеСвойства)
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТаблицаРозничныеПродажиПодакцизныхТоваров.Дата КАК Период,
	|	ТаблицаРозничныеПродажиПодакцизныхТоваров.Организация КАК Организация,
	|	ТаблицаРозничныеПродажиПодакцизныхТоваров.СтруктурнаяЕдиница КАК Склад,
	|	ВЫБОР
	|		КОГДА ТаблицаРозничныеПродажиПодакцизныхТоваров.СуммаВключаетНДС
	|			ТОГДА ТаблицаРозничныеПродажиПодакцизныхТоваров.Сумма - ТаблицаРозничныеПродажиПодакцизныхТоваров.СуммаАкцизногоНалога
	|		ИНАЧЕ ТаблицаРозничныеПродажиПодакцизныхТоваров.Сумма + ТаблицаРозничныеПродажиПодакцизныхТоваров.СуммаНДС
	|	КОНЕЦ КАК БазаАкцизногоНалога,
	|	ТаблицаРозничныеПродажиПодакцизныхТоваров.СуммаАкцизногоНалога,
	|	ТаблицаРозничныеПродажиПодакцизныхТоваров.ПодакцизныеТоварыДляКоммерческогоИспользования КАК ДляКоммерческогоИспользования,
	|	ТаблицаРозничныеПродажиПодакцизныхТоваров.СтатьяДекларацииПоАкцизномуНалогу
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК ТаблицаРозничныеПродажиПодакцизныхТоваров
	|ГДЕ
	|	ТаблицаРозничныеПродажиПодакцизныхТоваров.СтатьяДекларацииПоАкцизномуНалогу <> ЗНАЧЕНИЕ(Справочник.СтатьиНалоговыхДеклараций.ПустаяСсылка)";
	
	РезультатЗапроса = Запрос.Выполнить();
	
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаРозничныеПродажиПодакцизныхТоваров", РезультатЗапроса.Выгрузить());
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаКнигаДоходовРасходовПоЕН(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	|	ТаблицаЗапасы.Дата КАК Период,
	|	ТаблицаЗапасы.Организация КАК Организация,
	|	ТаблицаЗапасы.СтатьяДекларацииПоЕдиномуНалогу КАК Статья,
	|	СУММА(ТаблицаЗапасы.Сумма) КАК Сумма
	|ИЗ
	|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
	|ГДЕ
	|	ТаблицаЗапасы.СтатьяДекларацииПоЕдиномуНалогу <> ЗНАЧЕНИЕ(Справочник.СтатьиНалоговыхДеклараций.ПустаяСсылка)
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаЗапасы.Дата,
	|	ТаблицаЗапасы.Организация,
	|	ТаблицаЗапасы.СтатьяДекларацииПоЕдиномуНалогу";
	
	РезультатЗапроса = Запрос.Выполнить();
	
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаКнигаДоходовРасходовПоЕН", РезультатЗапроса.Выгрузить());
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаОтчетОРозничныхПродажах, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ОтчетОРозничныхПродажахЗапасы.Ссылка.СуммаВключаетНДС КАК СуммаВключаетНДС,
	|	ОтчетОРозничныхПродажахЗапасы.СтатьяДекларацииПоАкцизномуНалогу,
	|	ОтчетОРозничныхПродажахЗапасы.СуммаАкцизногоНалога,
	|	ОтчетОРозничныхПродажахЗапасы.ПодакцизныеТоварыДляКоммерческогоИспользования,
	|	ОтчетОРозничныхПродажахЗапасы.Ссылка.СтатьяДекларацииПоЕдиномуНалогу КАК СтатьяДекларацииПоЕдиномуНалогу, 
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ОтчетОРозничныхПродажахЗапасы.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
	...
	СформироватьТаблицаРозничныеПродажиПодакцизныхТоваров(ДокументСсылкаОтчетОРозничныхПродажах, СтруктураДополнительныеСвойства);
	
	Если ДокументСсылкаОтчетОРозничныхПродажах.УчитыватьВНУ Тогда
		СформироватьТаблицаКнигаДоходовРасходовПоЕН(ДокументСсылкаОтчетОРозничныхПродажах, СтруктураДополнительныеСвойства);
	КонецЕсли;
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыНаСкладахИзменение
	 ...
	 ИЛИ СтруктураВременныеТаблицы.ДвиженияСерийныеНомераИзменение Тогда
		...
		Если НЕ МассивРезультатов[0].Пустой() Тогда
			...
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыНаСкладах(ДокументОтчетОРозничныхПродажах, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;

		// Отрицательный остаток учета запасов.
		Если НЕ МассивРезультатов[1].Пустой() Тогда
			ВыборкаИзРезультатаЗапроса = МассивРезультатов[1].Выбрать();
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасы(ДокументОтчетОРозничныхПродажах, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
		
		// Отрицательный остаток запасов принятых.
		Если НЕ МассивРезультатов[2].Пустой() Тогда
			ВыборкаИзРезультатаЗапроса = МассивРезультатов[2].Выбрать();
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗапасыПринятые(ДокументОтчетОРозничныхПродажах, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
		
		// Отрицательный остаток по заказу покупателя.
		Если НЕ МассивРезультатов[3].Пустой() Тогда
			ВыборкаИзРезультатаЗапроса = МассивРезультатов[3].Выбрать();
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструЗаказыПокупателей(ДокументОтчетОРозничныхПродажах, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
		...
		Если НЕ МассивРезультатов[4].Пустой() Тогда
			...
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструСерийныеНомера(ДокументОтчетОРозничныхПродажах, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
...
Функция ОткрытьКассовуюСмену(КассаККМ, ОписаниеОшибки = "") Экспорт
	...
	Если СтруктураСостояниеКассовойСмены.СтатусКассовойСмены = Перечисления.СтатусыКассовойСмены.Открыта Тогда
		...
		Иначе
			...
			ОписаниеОшибки = НСтр("ru='С момента открытия кассовой смены истекло более 24 часов. Необходимо выполнить закрытие кассовой смены.';uk='З моменту відкриття касової зміни минуло більш 24 годин. Необхідно виконати закриття касової зміни.'");
	...
	Иначе
		...
		Иначе
			ВыполненоУспешно = Ложь;
			ОписаниеОшибки = НСтр("ru='Проверьте настройки розничного склада и кассы ККМ.';uk='Перевірте налаштування роздрібного складу й каси ККМ.'");
		КонецЕсли;
...
//
Функция ЗакрытьКассовуюСмену(КассоваяСменаОбъект) Экспорт
	...
	Попытка
		...
		УдалитьДвиженияПоРегиструЗапасыВПробитыхЧекахККМ(КассоваяСменаОбъект);
		...
		КассоваяСменаОбъект.Запасы.Очистить();
		КассоваяСменаОбъект.ОплатаПлатежнымиКартами.Очистить();
		
		Если Запасы.Количество() > 0 Тогда
			КассоваяСменаОбъект.ПоложениеОтветственный = ?(
		...
		Для каждого СтрокаТЧ Из Запасы Цикл
			...
			Если СтрокаТЧ.Всего <> 0 ИЛИ СтрокаТЧ.ПроцентСкидкиНаценки >= 100 
				...
				Тогда
				СтрокаТабличнойЧастиЗапасы = КассоваяСменаОбъект.Запасы.Добавить();
		...
		Для каждого СтрокаТЧ Из ОплатаПлатежнымиКартами Цикл
			...
			Если СтрокаТЧ.Сумма <> 0 Тогда
				СтрокаТабличнойЧасти = КассоваяСменаОбъект.ОплатаПлатежнымиКартами.Добавить();
		...
		// АвтоматическиеСкидки
		КассоваяСменаОбъект.СкидкиНаценки.Очистить();
		Если ПолучитьФункциональнуюОпцию("ИспользоватьАвтоматическиеСкидкиНаценки") Тогда
			...
			Для каждого СтрокаТЧ Из АвтоматическиеСкидки Цикл
				...
				Если СтрокаТЧ.Сумма <> 0 Тогда
					СтрокаТабличнойЧасти = КассоваяСменаОбъект.СкидкиНаценки.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТабличнойЧасти, СтрокаТЧ);
				КонецЕсли;
		...
		// Серийные номера
		КассоваяСменаОбъект.СерийныеНомера.Очистить();
		РаботаССерийнымиНомерамиКлиентСервер.ЗаполнитьКлючиСвязиВТабличнойЧастиТовары(КассоваяСменаОбъект, "Запасы");
		Если ПолучитьФункциональнуюОпцию("ИспользоватьСерийныеНомера") Тогда
			
			СерийныеНомера = Результат[5].Выгрузить();
			Для каждого СтрокаТЧ Из КассоваяСменаОбъект.Запасы Цикл
				...
				Если СерийныеНомераПоОтбору.Количество()>0 Тогда
					...
					Для каждого стр Из СерийныеНомераПоОтбору Цикл
						НовСтр = КассоваяСменаОбъект.СерийныеНомера.Добавить();
						НовСтр.КлючСвязи = КлючСвязи;
						НовСтр.СерийныйНомер = стр.СерийныйНомер;
					КонецЦикла;
				КонецЕсли;
				
				РаботаССерийнымиНомерамиКлиентСервер.ОбновитьСтроковоеПредставлениеСерийныхНомеровСтроки(СтрокаТЧ, КассоваяСменаОбъект, "КлючСвязи");
		...
		КассоваяСменаОбъект.СуммаДокумента         = КассоваяСменаОбъект.Запасы.Итог("Всего");
		
		Если Запасы.Количество() > 0 Тогда
			КассоваяСменаОбъект.Ответственный = Запасы[0].Ответственный;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(КассоваяСменаОбъект.Ответственный) Тогда
			КассоваяСменаОбъект.Ответственный = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(Пользователи.ТекущийПользователь(), "ОсновнойОтветственный");
		КонецЕсли;
		...
		СтруктураВозвращаемоеЗначение.ОтчетОРозничныхПродажах = КассоваяСменаОбъект.Ссылка;
	...
	Исключение
		...
		СтруктураВозвращаемоеЗначение.ОтчетОРозничныхПродажах = Неопределено;
		СтруктураВозвращаемоеЗначение.ОписаниеОшибки = НСтр("ru='При формировании отчета о розничных продажах произошла ошибка."
"Закрытие кассовой смены не выполнено.';uk='При формуванні звіту про роздрібні продажі відбулася помилка."
"Закриття касової зміни не виконане.'")
			+?(ВыполняетсяОбработкаЧеков, НСтр("ru=' Ошибка произошла при обработке чеков ККМ.';uk=' Помилка сталася при обробці чеків ККМ.'"), "");
...
"%ДополнительноеОписание%';uk='При видаленні відкладених чеків відбулася помилка."
"Додатковий опис:"
...
"Дополнительное описание:"
"%ДополнительноеОписание%';uk='При архівації чеків ККМ відбулася помилка."
...
//
Функция ЗакрытьКассовуюСменуВыполнитьАрхивацию(КассаККМ, ОписаниеОшибки = "") Экспорт
	...
	Если СтруктураСостояниеКассовойСмены.СтатусКассовойСмены = Перечисления.СтатусыКассовойСмены.Открыта Тогда
		...
		СтруктураВозвращаемоеЗначение = Документы.ОтчетОРозничныхПродажах.ЗакрытьКассовуюСмену(КассоваяСменаОбъект);
		...
		Иначе
			...
			МассивДокументов.Добавить(СтруктураВозвращаемоеЗначение.ОтчетОРозничныхПродажах);
			
			Если Константы.УдалятьНепробитыеЧекиПриЗакрытииКассовойСмены.Получить() Тогда
				УдалитьОтложенныеЧеки(СтруктураСостояниеКассовойСмены.КассоваяСмена.ПолучитьОбъект(), ОписаниеОшибки);
			КонецЕсли;
...
//  Структура - 
//
Функция НовоеОписаниеКассовойСмены(КассаККМ)
	
	Результат = Новый Структура;
	Результат.Вставить("ДатаИзмененияСтатуса");
	Результат.Вставить("СтатусКассовойСмены");
	Результат.Вставить("КассоваяСмена");
	Результат.Вставить("НаличностьВКассе");
	Результат.Вставить("НомерКассовойСмены");
	Результат.Вставить("СменаОткрыта", Ложь);
	
	// Описание реквизитов кассовой смены
	// Включает значения по-умолчанию
	Результат.Вставить("КассаККМ", КассаККМ);
	Результат.Вставить("ВалютаДокумента", КассаККМ.ВалютаДенежныхСредств);
	Результат.Вставить("ВалютаДокументаПредставление");
	Если ЗначениеЗаполнено(КассаККМ.СтруктурнаяЕдиница.РозничныйВидЦен) Тогда
		Результат.Вставить("ВидЦен", КассаККМ.СтруктурнаяЕдиница.РозничныйВидЦен);
	Иначе
		Результат.Вставить("ВидЦен", Справочники.ВидыЦен.Оптовая);
	КонецЕсли;
	Результат.Вставить("Организация", КассаККМ.Владелец);
	Результат.Вставить("Подразделение", КассаККМ.Подразделение);
	Результат.Вставить("СтруктурнаяЕдиница", КассаККМ.СтруктурнаяЕдиница);
	Результат.Вставить("СуммаВключаетНДС", Результат.ВидЦен.ЦенаВключаетНДС);
	Результат.Вставить("НДСВключатьВСтоимость", Истина);
	Результат.Вставить("НалогообложениеНДС", Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС);
...
Функция ПолучитьСостояниеКассовойСмены(КассаККМ) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ОтчетОРозничныхПродажах.Номер КАК НомерКассовойСмены,
	...
	|	ОтчетОРозничныхПродажах.ВалютаДокумента.Представление КАК ВалютаДокументаПредставление,
	|	ОтчетОРозничныхПродажах.ВидЦен КАК ВидЦен,
	...
	|	ОтчетОРозничныхПродажах.НДСВключатьВСтоимость КАК НДСВключатьВСтоимость,
	|	ВЫБОР
	|		КОГДА ОтчетОРозничныхПродажах.СтатусКассовойСмены = ЗНАЧЕНИЕ(Перечисление.СтатусыКассовойСмены.Открыта)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК СменаОткрыта,
	|	ВЫБОР
	|		КОГДА ОтчетОРозничныхПродажах.СтатусКассовойСмены = ЗНАЧЕНИЕ(Перечисление.СтатусыКассовойСмены.Открыта)
	|			ТОГДА ОтчетОРозничныхПродажах.НачалоКассовойСмены
	|		ИНАЧЕ ОтчетОРозничныхПродажах.ОкончаниеКассовойСмены
	|	КОНЕЦ КАК ДатаИзмененияСтатуса,
	|	ЕСТЬNULL(ДенежныеСредстваВКассахККМОстатки.СуммаВалОстаток, 0) КАК НаличностьВКассе,
	...
	|		ПО ОтчетОРозничныхПродажах.КассаККМ = ДенежныеСредстваВКассахККМОстатки.КассаККМ
	|ГДЕ
	...
	ОписаниеКассовойСмены = НовоеОписаниеКассовойСмены(КассаККМ);
...
Функция ПолучитьРеквизитыКассовойСменыНаДату(КассаККМ, ДатаВремя) Экспорт
	...
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ОтчетОРозничныхПродажах.КассаККМ) КАК КассовыеСмены
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетОРозничныхПродажах КАК ОтчетОРозничныхПродажах
	|		ПО КассовыеСмены.КассаККМ = ОтчетОРозничныхПродажах.КассаККМ
	...
	Если Выборка.Следующий() Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(Выборка.СтатусКассовойСмены) Тогда
			...
			Если Выборка.ОкончаниеКассовойСмены >= Дата И Выборка.НачалоКассовойСмены <= Дата Тогда
				СменаОткрыта = Истина;
			Иначе
				ОписаниеОшибки = НСтр("ru='Смена не открыта';uk='Зміна не відкрита'");
				СменаОткрыта = Ложь;
			КонецЕсли;
...
Функция СформироватьПечатнуюФормуОтчетаОРозничныхПродажах(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого ЧекККМ Из МассивОбъектов Цикл
		...
		СведенияОбОрганизации = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.Организация, Шапка.Дата, , , КодЯзыкаПечать);
		...
		ОбластьМакета.Параметры.ТекстЗаголовка = 
		  ...
		  + Формат(Шапка.Дата, "Л="+КодЯзыкаПечать+"; ДЛФ=DD");
		...
		ОбластьМакета.Параметры.СуммаПрописью = РаботаСКурсамиВалют.СформироватьСуммуПрописью(СуммаКПрописи, Шапка.Валюта, , КодЯзыкаПечать);
		ТабДокумент.Вывести(ОбластьМакета);
...
Функция СформироватьПечатнуюФормуКМ6(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого ОтчетОРозничныхПродажах Из МассивОбъектов Цикл
		...
		СведенияОПокупателе = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Продажи.Организация, Продажи.ДатаДокумента, , , КодЯзыкаПечать);
		...
		ОбластьМакета.Параметры.ОрганизацияПоЕДРПОУ			= СведенияОПокупателе.КодПоЕДРПОУ;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ТипКассы = Перечисления.ТипыКассККМ.АвтономнаяККМ Тогда
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Для документа %1 с типом кассы ""Автономная ККМ"" печать Акта о возврате денежных сумм по неиспользованным чекам (КМ-3) не требуется.';uk='Для документа %1 з типом каси ""Автономна ККМ"" друк Акту про повернення грошових сум по невикористаних чеках (КМ-3) не потрібен.'"),
				Выборка.Ссылка
			);
		...
		Если ВыборкаПоЧекамИтоги.НайтиСледующий(Новый Структура("ОтчетОРозничныхПродажах", Выборка.Ссылка)) Тогда
			...
			СведенияОбОрганизации							= УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Выборка.Организация, Выборка.ДатаДокумента, , , КодЯзыкаПечать);
			ОбластьШапки.Параметры.ПредставлениеОрганизации	= УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОбОрганизации, "ПолноеНаименование,ИНН,КодПоЕДРПОУ,Телефоны,");;
			ОбластьШапки.Параметры.НомерДокумента			= "";
			ОбластьШапки.Параметры.ОрганизацияПоЕДРПОУ		= СведенияОбОрганизации.КодПоЕДРПОУ;
			...
			ОбластьПодвала.Параметры.СуммаПрописью           = РаботаСКурсамиВалют.СформироватьСуммуПрописью(СуммаИтого, Выборка.Валюта, , КодЯзыкаПечать);
		...
		Если Не ДокументНапечатан Тогда
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Нет данных для печати Акта о возврате денежных сумм по неиспользованным чекам (КМ-3) по документу %1.';uk='Немає даних для друку Акту про повернення грошових сум по невикористаних чеках (КМ-3) по документу %1.'"),
				Выборка.Ссылка
			);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='КМ6 (Отчет за смену)';uk='КМ6 (Звіт за зміну)'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка,ФормаСпискаДокументов";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати.Представление = НСтр("ru='КМ3 (Акт о возврате денежных сумм)';uk='КМ3 (Акт про повернення грошових сум)'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка,ФормаСпискаДокументов";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
...
Функция ОтчетОРозничныхПродажахXML2_40(ДокументСсылка) Экспорт
	...
	Если Не Шапка.Следующий()
		...
		СообщениеXML.ТекстОшибки = НСтр("ru='Нет данных для выгрузки.';uk='Немає даних для вивантаження.'");
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		НоваяСтрока.doc_name   = НСтр("ru='Отчет о розничных продажах';uk='Звіт про роздрібні продажі'");
...
Функция ОтчетОРозничныхПродажахXML2_41(ДокументСсылка) Экспорт
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		doc.doc_type   = 1;
		doc.doc_name   = НСтр("ru='Чек';uk='Чек'");
		doc.doc_number = НСтр("ru='б/н';uk='б/н'");
