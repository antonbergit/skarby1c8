...
Процедура ЗаполнитьТабличнуюЧастьЗапасыПоИнвентаризацииТоваровНаСкладе(ДанныеЗаполнения)
	...
	Если Запасы.Количество() = 0 Тогда
		...
		Сообщение.Текст = НСтр("ru='Нет данных для заполнения по инвентаризации!';uk='Немає даних для заповнення по інвентаризації!'");
		Сообщение.Сообщить();
...
Процедура ЗаполнитьПоИнвентаризацииЗапасовНаСкладе(ДанныеЗаполнения) Экспорт
	...
	Если ДанныеЗаполнения.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы <> Перечисления.ТипыСтруктурныхЕдиниц.Розница Тогда
		ВызватьИсключение(НСтр("ru='Документ может быть заполнен только на основании инвентаризации по розничному складу!';uk='Документ може бути заповнений тільки на підставі інвентаризації по роздрібному складу!'"));
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОплатаПлатежнымиКартами.Итог("Сумма") > СуммаДокумента Тогда
		...
		ТекстОшибки = НСтр("ru='Сумма оплаты платежными картами превышает сумму документа';uk='Сума оплати платіжними картками перевищує суму документа'");
	...
	Если ТипКассы = Перечисления.ТипыКассККМ.ФискальныйРегистратор Тогда
		...
		Если ОткрытаяКассоваяСмена <> Неопределено
		   И ОткрытаяКассоваяСмена <> Ссылка Тогда
			...
			ТекстОшибки = НСтр("ru='По данной кассе на дату %Дата% уже зарегистрирован %КассоваяСмена%';uk='По даній касі на дату %Дата% уже зареєстрований %КассоваяСмена%'");
		...
		Если НЕ ЗначениеЗаполнено(ОкончаниеКассовойСмены)
			...
			ТекстОшибки = НСтр("ru='Поле ""Окончание смены"" не заполнено';uk='Поле ""Закінчення зміни"" не заповнене'");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьЗаказыВРозничнойТорговле") И ПолучитьФункциональнуюОпцию("РезервированиеЗапасов") Тогда
		...
		Для каждого СтрокаЗапасы Из Запасы Цикл
			Если СтрокаЗапасы.Количество = 0 И СтрокаЗапасы.Резерв = 0 Тогда
				ТекстСообщения = НСтр("ru='Не заполнена колонка ""Количество"" в строке %Номер% списка ""Запасы"".';uk='Не заповнена колонка ""Кількість"" в рядку %Номер% списку ""Запаси"".'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", СтрокаЗапасы.НомерСтроки);
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
					ЭтотОбъект,
					ТекстСообщения,
	...
	Иначе
		Для каждого СтрокаЗапасы Из Запасы Цикл
			Если СтрокаЗапасы.Количество = 0 Тогда
				ТекстСообщения = НСтр("ru='Не верно заполнена колонка ""Количество"" в строке %Номер% списка ""Запасы"".';uk='Не вірно заповнена колонка ""Кількість"" в рядку %Номер% списку ""Запаси"".'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", СтрокаЗапасы.НомерСтроки);
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
					ЭтотОбъект,
					ТекстСообщения,
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
    ...
    УправлениеНебольшойФирмойСервер.ОтразитьРозничныеПродажиПодакцизныхТоваров(ДополнительныеСвойства, Движения, Отказ);
