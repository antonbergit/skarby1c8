...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УчетАкцизногоНалога.УстановитьВидимостьПолейАкцизногоНалога(Объект, Элементы.ЗапасыГруппаАкциз);
...
Процедура СтатусКассовойСменыПриИзмененииНаСервере()
	...
	Если Объект.СтатусКассовойСмены = Перечисления.СтатусыКассовойСмены.Открыта Тогда
...
Процедура КассаККМПриИзмененииНаСервере()
	...
	Элементы.ГруппаКассоваяСмена.Видимость = РеквизитыКассыККМ.ТипКассы = Перечисления.ТипыКассККМ.ФискальныйРегистратор;
...
Процедура ЗапасыНоменклатураПриИзменении(Элемент)
	...
	СтрокаТабличнойЧасти.СтатьяДекларацииПоАкцизномуНалогу = СтруктураДанные.СтатьяДекларацииПоАкцизномуНалогу;
...
Процедура ЗапасыСуммаПриИзменении(Элемент)
	...
	РассчитатьСуммуНДСиАкцизногоНалога(СтрокаТабличнойЧасти);
...
&НаКлиенте
Процедура ЗапасыСтатьяДекларацииПоАкцизномуНалогуПриИзменении(Элемент)
...
&НаКлиенте
Процедура ЗапасыПодакцизныеТоварыДляКоммерческогоИспользованияПриИзменении(Элемент)
...
Процедура КомандаЗаполнитьПоОстаткамНаСкладе()
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьПоОстаткамНаСкладеЗавершение", ЭтотОбъект), НСтр("ru='Табличная часть будет очищена! Продолжить выполнение операции?';uk='Таблична частина буде очищена! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
		Возврат; 
	КонецЕсли;
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	Если СтруктураДанные.Номенклатура.ПодакцизныйТовар Тогда
		Если ЗначениеЗаполнено(СтруктураДанные.Номенклатура.СтатьяДекларацииПоАкцизномуНалогу) Тогда
			СтруктураДанные.Вставить("СтатьяДекларацииПоАкцизномуНалогу", СтруктураДанные.Номенклатура.СтатьяДекларацииПоАкцизномуНалогу);
		Иначе
			СтруктураДанные.Вставить("СтатьяДекларацииПоАкцизномуНалогу", Неопределено);
		КонецЕсли;
	Иначе
		СтруктураДанные.Вставить("СтатьяДекларацииПоАкцизномуНалогу", Неопределено);
	КонецЕсли; 
...
// Рассчитывается сумма НДС и Акцизного налога в строке табличной части.
//
&НаКлиенте
Процедура РассчитатьСуммуНДСиАкцизногоНалога(СтрокаТабличнойЧасти)
	
	СтавкаНДС = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеСтавкиНДС(СтрокаТабличнойЧасти.СтавкаНДС);
	
	Если УчетАкцизногоНалога.ДействуетАкцизныйНалог(ДатаДокумента) Тогда
		СуммаАкцизногоНалога = УчетАкцизногоНалога.РассчитатьСуммуАкцизногоНалога(
	...
	СтрокаТабличнойЧасти.СуммаАкцизногоНалога = СуммаАкцизногоНалога;
	
	СуммаСтрокиТабЧасти = СтрокаТабличнойЧасти.Сумма - СуммаАкцизногоНалога;
	
	СтрокаТабличнойЧасти.СуммаНДС = ?(
		Объект.СуммаВключаетНДС,
		СуммаСтрокиТабЧасти
	  - (СуммаСтрокиТабЧасти) / ((СтавкаНДС + 100) / 100),
	  СуммаСтрокиТабЧасти * СтавкаНДС / 100);
...
Процедура РассчитатьСуммуВСтрокеТабличнойЧасти(СтрокаТабличнойЧасти = Неопределено)
	...
	РассчитатьСуммуНДСиАкцизногоНалога(СтрокаТабличнойЧасти);
	СтрокаТабличнойЧасти.Всего = СтрокаТабличнойЧасти.Сумма + СтрокаТабличнойЧасти.СуммаАкцизногоНалога + ?(Объект.СуммаВключаетНДС, 0, СтрокаТабличнойЧасти.СуммаНДС);
...
Процедура ЗаполнитьПоЗаказуПокупателяНаСервере(Заказ)
	...
	Если НЕ (ВидОперацииЗаказаПокупателя = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу) Тогда
		ВызватьИсключение НСтр("ru='Для заполнения нужно выбрать заказа покупателя (на продажу)!';uk='Для заповнення потрібно вибрати замовлення покупця (на продаж)!'");
	КонецЕсли;
	...
	Если Объект.СуммаВключаетНДС <> Заказ.СуммаВключаетНДС Тогда
		Если Объект.СуммаВключаетНДС Тогда
			СтрокаПроНДС = НСтр("ru=' В заказе сумма не включает НДС.';uk=' У замовленні сума не включає ПДВ.'");
		Иначе
			СтрокаПроНДС = НСтр("ru=' В заказе сумма включает НДС.';uk=' У замовленні сума включає ПДВ.'");
		КонецЕсли;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Способ учета НДС в заказе отличен от текущего документа!';uk='Спосіб обліку ПДВ в замовленні відмінний від поточного документу!'") + СтрокаПроНДС;
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура СтатусГИСМОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru='Отчет о розничных продажах был изменен. Записать?';uk='Звіт про роздрібні продажі був змінений. Записати?'");
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения("СтатусОбработкаНавигационнойСсылкиЗавершение",
		                                                    ЭтотОбъект,
		                                                    Новый Структура("НавигационнаяСсылкаФорматированнойСтроки", НавигационнаяСсылкаФорматированнойСтроки));
		ТекстВопроса = НСтр("ru='Отчет о розничных продажах не записан. Записать?';uk='Звіт про роздрібні продажі не записаний. Записати?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
