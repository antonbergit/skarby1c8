...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаСдельныйНаряд, СтруктураДополнительныеСвойства) Экспорт
	...
	("ВЫБРАТЬ
	 ...
	 |////////////////////////////////////////////////////////////////////////////////
	 |ВЫБРАТЬ
	 |	СдельныйНарядОперацииПоСерийныхНомерах.Период,
	 |	&Организация КАК Организация,
	 |	СдельныйНарядОперацииПоСерийныхНомерах.Ссылка.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	 |	СдельныйНарядОперацииПоСерийныхНомерах.Ссылка.Исполнитель,
	 |	СдельныйНарядОперацииПоСерийныхНомерах.Операция,
	 |	СдельныйНарядОперацииПоСерийныхНомерах.Номенклатура,
	 |	СдельныйНарядОперацииПоСерийныхНомерах.гвСерийныйНомер КАК СерийныйНомер,
	 |	СдельныйНарядОперацииПоСерийныхНомерах.Характеристика,
	 |	СдельныйНарядОперацииПоСерийныхНомерах.ЗаказПокупателя,
	 |	СдельныйНарядОперацииПоСерийныхНомерах.Нормочасы КАК НормоЧасы,
	 |	ВЫБОР
	 |		КОГДА ТИПЗНАЧЕНИЯ(СдельныйНарядОперацииПоСерийныхНомерах.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
	 |			ТОГДА СдельныйНарядОперацииПоСерийныхНомерах.КоличествоПлан
	 |		ИНАЧЕ СдельныйНарядОперацииПоСерийныхНомерах.КоличествоПлан * СдельныйНарядОперацииПоСерийныхНомерах.ЕдиницаИзмерения.Коэффициент
	 |	КОНЕЦ КАК КоличествоПлан,
	 |	ВЫБОР
	 |		КОГДА ТИПЗНАЧЕНИЯ(СдельныйНарядОперацииПоСерийныхНомерах.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
	 |			ТОГДА СдельныйНарядОперацииПоСерийныхНомерах.КоличествоФакт
	 |		ИНАЧЕ СдельныйНарядОперацииПоСерийныхНомерах.КоличествоФакт * СдельныйНарядОперацииПоСерийныхНомерах.ЕдиницаИзмерения.Коэффициент
	 |	КОНЕЦ КАК Количество,
	 |	СдельныйНарядОперацииПоСерийныхНомерах.Стоимость КАК Сумма
	 |ИЗ
	 |	Документ.СдельныйНаряд.Операции КАК СдельныйНарядОперацииПоСерийныхНомерах
	 |ГДЕ
	 |	СдельныйНарядОперацииПоСерийныхНомерах.Ссылка = &Ссылка
	 |	И СдельныйНарядОперацииПоСерийныхНомерах.Ссылка.Закрыт");
	 // ...Гунько 2020-12-06
	...
	// Гунько 2020-12-06
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаСдельныеНарядыПоСерийныхНомерах", МассивРезультатов[8].Выгрузить());
	// ...Гунько 2020-12-06
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		ОблШапка.Параметры.ЗаголовокДокумента = НСтр("ru='Сдельный наряд № ';uk='Відрядний наряд № '", КодЯзыкаПечать) + Шапка.Номер + НСтр("ru=' от ';uk=' від '", КодЯзыкаПечать) + Формат(Шапка.ДатаДокумента, "Л="+КодЯзыкаПечать+"; ДЛФ=DD");
		ТабличныйДокумент.Вывести(ОблШапка);
		
		ОблШапкаТаблицы.Параметры.Стоимость = НСтр("ru='Стоимость"
"(';uk='Вартість"
"('", КодЯзыкаПечать) + Шапка.ВалютаДокумента + ")";
