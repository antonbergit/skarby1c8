...
Функция НачатьВыполнениеДлительнойОперацииНаСервере()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания	= НСтр("ru='Получение данных для заполнения плана продаж';uk='Отримання даних для заповнення плану продажів'");
...
Процедура ДлительнаяОперацияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(
			НСтр("ru='Не удалось получить данные для заполнения по причине: %1';uk='Не вдалося отримати дані для заповнення з причини: %1'"),
