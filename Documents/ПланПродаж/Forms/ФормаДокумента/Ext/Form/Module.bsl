		...
		Если РеквизитыСценария.ПланироватьКоличество Тогда
			...
			Если Не ЗначениеЗаполнено(СтрокаТаблицы.ВсегоКоличество) Тогда
				...
				ТекстОшибки = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
					"Колонка",
					"Заполнение",
					НСтр("ru='Количество';uk='Кількість'"),
					СтрокаТаблицы.НомерСтроки,
					Элементы.СтраницаПлан.Заголовок
				);
...
""
"Продолжить?';uk='План містить введені дані."
"При продовженні, склад колонок буде приведений у відповідність новому сценарію."
""
"Продовжити?'");
		ПараметрыВопроса = СтандартныеПодсистемыКлиент.ПараметрыВопросаПользователю();
		ПараметрыВопроса.Заголовок = НСтр("ru='Подтверждение изменения сценария';uk='Підтвердження зміни сценарію'");
...
Процедура ЗаполнитьПоПланируемымПродажам(Команда)
	...
	ОткрытьНастройкиЗаполненияПоСхеме("СКД_ПланПродаж", НСтр("ru='Настройки заполнения по запланированным продажам';uk='Налаштування заповнення по запланованих продажах'"));
...
Процедура ЗаполнитьПоФактическимПродажам(Команда)
	...
	ОткрытьНастройкиЗаполненияПоСхеме("СКД_ФактПродаж", НСтр("ru='Настройки заполнения по фактическим продажам';uk='Налаштування заповнення по фактичних продажах'"));
...
Процедура ПланВыполнитьДействие(Команда)
	...
	Если ПланИзменениеСтрокДействие <> ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.ДобавитьИзДокумента")
		...
		ТекстСообщения = НСтр("ru='Нет выбранных строк.';uk='Немає вибраних рядків.'");
...
Процедура УстановитьУсловноеОформлениеХарактеристикНоменклатуры(Форма,
	...
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Текст",					НСтр("ru='<характеристики не используются>';uk='<характеристики не використовуються>'"));
...
Процедура ОбновитьСтруктуруТаблицыПланирования()
	...
	Для Индекс = 0 По Периоды.Количество()-1 Цикл
		...
		Если РеквизитыСценария.ПланироватьКоличество Тогда
			...
			РеквизитФормы = Новый РеквизитФормы("КолонкаКоличество_" + Индекс, Число_15_3, "ТаблицаПланирования", НСтр("ru='Количество';uk='Кількість'"), Истина);
			ДобавляемыеРеквизиты.Добавить(РеквизитФормы);
			
		КонецЕсли;
		
		РеквизитФормы = Новый РеквизитФормы("КолонкаСумма_" + Индекс, Число_15_2, "ТаблицаПланирования", НСтр("ru='Сумма';uk='Сума'"), Истина);
		ДобавляемыеРеквизиты.Добавить(РеквизитФормы);
...
Процедура ЗаполнитьКроссТаблицуПланированияИзТаблицы(Таблица, ОчищатьПередЗаполнением = Истина)
	...
	Если РеквизитыСценария.ИзмерениеПланирования = Перечисления.ИзмеренияПланирования.Номенклатура Тогда
		СвойстваЗаполнения = СвойстваЗаполнения + ",Характеристика,ЕдиницаИзмерения";
	КонецЕсли;
...
"Для планирования на длительный срок, вы можете ввести несколько планов продаж или выбрать сценарий с большей периодичностью.';uk='Обрано занадто великий період. Період закінчення був змінений."
"Для планування на тривалий термін, ви можете ввести декілька планів продажів або вибрати сценарій з більшою періодичністю. '"), , , "Объект.ОкончаниеПланирования");
...
""
"Продолжить?';uk='План містить дані."
"При продовженні буде змінений склад періодів."
""
"Продовжити?'");
			ПараметрыВопроса = СтандартныеПодсистемыКлиент.ПараметрыВопросаПользователю();
			ПараметрыВопроса.Заголовок = НСтр("ru='Подтверждение изменения периода планирования';uk='Підтвердження зміни періоду планування'");
			ПараметрыВопроса.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
			СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, ПараметрыВопроса);
...
Процедура ИзменитьПометки(Форма, Пометка)
	...
	Форма.Элементы.ПланИзменитьПометки.Заголовок	= ?(Пометка, НСтр("ru='Снять пометки';uk='Зняти позначки'"), НСтр("ru='Установить пометки';uk='Встановити позначки'"));
...
Функция ПредставлениеДействияГрупповогоИзменения(ДействиеПеречисление)
	...
	Если ДействиеПеречисление = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.ИзменитьЦеныНаПроцент") Тогда
		Возврат НСтр("ru='Изменить суммы на процент';uk='Змінити суми на відсоток'");
	ИначеЕсли ДействиеПеречисление = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.РаспределитьСуммуПоКоличеству") Тогда
		Возврат НСтр("ru='Установить количество для всех периодов';uk='Встановити кількість для усіх періодів'");
	ИначеЕсли ДействиеПеречисление = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.РаспределитьСуммуПоСуммам") Тогда
		Возврат НСтр("ru='Установить сумму для всех периодов';uk='Встановити суму для усіх періодів'");
	КонецЕсли;
...
Функция ПредставлениеЗаголовкаОбъектаДействия(ДействиеПеречисление)
	...
	Если ДействиеПеречисление = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.РаспределитьСуммуПоКоличеству") Тогда
		Возврат НСтр("ru='Количество';uk='Кількість'");
	ИначеЕсли ДействиеПеречисление = ПредопределенноеЗначение("Перечисление.ДействияГрупповогоИзмененияСтрок.РаспределитьСуммуПоСуммам") Тогда
...
Процедура УправлениеВидимостьюГрупповогоИзменения(Форма)
	...
	Иначе
		...
		Элементы.ПланИзменениеСтрокЗначение.Заголовок = НСтр("ru='Значение';uk='Значення'");
