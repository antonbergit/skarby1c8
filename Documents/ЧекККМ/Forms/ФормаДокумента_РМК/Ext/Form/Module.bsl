...
Функция СформироватьНадписьЦеныИВалюта(СтруктураНадписи)
	...
	Если СтруктураНадписи.УчетВалютныхОпераций Тогда
		Если ЗначениеЗаполнено(СтруктураНадписи.ВалютаДокумента) Тогда
			ТекстНадписи = НСтр("ru='%Валюта%';uk='%Валюта%'");
	...
	Если ЗначениеЗаполнено(СтруктураНадписи.ВидЦен) Тогда
		Если ПустаяСтрока(ТекстНадписи) Тогда
			ТекстНадписи = ТекстНадписи + НСтр("ru='%ВидЦен%';uk='%ВидЦен%'");
		Иначе
			ТекстНадписи = ТекстНадписи + НСтр("ru=' • %ВидЦен%';uk=' • %ВидЦен%'");
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтруктураНадписи.ВидСкидки) Тогда
		Если ПустаяСтрока(ТекстНадписи) Тогда
			ТекстНадписи = ТекстНадписи + НСтр("ru='%ВидСкидкиНаценки%';uk='%ВидСкидкиНаценки%'");
		Иначе
			ТекстНадписи = ТекстНадписи + НСтр("ru=' • %ВидСкидкиНаценки%';uk=' • %ВидСкидкиНаценки%'");
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтруктураНадписи.ДисконтнаяКарта) Тогда
		Если ПустаяСтрока(ТекстНадписи) Тогда
			ТекстНадписи = ТекстНадписи + НСтр("ru='%ДисконтнаяКарта%';uk='%ДисконтнаяКарта%'");
		Иначе
			ТекстНадписи = ТекстНадписи + НСтр("ru=' • %ДисконтнаяКарта%';uk=' • %ДисконтнаяКарта%'");
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтруктураНадписи.НалогообложениеНДС) Тогда
		Если ПустаяСтрока(ТекстНадписи) Тогда
			ТекстНадписи = ТекстНадписи + НСтр("ru='%НалогообложениеНДС%';uk='%НалогообложениеНДС%'");
		Иначе
			ТекстНадписи = ТекстНадписи + НСтр("ru=' • %НалогообложениеНДС%';uk=' • %НалогообложениеНДС%'");
		КонецЕсли;	
...
Процедура СформироватьЗаголовок(СтруктураСостояниеКассовойСмены)
	...
	Если СтруктураСостояниеКассовойСмены.СменаОткрыта Тогда
		ТекстСообщения = НСтр("ru='%ТорговаяТочка%, Смена № %НомерСмены%  %ВремяИзменения%';uk='%ТорговаяТочка%, Зміна № %НомерСмены%  %ВремяИзменения%'");
	...
	Иначе
		ТекстСообщения = НСтр("ru='%ТорговаяТочка%';uk='%ТорговаяТочка%'");
...
&НаСервере
Функция ПодготовкаДанныхДляПробитияЧекаККМ(ДанныеЕГАИСДостаточны)
	
	ТаблицаТовары = Объект.Запасы.Выгрузить();
	ТаблицаАкцизы = Объект.АкцизныеМарки.Выгрузить();
	ДокументОбъект = РеквизитФормыВЗначение("Объект");
	
	ПодготовленнаяТаблицаДанных = УправлениеНебольшойФирмойСервер.ПодготовкаДанныхДляПробитияЧекаККМ(ТаблицаТовары, ТаблицаАкцизы, ДокументОбъект, ДанныеЕГАИСДостаточны);
	Возврат ПодготовленнаяТаблицаДанных;
	
КонецФункции

&НаСервереБезКонтекста
Функция СерийныйНомерКассыККМ(КассаККМ)
	СерийныйНомер = КассаККМ.ПодключаемоеОборудование.СерийныйНомер;
...
Процедура ПробитьЧек(СформироватьТоварныйЧек = Ложь)
	...
	Если УправлениеНебольшойФирмойКлиент.ПроверитьВозможностьПечатиЧека(ЭтаФорма, ПоказыватьПредупреждение) Тогда
		...
		Если ИспользоватьПодключаемоеОборудование // Проверка на включенную ФО "Использовать ПО"
			И НЕ КассаККМИспользоватьБезПодключенияОборудования Тогда 
			...
			Если ИдентификаторУстройства <> Неопределено Тогда
				...
				ТаблицаТоваров = ПодготовкаДанныхДляПробитияЧекаККМ(ДанныеЕГАИСДостаточны);
				
				Если Не ДанныеЕГАИСДостаточны Тогда
					ПоказатьПредупреждение(Неопределено,НСтр("ru='Не удалось выполнить операцию';uk='Не вдалося виконати операцію'"));
				...
				НомерСменыЧисло = СтроковыеФункцииКлиентСервер.СтрокаВЧисло("1" + Формат(Объект.Дата, "ДФ=yyyyMMdd"));
				
				НомерНаПечать = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Объект.Номер, Истина, Истина);
				НомерНаПечатьЧисло = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(НомерНаПечать);
				...
				ОбщиеПараметры.Добавить(СведенияОбОрганизации.КодПоЕДРПОУ);      // 18 - ЕДРПОУ организации (Для чека ЕНВД)
				...
				ОбщиеПараметры.Добавить(СерийныйНомерКассыККМ(Объект.КассаККМ)); // 21 - Заводской номер ККМ
				
				ВходныеПараметры.Добавить(ТаблицаТоваров);
				ВходныеПараметры.Добавить(ТаблицаОплат);
				ВходныеПараметры.Добавить(ОбщиеПараметры);
				
				ТранспортныйМодуль = ТранспортныйМодуль(Объект.Организация, Объект.СтруктурнаяЕдиница);
				
				Контекст = Новый Структура;
				Контекст.Вставить("ЧекККМСсылка"           , Объект.Ссылка);
				...
				МенеджерОборудованияКлиент.НачатьПечатьЧека(Оповещение, УникальныйИдентификатор, ИдентификаторУстройства, 
					ВходныеПараметры, Ложь, , ПослеОткрытияЧека, ПослеОшибкиПечатиЧека);
				
			Иначе
				
				ТекстСообщения = НСтр("ru='Устройство для печати чеков не выбрано.';uk='Пристрій для друку чеків не обраний.'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ПробитьЧекЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Исключение
			ЗаполнитьОписаниеДляТЧЗапасыНаКлиенте();
			ПоказатьПредупреждение(Неопределено, НСтр("ru='Не удалось выполнить проведение документа';uk='Не вдалося виконати проведення документа'")); // Асинхронный метод!
			Возврат;
		КонецПопытки;
...
Функция ПолучитьОстатокДенежныхСредствВКассеККМНаСервере(КассаККМ)
	...
	Возврат Документы.ОтчетОРозничныхПродажах.ПолучитьОстатокДенежныхСредствВКассеККМ(КассаККМ);
...
Процедура ПробитьЧекВыполнить(Команда, СформироватьТоварныйЧек = Ложь)
	...
	Если Объект.СуммаДокумента < Объект.ОплатаПлатежнымиКартами.Итог("Сумма") Тогда
		...
		ТекстОшибки = НСтр("ru='Сумма оплаты платежными картами превышает сумму чека';uk='Сума оплати платіжними картками перевищує суму чека'");
...
Процедура ПечатьОтчетаБезгашенияЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ТекстСообщения = НСтр("ru='Печать фискального отчета завершена.';uk='Друк фіскального звіту завершено.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ПечатьОтчетаБезГашенияВыполнить()
	...
	ОповещениеПриЗавершении = Новый ОписаниеОповещения("ПечатьОтчетаБезГашенияЗавершение", ЭтотОбъект);
...
&НаСервереБезКонтекста
Функция ПолучитьПоследнююЗакрытуюКассовуюСмену()
...
&НаСервере
Функция ВходныеПараметрыДляАннулированияЧекаККМ()
	
	// Подготовка таблицы общих параметров
	ТипЧека = 0; //?(ТипЗнч(ЧекККМ) = Тип("ДокументСсылка.ЧекККМВозврат"), 1, 0);
	ОбщиеПараметры = Новый Массив();
...
Процедура АннулироватьЧекККМ(КассаККМ)
	...
	Если ИдентификаторУстройстваФР <> Неопределено Тогда
		...
		// Готовим данные
		ПараметрыЧека = ВходныеПараметрыДляАннулированияЧекаККМ();
		ВыходныеПараметры = Неопределено;
		
		ОтображатьСообщения = Ложь; // Не отображать сообщения об ошибке. Сообщения на экран выводиться в процедуре обработки оповещения.
		ОповещениеПриЗавершении = Новый ОписаниеОповещения("АннулированиеЧекаЗавершение", ЭтотОбъект);
		МенеджерОборудованияКлиент.НачатьАннулированиеЧека(
	...
	Иначе
		ТекстСообщения = НСтр("ru='Не выбран фискальный регистратор.';uk='Не обраний фіскальний реєстратор.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
"Дополнительное описание:"
"%ДополнительноеОписание%';uk='При анулюванні чека відбулася помилка. Чек не анульований на фіскальному реєстраторові."
"Додатковий опис:"
"%ДополнительноеОписание%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения,
				"%ДополнительноеОписание%",
...
// Процедура - обработчик команды ПечатьКопииНаФискальномРегистраторе формы. Выполняет печать чека на ФР.
//
&НаКлиенте
Процедура ПечатьКопииНаФискальномРегистратореЗавершение(ИдентификаторУстройстваФР, Параметры) Экспорт
		...
		Если РезультатФР Тогда
			Если Не ПустаяСтрока(глПодключаемоеОборудование.ПоследнийСлипЧек) Тогда
				...
				Если Не РезультатФР Тогда
					ТекстСообщения = НСтр("ru='При печати слип-чека возникла ошибка: ""%ОписаниеОшибки%"".';uk='При друку сліп-чека виникла помилка: ""%ОписаниеОшибки%"".'"); 
					ТекстСообщения = СтрЗаменить(ТекстСообщения,
					                             "%ОписаниеОшибки%",
					                             ВыходныеПараметры[1]);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
"Можливо для даного сеансу ще не виконувалася еквайрингова операція.'"));
			...
			МенеджерОборудованияКлиент.ОтключитьОборудованиеПоИдентификатору(УникальныйИдентификатор,
			                                                                 ИдентификаторУстройстваФР);
		Иначе
			ТекстСообщения = НСтр("ru='При подключении фискального регистратора произошла ошибка: ""%ОписаниеОшибки%""."
"Слип-чек не напечатан.';uk='При підключенні фіскального реєстратора відбулася помилка: ""%ОписаниеОшибки%""."
"Сліп-чек не надрукований.'");
...
Процедура НастроитьКнопкиИКомандыМеню()
	...
	Для Каждого ТекущаяНастройкаКнопкиКоманды Из НастройкаРМК.КнопкиНижнейПанели Цикл
		Попытка
			Если ТекущаяНастройкаКнопкиКоманды.ИмяКнопки = "ЗначениеПоискаНоменклатуры" Тогда
				Если ЗначениеЗаполнено(ТекущаяНастройкаКнопкиКоманды.Клавиша) Тогда
					...
					Элементы.ЗначениеПоискаНоменклатуры.ПодсказкаВвода = НСтр("ru='Введите наименование, код или артикул ';uk='Введіть найменування, код або артикул '")+ПредставлениеСочетанияКлавиш(Элементы.ЗначениеПоискаНоменклатуры.СочетаниеКлавиш, Ложь);
				Иначе
					Элементы.ЗначениеПоискаНоменклатуры.СочетаниеКлавиш = Новый СочетаниеКлавиш(Клавиша.Нет);
					Элементы.ЗначениеПоискаНоменклатуры.ПодсказкаВвода = НСтр("ru='Введите наименование, код или артикул';uk='Введіть найменування, код або артикул'");
				КонецЕсли;
			Иначе
		...
		Исключение
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Произошла ошибка при настройке кнопок и команд меню. ';uk='Сталася помилка під час налаштування кнопок і команд меню.'")+ОписаниеОшибки();
			Сообщение.Сообщить();
		КонецПопытки;
	КонецЦикла;
...
Процедура ПоказатьСкрытьСдачуНаСервере(Показать = Истина, Проверять = Ложь)
	...
	Если Не Проверять ИЛИ Не Элементы.СтраницыДанныеПоСтрокеИСдача.ТекущаяСтраница = Элементы.СтраницаДанныеПоСтроке Тогда
		СдачаСтрока = НСтр("ru='Сдача: ';uk='Решта: '")+Сдача+" "+Объект.ВалютаДокумента;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(КассаККМ) Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru='Для пользователя не определена Касса ККМ!';uk='Для користувача не визначена Каса ККМ!'");
		Сообщение.Сообщить();
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	// РМК
	СменаОткрыта = Перечисления.СтатусыКассовойСмены.Открыта;
	...
	Список.Параметры.УстановитьЗначениеПараметра("Статус", Перечисления.СтатусыКассовойСмены.Открыта);
	...
	СписокЧековККМ.Параметры.УстановитьЗначениеПараметра("Статус", Перечисления.СтатусыКассовойСмены.Открыта);
	...
	СписокЧековККМНаВозврат.Параметры.УстановитьЗначениеПараметра("Статус", Перечисления.СтатусыКассовойСмены.Открыта);
...
Процедура ПриОткрытии(Отказ)
	...
	МенеджерОборудованияКлиентПереопределяемый.НачатьПодключениеОборудованиеПриОткрытииФормы(ЭтаФорма, "СканерШтрихкода,ДисплейПокупателя");
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если ИспользоватьАвтоматическиеСкидки И СкидкиРассчитаныПередЗаписью Тогда
		ПоказатьОповещениеПользователя(НСтр("ru='Изменение:';uk='Зміна:'"), 
										...
										Строка(Объект.Ссылка)+НСтр("ru='. Автоматические скидки (наценки) рассчитаны!';uk='. Автоматичні знижки (націнки) розраховані!'"), 
										БиблиотекаКартинок.Информация32);
	КонецЕсли;
	// Конец АвтоматическиеСкидки

	// РМК
	РабочееМестоКассираВызовСервера.ОбновитьНастройкиРМК(НастройкаРМК, НеПоказыватьПриОткрытииФормуВыбораКассы);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ИспользоватьАвтоматическиеСкидки Тогда
		Если Не Объект.СкидкиРассчитаны И СкидкиИзменились() Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Рассчитаны автоматические скидки (наценки)!';uk='Розраховані автоматичні знижки (націнки)!'");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "РМК_Запись_ПриходнаяНакладнаяВозврат" Тогда
		...
		Элементы.ДекорацияПриходнаяНакладная.Заголовок = НСтр("ru='Прих. нак-я №';uk='Приб. нак-а №!'")+НомерДокумента+НСтр("ru=' от ';uk=' від '")+Формат(Параметр.Дата, "ДФ=dd.MM.yyyy");
	...
	ИначеЕсли ИмяСобытия = "РМК_Запись_РКО" Тогда
		...
		РКО = Параметр.Ссылка;
		НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Параметр.Номер, Истина, Истина);
		Элементы.ДекорацияРКО.Заголовок = НСтр("ru='РКО №';uk='ВКО №!'")+СокрЛП(НомерДокумента)+НСтр("ru=' от ';uk=' від '")+Формат(Параметр.Дата, "ДФ=dd.MM.yyyy");
		Элементы.ДекорацияРКО.Видимость = Истина;
	ИначеЕсли ИмяСобытия = "РМК_Запись_ЧекККМВозврат" Тогда
		ЧекККМНаВозврат = Параметр.Ссылка;
		НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Параметр.Номер, Истина, Истина);
		Элементы.ДекорацияЧекККМНаВозврат.Заголовок = НСтр("ru='Чек ККМ на возврат №';uk='Чек ККМ на повернення №'")+НомерДокумента+НСтр("ru=' от ';uk=' від '")+Формат(Параметр.Дата, "ДФ=dd.MM.yyyy");
	...
	ИначеЕсли ИмяСобытия = "Запись_ЧекККМ_с_ЗаказомПокупателя" Тогда
		...
		НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Параметр.Номер, Истина, Истина);
		Элементы.ДекорацияЧекККМ.Заголовок = НСтр("ru='Чек ККМ на возврат №';uk='Чек ККМ на повернення №'")+НомерДокумента+НСтр("ru=' от ';uk=' від '")+Формат(Параметр.Дата, "ДФ=dd.MM.yyyy");
...
Процедура ОбновитьВидимостьНадписиИстеклоБолее24Часов(СтруктураСостояниеКассовойСмены = Неопределено)
	...
	Если СтруктураСостояниеКассовойСмены.СменаОткрыта Тогда
		ТекстСообщения = НСтр("ru='Кассовая смена открыта';uk='Касова зміна відкрита'");
		Если НЕ Документы.ОтчетОРозничныхПродажах.СменаОткрыта(Объект.КассоваяСмена, Дата, ТекстСообщения) Тогда
...
Процедура НастройкаБыстрыхТоваров(Команда)
	...
	Иначе
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не выбрана настройка РМК';uk='Не обране налаштування РМК'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура СоздатьЧекККМНаВозврат(Команда)
	...
	Если НЕ ЧекККМНаВозврат.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Чек на возврат уже создан.';uk='Чек на повернення вже створений.'");
...
Процедура СоздатьЧекККМНаОснованииЗазака(Команда)
	...
	Если НЕ ЧекККМ.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Чек уже создан.';uk='Чек вже створений.'");
	...
	Иначе
		...
		Иначе
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Не выбран заказ!';uk='Не обране замовлення!'");
...
Процедура Оплатить(Команда)
	...
	Если Объект.Запасы.Количество() = 0 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='В документе нет ни одной строки. Нет данных для оплаты!';uk='У документі немає жодного рядка. Немає даних для сплати!'");
	...
	Для Каждого ТекущаяСтрока Из Объект.Запасы Цикл
		Если ТекущаяСтрока.Цена = 0 Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='В строке не заполнена цена';uk='У рядку не заповнена ціна'");
		...
		Если ТекущаяСтрока.Количество = 0 Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='В строке не заполнено количество';uk='У рядку не заповнена кількість'");
		...
		Если ТекущаяСтрока.Номенклатура.Пустая() Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='В строке не выбрана номенклатура';uk='У рядку не обрана номенклатура'");
	...
	Если Объект.Запасы.Итог("Всего") <= 0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ОплатитьОбработкаОтветаНаВопрос", ЭтотОбъект),
			НСтр("ru='Сумма к оплате = 0! Уверены, что нужно продолжить?';uk='Сума до оплати = 0! Впевнені, що потрібно продовжити?'"),
			РежимДиалогаВопрос.ДаНет,
			,
			КодВозвратаДиалога.Нет,
			НСтр("ru='Запрос о подтверждении действия';uk='Запит про підтвердження дії'"));
	Иначе
		ОплатитьФрагмент();
	КонецЕсли;
...
Процедура ОплатитьФрагмент()
	...
	Если Не КонтрольНаСкладеОтключен Тогда
		Если Не ЗарезервироватьНаСервере() Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось выполнить резервирование!';uk='Не вдалося виконати резервування!'"));
			Возврат;
		Иначе
			ЗаполнитьОписаниеДляТЧЗапасыНаКлиенте();
		КонецЕсли;
...
Процедура ПечатьТоварногоЧека(Команда)
	...
	Если ЕстьОтчетыОРозничныхПродажах Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Для отчетов о розничных продажах товарный чек не формируется.';uk='Для звітів про роздрібні продажі товарний чек не формується.'");
...
Процедура СтруктураПодчиненности(Команда)
	...
	Иначе
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Выберите документ!';uk='Оберіть документ!'");
		Сообщение.Поле = "СписокЧековККМ";
		Сообщение.Сообщить();
	КонецЕсли;
...
Функция ЗарезервироватьНаСервере(ОтменаРезервирования = Ложь)
	...
	Иначе
		...
		Исключение
			...
			Если СтрНайти(ОписаниеОшибки, НСтр("ru='Не удалось провести';uk='Не вдалося провести'")) = 0 Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Описание ошибки: ';uk='Опис помилки: '")+ОписаниеОшибки;
				Сообщение.Сообщить();
			КонецЕсли;
		КонецПопытки;
...
Процедура ЧекНеПредъявленПриИзмененииНаСервере()
	...
	Если ЧекНеПредъявлен Тогда
		...
		Элементы.ДекорацияЗаголовокЧекиККМ.Заголовок = НСтр("ru='Создайте приходную накладную и РКО';uk='Створіть прибуткову накладну і ВКО'");
	...
	Иначе
		Элементы.ДекорацияЗаголовокЧекиККМ.Заголовок = НСтр("ru='Выберите основание для возврата';uk='Виберіть підставу для повернення'");
...
Процедура КассаККМПриИзменении(Элемент)
	...
	Если КассаККМ.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Касса ККМ не может быть пустой';uk='Каса ККМ не може бути порожньою'");
...
Процедура ЗаполнитьСоставЧекаИСсылкиНаДокументыНаСервере(ЧекККМ)
	...
	Если ЧекККМ.КассоваяСмена.СтатусКассовойСмены = Перечисления.СтатусыКассовойСмены.Открыта Тогда
		// Чек ККМ на возврат.
		...
		Если Выборка.Следующий() Тогда
			...
			Элементы.ДекорацияЧекККМНаВозврат.Заголовок = НСтр("ru='Чек ККМ на возврат №';uk='Чек ККМ на повернення №'")+НомерДокумента+НСтр("ru=' от ';uk=' від '")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
			Элементы.СоздатьЧекККМНаВозврат.ЦветТекста = WebЦвета.Серый;
		Иначе
		...
		Пока Выборка.Следующий() Цикл
			НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.Номер, Истина, Истина);
			Элементы.ДекорацияЧекККМНаВозврат.Заголовок = Элементы.ДекорацияЧекККМНаВозврат.Заголовок + "; "+" №"+НомерДокумента+НСтр("ru=' от ';uk=' від '")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
		КонецЦикла;
	...
	Иначе
		...
		Иначе
			...
			Если Выборка.Следующий() Тогда
				...
				НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.Номер, Истина, Истина);
				Элементы.ДекорацияПриходнаяНакладная.Заголовок = НСтр("ru='Прих. нак-я №';uk='Приб. нак-а №'")+СокрЛП(НомерДокумента)+НСтр("ru=' от ';uk=' від '")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
				
				Пока Выборка.Следующий() Цикл
					НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.Номер, Истина, Истина);
					Элементы.ДекорацияПриходнаяНакладная.Заголовок = Элементы.ДекорацияПриходнаяНакладная.Заголовок + "; "+" №"+НомерДокумента+НСтр("ru=' от ';uk=' від '")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
				КонецЦикла;
			Иначе
			...
			Если Выборка.Следующий() Тогда
				...
				НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.Номер, Истина, Истина);
				Элементы.ДекорацияРКО.Заголовок = НСтр("ru='РКО №';uk='ВКО №'")+НомерДокумента+НСтр("ru=' от ';uk=' від '")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
				Элементы.СоздатьРКОНаОснованииПриходнойНакладной.ЦветТекста = WebЦвета.Серый;
				
				Пока Выборка.Следующий() Цикл
					НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.Номер, Истина, Истина);
					Элементы.ДекорацияРКО.Заголовок = Элементы.ДекорацияРКО.Заголовок + "; "+" №"+НомерДокумента+НСтр("ru=' от ';uk=' від '")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
				КонецЦикла;
			Иначе
...
Процедура ЗаполнитьСоставЗаказаИСсылкиНаДокументыНаСервере(Заказ)
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.Порядок = 1 Тогда
				...
				ПредставлениеДокумента = НСтр("ru='Чек ККМ №';uk='Чек ККМ №'")+НомерДокумента+НСтр("ru=' от ';uk=' від '")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
			ИначеЕсли Выборка.Порядок = 2 Тогда
				ПредставлениеДокумента = НСтр("ru='Прих. накл. (возврат) №';uk='Приб. накл. (повернення) №'")+НомерДокумента+НСтр("ru=' от ';uk=' від '")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
			ИначеЕсли Выборка.Порядок = 3 Тогда
				ПредставлениеДокумента = НСтр("ru='Чек ККМ на возврат №';uk='Чек ККМ на повернення'")+НомерДокумента+НСтр("ru=' от ';uk=' від '")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
			КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Выборка.Номер, Истина, Истина);
		Элементы.ДекорацияЧекККМ.Заголовок = Элементы.ДекорацияЧекККМ.Заголовок + "; "+" №"+НомерДокумента+НСтр("ru=' от ';uk=' від '")+Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
	КонецЦикла;
...
Процедура УвиличитьКоличество(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		Если ТекущиеДанные.НеобходимостьВводаАкцизнойМарки Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Действие недоступно для маркируемой продукции!';uk='Дія недоступна для продукції, що маркірується!'");
	...
	Иначе
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не выбрана строка!';uk='Не обраний рядок!'");
		Сообщение.Поле = "Объект.Запасы";
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура ОчиститьТЧЗапасы(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Очистить табличную часть?';uk='Очистити табличну частину?'"), РежимДиалогаВопрос.ДаНет,,КодВозвратаДиалога.Да);
...
Процедура СписокСоздатьЧекККМНаВозврат(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Иначе
			ТекстСообщения = НСтр("ru='Чек ККК на возврат нельзя вводить на основании документов вида ';uk='Чек ККК на повернення не можна вводити на підставі документів виду '")+""""+ТипЗнч(ТекущиеДанные.Ссылка)+""".";
		КонецЕсли;
	Иначе
		ТекстСообщения = НСтр("ru='Не выбран чек ККМ!';uk='Не обраний чек ККМ!'");
	КонецЕсли;
...
Процедура ОткрытьИнформациюОСкидкахКлиент()
	...
	Если НЕ Объект.СкидкиРассчитаны Тогда
		ТекстВопроса = НСтр("ru='Скидки (наценки) не рассчитаны, рассчитать?';uk='Знижки (націнки) не розраховані, розрахувати?'");
...
Процедура ПримененныеСкидки(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		...
		|Переход к ""Примененные скидки"" возможен только после выбора документа в списке.';uk='Не вибраний документ.
		|Перехід до ""Застосовані знижки"" можливий тільки після вибору документа в списку'");
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = ТекстСообщения;
		Сообщение.Поле = "Список";
		Сообщение.Сообщить();
	...
	Иначе
		ТекстСообщения = НСтр("ru='Выберите чек ККМ или чек ККМ на возврат';uk='Виберіть чек ККМ або чек ККМ на повернення'");
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = ТекстСообщения;
		Сообщение.Поле = "Список";
		Сообщение.Сообщить();
...
Процедура ВыбранаДисконтнаяКарта(ДисконтнаяКарта)
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru='Считана дисконтная карта';uk='Зчитана дисконтна картка'"),
		ПолучитьНавигационнуюСсылку(ДисконтнаяКарта),
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Считана дисконтная карта %1';uk='Зчитана дисконтна картка %1'"), ДисконтнаяКарта),
		БиблиотекаКартинок.Информация32);
...
Процедура ВыбранаДисконтнаяКартаДополнительно(ДисконтнаяКарта)
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		Текст = НСтр("ru='Перезаполнить скидки во всех строках?';uk='Перезаповнити знижки у всіх рядках?'");
...
"Дополнительное описание:"
"%ДополнительноеОписание%';uk='При використанні дисплея покупця відбулася помилка."
"Додатковий опис:"
"%ДополнительноеОписание%'"
			);
			ТекстСообщения = СтрЗаменить(
...
Процедура ПолученыШтрихкодыФрагмент(НеизвестныеШтрихкоды) Экспорт
	...
	Для каждого ТекНеизвестныйШтрихкод Из НеизвестныеШтрихкоды Цикл
		...
		СтрокаСообщения = НСтр("ru='Данные по штрихкоду не найдены: %1%; количество: %2%';uk='Дані по штрихкоду не знайдені: %1%; кількість: %2%'");
...
Процедура ПолученыШтрихкодыНекорректногоТипа(ШтрихкодыНекорректногоТипа) Экспорт
	...
	Для каждого ТекНекорректныйШтрихкод Из ШтрихкодыНекорректногоТипа Цикл
		...
		СтрокаСообщения = НСтр("ru='Найденная по штрихкоду %1% номенклатура -%2%- имеет тип %3%, который не подходит для этой табличной части';uk='Знайдена по штрихкоду %1% номенклатура -%2%- має тип %3%, який не підходить для цієї табличної частини'");
...
Процедура ПоискПоШтрихкоду(Команда)
	...
	ПоказатьВводЗначения(ОписаниеОповещения, ТекШтрихкод, НСтр("ru='Введите штрихкод';uk='Введіть штрихкод'"));
...
Процедура УстановитьПериодНаСервере(ВидПериодаРМК, ИмяСписка, ПараметрСтандартныйПериод = Неопределено)
	...
	Если ВидПериодаРМК = ЗаТекущуюСмену Тогда
		...
		Если ИмяСписка = "Список" Тогда
			...
			ВидПериодаЖурнала = НСтр("ru='За текущую смену';uk='За поточну зміну'");
		ИначеЕсли ИмяСписка = "СписокЧековККМНаВозврат" Тогда
			ВидПериодаЧековККМНаВозврат = НСтр("ru='За текущую смену';uk='За поточну зміну'");
		ИначеЕсли ИмяСписка = "СписокЧековККМ" Тогда
			ВидПериодаЧековККМ = НСтр("ru='За текущую смену';uk='За поточну зміну'");
		КонецЕсли;
	...
	ИначеЕсли ВидПериодаРМК = ЗаПрошлуюСмену Тогда
		...
		ЭтотОбъект[ИмяСписка].Параметры.УстановитьЗначениеПараметра("КассоваяСмена", ПолучитьПоследнююЗакрытуюКассовуюСмену());
		Элементы[ИмяСписка].Период = Новый СтандартныйПериод;
		Элементы[ИмяСписка].Обновить();
		Если ИмяСписка = "Список" Тогда
			Элементы.Дата.Видимость = Ложь;
			ВидПериодаЖурнала = НСтр("ru='За прошлую смену';uk='За минулу зміну'");
		ИначеЕсли ИмяСписка = "СписокЧековККМНаВозврат" Тогда
			ВидПериодаЧековККМНаВозврат = НСтр("ru='За прошлую смену';uk='За минулу зміну'");
		ИначеЕсли ИмяСписка = "СписокЧековККМ" Тогда
			ВидПериодаЧековККМ = НСтр("ru='За прошлую смену';uk='За минулу зміну'");
		КонецЕсли;
	...
	ИначеЕсли ВидПериодаРМК = ЗаВчерашнийДень Тогда
		...
		Если ИмяСписка = "Список" Тогда
			Элементы.Дата.Видимость = Ложь;
			ВидПериодаЖурнала = НСтр("ru='За вчерашний день';uk='За вчорашній день'");
		ИначеЕсли ИмяСписка = "СписокЧековККМНаВозврат" Тогда
			ВидПериодаЧековККМНаВозврат = НСтр("ru='За вчерашний день';uk='За вчорашній день'");
		ИначеЕсли ИмяСписка = "СписокЧековККМ" Тогда
			ВидПериодаЧековККМ = НСтр("ru='За вчерашний день';uk='За вчорашній день'");
		КонецЕсли;
	...
	ИначеЕсли ВидПериодаРМК = ЗаВесьПериод Тогда
		...
		Если ИмяСписка = "Список" Тогда
			Элементы.Дата.Видимость = Истина;
			ВидПериодаЖурнала = НСтр("ru='За всё время';uk='За весь час'");
		ИначеЕсли ИмяСписка = "СписокЧековККМНаВозврат" Тогда
			ВидПериодаЧековККМНаВозврат = НСтр("ru='За всё время';uk='За весь час'");
		ИначеЕсли ИмяСписка = "СписокЧековККМ" Тогда
			ВидПериодаЧековККМ = НСтр("ru='За всё время';uk='За весь час'");
		КонецЕсли;
...
Процедура ЗаполнитьБыстрыеТовары(ПриОткрытии = Ложь)
	...
	Если НЕ ЗначениеЗаполнено(РабочееМесто) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не удалось определить рабочее место для работы с подключаемым оборудованием!';uk='Неможливо визначити робоче місце для роботи з підключаємим обладнанням!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(НастройкаРМК) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не удалось получить настройки РМК для текущего рабочего места!';uk='Не вдалося отримати налаштування РМК для поточного робочого місця!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Процедура УстановитьОтборТекущиеДела()
	...
	АвтоЗаголовок = Ложь;
	Заголовок = НСтр("ru='Заказы покупателей';uk='Замовлення покупців'");
	
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, "ПометкаУдаления", Ложь);
	
	РаботаСОтборами.СоздатьЭлементыМеток(ЭтотОбъект, "Закрыт", "ГруппаОтборЗакрыт", Ложь, НСтр("ru='Не закрытые';uk='Не закриті'"));
	...
	Если Параметры.Свойство("ПросроченоВыполнение") Тогда
		...
		Заголовок = Заголовок + ": " + НСтр("ru='просрочено выполнение';uk='прострочене виконання'");
	...
	ИначеЕсли Параметры.Свойство("ПросроченаОплата") Тогда
		...
		Заголовок = Заголовок + ": " + НСтр("ru='просрочена оплата';uk='прострочена оплата'");
	...
	ИначеЕсли Параметры.Свойство("НаСегодня") Тогда
		...
		Заголовок = Заголовок + ": " + НСтр("ru='на сегодня';uk='на сьогодні'");
	...
	ИначеЕсли Параметры.Свойство("ВРаботе") Тогда
		...
		Заголовок = Заголовок + ": " + НСтр("ru='в работе';uk='в роботі'");
	...
	ИначеЕсли Параметры.Свойство("Новые") Тогда
		...
		Заголовок = Заголовок + ": " + НСтр("ru='новые';uk='нові'");
