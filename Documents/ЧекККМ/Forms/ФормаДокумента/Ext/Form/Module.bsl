...
Процедура СформироватьПодсказкиКРеквизитам()
	...
	ЗаголовокПолученоНаличными = НСтр("ru='Получено наличными (%Валюта%)';uk='Отримане готівкою (%Валюта%)'");
	...
	ЗаголовокОплатаПлатежнымиКартами = НСтр("ru='Платежными картами (%Валюта%)';uk='Платіжними картками (%Валюта%)'");
	...
	Если Объект.СуммаДокумента < Объект.ОплатаПлатежнымиКартами.Итог("Сумма") Тогда
		...
		Если Элементы.Зарезервировать.Видимость И НЕ Элементы.РасчитатьСуммуНаличнойОплаты.Доступность Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС + 
				НСтр("ru='Прежде чем вводить сумму оплаты, требуется нажать кнопку %1.';uk='Перш ніж вводити суму оплати, потрібно натиснути кнопку %1.'");
			Если РезервироватьЗапасыИИспользоватьЗаказы Тогда	
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%1", НСтр("ru='""Провести""';uk='""Провести""'"));
			Иначе
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%1", НСтр("ru='""Зарезервировать""';uk='""Зарезервувати""'"));
			КонецЕсли;
		КонецЕсли;
...
Процедура УстановитьРежимТолькоПросмотр()
	...
	Если КассаККМИспользоватьБезПодключенияОборудования Тогда
		Элементы.ПробитьЧек.Заголовок = НСтр("ru='Отменить пробитие';uk='Скасувати пробиття'");
...
Процедура ОтменитьРежимТолькоПросмотр()
	...
	Элементы.ПробитьЧек.Заголовок = НСтр("ru='Пробить чек';uk='Пробити чек'");
	ТолькоПросмотр = Ложь;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РезервироватьЗапасыИИспользоватьЗаказы Тогда
		Элементы.Зарезервировать.Заголовок = НСтр("ru='Провести';uk='Провести'");
		Элементы.СнятьРезервирование.Заголовок = НСтр("ru='Отмена проведения';uk='Скасування проведення'");
		Элементы.СнятьРезервирование.ТолькоВоВсехДействиях = Истина;
	Иначе
		Элементы.Зарезервировать.Заголовок = НСтр("ru='Зарезервировать';uk='Зарезервувати'");
		Элементы.СнятьРезервирование.Заголовок = НСтр("ru='Снять резерв';uk='Зняти резерв'");
...
Процедура ПриОткрытии(Отказ)
	...
	// ПодключаемоеОборудование
	МенеджерОборудованияКлиентПереопределяемый.НачатьПодключениеОборудованиеПриОткрытииФормы(ЭтотОбъект, "СканерШтрихкода,ДисплейПокупателя");
	// Конец ПодключаемоеОборудование
...
Процедура ДобавитьОплатуКартой(Команда)
	...
	Если НЕ УправлениеНебольшойФирмойКлиент.ПроверитьВозможностьПечатиЧека(ЭтотОбъект, ПоказыватьПредупреждение) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Не удалось выполнить оплату через платежный терминал';uk='Не вдалося виконати оплату через платіжний термінал'"));
		Возврат;
	КонецЕсли;
...
Процедура УдалитьОплатуКартой(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Выберите строку удаляемой оплаты картой.';uk='Виберіть рядок оплати карткою, що видаляється.'"));
		Возврат;
	КонецЕсли;
	
	Если НЕ УправлениеНебольшойФирмойКлиент.ПроверитьВозможностьПечатиЧека(ЭтотОбъект, ПоказыватьПредупреждение) Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ru='Не удалось выполнить отмену оплаты картой.';uk='Не вдалося виконати скасування оплати карткою.'"));
		Возврат;
	КонецЕсли;
...
Функция КассаККМПриИзмененииНаСервере()
	...
	СостояниеКассовойСмены = Документы.ОтчетОРозничныхПродажах.ПолучитьРеквизитыКассовойСменыНаДату(Объект.КассаККМ, ?(ЗначениеЗаполнено(Объект.Ссылка), Объект.Дата, ТекущаяДата()));
	...
	Если ЗначениеЗаполнено(СостояниеКассовойСмены.СтатусКассовойСмены) Тогда
		...
		СменаОткрыта = (СостояниеКассовойСмены.СтатусКассовойСмены = Перечисления.СтатусыКассовойСмены.Открыта);
	Иначе
		СменаОткрыта = Ложь;
	КонецЕсли;
...
Процедура КассаККМПриИзменении(Элемент)
	...
	Если НЕ СменаОткрыта Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Кассовая смена не открыта. Открыть смену можно в журнале ""Чеки ККМ"". После открытия смены перевыберите кассу ККМ в текущем документе.';uk='Касова зміна не відкрита. Відкрити зміну можна в журналі ""Чеки ККМ"". Після відкриття зміни перевиберіть касу ККМ в поточному документі.'");
...
// Процедура - обработчик события ПриИзменении поля КассоваяСмена на сервере.
//
&НаСервере
Процедура КассоваяСменаПриИзмененииНаСервере()
	...
	Запрос.УстановитьПараметр("Ссылка", Объект.КассоваяСмена);
	...
	СостояниеКассовойСмены = Документы.ОтчетОРозничныхПродажах.ПолучитьСостояниеКассовойСмены(Объект.КассаККМ);
	ЗаполнитьЗначенияСвойств(Объект, СостояниеКассовойСмены);
	
	Элементы.Зарезервировать.Видимость = НЕ КонтрольНаСкладеОтключен;
	Элементы.СнятьРезервирование.Видимость = НЕ КонтрольНаСкладеОтключен;
	
КонецПроцедуры // КассоваяСменаПриИзмененииНаСервере()

// Процедура - обработчик события ПриИзменении поля КассоваяСмена.
//
&НаКлиенте
...
КонецПроцедуры // КассоваяСменаПриИзменении()
...
Функция ФормаОплатыПриИзмененииНаСервере(ОписаниеОшибки = "")
	...
	Если Объект.ФормаОплаты = Перечисления.ТипыДенежныхСредств.Наличные Тогда
		...
		Если Объект.ОплатаПлатежнымиКартами.Количество() > 0 Тогда
			...
			ОписаниеОшибки = НСтр("ru='Проведена оплата платежными картами! Установить форму оплаты ""Наличными"" невозможно';uk='Проведена оплата платіжними картками! Встановити форму оплати ""Готівкою"" неможливо'");
			Возврат Ложь;
...
Процедура ЗаказПриИзменении(Элемент)
	...
	Если НЕ Объект.Заказ.Пустая() Тогда
		...
		ПоказатьВопрос(
			ОписаниеОповещения, 
			НСтр("ru='Заполнить документ по выбранному заказу покупателя?';uk='Заповнити документ по обраному замовленню покупця?'"),
...
Функция ПроверитьВалютуДокументаОснования(БратьКурсНаТекущуюДату = Ложь)
	...
	Если НЕ Объект.Заказ.Пустая() Тогда
		Если Объект.ВалютаДокумента <> Объект.Заказ.ВалютаДокумента Тогда
			...
			ТекстСообщения = НСтр("ru='Валюта документа-основания (%1) отличается от валюты чека ККМ (%2)! Суммы были пересчитаны!';uk='Валюта документа-підстави (%1) відрізняється від валюти чека ККМ (%2)! Суми були перераховані!'");
