...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	Если ВводДоступен() Тогда
		...
		Если Результат = Неопределено Тогда
			ТекстСообщения = НСтр("ru='При обработке внешнего события от устройства произошла ошибка:""%ОписаниеОшибки%"".';uk='При обробці зовнішньої події від пристрою сталася помилка:""%ОписаниеОшибки%"".'");
...
Процедура ЗакрытьФормуПриСканировании()
	...
	Если НЕ ШтрихкодСоответствуетШаблону(КодАкцизнойМарки) Тогда
		...
		ТекстСообщения = НСтр("ru='Штрихкод не соответствует формату.';uk='Штрихкод не відповідає формату.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
	
	Если ШтрихкодМаркиПрисутствуетВЧеке() Тогда
		ТекстСообщения = НСтр("ru='Штрихкод марки уже присутствует в чеке.';uk='Штрихкод марки вже є присутнім в чеку.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
