...
Процедура ЗаполнитьДокументПоКассеККМ(КассаККМ) Экспорт
	...
	СостояниеКассовойСмены = Документы.ОтчетОРозничныхПродажах.ПолучитьСостояниеКассовойСмены(КассаККМ);
...
Процедура ОбойтиВыборкуКассККМ(Выборка)
	...
	Если Выборка.Количество() = 1 Тогда
		...
		Если СостояниеКассовойСмены.СтатусКассовойСмены = Перечисления.СтатусыКассовойСмены.Закрыта ИЛИ
			СостояниеКассовойСмены.СтатусКассовойСмены = Перечисления.СтатусыКассовойСмены.ЗакрытаЧекиЗаархивированы ИЛИ
			СостояниеКассовойСмены.СтатусКассовойСмены = Неопределено Тогда
			
			ТекстСообщения = НСтр("ru='Не открыта кассовая смена для кассы ККМ ""%1"". Открыть смену можно в журнале ""Чеки ККМ"". Также можно оформить чек в рабочем месте кассира (РМК).';uk='Не відкрита касова зміна для каси ККМ ""%1"". Відкрити зміну можна в журналі ""Чеки ККМ"". Також можна оформити чек в робочому місці касира (РМК).'");
	...
	ИначеЕсли Выборка.Количество() < 6 Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			СостояниеКассовойСмены = Документы.ОтчетОРозничныхПродажах.ПолучитьСостояниеКассовойСмены(ТекКассаККМ);
		...
		Если НЕ ЕстьОткрытаяСмена Тогда
			ТекстСообщения = НСтр("ru='Не открыта ни одна кассовая смена для склада, который выбран в заказе. Открыть смену можно в журнале ""Чеки ККМ"". Также можно оформить чек в рабочем месте кассира (РМК).';uk='Не відкрита жодна касова зміна для складу, який вибраний в замовленні. Відкрити зміну можна в журналі ""Чеки ККМ"". Також можна оформити чек в робочому місці касира (РМК).'");
			ВызватьИсключение ТекстСообщения;
		КонецЕсли;
...
Процедура ЗаполнитьПоЗаказуПокупателя(ДанныеЗаполнения) Экспорт
	...
	Если НЕ ЭтоВводНаОснованииЗаказНаПродажу(ЗаказОснование) Тогда
		ВызватьИсключение НСтр("ru='Чек ККМ ввести только на основании заказа покупателя (на продажу) или независимо!';uk='Чек ККМ ввести тільки на підставі замовлення покупця (на продаж) або незалежно!'");
	КонецЕсли;
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") И ДанныеЗаполнения.Свойство("РМК") И ЗначениеЗаполнено(ДанныеЗаполнения.КассаККМ) Тогда
		...
		Если СостояниеКассовойСмены.СтатусКассовойСмены = Перечисления.СтатусыКассовойСмены.Закрыта ИЛИ
			СостояниеКассовойСмены.СтатусКассовойСмены = Перечисления.СтатусыКассовойСмены.ЗакрытаЧекиЗаархивированы Тогда
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОплатаПлатежнымиКартами.Количество() > 0 И НЕ ЗначениеЗаполнено(ЭквайринговыйТерминал) Тогда
		...
		ТекстСообщения = НСтр("ru='Поле ""Эквайринговый терминал"" не заполнено';uk='Поле ""Еквайринговий термінал"" не заповнене'");
	...
	Если ОплатаПлатежнымиКартами.Итог("Сумма") > СуммаДокумента Тогда
		...
		ТекстСообщения = НСтр("ru='Сумма оплаты платежными картами превышает сумму документа';uk='Сума оплати платіжними картками перевищує суму документа'");
	...
	ТекстСообщения = НСтр("ru='Кассовая смена не открыта';uk='Касова зміна не відкрита'");
	
	Если НЕ Документы.ОтчетОРозничныхПродажах.СменаОткрыта(КассоваяСмена, Дата, ТекстСообщения) Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Статус = Перечисления.СтатусыЧековККМ.Пробит
	   ...
	   И НЕ КассаККМ.ИспользоватьБезПодключенияОборудования Тогда
		...
		ТекстСообщения = НСтр("ru='Чек ККМ пробит на фискальном регистраторе. Отмена проведения невозможна';uk='Чек ККМ пробитий на фіскальному реєстраторові. Скасування проведення неможливе'");
	...
	Если РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения
	   ...
	   И (КассоваяСмена.СтатусКассовойСмены = Перечисления.СтатусыКассовойСмены.Закрыта 
	   ИЛИ КассоваяСмена.СтатусКассовойСмены = Перечисления.СтатусыКассовойСмены.ЗакрытаЧекиЗаархивированы)
	   И (НомерЧекаККМ > 0 ИЛИ ОплатаПлатежнымиКартами.Количество() > 0)
	   Тогда
		
		ТекстСообщения = НСтр("ru='Кассовая смена закрыта. Отмена проведения невозможна';uk='Касова зміна закрита. Скасування проведення неможливе'");
