...
Процедура ЗаполнитьВидыОпераций()
	...
	Для каждого ВидОперации Из Метаданные.Перечисления.ВидыОперацийНалоговаяНакладная.ЗначенияПеречисления Цикл
		СписокОпераций.Добавить(Перечисления.ВидыОперацийНалоговаяНакладная[ВидОперации.Имя]);
	КонецЦикла;
