	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
				СообщениеXML,
				СтрШаблон(НСтр("ru='Не заполнен Код ТНВЭД для номенклатуры %1 в строке %2.';uk='Не заполнен Код ТНВЭД для номенклатуры %1 в строке %2.'"), ПредставлениеНоменклатуры, СтрокаТЧ.НомерСтроки));
		КонецПопытки;
		...
		Если ЗначениеЗаполнено(СтрокаТЧ.КодВидМеха) Тогда
			...
			Исключение
				ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
					СообщениеXML,
					СтрШаблон(НСтр("ru='Не корректно заполнен Вид меха для номенклатуры %1 в строке %2.';uk='Не корректно заполнен Вид меха для номенклатуры %1 в строке %2.'"), ПредставлениеНоменклатуры, СтрокаТЧ.НомерСтроки));
			КонецПопытки;
		КонецЕсли;
		...
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
				СообщениеXML,
				СтрШаблон(НСтр("ru='Не заполнено Описание товара для номенклатуры %1 в строке %2.';uk='Не заполнено Описание товара для номенклатуры %1 в строке %2.'"), ПредставлениеНоменклатуры, СтрокаТЧ.НомерСтроки));
		КонецПопытки;
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока ВыборкаИтоги.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Не удалось заблокировать документ: %Объект% по причине: %Причина%';uk='Не удалось заблокировать документ: %Объект% по причине: %Причина%'");
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ТекстСообщения = НСтр("ru='Не удалось обработать документ: %Объект% по причине: %Причина%';uk='Не удалось обработать документ: %Объект% по причине: %Причина%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Объект%", ВыборкаИтоги.Ссылка);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			                         УровеньЖурналаРегистрации.Предупреждение,
