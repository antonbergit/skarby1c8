...
Процедура ПроверитьВыпущенныеНомераКиз(Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(НСтр("ru='Номер КиЗ %1 не выпущен в строке %2 списка ""Товары""';uk='Номер КиЗ %1 не выпущен в строке %2 списка ""Товары""'"), Выборка.НомерКиЗ, Выборка.НомерСтроки),
			ЭтотОбъект,
