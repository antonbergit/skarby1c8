...
Процедура СтатусГИСМОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru='Уведомление об импорте маркированных товаров было изменено. Записать?';uk='Уведомление об импорте маркированных товаров было изменено. Записать?'");
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения("СтатусГИСМОбработкаНавигационнойСсылкиЗавершение",
		                                                    ЭтотОбъект,
		                                                    Новый Структура("НавигационнаяСсылкаФорматированнойСтроки", НавигационнаяСсылкаФорматированнойСтроки));
		ТекстВопроса = НСтр("ru='Уведомление об импорте маркированных товаров не записано. Записать?';uk='Уведомление об импорте маркированных товаров не записано. Записать?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Процедура НомераКиЗПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ТекстПредупреждения = НСтр("ru='Не выбрана строка с товаром';uk='Не выбрана строка с товаром'");
...
Процедура ОформитьМаркировкуТоваров(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru='Уведомление об импорте маркированных товаров ГИСМ было изменено. Записать?';uk='Уведомление об импорте маркированных товаров ГИСМ было изменено. Записать?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения("ОформитьМаркировкуТоваровОбработкаНавигационнойСсылкиЗавершение", ЭтотОбъект);
		ТекстВопроса = НСтр("ru='Уведомление об импорте маркированных товаров ГИСМ не записано. Записать?';uk='Уведомление об импорте маркированных товаров ГИСМ не записано. Записать?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Процедура ЗаполнитьНомераКиЗ(Команда)
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.НоменклатураКиЗ) Тогда
		ТекстПредупреждения = НСтр("ru='В строке не заполнена номенклатура КиЗ';uk='В строке не заполнена номенклатура КиЗ'");
		ПоказатьПредупреждение(,ТекстПредупреждения);
		Возврат;
	КонецЕсли;
	
	Если ТекущиеДанные.ХарактеристикиИспользуются И Не ЗначениеЗаполнено(ТекущиеДанные.ХарактеристикаКиЗ) Тогда
		ТекстПредупреждения = НСтр("ru='В строке не заполнена характеристика КиЗ';uk='В строке не заполнена характеристика КиЗ'");
		ПоказатьПредупреждение(,ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьВидМехаЗавершение(ВидМеха, ДополнительныеПараметры) Экспорт
	Если ЗначениеЗаполнено(ВидМеха) Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ru='Вид меха заполнен';uk='Вид меха заполнен'"),
			,
			СтрЗаменить(НСтр("ru='Для выделенных строк заполнен вид меха ""%ВидМеха%""';uk='Для выделенных строк заполнен вид меха ""%ВидМеха%""'"), "%ВидМеха%", ВидМеха),
	...
	Если ШтрихкодыПоТипам.КиЗ.Количество() > 0 Тогда
		...
		Для Каждого Штрихкод из ШтрихкодыПоТипам.КиЗ Цикл
			...
			Если ТекущаяСтрока.Количество <= СтрокиКиЗ.Количество() Тогда
				ТекстСообщения = НСтр("ru='Для текущей строки введены все номера КиЗ.';uk='Для текущей строки введены все номера КиЗ.'");
