...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	// Гунько 2021-02-14
	Если гвСоздатьНапоминание Тогда
		
		УстановитьПривилегированныйРежим(Истина);
		
		лСотрудник = Справочники.Сотрудники.НайтиПоРеквизиту("Физлицо", Объект.гвИсполнитель);
		лВыборкаПользователь = РегистрыСведений.СотрудникиПользователя.Выбрать(Новый Структура("Сотрудник", лСотрудник));
		
		Если лВыборкаПользователь.Следующий() Тогда
			лПользователь = лВыборкаПользователь.Пользователь;
		КонецЕсли;	
		
		Если ЗначениеЗаполнено(лПользователь) Тогда
			
			Напоминание = РегистрыСведений.НапоминанияПользователя.СоздатьМенеджерЗаписи();
			Напоминание.Описание = "Вам призначено завдання на виконання робіт.";
			Напоминание.СрокНапоминания = ТекущаяДатаСеанса();
			Напоминание.ВремяСобытия = ТекущаяДатаСеанса();
			Напоминание.Пользователь = лПользователь;
			...
			Напоминание.ПредставлениеИсточника = Строка(Объект.Ссылка);
			Напоминание.Записать(Истина);
			
			гвСоздатьНапоминание = Ложь;
...
"В строке №%Номер% табличной части ""Продукция"" номенклатура ""%НоменклатураПредставление%"" является работой (услугой)';uk='Операція розбирання не виконується для робіт і послуг!"
"У рядку №%Номер% табличної частини ""Продукція"" номенклатура ""%НоменклатураПредставление%"" є роботою (послугою)'");
...
Процедура СтруктурнаяЕдиницаПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.СтруктурнаяЕдиница) Тогда
		Объект.гвИсполнитель = УправлениеНебольшойФирмойВызовСервера.гвЗначениеРеквизитаОбъекта(Объект.СтруктурнаяЕдиница, "МОЛ");
	КонецЕсли;	
	// ...Гунько 2021-02-28
...
Процедура ДокументОснованиеНадписьОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "выбрать" Тогда
		...
		СписокОснований.Добавить("Документ.ЗаказНаПроизводство.ФормаВыбора", НСтр("ru='Заказ на производство';uk='Замовлення на виробництво'"));
...
Процедура ПродукцияНоменклатураПриИзменении(Элемент) Экспорт 
// ...Гунько 2021-02-10	
...
Процедура ЗаполнитьПоЗаказуПокупателя(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоЗаказуПокупателяЗавершение", ЭтотОбъект), НСтр("ru='Документ будет полностью перезаполнен по ""Заказу покупателя""! Продолжить выполнение операции?';uk='Документ буде повністю перезаповнений по ""Замовленню покупця""! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура КомандаЗаполнитьПоСпецификации(Команда)
	...
	Если Объект.Запасы.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьПоСпецификацииЗавершение", ЭтотОбъект), НСтр("ru='Табличная часть ""Материалы"" будет перезаполнена! Продолжить выполнение операции?';uk='Таблична частина ""Матеріали"" буде перезаповнена! Продовжити виконання операції?'"), 
