...
"Документ ""%ДокументСсылка"" не имеет ордерного склада!';uk='Неможливе введення операції ""Надходження на ордерний склад""."
"Документ ""%ДокументСсылка"" не має ордерного складу!'");
...
Процедура ЗаполнитьПоОтчетПереработчика(ДокументСсылкаОтчетПереработчика) Экспорт
	...
	|	ОтчетПереработчика.СерийныеНомераПредставление КАК СерийныеНомера
	|ИЗ
	|	(ВЫБРАТЬ
	|		0 КАК НомерСтроки,
	|		ОтчетПереработчика.Номенклатура КАК Номенклатура,
	...
	|		ОтчетПереработчика.Партия КАК Партия,
	|		ОтчетПереработчика.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|		ОтчетПереработчика.Количество КАК Количество,
	|		ОтчетПереработчика.СерийныеНомераПредставление КАК СерийныеНомераПредставление
	|	ИЗ
	...
	|	ОтчетПереработчика.СерийныеНомераПредставление
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
