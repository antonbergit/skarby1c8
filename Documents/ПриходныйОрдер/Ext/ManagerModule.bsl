...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		Если ИмяМакета = "ПриходныйОрдер" Тогда
			...
			ОбластьМакета.Параметры.ТекстЗаголовка = НСтр("ru='Приходный ордер № ';uk='Прибутковий ордер № '", КодЯзыкаПечать)
													+ НомерДокумента
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='М4 (Приходный ордер)';uk='М4 (Прибутковий ордер)'");
	...
	КомандаПечати.Представление = НСтр("ru='ТОРГ4 (Акт о приемке товара)';uk='ТОРГ4 (Акт приймання товару)'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
