...
Процедура СоздатьПоступление(Команда)
	...
	Если ПоступлениеТоваров = Неопределено И ТолькоПоиск Тогда
		Если ДополнительныеПараметры.СтатусОбработки = ПредопределенноеЗначение("Перечисление.СтатусыОбработкиТТНВходящейЕГАИС.ПереданАктПодтверждения")
			...
			СтрокаСообщения = НСтр("ru='Накладная уже подветждена актом, создание не возможно';uk='Накладная уже подветждена актом, создание не возможно'");
		...
		ИначеЕсли ДополнительныеПараметры.СтатусОбработки = ПредопределенноеЗначение("Перечисление.СтатусыОбработкиТТНВходящейЕГАИС.ПереданАктОтказа") Тогда
			СтрокаСообщения = НСтр("ru='В ЕГАИС уже передан отказ от накладной, , создание не возможно';uk='В ЕГАИС уже передан отказ от накладной, , создание не возможно'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения);
			Возврат;
		КонецЕсли;
...
Процедура ЗагрузитьТТН(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ТранспортныйМодуль) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Заполните транспортный модуль';uk='Заполните транспортный модуль'"));
		Возврат;
	КонецЕсли;
...
Процедура ОтказатьсяОтТТН(Команда)
	...
	Если ДанныеТТН.СтатусОбработки = ПредопределенноеЗначение("Перечисление.СтатусыОбработкиТТНВходящейЕГАИС.ПереданАктПодтверждения") Тогда
		СтрокаСообщения = НСтр("ru='Накладная уже подветждена актом, отказ не возможен';uk='Накладная уже подветждена актом, отказ не возможен'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения);
		Возврат;
	ИначеЕсли ДанныеТТН.СтатусОбработки = ПредопределенноеЗначение("Перечисление.СтатусыОбработкиТТНВходящейЕГАИС.ПереданАктОтказа") Тогда
		СтрокаСообщения = НСтр("ru='Отказ от данных уже передавался в ЕГАИС';uk='Отказ от данных уже передавался в ЕГАИС'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения);
		Возврат;
	Иначе
		ТекстВопроса = НСтр("ru='Вы уверены что хотите передать отказ от накладной в ЕГАИС?';uk='Вы уверены что хотите передать отказ от накладной в ЕГАИС?'");
...
Процедура ЗапроситьОтменуПроведения(Команда)
	...
	ТекстВопроса = НСтр("ru='Отправить грузоотправителю запрос на отмену проведения ТТН?';uk='Отправить грузоотправителю запрос на отмену проведения ТТН?'");
...
"Загружено документов: %ЗагруженоДокументов%';uk='Загрузка завершена."
...
Процедура ВыгрузкаДокумента_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Результат Тогда
		...
		Элементы.Список.Обновить();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Документ успешно подтвержден.';uk='Документ успешно подтвержден.'"));
	КонецЕсли;
