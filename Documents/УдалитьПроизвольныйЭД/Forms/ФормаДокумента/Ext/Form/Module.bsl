...
Процедура ДобавитьВложениеИзФайлаНаДиске(Команда)
	...
	ТекстПредупреждения = НСтр("ru='Для загрузки файла необходимо установить расширение для веб-клиента 1С:Предприятие.';uk='Для загрузки файла необходимо установить расширение для веб-клиента 1С:Предприятие.'");
...
Процедура ОбновитьЗаголовокФормы()
	...
	Если Объект.Направление = Перечисления.НаправленияЭД.Исходящий Тогда
		...
		КонецЕсли;
	Иначе
		ТекстЗаголовка = НСтр("ru='Входящий произвольный документ %1 от %2';uk='Входящий произвольный документ %1 от %2'");
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстЗаголовка, Объект.Номер, Объект.Дата);
	КонецЕсли;
