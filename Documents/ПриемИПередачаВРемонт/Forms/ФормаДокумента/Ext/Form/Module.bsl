...
// Функция возвращает ссылку на подчиненный документ
//
&НаСервере
...
Процедура ОбновитьДокументПродажи()
	...
	Если ИспользоватьСерийныеНомера И ЗначениеЗаполнено(Объект.СерийныйНомер) И ЗначениеЗаполнено(Объект.Номенклатура)
		...
		Иначе	
			ДокументПродажиСсылка = НСтр("ru='<Не найден>';uk='<Не знайдений>'");
			Элементы.ДокументПродажи.Гиперссылка = Ложь;
		КонецЕсли;
		
		Если ГарантийныйСрок>0 Тогда
			Элементы.ТекстГарантии.Заголовок = НСтр("ru='Гарантийный срок: ';uk='Гарантийный срок: '")+ГарантийныйСрок+НСтр("ru=' мес.';uk=' міс.'");
		Иначе
			Элементы.ТекстГарантии.Заголовок = НСтр("ru='Гарантия не предоставляется';uk='Гарантія від продавця'");
		КонецЕсли;
	Иначе	
...
Процедура ИсторияПродажНажатие(Элемент)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Номенклатура) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Выберите номенклатуру!';uk='Виберіть номенклатуру!'"));
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ДобавитьДокументРемонта(ТипДокумента)
...
Процедура ДобавитьАкт(Команда)
	...
	ДобавитьДокументРемонта(Тип("ДокументСсылка.АктВыполненныхРабот"));
...
Процедура ДобавитьЗаказНаряд(Команда)
	...
	ДобавитьДокументРемонта(Тип("ДокументСсылка.ЗаказПокупателя"));
...
Процедура ДобавитьРасходная(Команда)
	...
	ДобавитьДокументРемонта(Тип("ДокументСсылка.РасходнаяНакладная"));
...
Процедура ДобавитьОприходование(Команда)
	...
	ДобавитьДокументРемонта(Тип("ДокументСсылка.ОприходованиеЗапасов"));
...
Процедура ДобавитьВозврат(Команда)
	...
	ДобавитьДокументРемонта(Тип("ДокументСсылка.ПриходнаяНакладная"));
...
Процедура Подключаемый_МеткаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если Лев(НавигационнаяСсылкаФорматированнойСтроки,8) = "Документ" Тогда
		...
		//ОткрытьФорму("Документ."+ИмяФормыСтрока+".ФормаОбъекта", Новый Структура("Ключ", ДокСсылка), ЭтотОбъект);
...
Процедура ПолученыШтрихкодыНекорректногоТипа(ШтрихкодыНекорректногоТипа) Экспорт
	...
	Для каждого ТекНекорректныйШтрихкод Из ШтрихкодыНекорректногоТипа Цикл
		...
		СтрокаСообщения = НСтр("ru='Найденная по штрихкоду %1% номенклатура -%2%- имеет тип %3%, который не подходит для этого документа';uk='Знайдена по штрихкоду %1% номенклатура -%2% - має тип %3%, який не підходить для цього документу'");
