...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаПриемИПередачаВРемонт, СтруктураДополнительныеСвойства) Экспорт
	...
	|ГДЕ
	|	ТаблицаДокумента.Ссылка = &Ссылка И ТаблицаДокумента.Ссылка.РемонтВыполнен
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаПриемИПередачаВРемонт, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДатаРемонтВыполнен, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("ВозникновениеОбязательствПокупателя", НСтр("ru='Задолженность покупателя за услуги ремонта';uk='Заборгованість покупця за послуги ремонту'"));
...
Процедура СформироватьТаблицаСерийныеНомера(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	|	ВременнаяТаблицаЗапасы КАК ТаблицаЗапасы
	|ГДЕ
	|	ТаблицаЗапасы.ИспользоватьСерийныеНомера И ТаблицаЗапасы.ВыдачаИзРемонта";
...
//
Процедура ВыполнитьКонтроль(ДокументСсылкаПриемИПередачаВРемонт, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	ДвиженияРасчетыСПокупателями = ДополнительныеСвойства.РемонтВыполнен И СтруктураВременныеТаблицы.ДвиженияРасчетыСПокупателямиИзменение;
	...
	Если ДвиженияРасчетыСПокупателями Тогда
		...
		Запрос.УстановитьПараметр("СуммаДокумента", ДокументСсылкаПриемИПередачаВРемонт.СуммаДокумента);
...
Процедура СформироватьАктВозврата(МассивОбъектов, ОбъектыПечати, ТабличныйДокумент)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		Если Шапка.ИспользоватьСерийныеНомера Тогда
		...
		Если Шапка.СуммаДокумента > 0 Тогда
			...
			ОбластьМакета.Параметры.ИтоговаяСтрока = Нстр("ru='Всего наименований 1, на сумму ';uk='Всего наименований 1, на сумму '") + УправлениеНебольшойФирмойСервер.ФорматСумм(Шапка.СуммаДокумента, Шапка.ВалютаДокумента);
			...
			Если Шапка.СуммаНДС = 0 Тогда
				ОбластьМакета.Параметры.НДС = Нстр("ru='Без налога (НДС)';uk='Без податку (ПДВ)'");
				ОбластьМакета.Параметры.СуммаНДС = Нстр("ru='-';uk='-'");
			Иначе
				ОбластьМакета.Параметры.НДС = ?(Шапка.СуммаВключаетНДС, Нстр("ru='В том числе НДС:';uk='В тому числі ПДВ:'"), Нстр("ru='Сумма НДС:';uk='Сума ПДВ:'"));
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='Талон приемки';uk='Талон приймання'");
	...
	КомандаПечати.Представление = НСтр("ru='Акт возврата';uk='Акт повернення'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
