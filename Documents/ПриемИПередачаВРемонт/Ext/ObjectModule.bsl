...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВыдачаИзРемонта И (НЕ РемонтВыполнен) Тогда
		Если ВариантРемонта = Перечисления.ВариантыРемонта.СервисЦентр Тогда
		    ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не установлен ""Принят из сервисного центра""!';uk='Не встановлений ""Прийнятий з сервісного центру""!'"));
		Иначе	
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не установлен ""Ремонт выполнен""!';uk='Не встановлений ""Ремонт виконаний""!'"));
		КонецЕсли;
		Отказ = Истина;
	КонецЕсли;
	Если РемонтВыполнен И ВариантРемонта = Перечисления.ВариантыРемонта.СервисЦентр И (НЕ ПередачаВСервисныйЦентр) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не установлен ""Передан в сервисный центр""!';uk='Не встановлений ""Переданий в сервісний центр""!'"));
		Отказ = Истина;
	КонецЕсли;
	
	Если ПередачаВСервисныйЦентр И РемонтВыполнен И ДатаПередачаВСервисныйЦентр > ДатаРемонтВыполнен Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Дата передачи в сервисный центр не может быть больше даты приема из сервисного центра!';uk='Дата передачі в сервісний центр не може бути більше дати прийому з сервісного центру!'"));
		Отказ = Истина;
	КонецЕсли;
	
	Если РемонтВыполнен И ВыдачаИзРемонта И ДатаРемонтВыполнен > ДатаВыдачаИзРемонта Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Дата выпонения ремонта не может быть больше даты выдачи из ремонта!';uk='Дата виконання ремонту не може бути більше дати видачі з ремонту!'"));
		Отказ = Истина;
	КонецЕсли;
	...
	Если ПередачаВСервисныйЦентр И Дата > ДатаПередачаВСервисныйЦентр Тогда
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Дата передачи в сервисный центр не может быть меньше даты приема в ремонт (даты документа)!';uk='Дата передачі в сервісний центр не може бути менше дати прийому в ремонт (дати документа)!'"));
			Отказ = Истина;
		КонецЕсли; 
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если (ДополнительныеСвойства.ВариантРемонта = Перечисления.ВариантыРемонта.НашаМастерскаяПростойРемонт
		ИЛИ ДополнительныеСвойства.ВариантРемонта = Перечисления.ВариантыРемонта.СервисЦентр)
		И ДополнительныеСвойства.РемонтВыполнен Тогда
