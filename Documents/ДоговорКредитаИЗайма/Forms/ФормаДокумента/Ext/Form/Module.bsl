...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Объект.Ссылка.Пустая() Тогда
		Если ЕстьДвиженияПоДоговору(Объект.Ссылка) Тогда
			Элементы.ГруппаСчетаУчетаКолонки.Подсказка = НСтр("ru='В базе есть движения по этому договору! Изменение счетов учета не рекомендуется!';uk='У базі є рухи за цим договором! Зміна рахунків обліку не рекомендується!'");
		КонецЕсли;
	КонецЕсли;
...
Процедура СоздатьНапоминанияОбОплате(Команда)
	...
	Если Объект.Ссылка.Пустая() Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Пожалуйста, запишите документ!';uk='Будь ласка, запишіть документ!'"));
		Возврат;
	КонецЕсли;
	
	Если Объект.ГрафикПлатежейИНачислений.Количество() = 0 Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Не заполнен график платежей!';uk='Не заповнений графік платежів!'"));
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьВозможностьИзменений(Отказ)
	...
	Если Выборка.Следующий() Тогда
		ТекстСообщения = НСтр("ru='В базе присутствуют документы, в которых выбран текущий договор. Изменение организации, вида договора, банка-контрагента и сотрудника запрещено. Для просмотра списка связанных документах можно использовать команду ""Еще - Связанные документы"".';uk='У базі присутні документи, в яких вибраний поточний договір. Зміна організації, виду договору, банку-контрагента і співробітника заборонено. Щоб переглянути список пов`язаних документів можна використовувати команду ""Ще - Зв`язані документи"".'");
