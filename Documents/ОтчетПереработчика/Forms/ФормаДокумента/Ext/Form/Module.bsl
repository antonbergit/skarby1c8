...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ПодборПроизведен" 
		...
		ИмяТабличнойЧасти = ?(Элементы.Страницы.ТекущаяСтраница = Элементы.ГруппаЗапасы, "Запасы", "Отходы");
...
Процедура НоменклатураПриИзменении(Элемент)
	...
	Объект.Спецификация = СтруктураДанные.Спецификация;
	Объект.Количество = 1;
...
Процедура КоличествоПриИзменении(Элемент)
	...
	Если ПродукцияКоличество < объект.СерийныеНомера.Количество() И ПродукцияКоличество > 0 Тогда
		//МассивУдалитьСтроки = Новый ФиксированныйМассив(Объект[ИмяТабличнойЧастиСерийныеНомера].НайтиСтроки(Новый Структура(ИмяПоляКлючСвязи, СтрокаТабличнойЧасти[ИмяПоляКлючСвязи])));
		Для н=ПродукцияКоличество По объект.СерийныеНомера.Количество()-1 Цикл
			СтрокаУдалить = Объект.СерийныеНомера[н];
	...
	КонецЦикла;
	Объект.СерийныеНомераПредставление = Лев(СтроковоеПредставлениеСерийныхНомеров, Мин(СтрДлина(СтроковоеПредставлениеСерийныхНомеров)-2,150));
...
Процедура КомандаЗаполнитьПоСпецификации(Команда)
	...
	Если Объект.Запасы.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьПоСпецификацииЗавершение", ЭтотОбъект), НСтр("ru='Табличная часть ""Запасы"" будет перезаполнена! Продолжить выполнение операции?';uk='Таблична частина ""Запаси"" буде перезаповнена! Продовжити виконання операції?'"), 
							РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
...
"Пересчитать документ в соответствии с договором?';uk='Договір з контрагентом передбачає вид цін,"
"відмінний від установленого в документі! "
"Перерахувати документ відповідно до договору?'") + Символы.ПС + Символы.ПС;
...
Функция ПолучитьСерийныеНомераИзХранилища(АдресВоВременномХранилище, КлючСтроки)
	...
	Возврат РаботаСФормойДокумента.ПолучитьСерийныеНомераИзХранилищаДляПоляВвода(Объект, АдресВоВременномХранилище);
	
КонецФункции

Функция ПараметрыПодбораСерийныхНомеровВПолеВвода()
	
	Возврат РаботаСФормойДокумента.ПараметрыПодбораСерийныхНомеровВПолеВвода(Объект, ЭтотОбъект.УникальныйИдентификатор, Ложь);
