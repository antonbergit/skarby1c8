...
Процедура ИнициализироватьДанныеПоУслуге(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства, СуммаУслуги) Экспорт
	...
	|	ОтчетПереработчика.СтруктурнаяЕдиница КАК КоррСтруктурнаяЕдиница,
	|	ОтчетПереработчика.Расход.СчетУчетаЗатрат КАК СчетУчета,
	|	ВЫБОР
	...
	|	ОтчетПереработчика.ЗаказПокупателя КАК КоррЗаказПокупателя,
	|	0 КАК Количество,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ОтчетПереработчика.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|				ТОГДА ОтчетПереработчика.Всего * КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность)
	|			ИНАЧЕ ОтчетПереработчика.Всего * ОтчетПереработчика.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * ОтчетПереработчика.Кратность)
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК Сумма,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ОтчетПереработчика.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	|				ТОГДА ОтчетПереработчика.Всего * КурсыРегВалюты.Курс * ОтчетПереработчика.Ссылка.Кратность / (ОтчетПереработчика.Ссылка.Курс * КурсыРегВалюты.Кратность)
	|			ИНАЧЕ ОтчетПереработчика.Всего
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаВал,
	|	ВЫБОР
	|		КОГДА ОтчетПереработчика.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Подразделение)
	|			ТОГДА ОтчетПереработчика.Расход.СчетУчетаЗатрат
	|		ИНАЧЕ ОтчетПереработчика.Номенклатура.СчетУчетаЗапасов
	|	КОНЕЦ КАК СчетДт,
	|	НЕОПРЕДЕЛЕНО КАК ВалютаДт,
	|	0 КАК СуммаВалДт,
	|	ОтчетПереработчика.Контрагент.СчетУчетаРасчетовСПоставщиком КАК СчетКт,
	|	ВЫБОР
	|		КОГДА ОтчетПереработчика.Контрагент.СчетУчетаРасчетовСПоставщиком.Валютный
	|			ТОГДА ОтчетПереработчика.Договор.ВалютаРасчетов
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ВалютаКт,
	|	ВЫБОР
	|		КОГДА ОтчетПереработчика.Контрагент.СчетУчетаРасчетовСПоставщиком.Валютный
	...
	Запрос.УстановитьПараметр("ОтражениеРасходовПоПереработке", НСтр("ru='Отражение расходов по переработке';uk='Відображення витрат по переробці'"));
...
Процедура ИнициализироватьДанныеПоЗапасам(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства, СуммаСборки) Экспорт
	...
	|	ОтчетПереработчикаЗапасы.Ссылка.Дата КАК Период,
	|	&Организация КАК Организация,
	|	ЗНАЧЕНИЕ(Справочник.СценарииПланирования.Фактический) КАК СценарийПланирования,
	|	&Организация КАК КоррОрганизация,
	|	ОтчетПереработчикаЗапасы.Ссылка.Контрагент КАК СтруктурнаяЕдиница,
	|	ОтчетПереработчикаЗапасы.Ссылка.СтруктурнаяЕдиница КАК КоррСтруктурнаяЕдиница,
	|	ВЫБОР
	|		КОГДА &УчетПоЯчейкам
	|			ТОГДА ОтчетПереработчикаЗапасы.Ссылка.Ячейка
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Ячейки.ПустаяСсылка)
	|	КОНЕЦ КАК Ячейка,
	|	ОтчетПереработчикаЗапасы.Номенклатура.СчетУчетаЗапасов КАК СчетУчета,
	|	ВЫБОР
	|		КОГДА ОтчетПереработчикаЗапасы.Ссылка.СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = ЗНАЧЕНИЕ(Перечисление.ТипыСтруктурныхЕдиниц.Склад)
	|			ТОГДА ОтчетПереработчикаЗапасы.Ссылка.Номенклатура.СчетУчетаЗапасов
	|		ИНАЧЕ ОтчетПереработчикаЗапасы.Ссылка.Номенклатура.СчетУчетаЗатрат
	|	КОНЕЦ КАК КоррСчетУчета,
	...
	|	ОтчетПереработчикаЗапасы.Ссылка.Номенклатура КАК КоррНоменклатура,
	|	ВЫБОР
	|		КОГДА &ИспользоватьХарактеристики
	|			ТОГДА ОтчетПереработчикаЗапасы.Характеристика
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
	|	КОНЕЦ КАК Характеристика,
	|	ВЫБОР
	|		КОГДА &ИспользоватьХарактеристики
	|			ТОГДА ОтчетПереработчикаЗапасы.Ссылка.Характеристика
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
	|	КОНЕЦ КАК КоррХарактеристика,
	|	ВЫБОР
	|		КОГДА &ИспользоватьПартии
	|			ТОГДА ОтчетПереработчикаЗапасы.Партия
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)
	|	КОНЕЦ КАК Партия,
	|	ВЫБОР
	|		КОГДА &ИспользоватьПартии
	|			ТОГДА ОтчетПереработчикаЗапасы.Ссылка.Партия
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ПартииНоменклатуры.ПустаяСсылка)
	|	КОНЕЦ КАК КоррПартия,
	|	ОтчетПереработчикаЗапасы.Спецификация КАК Спецификация,
	|	ОтчетПереработчикаЗапасы.Ссылка.Спецификация КАК КоррСпецификация,
	|	ОтчетПереработчикаЗапасы.Ссылка.ЗаказПокупателя КАК ЗаказПокупателя,
	|	ОтчетПереработчикаЗапасы.Ссылка.ЗаказПокупателя КАК КоррЗаказПокупателя,
	|	ОтчетПереработчикаЗапасы.Ссылка.ДокументОснование КАК ЗаказПередчи,
	|	ОтчетПереработчикаЗапасы.Ссылка.Контрагент КАК Контрагент,
	|	ОтчетПереработчикаЗапасы.Ссылка.Договор КАК Договор,
	|	ВЫБОР
	...
	|		ИНАЧЕ ОтчетПереработчикаЗапасы.Количество * ОтчетПереработчикаЗапасы.ЕдиницаИзмерения.Коэффициент
	|	КОНЕЦ КАК Количество,
	|	0 КАК Сумма,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ОтчетПереработчикаЗапасы.Ссылка.ВалютаДокумента = КонстантаНациональнаяВалюта.Значение
	...
	|			ИНАЧЕ ОтчетПереработчикаЗапасы.Всего * ОтчетПереработчикаЗапасы.Ссылка.Курс * КурсыУпрВалюты.Кратность / (КурсыУпрВалюты.Курс * ОтчетПереработчикаЗапасы.Ссылка.Кратность)
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаОтчетПереработчика, СтруктураДополнительныеСвойства) Экспорт
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Ячейки.ПустаяСсылка)
	|	КОНЕЦ КАК Ячейка,
	|	ОтчетПереработчика.Спецификация КАК Спецификация,
	...
	|	ОтчетПереработчика.ЗаказПокупателя КАК ЗаказПокупателя,
	|	ОтчетПереработчика.ДокументОснование КАК ЗаказПоставщику,
	|	ВЫБОР
	...
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ОтчетПереработчика.ЕдиницаИзмерения) = ТИП(Справочник.КлассификаторЕдиницИзмерения)
	|			ТОГДА ОтчетПереработчика.Количество
	|		ИНАЧЕ ОтчетПереработчика.Количество * ОтчетПереработчика.ЕдиницаИзмерения.Коэффициент
	|	КОНЕЦ КАК Количество,
	|	0 КАК Сумма,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОтчетПереработчикаСерийныеНомера.Ссылка.Дата КАК Период,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
	|	ОтчетПереработчикаСерийныеНомера.Ссылка.Дата КАК ДатаСобытия,
	|	ЗНАЧЕНИЕ(Перечисление.ОперацииСерийныхНомеров.Приход) КАК Операция,
	|	&Организация КАК Организация,
	|	ОтчетПереработчикаСерийныеНомера.СерийныйНомер КАК СерийныйНомер,
	|	ОтчетПереработчикаСерийныеНомера.Ссылка.Номенклатура КАК Номенклатура,
	|	ОтчетПереработчикаСерийныеНомера.Ссылка.Характеристика КАК Характеристика,
	|	ОтчетПереработчикаСерийныеНомера.Ссылка.Партия КАК Партия,
	|	ОтчетПереработчикаСерийныеНомера.Ссылка.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	|	ОтчетПереработчикаСерийныеНомера.Ссылка.Ячейка КАК Ячейка,
	|	1 КАК Количество
	|ИЗ
	|	Документ.ОтчетПереработчика.СерийныеНомера КАК ОтчетПереработчикаСерийныеНомера
	|ГДЕ
	|	ОтчетПереработчикаСерийныеНомера.Ссылка = &Ссылка 
	|	И &ИспользоватьСерийныеНомера
	|	И НЕ ОтчетПереработчикаСерийныеНомера.Ссылка.СтруктурнаяЕдиница.ОрдерныйСклад";
