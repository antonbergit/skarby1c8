...
Процедура ЗаполнитьТабличнуюЧастьПоСпецификации(СтекСпецификацийУзлов, ТаблицаУзлы = Неопределено) Экспорт
	...
	Массив = Новый Массив;
	Массив.Добавить(Тип("СправочникСсылка.Спецификации"));
	ОписаниеТипов = Новый ОписаниеТипов(Массив, ,);
	Массив.Очистить();
	ТаблицаПродукция.Колонки.Добавить("Спецификация", ОписаниеТипов);
	
	Массив.Добавить(Тип("Число"));
	ОписаниеТипов = Новый ОписаниеТипов(Массив, ,);
	Массив.Очистить();
	ТаблицаПродукция.Колонки.Добавить("Количество", ОписаниеТипов);
	...
	Если ТаблицаУзлы = Неопределено Тогда
		...
		НоваяПродукция = ТаблицаПродукция.Добавить();
		НоваяПродукция.Спецификация = Спецификация;
		НоваяПродукция.Количество = Количество;
		...
		Если ЗначениеЗаполнено(ЕдиницаИзмерения)
			...
			НоваяПродукция.Коэффициент = ЕдиницаИзмерения.Коэффициент;
		Иначе
			НоваяПродукция.Коэффициент = 1;
		КонецЕсли;
			
		ТаблицаУзлы = ТаблицаПродукция.СкопироватьКолонки("Количество,Коэффициент,Спецификация");
...
"обнаружено рекурсивное вхождение элемента';uk='При спробі заповнити табличну частину Матеріали по специфікації,"
"виявлене рекурсивне входження елемента'")+" "+Выборка.Номенклатура+" "+НСтр("ru='в спецификации';uk='у специфікації'")+" "+Выборка.СпецификацияПродукции+"
...
Процедура ЗаполнитьПоЗаказПоставщику(ДокументСсылкаЗаказПоставщику) Экспорт
	...
	Если ТаблицаОстатков.Количество() > 0 Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Спецификация = УправлениеНебольшойФирмойСервер.ПолучитьПоУмолчаниюСпецификацию(Номенклатура, Характеристика);
			...
			Если МассивСтрокОстатков[0].КоличествоОстаток < 0 Тогда
				Количество = (КоличествоКСписанию + МассивСтрокОстатков[0].КоличествоОстаток) / Выборка.Коэффициент;
			Иначе
...
Процедура ЗаполнитьПоПриходныйОрдер(ДокументСсылкаПриходныйОрдер) Экспорт
	...
	Если ДокументСсылкаПриходныйОрдер.Запасы.Количество() > 0 Тогда
		
		СтрокаЗапасы_0 = ДокументСсылкаПриходныйОрдер.Запасы[0];
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, СтрокаЗапасы_0);
		
		РаботаССерийнымиНомерами.ЗаполнитьТЧСерийныеНомераВШапке(ЭтотОбъект, ДокументСсылкаПриходныйОрдер, СтрокаЗапасы_0);
		Спецификация = УправлениеНебольшойФирмойСервер.ПолучитьПоУмолчаниюСпецификацию(Номенклатура, Характеристика);
...
Процедура ЗаполнитьПоРасходнаяНакладная(ДокументСсылкаРасходнаяНакладная) Экспорт
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаРасходнаяНакладная, "ВидОперации")
		...
		ВызватьИсключение НСтр("ru='Отчет о переработке вводится только на основании передачи в переработку!';uk='Звіт про переробку вводиться тільки на підставі передачі в переробку!'");;
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	// Серийные номера
	РаботаССерийнымиНомерами.ПроверкаЗаполненияСерийныхНомеровВПолеВвода(Отказ, ЭтотОбъект);
