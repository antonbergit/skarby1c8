...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если Данные.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЗаказПокупателя.ЗаказНаряд") Тогда
		ЗаголовокПредставление = НСтр("ru='Заказ-наряд';uk='Замовлення-наряд'");
	Иначе
		ЗаголовокПредставление = НСтр("ru='Заказ покупателя';uk='Замовлення покупця'");
	КонецЕсли;
...
Процедура ПроверитьВозможностьВводаНаОснованииЗаказаПокупателя(ДанныеЗаполнения, ЗначенияРеквизитов) Экспорт
	...
	Если ЗначенияРеквизитов.Свойство("Проведен") Тогда
		Если НЕ ЗначенияРеквизитов.Проведен Тогда
			ТекстОшибки = НСтр("ru='Документ %Документ% не проведен. Ввод на основании непроведенного документа запрещен.';uk='Документ %Документ% не проведений. Введення на підставі непроведеного документа заборонено.'");
	...
	Если ЗначенияРеквизитов.Свойство("СостояниеЗаказа")
		...
		ТекстОшибки = НСтр("ru='Документ %Документ% в состоянии Завершен. Ввод на основании завершенного заказа запрещен.';uk='Документ %Документ% в стані Завершений. Введення на підставі завершеного замовлення заборонено.'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДанныеЗаполнения);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
			...
			Если ЗначенияРеквизитов.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
				Если ЗначенияРеквизитов.Свойство("ЗаказНарядВозврат") Тогда
					Если Не (ЗначенияРеквизитов.СостояниеЗаказа = Справочники.СостоянияЗаказНарядов.Завершен
						...
						ТекстОшибки = НСтр("ru='Документ %Документ% в состоянии %СостояниеЗаказа%. Ввод возврата от покупателя на основании запрещен.';uk='Документ %Документ% у стані %СостояниеЗаказа%. Введення повернення від покупця на підставі заборонено.'");
						ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДанныеЗаполнения);
				...
				ИначеЕсли ЗначенияРеквизитов.СостояниеЗаказа = Справочники.СостоянияЗаказНарядов.Завершен Тогда
					ТекстОшибки = НСтр("ru='Документ %Документ% в состоянии %СостояниеЗаказа%. Ввод на основании запрещен.';uk='Документ %Документ% у стані %СостояниеЗаказа%. Введення на підставі заборонено.'");
					ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДанныеЗаполнения);
					ТекстОшибки = СтрЗаменить(ТекстОшибки, "%СостояниеЗаказа%", ЗначенияРеквизитов.СостояниеЗаказа);
					ВызватьИсключение ТекстОшибки;
				КонецЕсли;
...
Функция ЕстьИсполнителиСПустойСуммойНачисления(Исполнители) Экспорт
	...
	Перем Ошибки;
	ШаблонТекстаСообщения = НСтр("ru='Не верно указана сумма начисления для сотрудника %1 в строке %2.';uk='Не вірно зазначена сума нарахування для співробітника %1 у рядку %2.'");
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	...
	ИначеЕсли ИмяМакета = "ГарантийныйТалон" Тогда
		...
		Возврат РаботаСНоменклатуройСервер.ПечатьГарантийныйТалон(МассивОбъектов, ОбъектыПечати, ИмяМакета);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='Калькуляция';uk='Калькуляція'");
	...
	КомандаПечати.Представление = НСтр("ru='Сводно о заказе';uk='Зведено про замовлення'");
	КомандаПечати.СписокФорм = "ФормаДокумента";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.МестоРазмещения = "ПодменюПечать";
	...
	// Комплект документов
	КомандаПечати = КомандыПечати.Добавить();
	ЗначениеИдентификатора = "МакетЗаказПокупателя,ЗаказНаряд,АктОбОказанииУслуг,Накладная,НакладнаяСУслугами,СчетНаОплату,СчетНаЧастичнуюОплату,СчетНаОплатуСФаксимильнойПодписью,СчетНаЧастичнуюОплатуСФаксимильнойПодписью,ТОРГ12,ТОРГ12СУслугами,ГарантийныйТалон";
	...
	КомандаПечати.Представление = НСтр("ru='Заказ-наряд';uk='Замовлення-наряд'");
	КомандаПечати.СписокФорм = "ФормаЗаказНаряда,ФормаСпискаЗаказНаряда,ФормаСпискаДокументовКОтгрузке,ФормаСпискаДокументовКОплате";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.МестоРазмещения = "ПодменюПечать";
	...
	КомандаПечати.Представление = НСтр("ru='ТОРГ12';uk='ТОРГ12'");
	КомандаПечати.СписокФорм = "ФормаЗаказНаряда,ФормаСпискаЗаказНаряда,ФормаСпискаДокументовКОтгрузке,ФормаСпискаДокументовКОплате";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.МестоРазмещения = "ПодменюПечать";
	...
	КомандаПечати.Представление = НСтр("ru='ТОРГ12 (с услугами)';uk='ТОРГ12 (з послугами)'");
	КомандаПечати.СписокФорм = "ФормаЗаказНаряда,ФормаСпискаЗаказНаряда,ФормаСпискаДокументовКОтгрузке,ФормаСпискаДокументовКОплате";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.МестоРазмещения = "ПодменюПечать";
	...
	КомандаПечати.Представление = НСтр("ru='Бланк товарного наполнения';uk='Бланк товарного наповнення'");
	КомандаПечати.СписокФорм = "ФормаЗаказНаряда,ФормаСпискаЗаказНаряда,ФормаСпискаДокументовКОтгрузке,ФормаСпискаДокументовКОплате";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.МестоРазмещения = "ПодменюПечать";
	...
	КомандаПечати.Представление = НСтр("ru='Гарантийный талон';uk='Гарантійний талон'");
	...
	КомандаПечати.Представление = НСтр("ru='Сводно о заказ-наряде';uk='Зведено про замовлення-наряд'");
...
Функция ПечатьЗаказПокупателя(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	...
	Пока Шапка.Следующий() Цикл
		...
		ОбластьМакета.Параметры.ТекстЗаголовка = НСтр("ru='Заказ покупателя № ';uk='Замовлення покупця № '", КодЯзыкаПечать)
												+ НомерДокумента
		...
		ОбластьМакета.Параметры.ПредставлениеПоставщика = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОбОрганизации, "ПолноеНаименование,ИНН,КодПоЕДРПОУ,НомерСвидетельства,ЮридическийАдрес,Телефоны,Факс,ПлательщикНалогаНаПрибыль");
		ТабличныйДокумент.Вывести(ОбластьМакета);
		
		ОбластьМакета = Макет.ПолучитьОбласть("Покупатель");
		ПредставлениеПолучателя = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОбКонтрагенте, "ПолноеНаименование,ИНН,КодПоЕДРПОУ,ЮридическийАдрес,Телефоны,");
...
Функция ПечатьСчетНаОплату(МассивОбъектов, ОбъектыПечати, ИмяМакета) Экспорт
	...
	Пока Шапка.Следующий() Цикл
		...
		КонецЕсли; 
		ОбластьМакета.Параметры.ЕДРПОУОрганизации = Шапка.ЕДРПОУОрганизации;
		...
		Если ИмяМакета = "СчетНаЧастичнуюОплату" Тогда
			...
			Иначе
				ОбластьМакета.Параметры.НДСкОплате = ?(Шапка.СуммаВключаетНДС, НСтр("ru='В том числе НДС оплаты:';uk='У тому числі ПДВ оплати:'", КодЯзыкаПечать), НСтр("ru='Сумма НДС оплаты:';uk='Сума ПДВ оплати:'", КодЯзыкаПечать));
...
Функция ПечатьАктОбОказанииУслуг(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	...
	Пока Шапка.Следующий() Цикл
		...
		Пока ВыборкаСтрокТовары.Следующий() Цикл
			...
			Иначе
				ОбластьМакета.Параметры.Запас = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(ВыборкаСтрокТовары.Товар, 
...
Функция ПечатьРасходнаяНакладная(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		Пока ВыборкаСтрокЗапасы.Следующий() Цикл
			...
			Если ИмяМакета = "Накладная" И (ВыборкаСтрокЗапасы.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга ИЛИ ВыборкаСтрокЗапасы.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Работа) Тогда
				Продолжить;
			КонецЕсли;
...
Функция ПечатьТОРГ12(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		СведенияОПоставщике       = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.Организация,      Шапка.ДатаДокумента, , , КодЯзыкаПечать);
		...
		СведенияОПокупателе       = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.Контрагент,       Шапка.ДатаДокумента, , , КодЯзыкаПечать);
		...
		ОбластьМакетаПодвал.Параметры.КоличествоПорядковыхНомеровЗаписейПрописью = ЧислоПрописью(КоличествоСтрок, "Л="+КодЯзыкаПечать, ",,,,,,,,0");
		ОбластьМакетаПодвал.Параметры.ВсегоМестПрописью     = ?(ИтогоМест = 0, "", ЧислоПрописью(ИтогоМест, "Л="+КодЯзыкаПечать, ",,,С,,,,,0")); 
...
Функция ПечатьЗаказНаряд(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		ОбластьМакета = Макет.ПолучитьОбласть("Заголовок");
		ОбластьМакета.Параметры.ТекстЗаголовка = НСтр("ru='Заказ-наряд № ';uk='Замовлення-наряд № '", КодЯзыкаПечать)
												+ НомерДокумента
												+ НСтр("ru=' от ';uk=' від '", КодЯзыкаПечать)
												+ Формат(Шапка.ДатаДокумента, "Л="+КодЯзыкаПечать+"; ДЛФ=DD");
		...
		ОбластьМакета = Макет.ПолучитьОбласть("Поставщик");
		ОбластьМакета.Параметры.ПредставлениеПоставщика = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОбОрганизации, "ПолноеНаименование,ИНН,ЮридическийАдрес,Телефоны,");
		ТабличныйДокумент.Вывести(ОбластьМакета);
		
		ОбластьМакета = Макет.ПолучитьОбласть("Покупатель");
		ОбластьМакета.Параметры.ПредставлениеПолучателя = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОбКонтрагенте, "ПолноеНаименование,ИНН,ЮридическийАдрес,Телефоны,");
		ТабличныйДокумент.Вывести(ОбластьМакета);
...
Функция ПечатьБланкТоварногоНаполнения(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		ОбластьМакета.Параметры.ВремяПечати = НСтр("ru='Дата и время печати: ';uk='Дата і час друку: '", КодЯзыкаПечать)
												 + ТекущаяДата()
...
Функция ПечатьКалькуляция(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	...
	Пока Шапка.Следующий() Цикл
		...
		Если НЕ Шапка.КалькуляцияРассчитана Тогда
			...
			НСтр("ru='Калькуляция не рассчитана для заказа %1 от %2';uk='Калькуляція не розрахована для замовлення %1 від %2'"),
			НомерДокумента,
		...
		ОбластьМакета = Макет.ПолучитьОбласть("Заголовок");
		ОбластьМакета.Параметры.ТекстЗаголовка = НСтр("ru='Калькуляция заказа покупателя № ';uk='Калькуляція замовлення покупця № '")
												+ НомерДокумента
												+ НСтр("ru=' от ';uk=' від '")
												+ Формат(Шапка.ДатаДокумента, "Л="+КодЯзыкаПечать+"; ДЛФ=DD");
...
Процедура ВывестиСекциюЗаголовок(Макет, Результат, Параметры)
	...
	Область.Параметры.ТекущаяДата = Формат(ТекущаяДата(), НСтр("ru=""ДФ='dd.MM.yyyy ЧЧ:мм'"";uk=""ДФ='dd.MM.yyyy ЧЧ:мм'"""));
	...
	Если Шапка.ПометкаУдаления Тогда
		Показатели.Вставить("СостояниеДокумента", НСтр("ru='Помечен на удаление';uk='Позначений на видалення'"));
	ИначеЕсли Шапка.Проведен Тогда
		Показатели.Вставить("СостояниеДокумента", НСтр("ru='Проведен';uk='Проведений'"));
	Иначе
		Показатели.Вставить("СостояниеДокумента", НСтр("ru='Не проведен';uk='Не проведений'"));
	КонецЕсли; 
...
Процедура ВывестиСекциюДоходыРасходы(Макет, Результат, Параметры)
	...
	Пока ВыборкаДопРасходы.Следующий() Цикл
		...
		Если ВыборкаДопРасходы.ДополнительныйРасход Тогда
			ОбластьГруппа.Параметры.Группа = НСтр("ru='Расходы';uk='Витрати'");;
		Иначе
			ОбластьГруппа.Параметры.Группа = НСтр("ru='Запасы';uk='Запаси'");;
		КонецЕсли; 
				...
				Пока ВыборкаАналитика.Следующий() Цикл
					Если ВыборкаАналитика.Количество()>1
						...
						Если НЕ ЗначениеЗаполнено(ВыборкаАналитика.Аналитика) Тогда
							ОбластьАналитика.Параметры.Аналитика = НСтр("ru='<Без аналитики>';uk='<Без аналітики>'");
						КонецЕсли; 
	...
	Если НЕ ВыборкаИтоги.ФинансовыйРезультатРассчитан Тогда
		...
		Область.Параметры.Текст = НСтр("ru='* По заказу не рассчитан финансовый результат. После закрытия месяца сумма фактических расходов может измениться.';uk='* По замовленню не розрахований фінансовий результат. Після закриття місяця сума фактичних витрат може змінитися.'");
		Результат.Вывести(Область, 1);
	КонецЕсли; 
...
Процедура ВывестиСекциюОбсуждение(Макет, Результат, Параметры)
	...
	Пока ВыборкаДетали.Следующий() Цикл
		...
		Если Год(ВыборкаДетали.ДатаСообщения)=Год(ТекущаяДатаСеанса()) Тогда
			Область.Параметры.ДатаСообщения = Формат(ВыборкаДетали.ДатаСообщения, НСтр("ru=""ДФ='д МММ ЧЧ:мм'"";uk=""ДФ='д МММ ЧЧ:мм'"""));
		Иначе
			Область.Параметры.ДатаСообщения = Формат(ВыборкаДетали.ДатаСообщения, НСтр("ru=""ДФ='д МММ гггг ЧЧ:мм'"";uk=""ДФ='д МММ гггг ЧЧ:мм'"""));
		КонецЕсли;
...
Функция ОбластьПоПоказателю(Макет, Показатель, Показатели, Левая)
	...
	Если Показатель="Номер" Тогда
		...
		Область.Параметры.Показатель = НСтр("ru='Номер заказа:';uk='Номер замовлення :'");
	...
	Если Показатель="Старт" Тогда
		Область = Макет.ПолучитьОбласть("ОбщиеДанные|"+Колонка);
		Область.Параметры.Показатель = НСтр("ru='Старт:';uk='Старт:'");
		СтрокаФормата = НСтр("ru='д МММ';uk='д МММ'");
		Если Год(ТекущаяДатаСеанса())<>Год(Показатели.Старт) Тогда
			СтрокаФормата = СтрокаФормата+НСтр("ru=' гггг';uk=' гггг'");
		КонецЕсли; 
		Если НачалоДня(Показатели.Старт)<>Показатели.Старт Тогда
			СтрокаФормата = СтрокаФормата+НСтр("ru=' ЧЧ:мм';uk=' ЧЧ:мм'");
		КонецЕсли;
	...
	Если Показатель="Финиш" Тогда
		Область = Макет.ПолучитьОбласть("ОбщиеДанные|"+Колонка);
		Область.Параметры.Показатель = НСтр("ru='Финиш:';uk='Фініш:'");
		СтрокаФормата = НСтр("ru='д МММ';uk='д МММ'");
	...
	Если Показатель="ПлановаяДатаОтгрузки" Тогда
		Область = Макет.ПолучитьОбласть("ОбщиеДанные|"+Колонка);
		Область.Параметры.Показатель = НСтр("ru='План. отгрузка:';uk='План. відвантаження:'");
		Если Год(ТекущаяДатаСеанса())=Год(Показатели.ПлановаяДатаОтгрузки) Тогда
			Область.Параметры.Значение = Формат(Показатели.ПлановаяДатаОтгрузки, НСтр("ru=""ДФ='д МММ'"";uk=""ДФ='д МММ'"""));
		Иначе
			Область.Параметры.Значение = Формат(Показатели.ПлановаяДатаОтгрузки, НСтр("ru=""ДФ='д МММ гггг'"";uk=""ДФ='д МММ гггг'"""));
		КонецЕсли; 
		Возврат Область;
	КонецЕсли; 
	...
	Если Показатель="СуммаДокумента" Тогда
		Область = Макет.ПолучитьОбласть("ОбщиеДанные|"+Колонка);
		Область.Параметры.Показатель = НСтр("ru='Сумма заказа:';uk='Сума замовлення:'");
	...
	Если Показатель="Валюта" Тогда
		Область = Макет.ПолучитьОбласть("ОбщиеДанные|"+Колонка);
		Область.Параметры.Показатель = НСтр("ru='Валюта:';uk='Валюта:'");
	...
	Если Показатель="СуммаСкидки" Тогда
		...
		Область = Макет.ПолучитьОбласть("ОбщиеДанные|"+Колонка);
		Область.Параметры.Показатель = СтрШаблон(НСтр("ru='Скидка (%1):';uk='Знижка (%1):'"), Строка(ПроцентСкиди)+"%");
	...
	Если Показатель="СуммаРучнойСкидки" Тогда
		...
		Если Показатели.СуммаРучнойСкидки=0 Тогда
			ЗаголовокПоказателя = НСтр("ru='Ручная скидка (%1):';uk='Ручна знижка (%1):'");
		Иначе
			ЗаголовокПоказателя = НСтр("ru='- ручная (%1):';uk='- ручна (%1):'");
		КонецЕсли; 
	...
	Если Показатель="СуммаАвтоматическойСкидки" Тогда
		...
		Область = Макет.ПолучитьОбласть("ОбщиеДанные|"+Колонка);
		Если Показатели.СуммаРучнойСкидки=0 Тогда
			ЗаголовокПоказателя = НСтр("ru='Авт. скидка (%1):';uk='Авт. знижка (%1):'");
		Иначе
			ЗаголовокПоказателя = НСтр("ru='- авто. (%1):';uk='- авто. (%1):'");
		КонецЕсли; 
		Область.Параметры.Показатель = СтрШаблон(ЗаголовокПоказателя, Строка(ПроцентСкиди)+"%");
	...
	Если Показатель="Организация" Тогда
		Область = Макет.ПолучитьОбласть("ОбщиеДанные|"+Колонка);
		Область.Параметры.Показатель = НСтр("ru='Организация:';uk='Організація:'");
	...
	Если Показатель="Подразделение" Тогда
		Область = Макет.ПолучитьОбласть("ОбщиеДанные|"+Колонка);
		Область.Параметры.Показатель = НСтр("ru='Подразделение:';uk='Підрозділ:'");
	...
	Если Показатель="Склад" Тогда
		Область = Макет.ПолучитьОбласть("ОбщиеДанные|"+Колонка);
		Область.Параметры.Показатель = НСтр("ru='Склад:';uk='Склад:'");
	...
	Если Показатель="Ответственный" Тогда
		Область = Макет.ПолучитьОбласть("ОбщиеДанные|"+Колонка);
		Область.Параметры.Показатель = НСтр("ru='Менеджер:';uk='Менеджер:'");
	...
	Если Показатель="КонтактноеЛицо" Тогда
		Область = Макет.ПолучитьОбласть("ОбщиеДанные|"+Колонка);
		Область.Параметры.Показатель = НСтр("ru='Контакт:';uk='Контакт:'");
	...
	Если Показатель="СуммаОплаты" Тогда
		...
		Если НЕ ЗначениеЗаполнено(Показатели.ДокументОплаты) Тогда
			Область = Макет.ПолучитьОбласть("ОбщиеДанныеБезЗаголовкаВажно|"+Колонка);
			Область.Параметры.Значение = НСтр("ru='Не оплачен';uk='Не сплачений'");
		ИначеЕсли ПроцентОплаты=100 Тогда
			Область = Макет.ПолучитьОбласть("ОбщиеДанныеБезЗаголовкаВажно|"+Колонка);
			Если Год(ТекущаяДатаСеанса())=Год(Показатели.ДатаОплаты) Тогда
				Область.Параметры.Значение = СтрШаблон(НСтр("ru='Оплачен %1';uk='Сплачений %1'"), Формат(Показатели.ДатаОплаты, НСтр("ru=""ДФ='д МММ'"";uk=""ДФ='д МММ'""")));
			Иначе
				Область.Параметры.Значение = СтрШаблон(НСтр("ru='Оплачен %1';uk='Сплачений %1'"), Формат(Показатели.ДатаОплаты, НСтр("ru=""ДФ='д МММ гггг'"";uk=""ДФ='д МММ гггг'""")));
			КонецЕсли;
		Иначе
			Область = Макет.ПолучитьОбласть("ОбщиеДанныеВажно|"+Колонка);
			Область.Параметры.Показатель = СтрШаблон(НСтр("ru='Оплачено%1:';uk='Сплачене%1:'"), ?(ПроцентОплаты<>100, " ("+Строка(ПроцентОплаты)+"%)", ""));
	...
	Если Показатель="СуммаОтгрузки" Тогда
		...
		Если НЕ ЗначениеЗаполнено(Показатели.ДокументОтгрузки) Тогда
			Область = Макет.ПолучитьОбласть("ОбщиеДанныеБезЗаголовкаВажно|"+Колонка);
			Область.Параметры.Значение = НСтр("ru='Не отгружен';uk='Не відвантажений'");
		ИначеЕсли ПроцентОтгрузки=100 Тогда
			Область = Макет.ПолучитьОбласть("ОбщиеДанныеБезЗаголовкаВажно|"+Колонка);
			Если Год(ТекущаяДатаСеанса())=Год(Показатели.ДатаОтгрузки) Тогда
				Область.Параметры.Значение = СтрШаблон(НСтр("ru='Отгружен %1';uk='Відвантажений %1'"), Формат(Показатели.ДатаОтгрузки, НСтр("ru=""ДФ='д МММ'"";uk=""ДФ='д МММ'""")));
			Иначе
				Область.Параметры.Значение = СтрШаблон(НСтр("ru='Отгружен %1';uk='Відвантажений %1'"), Формат(Показатели.ДатаОтгрузки, НСтр("ru=""ДФ='д МММ гггг'"";uk=""ДФ='д МММ гггг'""")));
			КонецЕсли; 
		Иначе
			Область = Макет.ПолучитьОбласть("ОбщиеДанныеВажно|"+Колонка);
			Область.Параметры.Показатель = СтрШаблон(НСтр("ru='Отгружено%1:';uk='Відвантажене%1:'"), ?(ПроцентОтгрузки<>100, " ("+ПроцентОтгрузки+"%)", ""));
...
Процедура ЗаполнитьСписокТекущихДелЗаказПокупателя(ТекущиеДела)
	...
	ГруппаДел	= НСтр("ru='Заказы покупателей';uk='Замовлення покупців'");
	...
	Дело.Важное			= Истина;
	Дело.Представление	= НСтр("ru='Просрочено выполнение';uk='Прострочене виконання'");
	...
	Дело.Важное			= Истина;
	Дело.Представление	= НСтр("ru='Просрочена оплата';uk='Прострочена оплата'");
	...
	Дело.Важное			= Ложь;
	Дело.Представление	= НСтр("ru='Не завершенные';uk='Не завершені'");
...
Процедура ЗаполнитьСписокТекущихДелЗаказНаряд(ТекущиеДела)
	...
	ГруппаДел	= НСтр("ru='Заказ наряды';uk='Замовлення наряди'");
...
Процедура СформироватьТаблицаЗапасыМатериалы(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПеремещениеЗапасов", НСтр("ru='Перемещение запасов';uk='Переміщення запасів'"));
...
Процедура СформироватьТаблицаЗапасыИсполнители(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("НачислениеЗарплаты", НСтр("ru='Начисление зарплаты';uk='Нарахування зарплати'"));	
	Запрос.УстановитьПараметр("РазноскаЗарплаты", 	НСтр("ru='Отнесение затрат на продукцию';uk='Віднесення витрат на продукцію'"));	
...
Процедура СформироватьТаблицаНДСПродаж(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("Организация", ДокументСсылкаЗаказПокупателя.Организация);
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения
	|ИЗ
	|	ВременнаяТаблицаРаботы КАК ТаблицаНДСПродаж
	|ГДЕ
	|	ТаблицаНДСПродаж.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаказПокупателя.ЗаказНаряд)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ТаблицаНДСПродаж.Период,
	|	&Организация,
	|	СУММА(ТаблицаНДСПродаж.СуммаНДСЗакупкиПродажи),
	|	СУММА(ТаблицаНДСПродаж.Сумма),
	|	ТаблицаНДСПродаж.Договор,
	|	ТаблицаНДСПродаж.СтавкаНДС,
	|	ЗНАЧЕНИЕ(Перечисление.СобытияНДСПродаж.РеализацияПокупателю),
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|ИЗ
	|	ВременнаяТаблицаТовары КАК ТаблицаНДСПродаж
	|ГДЕ
	|	ТаблицаНДСПродаж.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаказПокупателя.ЗаказНаряд)
	|
	|СГРУППИРОВАТЬ ПО
	...
	Если СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРасчетыСПокупателями.Количество() = 0 Тогда
	    СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаНДСПродаж", Новый ТаблицаЗначений);
	Иначе	
...
Процедура ИнициализироватьДанныеДокументаЗаказНаряд(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства) 
	...
	|	ЗаказНарядРаботы.Ссылка.ВидОперации
	...
	|	ЗаказНарядТовары.Ссылка.ВидОперации
	...
	|	И ЗаказНарядИсполнители.Ссылка.СостояниеЗаказа = ЗНАЧЕНИЕ(Справочник.СостоянияЗаказНарядов.Завершен)
	|	И ЗаказНарядИсполнители.Ссылка.ВариантЗавершения = ЗНАЧЕНИЕ(Перечисление.ВариантыЗавершенияЗаказа.Успешно)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("РаспределениеЗапасов", НСтр("ru='Распределение запасов';uk='Розподіл запасів'"));
	...
	СформироватьТаблицаНДСПродаж(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства);
...
Процедура ИнициализироватьДанныеДокументаЗаказПокупателя(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства) 
	...
	|	ЗаказПокупателяЗапасы.Ссылка.ВидОперации,
	|	ЗаказПокупателяЗапасы.СуммаНДС,
	|	ЗаказПокупателяЗапасы.Ссылка.Договор,
	|	ЗаказПокупателяЗапасы.СтавкаНДС
	...
	|		ИНАЧЕ ЗаказПокупателяЗапасы.Количество * ЗаказПокупателяЗапасы.ЕдиницаИзмерения.Коэффициент
	|	КОНЕЦ КАК Количество,
	|	ЗаказПокупателяЗапасы.гвСерийныйНомер КАК СерийныйНомер
	|ИЗ
	|	Документ.ЗаказПокупателя.Запасы КАК ЗаказПокупателяЗапасы
	|ГДЕ
	|	ЗаказПокупателяЗапасы.Ссылка = &Ссылка
	|	И ЗаказПокупателяЗапасы.гвЗарезервировано
	|	И НЕ ЗаказПокупателяЗапасы.Ссылка.ВариантЗавершения = ЗНАЧЕНИЕ(Перечисление.ВариантыЗавершенияЗаказа.Отменен)";
	// ...Гунько 2020-12-22
	...
	Запрос.УстановитьПараметр("ИспользоватьХарактеристики", СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьХарактеристики);
	Запрос.УстановитьПараметр("ИспользоватьПартии",  СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьПартии);
	Запрос.УстановитьПараметр("РезервированиеЗапасов", НСтр("ru='Резервирование запасов';uk='Резервування запасів'"));
	...
	// Гунько 2020-12-23
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаРезервированиеТоваровПоСерийныхНомерах", МассивРезультатов[7].Выгрузить());
	// ...Гунько 2020-12-23
...
// Гунько 2021-01-03
Функция гвВыполнитьЗапросКонтроляОстатковИРезервированияСерийныхНомеров(ДокументОбъектЗаказПокупателя)
	...
	|	ЗаказПокупателяЗапасы.гвСерийныйНомер,
	|	ЗаказПокупателяЗапасы.Ссылка.Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	гвРезервированиеТоваровПоСерийныхНомерахОстатки.Номенклатура.Наименование КАК Номенклатура,
	|	гвРезервированиеТоваровПоСерийныхНомерахОстатки.СерийныйНомер.Наименование КАК СерийныйНомер,
	|	гвРезервированиеТоваровПоСерийныхНомерахОстатки.ЗаказПокупателя.Представление КАК ЗаказПокупателя,
	|	гвРезервированиеТоваровПоСерийныхНомерахОстатки.ЗаказПокупателя.Ответственный.Наименование КАК Ответственный
	|ИЗ
	|	РегистрНакопления.гвРезервированиеТоваровПоСерийныхНомерах.Остатки(
	|			,
	|			Организация = &Организация
	|				И (Номенклатура, Характеристика, Партия, СерийныйНомер) В
	|					(ВЫБРАТЬ
	|						ВТ_ТЧТовары.Номенклатура,
	|						ВТ_ТЧТовары.Характеристика,
	|						ВТ_ТЧТовары.Партия,
	|						ВТ_ТЧТовары.СерийныйНомер
	|					ИЗ
	|						ВТ_ТЧТовары КАК ВТ_ТЧТовары)) КАК гвРезервированиеТоваровПоСерийныхНомерахОстатки
	|ГДЕ
	|	гвРезервированиеТоваровПоСерийныхНомерахОстатки.ЗаказПокупателя <> &Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ТЧТовары.Номенклатура.Наименование КАК Номенклатура,
	|	ВТ_ТЧТовары.СерийныйНомер.Наименование КАК СерийныйНомер,
	|	ЕСТЬNULL(СерийныеНомераОстатки.КоличествоОстаток, 0) - ВТ_ТЧТовары.Количество КАК КоличествоОстаток
	|ИЗ
	...
	|							ВТ_ТЧТовары.СерийныйНомер
	|						ИЗ
	|							ВТ_ТЧТовары КАК ВТ_ТЧТовары)) КАК СерийныеНомераОстатки
	|		ПО ВТ_ТЧТовары.Номенклатура = СерийныеНомераОстатки.Номенклатура
	|			И ВТ_ТЧТовары.Характеристика = СерийныеНомераОстатки.Характеристика
	|			И ВТ_ТЧТовары.Организация = СерийныеНомераОстатки.Организация
	|			И ВТ_ТЧТовары.Партия = СерийныеНомераОстатки.Партия
	|			И ВТ_ТЧТовары.СерийныйНомер = СерийныеНомераОстатки.СерийныйНомер
	|ГДЕ
	|	ЕСТЬNULL(СерийныеНомераОстатки.КоличествоОстаток, 0) - ВТ_ТЧТовары.Количество < 0";
	
	Запрос.УстановитьПараметр("Ссылка", ДокументОбъектЗаказПокупателя.Ссылка);
...
Процедура гвВыполнитьКонтрольРезервирования(ДокументОбъектЗаказПокупателя, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	
	лМассивРезультатов = гвВыполнитьЗапросКонтроляОстатковИРезервированияСерийныхНомеров(ДокументОбъектЗаказПокупателя);
	
	// Серийные номера ранее зарезервированные другими заказами
	Если НЕ лМассивРезультатов[1].Пустой() Тогда
		лВыборкаРезультат = лМассивРезультатов[1].Выбрать();
		Сообщить("Уже зарезервовано:");
		Пока лВыборкаРезультат.Следующий() Цикл
			Сообщить(лВыборкаРезультат.Номенклатура + ", серійний номер: " + лВыборкаРезультат.СерийныйНомер 
			+ ", документ: " + лВыборкаРезультат.ЗаказПокупателя + ", менеджер: " + лВыборкаРезультат.Ответственный + ".");
			Отказ = Истина;
		КонецЦикла;
	КонецЕсли;
	
	// Недостаточно остатков серийных номеров
	Если НЕ лМассивРезультатов[2].Пустой() Тогда
		лВыборкаРезультат = лМассивРезультатов[2].Выбрать();
		Сообщить("Не достатньо залишків серійних номерів:");
		Пока лВыборкаРезультат.Следующий() Цикл
			Сообщить(лВыборкаРезультат.Номенклатура + ", серійний номер: " + лВыборкаРезультат.СерийныйНомер + " - " + (- лВыборкаРезультат.КоличествоОстаток));
			Отказ = Истина;
		КонецЦикла;
	КонецЕсли;
