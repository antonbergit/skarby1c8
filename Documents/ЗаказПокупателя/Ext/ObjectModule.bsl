...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если АвторасчетНДС Тогда
		...
		Если ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд тогда
			...
			УчетНДССервер.ПересчитатьНДСсУчетомПогрешностиОкругления(Работы, ЭтотОбъект, ПогрешностиОкругления, "Работы");
			// пересчет сумм НДС с учетом ошибок округления
			УчетНДССервер.ПересчитатьНДСсУчетомПогрешностиОкругления(Запасы, ЭтотОбъект, ПогрешностиОкругления, "Товары");
		Иначе
			// пересчет сумм НДС с учетом ошибок округления
			УчетНДССервер.ПересчитатьНДСсУчетомПогрешностиОкругления(Запасы, ЭтотОбъект, ПогрешностиОкругления, "Товары,услуги");
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// Гунько 2021-01-09
	Документы.ЗаказПокупателя.гвВыполнитьКонтрольРезервирования(ЭтотОбъект, ДополнительныеСвойства, Отказ);
	// ...Гунько 2021-01-09
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Запасы.Итог("Резерв") > 0 Тогда
		...
		Для каждого СтрокаЗапасы Из Запасы Цикл
			...
			И НЕ ЗначениеЗаполнено(СтруктурнаяЕдиницаРезерв) Тогда
				...
				ТекстСообщения = НСтр("ru='Не заполнен склад резерва';uk='Не заповнений склад резерву'");
	...
	Если ЗапланироватьОплату
		...
		ТекстСообщения = НСтр("ru='Поле ""Дата оплаты"" не заполнено.';uk='Поле ""Дата оплати"" не заповнене.'");
	...
	Если Константы.ФункциональнаяОпцияРезервированиеЗапасов.Получить() Тогда
		...
		Если ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу Тогда
			...
			Для каждого СтрокаЗапасы Из Запасы Цикл
				...
				Если СтрокаЗапасы.Резерв > СтрокаЗапасы.Количество Тогда
					...
					ТекстСообщения = НСтр("ru='В строке №%Номер% табл. части ""Товары, услуги"" количество резервируемых позиций превышает общее количество запасов.';uk='У рядку №%Номер% табл. частини ""Товари, послуги"" кількість позицій, що резервуються, перевищує загальну кількість запасів.'");
		...
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
			...
			Если СтрокаСостояния <> Неопределено Тогда
				...
				Для Каждого СтрокаЗапасы Из Запасы Цикл
					...
					Если ЗаказНарядВСостоянииВыполнения
						...
						ТекстСообщения = НСтр("ru='В строке №%Номер% табл. части ""Товары"" количество списываемых позиций из резерва превышает общее количество запасов.';uk='У рядку №%Номер% табл. частини ""Товари"" кількість позицій, що списуються з резерву, перевищує загальну кількість запасів.'");
						ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", СтрокаЗапасы.НомерСтроки);
						УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
							ЭтотОбъект,
							ТекстСообщения,
					...
					ИначеЕсли СтрокаЗапасы.Резерв > СтрокаЗапасы.Количество Тогда	
						...
						ТекстСообщения = НСтр("ru='В строке №%Номер% табл. части ""Товары"" количество резервируемых позиций превышает общее количество запасов.';uk='У рядку №%Номер% табл. частини ""Товари"" кількість позицій, що резервуються, перевищує загальну кількість запасів.'");
						ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", СтрокаЗапасы.НомерСтроки);
						УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
							ЭтотОбъект,
							ТекстСообщения,
				...
				Для Каждого СтрокаЗапасы Из Материалы Цикл
					...
					Если ЗаказНарядВСостоянииВыполнения
						...
						ТекстСообщения = НСтр("ru='В строке №%Номер% табл. части ""Материалы"" количество списываемых позиций из резерва превышает общее количество запасов.';uk='У рядку №%Номер% табл. частини ""Матеріали"" кількість позицій, що списуються з резерву, перевищує загальну кількість запасів.'");
						ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", СтрокаЗапасы.НомерСтроки);
						УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
							ЭтотОбъект,
							ТекстСообщения,
					...
					ИначеЕсли СтрокаЗапасы.Резерв > СтрокаЗапасы.Количество Тогда
						...
						ТекстСообщения = НСтр("ru='В строке №%Номер% табл. части ""Материалы"" количество резервируемых позиций превышает общее количество запасов.';uk='У рядку №%Номер% табл. частини ""Матеріали"" кількість позицій, що резервуються, перевищує загальну кількість запасів.'");
						ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", СтрокаЗапасы.НомерСтроки);
						УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
							ЭтотОбъект,
							ТекстСообщения,
			...
			РаботаССерийнымиНомерами.ПроверкаЗаполненияСерийныхНомеров(Отказ, Материалы, СерийныеНомераМатериалы, ЭтотОбъект, "КлючСвязиСерийныеНомера");
	...
	Если ЕстьРучныеСкидки ИЛИ ЕстьАвтоматическиеСкидки Тогда
		Для каждого СтрокаЗапасы Из Запасы Цикл
			...
			Если СтрокаЗапасы.ПроцентСкидкиНаценки <> 100 И ТекСуммаСкидки < ТекСумма
				...
				ТекстСообщения = НСтр("ru='Не заполнена колонка ""Сумма"" в строке %Номер% списка ""Товары, работы, услуги"".';uk='Не заповнений стовпчик ""Сума"" у рядку %Номер% списку ""Товари, роботи, послуги"".'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", СтрокаЗапасы.НомерСтроки);
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
					ЭтотОбъект,
					ТекстСообщения,
	...
	Если ЕстьРучныеСкидки Тогда
		Для каждого СтрокаРаботы Из Работы Цикл
			...
			Если СтрокаРаботы.ПроцентСкидкиНаценки <> 100 И ТекСуммаСкидки < ТекСумма
				...
				ТекстСообщения = НСтр("ru='Не заполнена колонка ""Сумма"" в строке %Номер% списка ""Работы"".';uk='Не заповнений стовпчик ""Сума"" у рядку %Номер% списку ""Роботи"".'");
	...
	Если ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд
		...
		Для Каждого Стр Из Работы Цикл
			Если НЕ УправлениеНебольшойФирмойСервер.РазрешенаПродажаНоменклатурыПоДоговоруОбслуживания(Договор, Стр.Номенклатура) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Запрещено проводить незапланированные работы/услуги по текущему договору обслуживания!';uk='Заборонено проводити незаплановані роботи/послуги за поточним договором обслуговування!'"),
		...
		Для Каждого Стр Из Запасы Цикл
			Если НЕ УправлениеНебольшойФирмойСервер.РазрешенаПродажаНоменклатурыПоДоговоруОбслуживания(Договор, Стр.Номенклатура, Стр.Характеристика) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Запрещено проводить незапланированные товары по текущему договору обслуживания!';uk='Заборонено проводити незаплановані товари за поточним договором обслуговування!'"),
					Договор.ДоговорОбслуживанияТарифныйПлан,
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Работы", Стр.НомерСтроки, "Номенклатура"),,
					Отказ
				);
