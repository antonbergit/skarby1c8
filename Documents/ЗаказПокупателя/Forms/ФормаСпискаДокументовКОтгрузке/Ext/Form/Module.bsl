...
Процедура СоздатьОтгрузку(Команда)
	...
	Иначе
		...
		Если СтруктураДанных.СформироватьНесколькоЗаказов Тогда
			...
			ТекстСообщения = НСтр("ru='Заказы отличаются данными (%ПредставлениеДанных%) шапки документов! Сформировать несколько расходных накладных?';uk='Замовлення відрізняються даними (%ПредставлениеДанных%) шапки документів! Сформувати декілька видаткових накладних?'");
