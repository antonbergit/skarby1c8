...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.ИмяТабличнойЧасти = "Работы";
	...
	СформироватьСписокВыбораКонтактныхЛиц();
	...
	Иначе
		...
		// Команда скрыта
		Элементы[ИмяКомандыПечатиКалькуляция].Видимость = Ложь;
	КонецЕсли; 
	// Конец Калькуляция
	
	// Гунько 2020-12-22
	Элементы.ЗапасыСпецификация.Видимость = Ложь;
...
Процедура ЗапасыНоменклатураПриИзменении(Элемент) Экспорт 
// ...Гунько 2021-02-01	
...
Процедура ИзменитьРезервЗаполнитьПоОстаткам(Команда)
	...
	Если Объект.Запасы.Количество() = 0 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Табличная часть ""Товары, услуги"" не заполнена!';uk='Таблична частина ""Товари, послуги"" не заповнена!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	...
	Если ТипЗнч(РезультатЗакрытия) = Тип("Структура") 
		...
		ИначеЕсли ЗначениеЗаполнено(РезультатЗакрытия.ДисконтнаяКарта) И ЗначениеЗаполнено(РезультатЗакрытия.Контрагент) И Объект.Контрагент.Пустая() Тогда
			...
			ПоказатьОповещениеПользователя(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В документе заполнен контрагент и считана дисконтная карта %1';uk='У документі заповнений контрагент і зчитана дисконтна картка %1'"), Объект.ДисконтнаяКарта),
				БиблиотекаКартинок.Информация32);
		Иначе
			Объект.ДисконтнаяКарта = РезультатЗакрытия.ДисконтнаяКарта;
			Объект.ПроцентСкидкиПоДисконтнойКарте = РезультатЗакрытия.ПроцентСкидкиПоДисконтнойКарте;
		...
		Объект.АвторасчетНДС = РезультатЗакрытия.АвторасчетНДС;
		...
		Если РезультатЗакрытия.ПерезаполнитьЦены Тогда
			...
			Если РезультатЗакрытия.АвторасчетНДС Тогда
		...
		Если РезультатЗакрытия.АвторасчетНДС <> РезультатЗакрытия.ПредАвторасчетНДС И РезультатЗакрытия.АвторасчетНДС Тогда
			ПересчитаемСуммуТабличнойЧастиПоФлагуАвторасчетНДС();	
		КонецЕсли;
...
Процедура ПересчитаемСуммуТабличнойЧастиПоФлагуАвторасчетНДС()
	...
	УчетНДССервер.ПересчитатьНДСсУчетомПогрешностиОкругления(Объект.Запасы, Объект, ПогрешностиОкругления, "Товары,услуги");
	
КонецПроцедуры // ПересчитаемСуммуТабличнойЧастиПоФлагуАвторасчетНДС()	

&НаСервере
Процедура СформироватьСписокВыбораКонтактныхЛиц()
	...
	Пока Выборка.Следующий() Цикл
		Элементы.Получил.СписокВыбора.Добавить(Выборка.Представление);
	КонецЦикла;
...
Процедура ОбработатьПодготовленныеДанные(РезультатЗагрузки)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(Нстр("ru='Загрузка данных';uk='Завантаження даних'"), УровеньЖурналаРегистрации.Ошибка, Метаданные.Документы.ЗаказПокупателя, , ОписаниеОшибки());
		ОтменитьТранзакцию();
...
"Данные будут записаны.';uk='Заповнення калькуляції можливо тільки після запису даних."
...
Функция ПриИзмененииКалькуляцииСервер(АдресНастроек)
	...
	Объект.ВидЦенКалькуляции = СтруктураДанных.ВидЦенКалькуляции;
...
Функция ЗаполнитьПараметрыОткрытияКалькуляции(СтруктураПараметры)
	...
	СтруктураДанных.Вставить("СуммаВключаетНДС", Объект.СуммаВключаетНДС);
	СтруктураДанных.Вставить("ВидЦенКалькуляции", Объект.ВидЦенКалькуляции);
