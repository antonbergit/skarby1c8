...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ru='Заказ-наряд (Создание)';uk='Замовлення-наряд (Створення)'");
	КонецЕсли;
	...
	Иначе
		...
		// Команда скрыта
		Элементы[ИмяКомандыПечатиСводныйОтчет].Видимость = Ложь;
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ПодборСерий"
		...
		Если Элементы.ЗНСтраницы.ТекущаяСтраница = Элементы.ЗНГруппаРаботы Тогда
			ИзменилосьКоличество = ПолучитьСерийныеНомераМатериалыИзХранилища(Параметр.АдресВоВременномХранилище, Параметр.КлючСтроки);
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Элементы.ВариантЗавершенияРасширеннаяПодсказка.Заголовок = СтрШаблон(НСтр("ru='Предыдущее состояние: %1';uk='Попередній стан: %1'"),
...
Процедура ЗНСтартПриИзменении(Элемент)
	...
	Если Объект.Старт > Объект.Финиш Тогда
		...
		Сообщить(НСтр("ru='Дата старта не может быть больше даты финиша.';uk='Дата старту не може бути більше дати фінішу.'"));
	Иначе
		ПриИзмененииСтарт = Объект.Старт;
	КонецЕсли;
...
Процедура ЗНФинишПриИзменении(Элемент)
	...
	Если Объект.Финиш < Объект.Старт Тогда
		...
		Сообщить(НСтр("ru='Дата финиша не может быть меньше даты старта.';uk='Дата фінішу не може бути менше дати старту.'"));
	Иначе
		ПриИзмененииФиниш = Объект.Финиш;
	КонецЕсли;
...
Процедура ЗНМатериалыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Элемент.ТекущийЭлемент.Имя = "ЗНМатериалыСерийныеНомера" Тогда
		ОткрытьПодборМетариалыСерийныеНомера();
	КонецЕсли;
...
Процедура ЗНМатериалыПередУдалением(Элемент, Отказ)
	...
	РаботаССерийнымиНомерамиКлиентСервер.УдалитьСерийныеНомераПоКлючуСвязи(Объект.СерийныеНомераМатериалы, ТекущиеДанные, "КлючСвязиСерийныеНомера",ИспользоватьСерийныеНомераОстатки);
...
Процедура ЗаполнитьПоСпецификации(Команда)
	...
	Если ТекущаяСтрокаТЧ = Неопределено Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не выбрана строка основной табличной части!';uk='Не обраний рядок основної табличної частини!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	...
	Если РезультатПоиска.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоСпецификацииЗавершение", ЭтотОбъект, Новый Структура("РезультатПоиска", РезультатПоиска)),
			НСтр("ru='Табличная часть ""Материалы"" будет перезаполнена! Продолжить выполнение операции?';uk='Таблична частина ""Матеріали"" буде перезаповнена! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
...
Процедура ЗаполнитьМатериалыИзВсехСпецификаций(Команда)
	...
	Если НЕ Объект.Работы.Количество() > 0 Тогда
		...
		Сообщение		= Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Заполните табличную часть ""Работы"".';uk='Заповніть табличну частину ""Роботи"".'");
	...
	Если Объект.Материалы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьМатериалыИзВсехСпецификацийЗавершение", ЭтотОбъект),
			НСтр("ru='Для выполнения операции требуется очистить табличную часть ""Материалы""! Продолжить выполнение операции?';uk='Для виконання операції потрібно очистити табличну частину ""Матеріали""! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
...
Процедура ЗаполнитьПоРесурсамДляТекущейРаботы(Команда)
	...
	Если Объект.РесурсыПредприятия.Количество() = 0 Тогда
		...
		УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(Объект, НСтр("ru='В табличной части ""Задействованные ресурсы"" нет записей!';uk='У табличній частині ""Задіяні ресурси"" немає записів!'"));
		Возврат;
	...
	Если РезультатПоиска.Количество() <> 0 Тогда
		...
		ТекстВопроса = НСтр("ru='Табличная часть ""Исполнители"" для текущей работы будет перезаполнена! Продолжить выполнение операции?';uk='Таблична частина ""Виконавці"" для поточної роботи буде перезаповнена! Продовжити виконання операції?'");
...
Процедура ЗаполнитьПоРесурсамДляВсехРабот(Команда)
	...
	Если Объект.Исполнители.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоРесурсамДляВсехРаботЗавершение", ЭтотОбъект),
			НСтр("ru='Табличная часть ""Исполнители"" будет перезаполнена! Продолжить выполнение операции?';uk='Таблична частина ""Виконавці"" буде перезаповнена! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоБригадамДляТекущейРаботыЗавершение1(Результат, ДополнительныеПараметры) Экспорт
	...
	Если РезультатПоиска.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоБригадамДляТекущейРаботыЗавершение", ЭтотОбъект, Новый Структура("МассивБригад, РезультатПоиска", МассивБригад, РезультатПоиска)), НСтр("ru='Табличная часть ""Исполнители"" для текущей работы будет перезаполнена! Продолжить выполнение операции?';uk='Таблична частина ""Виконавці"" для поточної роботи буде перезаповнена! Продовжити виконання операції?'"),
...
Процедура РедактироватьСписком(Команда)
	...
	Если Элементы.РедактироватьСписком.Пометка И Объект.ПлатежныйКалендарь.Количество() > 1 Тогда
		...
		ПоказатьВопрос(
			ОписаниеОповещения,
			НСтр("ru='Все строки кроме первой будут удалены. Продолжить?';uk='Всі рядки крім першої будуть видалені. Продовжити?'"),
...
Процедура ЗаполнитьПоОснованию(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоПоступлениюВРемонтЗавершение", ЭтотОбъект), НСтр("ru='Документ будет полностью перезаполнен по документу-основанию! Продолжить выполнение операции?';uk='Документ буде повністю перезаповнений по документу-підставі! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
...
&НаКлиенте
Процедура ОткрытьПодборМетариалыСерийныеНомера()
...
&НаСервере
Функция ПараметрыПодбораСерийныхНомеровМатериалы(ИДСтроки, РежимПодбора = Неопределено, ИмяТЧ="Материалы", ИмяТЧСерийныхНомеров="СерийныеНомераМатериалы") Экспорт
	
	Возврат РаботаСФормойДокумента.ПараметрыПодбораСерийныхНомеров(Объект, ЭтотОбъект.УникальныйИдентификатор, ИДСтроки, Истина,
		"Материалы", "СерийныеНомераМатериалы", "КлючСвязиСерийныеНомера");
	
	//ДокОбъект = Объект;                                             	
	//УИДФормы = ЭтотОбъект.УникальныйИдентификатор;
	//
	//ТекСтрокаДанные = ДокОбъект[ИмяТЧ].НайтиПоИдентификатору(ИДСтроки);	
	//Если ТекСтрокаДанные.КлючСвязиСерийныеНомера=0 Тогда
	//	РаботаСФормойДокумента.ЗаполнитьКлючиСвязи(ДокОбъект, ИмяТЧ, "КлючСвязиМатериалы");
	//КонецЕсли;
	//
	//ПараметрыСерийныхНомеров = ПодготовитьПараметрыСерийныхНомеровМатериалы(ДокОбъект, ТекСтрокаДанные, УИДФормы, ИмяТЧ, ИмяТЧСерийныхНомеров);
...
&НаСервере
Функция ПолучитьСерийныеНомераМатериалыИзХранилища(АдресВоВременномХранилище, КлючСтроки)
	
	Модифицированность = Истина;
	Возврат РаботаСФормойДокумента.ПолучитьСерийныеНомераИзХранилища(Объект, АдресВоВременномХранилище, КлючСтроки, "Материалы", "СерийныеНомераМатериалы", "КлючСвязиСерийныеНомера");
...
Процедура РедактироватьЗачетПредоплаты(Команда)
	...
	ПараметрыПодбора = Новый Структура(
		...
		Истина, // ЗаказВШапке
		Компания, // Компания
		?(КонтрагентВестиРасчетыПоЗаказам, Объект.Ссылка, Неопределено), // Заказ
		Объект.Дата, // Дата
...
Процедура ЗНИзменитьТоварыРезервЗаполнитьПоОстаткам(Команда)
	...
	Если Объект.Запасы.Количество() = 0 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Табличная часть ""Товары"" не заполнена!';uk='Таблична частина ""Товари"" не заповнена!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Процедура ЗНИзменитьМатериалыРезервЗаполнитьПоОстаткам(Команда)
	...
	Если РезультатПоиска.Количество() = 0 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Табличная часть ""Материалы"" не заполнена!';uk='Таблична частина ""Матеріали"" не заповнена!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Процедура ЗНИзменитьМатериалыРезервЗаполнитьПоОстаткамДляВсех(Команда)
	...
	Если Объект.Работы.Количество() > 1 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗНИзменитьМатериалыРезервЗаполнитьПоОстаткамДляВсехЗавершение", ЭтотОбъект),
			НСтр("ru='В табличной части ""Материалы"" колонка ""Резерв"" будет перезаполнена для всех работ! Продолжить выполнение операции?';uk='У табличній частині ""Матеріали"" стовпчик ""Резерв"" буде перезаповнено для всіх робіт! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
	КонецЕсли;
