...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураДанных.Свойство("ВидЦенКалькуляции", ДанныеЗаказа.ВидЦенКалькуляции);
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(
		Оповещение, 
		НСтр("ru='Сохранить измерения калькуляции?';uk='Зберегти вимірювання калькуляції?'"), 
		РежимДиалогаВопрос.ДаНетОтмена, , 
		КодВозвратаДиалога.Да, НСтр("ru='Калькуляция';uk='Калькуляція'"));
	КонецЕсли; 
...
&НаКлиенте
Процедура ВидЦенКалькуляцииПриИзменении(Элемент)
...
Процедура УстановитьВидимостьИДоступность()
	...
	Элементы.ВидЦенКалькуляции.Видимость = ОтображатьСебестоимость;
	...
	Элементы.ВидЦенКалькуляции.Доступность = НЕ ТолькоПросмотр;
	...
	Элементы.СохранитьИзменения.Заголовок = ?(ТолькоПросмотр, НСтр("ru='Закрыть';uk='Закрити'"), НСтр("ru='Записать и закрыть';uk='Записати і закрити'"));
	...
	НСтр("ru='Использовать другой шаблон';uk='Використовувати інший шаблон'"),
	НСтр("ru='Добавить по шаблону';uk='Додати за шаблоном'"));
...
Процедура ЗаполнитьВидыЦенКалькуляции()
	...
	Элементы.ВидЦенКалькуляции.СписокВыбора.Очистить();
	Элементы.ВидЦенКалькуляции.СписокВыбора.Добавить(Справочники.ВидыЦен.ПустаяСсылка(), НСтр("ru='последней цене закупки';uk='останній ціні закупівлі'"));
	...
	Пока Выборка.Следующий() Цикл
		Элементы.ВидЦенКалькуляции.СписокВыбора.Добавить(Выборка.Ссылка, НСтр("ru='виду цен ';uk='виду цін '")+""""+СокрЛП(Выборка.Наименование)+"""");	
	КонецЦикла; 
...
Процедура ЗаполнитьСписокИзделий()
	...
	Элементы.ТекущаяСтрокаСостава.СписокВыбора.Добавить(-1, НСтр("ru='Калькуляция всего заказа';uk='Калькуляція всього замовлення'"));
...
Процедура ЗаполнитьКалькуляцию()
	...
	РассчитатьСебестоимость(ТаблицаМатериалов, ДанныеЗаказа.ВидЦенКалькуляции, ДанныеЗаказа.Курс, ДанныеЗаказа.Кратность, ДанныеЗаказа.НалогообложениеНДС, ДанныеЗаказа.Дата, ЗаказСсылка);
	...
	Для каждого Стр Из ДанныеЗаказа.Калькуляция Цикл
		...
		ИначеЕсли ТипЗнч(Стр.Номенклатура)=Тип("СправочникСсылка.Номенклатура") Тогда
			...
			СтруктураДанные.Вставить("ВидЦенКалькуляции", ДанныеЗаказа.ВидЦенКалькуляции);
...
Процедура РассчитатьСебестоимость(ТаблицаМатериалов, ВидЦен, Курс, Кратность, НалогообложениеНДС, Дата, Ссылка, ВыводитьОшибки = Истина)
	
	// Расчет себестоимости и стоимости расходов
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ВидЦенКалькуляции", ВидЦен);
	...
	|							Материалы КАК Материалы
	|						ГДЕ
	|							&ВидЦенКалькуляции = ЗНАЧЕНИЕ(Справочник.ВидыЦен.ПустаяСсылка)
	|							И НЕ Материалы.НоменклатураСебестоимость.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Операция))
	|			
	|			СГРУППИРОВАТЬ ПО
	...
	|					&Период,
	|					(ВидЦен = &ВидЦенКалькуляции
	|						ИЛИ &ВидЦенКалькуляции = ЗНАЧЕНИЕ(Справочник.ВидыЦен.ПустаяСсылка)
	...
	|								Материалы КАК Материалы
	|							ГДЕ
	|								(НЕ &ВидЦенКалькуляции = ЗНАЧЕНИЕ(Справочник.ВидыЦен.ПустаяСсылка)
	...
	|				ПО ЦеныНоменклатурыСрезПоследних.ВидЦен.ВалютаЦены = КурсыВалютСрезПоследних.Валюта) КАК Цены
	...
	Если ВыводитьОшибки Тогда
		Для каждого Стр Из ТаблицаОшибок Цикл
			...
			НСтр("ru='Не удалось определить себестоимость запаса %1';uk='Не вдалося визначити собівартість запасу %1'"),
	...
	Для каждого СтрокаСостава Из СоставШаблона Цикл
		...
		Если СтрокаСостава.ТипСтроки="Расход" Тогда
			Если СтрокаСостава.СпособРасчета=Перечисления.СпособыРасчетаСуммыЗатрат.ФиксированнаяСумма Тогда
				НоваяСтрока.Себестоимость = РаботаСКурсамиВалют.ПересчитатьВВалюту(
				СтрокаСостава.Значение, 
				?(ЗначениеЗаполнено(СтрокаСостава.Валюта), СтрокаСостава.Валюта, ВалютаУчета), 
				ВалютаУчета, 
				ДанныеЗаказа.Дата)*ДанныеЗаказа.Курс/ДанныеЗаказа.Кратность;
...
Процедура ОбновитьФактическиеДанные()
	...
	|	СУММА(ВложенныйЗапрос.СтоимостьФакт / &Курс * &Кратность) КАК СтоимостьФакт,
	|	СУММА(ВложенныйЗапрос.СебестоимостьФакт / &Курс * &Кратность) КАК СебестоимостьФакт,
	|	СУММА(ВложенныйЗапрос.СтоимостьФакт / &Курс * &Кратность) - СУММА(ВложенныйЗапрос.СебестоимостьФакт / &Курс * &Кратность) КАК ПрибыльФакт
	|ИЗ
	|	(ВЫБРАТЬ
...
Процедура ВывестиКалькуляциюКлиент(Идентификатор = Неопределено)
	...
	Если НЕ ТолькоПросмотр И Идентификатор<>Неопределено И НЕ СравнитьКалькуляциюИСпецификацию(Идентификатор) Тогда
		...
		ПоказатьВопрос(
		Оповещение, 
		НСтр("ru='Состав спецификации был изменен. Обновить калькуляцию заказа?';uk='Склад специфікації був змінений. Оновити калькуляцію замовлення?'"), 
	...
	Для каждого СтрМатериала Из СтрокиСпецификаций Цикл
		...
		Для каждого СтрСостава Из СтрокиСостава Цикл
			Если ЗначениеЗаполнено(СтрСостава.Спецификация) И СтрМатериала.ПроверкаЗацикливания.Найти(СтрСостава.Спецификация)<>Неопределено Тогда
				...
				НСтр("ru='Зацикливание сецификации %1 при изготовлении изделия %2';uk='Зациклення сеціфікації %1 при виготовленні виробу %2'"),
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	РассчитатьСебестоимость(ТаблицаМатериалов, СтруктураДанные.ВидЦенКалькуляции, СтруктураДанные.Курс, СтруктураДанные.Кратность, СтруктураДанные.НалогообложениеНДС, СтруктураДанные.Дата, СтруктураДанные.Заказ, Ложь);
...
Процедура ЗаписатьИЗакрытьСервер(РаспечататьКалькуляцию = Ложь)
	...
	СтруктураДанных.Вставить("ВидЦенКалькуляции", ДанныеЗаказа.ВидЦенКалькуляции);
...
Процедура ОбновитьСпецификации()
	...
	Для каждого СтрИдентификатор Из ТаблицаИдентификаторов Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрЗапас.Спецификация) 
			...
			ТекстСообщения = НСтр("ru='Создана автоматически из калькуляции';uk='Створена автоматично з калькуляції'");
			Если ЗначениеЗаполнено(СтрЗапас.Спецификация) Тогда
				...
				ТекстСообщения = ТекстСообщения+СтрШаблон(НСтр("ru=' на основании спецификации %1';uk=' на підставі специфікації %1'"), ТекстСсылки);
			КонецЕсли; 
		...
		Иначе
			...
			ОбсужденияСервер.ДобавитьСообщение(
			НСтр("ru='Обновлен состав по данным калькуляции';uk='Оновлено склад за даними калькуляції'"),
...
Процедура Подбор(Команда)
	...
	Если ЗначениеЗаполнено(ДанныеЗаказа.ВидЦенКалькуляции) Тогда
		ПараметрыПодбора.Вставить("ВидЦен", 				ДанныеЗаказа.ВидЦенКалькуляции);
...
Процедура ЗагрузкаИзФайлаЗапасы(Команда)
	...
	Если ЗагрузкаКалькуляции Тогда
		НастройкиЗагрузкиДанных.Вставить("Заголовок", НСтр("ru='Загрузка калькуляции из файла';uk='Завантаження калькуляції з файлу'"));
	Иначе
		НастройкиЗагрузкиДанных.Вставить("Заголовок", НСтр("ru='Загрузка спецификации из файла';uk='Завантаження специфікації з файлу'"));
	КонецЕсли; 
