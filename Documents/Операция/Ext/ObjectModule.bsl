	...
	Если УчетВалютныхОпераций Тогда
		Для каждого СтрокаТЧ Из Проводки Цикл
			...
			И НЕ ЗначениеЗаполнено(СтрокаТЧ.ВалютаДт) Тогда
				ТекстСообщения = НСтр("ru='Не заполнена колонка ""Валюта Дт"" для валютного счета в строке %НомерСтроки% списка ""Проводки"".';uk='Не заповнений стовпчик ""Валюта Дт"" для валютного рахунку в рядку %НомерСтроки% списку ""Проводки"".'");
			...
			И НЕ ЗначениеЗаполнено(СтрокаТЧ.СуммаВалДт) Тогда
				ТекстСообщения = НСтр("ru='Не заполнена колонка ""Сумма (вал.) Дт"" для валютного счета в строке %НомерСтроки% списка ""Проводки"".';uk='Не заповнений стовпчик ""Сума (вал.) Дт"" для валютного рахунку в рядку %НомерСтроки% списку ""Проводки"".'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НомерСтроки%", Строка(СтрокаТЧ.НомерСтроки));
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
					ЭтотОбъект,
					ТекстСообщения,
			...
			И НЕ ЗначениеЗаполнено(СтрокаТЧ.ВалютаКт) Тогда
				ТекстСообщения = НСтр("ru='Не заполнена колонка ""Валюта Кт"" для валютного счета в строке %НомерСтроки% списка ""Проводки"".';uk='Не заповнений стовпчик ""Валюта Кт"" для валютного рахунку в рядку %НомерСтроки% списку ""Проводки"".'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НомерСтроки%", Строка(СтрокаТЧ.НомерСтроки));
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
					ЭтотОбъект,
					ТекстСообщения,
			...
			И НЕ ЗначениеЗаполнено(СтрокаТЧ.СуммаВалКт) Тогда
				ТекстСообщения = НСтр("ru='Не заполнена колонка ""Сумма (вал.) Кт"" для валютного счета в строке %НомерСтроки% списка ""Проводки"".';uk='Не заповнений стовпчик ""Сума (вал.) Кт"" для валютного рахунку в рядку %НомерСтроки% списку ""Проводки"".'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НомерСтроки%", Строка(СтрокаТЧ.НомерСтроки));
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
					ЭтотОбъект,
					ТекстСообщения,
