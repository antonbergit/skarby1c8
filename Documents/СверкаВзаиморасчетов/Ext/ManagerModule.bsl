...
Функция ПолучитьПредставленияДокументовДляКонтрагентов()
	...
	ВидыДокументовКонтрагента.Вставить("АвансовыйОтчет",					НСтр("ru='Приходный кассовый ордер';uk='Прибутковий касовий ордер'"));
	ВидыДокументовКонтрагента.Вставить("АктВыполненныхРабот",				НСтр("ru='Поступление (товаров, услуг)';uk='Надходження (товарів, послуг)'"));
	ВидыДокументовКонтрагента.Вставить("Взаимозачет",						НСтр("ru='Взаимозачет задолженности';uk='Взаємозалік заборгованості'"));
	ВидыДокументовКонтрагента.Вставить("ЗаказПокупателя",					НСтр("ru='Заказ';uk='Замовлення'"));
	ВидыДокументовКонтрагента.Вставить("ЗаказПоставщику", 					НСтр("ru='Заказ';uk='Замовлення'"));
	ВидыДокументовКонтрагента.Вставить("КорректировкаРегистров",			НСтр("ru='Корректировка задолженности';uk='Коригування заборгованості'"));
	ВидыДокументовКонтрагента.Вставить("ОтчетКомиссионера",					НСтр("ru='Отчет комитенту';uk='Звіт комітенту'"));
	ВидыДокументовКонтрагента.Вставить("ОтчетКомитенту",					НСтр("ru='Отчет комиссионера';uk='Звіт комісіонера'"));
	ВидыДокументовКонтрагента.Вставить("ПоступлениеНаСчет", 				НСтр("ru='Расход со счета';uk='Витрата з рахунку'"));
	ВидыДокументовКонтрагента.Вставить("ПоступлениеВКассу",					НСтр("ru='Расходный кассовый ордер';uk='Видатковий касовий ордер'"));
	ВидыДокументовКонтрагента.Вставить("РасходСоСчета",						НСтр("ru='Поступление на счет';uk='Надходження на рахунок'"));
	ВидыДокументовКонтрагента.Вставить("РасходИзКассы",						НСтр("ru='Приходный кассовый ордер';uk='Прибутковий касовий ордер'"));
	ВидыДокументовКонтрагента.Вставить("ПриходнаяНакладная",				НСтр("ru='Реализация (товаров, услуг)';uk='Реалізація (товарів, послуг)'"));
	ВидыДокументовКонтрагента.Вставить("РасходнаяНакладная", 				НСтр("ru='Поступление (товаров, услуг)';uk='Надходження (товарів, послуг)'"));
	ВидыДокументовКонтрагента.Вставить("СчетНаОплату", 						НСтр("ru='Счет на оплату';uk='Рахунок на оплату'"));
...
Функция ОписаниеРасчетногоДокументаОрганизации(ДокументСсылка, НомерДокумента = "" , Знач ДатаДокумента, ОрганизациюВПредставление = Ложь) Экспорт
	...
	Если ДокументСсылка = Неопределено Тогда
		...
		СтрокаОписания =  НСтр("ru='Расчетный документ № %1 от %2 г.';uk='Розрахунковий документ № %1 від %2 р.'");
		СтрокаОписания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаОписания, 
			?(ПустаяСтрока(НомерДокумента), Нстр("ru='_______';uk='_______'"), ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(НомерДокумента, Ложь, Истина)), 
			?(ЗначениеЗаполнено(ДатаДокумента), Формат(ДатаДокумента, "ДЛФ=D"), Нстр("ru='___.___.________';uk='___.___.________'"))
			);
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаказПокупателя") 
		...
		ОписаниеДокумента = Нстр("ru='Заказ-наряд';uk='Замовлення-наряд'");
	...
	КонецЕсли;
	НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(НомерДокумента, Ложь, Истина);
	...
	Если ОрганизациюВПредставление Тогда
		...
		УказатьПредставлениеОрганизации = НСтр("ru=' (';uk=' ('") + ДокументСсылка.Организация.НаименованиеПолное + НСтр("ru=')';uk=')'");
		
	КонецЕсли;
	
	Возврат ОписаниеДокумента + ДобавитьКОписанию 
		+ НСтр("ru=' № ';uk=' № '") + НомерДокумента 
		+ НСтр("ru=' от ';uk=' від '") + ДатаДокумента + НСтр("ru=' г.';uk=' р.'") + УказатьПредставлениеОрганизации;
...
Функция ОписаниеРасчетногоДокументаКонтрагента(ДокументСсылка, Знач НомерДокумента, Знач ДатаДокумента, ОрганизациюВПредставление = Ложь) Экспорт
	...
	НомерДокумента	= ?(ПустаяСтрока(НомерДокумента), Нстр("ru='_______';uk='_______'"), ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(НомерДокумента, Ложь, Истина));
	ДатаДокумента	= ?(ЗначениеЗаполнено(ДатаДокумента), Формат(ДатаДокумента, "ДЛФ=D"), Нстр("ru='___.___._______';uk='___.___._______'"));
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаказПокупателя") 
		...
		ОписаниеДокумента = Нстр("ru='Поступление (товаров, услуг)';uk='Надходження (товарів, послуг)'");
	...
	Если ПустаяСтрока(ОписаниеДокумента) Тогда
		...
		ОписаниеДокумента = НСтр("ru='Расчетный документ';uk='Розрахунковий документ'");
...
Процедура ЗаменитьПустыеПараметрыПодчеркиванием(ПараметрыМакета)
	...
	Подчеркивание = НСтр("ru='__________________';uk='__________________'");
...
Функция СформироватьПечатнуюФормуАктБезРасхождений(ДокументПечати, ОбъектыПечати, ВывестиФаксимиле = Ложь)
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		ПредставлениеДатыСоставления = НСтр("ru='Документ сформирован %1';uk='Документ сформований %1'", КодЯзыкаПечать);
		ПредставлениеДатыСоставления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПредставлениеДатыСоставления, Формат(Шапка.Дата, "Л="+КодЯзыкаПечать+"; ДЛФ=DD"));
		...
		ПараметрыМакета.Вставить("ПредставлениеОрганизации", НСтр("ru='1. ';uk='1. '", КодЯзыкаПечать) + УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОбОрганизации, "ПолноеНаименование, ИНН, ЮридическийАдрес, Телефоны, Факс"));
		...
		СведенияОКонтрагенте = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.Контрагент,  Шапка.Дата, , , КодЯзыкаПечать);
		ПараметрыМакета.Вставить("ПредставлениеКонтрагента", НСтр("ru='2. ';uk='2. '", КодЯзыкаПечать) + УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОКонтрагенте, "ПолноеНаименование, ИНН, ЮридическийАдрес, Телефоны, Факс"));
		...
		Если ЗначениеЗаполнено(Шапка.НачалоПериода) Тогда
			...
			ПредставлениеПериода = НСтр("ru='в период с %1 по %2';uk='в період з %1 по %2'", КодЯзыкаПечать);
			ПредставлениеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПредставлениеПериода, Формат(Шапка.НачалоПериода, "Л="+КодЯзыкаПечать+"; ДЛФ=DD"), Формат(Шапка.КонецПериода, "Л="+КодЯзыкаПечать+"; ДЛФ=DD"));
			
		Иначе
			
			ПредставлениеПериода = НСтр("ru='по состоянию на %1';uk='станом на %1'", КодЯзыкаПечать);
			ПредставлениеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПредставлениеПериода, Формат(Шапка.КонецПериода, "Л="+КодЯзыкаПечать+"; ДЛФ=DD"));
		...
		ПараметрыМакета.Очистить();
		ПараметрыМакета.Вставить("КонецПериодаСверки", Формат(Шапка.КонецПериода, "Л="+КодЯзыкаПечать+"; ДЛФ=DD"));
...
Функция СформироватьПечатнуюФормуАктСДаннымиКонтрагента(ДокументПечати, ОбъектыПечати, ВывестиФаксимиле = Ложь)
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Для СчетчикСтрок = 0 По МаксИндекс Цикл
			...
			Если Шапка.СортироватьПоДоговорам Тогда
				...
				Если CтрокаДанных.Договор <> Договор Тогда
					...
					ПараметрыМакета.Вставить("ПредставлениеДоговора", НСтр("ru='По договору: ';uk='За договором: '", КодЯзыкаПечать) + Договор);
		...
		Если Расхождение <> 0 Тогда
			...
			ИтогСверки = НСтр("ru='В результате сверки выявлено расхождение информации о состоянии расчетов в размере ';uk='В результаті звірки виявлено розбіжність інформації про стан розрахунків у розмірі '", КодЯзыкаПечать)
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='Акт сверки взаиморасчетов (без расхождений)';uk='Акт звірки взаєморозрахунків (без розбіжностей)'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка,ФормаСпискаДокументов";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 4;
	
	//
	// Описание команды печати НСтр("ru='Акт сверки взаиморасчетов (без расхождений с факсимиле)';uk='Акт звірки взаєморозрахунків (без розбіжностей з факсиміле)'");
	//
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "АктСДаннымиКонтрагента";
	КомандаПечати.Представление = НСтр("ru='Акт сверки взаиморасчетов (с данными контрагента)';uk='Акт звірки взаєморозрахунків (з даними контрагента)'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.Порядок = 10;
	
	//
	// Описание команды печати НСтр("ru='Акт сверки взаиморасчетов (с данными контрагента и с факсимиле)';uk='Акт звірки взаєморозрахунків (з даними контрагента й з факсиміле)'");
	//
