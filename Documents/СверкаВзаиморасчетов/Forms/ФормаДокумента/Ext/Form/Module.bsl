...
Процедура ЗаполнитьЗаголовокПоВыбранымДоговорам()
	...
	Если Объект.ДоговорыКонтрагентов.Количество() = 0 Тогда
		...
		ТекстНадписи = НСтр("ru='Список договоров пуст';uk='Список договорів порожній'");
	...
	ИначеЕсли МассивОтмеченных.Количество() = 0 Тогда
		...
		ТекстНадписи = НСтр("ru='Договоры не выбраны';uk='Договори не обрані'");
...
Процедура ПолучитьДанныеКонтрагента(ДанныеКонтрагента)
	...
	Если ЗаполнитьДанные Тогда
		...
		Объект.ОписаниеВыбораДоговоров = НСтр("ru='Выбраны все договоры контрагента';uk='Обрані всі договори контрагента'");
...
"Для привязки необходимо включить видимость соответствующей колонки и указать документ самостоятельно.';uk='Рядок не прив`язаний до розрахункового документа. "
"Для прив`язки необхідно включити видимість відповідного стовпчика й указати документ самостійно.'");
...
Процедура СтатусОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение = ПредопределенноеЗначение("Перечисление.СтатусыСверокВзаиморасчетов.Сверена") Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.Ответственный) Тогда
			...
			ТекстСообщения = Нстр("ru='Неверно заполнено ответственное лицо организации.';uk='Невірно заповнена відповідальна особа організації.'");
		...
		Если НЕ ЗначениеЗаполнено(Объект.ПредставительКонтрагента) Тогда
			...
			ТекстСообщения = Нстр("ru='Неверно заполнен представитель контрагента.';uk='Невірно заповнений представник контрагента.'");
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	Иначе
		...
		Объект.ДоговорыКонтрагентов.Очистить();
		Объект.ОписаниеВыбораДоговоров = НСтр("ru='Список договоров пуст';uk='Список договорів порожній'");
...
Процедура ЗаполнитьПоДатамРасчетныхДокументов(Команда)
	...
	Если Объект.ДанныеКонтрагента.Количество() < 1 Тогда
		...
		ТекстСообщения = НСтр("ru='Табличная часть взаиморасчетов по данным контрагента пуста.';uk='Таблична частина взаєморозрахунків за даними контрагента порожня.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
...
Процедура ЗаполнитьПоДаннымУчета(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.КонецПериода) Тогда
		...
		ТекстСообщения	= НСтр("ru='Неверно заполнена дата окончания периода.';uk='Невірно заповнена дата закінчення періоду.'");
	...
	Если ВызватьПроцедуруЗаполненияТабличнойЧасти Тогда
		...
		Если Объект.ДанныеОрганизации.Количество() > 0 Тогда
			...
			ТестВопроса	= Нстр("ru='Табличная часть будет очищена и заполнена повторно. Продолжить?';uk='Таблична частина буде очищена й заповнена повторно. Продовжити?'");
...
Процедура ПеренестиИзДанныхОрганизации(Команда)
	...
	Если Объект.ДанныеОрганизации.Количество() < 1 Тогда
		...
		ТекстСообщения = НСтр("ru='Не заполнена табличная часть с данными организации.';uk='Не заповнена таблична частина з даними організації.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
