...
Функция СформироватьОписаниеОшибкиРаспределенияЗатрат(СчетУчета, СпособРаспределения, ФильтрПоЗаказ, Сумма)
	...
	ОписаниеОшибки = НСтр("ru='Затрата ""%СчетУчета%"" в сумме %Сумма%, распределяемая на выпуск продукции по %СпособРаспределения% не может быть распределена, т.к. в рассчитываемом периоде не было %ДополнительноеОписание%.';uk='Витрата ""%СчетУчета%"" у сумі %Сума%, що розподіляється на випуск продукції по %СпособРаспределения% не може бути розподілена, тому що в періоду, що розраховується, не було %ДополнительноеОписание%.'"
	);
...
Функция СформироватьОписаниеОшибкиРаспределенияРасходов(СчетУчета, СпособРаспределения, Сумма)
	...
	ОписаниеОшибки = НСтр("ru='Расход ""%СчетУчета%"" в сумме %Сумма%, распределяемый на финансовый результат по %СпособРаспределения% не может быть распределен, т.к. в рассчитываемом периоде не было %ДополнительноеОписание%.';uk='Витрата ""%СчетУчета%"" у сумі %Сума%, що розподіляється на фінансовий результат по %СпособРаспределения% не може бути розподілений, тому що в періоду, що розраховується, не було %Дополнительноеописание%.'"
	);
...
Процедура СформироватьДвиженияПоРегиструУчетаЗатрат(НаборЗаписей, НаборЗаписейУправленческий, СтрокаДвижения, Сумма, ФиксированнаяСтоимость, СодержаниеПроводки = Неопределено)
	...
	Если СодержаниеПроводки = Неопределено Тогда
		Если СтрокаДвижения.СчетУчетаТипСчета = Перечисления.ТипыСчетов.Запасы Тогда
			СодержаниеПроводки = НСтр("ru='Списание запасов со склада';uk='Списання запасів зі складу'");
		Иначе
			Если ЗначениеЗаполнено(СтрокаДвижения.Номенклатура) Тогда
				СодержаниеПроводки = НСтр("ru='Списание расходов';uk='Списання витрат'");
			Иначе
				СодержаниеПроводки = НСтр("ru='Списание запасов из производства';uk='Списання запасів з виробництва'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если СтрокаДвижения.КоррСчетУчетаТипСчета = Перечисления.ТипыСчетов.Запасы Тогда
		СодержаниеПроводки = НСтр("ru='Оприходование запасов на склад';uk='Оприбуткування запасів на склад'");
	Иначе
		Если ЗначениеЗаполнено(СтрокаДвижения.Номенклатура) Тогда
			СодержаниеПроводки = НСтр("ru='Поступление расходов';uk='Надходження витрат'");
		Иначе
			СодержаниеПроводки = НСтр("ru='Оприходование запасов в производство';uk='Оприбуткування запасів у виробництво'");
		КонецЕсли;
	КонецЕсли;
			...
			Если ВыборкаДетальныеЗаписи.КоррСчетУчета = ДополнительныеСвойства.ДляПроведения.ПустойСчет Тогда
				...
				Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ДокументПродажи)
				  ...
				  ИЛИ ТипЗнч(ВыборкаДетальныеЗаписи.ДокументПродажи) = Тип("ДокументСсылка.ОтчетОПереработке")) Тогда
					...
					НоваяСтрока.СодержаниеПроводки = НСтр("ru='Отражение расходов';uk='Відображення витрат'");
					...
					НоваяСтрока.Содержание = НСтр("ru='Отражение расходов';uk='Відображення витрат'");
				...
				ИначеЕсли ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ДокументПродажи)
					...
					Если СуммаКорректировки < 0 Тогда
						...
						НоваяСтрока.СуммаРасходов = СуммаКорректировки;
						НоваяСтрока.СодержаниеПроводки = НСтр("ru='Прочие расходы';uk='Інші витрати'");
					Иначе
						НоваяСтрока.СчетУчета = ПланыСчетов.Управленческий.ПрочиеДоходы;
						НоваяСтрока.СуммаДоходов = СуммаКорректировки;
						НоваяСтрока.СодержаниеПроводки = НСтр("ru='Прочие доходы';uk='Інші доходи'");
					Конецесли;
					...
					Если СуммаКорректировки < 0 Тогда
						...
						НоваяСтрока.СчетКт = ВыборкаДетальныеЗаписи.СчетУчета;
						НоваяСтрока.Сумма = - СуммаКорректировки;
						НоваяСтрока.Содержание = НСтр("ru='Прочие расходы';uk='Інші витрати'");
					Иначе
						НоваяСтрока.СчетДт = ВыборкаДетальныеЗаписи.СчетУчета;
						НоваяСтрока.СчетКт = ПланыСчетов.Управленческий.ПрочиеДоходы;
						НоваяСтрока.Сумма = СуммаКорректировки;
						НоваяСтрока.Содержание = НСтр("ru='Прочие доходы';uk='Інші доходи'");
					КонецЕсли;
				...
				ИначеЕсли ВыборкаДетальныеЗаписи.ПеремещениеВРозницуСуммовойУчет Тогда
					...
					НоваяСтрока.СодержаниеПроводки = НСтр("ru='Перемещение в розницу';uk='Переміщення в роздріб'");
					НоваяСтрока.Себестоимость = СуммаКорректировки;
					...
					НоваяСтрока.СчетКт = ВыборкаДетальныеЗаписи.СчетУчета;
					НоваяСтрока.Содержание = НСтр("ru='Перемещение в розницу';uk='Переміщення в роздріб'");
					НоваяСтрока.Сумма = СуммаКорректировки; 
				...
				Иначе
					...
					НоваяСтрока.СчетДт = ВыборкаДетальныеЗаписи.СчетУчетаСписания;
					НоваяСтрока.СчетКт = ВыборкаДетальныеЗаписи.СчетУчета;
					НоваяСтрока.Содержание = НСтр("ru='Списание запасов на произвольный счет';uk='Списання запасів на довільний рахунок'");
					НоваяСтрока.Сумма = СуммаКорректировки;
...
Процедура РаспределитьСуммыБезКоличества(ВидОперации, ТаблицаОшибок)
	...
	// Определяются направления списания всех сумм меньше гривны, для узлов по которым
	...
	Пока НЕ Результат.Пустой() Цикл
		...
		Если КоличествоИтераций > 60 Тогда
			ОписаниеОшибки = НСтр("ru='Не удалось скорректировать суммовые остатки по затратам.';uk='Не вдалося скорегувати сумові залишки по витратах.'");
			...
			Если ВыборкаДетальныеЗаписи.КоррСчетУчета = ДополнительныеСвойства.ДляПроведения.ПустойСчет Тогда
				...
				Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ДокументПродажи)
				  ...
				  ИЛИ ТипЗнч(ВыборкаДетальныеЗаписи.ДокументПродажи) = Тип("ДокументСсылка.ОтчетОПереработке")) Тогда
					...
					НоваяСтрока.СчетУчета = ВыборкаДетальныеЗаписи.НаправлениеДеятельностиПродажиСчетУчетаСебестоимостиПродаж;
					НоваяСтрока.СуммаРасходов = СуммаКорректировки;
					НоваяСтрока.СодержаниеПроводки = НСтр("ru='Отражение расходов по продаже';uk='Відображення витрат із продажу'");
					...
					НоваяСтрока.СчетДт = ВыборкаДетальныеЗаписи.НаправлениеДеятельностиПродажиСчетУчетаСебестоимостиПродаж;
					НоваяСтрока.СчетКт = ВыборкаДетальныеЗаписи.СчетУчета;
					НоваяСтрока.Содержание = НСтр("ru='Отражение расходов по продаже';uk='Відображення витрат із продажу'");
					НоваяСтрока.Сумма = СуммаКорректировки;
				...
				ИначеЕсли ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ДокументПродажи)
					...
					Если СуммаКорректировки < 0 Тогда
						НоваяСтрока.СчетУчета = ПланыСчетов.Управленческий.ПрочиеРасходы;
						НоваяСтрока.СуммаРасходов = СуммаКорректировки;
						НоваяСтрока.СодержаниеПроводки = НСтр("ru='Прочие расходы по возврату поставщику';uk='Інші витрати по поверненню постачальникові'");
					Иначе
						НоваяСтрока.СчетУчета = ПланыСчетов.Управленческий.ПрочиеДоходы;
						НоваяСтрока.СуммаДоходов = СуммаКорректировки;
						НоваяСтрока.СодержаниеПроводки = НСтр("ru='Прочие доходы по возврату поставщику';uk='Інші доходи по поверненню постачальникові'");
					Конецесли;
					НоваяСтрока.Аналитика = ВыборкаДетальныеЗаписи.Номенклатура;
					...
					Если СуммаКорректировки < 0 Тогда
						...
						НоваяСтрока.СчетКт = ВыборкаДетальныеЗаписи.СчетУчета;
						НоваяСтрока.Сумма = - СуммаКорректировки;
						НоваяСтрока.Содержание = НСтр("ru='Прочие расходы по возврату поставщику';uk='Інші витрати по поверненню постачальникові'");
					Иначе
						НоваяСтрока.СчетДт = ВыборкаДетальныеЗаписи.СчетУчета;
						НоваяСтрока.СчетКт = ПланыСчетов.Управленческий.ПрочиеДоходы;
						НоваяСтрока.Сумма = СуммаКорректировки;
						НоваяСтрока.Содержание = НСтр("ru='Прочие доходы по возврату поставщику';uk='Інші доходи по поверненню постачальникові'");
					КонецЕсли;
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если (НЕ Окр(СуммаКорректировки, 2) = 0) Тогда
			...
			СформироватьДвиженияПоРегиструУчетаЗатрат(
				...
				НСтр("ru='Себестоимость возврата от покупателя';uk='Собівартість повернення від покупця'")
			);
...
Процедура РассчитатьСебестоимостьВРозницеСуммовойУчет(Отказ, ТаблицаОшибок)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		НоваяСтрока.СодержаниеПроводки = НСтр("ru='Себестоимость';uk='Собівартість'");
		...
		НоваяСтрока.Содержание = НСтр("ru='Себестоимость';uk='Собівартість'");
		...
		НоваяСтрока.Содержание = НСтр("ru='Наценка';uk='Націнка'");
		НоваяСтрока.Сумма = - ВыборкаДетальныеЗаписи.СуммаКорректировки;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ТаблицаОшибок.Количество() > 0 Тогда
		ТекстСообщения = НСтр("ru='При закрытии месяца были сформированы предупреждения! Подробнее см. в отчете о закрытии месяца.';uk='При закритті місяця були сформовані попередження! Докладніше див. у звіті про закриття місяця.'");
