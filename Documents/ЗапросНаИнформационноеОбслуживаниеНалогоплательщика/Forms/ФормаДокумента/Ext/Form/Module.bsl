...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.ВидУслуги = Перечисления.ВидыУслугПриИОН.ПредставлениеСправкиОбИсполненииОбязанностейПоУплате Тогда
		ПолучательПредставление = НСтр("ru='ФНС';uk='ФНС'") + ?(ЗначениеЗаполнено(Объект.Организация), " " + Строка(РеквизитыОрганизации.КодНалоговогоОргана), "");
		Если Параметры.Ключ.Пустая() Тогда
...
Процедура ПолучательНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.Организация = Неопределено ИЛИ Объект.Организация = ПредопределенноеЗначение("Справочник.Организации.ПустаяСсылка") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Поле ""От кого"" не заполнено';uk='Поле ""От кого"" не заполнено'"));
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли Объект.Получатель = Неопределено ИЛИ НЕ ЗначениеЗаполнено(РеквизитыПолучателя.Код) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Код налогового органа необходимо задать в справочнике ""Организации"".';uk='Код налогового органа необходимо задать в справочнике ""Организации"".'"));
		
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Для выбора инспекции, необходимо завести соответствующую запись в справочнике ""Регистрации в налоговом органе"".';uk='Для выбора инспекции, необходимо завести соответствующую запись в справочнике ""Регистрации в налоговом органе"".'"));
	КонецЕсли;
...
Процедура ЗапрашиваемыеНалогиКБКПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(НаименованиеКБК) Тогда
		ТекущиеДанные.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 (%2)';uk='%1 (%2)'"), ТекущиеДанные.КБК, НаименованиеКБК);
	Иначе
...
Процедура ЗапрашиваемыеНалогиКБКНачалоВыбораЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено тогда
		...
		Если ЗначениеЗаполнено(Результат.Наименование) Тогда
			ТекущиеДанные.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 (%2)';uk='%1 (%2)'"), ТекущиеДанные.КБК, Результат.Наименование);
		Иначе
			ТекущиеДанные.Наименование = ТекущиеДанные.КБК;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ТекущиеДанные.ОКАТО) Тогда
...
Процедура ОбновитьЗаголовокФормы(Форма)
	...
	Если ЗначениеЗаполнено(Объект.ВидУслуги) Тогда 
		Заголовок = НСтр("ru='Запрос на сверку ""%1 (%2)""';uk='Запрос на сверку ""%1 (%2)""'");
	Иначе 
		Заголовок = НСтр("ru='Запрос на сверку';uk='Запрос на сверку'");
	КонецЕсли;
		
	Если Объект.ВидУслуги = ПредопределенноеЗначение("Перечисление.ВидыУслугПриИОН.ПредставлениеАктовСверкиРасчетов") Тогда
		ПредставлениеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 - %2';uk='%1 - %2'"), 
	...
	ИначеЕсли Объект.ВидУслуги = ПредопределенноеЗначение("Перечисление.ВидыУслугПриИОН.ПредставлениеВыпискиОперацийИзКарточкиРасчетыСБюджетом") Тогда
		ПредставлениеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1';uk='%1'"), 
...
Процедура УправлениеФормой(Форма)
	...
	Если Объект.ДополнительныйПараметр = 0 Тогда
		Форма.ГруппировкаОперацийПредставление = НСтр("ru='По дате';uk='За датою'"); 
	ИначеЕсли Объект.ДополнительныйПараметр = 1 Тогда
		Форма.ГруппировкаОперацийПредставление = НСтр("ru='По виду платежа';uk='По виду платежа'");
	Иначе
	...
	Для Каждого СтрокаТаблицы Из Объект.ЗапрашиваемыеНалоги Цикл
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			Если ЗначениеЗаполнено(НайденнаяСтрока.Наименование) Тогда
				...
				НСтр("ru='%1 (%2)';uk='%1 (%2)'"), СтрокаТаблицы.КБК, НайденнаяСтрока.Наименование);
			Иначе
