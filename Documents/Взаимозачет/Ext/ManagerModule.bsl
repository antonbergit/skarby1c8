...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаВзаимозачет, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ru='Курсовая разница';uk='Курсова різниця'"));
	Запрос.УстановитьПараметр("КорректировкаДолга", НСтр("ru='Корректировка долга';uk='Коригування боргу'"));
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаВзаимозачет, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВзаимозачет);
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("Взаимозачет", НСтр("ru='Взаимозачет';uk='Взаємозалік'"));
	Запрос.УстановитьПараметр("ПереуступкаДолга", НСтр("ru='Переуступка долга';uk='Переуступка боргу'"));
	Запрос.УстановитьПараметр("КорректировкаДолга", НСтр("ru='Корректировка долга';uk='Коригування боргу'"));
		...
		Если НЕ МассивРезультатов[0].Пустой() Тогда
			...
			Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
				Если ВыборкаИзРезультатаЗапроса.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
					ТекстСообщения = НСтр("ru='%ПредставлениеКонтрагента% - остаток задолженности покупателя по документу расчетов меньше списываемой суммы."
"Списываемая сумма: %СуммаВалПриЗаписи% %ВалютаПредставление%."
"Остаток задолженности покупателя: %СуммаОстаткаЗадолженности% %ВалютаПредставление%.';uk='%ПредставлениеКонтрагента% - залишок заборгованості покупця по документу розрахунків менше суми, що списується."
"Сума, що списується: %СуммаВалПриЗаписи% %ВалютаПредставление%."
"Залишок заборгованості покупця: %СуммаОстаткаЗадолженности% %ВалютаПредставление%.'"
				...
				Если ВыборкаИзРезультатаЗапроса.ТипРасчетов = Перечисления.ТипыРасчетов.Аванс Тогда
					Если ВыборкаИзРезультатаЗапроса.СуммаНепогашенныхАвансов = 0 Тогда
						ТекстСообщения = НСтр("ru='%ПредставлениеКонтрагента% - возможно, авансов от покупателя не было или они уже полностью зачтены в товарных документах.';uk='%ПредставлениеКонтрагента% - можливо, авансів від покупця не було або вони вже повністю зараховані в товарних документах.'"
						);
					Иначе
						ТекстСообщения = НСтр("ru='%ПредставлениеКонтрагента% - полученные авансы от покупателя уже частично зачтены в товарных документах."
"Остаток незачтенных авансов: %СуммаНепогашенныхАвансов% %ВалютаПредставление%.';uk='%ПредставлениеКонтрагента% - отримані аванси від покупця вже частково зараховані в товарних документах."
"Залишок незарахованих авансів: %СуммаНепогашенныхАвансов% %ВалютаПредставление%.'"
		...
		Если НЕ МассивРезультатов[1].Пустой() Тогда
			...
			Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
				Если ВыборкаИзРезультатаЗапроса.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
					ТекстСообщения = НСтр("ru='%ПредставлениеКонтрагента% - остаток задолженности перед поставщиком по документу расчетов меньше списываемой суммы."
"Списываемая сумма: %СуммаВалПриЗаписи% %ВалютаПредставление%."
"Остаток задолженности перед поставщиком: %СуммаОстаткаЗадолженности% %ВалютаПредставление%.';uk='%ПредставлениеКонтрагента% - залишок заборгованості перед постачальником по документу розрахунків менше суми, що списується."
"Сума, що списується: %СуммаВалПриЗаписи% %ВалютаПредставление%."
"Залишок заборгованості перед постачальником: %СуммаОстаткаЗадолженности% %ВалютаПредставление%.'"
				...
				Если ВыборкаИзРезультатаЗапроса.ТипРасчетов = Перечисления.ТипыРасчетов.Аванс Тогда
					Если ВыборкаИзРезультатаЗапроса.СуммаНепогашенныхАвансов = 0 Тогда
						ТекстСообщения = НСтр("ru='%ПредставлениеКонтрагента% - возможно, авансов поставщику не было или они уже полностью зачтены в товарных документах.';uk='%ПредставлениеКонтрагента% - можливо, авансів постачальникові не було або вони вже повністю зараховані в товарних документах.'"
						);
					Иначе
						ТекстСообщения = НСтр("ru='%ПредставлениеКонтрагента% - выданные авансы поставщику уже частично зачтены в товарных документах."
"Остаток незачтенных авансов: %СуммаНепогашенныхАвансов% %ВалютаПредставление%.';uk='%ПредставлениеКонтрагента% - видані аванси постачальникові вже частково зараховані в товарних документах."
"Залишок незарахованих авансів: %СуммаНепогашенныхАвансов% %ВалютаПредставление%.'"
						);
						ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаНепогашенныхАвансов%", Строка(ВыборкаИзРезультатаЗапроса.СуммаНепогашенныхАвансов));
					КонецЕсли;
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ТипДокумента)
	...
	Пока Шапка.Следующий() Цикл
		...
		ПредставлениеОрганизации = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОбОрганизации, "ПолноеНаименование,ИНН,КодПоЕДРПОУ,ЮридическийАдрес,Телефоны,");
		
		СведенияОбКонтрагенте = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.КонтрагентИсточник, Шапка.Дата);
		ПредставлениеКредитора= УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОбКонтрагенте, "ПолноеНаименование,ИНН,КодПоЕДРПОУ,РегистрационныйНомер,ЮридическийАдрес,Телефоны,");
		        
		СтрокаКредиторки = НСтр("ru='1. Задолженность %ПредставлениеОрганизации% перед %ПредставлениеКредитора% составляет';uk='1. Заборгованість %ПредставлениеОрганизации% перед %ПредставлениеКредитора% становить'")+ " ";
		...
		СтрокаШапки = НСтр("ru='Акт взаимозачета № %Номер% от %Дата%';uk='Акт взаємозаліку № %Номер% від %Дата%'"); 
		СтрокаШапки = СтрЗаменить(СтрокаШапки,"%Номер%", ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Шапка.Номер, Истина, Ложь));
		СтрокаШапки = СтрЗаменить(СтрокаШапки,"%Дата%",   Формат(Шапка.Дата, "Л="+КодЯзыкаПечать+"; ДЛФ=DD"));
		СтрокаКредиторки  = СтрокаКредиторки + Формат(КредиторскаяЗадолженность.Итог(КолонкаСуммы), "ЧЦ=15; ЧДЦ=2; ЧН=Ноль") + " " + ПредставлениеВалюты + " " +НСтр("ru='по следующим договорам:';uk='за наступними договорами:'");
		...
		ПредставлениеДебитора = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОбКонтрагенте, "ПолноеНаименование,ИНН,КодПоЕДРПОУ,РегистрационныйНомер,ЮридическийАдрес,Телефоны,");
		 
		СтрокаДебиторки       = НСтр("ru='2. Задолженность %ПредставлениеДебитора% перед %ПредставлениеОрганизации% составляет %Сумма% %ПредставлениеВалюты% по следующим договорам:';uk='2. Заборгованість %ПредставлениеДебитора% перед %ПредставлениеОрганизации% становить  %Сумма% %ПредставлениеВалюты% за наступними договорами:'");
		...
		СтрокаВзаимозачета = НСтр("ru='3. Взаимозачет производится на сумму';uk='3. Взаємозалік проводиться на суму'")+ " "
