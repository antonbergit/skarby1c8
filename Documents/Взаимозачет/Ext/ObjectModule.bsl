...
Процедура ЗаполнитьПоСверкеВзаиморасчетов(ДокументСверкаВзаиморасчетов) Экспорт
	...
	Для каждого СтрокаДоговораКонтрагента Из ДокументСверкаВзаиморасчетов.ДоговорыКонтрагентов Цикл
		...
		Если СтрокаДоговораКонтрагента.Выбрать Тогда
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВидОперации = Перечисления.ВидыОперацийВзаимозачет.Взаимозачет Тогда
		...
		Если ДебиторСуммаУчета <> КредиторСуммаУчета Тогда
			ТекстСообщения = НСтр("ru='Сумма учета табличной части ""Расчеты с покупателем"", не равна сумме учета по табличной части ""Расчеты с поставщиком""!';uk='Сума обліку табличної частини ""Розрахунки з покупцем"", не дорівнює сумі обліку по табличній частині ""Розрахунки з постачальником""!'");
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийВзаимозачет.ПереуступкаДолгаПокупателя Тогда
		...
		ДебиторСуммаУчета = Дебитор.Итог("СуммаУчета");
		ТекстСообщения = НСтр("ru='Сумма учета, не равна сумме учета табличной части ""Расчеты с покупателем""!';uk='Сума обліку, не дорівнює сумі обліку табличної частини ""Розрахунки з покупцем""!'");
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийВзаимозачет.ПереуступкаДолгаПоставщику Тогда
		...
		Если КредиторСуммаУчета <> СуммаУчета Тогда
			ТекстСообщения = НСтр("ru='Сумма учета, не равна сумме учета табличной части ""Расчеты с поставщиком""!';uk='Сума обліку, не дорівнює сумі обліку табличної частини ""Розрахунки з постачальником""!'");
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				ЭтотОбъект,
				ТекстСообщения,
				Неопределено,
