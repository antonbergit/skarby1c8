...
"Продолжить?';uk='Розсилання буде виконане для %1."
"Продовжити?'"),
					СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
						Объект.Получатели.Количество(), НСтр("ru='получателя,получателей,получателей';uk='одержувача,одержувачів,одержувачів'")));
	
	ПараметрыВопроса = СтандартныеПодсистемыКлиент.ПараметрыВопросаПользователю();
	ПараметрыВопроса.Заголовок = НСтр("ru='Подтверждение рассылки';uk='Підтвердження розсилки'");
...
"Настройка осуществляется в Еще больше возможностей - Администрирование - Почта, SMS, напоминания - Настройка отправки SMS.';uk='Для отправки SMS требуется настройка параметров отправки."
"Настройка осуществляется в Еще больше возможностей - Администрирование - Почта, SMS, напоминания - Настройка отправки SMS.'");
				ПоказатьПредупреждение(, ТекстСообщения);
				Возврат;
			Иначе
				ТекстСообщения = НСтр("ru='Для отправки SMS требуется настройка параметров отправки."
"Для выполнения настроек обратитесь к администратору.';uk='Для відправки SMS потрібно налаштування параметрів відправки."
"Для виконання налаштувань зверніться до адміністратора. '");
				ПоказатьПредупреждение(, ТекстСообщения);
				Возврат;
			КонецЕсли;
		КонецЕсли;
		
		ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Успешно отправлено: %1 сообщений';uk='Успішно відправлене: %1 повідомлень'"), УспешноОтправленных);
...
Функция ОтправитьРассылкуEmail()
	...
	Для Каждого СтрокаПолучатели Из Объект.Получатели Цикл
		...
		Исключение
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось отправить E-mail получателю: %1, по причине: %2';uk='Не вдалося відправити E-mail одержувачу %1, через: %2'"),
...
Функция ОтправитьРассылкуSMS()
	...
	Для Каждого СтрокаПолучатели Из Объект.Получатели Цикл
		...
		Если ПустаяСтрока(РезультатОтправки.ОписаниеОшибки) Тогда
			...
			УспешноОтправленных = УспешноОтправленных + 1;
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось отправить SMS получателю: %1, по причине: %2';uk='Не удалось отправить SMS получателю: %1, по причине: %2'"),
				СтрокаПолучатели.Контакт,
...
Функция ПолучитьЗначениеПараметра(Параметр, Контакт)
	...
	ИначеЕсли Лев(Параметр, 15) = "{Имя получателя" Тогда
		...
		Если НЕ Падеж = Неопределено Тогда
			...
			Попытка
				...
				ЗначениеПараметра = КомпонентаСклонения.Просклонять(ИмяКонтакта, Падеж);
			Исключение
				// Не удалось выполнить операции с внешней компонентой
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("Не удалось подключить внешнюю компоненту по причине: ") + ОписаниеОшибки());
