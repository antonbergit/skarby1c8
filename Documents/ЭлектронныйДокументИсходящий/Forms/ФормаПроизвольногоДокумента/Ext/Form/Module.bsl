...
Процедура НадписьДокументОснованиеОтсутствуетНажатие(Элемент)
	...
	Если СписокТиповДокументовОснований.Количество() > 0 Тогда
		...
		Иначе
			ЗаголовокСпискаВыбора = НСтр("ru='Выберите тип документа основания...';uk='Выберите тип документа основания...'");
...
Процедура ГиперссылкаОчиститьОснованиеНажатие(Элемент)
	...
	ТекстВопроса = НСтр("ru='Убрать документ-основание?';uk='Убрать документ-основание?'");
...
"Подробности см. в журнале регистрации.';uk='Во время выполнения операции произошла ошибка. Действие отменено."
...
"Для продолжения надо завершить редактирование.';uk='Вложение ""%1"" захвачено для редактирования."
"Для продолжения надо завершить редактирование.'");
			ТекстВопроса = СтрЗаменить(ТекстВопроса, "%1", ЭлектронныйДокументИмяФайла);
			СписокКнопок = Новый СписокЗначений;
			СписокКнопок.Добавить("ЗакончитьРедактирование", НСтр("ru='Закончить редактирование';uk='Закінчити редагування'"));
			СписокКнопок.Добавить("Освободить",              НСтр("ru='Отменить редактирование';uk='Відмінити редагування'"));
			СписокКнопок.Добавить("Отменить",                НСтр("ru='Отменить';uk='Скасувати'"));
...
"Очистить основание?';uk='%1 в основании (%2) отличается от выбранного значения."
"Очистить основание?'");
...
Процедура ПоказатьРезультатПодписи(Подписан)
	...
	ТекстСостояния = НСтр("ru='Подписано: (0)';uk='Подписано: (0)'");
	...
	Если Подписан Тогда
		...
		Прочитать();
		УстановитьДоступностьЭлементов();
		ТекстСостояния = НСтр("ru='Подписано: (1)';uk='Подписано: (1)'");
	КонецЕсли;
...
Процедура ЗаполнитьСтатусПодписи(НоваяСтрока, ТекСтрока)
	...
	Иначе
		НоваяСтрока.ПодписьВерна = НСтр("ru='Не проверена';uk='Не проверена'");
	КонецЕсли
...
Процедура ДобавитьСертификатВДоверенные(ДанныеПодписи)
	...
	Если ДанныеПодписи <> Неопределено И ДанныеПодписи.ОтсутствуетВСписке Тогда
		Текст = НСтр("ru='Добавить сертификат %1 в список ожидаемых сертификатов контрагента?';uk='Добавить сертификат %1 в список ожидаемых сертификатов контрагента?'");
...
Процедура ОбработатьОтветНаВопросПоСертификату(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Если НЕ СертификатДобавлен Тогда 
			ТекстСообщения = НСтр("ru='Ошибка добавления сертификата подписи в список ожидаемых сертификатов.';uk='Ошибка добавления сертификата подписи в список ожидаемых сертификатов.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Иначе
			ЗаполнитьТаблицуЭП();
		КонецЕсли;
...
Процедура УстановитьДоступностьЭлементов()
	...
	Если НаправлениеИсходящий Тогда
		...
		Если ЕстьВложения Тогда
			ДанныеФайла = ДанныеФайлаВложения(АдресВременногоХранилищаВложения, ЭлектронныйДокументСсылка, УникальныйИдентификатор);
			Если ДанныеФайла.ФайлРедактируется Тогда
				ТекстРедактируетФайл = НСтр("ru='Файл захвачен на редактирование пользователем %1';uk='Файл захвачен на редактирование пользователем %1'");
			...
			ГруппаКомандДобавитьЗаменить.Заголовок = СтрЗаменить(ГруппаКомандДобавитьЗаменить.Заголовок,
				НСтр("ru='Добавить';uk='Додати'"), НСтр("ru='Заменить на';uk='Заменить на'"));
			...
			КонецЕсли;
		Иначе
			ГруппаКомандДобавитьЗаменить.Заголовок = СтрЗаменить(ГруппаКомандДобавитьЗаменить.Заголовок, НСтр("ru='Заменить на';uk='Заменить на'"), НСтр("ru='Добавить';uk='Додати'"));
	...
	ЗаголовокПодписи = НСтр("ru='Подписи';uk='Підписи'");
	Если ЗначениеЗаполнено(ЭлектронныйДокументСсылка) Тогда
		КоличествоЭП = ЭлектронныйДокументСсылка.ЭлектронныеПодписи.Количество();
		Если КоличествоЭП > 0 Тогда
			ЗаголовокПодписи = СтрЗаменить(НСтр("ru='Подписи (%1)';uk='Подписи (%1)'"), "%1", КоличествоЭП);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьЗаголовокФормы()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ТекстЗаголовка = НСтр("ru='Исходящий произвольный документ %1 от %2';uk='Исходящий произвольный документ %1 от %2'");
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ТекстЗаголовка, Объект.Номер, Объект.Дата);
	Иначе
		Заголовок = НСтр("ru='Исходящий произвольный документ (создание)';uk='Исходящий произвольный документ (создание)'");
	КонецЕсли;
...
Процедура ПослеОтправкиПЭД(Результат, ДополнительныеПараметры)
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		Если Результат.Свойство("КолОтправленных", КолОтправленных)
			...
			ТекстСостояния = НСтр("ru='Отправлено: (%1)';uk='Отправлено: (%1)'");
		...
		ИначеЕсли Результат.Свойство("КолПодготовленных", КолПодготовленных)
			...
			ТекстСостояния = НСтр("ru='Подготовлено к отправке: (%1)';uk='Подготовлено к отправке: (%1)'");
...
"Записать документ?';uk='Для продолжения операции необходимо записать документ."
"Записать документ?'");
...
"%5';uk='%1, %2 (статус - %3, ответственный - %4):"
"%5'");
