...
Процедура ПереименоватьОтправкиНаПолучениеПатентаВЖурнале() Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Электронный документооборот с контролирующими органами. Обработка обновления';uk='Электронный документооборот с контролирующими органами. Обработка обновления'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗаполнитьИменаОтчетов() Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Обновление уведомлений завершено с ошибкой';uk='Обновление уведомлений завершено с ошибкой'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
КонецПроцедуры
...
Процедура ЗаполнитьДаннымиТС1(ДокОбъект, Выборка)
	...
	Исключение
		...
		СтрОш = НСтр("ru='Не удалось получить ФИО подписанта';uk='Не удалось получить ФИО подписанта'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл 
			...
			Исключение
				ОтменитьТранзакцию();
				Ошибка = ИнформацияОбОшибке();
				СтрОш = НСтр("ru='Создание уведомлений ТС-1, ТС-2';uk='Создание уведомлений ТС-1, ТС-2'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
				ЗаписьЖурналаРегистрации(СтрОш, УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
		КонецЦикла;
	Исключение
		Ошибка = ИнформацияОбОшибке();
		СтрОш = НСтр("ru='Ошибка при выполнении УведомлениеОСпецрежимахНалогообложения.СоздатьУведомления()';uk='Ошибка при выполнении УведомлениеОСпецрежимахНалогообложения.СоздатьУведомления()'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(СтрОш, УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
КонецПроцедуры
#КонецОбласти
