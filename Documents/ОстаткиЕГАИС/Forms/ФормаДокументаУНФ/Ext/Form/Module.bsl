...
Процедура ЗаполнитьПоРасхождениям(Команда)
	...
	Если Объект.СтатусОбработки <> ПредопределенноеЗначение("Перечисление.СтатусыОбработкиОстатковЕГАИС.ПолученыОстатки") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Для заполнения по расхождениям требуется получение остатков из ЕГАИС.';uk='Для заполнения по расхождениям требуется получение остатков из ЕГАИС.'"));
		Возврат;
	КонецЕсли;
	
	Если Модифицированность ИЛИ НЕ Объект.Проведен Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Документ не проведен.';uk='Документ не проведен.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВыгрузкаВЕГАИС_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Результат Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Запрос на загрузку остатков сформирован.';uk='Запрос на загрузку остатков сформирован.'"));
