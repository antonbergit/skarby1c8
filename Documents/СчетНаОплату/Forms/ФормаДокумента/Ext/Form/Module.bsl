...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьБиллинг") Тогда
		...
		Иначе
			...
			Элементы.РасшифровкаБиллингаОбъектДоговораОбслуживания.Заголовок = НСтр("ru='Номенклатура';uk='Номенклатура'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ДокументОснованиеНадписьОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "выбрать" Тогда
		...
		СписокОснований.Добавить("Документ.ОтчетОПереработке.ФормаВыбора", НСтр("ru='Отчет о переработке';uk='Звіт про переробку'"));
...
Процедура РасшифровкаБиллингаОбъектДоговораОбслуживанияПриИзменении(Элемент)
	...
	СтрокаТабличнойЧасти.ЭтоПланСчетов = ТипЗнч(СтрокаТабличнойЧасти.ОбъектДоговораОбслуживания) = Тип("СправочникСсылка.Номенклатура");
...
Процедура ПересчитаемСуммуТабличнойЧастиПоФлагуАвторасчетНДС()
	...
	УчетНДССервер.ПересчитатьНДСсУчетомПогрешностиОкругления(Объект.Запасы, Объект, ПогрешностиОкругления, "Товары и услуги");
...
Процедура ОбработатьИзменениеДоговора()
	...
	Если ДоговорПередИзменением <> Объект.Договор Тогда
		...
		Если НЕ ЭтоДоговорОбслуживания Тогда
...
&НаСервере
Функция ПредставлениеОбъектаДоговораОбслуживанияПоТарифномуПлану(ОбъектДоговораОбслуживания)
	
	ТарифныйПлан = Объект.Договор.ДоговорОбслуживанияТарифныйПлан;
...
Процедура РасшифровкаБиллингаПриИзмененииОбъектаДоговораОбслуживания(ДанныеЗаполнения)
	...
	ДанныеЗаполнения.ПредставлениеВСчете = 
		ПредставлениеОбъектаДоговораОбслуживанияПоТарифномуПлану(ДанныеЗаполнения.ОбъектДоговораОбслуживания);
