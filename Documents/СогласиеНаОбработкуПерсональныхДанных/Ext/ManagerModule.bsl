...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='Согласие на обработку ПДн (табличный документ 1С:Предприятия)';uk='Згода на обробку ПДн (табличний документ 1С:Підприємства)'");
	
	// В формате Microsoft Word.
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "СогласиеНаОбработкуПерсональныхДанных(MSWord)";
	КомандаПечати.Представление = НСтр("ru='Согласие на обработку ПДн (документ Microsoft Word)';uk='Згода на обробку ПДн (документ Microsoft Word)'");
	...
	КомандаПечати.Представление = НСтр("ru='Согласие на обработку ПДн (документ OpenOffice.org Writer)';uk='Згода на обробку ПДн (документ OpenOffice.org Writer)'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		Шаблон = НСтр("ru='Согласие на обработку ПДн [Субъект] - [Организация] (№[Номер] получено [ДатаПолучения])';uk='Згода на обробку ПДн [Субъект] - [Организация] (№[Номер] отримане [ДатаПолучения])'");
		...
		ПечатнаяФорма.СинонимМакета = НСтр("ru='Согласие на обработку ПДн';uk='Згода на обробку ПДн'");
	...
	Если НужноПечататьМакет Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ru='Согласие на обработку ПДн (документ Microsoft Word)';uk='Згода на обробку ПДн (документ Microsoft Word)'"),
	...
	Если НужноПечататьМакет Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ru='Согласие на обработку ПДн (документ OpenOffice.org Writer)';uk='Згода на обробку ПДн (документ OpenOffice.org Writer)'"),
			ПечатьСогласияНаОбработкуПДн(МассивОбъектов, ОбъектыПечати),
			,
...
Функция ДанныеДляПечатиСогласий(Согласия)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.СрокДействия) Тогда
			Описание.ПериодДействия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='с %1 по %2.';uk='с %1 по %2.'"), Формат(Выборка.ДатаСогласия, "ДЛФ=D"), Формат(Выборка.СрокДействия, "ДЛФ=D"));
		Иначе
			Описание.ПериодДействия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='с %1 бессрочно.';uk='с %1 безстроково.'"), Формат(Выборка.ДатаСогласия, "ДЛФ=D"));
		КонецЕсли;
...
Функция ОписаниеСогласия()
	...
	ОписаниеСогласия.ДатаСогласия = НСтр("ru='«____»_______________20___г.';uk='«____»_______________20___р.'");
	ОписаниеСогласия.Организация = НСтр("ru='<Организация>';uk='<Організація>'");
	ОписаниеСогласия.АдресОрганизации = НСтр("ru='<Адрес организации>';uk='<Адреса організації>'");
	ОписаниеСогласия.ОтветственныйЗаОбработкуПерсональныхДанных = НСтр("ru='<ФИО ответственного лица>';uk='<ПІБ відповідальної особи>'");
