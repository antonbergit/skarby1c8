...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(СрокДействия) И СрокДействия < ДатаПолучения Тогда
		ТекстСообщения = НСтр("ru='Срок действия согласия противоречит дате получения.';uk='Термін дії згоди суперечить даті одержання.'");
	...
	Если Согласие <> Неопределено Тогда
		Если ДатаПолучения = Согласие.ДатаПолучения Тогда
			ТекстСообщения = НСтр("ru='На указанную дату уже зарегистрировано согласие.';uk='На зазначену дату вже зареєстрована згода.'");
