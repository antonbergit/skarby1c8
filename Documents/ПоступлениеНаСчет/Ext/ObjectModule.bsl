...
Процедура ЗаполнитьБанковскийСчетКонтрагента()
	...
	Если ЗначениеЗаполнено(ДокументОснование) Тогда
		...
		Если УправлениеНебольшойФирмойСервер.ЕстьРеквизитДокумента("СчетКонтрагента", ДокументОснование.Метаданные()) Тогда
			
			БанковскийСчет = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "СчетКонтрагента");
			
		ИначеЕсли УправлениеНебольшойФирмойСервер.ЕстьРеквизитДокумента("БанковскийСчетКонтрагента", ДокументОснование.Метаданные()) Тогда
			
			БанковскийСчет = ДокументОснование.БанковскийСчетКонтрагента;
...
Процедура ЗаполнитьПоСчетуНаОплату(ДанныеЗаполнения, НомерСтроки = Неопределено, Сумма = Неопределено) Экспорт
	...
	Иначе
		...
		|		ГДЕ
		|			СтавкиНДС.Ставка = 20
		|			И СтавкиНДС.ПометкаУдаления = ЛОЖЬ) КАК СтавкиНДС
		|		ПО (ИСТИНА)
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ ПЕРВЫЕ 1
...
Процедура ЗаполнитьПоНачислениюНалогов(ДокументОснование) Экспорт
	...
	Если ДокументОснование.ВидОперации <> Перечисления.ВидыОперацийНачислениеНалогов.Возмещение Тогда
		ВызватьИсключение НСтр("ru='Поступление на счет можно ввести только на основании возмещения налогов, а не начисления.';uk='Надходження на рахунок можна ввести тільки на підставі відшкодування податків, а не нарахування.'");
	КонецЕсли;
...
Процедура ЗаполнитьПоРасходнойНакладной(ДокументОснование) Экспорт
	...
	Если ДокументОснование.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПередачаВПереработку Тогда
		ТекстИсключения = НСтр("ru='Невозможен ввод документа на основании операции - ""%ВидОперации""!';uk='Неможливе введення документа на підставі операції - ""%ВидОперации""!'");
	...
	МассивРезльтатов = Запрос.ВыполнитьПакет();
	
	РезультатЗапроса = МассивРезльтатов[0];
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		// Попробуем найти счет на оплату, если он выписан на основании расходной накладной (
		ТекущийСчетНаОплату = Документы.СчетНаОплату.ПустаяСсылка();
		Если Контрагент.ВестиУчетОплатыПоСчетам Тогда
			РезультатЗапроса = МассивРезльтатов[1];
			Если НЕ РезультатЗапроса.Пустой() Тогда
				ВыборкаСчетНаОплату = МассивРезльтатов[1].Выбрать();
				Если ВыборкаСчетНаОплату.Количество() = 1
					...
					Если ЗначениеЗаполнено(НоваяСтрока.Документ) И 
						НЕ ЗначениеЗаполнено(НоваяСтрока.СчетНаОплату) Тогда
...
Процедура ЗаполнитьПоПриемуВРемонт(ДанныеЗаполнения) Экспорт
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, ДанныеЗаполнения,,"Номер, Дата, Комментарий, Автор");
	ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПокупателя;
	
	спРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеЗаполнения, 
		"Договор, Договор.ВалютаРасчетов, Организация.БанковскийСчетПоУмолчанию, Организация.БанковскийСчетПоУмолчанию.ВалютаДенежныхСредств, Организация.СтавкаНДСПоУмолчанию, РемонтВыполнен, СуммаДокумента");
		
	Если спРеквизитов.ДоговорВалютаРасчетов = спРеквизитов.ОрганизацияБанковскийСчетПоУмолчаниюВалютаДенежныхСредств Тогда
		ЭтотОбъект.БанковскийСчет = спРеквизитов.ОрганизацияБанковскийСчетПоУмолчанию;
	КонецЕсли;
	
	ВалютаДенежныхСредств = спРеквизитов.ДоговорВалютаРасчетов;
	НалогообложениеНДС = УправлениеНебольшойФирмойСервер.НалогообложениеНДС(Организация, , Дата);
	...
	Если НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС Тогда
		СтавкаНДСПоУмолчанию = спРеквизитов.ОрганизацияСтавкаНДСПоУмолчанию;
	ИначеЕсли НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.НеОблагаетсяНДС Тогда
		СтавкаНДСПоУмолчанию = УправлениеНебольшойФирмойПовтИсп.ПолучитьСтавкуНДСБезНДС();
	Иначе
		СтавкаНДСПоУмолчанию = УправлениеНебольшойФирмойПовтИсп.ПолучитьСтавкуНДСНоль();
	...
	Если спРеквизитов.РемонтВыполнен Тогда
		...
		|				Организация = &Организация
		|					И Контрагент = &Контрагент
		|					И Документ = &ДокументРемонта
		|					И ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетов.Долг)) КАК РасчетыСПокупателямиОстатки) КАК РасчетыСПокупателямиОстатки
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&Период, Валюта = &Валюта) КАК КурсыВалютДокумента
		|		ПО (ИСТИНА)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&Период, ) КАК КурсыВалютРасчетов
		|		ПО РасчетыСПокупателямиОстатки.Договор.ВалютаРасчетов = КурсыВалютРасчетов.Валюта
		...
		Запрос.УстановитьПараметр("Организация", Компания);
		Запрос.УстановитьПараметр("Контрагент", Контрагент);
		Запрос.УстановитьПараметр("ДокументРемонта", ДанныеЗаполнения);
		Запрос.УстановитьПараметр("Период", Дата);
		Запрос.УстановитьПараметр("Валюта", ВалютаДенежныхСредств);
		Запрос.УстановитьПараметр("Ссылка", Ссылка);
		...
		Иначе
			...
			НоваяСтрока.Договор = спРеквизитов.Договор;
			НоваяСтрока.Курс = ?(
			СтруктураПоВалюте.Курс = 0,
			1,
			СтруктураПоВалюте.Курс
			...
			НоваяСтрока.Кратность = ?(
			СтруктураПоВалюте.Кратность = 0,
			1,
			СтруктураПоВалюте.Кратность
			);
			НоваяСтрока.СуммаРасчетов = 0;
			НоваяСтрока.ПризнакАванса = Истина;
			НоваяСтрока.СуммаПлатежа = 0;
			НоваяСтрока.СтавкаНДС = СтавкаНДСПоУмолчанию;
	...
	Иначе
		...
		НоваяСтрока.СуммаРасчетов = спРеквизитов.СуммаДокумента;
		НоваяСтрока.СуммаПлатежа = спРеквизитов.СуммаДокумента;
		НоваяСтрока.СтавкаНДС = СтавкаНДСПоУмолчанию;
		НоваяСтрока.СуммаНДС = НоваяСтрока.СуммаПлатежа - (НоваяСтрока.СуммаПлатежа) / ((СтавкаНДСПоУмолчанию.Ставка + 100) / 100);
...
Процедура ЗаполнитьЭквайринговыеОперации(ДатаНачала = '00010101', ДатаОкончания = '00010101') Экспорт
	...
	|	ВременнаяТаблицаДляЗаполненияЭквайринговыхОпераций.СуммаРасчетов - ВременнаяТаблицаДляЗаполненияЭквайринговыхОпераций.СуммаРасчетовВозврата - ВременнаяТаблицаДляЗаполненияЭквайринговыхОпераций.СуммаРасчетовКомиссииВозврата - ВременнаяТаблицаДляЗаполненияЭквайринговыхОпераций.СуммаРасчетовКомиссии КАК СуммаПлатежаИтогоПоСтроке,
	...
	ЭквайринговыеОперации.Загрузить(МассивРезультатов[3].Выгрузить());
	
	ВыборкаИтог = МассивРезультатов[4].Выбрать();
	ВыборкаИтог.Следующий();
		
	Если СуммаДокумента = 0 Тогда
		СуммаДокумента = ВыборкаИтог.СуммаДокумента;
	КонецЕсли;
	Если СуммаКомиссииДокумента = 0 Тогда
		СуммаКомиссииДокумента = ВыборкаИтог.СуммаКомиссии;
	КонецЕсли;
	
	Если ЭквайринговыеОперации.Количество() = 0 Тогда
		ЭквайринговыеОперации.Добавить();
		ЭквайринговыеОперации[0].СуммаРасчетов = СуммаДокумента;
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// Эквайринг
	Если ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам Тогда
		...
		Если Не ПолучитьФункциональнуюОпцию("РазноситьОплатуОтЭквайрераПоЭквайринговымОперациям") Тогда
			...
			НоваяСтрока.СуммаРасчетов = СуммаДокумента + СуммаКомиссииДокумента;
			НоваяСтрока.СуммаРасчетовКомиссии = СуммаКомиссииДокумента;
			НоваяСтрока.СуммаПлатежаИтогоПоСтроке = СуммаДокумента;
		КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПоставщика
	 ИЛИ ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПокупателя Тогда
		...
		Если НЕ (ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПокупателя И
		...
		ПогашатьАвтоматически = (ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПокупателя И 
					   ...
					   И НЕ СтрокаРасшифровкаПлатежа.ПризнакАванса)) Тогда
						Если РасшифровкаПлатежа.Количество() = 1 Тогда
							Если ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПокупателя Тогда
								ТекстСообщения = НСтр("ru='Укажите документ отгрузки или признак аванса платежа.';uk='Вкажіть документ відвантаження або ознаку авансу платежу.'");
							Иначе
								ТекстСообщения = НСтр("ru='Укажите документ расчетов.';uk='Вкажіть документ розрахунків.'");
							КонецЕсли;
						Иначе
							Если ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПокупателя Тогда
								ТекстСообщения = НСтр("ru='Укажите документ отгрузки или признак оплаты в строке %НомерСтроки% списка ""Расшифровка платежа"".';uk='Вкажіть документ відвантаження або ознаку оплати в рядку %НомерСтроки% списку ""Розшифрування платежу"".'");
							Иначе
								ТекстСообщения = НСтр("ru='Укажите документ расчетов в строке %НомерСтроки% списка ""Расшифровка платежа"".';uk='Вкажіть документ розрахунків у рядку %НомерСтроки% списку ""Розшифрування платежу"".'");
							КонецЕсли;
		...
		Иначе
			...
			Для каждого СтрокаДоговоры Из ДоговорыАвтоЗачетаДолгов Цикл
				...
				Если (СтрокаДоговоры.Договор.Пустая() И СтрокаДоговоры.СуммаПлатежа > 0) ИЛИ
					...
					ТекстСообщения = НСтр("ru='Для договора под №%НомерСтроки% нужно очистить сумму или выбрать договор.';uk='Для договору під №%НомерСтроки% потрібно очистити суму або вибрати договір.'");
		...
		Если СуммаПлатежа <> СуммаДокумента Тогда
			ТекстСообщения = ?(ПогашатьАвтоматическиСРазнесениемПоДоговорам,
			    НСтр("ru='Сумма документа: %СуммаДокумента% %ВалютаДенежныхСредств%, не соответствует сумме разнесенных платежей по договорам: %СуммаПлатежа% %ВалютаДенежныхСредств%!';uk='Сума документа: %СуммаДокумента% %ВалютаДенежныхСредств%, не відповідає сумі рознесених платежів по договорах: %СуммаПлатежа% %ВалютаДенежныхСредств%!'"),
				НСтр("ru='Сумма документа: %СуммаДокумента% %ВалютаДенежныхСредств%, не соответствует сумме разнесенных платежей в табличной части: %СуммаПлатежа% %ВалютаДенежныхСредств%!';uk='Сума документа: %СуммаДокумента% %ВалютаДенежныхСредств%, не відповідає сумі рознесених платежів у табличній частині: %СуммаПлатежа% %ВалютаДенежныхСредств%!'")
			);
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам Тогда
		...
		Если ПолучитьФункциональнуюОпцию("РазноситьОплатуОтЭквайрераПоЭквайринговымОперациям") Тогда
			...
			Если СуммаКомиссии <> СуммаКомиссииДокумента Тогда
				ТекстСообщения = НСтр("ru='Сумма комиссии: %СуммаКомиссииДокумента% %ВалютаДенежныхСредств%, не соответствует сумме разнесенных платежей в табличной части: %СуммаКомиссии% %ВалютаДенежныхСредств%!';uk='Сума комісії: %СуммаКомиссииДокумента% %ВалютаДенежныхСредств%, не відповідає сумі рознесених платежів у табличній частині: %СуммаКомиссии% %ВалютаДенежныхСредств%!'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаКомиссииДокумента%", Строка(СуммаКомиссииДокумента));
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаКомиссии%", Строка(СуммаКомиссии));
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ВалютаДенежныхСредств%", СокрЛП(Строка(ВалютаДенежныхСредств)));
				УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
					ЭтотОбъект,
					ТекстСообщения,
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ВозвратЗаймаСотрудником Тогда
		...
		Если Подотчетник.Пустая() Тогда
			ТекстСообщения = НСтр("ru='Поле ""Сотрудник"" не заполнено';uk='Поле ""Співробітник"" не заповнене'");
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				ЭтотОбъект,
				ТекстСообщения,
				,
