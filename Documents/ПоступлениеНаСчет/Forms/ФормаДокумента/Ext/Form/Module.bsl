...
Процедура СуммаДокументаАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если Объект.ВидОперации = ВидОперацииОтПокупателя
		...
		Если ОстатокВзаиморасчетов > 0 И (ОстатокВзаиморасчетов) <> Объект.СуммаДокумента Тогда
			Элемент.СписокВыбора.Добавить(ОстатокВзаиморасчетов, ""+(ОстатокВзаиморасчетов)+НСтр("ru=' (должен нам)';uk=' (повинен нам)'"));
		КонецЕсли;
...
Процедура ВидОперацииПриИзмененииНаСервере()
	...
	// Прочие расчеты
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПрочиеРасчеты Тогда
		СтавкаНДСПоУмолчанию = УправлениеНебольшойФирмойПовтИсп.ПолучитьСтавкуНДСБезНДС();
		СтавкаНДСПоУмолчаниюЧисло = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеСтавкиНДС(СтавкаНДСПоУмолчанию);
...
Процедура ОбновитьВидимостьИОстаткиВзаиморасчетов()
	...
	Элементы.ОстатокВзаиморасчетов.Видимость = ЗначениеЗаполнено(Объект.Контрагент);
	// Гунько 2020-04-26
...
Процедура ОчиститьРеквизитыНеОтносящиесяКОперации()
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.ОтПодотчетника") Тогда
		...
		КонецЦикла;
		Объект.ЭквайринговыеОперации.Очистить();
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеНаСчет.Прочее") Тогда
		Объект.Контрагент = Неопределено;
		Объект.ВидНалога = Неопределено;
		Объект.СчетКонтрагента = Неопределено;
		Объект.Подотчетник = Неопределено;
...
Процедура ЗаполнитьПоДоговоруКредита(Команда)
	...
	Если Объект.ДоговорКредитаЗайма.Пустая() Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Выберите договор';uk='Виберіть договір'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьИнформациюПоЗаймуНаСервере()
	...
	Если ВыборкаГрафикБудущиеМесяцы.Следующий() Тогда
		...
		НадписьИнформацияПоКредиту = НСтр("ru='Дата платежа: ';uk='Дата платежу: '")+ДатаПлатежа+НСтр("ru='. Сумма долга: ';uk='. Сума боргу: '")+Формат(ВыборкаГрафикБудущиеМесяцы.СуммаОсновногоДолга, "ЧДЦ=2; ЧН=0")+НСтр("ru='. Сумма %: ';uk='. Сума %: '")+
			...
			НСтр("ru='. Комиссия: ';uk='. Комісія: '")+Формат(ВыборкаГрафикБудущиеМесяцы.СуммаКомиссии, "ЧДЦ=2; ЧН=0")+" ("+ВыборкаГрафикБудущиеМесяцы.ВалютаРасчетовНаименование+")";
	...
	ИначеЕсли ВыборкаГрафик.Следующий() Тогда
		...
		НадписьИнформацияПоКредиту = НСтр("ru='Дата платежа: ';uk='Дата платежу: '")+ДатаПлатежа+НСтр("ru='. Сумма долга: ';uk='. Сума боргу: '")+Формат(ВыборкаГрафик.СуммаОсновногоДолга, "ЧДЦ=2; ЧН=0")+НСтр("ru='. Сумма %: ';uk='. Сума %: '")+
			...
			НСтр("ru='. Комиссия: ';uk='. Комісія: '")+Формат(ВыборкаГрафик.СуммаКомиссии, "ЧДЦ=2; ЧН=0")+" ("+ВыборкаГрафик.ВалютаРасчетовНаименование+")";
	...
	Если ВыборкаОстатки.Следующий() Тогда
		...
		НадписьОстатокДолгаПоКредиту = ИнфомрацияПоКредитуЗайму + НСтр("ru='Остаток долга: ';uk='Залишок боргу: '")+Формат(Множитель * ВыборкаОстатки.ОсновнойДолгВалОстаток, "ЧДЦ=2; ЧН=0")+НСтр("ru='. Сумма %: ';uk='. Сума %: '")+
			Формат(Множитель * ВыборкаОстатки.ПроцентыВалОстаток, "ЧДЦ=2; ЧН=0")+
			НСтр("ru='. Комиссия: ';uk='. Комісія: '")+Формат(Множитель * ВыборкаОстатки.КомиссияВалОстаток, "ЧДЦ=2; ЧН=0")+" ("+ВыборкаОстатки.ВалютаРасчетовНаименование+")";
	...
	Иначе
		...
		НадписьОстатокДолгаПоКредиту = ИнфомрацияПоКредитуЗайму + НСтр("ru='Остаток долга: <не определен> ';uk='Залишок боргу: <не визначений>'");
...
Процедура ЗаполнитьИнформациюПоКредитуНаСервере()
	...
	Если Выборка.Следующий() Тогда
		НадписьИнформацияПоКредиту = НСтр("ru='Сумма кредита: ';uk='Сума кредиту: '")+Выборка.СуммаДокумента+" ("+Выборка.ВалютаРасчетов+")";
		
		Если Выборка.СуммаДокумента < Выборка.ОсновнойДолгВалРасход Тогда
			НадписьОстатокДолгаПоКредиту = НСтр("ru='Осталось получить: ';uk='Залишилося отримати: '")+(Выборка.СуммаДокумента-Выборка.ОсновнойДолгВалРасход)+" ("+Выборка.ВалютаРасчетов+")"+
				НСтр("ru='. Уже получено ';uk='. Вже отримано '")+Выборка.ОсновнойДолгВалРасход+" ("+Выборка.ВалютаРасчетов+").";
			НадписьОстатокДолгаПоКредитуЦветТекста = ЦветаСтиля.ЦветОсобогоТекста;
		ИначеЕсли Выборка.СуммаДокумента = Выборка.ОсновнойДолгВалРасход Тогда
			НадписьОстатокДолгаПоКредиту = НСтр("ru='Осталось получить: 0 (';uk='Залишилося отримати: 0 ('")+Выборка.ВалютаРасчетов+")";
			НадписьОстатокДолгаПоКредитуЦветТекста = ЦветаСтиля.ЦветОсобогоТекста;
		Иначе
			НадписьОстатокДолгаПоКредиту = НСтр("ru='Осталось получить: ';uk='Залишилося отримати: '")+(Выборка.СуммаДокумента-Выборка.ОсновнойДолгВалРасход)+" ("+Выборка.ВалютаРасчетов+")"+
				НСтр("ru='. Уже получено ';uk='. Вже отримано '")+Выборка.ОсновнойДолгВалРасход+" ("+Выборка.ВалютаРасчетов+").";
		КонецЕсли;
	Иначе
		НадписьИнформацияПоКредиту = НСтр("ru='Сумма кредита: ';uk='Сума кредиту: '")+Объект.ДоговорКредитаЗайма.СуммаДокумента+" ("+Объект.ДоговорКредитаЗайма.ВалютаРасчетов+")";
		НадписьОстатокДолгаПоКредиту = НСтр("ru='Осталось получить: ';uk='Залишилося отримати: '")+Объект.ДоговорКредитаЗайма.СуммаДокумента+" ("+Объект.ДоговорКредитаЗайма.ВалютаРасчетов+")";
	КонецЕсли;
	...
	Если ЗначенияРеквитов.ТипСчета <> Перечисления.ТипыСчетов.Расходы Тогда
		Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.Прочее Тогда
			...
			Иначе // Очистить и не заполнена аналитика.
				Объект.АналитикаПрочихДоходовИРасходов = Неопределено;
				Элементы.АналитикаПрочихДоходовИРасходов.Заголовок = НСтр("ru='Аналитика';uk='Аналітика'");
...
Процедура СформироватьЗаголовокГруппыЗачетаДолгов()
	...
	Если Объект.ВариантЗаполненияРасшифровки = ВариантЗаполненияРасшифровкиАвтоматически Тогда
		...
		Если Объект.Контрагент.Пустая() Или НЕ Объект.Контрагент.ВестиРасчетыПоДоговорам Тогда
			Элементы.РасчетыСКонтрагентом.Заголовок = НСтр("ru='Зачет долгов: автоматически';uk='Залік боргів: автоматично'");
		Иначе
			...
			Если КоличествоСтрок = 1 И ЗначениеЗаполнено(Объект.ДоговорыАвтоЗачетаДолгов[0].Договор) Тогда
				Элементы.РасчетыСКонтрагентом.Заголовок = НСтр("ru='Зачет долгов: автоматически по договору ';uk='Залік боргів: автоматично за договором '")+""""+Объект.ДоговорыАвтоЗачетаДолгов[0].Договор+"""";
			ИначеЕсли КоличествоСтрок > 1 Тогда
				...
				КонецЕсли;
			Иначе
				Элементы.РасчетыСКонтрагентом.Заголовок = НСтр("ru='Зачет долгов: автоматически (выберите договор)';uk='Залік боргів: автоматично (виберіть договір)'");
			КонецЕсли;
		КонецЕсли;
...
Процедура ДобавитьДоговорЗачетаДолговНажатиеНаСервере(ТекущаяСтрокаТаблицыДоговоровАвтоЗачета = Неопределено)
	...
	ПолеДоговора.ПодсказкаВвода = НСтр("ru='Если не выбран, то игнорируется';uk='Якщо не вибраний, то ігнорується'");
