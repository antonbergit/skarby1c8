...
Функция УведомлениеОбОтгрузкеМаркированныхТоваровXML2_40(ДокументСсылка)
	...
	Если Шапка.ВидОборотаТовара <> Перечисления.ВидОборотаТовараГИСМ.ЭкспортВСтраныЕАЭС Тогда
		...
		Исключение
			СообщениеXML.ТекстОшибки = НСтр("ru='Не заполнен номер документа-основания.';uk='Не заполнен номер документа-основания.'");
		КонецПопытки;
		
		Попытка
			УведомлениеОбОтгрузкеМаркированныхТоваров.move_doc_date = Шапка.ДатаДокументаОснования;
		Исключение
			СообщениеXML.ТекстОшибки = НСтр("ru='Не заполнена дата документа-основания.';uk='Не заполнена дата документа-основания.'");
		КонецПопытки;
	...
	Иначе
		...
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru='Для контрагента %1 не заполнена страна регистрации.';uk='Для контрагента %1 не заполнена страна регистрации.'"), Шапка.Контрагент));
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.transborder.org_name = Шапка.НаименованиеПолное;
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru='Для контрагента %1 не заполнено полное наименование.';uk='Для контрагента %1 не заполнено полное наименование.'"), Шапка.Контрагент));
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.transborder.org_number = Шапка.РегистрационныйНомер;
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru='Для контрагента %1 не заполнен регистрационный номер.';uk='Для контрагента %1 не заполнен регистрационный номер.'"), Шапка.Контрагент));
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.transborder.org_address = ЮридическийАдрес;
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru='Для контрагента %1 не заполнен юридический адрес.';uk='Для контрагента %1 не заполнен юридический адрес.'"), Шапка.Контрагент));
		КонецПопытки;
		...
		Исключение
			СообщениеXML.ТекстОшибки = СтрШаблон(НСтр("ru='В документе %1 не заполнен номер документа.';uk='В документе %1 не заполнен номер документа.'"), Шапка.Основание);
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.export_doc_date = Шапка.ДатаДокументаОснования;
		Исключение
			СообщениеXML.ТекстОшибки = СтрШаблон(НСтр("ru='В документе %1 не заполнена дата документа.';uk='В документе %1 не заполнена дата документа.'"), , Шапка.Основание);
		КонецПопытки;
...
Функция УведомлениеОбОтгрузкеМаркированныхТоваровXML2_41(ДокументСсылка)
			...
			Для Каждого СтрокаТЧ Из Товары Цикл
				...
				Если Не ЗначениеЗаполнено(СтрокаТЧ.НомерКиЗ)
					...
					ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
						СообщениеXML,
						СтрШаблон(НСтр("ru='В строке %1 не указаны данные о КиЗ.';uk='В строке %1 не указаны данные о КиЗ.'"),
				...
				Если ЗначениеЗаполнено(СтрокаТЧ.НомерКиЗ) Тогда
					...
					Исключение
						ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
							СообщениеXML,
							СтрШаблон(НСтр("ru='В строке %1 указан некорректный номер КиЗ ""%2"".';uk='В строке %1 указан некорректный номер КиЗ ""%2"".'"),
								СтрокаТЧ.НомерСтроки,
								СтрокаТЧ.НомерКиЗ));
					КонецПопытки;
				КонецЕсли;
				...
				Если ЗначениеЗаполнено(СтрокаТЧ.TID) Тогда
					...
					Исключение
						ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
							СообщениеXML,
							СтрШаблон(НСтр("ru='В строке %1 указан некорректный TID ""%2"".';uk='В строке %1 указан некорректный TID ""%2"".'"),
								СтрокаТЧ.НомерСтроки,
								СтрокаТЧ.TID));
					КонецПопытки;
				КонецЕсли;
				...
				Если ЗначениеЗаполнено(СтрокаТЧ.EPC) Тогда
					...
					Исключение
						ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(
							СообщениеXML,
							СтрШаблон(НСтр("ru='В строке %1 указан некорректный EPC ""%2"".';uk='В строке %1 указан некорректный EPC ""%2"".'"),
								СтрокаТЧ.НомерСтроки,
								СтрокаТЧ.EPC));
					КонецПопытки;
				КонецЕсли;
		...
		Иначе
			...
			Иначе
				...
				Если РеквизитыКонтрагента.ЭтоФизическоеЛицо Тогда
					...
					Исключение
						СообщениеXML.ТекстОшибки = СтрШаблон(НСтр("ru='У контрагента %1 не правильно заполнено ИНН.';uk='У контрагента %1 не правильно заполнено ИНН.'"), Шапка.Контрагент);
					КонецПопытки;
				...
				Иначе
					...
					Исключение
						СообщениеXML.ТекстОшибки = СтрШаблон(НСтр("ru='У контрагента %1 не правильно заполнено КПП.';uk='У контрагента %1 не правильно заполнено КПП.'"), Шапка.Контрагент);
					КонецПопытки;
	...
	ИначеЕсли Шапка.ВидОборотаТовара = Перечисления.ВидОборотаТовараГИСМ.ЭкспортВСтраныЕАЭС Тогда
		...
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru='Для контрагента %1 указана неверно или не заполнена страна регистрации.';uk='Для контрагента %1 указана неверно или не заполнена страна регистрации.'"), Шапка.Контрагент));
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.transborder_info.org_name = РеквизитыКонтрагента.НаименованиеПолное;
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru='Для контрагента %1 указан неверно или не заполнено полное наименование.';uk='Для контрагента %1 указан неверно или не заполнено полное наименование.'"), Шапка.Контрагент));
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.transborder_info.org_number = РеквизитыКонтрагента.РегистрационныйНомер;
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru='Для контрагента %1 указан неверно или не заполнен регистрационный номер.';uk='Для контрагента %1 указан неверно или не заполнен регистрационный номер.'"), Шапка.Контрагент));
		КонецПопытки;
		
		Попытка
			УведомлениеОбЭкспорте.transborder_info.org_address = РеквизитыКонтрагента.ЮридическийАдрес;
		Исключение
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru='Для контрагента %1 указан неверно или не заполнен юридический адрес.';uk='Для контрагента %1 указан неверно или не заполнен юридический адрес.'"), Шапка.Контрагент));
		КонецПопытки;
