...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЭтоНовый() И ЗначениеЗаполнено(Основание) Тогда
		...
		Если Не Результат.Пустой() Тогда
			...
			ТекстОшибки = НСтр("ru='Для документа %1 уже существует текущая %2.';uk='Для документа %1 уже существует текущая %2.'");
	...
	Для каждого СтрокаНомераКиЗ Из НомераКиЗ Цикл
		...
		Если СтрокаНомераКиЗ.Индивидуализирован Тогда
			...
			Если НЕ ЗначениеЗаполнено(СтрокаНомераКиЗ.НомерКиЗ)
				...
				ТекстОшибки = НСтр("ru='В строке %НомерСтроки% списка ""Товары"" не указан ни ""Номер КиЗ"", ни ""RFID TID"", ни ""RFID EPC""';uk='В строке %НомерСтроки% списка ""Товары"" не указан ни ""Номер КиЗ"", ни ""RFID TID"", ни ""RFID EPC""'");
		...
		Иначе
			...
			Если НЕ ЗначениеЗаполнено(СтрокаНомераКиЗ.НомерКиЗ) Тогда
				...
				ТекстОшибки = НСтр("ru='В строке %НомерСтроки% списка ""Товары"" не указан ""Номер КиЗ""';uk='В строке %НомерСтроки% списка ""Товары"" не указан ""Номер КиЗ""'");
				ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", НомерСтроки);
			...
			Если НЕ ЗначениеЗаполнено(СтрокаНомераКиЗ.ПричинаСписания) Тогда
				...
				ТекстОшибки = НСтр("ru='В строке %НомерСтроки% списка ""Товары"" не указана ""Причина списания""';uk='В строке %НомерСтроки% списка ""Товары"" не указана ""Причина списания""'");
				ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", НомерСтроки);
			...
			Если НЕ ЗначениеЗаполнено(СтрокаНомераКиЗ.ЗаявкаНаВыпускКиЗ) Тогда
				...
				ТекстОшибки = НСтр("ru='В строке %НомерСтроки% списка ""Товары"" не указана ""Заявка на выпуск КиЗ""';uk='В строке %НомерСтроки% списка ""Товары"" не указана ""Заявка на выпуск КиЗ""'");
				ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", НомерСтроки);
