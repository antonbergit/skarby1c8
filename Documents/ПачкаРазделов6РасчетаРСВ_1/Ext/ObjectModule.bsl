	...
	Пока ВыборкаПоСтрокамДокумента.СледующийПоЗначениюПоля("НомерСтрокиСотрудник") Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Сотрудник) Тогда 
			...
			Если ВыборкаПоСтрокамДокумента.КонфликтующаяСтрока <> Null Тогда  
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о сотруднике %1 была введена в документе ранее.';uk='Інформація про співробітника %1 була введена в документі раніше.'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
			...
			ИначеЕсли ВыборкаПоСтрокамДокумента.КонфликтующаяСтрокаСтраховойНомер <> Null Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сотрудник %1: информация о сотруднике с таким же страховым номером была введена в документе ранее.';uk='Співробітник %1: інформація про співробітника з таким же страховим номером була введена в документі раніше.'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
...
Процедура ПроверитьДанныеШапкиДокумента(Ошибки, Отказ = Ложь, НеПроверяемыеРеквизиты = Неопределено)
	Если Сотрудники.Количество() > 200 Тогда
		ТекстОшибки = НСтр("ru='В документе должно быть не более 200 форм.';uk='У документі повинно бути не більш 200 форм.'");
	...
	Если ОтчетныйПериод < '20140101' Тогда
		ТекстОшибки = НСтр("ru='Разделы 6 Расчета РСВ-1 представляются начиная с 1 квартала 2014 года! Для формирования сведений за 2010-2012 годы используйте документ ""Пачка документов СЗВ-6-1"", ""Реестр СЗВ-6-2"" , за 2013 год используйте документ """"Пачка документов СЗВ-6-4"""".';uk='Розділи 6 Розрахунку РСВ-1 представляються починаючи з 1 кварталу 2014 року! Для формування відомостей за 2010-2012 роки використовуйте документ ""Пачка документів СЗВ-6-1"", ""Реєстр СЗВ-6-2"", за 2013 рік використовуйте документ """"Пачка документів СЗВ-6-4"""".'");
	...
	Если ТипСведенийСЗВ <> Перечисления.ТипыСведенийСЗВ.ИСХОДНАЯ Тогда
		Если Не ЗначениеЗаполнено(КорректируемыйПериод) Тогда
			ТекстОшибки = НСтр("ru='Не указан корректируемый отчетный период.';uk='Не зазначений коректований звітний період.'");
		...
		ИначеЕсли ОтчетныйПериод <= КорректируемыйПериод Тогда
			ТекстОшибки = НСтр("ru='Корректируемый период должен предшествовать отчетному.';uk='Коригувальний період повинен передувати звітному.'");
		
			ПерсонифицированныйУчетКлиентСервер.ДобавитьОшибкуЗаполненияЭлементаДокумента(Ошибки, Ссылка, ТекстОшибки, "КорректируемыйПериод", Отказ);
		ИначеЕсли КорректируемыйПериод < '20140101' Тогда
			ТекстОшибки = НСтр("ru='Корректирующие (отменяющие) Разделы 6 Расчета РСВ-1 представляются только за отчетные периоды c 1 квартала 2014 года.';uk='Коригувальні (що скасовують) Розділи 6 Розрахунків РСВ-1 представляються тільки за звітні періоди c 1 кварталу 2014 року.'");
...
Процедура ПроверитьСтрокиЗаработка(Ошибки, ВыборкаДанныхСотрудника, ВыборкаПоДоходам, ТарифыПоКатегориям, Отказ = Ложь)
		...
		Пока ВыборкаПоДоходам.СледующийПоЗначениюПоля("КатегорияЗастрахованныхЛиц") Цикл
			...
			Пока ВыборкаПоДоходам.Следующий() Цикл
				Если ЗначениеЗаполнено(ВыборкаПоДоходам.Месяц) 
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='в строках заработка указан месяц (%1), не принадлежащий отчетному периоду';uk='у рядках заробітку зазначений місяць (%1), що не належить звітному періоду'"), ВыборкаПоДоходам.Месяц);
				...
				Если Не ЗначениеЗаполнено(ВыборкаПоДоходам.КатегорияЗастрахованныхЛиц) Тогда
					ТекстОшибки = НСтр("ru='не указана категория застрахованных лиц.';uk='не зазначена категорія застрахованих осіб.'");
				...
				Если ВыборкаПоДоходам.Заработок = 0 И ВыборкаПоДоходам.ОблагаетсяВзносамиДоПредельнойВеличины = 0 И ВыборкаПоДоходам.ОблагаетсяВзносамиСвышеПредельнойВеличины = 0  Тогда
					ТекстОшибки = НСтр("ru='указаны нулевые данные.';uk='зазначені нульові дані.'");
					
					ДобавитьОшибкуЗаработка(Ошибки, ВыборкаДанныхСотрудника, ВыборкаПоДоходам, ТекстОшибки, Отказ);
				КонецЕсли;
				Если ВыборкаПоДоходам.Заработок < 0 Тогда
					ТекстОшибки = НСтр("ru='указан отрицательный заработок.';uk='зазначений негативний заробіток.'");
					
					ДобавитьОшибкуЗаработка(Ошибки, ВыборкаДанныхСотрудника, ВыборкаПоДоходам, ТекстОшибки, Отказ);
				КонецЕсли;
				Если ВыборкаПоДоходам.ОблагаетсяВзносамиДоПредельнойВеличины < 0 Тогда
					ТекстОшибки = НСтр("ru='указана отрицательная база, не превышающая предельную величину.';uk='зазначена негативна база, що не перевищує граничну величину.'");
					
					ДобавитьОшибкуЗаработка(Ошибки, ВыборкаДанныхСотрудника, ВыборкаПоДоходам, ТекстОшибки, Отказ);
				КонецЕсли;
				Если ВыборкаПоДоходам.ПоДоговорамГПХДоПредельнойВеличины < 0 Тогда
					ТекстОшибки = НСтр("ru='указана отрицательная база по договорам ГПХ, не превышающая предельную величину';uk='зазначена негативна база по договорах ЦПХ, що не перевищує граничну величину'");
					
					ДобавитьОшибкуЗаработка(Ошибки, ВыборкаДанныхСотрудника, ВыборкаПоДоходам, ТекстОшибки, Отказ);
				КонецЕсли;
				Если ВыборкаПоДоходам.ОблагаетсяВзносамиСвышеПредельнойВеличины < 0 Тогда
					ТекстОшибки = НСтр("ru='указана отрицательная база, превышающая предельную величину.';uk='зазначена негативна база, що перевищує граничну величину.'");
				...
				Если ВыборкаПоДоходам.Заработок < ВыборкаПоДоходам.ОблагаетсяВзносамиДоПредельнойВеличины + ВыборкаПоДоходам.ОблагаетсяВзносамиСвышеПредельнойВеличины Тогда
					ТекстОшибки = НСтр("ru='сумма, облагаемая взносами, превышает сумму начисленного заработка.';uk='сума, оподатковувана внесками, перевищує суму нарахованого заробітку.'");
		...
		Если ЗначениеЗаполнено(ПериодДанных) Тогда
			Для каждого СтрокаТаблицы Из МесяцыДохода Цикл
				Если СтрокаТаблицы.КоличествоСтрок > 1 Тогда
					ШаблонТекстаОшибки = НСтр("ru='Сотрудник %1: в разделе 6.4 (сведения о заработке) по категории %2 месяц №%3 указан более одного раза';uk='Співробітник %1: у розділі 6.4 (відомості про заробіток) по категорії %2 місяць №%3 зазначений більш одного разу'");
	...
	ТекстОшибки = НСтр("ru='- значение, указанное в подразделе 6.5 (начислено на ОПС) должно быть равно сумме значений, указанных в графе 5 строк 401,402 и 403  (411, 412, 413 и т.д., зависит от количества тарифов страховых взносов),  умноженных на соответствующий текущему периоду тариф страховых взносов.';uk='- значення, вказане в підрозділі 6.5 (нараховане на ОПС) повинне дорівнювати сумі значень, зазначених у графі 5 рядків 401,402 і 403  (411, 412, 413 і т.д., залежить від кількості тарифів страхових внесків),  помножених на відповідний до потокового періоду тариф страхових внесків.'");
	...
	Если Разность > 0.03 Тогда
		ШаблонТекстаОшибки = НСтр("ru='Сотрудник %1: %2';uk='Співробітник %1: %2'");	
	...
	Если Заработок = 0 И ВыборкаДанныхСотрудника.КоличествоОплачиваемыхСтрок > 0 Тогда
		...
		ТекстОшибки = НСтр("ru='Если в подразделе 6.8 содержится запись в графах 2 и 3, которая не сопровождается записью с кодом «АДМИНИСТР», «ДЕТИ», «УЧОТПУСК», «ДЛДЕТИ», «ДЛОТПУСК», «ВРНЕТРУД», «ДЕКРЕТ», «ЧАЭС», «НЕОПЛ», «НЕОПЛДОГ» и «НЕОПЛАВТ»  обязательно наличие значения в графе 4 строки 401-403  подраздела 6.4';uk='Якщо в підрозділі 6.8 міститься запис у графах 2 і 3, який не супроводжується записом з кодом «АДМІНІСТР», «ДІТИ», «УЧОТПУСК», «ДЛДЕТИ», «ДЛОТПУСК», «ВРНЕТРУД», «ДЕКРЕТ», «ЧАЕС», «НЕОПЛ», «НЕОПЛДОГ» і «НЕОПЛАВТ»  обов`язкова наявність значення в графі 4 рядка 401-403  підрозділу 6.4'");
...
Функция ТекстОшибкиЗаработка(НаименованиеСотрудника, Месяц = Неопределено, КатегорияЗастрахованныхЛиц = Неопределено, ТекcтОшибки)
	Если Месяц = Неопределено Тогда
		ОписаниеМесяца = "";	
	Иначе	
		ОписаниеМесяца = ?(Месяц = 0, НСтр("ru='- в показателе С начала года ';uk='- у показнику З початку року'"), НСтр("ru='- за ';uk='- за'") + Месяц + НСтр("ru='-й месяц ';uk='-й місяць'"));	
	КонецЕсли;	
	...
	Иначе
		ОписаниеКатегории = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='по категории %1 ';uk='по категорії %1'"),
								ОбщегоНазначения.ИмяЗначенияПеречисления(КатегорияЗастрахованныхЛиц));
		
	КонецЕсли;	
	
	ШаблонТекстаОшибки = НСтр("ru='Сотрудник %1: в разделе 6.4 (сведения о заработке) %2%3%4';uk='Співробітник %1: у розділі 6.4 (відомості про заробіток) %2%3%4'");
...
Процедура ПроверитьСтрокиВредногоЗаработка(Ошибки, ВыборкаДанныхСотрудника, ВыборкаПоДоходамЗанятыхНаВредныхИТяжелыхРаботах, Отказ = Ложь)
	...
	Если ВыборкаПоДоходамЗанятыхНаВредныхИТяжелыхРаботах.СведенияЗаполнены Тогда
		Пока ВыборкаПоДоходамЗанятыхНаВредныхИТяжелыхРаботах.СледующийПоЗначениюПоля("КлассУсловийТруда") Цикл
			Пока ВыборкаПоДоходамЗанятыхНаВредныхИТяжелыхРаботах.Следующий() Цикл 		
				Если ЗначениеЗаполнено(ВыборкаПоДоходамЗанятыхНаВредныхИТяжелыхРаботах.Месяц) 
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='в строках заработка указан месяц (%1), не принадлежащий отчетному периоду';uk='у рядках заробітку зазначений місяць (%1), що не належить звітному періоду'"), 
				...
				Если ВыборкаПоДоходамЗанятыхНаВредныхИТяжелыхРаботах.ОблагаетсяВзносамиЗаЗанятыхНаПодземныхИВредныхРаботах < 0 Тогда
					ТекстОшибки = НСтр("ru='указана отрицательная база взносов за занятых на подземных и вредных работах.';uk='зазначена негативна база внесків за зайнятих на підземних і шкідливих роботах.'");
				...
				Если ВыборкаПоДоходамЗанятыхНаВредныхИТяжелыхРаботах.ОблагаетсяВзносамиЗаЗанятыхНаТяжелыхИПрочихРаботах < 0 Тогда
					ТекстОшибки = НСтр("ru='указана отрицательная база взносов за занятых на тяжелых работах.';uk='зазначена негативна база внесків за зайнятих на важких роботах.'");
		...
		Если ЗначениеЗаполнено(ПериодДанных) Тогда 
			Для каждого СтрокаТаблицы Из МесяцыДохода Цикл
				Если СтрокаТаблицы.КоличествоСтрок > 1 Тогда
					ШаблонТекстаОшибки = НСтр("ru='Сотрудник %1: в разделе 6.7 (Сведения о выплатах занятых на работах дающих право на досрочную пенсию) по классу условий труда %2 месяц №%3 указан более одного раза';uk='Співробітник %1: у розділі 6.7 (Відомості про виплати зайнятих на роботах, що дають право на дострокову пенсію) по класу умов праці %2 місяць №%3 зазначений більш одного разу'");
	...
	Если ВыборкаДанныхСотрудника.Ст27_1ВПервомМесяце Тогда
		Если ОблагаетсяВзносамиЗаЗанятыхНаПодземныхИВредныхРаботахВПервомМесяце = 0 Тогда
			ТекстОшибки = НСтр("ru='При наличии кода 27-1 особых условий труда в строках подраздела 6.8 за первый месяц отчетного периода обязательно наличие значений в графе 4 подраздела 6.7 в строках 701,711 ...';uk='При наявності коду 27-1 особливих умов праці в рядках підрозділу 6.8 за перший місяць звітного періоду обов`язкова наявність значень у графі 4 підрозділу 6.7 у рядках 701,711 ...'");
	...
	ИначеЕсли ОблагаетсяВзносамиЗаЗанятыхНаПодземныхИВредныхРаботахВПервомМесяце <> 0 Тогда	
		ТекстОшибки = НСтр("ru='При наличии ненулевых значений в строках 701,711 ... графы 4 подраздела 6.7  обязательно наличие в графе «Особые условия труда (код)» кода особых условий труда 27-1 в строках подраздела 6.8 за первый месяц отчетного периода';uk='При наявності ненульових значень у рядках 701,711 ... графи 4 підрозділу 6.7  обов`язкова наявність у графі «Особливі умови праці (код)» коду особливих умов праці 27-1 у рядках підрозділу 6.8 за перший місяць звітного періоду'");
	...
	Если ВыборкаДанныхСотрудника.Ст27_1ВоВторомМесяце Тогда
		Если ОблагаетсяВзносамиЗаЗанятыхНаПодземныхИВредныхРаботахВоВторомМесяце = 0 Тогда
			ТекстОшибки = НСтр("ru='При наличии кода 27-1 особых условий труда в строках подраздела 6.8 за второй месяц отчетного периода обязательно наличие значений в графе 4 подраздела 6.7 в строках 702,712 ...';uk='При наявності коду 27-1 особливих умов праці в рядках підрозділу 6.8 за другий місяць звітного періоду обов`язкова наявність значень у графі 4 підрозділу 6.7 у рядках 702,712 ...'");
	...
	ИначеЕсли ОблагаетсяВзносамиЗаЗанятыхНаПодземныхИВредныхРаботахВоВторомМесяце <> 0 Тогда	
		ТекстОшибки = НСтр("ru='При наличии ненулевых значений в строках 702,712 ... графы 4 подраздела 6.7  обязательно наличие в графе «Особые условия труда (код)» кода особых условий труда 27-1 в строках подраздела 6.8 за второй месяц отчетного периода';uk='При наявності ненульових значень у рядках 702,712 ... графи 4 підрозділу 6.7  обов`язкова наявність у графі «Особливі умови праці (код)» коду особливих умов праці 27-1 у рядках підрозділу 6.8 за другий місяць звітного періоду'");
	...
	Если ВыборкаДанныхСотрудника.Ст27_1ВТретьемМесяце Тогда
		Если ОблагаетсяВзносамиЗаЗанятыхНаПодземныхИВредныхРаботахВТретьемМесяце = 0 Тогда
			ТекстОшибки = НСтр("ru='При наличии кода 27-1 особых условий труда в строках подраздела 6.8 за третий месяц отчетного периода обязательно наличие значений в графе 4 подраздела 6.7 в строках 703,713 ...';uk='При наявності коду 27-1 особливих умов праці в рядках підрозділу 6.8 за третій місяць звітного періоду обов`язкова наявність значень у графі 4 підрозділу 6.7 у рядках 703,713 ...'");
	...
	ИначеЕсли ОблагаетсяВзносамиЗаЗанятыхНаПодземныхИВредныхРаботахВТретьемМесяце <> 0 Тогда	
		ТекстОшибки = НСтр("ru='При наличии ненулевых значений в строках 703,713 ... графы 4 подраздела 6.7  обязательно наличие в графе «Особые условия труда (код)» кода особых условий труда 27-1 в строках подраздела 6.8 за третий месяц отчетного периода';uk='При наявності ненульових значень у рядках 703,713 ... графи 4 підрозділу 6.7  обов`язкова наявність у графові «Особливі умови праці (код)» коду особливих умов праці 27-1 у рядках підрозділу 6.8 за третій місяць звітного періоду'");
	...
	Если ВыборкаДанныхСотрудника.ОстальныеКодыВПервомМесяце Тогда
		Если ОблагаетсяВзносамиЗаЗанятыхНаТяжелыхИПрочихРаботахВПервомМесяце = 0 Тогда
			ТекстОшибки = НСтр("ru='При наличии в строках подраздела 6.8 за первый месяц отчетного периода в графе «Особые условия труда (код)» одного из кодов особых условий труда, кроме «27-1» и «28-СЕВ»  (27-2, 27-3, 27-4, 27-5, 27-6, 27-7, 27-8, 27-9, 27-10, 27-ОС, 27-ПЖ)  или в графе «Условия для досрочного назначения трудовой пенсии. Основание (код)» одного из кодов: 27-11-ГР, 27-11-ВП, 27-12, 27-СП, САМОЛЕТ, СПЕЦАВ, СПАСАВ, УЧЛЕТ, ВЫСШПИЛ, НОРМАПР, НОРМСП, РЕАКТИВН, ЛЕТРАБ, 27-14, 27-15 обязательно наличие значений в графе 5 подраздела 6.7 в строках 701,711 ...';uk='При наявності в рядках підрозділу 6.8 за перший місяць звітного періоду в графі «Особливі умови праці (код)» одного з кодів особливих умов праці, крім «27-1» і «28-СЕВ»  (27-2, 27-3, 27-4, 27-5, 27-6, 27-7, 27-8, 27-9, 27-10, 27-ОС, 27-ПЖ)  або в графі «Умови для дострокового призначення трудової пенсії. Підстава (код)» одного з кодів: 27-11-ГР, 27-11-ВП, 27-12, 27-СП, САМОЛЕТ, СПЕЦАВ, СПАСАВ, УЧЛЕТ, ВЫСШПИЛ, НОРМАПР, НОРМСП, РЕАКТИВН, ЛЕТРАБ, 27-14, 27-15 обов`язкова наявність значень у графі 5 підрозділу 6.7 у рядках 701,711 ...'");
	...
	ИначеЕсли ОблагаетсяВзносамиЗаЗанятыхНаТяжелыхИПрочихРаботахВПервомМесяце <> 0 Тогда
		ТекстОшибки = НСтр("ru='При наличии ненулевых значений в строках 701,711 ... графы 5 подраздела 6.7 обязательно наличие в строках подраздела 6.8 за первый месяц отчетного периода в графе «Особые условия труда (код)» одного из кодов особых условий труда, кроме «27-1» и «28-СЕВ» (27-2, 27-3, 27-4, 27-5, 27-6, 27-7, 27-8, 27-9, 27-10, 27-ОС, 27-ПЖ) или в графе «Условия для досрочного назначения трудовой пенсии. Основание (код)» одного из кодов: 27-11-ГР, 27-11-ВП, 27-12, 27-СП, САМОЛЕТ, СПЕЦАВ, СПАСАВ, УЧЛЕТ, ВЫСШПИЛ, НОРМАПР, НОРМСП, РЕАКТИВН, ЛЕТРАБ, 27-14, 27-15';uk='При наявності ненульових значень у рядках 701,711 ... графи 5 підрозділу 6.7 обов`язкова наявність у рядках підрозділу 6.8 за перший місяць звітного періоду в графі «Особливі умови праці (код)» одного з кодів особливих умов праці, крім «27-1» і «28-СЕВ» (27-2, 27-3, 27-4, 27-5, 27-6, 27-7, 27-8, 27-9, 27-10, 27-ОС, 27-ПЖ) або в графі «Умови для дострокового призначення трудової пенсії. Підстава (код)» одного з кодів: 27-11-ГР, 27-11-ВП, 27-12, 27-СП, САМОЛЕТ, СПЕЦАВ, СПАСАВ, УЧЛЕТ, ВЫСШПИЛ, НОРМАПР, НОРМСП, РЕАКТИВН, ЛЕТРАБ, 27-14, 27-15'");
	...
	Если ВыборкаДанныхСотрудника.ОстальныеКодыВоВторомМесяце Тогда
		Если ОблагаетсяВзносамиЗаЗанятыхНаТяжелыхИПрочихРаботахВоВторомМесяце = 0 Тогда
			ТекстОшибки = НСтр("ru='При наличии в строках подраздела 6.8 за второй месяц отчетного периода в графе «Особые условия труда (код)» одного из кодов особых условий труда, кроме «27-1» и «28-СЕВ» 27-2, 27-3, 27-4, 27-5, 27-6, 27-7, 27-8, 27-9, 27-10, 27-ОС, 27-ПЖ)  или в графе «Условия для досрочного назначения трудовой пенсии. Основание (код)» одного из кодов: 27-11-ГР, 27-11-ВП, 27-12, 27-СП, САМОЛЕТ, СПЕЦАВ, СПАСАВ, УЧЛЕТ, ВЫСШПИЛ, НОРМАПР, НОРМСП, РЕАКТИВН, ЛЕТРАБ, 27-14, 27-15 обязательно наличие значений в графе 5 подраздела 6.7 в строках 702,712 ...';uk='При наявності в рядках підрозділу 6.8 за другий місяць звітного періоду в графі «Особливі умови праці (код)» одного з кодів особливих умов праці, крім «27-1» і «28-СЕВ» 27-2, 27-3, 27-4, 27-5, 27-6, 27-7, 27-8, 27-9, 27-10, 27-ОС, 27-ПЖ)  або в графі «Умови для дострокового призначення трудової пенсії. Підстава (код)» одного з кодів: 27-11-ГР, 27-11-ВП, 27-12, 27-СП, САМОЛЕТ, СПЕЦАВ, СПАСАВ, УЧЛЕТ, ВЫСШПИЛ, НОРМАПР, НОРМСП, РЕАКТИВН, ЛЕТРАБ, 27-14, 27-15 обов`язкова наявність значень у графі 5 підрозділу 6.7 у рядках 702,712 ...'");
	...
	ИначеЕсли ОблагаетсяВзносамиЗаЗанятыхНаТяжелыхИПрочихРаботахВоВторомМесяце <> 0 Тогда	
		ТекстОшибки = НСтр("ru='При наличии ненулевых значений в строках 702,712 ... графы 5 подраздела 6.7 обязательно наличие в строках подраздела 6.8 за второй месяц отчетного периода в графе «Особые условия труда (код)» одного из кодов особых условий труда, кроме «27-1» и «28-СЕВ» (27-2, 27-3, 27-4, 27-5, 27-6, 27-7, 27-8, 27-9, 27-10, 27-ОС, 27-ПЖ)  или в графе «Условия для досрочного назначения трудовой пенсии. Основание (код)» одного из кодов: 27-11-ГР, 27-11-ВП, 27-12, 27-СП, САМОЛЕТ, СПЕЦАВ, СПАСАВ, УЧЛЕТ,  ВЫСШПИЛ, НОРМАПР, НОРМСП, РЕАКТИВН, ЛЕТРАБ, 27-14, 27-15';uk='При наявності ненульових значень у рядках 702,712 ... графи 5 підрозділу 6.7 обов`язкова наявність у рядках підрозділу 6.8 за другий місяць звітного періоду в графі «Особливі умови праці (код)» одного з кодів особливих умов праці, крім «27-1» і «28-СЕВ» (27-2, 27-3, 27-4, 27-5, 27-6, 27-7, 27-8, 27-9, 27-10, 27-ОС, 27-ПЖ)  або в графі «Умови для дострокового призначення трудової пенсії. Підстава (код)» одного з кодів: 27-11-ГР, 27-11-ВП, 27-12, 27-СП, САМОЛЕТ, СПЕЦАВ, СПАСАВ, УЧЛЕТ,  ВЫСШПИЛ, НОРМАПР, НОРМСП, РЕАКТИВН, ЛЕТРАБ, 27-14, 27-15'");
	...
	Если ВыборкаДанныхСотрудника.ОстальныеКодыВТретьемМесяце Тогда
		Если ОблагаетсяВзносамиЗаЗанятыхНаТяжелыхИПрочихРаботахВТретьемМесяце = 0 Тогда
			ТекстОшибки = НСтр("ru='При наличии в строках подраздела 6.8 за третий месяц отчетного периода в графе «Особые условия труда (код)» одного из кодов особых условий труда, кроме «27-1» и «28-СЕВ» (27-2, 27-3, 27-4, 27-5, 27-6, 27-7, 27-8, 27-9, 27-10, 27-ОС, 27-ПЖ)  или в графе «Условия для досрочного назначения трудовой пенсии. Основание (код)» одного из кодов: 27-11-ГР, 27-11-ВП, 27-12, 27-СП, САМОЛЕТ, СПЕЦАВ, СПАСАВ, УЧЛЕТ, ВЫСШПИЛ, НОРМАПР, НОРМСП, РЕАКТИВН, ЛЕТРАБ, 27-14, 27-15 обязательно наличие значений в графе 5 подраздела 6.7 в строках 701,711 ...';uk='При наявності в рядках підрозділу 6.8 за третій місяць звітного періоду в графі «Особливі умови праці (код)» одного з кодів особливих умов праці, крім «27-1» і «28-СЕВ» (27-2, 27-3, 27-4, 27-5, 27-6, 27-7, 27-8, 27-9, 27-10, 27-ОС, 27-ПЖ)  або в графі «Умови для дострокового призначення трудової пенсії. Підстава (код)» одного з кодів: 27-11-ГР, 27-11-ВП, 27-12, 27-СП, САМОЛЕТ, СПЕЦАВ, СПАСАВ, УЧЛЕТ, ВЫСШПИЛ, НОРМАПР, НОРМСП, РЕАКТИВН, ЛЕТРАБ, 27-14, 27-15 обов`язкова наявність значень у графі 5 підрозділу 6.7 у рядках 701,711 ...'");
	...
	ИначеЕсли ОблагаетсяВзносамиЗаЗанятыхНаТяжелыхИПрочихРаботахВТретьемМесяце <> 0 Тогда	
		ТекстОшибки = НСтр("ru='При наличии ненулевых значений в строках 701,711 ... графы 5 подраздела 6.7 обязательно наличие в строках подраздела 6.8 за третий месяц отчетного периода в графе «Особые условия труда (код)» одного из кодов особых условий труда, кроме «27-1» и «28-СЕВ» (27-2, 27-3, 27-4, 27-5, 27-6, 27-7, 27-8, 27-9, 27-10, 27-ОС, 27-ПЖ) или в графе «Условия для досрочного назначения трудовой пенсии. Основание (код)» одного из кодов: 27-11-ГР, 27-11-ВП, 27-12, 27-СП, САМОЛЕТ, СПЕЦАВ, СПАСАВ, УЧЛЕТ,  ВЫСШПИЛ, НОРМАПР, НОРМСП, РЕАКТИВН, ЛЕТРАБ, 27-14, 27-15';uk='При наявності ненульових значень у рядках 701,711 ... графи 5 підрозділу 6.7 обов`язкова наявність у рядках підрозділу 6.8 за третій місяць звітного періоду в графі «Особливі умови праці (код)» одного з кодів особливих умов праці, крім «27-1» і «28-СЕВ» (27-2, 27-3, 27-4, 27-5, 27-6, 27-7, 27-8, 27-9, 27-10, 27-ОС, 27-ПЖ) або в графі «Умови для дострокового призначення трудової пенсії. Підстава (код)» одного з кодів: 27-11-ГР, 27-11-ВП, 27-12, 27-СП, САМОЛЕТ, СПЕЦАВ, СПАСАВ, УЧЛЕТ,  ВЫСШПИЛ, НОРМАПР, НОРМСП, РЕАКТИВН, ЛЕТРАБ, 27-14, 27-15'");
...
Функция ТекстОшибкиВредногоЗаработка(НаименованиеСотрудника, Месяц = Неопределено, КлассУсловийТруда = Неопределено, ТекcтОшибки)
	Если Месяц = Неопределено Тогда
		ОписаниеМесяца = "";	
	Иначе	
		ОписаниеМесяца = ?(Месяц = 0, НСтр("ru='- в показателе С начала года ';uk='- у показнику З початку року'"), НСтр("ru='- за ';uk='- за'") + Месяц + "ru = '-й месяц '");	
	КонецЕсли;	
	...
	Иначе
		ОписаниеКлассаУсловийТруда = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru='по классу условий труда %1 ';uk='по класу умов праці %1'"),
										КлассУсловийТруда);
		
	КонецЕсли;	
	
	ШаблонТекстаОшибки = НСтр("ru='Сотрудник %1: в разделе 6.7 (Сведения о выплатах занятых на работах дающих право на досрочную пенсию) %2%3%4';uk='Співробітник %1: у розділі 6.7 (Відомості про виплати зайнятих на роботах, що дають право на дострокову пенсію) %2%3%4'");
