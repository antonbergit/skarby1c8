...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)	
	ЗаписиОСтажеТекст = НСтр("ru='Записи о стаже';uk='Записи про стаж'");
...
Процедура ДанныеТекущегоДокументаПоСотрудникуВДанныеФормы(АдресВоВременномХранилище)
	...
	Если ДанныеТекущейСтрокиПоСотруднику = Неопределено  Тогда
		...
		ВызватьИсключение НСтр("ru='В текущем документе не найдены данные по редактируемому сотруднику.';uk='У поточному документі не знайдені дані по співробітникові, що редагується.'");
	КонецЕсли;
...
Процедура ПредупреждениеОНедопустимомОтчетномПериоде()
	Текст = НСтр("ru='Форма предоставляется, начиная с 2014 г. За предыдущие периоды предоставляется формы СЗВ-1(2, 4).';uk='Форма надається, починаючи з 2014 р. За попередні періоди надаються форми СЗВ-1(2, 4).'");
