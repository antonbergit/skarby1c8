...
Процедура СтатусГИСМОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru='Маркировка товаров ГИСМ была изменена. Записать?';uk='Маркировка товаров ГИСМ была изменена. Записать?'");
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения("СтатусГИСМОбработкаНавигационнойСсылкиЗавершение",
		                                                    ЭтотОбъект,
		                                                    Новый Структура("НавигационнаяСсылкаФорматированнойСтроки", НавигационнаяСсылкаФорматированнойСтроки));
		ТекстВопроса = НСтр("ru='Маркировка товаров ГИСМ не записана. Записать?';uk='Маркировка товаров ГИСМ не записана. Записать?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	//Серийные номера
	РаботаССерийнымиНомерамиКлиентСервер.УдалитьСерийныеНомераПоКлючуСвязи(Объект.Серии, ТекущаяСтрока,,ИспользоватьСерийныеНомераОстатки);
	РаботаССерийнымиНомерамиКлиентСервер.УдалитьСерийныеНомераПоКлючуСвязи(Объект.СерииКиЗ, ТекущаяСтрока,,ИспользоватьСерийныеНомераОстатки);
...
Процедура ОткрытьПодборСерийныеНомера()
	...
	Если НЕ ЗначениеЗаполнено(Элементы.Товары.ТекущиеДанные.GTIN) Тогда
		ПоказатьПредупреждение(,НСтр("ru='Перед выбором серий укажите GTIN';uk='Перед выбором серий укажите GTIN'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Функция ПолучитьСерийныеНомераИзХранилища(АдресВоВременномХранилище, КлючСтроки)
	...
	ИзмененоКоличество = РаботаСФормойДокумента.ПолучитьСерийныеНомераИзХранилища(Объект, АдресВоВременномХранилище, КлючСтроки, "Товары", "Серии");;
