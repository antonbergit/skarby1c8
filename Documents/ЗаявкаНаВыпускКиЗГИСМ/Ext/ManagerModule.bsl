...
Функция ПодтверждениеПоступленияТоваровПоЗаявкеНаВыпускКиЗXML(ДокументСсылка)
	...
	Пока ВыборкаПоДокументамПоступления.Следующий() Цикл
		...
		Иначе
			ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru='Не заполнен номер ГИСМ документа %1.';uk='Не заполнен номер ГИСМ документа %1.'"), ДокументСсылка));
		КонецЕсли;
		...
		Исключение
			СообщениеXML.ТекстОшибки = СтрШаблон(НСтр("ru='В документе %1 не заполнен номер входящего документа.';uk='В документе %1 не заполнен номер входящего документа.'"), ВыборкаПоДокументамПоступления.ДокументПоступления);
		КонецПопытки;
		
		Попытка
			ПодтверждениеПоступления.commit_doc_date = ВыборкаПоДокументамПоступления.ДатаВходящегоДокумента;
		Исключение
			СообщениеXML.ТекстОшибки = СтрШаблон(НСтр("ru='В документе %1 не заполнена дата входящего документа.';uk='В документе %1 не заполнена дата входящего документа.'"), , ВыборкаПоДокументамПоступления.ДокументПоступления);
		КонецПопытки;
...
Функция ЗакрытиеЗаявкиНаВыпускКиЗXML(ДокументСсылка)
	...
	ОписаниеОперации = СтрШаблон(НСтр("ru='Закрытие документа %1';uk='Закрытие документа %1'"), ДокументСсылка);
...
Функция ЗаявкаНаВыпускКиЗXML2_40(ДокументСсылка)
	...
	Исключение
		СообщениеXML.ТекстОшибки = НСтр("ru='Не заполнен номер договора с эмитентом.';uk='Не заполнен номер договора с эмитентом.'");
	КонецПопытки;
	
	Попытка
		ЗаявкаНаВыпускКиЗ.contract_date = Шапка.ДатаДоговора;
	Исключение
		СообщениеXML.ТекстОшибки = НСтр("ru='Не заполнена дата договора с эмитентом.';uk='Не заполнена дата договора с эмитентом.'");
	КонецПопытки;
