	...
	Если ВыпущенныеКиЗ.Количество() > 0 Тогда
		...
		Для Каждого СтрокаТЧ Из ВыпущенныеКиЗ Цикл
			Если НЕ ЗначениеЗаполнено(СтрокаТЧ.ДокументПоступления) 
				...
				ТекстОшибки = НСтр("ru='Не указан документ поступления в строке %НомерСтроки% списка ""Выпущенные КиЗ""';uk='Не указан документ поступления в строке %НомерСтроки% списка ""Выпущенные КиЗ""'");
		...
		Пока ВыборкаНомераКиЗ.Следующий() Цикл
			...
			Если Не ТекущийДокументПоступленияПравильный Тогда
				...
				ТекстОшибки = НСтр("ru='Указан некорректный документ поступления в строке %НомерСтроки% списка ""Выпущенные КиЗ""';uk='Указан некорректный документ поступления в строке %НомерСтроки% списка ""Выпущенные КиЗ""'");
				ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", НомерСтроки);
