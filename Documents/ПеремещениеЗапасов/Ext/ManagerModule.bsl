	 ...
	 ИЛИ ВозвратИзРозницыСуммовойУчет Тогда
		...
		Если СуммаНаценка <> 0 Тогда
			...
			Если СтрокаТаблицы <> Неопределено Тогда
				...
				СтрокаТаблицаУправленческий.Содержание = НСтр("ru='Торговая наценка';uk='Торгова націнка'");
...
Процедура СформироватьТаблицаСуммовойУчетВРознице(ДокументСсылкаПеремещениеЗапасов, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПеремещениеВРозницу", НСтр("ru='Перемещение в розницу';uk='Переміщення в роздріб'"));
	Запрос.УстановитьПараметр("ПеремещениеВРознице", НСтр("ru='Перемещение в рознице';uk='Переміщення в роздробі'"));
	Запрос.УстановитьПараметр("ВозвратИзРозницы", НСтр("ru='Возврат из розницы';uk='Повернення з роздробу'"));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ru='Курсовая разница';uk='Курсова різниця'"));
	...
	Для каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		Если ИмяМакета = "ПеремещениеЗапасов" Тогда
			...
			ОбластьМакета.Параметры.ТекстЗаголовка = 
				НСтр("ru='Перемещение запасов № ';uk='Переміщення запасів № '", КодЯзыкаПечать)
			  + НомерДокумента
		...
		ИначеЕсли ИмяМакета = "М11" Тогда
			...
			ОбластьМакета.Параметры.Заголовок = 
				НСтр("ru='ТРЕБОВАНИЕ-НАКЛАДНАЯ № ';uk='ВИМОГА-НАКЛАДНА №'", КодЯзыкаПечать)
			  + НомерДокумента
			  + НСтр("ru=' от ';uk=' від '", КодЯзыкаПечать)
			  + Формат(Шапка.ДатаДокумента, "Л="+КодЯзыкаПечать+"; ДЛФ=DD");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='ТОРГ13 (Накладная на внутреннее перемещение)';uk='ТОРГ13 (Накладна на внутрішнє переміщення)'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати.Представление = НСтр("ru='Перемещение запасов';uk='Переміщення запасів'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати.Представление = НСтр("ru='Бланк товарного наполнения (Отправитель)';uk='Бланк товарного наповнення (Відправник)'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати.Представление = НСтр("ru='Бланк товарного наполнения (Получатель)';uk='Бланк товарного наповнення (Одержувач)'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
