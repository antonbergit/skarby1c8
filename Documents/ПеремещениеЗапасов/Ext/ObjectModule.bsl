	...
	Если Константы.ФункциональнаяОпцияРезервированиеЗапасов.Получить() 
		...
		Для каждого СтрокаЗапасы Из Запасы Цикл
			...
			Если СтрокаЗапасы.Резерв > СтрокаЗапасы.Количество Тогда
				...
				ТекстСообщения = НСтр("ru='В строке №%Номер% табл. части ""Запасы"" количество передаваемых в резерв позиций превышает общее количество запасов.';uk='У рядку №%Номер% табл. частини ""Запаси"" кількість переданих у резерв позицій перевищує загальну кількість запасів.'");
...
Процедура ЗаполнитьПоЗаказуНаПроизводство(ДанныеЗаполнения) Экспорт
	...
	Если ДанныеЗаполненияВидОперации = Перечисления.ВидыОперацийЗаказНаПроизводство.Сборка Тогда
		...
		гвЗаполнитьПоЗаказуНаПроизводство(ДанныеЗаполнения);
		// ...Гунько 2021-02-28
	КонецЕсли;
...
// Гунько 2021-02-28
Процедура гвЗаполнитьПоЗаказуНаПроизводство(ДанныеЗаполнения)
	...
	|		Ссылка.ЗаказПокупателя КАК ЗаказПокупателя
	// Гунько 2021-02-28
	|	,гвСерийныйНомер КАК СерийныйНомер,
	|	НомерСтроки КАК НомерСтроки,
	|	гвСерийныйНомер.Наименование КАК СерийныйНомерНаименование
	// ...Гунько 2021-02-28
	|	)
	|ИЗ
	|	Документ.ЗаказНаПроизводство КАК ЗаказНаПроизводство
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаПродукция.Следующий() Цикл
			...
			// Гунько 2021-02-28
			НоваяСтрока.СерийныеНомера = ВыборкаПродукция.СерийныйНомерНаименование;
			НоваяСтрока.КлючСвязи = ВыборкаПродукция.НомерСтроки;
			
			лНоваяСтрокаСерийныеНомера = СерийныеНомера.Добавить();
			лНоваяСтрокаСерийныеНомера.СерийныйНомер = ВыборкаПродукция.СерийныйНомер;
			лНоваяСтрокаСерийныеНомера.КлючСвязи = ВыборкаПродукция.НомерСтроки;			
			// ...Гунько 2021-02-28
