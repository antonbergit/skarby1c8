...
Процедура ЗаполнитьСписокДокументОснование()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодсистемуПроизводство") Тогда
		СписокДокументОснование.Добавить("Документ.ЗаказНаПроизводство.ФормаВыбора", НСтр("ru='Заказ на производство';uk='Замовлення на виробництво'"));
	...
	Если ПолучитьФункциональнуюОпцию("УчетПоЯчейкам") Тогда
		СписокДокументОснование.Добавить("Документ.ПеремещениеПоЯчейкам.ФормаВыбора", НСтр("ru='Перемещение по ячейкам';uk='Переміщення по комірках'"));
	КонецЕсли;
...
Процедура ЗаполнитьПоОстаткамНаСкладе(Команда)
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоОстаткамНаСкладеЗавершение", ЭтотОбъект), НСтр("ru='Табличная часть будет очищена! Продолжить выполнение операции?';uk='Таблична частина буде очищена! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
        Возврат; 
	КонецЕсли;
...
Процедура ИзменитьРезервЗаполнитьПоРезервам(Команда)
	...
	Если Объект.Запасы.Количество() = 0 Тогда
		...
		Сообщение.Текст = НСтр("ru='Табличная часть ""Запасы"" не заполнена!';uk='Таблична частина ""Запаси"" не заповнена!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Процедура УстановитьВидимостьОтФОИспользоватьПодсистемуПроизводство()
	...
	Если Константы.ФункциональнаяОпцияИспользоватьПодсистемуПроизводство.Получить()
		...
		Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийПеремещениеЗапасов.Перемещение, НСтр("ru='Перемещение';uk='Переміщення'"));
	...
	Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийПеремещениеЗапасов.СписаниеНаРасходы, НСтр("ru='Списание на расходы';uk='Списання на витрати'"));
	Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийПеремещениеЗапасов.ПередачаВЭксплуатацию, НСтр("ru='Передача в эксплуатацию';uk='Передача в експлуатацію'"));
	Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийПеремещениеЗапасов.ВозвратИзЭксплуатации, НСтр("ru='Возврат из эксплуатации';uk='Повернення з експлуатації'"));
...
// Гунько 2021-02-25
&НаСервере
Процедура гвОбновитьСписокОпераций(МассивНоменклатуры = Неопределено, СерийныйНомерМассив = Неопределено)	
	гвСписокОперацийПоСерийномуНомеру.Параметры.УстановитьЗначениеПараметра("НоменклатураМассив", МассивНоменклатуры);
	гвСписокОперацийПоСерийномуНомеру.Параметры.УстановитьЗначениеПараметра("СерийныйНомерМассив", СерийныйНомерМассив);	
КонецПроцедуры
...
Процедура ЗапасыПриАктивизацииСтроки(Элемент)
	...
	Если НЕ ТекущиеДанные = Неопределено Тогда
		лМассивСтрокСерийныеНомера = Объект.СерийныеНомера.НайтиСтроки(Новый Структура("КлючСвязи", ТекущиеДанные.КлючСвязи));
		лМассивНоменклатуры = Новый Массив;
		лМассивНоменклатуры.Добавить(ТекущиеДанные.Номенклатура);
		лСерийныйНомерМассив = Новый Массив;
		Для Каждого Строка Из лМассивСтрокСерийныеНомера Цикл			
			лСерийныйНомерМассив.Добавить(Строка.СерийныйНомер);
		КонецЦикла;	
		гвОбновитьСписокОпераций(лМассивНоменклатуры, лСерийныйНомерМассив);
	КонецЕсли;
КонецПроцедуры
// ...Гунько 2021-02-25
