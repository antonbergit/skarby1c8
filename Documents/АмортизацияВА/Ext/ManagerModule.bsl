...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаАмортизацияВА, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Сообщение1",	   НСтр("ru='амортизация в этом месяце уже начислялась!';uk='амортизація в цьому місяці вже нараховувалася!'"));
	Запрос.УстановитьПараметр("Сообщение2",	   НСтр("ru='не указан способ начисления амортизации!';uk='не зазначений спосіб нарахування амортизації!'"));
	Запрос.УстановитьПараметр("Сообщение3",	   НСтр("ru='стоимость равна 0!';uk='вартість рівна 0!'"));
	Запрос.УстановитьПараметр("Сообщение4",	   НСтр("ru='срок использования равен 0!';uk='Термін використання рівний 0!'"));
	Запрос.УстановитьПараметр("Сообщение5",	   НСтр("ru='объем продукции работ для вычисления амортизации не заполнен!';uk='обсяг продукції робіт для обчислення амортизації не заповнений!'"));
	Запрос.УстановитьПараметр("Сообщение6",	   НСтр("ru='первоначальная стоимость равна 0!';uk='первісна вартість рівна 0!'"));
...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаАмортизацияВА, СтруктураДополнительныеСвойства)
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("НачислениеАмортизации", НСтр("ru='Начисление амортизации';uk='Нарахування амортизації'"));
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
...
Процедура СформироватьТаблицаОшибкиЗакрытияМесяца(ДокументСсылкаАмортизацияВА, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("НачислениеАмортизации", НСтр("ru='Начисление амортизации';uk='Нарахування амортизації'"));
	Запрос.УстановитьПараметр("АмортизацияРавнаНулю", НСтр("ru='расчитанная амортизация равна 0!';uk='розрахована амортизація рівна 0!'"));
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	...
	Для каждого ТекСтрока Из ТаблицаРезультата Цикл
		ТекстОшибки = НСтр("ru='Для имущества %ВнеоборотныйАктивПредставление% (%Код%) %СообщениеОбОшибкеНачисленияАмортизации%.';uk='Для майна %ВнеоборотныйАктивПредставление% (%Код%) %СообщениеОбОшибкеНачисленияАмортизации%.'");
