	...
	Для каждого СтрокаНалоги Из Налоги Цикл
		...
		Если Константы.ФункциональнаяОпцияУчетПоНесколькимПодразделениям.Получить()
		 ...
		 И НЕ ЗначениеЗаполнено(СтрокаНалоги.Подразделение) Тогда
			ТекстСообщения = НСтр("ru='Для счета затрат ""%Корреспонденция%"" указанного в строке %НомерСтроки% списка ""Налоги"", должен быть заполнен реквизит ""Подразделение"".';uk='Для рахунку витрат ""%Корреспонденция%"" зазначеного в рядку %НомерСтроки% списку ""Податки"", повинен бути заповнений реквізит ""Підрозділ"".'"
			);
