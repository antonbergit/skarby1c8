...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.Контрагент) Тогда
		...
		Если Параметры.ПоказыватьОтбор Тогда
			Заголовок = СтрШаблон(
				НСтр("ru='Справки 1СПАРК Риски: %1';uk='Справки 1СПАРК Риски: %1'"),
	...
	// При работе в модели сервиса переход в личный кабинет не выполняется.
	Элементы.ВсеПолученныеСправки.Видимость = Не ОбщегоНазначенияПовтИсп.РазделениеВключено();
...
Процедура ЗапроситьСправку(Команда)
	...
	Если Не ЗначениеЗаполнено(Контрагент) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Поле ""Контрагент"" не заполнено.';uk='Поле ""Контрагент"" не заполнено.'"),
			,
			"Контрагент");
		Возврат;
	КонецЕсли;
...
Процедура ВсеПолученныеСправки(Команда)
	...
	ИнтернетПоддержкаПользователейКлиент.ОткрытьВебСтраницу(
		...
		НСтр("ru='Справки 1СПАРК Риски';uk='Справки 1СПАРК Риски'"));
...
Процедура ОткрытьСправкуСПАРКРиски(Результат, СправкаСсылка) Экспорт
	...
	Состояние(, , НСтр("ru='Открытие справки 1СПАРК Риски...';uk='Открытие справки 1СПАРК Риски...'"));
...
Процедура ЗапроситьНовуюСправку(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Состояние(, , НСтр("ru='Запрос новой справки 1СПАРК Риски';uk='Запрос новой справки 1СПАРК Риски'"));
	...
	Если Не РезультатЗапросаСправки.СправкаСсылка.Пустая() Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ru='Справки 1СПАРК Риски';uk='Справки 1СПАРК Риски'"),
			,
			НСтр("ru='Подготовка новой справки 1СПАРК Риски.';uk='Подготовка новой справки 1СПАРК Риски.'"),
...
"Повторите попытку подключения позже.';uk='Не удалось подключиться к сервису 1СПАРК Риски. Сервис временно недоступен."
"Повторите попытку подключения позже.'"));
	...
	ИначеЕсли РезультатДействия.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ИспользованиеЗапрещено") Тогда
		...
		ПоказатьПредупреждение(
			,
			НСтр("ru='Использование сервиса 1СПАРК Риски недоступно в текущем режиме работы.';uk='Использование сервиса 1СПАРК Риски недоступно в текущем режиме работы.'"));
...
"Подключить?';uk='Для работы с сервисом 1СПАРК Риски необходимо подключить Интернет-поддержку пользователей."
"Подключить?'"),
...
"ПриОпределенииСвойствСправочниковКонтрагентов() общего модуля СПАРКРискиПереопределяемый.';uk='Неизвестный тип контрагента (%1). Необходимо заполнить описание справочника в методе"
"ПриОпределенииСвойствСправочниковКонтрагентов() общего модуля СПАРКРискиПереопределяемый.'"),
	...
	Если ЗначенияРеквизитов.Количество() = 0 Тогда
		...
		Результат.Сообщение = НСтр("ru='Не удалось получить данные контрагента. Данные контрагента были удалены или недостаточно прав.';uk='Не удалось получить данные контрагента. Данные контрагента были удалены или недостаточно прав.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если Не ДанныеКонтрагента.ПодлежитПроверке Тогда
		...
		Результат.Сообщение = НСтр("ru='Выбранный контрагент не может быть проверен в сервисе 1СПАРК Риски.';uk='Выбранный контрагент не может быть проверен в сервисе 1СПАРК Риски.'");
		Возврат Результат;
...
"Сервис не возвратил идентификатор справки.';uk='Ошибка при запросе новой справки для ИНН %1."
...
"недостаточно прав для просмотра.';uk='Выбранная справка отсутствует в информационной базе или"
...
"Повторите попытку позже.';uk='На текущий момент подготовка справки не завершена."
...
"в связи с ошибкой подготовки справки в сервисе 1СПАРК Риски.';uk='Справка не может быть открыта для просмотра"
...
Процедура ЗагрузитьФайлСправки(ДокументСсылка, ИдентификаторСправки, ИНН, ФайлСсылка, Результат)
	...
	ПараметрыФайла.Вставить("ИмяБезРасширения",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Справка 1СПАРК Риски %1';uk='Справка 1СПАРК Риски %1'"),
			ИНН));
...
"%2';uk='Ошибка при подготовке справки с идентификатором %1."
"%2'"),
...
"%1"
""
"Обратитесь к администратору.';uk='Не удалось подключиться к сервису 1СПАРК Риски."
"%1"
""
...
"Обратитесь к администратору.';uk='Для работы с сервисом 1СПАРК Риски необходимо подключить Интернет-поддержку пользователей."
"Обратитесь к администратору.'");
...
"<br/><a href=""%1"">Подробнее о сервисе</a></body>';uk='<body>Сервис 1СПАРК Риски не подключен."
