...
Процедура УстановитьСведенияДействующегоСогласия()
	...
	Если Согласие = Неопределено Тогда
		...
		Элементы.СведенияТекст.Заголовок = НСтр("ru='Не обнаружено действующего согласия на обработку персональных данных субъекта.';uk='Не виявленл діючої згоди на обробку персональних даних суб`єкта.'");
		Возврат;
	КонецЕсли;
	...
	ЧастиСообщения.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='У субъекта %1 было получено';uk='У суб`єкта %1 було отримане'"), Формат(Согласие.ДатаПолучения, "ДЛФ=D")) + " ");
	ЧастиСообщения.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='согласие';uk='згода'"), , , , ПолучитьНавигационнуюСсылку(Согласие.ДокументОснование)));
	Если ЗначениеЗаполнено(Согласие.СрокДействия) Тогда
		ЧастиСообщения.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru=', которое действует до %1.';uk=', яке діє до %1.'"), Формат(Согласие.СрокДействия, "ДЛФ=D")));
	Иначе	
		ЧастиСообщения.Добавить(НСтр("ru=', которое действует бессрочно.';uk=', яке діє безстроково.'"));
	КонецЕсли;
