...
Процедура ЗаполнитьПоРасходДСПлан(ДокументСсылкаРасходДСПлан, Сумма = Неопределено) Экспорт
	...
	Если ДокументСсылкаРасходДСПлан.СтатусУтвержденияПлатежа = Перечисления.СтатусыУтвержденияПлатежей.НеУтвержден Тогда
		ВызватьИсключение НСтр("ru='Нельзя ввести расход денег на основании неутвержденной заявки!';uk='Не можна ввести витрату грошей на підставі незатвердженої заявки!'");
	КонецЕсли;
	Если ДокументСсылкаРасходДСПлан.ТипДенежныхСредств = Перечисления.ТипыДенежныхСредств.Наличные Тогда
		ВызватьИсключение НСтр("ru='Нельзя ввести расход денег. В заявке указан не верный способ оплаты (тип денежных средств)!';uk='Не можна ввести витрату грошей. У заявці зазначений не вірний спосіб оплати (тип коштів)!'");
	КонецЕсли;
...
Процедура ЗаполнитьПоПеремещениеДСПлан(ДокументСсылкаПеремещениеДСПлан, Сумма = Неопределено) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.Прочее) КАК ВидОперации,
	|	&Ссылка КАК ДокументОснование,
	|	ТаблицаДокумента.Организация КАК Организация,
...
Процедура ЗаполнитьПоНачислениеНалогов(ДокументСсылкаНачислениеНалогов) Экспорт
	...
	Если ДокументСсылкаНачислениеНалогов.ВидОперации <> Перечисления.ВидыОперацийНачислениеНалогов.Начисление Тогда
		ВызватьИсключение НСтр("ru='Расход со счета можно ввести только на основании начисления налогов, а не возмещения.';uk='Витрату з рахунку можна ввести тільки на підставі нарахування податків, а не відшкодування.'");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Зарплата Тогда
		...
		Для Каждого СтрокаВедомости Из ВыплатаЗаработнойПлаты Цикл
			Если СтрокаВедомости.Ведомость.ВалютаДокумента <> ВалютаДенежныхСредств Тогда
				ТекстСообщения = НСтр("ru='Валюта документа: %ВалютаДенежныхСредств%, не соответствует валюте платежной ведомости: %ВалютаВедомости%!';uk='Валюта документа: %ВалютаДенежныхСредств%, не відповідає валюті платіжної відомості: %ВалютаВедомости%!'");
	...
	Если ПроверитьДокументыПланирования Тогда
		Если ПолучитьФункциональнуюОпцию("ПлатежныйКалендарь") Тогда
			...
			ЕстьОшибка = Ложь;
			СуммаПлатежа = РасшифровкаПлатежа.Итог("СуммаПлатежа");
			СуммаРасчетов = РасшифровкаПлатежа.Итог("СуммаРасчетов");
			СуммаДляСообщения = 0;
			
			Если ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций") И СуммаПлатежа > СуммаДокумента Тогда
				СуммаДляСообщения = СуммаПлатежа;
				ЕстьОшибка = Истина;
			ИначеЕсли НЕ ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций") И СуммаРасчетов > СуммаДокумента Тогда
				СуммаДляСообщения = СуммаРасчетов;
				ЕстьОшибка = Истина;
			КонецЕсли;
			
			Если ЕстьОшибка Тогда
				ТекстСообщения = НСтр("ru='Сумма документа: %СуммаДокумента% %ВалютаДенежныхСредств% меньше суммы разнесенных платежей в табличной части: %СуммаПлатежа% %ВалютаДенежныхСредств%!';uk='Сума документу: %СуммаДокумента% %ВалютаДенежныхСредств% менше суми рознесених платежів в табличній частині: %СуммаПлатежа% %ВалютаДенежныхСредств%!'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаДокумента%", Строка(СуммаДокумента));
		...
		Если Выборка.Следующий() Тогда
			Если НЕ УправлениеНебольшойФирмойСервер.РазрешенаПродажаНоменклатурыПоДоговоруОбслуживания(Выборка.Ссылка, Корреспонденция) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Запрещено проводить незапланированные расходы по текущему договору обслуживания!';uk='Заборонено проводити незаплановані витрати за поточним договором обслуговування!'"),,
					"Корреспонденция",,
					Отказ
				);
			КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Сотрудники.ОтразитьУчетЕСВ(ДополнительныеСвойства, Движения, Отказ);
	Сотрудники.ОтразитьДвиженияПоВзаиморасчетыПоВзносамВФонды(ДополнительныеСвойства, Движения, Отказ);
