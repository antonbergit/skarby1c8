...
Процедура СформироватьТаблицаДенежныеСредства(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("РасходДенежныхСредств", НСтр("ru='Расход денежных средств';uk='Витрата коштів'"));
	...
	|	И (ТаблицаДокумента.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.Подотчетнику)
	|			ИЛИ ТаблицаДокумента.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.Прочее)
	...
	|			ИЛИ ТаблицаДокумента.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.Налоги))
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура СформироватьТаблицаРасчетыСПодотчетниками(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("ВозникновениеДолгаПодотчетника", НСтр("ru='Возникновение долга подотчетника';uk='Виникнення боргу підзвітника'"));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ru='Курсовая разница';uk='Курсова різниця'"));
	Запрос.УстановитьПараметр("ВалютаУчета",	Константы.ВалютаУчета.Получить());
...
Процедура СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("ПогашениеАвансаПокупателя", НСтр("ru='Погашение аванса покупателя';uk='Погашення авансу покупця'"));
...
Процедура СформироватьТаблицаРасчетыСПерсоналом(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("ПогашениеОбязательствПередПерсоналом", НСтр("ru='Погашение обязательств перед персоналом';uk='Погашення зобов`язань перед персоналом'"));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ru='Курсовая разница';uk='Курсова різниця'"));
	Запрос.УстановитьПараметр("ВалютаУчета",	Константы.ВалютаУчета.Получить());
...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("ОтражениеРасходов", НСтр("ru='Отражение расходов';uk='Відображення витрат'"));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ru='Курсовая разница';uk='Курсова різниця'"));
	Запрос.УстановитьПараметр("ВалютаУчета",	Константы.ВалютаУчета.Получить());
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ru='Курсовая разница';uk='Курсова різниця'"));
	Запрос.УстановитьПараметр("Содержание", НСтр("ru='Списание денежных средств на произвольный счет';uk='Списання коштів на довільний рахунок'"));
	Запрос.УстановитьПараметр("ОплатаНалога", НСтр("ru='Оплата налога';uk='Оплата податку'"));
	Запрос.УстановитьПараметр("СодержаниеОплатаОтБанкаЭквайрера", НСтр("ru='Оплата от банка-эквайрера';uk='Оплата від банку-еквайрера'"));
	Запрос.УстановитьПараметр("СодержаниеКомиссияБанкаЭквайрера", НСтр("ru='Комиссия банка-эквайрера';uk='Комісія банку-еквайрера'"));
	Запрос.УстановитьПараметр("ВалютаУчета",	Константы.ВалютаУчета.Получить());
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаВзаиморасчетыПоВзносамВФонды(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства)
	...
	"ВЫБРАТЬ
	|	&Организация КАК Организация,
	|	ВзаиморасчетыПоВзносамВФонды.Период,
	|	ВзаиморасчетыПоВзносамВФонды.ВидДвижения,
	|	ВзаиморасчетыПоВзносамВФонды.ПериодРегистрации,
	|	ВзаиморасчетыПоВзносамВФонды.Налог,
	|	ВзаиморасчетыПоВзносамВФонды.Ставка,
	|	ВзаиморасчетыПоВзносамВФонды.СуммаДокумента
	|ИЗ
	|	ВТВзаиморасчетыПоВзносамВФонды КАК ВзаиморасчетыПоВзносамВФонды";
	...
	ТаблицаВзносы.Колонки.Добавить("ВидДвижения",Новый ОписаниеТипов("ВидДвиженияНакопления"));
	ТаблицаВзносы.Колонки.Добавить("НалоговыйПериод",Новый ОписаниеТипов("Дата"));
	ТаблицаВзносы.Колонки.Добавить("Налог",Новый ОписаниеТипов("СправочникСсылка.ВидыНачисленийИУдержаний"));
	ТаблицаВзносы.Колонки.Добавить("Ставка",Новый ОписаниеТипов("Число"),Новый КвалификаторыЧисла(15,2));
	ТаблицаВзносы.Колонки.Добавить("База",Новый ОписаниеТипов("Число"),Новый КвалификаторыЧисла(15,2));
	ТаблицаВзносы.Колонки.Добавить("Результат",Новый ОписаниеТипов("Число"),Новый КвалификаторыЧисла(15,2));
	ТаблицаВзносы.Колонки.Добавить("Предел",Новый ОписаниеТипов("Число"),Новый КвалификаторыЧисла(15,2));
	
	Если ВыборкаВзносы.Следующий() Тогда
		...
		|	СУММА(ВзаиморасчетыПоВзносамВФонды.База) КАК База,
		|	СУММА(ВзаиморасчетыПоВзносамВФонды.Результат) КАК Результат,
		|	СУММА(ВзаиморасчетыПоВзносамВФонды.Предел) КАК Предел
		|ИЗ
		|	РегистрНакопления.ВзаиморасчетыПоВзносамВФонды КАК ВзаиморасчетыПоВзносамВФонды
		|ГДЕ
		|	ВзаиморасчетыПоВзносамВФонды.НалоговыйПериод <= &Дата
		|	И ВзаиморасчетыПоВзносамВФонды.НалоговыйПериод = &НалоговыйПериод
		|	И ВзаиморасчетыПоВзносамВФонды.Налог = &Налог
		|	И ВзаиморасчетыПоВзносамВФонды.Ставка = &Ставка
		...
		|	ВзаиморасчетыПоВзносамВФонды.Налог,
		|	ВзаиморасчетыПоВзносамВФонды.Ставка";
		ЗапросКРегистру.УстановитьПараметр("Дата", ВыборкаВзносы.Период);
		ЗапросКРегистру.УстановитьПараметр("НалоговыйПериод", ВыборкаВзносы.ПериодРегистрации);
		ЗапросКРегистру.УстановитьПараметр("Налог", ВыборкаВзносы.Налог);
		ЗапросКРегистру.УстановитьПараметр("Ставка", ВыборкаВзносы.Ставка);
		ЗапросКРегистру.УстановитьПараметр("Организация", ВыборкаВзносы.Организация);
		
		тзЗапроса = ЗапросКРегистру.Выполнить().Выгрузить();
		
		// списание
		Если тзЗапроса.Количество() > 0 Тогда
			СуммаРезультат = тзЗапроса[0].Результат;
			Если СуммаРезультат <> 0 Тогда
				Если СуммаРезультат > ВыборкаВзносы.СуммаДокумента Тогда
					// если сумма в регистре больше, чем надо списать, спишем другие ресурсы пропорционально ресурсу "Результат"
					Коэфициент = ВыборкаВзносы.СуммаДокумента / СуммаРезультат;
				Иначе
					// спишем из других ресурсов все
					Коэфициент = 1;
				КонецЕсли;
			Иначе
				Коэфициент = 1;
			КонецЕсли;
			СуммаБаза = тзЗапроса[0].База * Коэфициент;
			СуммаПредел = тзЗапроса[0].Предел * Коэфициент;
		Иначе
			СуммаБаза = 0;
			СуммаПредел = 0;
		КонецЕсли;
		...
		НоваяСтрока.Период = ВыборкаВзносы.Период;
		НоваяСтрока.Организация = ВыборкаВзносы.Организация;
		НоваяСтрока.ВидДвижения = ВыборкаВзносы.ВидДвижения;
		НоваяСтрока.НалоговыйПериод = ВыборкаВзносы.ПериодРегистрации;
		НоваяСтрока.Налог = ВыборкаВзносы.Налог;
		НоваяСтрока.Ставка = ВыборкаВзносы.Ставка; 
		НоваяСтрока.База = СуммаБаза;
		НоваяСтрока.Результат = ВыборкаВзносы.СуммаДокумента;
		НоваяСтрока.Предел = СуммаПредел;
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаУчетЕСВ(ДокументСсылкаРасходИзКассы, СтруктураДополнительныеСвойства)
	...
	|	УчетЕСВ.Налог,
	|	УчетЕСВ.Процент,
	|	УчетЕСВ.СуммаДляНачисления,
	|	УчетЕСВ.СуммаЕСВ
	|ИЗ
	|	ВТУчетЕСВ КАК УчетЕСВ";
	
	ТаблицаПоЕСВ = Запрос.Выполнить().Выгрузить();
	
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаУчетЕСВ", ТаблицаПоЕСВ);
	
КонецПроцедуры // СформироватьТаблицаУчетЕСВ()
...
Процедура СформироватьТаблицаКнигаДоходовРасходовПоЕН(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	|	ВременнаяТаблицаРасшифровкаПлатежа.СтатьяДекларацииПоЕдиномуНалогу КАК Статья,
	|	СУММА(ВременнаяТаблицаРасшифровкаПлатежа.СуммаУчета) КАК Сумма
	|ИЗ
	|	ВременнаяТаблицаРасшифровкаПлатежа КАК ВременнаяТаблицаРасшифровкаПлатежа
	|ГДЕ
	|	ВременнаяТаблицаРасшифровкаПлатежа.СтатьяДекларацииПоЕдиномуНалогу <> ЗНАЧЕНИЕ(Справочник.СтатьиНалоговыхДеклараций.ПустаяСсылка)
	|
	|СГРУППИРОВАТЬ ПО
	|	ВременнаяТаблицаРасшифровкаПлатежа.Дата,
	|	ВременнаяТаблицаРасшифровкаПлатежа.СтатьяДекларацииПоЕдиномуНалогу";
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ТаблицаДокумента.СтатьяДекларацииПоЕдиномуНалогу КАК СтатьяДекларацииПоЕдиномуНалогу
	...
	|			ТОГДА ""Оплата комиссии по кредиту""
	|	КОНЕЦ,
	|	ТаблицаДокумента.СтатьяДекларацииПоЕдиномуНалогу,
	...
	|	ТаблицаДокумента.НалогЕСВ КАК НалогЕСВ,
	|	ТаблицаДокумента.СтавкаЕСВ КАК СтавкаЕСВ,
	|	ТаблицаДокумента.СуммаДляНачисленияЕСВ КАК СуммаДляНачисленияЕСВ,
	|	ТаблицаДокумента.ПеречислениеЕдиногоНалога КАК ПеречислениеЕдиногоНалога,
	...
	|	РасходСоСчета.НалогЕСВ КАК Налог,
	|	РасходСоСчета.СтавкаЕСВ КАК Процент,
	|	РасходСоСчета.СуммаДляНачисленияЕСВ КАК СуммаДляНачисления,
	|	РасходСоСчета.ПеречислениеЕдиногоНалога,
	|	РасходСоСчета.Сумма КАК СуммаЕСВ
	|ПОМЕСТИТЬ ВТУчетЕСВ
	|ИЗ
	|	ВременнаяТаблицаШапка КАК РасходСоСчета
	|ГДЕ
	|	РасходСоСчета.Сумма <> 0
	|	И РасходСоСчета.НалогЕСВ = ЗНАЧЕНИЕ(Справочник.ВидыНачисленийИУдержаний.ЕСВ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РасходСоСчета.Дата КАК Период,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	|	НАЧАЛОПЕРИОДА(РасходСоСчета.Дата, МЕСЯЦ) КАК ПериодРегистрации,
	|	РасходСоСчета.НалогЕСВ КАК Налог,
	|	РасходСоСчета.СтавкаЕСВ КАК Ставка,
	|	РасходСоСчета.Сумма КАК СуммаДокумента
	|ПОМЕСТИТЬ ВТВзаиморасчетыПоВзносамВФонды
	|ИЗ
	|	ВременнаяТаблицаШапка КАК РасходСоСчета
	|ГДЕ
	|	РасходСоСчета.Сумма <> 0
	|	И РасходСоСчета.НалогЕСВ <> ЗНАЧЕНИЕ(Справочник.ВидыНачисленийИУдержаний.ЕСВ)
	|	И РасходСоСчета.НалогЕСВ <> ЗНАЧЕНИЕ(Справочник.ВидыНачисленийИУдержаний.ПустаяСсылка)";
	...
	Если ДокументСсылкаРасходСоСчета.УчитыватьВНУ Тогда
		СформироватьТаблицаКнигаДоходовРасходовПоЕН(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства);
	КонецЕсли;
	СформироватьТаблицаУчетЕСВ(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства);
	СформироватьТаблицаВзаиморасчетыПоВзносамВФонды(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства);
	// Биллинг
...
Процедура СформироватьТаблицаДоходыИРасходыЭквайринг(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Для каждого ТекущаяСтрокаРасчетов Из ТаблицаРасчетыПоЭквайрингу Цикл
		...
		Движение.СодержаниеПроводки = НСтр("ru='Комиссия банка-эквайера';uk='Комісія банку-еквайера'");
...
Процедура СформироватьТаблицаРасчетыСПрочимиКонтрагентами(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("УчетРасчетовПоПрочимОперациям", НСтр("ru='Учет расчетов по прочим операциям';uk='Облік розрахунків по іншим операціям'"));
	Запрос.УстановитьПараметр("Комментарий", НСтр("ru='Увеличение долга контрагента';uk='Збільшення боргу контрагента'"));
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаРасходСоСчета);
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ru='Курсовая разница';uk='Курсова різниця'"));
...
Процедура СформироватьТаблицаРасчетыПоКредитамИЗаймам(ДокументСсылкаРасходСоСчета, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("УчетРасчетовПоЗаймамСотрудникам", НСтр("ru='Выдача займа сотруднику';uk='Видача позики співробітникові'"));
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаРасходСоСчета);
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("ПериодКонтроля", СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени.Дата);
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ru='Курсовая разница';uk='Курсова різниця'"));
