	...
	Если ВидОперации = ВидОперацииПоставщику Тогда
		...
		//Если Элементы.СтраницыИтоги.ТекущаяСтраница = Элементы.СтраницаРаспределениеСПомощником Тогда
			...
			ИначеЕсли ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение И
				...
				Сообщение.Текст = НСтр("ru='Не выбран договор для учета аванса. Документ не проведен!';uk='Не обрано договір для обліку авансу. Документ не проведений!'");
		...
		Если ПолучитьФункциональнуюОпцию("РазноситьОплатуОтЭквайрераПоЭквайринговымОперациям") И ТекущийОбъект.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ВозвратОплатыНаПлатежныеКарты Тогда
			...
			Для Каждого ТекущаяСтрока Из ТекущийОбъект.ЭквайринговыеОперации Цикл
				Если ТекущаяСтрока.Документ <> Неопределено И ТекущаяСтрока.Документ.Дата >= ТекущийОбъект.Дата Тогда
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = НСтр("ru='Дата операции больше или равна дате документа!';uk='Дата операції більше або дорівнює даті документа!'");
			...
			Если Отказ Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Дата документа должна быть больше даты всех операций, которые указаны в табличной части ""Эквайринговые операции"". Документ не проведен!';uk='Дата документа повинна бути більше дати всіх операцій, які зазначені в табличній частині ""Еквайрингові операції"". Документ не проведений!'");
				Сообщение.Сообщить();
...
Функция ПолучитьСписокДляВыбораДокументаОснования()
	...
	СписокОснований.Добавить("Документ.ПлатежноеПоручение.ФормаВыбора", НСтр("ru='Платежное поручение';uk='Платіжне доручення'"));
	...
	СписокОснований.Добавить("Документ.ДополнительныеРасходы.ФормаВыбора", НСтр("ru='Дополнительные расходы';uk='Додаткові витрати'"));
	СписокОснований.Добавить("Документ.ПлатежнаяВедомость.ФормаВыбора", НСтр("ru='Платежная ведомость';uk='Платіжна відомість'"));
	Если ПолучитьФункциональнуюОпцию("ПередачаТоваровНаКомиссию") Тогда
		СписокОснований.Добавить("Документ.ОтчетКомиссионера.ФормаВыбора", НСтр("ru='Отчет комиссионера';uk='Звіт комісіонера'"));
	КонецЕсли;
	Если ПолучитьФункциональнуюОпцию("ПриемТоваровНаКомиссию") Тогда
		СписокОснований.Добавить("Документ.ОтчетКомитенту.ФормаВыбора", НСтр("ru='Отчет комитенту';uk='Звіт комітенту'"));
	КонецЕсли;
	Если ПолучитьФункциональнуюОпцию("ПлатежныйКалендарь") Тогда
		СписокОснований.Добавить("Документ.ПеремещениеДСПлан.ФормаВыбора", НСтр("ru='Перемещение денег (план)';uk='Переміщення грошей (план)'"));
	...
	Если ПолучитьФункциональнуюОпцию("КредитыИЗаймы") Тогда
		СписокОснований.Добавить("Документ.НачисленияПоКредитамИЗаймам.ФормаВыбора", НСтр("ru='Начисления по кредитам и займам';uk='Нарахування по кредитах і позиках'"));
		СписокОснований.Добавить("Документ.ДоговорКредитаИЗайма.ФормаВыбора", НСтр("ru='Договор кредита (займа)';uk='Договір кредиту (позики)'"));
	КонецЕсли;
...
Процедура ДатаПриИзменении(Элемент)
	...
	Если Объект.Дата <> ДатаПередИзменением Тогда
		...
		СтруктураДанные.Вставить("ОбновитьКурсыРасчетовТекстВопроса", НСтр("ru='Изменилась дата документа. Установить курс расчетов в соответствии с курсом валюты договора?';uk='Змінилася дата документа. Встановити курс розрахунків відповідно до курсу валюти договору?'"));
		// Конец Обновление курсов расчетов
		
		ТекстСообщения = НСтр("ru='Изменился курс валюты банковского счета. Пересчитать суммы документа?';uk='Змінився курс валюти банківського рахунку. Перерахувати суми документа?'");
...
Процедура БанковскийСчетПриИзменении(Элемент)
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.Зарплата") Тогда
		ТекстСообщения = НСтр("ru='Изменилась валюта банковского счета. Список ""Платежные ведомости"" будет очищен.';uk='Змінилася валюта банківського рахунку. Список ""Платіжні відомості"" буде очищений.'");
...
Процедура УстановитьВидимостьЭлементовВЗависимостиОтВидаОперации(ПриСозданииНаСервере = Ложь)
	...
	Элементы.СуммаДокумента.Заголовок = НСтр("ru='Сумма';uk='Сума'");
	// Конец Эквайринг
	
	// Прочие расчеты
	Элементы.РасчетыПоКредитам.Видимость = Ложь;
	Элементы.РасчетыПоКредитам.Заголовок = НСтр("ru='Расчеты по кредитам';uk='Розрахунки по кредитах'");
	...
	Элементы.Корреспонденция.Заголовок = НСтр("ru='Корреспонденция';uk='Кореспонденція'");
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.Поставщику") Тогда
		...
		Элементы.Контрагент.Заголовок = НСтр("ru='Поставщик';uk='Постачальник'");
		...
		Элементы.СуммаПлатежа.Заголовок = НСтр("ru='Разнесено';uk='Рознесене'");
		...
		Элементы.СуммаРазнесеноВручную.Заголовок = НСтр("ru='Итого разнесено вручную';uk='Разом рознесено вручну'");
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.Покупателю") Тогда
		...
		Элементы.СтраницыРасчетовСКонтрагентом.ТекущаяСтраница = Элементы.СтраницаРучноеЗаполнение;
		Элементы.РасчетыСКонтрагентом.Заголовок = НСтр("ru='Зачет долгов';uk='Залік боргів'");
		
		// Подвал
		Элементы.Подвал.Видимость = Ложь;
		Элементы.СтраницыРасчетовСКонтрагентомПодвал.Видимость = Истина;
		Элементы.СуммаРазнесеноВручную.Заголовок = НСтр("ru='Итого разнесено';uk='Разом рознесено'");
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.Подотчетнику") Тогда
		...
		Элементы.СуммаПлатежа.Видимость = Истина;
		Элементы.СуммаПлатежа.Заголовок = ?(ПолучитьФункциональнуюОпцию("ПлатежныйКалендарь"), НСтр("ru='Сумма (план)';uk='Сума (план)'"), НСтр("ru='Сумма платежа';uk='Сума платежу'"));
		Элементы.СуммаРасчетов.Видимость = НЕ ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций");
		
		Элементы.РасчетыСПодотчетникомРасшифровкаПлатежаСуммаПлатежа.Видимость = ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций");
		Элементы.РасчетыСПодотчетникомРасшифровкаПлатежаСуммаРасчетов.Видимость = НЕ ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций");
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВозвратОплатыНаПлатежныеКарты") Тогда
		...
		Элементы.ГруппаКонтрагент.Видимость = Истина;
		Элементы.Контрагент.Видимость = Истина;
		Элементы.Контрагент.Заголовок = НСтр("ru='Банк-эквайрер';uk='Банк-еквайрер'");
		Элементы.СчетКонтрагента.Видимость = Истина;
		...
		Если ПолучитьФункциональнуюОпцию("РазноситьОплатуОтЭквайрераПоЭквайринговымОперациям") Тогда
			...
			Если НЕ ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций") Тогда
				Элементы.СуммаПлатежаЭквайринг.Заголовок = НСтр("ru='Сумма расчетов';uk='Сума розрахунків'");
			Иначе
				Элементы.СуммаПлатежаЭквайринг.Заголовок = НСтр("ru='Сумма платежа';uk='Сума платежу'");
			КонецЕсли;
		...
		Иначе
			...
			Элементы.СуммаДокумента.Заголовок = НСтр("ru='Сумма/комиссия';uk='Сума/комісія'");
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.НаРасходы") Тогда
		...
		Элементы.Корреспонденция.Заголовок = НСтр("ru='Статья расходов';uk='Стаття витрат'");
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.ВыдачаЗаймаСотруднику") Тогда
		...
		Элементы.РасчетыПоКредитам.Заголовок = НСтр("ru='Расчеты по займам';uk='Розрахунки за позиками'");
...
Процедура ДокументНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.Подотчетник.Пустая() Тогда
		УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(Объект,НСтр("ru='Сначала нужно выбрать подотчетника';uk='Спочатку потрібно вибрати підзвітника'"),,,"Объект.Подотчетник");
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
	...
	Если Объект.ВидОперации = ВидОперацииПоставщику Тогда
		...
		Если СуммаИтог <> 0 И СуммаИтог <> Объект.СуммаДокумента
			...
			НадписьВСкобках = ?(Объект.ВариантЗаполненияРасшифровки = ВариантЗаполненияРасшифровкиВручную, 
				НСтр("ru='разнесено вручную';uk='рознесене вручну'"), 
				НСтр("ru='зачтено + аванс';uk='зараховане + аванс'"));
		...
		Если Объект.ВариантЗаполненияРасшифровки = ВариантЗаполненияРасшифровкиСПомощником Тогда
			...
			Если СуммаИтогЗачесть <> 0 И СуммаИтогЗачесть <> Объект.СуммаДокумента Тогда
				Элемент.СписокВыбора.Добавить(СуммаИтогЗачесть, ""+СуммаИтогЗачесть+НСтр("ru=' (текущий долг)';uk=' (поточний борг)'"));
			КонецЕсли;
		ИначеЕсли Объект.ВариантЗаполненияРасшифровки = ВариантЗаполненияРасшифровкиАвтоматически Тогда
			СуммаИтогЗачесть = Объект.ДоговорыАвтоЗачетаДолгов.Итог("СуммаПлатежа");
			Если СуммаИтогЗачесть <> 0 И СуммаИтогЗачесть <> Объект.СуммаДокумента Тогда
				Элемент.СписокВыбора.Добавить(СуммаИтогЗачесть, ""+СуммаИтогЗачесть+НСтр("ru=' (разнесено по договорам)';uk=' (рознесено по договорам)'"));
			КонецЕсли;
		КонецЕсли;
		
		Если ОстатокВзаиморасчетов < 0 И (-ОстатокВзаиморасчетов) <> Объект.СуммаДокумента Тогда
			Элемент.СписокВыбора.Добавить(-ОстатокВзаиморасчетов, ""+(-ОстатокВзаиморасчетов)+НСтр("ru=' (мы должны)';uk=' (мы повинні)'"));
		КонецЕсли;
		
		Если ОстатокДенежныхСредств > 0 И (ОстатокДенежныхСредств) <> Объект.СуммаДокумента Тогда
			Элемент.СписокВыбора.Добавить(ОстатокДенежныхСредств, ""+(ОстатокДенежныхСредств)+НСтр("ru=' (остаток на счете)';uk=' (залишок на рахунку)'"));
		КонецЕсли;
		
		Если Элемент.СписокВыбора.Количество() = 0 Тогда
			Элемент.СписокВыбора.Добавить(Объект.СуммаДокумента, НСтр("ru='Нет данных для заполнения';uk='Немає даних для заповнення'"));
		КонецЕсли;
...
Процедура НазначениеПлатежаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ПоказатьФормуРедактированияКомментария(Элемент.ТекстРедактирования, ЭтотОбъект, "Объект.НазначениеПлатежа", НСтр("ru='Назначение платежа';uk='Призначення платежу'"));
	СтатистикаИспользованияФормКлиент.ПриИнтерактивномДействии(ЭтотОбъект, Элемент, "НачалоВыбора");
...
&НаКлиенте
Процедура НалогЕСВПриИзменении(Элемент)
	СтруктураДанные = ПолучитьДанныеНалогЕСВПриИзменении(Объект.НалогЕСВ);
	
	Если СтруктураДанные.НалогЕСВ Тогда
		Ставка = УправлениеНебольшойФирмойСервер.ПолучитьПроцентЕСВ(Объект.Дата, Объект.Организация);
		СуммаДляНачисленияЕСВ = УправлениеНебольшойФирмойСервер.ПолучитьСуммуОкладМинимум(Объект.Дата);
		
		Если Объект.РасшифровкаПлатежа.Количество() = 1 Тогда
			
			СуммаПлатежа = СуммаДляНачисленияЕСВ * Ставка / 100;
			Объект.РасшифровкаПлатежа[0].СуммаПлатежа = СуммаПлатежа;
			Объект.РасшифровкаПлатежа[0].СуммаРасчетов = СуммаПлатежа;
			
		КонецЕсли;
		Объект.СуммаДляНачисленияЕСВ = СуммаДляНачисленияЕСВ;
		Объект.СуммаДокумента = СуммаПлатежа;
		Объект.СтавкаЕСВ = Ставка;
	Иначе
		Объект.СтавкаЕСВ = 0;
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура СтавкаЕСВНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	...
	Если СтруктураДанные.НалогЕСВ Тогда
		ПроцентЕСВ = УправлениеНебольшойФирмойСервер.ПолучитьПроцентЕСВ(Объект.Дата, Объект.Организация);
		СписокСтавок.Добавить(ПроцентЕСВ);
	Иначе
		массивЗапроса = УправлениеНебольшойФирмойСервер.ПолучитьСписокДоступныхСтавок(Объект.Дата, Объект.НалогЕСВ);
		СписокСтавок.ЗагрузитьЗначения(массивЗапроса);
	КонецЕсли;
	
	Элементы.СтавкаЕСВ.СписокВыбора.ЗагрузитьЗначения(СписокСтавок);
...
&НаКлиенте
Процедура РасчетыСПодотчетникомРасшифровкаПлатежаСуммаРасчетовПриИзменении(Элемент)
...
Процедура Подбор(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.БанковскийСчет)
	   И НЕ ЗначениеЗаполнено(Объект.ВалютаДенежныхСредств) Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ru='Укажите вначале банковский счет!';uk='Вкажіть спочатку банківський рахунок!'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоОснованию(Команда)
	...
	Если (ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ПеремещениеДСПлан")
		...
		ВызватьИсключение НСтр("ru='Нельзя ввести перемещение денег на основании неутвержденного планового документа!';uk='Не можна ввести переміщення грошей на підставі незатвердженого планового документа!'");
	КонецЕсли;
	
	Ответ = Неопределено;
	
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоОснованиюЗавершение", ЭтотОбъект), НСтр("ru='Документ будет полностью перезаполнен по ""Документу-основанию""! Продолжить выполнение операции?';uk='Документ буде повністю перезаповнений по ""Документу-підставі""! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура ОбновитьТаблицаДолгов(Команда)
	...
	Если Объект.Организация.Пустая() Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ru='Укажите вначале организацию!';uk='Вкажіть спочатку організацію!'"));
		Возврат;
	КонецЕсли;
...
&НаСервере
Функция ПолучитьДанныеНалогЕСВПриИзменении(НалогЕСВ)
	
	СтруктураДанные = Новый Структура;
	
	СтруктураДанные.Вставить(
		"НалогЕСВ",
		НалогЕСВ = Справочники.ВидыНачисленийИУдержаний.ЕСВ
	);
...
Процедура ВидОперацииПриИзмененииНаСервере(ЗаполнятьНалогообложение = Истина)
	...
	// Прочие расчеты
	Если Объект.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ПрочиеРасчеты Тогда
		СтавкаНДСПоУмолчанию = УправлениеНебольшойФирмойПовтИсп.ПолучитьСтавкуНДСБезНДС();
...
Процедура ОчиститьРеквизитыНеОтносящиесяКОперации()
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.Зарплата") Тогда
		...
		Объект.РасшифровкаПлатежа.Очистить();
		Объект.ЭквайринговыеОперации.Очистить();
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходСоСчета.Прочее") Тогда
		Объект.Контрагент = Неопределено;
		Объект.СчетКонтрагента = Неопределено;
		Объект.Подотчетник = Неопределено;
		Объект.Документ = Неопределено;
...
Процедура УстановитьВидимостьРеквизитовВЗависимостиОтКорреспонденции(ОчиститьАналитику = Ложь)
	...
	Если (Объект.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Прочее 
		...
		Иначе // Очистить и не заполнена аналитика.
			Элементы.АналитикаПрочихДоходовИРасходов.Заголовок = НСтр("ru='Аналитика расходов';uk='Аналітика витрат'");
			Элементы.АналитикаПрочихДоходовИРасходов.ПодсказкаВвода = НСтр("ru='<Не настроена у этого счета>';uk='<Не налаштована у цього рахунку>'");
...
Процедура НастроитьЭлементДоговораЗайма()
	...
	Иначе
		Элементы.ДоговорЗаймаСотруднику.ПодсказкаВвода = НСтр("ru='Чтобы выбрать договор, выберите сотрудника';uk='Щоб вибрати договір, виберіть співробітника'");
	КонецЕсли;
	...
	Иначе
		Элементы.ДоговорКредита.ПодсказкаВвода = НСтр("ru='Чтобы выбрать договор, выберите банк';uk='Щоб вибрати договір, виберіть банк'");
	КонецЕсли;
...
Процедура РасшифровкаПлатежаПрочиеРасчетыДоговорНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.Контрагент.Пустая() Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Сначала выберите контрагента';uk='Спочатку виберіть контрагента'");
...
Процедура ЗаполнитьИнформациюПоКредитуЗаймуНаСервере()
	...
	Если Объект.ДоговорКредитаЗайма.Пустая() Тогда
		Элементы.НадписьИнформацияПоКредиту.Заголовок = НСтр("ru='<Выберите договор кредита (займа)>';uk='<Виберіть договір кредиту (позики)>'");
...
Процедура ЗаполнитьИнформациюПоКредитуНаСервере();
	...
	Если ВыборкаГрафикБудущиеМесяцы.Следующий() Тогда
		...
		Иначе
			ДатаПлатежа = "" + Формат(ВыборкаГрафикБудущиеМесяцы.Период, "ДФ=dd.MM.yyyy") + НСтр("ru=' (не в тек. месяце)';uk=' (не в пот. місяці)'");
			НадписьИнформацияПоКредитуЦветТекста = ЦветаСтиля.ЦветТекстаФормы;
		КонецЕсли;
			
		НадписьИнформацияПоКредиту = НСтр("ru='Дата платежа: ';uk='Дата платежу: '")+ДатаПлатежа+НСтр("ru='. Сумма долга: ';uk='. Сума боргу: '")+Формат(ВыборкаГрафикБудущиеМесяцы.СуммаОсновногоДолга, "ЧДЦ=2; ЧН=0")+НСтр("ru='. Сумма %: ';uk='. Сума %: '")+
			Формат(ВыборкаГрафикБудущиеМесяцы.СуммаПроцентов, "ЧДЦ=2; ЧН=0")+
			НСтр("ru='. Комиссия: ';uk='. Комісія: '")+Формат(ВыборкаГрафикБудущиеМесяцы.СуммаКомиссии, "ЧДЦ=2; ЧН=0")+" (" + ВыборкаГрафикБудущиеМесяцы.ВалютаРасчетовПредставление +")";
	...
	ИначеЕсли ВыборкаГрафик.Следующий() Тогда
		...
		Иначе
			ДатаПлатежа = "" + Формат(ВыборкаГрафик.Период, "ДФ=dd.MM.yyyy") + НСтр("ru=' (не в тек. месяце)';uk=' (не в пот. місяці)'");
			НадписьИнформацияПоКредитуЦветТекста = ЦветаСтиля.ЦветТекстаФормы;
		КонецЕсли;
			
		НадписьИнформацияПоКредиту = НСтр("ru='Дата платежа: ';uk='Дата платежу: '")+ДатаПлатежа+НСтр("ru='. Сумма долга: ';uk='. Сума боргу: '")+Формат(ВыборкаГрафик.СуммаОсновногоДолга, "ЧДЦ=2; ЧН=0")+НСтр("ru='. Сумма %: ';uk='. Сума %: '")+
			Формат(ВыборкаГрафик.СуммаПроцентов, "ЧДЦ=2; ЧН=0")+
			НСтр("ru='. Комиссия: ';uk='. Комісія: '")+Формат(ВыборкаГрафик.СуммаКомиссии, "ЧДЦ=2; ЧН=0")+" (" + ВыборкаГрафик.ВалютаРасчетовПредставление +")";
		
	Иначе
		
		НадписьИнформацияПоКредиту = НСтр("ru='Дата платежа: <не определена>';uk='Дата платежу: <не визначена>'");
	...
	Если ВыборкаОстатки.Следующий() Тогда
		...
		НадписьОстатокДолгаПоКредиту = НСтр("ru='Остаток долга: ';uk='Залишок боргу: '")+Формат(Множитель * ВыборкаОстатки.ОсновнойДолгВалОстаток, "ЧДЦ=2; ЧН=0")+НСтр("ru='. Сумма %: ';uk='. Сума %: '")+
			Формат(Множитель * ВыборкаОстатки.ПроцентыВалОстаток, "ЧДЦ=2; ЧН=0")+
			НСтр("ru='. Комиссия: ';uk='. Комісія: '")+Формат(Множитель * ВыборкаОстатки.КомиссияВалОстаток, "ЧДЦ=2; ЧН=0")+" (" + ВыборкаОстатки.ВалютаРасчетовПредставление +")";
	...
	Иначе
		...
		НадписьОстатокДолгаПоКредиту = НСтр("ru='Остаток долга: <не определен> ';uk='Залишок боргу: <не визначений>'");
...
Процедура ЗаполнитьИнформациюПоЗаймуНаСервере()
	...
	Если Выборка.Следующий() Тогда
		НадписьИнформацияПоКредиту = НСтр("ru='Сумма займа: ';uk='Сума займу: '")+Выборка.СуммаДокумента+" ("+Выборка.ВалютаРасчетов+")";
		
		Если Выборка.СуммаДокумента < Выборка.ОсновнойДолгВалПриход Тогда
			НадписьОстатокДолгаПоКредиту = НСтр("ru='Осталось выдать: ';uk='Залишилося видати: '")+(Выборка.СуммаДокумента-Выборка.ОсновнойДолгВалПриход)+" ("+Выборка.ВалютаРасчетов+")"+
				НСтр("ru='. Уже выдано ';uk='. Вже видано '")+Выборка.ОсновнойДолгВалПриход+" ("+Выборка.ВалютаРасчетов+").";
			НадписьОстатокДолгаПоКредитуЦветТекста = ЦветаСтиля.ЦветОсобогоТекста;
		ИначеЕсли Выборка.СуммаДокумента = Выборка.ОсновнойДолгВалПриход Тогда
			НадписьОстатокДолгаПоКредиту = НСтр("ru='Осталось выдать: 0 (';uk='Залишилося видати: 0 ('")+Выборка.ВалютаРасчетов+")";
			НадписьОстатокДолгаПоКредитуЦветТекста = ЦветаСтиля.ЦветОсобогоТекста;
		Иначе
			НадписьОстатокДолгаПоКредиту = НСтр("ru='Осталось выдать: ';uk='Залишилося видати: '")+(Выборка.СуммаДокумента-Выборка.ОсновнойДолгВалПриход)+" ("+Выборка.ВалютаРасчетов+")"+
				НСтр("ru='. Уже выдано ';uk='. Вже видано '")+Выборка.ОсновнойДолгВалПриход+" ("+Выборка.ВалютаРасчетов+").";
		КонецЕсли;
	Иначе
		НадписьИнформацияПоКредиту = НСтр("ru='Сумма займа: ';uk='Сума займу: '")+Объект.ДоговорКредитаЗайма.СуммаДокумента+" ("+Объект.ДоговорКредитаЗайма.ВалютаРасчетов+")";
		НадписьОстатокДолгаПоКредиту = НСтр("ru='Осталось выдать: ';uk='Залишилося видати: '")+Объект.ДоговорКредитаЗайма.СуммаДокумента+" ("+Объект.ДоговорКредитаЗайма.ВалютаРасчетов+")";
	КонецЕсли;
...
Процедура ЗаполнитьЭквайринговыеОперации(Команда)
	...
	Если Объект.ЭквайринговыеОперации.Количество() > 0 Тогда
		ПоказатьВопрос(ОписаниеОповещенияОЗавершении, НСтр("ru='Эквайринговые операции будут полностью перезаполнены. Продолжить?';uk='Еквайрингові операції будуть повністю перезаповнені. Продовжити?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
	Иначе
...
Функция ПроверитьРеквизитыПередЗаполнениемТабличнойЧасти()
	...
	Если Объект.Контрагент.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Укажите вначале контрагента!';uk='Вкажіть спочатку контрагента!'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.БанковскийСчет)
	   И НЕ ЗначениеЗаполнено(Объект.ВалютаДенежныхСредств) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Укажите вначале банковский счет!';uk='Вкажіть спочатку банківський рахунок!'");
	...
	Если Объект.ЭквайринговыйТерминал.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Укажите вначале эквайринговый терминал!';uk='Вкажіть спочатку еквайринговий термінал!'");
...
Процедура ЭквайринговыйТерминалНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.Контрагент.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Сначала выберите банк-эквайрер!';uk='Спочатку виберіть банк-еквайрер!'");
		Сообщение.Поле = "Объект.Контрагент";
		Сообщение.Сообщить();
	...
	Если Объект.БанковскийСчет.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Сначала выберите банковский счет!';uk='Спочатку виберіть банківський рахунок!'");
...
Процедура ПодборЭквайринговыхОпераций(Команда)
	...
	Если (НЕ ЗначениеЗаполнено(Объект.ЭквайринговыйТерминал) ИЛИ НЕ ЗначениеЗаполнено(Объект.Контрагент) 
		...
		И НЕ ЗначениеЗаполнено(Объект.ВалютаДенежныхСредств))) Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ru='Должнен быть выбран банк-эквайрер, эквайринговый терминал и банковский счет!';uk='Повинен бути обраний банк-еквайрер, еквайринговий термінал і банківський рахунок!'"));
		Возврат;
	КонецЕсли;
...
Процедура РаспределитьРасшифровкуПоТаблицеДолгов()
	...
	Для Каждого ТекущаяСтрокаРасшифровки Из Объект.РасшифровкаПлатежа Цикл
		...
		КонецЕсли;
		Если Объект.Контрагент.ВестиРасчетыПоЗаказам Тогда
			ПараметрыОтбора.Вставить("Заказ", ?(ЗначениеЗаполнено(ТекущаяСтрокаРасшифровки.Заказ), ТекущаяСтрокаРасшифровки.Заказ, Неопределено));
		КонецЕсли;
...
Процедура СформироватьЗаголовокГруппыЗачетаДолгов()
		...
		Иначе
			...
			ИначеЕсли КоличествоСтрок > 1 Тогда
				Если КоличествоСтрок > 4 Тогда
					Элементы.РасчетыСКонтрагентом.Заголовок = НСтр("ru='Зачет долгов: автоматически по нескольким договорам (';uk='Зарахування боргів: автоматично по декільком договорам ('") + КоличествоСтрок + ")";
				Иначе
					ЗаголовокГруппы = НСтр("ru='Зачет долгов: автоматически по %КоличествоСтрок%-м договорам';uk='Зарахування боргів: автоматично по %КоличествоСтрок%-м договорам'");
	...
	ИначеЕсли Объект.ВариантЗаполненияРасшифровки = ВариантЗаполненияРасшифровкиСПомощником Тогда
		...
		ОпределитьТекущуюСтраницуПомощника();
		Элементы.РасчетыСКонтрагентом.Заголовок = НСтр("ru='Зачет долгов: с помощником';uk='Зарахування боргів: з помічником'");
		
	// Ручное заполнение.
	ИначеЕсли Объект.ВариантЗаполненияРасшифровки = ВариантЗаполненияРасшифровкиВручную Тогда
		
		Элементы.РасчетыСКонтрагентом.Заголовок = НСтр("ru='Зачет долгов: вручную';uk='Зарахування боргів: вручну'");
...
Процедура ДобавитьДоговорЗачетаДолговНажатиеНаСервере(ТекущаяСтрокаТаблицыДоговоровАвтоЗачета = Неопределено)
	...
	ПолеДоговора.Заголовок = ?(ИндексДоговораУчетаРасчетов = 0, НСтр("ru='По договору';uk='За договором'"), НСтр("ru='и по договору';uk='і за договором'"));
...
Процедура ДобавитьСуммуПлатежаЗачетаДолгов(ИндексДоговораУчетаРасчетов)
	...
	ПолеСуммыПлатежа.Заголовок = НСтр("ru='сумма';uk='сума'");
...
Процедура ДоговорДляЗачетаАванса2НачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.Контрагент.Пустая() Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Сначала нужно выбрать поставщика';uk='Спочатку потрібно вибрати постачальника'"));
...
Процедура УстановитьЗаголовкиКолонокТабличныхЧастей(ВалютаСсылка)
	...
	Элементы.ТаблицаДолговДолгВалПлатежа.Заголовок = НСтр("ru='Текущий долг ';uk='Поточний борг'")+ПредставлениеВалюты;
	Элементы.ТаблицаДолговЗачесть.Заголовок = НСтр("ru='Зачитываем ';uk='Зараховуємо'")+ПредставлениеВалюты;
	
	Элементы.РасшифровкаПлатежаСуммаПлатежа.Заголовок = НСтр("ru='Сумма ';uk='Сума '")+ПредставлениеВалюты;
