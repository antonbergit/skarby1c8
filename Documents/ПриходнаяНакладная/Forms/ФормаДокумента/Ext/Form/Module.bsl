		...
		Если Объект.Предоплата.Количество() > 0
		   И Объект.Договор <> ДоговорПередИзменением Тогда
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("КонтрагентПриИзмененииЗавершение", ЭтотОбъект, Новый Структура("КонтрагентПередИзменением, ДоговорПередИзменением, КонтрагентВестиРасчетыПоЗаказамПередИзменением, ВидимостьДоговораПередИзменением, СтруктураДанные", КонтрагентПередИзменением, ДоговорПередИзменением, КонтрагентВестиРасчетыПоЗаказамПередИзменением, ВидимостьДоговораПередИзменением, СтруктураДанные)),
				НСтр("ru='Зачет предоплаты будет очищен, продолжить?';uk='Залік передоплати буде очищений, продовжити?'"),
...
"Пересчитать документ в соответствии с договором?';uk='Договір з контрагентом передбачає вид цін, відмінний від установленого в документі! "
...
Процедура ЗаказПриИзменении(Элемент)
	...
	Если Объект.Предоплата.Количество() > 0
	   ...
	   И Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика") Тогда
		...
		Ответ = Неопределено;
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаказПриИзмененииЗавершение", ЭтотОбъект, Новый Структура("ЗаказПередИзменением", ЗаказПередИзменением)), НСтр("ru='Зачет предоплаты будет очищен, продолжить?';uk='Залік передоплати буде очищений, продовжити?'"), Режим, 0);
	КонецЕсли;
...
Процедура ЗаполнитьСписокДокументОснование()
	...
	СписокДокументОснование.Добавить("Документ.ЗаказПокупателя.ФормаВыбора", НСтр("ru='Заказ покупателя';uk='Замовлення покупця'"));
	СписокДокументОснование.Добавить("Документ.ЗаказПоставщику.ФормаВыбора", НСтр("ru='Заказ поставщику';uk='Замовлення постачальникові'"));
	Если ПолучитьФункциональнуюОпцию("ИспользоватьРемонты") Тогда
		СписокДокументОснование.Добавить("Документ.ПриемИПередачаВРемонт.ФормаВыбора", НСтр("ru='Прием и передача в ремонт';uk='Прийом і передача в ремонт'"));
	КонецЕсли;
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОрдерныйСклад") Тогда
		СписокДокументОснование.Добавить("Документ.ПриходныйОрдер.ФормаВыбора", НСтр("ru='Приходный ордер';uk='Прибутковий ордер'"));
	КонецЕсли;
	СписокДокументОснование.Добавить("Документ.РасходнаяНакладная.ФормаВыбора", НСтр("ru='Расходная накладная';uk='Видаткова накладна'"));
	СписокДокументОснование.Добавить("Документ.СчетНаОплатуПоставщика.ФормаВыбора", НСтр("ru='Счет на оплату поставщика';uk='Рахунок на оплату постачальника'"));
	Если ПолучитьФункциональнуюОпцию("УчетРозничныхПродаж") Тогда
		СписокДокументОснование.Добавить("Документ.ЧекККМ.ФормаВыбора", НСтр("ru='Чек ККМ';uk='Чек ККМ'"));
	КонецЕсли;
	Если ПолучитьФункциональнуюОпцию("ВестиСведенияДляДекларацийПоАлкогольнойПродукции") Тогда
		СписокДокументОснование.Добавить("Документ.ТТНВходящаяЕГАИС.ФормаВыбора", НСтр("ru='Товарно-транспортная накладная ЕГАИС';uk='Товарно-транспортна накладна ЄДАІС'"));
	КонецЕсли;
...
Процедура РасходыНаправлениеДеятельностиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ СтруктураДанные.ВозможностьУказанияНаправленияДеятельности Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьПредупреждение(, НСтр("ru='Для данного расхода направление деятельности не указывается!';uk='Для даної витрати напрямок діяльності не вказується!'"));
	КонецЕсли;
...
Процедура РасходыСтруктурнаяЕдиницаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ СтруктураДанные.ВозможностьУказанияПодразделения Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьПредупреждение(, НСтр("ru='Для этого расхода подразделение не указывается!';uk='Для цієї витрати підрозділ не вказується!'"));
	КонецЕсли;
...
Процедура РасходыЗаказНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ СтруктураДанные.ВозможностьУказанияЗаказа Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьПредупреждение(, НСтр("ru='Для этого расхода заказ не указывается!';uk='Для цієї витрати замовлення не вказується!'"));
	КонецЕсли;
...
Процедура ЗаполнитьТоварыПоФактуПоДаннымПоставщика(Команда)
	...
	Если ЕстьОшибкиСоответствияЕГАИСКлиент() Тогда
		ПоказатьПредупреждение(,НСтр("ru='Заполните соответствие номенлатуры ЕГАИС';uk='Заповніть відповідність номенлатури ЄДАІС'"));
		Возврат;
	КонецЕсли;
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Табличная часть ""Товары"" будет перезаполнена. Продолжить?';uk='Таблична частина ""Товари"" буде заповнена. Продовжити?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ОтправитьАктЕГАИС(Команда)
	...
	Если Объект.Запасы.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Для выполнения операции товары должны быть заполнены.';uk='Для виконання операції товари повинні бути заповнені.'"),, "Запасы", "Объект");
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		Если ЗначениеЗаполнено(Объект.ПодписьЧекаЕГАИС) Тогда
			ТекстСообщения = НСтр("ru='Данные уже переданы в ЕГАИС';uk='Дані вже передані в ЄДАІС'");
...
Функция СформироватьНадписьЦеныИВалюта(СтруктураНадписи)
	...
	Если ЗначениеЗаполнено(СтруктураНадписи.ВидЦенКонтрагента) Тогда
		Если ПустаяСтрока(ТекстНадписи) Тогда
			ТекстНадписи = ТекстНадписи + НСтр("ru='%ВидЦенКонтрагента%';uk='%ВидЦенКонтрагента%'");
		Иначе	
			ТекстНадписи = ТекстНадписи + НСтр("ru=' • %ВидЦенКонтрагента%';uk=' • %ВидЦенКонтрагента%'");
		КонецЕсли;	
...
Процедура УстановитьВидимостьИДоступность(ИзмененВидОперации = Ложь)
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПриходнаяНакладная.ВозвратСОтветхранения") Тогда
		Элементы.Контрагент.Заголовок = НСтр("ru='Получатель';uk='Одержувач'");
	Иначе	
	...
	//Серийные номера
	//Если УчетСерийныхНомеров = Перечисления.УчетСерийныхНомеров.ОстаткиСерийныхНомеров Тогда
	//	Элементы.ЗапасыСерийныеНомера.Видимость = Истина;
	//Иначе
	//	Элементы.ЗапасыСерийныеНомера.Видимость = Ложь;
	//КонецЕсли;
...
Функция ПодготовкаДанныхДляПередачиДанныеВЕГАИС(ТранспортныйМодуль, ДанныеЕГАИСДостаточны)
	...
	ТаблицаТоваров = УправлениеНебольшойФирмойСервер.ПодготовкаДанныхДляПробитияЧекаККМ(ТаблицаТовары, ТаблицаАкцизы, ДокументОбъект, ДанныеЕГАИСДостаточны);
...
"Дополнительное описание: %ДополнительноеОписание%';uk='При друці чека сталася помилка."
...
Процедура ЗагрузкаИзФайлаУслуги(Команда)
	...
	НастройкиЗагрузкиДанных.Вставить("Заголовок", НСтр("ru='Загрузка услуг из файла';uk='Завантаження послуг з файлу'"));
...
Процедура СформироватьНадписьМаркировкаПродукции()
	...
	Если ПравоДобавления Тогда
		ТекстНадписи = НСтр("ru='Оформить маркировку товаров';uk='Оформити маркування товарів'");
		ИмяКоманды   = "ОформитьМаркировкуТоваров";
	Иначе
		ТекстНадписи = НСтр("ru='Маркировка товаров не создана';uk='Маркування товарів не створене'");
		ИмяКоманды   = "";
	КонецЕсли;
	...
	Если Выборка.Следующий() Тогда
		...
		ТекстНадписи  = СтрШаблон(НСтр("ru='Маркировка товаров: %1';uk='Маркування товарів: %1'"), НРег (Строка(Выборка.Статус)));
...
Процедура СтатусГИСМОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru='Возврат товаров от клиента был изменен. Записать?';uk='Повернення товарів від клієнта було змінено. Записати?'");
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения("СтатусГИСМОбработкаНавигационнойСсылкиЗавершение",
		                                                    ЭтотОбъект,
		                                                    Новый Структура("НавигационнаяСсылкаФорматированнойСтроки", НавигационнаяСсылкаФорматированнойСтроки));
		ТекстВопроса = НСтр("ru='Возврат товаров от клиента не записан. Записать?';uk='Повернення товарів від клієнта не записане. Записати?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
