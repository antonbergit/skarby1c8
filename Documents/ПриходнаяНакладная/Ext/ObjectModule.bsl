...
Процедура ЗаполнитьПоРасходнойНакладной(ДанныеЗаполнения) Экспорт
	...
	Иначе
		ТекстИсключения = НСтр("ru='Невозможен ввод операции ""Поступления"" на основании операции - ""%ВидОперации""!';uk='Неможливе введення операції ""Надходження"" на підставі операції - ""%ВидОперации""!'");
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
		...
		Для каждого СтрокаРасходы Из Расходы Цикл
			...
			Если Константы.ФункциональнаяОпцияУчетПоНесколькимПодразделениям.Получить()
			 ...
			 И НЕ ЗначениеЗаполнено(СтрокаРасходы.СтруктурнаяЕдиница) Тогда
				ТекстСообщения = НСтр("ru='Для номенклатуры ""%Номенклатура%"" указанной в строке %НомерСтроки% списка ""Услуги"", должен быть заполнен реквизит ""Подразделение"".';uk='Для номенклатури ""%Номенклатура%"" зазначеної в рядку %НомерСтроки% списку ""Послуги"", повинен бути заповнений реквізит ""Підрозділ"".'"
				);
	...
	Если ВключатьРасходыВСебестоимость
		...
		ТекстСообщения = НСтр("ru='Сумма услуг не равна распределенной сумме по запасам!';uk='Сума послуг не дорівнює розподіленій сумі по запасах!'");
		УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
			,
			ТекстСообщения,
			Неопределено,
	...
	Для каждого Строка Из ТаблицаПредоплата Цикл
		...
		Если НайденнаяСтрокаЗапасы = Неопределено
		   ...
		   И Контрагент.ВестиРасчетыПоЗаказам Тогда
			ТекстСообщения = НСтр("ru='Нельзя зачесть аванс по заказу отличному от указанных в табличных частях ""Запасы"" или ""Услуги""!';uk='Не можна зарахувати аванс по замовленню відмінному від зазначених у табличних частинах ""Запаси"" або ""Послуги""!'");
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				,
				ТекстСообщения,
				Неопределено,
	...
	Если СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Розница
	 ИЛИ СтруктурнаяЕдиница.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.РозницаСуммовойУчет Тогда
		...
		Пока ВыборкаРезультатаЗапроса.Следующий() Цикл
			...
			ТекстСообщения = НСтр("ru='Для номенклатуры %ПредставлениеНоменклатуры% в строке %НомерСтроки% списка ""Запасы"" не установлена розничная цена!';uk='Для номенклатури %ПредставлениеНоменклатуры% у рядку %НомерСтроки% списку ""Запаси"" не встановлена роздрібна ціна!'");
