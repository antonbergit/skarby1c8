...
Процедура ОткрытьСертификат(Команда)
	...
	Иначе
		ОчиститьСообщения();
		ТекстОшибки = НСтр("ru='Выберите сертификат в списке установленных подписей.';uk='Выберите сертификат в списке установленных подписей.'");
...
Процедура ЗапроситьСостояниеЭД(Команда)
	...
	Если НЕ ЕстьПоддержкаВидаЭД(Объект.НастройкаОбмена, ВидЭДЗапросОСостоянии) Тогда
		ТекстСообщения = НСтр("ru='Банк не поддерживает данный функционал';uk='Банк не поддерживает данный функционал'");
	...
	Если ТребуетсяПодписьЗапроса Тогда
		...
		Иначе // Обмен через ВК
			...
			Иначе
				...
				ВидОперации = НСтр("ru='Подписание электронного документа';uk='Підписання електронного документу'");
...
Процедура ОтозватьЭД(Команда)
	...
	Если МожноОтозватьЭД(Объект.Ссылка) Тогда
		...
		ЗаголовокОкна = НСтр("ru='Укажите причину отзыва электронного документа';uk='Укажите причину отзыва электронного документа'");
...
Процедура ИзменитьВидимостьДоступность()
	...
	Если НЕ ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ЗначениеФункциональнойОпции("ИспользоватьЭлектронныеПодписиЭД")
		...
		Элементы.КомандаУтвердитьЭД.Заголовок = НСтр("ru='Утвердить и отправить';uk='Утвердить и отправить'");
	КонецЕсли;
	...
	Если ПодготовленКОтправке Тогда
		КомандаПодписиОтправки.Заголовок = НСтр("ru='Отправить';uk='Відправити'");
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуЭП()
	...
	ШаблонЗаголовка = НСтр("ru='Подписи %1и статусы';uk='Подписи %1и статусы'");
...
Процедура ЗаполнитьСтатусПодписи(НоваяСтрока, ТекСтрока)
	...
	Если ЗначениеЗаполнено(ТекСтрока.ДатаПроверкиПодписи) Тогда
		НоваяСтрока.Представление = ?(ТекСтрока.ПодписьВерна, НСтр("ru='Верна';uk='Вірна'"), НСтр("ru='Неверна';uk='Невірна'"))
			+" (" + ТекСтрока.ДатаПроверкиПодписи + ")";
	Иначе
		НоваяСтрока.Представление = НСтр("ru='Не проверена';uk='Не проверена'");
	КонецЕсли
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка подтверждения платежного поручения."
"Код ошибки: ДО-%1"
"%2'");
			...
			Операция = НСтр("ru='Подтверждение платежного поручения';uk='Подтверждение платежного поручения'");
		...
		Если Результат.Количество() = 0 Тогда
			ПоказатьОповещениеПользователя(НСтр("ru='Нет данных для подтверждения';uk='Нет данных для подтверждения'"));
		КонецЕсли;
		
		Если Не ПустаяСтрока(Результат[0].ТекстОшибки) Тогда
			ТекстСообщения = НСтр("ru='Ошибка при подтверждении платежного поручения:';uk='Ошибка при подтверждении платежного поручения:'") + " " + Результат[0].ТекстОшибки;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
		...
		ПоказатьОповещениеПользователя(НСтр("ru='Документ подтвержден';uk='Документ подтвержден'"));
	КонецЕсли;
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка инициализации сессии подтверждения."
"Код ошибки: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ru='Инициализация сессии подтверждения';uk='Инициализация сессии подтверждения'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, Объект.Ссылка);
		Возврат;
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка отправки кода подтверждения платежного поручения."
"Код ошибки: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ru='Отправка кода подтверждения платежного поручения';uk='Отправка кода подтверждения платежного поручения'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, Объект.Ссылка);
		Возврат;
	КонецПопытки;
...
Процедура ПослеПолученияОтпечатковЗапроситьСостояниеЭД(Отпечатки, ПараметрыОбработки) Экспорт
	...
	Иначе
		ТекстСообщения = НСтр("ru='Нет доступных сертификатов.';uk='Нет доступных сертификатов.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Функция СформироватьЗапросНаОтзывЭД(Знач НастройкаОбмена, Знач СообщениеОбменаПлатежноеПоручение, Знач Причина)
	...
	НазваниеЭД = НСтр("ru='Запрос на отзыв электронного документа';uk='Запрос на отзыв электронного документа'");
...
"Проверьте настройки обмена через сервис 1С:ДиректБанк.';uk='Не найден подходящий сертификат подписи для документа Запрос состояния электронного документа."
...
"Установите сертификаты или обратитесь к администратору.';uk='На компьютере не установлен ни один сертификат, указанный в настройке обмена."
"Установите сертификаты или обратитесь к администратору.'");
	...
	Если РеквизитыНастройкиОбмена.Недействительна Тогда
		ШаблонСообщения = НСтр("ru='Настройка обмена с банком %1 недействительна';uk='Настройка обмена с банком %1 недействительна'");
	...
	НазваниеЭД = НСтр("ru='Запрос состояния электронного документа';uk='Запрос состояния электронного документа'");
...
Процедура ОбработатьОшибкуПодтвержденияПлатежногоПорученияЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ru='При подтверждении платежного поручения произошла ошибка.';uk='При подтверждении платежного поручения произошла ошибка.'");
	ВидОперации = НСтр("ru='Подтверждение платежного поручения.';uk='Подтверждение платежного поручения.'");
...
Процедура ОбработатьОшибкуОтправкиКодаПодтвержденияЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ru='При отправке кода подтверждения через SMS произошла ошибка.';uk='При отправке кода подтверждения через SMS произошла ошибка.'");
	ВидОперации = НСтр("ru='Отправка кода подтверждения платежного поручения.';uk='Отправка кода подтверждения платежного поручения.'");
	Оповещение = Новый ОписаниеОповещения("СообщитьОшибкуПользователю", ЭтотОбъект);
...
Процедура ОбработатьОшибкуНачалаПодтвержденияПлатежногоПоручения(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ru='При начале подтверждения платежного поручения произошла ошибка.';uk='При начале подтверждения платежного поручения произошла ошибка.'");
	ВидОперации = НСтр("ru='Начало подтверждения платежного поручения.';uk='Начало подтверждения платежного поручения.'");
	Оповещение = Новый ОписаниеОповещения("СообщитьОшибкуПользователю", ЭтотОбъект);
