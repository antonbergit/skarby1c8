...
Функция ПечатнаяФормаЭД(СтруктураРазбораФайла, ИД = Неопределено) Экспорт
	...
	Попытка
		...
		ИначеЕсли СтрокаОбъекта.ВидЭД = Перечисления.ВидыЭДОбменСБанками.ЗапросОСостоянииЭД Тогда
			...
			ДанныеЭДДляПечати = ДанныеДляПечати(СтрокаОбъекта);
			ЗаполнитьТабличныйДокумент_ЭД(ТабличныйДокумент, ДанныеЭДДляПечати, "ЭД_ЗапросОСостоянииЭД");
			
		ИначеЕсли СтрокаОбъекта.ВидЭД = Перечисления.ВидыЭДОбменСБанками.ЗапросЗонд Тогда
			
			ДанныеЭДДляПечати = ДанныеДляПечати(СтрокаОбъекта);
			ЗаполнитьТабличныйДокумент_ЭД(ТабличныйДокумент, ДанныеЭДДляПечати, "ЭД_ЗапросЗонд");
		...
		ИначеЕсли СтрокаОбъекта.ВидЭД = Перечисления.ВидыЭДОбменСБанками.ЗапросНаОтзывЭД Тогда
			...
			ДанныеЭДДляПечати = ДанныеДляПечати(СтрокаОбъекта);
			ЗаполнитьТабличныйДокумент_ЭД(ТабличныйДокумент, ДанныеЭДДляПечати, "ЭД_ЗапросНаОтзыв");
...
"%1.';uk='Ошибка формирования табличного документа:"
"%1.'");
		КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонСообщения, КраткоеПредставлениеОшибки);
		Операция = НСтр("ru='формирования табличного документа';uk='формирования табличного документа'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечататьКарточкуЭД Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			"КарточкаЭД",
			НСтр("ru='Карточка электронного документа';uk='Карточка электронного документа'"),
...
Процедура ЗаполнитьТабличныйДокументПлатежноеТребование_ЭД(ТабличныйДокумент, Реквизиты, ИД)
	...
	Макет = Документы.СообщениеОбменСБанками.ПолучитьМакет("ЭД_ПлатежноеТребование");
	...
	Если Реквизиты.УсловиеОплаты = "1" Тогда
		ОбластьМакета.Параметры.УсловиеОплаты = НСтр("ru='1 - заранее данный акцепт плательщика';uk='1 - заранее данный акцепт плательщика'");
	Иначе
		ОбластьМакета.Параметры.УсловиеОплаты = НСтр("ru='2 - требуется получение акцепта плательщика';uk='2 - требуется получение акцепта плательщика'");
	КонецЕсли;
	...
	ОбластьМакета.Параметры.НаименованиеНомер = НСтр("ru='ПЛАТЕЖНОЕ ТРЕБОВАНИЕ №';uk='ПЛАТЕЖНОЕ ТРЕБОВАНИЕ №'") + " " + Реквизиты.Номер;
...
Процедура ЗаполнитьТабличныйДокументПлатежноеПоручениеИсходящее_ЭД(ТабличныйДокумент, Реквизиты, ИД)
	...
	Макет = Документы.СообщениеОбменСБанками.ПолучитьМакет("ЭД_ПлатежноеПоручение");

	ОбластьМакета = Макет.ПолучитьОбласть("ЗаголовокТаблицы");
	
	ЗаполнитьЗначенияСвойств(ОбластьМакета.Параметры, Реквизиты);
	
	ОбластьМакета.Параметры.НаименованиеНомер = НСтр("ru='ПЛАТЕЖНОЕ ПОРУЧЕНИЕ №';uk='ПЛАТЕЖНОЕ ПОРУЧЕНИЕ №'") + " " + Реквизиты.Номер;
	ОбластьМакета.Параметры.ДатаДокумента = Формат(Реквизиты.Дата, "ДЛФ=D");
...
Процедура ЗаполнитьТабличныйДокументЗапросВыписки_ЭД(ТабличныйДокумент, ДанныеПечати)
	...
	Макет = Документы.СообщениеОбменСБанками.ПолучитьМакет("ЭД_ЗапросВыписки");
...
Процедура ЗаполнитьТабличныйДокументИзвещениеОСостоянии_ЭД(ТабличныйДокумент, Реквизиты)
	...
	Макет = Документы.СообщениеОбменСБанками.ПолучитьМакет("ЭД_ИзвещениеОСостоянииЭД");
...
Процедура ЗаполнитьТабличныйДокументКвитанция_ЭД(ТабличныйДокумент, ДанныеПечати)
	...
	Макет = Документы.СообщениеОбменСБанками.ПолучитьМакет("ЭД_Квитанция");
...
Процедура ЗаполнитьТабличныйДокументВыписки_ЭД(ТабличныйДокумент, ДанныеПечати)
	...
	Макет = Документы.СообщениеОбменСБанками.ПолучитьМакет("ЭД_ВыпискаБанка");
...
Функция ПолучитьДанныеВыпискиДляПечати(СтрокаОбъекта, ДеревоРазбора)
	...
	Иначе	
		ДанныеЗаполненияШапки.Вставить("Организация", НСтр("ru='<не указано>';uk='<не указано>'"));
	КонецЕсли;
	...
	Иначе
		ДанныеЗаполненияШапки.Вставить("Банк", НСтр("ru='<не указано>';uk='<не указано>'"));
	КонецЕсли;
...
Функция СтатусПодписи(ВыборкаЭД)
	...
	Если ВыборкаЭД.ПодписьВерна Тогда
		СтатусПодписи = НСтр("ru='Верна (';uk='Верна ('") + Формат(ВыборкаЭД.ДатаПроверкиПодписи,"ДЛФ=DT") + ")";
	Иначе
		СтатусПодписи = НСтр("ru='Неверна (';uk='Неверна ('") + Формат(ВыборкаЭД.ДатаПроверкиПодписи,"ДЛФ=DT") + ")";
	КонецЕсли;
