...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Не ЗначениеЗаполнено(ТекущийОбъект.ОрганизацияЕГАИС) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось идентифицировать организацию по классификатору ЕГАИС. Проверьте наличие настроек ЕГАИС для выбранной организации.';uk='Не удалось идентифицировать организацию по классификатору ЕГАИС. Проверьте наличие настроек ЕГАИС для выбранной организации.'"),,"Объект.Организация",, Отказ);
	КонецЕсли;
...
Процедура УказатьАкцизнуюМарку(Команда)
	...
	Если Объект.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.АктПостановкиНаБалансВТорговомЗале") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Для постановки на баланс в торговом зале, ввод акцизной марки не требуется.';uk='Для постановки на баланс в торговом зале, ввод акцизной марки не требуется.'"));
		Возврат;
	КонецЕсли;
	
	Если НЕ МаркируемаяПродукция(ТекущиеДанные.АлкогольнаяПродукция) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ввод акцизной марки возможен только для маркируемой алкогольной продукции.';uk='Ввод акцизной марки возможен только для маркируемой алкогольной продукции.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВыгрузитьАкт(Команда)
	...
	Если Модифицированность Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Документ не сохранен.';uk='Документ не сохранен.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВыгрузкаАкта_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Результат Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Документ успешно выгружен.';uk='Документ успешно выгружен.'"));
...
Процедура ЗапросНаОтменуПроведения_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Результат Тогда
		ЭтотОбъект.Прочитать();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Запрос на отмену проведения успешно выгружен.';uk='Запрос на отмену проведения успешно выгружен.'"));
	КонецЕсли;
