...
Процедура ЗаполнитьПоЗаказуПокупателя(ДокументСсылкаЗаказПокупателя) Экспорт
	...
	Если ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
		ВызватьИсключение НСтр("ru='Нельзя ввести Распределение затрат на основании заказ-наряда!';uk='Не можна ввести Розподіл витрат на підставі замовлення-наряду!'");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Запасы.Итог("Количество") <> РаспределениеЗапасов.Итог("Количество") Тогда
		...
		ТекстСообщения = НСтр("ru='Количество запасов не соответствует количеству распределения!';uk='Кількість запасів не відповідає кількості розподілу!'");
	...
	Если Затраты.Итог("Сумма") <> РаспределениеЗатрат.Итог("Сумма") Тогда
		...
		ТекстСообщения = НСтр("ru='Сумма расходов не соответствует сумме распределения!';uk='Сума витрат не відповідає сумі розподілу!'");
	...
	Если Константы.ФункциональнаяОпцияРезервированиеЗапасов.Получить() Тогда
		...
		Для каждого СтрокаРаспределениеЗапасов Из РаспределениеЗапасов Цикл
			...
			Если СтрокаРаспределениеЗапасов.Резерв > СтрокаРаспределениеЗапасов.Количество Тогда
				...
				ТекстСообщения = НСтр("ru='В строке №%Номер% табл. части ""Распределение запасов"" количество позиций к списанию из резерва превышает общее количество запасов.';uk='У рядку №%Номер% табл. частини ""Розподіл запасів"" кількість позицій до списання з резерву перевищує загальну кількість запасів.'");
