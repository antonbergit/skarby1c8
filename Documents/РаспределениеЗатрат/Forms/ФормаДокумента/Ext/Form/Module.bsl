...
Процедура ЗапасыЗаполнитьПоНормативам(Команда)
	...
	Если Объект.Запасы.Количество() <> 0 Тогда
		...
		ТекстВопроса = НСтр("ru='Табличная часть ""Запасы"" будет перезаполнена!';uk='Таблична частина ""Запаси"" буде перезаповнена!'") + Символы.ПС;
		Если Объект.РаспределениеЗапасов.Количество() <> 0 Тогда
			ТекстВопроса = ТекстВопроса + НСтр("ru='Табличная часть ""Распределение запасов"" будет очищена!';uk='Таблична частина ""Розподіл запасів"" буде очищена!'") + Символы.ПС;
		КонецЕсли;	
		ТекстВопроса = ТекстВопроса + НСтр("ru='Продолжить выполнение операции?';uk='Продовжити виконання операції?'");
		Ответ = Неопределено;
...
Процедура ЗапасыРаспределитьПоНормативам(Команда)
	...
	Если Объект.РаспределениеЗапасов.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗапасыРаспределитьПоНормативамЗавершение", ЭтотОбъект), НСтр("ru='Табличная часть ""Распределение запасов"" будет перезаполнена! Продолжить выполнение операции?';uk='Таблична частина ""Розподіл запасів"" буде перезаповнена! Продовжити виконання операції?'"), 
...
Процедура ЗапасыРаспределитьПоКоличеству(Команда)
	...
	Если Объект.РаспределениеЗапасов.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗапасыРаспределитьПоКоличествуЗавершение", ЭтотОбъект), НСтр("ru='Табличная часть ""Распределение запасов"" будет перезаполнена! Продолжить выполнение операции?';uk='Таблична частина ""Розподіл запасів"" буде перезаповнена! Продовжити виконання операції?'"), 
							РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
...
Процедура ЗатратыЗаполнитьПоОстаткам(Команда)
	...
	Если Объект.Затраты.Количество() <> 0 Тогда
		...
		ТекстВопроса = НСтр("ru='Табличная часть ""Затраты"" будет перезаполнена!';uk='Таблична частина ""Витрати"" буде перезаповнена!'") + Символы.ПС;
		ТекстВопроса = ТекстВопроса + НСтр("ru='Табличная часть ""Распределение расходов"" будет очищена!';uk='Таблична частина ""Розподіл витрат"" буде очищена!'") + Символы.ПС;
		ТекстВопроса = ТекстВопроса + НСтр("ru='Продолжить выполнение операции?';uk='Продовжити виконання операції?'");
		Ответ = Неопределено;
...
Процедура ЗатратыРаспределитьПоКоличеству(Команда)
	...
	Если Объект.РаспределениеЗатрат.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗатратыРаспределитьПоКоличествуЗавершение", ЭтотОбъект), НСтр("ru='Табличная часть ""Распределение расходов"" будет перезаполнена! Продолжить выполнение операции?';uk='Таблична частина ""Розподіл витрат"" буде перезаповнена! Продовжити виконання операції?'"), 
							РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
...
Процедура ПродукцияЗаполнитьПоВыпуску(Команда)
	...
	Если Объект.Продукция.Количество() <> 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПродукцияЗаполнитьПоВыпускуЗавершение", ЭтотОбъект), НСтр("ru='Табличная часть ""Продукция"" будет перезаполнена! Продолжить выполнение операции?';uk='Таблична частина ""Продукція"" буде перезаповнена! Продовжити виконання операції?'"), 
							РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
