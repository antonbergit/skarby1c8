...
Функция ПредставлениеКонтрагента(Строка, Представление) Экспорт
	...
	ПредставлениеИНН = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1';uk='%1'"), Строка.ИНН);
	
	Если ЗначениеЗаполнено(Представление) Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 (%2)';uk='%1 (%2)'"), ПредставлениеИНН, Представление);
	Иначе
...
Функция КонтрагентыПоСпискуИНН(ТаблицаИНН) Экспорт
	...
	ТаблицаИННДляЗапроса = ТаблицаИНН.СкопироватьКолонки("ИНН");
	
	Для Каждого СтрокаТаблицыИНН Из ТаблицаИНН Цикл
		Если СокрЛП(СтрокаТаблицыИНН.ИНН) <> "" Тогда
			СтрокаТаблицыИННДляЗапроса = ТаблицаИННДляЗапроса.Добавить();
			СтрокаТаблицыИННДляЗапроса.ИНН = СокрЛП(СтрокаТаблицыИНН.ИНН);
		КонецЕсли;
	КонецЦикла;
	
	Запрос = Новый Запрос();
	Запрос.Параметры.Вставить("ИНН", ТаблицаИННДляЗапроса);
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	| ТаблицаИНН.ИНН КАК ИНН
...
Функция СведенияОКонтрагентеПоИНН(ИНН) Экспорт
	
	ПредставлениеКонтрагента = "";
	
	ТаблицаИНН = Документы.ПоясненияКДекларацииПоНДС.ПустаяСсылка().Контрагенты.ВыгрузитьКолонки("ИНН");
	СтрокаТаблицы = ТаблицаИНН.Добавить();
	СтрокаТаблицы.ИНН = ИНН;
	
	ТаблицаКонтрагентов = Документы.ПоясненияКДекларацииПоНДС.КонтрагентыПоСпискуИНН(ТаблицаИНН);
	
	Возврат СведенияОКонтрагентеПоТаблицеКонтрагентов(ИНН, ТаблицаКонтрагентов);
	
КонецФункции

Функция СведенияОКонтрагентеПоТаблицеКонтрагентов(ИНН, ТаблицаКонтрагентов) Экспорт
	
	Сведения = Новый Структура("ИНН, Представление, Контрагенты");
	Сведения.ИНН = ИНН;
	Сведения.Контрагенты = Новый СписокЗначений();
	
	Если ТаблицаКонтрагентов <> Неопределено Тогда
		СтрокиКонтрагента = ТаблицаКонтрагентов.НайтиСтроки(Новый Структура("ИНН", ИНН));
	...
	Сведения.Представление = Документы.ПоясненияКДекларацииПоНДС.ПредставлениеКонтрагента(Сведения, ПредставлениеКонтрагента);
...
Функция ПредставлениеДокументаОплаты(Строка) Экспорт
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("ru='%1 от %2';uk=' %1 від %2'"), Строка.Номер, Формат(Строка.Дата, "ДЛФ=D"));
КонецФункции
...
Функция НаименованиеОтветаПоТребованию(Требование) Экспорт
	...
	Наименование 			= НСтр("ru='Пояснения к декларации по НДС %ПредставлениеДекларации%';uk='Пояснения к декларации по НДС %ПредставлениеДекларации%'");
...
Функция ПредставлениеОшибкиРаздела8(КодОшибки, ГрафаОшибки) Экспорт
	...
	Если КодОшибки = 1 Тогда
		Возврат НСтр("ru='У контрагента нет записи об операции';uk='У контрагента нет записи об операции'");
	ИначеЕсли КодОшибки = 2 Тогда
		Возврат НСтр("ru='Операция отражена по разному в разделах 8 и 9';uk='Операция отражена по разному в разделах 8 и 9'");
	ИначеЕсли КодОшибки = 3 Тогда
		Возврат НСтр("ru='Операция отражена по разному в разделах 10 и 11';uk='Операция отражена по разному в разделах 10 и 11'")
	ИначеЕсли КодОшибки = 4 Тогда
		...
		Для Каждого Графа Из МассивГраф Цикл
			Если Графа = "1" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Номер строки декл.""';uk='""Номер строки декл.""'"));
			ИначеЕсли Графа = "2" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Код вида опер.""';uk='""Код вида опер.""'"));
			ИначеЕсли Графа = "3" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Номер с/ф""';uk='""Номер с/ф""'"));
			ИначеЕсли Графа = "4" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Дата с/ф""';uk='""Дата с/ф""'"));
			ИначеЕсли Графа = "5" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Номер испр. с/ф""';uk='""Номер испр. с/ф""'"));
			ИначеЕсли Графа = "6" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Дата испр. с/ф""';uk='""Дата испр. с/ф""'"));
			ИначеЕсли Графа = "7" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Номер корр. с/ф""';uk='""Номер корр. с/ф""'"));
			ИначеЕсли Графа = "8" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Дата корр. с/ф""';uk='""Дата корр. с/ф""'"));
			ИначеЕсли Графа = "9" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Номер испр. корр. с/ф""';uk='""Номер испр. корр. с/ф""'"));
			ИначеЕсли Графа = "10" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Дата испр. корр. с/ф""';uk='""Дата испр. корр. с/ф""'"));
			ИначеЕсли Графа = "11" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Номер док.оплаты""';uk='""Номер док.оплаты""'"));
			ИначеЕсли Графа = "12" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Дата док.оплаты""';uk='""Дата док.оплаты""'"));
			ИначеЕсли Графа = "13" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Дата принятия на учет""';uk='""Дата принятия на учет""'"));
			ИначеЕсли Графа = "14" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""ИНН продавца""';uk='""ИНН продавца""'"));
			ИначеЕсли Графа = "15" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""ИНН посредника""';uk='""ИНН посредника""'"));
			ИначеЕсли Графа = "16" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Номер тамож.декларации""';uk='""Номер тамож.декларации""'"));
			ИначеЕсли Графа = "17" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Код валюты""';uk='""Код валюты""'"));
			ИначеЕсли Графа = "18" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Стоимость""';uk='""Стоимость""'"));
			ИначеЕсли Графа = "19" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Сумма НДС к вычету""';uk='""Сумма НДС к вычету""'"));
			КонецЕсли;
		КонецЦикла;
		ГрафыОшибки = СтрСоединить(МассивПредставлений, ", ");
		Если МассивПредставлений.Количество()> 1 Тогда
			СообщениеОбОшибке = НСтр("ru='Ошибка в графах %1';uk='Ошибка в графах %1'");
		Иначе
			СообщениеОбОшибке = НСтр("ru='Ошибка в графе %1';uk='Ошибка в графе %1'");
		КонецЕсли;
	...
	ИначеЕсли КодОшибки = 4 Тогда
		...
		Для Каждого Графа Из МассивГраф Цикл
			...
			ИначеЕсли Графа = "10" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Дата испр. корр. с/ф""';uk='""Дата испр. корр. с/ф""'"));
			ИначеЕсли Графа = "11" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""ИНН покупателя""';uk='""ИНН покупателя""'"));
			ИначеЕсли Графа = "12" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""ИНН посредника""';uk='""ИНН посредника""'"));
			ИначеЕсли Графа = "13" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Номер док.оплаты""';uk='""Номер док.оплаты""'"));
			...
			ИначеЕсли Графа = "15" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Код валюты""';uk='""Код валюты""'"));
			ИначеЕсли Графа = "16" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Стомость (всего)""';uk='""Стомость (всего)""'"));
			ИначеЕсли Графа = "17" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Стоимость (НДС 18%)""';uk='""Стоимость (НДС 18%)""'"));
			ИначеЕсли Графа = "18" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Стоимость (НДС 10%)""';uk='""Стоимость (НДС 10%)""'"));
			ИначеЕсли Графа = "19" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Стоимость (НДС 0%)""';uk='""Стоимость (НДС 0%)""'"));
			ИначеЕсли Графа = "20" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""НДС 18%""';uk='""НДС 18%""'"));
			ИначеЕсли Графа = "21" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""НДС 10%""';uk='""НДС 10%""'"));
			ИначеЕсли Графа = "22" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Стоимость (без НДС)""';uk='""Стоимость (без НДС)""'"));
			КонецЕсли;
		КонецЦикла;
		ГрафыОшибки = СтрСоединить(МассивПредставлений, ", ");
		Если МассивПредставлений.Количество()> 1 Тогда
	...
	ИначеЕсли КодОшибки = 4 Тогда
		...
		Для Каждого Графа Из МассивГраф Цикл
			...
			ИначеЕсли Графа = "11" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""ИНН покупателя""';uk='""ИНН покупателя""'"));
			ИначеЕсли Графа = "12" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""ИНН субкомиссионера""';uk='""ИНН субкомиссионера""'"));
			ИначеЕсли Графа = "13" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Код вида сделки""';uk='""Код вида сделки""'"));
			ИначеЕсли Графа = "14" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Код валюты""';uk='""Код валюты""'"));
			ИначеЕсли Графа = "15" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Стоимость (всего)""';uk='""Стоимость (всего)""'"));
			ИначеЕсли Графа = "16" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Сумма НДС""';uk='""Сумма НДС""'"));
			ИначеЕсли Графа = "17" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Увеличение стоимости""';uk='""Увеличение стоимости""'"));
			ИначеЕсли Графа = "18" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Уменьшение стоимости""';uk='""Уменьшение стоимости""'"));
			ИначеЕсли Графа = "19" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Увеличение суммы НДС""';uk='""Увеличение суммы НДС""'"));
			ИначеЕсли Графа = "20" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Уменьшение суммы НДС""';uk='""Уменьшение суммы НДС""'"));
			КонецЕсли;
		КонецЦикла;
		ГрафыОшибки = СтрСоединить(МассивПредставлений, ", ");
		Если МассивПредставлений.Количество()> 1 Тогда
	...
	ИначеЕсли КодОшибки = 4 Тогда
		...
		Для Каждого Графа Из МассивГраф Цикл
			...
			ИначеЕсли Графа = "12" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""ИНН продавца""';uk='""ИНН продавца""'"));
			ИначеЕсли Графа = "13" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Номер с/ф продавца""';uk='""Номер с/ф продавца""'"));
			ИначеЕсли Графа = "14" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Дата с/ф продавца""';uk='""Дата с/ф продавца""'"));
			ИначеЕсли Графа = "15" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Код валюты""';uk='""Код валюты""'"));
			ИначеЕсли Графа = "16" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Стоимость""';uk='""Стоимость""'"));
	...
	ИначеЕсли КодОшибки = 4 Тогда
		...
		Для Каждого Графа Из МассивГраф Цикл
			...
			ИначеЕсли Графа = "6" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Сумма НДС""';uk='""Сумма НДС""'"));
			ИначеЕсли Графа = "7" Тогда
				МассивПредставлений.Добавить(НСтр("ru='""Стомоисть всего""';uk='""Стомоисть всего""'"));
			КонецЕсли;
		КонецЦикла;
		ГрафыОшибки = СтрСоединить(МассивПредставлений, ", ");
		Если МассивПредставлений.Количество()> 1 Тогда
