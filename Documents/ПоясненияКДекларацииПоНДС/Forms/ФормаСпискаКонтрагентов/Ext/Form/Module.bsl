...
Функция ПроверитьЗаполнениеНаКлиенте()
	...
	Для Каждого Контрагент Из Контрагенты Цикл
		Если НЕ ЗначениеЗаполнено(Контрагент.ИНН) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ru='ИНН';uk='ІПН'"));
		...
		ИначеЕсли НЕ (СтрДлина(Контрагент.ИНН) = 10 ИЛИ СтрДлина(Контрагент.ИНН) = 12) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Корректность", НСтр("ru='ИНН';uk='ІПН'"), , , НСтр("ru='Длина ИНН должна быть 10 или 12 символов';uk='Длина ИНН должна быть 10 или 12 символов'"));
			Поле = "Контрагенты["+ Формат(Контрагенты.Индекс(Контрагент), "ЧДЦ=; ЧГ=0") +"].ИНН";
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , Поле, "", Отказ);
		КонецЕсли;
	КонецЦикла;
