...
Процедура ПредставлениеДекларацииОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	ИначеЕсли Найти(НавигационнаяСсылка, "Выбрать файл декларации по НДС") > 0 Тогда
...
Процедура КнигаПокупокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	//ИначеЕсли Поле.Имя = "КнигаПокупокНомерТаможеннойДекларацииРасхождение" Тогда
	//	СтандартнаяОбработка = Ложь;
...
"Изменения данного документа будут потеряны.';uk='Перезаполнить пояснение без учета ранее отправленных пояснений?"
...
Процедура ПодготовитьФормуНаСервере()
	...
	ПроверитьСостояниеОтпрвкиПояснения();
...
&НаСервере
Процедура ПроверитьСостояниеОтпрвкиПояснения()
...
Процедура УправлениеДекларацией(Форма)
	...
	Если ЗначениеЗаполнено(Декларация) И ТипЗнч(Декларация) <> Тип("Строка") Тогда
		
		// Мы нашли декларацию в базе
		Если ТипЗнч(Декларация) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда 
			
			ПредставлениеДекларации = РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(Декларация);
		...
		Форма.ПредставлениеДекларации = Новый ФорматированнаяСтрока(ПредставлениеДекларации,,,, "Декларация");
	...
	Иначе
		...
		Иначе
			...
			ПредставлениеДекларации = НСтр("ru='Декларация не найдена';uk='Декларация не найдена'");
		...
		Если НЕ Форма.ПояснениеОтправлено Тогда
			Подстроки.Добавить(Новый ФорматированнаяСтрока("Выбрать файл декларации по НДС",,,, "Выбрать файл декларации по НДС"));
		КонецЕсли;
...
Процедура УстановитьВидимостьТабличныхЧастей()
	...
	Для Каждого ТабличнаяЧасть Из ТабличныеЧасти Цикл
		...
		Если КоличествоСтрок > 0 Тогда
			...
			ТабличнаяЧастьСоСтроками = ТабличнаяЧасть.Ключ;
		КонецЕсли;
	КонецЦикла;
	
	Элементы.Страницы.Видимость = КоличествоЗаполненныхТабличныхЧастей > 0;
	Элементы.ДекорацияНаВсеСтрокиТребованияУжеБылДанОтвет.Видимость = КоличествоЗаполненныхТабличныхЧастей = 0;
	...
	КонецЕсли;
	Элементы.ФормаОтправить.Доступность = КоличествоЗаполненныхТабличныхЧастей > 0;
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц()
	...
	ТаблицаИНН = Объект.Контрагенты.Выгрузить(, "ИНН");
	
	// Дополняем таблицу с ИНН значениями, указанными в колонках табличных частей
	ДополнитьТаблицуИНН(ТаблицаИНН, Объект.КнигаПокупок, "ПосредникИНН");
	ДополнитьТаблицуИНН(ТаблицаИНН, Объект.КнигаПокупок, "ПосредникИННРасхождение");
	ДополнитьТаблицуИНН(ТаблицаИНН, Объект.КнигаПокупокДл, "ПосредникИНН");
	ДополнитьТаблицуИНН(ТаблицаИНН, Объект.КнигаПокупокДл, "ПосредникИННРасхождение");
	ДополнитьТаблицуИНН(ТаблицаИНН, Объект.КнигаПродаж, "ПосредникИНН");
	ДополнитьТаблицуИНН(ТаблицаИНН, Объект.КнигаПродаж, "ПосредникИННРасхождение");
	ДополнитьТаблицуИНН(ТаблицаИНН, Объект.КнигаПродажДл, "ПосредникИНН");
	ДополнитьТаблицуИНН(ТаблицаИНН, Объект.КнигаПродажДл, "ПосредникИННРасхождение");
	ДополнитьТаблицуИНН(ТаблицаИНН, Объект.ЖурналУчетаВыставленныхСчетовФактур, "ПокупательИНН");
	ДополнитьТаблицуИНН(ТаблицаИНН, Объект.ЖурналУчетаВыставленныхСчетовФактур, "ПокупательИННРасхождение");
	ДополнитьТаблицуИНН(ТаблицаИНН, Объект.ЖурналУчетаПолученныхСчетовФактур, "ПродавецИНН");
	ДополнитьТаблицуИНН(ТаблицаИНН, Объект.ЖурналУчетаПолученныхСчетовФактур, "ПродавецИННРасхождение");
	ДополнитьТаблицуИНН(ТаблицаИНН, Объект.ЖурналУчетаПолученныхСчетовФактур, "СубкомиссионерИНН");
	ДополнитьТаблицуИНН(ТаблицаИНН, Объект.ЖурналУчетаПолученныхСчетовФактур, "СубкомиссионерИННРасхождение");
	ДополнитьТаблицуИНН(ТаблицаИНН, Объект.ВыставленныеСчетаФактурыНеплательщиковНДС, "ПокупательИНН");
	ДополнитьТаблицуИНН(ТаблицаИНН, Объект.ВыставленныеСчетаФактурыНеплательщиковНДС, "ПокупательИННРасхождение");
	...
	Для Каждого СведениеОКонтрагенте ИЗ СведенияОКонтрагентах Цикл
		...
		Если ТаблицаКонтрагентов <> Неопределено Тогда
			СтрокиКонтрагента = ТаблицаКонтрагентов.НайтиСтроки(Новый Структура("ИНН", СведениеОКонтрагенте.ИНН));
...
Процедура УстановитьПредставлениеСчетаФактуры(СтрокаТаблицы, ТипРеквизитов)
	...
	Если ЗначениеЗаполнено(СтрокаТаблицы["НомерКорректировочногоСчетаФактуры"+ТипРеквизитов]) Тогда
		Если ЗначениеЗаполнено(СтрокаТаблицы["НомерИсправленияКорректировочногоСчетаФактуры"+ТипРеквизитов]) Тогда
			РеквизитыКоррСчетаФактуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Корр. %1 от %2 (испр.%3 от %4) по с/ф ';uk='Корр. %1 от %2 (испр.%3 от %4) по с/ф '"),
		...
		Иначе
			РеквизитыКоррСчетаФактуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Корр. %1 от %2 по с/ф ';uk='Корр. %1 от %2 по с/ф '"),
									СтрокаТаблицы["НомерКорректировочногоСчетаФактуры"+ТипРеквизитов],
									Формат(СтрокаТаблицы["ДатаКорректировочногоСчетаФактуры"+ТипРеквизитов], "ДЛФ=D"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтрокаТаблицы["НомерСчетаФактуры"+ТипРеквизитов]) Тогда
		Если ЗначениеЗаполнено(СтрокаТаблицы["НомерИсправленияСчетаФактуры"+ТипРеквизитов]) Тогда
			РеквизитыСчетаФактуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='%1 от %2(испр.%3 от %4)';uk='%1 от %2(испр.%3 от %4)'"),
	...
	СтрокаТаблицы["РеквизитыСчетаФактуры"+ТипРеквизитов] = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																НСтр("ru='%1%2';uk='%1%2'"), РеквизитыКоррСчетаФактуры, РеквизитыСчетаФактуры);
...
&НаСервере
Процедура УстановитьДанныеКонтрагента(СтрокаТаблицы, ПолеИНН, ТаблицаКонтрагентов)
	
	СведенияОКонтрагенте = Новый Структура(РеквизитыКонтрагентов);
	СведенияОКонтрагенте.ИНН = СтрокаТаблицы[ПолеИНН];
	
	Сведения = Документы.ПоясненияКДекларацииПоНДС.СведенияОКонтрагентеПоТаблицеКонтрагентов(СведенияОКонтрагенте.ИНН, ТаблицаКонтрагентов);
...
&НаСервере
Процедура ДополнитьТаблицуИНН(ТаблицаИНН, ТаблицаДанных, ПолеИНН)
	
	Для Каждого СтрокаТаблицыДанных Из ТаблицаДанных Цикл
		СтрокаИНН = ТаблицаИНН.Добавить();
...
Процедура ОткрытьФормуСчетаФактуры(ИмяТабличнойЧасти, ДанныеСтроки, ТипРеквизитов)
	...
	Если ПоискВыполнен Тогда
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ru='Счет-фактура не найден';uk='Счет-фактура не найден'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьСведенияОКонтрагентеРеквизита(ДанныеСтроки, ИмяРеквизита, Сведения)
	...
	ДанныеСтроки[ИмяРеквизита]                 = Сведения.ИНН;
...
&НаКлиенте
Процедура ВыбратьКонтрагента(ИмяТабличнойЧасти, ДанныеСтроки, ИмяРеквизита, ПолеСпискаЗначений)
	...
	ДополнительныеПараметры.Вставить("ПолеСпискаЗначений",  ПолеСпискаЗначений);
...
Процедура ВыбратьКонтрагентаЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	Если ЗначениеЗаполнено(РезультатЗакрытия) Тогда
		...
		Если ДополнительныеПараметры.ПолеСпискаЗначений Тогда
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект[ИмяТабличнойЧасти] Цикл
		Если СтрокаТабличнойЧасти.РезультатПроверки = Перечисления.РезультатПроверкиСтрокиДекларации.ВыявленыРасхождения Тогда
			...
			Если РасхожденийНеНайдено Тогда
				ТекстСообщения = НСтр("ru='В строке %1 %2 указан ответ ""Найдены расхождения"", но уточненные сведения соответствуют данным ФНС';uk='В строке %1 %2 указан ответ ""Найдены расхождения"", но уточненные сведения соответствуют данным ФНС'");
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , ИмяТабличнойЧасти, "Объект", Отказ);
			КонецЕсли;
...
Функция ИдДекларации(АдресДанных, ВыбранноеИмяФайла)
	...
	Если КонтекстЭДОСервер.ЭтоФайлВыгрузкиДекларацияНДС(СвойстваФайла.Имя) Тогда
		...
		Если НРег(СвойстваФайла.Расширение) = НРег(".xml") Тогда
			...
			Исключение
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Файл ""%1"" не является файлом выгрузки в формате XML.';uk='Файл ""%1"" не является файлом выгрузки в формате XML.'"), СвойстваФайла.Имя);
			...
			Если КНД = Неопределено ИЛИ КНД = "" ИЛИ КНД <> "1151001" Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Выбранный файл не является файлом декларации по НДС';uk='Выбранный файл не является файлом декларации по НДС'"));
				Возврат Неопределено;
			КонецЕсли;
			...
			Если ОрганизацияВФайле <> Объект.Организация Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Организация в выбранном файле декларации по НДС не совпадает с организацией, указанной в требовании';uk='Организация в выбранном файле декларации по НДС не совпадает с организацией, указанной в требовании'"));
				Возврат Неопределено;
			КонецЕсли;
			...
			Если НомКорр <> СвойстваДекларации.ТребованиеНомКорр Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Номер корректировки в выбранном файле декларации по НДС не совпадает с номером корректировки, указанном в требовании';uk='Номер корректировки в выбранном файле декларации по НДС не совпадает с номером корректировки, указанном в требовании'"));
				Возврат Неопределено;
			КонецЕсли;
			
			Период = ПоказателиВыгрузки["Период"];
			Если Период <> СвойстваДекларации.ТребованиеПериод Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Период в выбранном файле декларации по НДС не совпадает с периодом, указанном в требовании';uk='Период в выбранном файле декларации по НДС не совпадает с периодом, указанном в требовании'"));
				Возврат Неопределено;
			КонецЕсли;
			
			ОтчетГод = ПоказателиВыгрузки["ОтчетГод"];
			Если ОтчетГод <> СвойстваДекларации.ТребованиеОтчетГод Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Год в выбранном файле декларации по НДС не совпадает с годом, указанном в требовании';uk='Год в выбранном файле декларации по НДС не совпадает с годом, указанном в требовании'"));
				Возврат Неопределено;
			КонецЕсли;
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Файл декларации по НДС должен иметь расширение xml';uk='Файл декларации по НДС должен иметь расширение xml'"));
			Возврат Неопределено;
		КонецЕсли;
...
Процедура ПослеОтправкиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	КонтекстЭДОКлиент.ПредупредитьЕслиСтатусОтправкиВКонверте(Объект.Ссылка, "пояснение");
