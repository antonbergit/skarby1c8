...
Функция ПроверитьЗаполнениеНаКлиенте()
	...
	Для Каждого Элемент Из ПосредническаяДеятельность Цикл
		Если ЗначениеЗаполнено(Элемент.ПродавецИНН) 
			...
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Корректность", НСтр("ru='ИНН продавца';uk='ИНН продавца'"), , , НСтр("ru='Длина ИНН должна быть 10 или 12 символов';uk='Длина ИНН должна быть 10 или 12 символов'"));
		...
		Если НЕ ЗначениеЗаполнено(Элемент.НомерСчетаФактуры) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ru='Номер с/ф';uk='Номер с/ф'"));
		...
		Если НЕ ЗначениеЗаполнено(Элемент.ДатаСчетаФактуры) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ru='Дата с/ф';uk='Дата с/ф'"));
...
Процедура УстановитьНадписиСтроки(Строка)
	Строка.НадписьСумма = НСтр("ru='Сумма';uk='Сума'");
	Строка.НадписьНДС   = НСтр("ru='в т.ч. НДС';uk='у т.ч. ПДВ'");	
КонецПроцедуры
