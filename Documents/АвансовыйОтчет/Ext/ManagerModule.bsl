...
Процедура СформироватьТаблицаЗапасы(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПрочиеРасходы", НСтр("ru='Прочие расходы';uk='Інші витрати'"));
...
Процедура СформироватьТаблицаРасчетыСПодотчетниками(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПогашениеДолгаПодотчетника", НСтр("ru='Погашение долга подотчетника';uk='Погашення боргу підзвітника'"));
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("КурсоваяРазница", НСтр("ru='Курсовая разница';uk='Курсова різниця'"));
	Запрос.УстановитьПараметр("ОприходованиеЗапасов", НСтр("ru='Оприходование запасов';uk='Оприбуткування запасів'"));
	Запрос.УстановитьПараметр("ОплатаПоставщику", НСтр("ru='Оплата поставщику';uk='Оплата постачальнику'"));
	Запрос.УстановитьПараметр("ПрочиеРасходы", НСтр("ru='Прочие расходы';uk='Інші витрати'"));
...
Процедура СформироватьТаблицаКнигаДоходовРасходовПоЕН(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства)
	...
	|	&Организация КАК Организация,
	|	ТаблицаДокумента.Период КАК Период,
	|	ТаблицаДокумента.СтатьяДекларацииПоЕдиномуНалогу КАК Статья,
	|	СУММА(ТаблицаДокумента.Сумма) КАК Сумма
	|ИЗ
	|	ВременнаяТаблицаОплаты КАК ТаблицаДокумента
	|ГДЕ
	|	ТаблицаДокумента.СтатьяДекларацииПоЕдиномуНалогу <> ЗНАЧЕНИЕ(Справочник.СтатьиНалоговыхДеклараций.ПустаяСсылка)
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаДокумента.Период,
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства) Экспорт
	...
	Если ДокументСсылкаАвансовыйОтчет.ВыданныеАвансы.Количество() > 0 Тогда
	...
	Если ДокументСсылкаАвансовыйОтчет.УчитыватьВНУ Тогда
		СформироватьТаблицаКнигаДоходовРасходовПоЕН(ДокументСсылкаАвансовыйОтчет, СтруктураДополнительныеСвойства);
	КонецЕсли;
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ИмяМакета = "АвансовыйОтчет")
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		|		Контрагент
		|	),
		|	АвансовыйОтчет.Сотрудник.СчетРасчетовСПодотчетниками.Код КАК СчетРасчетовСПодотчетниками,
		|	АвансовыйОтчет.Организация.ЮридическоеФизическоеЛицо КАК ОрганизацияЮрФизЛицо,
		|	АвансовыйОтчет.Организация.КодПоЕДРПОУ,
		|	АвансовыйОтчет.Сотрудник.Физлицо.КодПоДРФО КАК ДРФОПодотчетногоЛица,
		...
		Вариант2014 = Ложь;
		Если ИмяМакета = "АвансовыйОтчет2014" Тогда
			Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.АвансовыйОтчет.ПФ_MXL_АвансовыйОтчет2014");
			Вариант2014 = Истина;
		КонецЕсли; 
		Если ИмяМакета = "АвансовыйОтчет2015" Тогда
			Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.АвансовыйОтчет.ПФ_MXL_АвансовыйОтчет2015");
			Вариант2014 = Истина;
		КонецЕсли; 
		Если ИмяМакета = "АвансовыйОтчет2016" Тогда
			Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.АвансовыйОтчет.ПФ_MXL_АвансовыйОтчет2016");
			Вариант2014 = Истина;
		КонецЕсли; 
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	СУММА(РасходИзКассы.СуммаДокумента) КАК СуммаДокумента
		|ИЗ
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	СУММА(РасходСоСчета.СуммаДокумента) КАК СуммаДокумента
		|ИЗ
		...
		Если РезультатЗапроса.Пустой() Тогда
			...
			ПодотчетноеЛицоКратко = Шапка.ПодотчетноеЛицо;
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ДанныеПодотчетногоЛица.Фамилия) Тогда
				ПодотчетноеЛицоКратко = УправлениеНебольшойФирмойСервер.ПолучитьФамилиюИмяОтчество(ДанныеПодотчетногоЛица.Фамилия, ДанныеПодотчетногоЛица.Имя, ДанныеПодотчетногоЛица.Отчество, Истина);
			Иначе
				ПодотчетноеЛицоКратко = ДанныеПодотчетногоЛица.Представление;
			КонецЕсли;
			ПодотчетноеЛицо   = ПодотчетноеЛицоКратко;
		...
		ЕДРПОУОрганизации = Шапка.ОрганизацияКодПоЕДРПОУ;
		...
		Если Шапка.ОрганизацияЮрФизЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		...
		ОбластьМакета.Параметры.ФИОПодотчетногоЛицаКратко              = ПодотчетноеЛицоКратко;
		ОбластьМакета.Параметры.ФИОПодотчетногоЛица                    = ПодотчетноеЛицо;										
		...
		ОбластьМакета.Параметры.СуммаОтчетПодтверждаю = РаботаСКурсамиВалют.СформироватьСуммуПрописью(Шапка.СуммаДокумента, Шапка.ВалютаДокумента,,КодЯзыкаПечать);
		
		КоличествоДокументов = ?(ВыданныеАвансы.Количество()>3, 3, ВыданныеАвансы.Количество());
		Для Инд = 1 По КоличествоДокументов Цикл
			
			ДокументАванса = ВыданныеАвансы[Инд-1].Документ;
			
			Если ДокументАванса.Метаданные().Имя = "РасходИзКассы" Тогда
				Документ = "Видатковий касовий ордер №" + Строка(ДокументАванса.НомерОрдера) + " від " 
				+ Строка(Формат(ДокументАванса.Дата, "ДФ=dd.MM.yyyy"));
				
			ИначеЕсли ДокументАванса.Метаданные().Имя = "РасходСоСчета" Тогда
		...
		СведенияОбОрганизации = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Компания, Шапка.ДатаДокумента, , , КодЯзыкаПечать);
		...
		Кассир = Руководители.РуководительДолжность;
		...
		Пока ВыборкаПроводок.Следующий() Цикл
			...
			Если Вариант2014 Тогда
				
				ИспСумма     = Окр(?(ВыборкаПроводок.Сумма = Null, 0, ВыборкаПроводок.Сумма), 2, 1);
				ЦелСумма     = Цел(?(ВыборкаПроводок.Сумма = Null, 0, ВыборкаПроводок.Сумма));		
				КопСумма     = 100 * (Окр(ИспСумма - ЦелСумма, 2, 1));
				...
				ОбластьМакета.Параметры["СчетДт"  + (Счетчик + 1)] = ВыборкаПроводок.СчетДт;			
				ОбластьМакета.Параметры["СчетКт" + (Счетчик + 1)] = ВыборкаПроводок.СчетКт;			
				ОбластьМакета.Параметры["СуммаБезКопеек" 	+ (Счетчик + 1)] = ЦелСумма;
				ОбластьМакета.Параметры["СуммаКопейки"   	+ (Счетчик + 1)] = КопСумма;
				
				СуммаСНДСЧислом = СуммаСНДСЧислом + ИспСумма;
		...
		ОбластьМакета.Параметры.СуммаСНДСЧислом = СуммаСНДСЧислом;
		...
		Пока Запасы.Следующий() Цикл
			...
			ОбластьМакета = ?((Шапка.НДСВключатьВСтоимость ИЛИ (Запасы.СуммаНДС = 0)), Макет.ПолучитьОбласть("Строка"), Макет.ПолучитьОбласть("СтрокаСНДС"));
			
			ОбластьМакета.Параметры.Заполнить(Запасы);
			НомерСтроки = НомерСтроки + 1;
			ОбластьМакета.Параметры.НомерСтроки         = НомерСтроки;			
			ОбластьМакета.Параметры.ОснованиеПлатежа = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(
			...
			Если     (Запасы.СуммаНДС > 0) Тогда
				
				ОбластьМакета.Параметры.СуммаБезНДС = Запасы.Сумма - ?(Шапка.СуммаВключаетНДС, Запасы.СуммаНДС, 0);
				ОбластьМакета.Параметры.СуммаНДС = Запасы.СуммаНДС;
			Иначе	
				ОбластьМакета.Параметры.СуммаБезНДС = Запасы.Сумма + ?(Шапка.СуммаВключаетНДС, 0, Запасы.СуммаНДС);
			КонецЕсли;
		...
		Пока Расходы.Следующий() Цикл
			...
			ОбластьМакета = ?((Шапка.НДСВключатьВСтоимость ИЛИ (Расходы.СуммаНДС = 0)), Макет.ПолучитьОбласть("Строка"), Макет.ПолучитьОбласть("СтрокаСНДС"));
		
			ОбластьМакета.Параметры.Заполнить(Расходы);
			НомерСтроки = НомерСтроки + 1;
			ОбластьМакета.Параметры.НомерСтроки         = НомерСтроки;
			Если ЗначениеЗаполнено(Расходы.Содержание) Тогда
				ОбластьМакета.Параметры.ОснованиеПлатежа = Расходы.Содержание;
			Иначе
				ОбластьМакета.Параметры.ОснованиеПлатежа = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(
			...
			Если     (Расходы.СуммаНДС > 0) Тогда
				
				ОбластьМакета.Параметры.СуммаБезНДС = Расходы.Сумма - ?(Шапка.СуммаВключаетНДС, Расходы.СуммаНДС, 0);
				ОбластьМакета.Параметры.СуммаНДС = Расходы.СуммаНДС;
			Иначе	
				ОбластьМакета.Параметры.СуммаБезНДС = Расходы.Сумма + ?(Шапка.СуммаВключаетНДС, 0, Расходы.СуммаНДС);
			КонецЕсли;
		...
		Пока Оплаты.Следующий() Цикл
			...
			ОбластьМакета.Параметры.ОснованиеПлатежа = НСтр("ru='Оплата контрагенту ';uk='Оплата контрагентові'") + Оплаты.Контрагент;
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АвансовыйОтчет") Тогда
		Если МассивОбъектов[0].Дата > Дата("20140204000000") Тогда
			Если МассивОбъектов[0].Дата > Дата("20160419000000") Тогда
				УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "АвансовыйОтчет", "Авансовый отчет", ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "АвансовыйОтчет2016"));
			ИначеЕсли МассивОбъектов[0].Дата > Дата("20151106000000") Тогда
				УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "АвансовыйОтчет", "Авансовый отчет", ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "АвансовыйОтчет2015"));
			Иначе	
				УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "АвансовыйОтчет", "Авансовый отчет", ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "АвансовыйОтчет2014"));
			КонецЕсли; 
		Иначе
			УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "АвансовыйОтчет", "Авансовый отчет", ПечатнаяФорма(МассивОбъектов, ОбъектыПечати));
		КонецЕсли;
	КонецЕсли;
