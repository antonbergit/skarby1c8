...
Процедура КомандаЗаполнитьТолькоУчетныеДанные()
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьТолькоУчетныеДанныеЗавершение", ЭтотОбъект), НСтр("ru='Учетные данные будут очищены! Продолжить выполнение операции?';uk='Облікові дані будуть очищені! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
        Возврат; 
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура КомандаЗаполнитьПоВидуЦен(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("КомандаЗаполнитьПоВидуЦенЗавершение1", ЭтотОбъект), НСтр("ru='Цены будут перезаполнены! Продолжить выполнение операции?';uk='Ціни будуть перезаповнені! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура КомандаОбнулитьКоличествоИСумму(Команда)
	...
	Если Объект.Запасы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("КомандаОбнулитьКоличествоИСуммуЗавершение", ЭтотОбъект), НСтр("ru='Колонки ""Количество"" и ""Сумма"" будут очищены! Продолжить выполнение операции?';uk='Стовпчики ""Кількість"" і ""Сума"" будуть очищені! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
        Возврат;
	Иначе
		Возврат;
	КонецЕсли;
...
Функция СформироватьЗаголовокОтбораИзСписка(СписокЭлементов)
	...
	Если ЗаголовокОтбораСтрока = "" Тогда
		ЗаголовокОтбораСтрока = НСтр("ru='Отбор не установлен';uk='Відбір не встановлений'");
	КонецЕсли;
...
Процедура ОчиститьУсловиеОтбораПоНоменклатуре()
	...
	Элементы.УстановитьОтборПоНоменклатуре.Заголовок = НСтр("ru='Отбор не установлен';uk='Відбір не встановлений'");
...
Процедура ОчиститьУсловиеОтбораПоГруппамНоменклатуры()
	...
	Элементы.УстановитьОтборПоГруппамНоменклатуры.Заголовок = НСтр("ru='Отбор не установлен';uk='Відбір не встановлений'");
...
Процедура ОчиститьУсловиеОтбораПоКатегориямНоменклатуры()
	...
	Элементы.УстановитьОтборПоКатегориямНоменклатуры.Заголовок = НСтр("ru='Отбор не установлен';uk='Відбір не встановлений'");
