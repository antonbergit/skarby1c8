...
Процедура ПриКопировании(ОбъектКопирования)
	...
	Если ТипСобытия = Перечисления.ТипыСобытий.ЭлектронноеПисьмо
		...
		ВызватьИсключение НСтр("ru='Копирование входящего письма невозможно.';uk='Копіювання вхідного листа неможливо.'");
	КонецЕсли;
...
Процедура ОтправитьЭлектронноеПисьмо(Записывать = Истина, Знач ВложенияПисьма = Неопределено) Экспорт
	...
	Если ЭтотОбъект.ТипСобытия <> Перечисления.ТипыСобытий.ЭлектронноеПисьмо Тогда
		ВызватьИсключение НСтр("ru='Отправляемое письмо должно иметь тип события ""Электронное письмо""';uk='Лист, що відправляється, повинен мати тип події ""Електронний лист""'");
	КонецЕсли;
	...
	Исключение
		ВызватьИсключение НСтр("ru='Некорректный адрес получателя для отправляемого письма';uk='Некоректна адреса одержувача для листа, що відправляється'") + ":" + Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	...
	Исключение
		ВызватьИсключение НСтр("ru='Не удалось выполнить отправку письма по причине';uk='Не вдалося виконати відправку листа з причини'") + ":" + Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
...
Процедура ОтправитьSMS(Записывать = Истина) Экспорт
	...
	Если ЭтотОбъект.ТипСобытия <> Перечисления.ТипыСобытий.СообщениеSMS Тогда
		ВызватьИсключение НСтр("ru='Отправляемое сообщение SMS должно иметь тип события ""SMS""';uk='Повідомлення SMS, що відправляється, повинне мати тип події ""SMS""'");
	КонецЕсли;
	...
	Для Каждого Получатель Из ЭтотОбъект.Участники Цикл
		...
		Иначе
			ВызватьИсключение НСтр("ru='Неверный формат номера в строке %1 списка получателей';uk='Невірний формат номера в рядку %1 списку одержувачів'") + ":" + Символы.ПС + РезультатПроверки.СообщениеОбОшибке;
		КонецЕсли;
	КонецЦикла;
	...
	Если Не ПустаяСтрока(РезультатОтправки.ОписаниеОшибки) Тогда
		ВызватьИсключение НСтр("ru='Не удалось выполнить отправку SMS по причине';uk='Не вдалося виконати відправку SMS з причини'") + ":" + Символы.ПС + РезультатОтправки.ОписаниеОшибки;
	КонецЕсли;
...
Процедура ЗаполнитьПоКонтрагент(Контрагент)
	...
	Если Контрагент.ЭтоГруппа Тогда
		ВызватьИсключение НСтр("ru='Нельзя выбирать группу контрагентов.';uk='Не можна вибирати групу контрагентів.'");
	КонецЕсли;
...
Функция ДопустимыйТипДляВводаНаОсновании(ДокОснование)
	...
	Если ТипЗнч(ДокОснование) = Тип("ДокументСсылка.ЗаказПоставщику")
		ИЛИ ТипЗнч(ДокОснование) = Тип("ДокументСсылка.ПриходнаяНакладная")
		ИЛИ ТипЗнч(ДокОснование) = Тип("ДокументСсылка.РасходнаяНакладная")
		ИЛИ ТипЗнч(ДокОснование) = Тип("ДокументСсылка.СчетНаОплату")
		ИЛИ ТипЗнч(ДокОснование) = Тип("ДокументСсылка.СчетНаОплатуПоставщика")
		ИЛИ ТипЗнч(ДокОснование) = Тип("ДокументСсылка.РасходСоСчета")
		ИЛИ ТипЗнч(ДокОснование) = Тип("ДокументСсылка.РасходИзКассы")
		ИЛИ ТипЗнч(ДокОснование) = Тип("ДокументСсылка.ПоступлениеНаСчет")
		ИЛИ ТипЗнч(ДокОснование) = Тип("ДокументСсылка.ПоступлениеВКассу")
		ИЛИ ТипЗнч(ДокОснование) = Тип("ДокументСсылка.ПлатежноеПоручение")
		ИЛИ ТипЗнч(ДокОснование) = Тип("ДокументСсылка.ОтчетКомиссионера")
		ИЛИ ТипЗнч(ДокОснование) = Тип("ДокументСсылка.ОтчетКомитенту")
		ИЛИ ТипЗнч(ДокОснование) = Тип("ДокументСсылка.ОтчетОПереработке")
		ИЛИ ТипЗнч(ДокОснование) = Тип("ДокументСсылка.ОтчетПереработчика")
		ИЛИ ТипЗнч(ДокОснование) = Тип("ДокументСсылка.ОперацияПоПлатежнымКартам") // Эквайринг
		ИЛИ ТипЗнч(ДокОснование) = Тип("ДокументСсылка.АктВыполненныхРабот") Тогда
