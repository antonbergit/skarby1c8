...
Процедура УстановитьОтборТекущиеДела()
	...
	АвтоЗаголовок = Ложь;
	Заголовок = НСтр("ru='События';uk='Події'");
...
Процедура УстановитьОтборТипСобытия()
	...
	Если Параметры.ТипСобытия = Перечисления.ТипыСобытий.ТелефонныйЗвонок Тогда
		Заголовок = НСтр("ru='События: телефонные звонки';uk='Події: телефонні дзвінки'");
	ИначеЕсли Параметры.ТипСобытия = Перечисления.ТипыСобытий.ЭлектронноеПисьмо Тогда
		Заголовок = НСтр("ru='События: электронные письма';uk='Події: електронні листи'");
	ИначеЕсли Параметры.ТипСобытия = Перечисления.ТипыСобытий.СообщениеSMS Тогда
		Заголовок = НСтр("ru='События: сообщения SMS';uk='Події: повідомлення SMS'");
	ИначеЕсли Параметры.ТипСобытия = Перечисления.ТипыСобытий.ЛичнаяВстреча Тогда
		Заголовок = НСтр("ru='События: личные встречи';uk='Події: особисті зустрічі'");
	ИначеЕсли Параметры.ТипСобытия = Перечисления.ТипыСобытий.Прочее Тогда
		Заголовок = НСтр("ru='События: прочие';uk='Події: інші'");
	КонецЕсли;
...
Процедура НастроитьГруппуПочтоваяСлужба()
	...
	Если ПочтовыеСлужбыПодключены Тогда
		...
		Если ЗначениеЗаполнено(КонтекстКонтакт) Тогда
			Команды.ПроверитьПочту.Заголовок = НСтр("ru='Загрузить переписку';uk='Завантажити переписку'");
			Команды.ПроверитьПочту.Подсказка = СтрШаблон(НСтр("ru='Загрузить переписку с ""%1""';uk='Завантажити переписку з ""%1""'"), КонтекстКонтакт);
...
Функция ЗаголовокПрогресса(Режим, Контакт)
	...
	Если ЗначениеЗаполнено(Контакт) Тогда
		Возврат НСтр("ru='Загрузка переписки';uk='Завантаження листування'");
	КонецЕсли;
	
	Если Режим = ЭлектроннаяПочтаУНФКлиентСервер.РежимЗагрузкиНовыеСообщения() Тогда
		Возврат НСтр("ru='Проверка почты';uk='Перевірка пошти'");
	КонецЕсли;
	
	Если Режим = ЭлектроннаяПочтаУНФКлиентСервер.РежимЗагрузкиНовыеСообщения() Тогда
		Возврат НСтр("ru='Загрузка предыдущих сообщений';uk='Завантаження попередніх повідомлень'");
	КонецЕсли;
...
Процедура ОбработатьРезультатВыполненияЗаданияНаСервере()
	...
	НСтр("ru='Загружено %1';uk='Завантажено %1'"),
	СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
	ЗагруженоПисем,
	НСтр("ru='письмо,письма,писем';uk='лист, листи, листів'")));
