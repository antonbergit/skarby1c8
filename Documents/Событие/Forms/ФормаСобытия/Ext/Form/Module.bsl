...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		АвтоЗаголовок = Ложь;
		Заголовок = СтрШаблон(
		НСтр("ru='Событие: %1 (создание)';uk='Подія: %1 (створення)'"),
		Объект.ТипСобытия);
...
Процедура КонтактыКонтактНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Контрагент) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Необходимо выбрать контрагента.';uk='Необхідно вибрати контрагента.'"), , "Контрагент");
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоОснованию(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоОснованиюЗавершение", ЭтотОбъект),
		НСтр("ru='Документ будет полностью перезаполнен по ""Основанию""! Продолжить выполнение операции?';uk='Документ буде повністю перезаповнений по ""Підставі""! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура ЗаполнитьПоКонтрагенту(Команда)
	...
	Если Контакты.Количество() > 0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоКонтрагентуЗавершение", ЭтотОбъект),
			НСтр("ru='Контакты будут полностью перезаполнены по контрагенту! Продолжить выполнение операции?';uk='Контакти будуть повністю перезаповнені по контрагенту! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
	Иначе
...
Процедура СформироватьПредставлениеПродолжительности(Форма)
	...
	ПодписьМинут = УправлениеНебольшойФирмойКлиентСервер.ФормаМножественногоЧисла(
		НСтр("ru='минута';uk='хвилина'"),
		НСтр("ru='минуты';uk='хвилини'"),
		НСтр("ru='минут';uk='хвилин'"),
		Минут
	);
