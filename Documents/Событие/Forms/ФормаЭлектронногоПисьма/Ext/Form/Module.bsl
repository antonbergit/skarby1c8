...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Иначе // Копирование
			...
			Картинки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.ЗначениеКопирования, "КартинкиHTML").Получить();
	...
	СодержаниеHTML = Гипертекст.ОбработатьТекстHTML(Объект.СодержаниеHTML,, Вложения);
...
Процедура Отправить(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.УчетнаяЗапись)
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Выберите учетную запись для отправки почты.';uk='Виберіть обліковий запис для відправлення пошти.'"), ,
			"Объект.УчетнаяЗапись");
		Возврат;
	КонецЕсли;
	...
	СписокОтветов.Добавить(КодВозвратаДиалога.ОК, НСтр("ru='Продублировать отправку';uk='Продублювати відправку'"));
	СписокОтветов.Добавить(КодВозвратаДиалога.Отмена);
	Если ЗначениеЗаполнено(ИдентификаторСобытия) Тогда
		ПоказатьВопрос(
		ОписаниеОповещения,
		СтрШаблон(НСтр("ru='Это письмо уже отправлялось %1';uk='Цей лист вже відправлявся %1'"), Объект.НачалоСобытия),
		СписокОтветов, 30,
		КодВозвратаДиалога.Отмена,
		НСтр("ru='Это письмо уже отправлялось';uk='Цей лист вже відправлявся'"),
		КодВозвратаДиалога.Отмена)
	Иначе
...
Процедура ЗаполнитьНовоеПисьмоПоУмолчанию()
	...
	АвтоЗаголовок = Ложь;
	Заголовок = НСтр("ru='Исходящее письмо  (создание)';uk='Вихідний лист  (створення)'");
...
Процедура ОбработатьПереданныеПараметры(Параметры)
	...
	ИначеЕсли ТипЗнч(Параметры.Кому) = Тип("Массив") Тогда
		Для Каждого СтруктураПолучателя Из Параметры.Кому Цикл
			НоваяСтрока = Объект.Участники.Добавить();
			НоваяСтрока.Контакт = СтруктураПолучателя.Представление;
...
Функция СформироватьПараметрыПисьмаНаСервере(ПриведенныйПочтовыйАдрес)
	...
	Если ВложенияКартинки.Количество() > 0 Тогда
		ДобавитьВложенияКартинкиНаСервере(ТелоПисьма, ВложенияПисьма, ВложенияКартинки);
	КонецЕсли;
...
Процедура ДобавитьВложенияКартинкиНаСервере(ТекстHTML, ВложенияПисьма, ВложенияКартинки)
	
	УправлениеНебольшойФирмойВзаимодействия.ДобавитьВложенияКартинкиВПисьмо(ТекстHTML, ВложенияПисьма, ВложенияКартинки);
...
&НаСервере
Процедура ПрочитатьВложения(ВладелецФайла)
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрока.Идентификатор = Выборка.ИДФайлаЭлектронногоПисьма;
...
Процедура УстановитьЗаголовокГиперссылкиДокументовОснований()
	...
	Если Объект.ДокументыОснования.Количество() = 0 Тогда
		Элементы.ОткрытьДокументыОснования.Заголовок = НСтр("ru='Список документов оснований';uk='Список документів підстав'");
		Возврат;
	КонецЕсли;
	...
	Элементы.ОткрытьДокументыОснования.Заголовок = СтрШаблон(НСтр("ru='%1 и ещё %2.';uk='%1 і ще %2.'"),
	ПервыйДокумент,
	СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
	Объект.ДокументыОснования.Количество() - 1,
	НСтр("ru='основание,основания,оснований';uk='підстава, підстави, підстав'")));
...
Процедура Ответить(Команда)
	...
	ПараметрыОткрытия.ЗначенияЗаполнения.Вставить("Команда", ЭлектроннаяПочтаУНФКлиентСервер.КомандаОтветить());
	...
	ПараметрыОткрытия.ЗначенияЗаполнения.ПараметрыОснования.Вставить("Содержание", Содержание);
	ПараметрыОткрытия.ЗначенияЗаполнения.ПараметрыОснования.Вставить("СодержаниеHTML", СодержаниеHTML);
...
Процедура Переслать(Команда)
	...
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("ЗначенияЗаполнения", Новый Структура);
	ПараметрыОткрытия.ЗначенияЗаполнения.Вставить("Команда", ЭлектроннаяПочтаУНФКлиентСервер.КомандаПереслать());
	ПараметрыОткрытия.ЗначенияЗаполнения.Вставить("ТипСобытия", ПредопределенноеЗначение("Перечисление.ТипыСобытий.ЭлектронноеПисьмо"));
	ПараметрыОткрытия.ЗначенияЗаполнения.Вставить("ОснованиеЗаполнения", Объект.Ссылка);
	ПараметрыОткрытия.ЗначенияЗаполнения.Вставить("ПараметрыОснования", Новый Структура);
	ПараметрыОткрытия.ЗначенияЗаполнения.ПараметрыОснования.Вставить("НачалоСобытия", Объект.НачалоСобытия);
	ПараметрыОткрытия.ЗначенияЗаполнения.ПараметрыОснования.Вставить("СписокУчастников", "");
	ПараметрыОткрытия.ЗначенияЗаполнения.ПараметрыОснования.Вставить("Тема", Объект.Тема);
	ПараметрыОткрытия.ЗначенияЗаполнения.ПараметрыОснования.Вставить("Содержание", Содержание);
	ПараметрыОткрытия.ЗначенияЗаполнения.ПараметрыОснования.Вставить("СодержаниеHTML", СодержаниеHTML);
...
Процедура ЗаполнитьОтветНаПисьмо()
	...
	Если ДанныеИсходногоСообщения.Свойство("ИдентификаторСобытия") Тогда
		// Содержание исходного события ещё не загружено - сохраняем значения свойств для заполнения после дальнейшей загрузки.
...
Процедура ЗаполнитьТекстОтвета(ДанныеИсходногоСообщения)
	...
	Если ЗначениеЗаполнено(ДанныеИсходногоСообщения.СодержаниеHTML) Тогда
		...
		ТекстОтвета = СтрШаблон(НСтр("ru='<div>&nbsp;</div>"
...
"%4'"),
		...
		ФорматированныйДокумент.УстановитьHTML(ТекстОтвета, Новый Структура);
		
	ИначеЕсли ЗначениеЗаполнено(ДанныеИсходногоСообщения.Содержание) Тогда
		
		ТекстОтвета = СтрШаблон(НСтр("ru='"
"---------- Исходное сообщение ----------"
"Отправлено: %1"
"Участники: %2"
"Тема: %3"
...
"%4'"),
		...
		ФорматированныйДокумент.УстановитьФорматированнуюСтроку(Новый ФорматированнаяСтрока(ТекстОтвета));
...
Функция ДанныеИсходногоСообщения()
	...
	ОснованиеЗаполнения,
	"НачалоСобытия,СписокУчастников,Тема,Содержание,СодержаниеHTML,УчетнаяЗапись");
...
Процедура ПередЗаписьюНаСервереИсходящегоСобытия(ТекущийОбъект)
	...
	Если ЗначениеЗаполнено(ТекущийОбъект.Содержание) Тогда
...
Процедура ПродолжитьОтправкуПисьма(Результат, Параметры) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ru='Сообщение успешно отправлено';uk='Повідомлення успішно відправлене'"), ПолучитьНавигационнуюСсылку(Объект.Ссылка), Строка(Объект.Ссылка), БиблиотекаКартинок.Информация32);
	Закрыть(Успешно);
...
Процедура ЗаполнитьСодержаниеСобытия(ТемаСобытия)
	...
	Если НЕ ПустаяСтрока(ФорматированныйДокумент.ПолучитьТекст()) Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьСодержаниеСобытияЗавершение", ЭтотОбъект, Новый Структура("ТемаСобытия", ТемаСобытия)),
			НСтр("ru='Перезаполнить содержание по выбранной теме?';uk='Перезаповнити зміст по обраній темі?'"), РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
