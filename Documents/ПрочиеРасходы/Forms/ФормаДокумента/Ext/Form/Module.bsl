...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Гунько 2021-04-15
	Если Объект.Ссылка.Пустая() И НЕ ЗначениеЗаполнено(Объект.Корреспонденция) Тогда
		Объект.Корреспонденция = ПланыСчетов.Управленческий.ПрочиеДоходы;	
	КонецЕсли;
	Элементы.РасходыСчетЗатрат.СписокВыбора.ЗагрузитьЗначения(гвПолучитьМассивСчетов());
	// ...Гунько 2021-04-15
...
Процедура УправлениеФормой(Форма)
	...
	Если (Форма.ТипСчета = ПредопределенноеЗначение("Перечисление.ТипыСчетов.ПрочиеДоходы")
		...
		Иначе
			Элементы.АналитикаПрочихДоходовИРасходов.Видимость = Истина;
			Объект.АналитикаПрочихДоходовИРасходов = Неопределено;
			Элементы.АналитикаПрочихДоходовИРасходов.Заголовок = НСтр("ru='Аналитика прочих доходов и расходов';uk='Аналітика інших доходів і витрат'");
...
Процедура УстановитьПараметрыВыбораПоУчетуПрочихРасчетовНаСервереДляЭлемента(Форма, Элемент)
	...
	Для Каждого Параметр Из Элемент.ПараметрыВыбора Цикл
		Если Параметр.Имя = "Отбор.ТипСчета" Тогда
			...
			Если Форма.УчетПрочихРасчетов Тогда
...
// Гунько 2021-04-18
&НаСервереБезКонтекста
Функция гвПолучитьМассивСчетов()
	...
	Запрос.УстановитьПараметр("Родитель", ПланыСчетов.Управленческий.НайтиПоКоду("91"));
