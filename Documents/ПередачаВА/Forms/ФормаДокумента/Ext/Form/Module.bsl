...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Авторасчет НДС
	Элементы.ВнеоборотныеАктивыСуммаНДС.ТолькоПросмотр = Объект.АвторасчетНДС;
...
Процедура Рассчитать(Команда)
	...
	Если Объект.Проведен Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ru='Рассчет амортизации возможен только в непроведенном документе.';uk='Розрахунок амортизації можливий тільки в непроведеному документі.'"));
		Возврат;
	КонецЕсли;
	
	ПоказатьВопрос(Новый ОписаниеОповещения("РассчитатьЗавершение", ЭтотОбъект), НСтр("ru='Введенные данные будут пересчитаны! Продолжить?';uk='Введені дані будуть перелічені! Продовжити?'"),РежимДиалогаВопрос.ДаНет);
...
Процедура ПересчитаемСуммуТабличнойЧастиПоФлагуАвторасчетНДС()
	...
	УчетНДССервер.ПересчитатьНДСсУчетомПогрешностиОкругления(Объект.ВнеоборотныеАктивы, Объект, ПогрешностиОкругления, "Имущество");
