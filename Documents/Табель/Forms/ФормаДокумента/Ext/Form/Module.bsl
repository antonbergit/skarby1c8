...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.СпособВводаДанных <> ПредопределенноеЗначение("Перечисление.СпособыВводаДанныхОВремени.ВцеломЗаПериод") Тогда
		...
		СоответствиеДнейНедели.Вставить(7, НСтр("ru='Вс';uk='Нд'")); 
	...
	// Гунько 2021-03-21
	Для лНомерДня = 1 По 31 Цикл
		
		Элементы["ОтработанноеВремяПоДнямПервыйЧасы" + лНомерДня].Видимость = Ложь;
		Элементы["Группа2" + лНомерДня].Видимость = Ложь;
		Элементы["Группа3" + лНомерДня].Видимость = Ложь;
		
		Элементы["ОтработанноеВремяПоДнямПервыйВидВремени" + лНомерДня].УстановитьДействие("ПриИзменении", "гвЗаполнитьЧасы");
...
Процедура Заполнить(Команда)
	...
	Если Объект.ОтработанноеВремяПоДням.Количество() > 0
	 ИЛИ Объект.ОтработанноеВремяЗаПериод.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьЗавершение", ЭтотОбъект), НСтр("ru='Табличная часть будет очищена! Продолжить?';uk='Таблична частина буде очищена! Продовжити?'"), Режим, 0);
	Иначе
		ЗаполнитьТабель();
	КонецЕсли;
...
// Гунько 2021-03-21
&НаКлиенте
Процедура гвЗаполнитьЧасы(Элемент)
	Элементы.ОтработанноеВремяПоДням.ТекущиеДанные["ПервыйЧасы" + Элемент.Заголовок] = 8;
КонецПроцедуры	
// ...Гунько 2021-03-21
