...
Процедура ОбновитьЗаголовокФормы(Форма)
	...
	Если ЗначениеЗаполнено(СтрокаПодсказки) Тогда
		Заголовок = НСтр("ru='Запрос на выписку из ЕГРЮЛ/ЕГРИП по %1';uk='Запрос на выписку из ЕГРЮЛ/ЕГРИП по %1'");
	Иначе
		Заголовок = НСтр("ru='Запрос на выписку из ЕГРЮЛ/ЕГРИП';uk='Запрос на выписку из ЕГРЮЛ/ЕГРИП'");
	КонецЕсли;
...
Процедура ПрисвоитьЗначениеПараметруЗапроса(РезультатВыбора, Параметры) Экспорт
	...
	Если ПустаяСтрока(ИННКонтрагента) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='У контрагента не заполнен ИНН.';uk='У контрагента не заполнен ИНН.'"));
		Объект.Контрагент = Неопределено;
