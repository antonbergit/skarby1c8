...
&НаСервере
Процедура ЗаполнитьСведенияОЗаработкеНаСервере()
	ТаблицаСведенийОЗаработке = Документы.ПачкаДокументовСЗВ_6_3.ПолучитьСведенияОЗаработке(Объект.Организация, Объект.ОтчетныйПериод, 
																							Объект.КатегорияЗастрахованныхЛиц, Объект.ТипДоговора,
																							Объект.Сотрудники.Выгрузить().ВыгрузитьКолонку("Сотрудник"));	
	Объект.СведенияОЗаработке.Загрузить(ТаблицаСведенийОЗаработке);	
...
Процедура ОтобразитьДанныеСотрудника()
	...
	Если ДанныеТекущейСтроки <> Неопределено Тогда 
		Фамилия  = ?(ЗначениеЗаполнено(ДанныеТекущейСтроки.Фамилия), ВРег(ДанныеТекущейСтроки.Фамилия), НСтр("ru='<Не заполнено>';uk='<Не заповнене>'"));
		Имя      = ?(ЗначениеЗаполнено(ДанныеТекущейСтроки.Имя), ВРег(ДанныеТекущейСтроки.Имя), НСтр("ru='<Не заполнено>';uk='<Не заповнене>'"));
		Отчество = ВРег(ДанныеТекущейСтроки.Отчество);
		СтраховойНомерПФР =  ?(ЗначениеЗаполнено(ДанныеТекущейСтроки.СтраховойНомерПФР), ВРег(ДанныеТекущейСтроки.СтраховойНомерПФР), НСтр("ru='<Не заполнено>';uk='<Не заповнене>'")); 
		ДанныеСотрудникаТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru='Данные сотрудника: %1';uk='Дані співробітника: %1'"), Строка(ДанныеТекущейСтроки.Сотрудник));
	Иначе
		Фамилия  ="";
		Имя      = "";
		Отчество = "";
		СтраховойНомерПФР = "";
		ДанныеСотрудникаТекст = Нстр("ru='Данные сотрудника:';uk='Дані співробітника:'");
	КонецЕсли;	
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;	
	Элементы.КатегорияЗастрахованныхЛиц.СписокВыбора.ЗагрузитьЗначения(СписокКатегорийЗастрахованныхЛиц.ВыгрузитьЗначения());
...
Процедура ОтчетныйПериодПриИзменении(Элемент)
	СписокКатегорийЗастрахованныхЛиц = КатегорииЗастрахованныхЛицОрганизации(Объект.Организация, Объект.ОтчетныйПериод);
	Если Не ЗначениеЗаполнено(Объект.КатегорияЗастрахованныхЛиц) Или
		СписокКатегорийЗастрахованныхЛиц.НайтиПоЗначению(Объект.КатегорияЗастрахованныхЛиц) = Неопределено Тогда 
...
&НаКлиенте
Процедура ЗаполнитьСведенияОЗаработке(Команда)
	
	ЗаполнитьСведенияОЗаработкеНаСервере();
	
	Если Объект.СведенияОЗаработке.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Не обнаружены данные для заполнения сведений о заработке.';uk='Не виявлені дані для заповнення відомостей про заробіток.'"));
	КонецЕсли;	
КонецПроцедуры

&НаКлиенте
Процедура ИмяФайлаДляПФРНажатие(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ИмяФайла = ПолучитьИмяФайла(Объект.Ссылка);
	АдресФайла = УчетСтраховыхВзносов.СохранитьФайлВыгрузкиСЗВ63(Объект.Ссылка);
...
Процедура СотрудникиПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если ТекущиеДанные <> Неопределено И Не ОтменаРедактирования Тогда
		...
		Если Объект.Сотрудники.НайтиСтроки(СтруктураПоиска).Количество() > 1 Тогда
			ПоказатьПредупреждение(,НСтр("ru='В пачке сотрудники не могут повторяться!';uk='У пачці співробітники не можуть повторюватися!'"));
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;	
КонецПроцедуры
...
Функция ПолучитьИмяФайла(Ссылка)
	...
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "ИмяФайлаДляПФР");
