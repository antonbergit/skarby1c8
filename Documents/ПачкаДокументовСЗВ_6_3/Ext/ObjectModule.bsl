...
Процедура ПроверкаЗаполненияДокумента(ПроверяемыеРеквизиты, НеПроверяемыеРеквизиты, Отказ)Экспорт
	...
	Если КоличествоДокументов = 0 Тогда
		ТекстОшибки = Нстр("ru='Список застрахованных лиц пуст!';uk='Список застрахованих осіб порожній!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, , ,Отказ);
	КонецЕсли;
	Если КоличествоДокументов > 200 Тогда
		ТекстОшибки = ТекстОшибки = Нстр("ru='В документе должно быть не более 200 форм!';uk='У документі повинно бути не більш 200 форм!'"); 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, , ,Отказ);
	КонецЕсли;
	...
	Пока ВыборкаПоСтрокамДокумента.Следующий() цикл
		...
		Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Сотрудник) Тогда
			...
			Если Не ВыборкаПоСтрокамДокумента.СотрудникРаботаетВОрганизации тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru='Сотрудник %1 в течение отчетного периода не работал в организации %2';uk='Співробітник %1 протягом звітного періоду не працював в організації %2'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование, НаименованиеОрганизации); 
			...
			Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерПовторяющейсяСтроки) Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru='Сотрудник %1 указан в документе дважды';uk='Співробітник %1 зазначений у документі двічі'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаПоСтрокамДокумента.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник", , Отказ);
			...
			Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ВсегоЗаработка) Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru='Сотрудник %1: отсутствуют данные о заработке';uk='Співробітник %1: відсутні дані про заробіток'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаПоСтрокамДокумента.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник", , Отказ);
			Иначе
				Если ВыборкаПоСтрокамДокумента.НаименьшийМесяц < 1 Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru='Сотрудник %1: неверно указан один из месяцев - меньше 1';uk='Співробітник %1: невірно зазначено один з місяців - менше 1'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаПоСтрокамДокумента.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник", , Отказ);
				КонецЕсли;
				Если ВыборкаПоСтрокамДокумента.МинимальныйЗаработок < 0 Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru='Сотрудник %1: за один или несколько месяцев указан отрицательный заработок';uk='Співробітник %1: за один або кілька місяців зазначений негативний заробіток'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаПоСтрокамДокумента.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник", , Отказ);
				КонецЕсли;
				Если ВыборкаПоСтрокамДокумента.МинимальныйОблагаетсяВзносами < 0 Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru='Сотрудник %1: за один или несколько месяцев указана отрицательная облагаемая сумма';uk='Співробітник %1: за один або кілька місяців зазначена негативна оподатковувана сума'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаПоСтрокамДокумента.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник", , Отказ);
				КонецЕсли;
			КонецЕсли;
			Если ВыборкаПоСтрокамДокумента.НаибольшийМесяц > 12 Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru='Сотрудник %1: неверно указан один из месяцев, превышает 12';uk='Співробітник %1: невірно зазначено один з місяців, перевищує 12'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаПоСтрокамДокумента.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник", , Отказ);
			КонецЕсли;
			Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ФизЛицоСДвойнымиМесяцами) Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru='Сотрудник %1: в данных о заработке один из месяцев повторяется несколько раз!';uk='Співробітник %1: у даних про заробіток один з місяців повторюється кілька раз!'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаПоСтрокамДокумента.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник", , Отказ);
			КонецЕсли;
			Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ФизЛицоСНевернымиОблагаемымиСуммами) Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru='Сотрудник %1: в данных о заработке сумма, облагаемая взносами, превышает сумму начисленного заработка!';uk='Співробітник %1: у даних про заробіток сума, оподатковувана внесками, перевищує суму нарахованого заробітку!'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаПоСтрокамДокумента.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник", , Отказ);
			КонецЕсли;
