	...
	Если Константы.ФункциональнаяОпцияРезервированиеЗапасов.Получить()
		...
		Для каждого СтрокаМатериалы Из Материалы Цикл
			...
			Если СтрокаМатериалы.Резерв > СтрокаМатериалы.Количество Тогда
				...
				ТекстСообщения = НСтр("ru='В строке №%Номер% табл. части ""Материалы в переработку"" количество позиций к списанию из резерва превышает общее количество материалов.';uk='У рядку №%Номер% табл. частини ""Матеріали в переробку"" кількість позицій до списання з резерву перевищує загальну кількість матеріалів.'");
