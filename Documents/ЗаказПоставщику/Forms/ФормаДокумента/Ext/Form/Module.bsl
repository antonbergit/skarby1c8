...
Процедура ЗаполнитьПоОснованию(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоОснованиюЗавершение", ЭтотОбъект), НСтр("ru='Документ будет полностью перезаполнен по ""Заказу покупателя""! Продолжить выполнение операции?';uk='Документ буде повністю перезаповнений по ""Замовленню покупця""! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
...
"Возможно, необходимо перезаполнить цены.';uk='Договір з контрагентом передбачає вид цін контрагента,"
