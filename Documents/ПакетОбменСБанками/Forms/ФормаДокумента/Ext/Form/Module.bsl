...
Процедура РаспаковатьМассивПакетов(МассивПакетов)
	...
	Для Каждого ДанныеВозврата Из ДанныеДляОбработкиНаКлиенте Цикл
		Если Не ДанныеВозврата.Свойство("ЕстьОшибка") Тогда
...
Процедура РаспаковатьПакетыНаСервере(Знач ПакетыЭД, ДанныеДляОбработкиНаКлиенте)
	...
	ПакетыЭДКРаспаковке = ОбменСБанкамиСлужебныйВызовСервера.ГотовыеКРаспаковкеПакетыЭД(ПакетыЭД);
	Если ПакетыЭДКРаспаковке.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого ПакетЭД Из ПакетыЭДКРаспаковке Цикл
		ДанныеВозврата = Новый Структура("ДанныеЭП, ПараметрОповещения", Новый Соответствие, Новый Соответствие);
		ОбменСБанкамиСлужебныйВызовСервера.РаспаковатьПакетОбменСБанками(ПакетЭД, ДанныеВозврата);
...
Процедура СообщитьРезультатыРаспаковкиПакетовЭД(ВсегоРаспаковано)
	...
	ТекстСообщения = НСтр("ru='Распаковано электронных документов: (%1)';uk='Распаковано электронных документов: (%1)'");
