...
Процедура ЗаполнитьПоОперацияПоПлатежнымКартам(ДокументСсылкаОперацияПоПлатежнымКартам) Экспорт
	...
	Если ДокументСсылкаОперацияПоПлатежнымКартамВидОперации = Перечисления.ВидыОперацийЭквайринга.ВозвратОплатыПокупателю Тогда
		ВызватьИсключение НСтр("ru='Эквайринговая операция не может вводиться на основании эквайринговой операции на возврат';uk='Еквайрингова операція не може вводитися на підставі еквайринговойї операції на повернення'");
	КонецЕсли;
...
Процедура ЗаполнитьПоПриходнаяНакладная(ДокументСсылкаПриходнаяНакладная) Экспорт
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылкаПриходнаяНакладная, "ВидОперации")
		...
		ВызватьИсключение НСтр("ru='Эквайринговую операцию можно вводить только на основании приходной накладной на возврат от покупателя';uk='Еквайрингову операцію можна вводити тільки на підставі прибуткової накладної на повернення від покупця'");
	КонецЕсли;
...
Процедура ЗаполнитьПоРасходнаяНакладная(ДокументСсылкаРасходнаяНакладная) Экспорт
	...
	Если ДокументСсылкаРасходнаяНакладнаяВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПередачаВПереработку Тогда
		...
		НСтр("ru='Невозможен ввод документа на основании операции - ""%1""!';uk='Неможливе введення документа на підставі операції - ""%1""!'"),
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(ЭквайринговыйТерминал) Тогда
		ТекстСообщения = НСтр("ru='Эквайринговый терминал должен быть выбран.';uk='Еквайринговий термінал повинен бути обраний.'");
...
"Далее очистите поле эквайрингового терминала, а потом выберите его еще раз.';uk='У картці еквайрингового терміналу заповнені не всі реквізити."
"Відкрийте картку еквайрингового терміналу і перевірте реквізити ""Еквайрер"", ""Каса"", ""Банківський рахунок"" і ""Рахунок витрат"" (команда ""Рахунки обліку"")."
"Далі очистіть поле еквайрингового терміналу, а потім виберіть його ще раз. '");
