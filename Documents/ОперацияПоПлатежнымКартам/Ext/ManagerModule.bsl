...
// Функция формирует поле СуммаГрнКоп для печатной формы.
//
Функция ГрнКоп(Сумма)
	
	Грн = Цел(Сумма);
	Коп = ОКР(100 * (Сумма - Грн), 0, 1);
	СуммаГрнКоп = "" + Грн + " грн. " + Цел(Коп/10) + (Коп - 10 * Цел(Коп / 10)) + " коп.";
	Возврат СуммаГрнКоп;
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		|	ПоступлениеВКассу.Организация.КодПоЕДРПОУ КАК ОрганизацияПоЕДРПОУ,
		...
		ОбластьМакета.Параметры.СуммаРубКоп = ?(Валютный, Формат(ВыборкаДляПечати.СуммаДокумента, "ЧЦ=15; ЧДЦ=2") + " " + СокрЛП(ВыборкаДляПечати.ВалютаДенежныхСредств), ГрнКоп(ВыборкаДляПечати.СуммаДокумента));
