...
Процедура СформироватьПеремещениеЗапасовПоЯчейкам(ТекущийДокумент, ТабличныйДокумент, ИмяМакета)
	...
	ТекстЗаголовка = Нстр("ru='Перемещение запасов по ячейкам № ';uk='Переміщення запасів по комірках №'", КодЯзыкаПечать) + ШапкаДокумента.НомерДокумента + Нстр("ru=' от ';uk=' від '", КодЯзыкаПечать) + Формат(ШапкаДокумента.ДатаДокумента, "Л="+КодЯзыкаПечать+"; ДЛФ=DD");
	...
	Если ШапкаДокумента.ВидОперации = Перечисления.ВидыОперацийПеремещениеПоЯчейкам.ИзНесколькихВОдну Тогда
	    ВидОперацииПредставление = Нстр("ru='Из нескольких в одну';uk='З декількох в одну'", КодЯзыкаПечать);
	Иначе
		ВидОперацииПредставление = Нстр("ru='Из одной в несколько';uk='З однієї в декілька'", КодЯзыкаПечать);
	КонецЕсли; 
	СтруктураЗаполненияСекции.Вставить("ВидПеремещения", Нстр("ru='Вид перемещения: ';uk='Вид переміщення:'", КодЯзыкаПечать) + ВидОперацииПредставление);
	...
	Пока ТабличнаяЧасть.Следующий() Цикл
		...
		//	УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(ТабличнаяЧасть.Номенклатура, ТабличнаяЧасть.Характеристика, ТабличнаяЧасть.АртикулНоменклатуры);
	...
	Если ВыборкаИтогов.ИтогКоличество = 0 Тогда
		...
		КоличествоПеремещеныхЗапасовПрописью = Нстр("ru='В документе не указаны перемещаемые запасы.';uk='У документі не вказані переміщувані запаси.'", КодЯзыкаПечать);
		
	ИначеЕсли ЭтоПеремещениеИзОднойВНесколько Тогда
		
		КоличествоПеремещеныхЗапасовПрописью = Нстр("ru='Из ячейки ""%1"" изъято позиций: %2."
"Общим количеством: %3.';uk='З комірки ""%1"" вилучено позицій: %2."
"Загальною кількістю: %3.'", КодЯзыкаПечать);
		
	Иначе
		
		КоличествоПеремещеныхЗапасовПрописью = Нстр("ru='В ячейку ""%1"" поступило позиций: %2."
"Общим количеством: %3.';uk='У комірку ""%1"" надійшло позицій: %2."
"Загальною кількістю: %3.'", КодЯзыкаПечать);
	...
	КоличествоПеремещеныхЗапасовПрописью = 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(КоличествоПеремещеныхЗапасовПрописью
			...
			,ЧислоПрописью(?(ВыборкаИтогов.ИтогКоличество = Неопределено, 0, ВыборкаИтогов.ИтогКоличество), "L= "+КодЯзыкаПечать+";SN=true;FN=false;FS=false", Нстр("ru='единица, единицы, единиц, ж, , , , , 0';uk='одиниця , одиниці, одиниць, ж, , , , , 0'", КодЯзыкаПечать))
		);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='Перемещение запасов по ячейкам';uk='Переміщення запасів по комірках'");
