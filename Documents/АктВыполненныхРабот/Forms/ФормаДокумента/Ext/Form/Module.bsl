...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Авторасчет НДС
	Элементы.РаботыИУслугиСуммаНДС.ТолькоПросмотр = Объект.АвторасчетНДС;
...
Процедура ЗаполнитьПоЗаказу(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоЗаказуЗавершение", ЭтотОбъект), НСтр("ru='Документ будет полностью перезаполнен по ""Заказу покупателя""! Продолжить выполнение операции?';uk='Документ буде повністю перезаповнений по ""Замовленню покупця""! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
...
Процедура ЗаказПокупателяПриИзменении(Элемент)
	...
	Если Объект.Предоплата.Количество() > 0
	   ...
	   И КонтрагентВестиРасчетыПоЗаказам Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаказПокупателяПриИзмененииЗавершение", ЭтотОбъект), НСтр("ru='Зачет предоплаты будет очищен, продолжить?';uk='Залік передоплати буде очищений, продовжити?'"), Режим, 0);
        Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСписокДокументОснование()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьРемонты") Тогда
		СписокДокументОснование.Добавить("Документ.ПриемВРемонт.ФормаВыбора", "Прием в ремонт");
	КонецЕсли;
...
Процедура ПересчитаемСуммуТабличнойЧастиПоФлагуАвторасчетНДС()
	...
	УчетНДССервер.ПересчитатьНДСсУчетомПогрешностиОкругления(Объект.РаботыИУслуги, Объект, ПогрешностиОкругления, "Работы и услуги");
