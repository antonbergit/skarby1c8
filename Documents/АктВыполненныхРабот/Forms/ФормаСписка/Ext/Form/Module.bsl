...
Процедура СоздатьАктВыполненныхРабот(Команда)
	...
	Иначе
		...
		Если СтруктураДанных.СформироватьНесколькоЗаказов Тогда
			...
			ТекстСообщения = НСтр("ru='Заказы отличаются данными (%ПредставлениеДанных%) шапки документов! Сформировать несколько актов выполненных работ?';uk='Замовлення відрізняються даними (%ПредставлениеДанных%) шапки документів! Сформувати декілька актів виконаних робіт?'");
...
Процедура СоздатьАктВыполненныхРаботЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		Текст = НСтр("ru='Создание:';uk='Створення:'");
