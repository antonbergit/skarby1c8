...
Процедура ЗаполнитьПоЗаказуПокупателя(ДокументСсылкаЗаказПокупателя) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
			...
			НСтр("ru='Нельзя ввести Акт выполненных работ на основании заказ-наряда %1!';uk='Не можна ввести Акт виконаних робіт на підставі замовлення-наряду %1!'"),
...
Процедура ЗаполнитьПоСчетуНаОплату(ДокументСсылкаСчетНаОплату) Экспорт
	...
	Если ТипЗнч(ОснованиеСчетаНаОплату) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		Если ОснованиеСчетаНаОплату.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
			ВызватьИсключение НСтр("ru='Нельзя ввести Акт выполненных работ на основании Счета на оплату, выписанного на Заказ-наряд!';uk='Не можна ввести Акт виконаних робіт на підставі Рахунку на оплату, виписаного на Замовлення-наряд!'");;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьРучныеСкидки ИЛИ ЕстьАвтоматическиеСкидки Тогда
		Для каждого СтрокаРаботыИУслуги Из РаботыИУслуги Цикл
			...
			Если СтрокаРаботыИУслуги.ПроцентСкидкиНаценки <> 100 И ТекСуммаСкидки < ТекСумма
				...
				ТекстСообщения = НСтр("ru='Не заполнена колонка ""Сумма"" в строке %Номер% списка ""Работы и услуги"".';uk='Не заповнений стовпчик ""Сума"" у рядку %Номер% списку ""Роботи й послуги"".'");
	...
	Для каждого Строка Из ТаблицаПредоплата Цикл
		...
		Если НайденнаяСтрокаРаботыИУслуги = Неопределено
		   ...
		   И Контрагент.ВестиРасчетыПоЗаказам Тогда
			ТекстСообщения = НСтр("ru='Нельзя зачесть аванс по заказу отличному от указанных в табличной части ""Работы и услуги""!';uk='Не можна зарахувати аванс по замовленню відмінному від зазначених у табличній частині ""Роботи й послуги""!'");
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				,
				ТекстСообщения,
				Неопределено,
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если АвторасчетНДС Тогда
		...
		УчетНДССервер.ПересчитатьНДСсУчетомПогрешностиОкругления(РаботыИУслуги, ЭтотОбъект, ПогрешностиОкругления, "Работы и услуги");
	КонецЕсли;
