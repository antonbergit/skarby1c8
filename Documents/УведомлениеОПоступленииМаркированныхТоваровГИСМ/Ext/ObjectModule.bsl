	...
	Если НомераКиЗ.Количество() > 0 Тогда
		...
		Пока ВыборкаНомераКиЗ.Следующий() Цикл
			...
			Если Не ТекущийДокументПоступленияПравильный Тогда
				...
				ТекстОшибки = НСтр("ru='Указан некорректный документ поступления в строке %НомерСтроки% списка ""Номера КиЗ""';uk='Указан некорректный документ поступления в строке %НомерСтроки% списка ""Номера КиЗ""'");
				ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", НомерСтроки);
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	ВызватьИсключение НСтр("ru='Копирование не поддерживается.';uk='Копирование не поддерживается.'")
