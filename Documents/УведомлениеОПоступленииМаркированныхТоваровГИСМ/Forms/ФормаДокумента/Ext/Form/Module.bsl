...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.НомераКиЗСтоимость.Заголовок = СтрЗаменить(НСтр("ru='Стоимость (%Валюта%)';uk='Стоимость (%Валюта%)'"), "%Валюта%", ВалютаРегламентированногоУчета);
...
Процедура СтатусГИСМОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru='Уведомление о поступлении маркированных товаров ГИСМ было изменено. Записать?';uk='Уведомление о поступлении маркированных товаров ГИСМ было изменено. Записать?'");
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения("СтатусГИСМОбработкаНавигационнойСсылкиЗавершение",
		                                                    ЭтотОбъект,
		                                                    Новый Структура("НавигационнаяСсылкаФорматированнойСтроки", НавигационнаяСсылкаФорматированнойСтроки));
		ТекстВопроса = НСтр("ru='Уведомление о поступлении маркированных товаров ГИСМ не записано. Записать?';uk='Уведомление о поступлении маркированных товаров ГИСМ не записано. Записать?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Процедура СопоставитьПоИННКПП(Команда)
	...
	Если Не КонтрагентНайден Тогда
		Если ЕстьПравоСозданияКонтрагента Тогда
			ТекстВопроса = НСтр("ru='Контрагент с указанными ИНН и КПП не найден. Создать нового?';uk='Контрагент с указанными ИНН и КПП не найден. Создать нового?'");
			ПоказатьВопрос(Новый ОписаниеОповещения("СоздатьНовогоКонтрагента", ЭтотОбъект), ТекстВопроса,РежимДиалогаВопрос.ДаНет);
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Контрагент с указанными ИНН и КПП не найден.';uk='Контрагент с указанными ИНН и КПП не найден.'"),
			                                                  ,
			                                                  "GLNКонтрагента",
			                                                  "Объект");
		КонецЕсли;
...
Процедура ЗаполнитьПоступления(Команда)
	...
	ТекстВопроса = НСтр("ru='Документы поступления по неподтвержденным КиЗ будут перезаполнены. Продолжить?';uk='Документы поступления по неподтвержденным КиЗ будут перезаполнены. Продолжить?'");
	ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Процедура ПодтвердитьПолучениеВыделенныхСтрок(Команда)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru='Не выбрано ни одной строки для подтверждения.';uk='Не выбрано ни одной строки для подтверждения.'");
	...
	Если КоличествоУстановленныхСостоянийКПодтверждению > 0 Тогда
		ТекстОповещения = НСтр("ru='Состояние ""К передаче"" установлено для %1 из %2 строк';uk='Состояние ""К передаче"" установлено для %1 из %2 строк'");
	...
	Иначе
		ТекстОповещения = НСтр("ru='Отсутствуют строки с состоянием ""Подтвердить"" или ""Отклонено ГИСМ""';uk='Отсутствуют строки с состоянием ""Подтвердить"" или ""Отклонено ГИСМ""'");
	КонецЕсли;
	ТекстЗаголовка  = НСтр("ru='Подтверждение получения';uk='Подтверждение получения'");
...
Процедура ПодтвердитьПолучение(Команда)
	...
	Если КоличествоСтрок = 0 Тогда
		ТекстСообщения = НСтр("ru='Нет строк для подтверждения.';uk='Нет строк для подтверждения.'");
		ПоказатьПредупреждение(,ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<Не поступало>';uk='<Не поступало>'"));
