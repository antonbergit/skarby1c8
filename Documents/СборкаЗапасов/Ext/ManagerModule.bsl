	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ТипСтрокиСостава = Перечисления.ТипыСтрокСоставаСпецификации.Узел Тогда
			Если НЕ СтекСпецификацийУзлов.Найти(Выборка.Спецификация) = Неопределено Тогда
				ТекстСообщения = НСтр("ru='Обнаружено рекурсивное вхождение элемента';uk='Виявлене рекурсивне входження елемента'")+" "+Выборка.Номенклатура+" "+НСтр("ru='в спецификации';uk='у специфікації'")+" "+СтрокаПродукция.КоррСпецификация+"
...
Процедура СформироватьТаблицаЗапасыОтходы(ДокументСсылкаСборкаЗапасов, СтруктураДополнительныеСвойства)
	...
	Для н = 0 По СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыОтходы.Количество() - 1 Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицаЗапасы.СтруктурнаяЕдиницаОтходов) Тогда
			...
			СтрокаТаблицыРасход.СодержаниеПроводки = НСтр("ru='Перемещение запасов';uk='Переміщення запасів'");
            СтрокаТаблицыРасход.Содержание = НСтр("ru='Перемещение запасов';uk='Переміщення запасів'");
			...
			СтрокаТаблицыПриход.СодержаниеПроводки = НСтр("ru='Перемещение запасов';uk='Переміщення запасів'");
			СтрокаТаблицыПриход.Содержание = НСтр("ru='Перемещение запасов';uk='Переміщення запасів'");
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	...
	Для каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		Если ИмяМакета = "М11" Тогда
			...
			Заголовок       = НСтр("ru='ТРЕБОВАНИЕ-НАКЛАДНАЯ № ';uk='ВИМОГА-НАКЛАДНА № '", КодЯзыкаПечать)
							+ НомерДокумента
			...
			СтруктураЗаполненияСекции.Вставить("ДатаСоставления", Формат(Шапка.ДатаДокумента, "Л="+КодЯзыкаПечать+"; ДФ='dd MMMM yyyy'"));
		...
		ИначеЕсли ИмяМакета = "МХ18" Тогда
			...
			ЗаголовокТаблицы.Параметры.НомерСтраницы = НСтр("ru='Страница  ';uk='Сторінка '", КодЯзыкаПечать) + НомерСтраницы; 
		...
		ИначеЕсли ИмяМакета = "БланкТоварногоНаполнения" Тогда
			...
			ОбластьМакета.Параметры.ТекстЗаголовка = НСтр("ru='Производство № ';uk='Виробництво № '", КодЯзыкаПечать)
													+ НомерДокумента
													+ НСтр("ru=' от ';uk=' від '", КодЯзыкаПечать)
		                                       		+ Формат(Шапка.ДатаДокумента, "Л="+КодЯзыкаПечать+"; ДЛФ=DD");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='М11 (Требование-накладная)';uk='М11 (Вимога-накладна)'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка,ФормаСпискаДокументов";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати.Представление = НСтр("ru='МХ18 (Накладная на передачу готовой продукции)';uk='МХ18 (Накладна на передачу готової продукції)'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка,ФормаСпискаДокументов";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати.Представление = НСтр("ru='Гарантийный талон на продукцию';uk='Гарантійний талон на продукцію'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	...
	КомандаПечати.Представление = НСтр("ru='Гарантийный талон на комплектующие';uk='Гарантійний талон на комплектуючі'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	КомандаПечати.ФункциональныеОпции = "ИспользоватьСерийныеНомера";
	КомандаПечати.Порядок = 8;
