...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Запасы.Итог("Резерв") > 0 Тогда
		...
		Если НЕ ЗначениеЗаполнено(ЗаказПокупателя) Тогда
			...
			ТекстСообщения = НСтр("ru='Не указан заказ покупателя- источник резерва!';uk='Не зазначене замовлення покупця- джерело резерву!'");
		...
		Если ВидОперации = Перечисления.ВидыОперацийСборкаЗапасов.Сборка Тогда
			...
			Для каждого СтрокаЗапасы Из Запасы Цикл
				...
				Если СтрокаЗапасы.Резерв > СтрокаЗапасы.Количество Тогда
					...
					ТекстСообщения = НСтр("ru='В строке №%Номер% табл. части ""Запасы"" количество позиций к списанию из резерва превышает общее количество запасов.';uk='У рядку №%Номер% табл. частини ""Запаси"" кількість позицій до списання з резерву перевищує загальну кількість запасів.'");
		...
		Иначе
			...
			Для каждого СтрокаПродукция Из Продукция Цикл
				...
				Если СтрокаПродукция.Резерв > СтрокаПродукция.Количество Тогда
					...
					ТекстСообщения = НСтр("ru='В строке №%Номер% табл. части ""Продукция"" количество позиций к списанию из резерва превышает общее количество продукции.';uk='У рядку №%Номер% табл. частини ""Продукція"" кількість позицій до списання з резерву перевищує загальну кількість продукції.'");
	...
	// Серийные номера
	РаботаССерийнымиНомерами.ПроверкаЗаполненияСерийныхНомеров(Отказ, Продукция, СерийныеНомераПродукция, ЭтотОбъект);
