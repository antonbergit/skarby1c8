...
Процедура ЗаполнитьСписокВыбораТекущихСотрудников()
	...
	Элементы.ТекущийСотрудникЛьготыСотрудников.СписокВыбора.Очистить();
	Для каждого СтрокаСотрудник Из Объект.Сотрудники Цикл
		
		ПредставлениеСтроки = Строка(СтрокаСотрудник.Сотрудник) + НСтр("ru=', ТН: ';uk=', ТН:'") + Строка(СтрокаСотрудник.Сотрудник.Код);
		...
		Элементы.ТекущийСотрудникЛьготыСотрудников.СписокВыбора.Добавить(СтрокаСотрудник.ПолучитьИдентификатор(), ПредставлениеСтроки);
...
Процедура СтраницыОсновнаяПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница = Элементы.СтраницаНачисленияУдержания
		ИЛИ ТекущаяСтраница = Элементы.СтраницаНалоги
		ИЛИ ТекущаяСтраница = Элементы.СтраницаЛьготыСотрудников Тогда
...
&НаКлиенте
// Процедура - обработчик события ПриИзменении поля ТекущийСотрудникЛьготыСотрудников
//
Процедура ТекущийСотрудникЛьготыСотрудниковПриИзменении(Элемент)
...
Процедура ЗаполнитьНачисленияУдержания(Команда)	
	...
	Если СтрокаТабличнойЧасти = Неопределено Тогда
		...
		Сообщение.Текст = НСтр("ru='Не выбрана строка табличной части ""Сотрудники""!';uk='Не обраний рядок табличної частини ""Співробітники""!'");
		Сообщение.Сообщить();	
		Возврат;
	...
	Если Объект.НачисленияУдержания.НайтиСтроки(Новый Структура("КлючСвязи", СтрокаТабличнойЧасти.КлючСвязи)).Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьНачисленияУдержанияЗавершение", ЭтотОбъект, Новый Структура("СтрокаТабличнойЧасти", СтрокаТабличнойЧасти)), НСтр("ru='Табличная часть ""Начисления и удержания"" будет очищена! Продолжить выполнение операции?';uk='Таблична частина ""Нарахування й утримання"" буде очищена! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
        Возврат; 
	КонецЕсли;
...
Процедура ЗаполнитьНалогиНаДоходы(Команда)	
	...
	Если Объект.НалогиНаДоходы.НайтиСтроки(Новый Структура("КлючСвязи", СтрокаТабличнойЧасти.КлючСвязи)).Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьНалогиНаДоходыЗавершение", ЭтотОбъект, Новый Структура("СтрокаТабличнойЧасти", СтрокаТабличнойЧасти)), НСтр("ru='Табличная часть ""Налоги на доходы"" будет очищена! Продолжить выполнение операции?';uk='Таблична частина ""Податки на доходи"" буде очищена! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
        Возврат; 
	КонецЕсли;
...
Процедура СотрудникиПриАктивизацииСтроки(Элемент)
	...
	УправлениеНебольшойФирмойКлиент.УстановитьОтборНаПодчиненнуюТабличнуюЧасть(ЭтаФорма, "ЛьготыСотрудников");
...
Процедура СотрудникиПередУдалением(Элемент, Отказ)
	...
	УправлениеНебольшойФирмойКлиент.УдалитьСтрокиПодчиненнойТабличнойЧасти(ЭтаФорма, "НачисленияУдержания");
	УправлениеНебольшойФирмойКлиент.УдалитьСтрокиПодчиненнойТабличнойЧасти(ЭтаФорма, "НалогиНаДоходы");
    УправлениеНебольшойФирмойКлиент.УдалитьСтрокиПодчиненнойТабличнойЧасти(ЭтаФорма, "ЛьготыСотрудников");
...
&НаКлиенте
// Процедура - обработчик события ПередНачаломДобавления табличной части ЛьготыСотрудников.
//
Процедура ЛьготыСотрудниковПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
...
&НаКлиенте
// Процедура - обработчик события ПриНачалеРедактирования табличной части ЛьготыСотрудников.
//
Процедура ЛьготыСотрудниковПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
