...
Процедура ВыполнитьКонтроль(ДополнительныеСвойства, Отказ) 
	...
	Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
		Если Не ЗначениеЗаполнено(ВыборкаИзРезультатаЗапроса.СтруктурнаяЕдиница) Тогда
		    ТекстСообщения = НСтр("ru='В строке №%Номер% табл. части ""Сотрудники"" сотрудник %Сотрудник% не принят на работу в организацию %Организация%.';uk='У рядку №%Номер% табл. частини ""Співробітники"" співробітник %Сотрудник% не прийнятий на роботу в організацію %Организация%.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", ВыборкаИзРезультатаЗапроса.НомерСтроки); 
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Документы.КадровоеПеремещение.ЗарегистрироватьЛьготыСотрудника(Ссылка);
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	Документы.КадровоеПеремещение.УдалитьЛьготыСотрудника(Ссылка);
