	...
	Если НЕ ЭТИспользоватьБезПодключенияОборудования Тогда
		Для каждого ТекСтрока Из Объект.ОплатаПлатежнымиКартами Цикл
			Если НЕ ТекСтрока.ОплатаОтменена Тогда
				ТекстСообщения = НСтр("ru='Вначале нужно выполнить возврат оплаты по картам!';uk='Спочатку потрібно виконати повернення оплати по картках!'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
				Возврат;
			КонецЕсли;
		КонецЦикла;
...
Процедура ОтменитьОплату(Команда)
	...
	Если НЕ УправлениеНебольшойФирмойКлиент.ПроверитьВозможностьПечатиЧека(ЭтотОбъект, ПоказыватьПредупреждение) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Не удалось выполнить возврат оплаты картой.';uk='Не вдалося виконати повернення оплати карткою.'"));
		Возврат;
	КонецЕсли;
