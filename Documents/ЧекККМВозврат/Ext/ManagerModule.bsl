...
Функция СформироватьПечатнуюФормуТоварногоЧека(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого ЧекККМ Из МассивОбъектов Цикл
		...
		ОбластьМакета.Параметры.ТекстЗаголовка = НСтр("ru='Чек ККМ на возврат № ';uk='Чек ККМ на повернення № '", КодЯзыкаПечать)
												+ НомерДокумента
		...
		Если ИспользоватьЗаказыВРозничнойТорговле И ЗначениеЗаполнено(Шапка.Контрагент) Тогда
			...
			ПредставлениеПолучателя = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОПокупателе, "ПолноеНаименование,ИНН,КодПоЕДРПОУ,ЮридическийАдрес,Телефоны,");
...
Функция СформироватьПечатнуюФормуКМ3(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ЧекККМВозврат ИЗ МассивОбъектов Цикл
		...
		ОбластьМакета.Параметры.ПредставлениеОрганизации = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОбОрганизации, "ПолноеНаименование,ИНН,КодПоЕДРПОУ,Телефоны,");
		ОбластьМакета.Параметры.СуммаПрописью            = РаботаСКурсамиВалют.СформироватьСуммуПрописью(Шапка.СуммаДокумента, Шапка.Валюта, , КодЯзыкаПечать);

		ОбластьМакета.Параметры.НомерДокумента           = "";
		
		ОбластьМакета.Параметры.ОрганизацияПоЕДРПОУ   = СведенияОбОрганизации.КодПоЕДРПОУ;
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='КМ3 (Акт о возврате денежных сумм покупателям)';uk='КМ3 (Акт про повернення грошових сум покупцям)'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка";
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
