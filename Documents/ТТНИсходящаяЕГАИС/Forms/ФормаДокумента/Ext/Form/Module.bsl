...
Процедура ПодобратьСправкиБ(Команда)
	...
	Если Объект.Грузоотправитель.Пустая() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбран грузоотправитель';uk='Не выбран грузоотправитель'"));
		Возврат;
	КонецЕсли;
...
Процедура ВыгрузкаДокумента_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Результат Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Документ успешно выгружен в УТМ.';uk='Документ успешно выгружен в УТМ.'"));
