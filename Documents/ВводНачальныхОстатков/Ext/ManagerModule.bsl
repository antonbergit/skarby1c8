...
Процедура ИнициализироватьДанныеДокументаУправленческий(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("Содержание", НСтр("ru='Ввод начальных остатков';uk='Введення початкових залишків'"));
...
Процедура ИнициализироватьДанныеДокументаРасчетыСПодотчетниками(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("ВозникновениеДолгаПодотчетника", НСтр("ru='Ввод остатков задолженности подотчетника';uk='Введення залишків заборгованості підзвітної особи'"));
	Запрос.УстановитьПараметр("ПогашениеДолгаПодотчетнику", НСтр("ru='Ввод остатков задолженности перед подотчетником';uk='Введення залишків заборгованості перед подотчетником'"));
...
Процедура ИнициализироватьДанныеДокументаРасчетыСПерсоналом(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("ВозникновениеОбязательствПередПерсоналом", НСтр("ru='Возникновение обязательств перед персоналом';uk='Виникнення зобов`язань перед персоналом'"));
...
Процедура ИнициализироватьДанныеДокументаРасчетыПоНалогам(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("НачислениеНалога", НСтр("ru='Ввод остатков задолженности перед бюджетом';uk='Введення залишків заборгованості перед бюджетом'"));
...
Процедура ИнициализироватьДанныеДокументаРасчетыСПокупателями(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	|	ВЫРАЗИТЬ(РасчетыСПокупателями.СуммаНДС * КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность / (КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность) КАК ЧИСЛО(15, 2)) КАК СуммаНДС,
	|	ВЫРАЗИТЬ(РасчетыСПокупателями.Сумма * КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность / (КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность) КАК ЧИСЛО(15, 2)) КАК СуммаРегл,
	|	РасчетыСПокупателями.НетНалоговойНакладной
	|ИЗ
	|	Документ.ВводНачальныхОстатков.РасчетыСПокупателями КАК РасчетыСПокупателями
	...
	|	ВЫРАЗИТЬ(РасчетыСПокупателями.СуммаНДС * КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность / (КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность) КАК ЧИСЛО(15, 2)),
	|	РасчетыСПокупателями.НетНалоговойНакладной,
	|	ВЫРАЗИТЬ(РасчетыСПокупателями.Сумма * КурсыУпрВалюты.Курс * КурсыРегВалюты.Кратность / (КурсыРегВалюты.Курс * КурсыУпрВалюты.Кратность) КАК ЧИСЛО(15, 2)),
	|	ВЫБОР
	|		КОГДА РасчетыСПокупателями.Контрагент.ВестиРасчетыПоЗаказам
	|			ТОГДА РасчетыСПокупателями.ЗаказПокупателя
	|		ИНАЧЕ ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.пустаяСсылка)
	...
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("МоментВремени", Новый Граница(СтруктураДополнительныеСвойства.ДляПроведения.МоментВремени, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("ВозникновениеОбязательствПокупателя", НСтр("ru='Ввод остатков задолженности покупателей';uk='Введення залишків заборгованості покупців'"));
	Запрос.УстановитьПараметр("ПогашениеОбязательствПокупателя", НСтр("ru='Ввод остатков авансов от покупателей';uk='Введення залишків авансів від покупців'"));
	...
	ТаблицаНДСПродаж = Новый ТаблицаЗначений;
	
	ТаблицаНДСПродаж.Колонки.Добавить("ВидДвижения");
	ТаблицаНДСПродаж.Колонки.Добавить("Период");
	ТаблицаНДСПродаж.Колонки.Добавить("Организация");
	ТаблицаНДСПродаж.Колонки.Добавить("ДоговорКонтрагента");
	ТаблицаНДСПродаж.Колонки.Добавить("СобытиеНДС");
	ТаблицаНДСПродаж.Колонки.Добавить("СтавкаНДС");
	ТаблицаНДСПродаж.Колонки.Добавить("Сумма");
	ТаблицаНДСПродаж.Колонки.Добавить("СуммаНДС");

	ОрганизацияВДокументе = ДокументСсылкаВводНачальныхОстатков.Организация;
	
	ТаблицаРасчетыСПокупателями = МассивРезультатов[0].Выгрузить();
	Для каждого Строка Из ТаблицаРасчетыСПокупателями Цикл
		Если Строка.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
			СтрокаНДСПродаж = ТаблицаНДСПродаж.Добавить();
		    СтрокаНДСПродаж.Период =  Строка.Период;
		    СтрокаНДСПродаж.ВидДвижения = ВидДвиженияНакопления.Приход;
		    СтрокаНДСПродаж.ДоговорКонтрагента =  Строка.Договор;
		    СтрокаНДСПродаж.Организация =  ОрганизацияВДокументе;
		    СтрокаНДСПродаж.СобытиеНДС = Перечисления.СобытияНДСПродаж.РеализацияПокупателю;
		    СтрокаНДСПродаж.СтавкаНДС =  Строка.СтавкаНДС;
			СтрокаНДСПродаж.Сумма =  Строка.СуммаРегл;
		    СтрокаНДСПродаж.СуммаНДС = Строка.СуммаНДС;
		
		ИначеЕсли Строка.ТипРасчетов = Перечисления.ТипыРасчетов.Аванс Тогда
		    ...
		    СтрокаНДСПродаж.ДоговорКонтрагента =  Строка.Договор;
		    СтрокаНДСПродаж.Организация =  ОрганизацияВДокументе;
		    СтрокаНДСПродаж.СобытиеНДС = Перечисления.СобытияНДСПродаж.ОплатаПокупателем;
			СтрокаНДСПродаж.Сумма =  Строка.СуммаРегл;
			
		КонецЕсли; 
		
		Если НЕ Строка.НетНалоговойНакладной Тогда
		
			СтрокаНДСПродаж = ТаблицаНДСПродаж.Добавить();
		    СтрокаНДСПродаж.Период =  Строка.Период;
		    СтрокаНДСПродаж.ВидДвижения = ВидДвиженияНакопления.Расход;
		    СтрокаНДСПродаж.ДоговорКонтрагента =  Строка.Договор;
		    СтрокаНДСПродаж.Организация =  ОрганизацияВДокументе;
		    СтрокаНДСПродаж.СобытиеНДС = Перечисления.СобытияНДСПродаж.РеализацияПокупателю;
		    СтрокаНДСПродаж.СтавкаНДС =  Строка.СтавкаНДС;
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаНДСПродаж", ТаблицаНДСПродаж);
...
Процедура ИнициализироватьДанныеДокументаРасчетыСПоставщиками(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("ВозникновениеОбязательствПередПоставщиком", НСтр("ru='Ввод остатков задолженности поставщикам';uk='Введення залишків заборгованості постачальникам'"));
	Запрос.УстановитьПараметр("ПогашениеОбязательствПередПоставщиком", НСтр("ru='Ввод остатков авансов поставщикам';uk='Введення залишків авансів постачальникам'"));
...
Процедура ИнициализироватьДанныеДокументаДенежныеСредства(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("СодержаниеПроводки", НСтр("ru='Ввод остатков денежных средств';uk='Введення залишків коштів'"));
...
Процедура ИнициализироватьДанныеДокументаЗапасы(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("ОприходованиеЗатрат", НСтр("ru='Оприходование затрат';uk='Оприбуткування витрат'"));
	Запрос.УстановитьПараметр("ОприходованиеЗапасовПринятых", НСтр("ru='Оприходование запасов принятых';uk='Оприбуткування запасів прийнятих'"));
	Запрос.УстановитьПараметр("ОприходованиеЗапасовПереданных", НСтр("ru='Оприходование запасов переданных';uk='Оприбуткування запасів переданих'"));
	Запрос.УстановитьПараметр("ПриемЗапасов", НСтр("ru='Прием запасов';uk='Приймання запасів'"));
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаУправленческий", МассивРезультатов[4].Выгрузить());

	// Серийные номера
	РезультатЗапроса = МассивРезультатов[5].Выгрузить();
...
Процедура ИнициализироватьДанныеВнеоборотныеАктивы(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("ПринятиеВнеоборотногоАктиваКУчету", НСтр("ru='Ввод начальных остатков по внеоборотным активам';uk='Введення початкових залишків по необоротним активам'"));
	Запрос.УстановитьПараметр("НачислениеАмортизации", НСтр("ru='Ввод начальных остатков по амортизации';uk='Введення початкових залишків по амортизації'"));
...
Процедура ИнициализироватьДанныеДокументаРасчетыПоЭквайрингу(ДокументСсылкаВводНачальныхОстатков, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылкаВводНачальныхОстатков);
	Запрос.УстановитьПараметр("Организация", СтруктураДополнительныеСвойства.Дляпроведения.Организация);
	Запрос.УстановитьПараметр("РасчетыПоЭквайрингу", НСтр("ru='Ввод остатков расчетов по эквайрингу';uk='Введення залишків розрахунків по еквайрингу'"));
