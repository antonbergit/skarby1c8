...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	СтавкаНДСПоУмолчанию = СтруктураДанные.СтавкаНДСПоУмолчанию;
...
Процедура ДенежныеСредстваВалютаДенежныхСредствНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(СтруктураДанные.ТипДенежныхСредств)
	   И СтруктураДанные.ТипДенежныхСредств <> Наличные Тогда
		ПоказатьПредупреждение(Неопределено,НСТР("ru='Для банковского счета нельзя изменить валюту денежных средств!';uk='Для банківського рахунку не можна змінити валюту коштів!'"));
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
...
&НаКлиенте
Процедура РасчетыСПокупателямиСтавкаНДСПриИзменении(Элемент)
...
Процедура СформироватьСписокРазделовУчета()
	...
	Если Константы.ФункциональнаяОпцияИспользоватьПодсистемуЗарплата.Получить()
		...
		Элементы.РазделУчета.СписокВыбора.Добавить("Расчеты с персоналом", НСтр("ru='Расчеты с персоналом';uk='Розрахунки з персоналом'"));
	КонецЕсли;
	...
	Если Константы.ФункциональнаяОпцияУчетВнеоборотныхАктивов.Получить()
		...
		Элементы.РазделУчета.СписокВыбора.Добавить("Имущество", НСтр("ru='Имущество';uk='Майно'"));
	КонецЕсли;
	
	// Прочее.
	Элементы.РазделУчета.СписокВыбора.Добавить("Прочие разделы", НСтр("ru='Прочие разделы';uk='Інші розділи'"));
	...
	Если Не ПолучитьФункциональнуюОпцию("РазноситьОплатуОтЭквайрераПоЭквайринговымОперациям") 
		...
		Элементы.РазделУчета.СписокВыбора.Добавить("Эквайринг", НСтр("ru='Эквайринг';uk='Еквайринг'"));
	КонецЕсли;
	// Конец Эквайринг
...
Функция ПолучитьДанныеОрганизацияПриИзменении(Организация)
	...
	СтруктураДанные.Вставить(
		...
		Организация.СтавкаНДСПоУмолчанию		
	);
...
Процедура ОбработатьПодготовленныеДанныеЗапасы(РезультатЗагрузки)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(Нстр("ru='Загрузка данных';uk='Завантаження даних'"), УровеньЖурналаРегистрации.Ошибка, Метаданные.Документы.ВводНачальныхОстатков, , Нстр("ru='Запасы: ';uk='Запаси:'") + ОписаниеОшибки());
		ОтменитьТранзакцию();
...
Процедура ОбработатьПодготовленныеДанныеРасчетыСПокупателями(РезультатЗагрузки)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(Нстр("ru='Загрузка данных';uk='Завантаження даних'"), УровеньЖурналаРегистрации.Ошибка, Метаданные.Документы.ВводНачальныхОстатков, , Нстр("ru='Расчеты с покупателями: ';uk='Розрахунки з покупцями:'") + ОписаниеОшибки());
		ОтменитьТранзакцию();
