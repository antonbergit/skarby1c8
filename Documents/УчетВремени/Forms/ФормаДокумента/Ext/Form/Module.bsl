...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ОперацииПнДлительность.Заголовок = НСтр("ru='Пн ';uk='Пн '") + Формат(Объект.ДатаС, "ДФ=dd.MM");
	Элементы.ОперацииВтДлительность.Заголовок = НСтр("ru='Вт ';uk='Вт '") + Формат(Объект.ДатаС + 86400, "ДФ=dd.MM");
	Элементы.ОперацииСрДлительность.Заголовок = НСтр("ru='Ср ';uk='Ср '") + Формат(Объект.ДатаС + 86400*2, "ДФ=dd.MM");
	Элементы.ОперацииЧтДлительность.Заголовок = НСтр("ru='Чт ';uk='Чт '") + Формат(Объект.ДатаС + 86400*3, "ДФ=dd.MM");
	Элементы.ОперацииПтДлительность.Заголовок = НСтр("ru='Пт ';uk='Пт '") + Формат(Объект.ДатаС + 86400*4, "ДФ=dd.MM");
	Элементы.ОперацииСбДлительность.Заголовок = НСтр("ru='Сб ';uk='Сб '") + Формат(Объект.ДатаС + 86400*5, "ДФ=dd.MM");
	Элементы.ОперацииВсДлительность.Заголовок = НСтр("ru='Вс ';uk='Нд '") + Формат(Объект.ДатаС + 86400*6, "ДФ=dd.MM");
...
Процедура ЗаполнитьПоПлану(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		Сообщение.Текст = НСтр("ru='Не заполнена организация! Заполнение отменено.';uk='Не заповнена організація! Заповнення скасоване.'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.СтруктурнаяЕдиница) Тогда
        Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru='Не заполнено подразделение! Заполнение отменено.';uk='Не заповнений підрозділ! Заповнення скасоване.'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.Сотрудник) Тогда
        Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru='Не выбран сотрудник! Заполнение отменено.';uk='Не обраний співробітник! Заповнення скасоване.'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДатаС) Тогда
        Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru='Не выбрано начало недели! Заполнение отменено.';uk='Не обраний початок тижня! Заповнення скасоване.'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДатаПо) Тогда
        Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru='Не выбрано окончание недели! Заполнение отменено.';uk='Не обране закінчення тижня! Заповнення скасоване.'");
	...
	Если Объект.Операции.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоПлануЗавершение", ЭтотОбъект), НСтр("ru='Табличная часть документа будет очищена! Продолжить выполнение операции?';uk='Таблична частина документа буде очищена! Продовжити виконання операції?'"), РежимДиалогаВопрос.ДаНет, 0);
        Возврат; 
	КонецЕсли;
...
Процедура УстановитьЗаголовкиКолонок()
	...
	Элементы.ОперацииПтДлительность.Заголовок = НСтр("ru='Пт ';uk='Пт'") + Формат(Объект.ДатаС + 86400*4, "ДФ=dd.MM");
	Элементы.ОперацииСбДлительность.Заголовок = НСтр("ru='Сб ';uk='Сб'") + Формат(Объект.ДатаС + 86400*5, "ДФ=dd.MM");
	Элементы.ОперацииВсДлительность.Заголовок = НСтр("ru='Вс ';uk='Вс'") + Формат(Объект.ДатаС + 86400*6, "ДФ=dd.MM");
