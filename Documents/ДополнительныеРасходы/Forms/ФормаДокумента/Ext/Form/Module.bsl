...
Процедура ЗаказПоставщикуПриИзменении(Элемент)
	...
	Если Объект.Предоплата.Количество() > 0
	   ...
	   И КонтрагентВестиРасчетыПоЗаказам Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаказПоставщикуПриИзмененииЗавершение", ЭтотОбъект), НСтр("ru='Зачет предоплаты будет очищен, продолжить?';uk='Залік передоплати буде очищений, продовжити?'"), Режим, 0);
		Возврат;
	КонецЕсли;
...
Процедура РаспределитьРасходыПоКоличеству(Команда)
	...
	Если Объект.Запасы.Количество() = 0 Тогда
		УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(Объект, НСтр("ru='В табличной части ""Запасы"" нет записей!';uk='У табличній частині ""Запаси"" немає записів!'"));
		Возврат;
	КонецЕсли;
	
	Если Объект.Расходы.Количество() = 0 Тогда
		УправлениеНебольшойФирмойКлиент.СообщитьОбОшибке(Объект, НСтр("ru='В табличной части ""Расходы"" нет записей!';uk='У табличній частині ""Витрати"" немає записів!'"));
		Возврат;
	КонецЕсли;
