...
Процедура ВыполнитьПредварительныйКонтроль(Отказ)
	...
	Для каждого СтрокаНачисленияУдержания Из ТаблицаНачисленияУдержания Цикл
		...
		Иначе
			...
			ТекстСообщения = НСтр("ru='Не верное условие связи в строке №%Номер% табл. части ""Начислений и удержаний"".';uk='Не вірна умова зв`язку в рядку №%Номер% табл. частини ""Нарахувань і втримань"".'");
	...
	Если НЕ МассивРезультатов[2].Пустой() Тогда
		...
		Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
			ТекстСообщения = НСтр("ru='В строке №%Номер% табл. части ""Сотрудники"" период действия приказа противоречит кадровому приказу ""%КадровыйПриказ%"".';uk='У рядку №%Номер% табл. частини ""Співробітники"" період дії наказу суперечить кадровому наказу ""%КадровыйПриказ%"".'");
	...
	Если НЕ МассивРезультатов[3].Пустой() Тогда
		...
		Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
			ТекстСообщения = НСтр("ru='В строке №%Номер% табл. части ""Начисления и удержания"" период действия приказа противоречит кадровому приказу ""%КадровыйПриказ%"".';uk='У рядку №%Номер% табл. частини ""Нарахування й утримання"" період дії наказу суперечить кадровому наказу ""%КадровыйПриказ%"".'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", ВыборкаИзРезультатаЗапроса.НомерСтроки); 
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КадровыйПриказ%", ВыборкаИзРезультатаЗапроса.Регистратор);
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				ЭтотОбъект,
	...
	Если НЕ МассивРезультатов[4].Пустой() Тогда
		...
		Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
			ТекстСообщения = НСтр("ru='В строке №%Номер% табл. части ""Сотрудники"" сотрудник указывается повторно.';uk='У рядку №%Номер% табл. частини ""Співробітники"" співробітник вказується повторно.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", ВыборкаИзРезультатаЗапроса.НомерСтроки);
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				ЭтотОбъект,
				ТекстСообщения,
...
Процедура ВыполнитьКонтроль(ДополнительныеСвойства, Отказ)
	...
	Если НЕ МассивРезультатов[0].Пустой() Тогда
		...
		Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
			ТекстСообщения = НСтр("ru='В строке №%Номер% табл. части ""Сотрудники"" сотрудник %Сотрудник% уже работает в подразделении %СтруктурнаяЕдиница%.';uk='У рядку №%Номер% табл. частини ""Співробітники"" співробітник %Сотрудник% уже працює в підрозділі %СтруктурнаяЕдиница%.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", ВыборкаИзРезультатаЗапроса.НомерСтроки); 
	...
	Если НЕ МассивРезультатов[1].Пустой() Тогда
		...
		Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
			ТекстСообщения = НСтр("ru='В строке №%Номер% табл. части ""Сотрудники"" для физлица %Физлицо% уже принят на основное место работы сотрудник %Сотрудник% в подразделение %СтруктурнаяЕдиница%.';uk='У рядку №%Номер% табл. частини ""Співробітники"" для фізособи %Физлицо% уже прийнятий на основне місце роботи співробітник %Сотрудник% у підрозділ %СтруктурнаяЕдиница%.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", ВыборкаИзРезультатаЗапроса.НомерСтроки); 
	...
	Если НЕ МассивРезультатов[2].Пустой() Тогда
		ВыборкаИзРезультатаЗапроса = МассивРезультатов[2].Выбрать();
		Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
			ТекстСообщения = НСтр("ru='В строке №%НомерДубль% табл. части ""Сотрудники"" для физлица %Физлицо% повторно принимается на основное место работы сотрудник %Сотрудник%. Физлицо уже указано в строке №%Номер%.';uk='У рядку №%НомерДубль% табл. частини ""Співробітники"" для фізособи %Физлицо% повторно ухвалюється на основне місце роботи співробітник %Сотрудник%. Фізособа вже зазначена в рядку №%Номер%.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", ВыборкаИзРезультатаЗапроса.НомерСтроки); 
	...
	Если НЕ МассивРезультатов[3].Пустой() Тогда
		ВыборкаИзРезультатаЗапроса = МассивРезультатов[3].Выбрать();
		Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
			ТекстСообщения = НСтр("ru='В строке №%Номер% табл. части ""Сотрудники"": сотрудник %Сотрудник% уже работал в компании. Для повторного приема на работу необходимо создать нового сотрудника.';uk='У рядку №%Номер% табл. частини ""Співробітники"": співробітник %Сотрудник% уже працював у компанії. Для повторного приймання на роботу необхідно створити нового співробітника.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", ВыборкаИзРезультатаЗапроса.НомерСтроки); 
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Сотрудник%", ВыборкаИзРезультатаЗапроса.Сотрудник);
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(
				ЭтотОбъект,
...
Процедура ВыполнитьКонтрольШтатногоРасписания(ДополнительныеСвойства, Отказ) 
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ПротиворечиеШР Тогда
			ТекстСообщения = НСтр("ru='Строка №%Номер% табл. части ""Сотрудники"": в штатном расписании не предусмотрены ставки для приема сотрудника!';uk='Рядок №%Номер% табл. частини ""Співробітники"": у штатному розкладі не передбачені ставки для приймання співробітника!'");
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// Запись льгот сотрудников в регистр сведений НДФЛЛьготыСотрудников.
	Документы.ПриемНаРаботу.ЗарегистрироватьЛьготыСотрудника(Ссылка);
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	// Удаление льгот сотрудников из регистра сведений НДФЛЛьготыСотрудников.
	Документы.ПриемНаРаботу.УдалитьЛьготыСотрудника(Ссылка);
