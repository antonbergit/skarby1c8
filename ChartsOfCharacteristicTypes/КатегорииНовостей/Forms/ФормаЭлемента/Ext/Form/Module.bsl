		...
		Для каждого КлючЗначение Из СтруктураДействий Цикл
			...
			ИначеЕсли КлючЗначение.Ключ = "ОбновляетсяССервера" Тогда
				...
				Если Объект.ОбновляетсяССервера = Истина Тогда
					...
					Элементы.ЗначенияКатегорийНовостейСправочник.Заголовок = НСтр("ru='Список доступных значений (обновляется с сервера, редактирование запрещено)';uk='Список доступних значень (оновлюється із сервера, редагування заборонене)'");
				Иначе
	...
	Если СтруктураПараметров.ЗаполняетсяАвтоматически = Истина Тогда
		...
		Иначе
			СтруктураДействий.Вставить(
				...
				СтрШаблон(
					НСтр("ru='Для этой конфигурации значение этой категории всегда равно %1';uk='Для этой конфигурации значение этой категории всегда равно %1'"),
					Значение));
		КонецЕсли;
	Иначе
		...
		Если Объект.ОбновляетсяССервера Тогда
			...
			Если Запись.Выбран() Тогда
				Если Запись.ТекущаяВерсияНаКлиенте >= Запись.ТекущаяВерсияНаСервере Тогда
					ТекстНадписи = НСтр("ru='Данные актуальны и соответствуют данным с сервера от %ТекущаяВерсияНаСервере%.';uk='Дані актуальні й відповідають даним із сервера від %ТекущаяВерсияНаСервере%.'");
...
"а на сервере уже версия от %ТекущаяВерсияНаСервере%.';uk='Дані ніколи не оновлялись із сервера,"
...
"а на сервере уже версия от %ТекущаяВерсияНаСервере%.';uk='Останній раз дані оновлялися із сервера %ТекущаяВерсияНаКлиенте%,"
"а на сервері вже версія від %ТекущаяВерсияНаСервере%.'");
					КонецЕсли;
				...
				КонецЕсли;
			Иначе
				ТекстНадписи = НСтр("ru='Данные никогда не обновлялись с сервера.';uk='Дані ніколи не оновлювались із сервера.'");
				ТребуетсяОбновление = Истина;
			КонецЕсли;
					...
					Иначе
						Элементы.ДекорацияТребуетсяОбновлениеССервера.Заголовок   = Новый ФорматированнаяСтрока(
							...
							Новый ФорматированнаяСтрока(
								НСтр("ru='Проверить обновления.';uk='Проверить обновления.'"),
								,
		...
		Иначе
			Элементы.ДекорацияТребуетсяОбновлениеССервера.Заголовок  = НСтр("ru='Данные вводятся вручную и обновление с сервера не требуется.';uk='Дані вводяться вручну й оновлення із сервера не потрібно.'");
			Элементы.ДекорацияТребуетсяОбновлениеССервера.ЦветТекста = ЦветаСтиля.ЦветМикротекста;
		КонецЕсли;
