...
"Разрешено использовать цифры, английские буквы, подчеркивание и минус.';uk='В идентификаторе присутствуют запрещенные символы: %1."
"Разрешено использовать цифры, английские буквы, подчеркивание и минус.'"),
	...
	Если ЭтотОбъект.ТипЗначения.Типы().Количество() > 1 Тогда
		...
		Сообщение.ПутьКДанным = "Объект";
		Сообщение.Текст = НСтр("ru='Составные типы не поддерживаются. Выберите только один тип значения.';uk='Складені типи не підтримуються. Виберіть тільки один тип значення.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ)
	...
	Если ЭтотОбъект.ЗагруженоССервера = Истина Тогда
		...
		Если (ЭтотОбъект.ПометкаУдаления = Истина)
			...
			ТекстСообщения = НСтр("ru='Нельзя помечать на удаление категории, загруженные автоматически с сервера новостей.';uk='Не можна позначати на видалення категорії, завантажені автоматично із сервера новин.'");
			Сообщение = Новый СообщениеПользователю();
