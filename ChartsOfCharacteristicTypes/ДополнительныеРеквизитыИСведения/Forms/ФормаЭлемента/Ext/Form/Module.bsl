...
Процедура КомандаДалее(Команда)
	...
	Если ИндексСтраницы = 0
		...
		ТекстПредупреждения = НСтр("ru='Элемент не выбран!';uk='Елемент не обраний!'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура РедактироватьФорматЗначенияЗавершение(Текст, Неопределен) Экспорт
	...
	Если Текст <> Неопределено Тогда
		...
		ТекстПредупреждения = НСтр("ru='Следующие настройки формата автоматически не применяются в большинстве мест:';uk='Наступні налаштування формату автоматично не застосовуються у більшості місць:'");
...
Процедура УстановитьНастройкиПомощника(ТекущаяСтраница = Неопределено)
	...
	ШаблонЗаголовкаСписка        = НСтр("ru='Выберите %1 для включения в набор ""%2""';uk='Виберіть %1 для включення в набір ""%2""'");
	ШаблонЗаголовкаПереключателя = НСтр("ru='Выберите вариант добавления дополнительного %1 ""%2"" в набор ""%3""';uk='Виберіть варіант додавання додаткового %1 ""%2"" у набір ""%3""'");
...
""
"Удаленный тип восстановлен.';uk='Неприпустимо видаляти тип ""%1"","
...
"В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.';uk='Додати загальну відомість у набір (рекомендується)"
""
"Обрана відомість вже входить у кілька наборів, тому рекомендується також включити його в цей набір ""як є""."
"У цьому випадку буде можливо відбирати по ньому дані різних типів у списках і звітах.'");
...
"В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.';uk='Зробити додаткову відомість загальною і додати в набір"
""
"Цей варіант підходить для тих випадків, коли відомість повинна бути однакова для обох наборів."
"У цьому випадку буде можливо відбирати по ньому дані різних типів у списках і звітах.'");
...
"При этом можно отредактировать наименование и ряд других свойств сведения.';uk='Зробити копію відомості за зразком (із загальним списком значень)"
""
"Список значень цієї відомості буде однаковий для обох наборів."
"За допомогою цього варіанта зручно виконувати централізоване налаштування списку значень відразу для декількох однотипних відомостей."
"При цьому можна відредагувати найменування й ряд інших властивостей відомості.'");
		СоздатьКопированием = НСтр("ru='Сделать копию сведения"
""
"Будет создана копия сведения%1';uk='Зробити копію відомості"
""
...
"В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.';uk='Додати загальний реквізит у набір (рекомендується)"
""
"Обраний реквізит вже входить у кілька наборів, тому рекомендується також включити його в цей набір ""як є""."
"У цьому випадку буде можливо відбирати по ньому дані різних типів у списках і звітах.'");
...
"В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.';uk='Зробити реквізит загальним і додати в набір"
""
...
"При этом можно отредактировать наименование и ряд других свойств реквизита.';uk='Зробити копію реквізиту за зразком (із загальним списком значень)"
""
"Список значень цього реквізиту буде однаковий для обох наборів."
"За допомогою цього варіанта зручно виконувати централізоване налаштування списку значень відразу для декількох однотипних реквізитів."
"При цьому можна відредагувати найменування й ряд інших властивостей реквізиту.'");
		СоздатьКопированием = НСтр("ru='Сделать копию реквизита"
""
"Будет создана копия реквизита%1';uk='Зробити копію реквізиту"
""
...
Процедура УстановитьЗаголовкиГиперссылок()
	...
	ШаблонЗависимостьЗадана = НСтр("ru='при заданном условии';uk='при заданій умові'");
		...
		Если Выборка.ТипЗначения <> NULL
		   И УправлениеСвойствамиСлужебный.ТипЗначенияСодержитЗначенияСвойств(Выборка.ТипЗначения) Тогда
			...
			Если ПервыеЗначения.Количество() = 0 Тогда
				...
				Иначе
					ПредставлениеЗначений = НСтр("ru='Значения помечены на удаление';uk='Значення позначені на видалення'");
				КонецЕсли;
			Иначе
...
"иначе программа может работать некорректно.';uk='Рекомендується використати інше найменування,"
