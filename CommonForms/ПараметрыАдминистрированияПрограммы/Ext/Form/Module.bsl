...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() И Параметры.ЗапрашиватьПараметрыАдминистрированияКластера Тогда
		ВызватьИсключение НСтр("ru='Настройка параметров кластера серверов доступна только в клиент-серверном режиме работы.';uk='Налаштування параметрів кластера серверів доступне тільки в клієнт-серверному режимі роботи.'");
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если Параметры.ЗапрашиватьПараметрыАдминистрированияКластера
		И ОбщегоНазначенияКлиентСервер.ЭтоOSXКлиент() Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ru='Подключение к кластеру серверов недоступно в клиенте под управлением ОС X.';uk='Підключення до кластера серверів недоступне в клієнтові під управлінням ОС X.'");
