...
Процедура ОткрытьФормуДокумента()
	...
	Попытка
		...
		Если Элементы.ДеревоДокументов.ТекущиеДанные.Ссылка = ДокументСсылка И Форма.Открыта() Тогда
			...
			Сообщение.Текст = НСтр("ru='Документ уже открыт!';uk='Документ вже відкритий!'");
			Сообщение.Сообщить();
		КонецЕсли;
...
Функция ПровестиСервер(ДокументСсылка)
	...
	Исключение
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru='Невозможно заблокировать документ.';uk='Неможливо заблокувати документ.'");
		Сообщение.Сообщить();
	КонецПопытки; 
...
"подчиненности был удален, или же стал недоступен.';uk='Документ, для якого сформований звіт про структуру"
"підпорядкованості був вилучений, або ж став недоступний.'"));
	КонецЕсли;		
...
Процедура Провести(Кнопка)
	...
	Исключение
		...
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru='Не удалось провести документ %Документ%!';uk='Не вдалося провести документ %Документ%!'");;
...
Процедура ОтменаПроведения(Кнопка)
	...
	Исключение
		...
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru='Не удалось сделать непроведенным документ %Документ%!';uk='Не вдалося зробити непроведеним документ %Документ%!'");;
		Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%Документ%", Элементы.ДеревоДокументов.ТекущиеДанные.ДокументПредставление);
		Сообщение.Сообщить();
...
Процедура УстановитьПометкуУдаления(Кнопка)
	...
	Исключение
		...
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru='Не удалось установить пометку удаления на документ %Документ%!';uk='Не вдалося встановити позначку видалення на документ %Документ%!'");;
