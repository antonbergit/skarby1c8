...
Функция ПроверитьЗаполнениеРеквизитов(МассивОшибок)
	...
	Если Не ЗначениеЗаполнено(Тип) Тогда
		ТекстОшибки = НСтр("ru='Не заполнен реквизит Тип поля.';uk='Не заповнений реквізит Тип поля.'");
		МассивОшибок.Добавить(ТекстОшибки);
	КонецЕсли;
	
	// Проверка типа заполнения.
	Если Не ЗначениеЗаполнено(ТипЗаполнения) Тогда
		ТекстОшибки = НСтр("ru='Не заполнен реквизит Тип заполнения.';uk='Не заповнений реквізит Тип заповнення.'");
		МассивОшибок.Добавить(ТекстОшибки);
	КонецЕсли;
	
	// Проверка значения.
	Если Не ЗначениеЗаполнено(Значение) И ТипЗаполнения = "Parameter" Тогда
		ТекстОшибки = НСтр("ru='Не заполнен реквизит Значение.';uk='Не заповнений реквізит Значення.'");
		МассивОшибок.Добавить(ТекстОшибки);
	КонецЕсли;
	...
	Если Тип <> "UserData" Тогда
		...
		Если Верх>Низ Тогда
			ТекстОшибки = НСтр("ru='Верхней координата не может быть больше нижней.';uk='Верхньої координата не може бути більше нижньої.'");
			МассивОшибок.Добавить(ТекстОшибки);
		КонецЕсли;
	...
	Если Тип = "Text" Тогда
		...
		Если Не ЗначениеЗаполнено(ПоложениеПоГоризонтали) Тогда
			ТекстОшибки = НСтр("ru='Не заполнен реквизит Горизонтальное положение.';uk='Не заповнений реквізит Горизонтальне положення.'");
			МассивОшибок.Добавить(ТекстОшибки);
		КонецЕсли;
		
		// Проверка ПоложениеПоВертикали.
		Если Не ЗначениеЗаполнено(ПоложениеПоВертикали) Тогда
			ТекстОшибки = НСтр("ru='Не заполнен реквизит Вертикальное положение.';uk='Не заповнений реквізит Вертикальне положення.'");
			МассивОшибок.Добавить(ТекстОшибки);
		КонецЕсли;
	...
	ИначеЕсли Тип = "Barcode" Тогда
		...
		Если Не ЗначениеЗаполнено(ТипШтрихкода) Тогда
			ТекстОшибки = НСтр("ru='Не заполнен реквизит Тип штрихкода.';uk='Не заповнений реквізит Тип штрихкода.'");
			МассивОшибок.Добавить(ТекстОшибки);
		КонецЕсли;
		...
		Если ПодписьШтрихкода Тогда
			Если Не ЗначениеЗаполнено(РазмерШрифта) Тогда
				ТекстОшибки = НСтр("ru='Не заполнен реквизит Размер шрифта подписи.';uk='Не заповнений реквізит Розмір шрифту підпису.'");
				МассивОшибок.Добавить(ТекстОшибки);
			КонецЕсли;
		КонецЕсли;
	...
	Если Тип = "Text" ИЛИ Тип = "Image" Тогда
		...
		Если РамкаСверху ИЛИ РамкаСлева ИЛИ РамкаСнизу ИЛИ РамкаСправа Тогда
			...
			Если Не ЗначениеЗаполнено(ТипРамки) Тогда
				ТекстОшибки = НСтр("ru='Не заполнен реквизит Тип рамки.';uk='Не заповнений реквізит Тип рамки.'");
				МассивОшибок.Добавить(ТекстОшибки);
			КонецЕсли;
			
			// Проверка ТолщинаРамки.
			Если Не ЗначениеЗаполнено(ТолщинаРамки) Тогда
				ТекстОшибки = НСтр("ru='Не заполнен реквизит Толщина рамки.';uk='Не заповнений реквізит Товщина рамки.'");
				МассивОшибок.Добавить(ТекстОшибки);
			КонецЕсли;
