...
Процедура Начать(Команда)
	...
	Если Не ЕстьОборудованиеДляОбмена() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбрано оборудование для обмена';uk='Не обране обладнання для обміну'"),,"Оборудование");
		Возврат;
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ОбменВыполняется Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='После закрытия формы обмен с оборудованием выполняться не будет.';uk='Після закриття форми обмін з обладнанням виконуватися не буде.'"));
