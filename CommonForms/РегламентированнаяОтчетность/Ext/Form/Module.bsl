...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Новости") Тогда
		...
		МодульОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
			...
			"ОбщаяФорма",
			,
			НСтр("ru='Новости: Регламентированная отчетность';uk='Новости: Регламентированная отчетность'"),
			Ложь,
	...
	ТекстОшибкиИнициализацииКонтекстаЭДО = НСтр("ru='Подождите, пожалуйста. Выполняется обновление модуля 1С-Отчетности...';uk='Подождите, пожалуйста. Выполняется обновление модуля 1С-Отчетности...'");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ПриЗакрытии_ЭлектронныеПредставленияРегламентированныхОтчетов" Тогда	
		Если ТипЗнч(Параметр) = Тип("Структура") И Параметр.Свойство("ЭтоУведомление") Тогда
			ШаблонПредупреждения = НСтр("ru='""%1"" отображается в разделе ""%2""';uk='""%1"" отображается в разделе ""%2""'");
			Если Параметр.ЭтоУведомление И Элементы.Разделы.ТекущаяСтраница = Элементы.СтраницаОтчеты Тогда
				ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПредупреждения, Строка(Источник), НСтр("ru='Уведомления';uk='Повідомлення'"));
				ПоказатьПредупреждение(, ТекстПредупреждения);
			ИначеЕсли Не Параметр.ЭтоУведомление И Элементы.Разделы.ТекущаяСтраница = Элементы.СтраницаУведомления Тогда
				ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПредупреждения, Строка(Источник), НСтр("ru='Отчеты';uk='Звіти'"));
				ПоказатьПредупреждение(, ТекстПредупреждения);	
			КонецЕсли;
		КонецЕсли;
...
Процедура УправлениеЭУ(Форма, ВыполнитьУстановкуСтраницы = Ложь)
	...
	Если Форма.КоличествоНовых > 0 Тогда
		...
		Форма.ГиперссылкаНовое = СтрШаблон(НСтр("ru='Новое (%1)';uk='Новое (%1)'"), ?(Форма.КоличествоНовых > 99, "99+", Форма.КоличествоНовых));
	КонецЕсли;
...
"Установка компоненты печати машиночитаемых форм невозможна.';uk='Не удалось подключить расширение работы с файлами!"
...
Процедура ОтправитьОтчет(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		Если ТипЗнч(ТекущиеДанные.РегламентированныйОтчет) = Тип("СправочникСсылка.МакетыПенсионныхДел")
			...
			ТекстСообщения = НСтр("ru='Макет пенсионного дела уже отправлен';uk='Макет пенсионного дела уже отправлен'");			
		...
		// заглушка до появления выгрузки
		Если СтрНайти(ТекущиеДанныеНаименованиеОтчета, "винограда") > 0 Тогда
			ТекстПредупреждения = НСтр("ru='Выгрузка, проверка выгрузки, проверка в интернете, отправка в электронном виде редакции формы, утвержденной ПП РФ №411 от 13.05.2016, будет реализована в будущих версиях программы.';uk='Выгрузка, проверка выгрузки, проверка в интернете, отправка в электронном виде редакции формы, утвержденной ПП РФ №411 от 13.05.2016, будет реализована в будущих версиях программы.'");
			ПоказатьПредупреждение(, ТекстПредупреждения);
			Возврат;
		КонецЕсли;
		...
		Если РегламентированнаяОтчетностьКлиент.ВебБраузер() = "MSIE" Тогда
			Если (ТекущиеДанныеНаименованиеОтчета = "РСВ-1 ПФР" И ТипЗнч(ТекущиеДанные.РегламентированныйОтчет) <> Тип("СправочникСсылка.ЭлектронныеПредставленияРегламентированныхОтчетов"))
				...
				ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Для выгрузки (проверки выгрузки, проверки в интернете, отправки) в браузере ""Internet Explorer"" необходимо открыть форму регламентированного отчета ""%1""';uk='Для выгрузки (проверки выгрузки, проверки в интернете, отправки) в браузере ""Internet Explorer"" необходимо открыть форму регламентированного отчета ""%1""'"),
			...
			Если ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
				...
				Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
					Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
						...
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 Отправка прервана.';uk='%1 Отправка прервана.'"), ОписаниеДохИно.ТекстСообщения);
						Сообщение.Сообщить();
					КонецЕсли;
					Возврат;
				КонецЕсли;
...
"отчетности в ПФР. Отправка прервана.';uk='Внимание! Отправлять в электронном виде форму РСВ-1 необходимо"
"из специализированного рабочего места подготовки квартальной"
"отчетности в ПФР. Отправка прервана.'"));
		...
		Если ТекущиеДанныеНаименованиеОтчета = "4-ФСС" Тогда
			Описание4_ФСС = ОписаниеРегОтчета(ТекущиеДанные.РегламентированныйОтчет);
			Если Описание4_ФСС.ВыбраннаяФорма = "ФормаОтчета2016Кв3" Тогда
				ПоказатьПредупреждение(, НСтр("ru='Выгрузка, проверка выгрузки, проверка в интернете, отправка в электронном виде формы в редакции приказа ФСС РФ от 04.07.2016 № 260 будут реализованы в будущих версиях программы.';uk='Выгрузка, проверка выгрузки, проверка в интернете, отправка в электронном виде формы в редакции приказа ФСС РФ от 04.07.2016 № 260 будут реализованы в будущих версиях программы.'"));
				Возврат;
			КонецЕсли;
		КонецЕсли;
		...
		Если ВыполнитьСтандартнуюОтправкуОтчета Тогда
			...
			Если ТекущиеДанные.НеОтправляетсяВКонтролирующийОрган Тогда
				...
				ПредупредитьОбОграничениях(Закладка, НСтр("ru='Отправка не предусмотрена';uk='Отправка не предусмотрена'"));
			...
			Иначе
				...
				Если ЗначениеЗаполнено(КонтролирующийОрган) Тогда
					...
					КонецЕсли;
				Иначе
					ПоказатьПредупреждение(, НСтр("ru='Не указан контролирующий орган. Отправка невозможна';uk='Не указан контролирующий орган. Отправка невозможна'"));
				КонецЕсли;
...
Процедура ПроверитьВыгрузку(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.ЭлектронныеПредставленияРегламентированныхОтчетов") Тогда
			...
			ПредупредитьОбОграничениях(Закладка, НСтр("ru='Проверка выгрузки не предусмотрена';uk='Проверка выгрузки не предусмотрена'"));
			Возврат;
		...
		ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			...
			Если ТекущиеДанныеНаименованиеОтчета = "Доходы, выплаченные иностранным организациям" Тогда
				...
				Если ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
					...
					Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
						Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
							Сообщение = Новый СообщениеПользователю;
							Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 Проверка выгрузки прервана.';uk='%1 Проверка выгрузки прервана.'"), ОписаниеДохИно.ТекстСообщения);
							Сообщение.Сообщить();
						КонецЕсли;
						Возврат;
					КонецЕсли;
			...
			Если ТекущиеДанныеНаименованиеОтчета = "4-ФСС" Тогда
				Описание4_ФСС = ОписаниеРегОтчета(Элементы.Отчеты.ТекущиеДанные.РегламентированныйОтчет);
				Если Описание4_ФСС.ВыбраннаяФорма = "ФормаОтчета2016Кв3" Тогда
					ПоказатьПредупреждение(, НСтр("ru='Выгрузка, проверка выгрузки, проверка в интернете, отправка в электронном виде формы в редакции приказа ФСС РФ от 04.07.2016 № 260 будут реализованы в будущих версиях программы.';uk='Выгрузка, проверка выгрузки, проверка в интернете, отправка в электронном виде формы в редакции приказа ФСС РФ от 04.07.2016 № 260 будут реализованы в будущих версиях программы.'"));
					Возврат;
				КонецЕсли;
...
Процедура ПроверитьВыгрузкуЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	Если НЕ РегламентированнаяОтчетностьКлиент.ПроверитьРегламентированныеОтчеты(СписокОтчетов) Тогда
		ПоказатьПредупреждение(,НСтр("ru='Проверка прервана!';uk='Проверка прервана!'"));
	КонецЕсли;
...
Процедура ПроверитьВыгрузкуЗавершениеИзСтатформы(Отказ, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыгрузки = Истина Тогда
		ПоказатьПредупреждение(,НСтр("ru='Ошибок не обнаружено!';uk='Ошибок не обнаружено!'"));
	КонецЕсли;
...
Процедура ОтправитьСверку(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ЕщеНеОтправлялся(ОтправляемыйОбъект) Тогда
			...
			КонецЕсли;
		Иначе
			ПоказатьПредупреждение(, НСтр("ru='Запрос на сверку уже отправлен';uk='Запрос на сверку уже отправлен'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ОтправитьЗапросНаВыпискуИзЕГРЮЛ_ЕГРИП(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ru='Запрос уже отправлен';uk='Запрос уже отправлен'"));
		КонецЕсли;
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Иначе
			...
			Если ТипЗнч(ОтправляемыйОбъект) = Тип("СправочникСсылка.ОписиИсходящихДокументовВНалоговыеОрганы") Тогда
				ТекстСообщения = НСтр("ru='Ответ на требование уже отправлен';uk='Ответ на требование уже отправлен'");
			ИначеЕсли ТипЗнч(ОтправляемыйОбъект) = Тип("ДокументСсылка.ПоясненияКДекларацииПоНДС") Тогда
				ТекстСообщения = НСтр("ru='Пояснение уже отправлено';uk='Пояснение уже отправлено'");
			Иначе
				ТекстСообщения = НСтр("ru='Письмо уже отправлено';uk='Письмо уже отправлено'");
			КонецЕсли;
...
Процедура ПечатьПисьма(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ТипЗнч(ПечатаемыйОбъект) = Тип("СправочникСсылка.ДокументыРеализацииПолномочийНалоговыхОрганов") Тогда
			ПоказатьПредупреждение(, НСтр("ru='Печать требования не предусмотрена';uk='Печать требования не предусмотрена'"));
			Возврат;
		ИначеЕсли ТипЗнч(ПечатаемыйОбъект) = Тип("СправочникСсылка.ОписиИсходящихДокументовВНалоговыеОрганы")
			...
			ПоказатьПредупреждение(, НСтр("ru='Печать ответа на требование не предусмотрена';uk='Печать ответа на требование не предусмотрена'"));
			Возврат;
		КонецЕсли;
...
Процедура СоздатьОтвет(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ТранспортноеСообщение") Тогда
			...
			ТекстПредупреждения = НСтр("ru='Создание ответа на это письмо не предусмотрено';uk='Создание ответа на это письмо не предусмотрено'");
			ПоказатьПредупреждение(,ТекстПредупреждения);
...
Процедура ПодтвердитьСкрытие(ВыполняемоеОповещение)
	...
	Если ТекущаяСтраница = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Отчеты") Тогда
		...
		Если ВыделеноНесколькоСтрок Тогда 
			ВидОбъекта = НСтр("ru='отчеты';uk='отчеты'");
		Иначе
			ВидОбъекта = НСтр("ru='отчет';uk='отчет'");
		КонецЕсли;
		
	ИначеЕсли ТекущаяСтраница = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Уведомления") Тогда
		
		Если ВыделеноНесколькоСтрок Тогда 
			ВидОбъекта = НСтр("ru='уведомления';uk='повідомлення'");
		Иначе
			ВидОбъекта = НСтр("ru='уведомление';uk='уведомление'");
		КонецЕсли;

	ИначеЕсли ТекущаяСтраница = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Сверки") Тогда
		
		Если ВыделеноНесколькоСтрок Тогда 
			ВидОбъекта = НСтр("ru='запросы на сверку';uk='запросы на сверку'");
		Иначе
			ВидОбъекта = НСтр("ru='запрос на сверку';uk='запрос на сверку'");
		КонецЕсли;

	ИначеЕсли ТекущаяСтраница = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.ЕГРЮЛ") Тогда
		
		Если ВыделеноНесколькоСтрок Тогда 
			ВидОбъекта = НСтр("ru='выписки из ЕГРЮЛ';uk='выписки из ЕГРЮЛ'");
		Иначе
			ВидОбъекта = НСтр("ru='выписку из ЕГРЮЛ';uk='выписку из ЕГРЮЛ'");
		КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Входящие")
		...
		Если ВыделеноНесколькоСтрок Тогда 
			ВидОбъекта = НСтр("ru='письма';uk='письма'");
		Иначе
			ВидОбъекта = НСтр("ru='письмо';uk='письмо'");
		КонецЕсли;
...
"""Настройка"" - ""Скрытые отчеты и документы"" формы 1С-Отчетность';uk='Вы уверены, что хотите скрыть %1?"
""
"Скрытые отчеты и документы можно восстановить в разделе"
...
Процедура ОбновитьКоличествоНовых(Форма)
	...
	Если КоличествоПолученныеСообщения = 0 Тогда
		СтрокаБлокПолученныеСообщения.ЗаголовокБлока = НСтр("ru='Полученные сообщения';uk='Полученные сообщения'");	
	Иначе
		СтрокаБлокПолученныеСообщения.ЗаголовокБлока = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Полученные сообщения (%1)';uk='Полученные сообщения (%1)'"), КоличествоПолученныеСообщения);
	КонецЕсли;
	
	Если КоличествоОбработанныеЗапросы = 0 Тогда
		СтрокаБлокОбработанныеЗапросы.ЗаголовокБлока = НСтр("ru='Обработанные запросы';uk='Обработанные запросы'");	
	Иначе
		СтрокаБлокОбработанныеЗапросы.ЗаголовокБлока = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Обработанные запросы (%1)';uk='Обработанные запросы (%1)'"), КоличествоОбработанныеЗапросы);
	КонецЕсли;

	Если КоличествоЗавершенныеОтправки = 0 Тогда
		СтрокаБлокЗавершенныеОтправки.ЗаголовокБлока = НСтр("ru='Завершенные отправки';uk='Завершенные отправки'");	
	Иначе
		СтрокаБлокЗавершенныеОтправки.ЗаголовокБлока = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Завершенные отправки (%1)';uk='Завершенные отправки (%1)'"), КоличествоЗавершенныеОтправки);
	КонецЕсли;

	// Количество в разделе "Новое"
	Если Форма.КоличествоНовых > 0 Тогда
		ТекстНовое = НСтр("ru='Новое (%1)';uk='Новое (%1)'");
		ТекстНовое = СтрШаблон(ТекстНовое, ?(Форма.КоличествоНовых > 99, "99+", Форма.КоличествоНовых));
		Форма.ГиперссылкаНовое = ТекстНовое;
	Иначе
		Форма.ГиперссылкаНовое = НСтр("ru='Новое';uk='Нове'");
	КонецЕсли;
...
Процедура УправлениеЭУПанельУстановкаКомпонентыПечати(ОперацияСКомпонентойСовершена = Ложь)
	...
	Если РегламентированнаяОтчетностьКлиент.КомпонентаПечатиМашиночитаемыхФормДоступна() Тогда
		Элементы.ПереустановитьКомпонентыПечатиМашиночитаемыхФорм.Заголовок = НСтр("ru='Переустановить компоненты печати машиночитаемых форм';uk='Переустановить компоненты печати машиночитаемых форм'");
	Иначе
		Элементы.ПереустановитьКомпонентыПечатиМашиночитаемыхФорм.Заголовок = НСтр("ru='Установка компоненты печати';uk='Установка компоненты печати'");
	КонецЕсли;
...
Процедура ПроизвестиОбменСКонтролирующимОрганами()
	...
	Если ВыбранныеОрганизации.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Организация не выбрана';uk='Организация не выбрана'"), ,"Организация");
		Возврат;
	КонецЕсли;
	...
	Если ТекущиеДанные <> Неопределено Тогда
				...
				Если НаименованиеОтчета = "Доходы, выплаченные иностранным организациям" Тогда
					...
					Если ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
						...
						Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
							Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
								Сообщение = Новый СообщениеПользователю;
								Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 Печать прервана.';uk='%1 Печать прервана.'"), ОписаниеДохИно.ТекстСообщения);
								Сообщение.Сообщить();
							КонецЕсли;
							Возврат;
						КонецЕсли;
		...
		ИначеЕсли ВидПечати = "ПечатьОтчетШтрих" Тогда
			...
			Иначе
				...
				// Печать объекта, не входящего в БРО
				ПредупредитьОбОграничениях(Закладка, НСтр("ru='Печать со штрихкодом PDF417 не предусмотрена';uk='Печать со штрихкодом PDF417 не предусмотрена'"));
...
Процедура ПечатьРегламентированногоОтчетаШтрих(Ссылка, Наименование)
	...
	Если Наименование = "Декларация по УСН" Тогда
		...
		Если ОписаниеУСН.ВыбраннаяФорма = "ФормаОтчета2014Кв1" И ОписаниеУСН.РекомендованнаяФорма Тогда
			ПоказатьПредупреждение(, НСтр("ru='Печать с двухмерным штрихкодом PDF417 формы, рекомендованной письмом ФНС от 20.05.2015 № ГД-4-3/8533@, не предусмотрена.';uk='Печать с двухмерным штрихкодом PDF417 формы, рекомендованной письмом ФНС от 20.05.2015 № ГД-4-3/8533@, не предусмотрена.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если Наименование = "Доходы, выплаченные иностранным организациям" Тогда
		...
		Если ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
			...
			Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
				Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 Печать формы с двухмерным штрихкодом PDF417 прервана.';uk='%1 Печать формы с двухмерным штрихкодом PDF417 прервана.'"), ОписаниеДохИно.ТекстСообщения);
					Сообщение.Сообщить();
				КонецЕсли;
				Возврат;
			КонецЕсли;
...
Процедура ВыгрузкаРегламентированногоОтчета(Ссылка, ОрганизацияСсылка, НаименованиеОтчета, ВариантОтчета, ПредставлениеПериода);
	...
	// заглушка до появления выгрузки
	Если СтрНайти(НаименованиеОтчета, "винограда") > 0 Тогда
		ТекстПредупреждения = НСтр("ru='Выгрузка, проверка выгрузки, проверка в интернете, отправка в электронном виде редакции формы, утвержденной ПП РФ №411 от 13.05.2016, будет реализована в будущих версиях программы.';uk='Выгрузка, проверка выгрузки, проверка в интернете, отправка в электронном виде редакции формы, утвержденной ПП РФ №411 от 13.05.2016, будет реализована в будущих версиях программы.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
	...
	Если НаименованиеОтчета = "Доходы, выплаченные иностранным организациям" Тогда
		...
		Если ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
			...
			Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
				Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 Выгрузка прервана.';uk='%1 Выгрузка прервана.'"), ОписаниеДохИно.ТекстСообщения);
					Сообщение.Сообщить();
				КонецЕсли;
				Возврат;
			КонецЕсли;
...
"отчетности в ПФР. Выгрузка прервана.';uk='Внимание! Выгружать в электронном виде первичную форму РСВ-1 необходимо"
"в комплекте с документами квартальной отчетности персонифицированного"
"учета из специализированного рабочего места подготовки квартальной"
...
"Необходимо сначала осуществить выгрузку из формы отчета! Выгрузка прервана.';uk='Для отчета ""Статистика: Форма П-1 за %1"" не выбрано Приложение №3."
"Необходимо сначала осуществить выгрузку из формы отчета! Выгрузка прервана.'"), ПредставлениеПериода);
	...
	Если НаименованиеОтчета = "Статистика: Форма П-1 Приложение 3" Тогда 
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выгрузка отчета ""Статистика: форма П-1 (приложение 3) за %1"" производится из формы П-1 за соответствующий период.';uk='Выгрузка отчета ""Статистика: форма П-1 (приложение 3) за %1"" производится из формы П-1 за соответствующий период.'"), ПредставлениеПериода);
		Сообщение.Сообщить();
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
	
	Если НаименованиеОтчета = "4-ФСС" Тогда
		Описание4_ФСС = ОписаниеРегОтчета(Ссылка);
		Если Описание4_ФСС.ВыбраннаяФорма = "ФормаОтчета2016Кв3" Тогда
			ПоказатьПредупреждение(, НСтр("ru='Выгрузка, проверка выгрузки, проверка в интернете, отправка в электронном виде формы в редакции приказа ФСС РФ от 04.07.2016 № 260 будут реализованы в будущих версиях программы.';uk='Выгрузка, проверка выгрузки, проверка в интернете, отправка в электронном виде формы в редакции приказа ФСС РФ от 04.07.2016 № 260 будут реализованы в будущих версиях программы.'"));
			Возврат;
		КонецЕсли;
...
Процедура ВыгрузкаРегламентированногоОтчетаПослеПодключенияРасширенияРаботыСФайлами(Подключено, ДополнительныеПараметры) Экспорт
	...
	НСтр("ru='%1';uk='%1'"), ТекстВопроса), РежимДиалогаВопрос.ДаНет);
...
Процедура ВыгрузкаРегламентированногоОтчетаЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	Если НЕ РегламентированнаяОтчетностьКлиент.ВыгрузитьОтчеты(
		...
		ПоказатьПредупреждение(,НСтр("ru='Выгрузка прервана!';uk='Выгрузка прервана!'"));
	КонецЕсли;
...
Процедура НапечататьОбъектНеВходящийВБРО(Ссылка, СведенияПоОбъекту)
	...
	Если СтандартнаяПечатьОбъекта Тогда
		...
		Если МакетПечати <> Неопределено Тогда
			...
			Исключение
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='При печати возникла ошибка';uk='При печати возникла ошибка'"));
			КонецПопытки;
...
Процедура ПечатьПротоколовПоСсылке(
	...
	Если НеОтправляетсяВКонтролирующийОрган Тогда
		ПредупредитьОбОграничениях(Закладка, НСтр("ru='Печать протоколов не предусмотрена';uk='Печать протоколов не предусмотрена'"));
	Иначе
		...
		Если НЕ ЗначениеЗаполнено(ПоследнийЦиклИлиОтправка) ИЛИ НЕ ПротоколЕсть Тогда
			...
			Иначе
				ТекстПредупреждения = НСтр("ru='У пользователя недостаточно прав для просмотра и печати протоколов.';uk='У пользователя недостаточно прав для просмотра и печати протоколов.'");
				ПоказатьПредупреждение(,ТекстПредупреждения);
			КонецЕсли;
			Возврат;
		КонецЕсли;
...
"%1';uk='Ошибка при проверке результата обновления информации механизма онлайн-сервисов регламентированной отчетности"
"%1'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
					 
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ru='Регламентированная отчетность. Проверка результата обновления информации механизма онлайн-сервисов регламентированной отчетности';uk='Регламентированная отчетность. Проверка результата обновления информации механизма онлайн-сервисов регламентированной отчетности'"), 
...
Процедура ПоказатьРезультатПроверкиОбновленияИнформацииМОС(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Обновлены внешние формы отчетов - %1';uk='Обновлены внешние формы отчетов - %1'"),
	...
	Для каждого СтрокаСообщения Из РезультатПроверки Цикл
		...
		ТекстСообщения = ТекстСообщения + Символы.ПС + Символы.ПС
					   + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1';uk='%1'"), СтрокаСообщения);
...
"%1';uk='Ошибка при подсчете количества объектов, не отображаемых в форме 1С-Отчетность "
"%1'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
					 
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ru='Регламентированная отчетность. Проверка количества незаписанных объектов';uk='Регламентированная отчетность. Проверка количества незаписанных объектов'"), 
			"Ошибка", 
			ТекстСообщенияОбОшибке,
			,
			Истина);
...
Процедура ПоказатьРезультатПроверки()
	...
	ТекстВопроса = НСтр("ru='Для корректной работы формы необходимо проиндексировать объекты';uk='Для корректной работы формы необходимо проиндексировать объекты'");
...
Процедура ОнлайнПроверка(Закладка)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если Представление = "4-ФСС" Тогда
			Описание4_ФСС = ОписаниеРегОтчета(ТекущиеДанные.РегламентированныйОтчет);
			Если Описание4_ФСС.ВыбраннаяФорма = "ФормаОтчета2016Кв3" Тогда
				ПоказатьПредупреждение(, НСтр("ru='Выгрузка, проверка выгрузки, проверка в интернете, отправка в электронном виде формы в редакции приказа ФСС РФ от 04.07.2016 № 260 будут реализованы в будущих версиях программы.';uk='Выгрузка, проверка выгрузки, проверка в интернете, отправка в электронном виде формы в редакции приказа ФСС РФ от 04.07.2016 № 260 будут реализованы в будущих версиях программы.'"));
				Возврат;
		...
		Если Представление = "Доходы, выплаченные иностранным организациям" Тогда
			...
			Если ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
				...
				Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
					Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
						Сообщение = Новый СообщениеПользователю;
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 Проверка в интернете прервана.';uk='%1 Проверка в интернете прервана.'"), ОписаниеДохИно.ТекстСообщения);
						Сообщение.Сообщить();
					КонецЕсли;
					Возврат;
				КонецЕсли;
...
Процедура ПредупредитьОбОграничениях(Закладка, Знач ТекстПредупреждения, Знач ВидОбъекта = Неопределено)
	...
	ТекстПредупреждения = НСтр("ru='Для %1 ""%2"" ';uk='Для %1 ""%2"" '") + ТекстПредупреждения;
	...
	Если ВидОбъекта = Неопределено Тогда
		Если ЭтоОтчеты Тогда
			ВидОбъекта = НСтр("ru='отчета';uk='отчета'");
		Иначе
			ВидОбъекта = НСтр("ru='уведомления';uk='повідомлення'"); 
		КонецЕсли;
	КонецЕсли;
...
Процедура СоздатьНовоеПисьмо()
	...
	СписокВидовПисьма.Добавить(НСтр("ru='Письмо';uk='Лист'"));
	СписокВидовПисьма.Добавить(НСтр("ru='Ответ на требование';uk='Ответ на требование'"));
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗавершениеВыбораВидаПисьма", ЭтотОбъект);
	СписокВидовПисьма.ПоказатьВыборЭлемента(ОписаниеОповещения, НСтр("ru='Выберите вид письма';uk='Выберите вид письма'"));
...
Процедура ЗавершениеВыбораВидаПисьма(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат.Значение = НСтр("ru='Письмо';uk='Лист'") Тогда
		 СоздатьНовоеИсходящееПисьмо();
	ИначеЕсли Результат.Значение = НСтр("ru='Ответ на требование';uk='Ответ на требование'") Тогда 
...
Процедура СоздатьНовуюСверку()
	...
	СписокВыбораВидаСверки.ПоказатьВыборЭлемента(ОписаниеОповещения, НСтр("ru='Выберите вид сверки';uk='Выберите вид сверки'"));
