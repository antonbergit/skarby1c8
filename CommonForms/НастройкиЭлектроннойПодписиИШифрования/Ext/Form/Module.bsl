...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Элементы.Программы.Заголовок =
			НСтр("ru='Список программ, предусмотренных администратором, которые можно использовать на компьютере';uk='Список программ, предусмотренных администратором, которые можно использовать на компьютере'");
	КонецЕсли;
...
"Обратитесь к администратору.';uk='Невозможно сохранить путь к программе. Отсутствует право сохранения данных."
	...
	Если Константы.ИспользоватьШифрование.Получить() Тогда
		Если ЭлектроннаяПодписьСлужебный.ИспользоватьЭлектроннуюПодписьВМоделиСервиса() Тогда
			Элементы.ДобавитьИзУстановленныхНаКомпьютере.Заголовок =
				НСтр("ru='Из установленных в облачном сервисе и на компьютере...';uk='Из установленных в облачном сервисе и на компьютере...'");
		Иначе
			Элементы.ДобавитьИзУстановленныхНаКомпьютере.Заголовок =
				НСтр("ru='Из установленных на компьютере...';uk='З установлених на комп`ютері...'");
		КонецЕсли;
	Иначе
		Если ЭлектроннаяПодписьСлужебный.ИспользоватьЭлектроннуюПодписьВМоделиСервиса() Тогда
			Элементы.ДобавитьИзУстановленныхНаКомпьютере.Заголовок =
...
"должна быть установлена хотя бы одна из программ в списке.';uk='Дозволяє не встановлювати програму на комп`ютер користувача"
"для перевірки електронних підписів і сертифікатів."
""
"Важливо: на кожний комп`ютер, де працює сервер 1С:Підприємства"
"або веб-сервер, що використовує файлову інформаційну базу,"
"повинна бути встановлена хоча б одна із програм у списку.'");
...
"повинна бути встановлена хоча б одна із програм у списку.'");
	...
	Если Не Константы.ИспользоватьЭлектронныеПодписи.Получить() Тогда
		ЗаголовокФлажка = НСтр("ru='Шифровать и расшифровывать на сервере';uk='Шифрувати й розшифровувати на сервері'");
		ПодсказкаФлажка =
...
"Важно: на каждый компьютер, где работает сервер 1С:Предприятия"
"или веб-сервер, использующий файловую информационную базу,"
"должна быть установлена программа и сертификат с закрытым ключом.';uk='Дозволяє не встановлювати програму й сертифікат"
"на комп`ютер користувача для шифрування й розшифрування."
""
"Важливо: на кожний комп`ютер, де працює сервер 1С:Підприємства"
"або веб-сервер, що використовує файлову інформаційну базу,"
"повинна бути встановлена програма й сертифікат із закритим ключем.'");
...
"повинна бути встановлена програма й сертифікат із закритим ключем.'");
	Иначе
		ЗаголовокФлажка = НСтр("ru='Подписывать и шифровать на сервере';uk='Підписувати й шифрувати на сервері'");
		ПодсказкаФлажка =
			НСтр("ru='Позволяет не устанавливать программу и сертификат"
...
"или веб-сервер, использующий файловую информационную базу,"
"должна быть установлена программа и сертификат с закрытым ключом.';uk='Дозволяє не встановлювати програму й сертифікат"
"на комп`ютер користувача для підписання, шифрування й розшифрування."
""
"Важливо: на кожний комп`ютер, де працює сервер 1С:Підприємства"
"або веб-сервер, що використовує файлову інформаційну базу,"
"повинна бути встановлена програма й сертифікат із закритим ключем.'");
...
Процедура ОбработчикОжиданияОпределитьУстановленныеПрограммыЦиклНачало(Контекст)
	...
	ИначеЕсли ОписаниеПрограммы.ЭтоПрограммаОблачногоСервиса Тогда
		ОбновитьЗначение(ОписаниеПрограммы.РезультатПроверки, НСтр("ru='Доступен.';uk='Доступен.'"));
...
Процедура ОбработчикОжиданияОпределитьУстановленныеПрограммыЦиклПродолжение(Менеджер, Контекст) Экспорт
	...
	Если Ошибка.НеУказанПуть Тогда
		ОбновитьЗначение(ОписаниеПрограммы.РезультатПроверки, НСтр("ru='Не указан путь к программе.';uk='Не зазначений шлях до програми.'"));
		ОбновитьЗначение(ОписаниеПрограммы.Установлена, "");
	Иначе
