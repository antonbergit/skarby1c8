...
Процедура ПриОткрытии(Отказ)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Просмотр сертификата: %1';uk='Просмотр сертификата: %1'"), Сертификат.Наименование);
...
Процедура ПроверитьЗавершение(Результат, ВходящийКонтекст) Экспорт
	...
	ИначеЕсли Результат.Выполнено И Не Результат.Валиден Тогда
		ПоказатьПредупреждение(, НСтр("ru='Сертификат не действителен.';uk='Сертификат не действителен.'"));
	КонецЕсли;
...
Процедура ОткрытьСертификатПослеЭкспортаСертификата(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось открыть сертификат.';uk='Не удалось открыть сертификат.'"));
	КонецЕсли;
