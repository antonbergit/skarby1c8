...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей") Тогда
		ТекстИсключения = НСтр("ru='Заполнение реквизитов в данной конфигурации невозможно.';uk='Заполнение реквизитов в данной конфигурации невозможно.'");
	...
	Элементы.Регион.СписокВыбора.Вставить(0, "", НСтр("ru='Все регионы';uk='Всі регіони'"));
	Регион = "";
...
"Указать сейчас?';uk='Необхідно вказати пароль до Інтернет-підтримки користувачів"
"і встановити прапорець ""Запам`ятати пароль""."
"Вказати зараз?'");
			ОписаниеОповещения = Новый ОписаниеОповещения("ПодключитьИнтернетПоддержку", ЭтотОбъект);
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	...
	ИначеЕсли КоличествоНайденных > 20 Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Найдено слишком много похожих контрагентов (%1). Уточните реквизиты для поиска.';uk='Знайдене занадто багато схожих контрагентів (%1). Уточніть реквізити для пошуку.'"),
			КоличествоНайденных);
		ПоказатьПредупреждение(, ТекстПредупреждения);
	ИначеЕсли НЕ ПустаяСтрока(СтрокаПоиска) И КоличествоНайденных = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Ничего не найдено. Уточните реквизиты для поиска.';uk='Нічого не знайдене. Уточніть реквізити для пошуку.'"));
	КонецЕсли;
...
Процедура ВыполнитьВыборКонтрагента()
	...
	Если ТекДанные = Неопределено Тогда
		Если Контрагенты.Количество() = 0 Тогда
			ТекстПредупреждения = НСтр("ru='Ничего не найдено. Уточните реквизиты и нажмите ""Найти"".';uk='Нічого не знайдене. Уточніть реквізити й натисніть ""Знайти"".'");
			ПоказатьПредупреждение(, ТекстПредупреждения);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ТекДанные.ИНН) Тогда
		ТекстПредупреждения = НСтр("ru='Выберите контрагента, у которого указан ИНН.';uk='Выберите контрагента, у которого указан ИНН.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура НайтиРеквизитыНаСервере(ОписаниеОшибки)
	...
	КоличествоНайденных = 0;
		...
		Информация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Показаны первые 20 контрагентов из %1 найденных. Уточните реквизиты для поиска.';uk='Показані перші 20 контрагентів з %1 знайдених. Уточніть реквізити для пошуку.'"), 
			12999);
		// Отладка конец
