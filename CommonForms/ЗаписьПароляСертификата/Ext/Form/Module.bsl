...
""
"Запоминание и запись пароля невозможны.';uk='У сертифіката встановлений посилений захист закритого ключа."
"У такому випадку пароль запитує програма електронного підпису й шифрування,"
"а програма 1С:Підприємства повинна передати порожній пароль, щоб не було помилки."
""
"Запам`ятовування й запис пароля неможливий.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
"Невозможно проверить пароль перед записью.';uk='У сертифіката не зазначена програма для закритого ключа."
...
"сертификата ""Пользователь"" и ""Добавил"" или администратор.';uk='Недостатньо прав для запису пароля."
""
"Запис пароля може зробити користувач, зазначений у полях"
...
Процедура УстановитьЗаголовокНадписиИПометкуКомандыДоступенВсем()
	...
	Если ЗначениеЗаполнено(Пользователь) Тогда
		Элементы.НадписьДоступенПользователю.Заголовок = НСтр("ru='Пароль доступен пользователю:';uk='Пароль доступний користувачеві:'")
			+ " " + Пользователь;
	Иначе
		Элементы.НадписьДоступенПользователю.Заголовок = НСтр("ru='Пароль доступен всем пользователям.';uk='Пароль доступний усім користувачам.'")
	КонецЕсли;
