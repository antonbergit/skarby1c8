...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ПустаяСтрока(АдресВХранилище) И ВыбранныеАдресаты.ПолучитьЭлементы().Количество() > 0 Тогда
		...
		Если ЗавершениеРаботы Тогда
			ТекстПредупреждения = НСтр("ru='Выбранные адресаты не перенесены.';uk='Выбранные адресаты не перенесены.'");
			Возврат;
		КонецЕсли;
		
		ТекстВопроса = НСтр("ru='Перенести отмеченных адресатов?';uk='Перенести відзначених адресатів?'");
		
		Оповещение = Новый ОписаниеОповещения("ПередЗакрытиемПереносПредложен", ЭтотОбъект);
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНетОтмена, , КодВозвратаДиалога.Да, НСтр("ru='Адресная книга';uk='Адресна книга'"));
	...
	Если НеПоказыватьВопросВыбораГруппы = Ложь Тогда
		Для Каждого ВыделеннаяСтрока Из Элементы.СписокКонтрагентов.ВыделенныеСтроки Цикл
			...
			Если ВыделеннаяСтрока.Пустая() Или Данные.ЭтоГруппа Тогда
				...
				ПараметрыВопроса.Вставить("Заголовок", НСтр("ru='Подтверждение выбора группы';uk='Підтвердження вибору групи'"));
				ПараметрыВопроса.Вставить("ТекстСообщения", НСтр("ru='Выбрать всех контрагентов группы?';uk='Вибрати всіх контрагентів групи?'"));
