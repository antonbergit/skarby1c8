...
Процедура ЗагрузитьДанные(ОтборПрофилейТолькоТекущегоПользователя)
	...
	Запрос.УстановитьПараметр("ИдентификаторПрофиляРедактированиеЦенДокументов", Новый УникальныйИдентификатор("76337579-bff4-11df-9174-e0cb4ed5f4c3"));
	Запрос.УстановитьПараметр("ИдентификаторПрофиляРедактированиеНоменклатуры" , Новый УникальныйИдентификатор("76337580-bff4-11df-9174-e0cb4ed5f4c3"));
	Запрос.УстановитьПараметр("ИдентификаторПрофиляВозвратыОтПокупателей" 	   , Новый УникальныйИдентификатор("76337581-bff4-11df-9174-e0cb4ed5f4c3"));
	Запрос.УстановитьПараметр("ИдентификаторПрофиляВозвратыПоставщикам" 	   , Новый УникальныйИдентификатор("76337582-bff4-11df-9174-e0cb4ed5f4c3"));
	Запрос.УстановитьПараметр("ИдентификаторПрофиляСинхронизацияДанных" 	   , Новый УникальныйИдентификатор("04937803-5dba-11df-a1d4-005056c00008"));
	...
	// УНФ
	РазрешитьРедактированиеЦенВДокументах = РазрешитьРедактированиеЦенВДокументах(Параметры.Пользователь);
	РазрешитьРедактированиеНоменклатуры   = РазрешитьРедактированиеНоменклатуры(Параметры.Пользователь);
			
	УстановитьПривилегированныйРежим(Истина);
	Для каждого СвойстваПрофиля Из Профили Цикл
		
		СвойстваПрофиля.ПрофильОписание = Справочники.ПрофилиГруппДоступа.ОписаниеПоставляемогоПрофиля(СвойстваПрофиля.Профиль);
...
Процедура ЗаписатьИзмененияНаСервере(Отказ)
	...
	// УНФ
	ВыбранныеПрофили = Профили.Выгрузить(, "Профиль, Пометка");
	ДобавитьДополнительныеПрофили(ВыбранныеПрофили);
	// Конец УНФ
...
"якому дозволений вхід в програму.'");
					...
					КонецЕсли;
				КонецЕсли;
			ИначеЕсли Выборка.Профиль.ИдентификаторПоставляемыхДанных = Новый УникальныйИдентификатор("76337581-bff4-11df-9174-e0cb4ed5f4c3") Тогда // УНФ - ВозвратыОтПокупателей
				...
				ИдентификаторПоставляемыхДанных = Новый УникальныйИдентификатор("76337576-bff4-11df-9174-e0cb4ed5f4c3");
				ПрофильПродажи = Справочники.ПрофилиГруппДоступа.НайтиПоРеквизиту("ИдентификаторПоставляемыхДанных", ИдентификаторПоставляемыхДанных);
				
				Отбор = Новый Структура("ГруппаДоступа, ВидДоступа", ПрофильПродажи, Справочники.Контрагенты.ПустаяСсылка());
...
Процедура ПриИзмененииТекущегоПрофиля(Знач Форма)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		// Гунько 2022-02-14
		//Если ИмяТекущегоПрофиля = "Продажи" Тогда
...
Функция ИмяТекущегоПрофиля(Профиль)
	
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Профиль, "Наименование");
	
КонецФункции

&НаСервереБезКонтекста
Функция НастройкаВидовДоступаДоступнаДляПрофиля(ВидыДоступа, Профиль)
	
	Если НЕ ПолучитьФункциональнуюОпцию("ОграничиватьДоступНаУровнеЗаписей") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ДоступныеВидыДоступа = ВидыДоступа.НайтиСтроки(Новый Структура("ГруппаДоступа", Профиль));
	Возврат ДоступныеВидыДоступа.Количество() > 0;
	
КонецФункции

&НаСервереБезКонтекста
Функция РазрешитьРедактированиеНоменклатуры(Пользователь)
...
&НаСервереБезКонтекста
Функция РазрешитьРедактированиеЦенВДокументах(Пользователь)
...
&НаСервере
Функция ДобавитьДополнительныеПрофили(ВыбранныеПрофили)

	ЕстьПрофильПродажи = Ложь;
	НуженДоступКВозвратамОтПокупателей = Ложь;
	НуженДоступКВозвратамПоставщикам   = Ложь;
	...
	Запрос.УстановитьПараметр("ИдентификаторПрофиляАдминистратор", Новый УникальныйИдентификатор("6c4b0307-43a4-4141-9c35-3dd7e9586d41"));
	Запрос.УстановитьПараметр("ИдентификаторПрофиляПродажи", Новый УникальныйИдентификатор("76337576-bff4-11df-9174-e0cb4ed5f4c3"));
	Запрос.УстановитьПараметр("ИдентификаторПрофиляЗакупки", Новый УникальныйИдентификатор("76337577-bff4-11df-9174-e0cb4ed5f4c3"));
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		
		ЕстьПрофильПродажи = Выборка.ЕстьПрофильПродажи;
		
		Если НЕ Выборка.ЕстьПрофильАдминистратор
			И НЕ (Выборка.ЕстьПрофильПродажи И Выборка.ЕстьПрофильЗакупки) Тогда
			
			НуженДоступКВозвратамОтПокупателей = Выборка.ЕстьПрофильПродажи;
			НуженДоступКВозвратамПоставщикам   = Выборка.ЕстьПрофильЗакупки;
			
		КонецЕсли;
		
	КонецЕсли;
	
	ИдентификаторПоставляемыхДанных = Новый УникальныйИдентификатор("76337579-bff4-11df-9174-e0cb4ed5f4c3");
	ПрофильРедактированиеЦенДокументов = Справочники.ПрофилиГруппДоступа.НайтиПоРеквизиту("ИдентификаторПоставляемыхДанных", ИдентификаторПоставляемыхДанных);
	Если ЗначениеЗаполнено(ПрофильРедактированиеЦенДокументов) Тогда
		НоваяСтрока = ВыбранныеПрофили.Добавить();
		НоваяСтрока.Профиль = ПрофильРедактированиеЦенДокументов;
		НоваяСтрока.Пометка = ЕстьПрофильПродажи И РазрешитьРедактированиеЦенВДокументах;
	КонецЕсли;
	
	ИдентификаторПоставляемыхДанных = Новый УникальныйИдентификатор("76337580-bff4-11df-9174-e0cb4ed5f4c3");
	ПрофильРедактированиеНоменклатуры = Справочники.ПрофилиГруппДоступа.НайтиПоРеквизиту("ИдентификаторПоставляемыхДанных", ИдентификаторПоставляемыхДанных);
	Если ЗначениеЗаполнено(ПрофильРедактированиеНоменклатуры) Тогда
		НоваяСтрока = ВыбранныеПрофили.Добавить();
		НоваяСтрока.Профиль = ПрофильРедактированиеНоменклатуры;
		НоваяСтрока.Пометка = ЕстьПрофильПродажи И РазрешитьРедактированиеНоменклатуры;
	КонецЕсли;
	
	ИдентификаторПоставляемыхДанных = Новый УникальныйИдентификатор("76337581-bff4-11df-9174-e0cb4ed5f4c3");
	ПрофильВозвратыОтПокупателей = Справочники.ПрофилиГруппДоступа.НайтиПоРеквизиту("ИдентификаторПоставляемыхДанных", ИдентификаторПоставляемыхДанных);
	Если ЗначениеЗаполнено(ПрофильВозвратыОтПокупателей) Тогда
		НоваяСтрока = ВыбранныеПрофили.Добавить();
		НоваяСтрока.Профиль = ПрофильВозвратыОтПокупателей;
		НоваяСтрока.Пометка = НуженДоступКВозвратамОтПокупателей;
	КонецЕсли;
	
	ИдентификаторПоставляемыхДанных = Новый УникальныйИдентификатор("76337582-bff4-11df-9174-e0cb4ed5f4c3");
	ПрофильВозвратыПоставщикам = Справочники.ПрофилиГруппДоступа.НайтиПоРеквизиту("ИдентификаторПоставляемыхДанных", ИдентификаторПоставляемыхДанных);
	Если ЗначениеЗаполнено(ПрофильВозвратыПоставщикам) Тогда
		НоваяСтрока = ВыбранныеПрофили.Добавить();
		НоваяСтрока.Профиль = ПрофильВозвратыПоставщикам;
		НоваяСтрока.Пометка = НуженДоступКВозвратамПоставщикам;
	КонецЕсли;
