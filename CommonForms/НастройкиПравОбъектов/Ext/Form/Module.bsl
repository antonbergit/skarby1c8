...
Процедура ЗаписатьНачало(Закрыть = Ложь)
	...
	Если ПодтвердитьОтказОтУправленияПравами = Истина Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru='После записи настройка прав станет недоступной.';uk='Після запису налаштування прав стане недоступне.'"),
			Кнопки,, "Отмена");
	Иначе
		Если Закрыть Тогда
			Закрыть();
...
"але і для її нижчестоящих папок'");
		...
		Элемент.Подсказка = НСтр("ru='Папка, от которой наследуются настройка прав';uk='Папка, від якої успадковуються налаштування прав'");
	...
	Пока НЕ Отказ И НомерСтроки >= 0 Цикл
		...
		Если ГруппыПрав.НайтиСтроки(Отбор).Количество() > 1 Тогда
			Если ТипЗнч(Отбор.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстСообщения = НСтр("ru='Настройка для пользователя ""%1"" уже есть.';uk='Налаштування для користувача ""%1"" вже є.'");
			Иначе
				ТекстСообщения = НСтр("ru='Настройка для группы пользователей ""%1"" уже есть.';uk='Налаштування для групи користувачів ""%1"" вже є.'");
			КонецЕсли;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
...
Процедура ЗаписатьПрава()
	...
	Попытка
		...
		Иначе
			...
			ВызватьИсключение НСтр("ru='После записи настройка прав станет недоступной.';uk='Після запису налаштування прав стане недоступне.'");
		КонецЕсли;
...
"вышестоящей папки ""%1"".';uk='Ці права успадковані, їх можна змінити у формі налаштування прав"
"вищестоящої групи ""%1"".'"),
...
"снять флажок ""%1"".';uk='Для видалення всіх успадкованих прав слід"
"зняти прапорець ""%1"".'"),
...
Процедура ПроверитьРазрешениеНаУправлениеПравами()
	...
	ВызватьИсключение НСтр("ru='Настройка прав недоступна.';uk='Налаштування прав недоступне.'");
