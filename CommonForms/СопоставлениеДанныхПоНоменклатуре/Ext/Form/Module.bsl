...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(ЭлектронныйДокумент) Тогда
		ТекстСообщения = НСтр("ru='Не выбран электронный документ';uk='Не выбран электронный документ'");
...
"Продолжить?';uk='Несохраненные изменения будут утеряны."
"Продолжить?'");
			
		ОбработчикОповещения = Новый ОписаниеОповещения("ПрочитатьНоменклатуруОповещение", ЭтотОбъект);
		ПоказатьВопрос(ОбработчикОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет,,, НСтр("ru='Заполнение списка номенклатуры';uk='Заполнение списка номенклатуры'"));
...
Процедура СохранитьДанныеКаталога(СсылкаНаЭД)
	...
	Если ДопИнформацияПоЭД.Свойство("СсылкаНаДвоичныеДанныеФайла")
		...
		Если ИмяФайла = Неопределено Тогда
			ТекстОшибки = НСтр("ru='Не удалось просмотреть электронный документ. Проверьте настройку рабочего каталога';uk='Не удалось просмотреть электронный документ. Проверьте настройку рабочего каталога'");
...
" Проверьте настройку рабочего каталога';uk='Не удалось получить доп. данные электронного документа."
" Проверьте настройку рабочего каталога'");
		...
		Если СтрНайти(ДопИнформацияПоЭД.Расширение, "zip") > 0 Тогда
			...
			Исключение
				...
				КонецЕсли;
				Операция = НСтр("ru='Распаковка ЭД';uk='Распаковка ЭД'");
...
"Необходимо заполнить процедуру ОбменСКонтрагентамиПереопределяемый.ТекстЗапросаСопоставленияНоменклатуры.';uk='Не определено сопоставление номенклатуры и номенклатуры поставщиков."
