	...
	Элементы.НадписьСообщенияВТехПоддержку.Заголовок =
		ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
			ИнтернетПоддержкаПользователейКлиентСервер.ПодставитьДомен(
				НСтр("ru='<body>При возникновении проблем отправьте письмо по адресу <a href=""SendMail"">webits-info@1c.ru</a></body>';uk='<body>При виникненні проблем надішліть листа на адресу <a href=""SendMail"">web-its@1c.ua</a></body>'")));
...
Процедура НадписьСообщенияВТехПоддержкуОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "SendMail" Тогда
		...
		ИначеЕсли МестоЗапуска = "taxcomGetID" Тогда
			...
			Тема = НСтр("ru='Интернет-поддержка. Обращение к 1С-Такском';uk='Інтернет-підтримка. Звернення до 1С-Такском'");
			Тело = НСтр("ru='Возникает ошибка при обращении к сервису получения идентификатора 1С-Такском.';uk='Виникає помилка при зверненні до сервісу отримання ідентифікатора 1С-Такском.'");
