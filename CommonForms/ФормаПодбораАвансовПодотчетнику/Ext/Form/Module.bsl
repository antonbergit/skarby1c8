...
Процедура ПроверитьЗаполнениеРеквизитовФормы(Отказ)
	...
	Для каждого СтрокаПредоплата Из ОтобранныеАвансы Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаПредоплата.Документ) Тогда
			...
			Сообщение.Текст = НСтр("ru='Не заполнена колонка ""Документ"" в строке ';uk='Не заповнений стовпчик ""Документ"" у рядку'")
				+ Строка(НомерСтроки)
				+ НСтр("ru=' списка ""Отобранные авансы"".';uk=' списку ""Відібрані аванси"".'");
		...
		Если НЕ ЗначениеЗаполнено(СтрокаПредоплата.Сумма) Тогда
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ru='Не заполнена колонка ""Сумма"" в строке ';uk='Не заповнений стовпчик ""Сума"" у рядку'")
				+ Строка(НомерСтроки)
				+ НСтр("ru=' списка ""Отобранные авансы"".';uk=' списку ""Відібрані аванси"".'");
			Сообщение.Поле = "Документ";
			Сообщение.Сообщить();
