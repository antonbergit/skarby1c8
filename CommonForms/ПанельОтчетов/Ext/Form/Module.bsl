...
Процедура СброситьНастройки(Команда)
	ТекстВопроса = НСтр("ru='Сбросить настройки расположения отчетов?';uk='Скинути налаштування розташування звітів?'");
...
Функция НачатьЗамер(Событие, Комментарий = Неопределено)
	...
	Если Событие = "ВыходИзРежимаНастройки" Тогда
		Комментарий = Комментарий + "; " + НСтр("ru='Выход из режима настройки';uk='Вихід з режиму налаштувань'");
	КонецЕсли;
...
Процедура ОбновитьПанельОтчетовНаСервере(Знач Событие = "")
	...
	// Заголовок.
	СуффиксРежимаНастройки = " (" + НСтр("ru='настройка';uk='налаштування'") + ")";
...
Процедура ЗаполнитьПанельОтчетов()
	...
	Иначе
		...
		Если ПараметрыЗаполнения.НеВыведено > 0 Тогда // Вывод информационной надписи.
			ЗаголовокНадписи = НСтр("ru='Выведены первые %1 отчетов из других разделов, уточните поисковый запрос.';uk='Виведені перші %1 звітів з інших розділів, уточніть пошуковий "
"запит.'");
		...
		Если ПараметрыПодсветки <> Неопределено Тогда
			...
			Если ПараметрыПодсветки.НаименованияПользовательскихНастроек.КоличествоНайденныхСлов > 0 Тогда
				Если СоставПодсказки.Количество() > 0 Тогда
					СоставПодсказки.Добавить(Символы.ПС);
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("ru='Сохраненные настройки:';uk='Збережені налаштування:'") + " ");
			...
			Если ПараметрыПодсветки.НаименованияПараметровИОтборов.КоличествоНайденныхСлов > 0 Тогда
				Если СоставПодсказки.Количество() > 0 Тогда
					СоставПодсказки.Добавить(Символы.ПС);
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("ru='Настройки:';uk='Налаштування:'") + " ");
