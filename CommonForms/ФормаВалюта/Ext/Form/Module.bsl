...
Процедура ПолучитьЗначенияПараметровФормы()
	...
	Если Параметры.Свойство("АвторасчетНДС") Тогда
		...
		АвторасчетНДСПередИзменением	= Параметры.АвторасчетНДС;
	Иначе
...
Процедура ПроверитьЗаполнениеРеквизитовФормы(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ВалютаДокумента) Тогда
		...
		Сообщение.Текст = НСтр("ru='Не выбрана валюта для заполнения!';uk='Не обрана валюта для заповнення!'");
	...
	Если НЕ ЗначениеЗаполнено(Курс) Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru='Обнаружен нулевой курс валюты документа!';uk='Виявлений нульовий курс валюти документа!'");
		Сообщение.Поле = "Курс";
		Сообщение.Сообщить();
		Отказ = Истина;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Кратность) Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru='Обнаружена нулевая кратность курса валюты документа!';uk='Виявлена нульова кратність курсу валюти документа!'");
	...
	Если НалогообложениеНДСЕстьРеквизит Тогда
		Если НЕ ЗначениеЗаполнено(НалогообложениеНДС) Тогда
            Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ru='Не заполнено налогообложение!';uk='Не заповнене оподатковування!'");
...
Функция ПроверитьМодифицированностьФормы()
	...
	Если ПересчитатьЦеныПоВалюте 
		...
		ИЛИ (АвторасчетНДСПередИзменением <> АвторасчетНДС)
...
Процедура КнОКВыполнить()
	...
	Если НЕ Отказ Тогда
		...
		СтруктураВозврата.Вставить("АвторасчетНДС", АвторасчетНДС);
