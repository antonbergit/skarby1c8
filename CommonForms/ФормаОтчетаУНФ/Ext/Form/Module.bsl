...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("СложныйАлгоритм") И Параметры.СложныйАлгоритм Тогда
		...
		Попытка
			ОтчетОбъект.ПриКонтекстномОткрытии(Параметры.Контекст, Параметры.ПолеСвязи, ФормаПараметры.Отбор);
		Исключение
	...
	Для Каждого ПользовательскаяНастройкаКД Из ПользовательскиеНастройкиКД.Элементы Цикл
		...
		Если Тип = "ЗначениеПараметраНастроек"
			...
			ИначеЕсли Значение.ДатаНачала > Значение.ДатаОкончания Тогда
				ТекстОшибки = НСтр("ru='Конец периода должен быть больше начала';uk='Кінець періоду повинен бути більше початку'");
...
Процедура ПредставлениеПериодаНажатие(Элемент, СтандартнаяОбработка)
	...
	ИначеЕсли НастройкиОтчета.РежимПериода="НаДату" Тогда
		ПоказатьВводДаты(Оповещение, Период.ДатаОкончания, НСтр("ru='На дату';uk='На дату'"), ЧастиДаты.Дата);
	КонецЕсли; 
...
Процедура СтрокаОтбораАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если СписокВыбора.Количество()=0 И СтрДлина(Текст)>2 Тогда
		СписокВыбора.Добавить("", Новый ФорматированнаяСтрока(НСтр("ru='Совпадений не найдено';uk='Збігів не знайдено'"),, WebЦвета.НейтральноСерый));
	КонецЕсли; 
...
Процедура ОтчетТабличныйДокументОбработкаДополнительнойРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если Данные.ЭтоСсылка Тогда
	    ДополнительныеДействия.Добавить(ДействиеОбработкиРасшифровкиКомпоновкиДанных.ОткрытьЗначение, НСтр("ru='Открыть';uk='Відкрити'"));
	КонецЕсли; 
	Если ЗначениеЗаполнено(Данные.Поле) И НЕ ДоступноеПоле=Неопределено И НЕ ДоступноеПоле.Ресурс Тогда
		ДополнительныеДействия.Добавить(ДействиеОбработкиРасшифровкиКомпоновкиДанных.Отфильтровать, НСтр("ru='Отфильтровать ""';uk='Відфільтрувати ""'")+Данные.Поле+" = "+Данные.Значение+"""");
	КонецЕсли;
	Если ЗначениеЗаполнено(Данные.Поле) И НЕ ДоступноеПоле=Неопределено И ДоступноеПоле.ТипЗначения.Типы().Количество()>0 Тогда
		ДополнительныеДействия.Добавить(СформироватьПодменюОформить(Данные.Поле, Данные.Значение), НСтр("ru='Оформить';uk='Оформити'"));
		ПодменюСортировка = Новый СписокЗначений;
		ПодменюСортировка.Добавить(НаправлениеСортировкиКомпоновкиДанных.Возр, Данные.Поле+НСтр("ru=' по возрастанию';uk=' по зростанню'"),, БиблиотекаКартинок.СортироватьСписокПоВозрастанию); 
		ПодменюСортировка.Добавить(НаправлениеСортировкиКомпоновкиДанных.Убыв, Данные.Поле+НСтр("ru=' по убыванию';uk=' по убуванню'"),, БиблиотекаКартинок.СортироватьСписокПоУбыванию); 
	    ДополнительныеДействия.Добавить(ПодменюСортировка, НСтр("ru='Сортировать';uk='Сортувати'"));
	КонецЕсли;
...
Функция СформироватьПодменюОформить(Поле, Значение)
	...
	Для каждого Тип Из ТипЗначения.Типы() Цикл
		...
		НСтр("ru='Редактировать список быстрых настроек';uk='Редагувати список швидких налаштувань'"),, 
		БиблиотекаКартинок.УсловноеОформлениеКомпоновкиДанных);
		СписокКоманд.Добавить(
		"РучнаяНастройкаОформления", 
		НСтр("ru='Ручная настройка оформления';uk='Ручне налаштування оформлення'"),, 
...
Функция ПолучитьДанныеРасшифровки(Расшифровка)
	...
	СтруктураВозврата = Новый Структура("Тип", ?(ТипЗнч(ЭлементРасшифровки)=Тип("ЭлементРасшифровкиКомпоновкиДанныхГруппировка"), "Группировка", "Поле"));
...
Процедура ОткрытьФормуОтчета()
	...
	Если СтандартнаяОбработка Тогда
		...
		СтруктураОткрытия.Вставить("ПредставлениеВарианта", Строка(НастройкиОтчета.НаименованиеВарианта)+НСтр("ru=' (Расшифровка)';uk=' (Расшифровка)'"));
...
Процедура ВидДиаграммыНачалоВыбораЗадержка()
	...
	Меню.Добавить(Неопределено, НСтр("ru='Не выводить';uk='Не выводить'"),, БиблиотекаКартинок.Пустая);
	Меню.Добавить(ТипДиаграммы.График, НСтр("ru='Графики';uk='Графики'"),, КартинкаДиаграммы(ТипДиаграммы.График));
	Меню.Добавить(ТипДиаграммы.Гистограмма, НСтр("ru='Обычные гистограммы';uk='Обычные гистограммы'"),, КартинкаДиаграммы(ТипДиаграммы.Гистограмма));
	Меню.Добавить(ТипДиаграммы.ГистограммаГоризонтальная, НСтр("ru='Горизонт. гистограммы';uk='Горизонт. гистограммы'"),, КартинкаДиаграммы(ТипДиаграммы.ГистограммаГоризонтальная));
	Меню.Добавить(ТипДиаграммы.Круговая, НСтр("ru='Круговые диаграммы';uk='Круговые диаграммы'"),, КартинкаДиаграммы(ТипДиаграммы.Круговая));
	Меню.Добавить(ТипДиаграммы.Изометрическая, НСтр("ru='Изометрические диаграммы';uk='Изометрические диаграммы'"),, КартинкаДиаграммы(ТипДиаграммы.Изометрическая));
	Меню.Добавить("<Прочие>", НСтр("ru='Прочие';uk='Інші'"),, КартинкаДиаграммы(ТипДиаграммы.Поверхность));
...
Процедура ВидДиаграммыВыборГруппы(Значение, ДополнительныеДанные) Экспорт
	...
	Если Значение.Значение=ТипДиаграммы.График Тогда
		Меню.Добавить(ТипДиаграммы.График, НСтр("ru='Обычный';uk='Звичайний'"));
		Меню.Добавить(ТипДиаграммы.ГрафикПоШагам, НСтр("ru='По шагам';uk='По шагам'"));
		Меню.Добавить(ТипДиаграммы.ГрафикСНакоплением, НСтр("ru='С накоплением';uk='С накоплением'"));
		Меню.Добавить(ТипДиаграммы.ГрафикСОбластями, НСтр("ru='С областями';uk='С областями'"));
		Меню.Добавить(ТипДиаграммы.ГрафикСОбластямиИНакоплением, НСтр("ru='С областями и накоплением';uk='С областями и накоплением'"));
		Меню.Добавить(ТипДиаграммы.ГрафикСОбластямиНормированный, НСтр("ru='Нормированный';uk='Нормированный'"));
	ИначеЕсли Значение.Значение=ТипДиаграммы.Гистограмма Тогда
		Меню.Добавить(ТипДиаграммы.Гистограмма, НСтр("ru='Обычная';uk='Звичайна'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаСНакоплением, НСтр("ru='С накоплением';uk='С накоплением'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаНормированная, НСтр("ru='Нормированная';uk='Нормированная'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаОбъемная, НСтр("ru='Объемная';uk='Объемная'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаСНакоплениемОбъемная, НСтр("ru='Объемная с накоплением';uk='Объемная с накоплением'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаНормированнаяОбъемная, НСтр("ru='Объемная нормированная';uk='Объемная нормированная'"));
	ИначеЕсли Значение.Значение=ТипДиаграммы.ГистограммаГоризонтальная Тогда
		Меню.Добавить(ТипДиаграммы.ГистограммаГоризонтальная, НСтр("ru='Обычная';uk='Звичайна'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаСНакоплениемГоризонтальная, НСтр("ru='С накоплением';uk='С накоплением'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаНормированнаяГоризонтальная, НСтр("ru='Нормированная';uk='Нормированная'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаГоризонтальнаяОбъемная, НСтр("ru='Объемная';uk='Объемная'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаСНакоплениемГоризонтальнаяОбъемная, НСтр("ru='Объемная с накоплением';uk='Объемная с накоплением'"));
		Меню.Добавить(ТипДиаграммы.ГистограммаНормированнаяГоризонтальнаяОбъемная, НСтр("ru='Объемная нормированная';uk='Объемная нормированная'"));
	ИначеЕсли Значение.Значение=ТипДиаграммы.Круговая Тогда
		Меню.Добавить(ТипДиаграммы.Круговая, НСтр("ru='Обычная';uk='Звичайна'"));
		Меню.Добавить(ТипДиаграммы.КруговаяОбъемная, НСтр("ru='Объемная';uk='Объемная'"));
	ИначеЕсли Значение.Значение=ТипДиаграммы.Изометрическая Тогда
		Меню.Добавить(ТипДиаграммы.Изометрическая, НСтр("ru='Обычная';uk='Звичайна'"));
		Меню.Добавить(ТипДиаграммы.ИзометрическаяНепрерывная, НСтр("ru='Непрерывная';uk='Непрерывная'"));
		Меню.Добавить(ТипДиаграммы.ИзометрическаяЛента, НСтр("ru='Лента';uk='Лента'"));
		Меню.Добавить(ТипДиаграммы.ИзометрическаяПирамида, НСтр("ru='Пирамида';uk='Пирамида'"));
...
Процедура ДобавитьФильтрЗавершение(Элемент, ДополнительныеДанные) Экспорт
	...
	Если Элемент.Значение=Неопределено Тогда
		СтруктураОтбора = Новый Структура;
		СтруктураОтбора.Вставить("Тип", "Фильтр");
...
Процедура ПриИзмененииГруппировкиКолонки()
	...
	Если НЕ ПустаяСтрока(ПолеГруппировкиКолонок) Тогда
		СтрокаНастроек = СтрокаОписанияПоля("Группировка", ПолеГруппировкиКолонок, ПоляНастроек);
		Если СтрокаНастроек=Неопределено Тогда
			СтрокаНастроек = СтрокаОписанияПоля("Реквизит", ПолеГруппировкиКолонок, ПоляНастроек);
		КонецЕсли; 
	Иначе
...
Функция СписокДополнительныхКолонок()
	...
	Для каждого Показатель Из Отчет.КомпоновщикНастроек.Настройки.ДоступныеПоляВыбора.Элементы Цикл
		...
		Если НЕ СтрокаОписанияПоля("Показатель", ИмяПоля, ПоляНастроек)=Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ПриИзмененииПоляСравнения()
	...
	Если ТипЗнч(ПолеСравнения)=Тип("Строка") И НЕ ПустаяСтрока(ПолеСравнения) Тогда
		СтрокаНастроек = СтрокаОписанияПоля("Группировка", ПолеСравнения, ПоляНастроек);
		Если СтрокаНастроек=Неопределено Тогда
			СтрокаНастроек = СтрокаОписанияПоля("Реквизит", ПолеСравнения, ПоляНастроек);
		КонецЕсли; 
	Иначе
		СтрокаНастроек = Неопределено;
	КонецЕсли; 
...
Процедура Подключаемый_ИерархияФильтраНажатие(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ЭлементПоле.ВыборГруппИЭлементов=ГруппыИЭлементы.Элементы Тогда
		...
		ЭлементПоле.ПодсказкаВвода = Стр.Заголовок+НСтр("ru=' (группа)';uk=' (група)'");
		Элемент.Подсказка = НСтр("ru='Используется выбор групп. Нажмите для выбора элементов.';uk='Используется выбор групп. Нажмите для выбора элементов.'");
	...
	Иначе
		...
		Элемент.Подсказка = НСтр("ru='Используется выбор элементов. Нажмите для выбора групп.';uk='Используется выбор элементов. Нажмите для выбора групп.'");
...
Процедура ПриИзмененииГруппировкиРеквизитаСервер(Идентификатор, ИмяЭлемента)
	...
	Если Стр.Тип="Реквизит" Тогда
		...
		ИначеЕсли НЕ Значение Тогда
			...
			КонецЕсли; 
		КонецЕсли; 
	ИначеЕсли Стр.Тип="Группировка" Тогда
	...
	Иначе
		...
		Иначе
			ОбновитьГруппировкиИРеквизиты(Истина);
		КонецЕсли; 
		УстановитьВидимостьИДоступность(?(Стр.Тип="Реквизит", "ИзмененыРеквизиты", "ИзмененыГруппировки"));
	КонецЕсли;
...
Процедура Подключаемый_СместитьВверх(Элемент)
	...
	Если Стр.Тип="Группировка" Тогда
	...
	Если Стр.Тип="Показатель" ИЛИ Стр.Тип="ГруппаПоказателей" Тогда
...
Процедура ПриИзмененииГруппыПоказателейСервер(Идентификатор, ИмяЭлемента)
	...
	Для каждого СтрПоля Из Стр.ПолучитьЭлементы() Цикл
		Если СтрПоля.Тип="Показатель" Тогда
...
Процедура ДобавитьСтроку(Команда)
	...
	Если НастройкиОтчета.РазрешитьДобавлениеПрочихГруппировокСтрок Тогда
		Список.Добавить(Неопределено, НСтр("ru='Прочие поля';uk='Інші поля'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли;
...
Процедура ДобавитьФильтр(Команда)
	...
	Если НастройкиОтчета.РазрешитьДобавлениеПрочихФильтров Тогда
		Список.Добавить(Неопределено, НСтр("ru='Прочее';uk='Інше'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли; 
	Если Список.Количество()=1 Тогда
...
Процедура ЗапомнитьНастройки(Команда)
	...
	Если Форма=Неопределено Тогда
		Состояние(НСтр("ru='Нет права редактирования настроек выбранного отчета';uk='Немає права редагування налаштувань обраного звіту'"));
	КонецЕсли;
...
Процедура ОбновитьПредставлениеПериода()
	...
	Если НастройкиОтчета.РежимПериода="ЗаПериод" Тогда
		Если НЕ ЗначениеЗаполнено(Период.ДатаНачала) И НЕ ЗначениеЗаполнено(Период.ДатаОкончания) Тогда
			ПредставлениеПериода = НСтр("ru='за все время';uk='за весь час'");
		ИначеЕсли Элементы.ПредставлениеПериода.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет Тогда
			ПредставлениеПериода = НСтр("ru='за ';uk='за '")+НРег(ПредставлениеПериода(Отчет.Период.ДатаНачала, КонецДня(Период.ДатаОкончания), "Л="+Локализация.ОпределитьКодЯзыкаДляФормат(КодЯзыка)));
		Иначе
			ПредставлениеПериода = НРег(ПредставлениеПериода(НачалоДня(Период.ДатаНачала), ?(ЗначениеЗаполнено(Период.ДатаОкончания), КонецДня(Период.ДатаОкончания), Период.ДатаОкончания), "Л="+Локализация.ОпределитьКодЯзыкаДляФормат(КодЯзыка)));
		КонецЕсли;
	ИначеЕсли НастройкиОтчета.РежимПериода="НаДату" Тогда
		Если НЕ ЗначениеЗаполнено(Период.ДатаОкончания) Тогда
			ПредставлениеПериода = НСтр("ru='актуальные данные';uk='актуальні дані'");
		Иначе
...
Процедура ОбновитьЗаголовокОтчета()
	...
	Если РежимРасшифровки Тогда
		ЗаголовокОтчета = ЗаголовокОтчета + " (" + НСтр("ru='Расшифровка';uk='Розшифровка'") + ")";
	КонецЕсли; 
...
Процедура ПоказатьДиалогОтправкиПоЭлектроннойПочте()
	...
	ЗаголовокСохранения = СтрШаблон(НСтр("ru='Отправка отчета ""%1"" по почте';uk='Відправлення звіту ""%1"" поштою'"), ОтчетНаименованиеТекущегоВарианта);
...
Процедура УстановитьВидимостьИДоступность(Событие = "")
	...
	Если ПолнаяИнициализация Тогда
		...
		НСтр("ru='На дату';uk='На дату'"), 
		НСтр("ru='Период';uk='Період'"));
	КонецЕсли;
	...
	Если ИзмененРежимРедактирования Тогда
		ОбрабатываемыеТипы = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("Группировка,Реквизит,Показатель,ГруппаПоказателей");
		СтруктураОтбора = Новый Структура;
		СтруктураОтбора.Вставить("Тип", ОбрабатываемыеТипы);
		Строки = СтрокиДерева(ПоляНастроек, СтруктураОтбора, Истина);
		Для каждого Стр Из Строки Цикл
			Если Стр.Тип="Группировка" ИЛИ Стр.Тип="Реквизит" Тогда
				ГруппаКоманд = Элементы[СтрЗаменить(Стр.ИмяГруппы, "_Группа", "_ГруппаКоманды")];
				ГруппаКоманд.Видимость = РежимРедактированияСтрок;
			ИначеЕсли Стр.Тип="Показатель" ИЛИ Стр.Тип="ГруппаПоказателей" Тогда 
				ГруппаКоманд = Элементы[СтрЗаменить(Стр.ИмяГруппы, "_Группа", "_ГруппаКоманды")];
...
Процедура ОбновитьЗаголовокЭлемента(Стр)
	...
	Если Стр.Тип<>"Группировка" И Стр.Тип<>"Реквизит" И Стр.Тип<>"Показатель" И Стр.Тип<>"ГруппаПоказателей" Тогда
		Возврат;
	КонецЕсли;
	Если Стр.Тип="Реквизит" Тогда
	...
	ИначеЕсли Стр.Тип="Показатель" И НЕ Стр.ПолучитьРодителя()=Неопределено Тогда
...
Процедура УстановитьПараметрыИОтборыРасшифровки(ОтчетОбъект)
	...
	Для Каждого КлючИЗначение Из ФормаПараметры.Отбор Цикл
		...
		ТекстОшибки = НСтр("ru='Не удалось установить фиксированный отбор ""%1"".';uk='Не вдалося встановити фіксований відбір ""%1"".'");
...
Функция ФоновоеЗаданиеЗапустить()
	...
	Если Отчет.КомпоновщикНастроек.Настройки.Структура.Количество()=0 Тогда
		ПоказатьОшибкиФормирования(НСтр("ru='Не выбрано ни одной группировки';uk='Не обране жодного угруповання'"));
		Возврат Ложь;
	КонецЕсли; 
...
Процедура ОтметитьНеактуальным()
	...
	ОтображениеСостояния = Элементы.ОтчетТабличныйДокумент.ОтображениеСостояния;
	ОтображениеСостояния.Видимость = Истина;
	ОтображениеСостояния.Текст = НСтр("ru='Отчет не сформирован. Нажмите ""Сформировать"" (F5) для получения отчета.';uk='Звіт не сформований. Натисніть ""Сформувати"" (F5) для одержання звіту.'");
	ОтображениеСостояния.ДополнительныйРежимОтображения = ДополнительныйРежимОтображения.Неактуальность;
...
Процедура Сформировать()
	...
	Если НастройкиОтчета.РежимПериода="ЗаПериод" Тогда
		Если ЗначениеЗаполнено(Период.ДатаНачала) Тогда
			ПредставлениеДляЗаголовка = НСтр("ru='за ';uk='за '")+ПредставлениеПериода;
		Иначе
	...
	ИначеЕсли НастройкиОтчета.РежимПериода="НаДату" Тогда
		Если НЕ ЗначениеЗаполнено(Период.ДатаОкончания) Тогда
			ПредставлениеДляЗаголовка = НСтр("ru='текущий момент';uk='теперішній момент'");
		Иначе
			ПредставлениеДляЗаголовка = ПредставлениеПериода;
		КонецЕсли;
		Отчет.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("Заголовок", НастройкиОтчета.НаименованиеВарианта+НСтр("ru=' на ';uk=' на '")+ПредставлениеДляЗаголовка);
	Иначе
...
Процедура ЗаполнитьВариантыСерийТочекЗначений()
	...
	ДоступныеСерии.Добавить(Неопределено, НСтр("ru='Не использовать';uk='Не використовувати'"));
	ДоступныеТочки.Добавить(Неопределено, НСтр("ru='Не использовать';uk='Не використовувати'"));
	...
	Если НастройкиОтчета.РазрешитьДобавлениеПрочихГруппировокКолонок Тогда
		ДоступныеСерии.Добавить("<Прочие>", НСтр("ru='Прочее';uk='Інше'"),, БиблиотекаКартинок.Выбрать);
		ДоступныеТочки.Добавить("<Прочие>", НСтр("ru='Прочее';uk='Інше'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли;
...
Процедура ВнестиИзмененияВРеквизитыНаФорме(ПолнаяЗамена = Истина)
	...
	Для каждого Идентификатор Из Идентификаторы Цикл
		...
		ИначеЕсли Стр.Тип="Реквизит" Тогда
		...
		ИначеЕсли Стр.Тип="Фильтр" Тогда
	...
	Для каждого Идентификатор Из Идентификаторы Цикл
		Стр = ПоляНастроек.НайтиПоИдентификатору(Идентификатор);
		Если НЕ Стр.Тип="Фильтр" И НЕ Стр.Тип="Параметр" Тогда
			Продолжить;
		КонецЕсли;
		...
		ИначеЕсли Стр.ВидЭлемента="Тумблер" И ЗначениеЗаполнено(Стр.ЗначениеПоУмолчанию) Тогда
			...
			КонецЦикла; 
		ИначеЕсли НЕ Стр.ВидЭлемента="Поле" ИЛИ Стр.Тип="Параметр" Тогда
			Для каждого Реквизит Из Стр.Реквизиты Цикл
...
Процедура ЗаполнитьВариантыГруппировкиКолонок()
	...
	Элементы.ПолеГруппировкиКолонок.СписокВыбора.Очистить();
	Элементы.ПолеГруппировкиКолонок.СписокВыбора.Добавить("", НСтр("ru='Не группировать';uk='Не групувати'"));
	...
	Если НастройкиОтчета.РазрешитьДобавлениеПрочихГруппировокКолонок Тогда
		Элементы.ПолеГруппировкиКолонок.СписокВыбора.Добавить("<Прочие>", НСтр("ru='Прочее';uk='Інше'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли; 
...
Функция ДобавитьОписаниеГруппировки(Путь, НестандартныйОбработчик = Ложь, ТипГруппировки = Неопределено)
	...
	Стр = СтрокаОписанияПоля(ТипКолонки, Путь, ПоляНастроек);
	Если Стр=Неопределено Тогда
...
Функция ИндексВставкиГруппировки(ИмяПоля)
	 ...
	 Если ТипЗнч(ПолеСхемы)=Тип("ПолеНабораДанныхСхемыКомпоновкиДанных") И ПолеСхемы.Роль.НомерПериода>0 Тогда
		 Для каждого Стр Из ПоляНастроек.ПолучитьЭлементы() Цикл
			 Если НЕ Стр.Тип="Группировка" Тогда
				 Продолжить;
			 КонецЕсли; 
			 ...
			 Иначе
				 Для каждого ПодСтр Из Стр.ПолучитьЭлементы() Цикл
					 Если НЕ ПодСтр.Тип="Реквизит" Тогда
						 Продолжить;
					 КонецЕсли; 
		...
		Если (ГруппировкаКолонокПериод И НЕ ИмяПоля=ПолеГруппировкиКолонок) ИЛИ (СравнениеПериод И НЕ ИмяПоля=ИмяСравнения) Тогда
			Если НЕ ПустаяСтрока(ИмяПоляПроверки) Тогда
				Если ГруппировкаКолонокПериод И НЕ ИмяПоля=ПолеГруппировкиКолонок Тогда
					...
					ОписаниеПоля = ОписаниеПоля+НСтр("ru=' в колонках отчета';uk=' в колонках отчета'");
			...
			Иначе
				...
				ОписаниеПоля = ОписаниеПоля+НСтр("ru=' в строках отчета';uk=' в строках отчета'");
				Если ГруппировкаКолонокПериод И НЕ ИмяПоля=ПолеГруппировкиКолонок Тогда
			...
			НСтр("ru='Запрещено использование в строках и колонках различных полей-периодов. Предварительно отключите поле %1';uk='Запрещено использование в строках и колонках различных полей-периодов. Предварительно отключите поле %1'"),
			ОписаниеПоля));
			Результат = Ложь;
		КонецЕсли; 
	КонецЦикла; 
...
Процедура ДобавитьОписанияГруппировокРекурсивно(ЭлементыСтруктуры, ЭтоСтрока = Истина)
	...
	Для каждого Элемент Из ЭлементыСтруктуры Цикл
		...
		ИначеЕсли ЭтоСтрока И (ТипЗнч(Элемент)=Тип("ГруппировкаКомпоновкиДанных") ИЛИ ТипЗнч(Элемент)=Тип("ГруппировкаТаблицыКомпоновкиДанных")) Тогда
			...
			Стр.Тип = ТипКолонки;
			ИмяРеквизита = НовоеИмяРеквизита(ТипКолонки);
			Стр.Заголовок = Строка(Элемент.ПоляГруппировки);
			Если ПустаяСтрока(Стр.Заголовок) Тогда
				Стр.Заголовок = НСтр("ru='<Детальные записи>';uk='<Детальні записи>'");
			КонецЕсли; 
			Стр.Реквизиты = Новый Структура(ИмяРеквизита, Элемент.Использование);
			Стр.Элементы = Новый Структура;
			Стр.ВидЭлемента = "Флажок";
			Стр.ТипЗначения = Новый ОписаниеТипов("Булево");
			Стр.ДополнительныеПараметры = Новый Структура;
			...
			Иначе
				...
				Для каждого Поле Из Элемент.ПоляГруппировки.Элементы Цикл
					...
					КонецЕсли;
					ТипКолонки = "Реквизит";
					СтрРеквизит = Стр.ПолучитьЭлементы().Добавить();
					СтрРеквизит.Тип = ТипКолонки;
					СтрРеквизит.Поле = Путь;
					ИмяРеквизита = НовоеИмяРеквизита(ТипКолонки);
					СтрРеквизит.Заголовок = ДоступноеПоле.Заголовок;
					СтрРеквизит.Реквизиты = Новый Структура(ИмяРеквизита, Поле.Использование И Элемент.Использование);
					СтрРеквизит.Элементы = Новый Структура;
					СтрРеквизит.ВидЭлемента = "Флажок";
					СтрРеквизит.ТипЗначения = Новый ОписаниеТипов("Булево");
					СтрРеквизит.ДополнительныеПараметры = Новый Структура;
...
Процедура УстановитьПредставлениеКнопкиИерархииГруппировки(Кнопка, Стр)
	...
	Если Стр.ДополнительныеПараметры.ТипГруппировки=ТипГруппировкиКомпоновкиДанных.Элементы Тогда
		...
		Кнопка.Подсказка = НСтр("ru='Отображать только элементы';uk='Відображати тільки елементи'");
	ИначеЕсли Стр.ДополнительныеПараметры.ТипГруппировки=ТипГруппировкиКомпоновкиДанных.ТолькоИерархия Тогда
		Кнопка.Картинка = БиблиотекаКартинок.РежимВыбораГрупп;
		Кнопка.Подсказка = НСтр("ru='Отображать только иерархию';uk='Відображати тільки ієрархію'");
	ИначеЕсли Стр.ДополнительныеПараметры.ТипГруппировки=ТипГруппировкиКомпоновкиДанных.Иерархия Тогда
		Кнопка.Картинка = БиблиотекаКартинок.РежимВыбораГруппИЭлементов;
		Кнопка.Подсказка = НСтр("ru='Отображать элементы и иерархию';uk='Відображати елементи й ієрархію'");
	КонецЕсли; 
...
Функция СписокПолейГруппировок(Сравнение = Ложь)
	...
	Если (НастройкиОтчета.РазрешитьДобавлениеПрочихГруппировокКолонок И НЕ Сравнение) 
		...
		СписокВыбора.Добавить("<Прочие>", НСтр("ru='Прочее';uk='Інше'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли; 
...
Функция СписокДобавленныхГруппировок()
	...
	СтруктураОтбора = Новый Структура;
	СтруктураОтбора.Вставить("Тип", "Реквизит");
...
Процедура ДобавитьГруппировкуРеквизит(Путь)
	...
	Если Найти(Путь, ".")>0 Тогда
		...
		Стр = СтрокаОписанияПоля("Группировка", ИмяРодителя, ПоляНастроек);
		СтрРеквизит = СтрокаОписанияПоля("Реквизит", ИмяРодителя, ПоляНастроек);
		...
		Если (ТочкаПериод И НЕ ИмяПоля=ДиаграммаТочки) ИЛИ (СерияПериод И НЕ ИмяПоля=ДиаграммаСерии) Тогда
			Если НЕ ПустаяСтрока(ИмяПоляПроверки) Тогда
				Если ТочкаПериод И НЕ ИмяПоля=ДиаграммаТочки Тогда
					...
					ОписаниеПоля = ОписаниеПоля+НСтр("ru=' в диаграмме';uk=' в диаграмме'");
			...
			Иначе
				ОписаниеПоля = """"+?(ПустаяСтрока(Поле.Заголовок), Поле.Поле, Поле.Заголовок)+"""";
				ОписаниеПоля = ОписаниеПоля+НСтр("ru=' в структуре отчета';uk=' в структуре отчета'");
				Если ТочкаПериод И НЕ ИмяПоля=ДиаграммаТочки Тогда
					ДиаграммаТочки = Неопределено;
				КонецЕсли; 
			...
			КонецЕсли; 
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Запрещено использование в отчете и диаграмме различных полей-периодов. Предварительно отключите поле %1';uk='Запрещено использование в отчете и диаграмме различных полей-периодов. Предварительно отключите поле %1'"),
			ОписаниеПоля));
			Результат = Ложь;
		КонецЕсли; 
	КонецЦикла; 
...
&НаСервере
Функция ДобавитьОписаниеПоказателя(Путь, НестандартныйОбработчик = Ложь, ТипКолонки = "Показатель")
...
Процедура ЗаполнитьОписанияПоказателей(ЭлементыВыбор = Неопределено, СтрРодитель = Неопределено)
	...
	ТипПоказатель = "Показатель";
	ТипГруппа = "ГруппаПоказателей";
	...
	Для каждого Показатель Из ЭлементыВыбор Цикл
		...
		Если ТипЗнч(Показатель)=Тип("ГруппаВыбранныхПолейКомпоновкиДанных") Тогда
			ИмяРеквизита = ТипГруппа+СтрЗаменить(Новый УникальныйИдентификатор, "-", "");
			СтрГруппы = СтрРодитель.ПолучитьЭлементы().Добавить();
			СтрГруппы.Тип = ТипГруппа;
...
Процедура ДобавитьЭлементГруппаПоказателейПоказатель(Стр)
	...
	Для каждого Реквизит Из Стр.Реквизиты Цикл
		...
		Элемент.ПутьКДанным = ИмяРеквизита;
		Элемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
		Если Стр.Тип="ГруппаПоказателей" Тогда
...
Процедура ИзменитьПоказатели(ЭлементыВыбор = Неопределено, СтрРодитель = Неопределено)
	...
	Для каждого Стр Из СтрРодитель.ПолучитьЭлементы() Цикл
		...
		Если Стр.Тип="ГруппаПоказателей" Тогда
			...
			Продолжить;
		КонецЕсли; 
		Если НЕ Стр.Тип="Показатель" Тогда
			Продолжить;
		КонецЕсли; 
		Для каждого ОписаниеРеквизита Из Стр.Реквизиты Цикл
			Если НЕ ЭтаФорма[ОписаниеРеквизита.Ключ] Тогда
...
Функция СместитьПоказатель(Идентификатор, Шаг)
	...
	Стр = ПоляНастроек.НайтиПоИдентификатору(Идентификатор);
	ЭтоГруппа = (Стр.Тип="ГруппаПоказателей");
	Группа = Элементы[Стр.ИмяГруппы];
	...
	Если Стр.ПолучитьРодителя()=Неопределено Тогда
		...
		МассивТипов.Добавить("Показатель");
		МассивТипов.Добавить("ГруппаПоказателей");
...
Процедура ЗаполнитьВариантыСравнения()
	...
	Элементы.ПолеСравнения.СписокВыбора.Очистить();
	Элементы.ПолеСравнения.СписокВыбора.Добавить(Неопределено, НСтр("ru='Не сравнивать';uk='Не порівнювати'"));
	
	Если НастройкиОтчета.ИспользоватьПериодичность Тогда
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.Периодичность.День, НСтр("ru='Дни';uk='Дні'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.Периодичность.Неделя, НСтр("ru='Недели';uk='Тижня'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.Периодичность.Месяц, НСтр("ru='Месяцы';uk='Місяці'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.Периодичность.Квартал, НСтр("ru='Кварталы';uk='Квартали'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.Периодичность.Полугодие, НСтр("ru='Полугодия';uk='Півріччя'"));
		Элементы.ПолеСравнения.СписокВыбора.Добавить(Перечисления.Периодичность.Год, НСтр("ru='Годы';uk='Роки'"));
	КонецЕсли;
	...
	Если НастройкиОтчета.РазрешитьДобавлениеПрочихПолейСравнения Тогда
		Элементы.ПолеСравнения.СписокВыбора.Добавить("<Прочие>", НСтр("ru='Прочее';uk='Інше'"),, БиблиотекаКартинок.Выбрать);
	КонецЕсли; 
...
Процедура ДобавитьОписаниеФильтра(Путь, НестандартныйОбработчик = Ложь, УстановитьФильтр = Истина)
	...
	Поле = Отчет.КомпоновщикНастроек.Настройки.ДоступныеПоляОтбора.НайтиПоле(Новый ПолеКомпоновкиДанных(Путь));
	Если Поле=Неопределено Тогда
		Возврат;
	КонецЕсли; 
	
	ТипПоляФильтр = "Фильтр";
	СтруктураОтбора = Новый Структура;
	СтруктураОтбора.Вставить("Тип", ТипПоляФильтр);
	СтруктураОтбора.Вставить("Поле", Путь);
	Идентификаторы = ИдентификаторыСтрок(ПоляНастроек, СтруктураОтбора);
	Если Идентификаторы.Количество()=0 Тогда
		Стр = ПоляНастроек.ПолучитьЭлементы().Добавить();
		Стр.Тип = ТипПоляФильтр;
		Стр.Поле = Путь;
	Иначе
		...
		Стр.Скрыт = Ложь;
	КонецЕсли;
	Стр.ТипЗначения = Поле.ТипЗначения;
	Стр.Заголовок = Поле.Заголовок;
	Стр.НестандартныйОбработчик = НестандартныйОбработчик;
	Стр.ДополнительныеПараметры = Новый Структура;
	ОпределитьЭлементыИРеквизитыФильтра(Стр,, Поле, Истина);
...
Процедура ЗаполнитьОписанияФильтров()
	...
	Для каждого ЭлементОтбора Из Настройки.Отбор.Элементы Цикл
		...
		Если ЗначениеЗаполнено(ЭлементОтбора.Представление) Тогда
			...
			Идентификатор = НовоеИмяРеквизита(ТипПоляФильтр);
			Стр.Элементы = Новый Структура;
		...
		Иначе
			...
			Если НЕ СтрокаОписанияПоля("Фильтр", ИмяПоля, ПоляНастроек)=Неопределено Тогда
				Продолжить;
			КонецЕсли; 
			Поле = Отчет.КомпоновщикНастроек.Настройки.ДоступныеПоляОтбора.НайтиПоле(ЭлементОтбора.ЛевоеЗначение);
			Если Поле=Неопределено Тогда
				Продолжить;
			КонецЕсли; 
			Стр = ПоляНастроек.ПолучитьЭлементы().Добавить();
			...
			КонецЕсли;
			Стр.ДополнительныеПараметры = Новый Структура;
			ОпределитьЭлементыИРеквизитыФильтра(Стр, ЭлементОтбора, Поле, Ложь);
		КонецЕсли; 
	КонецЦикла; 
...
Процедура ОпределитьЭлементыИРеквизитыФильтра(Стр, ЭлементОтбора = Неопределено, ДоступноеПоле, РучноеДобавление)
	...
	ИначеЕсли ТипЗначения.СодержитТип(Тип("Булево")) Тогда
		...
		Значения.Добавить(Перечисления.БулевоТриСостояния.НеИзменять, НСтр("ru='Все';uk='Всі'"), Значение=Неопределено);
...
Процедура УстановитьЗначениеФильтра(Поле, Значение, ВидСравнения = Неопределено, ОбновлятьВПанели = Истина, ЗаменитьЗначение = Ложь)
		...
		Иначе
			...
			Иначе
				...
				ИначеЕсли ТипЗнч(НайденныйЭлемент.ПравоеЗначение)=Тип("СписокЗначений") И ТипЗнч(Значение)=Тип("СписокЗначений") Тогда
					...
					КонецЦикла; 
				//ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.Равно ИЛИ ВидСравнения=ВидСравненияКомпоновкиДанных.НеРавно Тогда  
				//	ЗначениеФильтра = Новый СписокЗначений;
				//	ЗначениеФильтра.Добавить(НайденныйЭлемент.ПравоеЗначение);
				//	ЗначениеФильтра.Добавить(Значение);
				//	НайденныйЭлемент.ВидСравнения = ?(ВидСравнения=ВидСравненияКомпоновкиДанных.Равно, ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии, ВидСравненияКомпоновкиДанных.НеВСпискеПоИерархии);
				//	НайденныйЭлемент.ПравоеЗначение = ЗначениеФильтра;
				Иначе
					НайденныйЭлемент.ПравоеЗначение = Значение;
				КонецЕсли; 
	...
	Если ОбновлятьВПанели Тогда
		Если СтрокаОписанияПоля("Фильтр", Поле, ПоляНастроек)=Неопределено Тогда
...
Процедура ОбновитьРеквизитыФильтровВПанели(Поле, Значение, ВидСравнения = Неопределено, Удаление = Ложь)
	...
	Стр = СтрокаОписанияПоля("Фильтр", Поле, ПоляНастроек);
	Если Стр=Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьОписаниеПараметра(ИмяПараметра, НестандартныйОбработчик = Ложь)
	...
	Стр = ПоляНастроек.ПолучитьЭлементы().Добавить();
	Стр.Тип = ТипПоляФильтр;
	Стр.Поле = ИмяПараметра;
	Стр.ТипЗначения = ДоступныйПараметр.ТипЗначения;
	Стр.Заголовок = ДоступныйПараметр.Заголовок;
	Стр.ДополнительныеПараметры = Новый Структура;
	ОпределитьЭлементыИРеквизитыПараметра(Стр, Параметр, ДоступныйПараметр);
...
Процедура ЗаполнитьОписанияПараметров()
	...
	Для каждого Параметр Из Настройки.ПараметрыДанных.Элементы Цикл
		...
		Стр.Заголовок = ДоступныйПараметр.Заголовок;
		Стр.ДополнительныеПараметры = Новый Структура;
		Стр.ДополнительныеПараметры.Вставить("АвтоОтметкаНезаполненного", ДоступныйПараметр.ЗапрещатьНезаполненныеЗначения);
		ОпределитьЭлементыИРеквизитыПараметра(Стр, Параметр, ДоступныйПараметр);
	КонецЦикла; 
...
Процедура ОпределитьЭлементыИРеквизитыПараметра(Стр, Параметр = Неопределено, ДоступныйПараметр)
	...
	ТипПоляПараметр = "Фильтр";
	
	ТипЗначения = Стр.ТипЗначения;
	Идентификатор = НовоеИмяРеквизита(ТипПоляПараметр);
...
Процедура ЗаполнитьТаблицуНаложенныхФильтров()
	...
	Для каждого ЭлементОтбор Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		...
		Если ТипЗнч(ЭлементОтбор)=Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			...
			Если ПустаяСтрока(ЭлементОтбор.ПредставлениеПользовательскойНастройки) Тогда
				ЭлементОтбор.ПредставлениеПользовательскойНастройки = "ОтметкаФильтра"+СтрЗаменить(Строка(Новый УникальныйИдентификатор), "-", "");
			КонецЕсли; 
			Стр.ИмяКнопки = ЭлементОтбор.ПредставлениеПользовательскойНастройки;
	...
	Для каждого Стр Из НаложенныеФильтры Цикл
		...
		Если НЕ ЗначениеЗаполнено(Стр.Значение) И НЕ ТипЗнч(Стр.Значение)=Тип("Число") Тогда
			Продолжить;
		КонецЕсли;
		...
		ИначеЕсли ТипЗнч(Стр.Значение)=Тип("Структура") Тогда
			Если НЕ Стр.Значение.ЗначениеОт=Неопределено И НЕ Стр.Значение.ЗначениеДо=Неопределено Тогда
				ПредставлениеЗначения = НСтр("ru='от ';uk='від '")+Строка(Стр.Значение.ЗначениеОт) + НСтр("ru=' до ';uk=' до '") + Строка(Стр.Значение.ЗначениеДо);
			ИначеЕсли НЕ Стр.Значение.ЗначениеОт=Неопределено Тогда
				ПредставлениеЗначения = НСтр("ru='от ';uk='від '")+Строка(Стр.Значение.ЗначениеОт);
			ИначеЕсли НЕ Стр.Значение.ЗначениеДо=Неопределено Тогда
				ПредставлениеЗначения = НСтр("ru='до ';uk='до'")+Строка(Стр.Значение.ЗначениеДо);
			КонецЕсли; 
			Префикс = ЗаголовокФильтра;
...
Функция ПредставлениеВидаСравнения(ВидСравнения)
	...
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.ВИерархии Тогда
		Возврат НСтр("ru=' в группе ';uk=' в группе '");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.ВСписке Тогда
		Возврат НСтр("ru=' в списке ';uk=' в списке '");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии Тогда
		Возврат НСтр("ru=' в списке групп ';uk=' в списке групп '");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.Заполнено Тогда
		Возврат НСтр("ru=' заполнено';uk=' заполнено'");
	...
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.НеСодержит Тогда
		Возврат НСтр("ru=' не содержит ';uk=' не содержит '");
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.Подобно Тогда
		Возврат НСтр("ru=' подобно ';uk=' подобно '");
	...
	ИначеЕсли ВидСравнения=ВидСравненияКомпоновкиДанных.Содержит Тогда
		Возврат НСтр("ru=' содержит ';uk=' містить'");
	Иначе
		Возврат "";
	КонецЕсли; 	
...
Функция СформироватьСписокЭлементовСервер(Текст, МассивТипов, УправляющиеСимволы)
	...
	Пока ВыборкаТип.Следующий() Цикл
		...
		Если Выборка.Количество()>1 ИЛИ (УправляющиеСимволы="""" И Выборка.Количество()>0) Тогда
			...
			Представление = ПредставлениеТипа+НСтр("ru=' содержит ';uk=' містить'")+Текст;
...
Процедура РедактироватьСКД()
	...
	Если Не НастройкиОтчета.РазрешитьРедактироватьСКД Тогда
		Состояние(НСтр("ru='Редактирование настроек данного отчета запрещено';uk='Редактирование настроек данного отчета запрещено'"));
		Возврат;
	КонецЕсли;
...
&НаСервере
Процедура ВыполнитьНестандартнуюОбработку(ИдентификаторСтроки, ИмяЭлемента)
...
Функция СуммаЯчеек(ТабличныйДокумент, ВыделенныеОбласти)
	...
	Если ВыделенныеОбласти = Неопределено Тогда
		...
		#Иначе
			Возврат НСтр("ru='Не указано значение параметра ""ВыделенныеОбласти"".';uk='Не зазначене значення параметра ""ВыделенныеОбласти"".'");
		#КонецЕсли
	КонецЕсли;
...
//
&НаСервере
Функция ДобавитьОписаниеПоказателяВызов(Путь, НестандартныйОбработчик = Ложь, ТипКолонки = "Показатель") Экспорт
