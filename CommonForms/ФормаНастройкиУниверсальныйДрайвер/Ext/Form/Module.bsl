...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Если СчитывательМагнитныхКарт Тогда
		...
		Если ПрефиксДрайвера > 0 И СуффиксДрайвера > 0 И ПрефиксДрайвера = СуффиксДрайвера Тогда
			ТекстСообщения = НСтр("ru='Префикс первой используемой дорожки совпадает с суффикс последней используемой дорожки';uk='Префикс первой используемой дорожки совпадает с суффикс последней используемой дорожки'");
		...
		ИначеЕсли ДорожкаСПустымСуффиксом Тогда
			ТекстСообщения = НСтр("ru='Для каждой используемой дорожки должен быть указан суффикс';uk='Для каждой используемой дорожки должен быть указан суффикс'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
...
Процедура НачатьЗапускПриложенияЗавершение(Результат, Параметры) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Начата установка основной поставки драйвера.';uk='Почата установка основної поставки драйвера.'")); 
...
"Продолжить выполнение операции?';uk='Для перехода на сайт производителя необходимо подключение к Интернету."
...
Процедура НачатьВыполнениеДополнительнойКомандыЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если ИнтеграционныйКомпонент И НЕ ОсновнойДрайверУстановлен Тогда
			ДрайверУстановлен = НСтр("ru='Установлен интеграционный компонент';uk='Установлен интеграционный компонент'");
...
#Если Не ВебКлиент Тогда  
	Если ТребуетсяПереустановка Тогда
		...
		Элементы.ДекорацияПереустановитьДрайвер.Заголовок = НСтр("ru='Драйвер будет переустановлен после перезагрузки ""1С:Предприятие"".';uk='Драйвер будет переустановлен после перезагрузки ""1С:Предприятие"".'");
	...
	ИначеЕсли Не ПустаяСтрока(ВерсияДрайвераВМакете) И Не ПустаяСтрока(ВерсияДрайвера) 
		...
		Элементы.Переустановка.Видимость = Истина;
		ТекстСообщения = НСтр("ru='Версия драйвера в конфигурации (%ВКонфигурации%) отличается от установленного.';uk='Версия драйвера в конфигурации (%ВКонфигурации%) отличается от установленного.'");
