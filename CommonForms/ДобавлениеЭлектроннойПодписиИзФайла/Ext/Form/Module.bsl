...
Процедура ОК(Команда)
	...
	Если Подписи.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Не выбрано ни одного файла подписи';uk='Не обране жодного файлу підпису'"));
		Возврат;
	КонецЕсли;
...
Процедура ОКЗавершение(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru='Не удалось добавить электронную подпись из файла';uk='Не вдалося додати електронний підпис із файлу'"), "", Ошибка, Новый Структура);
		Возврат;
	КонецЕсли;
...
Процедура ВыбратьФайлПослеПодключенияРасширенияРаботыСФайлами(Подключено, Контекст) Экспорт
	...
	Диалог.Заголовок = НСтр("ru='Выберите файл электронной подписи';uk='Виберіть файл електронного підпису'");
	Диалог.Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Файлы подписи (*.%1)|*.%1|Все файлы(*.*)|*.*';uk='Файли підпису (*.% 1)|*.% 1|Всі файли(*.*)|*.*'"),
...
"%1';uk='При одержанні сертифікатів з файлу підпису відбулася помилка:"
"%1'"),
...
Процедура ВыбратьФайлПослеПолученияСертификатовИзПодписи(Сертификаты, Контекст) Экспорт
	...
	Если Сертификаты.Количество() = 0 Тогда
		ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки",
			НСтр("ru='В файле подписи нет ни одного сертификата.';uk='У файлі підпису немає жодного сертифіката.'"));
...
"%1'"),
	...
	Если Сертификаты.Количество() = 0 Тогда
		ОшибкаНаСервере.Вставить("ОписаниеОшибки", НСтр("ru='В файле подписи нет ни одного сертификата.';uk='У файлі підпису немає жодного сертифіката.'"));
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПоказатьОшибку(ОшибкаНаКлиенте, ОшибкаНаСервере)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ru='Не удалось получить подпись из файла';uk='Не вдалося одержати підпис із файлу'"), "", ОшибкаНаКлиенте, ОшибкаНаСервере);
