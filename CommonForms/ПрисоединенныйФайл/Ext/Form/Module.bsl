...
"Записати зміни і зняти помітку на видалення з файлу"
"""%1""?'");
		Иначе
			ТекстВопроса = НСтр("ru='Для выполнения действия требуется записать изменения файла."
...
"""%1""?';uk='Зняти помітку на видалення з файлу"
"""%1""?'");
		Иначе
			ТекстВопроса = НСтр("ru='Пометить на удаление файл"
"""%1""?';uk='Позначити на видалення файл"
"""%1""?'");
		КонецЕсли;
	КонецЕсли;
...
Процедура УдалитьЭП(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Удалить выделенные подписи?';uk='Видалити виділені підписи?'"), РежимДиалогаВопрос.ДаНет);
