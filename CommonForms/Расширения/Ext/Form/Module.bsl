...
Процедура СохранитьКакПослеУстановкиРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	Если РасширениеПодключено Тогда
		...
		Диалог.Заголовок = НСтр("ru='Выберите файл для сохранения расширения конфигурации';uk='Виберіть файл для збереження розширення конфігурації'");
...
Процедура УдалитьУстаревшиеКэшиРасширений(Команда)
	...
	ПоказатьПредупреждение(, НСтр("ru='Выполнено удаление устаревших версий параметров работы расширений.';uk='Виконане видалення застарілих версій параметрів роботи розширень.'"));
...
Процедура УдалитьРасширения(ВыделенныеСтроки)
	...
	Если ВыделенныеСтроки.Количество() > 1 Тогда
		ТекстВопроса = НСтр("ru='Вы уверены, что хотите удалить выделенные расширения?';uk='Ви впевнені, що прагнете видалити виділені розширення?'", "ru");
	Иначе
		ТекстВопроса = НСтр("ru='Вы уверены, что хотите удалить расширение?';uk='Ви впевнені, що прагнете видалити розширення?'", "ru");
	КонецЕсли;
...
""
"%1';uk='Не вдалося видалити з причини:"
""
...
"%1';uk='При спробі відновити видалені розширення відбулася ще одна помилка:"
"%1'"), КраткоеПредставлениеОшибки(ТекущаяИнформацияОбОшибке));
		КонецПопытки;
		Если Не ЗначениеЗаполнено(ТекстОшибки) Тогда
			ТекстОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке) + Символы.ПС + Символы.ПС
				+ НСтр("ru='Удаленные расширения были восстановлены.';uk='Видалені розширення були відновлені.'");
		КонецЕсли;
	КонецПопытки;
...
""
"%1';uk='Після видалення, при підготовці розширень, що залишилися, до роботи, сталася помилка:"
""
"%1'"), ТекстОшибки);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Процедура ЗагрузитьРасширениеПослеУстановкиРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	Если РасширениеПодключено Тогда
		...
		Диалог.Заголовок = НСтр("ru='Выберите файл расширения конфигурации';uk='Виберіть файл розширення конфігурації'", "ru");
...
"%1';uk='Після додавання, при підготовці розширень до роботи, відбулася помилка:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
		Возврат;
	КонецПопытки;
	СкрытьДлительнуюОперацию();
	...
	Если Контекст.ИдентификаторРасширения = Неопределено Тогда
		Если Контекст.ПомещенныеФайлы.Количество() > 1 Тогда
			ТекстОповещения = НСтр("ru='Расширения конфигурации добавлены';uk='Розширення конфігурації додані'", "ru");
		Иначе
			ТекстОповещения = НСтр("ru='Расширение конфигурации добавлено';uk='Розширення конфігурації додане'", "ru");
		КонецЕсли;
	Иначе
		ТекстОповещения = НСтр("ru='Расширение конфигурации обновлено';uk='Розширення конфігурації оновлене'", "ru");
	КонецЕсли;
		...
		Если ИдентификаторРасширения <> Неопределено Тогда
			...
			Если ПрежнееИмяРасширения <> Расширение.Имя Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Нельзя загрузить расширение ""%1"" в ""%2"".';uk='Не можна завантажити розширення ""%1"" в ""%2"".'"),
					Расширение.Имя,
...
"%1';uk='При спробі відновити змінене розширення відбулася ще одна помилка:"
"%1'"), КраткоеПредставлениеОшибки(ТекущаяИнформацияОбОшибке));
				Иначе
					ВызватьИсключение КраткоеПредставлениеОшибки(ИнформацияОбОшибке) + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='При попытке удалить добавленные расширения произошла еще одна ошибка:"
"%1';uk='При спробі вилучити додані розширення відбулася ще одна помилка:"
"%1'"), КраткоеПредставлениеОшибки(ТекущаяИнформацияОбОшибке));
				КонецЕсли;
			КонецПопытки;
		КонецЕсли;
		ВызватьИсключение КраткоеПредставлениеОшибки(ИнформацияОбОшибке) + Символы.ПС + Символы.ПС
			+ НСтр("ru='Добавленные расширения были удалены.';uk='Додані розширення були видалені.'");
	КонецПопытки;
...
"возможно оно было удалено в другом сеансе.';uk='Поточне розширення не знайдене у базі даних,"
"можливо воно було видалене в іншому сеансі.'");
				КонецЕсли;
			КонецЕсли;
...
Процедура ПроверитьДоступностьРазделенныхДанных()
	...
	Если Не ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение НСтр("ru='Расширения недоступны в неразделенном режиме.';uk='Розширення недоступні в нерозділеному режимі.'");
	КонецЕсли;
