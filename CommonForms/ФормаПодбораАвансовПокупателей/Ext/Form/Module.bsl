	...
	Для каждого СтрокаПредоплата Из Предоплата Цикл
		...
		И НЕ ЗначениеЗаполнено(СтрокаПредоплата.Курс) Тогда
			...
			Сообщение.Текст = НСтр("ru='Не заполнена колонка ""Курс"" в строке ';uk='Не заповнений стовпчик ""Курс"" у рядку'")
				+ Строка(НомерСтроки)
				+ НСтр("ru=' списка ""Расшифровка расчетов"".';uk=' списку ""Розшифрування розрахунків"".'");
		...
		И НЕ ЗначениеЗаполнено(СтрокаПредоплата.СуммаПлатежа) Тогда
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = НСтр("ru='Не заполнена колонка ""Сумма платежа"" в строке ';uk='Не заповнений стовпчик ""Сума платежу"" у рядку'")
				+ Строка(НомерСтроки)
				+ НСтр("ru=' списка ""Расшифровка расчетов"".';uk=' списку ""Розшифрування розрахунків"".'");
			Сообщение.Поле = "Документ";
			Сообщение.Сообщить();
...
Процедура ЗаполнитьАвансы()
	...
	СписокАвансов.Загрузить(Запрос.Выполнить().Выгрузить());
