...
Процедура ЗаписатьИЗакрытьВыполнить()
	...
	ИначеЕсли ДорожкаСПустымСуффиксом Тогда
		ТекстСообщения = НСтр("ru='Для каждой используемой дорожки должен быть указан суффикс, отличный от 0';uk='Для кожної використовуваної доріжки повинен бути зазначений суфікс, відмінний від 0'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ОбновитьИнформациюОДрайвере()
	...
	Если МенеджерОборудованияКлиент.ВыполнитьДополнительнуюКоманду("ПолучитьВерсиюДрайвера",
		...
		Если ВерсияИзБПОСтр > ВерсияСтр Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Установленная на компьютере версия драйвера устарела! Необходимо обновление до версии:';uk='Встановлена на комп`ютері версія драйвера застаріла! Необхідне оновлення до версії:'") + Символы.НПП + ВерсияИзБПО);
		КонецЕсли;
	Иначе
