...
Процедура ШагСеткиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение<>ШагСетки Тогда
		...
		Если УпраздняемыеПоля.Количество()>0 Тогда
			...
			СтрокаВопрос = НСтр("ru='Поля:%Поля%имеют слишком маленькие размеры.%ПереносСтроки%Продолжить изменение шага сетки, удалив указанные поля?';uk='Поля:%Поля%мають занадто малі розміри.% ПереносСтроки%Продовжити зміну кроку сітки, видалівши зазначені поля?'");
...
Процедура СохранитьВФайл(Команда)
	...
	ДОФ.Фильтр = НСтр("en='XML file';ru='XML-файл';uk='XML-файл'") + "(*.xml)|*.xml";
	ДОФ.МножественныйВыбор = Ложь;
	ДОФ.Заголовок = НСтр("ru='Сохранение файла-макета';uk='Збереження файла-макету'");
...
Процедура ВыборФайла()
	...
	ДОФ.Фильтр = НСтр("en='XML file';ru='XML-файл';uk='XML-файл'") + "(*.xml)|*.xml";
	ДОФ.МножественныйВыбор = Ложь;
	ДОФ.Заголовок = НСтр("ru='Выберите файл макета';uk='Виберіть файл макета'");
...
Процедура ОпроситьПользователяОбУдаленииПересекаемыхПолей(ПередаваемыеПараметры)
	...
	Если ПередаваемыеПараметры.Свойство("РедактируемоеПоле") Тогда
		ТекстВопроса = НСтр("ru='Новые границы редактируемого поля пересекаются с полями:%СписокПолей%Отменить внесенные изменения?';uk='Нові границі поля, що редагується, перетинаються з полями:%СписокПолей%Скасувати внесені зміни?'");
	Иначе
		ТекстВопроса = НСтр("ru='Создаваемое поле пересекается с уже существующими полями:%СписокПолей%Отменить внесенные изменения?';uk='Створюване поле перетинається із уже існуючими полями:%СписокПолей%Скасувати внесені зміни?'");
	КонецЕсли;
