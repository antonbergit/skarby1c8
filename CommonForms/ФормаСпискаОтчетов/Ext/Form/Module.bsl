...
Процедура Подключаемый_Избранное(Команда)
	...
	Если Добавлен = Истина Тогда
		Состояние(НСтр("ru='Отчет добавлен в избранное';uk='Звіт доданий у вибране'"),,, БиблиотекаКартинок.БольшаяЖелтаяЗвезда);
...
Процедура Подключаемый_УдалитьОтчет(Команда)
	...
	ПоказатьВопрос(
	ОписаниеОповещения,
	СтрШаблон(НСтр("ru='Удалить отчет ""%1""?';uk='Видалити звіт ""%1""?'"), ДанныеОтчета.Наименование),
...
Процедура УстановитьВидимостьКнопкиПоказатьЕще()
	...
	Если Элементы.ГруппаПоказатьЕще.Видимость <> ВидимостьКнопкиЕще Тогда
		Элементы.ГруппаПоказатьЕще.Видимость = ВидимостьКнопкиЕще;
	КонецЕсли;
...
Процедура ЗаполнитьАдресаКартинок()
	...
	Для каждого Стр Из СписокОтчетов Цикл
		...
		Если СписокОтчетов.Индекс(Стр) >= КоличествоОтображаемыхОтчетов
			И Не КоличествоОтображаемыхОтчетов = 0 Тогда
			Прервать;
		КонецЕсли;
...
Процедура ДобавитьКомандуНастройкиОтчета(КонтекстноеМеню, Знач Стр)
	...
	Команда = Команды.Добавить(ИмяКоманды);
	Команда.Заголовок = НСтр("ru='Настройки отчета';uk='Налаштування звіту'");
...
Процедура ДобавитьКомандуУдалитьОтчет(КонтекстноеМеню, Стр, ПолныеПраваНаВарианты)
	...
	Команда = Команды.Добавить(ИмяКоманды);
	Команда.Заголовок = НСтр("ru='Удалить';uk='Видалити'");
...
Процедура ОбновитьКомандуИзбранное(ИмяКоманды, НеВИзбранном)
	...
	Если НеВИзбранном Тогда
		Команда.Заголовок = НСтр("ru='В избранное';uk='В обране'");
		Команда.Картинка = БиблиотекаКартинок.БольшаяСераяЗвезда;
	Иначе
		Команда.Заголовок = НСтр("ru='Удалить из избранного';uk='Видалити з вибраного'");
...
Функция ТекстПодсказкиДляПустогоСписка()
	...
	Если Раздел = РазделВсеОтчеты()
		...
		Возврат НСтр("ru='Не найдены отчеты, отвечающие отбору.';uk='Не найдены отчеты, отвечающие отбору.'");
		
	ИначеЕсли Раздел = РазделИзбранные() Тогда
		
		Возврат НСтр("ru='Пока ни один отчет не добавлен в избранное.';uk='Пока ни один отчет не добавлен в избранное.'");
		
	ИначеЕсли Раздел = РазделПоследние() Тогда
		
		Возврат НСтр("ru='В этот список автоматически добавляются последние использованные отчеты. Список пока пуст.';uk='У цей список автоматично додаються останні використані звіти. Список поки порожній.'");
...
Процедура УстановитьВидимостьИДостпность()
	...
	Если ПараметрыФормы.ЕстьВнешниеОтчеты Тогда
		...
		Если Элемент = Неопределено Тогда
			Элементы.Раздел.СписокВыбора.Добавить("Внешние", НСтр("ru='Внешние';uk='Зовнішні'"));
		КонецЕсли; 
	Иначе
		Элемент = Элементы.Раздел.СписокВыбора.НайтиПоЗначению("Внешние");
		Если Элемент <> Неопределено Тогда
...
Процедура ОбновитьЗаголовок()
	...
	Если ЗначениеЗаполнено(ПоОбъекту) Тогда
		...
		Заголовок = НСтр("ru='Отчеты по: ';uk='Звіти по: '") + ПоОбъекту;
		
	ИначеЕсли Раздел = РазделВсеОтчеты() Тогда
		
		Заголовок = НСтр("ru='Все отчеты';uk='Всі звіти'");
		
	ИначеЕсли Раздел = РазделИзбранные() Тогда
		
		Заголовок = НСтр("ru='Избранные отчеты';uk='Вибрані звіти'");
		
	ИначеЕсли Раздел = РазделПоследние() Тогда
		
		Заголовок = НСтр("ru='Последние отчеты';uk='Останні звіти'");
		
	ИначеЕсли Раздел = РазделВнешние() Тогда
		
		Заголовок = НСтр("ru='Внешние отчеты';uk='Внешние отчеты'");
...
Процедура ОткрытьДополнительныйОтчет(Вариант)
	...
	ИначеЕсли ВыполняемаяКоманда.ВариантЗапуска = ПредопределенноеЗначение("Перечисление.СпособыВызоваДополнительныхОбработок.ВызовСерверногоМетода")
		...
		Иначе
			Состояние(НСтр("ru='Выполняется команда...';uk='Виконується команда...'"),,, БиблиотекаКартинок.ДлительнаяОперация48);
