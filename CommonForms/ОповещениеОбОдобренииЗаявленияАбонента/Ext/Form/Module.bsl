		...
		Иначе
			...
			Если НЕ РезультатОтветаСервера.УдалосьСоединиться Тогда
				...
				ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ПоказатьДиалогОшибкиДоступаВИнтернет(
					НСтр("ru='Не удалось обратиться к серверу регистрации';uk='Не удалось обратиться к серверу регистрации'"));
...
Процедура СформироватьНадпись()
	...
	Если ЭтоВторичноеЗаявление Тогда
		ВидЗаявления = НСтр("ru='Заявление №%1 на изменение подключения';uk='Заявление №%1 на изменение подключения'");
	Иначе
		ВидЗаявления = НСтр("ru='Заявление №%1 на подключение';uk='Заявление №%1 на подключение'");
	КонецЕсли;
	...
	Если Одобрено Тогда
		Результат = НСтр("ru='одобрено';uk='одобрено'");
	Иначе
		Результат = НСтр("ru='отклонено';uk='отклонено'");
	КонецЕсли;
	
	ТекстОповещения = Новый ФорматированнаяСтрока(НСтр("ru=' организации ""%1"" к 1С-Отчетности %2.';uk=' организации ""%1"" к 1С-Отчетности %2.'"));
	...
	Если Одобрено Тогда
		ЧтоНужноСделать = НСтр("ru='Для завершения нужно выполнить несколько простых шагов.';uk='Для завершения нужно выполнить несколько простых шагов.'");
	Иначе
		ЧтоНужноСделать = НСтр("ru='Устраните проблему и отправьте новое заявление.';uk='Устраните проблему и отправьте новое заявление.'");
	КонецЕсли;
...
Процедура СформироватьЗаголовок()
	...
	Если ЭтоВторичноеЗаявление Тогда
		ВидЗаявления = НСтр("ru='Заявление на изменение подключения к 1С-Отчетности ';uk='Заявление на изменение подключения к 1С-Отчетности '");
	Иначе
		ВидЗаявления = НСтр("ru='Заявление на подключение к 1С-Отчетности ';uk='Заявление на подключение к 1С-Отчетности '");
	КонецЕсли;
...
Процедура СформироватьЗаголовокКнопок()
	...
	Если Одобрено Тогда
		Элементы.КнопкаНачатьИспользование.Заголовок = НСтр("ru='Продолжить сейчас';uk='Продолжить сейчас'");
	Иначе
		Элементы.КнопкаНачатьИспользование.Заголовок = НСтр("ru='Подготовить новое заявление';uk='Подготовить новое заявление'");
	КонецЕсли;
