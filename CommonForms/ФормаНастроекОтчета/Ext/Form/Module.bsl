...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		КонецЕсли;
		Заголовок = НСтр("ru='Настройки отчета';uk='Налаштування звіту'") + " """ + ВариантНаименование + """";
	КонецЕсли;
...
Процедура Подключаемый_СписокСПодбором_ОбработкаВыбора(Элемент, РезультатВыбора, СтандартнаяОбработка)
	...
	Если Дополнение.Всего > 0 Тогда
		Если Дополнение.Всего = 1 Тогда
			ЗаголовокОповещения = НСтр("ru='Элемент добавлен в список';uk='Елемент доданий у список'");
		Иначе
...
Процедура ДобавитьТаблицуИлиДиаграмму(ТипЭлемента)
	...
	ИначеЕсли СтрокаТаблицы.Тип = "Таблица" Тогда
		...
		ВложеннаяСтрока.ИдентификаторКД = УзелКД.ПолучитьИдентификаторПоОбъекту(ВложенныйЭлементКД);
		ВложеннаяСтрока.ИндексКартинки = -1;
		ВложеннаяСтрока.Представление = НСтр("ru='Колонки';uk='Стовпчика'");
...
Процедура СтруктураВарианта_СохранитьВФайл(Команда)
	...
	ПолучитьФайл(Адрес, НСтр("ru='Настройки.xml';uk='Налаштування.xml'"), Истина);
КонецПроцедуры
...
Функция ШагОпределитьВыделенныеСтроки(ВыборПользователя, Сценарий)
	...
	Если Сценарий.СтрокиДерева.Количество() = 0 Тогда
		Сценарий.ПричинаОтказа = НСтр("ru='Выберите элементы.';uk='Виберіть елементи.'");
		Возврат Ложь; // Завершить сценарий.
	КонецЕсли;
	Если Сценарий.ТекущаяСтрока = Неопределено Тогда
		Если Сценарий.Действие = "ИзменитьГруппу" Тогда
			Сценарий.ПричинаОтказа = НСтр("ru='Выберите группу.';uk='Виберіть групу.'");
			Возврат Ложь; // Завершить сценарий.
		КонецЕсли;
	КонецЕсли;
...
Функция ЗарегистрироватьВыделеннуюСтроку(Сценарий, СтрокаДерева, Особенности)
	...
	Если Особенности.МогутБытьПараметры И СтрокаДерева.ЭтоПараметр Тогда
		...
		ИначеЕсли Сценарий.Действие = "Удалить" Тогда
			Сценарий.ПричинаОтказа = НСтр("ru='Параметры не могут быть удалены.';uk='Параметри не можуть бути видалені.'");
		КонецЕсли;
		Возврат Ложь; // Завершить сценарий.
	КонецЕсли;
	Если Особенности.ТребоватьОдногоРодителя Тогда
		...
		ИначеЕсли Сценарий.ТекущийРодитель <> Родитель Тогда
			Если Сценарий.Действие = "Переместить" Тогда
				Сценарий.ПричинаОтказа = НСтр("ru='Выбранные элементы не могут быть перемещены, поскольку они принадлежат разным родителям.';uk='Обрані елементи не можуть бути переміщені, оскільки вони належать різним родителям.'");
			ИначеЕсли Сценарий.Действие = "Сгруппировать" Тогда
				Сценарий.ПричинаОтказа = НСтр("ru='Выбранные элементы не могут быть сгруппированы, поскольку они принадлежат разным родителям.';uk='Обрані елементи не можуть бути згруповані, оскільки вони належать різним родителям.'");
			КонецЕсли;
			Возврат Ложь; // Завершить сценарий.
		КонецЕсли;
	КонецЕсли;
...
Процедура ОтборыВыбратьВидСравнения(СтрокаТаблицы)
	...
	Если ИнформацияОТипах.ОграниченнойДлины
		...
		Список.Добавить(ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии); // НСтр("ru='В списке включая подчиненные';uk='У списку включаючи підлеглі'")
		Список.Добавить(ВидСравненияКомпоновкиДанных.НеВСпискеПоИерархии); // НСтр("ru='Не в списке включая подчиненные';uk='Не в списку включаючи підлеглі'").
...
Процедура СтруктураВариантаИзменитьУзел(ТаблицаИмя, СтрокаИдентификатор, СтрокаТаблицы, ИмяСтраницы)
	...
	ШаблонЗаголовка = НСтр("ru='Настройка %1 отчета ""%2""';uk='Налаштування %1 звіту ""%2""'");
	Если СтрокаТаблицы.Тип = "Диаграмма" Тогда
		ПредставлениеУзла = НСтр("ru='диаграммы';uk='діаграми'");
	Иначе
		ПредставлениеУзла = НСтр("ru='группировки';uk='угруповання'");
	КонецЕсли;
...
Процедура ТаблицыПолейИзменитьУровеньДоступа(ТаблицаИмя, СтрокаИдентификатор, ПоказыватьРежимыВвода, ПоказыватьРежимыФлажков)
	...
	Если ПоказыватьРежимыВвода Тогда
		Список.Добавить(2, НСтр("ru='В шапке отчета';uk='У шапці звіту'"), , БиблиотекаКартинок.БыстрыйДоступ);
	КонецЕсли;
	Если ПоказыватьРежимыФлажков Тогда
		Список.Добавить(1, НСтр("ru='Только флажок в шапке отчета';uk='Тільки прапорець у шапці звіту'"), , БиблиотекаКартинок.БыстрыйДоступСФлажком);
	КонецЕсли;
	Если ПоказыватьРежимыВвода Тогда
		Список.Добавить(4, НСтр("ru='В настройках отчета';uk='У налаштуваннях звіту'"), , БиблиотекаКартинок.Реквизит);
	КонецЕсли;
	Если ПоказыватьРежимыФлажков Тогда
		Список.Добавить(3, НСтр("ru='Только флажок в настройках отчета';uk='Тільки прапорець у налаштуваннях звіту'"), , БиблиотекаКартинок.ОбычныйДоступСФлажком);
	КонецЕсли;
...
Процедура ТаблицыПолейРазгруппировать(ТаблицаИмя)
	...
	Если ВыделенныеСтроки.Количество() <> 1 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Выберите одну группу.';uk='Виберіть одну групу.'"));
		Возврат;
	КонецЕсли;
	...
	Если ГруппаДерева = Неопределено Или Не ГруппаДерева.ЭтоГруппа Тогда
		ПоказатьПредупреждение(, НСтр("ru='Выберите группу.';uk='Виберіть групу.'"));
		Возврат;
	КонецЕсли;
...
Процедура РасширенныеНастройкиЗагрузитьЗначения(ПараметрыЗаполнения, Информация)
	...
	Если ВидимостьВыбранныхПолей И ВидимостьСортировок Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = НСтр("ru='Поля и сортировки';uk='Поля й сортування'");
...
Функция ЗарегистрироватьЭлементыСортировки()
	...
	Для Каждого ЭлементКД Из НаборСтрокКД Цикл
		...
		Если ТипНастройки = "АвтоЭлементПорядка" Тогда
			СтрокаТаблицы.Представление = НСтр("ru='Авто (сортировки родителя)';uk='Авто (сортування родителя)'");
...
Функция ЗарегистрироватьЭлементыВыбранныхПолей(УзелКД, НаборСтрокКД = Неопределено, НаборСтрок = Неопределено, ДоступныеПоля = Неопределено)
	...
	Для Каждого ЭлементКД Из НаборСтрокКД Цикл
		...
		Если ТипНастройки = "АвтоВыбранноеПоле" Тогда
			Представление = НСтр("ru='Авто (поля родителя)';uk='Авто (поля родителя)'");
			ИндексКартинки = 6;
		Иначе
