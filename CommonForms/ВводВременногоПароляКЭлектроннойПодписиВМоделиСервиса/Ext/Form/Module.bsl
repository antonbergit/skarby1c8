...
Процедура Подтвердить(Команда)
	...
	Если ЗначениеЗаполнено(Пароль) И СтрДлина(Пароль) = 6 Тогда
		...
		Исключение
			...
			Если СтрНайти(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке), КриптосервисВМоделиСервисаКлиентСервер.ТекстИсключенияУказанНеВерныйВременныйПароль()) > 0 Тогда
				ТекстОшибки = НСтр("ru='Указан неверный временный пароль.';uk='Указан неверный временный пароль.'");
			Иначе 
	...
	ИначеЕсли ЗначениеЗаполнено(Пароль) И СтрДлина(Пароль) <> 6 Тогда
		ТекстОшибки = НСтр("ru='Временный пароль должен состоять из 6 цифр.';uk='Временный пароль должен состоять из 6 цифр.'");
	Иначе
		ТекстОшибки = НСтр("ru='Временный пароль не указан.';uk='Временный пароль не указан.'");
	КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	Если Форма.СпособДоставкиПаролей = "phone" Тогда
		Пояснение = НСтр("ru='На номер %1 отправлено SMS-сообщение с временным паролем.';uk='На номер %1 отправлено SMS-сообщение с временным паролем.'");
	...
	ИначеЕсли Форма.СпособДоставкиПаролей = "email" Тогда
		Пояснение = НСтр("ru='На адрес %1 отправлено письмо с временным паролем.';uk='На адрес %1 отправлено письмо с временным паролем.'");
