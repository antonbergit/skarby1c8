...
"<li>Попробуйте уменьшить количество искомых слов.';uk='Шукана комбінація слів ніде не зустрічається<br><br>"
"<b>Рекомендації:</b>"
"<li>Переконайтеся, що всі слова написані без помилок."
"<li>Спробуйте використовувати інші ключові слова."
...
Процедура СформироватьПредставлениеОбластиПоиска()
	...
	Если Не ИскатьВРазделах Или ОбластиПоиска.Количество() = 0 Тогда
		Элементы.ОбластиПоискаПредставление.Заголовок = НСтр("ru='Везде';uk='Всюди'");
		Возврат;
	КонецЕсли;
	...
	Иначе
		Элементы.ОбластиПоискаПредставление.Заголовок = НСтр("ru='В выбранных разделах';uk='В обраних розділах'");
	КонецЕсли;
