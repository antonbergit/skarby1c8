...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НаименованиеЗадания = Нстр("ru='Обновление списка банков, поддерживающих обмен DirectBank';uk='Оновлення списку банків, що підтримують обмін DirectBank'");
...
"Обратитесь к администратору';uk='У вас немає прав налаштування прямого обміну з банком."
...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ КлассификаторЗаполнен Тогда
		Состояние(
		НСтр("ru='Обновление классификатора банков';uk='Оновлення класифікатора банків'"),,,
...
"Прервать работу помощника?';uk='DirectBank не підключений."
...
Процедура ПутьКФайлуКомпонентыСбербанкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Иначе
		...
		Фильтр = НСтр("ru='Внешний модуль';uk='Зовнішній модуль'") + "(*.zip)|*.zip";
		ДиалогОткрытияФайла.Фильтр = Фильтр;
		ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
		ДиалогОткрытияФайла.Заголовок = Нстр("ru='Выберите файл внешнего модуля банка';uk='Виберіть файл зовнішнього модуля банку'");
...
Процедура ПутьКФайлуСертификатаСбербанкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Иначе
		Режим = РежимДиалогаВыбораФайла.Открытие;
		ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(Режим);
		Фильтр = НСтр("ru='Сертификат банка';uk='Сертифікат банку'") + "(*.cer)|*.cer";
		ДиалогОткрытияФайла.Фильтр = Фильтр;
		ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
		ДиалогОткрытияФайла.Заголовок = НСтр("ru='Выберите файл сертификата банка';uk='Виберіть файл сертифіката банку'");
...
Процедура ПутьКФайлуВнешнегоМодуляНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Иначе
		Режим = РежимДиалогаВыбораФайла.Открытие;
		ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(Режим);
		Фильтр = НСтр("ru='Внешний модуль';uk='Зовнішній модуль'") + "(*.epf)|*.epf";
		ДиалогОткрытияФайла.Фильтр = Фильтр;
		ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
		ДиалогОткрытияФайла.Заголовок = НСтр("ru='Выберите файл внешнего модуля';uk='Виберіть файл зовнішнього модуля'");
...
Процедура ПутьКФайлуНастроекНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Иначе
		Режим = РежимДиалогаВыбораФайла.Открытие;
		ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(Режим);
		Фильтр = НСтр("ru='Файл настроек обмена';uk='Файл налаштувань обміну'") + "(*.xml)|*.xml";
		ДиалогОткрытияФайла.Фильтр = Фильтр;
		ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
		ДиалогОткрытияФайла.Заголовок = Нстр("ru='Выберите файл настроек обмена';uk='Виберіть файл налаштувань обміну'");
...
Процедура Готово(Команда)
	...
	Если Элементы.Шаги.ТекущаяСтраница=Элементы.СтраницаДобавлениеСчета Тогда
		...
		Если Не ЗначениеЗаполнено(НомерСчета) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите номер счета';uk='Вкажіть номер рахунку'"), , "НомерСчета", , Отказ);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ВалютаДенежныхСредств) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите валюту счета';uk='Вкажіть валюту рахунку'"), , "ВалютаДенежныхСредств", , Отказ);
		КонецЕсли;
		Если Отказ Тогда
			Возврат;
		КонецЕсли;
...
Процедура СледующийШаг()
	...
	Если Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаВыборОрганизацииИБанка Тогда
		...
		Если Не ЗначениеЗаполнено(Организация) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Выберите организацию';uk='Виберіть організацію'"), , "Организация", , Отказ);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Банк) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Выберите банк';uk='Виберіть банк'"), , "Банк", , Отказ);
		КонецЕсли;
		...
		Если ИспользуетсяВнешнийМодуль Тогда
			...
			#Если ВебКлиент Тогда
				Если Не ВыбранФайлВнешнегоМодуля Тогда
					ТекстСообщения = НСтр("ru='Не выбран файл внешнего модуля';uk='Не вибраний файл зовнішнього модуля'");
			...
			#Иначе
				Если ЗначениеЗаполнено(ПутьКФайлуВнешнегоМодуля) Тогда
					...
					Исключение
						ТекстСообщения = НСтр("ru='Неверно указан путь к файлу внешнего модуля';uk='Невірно вказаний шлях до файлу зовнішнього модуля'");
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуВнешнегоМодуля");
						Возврат;
					КонецПопытки;
...
"Файл внешнего модуля должен иметь расширение ""epf""';uk='Вказаний невірний файл."
"Файл зовнішнього модуля повинен мати розширення ""epf""'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуВнешнегоМодуля");
				Возврат;
			КонецЕсли;
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаАсинхронныйОбмен Тогда
		...
		Если Не ЗначениеЗаполнено(АдресСервера) Тогда
			ТекстСообщения = НСтр("ru='Не указан адрес сервера банка';uk='Не вказана адреса сервера банку'");
		...
		Если ЗначениеЗаполнено(АдресСервера) И НЕ ОбменСБанкамиСлужебныйКлиент.ПравильныйФорматАдреса(АдресСервера) Тогда
			ТекстСообщения = НСтр("ru='Адрес сервера банка должен начинаться с ""https://"" или ""http://""';uk='Адреса сервера банку повинна починатися з ""https://"" або ""http://""'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "АдресСервера", , Отказ);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ИдентификаторОрганизации) Тогда
			ТекстСообщения = НСтр("ru='Не указан идентификатор организации';uk='Не вказаний ідентифікатор організації'");
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаСинхронныйОбмен Тогда
		...
		Если Не ЗначениеЗаполнено(РесурсПриемник) Тогда
			ТекстСообщения = НСтр("ru='Не указан ресурс для отправки';uk='Не вказаний ресурс для відправки'");
		...
		Если Не ЗначениеЗаполнено(РесурсИсточник) Тогда
			ТекстСообщения = НСтр("ru='Не указан ресурс для получения';uk='Не вказаний ресурс для отримання'");
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаВыборСпособаАутентификации Тогда
		...
		Если СпособАутентификации = 1 И НЕ ЗначениеЗаполнено(Логин) Тогда
			ТекстСообщения = НСтр("ru='Введите логин';uk='Введіть логін'");
		...
		ИначеЕсли СпособАутентификации = 0 И НЕ ЗначениеЗаполнено(СертификатАутентификации) Тогда
			ТекстСообщения = НСтр("ru='Выберите сертификат';uk='Виберіть сертифікат'");
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаСертификаты Тогда
		Если ИспользуетсяКриптография И НЕ Сертификаты.Количество() Тогда
			ТекстСообщения = НСтр("ru='Добавьте сертификаты электронной подписи';uk='Додайте сертифікати електронного підпису'");
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаСбербанк Тогда
		...
		#Если ВебКлиент Тогда
			Если Не ВыбранФайлСертификатаСбербанка Тогда
				ТекстСообщения = НСтр("ru='Не выбран сертификат банка';uk='Не вибраний сертифікат банку'");
		...
		#Иначе
			Если ЗначениеЗаполнено(ПутьКФайлуСертификатаСбербанка) Тогда
				...
				Исключение
					ТекстСообщения = НСтр("ru='Неверно указан путь к файлу сертификата банка';uk='Невірно вказаний шлях до файлу сертифікату банку'");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуСертификатаСбербанка");
					Возврат;
				КонецПопытки;
				АдресФайлаСертификатаСбербанка = ПоместитьВоВременноеХранилище(ДвоичныеДанныеСертификатаСбербанка, УникальныйИдентификатор);
			Иначе
				ТекстСообщения = НСтр("ru='Не выбран файл сертификата банка';uk='Не вибраний файл сертифікату банку'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуСертификатаСбербанка");
				Возврат;
			КонецЕсли;
			...
			Иначе
				ТекстСообщения = НСтр("ru='Не выбран файл внешнего модуля банка';uk='Не вибраний файл зовнішнього модуля банку'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуВКСбербанка");
				Возврат;
			КонецЕсли;
		#КонецЕсли
		...
		Если Не Файл.Расширение = ".cer" Тогда
			ТекстСообщения = НСтр("ru='Указан неверный файл."
"Файл сертификата банка должен иметь расширение ""cer""';uk='Вказаний невірний файл."
"Файл сертифікату банку повинен мати розширення ""cer""'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ,"ПутьКФайлуСертификатаСбербанка");
			Возврат;
		КонецЕсли;
		...
		Если Не Файл.Расширение = ".zip" Тогда
			ТекстСообщения = НСтр("ru='Указан неверный файл."
"Файл внешнего модуля должен иметь расширение ""zip""';uk='Вказаний невірний файл."
"Файл зовнішнього модуля повинен мати розширення ""zip""'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ,"ПутьКФайлуВКСбербанка");
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаПолучениеССервера И (СпособАутентификации=0 ИЛИ СпособАутентификации=1) Тогда
		...
		Если НЕ ЗначениеЗаполнено(АдресСервераНастроек) Тогда
			ТекстСообщения = НСтр("ru='Укажите адрес сервера банка';uk='Вкажіть адресу сервера банку'");
		...
		Если СпособАутентификации = 0 И Не ЗначениеЗаполнено(СертификатАутентификации) Тогда  // по сертификату
			ТекстСообщения = НСтр("ru='Выберите сертификат для аутентификации на сервере банка';uk='Виберіть сертифікат для аутентифікації на сервері банку'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "СертификатАутентификации", , Отказ);
		КонецЕсли;
		
		Если СпособАутентификации = 1 И Не ЗначениеЗаполнено(Логин) Тогда
			ТекстСообщения = НСтр("ru='Введите логин для аутентификации на сервере банка';uk='Введіть логін для аутентифікації на сервері банку'");
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаПолучениеССервера И СпособАутентификации=2 Тогда
		...
		#Если ВебКлиент Тогда
			Если Не ВыбранФайлНастроекОбмена Тогда
				ТекстСообщения = НСтр("ru='Не выбран файл настроек обмена';uk='Не вибраний файл налаштувань обміну'");
		...
		#Иначе
			Если ЗначениеЗаполнено(ПутьКФайлуНастроекОбмена) Тогда
				...
				Исключение
					ТекстСообщения = НСтр("ru='Неверно указан путь к файлу настроек обмена';uk='Невірно вказаний шлях до файлу налаштувань обміну'");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуНастроекОбмена");
					Возврат;
				КонецПопытки;
		...
		Если Не Файл.Расширение = ".xml" Тогда
			ТекстСообщения = НСтр("ru='Указан неверный файл."
"Файл настроек должен иметь расширение ""xml""';uk='Вказаний невірний файл."
"Файл налаштувань повинен мати розширення ""xml""'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПутьКФайлуНастроекОбмена");
			Возврат;
		КонецЕсли;
...
Процедура СменитьСтраницуШаги(НоваяСтраница)
	...
	ИначеЕсли НоваяСтраница = Элементы.СтраницаПолучениеССервера Тогда
		...
		Если НЕ ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.СбербанкОнлайн") Тогда
			Элементы.СпособАутентификации.СписокВыбора.Добавить(1, НСтр("ru='Логин и пароль';uk='Логін і пароль'"));
			Элементы.СпособАутентификации.СписокВыбора.Добавить(0, НСтр("ru='Сертификат';uk='Сертифікат'"));
		КонецЕсли; 
		Элементы.СпособАутентификации.СписокВыбора.Добавить(2, НСтр("ru='Настройки из файла';uk='Налаштування з файлу'"));
...
"Убедитесь, что сертификаты, указанные в настройке обмена, установлены в личное хранилище сертификатов операционной системы';uk='Не знайдений відповідний сертифікат підпису."
"Переконайтеся, що сертифікати, вказані в налаштуванні обміну, встановлені в особисте сховище сертифікатів операційної системи'");
...
Функция ЗагрузитьНастройкуОбменаИзФайлаНаСервере(АдресФайлаНастроекОбмена)
	...
	Попытка
		...
		НазваниеЗадания = НСтр("ru='Прямой обмен с банком: Загрузка настроек обмена из файла';uk='Прямий обмін з банком: Завантаження налаштувань обміну з файлу'");
	...
	Исключение
		Задание.Завершено = Истина;
		Задание.Успешно = Ложь;
		Текст = НСтр("ru='Не удалось загрузить настройки обмена.';uk='Не вдалося завантажити налаштування обміну.'");
		ВидОперации = НСтр("ru='Загрузка настроек обмена из файла';uk='Завантаження налаштувань обміну з файлу'");
...
Процедура ПолучитьИдентификаторСессииПослеПолученияИнформацииОМенеджереКриптографии(ИнформацияМодуля, ДополнительныеПараметры) Экспорт
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция", НСтр("ru='Аутентификация на сервере банка';uk='Аутентификация на сервере банка'"));
	ОписаниеДанных.Вставить("ЗаголовокДанных", НСтр("ru='Аутентификация на сервере банка';uk='Аутентификация на сервере банка'"));
...
Функция ЗагрузитьНастройкиОбменаНаСервере(Организация, Банк, ИдентификаторСессии, АдресФайлаНастроекОбмена)
	...
	Попытка
		НазваниеЗадания = НСтр("ru='Прямой обмен с банком: Получение настроек обмена с сервера банка';uk='Прямий обмін з банком: Отримання налаштувань обміну з сервера банку'");
	...
	Исключение
		...
		Текст = НСтр("ru='Не удалось загрузить настройки обмена с сервера банка.';uk='Не вдалося завантажити налаштування обміну з сервера банку.'");
		ВидОперации = НСтр("ru='Получение настроек обмена с сервера банка';uk='Отримання налаштувань обміну з сервера банку'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробныйТекстОшибки, Текст, 1);
	КонецПопытки;
...
Процедура ВыбратьСертификатПослеПолученияСпискаСТокенаСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Элемент Из СоответствиеСертификатов Цикл
		...
		Если ЗначениеЗаполнено(СтруктураСертификата) Тогда
			ШаблонПредставления = Нстр("ru='%1, до %2';uk='%1, до %2'");
	...
	ЗаголовокФормыВыбора = НСтр("ru='Выберите добавляемый сертификат подписи';uk='Виберіть сертифікат підпису, що додається'");
...
"Код ошибки: %1"
"%2';uk='Помилка установки підпису."
"Код помилки : %1"
"%2'");
...
"Код ошибки: %1"
"%2';uk='Помилка перевірки підпису."
"Код помилки: %1"
"%2'");
		ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
"Код ошибки: %1"
"%2';uk='Помилка відправки тестового запиту."
"Код помилки: %1"
"%2'");
		ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
"Код ошибки: %1"
"%2';uk='Помилка отримання банківських сертифікатів."
"Код помилки: %1"
"%2'");
		ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
"Код ошибки: %1"
"%2';uk='При завантаженні сертифікату сталася помилка."
"Код помилки: %1"
"%2'");
		ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
"Код ошибки: %1"
"%2';uk='Помилка установки з`єднання."
"Код помилки: %1"
"%2'");
		ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
"%2'");
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								ШаблонОшибки, ДеталиОшибки.Код, ДеталиОшибки.Сообщение);
			Операция = НСтр("ru='Определение необходимости расширенной аутентификации';uk='Визначення необхідності розширеної аутентифікації'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ПодробноеПредставлениеОшибки, , 1);
			ОбменСБанкамиСлужебныйКлиент.СообщитьПользователю(ТекстСообщения, УникальныйИдентификатор);
			ВыполнитьОбработкуОповещения(Оповещение, Ложь);
		...
		Если ТребуетсяРасширеннаяАутентификация Тогда
			Если ПараметрыРасширеннойАутентификации.Способы.Количество() = 0 Тогда
				ТекстСообщения = НСтр("ru='Ошибка установки соединения с банком.';uk='Помилка установки з`єднання з банком.'");
				ТекстОшибки = НСтр("ru='Не определены способы расширенной аутентификации.';uk='Не визначені способи розширеної аутентифікації.'");
				Операция = НСтр("ru='Определение необходимости расширенной аутентификации';uk='Визначення необхідності розширеної аутентифікації'");
			...
			Если НЕ ПараметрыРасширеннойАутентификации.Способы.Свойство("SMS") Тогда
				ТекстСообщения = НСтр("ru='Ошибка установки соединения с банком.';uk='Помилка установки з`єднання з банком.'");
				ТекстОшибки =НСтр("ru='Расширенная аутентификация по SMS не поддерживается.';uk='Розширена аутентифікація по SMS не підтримується.'");
				Операция = НСтр("ru='Определение необходимости расширенной аутентификации';uk='Визначення необхідності розширеної аутентифікації'");
				ОбменСБанкамиСлужебныйКлиент.СообщитьПользователю(ТекстСообщения, УникальныйИдентификатор);
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, , 1);
				ВыполнитьОбработкуОповещения(Оповещение, Ложь);
			...
			Иначе
				ТекстСообщения = НСтр("ru='Ошибка установки соединения';uk='Помилка установки з`єднання'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
				ВыполнитьОбработкуОповещения(Оповещение, Ложь);
			КонецЕсли;
	КонецЕсли;
...
"Код ошибки: %1"
"%2';uk='Помилка запуску аутентифікації по SMS."
"Код помилки: %1"
"%2'");
		ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонОшибки, ДеталиОшибки.Код, ДеталиОшибки.Сообщение);
		Операция = НСтр("ru='Запрос одноразового пароля по SMS';uk='Запит одноразового пароля по SMS'");
		ОбменСБанкамиСлужебныйКлиент.СообщитьПользователю(ТекстСообщения, УникальныйИдентификатор);
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ПодробноеПредставлениеОшибки, , 1);
		ВыполнитьОбработкуОповещения(Оповещение, Ложь);
	КонецПопытки;
...
Процедура ПослеВводаОдноразовогоПароляЧерезДополнительнуюОбработку(ОдноразовыйПароль, ДополнительныеПараметры) Экспорт
	...
	Исключение
		ВидОперации = НСтр("ru='Отправка одноразового пароля из SMS в банк';uk='Відправка одноразового пароля з SMS у банк'");
		ТекстСообщения = НСтр("ru='Ошибка аутентификации.';uk='Помилка аутентифікації.'");
...
Функция ПодключитьДополнительнуюОбработку()
	...
	#Иначе
		...
		Исключение
			Операция = НСтр("ru='Получение формы внешней обработки.';uk='Отримання форми зовнішньої обробки.'");
...
"Модуль какой версии нужно теперь использовать?';uk='В інформаційній базі вже є модуль версії %1."
"Обраний модуль має іншу версію %2."
"Модуль якої версії потрібно тепер використовувати? '");
		...
		ЗаголовокВопроса = НСтр("ru='Выбор модуля для использования';uk='Вибір модуля для використання'");
...
"Код ошибки: %1"
"%2';uk='Помилка ініціалізації зовнішнього модуля."
"Код помилки: %1"
"%2'");
			ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
"Код ошибки: %1"
"%2';uk='Помилка завершення ініціалізації зовнішнього модуля."
"Код помилки: %1"
"%2'");
		ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ПослеОшибкиУстановкиВнешнейКомпонентыДополнительногоМодуля(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ВидОперации = НСтр("ru='Установка внешней компоненты дополнительной обработки';uk='Установка зовнішньої компоненти додаткової обробки'");
	ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
	ТекстСообщения = НСтр("ru='При подключении внешнего модуля произошла ошибка.';uk='При підключенні зовнішнього модуля сталася помилка.'");
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеИнициализацииДополнительнойОбработки, Ложь);
...
Процедура ТестНастройкиОбменаЧерезВнешнююКомпоненту(Обработчик)
	...
	Исключение
		ВидОперации = НСтр("ru='Отправка текстового запроса';uk='Відправка текстового запиту'");
		ТекстСообщения = НСтр("ru='Ошибка при отправке тестового сообщения в банк.';uk='Помилка при відправці тестового повідомлення у банк.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
...
Функция ЗагрузитьКомпонентуНаСервере(Знач УникальныйИдентификатор, Знач АдресСервера)
	...
	Попытка
		НазваниеЗадания = НСтр("ru='Прямой обмен с банком: Получение внешней компоненты';uk='Прямий обмін з банком: Отримання зовнішньої компоненти'");
		РезультатФоновогоЗадания = ДлительныеОперации.ЗапуститьВыполнениеВФоне(УникальныйИдентификатор,
	...
	Исключение
		...
		Задание.Успешно = Ложь;
		Задание.Значение = Новый Структура;
		Текст = НСтр("ru='Не удалось загрузить внешний модуль с сервера банка.';uk='Не вдалося завантажити зовнішній модуль з сервера банку.'");
		ВидОперации = НСтр("ru='Получение внешнего модуля с сервера банка';uk='Отримання зовнішнього модуля з сервера банку'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробныйТекстОшибки, Текст, 1);
	КонецПопытки;
...
"Убедитесь что ключ подключен к компьютеру и повторите попытку';uk='Не виявлені підключені електронні ключі."
"Переконайтеся що ключ підключений до комп`ютера і спробуйте ще раз'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		...
		КонецЕсли;
	Исключение
		ВидОперации = НСтр("ru='Получение списка устройств ключей';uk='Отримання списку обладнань ключів'");
		ТекстСообщения = НСтр("ru='Ошибка при получении списка устройств ключей.';uk='Помилка при отриманні списку обладнань ключів.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(ОписаниеОповещения, Неопределено);
	КонецПопытки;
...
Процедура ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения)
	...
	Исключение
		ВидОперации = НСтр("ru='Получение информации об ошибке';uk='Отримання інформації про помилку'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = НСтр("ru='Ошибка при работе с внешним модулем банка.';uk='Помилка при роботі із зовнішнім модулем банку.'");
		ОбменСБанкамиСлужебныйКлиент.СообщитьПользователю(ТекстСообщения, УникальныйИдентификатор);
...
Процедура УстановитьPINКлюча(Обработчик, ИдентификаторХранилища, Пин)
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Ошибка при установке PIN-кода.';uk='Помилка при установці PIN -кода.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Ложь);
	КонецПопытки;
...
Процедура ПолучитьСертификатыХранилища(Обработчик, ИдентификаторХранилища)
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ВидОперации = НСтр("ru='Получение сертификатов с банковского ключа';uk='Отримання сертифікатів з банківського ключа'");
		ТекстСообщения = НСтр("ru='Ошибка при получении сертификатов банковского ключа.';uk='Помилка при отриманні сертифікатів банківського ключа.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Неопределено);
	КонецПопытки;
...
Процедура АутентифицироватьсяНаЭлектронномКлюче(Оповещение, СертификатXML, Пароль)
	...
	Попытка
		...
		ВыполнитьОбработкуОповещения(Оповещение, Истина);
	Исключение
		ВидОперации = НСтр("ru='Аутентификация';uk='Аутентифікація'");
		ТекстСообщения = НСтр("ru='Ошибка аутентификации.';uk='Помилка аутентифікації.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Оповещение, Ложь);
	КонецПопытки;
...
Процедура УстановитьНастройкиПрокси(Обработчик)
	...
	Если ЗначениеЗаполнено(НастройкиПроксиСервера) Тогда
		...
		Исключение
			ВидОперации = НСтр("ru='Установка настроек прокси-сервера';uk='Установка налаштувань проксі-сервера'");
			ТекстСообщения = НСтр("ru='Ошибка установки соединения.';uk='Помилка установки з`єднання.'");
			ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
			ВыполнитьОбработкуОповещения(Обработчик, Ложь);
			Возврат;
...
Процедура ДополнитьСертификат(Обработчик, СертификатXML)
	...
	Исключение
		ВидОперации = НСтр("ru='Дополнение сертификата';uk='Доповнення сертифіката'");
		ТекстСообщения = НСтр("ru='Ошибка загрузки сертификата.';uk='Помилка завантаження сертифікату.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Неопределено);
	КонецПопытки
...
Процедура УстановитьСоединениеЧерезВнешнююКомпоненту(Обработчик, Идентификатор, СертификатXML)
	...
	Исключение
		ВидОперации = НСтр("ru='Установка соединения с сервером банка';uk='Установка з`єднання з сервером банку'");
		ТекстСообщения = НСтр("ru='Ошибка установки соединения.';uk='Помилка установки з`єднання.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Ложь);
		Возврат;
...
Процедура ПроверитьНеобходимостьРасширеннойАутентификации(Обработчик)
	...
	Попытка
		...
		ПараметрыРасширеннойАутентификации = ОбменСБанкамиСлужебныйКлиент.ДеСериализованныеДанные(
			ПараметрыРасширеннойАутентификацииXML);
	Исключение
		ВидОперации = НСтр("ru='Проверка необходимости расширенной аутентификации';uk='Перевірка необхідності розширеної аутентифікації'");
		ТекстСообщения = НСтр("ru='Ошибка установки соединения.';uk='Помилка установки з`єднання.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Неопределено);
		Возврат;
...
Процедура ПослеПроверкиРасширеннойАутентификации(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.ТребуетсяРасширеннаяАутентификация Тогда
		Если Результат.ПараметрыРасширеннойАутентификации.Способы.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не определены способы расширенной аутентификации.';uk='Не визначені способи розширеної аутентифікації.'"));
			ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработчикПослеУстановкиСоединения, Ложь);
			Возврат;
		КонецЕсли;
		Если НЕ Результат.ПараметрыРасширеннойАутентификации.Способы.Свойство("SMS") Тогда
			ТекстСообщения = НСтр("ru='Расширенная аутентификация по SMS не поддерживается.';uk='Розширена аутентифікація по SMS не підтримується.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработчикПослеУстановкиСоединения, Ложь);
			Возврат;
		КонецЕсли;
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка установки соединения';uk='Помилка установки з`єднання'"));
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработчикПослеУстановкиСоединения, Ложь);
	КонецЕсли
...
Процедура ЗапроситьПарольПоSMSЧерезВнешнююКомпоненту(Оповещение, Сессия)
	...
	Попытка
		...
		ВыполнитьОбработкуОповещения(Оповещение, Истина);
	Исключение
		ВидОперации = НСтр("ru='Запрос одноразового пароля по SMS';uk='Запит одноразового пароля по SMS'");
		ТекстСообщения = НСтр("ru='Ошибка установки соединения.';uk='Помилка установки з`єднання.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Оповещение, Ложь);
	КонецПопытки;
...
Процедура ПолучитьДанныеСертификатаЧерезВнешнююКомпоненту(Обработчик, СертификатXML)
	...
	Исключение
		ВидОперации = НСтр("ru='Получение данных сертификата с банковского ключа';uk='Отримання даних сертифікату з банківського ключа'");
		ТекстСообщения = НСтр("ru='Ошибка при получении данных сертификата банковского ключа.';uk='Помилка при отриманні даних сертифікату банківського ключа.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Неопределено);
	КонецПопытки;
...
Процедура ПроверитьНеобходимостьУстановкиPIN(Обработчик, ИдентификаторХранилища)
	...
	Исключение
		ВидОперации = НСтр("ru='Определение наличия PIN-кода банковского ключа';uk='Визначення наявності PIN -кода банківського ключа'");
		ТекстСообщения = НСтр("ru='Ошибка при работе с банковским ключом.';uk='Помилка при роботі з банківським ключем.'");
		ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Обработчик, Неопределено);
	КонецПопытки
...
Функция ПрограммыКриптографии()
	...
	Настройка.Представление       = НСтр("ru='КриптоПро eToken CSP';uk='КриптоПро eToken CSP'");
...
Процедура СохранитьНовыйСчет()
	...
	НовыйСчет.ВариантУказанияКодПоЕДРПОУ = Перечисления.ВариантыУказанияКодПоЕДРПОУ.ВоВсехПлатежныхПоручениях;
...
Процедура ЗапуститьСозданиеНастройкиОбмена(Знач СтруктураПараметров, Знач АдресХранилища, ИдентификаторЗадания, ЗаданиеВыполнено, ЕстьОшибка)
	...
	НаименованиеЗадания = НСтр("ru='Создание настройки прямого обмена с банком';uk='Створення налаштування прямого обміну з банком'");
