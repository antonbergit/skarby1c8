...
Процедура ВариантыОтчетаПередУдалением(Элемент, Отказ)
	...
	Если Не Контекст.ПолныеПраваНаВарианты И НЕ Вариант.АвторТекущийПользователь Тогда
		ТекстПредупреждения = НСтр("ru='Недостаточно прав для удаления варианта отчета ""%1"".';uk='Недостатньо прав для видалення варіанта звіту ""%1"".'");
	...
	Если Не Вариант.Пользовательский Тогда
		ПоказатьПредупреждение(, НСтр("ru='Невозможно удалить предопределенный вариант отчета.';uk='Неможливо видалити визначений варіант звіту.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВариантОбразецНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыДиалога.Вставить("Заголовок", НСтр("ru='Выберите файл изображения';uk='Виберіть файл зображення'"));
	ПараметрыДиалога.Вставить("ПредварительныйПросмотр", Истина);
	ПараметрыДиалога.Вставить("Фильтр", НСтр("ru='Все картинки (*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf)|*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf';uk='Всі картинки (*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf)|*.bmp;*.gif;*.png;*.jpeg;*.dib;*.rle;*.tif;*.jpg;*.ico;*.wmf;*.emf'"));
...
"внешнего отчета ""%1"":';uk='Не вдалося одержати список визначених варіантів"
"зовнішнього звіту ""%1"":'"),
	...
	Если Пакет.ПроверитьСтраницу1 = Истина Тогда
		...
		Если Не Пакет.ВариантЭтоНовый Тогда
			...
			Если НЕ ПравоЗаписиВарианта(Вариант, ПравоНастройкиВарианта(Вариант, Контекст.ПолныеПраваНаВарианты), Контекст.ЭтоПолныеПрава, Контекст.РежимНастройки) Тогда
				ТекстОшибки = НСтр("ru='Недостаточно прав для изменения варианта ""%1"". Необходимо выбрать другой вариант или изменить Наименование.';uk='Недостатньо прав для зміни варіанта ""%1"". Необхідно вибрати інший варіант або змінити Найменування.'");
...
"Заменить помеченный на удаление вариант отчета?';uk='Варіант звіту ""%1"" позначений на видалення."
"Замінити позначений на видалення варіант звіту?'");
...
Процедура СохранитьИЗакрыть()
	...
	Если НЕ Элементы.ДобавитьПривязку.Видимость Тогда
		Состояние(НСтр("ru='Отмените или завершите редактирование описания контекстного открытия отчета';uk='Скасуйте або завершите редагування опису контекстного відкриття звіту'"));
		Возврат;
	КонецЕсли; 
...
Процедура ПроверитьИЗаписатьНаСервере(Пакет)
	...
	Если ВариантЭтоНовый И ВариантыОтчетов.НаименованиеЗанято(Контекст.ОтчетСсылка, 
		...
		ТекстОшибки = СтрШаблон(НСтр("ru='""%1"" занято, необходимо указать другое Наименование.';uk='""%1"" зайняте, необхідно вказати інше Найменування.'"), ВариантНаименование);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "ВариантНаименование");
		Пакет.Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Функция ПредставлениеПривязки(Идентификатор, Синоним = "")
	...
	Если Метаданные.Справочники.Содержит(ОбъектМетаданных) Тогда
		Представление = НСтр("ru='Из справочника ';uk='З довідника'")+""""+Синоним+"""";
	ИначеЕсли Метаданные.Документы.Содержит(ОбъектМетаданных) Тогда
		Представление = НСтр("ru='Из документа ';uk='З документа'")+""""+Синоним+"""";
	Иначе
		Представление = НСтр("ru='Из ';uk='З '")+""""+Синоним+"""";
	КонецЕсли; 
