...
Процедура ВыполнитьВыгрузкуДанных(ОбработкаДляЗагрузкиДанных = Неопределено) Экспорт
	...
	Если ЭтоОбменЧерезВнешнееСоединение() Тогда
		...
		Если ОбработкаДляЗагрузкиДанных().ФлагОшибки() Тогда
			...
			СтрокаСообщения = НСтр("ru='Ошибка в базе-корреспонденте: %1';uk='Помилка у базі-кореспонденті: %1'");
...
Функция ПолучитьИнформациюОПравилах(ЭтоПравилаКорреспондента = Ложь) Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаИнформации,
							...
							Формат(Конвертация.ДатаВремяСоздания, "Л="+Локализация.КодЯзыкаИнтерфейса()+";ДЛФ =ДД"));
КонецФункции
...
Процедура ВыполнитьПереходНаНовыйОбмен()
	...
	ВызватьИсключение НСтр("ru='Синхронизация данных по старой настройке отменена.';uk='Синхронізація даних по старому налаштуванню скасована.'");
КонецПроцедуры
		...
		Иначе
			...
			Для Каждого Реквизит Из СтрокаТаблицы.СтруктураРеквизитовРегистрации Цикл
				...
				Если Не РеквизитНайденВТабличнойЧастиРеквизитовРегистрацииОбъектов(МетаТаблица, Реквизит.Ключ) Тогда
					ЗаписатьВПротоколВыполнения(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Реквизит ""%3"" не существует в табличной части (стандартной табличной части, движениях) ""%1"" объекта ""%2"".';uk='Реквізит ""%3"" не існує в табличній частині (стандартної табличної частини, рухах) ""%1"" об`єкта ""%2"".'"),
...
"Не можна позначати на видалення напередвизначений елемент. Позначка на видалення для об`єкта не встановлена.'"));
	//
...
Процедура ПрочитатьДанныеПоОбмену(ЧтениеСообщения, АнализДанных)
	...
	Если РежимЗагрузкиДанныхВИнформационнуюБазу() Тогда
		...
		Если УдалитьРегистрациюИзменений Тогда // Удаляем регистрацию изменений.
			...
			Если ТранзакцияАктивна() Тогда
				ВызватьИсключение НСтр("ru='Удаление регистрации изменений данных не может быть выполнено в активной транзакции.';uk='Видалення реєстрації змін даних не може бути виконане в активній транзакції.'");
			КонецЕсли;
