...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_ОтключеноОбновлениеЧерезИнтернет", НСтр("ru='(отключено обновление через интернет)';uk='(отключено обновление через интернет)'"));
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_СоздаютсяНапрямуюВСправочнике", НСтр("ru='Создаются напрямую в справочнике';uk='Создаются напрямую в справочнике'"));
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_ЗагружаютсяИзФайла", НСтр("ru='Загружаются из файла';uk='Загружаются из файла'"));
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_ЗагружаютсяЧерезИнтернетПоПротоколу", НСтр("ru='Загружаются через интернет по протоколу';uk='Загружаются через интернет по протоколу'"));

	ЭтотОбъект.СписокОбновляемыхДанных.Параметры.УстановитьЗначениеПараметра("Надпись_СписокЗначенийКлассификатора", НСтр("ru='Список значений классификатора';uk='Список значений классификатора'"));
	ЭтотОбъект.СписокОбновляемыхДанных.Параметры.УстановитьЗначениеПараметра("Надпись_ТребуетсяОбновление", НСтр("ru='Требуется обновление';uk='Потрібне відновлення'"));
	ЭтотОбъект.СписокОбновляемыхДанных.Параметры.УстановитьЗначениеПараметра("Надпись_ДанныеАктуальны", НСтр("ru='Данные актуальны';uk='Данные актуальны'"));
...
"или закройте окно вручную, до выхода из программы.';uk='Данные не сохранены."
"Нажмите кнопку [Записать и закрыть]"
...
Процедура ДекорацияУдалитьНовостиОбработкаНавигационнойСсылки(Элемент, лкНавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ВРег(лкНавигационнаяСсылка) = ВРег("DeleteNow") Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru='Удалить устаревшие новости?';uk='Видалити застарілі новини?'"),
			РежимДиалогаВопрос.ДаНет,
			0,
			КодВозвратаДиалога.Нет,
			НСтр("ru='Удаление устаревших новостей вручную';uk='Видалення застарілих новин вручну'"));
	КонецЕсли;
...
"Это может занять некоторое время...';uk='Відбувається завантаження новин"
...
Процедура КомандаЗагрузитьСтандартныеСписки(Команда)
	...
	ФормаСостоянияОперации = ОткрытьФорму(
		"Обработка.УправлениеНовостями.Форма.ФормаСостоянияОперации",
		Новый Структура("Заголовок, СостояниеОперации", 
			НСтр("ru='Обновление классификаторов с сервера';uk='Оновлення класифікаторів із сервера'"),
			НСтр("ru='Происходит обновление классификаторов с сервера"
"Это может занять некоторое время...';uk='Відбувається оновлення класифікаторів із сервера"
"Це може зайняти якийсь час...'")),
		ЭтотОбъект,
...
""
"Обновление дат актуальности классификаторов';uk='Происходит обновление классификаторов с сервера"
"Это может занять некоторое время..."
""
...
"Это может занять некоторое время..."
""
"Обновление классификаторов';uk='Происходит обновление классификаторов с сервера"
"Это может занять некоторое время..."
""
...
""
"Данные получены."
"Обновление классификаторов на клиенте...';uk='Происходит обновление классификаторов с сервера"
"Это может занять некоторое время..."
""
"Данные получены."
...
""
"Данные получены."
"Обновление лент новостей на клиенте...';uk='Происходит обновление классификаторов с сервера"
"Это может занять некоторое время..."
""
"Данные получены."
...
Процедура КомандаПолучитьОтладочнуюИнформациюОНовости(Команда)
	...
	Если НЕ ПустаяСтрока(ЭтотОбъект.УИННовости) Тогда
		...
		#Если ВебКлиент Тогда
			...
			ТекстовыйДокумент.Показать(НСтр("ru='Отладочная информация о новости';uk='Отладочная информация о новости'"));
		...
		#Иначе
			...
			ТекстовыйДокумент = Новый ТекстовыйДокумент;
			ТекстовыйДокумент.УстановитьТекст(ИнформацияОНовости);
			ТекстовыйДокумент.Показать(НСтр("ru='Отладочная информация о новости';uk='Отладочная информация о новости'"), ИмяВременногоФайла);
...
Процедура КомандаДиагностикаПолученияНовостей(Команда)
	...
	СписокШагов.Добавить(1, "Настройки программы: значения констант"); // Идентификатор.
	СписокШагов.Добавить(2, "Настройки программы: значения предопределенных категорий"); // Идентификатор.
	СписокШагов.Добавить(3, "Настройки программы: заполненность объектов метаданных"); // Идентификатор.
	СписокШагов.Добавить(4, "Настройки программы: фильтры"); // Идентификатор.
	СписокШагов.Добавить(5, "Настройки программы: правильность внедрения подсистемы"); // Идентификатор.
	СписокШагов.Добавить(6, "Настройки программы: параметры сеанса"); // Идентификатор.
	СписокШагов.Добавить(7, "Настройки программы: данные о конфигурации из метаданных"); // Идентификатор.
	СписокШагов.Добавить(8, "Настройки программы: список важных и очень важных новостей"); // Идентификатор.
	СписокШагов.Добавить(9, "Проверка подключения: доступ к HTTP соединению"); // Идентификатор.
	СписокШагов.Добавить(10, "Проверка окружения: размер свободного места во временном каталоге"); // Идентификатор.
...
"'"),
	...
	Для Каждого ТекущийШаг Из СписокШагов Цикл
		Состояние(
			НСтр("ru='Выполнение проверки';uk='Выполнение проверки'"),
...
"'"),
	...
	#Если ВебКлиент Тогда
		...
		ТекстовыйДокумент.Показать(НСтр("ru='Результат диагностики получения новостей';uk='Результат диагностики получения новостей'"));
	...
	#Иначе
		...
		ТекстовыйДокумент = Новый ТекстовыйДокумент;
		ТекстовыйДокумент.УстановитьТекст(РезультатДиагностикиПолученияНовостей);
		ТекстовыйДокумент.Показать(НСтр("ru='Результат диагностики получения новостей';uk='Результат диагностики получения новостей'"), ИмяВременногоФайла);
...
Процедура УстановитьУсловноеОформление()
	...
	// 5. Частота обновления = 4 (Каждую минуту), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Каждую минуту';uk='Щохвилини'"));
	...
	// 7. Дата последнего обновления = пустая, то вывести текст "никогда не обновлялся".
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='никогда не обновлялся';uk='ніколи не оновлювался'"));
...
"%РезультатВыполнения%';uk='Результат виконання:"
"%РезультатВыполнения%'");
...
"';uk='Подсистема Новости внедрена правильно"
"'");
		КонецЕсли;
...
"';uk='Категории новостей (первые 20 значений):"
"'");
...
"';uk='Значения категорий новостей (первые 20 значений):"
"'");
...
"';uk='Ленты новостей (первые 20 значений):"
"'");
...
"';uk='Отборы, настроенные администратором (первые 40 значений):"
"'")
		+ Список
		+ Символы.ПС;
...
"';uk='Отборы, настроенные пользователями (первые 40 значений):"
"'")
		+ Список
		+ Символы.ПС;
...
Функция СформироватьПолныйАдресЛентыНовостей(лкСсылка, лкПротокол, лкСайт, лкИмяФайла)
	...
	Если Найти(ВРег(лкПротокол), ВРег("http")) = 1 Тогда // http, https
		Если ПустаяСтрока(лкСайт) Тогда
			Результат = НСтр("ru='Не заполнено поле Сайт';uk='Не заповнене поле Сайт'");
		ИначеЕсли ПустаяСтрока(лкИмяФайла) Тогда
			Результат = НСтр("ru='Не заполнено поле Имя файла';uk='Не заповнене поле Ім`я файлу'");
		...
		КонецЕсли;
	Иначе
		Результат = НСтр("ru='Адрес ленты новостей можно получить только для протоколов http и https';uk='Адреса стрічки новин можна одержати тільки для протоколів http і https'");
	КонецЕсли;
...
"';uk='Список очень важных новостей:"
"'")
		+ Список;
...
"';uk='Получение новостей по первым нескольким лентам новостей:"
"'") ;
...
""
""
"';uk='%1Успешное получение файла новостей."
"%1Код HTTP ответа: %2"
"%1Заголовки HTTP ответа:"
"%3"
"%1Имя временного файла: %4"
...
"%3"
"%1Имя временного файла: %4"
"';uk='%1Неудачная попытка получения файла новостей."
"%1Код HTTP ответа: %2"
"%1Заголовки HTTP ответа:"
"%3"
"%1Имя временного файла: %4"
...
"%1"
"';uk='ОШИБКА при получении новостей:"
"%1"
"'"),
...
"';uk='Нет подходящих лент новостей"
"'");
	КонецЕсли;
...
" на любом из серверов, поэтому проверять свободное место необходимо на всех серверах.';uk='Автоматическая проверка НЕ реализована."
"необходимо вручную проверить, достаточно ли места для хранения файлов по следующему пути:"
"%1"
"Также необходимо помнить, что в случае использования кластера серверов, получение новостей может осуществляться"
