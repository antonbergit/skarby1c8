...
"%1';uk='Завантаження пріоритетних даних не виконане."
"При завантаженні визначених елементів знайдені не унікальні записи."
"По наступних причинах продовження неможливе."
"%1'"),
					ОписаниеОтказа);
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(ДублиПредопределенных) Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Предопределенные элементы.Найдены не уникальные записи.';uk='Наперед визначені елементи.Знайдені не унікальні записи.'",
...
"%1';uk='При завантаженні передобумовлених елементів знайдені не унікальні записи."
"%1'"),
						ДублиПредопределенных));
			КонецЕсли;
...
Процедура ДобавитьОписаниеДублейПредопределенного(ЗаписанныйОбъект, ДублиПредопределенных, Отказ, ОписаниеОтказа)
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонОшибкиНеУникальностиЗаписей(),
				НСтр("ru='При загрузке предопределенных элементов найдены не уникальные записи.';uk='При завантаженні передобумовлених елементів знайдені не унікальні записи.'"));
		КонецЕсли;
...
"   - нажмите Выполнить."
"3. После этого запустите 1С:Предприятие и выполните повторную синхронизацию данных.';uk='Завантаження пріоритетних даних не виконане."
"%1"
"Потрібне виправлення інформаційної бази."
"1. Відкрийте конфігуратор, перейдіть у меню Адміністрування,"
"   виберіть пункт ""Тестування й виправлення ...""."
"2. У формі, що відкрилась"
"   - включіть тільки пункт ""Перевірка логічної цілісності інформаційної бази"";"
"   - виберіть варіант ""Тестування й виправлення"", а не ""Тільки тестування"""
"   - натисніть Виконати."
"3. Після цього запустіть 1С:Підприємство й виконайте повторну синхронізацію даних.'");
КонецФункции
