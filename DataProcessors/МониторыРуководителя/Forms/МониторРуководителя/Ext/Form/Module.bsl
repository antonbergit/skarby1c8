...
Процедура ОбновитьДанные()
	...
	ПредставлениеПериода = Формат(ДобавитьМесяц(Период, -1), "ДЛФ=DD") + " — " + Формат(Период, "ДЛФ=DD") + ?(НачалоДня(Период) = НачалоДня(ТекущаяДатаСеанса()), НСтр("ru=' (Сегодня)';uk=' (Сьогодні)'"), "");
...
Процедура ОбновитьВиджетДенежныеСредства()
	...
	Пока Выборка.Следующий() Цикл
		...
		Подсказка = НСтр("ru='Остаток ';uk='Залишок '") + Выборка.СуммаКонечныйОстаток + " на " + Формат(Выборка.Период, "ДФ=dd.MM.yyyy");
...
Процедура ОбновитьВиджетЗаказы()
	...
	Запрос.Текст = 
		...
		|	И НЕ ДокЗаказПокупателя.СостояниеЗаказа = ЗНАЧЕНИЕ(Справочник.СостоянияЗаказНарядов.Завершен)
		|	И ВЫБОР
