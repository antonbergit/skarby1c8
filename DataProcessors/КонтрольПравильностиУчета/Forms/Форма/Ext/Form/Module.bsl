...
Процедура ВывестиДанныеПоРазделу()
	...
	Исключение
		МассивОписанийОшибок = Новый Массив;
		МассивОписанийОшибок.Добавить(СтрШаблон("ru = 'Не удалось выполнить процедуру ""%1"".'", ИмяПроцедурыОбработчика));
		МассивОписанийОшибок.Добавить(ОписаниеОшибки());
		ВывестиСтрокуОшибки(МассивОписанийОшибок);
	КонецПопытки;
...
Процедура ЗаполнитьДанныеПоОтчетыПерерБезСпецификаций()
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	|	ОтчетПереработчика.Ссылка КАК ДокументСсылка,
	|	ОтчетПереработчика.Номенклатура.Спецификация КАК Спецификация,
	|	ОтчетПереработчика.Дата,
	|	ЛОЖЬ КАК ОбработкаВыполнена
	|ИЗ
	|	Документ.ОтчетПереработчика КАК ОтчетПереработчика
	|ГДЕ
	|	ОтчетПереработчика.Дата МЕЖДУ &ДатаНач И &ДатаКон
	|	И ОтчетПереработчика.Проведен = ИСТИНА
	|	И ОтчетПереработчика.Номенклатура.Спецификация <> ЗНАЧЕНИЕ(Справочник.Спецификации.ПустаяСсылка)
	|	И ОтчетПереработчика.Спецификация = ЗНАЧЕНИЕ(Справочник.Спецификации.ПустаяСсылка)
	|	И ВЫБОР
	|			КОГДА НЕ &ВестиУчетПоКомпании
	|				ТОГДА ОтчетПереработчика.Организация = &Организация
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ");
...
Процедура ЗаполнитьДанныеПоОтчетыПерерНесоответСписаний()
	...
	|	ОтчетПереработчика.Количество
	|ИЗ
	|	Документ.ОтчетПереработчика КАК ОтчетПереработчика
	|ГДЕ
	|	ОтчетПереработчика.Проведен = ИСТИНА
	|	И ВЫБОР
	|			КОГДА НЕ &ВестиУчетПоКомпании
	|				ТОГДА ОтчетПереработчика.Организация = &Организация
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|	И ОтчетПереработчика.Дата МЕЖДУ &ДатаНач И &ДатаКон
	|	И ОтчетПереработчика.Спецификация <> ЗНАЧЕНИЕ(Справочник.Спецификации.ПустаяСсылка)
	|
	|УПОРЯДОЧИТЬ ПО
	|	ОтчетПереработчика.Дата");
