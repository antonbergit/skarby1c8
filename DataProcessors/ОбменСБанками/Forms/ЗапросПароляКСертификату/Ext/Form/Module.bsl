...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ВидОперации = НСтр("ru='Подписание электронного документа';uk='Підписання електронного документу'")
		ИЛИ Параметры.ВидОперации = НСтр("ru='Подписание электронных документов';uk='Подписание электронных документов'") Тогда
		Элементы.Выбрать.Заголовок = НСтр("ru='Подписать';uk='Підписати'");
	КонецЕсли;
	...
	Если ТипЗнч(Параметры.ОбъектыДляОбработки) = Тип("Массив") И Параметры.ОбъектыДляОбработки.Количество() > 0 Тогда
		Если Параметры.ОбъектыДляОбработки.Количество() = 1 Тогда
			...
			ШаблонГиперссылки = НСтр("ru='%1 № %2 от %3';uk='%1 № %2 от %3'");
		...
		Иначе
			...
			ШаблонГиперссылки = НСтр("ru='Электронные документы (%1)';uk='Электронные документы (%1)'");
...
Процедура КомандаГотово(Команда)
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаВводПароляКСертификату
		...
		ТекстСообщения = ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения(
			"Поле", "Заполнение", НСтр("ru='Сертификат подписи';uk='Сертификат подписи'"));
	...
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаВводЛогинаИПароля И ПустаяСтрока(Пользователь) Тогда
		ТекстСообщения = ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения(
			"Поле", "Заполнение", НСтр("ru='Пользователь';uk='Користувач'"));
