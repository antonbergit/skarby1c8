...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ТипУстройства = "SCR" Тогда // экранный
		Элементы.Пояснение.Заголовок = НСтр("ru='Подтвердите подпись на электронном ключе';uk='Подтвердите подпись на электронном ключе'");
	КонецЕсли;
...
Процедура Подключаемый_ПроверитьУстановкуПодписи()
	...
	Элементы.Отсчет.Заголовок = СтрШаблон(НСтр("ru='%1 сек.';uk='%1 сек.'"), Счетчик);
...
"Код состояния: %1'");
		...
		ТекстСообщения = НСтр("ru='При подписании электронного документа произошла ошибка';uk='При подписании электронного документа произошла ошибка'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, , 1, Параметры.ОбъектПодписи);
		Результат = Новый Структура("Успех, ТекстОшибки", Ложь, ТекстСообщения);
		Закрыть(Результат);
	ИначеЕсли Состояние = "REJECTED" Тогда
		ТекстОповещения = НСтр("ru='Подпись документа не подтверждена';uk='Подпись документа не подтверждена'");
