...
"т.к. процедуры отработки данных могут зависеть друг от друга.';uk='Зупинка додаткових процедур обробки даних"
"може привести до нестабільній роботи або непрацездатності програми."
"Виконувати відключення рекомендується у випадку виявлення помилки"
"в процедурі обробки даних і тільки після консультації зі службою підтримки,"
"тому що процедури відпрацьовування даних можуть залежати друг від друга.'");
...
"%2'");
	...
	Иначе
		...
		ВыполненыВсеОбработчики        = Ложь;
		ПредставлениеСтатусаВыполнения = НСтр("ru='еще не выполнялась';uk='ще не виконувалася'");
...
Функция ДлительностьВыполненияПроцедурыОбновления(ДлительностьВыполнения)
	...
	Если ДлительностьВСекундах < 1 Тогда
		Возврат НСтр("ru='менее секунды';uk='менш секунди'")
