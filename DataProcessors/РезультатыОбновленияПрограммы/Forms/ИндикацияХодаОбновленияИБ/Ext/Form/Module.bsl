...
Процедура ЗавершитьОбновлениеПараметровРаботыПрограммы(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Неопределено Или Результат.Статус = "Отменено" Тогда
		...
		КраткоеСообщениеОбОшибке = НСтр("ru='Ошибка получения результата от фонового задания при обновлении параметров работы программы.';uk='Помилка отримання результату від фонового завдання при оновленні параметрів роботи програми.'");
...
Функция ЗапускЗадания()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Фоновая регистрация данных отложенного обновления';uk='Фонова реєстрація даних відкладеного оновлення'");
