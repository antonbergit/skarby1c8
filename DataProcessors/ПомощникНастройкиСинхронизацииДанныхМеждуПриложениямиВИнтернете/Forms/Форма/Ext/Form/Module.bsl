...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = СтрШаблон(НСтр("ru='Настройка синхронизации данных с ""%1""';uk='Налаштування синхронізації даних з ""%1""'"), НаименованиеКорреспондента);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("ru='Отменить настройку синхронизации данных?';uk='Скасувати налаштування синхронізації даних?'");
...
Функция Подключаемый_Начало_ПриПереходеДалее(Отказ)
	...
	Если ПредусмотреныНастройкиОтборовНаУзле Тогда
		...
		Если Не ФормаНастройки.ПроверитьЗаполнение() Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Необходимо задать обязательные настройки.';uk='Необхідно задати обов`язкові налаштування.'"),,, "ОписаниеНастроекВыгрузкиДанных", Отказ);
...
Процедура ОжиданиеАнализаДанных_ОбработкаДлительнойОперации(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru='Настройка синхронизации данных между приложениями в сети Интернет (шаг 1)';uk='Налаштування синхронізації даних між додатками в мережі Інтернет (крок 1)'"));
...
Процедура ВыполнитьАвтоматическоеСопоставлениеДанных(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru='Настройка синхронизации данных между приложениями в сети Интернет (автоматическое сопоставление данных)';uk='Налаштування синхронізації даних між додатками в мережі Інтернет (автоматичне співставлення даних)'"));
...
Процедура ПолучитьСтатистикуСопоставления(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru='Настройка синхронизации данных между приложениями в сети Интернет (получение статистики сопоставления)';uk='Налаштування синхронізації даних між додатками в мережі Інтернет (одержання статистики зіставлення)'"));
...
Процедура ОжиданиеСинхронизацииДанных_ОбработкаДлительнойОперации(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru='Настройка синхронизации данных между приложениями в сети Интернет (синхронизация справочников)';uk='Налаштування синхронізації даних між додатками в мережі Інтернет (синхронізація довідників)'"));
...
Функция Подключаемый_ПравилаПолученияДанных_ПриПереходеДалее(Отказ)
	...
	Если ПредусмотреныЗначенияПоУмолчаниюНаУзле Тогда
		...
		Если Не ФормаНастройки.ПроверитьЗаполнение() Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Необходимо задать обязательные настройки.';uk='Необхідно задати обов`язкові налаштування.'"),,, "ОписаниеЗначенийПоУмолчанию", Отказ);
	...
	Если ПредусмотреныЗначенияПоУмолчаниюНаУзлеБазыКорреспондента Тогда
		...
		Если Не ФормаНастройки.ПроверитьЗаполнение() Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Необходимо задать обязательные настройки для второго приложения.';uk='Необхідно задати обов`язкові налаштування для другого додатка.'")
...
Процедура ОжиданиеСохраненияНастроек_ОбработкаДлительнойОперации(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru='Настройка синхронизации данных между приложениями в сети Интернет (шаг 2)';uk='Налаштування синхронізації даних між додатками в мережі Інтернет (крок 2)'"));
