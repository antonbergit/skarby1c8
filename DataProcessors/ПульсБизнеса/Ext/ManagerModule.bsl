...
Процедура ПолучитьДанные(Параметры, ВременноеХранилищеРезультата) Экспорт
	...
	Результат.Вставить("Показатели", РассчитатьПоказатели(Параметры.Показатели));
	Результат.Вставить("Диаграммы", РассчитатьДиаграммы(Параметры.Диаграммы));
...
Процедура ВыполнитьЗапросПоказатели(Запрос, Показатели, Результат)
	
	Попытка
		Выборка = Запрос.Выполнить().Выбрать();
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Пульс бизнеса: ошибка расчета показателя';uk='Пульс бізнесу: помилка розрахунків показника'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , ОписаниеОшибки());
		Возврат;
	КонецПопытки;
	
	Пока Выборка.Следующий() Цикл
		Стр = Показатели[Выборка.Индекс];
...
Функция РассчитатьДиаграммы(Диаграммы)
	
	Результат = Новый Соответствие;
	
	Запрос = Новый Запрос;
	ПараметрыЗапросов = Новый Массив; 
	Для каждого Стр Из Диаграммы Цикл
		Если ПустаяСтрока(Стр.Серия) ИЛИ ПустаяСтрока(Стр.Точка) Тогда
			Продолжить;
		КонецЕсли;
		Индекс = Диаграммы.Индекс(Стр);
...
Процедура ВыполнитьЗапросДиаграммы(Запрос, ПараметрыЗапросов, Результат)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Пульс бизнеса: ошибка получения данных диаграммы';uk='Пульс бізнесу: помилка одержання даних діаграми'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , ОписаниеОшибки());
		Возврат;
	КонецПопытки; 
...
Процедура Подключаемый_ПродажиРасчет(Запрос, Стр, Индекс)
	...
	"&Ресурс",
	ТекстРесурса);
	ДобавитьЭлементВЗапросПоказатели(Запрос, ТекстЭлемента, Стр, Индекс);
	
КонецПроцедуры

Процедура Подключаемый_ДеньгиРасчет(Запрос, Стр, Индекс)
...
Процедура Подключаемый_ТоварыРасчет(Запрос, Стр, Индекс)
...
Процедура Подключаемый_СчетаЗаказыРасчет(Запрос, Стр, Индекс)
...
Процедура Подключаемый_ЗакупкиРасчет(Запрос, Стр, Индекс)
...
Процедура Подключаемый_ДолгиРасчет(Запрос, Стр, Индекс)
...
Процедура Подключаемый_ФинАнализРасчет(Запрос, Стр, Индекс)
...
Процедура ДобавитьЭлементВЗапросПоказатели(Запрос, ТекстЭлемента, Стр, Индекс)
...
Процедура ОбработатьЭлементЗапросаПоказатели(Стр, Запрос, Элемент, Индекс, Суффикс = "")
	...
	ИначеЕсли ТипЗнч(ЗначениеПериода)=Тип("СтандартнаяДатаНачала") Тогда
		Если ЗначениеПериода.Вариант=ВариантСтандартнойДатыНачала.ПроизвольнаяДата Тогда
			Запрос.УстановитьПараметр("Период"+Суффикс+Индекс, КонецДня(ЗначениеПериода.Дата));
		Иначе
			Запрос.УстановитьПараметр("Период"+Суффикс+Индекс, ЗначениеПериода.Дата);
		КонецЕсли; 
	ИначеЕсли ТипЗнч(ЗначениеПериода)=Тип("Структура") Тогда
		...
		ИначеЕсли ЗначениеПериода.Вариант="ПредыдущийПлавающийПериод" Тогда
			Период = ПредыдущийПлавающийПериод(Стр.Период);
		...
		ИначеЕсли ЗначениеПериода.Вариант="ЗаПрошлыйГод" Тогда
			Период = АналогичныйПериодПрошлогоГода(Стр.Период);
...
Функция АналогичныйПериодПрошлогоГода(Период)
	...
	Год = Год(Период.ДатаНачала);
	Месяц = Месяц(Период.ДатаНачала);
	День = День(Период.ДатаНачала);
	...
	ИначеЕсли (Год-1)%4=0 И Месяц=2 И День=28 Тогда
		...
		День = 29;
	КонецЕсли; 
	ГодКонца = Год(Период.ДатаОкончания);
	МесяцКонца = Месяц(Период.ДатаОкончания);
	ДеньКонца = День(Период.ДатаОкончания);
	...
	ИначеЕсли Период.Вариант=ВариантСтандартногоПериода.СНачалаЭтойНедели Тогда
		ПродолжительностьСекунд = НачалоДня(Период.ДатаОкончания)-НачалоНедели(Период.ДатаОкончания);
		Неделя = НеделяГода(Период.ДатаНачала);
