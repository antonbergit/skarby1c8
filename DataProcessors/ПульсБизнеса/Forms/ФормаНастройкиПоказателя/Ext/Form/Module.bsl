...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ИдентификаторСтроки") Тогда
		...
		Заголовок = НСтр("ru='Настройка показателя';uk='Налаштування показника'");
	Иначе
		ИдентификаторСтроки = -1;
		Заголовок = НСтр("ru='Добавление показателя';uk='Додавання показника'");
	КонецЕсли; 
...
Процедура УстановитьВидимостьПолейНастроек()
	...
	Элементы.ПериодПредставление.Видимость = ЗначениеЗаполнено(Показатель) И ЗначениеЗаполнено(Ресурс);
	Элементы.ПериодСравненияПредставление.Видимость = ЗначениеЗаполнено(Показатель) И ЗначениеЗаполнено(Ресурс);
...
Процедура ОбновитьПараметрыКомпоновщика()
	...
	// Возможность добавления дополнительных настроек показателей
	
	//НаборДанных = СхемаКомпоновкиДанных.НаборыДанных.Получить(0);
	//Поле = НаборДанных.Поля.Найти("Валюта");
	//Если НЕ Поле=Неопределено И Поле.Роль.Измерение Тогда
	//	ИмяНастройки = "ВРазрезеВалют";
	//	Пропустить = Ложь;
	//	Для каждого НайденныйЭлемент Из МассивНастроек Цикл
	//		Если НайденныйЭлемент.Имя=ИмяНастройки Тогда
	//			Пропустить = Истина;
	//			Прервать;
	//		КонецЕсли; 
	//	КонецЦикла;
	...
	//		СтруктураНастройки.Вставить("Имя", ИмяНастройки);
	//		СтруктураНастройки.Вставить("Заголовок", НСтр("ru='В разрезе валют';uk='У розрізі валют'"));
	//		СтруктураНастройки.Вставить("Значение", Ложь);
...
Процедура ЗаполнитьТаблицуНаложенныхФильтров()
	...
	Для каждого ЭлементОтбор Из Фильтры Цикл
		...
		ИначеЕсли ЭлементОтбор.ВидСравнения=ВидСравненияКомпоновкиДанных.МеньшеИлиРавно 
			...
			Иначе
				Стр.Значение.ЗначениеДо = ?(Стр.Значение.ЗначениеДо=0, ЭлементОтбор.ПравоеЗначение, Мин(ЭлементОтбор.ПравоеЗначение, Стр.Значение.ЗначениеДо));
			КонецЕсли; 
...
Процедура УстановитьСвостваПолейПериодов()
	...
	ИначеЕсли НЕ РежимОстатка И НЕ ТипЗнч(Период)=Тип("СтандартныйПериод") Тогда
	...
	Если РежимОстатка Тогда
		...
		КонецЕсли; 
		Элементы.ПериодПредставление.Заголовок = НСтр("ru='На дату';uk='На дату'");
	...
	Иначе
		...
		КонецЕсли; 
		Элементы.ПериодПредставление.Заголовок = НСтр("ru='Период';uk='Період'");
...
Процедура ВыборПериодаНачало(Элемент)
	...
	Если РежимОстатка Тогда
		Если Элемент=Элементы.ПериодПредставление Тогда
			...
			ДанныеВыбора.Добавить(Новый СтандартнаяДатаНачала(ВариантСтандартнойДатыНачала.ПроизвольнаяДата), НСтр("ru='Произвольная дата';uk='Довільна дата'"));
			СтруктураПараметров.Вставить("Значение", Период);
		ИначеЕсли Элемент=Элементы.ПериодСравненияПредставление Тогда
			// Дата сравнения
			ДанныеВыбора.Добавить(Неопределено, НСтр("ru='Не сравнивать';uk='Не порівнювати'"));
	...
	Иначе
		Если Элемент=Элементы.ПериодПредставление Тогда
			...
			ДанныеВыбора.Добавить(Новый СтандартныйПериод(ВариантСтандартногоПериода.ПроизвольныйПериод), НСтр("ru='Произвольный период';uk='Довільний період'"));
			СтруктураПараметров.Вставить("Значение", Период);
		ИначеЕсли Элемент=Элементы.ПериодСравненияПредставление Тогда
			// Период сравнения
			ДанныеВыбора.Добавить(Неопределено, НСтр("ru='Не сравнивать';uk='Не порівнювати'"));
...
Процедура ВыборПериодаЗавершение(ВыбранноеЗначение, СтруктураПараметров) Экспорт
	...
	Если РежимОстатка Тогда
		Если НЕ ЗначениеЗаполнено(ПериодРасчета) И НЕ ПериодРасчета=Неопределено Тогда
			...
			ПоказатьВводДаты(Оповещение, ТекущееЗначение.Дата, НСтр("ru='Укажите дату';uk='Вкажіть дату'"), ЧастиДаты.Дата); 
