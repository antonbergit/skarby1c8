...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		ТекстОшибки = НСтр("ru='Для удаления помеченных необходимо войти в область данных.';uk='Для видалення позначених необхідно увійти в область даних.'");
		Возврат; // Отказ устанавливается в ПриОткрытии.
	КонецЕсли;
...
"Прервать?';uk='Видалення помічених ще виконується."
"Перервати?'");
...
Процедура ПометитьВыделенныеОбъектыТаблицыНаУдаление(ТаблицаЭлемент)
	...
	Если КоличествоМожноУдалить = 0 Тогда
		...
		Если КоличествоВыделено = 1 Тогда
			Если ЕстьЗаписиРегистров Тогда
				ТекстОшибки = НСтр("ru='Удаление записи регистра выполняется из ее карточки.';uk='Видалення запису регістра виконується з її картки.'");
		...
		Иначе
			Если ЕстьЗаписиРегистров Или ЕстьКонстанты Тогда
				ТекстОшибки = НСтр("ru='Удаление записей регистров или очистка значений констант выполняется из их карточек.';uk='Видалення записів регістрів або очищення значень констант виконується з їхніх карток.'");
			КонецЕсли;
		КонецЕсли;
		ПоказатьПредупреждение(, ТекстОшибки);
		Возврат;
	...
	Если КоличествоМожноУдалить = 1 Тогда
		Если УстанавливатьПометку Тогда
			ТекстВопроса = НСтр("ru='Пометить ""%1"" на удаление?';uk='Позначити ""%1"" на видалення?'");
		Иначе
			ТекстВопроса = НСтр("ru='Снять с ""%1"" пометку на удаление?';uk='Зняти з ""%1"" позначку на видалення?'");
		КонецЕсли;
	...
	Иначе
		...
		Иначе
			ТекстВопроса = НСтр("ru='Снять с выделенных объектов (%1) пометку на удаление?';uk='Зняти з виділених об`єктів (%1) позначку на видалення?'");
		КонецЕсли;
...
Процедура ПометитьВыделенныеОбъектыТаблицыНаУдалениеЗавершение(Ответ, ПараметрыВыполнения) Экспорт
	...
	Иначе
		Если ПараметрыВыполнения.УстанавливатьПометку Тогда
			ОповещениеЗаголовок = НСтр("ru='Пометка удаления установлена';uk='Позначка видалення встановлена'");
		Иначе
			ОповещениеЗаголовок = НСтр("ru='Пометка удаления снята';uk='Позначка видалення знята'");
		КонецЕсли;
...
Процедура ВидимостьДоступность(Форма)
	...
	Элементы.КнопкаДалее.Заголовок = НСтр("ru='Удалить';uk='Видалити'");
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаПричиныНевозможностиУдаления Тогда
		Элементы.КнопкаНазад.Видимость = Истина;
		Элементы.КнопкаДалее.Видимость = Истина;
		Элементы.КнопкаДалее.Заголовок = НСтр("ru='Повторить удаление';uk='Повторити видалення'");
		Элементы.КнопкаДалее.КнопкаПоУмолчанию = Истина;
	...
	Если ОбнаруженыРегистры И Не ОбнаруженыСсылочныеНеОтмеченные Тогда
		Если Не ПодписьДобавлена Тогда
			Элементы.НадписьВПодвале.Заголовок = ЗаголовокПодвала + " "
				+ НСтр("ru='Для устранения сложных зависимостей рекомендуется повторить удаление с включенным флажком ""Заблокировать всю работу в программе и ускорить удаление"" (нажмите кнопку ""< В начало"").';uk='Для усунення складних залежностей рекомендується повторити видалення з включеним прапорцем ""Заблокувати всю роботу в програмі і прискорити видалення"" (натисніть кнопку ""< На початок"").'");
		КонецЕсли;
	Иначе
...
Процедура ЗапуститьФоновоеЗаданиеКлиент(Режим)
	...
	Текст = НСтр("ru='Удаляются объекты, помеченные на удаление...';uk='Видаляються об`єкти, позначені на видалення...'");
	Если ПараметрыМетода.ИскатьПомеченные И ПараметрыМетода.УдалятьПомеченные Тогда
		Состояние(НСтр("ru='Поиск и удаление помеченных объектов...';uk='Пошук і видалення позначених об`єктів...'"));
	ИначеЕсли ПараметрыМетода.ИскатьПомеченные Тогда
		Текст = НСтр("ru='Поиск помеченных на удаление объектов...';uk='Пошук позначених на видалення об`єктів...'");
		Состояние(Текст);
	Иначе
		Состояние(НСтр("ru='Удаление выбранных объектов...';uk='Видалення обраних об`єктів...'"));
	КонецЕсли;
	...
	ИначеЕсли Задание.ОшибкаПриУстановкеМонопольногоРежима Тогда
		...
		Иначе
			...
			ПараметрыПредупреждения.Текст    = НСтр("ru='Не удалось установить монопольный режим';uk='Не вдалося встановити монопольний режим'");
...
Функция ЗапуститьФоновоеЗадание(Знач ПараметрыМетода)
	...
	ИначеЕсли ПараметрыМетода.УдалятьПомеченные Тогда
		Элементы.НадписьДлительнаяОперация.Заголовок = НСтр("ru='Удаляются объекты, помеченные на удаление...';uk='Видаляються об`єкти, позначені на видалення...'");
	...
	Иначе
		Элементы.НадписьДлительнаяОперация.Заголовок = НСтр("ru='Поиск объектов, помеченных на удаление...';uk='Пошук об`єктів, позначених на видалення...'");
		Элементы.НадписьДлительнаяОперация.ГоризонтальноеПоложениеВГруппе = ГоризонтальноеПоложениеЭлемента.Центр;
		Элементы.ФоновоеЗаданиеСостояние.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Центр;
	КонецЕсли;
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ru='Удаление помеченных объектов (интерактивное)';uk='Видалення помічених об`єктів (інтерактивне)'");
...
Процедура ПослеЗавершенияФоновогоЗадания(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Выполнено" Тогда
		...
		Если Результат = Неопределено Тогда
			ВывестиОшибку(НСтр("ru='Не удалось прочитать результат удаления помеченных';uk='Не вдалося прочитати результат видалення помічених'"), "");
			Возврат;
		КонецЕсли;
		...
		Если Не ПустаяСтрока(Результат.ОповещениеТекст) Тогда
			ПоказатьОповещениеПользователя(
				НСтр("ru='Удаление помеченных';uk='Видалення помічених'"),
				НавигационнаяСсылка,
	...
	Если РезультатВыполненияВФоне.УдалятьПомеченные Тогда
		...
		ИначеЕсли НеУдаленныеКоличество = 0 Тогда
			...
			Результат.ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Удалено объектов: %1.';uk='Видалено об`єктів: %1.'"),
		...
		Иначе
			...
			Результат.ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Удалено: %1, не удалено: %2.';uk='Видалено: %1, не видалено: %2.'"),
				Формат(УдаленныеКоличество, "ЧН=0; ЧГ="),
			...
			Если УдаленныеКоличество = 0 Тогда
				Элементы.НадписьИтогиЧастичногоУдаления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не получилось удалить объекты, помеченные на удаление (%1):';uk='Не вийшло видалити об`єкти, позначені на видалення (%1):'"),
					Формат(НеУдаленныеКоличество, "ЧН=0; ЧГ="));
			Иначе
				Элементы.НадписьИтогиЧастичногоУдаления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Успешно удалено: %1 из %2, остальные объекты не удалены (%3):';uk='Успішно видалено: %1 з %2, інші об`єкти не видалені (%3):'"),
					Формат(УдаленныеКоличество, "ЧН=0; ЧГ="),
...
Процедура ВывестиОшибку(КраткоеПредставлениеОшибки, ПодробноеПредставлениеОшибки);
	Кратко = НСтр("ru='При удалении помеченных возникла ошибка:';uk='При видаленні помічених виникла помилка:'") + Символы.ПС + КраткоеПредставлениеОшибки;
