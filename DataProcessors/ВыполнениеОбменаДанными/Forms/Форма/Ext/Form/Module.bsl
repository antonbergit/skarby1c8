...
"Повторите попытку синхронизации.';uk='Не вдалося підключитися до додатка в Інтернеті, з причини ""%1""."
"Переконайтеся, що:"
"- введений правильний пароль;"
"- додаток доступний за вказаною в налаштуваннях адресою."
"Спробуйте ще раз синхронізацію.'");
...
Функция ФоновоеЗаданиеЗапуститьНаСервере(ПараметрыЗадания, ОшибкаРасхожденияВерсийПриПолученииДанных, ПроверятьРасхождениеВерсий)
	...
	Иначе
		ШаблонНаименования = НСтр("ru='Выполняется выгрузка данных в %1';uk='Виконується вивантаження даних в %1'");
	КонецЕсли;
...
Процедура ФоновоеЗаданиеПолучитьРезультатНаСервере()
	...
	ТиповоеПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Не удалось выполнить %1. Подробности см. в журнале регистрации';uk='Не вдалося виконати %1. Подробиці див в журналі реєстрації'"),
		?(ФоновоеЗаданиеВыполняемоеДействие = 1, НСтр("ru='получение данных';uk='отримання даних'"), НСтр("ru='отправку данных';uk='відправку даних'")));
