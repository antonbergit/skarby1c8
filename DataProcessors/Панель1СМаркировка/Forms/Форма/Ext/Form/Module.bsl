...
Процедура ОбновитьСпискиДокументов()
	...
	ТекстОформите    = НСтр("ru='оформите';uk='оформите'");
	ТекстОтработайте = НСтр("ru='отработайте';uk='отработайте'");
	ТекстОжидайте    = НСтр("ru='ожидайте';uk='ожидайте'");
	...
	Для ТекИндекс = 0 По МассивЗапросов.Количество()-1 Цикл
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.УведомлениеОбОтгрузкеМаркированныхТоваровГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаУведомленияОбОтгрузке" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьУведомленияОбОтгрузке, НСтр("ru='Уведомления об отгрузке';uk='Уведомления об отгрузке'"));
			КонецЕсли;
		...
		Если ИнтеграцияГИСМПереопределяемый.ДоступенОтчетОРозничныхПродажах() Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаОтчетыОРозничныхПродажах" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьОтчетыОРозничныхПродажах, НСтр("ru='Уведомления о розничных продажах';uk='Уведомления о розничных продажах'"));
			КонецЕсли;
		...
		Если ИнтеграцияГИСМПереопределяемый.ДоступенВозвратТоваровОтРозничногоКлиента() Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаВозвратыТоваровОтРозничныхКлиентов" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьВозвратыТоваровОтРозничныхКлиентов, НСтр("ru='Уведомления о розничных возвратах';uk='Уведомления о розничных возвратах'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.МаркировкаТоваровГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаМаркировкаТоваров" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьМаркировкуТоваров, НСтр("ru='Маркировка товаров';uk='Маркировка товаров'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.ПеремаркировкаТоваровГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаПеремаркировкаТоваров" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьПереМаркировкуТоваров, НСтр("ru='Перемаркировка товаров';uk='Перемаркировка товаров'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.УведомлениеОСписанииКиЗГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаУведомленияОСписанииКиЗГИСМ" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьСписанияКиЗМаркировки, НСтр("ru='Уведомления о списании КиЗ';uk='Уведомления о списании КиЗ'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаявкаНаВыпускКиЗГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаЗаявкиНаВыпускКиЗГИСМ" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьЗаявкиНаВыпускКиЗДляМаркировки, НСтр("ru='Заявки на выпуск КиЗ';uk='Заявки на выпуск КиЗ'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.УведомлениеОПоступленииМаркированныхТоваровГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаУведомленияОПоступлении" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьУведомленияОПоступленииМаркированныхТоваров, НСтр("ru='Уведомления о поступлении и возврате';uk='Уведомления о поступлении и возврате'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.УведомлениеОВвозеМаркированныхТоваровИзЕАЭСГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаУведомленияОВвозеИзЕАЭС" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьУведомленияОВвозеМаркированныхТоваровИзЕАЭС, НСтр("ru='Уведомления о ввозе из ЕАЭС';uk='Уведомления о ввозе из ЕАЭС'"));
			КонецЕсли;
		...
		Если ПравоДоступа("Чтение", Метаданные.Документы.УведомлениеОбИмпортеМаркированныхТоваровГИСМ) Тогда
			...
			Если МассивЗапросов[ТекИндекс] = "ТекстЗапросаУведомленияОбИмпорте" Тогда
				ВывестиПоказатель(Выборка, Элементы.ОткрытьУведомленияОбИмпортеМаркированныхТоваров, НСтр("ru='Уведомления об импорте';uk='Уведомления об импорте'"));
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекстЗапроса) Тогда
		...
		Если Выборка.Следующий() И Выборка.КоличествоДокументов > 0 Тогда
			ТекстЗаголовка = СтрШаблон(НСтр("ru='Есть документы (%1), ожидающие отправки в ГИСМ. Выполните обмен.';uk='Есть документы (%1), ожидающие отправки в ГИСМ. Выполните обмен.'"), Выборка.КоличествоДокументов);
