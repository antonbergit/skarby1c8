...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоПодчиненныйУзелРИБ
		...
		СообщениеЖурнала = НСтр("ru='Обновление программы невозможно. В подчиненном узле информационной базы обновление получается из главного узла.';uk='Оновлення програми неможливе. У підпорядкованому вузлі інформаційної бази оновлення отримується з головного вузла.'");
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		ОтобразитьВнутреннююОшибку(
			НСтр("ru='<b>Обновление программы невозможно</b><br />В подчиненном узле информационной базы обновление получается из главного узла.';uk='<b>Оновлення програми неможливе</b><br />В підлеглому вузлі інформаційної бази оновлення одержується з головного вузла.'"),
			,
			СообщениеЖурнала);
	...
	ИначеЕсли ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		СообщениеЖурнала = НСтр("ru='Обновление программы невозможно. Обновление программы недоступно при работе в режиме веб-клиента.';uk='Оновлення програми неможливе. Оновлення програми недоступне при роботі в режимі веб-клієнта.'");
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		ОтобразитьВнутреннююОшибку(
			НСтр("ru='<b>Обновление программы невозможно</b><br />"
"Обновление программы недоступно при работе в режиме веб-клиента.<br />"
"Откройте программу в тонком или толстом клиенте и повторите попытку.';uk='<b>Оновлення програми неможливе</b><br />"
"Оновлення програми недоступне при роботі в режимі веб-клієнта. <br />"
"Відкрийте програму в тонкому або товстому клієнті і спробуйте ще раз. '"),
	...
	Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
		...
		ТекстИсключения =
			НСтр("ru='Переход на новую версию платформы 1С:Предприятие недоступен в текущем режиме работы.';uk='Перехід на нову версію платформи 1С:Підприємство недоступний у поточному режимі роботи.'");
	...
	Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
		...
		ЭтотОбъект.Заголовок     = НСтр("ru='Переход на новую версию платформы 1С:Предприятие';uk='Перехід на нову версію платформи 1С:Підприємство'");
	...
	Элементы.ДекорацияНаписатьВТехПоддержку.Заголовок =
		ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
			ИнтернетПоддержкаПользователейКлиентСервер.ПодставитьДомен(
				НСтр("ru='<body>При возникновении проблем напишите в <a href=""mailto:webits-info@1c.ru"">техподдержку</a>.</body>';uk='<body>При виникненні проблем напишіть в <a href=""mailto:web-its@1c.ua"">техпідтримку</a>.</body>'")));
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПолучениеИУстановка
		...
		Если ЗавершениеРаботы Тогда
			ТекстПредупреждения = НСтр("ru='Не завершена установка платформы 1С:Предприятие.';uk='Не завершене встановлення платформи 1С:Підприємство.'");
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если ЗавершениеРаботы Тогда
		Если ЗначениеЗаполнено(ИДФоновогоЗадания) Тогда
			Отказ               = Истина;
			ТекстПредупреждения = НСтр("ru='Получение и установка обновлений не завершены.';uk='Отримання і встановлення оновлень не завершені.'");
...
Процедура КаталогСохраненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбора.Заголовок = НСтр("ru='Каталог хранения дистрибутивов платформы 1С:Предприятие';uk='Каталог зберігання дистрибутивів платформи 1С:Підприємство'");
...
Процедура ОбработатьНавигационнуюСсылку(Ссылка)
	...
	Если Ссылка = "open:RCInstruction" Тогда
		...
		ОткрытьВнутреннийФайлИнструкции(
			...
			НСтр("ru='Создание резервной копии.htm';uk='Створення резервної копії.htm'"));
		
	ИначеЕсли Ссылка = "open:V8Update" Тогда
		
		ОткрытьВебСтраницу(
			ИнформацияОДоступномОбновлении.Платформа.URLОсобенностиПерехода,
			НСтр("ru='Особенности перехода на новую версию платформы 1С:Предприятие';uk='Особливості переходу на нову версію платформи 1С:Підприємство'"));
...
Процедура ОтправитьСообщениеВТехПоддержку(АдресПолучателя)
	...
	Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
		Или ЭтоСценарийСообщениеОНерекомендуемойВерсииПлатформы(ЭтотОбъект) Тогда
		Тема = НСтр("ru='Интернет-поддержка. Переход на новую версию Платформы 1С:Предприятие';uk='Інтернет-підтримка. Перехід на нову версію Платформи 1С:Підприємство'");
	Иначе
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПодключениеКПорталу Тогда
		...
		Сообщение = НСтр("ru='Не удалось подключиться к Порталу 1С:ИТС.';uk='Не вдалося підключитися до Порталу 1С:ІТС.'")
	...
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаСообщениеСервиса Тогда
		...
		Если ПустаяСтрока(ПодробноеОписаниеОшибки) Тогда
			Сообщение = НСтр("ru='Сообщение при подключении сервиса автоматического обновления программы:';uk='Повідомлення при підключенні сервісу автоматичного оновлення програми:'")
				+ Символы.ПС
		...
		Иначе
				...
				Если ПолучениеОбновленийПрограммыКлиентСервер.ФайлСуществует(КонтекстОбновления.ПутьФайлаПротокола) Тогда
					Вложения.Добавить(
						Новый Структура("Представление, ИмяФайла",
							НСтр("ru='Протокол установки платформы 1С_Предприятие.txt';uk='Протокол установки платформи 1С_Підприємство.txt'"),
			...
			Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
				...
				Сообщение = НСтр("ru='Ошибка при получении и установке платформы 1С:Предприятие:';uk='Помилка при отриманні і установці платформи 1С:Підприємство :'")
					+ Символы.ПС
					+ ПодробноеОписаниеОшибки;
				
			Иначе
				
				Сообщение = НСтр("ru='Не удалось обновить программу.';uk='Не вдалося відновити програму.'")
					+ Символы.ПС
...
"%2';uk='Помилка при відкритті внутрішнього файлу інструкції (%1)."
"%2'"),
				ИмяФайла,
...
Функция ПодготовитьФайлМакетаИнструкции(Знач ИмяМакета, Знач ИмяФайла)
	...
	Если Не ПолучениеОбновленийПрограммы.СлужебнаяДоступноИспользованиеПолученияОбновленийПлатформы() Тогда
		ВызватьИсключение НСтр("ru='Использование обновления платформы 1С:Предприятие недоступно в текущем режиме работы.';uk='Використання оновлення платформи 1С:Підприємство недоступне в поточному режимі роботи.'");
	КонецЕсли;
...
"%2';uk='Помилка при створенні каталогу для роботи з оновленнями платформи 1С:Підприємство (%1)."
"%2'"),
					КаталогДляРаботыСОбновлениями,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
...
"%2';uk='Помилка при записі даних макета інструкції у файл на диску (%1)."
"%2'"),
				ИмяФайла,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
...
"Файлы обновления: %1"
"Платформа 1С:Предприятие: %2';uk='Виклик інтерфейсу встановлення оновлення (СтандартныеПодсистемы.ОбновлениеКонфигурации)."
"Файли оновлення: %1"
"Платформа 1С:Підприємство: %2'"),
			СтрОписаниеФайловОбновлений,
...
Процедура ПолучитьИОтобразитьИнформациюОДоступномОбновлении()
	...
	Состояние(, , НСтр("ru='Получение информации о доступном обновлении';uk='Одержання інформації про доступне оновлення'"));
...
Функция НовыйИнформацияОДоступномОбновлении(ФормаПомощника, ПараметрыКлиента)
	...
	Возврат ПолучениеОбновленийПрограммыКлиентСервер.ИнформацияОДоступномОбновлении(
		...
		ПараметрыКлиента,
		ФормаПомощника.НастройкиПроксиСервера);
...
Процедура ОтобразитьИнформациюОДоступномОбновленииПлатформы(ЗаполнитьНачальныеНастройки = Ложь)
	...
	Если НЕ ЗначениеЗаполнено(ОбновлениеПлатформы.Версия) Тогда
		...
		Элементы.ДекорацияНетВерсии.Заголовок =
			ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<body>Используемая сейчас версия: %1</body>';uk='<body>Використовувана зараз версія: %1</body>'"),
...
"<br />Используемая сейчас версия: %2</body>';uk='<body>Рекомендована версія платформи 1С:Підприємство: <b>%1</b>"
...
"<br />с <a href=""open:V8Update"">особенностями перехода</a> на эту версию платформы.</body>';uk='<body>Версія платформи %1 уже встановлена на комп`ютері."
...
Процедура ОтобразитьКраткуюИнформациюОДоступномОбновлении()
	...
	Если Не ОбновлениеКомКонф.ДоступноОбновление И Не ОбновлениеКомПл.ДоступноОбновление Тогда
		...
		ТекстСообщения = ТекстСообщения + НСтр("ru='Обновление не требуется. Установлена актуальная версия программы.';uk='Оновлення не потрібно. Встановлена актуальна версія програми.'");
...
"%2</body>';uk='<body>Оновлення не потрібне. Поточна версія конфігурації: <b>%1</b><br />"
"%2</body>'"),
...
"%4</body>';uk='<body><b>Версія %1</b>   <a href=""%2"">Нове в версії</a><br />"
...
"%3</body>';uk='<body><b>Версія %1</b><br />"
...
Процедура ОтобразитьКомпонентПлатформы()
	...
	Если Не ОбновлениеКомПл.ДоступноОбновление Тогда
		...
		Элементы.ДекорацияЗаголовокОбновлениеПлатформы.Заголовок =
			НСтр("ru='Обновление платформы 1С:Предприятие';uk='Оновлення платформи 1С:Підприємство'");
		Элементы.ДекорацияНомерВерсииПлатформы.Заголовок =
			ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<body>Обновление не требуется. Текущая версия платформы 1С:Предприятие: <b>%1</b></body>';uk='<body>Оновлення не потрібне. Поточна версія платформи 1С:Підприємство: <b>%1</b></body>'"),
					ПолучениеОбновленийПрограммыКлиентСервер.ТекущаяВерсияПлатформы1СПредприятие()));
	...
	Иначе
		...
		Если ЭтоФайловаяИБ Тогда
			Элементы.ДекорацияЗаголовокОбновлениеПлатформы.Заголовок =
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='<body>Обновление платформы 1С:Предприятие, <b>%1</b></body>';uk='<body>Оновлення платформи 1С:Підприємство, <b>%1</b></body>'"),
						ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеРазмераФайла(
							ОбновлениеКомПл.РазмерОбновления)));
		Иначе
			Элементы.ДекорацияЗаголовокОбновлениеПлатформы.Заголовок =
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
						НСтр("ru='<body>Обновление платформы 1С:Предприятие</body>';uk='<body>Оновлення платформи 1С:Підприємство</body>'"));
		КонецЕсли;
...
"Текущая версия платформы 1С:Предприятие: %3</body>';uk='<body><b>Версія %1</b>   <a href=""%2"">Особливості переходу на нову версію платформи</a><br />"
"Поточна версія платформи 1С:Підприємство: %3</body>'"),
		...
		Иначе
			...
			Элементы.ДекорацияНомерВерсииПлатформы.Заголовок =
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='<body><b>Версия %1</b><br />"
"Текущая версия платформы 1С:Предприятие: %2</body>';uk='<body><b>Версія %1</b><br />"
"Поточна версія платформи 1С:Підприємство: %2</body>'"),
						ОбновлениеКомПл.Версия,
						ПолучениеОбновленийПрограммыКлиентСервер.ТекущаяВерсияПлатформы1СПредприятие()));
		...
		Если ЭтоФайловаяИБ Тогда
			...
			Если ОбновитьКонфигурацию Тогда
				...
				ИначеЕсли ОбновлениеКомПл.ОбязательностьУстановки = 1 Тогда
					Элементы.ДекорацияПояснениеПлатформа.Заголовок =
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Новая версия %1 платформы 1С:Предприятие рекомендуется для работы с новой версией конфигурации %2.';uk='Нова версія %1 платформи 1С:Підприємство рекомендується для роботи з новою версією конфігурації %2.'"),
							ОбновлениеКомПл.Версия,
							ИнформацияОДоступномОбновлении.Конфигурация.Версия);
				Иначе
					Элементы.ДекорацияПояснениеПлатформа.Заголовок =
			...
			ИначеЕсли ОбновлениеКомПл.ОбязательностьУстановки < 2 Тогда
				...
				Элементы.ДекорацияПояснениеПлатформа.Заголовок =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Рекомендуется установить новую версию платформы 1С:Предприятие.';uk='Рекомендується встановити нову версію платформи 1С:Підприємство.'"),
						ИнформацияОДоступномОбновлении.Конфигурация.Версия);
			...
			Если ОбновитьКонфигурацию Тогда
				...
				Если ОбновлениеКомПл.ОбязательностьУстановки = 0 Тогда
					Элементы.ДекорацияПояснениеПлатформа.Заголовок =
						ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='<body>Для работы новой версии конфигурации требуется платформа 1С:Предприятие не ниже версии <b>%1</b>.</body>';uk='<body>Для роботи нової версії конфігурації потрібно платформа 1С:Підприємство не нижче версії <b>%1</b>.</body>'"),
...
"Имя файла обновления (cfu): %7.';uk='Отримання оновлення конфігурації."
"URL: %1;"
"Розмір: %2;"
...
"Ім`я файлу оновлення (cfu): %7.'"),
		...
		Иначе
			...
			ПолучениеОбновленийПрограммыКлиент.ЗаписатьИнформациюВЖурналРегистрации(
				НСтр("ru='Обновление конфигурации уже было получено ранее.';uk='Оновлення конфігурації вже було отримане раніше.'"));
...
Процедура ЗагрузитьОбновлениеКонфигурации()
	...
	Иначе
		...
		ПолучениеОбновленийПрограммыКлиент.ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Загрузка файла обновления конфигурации %1 (выполняется на компьютере пользователя).';uk='Завантаження файлу оновлення конфігурації %1 (виконується на комп`ютері користувача).'"),
				Обновление.ИмяПолученногоФайла));
...
Процедура ИнициализироватьСтраницуПолучениеОбновления()
	...
	Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
		...
		Элементы.ДекорацияПолучениеДистрибутива.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Выполняется получение файла дистрибутива платформы 1С:Предприятие (%1).';uk='Виконується одержання файлу дистрибутива платформи 1С:Підприємство (%1).'") + " "
					+ НСтр("ru='Получение файла может занять от нескольких минут до нескольких часов в зависимости от размера обновления,';uk='Одержання файлу може зайняти від декількох хвилин до декількох годин залежно від розміру оновлення,'") + " "
					+ НСтр("ru='скорости подключения к Интернету и производительности компьютера.';uk='швидкості підключення до Інтернету й продуктивності комп`ютера.'"),
		...
		Элементы.ДекорацияПолучениеДистрибутива.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Выполняется получение файлов обновления (%1). Получение файлов может занять от нескольких минут до';uk='Виконується отримання файлів оновлення (%1). Отримання файлів може зайняти від декількох хвилин до'")
					+ " " + НСтр("ru='нескольких часов в зависимости от размера обновления,';uk='декількох годин залежно від розміру оновлення'")
					+ " " + НСтр("ru='скорости подключения к Интернету и производительности компьютера.';uk='швидкості підключення до Інтернету й продуктивності комп`ютера.'"),
				РазмерОбновленияСтр(ЭтотОбъект));
	КонецЕсли;
...
Процедура НачатьПолучениеОбновленийВФоновомРежиме()
	...
	Попытка
		Задание = ФоновыеЗадания.Выполнить(
			...
			,
			НСтр("ru='Получение и установка обновлений программы';uk='Отримання і встановлення оновлень програми'"));
	Исключение
		...
		Если ОписательСостояния.КодСостояния = "Ошибка" Тогда
			...
			Иначе
				Если РежимУстановкиПлатформы = 0
					...
					ОтобразитьВнутреннююОшибку(
						КонтекстОбновления.Сообщение,
						НСтр("ru='Установить с ручными настройками >';uk='Встановити з ручними налаштуваннями >'"),
						СообщениеЖурналаРегистрации);
				Иначе
					ОтобразитьВнутреннююОшибку(
						КонтекстОбновления.Сообщение,
...
Функция СостояниеЗадания(Знач ИДЗадания)
	...
	Исключение
		...
		СообщениеЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при обращении к заданию. %1';uk='Помилка при звертанні до завдання. % 1'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
	...
	Если Задание = Неопределено Тогда
		...
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(НСтр("ru='Задание не найдено.';uk='Завдання не знайдене.'"));
		
		Результат.КодСостояния = "Ошибка";
		Результат.Вставить(
			"ДопПараметры",
			Новый Структура("Сообщение, ИнформацияОбОшибке",
				НСтр("ru='Задание не найдено.';uk='Завдання не знайдене.'"),
				НСтр("ru='Задание не найдено.';uk='Завдання не знайдене.'")));
	...
	Если Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
		...
		СообщениеЖурнала = НСтр("ru='Задание отменено администратором системы.';uk='Завдання скасоване адміністратором системи.'");
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		...
		Результат.Вставить(
			"ДопПараметры",
			Новый Структура("Сообщение, ИнформацияОбОшибке",
				НСтр("ru='Задание отменено администратором системы.';uk='Завдання скасоване адміністратором системи.'"),
				СообщениеЖурнала));
	...
	ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно Тогда
		...
		СообщениеЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Задание завершено с ошибкой. %1';uk='Завдання завершене з помилкою. % 1'"),
	...
	Если ВРег(Лев(Сообщение, 6)) <> "<BODY>" Тогда
		...
		ПредставлениеHTML =
			ИнтернетПоддержкаПользователейКлиентСервер.ПодставитьДомен(
				...
				+ "<br />"
				+ "<br />" + НСтр("ru='Технические подробности см. в <a href=""open:log"" >журнале регистрации</a>.';uk='Технічні подробиці див. в <a href=""open:log"" >журналі реєстрації</a>.'")
				+ "<br /><br />" + НСтр("ru='При возникновении проблем напишите в <a href=""mailto:webits-info@1c.ru"">техподдержку</a>.';uk='При виникненні проблем напишіть в <a href=""mailto:web-its@1c.ua"">техпідтримку</a>.'")
				+ "</body>");
...
Процедура ОтобразитьПолучениеОбновленийЗавершено(Инициализировать = Ложь)
	...
	Иначе
		...
		Если ПараметрыАдминистрирования = Неопределено Тогда
			...
			ЗаголовокФормы = Нстр("ru='Установка обновления';uk='Установка відновлення'");
			Если ЭтоФайловаяИБ Тогда
				ПоясняющаяНадпись = Нстр("ru='Для установки обновления необходимо ввести"
"параметры администрирования информационной базы';uk='Для встановлення оновлення необхідно ввести"
...
"администрирования кластера серверов и информационной базы';uk='Для встановлення оновлення необхідно ввести параметри"
...
"корректные параметры администрирования информационной базы.</body>';uk='Не вдалося встановити оновлення програми, оскільки не були введені"
...
"<br />Дистрибутив платформы скопирован в каталог <a href=""open:DistribFolder"">%2</a></body>';uk='<body>Встановлена версія: <b>%1</b>"
"<br />Дистрибутів платформи скопійовано в каталог <a href=""open:DistribFolder"">%2</a></body>'");
...
Функция ТекстИнструкцииПоПереходуНаНовуюВерсиюПлатформы(ПараметрСоздатьРезервнуюКопию)
	...
	Если ВнедренаПодсистемаРезервногоКопирования
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЗавершениеРаботыПользователей") Тогда
			Строки.Добавить(
				НСтр("ru='<body>Будет автоматически завершена работа <a href=""open:ActiveUsers"">активных пользователей</a> программы, создана резервная копия';uk='<body>Буде автоматично завершена робота <a href=""open:ActiveUsers"">активних користувачів</a> програми, створена резервна копія'"));
		Иначе
			Строки.Добавить(
				НСтр("ru='<body>Будет автоматически завершена работа активных пользователей программы, создана резервная копия';uk='<body>Буде автоматично завершена робота активних користувачів програми, створена резервна копія'"));
		КонецЕсли;
		
		Если ЭтоБазоваяВерсияКонфигурации Тогда
			Строки.Добавить(НСтр("ru='информационной базы и запущен сеанс работы с программой на новой версии платформы.<body>';uk='інформаційної бази й запущений сеанс роботи із програмою на новій версії платформи.<body>'"));
		Иначе
			Строки.Добавить(НСтр("ru='информационной базы и запущен сеанс работы с программой на новой версии платформы."
"<br />На компьютерах других пользователей программы необходимо обновить платформу 1С:Предприятие вручную.</body>';uk='інформаційної бази й запущений сеанс роботи із програмою на новій версії платформи."
"<br />На комп`ютерах інших користувачів програми необхідно оновити платформу 1С:Підприємство вручну.</body>'"));
		КонецЕсли;
	...
	Иначе
		...
		Строки.Добавить(НСтр("ru='- завершите текущий сеанс работы с программой;';uk='- завершіть поточний сеанс роботи із програмою;'"));
