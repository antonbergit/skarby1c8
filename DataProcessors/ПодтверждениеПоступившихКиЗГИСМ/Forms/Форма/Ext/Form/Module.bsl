...
Процедура ТекстПоЗаявкамОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ОткрытьДокументы" Тогда
		...
		ИначеЕсли ДокументыКПодтверждению.Количество() > 1 Тогда
			...
			ИнтеграцияГИСМКлиентПереопределяемый.ОткрытьФормуСпискаДокументов(ДокументыКПодтверждению, НСтр("ru='Заявки на выпуск КиЗ';uk='Заявки на выпуск КиЗ'"));
...
Процедура Обновить(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru='Таблица ""КиЗ к подтверждению"" будет перезаполнена. Продолжить?';uk='Таблица ""КиЗ к подтверждению"" будет перезаполнена. Продолжить?'");
...
Функция ПодтвердитьПоступлениеНаСервере()
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Исключение
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ru='Не удалось заблокировать для записи документ %1';uk='Не удалось заблокировать для записи документ %1'"), ВыборкаПоДокументам.Документ),
	...
	Исключение
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(НСтр("ru='Не удалось изменить состояние подтверждения для документа %1';uk='Не удалось изменить состояние подтверждения для документа %1'"), ДокументСОшибкой),
			ДокументСОшибкой);
...
Процедура СформироватьТекстПоОснованиям()
	...
	Если КоличествоДокументов = 1 Тогда
		...
		ТекстЗаголовок = НСтр("ru='По документу:';uk='По документу:'");
	...
	ИначеЕсли КоличествоДокументов > 1 Тогда
		...
		Основание = Новый ФорматированнаяСтрока(СтрШаблон(НСтр("ru='По документам (%1).';uk='По документам (%1).'"), КоличествоДокументов),
		                                             Новый Шрифт(,,,,Истина),
		                                             ЦветаСтиля.ЦветГиперссылкиГИСМ,
		                                             ,
		                                             "ОткрытьДокументы");
