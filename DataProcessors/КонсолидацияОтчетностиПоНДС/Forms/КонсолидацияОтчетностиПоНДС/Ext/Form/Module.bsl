...
"Закройте окно вручную, до выхода из программы.';uk='Работа в форме не завершена."
"Закройте окно вручную, до выхода из программы.'");
	...
	Если ТребуетсяПодтверждениеЗакрытияФормы Тогда
		...
		ТекстВопроса = НСтр("ru='Закрыть форму?';uk='Закрити форму?'");
...
Процедура КонсолидироватьИСформироватьДекларациюПоНДС(Команда);
	...
	Если ФайлыВыгрузки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Не добавлены файлы для консолидации';uk='Не добавлены файлы для консолидации'"));
		Возврат;
	КонецЕсли;
	...
	Если Организация.Пустая() Тогда
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не выбрана или не найдена организация %1 для формирования декларации по НДС';uk='Не выбрана или не найдена организация %1 для формирования декларации по НДС'"), ПараметрОписания);
	...
	Если НЕ ПараметрыВыгрузки.Свойство("ПараметрыФормыОтчета", ПараметрыФормыОтчета) ИЛИ ПараметрыФормыОтчета = Неопределено Тогда
		ОписаниеОшибки = НСтр("ru='Не сформированы параметры для создания декларации по НДС!';uk='Не сформированы параметры для создания декларации по НДС!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ОписаниеОшибки, , "");
		Возврат;
	КонецЕсли;
	...
	Если ФормаДекларации = Неопределено Тогда
		ОписаниеОшибки = НСтр("ru='Декларация по НДС не сформирована!';uk='Декларация по НДС не сформирована!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ОписаниеОшибки, , "");
		Возврат;
	КонецЕсли;
...
Процедура ВыбратьФайлы(ЗаменитьСтроку = Ложь)
	...
	Если ПоддерживаетсяРасширениеРаботыСФайлами = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ru='Для множественного выбора файлов выгрузки рекомендуется установить расширение работы с файлами.';uk='Для множественного выбора файлов выгрузки рекомендуется установить расширение работы с файлами.'");
...
Процедура ВыбратьФайлыПослеПодключенияРасширенияРаботыСФайлами(Подключено, Контекст) Экспорт
	...
	Диалог.Заголовок = НСтр("ru='Выберите файл(ы) выгрузки декларации по НДС';uk='Выберите файл(ы) выгрузки декларации по НДС'");
	Диалог.Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Файлы выгрузки (*.%1)|*.%1|Все файлы(*.*)|*.*';uk='Файлы выгрузки (*.%1)|*.%1|Все файлы(*.*)|*.*'"), "xml");
...
"%2';uk='При получении параметров из файла выгрузки ""%1"" произошла ошибка:"
...
"%2';uk='В файла выгрузки ""%1"" установлен признак актуальности ранее представленных сведений для корректирующего отчета:"
...
"%2';uk='При получении параметров из идентификатора файла выгрузки ""%1"" произошла ошибка:"
...
"%2';uk='При добавлении файла выгрузки ""%1"" произошла ошибка:"
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ru='Заполнение консолидированной декларации по НДС';uk='Заполнение консолидированной декларации по НДС'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ЗагрузитьВЭлектронноеПредставление(ФайлыРазделовДекларации)
	...
	Если КонтекстЭДОКлиент = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='ТекстОшибкиИнициализацииКонтекстаЭДО';uk='ТекстОшибкиИнициализацииКонтекстаЭДО'"));
		Возврат;
	КонецЕсли;
...
"%2';uk='При формировании параметров выгрузки из файла ""%1"" произошла ошибка:"
...
"%2';uk='При заполнении данных разделов 1-7 из файла выгрузки ""%1"" произошла ошибка:"
"%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
	...
	Если НЕ ПараметрыВыгрузки.Свойство("ИдФайл") Тогда
		ОшибкаНаСервере.Вставить("ОписаниеОшибки", НСтр("ru='Не сформированы параметры для консолидации. Нет файлов выгрузки разделов 1-7.';uk='Не сформированы параметры для консолидации. Нет файлов выгрузки разделов 1-7.'"));
		Возврат Ложь;
	КонецЕсли;
...
"%2';uk='При заполнении данных раздела 8 из файла выгрузки ""%1"" произошла ошибка:"
"%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
...
"%2';uk='При заполнении данных приложения 1 к разделу 8 из файла выгрузки ""%1"" произошла ошибка:"
"%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
...
"%2';uk='При заполнении данных раздела 9 из файла выгрузки ""%1"" произошла ошибка:"
"%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
...
"%2';uk='При заполнении данных приложения 1 к разделу 9 из файла выгрузки ""%1"" произошла ошибка:"
"%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
...
"%2';uk='При заполнении данных раздела 10 из файла выгрузки ""%1"" произошла ошибка:"
"%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
...
"%2';uk='При заполнении данных раздела 11 из файла выгрузки ""%1"" произошла ошибка:"
"%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
...
"%2';uk='При заполнении данных раздела 12 из файла выгрузки ""%1"" произошла ошибка:"
"%2'"), ДанныеФайлаВыгрузки.НаименованиеФайла, ОписаниеОшибки));
				Возврат Ложь;
			КонецЕсли;
	...
	Если ЗаполнитьДекларацию Тогда
		...
		Если СохраненныйДок = Неопределено Тогда
			ОшибкаНаСервере.Вставить("ОписаниеОшибки", НСтр("ru='Не сформирован документ для заполнения консолидированными данными.';uk='Не сформирован документ для заполнения консолидированными данными.'"));
			Возврат Ложь;
		КонецЕсли;
...
"%2';uk='При заполнении сохраненного отчета ""%1"" произошла ошибка:"
...
Функция КонсолидироватьВФоновомЗадании(ТекстВыгрузки, ОшибкаНаСервере, ЗаполнитьДекларацию = Ложь, ЗаполнитьТолькоПараметры = Ложь, СохраненныйДок = Неопределено)
	...
	РезультатВыполнения = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		...
		ПараметрыЗадания,
		НСтр("ru='Выполняется заполнение отчета консолидированными данными. Пожалуйста, подождите...';uk='Выполняется заполнение отчета консолидированными данными. Пожалуйста, подождите...'"));
...
Функция ПолучитьФормуОтчетаНаКлиенте(ПараметрыФормыОтчета)
	...
	ИначеЕсли ПараметрыФормыОтчета = "Не удалось открыть отчет" Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Не удалось открыть отчет!';uk='Не вдалося відкрити звіт!'"));
		Возврат Неопределено;
		
	ИначеЕсли ПараметрыФормыОтчета = "Открыть внутренний отчет-документ" Тогда
		
		ПоказатьПредупреждение(, НСтр("ru='Заполнение отчета-документа не предусмотрено!';uk='Заполнение отчета-документа не предусмотрено!'"));
		Возврат Неопределено;
	...
	Иначе
		...
		ПоказатьПредупреждение(, НСтр("ru='Недостаточно параметров для создания отчета!';uk='Недостаточно параметров для создания отчета!'"));
		Возврат Неопределено;
	...
	Если ОсновнаяФормаОтчета = Неопределено Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Не удалось получить основную форму отчета!';uk='Не удалось получить основную форму отчета!'"));
		Возврат Неопределено;
	...
	Если ВыбраннаяФормаОтчета = Неопределено Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Не удалось получить форму отчета по периоду!';uk='Не удалось получить форму отчета по периоду!'"));
		Возврат Неопределено;
...
Функция ПолучитьФормуОтчетаИзДокумента(СохраненныйДок)
		...
		Исключение
			...
			Если ИнформацияОбОшибке.Причина = Неопределено Тогда
				Сообщение.Текст = НСтр("ru='При открытии формы регламентированного отчета произошла ошибка!';uk='При открытии формы регламентированного отчета произошла ошибка!'");
			ИначеЕсли Найти(ИнформацияОбОшибке.Причина.Описание, ".Форма.") > 0 Тогда
				Сообщение.Текст = НСтр("ru='Отчет не может быть открыт! Устаревшая редакция формы отчета не поддерживается текущей версией конфигурации.';uk='Отчет не может быть открыт! Устаревшая редакция формы отчета не поддерживается текущей версией конфигурации.'");
			Иначе
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела8(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ru='Заполнен раздел 8';uk='Заполнен раздел 8'"));
	КонецЕсли;
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела81(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ru='Заполнен раздел 8 приложение 1';uk='Заполнен раздел 8 приложение 1'"));
	КонецЕсли;
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела9(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ru='Заполнен раздел 9';uk='Заполнен раздел 9'"));
	КонецЕсли;
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела91(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ru='Заполнен раздел 9 приложение 1';uk='Заполнен раздел 9 приложение 1'"));
	КонецЕсли;
...
Процедура ЗаписатьВСегментДанныхСтрокуРаздела10(СтрокаРаздела10, СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП = 0 Тогда
		...
		ПараметрыРаздела.Вставить("ТабличныйДокумент", Новый ТабличныйДокумент);
		ПараметрыРаздела.ТабличныйДокумент.Очистить();
		ПараметрыРаздела.ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЖурналУчета";
		
		ПараметрыРаздела.Вставить("Макет", Неопределено);
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела10(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ru='Заполнен раздел 10';uk='Заполнен раздел 10'"));
	КонецЕсли;
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела11(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ru='Заполнен раздел 11';uk='Заполнен раздел 11'"));
	КонецЕсли;
...
Процедура ЗаписатьПоследнийСегментДанныхРаздела12(СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП > 0 Тогда
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,,, НСтр("ru='Заполнен раздел 12';uk='Заполнен раздел 12'"));
	КонецЕсли;
