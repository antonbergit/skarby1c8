...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = НСтр("ru='Скрытие конфиденциальной информации';uk='Скрытие конфиденциальной информации'");
	...
	Если ДеревоМетаданныхЗаполнено Тогда
		Если Элементы.СтраницыВидФормы.ТекущаяСтраница = Элементы.СтраницаЧастотныеВозможности Тогда
			...
			Если МакетОпределен Тогда
				Заголовок = НСтр("ru='Скрытие конфиденциальной информации - полные возможности';uk='Скрытие конфиденциальной информации - полные возможности'");
			КонецЕсли;
			Элементы.РасширенныеВозможности.Заголовок = НСтр("ru='Обычный вид';uk='Обычный вид'");
		Иначе
...
Процедура ЗначениеЧислоПриИзменении(Элемент)
	Если ЗначениеЧисло = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Значение должно быть не нулевым.';uk='Значение должно быть не нулевым.'");
...
"Продолжить?';uk='Скрытие конфиденциальной информации необходимо выполнять на копии рабочей базы,"
"все изменения будут выполнены безвозвратно."
"Продолжить?'");
...
Процедура Подключаемый_ВыполнитьСкрытиеЗавершение()
	...
	Если РезультатЗапуска.Статус <> "Выполнено" Тогда
		...
		КонецЕсли;
	Иначе
		СообщитьПользователю(НСтр("ru='Скрытие конфиденциальной информации завершено.';uk='Скрытие конфиденциальной информации завершено.'"));
	КонецЕсли;
...
Процедура СохранитьНастройкиПродолжение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		ДиалогСохранения.Фильтр = НСтр("ru='Файл настроек';uk='Файл настроек'") + "(*.xml)|*.xml";
...
Процедура ЗагрузитьНастройкиПродолжение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		ДиалогВыбора.Фильтр = НСтр("ru='Файл настроек';uk='Файл настроек'") + "(*.xml)|*.xml";
...
Процедура ЗаполнитьДеревоОбрабатываемыхОбъектов(СохраненныеНастройки, ОбработкаОбъект)
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ru='Скрытие конфиденциальной информации';uk='Скрытие конфиденциальной информации'");
...
Функция СкрытиеКонфиденциальнойИнформации()
	...
	НаименованиеЗадания = НСтр("ru='Скрытие конфиденциальной информации';uk='Скрытие конфиденциальной информации'");
