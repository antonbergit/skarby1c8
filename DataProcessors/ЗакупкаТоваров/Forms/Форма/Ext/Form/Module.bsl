...
Процедура СформироватьЗаказы(Команда)
	...
	Если СформироватьЗаказыСервер() Тогда
		...
		ПоказатьОповещениеПользователя(
			,,
			НСтр("ru='Заказы поставщикам успешно созданы.';uk='Замовлення постачальникам успішно створені.'"),
	...
	Иначе
		...
		Сообщение.Текст = НСтр("ru='Нет данных для формирования заказов.';uk='Немає даних для формування замовлень.'");
		Сообщение.Сообщить();
...
Процедура УдалитьЗаказ(Команда)
	...
	ИначеЕсли ЗаказыНаУдаление.Количество() = 1 Тогда
		...
		ТекстВопроса = НСтр("ru='Выбранный документ будет удален. Продолжить?';uk='Обраний документ буде видален. Продовжити?'");
		
	Иначе
		
		ТекстВопроса = НСтр("ru='Выбранные документы будут удалены. Продолжить?';uk='Обрані документи будуть видалені. Продовжити?'");
...
Функция СформироватьЗаказыСервер()
	...
	Попытка
		...
		Для каждого СтрокаПоставщик Из ДеревоПотребность.Строки Цикл
			...
			ДокументОбъект.Комментарий = НСтр("ru='Создан автоматически обработкой ""Расчет потребности товаров""';uk='Створений автоматично обробкою ""Розрахунки потреби товарів""'");
	...
	Исключение
		...
		ОписаниеОшибки = НСтр("ru='При формировании заказов произошла ошибка."
"Формирование заказов отменено."
"Дополнительное описание:"
"%ДополнительноеОписание%';uk='При формуванні замовлень відбулася помилка."
"Формування замовлень скасоване."
"Додатковий опис:"
	...
	Для каждого СтрокаТаблицы Из Заказы Цикл
		...
		Иначе
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось провести документ: %1.';uk='Не вдалося провести документ: %1.'"), Строка(ДокументОбъект));
...
Процедура УдалитьЗаказыСервер()
	...
	Для Каждого СтрокаТаблицы Из Заказы Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось пометить на удаление документ: %1.';uk='Не вдалося позначити на видалення документ: %1.'"), Строка(ДокументОбъект));
