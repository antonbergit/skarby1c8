...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(ДляОрганизации) Тогда
		...
		Сообщение.Текст = НСтр("ru='Необходимо выбрать организацию, для составления календаря.';uk='Необхідно вибрати організацію, для складання календаря.'");
	...
	Если СрокСдачи = КонецГода(СрокСдачи) Или СрокСдачи = НачалоДня(КонецГода(СрокСдачи)) Тогда
		...
		Иначе
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							  НСтр("ru='Невозможно определить ближайшую рабочую дату для даты %1, возможно, календарь не заполнен.';uk='Неможливо визначити найближчу робочу дату для дати %1, "
"можливо, календар не заповнений.'"), 
