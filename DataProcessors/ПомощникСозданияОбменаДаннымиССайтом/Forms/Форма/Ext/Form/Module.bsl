...
Процедура ВыбратьКаталогЗавершение1(Подключено, ДополнительныеПараметры) Экспорт
    ...
    Если Не Подключено Тогда
        ПоказатьПредупреждение(Неопределено,НСтр("ru='Для данной операции необходимо установить расширение работы с файлами!';uk='Для даної операції необхідно встановити розширення роботи з файлами!'"));
        Возврат;
    КонецЕсли;
...
Процедура ВыполнитьПроверкиПриНажатииГотово(Отказ)
	...
	Если Объект.ИспользоватьРегламентныеЗадания Тогда
		...
		Если Объект.РасписаниеРегламентногоЗадания = Неопределено
			...
			Сообщение = НСтр("ru='При использовании автоматического обмена расписание должно быть установлено!';uk='При використанні автоматичного обміну розклад повинний бути встановлений!'");
...
Процедура СоздатьНовыйОбменДаннымиНаКлиенте(Отказ)
	...
	Состояние(НСтр("ru='Выполняется создание настройки обмена данными с web-сайтом';uk='Виконується створення налаштування обміну даними з web-сайтом'"));
	
	СоздатьНовыйОбменДаннымиНаСервере(КомпоновщикНастроекИнициализирован, Отказ);
	
	Если Отказ Тогда
		
		ПоказатьПредупреждение(Неопределено,НСтр("ru='При создании настройки обмена данными возникли ошибки!';uk='При створенні налаштування обміну даними виникли помилки!'"));
		
	Иначе
		
		ПоказатьОповещениеПользователя(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='""%1""';uk='""%1""'"), Объект.СсылкаНаУзелОбмена),,
			НСтр("ru='Новый узел обмена с WEB - сайтом создан успешно!';uk='Новий вузол обміну з WEB - сайтом створений успішно!'"),
...
Процедура ИнициализацияОбменаДаннымиНаКлиенте()
	...
	ОбменССайтом.ВыполнитьОбмен(УзелОбмена, НСтр("ru='Интерактивный обмен';uk='Інтерактивний обмін'"));
...
Процедура ВыполнитьПроверкиПриПереходеНаСледующуюСтраницу(Отказ)
	...
	Если ТекущийНомерСтраницы = 2 Тогда 
		...
		Если НЕ Объект.ОбменТоварами
			...
			Сообщение = НСтр("ru='Для продолжения настроек должен быть выбран хотя бы один из режимов обмена данными!';uk='Для продовження налаштувань повинен бути обраний хоча б один з режимів обміну даними!'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение,,,, Отказ);
	...
	ИначеЕсли ТекущийНомерСтраницы = 3 Тогда 
		...
		Если Объект.ВыгружатьНаСайт Тогда 
			...
			Если ПустаяСтрока(Объект.АдресСайта) Тогда 
				...
				Сообщение = НСтр("ru='Укажите адрес сайта';uk='Вкажіть адресу сайту'");
			...
			Если ПустаяСтрока(Объект.ИмяПользователя) Тогда 
				...
				Сообщение = НСтр("ru='Укажите имя пользователя';uk='Вкажіть ім`я користувача'");
		...
		Иначе
			...
			Если ПустаяСтрока(Объект.КаталогВыгрузки) Тогда 
				...
				Сообщение = НСтр("ru='Укажите каталог выгрузки';uk='Вкажіть каталог вивантаження'");
			...
			Если Объект.ОбменЗаказами И ПустаяСтрока(Объект.КаталогЗагрузки) Тогда 
				...
				Сообщение = НСтр("ru='Укажите каталог загрузки';uk='Вкажіть каталог завантаження'");
	...
	ИначеЕсли ТекущийНомерСтраницы = 5 Тогда 
		...
		Если Объект.ОбменЗаказами Тогда
			...
			Если НЕ ЗначениеЗаполнено(Объект.СпособИдентификацииКонтрагентов) Тогда
				...
				Сообщение = НСтр("ru='Укажите способ идентификации контрагентов';uk='Вкажіть спосіб ідентифікації контрагентів'");
			...
			ИначеЕсли НеСоздаватьКонтрагентовПриЗагрузкеЗаказа 
				...
				Сообщение = НСтр("ru='Выберите контрагента для подстановки в заказы';uk='Виберіть контрагента для підстановки в замовлення'");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ РольДоступнаДобавлениеИзменениеОбменовДанными Тогда
		...
		ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ru='Недостаточно прав доступа!';uk='Недостатньо прав доступу!'"), Отказ);
		Возврат;
	...
	Если НЕ Параметры.Свойство("ВызовИзПланаОбмена") Тогда
		...
		Сообщение = НСтр("ru='Работа помощника поддерживается только при вызове из командного интерфейса! Работа помощника завершена.';uk='Робота помічника підтримується тільки при виклику з командного інтерфейсу! Робота помічника завершена.'");
	...
	Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Элементы.НастроитьРасписаниеРегламентногоЗадания.Заголовок = НСтр("ru='Расписание не задано';uk='Розклад не заданий'");
	Иначе
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Отменить настройку обмена данными с web-сайтом и выйти из помощника?';uk='Скасувати налаштування обміну даними з web-сайтом і вийти з помічника?'"), РежимДиалогаВопрос.ДаНет);
