...
&НаКлиенте
Процедура ПереходКОтчету6НДФЛ(Элемент)
	...
	Элементы.СтраницаДекларация.ТекущаяСтраница = Элементы.Отчет6НДФЛ;
...
Процедура ПереходВыполнил(Команда)
	...
	ПоказатьВопрос(оп, НСтр("ru='ИФНС приняла отчет 6-НДФЛ?';uk='ИФНС приняла отчет 6-НДФЛ?'"), РежимДиалогаВопрос.ДаНет);
	Возврат;
...
&НаКлиенте
Процедура Открыть6НДФЛ(Команда)
...
Процедура ОтправитьОтчетность(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ru='Вы подтверждаете, что отчет 6-НДФЛ заполнен полностью и корректно?';uk='Вы подтверждаете, что отчет 6-НДФЛ заполнен полностью и корректно?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура УстановитьТекущуюЗакладку(ЭтаФорма, Объект)
	...
	ИначеЕсли Объект.СостояниеСобытия = ПредопределенноеЗначение("Перечисление.СостоянияСобытийКалендаря.Распечатать")
		...
		ЭтаФорма.Элементы.СтраницаДекларация.ТекущаяСтраница = ЭтаФорма.Элементы.Отчет6НДФЛ;
