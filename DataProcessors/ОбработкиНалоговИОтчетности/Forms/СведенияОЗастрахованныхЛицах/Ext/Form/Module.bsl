...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(ДокументОтчетности) Тогда
		...
		Иначе
			ДокументОтчетности = РегламентированнаяОтчетностьУСН.ПолучитьСЗВМПоСобытиюКалендаря(Объект.Организация, Объект.СобытиеКалендаря);
		КонецЕсли;
	КонецЕсли;
...
Процедура СохранитьФайлВыгрузки(Команда)
	...
	СтруктураФайла = ПолучитьИмяИАдресФайла(ДокументОтчетности);
	ПолучитьФайл(СтруктураФайла.АдресФайла, СтруктураФайла.ИмяФайла+".xml", Истина);
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПолучитьИмяИАдресФайла(ДокументОтчетности)
...
Процедура ПереходСЗВМ(Элемент)
	...
	Элементы.СтраницаСЗВМ.ТекущаяСтраница = Элементы.СведенияОЗастрахованныхЛицах;
...
Процедура ПереходВыполнил(Команда)
	...
	ПоказатьВопрос(оп, НСтр("ru='ПФР принял СЗВ-М?';uk='ПФР принял СЗВ-М?'"), РежимДиалогаВопрос.ДаНет);
	Возврат;
...
Процедура ОповещениеПодтверждениеПриема(Ответ, Параметры) Экспорт
	...
	Элементы.СтраницаСЗВМ.ТекущаяСтраница = Элементы.ЗадачаВыполнена;
	УстановитьЗаголовокФормыПоТекущейСтранице();
...
Процедура ОтправитьОтчетность(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ru='Вы подтверждаете, что СЗВ-М заполнены полностью и корректно?';uk='Вы подтверждаете, что СЗВ-М заполнены полностью и корректно?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ПриОтветеНаВопросОЗаполнении(РезультатВопроса, ДопПараметры) Экспорт
	...
	Если ИспользуетсяДКО Тогда
		КонтекстЭДОКлиент.ОтправкаРегламентированногоОтчетаВПФР(ДокументОтчетности, ОписаниеОповещения);
	КонецЕсли;
...
Процедура ОтправкаВКонтролирующийОрганПослеОтправки(РезультатОтправки, ДополнительныеПараметры) Экспорт
	...
	Если РезультатОтправки Тогда
		...
		Элементы.СтраницаСЗВМ.ТекущаяСтраница = Элементы.ЗадачаОжидатьПодтверждения;
		УстановитьЗаголовокФормыПоТекущейСтранице();
	КонецЕсли;
...
&НаКлиенте
Процедура РаспечататьСЗВМ(Команда)
	
	МассивОбъектов = Новый Массив;
	МассивОбъектов.Добавить(ДокументОтчетности);
	
	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.СведенияОЗастрахованныхЛицахСЗВ_М", "ФормаСВЗ_М", МассивОбъектов, ЭтаФорма, Новый Структура());
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьСЗВМ(Команда)
...
Процедура УстановитьЗаголовокФормыПоТекущейСтранице()
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Элементы.СтраницаСЗВМ.ТекущаяСтраница.Заголовок, ПериодЗадачиПредставление);
...
Процедура УстановитьТекущуюЗакладку(ЭтаФорма, Объект)
	...
	Если Объект.СостояниеСобытия = ПредопределенноеЗначение("Перечисление.СостоянияСобытийКалендаря.ОтчетНеСдан") Тогда
		...
		ЭтаФорма.Элементы.СтраницаСЗВМ.ТекущаяСтраница = ЭтаФорма.Элементы.ЗадачаОзнакомиться;
	...
	ИначеЕсли Объект.СостояниеСобытия = ПредопределенноеЗначение("Перечисление.СостоянияСобытийКалендаря.Распечатать")
		...
		ЭтаФорма.Элементы.СтраницаСЗВМ.ТекущаяСтраница = ЭтаФорма.Элементы.СведенияОЗастрахованныхЛицах;
	...
	ИначеЕсли Объект.СостояниеСобытия = ПредопределенноеЗначение("Перечисление.СостоянияСобытийКалендаря.Завершить") Тогда
		...
		ЭтаФорма.Элементы.СтраницаСЗВМ.ТекущаяСтраница = ЭтаФорма.Элементы.ЗадачаПроверена;
	...
	ИначеЕсли Объект.СостояниеСобытия = ПредопределенноеЗначение("Перечисление.СостоянияСобытийКалендаря.Завершено") Тогда
		...
		ЭтаФорма.Элементы.СтраницаСЗВМ.ТекущаяСтраница = ЭтаФорма.Элементы.ЗадачаВыполнена;
	...
	ИначеЕсли Объект.СостояниеСобытия = ПредопределенноеЗначение("Перечисление.СостоянияСобытийКалендаря.ПолучитьПодтверждение") Тогда
		...
		ЭтаФорма.Элементы.СтраницаСЗВМ.ТекущаяСтраница = ЭтаФорма.Элементы.ЗадачаОжидатьПодтверждения;
