...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Год(ДатаДокументаОбработкиСобытия) > 2013 Тогда
		Элементы.ДекорацияПФРСтраховая.Заголовок = НСтр("ru='ПФР';uk='ПФР'");
		Элементы.ДекорацияИтого.Заголовок = НСтр("ru='Итого (2-в-1):';uk='Итого (2-в-1):'");
...
Процедура ОтразитьРасходыВПрограмме(Команда)
	...
	Список.Добавить(ДокументВзаиморасчетовСБюджетомФФОМС,НСтр("ru='ФФОМС';uk='ФФОМС'"));
	
	Если Год(ДатаДокументаОбработкиСобытия) < 2014 Тогда
		Список.Добавить(ДокументВзаиморасчетовСБюджетомПФРСтраховая,НСтр("ru='ПФР страховая';uk='ПФР страхова'"));
		Список.Добавить(ДокументВзаиморасчетовСБюджетомПФРНакопительная,НСтр("ru='ПФР накопительная';uk='ПФР накопичувальна'"));
	Иначе
		Список.Добавить(ДокументВзаиморасчетовСБюджетомПФРСтраховая,НСтр("ru='ПФР';uk='ПФР'"));
	КонецЕсли;
...
"Создать его автоматически?';uk='У довіднику «Контрагенти» не задане відділення ПФ РФ для сплати внеску."
...
"пользователей. Подключиться сейчас?';uk='Для автоматичного створення відділення ПФ РФ"
...
Процедура ЗаполнитьСведенияОбОтделенииПФРПоКоду(ОписаниеОшибки = "", КодНалоговогоОрганаПолучателя)
	...
	РеквизитыОрганаПФР = ДанныеГосударственныхОрганов.РеквизитыОтделенияПФРПоКоду(КодНалоговогоОрганаПолучателя);
