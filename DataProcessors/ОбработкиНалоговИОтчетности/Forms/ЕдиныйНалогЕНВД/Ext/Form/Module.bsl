...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПериодЗадачиПредставление = ПредставлениеПериода(
		...
		"ФП=Истина");
	ПериодЗадачиПредставление = Лев(ПериодЗадачиПредставление, 1)+ Нстр("ru='-й';uk='-й'") + Прав(ПериодЗадачиПредставление, СтрДлина(ПериодЗадачиПредставление)-1);
...
Процедура ОтразитьРасходыВПрограмме(Команда)
	...
	Иначе
		...
		ПоказатьВводДаты(оп, ДатаУплаты, НСтр("ru='Когда был уплачен ЕНВД?';uk='Коли був сплачений ЄП?'"));
		Возврат;
		...
		ИначеЕсли Выборка.ПоказательОтчетности = ПланыВидовХарактеристик.ПоказателиОтчетности.хзТаблицаЕНВД Тогда
			...
			Если Табличка = Неопределено Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru='ЕНВД.Заполнение задачи';uk='ЄП.Заповнення завдання'"),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
					НСтр("ru='Не удалось получить данные для ТаблицаЕНВД';uk='Не вдалося одержати дані для ТаблицаЕНВД'"));
			Иначе
		...
		ИначеЕсли Выборка.ПоказательОтчетности = ПланыВидовХарактеристик.ПоказателиОтчетности.хзТаблицаЕНВДПоОкато Тогда
			...
			Если Табличка = Неопределено Тогда
				ЗаписьЖурналаРегистрации(
					...
					,
					,
					НСтр("ru='Не удалось получить данные для ТаблицаЕНВДПоОкато';uk='Не вдалося одержати дані для ТаблицаЕНВДПоОкато'"));
			Иначе
...
Процедура СверстатьФорму()
	...
	Для м = 0 по ТаблицаЕНВДПоОкато.Количество()-1 Цикл
		...
		реквизит = Новый РеквизитФормы(
			...
			Новый ОписаниеТипов("Строка"),
			,
			НСтр("ru='Сумма налога';uk='Сума податку'"));
		...
		реквизит = Новый РеквизитФормы(
			...
			Новый ОписаниеТипов("Строка"),
			,
			НСтр("ru='Страховые взносы';uk='Страхові внески'"));
