...
Процедура ОтразитьРасходыВПрограмме(Команда)
	...
	оп = Новый ОписаниеОповещения("ОповещениеВыборДаты", ЭтотОбъект);
	ПоказатьВводДаты(оп, ДатаУплаты, НСтр("ru='Когда был уплачен налог?';uk='Коли був сплачений податок?'"), ЧастиДаты.Дата);
	
КонецПроцедуры

&НаКлиенте
Процедура ОповещениеВыборДаты(ВыбраннаяДата, Параметры) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru='Расходы зарегистрированы';uk='Витрати зареєстровані'"),
		ПолучитьНавигационнуюСсылку(Ссылка),
		НСтр("ru='Документ регистрации расходов на налоги';uk='Документ реєстрації витрат на податки'"),
...
Процедура СуммаРасходовЗаГодЕННажатие(Элемент, СтандартнаяОбработка)
	...
	Если СтавкаНалогаЕН = "1%" Тогда
		...
		СпсКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Показать расходы';uk='Показати витрати'"));
		СпсКнопок.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Закрыть';uk='Закрити'"));
...
"Доходы * 1%.';uk='При розрахунках податку по ставці в 1% витрати не враховуються."
"Ставка в 1% використовується, коли податок, обчислений по формулі"
"(Доходи – Витрати) * 15%,"
"менше, чим податок, обчислений по формулі"
"Доходи * 1%.'"),
			СпсКнопок);
		Возврат;
...
"В этом случае расходы не учитываются при расчете налога.';uk='Ви використовуєте об`єкт оподаткування «Доходи»."
"В цьому випадку витрати не враховуються при розрахунках податку.'"),
			СпсКнопок);
...
"чем налог, исчисленный по формуле Доходы * 1%.';uk='Ставка в 1% використовується, коли податок обчислений по формулі (Доходи – Витрати) * 15% менше,"
"чим податок, обчислений по формулі Доходи * 1%.'"));
	Иначе
...
Процедура СуммаСтраховыхВзносовЕННажатие(Элемент, СтандартнаяОбработка)
	...
	Если ОбъектНалогообложения = ПредопределенноеЗначение("Перечисление.ВидыОбъектовНалогообложения.ДоходыМинусРасходы") Тогда
		...
		СпсКнопок = Новый СписокЗначений;
		СпсКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Показать страховые взносы';uk='Показати страхові внески'"));
		СпсКнопок.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Закрыть';uk='Закрити'"));
...
"В этом случае страховые взносы включены в состав расходов.';uk='Ви використовуєте об`єкт оподаткування «Доходи, зменшені на величину витрат»."
"В цьому випадку страхові внески включені до складу витрат.'"),
			СпсКнопок);
		Возврат;
...
"В этом случае сумма страховых взносов вычитается не полностью.';uk='Сума страхових внесків перевищила суму податку."
"У цьому випадку сума страхових внесків віднімається не повністю.'"),
				СпсКнопок);
		Иначе
			ПоказатьВопрос(
				оп,
				НСтр("ru='Сумма страховых взносов превысила 50% от суммы налога."
"В этом случае сумма налога уменьшается на 50%.';uk='Сума страхових внесків перевищила 50% від суми податку."
"У цьому випадку сума податку зменшується на 50%.'"),
				СпсКнопок);
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура РаспечататьПлатежноеПоручение(Команда)
	...
	Если ВсегоКУплатеЕН = "0,00 р." Тогда
		ПоказатьПредупреждение(,НСтр("ru='Сумма нулевая. Уплата не требуется.';uk='Сума нульова. Сплата не потрібна.'"));
		Возврат;
	КонецЕсли;
...
"Вы уверены, что все выполнено?';uk='Платіжки не створювалися. Витрати через касу не зареєстровані."
"Ви впевнені, що все виконано?'"),
...
Процедура ПолучитьДанныеОтчетности()
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.КодОтчетности = "СтавкаНалогаЕН" Тогда
...
Функция ПолучитьИмяЭлементаКнопкиПоУмолчанию(ИмяСтраницы)
	...
	ИначеЕсли ИмяСтраницы = "ЗадачаВыполнена" Тогда
		Возврат "";
	Иначе
		ВызватьИсключение НСтр("ru='Неизвестное имя текущей страницы';uk='Невідоме ім`я поточної сторінки'");
	КонецЕсли;
...
Процедура РаспечататьКвитанциюЗавершение() Экспорт
	...
	Если СтруктураПечати.АдресФайла = Неопределено Тогда
		...
		Сообщить(Нстр("ru='Произошла ошибка при выгрузке документа ';uk='Відбулася помилка при вивантаженні документа'")+Объект);
		Возврат;
...
"Создать его автоматически?';uk='У довіднику «Контрагенти» не заданий податковий орган для сплати податку."
"Створити його автоматично?'");
		ОписаниеОповещения = Новый ОписаниеОповещения("ПриОтветеНаВопросОСозданииГосОргана",
			ЭтаФорма, ДопПараметры);
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Создать автоматически';uk='Створити автоматично'"));
		Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Создать вручную';uk='Створити вручну'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru='Отмена';uk='Відмінити'"));
		
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки,,КодВозвратаДиалога.Да, НСтр("ru='Отсутствует налоговый орган';uk='Відсутній податковий орган'"));
...
"пользователей. Подключиться сейчас?';uk='Для автоматичного створення податкового органа"
"в довіднику «Контрагенти» необхідно підключитися до Інтернет-підтримки"
"користувачів. Підключитися зараз?'");
...
Функция ВыгрузитьДокументВФайл(Объект, ВидФайла)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Ошибка сохранения файла на сервере';uk='Помилка збереження файлу на сервері'"),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='При сохранении файла на сервере в области данных %1 произошла ошибка:"
"%2';uk=' При збереженні файлу на сервері в області даних %1 відбулася помилка:"
" %2"
"'"),
