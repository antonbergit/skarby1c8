...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ДокументОтчетности) И ЗначениеЗаполнено(ДокументОтчетности.ДатаОкончания) И Реквизиты.ДатаОкончания >= '20140101' И Реквизиты.ВыбраннаяФорма = "ФормаОтчета2009Кв1" Тогда
...
Процедура ПереходДекларацияПоУСН(Элемент)
	...
	Элементы.СтраницаДекларация.ТекущаяСтраница = Элементы.ДекларацияПоУСН;
...
&НаКлиенте
Процедура ОповещениеПриемаДекларации(Ответ, Параметры) Экспорт
...
&НаКлиенте
Процедура ОповещениеДекларацияЗаполнена(Ответ, Параметры) Экспорт
...
Процедура ДекорацияОписаниеРазеделовНажатие(Элемент)
	...
	ПараметрыФормы.Вставить("ИмяМакета","ОписаниеПорядкаЗаполненияДекларацииПоУСН");
	ПараметрыФормы.Вставить("Заголовок",НСтр("ru='Декларация по УСН';uk='Декларація по ССО'"));
...
Процедура УстановитьТекущуюЗакладку(ЭтаФорма, Объект)
	...
	ИначеЕсли Объект.СостояниеСобытия = ПредопределенноеЗначение("Перечисление.СостоянияСобытийКалендаря.Распечатать")
		...
		ЭтаФорма.Элементы.СтраницаДекларация.ТекущаяСтраница = ЭтаФорма.Элементы.ДекларацияПоУСН;
		ЭтаФорма.Элементы.ПереходВыполнил.КнопкаПоУмолчанию = Истина;
		ЭтаФорма.Элементы.Далее.КнопкаПоУмолчанию = Ложь;
