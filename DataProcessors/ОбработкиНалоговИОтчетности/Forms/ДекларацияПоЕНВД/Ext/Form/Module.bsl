...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Элементы.ГруппаПроверкиОтвета.Видимость = НЕ ОбщегоНазначенияПовтИсп.РазделениеВключено();
	...
	Если ЗначениеЗаполнено(ДокументОтчетности) И (Реквизиты.ДатаОкончания >= '20150101' И Реквизиты.ВыбраннаяФорма = "ФормаОтчета2013Кв4"
...
&НаКлиенте
Процедура ПереходДекларацияПоЕНВД(Элемент)
	...
	Элементы.СтраницаДекларация.ТекущаяСтраница = Элементы.ДекларацияПоЕНВД;
...
Процедура ПереходВыполнил(Команда)
	...
	оп  = Новый ОписаниеОповещения("ОповещениеПриемаДекларации", ЭтотОбъект);
	
	ПоказатьВопрос(оп, НСтр("ru='ИФНС приняла декларацию?';uk='ІФПС прийняла декларацію?'"), РежимДиалогаВопрос.ДаНет);
	Возврат;
	
КонецПроцедуры

&НаКлиенте
Процедура ОповещениеПриемаДекларации(Ответ, ЭтотОбъект) Экспорт
...
Процедура ОтправитьОтчетность(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ru='Вы подтверждаете, что Декларация заполнена полностью и корректно?';uk='Ви підтверджуєте, що Декларація заповнена повністю й коректно?'"), РежимДиалогаВопрос.ДаНет);
...
&НаКлиенте
Процедура ДекорацияОписаниеРазеделовНажатие(Элемент)
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ИзМакета", Истина);
	ПараметрыФормы.Вставить("ПутьДоМакета", "Обработка.ОбработкиНалоговИОтчетности");
	ПараметрыФормы.Вставить("ИмяМакета","ОписаниеПорядкаЗаполненияДекларацииПоЕНВД");
	ПараметрыФормы.Вставить("Заголовок",НСтр("ru='Декларация по ЕНВД';uk='Декларація по ЄП'"));
...
Процедура УстановитьТекущуюЗакладку(ЭтаФорма, Объект)
	...
	ИначеЕсли Объект.СостояниеСобытия = ПредопределенноеЗначение("Перечисление.СостоянияСобытийКалендаря.Распечатать")
		...
		ЭтаФорма.Элементы.СтраницаДекларация.ТекущаяСтраница = ЭтаФорма.Элементы.ДекларацияПоЕНВД;
		ЭтаФорма.Элементы.ПереходВыполнил.КнопкаПоУмолчанию = Истина;
		ЭтаФорма.Элементы.Далее.КнопкаПоУмолчанию = Ложь;
