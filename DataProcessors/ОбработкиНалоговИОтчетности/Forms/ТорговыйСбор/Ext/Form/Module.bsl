...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		Иначе
			Сообщить(НСтр("ru='Не выбрана текущая организация';uk='Не выбрана текущая организация'"));
			Отказ = Истина;
			Возврат;
			//Объект.Организация = ПолучениеУчетныхДанныхПовтИсп.ПолучитьТекущуюОрганизацию();
		КонецЕсли;
...
Процедура ОтразитьРасходыВПрограмме(Команда)
	...
	Если ТаблицаТорговыйСбор.Количество() > 1 Тогда
		...
		Для Каждого Строка Из ТаблицаТорговыйСбор Цикл
			Список.Добавить(Строка.ДокументВзаиморасчетовСБюджетом,НСтр("ru='ОКАТО: ';uk='КОАТУУ:'")+Строка.ОКАТО);
		КонецЦикла;
	...
	Иначе
		...
		ПоказатьВводДаты(оп, ДатаУплаты, НСтр("ru='Когда был уплачен торговый сбор?';uk='Когда был уплачен торговый сбор?'"));
		Возврат;
...
Процедура ОповещениеВыбораДаты(ВыбраннаяДата, Параметры) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru='Расходы зарегестрированы';uk='Витрати зареєстровані'"),
		...
		Если Выборка.ПоказательОтчетности = ПланыВидовХарактеристик.ПоказателиОтчетности.хзТаблицаТорговыхТочек Тогда
			...
			Если Табличка = Неопределено Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru='ТорговыйСбор.Заполнение задачи';uk='ТорговыйСбор.Заполнение задачи'"),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
					НСтр("ru='Не удалось получить данные для ТаблицаТорговыйСбор';uk='Не удалось получить данные для ТаблицаТорговыйСбор'"));
			Иначе
				...
				Для Каждого стр из Табличка Цикл
					...
					Если СсылкиИФНС.Получить(Стр.КодИФНС) = Неопределено Тогда
						Если ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() Тогда
							ЗаписьЖурналаРегистрации(НСтр("ru='Не загружены классификаторы налоговых органов';uk='Не загружены классификаторы налоговых органов'"),УровеньЖурналаРегистрации.Ошибка,Метаданные.Справочники.НалоговыеОрганы, , НСтр("ru='Необходимо выполнить загрузку налоговых органов';uk='Необходимо выполнить загрузку налоговых органов'"));
						Иначе
							Сообщить(НСтр("ru='Не загружены классификаторы налоговых органов';uk='Не загружены классификаторы налоговых органов'"));
						КонецЕсли;
					Иначе
...
Процедура НайтиДокументОплаты()
	...
	//|ИЗ
	//|	Документ.РасходДенежныхСредствИзКассы КАК РасходДенежныхСредствИзКассы
	//|ГДЕ
	...
	//Иначе
	//	СсылкаДокументаНаличнойУплаты = Документы.РасходДенежныхСредствИзКассы.ПустаяСсылка();
	//КонецЕсли;
...
Процедура ОбновитьДанныеРегистрацииРасходов()
	Если ТаблицаТорговыйСбор.Количество() > 1 Тогда
		Элементы.ОтразитьРасходыВПрограмме.Заголовок = НСтр("ru='Зарегистрировать расходы в программе';uk='Зареєструвати витрати в програмі'");
	Иначе
...
Функция ВыгрузитьДокументВФайл(МассивОбъектов, ВидФайла)
	...
	ПризнакОшибки = Ложь;
	РасширениеФайла = ".pdf" ;
	НаименованиеФайла = Строка(?(МассивОбъектов.Количество() > 0,МассивОбъектов[0], НСтр("ru='Результат';uk='Результат'")));
