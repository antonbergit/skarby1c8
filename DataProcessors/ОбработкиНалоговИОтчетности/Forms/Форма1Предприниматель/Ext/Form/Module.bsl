...
Процедура РаспечататьДекларацию(Команда)
	...
	ПараметрыФормыДокумента = ПолучитьДанныеОткрытияФормыФСГС(ДокументОтчетности);
	...
	Форма = ПолучитьФорму("Отчет.РегламентированныйОтчетСтатистикаФорма1Предприниматель.Форма."+ПараметрыФормыДокумента.ВыбраннаяФорма, ПараметрыФормы);
	Форма.Печать("ПоказатьБланк");
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПолучитьДанныеОткрытияФормыФСГС(ДокументОтчетности)
...
Процедура ОшибкиСдачи(Команда)
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ПоказатьПротоколОтправки(ДокументОтчетности, "ФСГС");
КонецПроцедуры
...
&НаКлиенте
Процедура ПереходКФорме(Элемент)
	...
	Элементы.СтраницаДекларация.ТекущаяСтраница = Элементы.Форма1Предприниматель;
...
Процедура ПереходВыполнил(Команда)
	...
	ПоказатьВопрос(оп, НСтр("ru='Росстат принял форму №1-предприниматель?';uk='Росстат принял форму №1-предприниматель?'"), РежимДиалогаВопрос.ДаНет);
	Возврат;
...
Процедура ОтправитьОтчетность(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ru='Вы подтверждаете, что отчет по форме №1-предприниматель заполнен полностью и корректно?';uk='Вы подтверждаете, что отчет по форме №1-предприниматель заполнен полностью и корректно?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ПриОтветеНаВопросОЗаполнении(РезультатВопроса, ДопПараметры) Экспорт
	...
	Если ИспользуетсяДКО Тогда
		КонтекстЭДОКлиент.ОтправкаРегламентированногоОтчетаВФСГС(ДокументОтчетности, ОписаниеОповещения);
	КонецЕсли;
...
Процедура УстановитьТекущуюЗакладку(ЭтаФорма, Объект)
	...
	ИначеЕсли Объект.СостояниеСобытия = ПредопределенноеЗначение("Перечисление.СостоянияСобытийКалендаря.Распечатать")
		...
		ЭтаФорма.Элементы.СтраницаДекларация.ТекущаяСтраница = ЭтаФорма.Элементы.Форма1Предприниматель;
