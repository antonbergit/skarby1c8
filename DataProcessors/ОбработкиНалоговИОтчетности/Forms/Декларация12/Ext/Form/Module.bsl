...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ИзменениеСостоянияСобытияКалендаря" Тогда
		Если Параметр = Объект.СобытиеКалендаря
			...
			Закрыть();
		КонецЕсли;
	ИначеЕсли ИмяСобытия = "Запись_ОтправкиФСРАР" Тогда
		Если ЗначениеЗаполнено(Источник) Тогда
			Если ТипЗнч(Параметр) = Тип("Структура") И Параметр.Свойство("Ссылка") И Параметр.Ссылка = ДокументОтчетности Тогда
				ОтправитьОтчетностьНаСервере();
				Элементы.СтраницаДекларация.ТекущаяСтраница = Элементы.ЗадачаОжидатьПодтверждения;
				УстановитьЗаголовокФормыПоТекущейСтранице();
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли ИмяСобытия = "Завершение автообмена с сервером ФСРАР" Тогда
...
&НаКлиенте
Процедура ПереходКДекларации12(Элемент)
	...
	Элементы.СтраницаДекларация.ТекущаяСтраница = Элементы.АлкогольнаяДекларация12;
...
Процедура ПереходВыполнил(Команда)
	...
	ПоказатьВопрос(оп, НСтр("ru='ФСРАР приняла Декларацию №12?';uk='ФСРАР приняла Декларацию №12?'"), РежимДиалогаВопрос.ДаНет);
	Возврат;
...
Процедура ОтправитьОтчетность(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ru='Вы подтверждаете, что Декларация №12 заполнена полностью и корректно?';uk='Вы подтверждаете, что Декларация №12 заполнена полностью и корректно?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ПриОтветеНаВопросОЗаполнении(РезультатВопроса, ДопПараметры) Экспорт
	...
	Если ИспользуетсяДКО Тогда
		КонтекстЭДОКлиент.ПриНажатииНаКнопкуОтправкиВКонтролирующийОрганДляФСРАР(ЭтаФорма, , Истина, ДокументОтчетности, Объект.Организация);
	КонецЕсли;
...
Процедура УстановитьТекущуюЗакладку(ЭтаФорма, Объект)
	...
	ИначеЕсли Объект.СостояниеСобытия = ПредопределенноеЗначение("Перечисление.СостоянияСобытийКалендаря.Распечатать")
		...
		ЭтаФорма.Элементы.СтраницаДекларация.ТекущаяСтраница = ЭтаФорма.Элементы.АлкогольнаяДекларация12;
