...
Процедура ПеренестиВАрхив(Команда)
	...
	ТекстВопроса = Нстр("ru='Задача будет перенесена в архив и удалена из списка актуальных событий. Перенести в архив?';uk='Завдання буде перенесено в архів і видалене зі списку актуальних подій. Перенести в архів?'");
...
Процедура ОбработатьПараметрСобытияКалендаря()
	...
	Если СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ЕдиныйНалог Тогда
		...
		Заголовок = НСтр("ru='Единый налог за ';uk='Єдиний податок за'") 
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Рассчитать Единый налог »';uk='Розрахувати Єдиний податок »'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.АвансовыйПлатежПоУСН Тогда
		...
		Заголовок = НСтр("ru='Авансовый платеж по УСН за ';uk='Авансовий платіж по ССО за'") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Рассчитать Авансовый платеж »';uk='Розрахувати Авансовий платіж »'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ДекларацияПоУСН Тогда
		...
		Заголовок = НСтр("ru='Декларация по УСН за ';uk='Декларація по ССО за'") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Сформировать Декларацию »';uk='Сформувати Декларацію »'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СтраховыеВзносыИП Тогда
		...
		Заголовок = НСтр("ru='Страховые взносы индивидуального предпринимателя за ';uk='Страхові внески індивідуального підприємця за'") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Рассчитать Страховые взносы »';uk='Розрахувати Страхові внески »'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ДекларацияПоЕНВД Тогда
		...
		Заголовок = НСтр("ru='Декларация по ЕНВД ';uk='Декларація по ЄП'") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ДекорацияТекстОписание20.Заголовок = 
			Новый ФорматированнаяСтрока(Новый ФорматированнаяСтрока(НСтр("ru='Заполните';uk='Заповніть'"),,,,"ЕНВД"),
			НСтр("ru=' сведения по ЕНВД за ';uk=' відомості про єдиний податок за'"),
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ЕдиныйНалогЕНВД Тогда
		...
		Заголовок = НСтр("ru='Уплата ЕНВД за ';uk='Сплата ЄП за'") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Рассчитать и уплатить ЕНВД »';uk='Розрахувати й сплатити ЄП »'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СреднесписочнаяЧисленность Тогда
		...
		Заголовок = НСтр("ru='Сведения о среднесписочной численности за ';uk='Відомості про середньооблікову чисельність за'") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Сформировать и отправить Сведения о численности »';uk='Сформувати й відправити Відомості про чисельність »'");
		
	// Справки 2-НДФЛ
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Справки2НДФЛ Тогда
		
		Заголовок = НСтр("ru='Справки 2-НДФЛ по сотрудникам за ';uk='Довідки 2-ПДФО по співробітниках за'") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Сформировать справки 2-НДФЛ по сотрудникам »';uk='Сформувати довідки 2-ПДФО по співробітниках »'");
		
	// Отчетность в ПФР
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ОтчетностьВПФР Тогда
		
		Заголовок = НСтр("ru='Отчетность в ПФР за ';uk='Звітність у ПФР за'") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Сформировать пакет отчетности в ПФР »';uk='Сформувати пакет звітності в ПФР »'");
	// Налоги сотрудников
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.НалогиСотрудников Тогда
		
		Заголовок = НСтр("ru='Налоги сотрудников за ';uk='Податки співробітників за'") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Рассчитать налоги сотрудников »';uk='Розрахувати податки співробітників »'");
	// Форма 4-ФСС
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Форма4ФСС Тогда
		
		Заголовок = НСтр("ru='Отчет 4-ФСС ';uk='Звіт 4-ФСС'") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Сформировать отчет 4-ФСС »';uk='Сформувати звіт 4-ФСС »'");
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СтраховыеВзносыПриДоходахСвыше300тр Тогда
		...
		Заголовок = НСтр("ru='Страховые взносы индивидуального предпринимателя в ПФР при доходах свыше 300 000 рублей';uk='Страхові внески індивідуального підприємця в ПФР при доходах понад 300 000 гривень'") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.НалогПатент Тогда
		...
		Если НачалоДня(ДатаПервогоПлатежа) = НачалоДня(ПериодДокументовОкончание) Тогда
			Если Не ЗначениеЗаполнено(ДатаВторогоПлатежа) Тогда
				Заголовок = НСтр("ru='Налог с патента ';uk='Налог с патента '");
			Иначе
				Заголовок = НСтр("ru='1/3 налога с патента ';uk='1/3 налога с патента '");
			КонецЕсли;
		ИначеЕсли НачалоДня(ДатаВторогоПлатежа) = НачалоДня(ПериодДокументовОкончание) Тогда
			Заголовок = НСтр("ru='2/3 налога с патента ';uk='2/3 налога с патента '");
		Иначе
			Заголовок = НСтр("ru='Налог с патента ';uk='Налог с патента '");
		КонецЕсли;
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Заплатить налог с патента »';uk='Заплатить налог с патента »'");
	// Торговый сбор
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.ТорговыйСбор Тогда
		
		Заголовок = НСтр("ru='Торговый сбор за ';uk='Торговый сбор за '") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Уплатить торговый сбор »';uk='Уплатить торговый сбор »'");
		
	// №1-Предприниматель
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Форма1Предприниматель Тогда
		
		Заголовок = НСтр("ru='Форма №1-предприниматель';uk='Форма №1-предприниматель'");
		
		Элементы.ГруппаДекорацииОписанияФорма1Предприниматель.Видимость = Истина;
		
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Сформировать форму »';uk='Сформировать форму »'");
		
	// 6-НДФЛ
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Форма6НДФЛ Тогда
		
		Заголовок = НСтр("ru='Форма 6-НДФЛ по сотрудникам за ';uk='Форма 6-НДФЛ по сотрудникам за '") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Сформировать форму 6-НДФЛ »';uk='Сформировать форму 6-НДФЛ »'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.СведенияОЗастрахованныхЛицах Тогда
		...
		Заголовок = НСтр("ru='Сведения о застрахованных лицах (СЗВ-М) по сотрудникам за ';uk='Сведения о застрахованных лицах (СЗВ-М) по сотрудникам за '") 
					+ ПредставлениеПериода(
						ПериодДокументовНачало,
						ПериодДокументовОкончание,
						"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Сформировать СЗВ-М »';uk='Сформировать СЗВ-М »'");
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.Декларация12 Тогда
		...
		Заголовок = НСтр("ru='Декларация об объеме розничной продажи пива и пивных напитков (Приложение №12) за ';uk='Декларация об объеме розничной продажи пива и пивных напитков (Приложение №12) за '") 
			+ ПредставлениеПериода(
				ПериодДокументовНачало,
				ПериодДокументовОкончание,
				"ФП=Истина");
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Сформировать Декларацию»';uk='Сформировать Декларацию»'");
	КонецЕсли;
...
Функция ВыполнитьКонтроль()
	...
	Если РезультатПроверки.ЕстьОшибки Тогда
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Есть ошибки. Повторить проверку »';uk='Є помилки. Повторити перевірку »'");
	ИначеЕсли РезультатПроверки.ЕстьПредупреждения Тогда
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Игнорировать предупреждения »';uk='Ігнорувати попередження »'");
		Элементы.ПовторитьПроверку.Видимость = Истина;
	Иначе
		//Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Ошибок нет. Продолжить »';uk='Помилок немає. Продовжити »'");
	КонецЕсли;
...
Процедура ДекорацияТекстОписаниеТорговыйСборОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ВыполнитьПереходКДаннымТорговогоСбора(НСтр("ru='« Вернуться к уплате торогового сбора';uk='« Вернуться к уплате торогового сбора'"));
КонецПроцедуры
...
Процедура ДекорацияТекстОписание20ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ВыполнитьПереходКДаннымЕНВД(НСтр("ru='Декларации по ЕНВД';uk='Декларації по ЄП'"));
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияТекстОписаниеЕНВДНалог2ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ВыполнитьПереходКДаннымЕНВД(НСтр("ru='Уплаты ЕНВД';uk='Сплати ЄП'"));
КонецПроцедуры
