...
"рекомендуется увеличить объем памяти до %2 Гб.';uk='На комп`ютері встановлене %1 Гб оперативної пам`яті."
"Для того щоб програма працювала швидше, "
"рекомендується збільшити обсяг пам`яті до %2 Гб.'");
	
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ДоступныйОбъем, РекомендуемыйОбъем);
	
	ЗаголовокСообщения = НСтр("ru='Рекомендация по повышению скорости работы';uk='Рекомендація з підвищення швидкості роботи'");
