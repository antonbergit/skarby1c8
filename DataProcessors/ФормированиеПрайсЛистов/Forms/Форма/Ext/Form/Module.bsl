...
"Рекомендуется повторно сформировать прайс-лист.';uk='Можливо дані прайс-листа змінилися. "
"Рекомендується повторно сформувати прайс-лист.'");
...
Процедура ИсключитьЗаписьОЦене(ДанныеРасшифровки)
	...
	Иначе
		...
		ТекстСообщения = Нстр("ru='Для исключения позиции из прайс-листа необходимо указать номенклатуру.';uk='Для виключення позиції із прайс-листа необхідно вказати номенклатуру.'");
...
Процедура СформироватьПрайсЛистНаСервере(ДокументСформирован)
	...
	ЗаданиеРезультат	= ДлительныеОперации.ЗапуститьВыполнениеВФоне(УникальныйИдентификатор, "Обработки.ФормированиеПрайсЛистов.СформироватьПЛ", ПараметрыФЗ, НСтр("ru='Формирование прайс-листа.';uk='Формування прайс-листа.'"));
	...
	Иначе
		...
		УправлениеНебольшойФирмойСервер.СостояниеТабличногоДокументаДлительнаяОперация(Элементы.ТабличныйДокумент, НСтр("ru='Отчет формируется...';uk='Звіт формується...'"));
...
Функция ПоместитьТабличныеДокументыВоВременноеХранилище(НастройкиСохранения)
	...
	Если ЗаписьZipФайла <> Неопределено Тогда
		...
		ПутьВоВременномХранилище = ПоместитьВоВременноеХранилище(ДвоичныеДанные, ЭтотОбъект.УникальныйИдентификатор);
		ОписаниеФайла = Новый Структура;
		ОписаниеФайла.Вставить("Представление", Нстр("ru='Price.zip';uk='Price.zip'"));
		ОписаниеФайла.Вставить("АдресВоВременномХранилище", ПутьВоВременномХранилище);
		Результат.Добавить(ОписаниеФайла);
	КонецЕсли;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ВРег(ИсточникВыбора.ИмяФормы) = ВРег("ОбщаяФорма.ВыборФорматаВложений") Тогда
		...
		Если ВыбранноеЗначение <> Неопределено 
			...
			Тема = НСтр("ru='Прайс-лист ';uk='Прайс-лист'") + ?(ЗначениеЗаполнено(КэшЗначений.Организация), КэшЗначений.ОрганизацияНаименование, "");
			ПараметрыНовогоПисьма.Вставить("Тема", Тема);
			
			Тело = ?(ЗначениеЗаполнено(КэшЗначений.ВыводитьДатуФормирования), Нстр("ru='Сформирован ';uk='Сформований'") + Формат(КэшЗначений.ДатаФормирования, "ДЛФ=DD"), "");
