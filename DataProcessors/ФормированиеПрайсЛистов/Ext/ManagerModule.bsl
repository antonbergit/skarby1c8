...
Процедура ВывестиГруппуВТабличныйДокументПолотно(ГруппаИерархии, КоллекцияСтрок, НастройкиПрайсЛиста, ТабличныйДокумент)
	...
	ПредставлениеГруппы = ?(ЗначениеЗаполнено(ГруппаИерархии), ГруппаИерархии, НСтр("ru='<...>';uk='<...>'"));
		...
		Если НастройкиПрайсЛиста.СвободныйОстаток Тогда
			...
			Если НастройкиПрайсЛиста.ПредставлениеОстатков = 2 Тогда
				...
				Если Строка.НижняяГраницаОстатков = 0 
					...
					ОбластьСтрока.Параметры.СвободныйОстаток = НСтр("ru='Не указано';uk='Не зазначено'");
...
Функция СоздатьОбластьШапка(НастройкиПрайсЛиста, ТабличныйДокумент, МакетПолотно)
	...
	Для ИндексЦены = 1 По КоличествоЦен Цикл
		...
		ВыделениеОбласти.Объединить();
		ВыделениеОбласти.Текст = ?(НастройкиПрайсЛиста.НазваниеЦеныПредставлением, Строка(ТаблицаВидовЦен[ИндексЦены - 1].ВидЦен), Нстр("ru='Цена ';uk='Ціна'") + ИндексЦены);
		ВыделениеОбласти.РазмещениеТекста = ТипРазмещенияТекстаТабличногоДокумента.Переносить;
...
Процедура ЗаполнитьВВидеПолотноИерархияНоменклатуры(НастройкиПрайсЛиста, ТабличныйДокумент, ДеревоИерархии)
	...
	МакетПолотно.КодЯзыкаМакета = КодЯзыкаПечать;
	...
	Если ПолучитьФункциональнуюОпцию("УчетВалютныхОпераций") Тогда
		...
		ЗначенияПараметров.Очистить();
		ЗначенияПараметров.Вставить("ОписаниеВалюты", НСтр("ru='Валюта: ';uk='Валюта:'") + НастройкиПрайсЛиста.Валюта);
		Если ЗначениеЗаполнено(НастройкиПрайсЛиста.Валюта) 
			...
			ЗначенияПараметров.ОписаниеВалюты = ЗначенияПараметров.ОписаниеВалюты + НСтр("ru=', курс ';uk=', курс'") + КурсыВалюты.Курс;
			Если КурсыВалюты.Кратность <> 1 Тогда
				
				ЗначенияПараметров.ОписаниеВалюты = ЗначенияПараметров.ОписаниеВалюты + НСтр("ru=', кратность ';uk=', кратність'") + КурсыВалюты.Кратность;
	...
	Если НастройкиПрайсЛиста.ВыводитьКонтактнуюИнформацию Тогда
		...
		ЗначенияПараметров.Очистить();
		ЗначенияПараметров.Вставить("Организация",	НСтр("ru='Организация: ';uk='Організація: '") + НастройкиПрайсЛиста.Организация);
		ЗначенияПараметров.Вставить("Адрес", 		НСтр("ru='Адрес: ';uk='Адреса: '") + УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(НастройкиПрайсЛиста.Организация, Справочники.ВидыКонтактнойИнформации.ФактАдресОрганизации));
		ЗначенияПараметров.Вставить("Телефон", 		НСтр("ru='Телефон, факс: ';uk='Телефон, факс:'") + УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(НастройкиПрайсЛиста.Организация, Справочники.ВидыКонтактнойИнформации.ТелефонОрганизации) + ", " + УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(НастройкиПрайсЛиста.Организация, Справочники.ВидыКонтактнойИнформации.ФаксОрганизации));
		ЗначенияПараметров.Вставить("Email", 		НСтр("ru='E-mail: ';uk='E-mail: '") + УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(НастройкиПрайсЛиста.Организация, Справочники.ВидыКонтактнойИнформации.EmailОрганизации));
	...
	Если НастройкиПрайсЛиста.ВыводитьДатуФормирования Тогда
		...
		ЗначенияПараметров.Очистить();
		ДатаФормирования = Нстр("ru='Сформирован ';uk='Сформований'") + Формат(НастройкиПрайсЛиста.ДатаФормирования, "ДЛФ=DD");
...
Процедура ЗаполнитьВВидеДвеКолонкиИерархияНоменклатуры(НастройкиПрайсЛиста, ТабличныйДокумент, ДеревоИерархии)
	...
	МакетДвеКолонки.КодЯзыкаМакета = КодЯзыкаПечать;
...
Процедура ЗаполнитьВВидеДиафильмИерархияНоменклатуры(НастройкиПрайсЛиста, ТабличныйДокумент, ДеревоИерархии)
	...
	МакетДиафильм.КодЯзыкаМакета = КодЯзыкаПечать;
