...
Процедура ОчиститьАгрегатыПоРегистрам(Команда)
	...
	ТекстВопроса = НСтр("ru='Очистка агрегатов может привести к существенному замедлению отчетов.';uk='Очищення агрегатів може призвести до значного уповільнення "
"звітів.'");
...
Процедура ЗаполнитьАгрегатыИВыполнитьПерестроение(Команда)
	...
	Если МассивДействий.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Отсутствуют регистры, для которых можно выполнить выбранное действие.';uk='Відсутні регістри, для яких можна виконати обрану "
"дію.'"));
		Возврат;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Инструкция.Оформление.Вставить("Текст", НСтр("ru='Итоги';uk='Підсумки'"));
	...
	Инструкция.Оформление.Вставить("Текст", НСтр("ru='Агрегаты';uk='Агрегати'"));
	СтандартныеПодсистемыСервер.ДобавитьЭлементУсловногоОформления(ЭтотОбъект, Инструкция);
	...
	Инструкция.Оформление.Вставить("Текст", НСтр("ru='Только итоги';uk='Тільки підсумки'"));
	СтандартныеПодсистемыСервер.ДобавитьЭлементУсловногоОформления(ЭтотОбъект, Инструкция);
...
Функция ПолучитьОптимальныеАгрегатыСервер()
	...
	Для НомерСтроки = 1 По Всего Цикл
		...
		Если ИнформацияОбОшибке <> Неопределено Тогда
			...
			ЗаголовокОшибки = НСтр("ru='Не удалось получить оптимальные агрегаты регистра ""%1"":';uk='Не вдалося одержати оптимальні агрегати регістру ""%1"":'");
	...
	Если Всего = 1 Тогда
		...
		Если Результат.ЕстьОшибки Тогда
			Результат.ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось получить оптимальные агрегаты регистра накопления ""%1"".';uk='Не вдалося одержати оптимальні агрегати регістру накопичення ""%1"".'"), ИмяРегистра);
	...
	ИначеЕсли Успешно = 0 Тогда
		...
		Результат.ЕстьОшибки = Истина;
		Результат.ТекстСообщения = НСтр("ru='Не удалось получить оптимальные агрегаты регистров накопления.';uk='Не вдалося одержати оптимальні агрегати регістрів накопичення.'");
		Результат.ПодробныйТекстСообщения = ПодробныйТекстОшибок;
	ИначеЕсли Результат.ЕстьОшибки Тогда
		// Частично успешно.
		Результат.ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Агрегаты успешно получены для %1 из %2 регистров."
"Не получены: %3.';uk='Агрегати успішно отримані для %1 з %2 регістрів."
"Не отримані: %3.'"),
				Успешно,
				Всего,
				Всего - Успешно);
