...
Процедура ПерейтиСоСтраницыФайлаОбновления()
	...
	Если НуженФайлОбновления = 1 Тогда
		Если Не ЗначениеЗаполнено(Объект.ИмяФайлаОбновления) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите файл поставки обновления конфигурации.';uk='Вкажіть файл поставки оновлення конфігурації.'"),,"Объект.ИмяФайлаОбновления");
...
Функция ВозможенЗапускОбновления()
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		ВозможенЗапускОбновления = Ложь;
		ТекстСообщения = НСтр("ru='Обновление программы доступно только в клиенте под управлением ОС Windows.';uk='Оновлення програми доступне тільки в клієнтові під управлінням ОС Windows.'");
	КонецЕсли;
...
Процедура ПрочитатьДанныеВЖурналРегистрации()
	...
	Для НомерСтроки = 1 По ТекстовыйДокумент.КоличествоСтрок() Цикл
		...
		Если Сред(ТекущаяСтрока, 3, 1) = "." И Сред(ТекущаяСтрока, 6, 1) = "." Тогда // Строка с датой
			...
			Если Комментарий = НСтр("ru='Обновление выполнено';uk='Оновлення виконане'") Тогда
				Объект.РезультатОбновления = Истина;
				Продолжить;
			ИначеЕсли Комментарий = НСтр("ru='Обновление не выполнено';uk='Оновлення не виконане'") Тогда
