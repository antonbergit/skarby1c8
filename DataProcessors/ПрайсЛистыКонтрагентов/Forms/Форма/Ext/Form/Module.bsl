...
Процедура ОбновитьЗаголовокФормыНаСервере()
	...
	ЭтаФорма.Заголовок	= НСтр("ru='Прайс-листы поставщиков';uk='Прайс-листи постачалльників'") + 
		?(ЗначениеЗаполнено(НаДату), Нстр("ru=' на ';uk=' на '") + Формат(НаДату, "ДЛФ=DD"), Нстр("ru='.';uk='.'"));
...
"Возможно выбрана пустая ячейка.';uk='Неможливо скопіювати ціну."
"Можливо обрана порожня комірка.'")
					);
		Возврат;
	...
	ИначеЕсли РасшифровкаИзОбласти.Свойство("СоответствиеРасшифровок") Тогда
		...
		Если СписокДоступныхВидовЦен.Количество() < 1 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='В текущем прайс-листе для данной номенклатурной позиции нет цен, доступных для копирования.';uk='У поточному прайс-листі для даної номенклатурної позиції немає цін, доступних для копіювання.'")
						);
...
"Копирование не возможно.';uk='Можливо зазначена порожня комірка."
"Копіювання не можливе.'")
				);
...
"Возможно выбрана пустая ячейка.';uk='Неможливо змінити ціну."
"Можливо обрана порожня комірка.'")
					);
		Возврат;
	...
	ИначеЕсли РасшифровкаИзОбласти.Свойство("СоответствиеРасшифровок") Тогда
		...
		Если СписокДоступныхВидовЦен.Количество() < 1 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='В текущем прайс-листе для данной номенклатурной позиции нет цен, доступных для изменения.';uk='У поточному прайс-листі для даної номенклатурної позиції немає цін, доступних для зміни.'")
						);
...
"Возможно выбрана пустая ячейка.';uk='Неможливо видалити ціну."
"Можливо обрана порожня комірка.'")
					);
		Возврат;
	...
	ИначеЕсли РасшифровкаИзОбласти.Свойство("СоответствиеРасшифровок") Тогда
		...
		Если СписокДоступныхВидовЦен.Количество() < 1 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='В текущем прайс-листе для данной номенклатурной позиции нет цен, доступных для удаления.';uk='У поточному прайс-листі для даної номенклатурної позиції немає цін, доступних для видалення.'")
						);
...
Процедура История(Команда)
	...
	Если НЕ ТипЗнч(РасшифровкаИзОбласти) = Тип("Структура")
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Невозможно отобразить историю формирования цен.';uk='Неможливо відобразити історію формування цін.'")
					);
		Возврат;
	...
	ИначеЕсли РасшифровкаИзОбласти.Свойство("СоответствиеРасшифровок") Тогда
		...
		Если СписокДоступныхВидовЦен.Количество() < 1 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Невозможно отобразить историю формирорвания цен для данной номенклатурной позиции.';uk='Неможливо відобразити історію формування цін для даної номенклатурної позиції.'")
						);
