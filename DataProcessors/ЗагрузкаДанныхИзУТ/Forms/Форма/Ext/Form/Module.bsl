...
Процедура ЗагрузитьНаСервере(АдресВременногоХранилища)
	...
	УОД.ФлагРежимОтладкиОбработчиков = Ложь;
	УОД.ИмяФайлаВнешнейОбработкиОбработчиковСобытий = "ОбработчикиЗагрузкиИзУправлениеТорговлей";
...
Процедура ЗагрузитьЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Состояние(НСтр("ru='Загрузка данных...';uk='Завантаження даних...'"),, НСтр("ru='Выполняется загрузка данных из УТ 2.3';uk='Виконується завантаження даних з УТ 2.3'"));
...
Процедура ИмяФайлаОбменаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если РасширениеРаботыСФайламиПодключено Тогда
		...
		ДиалогОткрытияФайла.Заголовок = НСтр("ru='Выберите путь к файлу выгрузки данных из УТ';uk='Виберіть шлях до файлу вивантаження даних з УТ'");
