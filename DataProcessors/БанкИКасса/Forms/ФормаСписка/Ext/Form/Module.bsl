...
Процедура Подключаемый_ОбработатьАктивизациюСтрокиСписка()
	...
	Если ТекущаяСтрока <> Неопределено
		...
		Дата = НСтр("ru='Ведомость за ';uk='Відомість за '") + Формат(ТекущаяСтрока.Дата, "ДЛФ=D");
		НомерБанковскогоСчета = СтруктураДанные.НомерБанковскогоСчета;
		Если СтруктураДанные.УчетВалютныхОпераций Тогда
			НомерБанковскогоСчета = НомерБанковскогоСчета + " (" + ?(ЗначениеЗаполнено(ТекущаяСтрока.Валюта), ТекущаяСтрока.Валюта, НСтр("ru='<валюта пустая>';uk='<валюта порожня>'")) + ")";
		КонецЕсли;
	...
	Иначе
		...
		Дата = НСтр("ru='<Не выбран документ>';uk='<Не обраний документ>'");
...
Функция ПолучитьПредставлениеПериода(СтандартныйПериод, Разделитель = " по ")
	...
	Если Не ЗначениеЗаполнено(ДатаНачала) И Не ЗначениеЗаполнено(ДатаОкончания) Тогда
		Возврат НСтр("ru='Интервал: <За все время>';uk='Інтервал: <За весь час>'");
	ИначеЕсли СтандартныйПериод.Вариант <> ВариантСтандартногоПериода.ПроизвольныйПериод Тогда
		Возврат НСтр("ru='Интервал: <';uk='Інтервал: <'") + СтандартныйПериод.Вариант + ">";
	ИначеЕсли Год(ДатаНачала) = Год(ДатаОкончания) И Месяц(ДатаНачала) = Месяц(ДатаОкончания) Тогда
		Возврат НСтр("ru='Интервал: ';uk='Інтервал: '") + Формат(ДатаНачала, "ДФ=dd")+Разделитель+Формат(ДатаОкончания, "ДФ=dd.MM.yyyy");
	ИначеЕсли Год(ДатаНачала) = Год(ДатаОкончания) Тогда
		Возврат НСтр("ru='Интервал: ';uk='Інтервал: '") + Формат(ДатаНачала, "ДФ=dd.MM")+Разделитель+Формат(ДатаОкончания, "ДФ=dd.MM.yyyy");
	Иначе
		Возврат НСтр("ru='Интервал: ';uk='Інтервал: '") + Формат(ДатаНачала, "ДФ=dd.MM.yyyy")+Разделитель+Формат(ДатаОкончания, "ДФ=dd.MM.yyyy");
	КонецЕсли;
...
Процедура НастройкаИтоговЗавершениеНаСервере() Экспорт
	...
	Если ВариантОтображенияИтогов = ПредопределенноеЗначение("Перечисление.ВариантыОтображенияИтоговБанкИКасса.ВедомостьЗаДень") Тогда
		...
		Элементы.ИнформацияСуммаВалНачальныйОстаток.Заголовок = НСтр("ru='Начало дня';uk='Початок дня'");
		Элементы.ИнформацияСуммаВалКонечныйОстаток.Заголовок = НСтр("ru='Конец дня';uk='Кінець дня'");
		Элементы.ИнформацияСуммаВалКонечныйОстаток.Шрифт = Новый Шрифт(Элементы.ИнформацияСуммаВалКонечныйОстаток.Шрифт,,, Ложь);
		
		ТаблицаИтогов[0].Показатель = НСтр("ru='Начало дня';uk='Початок дня'");
		ТаблицаИтогов[3].Показатель = НСтр("ru='Конец дня';uk='Кінець дня'");
	...
	ИначеЕсли ВариантОтображенияИтогов = ПредопределенноеЗначение("Перечисление.ВариантыОтображенияИтоговБанкИКасса.ВедомостьЗаПериод") Тогда
		...
		//Элементы.ДекорацияВедомостьПоВалюте.Заголовок = НСтр("ru='Ведомость по валюте ';uk='Відомість по валюті'")+""""+ОтборВалютаВедомости + """";
		Элементы.ДекорацияВедомостьПоВалюте.Заголовок = НСтр("ru='Ведомость по валюте: ';uk='Відомість по валюті:'")+ОтборВалютаВедомости;
		...
		Элементы.ИнформацияСуммаВалНачальныйОстаток.Заголовок = НСтр("ru='Начало периода';uk='Початок періоду'");
		Элементы.ИнформацияСуммаВалКонечныйОстаток.Заголовок = НСтр("ru='Конец периода';uk='Кінець періоду'");
		Элементы.ИнформацияСуммаВалКонечныйОстаток.Шрифт = Новый Шрифт(Элементы.ИнформацияСуммаВалКонечныйОстаток.Шрифт,,, Ложь);
		
		Если НЕ ЗначениеЗаполнено(ОтборПериод.ДатаНачала) И НЕ ЗначениеЗаполнено(ОтборПериод.ДатаОкончания) Тогда
			ТаблицаИтогов[0].Показатель = НСтр("ru='За все время';uk='За весь час'");
			ТаблицаИтогов[3].Показатель = НСтр("ru='Остаток';uk='Залишок'");
		Иначе
			ТаблицаИтогов[0].Показатель = НСтр("ru='С ';uk='З '")+Формат(ОтборПериод.ДатаНачала, "ДФ=dd.MM.yyyy");
			ТаблицаИтогов[3].Показатель = НСтр("ru='По ';uk='По'")+Формат(ОтборПериод.ДатаОкончания, "ДФ=dd.MM.yyyy");
		КонецЕсли;
	...
	ИначеЕсли ВариантОтображенияИтогов = ПредопределенноеЗначение("Перечисление.ВариантыОтображенияИтоговБанкИКасса.ОстаткиПересчитанныеВВалюту") Тогда
		...
		Элементы.ОтборКурс.Заголовок = НСтр("ru='Курс на ';uk='Курс на'")+Формат(ТекущаяДатаСеанса(), "ДФ=dd.MM.yyyy")+"";
		ОтборКурс = ""+СтруктураДанных.Курс;
		ИнформацияСуммаВалКонечныйОстаток = СтруктураДанных.Остаток;
		Элементы.ДекорацияОстатокВВалюте.Заголовок = НСтр("ru='Остатки в валюте: ';uk='Залишки у валюті:'")+ОтборВалютаОстатков;
		...
		Элементы.ИнформацияСуммаВалКонечныйОстаток.Заголовок = НСтр("ru='Остаток';uk='Залишок'");
...
Процедура ЗаполнитьТаблицуИтогов()
	...
	СтрокаИтогов.Показатель = НСтр("ru='Начало дня';uk='Початок дня'");
	СтрокаИтогов.Значение = 0;
	
	// Начальный остаток
	СтрокаИтогов = ТаблицаИтогов.Добавить();
	СтрокаИтогов.Показатель = НСтр("ru='Поступило';uk='Надійшло'");
	СтрокаИтогов.Значение = 0;
	
	// Начальный остаток
	СтрокаИтогов = ТаблицаИтогов.Добавить();
	СтрокаИтогов.Показатель = НСтр("ru='Списано';uk='Списано'");
	СтрокаИтогов.Значение = 0;
	
	// Начальный остаток
	СтрокаИтогов = ТаблицаИтогов.Добавить();
	СтрокаИтогов.Показатель = НСтр("ru='Конец дня';uk='Кінець дня'");
	СтрокаИтогов.Значение = 0;
