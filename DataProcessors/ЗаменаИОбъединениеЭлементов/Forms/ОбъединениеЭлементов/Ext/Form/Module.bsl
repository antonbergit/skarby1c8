...
Процедура МестаИспользованияПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Удалить из списка для объединения элемент ""%1""?';uk='Видалити зі списку об`єднання елемент ""%1""?'"),
...
"Группы не могут быть объединены.';uk='Один з поєднуваних елементів є групою."
...
"Такие элементы не могут быть объединены.';uk='У поєднуваних елементів різні власники."
"Такі елементи не можуть бути об`єднані.'");
	ИначеЕсли Контроль.КоличествоСсылок <> КоличествоСсылок Тогда
		Результат.Ошибка = НСтр("ru='Все объединяемые элементы должны быть одного типа.';uk='Всі поєднувані елементи повинні бути одного типу.'");
	Иначе 
		// Все нормально
...
"использования на ""%2"" (отмечен стрелкой).';uk='Елементи (%1) будуть <a href = ""ПереключениеРежимаУдаления"">позначені на видалення</a> і замінені у всіх місцях"
...
"использования на ""%2"" (отмечен стрелкой).';uk='Елементи (%1) будуть <a href = ""ПереключениеРежимаУдаления"">видалені безповоротно</a> і замінені у всіх місцях"
"використання на ""%2"" (відзначений стрілкою).'");
		КонецЕсли;
	Иначе
		ТекстПодсказки = НСтр("ru='Элементы (%1) будут помечены на удаление и заменены во всех местах"
"использования на ""%2"" (отмечен стрелкой).';uk='Елементи (%1) будуть позначені на видалення й замінені у всіх місцях"
"використання на ""%2"" (відзначений стрілкою).'");
	КонецЕсли;
...
Функция СообщениеОЗавершении()
	Возврат СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
		...
		НСтр("ru='элемент объединен в:';uk='елемент об`єднаний в:'") + "," + НСтр("ru='элемента объединено в:';uk='елементу об`єднано в:'") + "," + НСтр("ru='элементов объединено в:';uk='елементів об`єднано в:'"));
КонецФункции
...
"автоматическая замена на ""%1""';uk='Об`єднання елементів не виконане. У деяких місцях використання не може бути зроблена"
...
Функция ЗапуститьФоновоеЗадание()
	...
	Если НастройкиПошаговогоМастера.ИмяПроцедуры = "ОпределитьМестаИспользования" Тогда
		...
		НаименованиеМетода = НСтр("ru='Поиск и удаление дублей: Определение мест использования';uk='Пошук і видалення дублів: Визначення місць використання'");
	...
	ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ЗаменитьСсылки" Тогда
		...
		НаименованиеМетода = НСтр("ru='Поиск и удаление дублей: Объединение элементов';uk='Пошук і видалення дублів: Об`єднання елементів'");
