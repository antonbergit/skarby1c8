...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗаменяемыеСсылки.Количество() > 1 Тогда
		Элементы.НадписьТипВыбираемого.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Выберите один из элементов ""%1"", на который следует заменить выбранные значения (%2):';uk='Виберіть один з елементів ""%1"", на який слід замінити обрані значення (%2):'"),
			ОсновныеМетаданные.Представление(), ЗаменяемыеСсылки.Количество());
	Иначе
		Заголовок = НСтр("ru='Замена элемента';uk='Заміна елемента'");
		Элементы.НадписьТипВыбираемого.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Выберите один из элементов ""%1"", на который следует заменить ""%2"":';uk='Виберіть один з елементів ""%1"", на який слід замінити ""%2"":'"),
			ОсновныеМетаданные.Представление(), ЗаменяемыеСсылки[0].Ссылка);
	КонецЕсли;
	Элементы.ПодсказкаВыбораЦелевогоЭлемента.Заголовок = НСтр("ru='Элемент для замены не выбран.';uk='Елемент для заміни не обраний.'");
...
"и <a href = ""ПереключениеРежимаУдаления"">помечен на удаление</a>.';uk='Обраний елемент буде замінено на ""%1"""
"і <a href = ""ПереключениеРежимаУдаления"">позначено на видалення</a>.'");
			Иначе
				ТекстПодсказки = НСтр("ru='Выбранный элемент будет заменен на ""%1"""
"и <a href = ""ПереключениеРежимаУдаления"">удален безвозвратно</a>.';uk='Обраний елемент буде замінено на ""%1"""
"і <a href = ""ПереключениеРежимаУдаления"">вилучено безповоротно</a>.'");
			КонецЕсли;
		Иначе
			ТекстПодсказки = НСтр("ru='Выбранный элемент будет заменен на ""%1"""
"и помечен на удаление.';uk='Обраний елемент буде замінено на ""%1"""
"і позначено на видалення.'");
		КонецЕсли;
...
"и <a href = ""ПереключениеРежимаУдаления"">помечены на удаление</a>.';uk='Обрані елементи (%1) будуть замінені на ""%2"""
"і <a href = ""ПереключениеРежимаУдаления"">позначені на видалення</a>.'");
			Иначе
				ТекстПодсказки = НСтр("ru='Выбранные элементы (%1) будут заменены на ""%2"""
"и <a href = ""ПереключениеРежимаУдаления"">удалены безвозвратно</a>.';uk='Обрані елементи (%1) будуть замінені на ""%2"""
"і <a href = ""ПереключениеРежимаУдаления"">видалені безповоротно</a>.'");
			КонецЕсли;
		Иначе
			ТекстПодсказки = НСтр("ru='Выбранные элементы (%1) будут заменены на ""%2"""
"и помечен на удаление.';uk='Обрані елементи (%1) будуть замінені на ""%2"""
"і позначені на видалення.'");
		КонецЕсли;
...
"автоматическая замена на ""%3""';uk='Не вдалося замінити елементи (%1 з %2). В деяких місцях "
"використання не може бути проведена"
...
"Группы не могут быть заменены.';uk='Один із замінних елементів є групою."
...
"Такие элементы не могут быть заменены.';uk='У замінних елементів різні власники."
"Такі елементи не можуть бути замінені.'");
		Возврат;
	ИначеЕсли Условия.КоличествоСсылок <> КоличествоСсылок Тогда
		ТекстОшибкиПараметров = НСтр("ru='Все заменяемые элементы должны быть одного типа.';uk='Всі замінні елементи повинні бути одного типу.'");
		Возврат;
	КонецЕсли;
	...
	Если Результат[2].Выгрузить().Количество() = 0 Тогда
		Если КоличествоСсылок > 1 Тогда
			ТекстОшибкиПараметров = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Выбранные элементы (%1) не на что заменить.';uk='Обрані елементи (%1) немає на що замінити.'"), КоличествоСсылок);
		Иначе
			ТекстОшибкиПараметров = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Выбранный элемент ""%1"" не на что заменить.';uk='Обраний елемент ""%1"" немає на що замінити.'"), ОбщегоНазначения.ПредметСтрокой(ЦелевойЭлемент));
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
"У выбранного элемента владелец ""%1"", а у заменяемого - ""%2"".';uk='Не можна замінювати на елемент, підлеглий іншому власнику."
"У вибраного елемента власник ""%1"", а у того, що заміняється - ""%2"".'");
	...
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Элемент %1 помечен на удаление. Продолжить?';uk='Елемент %1 позначений на видалення. Продовжити?'"), ТекущиеДанные.Ссылка);
	...
	Для Каждого ОписаниеШага Из НастройкиПошаговогоМастера.Шаги Цикл
		Для Каждого КлючЗначение Из ВозможныеДействия Цикл
			...
			Если Не ПустаяСтрока(ИмяОбработчика) Тогда
				...
				Исключение
					Текст = НСтр("ru='Ошибка создания обработчика события %1 для страницы %2, не определена процедура %3';uk='Помилка створення обробника події %1 для сторінки %2, не "
"визначена процедура %3'");
...
Функция НомерШагаМастераПоИдентификатору(Знач ИдентификаторШага)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не найден шаг ""%1"".';uk='Не знайдений крок ""%1"".'"), ИмяПоиска);
КонецФункции
...
Функция ЗапуститьФоновоеЗадание(Знач ПараметрыМетода, Знач УникальныйИдентификатор)
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ru='Поиск и удаление дублей: Замена ссылок';uk='Пошук і видалення дублів: Заміна посилань'");
...
Функция ЗаполнитьНеудачныеЗамены(Знач АдресРезультата)
	...
	Для Каждого СтрокаРезультата Из РезультатыЗамены Цикл
		...
		ИначеЕсли ТипОшибки = "ОшибкаУдаления" Тогда
			СтрокаОшибки.Причина = НСтр("ru='Невозможно удалить данные.';uk='Неможливо видалити дані.'");
