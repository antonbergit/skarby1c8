...
// Используется для сравнения имеющегося и нового значения реквизита ДатаФормированияДокументов.
// См. Процедуру ДатаФормированияДокументовОкончаниеВводаТекста
&НаКлиенте
Перем ДатаФормированияДокументовКэш;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолучитьПериодОтбораПоПериодичности(ОтборПериод, ПредставлениеПериода, ПериодичностьПериода, ДатаФормированияДокументов);
...
Процедура ДатаФормированияДокументовПриИзменении(Элемент)
	...
	Если ДатаФормированияДокументовКэш <> Неопределено Тогда
		ИзменениеДаты = ПроверитьКорректностьВыбораДатыФормированияДокументов(ДатаФормированияДокументов);
	КонецЕсли;
...
&НаКлиенте
Процедура ДатаФормированияДокументовОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	ПроверитьКорректностьВыбораДатыФормированияДокументов(ВыбранноеЗначение, СтандартнаяОбработка);
	
КонецПроцедуры

&НаКлиенте
Процедура ДатаФормированияДокументовОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	
	ДатаФормированияДокументовКэш = ДатаФормированияДокументов;
...
Процедура ФормированиеСчетовПечатныеФормыПредставлениеНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("ЗаголовокФормы",          Нстр("ru='Выбор печатной формы';uk='Вибір друкарської форми'"));
...
&НаКлиенте
Процедура ПериодВлево(Команда)
	
	ПолучитьПериодОтбораПоПериодичности(ОтборПериод, ПредставлениеПериода, ПериодичностьПериода, ДатаФормированияДокументов, -1);
	ЗапуститьОбновлениеДанныхФормы();
	
КонецПроцедуры

&НаКлиенте
Процедура ПериодВправо(Команда)
...
&НаСервере
Процедура ПолучитьПериодичностиДоговоровОбслуживания()
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	БиллингПериодичностьДоговораОбслуживания.Ссылка,
	|	БиллингПериодичностьДоговораОбслуживания.Порядок
	|ПОМЕСТИТЬ ВТПериодичностиДоговоров
	|ИЗ
	|	Перечисление.БиллингПериодичностьДоговораОбслуживания КАК БиллингПериодичностьДоговораОбслуживания
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВТПериодичностиДоговоров.Порядок КАК Порядок,
	|	ДоговорыКонтрагентов.ДоговорОбслуживанияПериодичность КАК Периодичность
	|ИЗ
	|	Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТПериодичностиДоговоров КАК ВТПериодичностиДоговоров
	|		ПО ДоговорыКонтрагентов.ДоговорОбслуживанияПериодичность = ВТПериодичностиДоговоров.Ссылка
	|ГДЕ
	...
	Если ПериодичностиДоговоров.Количество() > 1 Тогда
		...
		Элементы.ПредставлениеПериода.Подсказка = Нстр("ru='Изменить периодичность';uk='Змінити періодичність'");
	Иначе
...
Процедура ВыбратьПериодичность(ВыбраннаяПериодичность)
	...
	Если ПериодичностьПериода = Перечисления.БиллингПериодичностьДоговораОбслуживания.День Тогда
		ПодсказкаПредыдущийИнтервал = НСтр("ru='Предыдущий день';uk='Попередній день'");
		ПодсказкаСледующийИнтервал  = НСтр("ru='Следующий день';uk='Наступний день'");
	ИначеЕсли ПериодичностьПериода = Перечисления.БиллингПериодичностьДоговораОбслуживания.Неделя Тогда
		ПодсказкаПредыдущийИнтервал = НСтр("ru='Предыдущая нелеля ';uk='Предыдущая нелеля '");
		ПодсказкаСледующийИнтервал  = НСтр("ru='Следующая неделя';uk='Наступний тиждень'");
	ИначеЕсли ПериодичностьПериода = Перечисления.БиллингПериодичностьДоговораОбслуживания.Месяц Тогда
		ПодсказкаПредыдущийИнтервал = НСтр("ru='Предыдущий месяц';uk='Попередній місяць'");
		ПодсказкаСледующийИнтервал  = НСтр("ru='Следующий месяц';uk='Наступний місяць'");
	ИначеЕсли ПериодичностьПериода = Перечисления.БиллингПериодичностьДоговораОбслуживания.Квартал Тогда
		ПодсказкаПредыдущийИнтервал = НСтр("ru='Предыдущий квартал';uk='Попередній квартал'");
		ПодсказкаСледующийИнтервал  = НСтр("ru='Следующий квартал';uk='Наступний квартал'");
	ИначеЕсли ПериодичностьПериода = Перечисления.БиллингПериодичностьДоговораОбслуживания.Год Тогда
		ПодсказкаПредыдущийИнтервал = НСтр("ru='Предыдущий год';uk='Попередній рік'");
		ПодсказкаСледующийИнтервал  = НСтр("ru='Следующий год';uk='Наступний рік'");
	КонецЕсли;
	
	Команды.Найти("ПериодВлево").Подсказка = ПодсказкаПредыдущийИнтервал;
	Команды.Найти("ПериодВправо").Подсказка = ПодсказкаСледующийИнтервал;
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ПолучитьПериодОтбораПоПериодичности(ОтборПериод, ПредставлениеПериода, Периодичность, ДатаФормированияДокументов, Направление = 0)
	...
	Иначе
		...
		ДатаФормированияДокументов = ОтборПериод.ДатаОкончания;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Функция ПроверитьКорректностьВыбораДатыФормированияДокументов(НовоеЗначение, СтандартнаяОбработка = Неопределено)
	...
	Иначе
		...
		ДатаФормированияДокументов = ДатаФормированияДокументовКэш;
	КонецЕсли;
	
	ДатаФормированияДокументовКэш = Неопределено;
	
	ПоказатьПредупреждение(,
		СтрШаблон(
			НСтр("ru='Дата формируемых документов должна находиться в интервале выбранного периода: %1 — %2';uk='Дата документів, що формуються, повинна знаходитися в інтервалі обраного періоду: %1 — %2'"),
...
Процедура ЗапуститьФоновоеЗаданиеОбновлениеДанныхФормы()
	...
	ПараметрыПроцедуры.Вставить("Периодичность", ПериодичностьПериода);
	...
	ПараметрыПроцедуры.Вставить("ДатаФормированияДокументов", ДатаФормированияДокументов);
	
	НаименованиеЗадания = НСтр("ru='Получение данных по договорам обслуживания';uk='Отримання даних за договорами обслуговування'");
...
Процедура ЗапуститьФоновоеЗаданиеСозданиеПакетаДокументов()
	...
	ПараметрыПроцедуры = Новый Структура;
	ПараметрыПроцедуры.Вставить("УникальныйИдентификаторФормы", УникальныйИдентификатор);
	ПараметрыПроцедуры.Вставить("ДатаДокумента", ДатаФормированияДокументов);
	ПараметрыПроцедуры.Вставить("ДатаНачала",    ОтборПериод.ДатаНачала);
	ПараметрыПроцедуры.Вставить("ДатаОкончания", ОтборПериод.ДатаОкончания);
	ПараметрыПроцедуры.Вставить("ДатаФормированияДокументов", ДатаФормированияДокументов);
	ПараметрыПроцедуры.Вставить("Периодичность", ПериодичностьПериода);
	...
	ПараметрыПроцедуры.Вставить("ФормированиеАктовПечатать",                ФормированиеАктовПечатать);
	...
	ПараметрыПроцедуры.Вставить("ФормированиеАктовКомандыПечатиАдрес",      ФормированиеАктовКомандыПечатиАдрес);
	ПараметрыПроцедуры.Вставить("ФормированиеАктовПечатныеФормы",           ФормированиеАктовПечатныеФормы);
	ПараметрыПроцедуры.Вставить("ФормированиеСчетовКомандыПечатиАдрес", ФормированиеСчетовКомандыПечатиАдрес);
	ПараметрыПроцедуры.Вставить("ФормированиеСчетовОтправлятьПоПочте",  ФормированиеСчетовОтправлятьПоПочте);
	ПараметрыПроцедуры.Вставить("ФормированиеСчетовПечатныеФормы",      ФормированиеСчетовПечатныеФормы);
	ПараметрыПроцедуры.Вставить("ФормированиеСчетовШаблонСообщения",    ФормированиеСчетовШаблонСообщения);
	ПараметрыПроцедуры.Вставить("ФормированиеСчетовКомандыПечати",      ПолучитьИзВременногоХранилища(ФормированиеСчетовКомандыПечатиАдрес));
	ПараметрыПроцедуры.Вставить("ФормированиеАктовКомандыПечати",      ПолучитьИзВременногоХранилища(ФормированиеАктовКомандыПечатиАдрес));
	...
	Если ФоновоеЗадание = "ФоновоеЗаданиеСозданиеПакетаДокументов" Тогда
		...
		Если Результат.Свойство("АктыВыполненныхРабот") Тогда
			...
			ТекстОповещениеАкты = СтрШаблон(
				НСтр("ru='Актов выполненных работ — %1';uk='Актів виконаних робіт - %1'"),
		...
		Если Результат.Свойство("СчетаНаОплату") Тогда
			...
			ТекстОповещениеСчета = СтрШаблон(
				НСтр("ru='Счетов на оплату — %1';uk='Рахунків на оплату — %1'"),
		...
		Если Результат.Свойство("ЭлектронныеПисьма") Тогда
			...
			ТекстОповещениеЭлектронныеПисьма = СтрШаблон(
				НСтр("ru='Электронных писем — %1';uk='Електронних листів — %1'"),
		...
		ТекстОповещение = СтрШаблон(
			НСтр("ru='Создано документов:%1%2%3';uk='Створено документів:%1%2%3'"),
			ТекстОповещениеСчета,
			ТекстОповещениеАкты,
			ТекстОповещениеЭлектронныеПисьма
		);
		
		Текст = НСтр("ru='Создано документов:';uk='Створено документів:'");
		Пояснение = СтрШаблон(
			"%1%2%3",
			ТекстОповещениеСчета,
...
Процедура ВосстановитьНастройкиФормы()
	...
	Для каждого Настройка Из Настройки Цикл
		...
		ЗначениНастройки = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(КлючОбъекта, Настройка.Ключ, Настройка.Значение);
		ЭтаФорма[Настройка.Ключ] = ЗначениНастройки;
...
Процедура УправлениеФормой()
	...
	Если ФоновоеЗаданиеЗапущено И ФоновоеЗадание = "ФоновоеЗаданиеОбновлениеДанныхФормы" Тогда
		Элементы.СтраницыДоговоры.ТекущаяСтраница = Элементы.СтраницаОжиданиеОбновлениеДанных;
	ИначеЕсли ФоновоеЗаданиеЗапущено И ФоновоеЗадание = "ФоновоеЗаданиеСозданиеПакетаДокументов" Тогда
		Элементы.СтраницыДоговоры.ТекущаяСтраница = Элементы.СтраницаОжиданиеСозданиеДокументов;
	...
	Элементы.ФормированиеСчетовПечатныеФормыПредставление.Доступность = ФормированиеСчетовОтправлятьПоПочте ИЛИ ФормированиеСчетовПечатать;
	...
	Элементы.ФормированиеАктовПечатныеФормыПредставление.Доступность = ФормированиеАктов И (ФормированиеАктовОтправлятьПоПочте ИЛИ ФормированиеАктовПечатать);
...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	Текст = НСтр("ru='Без цены';uk='Без ціни'");
...
Процедура ПроверитьНастройкиОбработкиПередФормированиемДокументов()
	...
	Если ФормированиеСчетовОтправлятьПоПочте ИЛИ ФормированиеСчетовПечатать Тогда
		Если ФормированиеСчетовПечатныеФормы.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не выбрана ни одна печатная форма для Счетов на оплату!';uk='Не вибрана жодна друкарська форма для Рахунків на оплату!'"),,
	...
	Если ФормированиеАктов И ФормированиеАктовПечатать Тогда
		Если ФормированиеАктовПечатныеФормы.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не выбрана ни одна печатная форма для Актов выполненных работ!';uk='Не вибрана жодна друкарська форма для Актів виконаних робіт!'"),,
...
Процедура ЗаполнитьСписокПечатныхФорм()
	...
	Если НЕ ЗначениеЗаполнено(ФормированиеСчетовПечатныеФормыПредставление) Тогда
		ФормированиеСчетовПечатныеФормыПредставление = НСтр("ru='<Не выбрана печатная форма>';uk='<Не обрана друкована форма>'");
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ФормированиеАктовПечатныеФормыПредставление) Тогда
		ФормированиеАктовПечатныеФормыПредставление = НСтр("ru='<Не выбрана печатная форма>';uk='<Не обрана друкована форма>'");
	КонецЕсли;
