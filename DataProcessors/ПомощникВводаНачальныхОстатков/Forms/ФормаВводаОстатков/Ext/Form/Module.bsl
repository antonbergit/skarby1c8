...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Настройки = Новый Структура("Запасы,ЗапасыПринятые,ЗапасыПереданные,РасчетыСПоставщиками,РасчетыСПокупателями,РасчетыСПерсоналом,ВнеоборотныеАктивы");
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстЗавершения, СтандартнаяОбработка)
	...
	Если НЕ мЗаписьФормыОсуществлена
		...
		Текст = НСтр("ru='Сохранить внесенные изменения?';uk='Зберегти внесені зміни?'");
...
Процедура ВидЦенСебестоимостиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("ru='Обновить цены в таблице товаров?';uk='Обновить цены в таблице товаров?'"), РежимДиалогаВопрос.ДаНетОтмена,, КодВозвратаДиалога.Да);
	КонецЕсли; 
...
Процедура ВводНачальныхОстатковИмуществоВнеоборотныеАктивыОбъемПродукцииРаботДляВычисленияАмортизацииПриИзменении(Элемент)
	...
	Если НЕ СтруктураДанные.СпособАмортизацииПропорциональноОбъемуПродукции Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ru='""Объем продукции (работ) для исчисления амортизации"" не может быть заполнен для указанного способа начисления амортизации!';uk='""Обсяг продукції (робіт) для вирахування амортизації"" не може бути заповнений для зазначеного способу нарахування амортизації!'"));
...
Процедура ВводНачальныхОстатковИмуществоВнеоборотныеАктивыСрокИспользованияДляВычисленияАмортизацииПриИзменении(Элемент)
	...
	Если СтруктураДанные.СпособАмортизацииПропорциональноОбъемуПродукции Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ru='""Срок использования для вычисления амортизации"" не может быть заполнен для указанного способа начисления амортизации!';uk='""Термін використання для обчислення амортизації"" не може бути заповнений для зазначеного способу нарахування амортизації!'"));
...
Процедура ВводНачальныхОстатковПрочееПрочиеРазделыВалютаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ СтруктураДанные.Валютный Тогда
		...
		Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Счет) Тогда
			ПоказатьПредупреждение(Неопределено,НСтр("ru='У выбранного счета не установлен признак валютный!';uk='В обраного рахунку не встановлена ознака валютний!'"));
		Иначе
			ПоказатьПредупреждение(Неопределено,НСтр("ru='Укажите в начале счет!';uk='Вкажіть на початку рахунок!'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ДекорацияЗагрузкаУТОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = ЛОжь;
	ОткрытьФорму("Обработка.ЗагрузкаДанныхИзУТ.Форма");
	Закрыть();
...
Процедура ПоискПоШтрихкоду(Команда)
	...
	ПоказатьВводЗначения(Оповещение, ТекШтрихкод, НСтр("ru='Введите штрихкод';uk='Введіть штрихкод'"));
...
Процедура ЗагрузитьВводыОстатков()
	...
	Запрос.УстановитьПараметр("Комментарий", НСтр("ru='# Документ введен помощником ввода остатков.';uk='# Документ введений помічником введення залишків.'"));
	...
	Пока ВыборкаРезультатЗапроса.Следующий() Цикл
		...
		Элементы.ДатаОстатков.ПредупреждениеПриРедактировании = НСтр("ru='Дата именится также для всех остатков, введенных ранее';uk='Дата именится также для всех остатков, введенных ранее'");
...
Процедура УстановитьТекстПояснения()
	...
	Если ТекущийНомерСтраницы = 0 Тогда
		Элементы.ДекорацияПояснеиеСледующегоДействия.Заголовок = НСтр("ru='Нажмите кнопку ""Далее"" для заполнения остатков денежных средств';uk='Натисніть кнопку ""Далі"" для заповнення залишків грошових коштів'");
	ИначеЕсли ТекущийНомерСтраницы = 1 Тогда
		Элементы.ДекорацияПояснеиеСледующегоДействия.Заголовок = НСтр("ru='Нажмите кнопку ""Далее"" для заполнения остатков товаров';uk='Натисніть кнопку ""Далі"" для заповнення залишків товарів'");
	ИначеЕсли ТекущийНомерСтраницы = 2 Тогда
		Элементы.ДекорацияПояснеиеСледующегоДействия.Заголовок = НСтр("ru='Нажмите кнопку ""Далее"" для заполнения остатков по расчетам';uk='Натисніть кнопку ""Далі"" для заповнення залишків за розрахунками'");
	ИначеЕсли ТекущийНомерСтраницы = 3 Тогда
		Элементы.ДекорацияПояснеиеСледующегоДействия.Заголовок = НСтр("ru='Нажмите кнопку ""Далее"" для заполнения прочих остатков';uk='Натисніть кнопку ""Далі"" для заповнення інших залишків'");
	ИначеЕсли ТекущийНомерСтраницы = 4 Тогда
		Элементы.ДекорацияПояснеиеСледующегоДействия.Заголовок = НСтр("ru='Для завершения необходимо нажать кнопку ""Закончить""';uk='Для завершення необхідно натиснути кнопку ""Закінчити""'");
	КонецЕсли;
...
Процедура УстановитьАктивнуюСтраницу()
	...
	ЭтаФорма.Заголовок = НСтр("ru='Помощник ввода начальных остатков (Шаг ';uk='Помічник введення початкових залишків (Крок '") + Строка(ТекущийНомерСтраницы)+ "/" + Строка(ПоследняяСтраница) + ")";
...
Процедура УстановитьДоступностьКнопок()
	...
	Если ТекущийНомерСтраницы >= ПоследняяСтраница - 1 Тогда
		Элементы.Далее.Заголовок = НСтр("ru='Закончить';uk='Закінчити'");
...
Процедура ЗаписатьИзмененияФормы(ЗакончитьВвод = Ложь)
	...
	Если ВводНачальныхОстатковБанкИКасса.ДенежныеСредства.Количество() > 0
		...
		ВводНачальныхОстатковБанкИКассаОбъект.Комментарий = НСтр("ru='# Документ введен помощником ввода остатков.';uk='# Документ введений помічником введення залишків.'");
	...
	Если ВводНачальныхОстатковПодотчет.РасчетыСПодотчетниками.Количество() > 0
		...
		ВводНачальныхОстатковПодотчетОбъект.Комментарий = НСтр("ru='# Документ введен помощником ввода остатков.';uk='# Документ введений помічником введення залишків.'");
	...
	Если ВводНачальныхОстатковТовары.Запасы.Количество() > 0
		...
		ВводНачальныхОстатковТоварыОбъект.Комментарий = НСтр("ru='# Документ введен помощником ввода остатков.';uk='# Документ введений помічником введення залишків.'");
	...
	Если ВводНачальныхОстатковРасчетыСКонтрагентами.РасчетыСПоставщиками.Количество() > 0
	 ИЛИ ВводНачальныхОстатковРасчетыСКонтрагентами.РасчетыСПокупателями.Количество() > 0
		...
		ВводНачальныхОстатковРасчетыСКонтрагентамиОбъект.Комментарий = НСтр("ru='# Документ введен помощником ввода остатков.';uk='# Документ введений помічником введення залишків.'");
	...
	Если ВводНачальныхОстатковЗарплата.РасчетыСПерсоналом.Количество() > 0
		...
		ВводНачальныхОстатковЗарплатаОбъект.Комментарий = НСтр("ru='# Документ введен помощником ввода остатков.';uk='# Документ введений помічником введення залишків.'");
	...
	Если ВводНачальныхОстатковНалоги.РасчетыПоНалогам.Количество() > 0
		...
		ВводНачальныхОстатковНалогиОбъект.Комментарий = НСтр("ru='# Документ введен помощником ввода остатков.';uk='# Документ введений помічником введення залишків.'");
	...
	Если ВводНачальныхОстатковЭквайринг.РасчетыПоЭквайрингу.Количество() > 0
		...
		ВводНачальныхОстатковЭквайрингОбъект.Комментарий = НСтр("ru='# Документ введен помощником ввода остатков.';uk='# Документ введений помічником введення залишків.'");
	...
	Если ВводНачальныхОстатковИмущество.ВнеоборотныеАктивы.Количество() > 0
		...
		ВводНачальныхОстатковИмуществоОбъект.Комментарий = НСтр("ru='# Документ введен помощником ввода остатков.';uk='# Документ введений помічником введення залишків.'");
	...
	Если ВводНачальныхОстатковПрочее.ПрочиеРазделы.Количество() > 0
		...
		ВводНачальныхОстатковПрочееОбъект.Комментарий = НСтр("ru='# Документ введен помощником ввода остатков.';uk='# Документ введений помічником введення залишків.'");
...
Процедура ВыполнитьПроверкиПриПереходеНаСледующуюСтраницу(Отказ)
	...
	Если ТекущийНомерСтраницы = 1 Тогда
		...
		Если НЕ ЗначениеЗаполнено(ВалютаУчета) Тогда
			ТекстСообщения = НСтр("ru='Укажите валюту учета.';uk='Вкажіть валюту обліку.'");
		...
		Если НЕ ЗначениеЗаполнено(НациональнаяВалюта) Тогда
			ТекстСообщения = НСтр("ru='Укажите национальную валюту.';uk='Вкажіть національну валюту.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ТекстСообщения, , , ,
				Отказ
			);
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковБанкИКасса.ДенежныеСредства Цикл
			...
			ТекСтрока,
			"БанковскийСчетКасса",
			НСтр("ru='Укажите банковский счет или кассу в строке %1.';uk='Укажите банковский счет или кассу в строке %1.'"),
			Отказ);
			...
			НСтр("ru='Укажите валюту в строке %1.';uk='Укажите валюту в строке %1.'"),
			Отказ);
	...
	ИначеЕсли ТекущийНомерСтраницы = 2 Тогда
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковТовары.Запасы Цикл
			...
			ТекСтрока,
			"СтруктурнаяЕдиница",
			НСтр("ru='Укажите структурную единицу в строке %1.';uk='Укажите структурную единицу в строке %1.'"),
			Отказ);
			...
			ТекСтрока,
			"Номенклатура",
			НСтр("ru='Укажите номенклатуру в строке %1.';uk='Укажите номенклатуру в строке %1.'"),
			Отказ);
			...
			ТекСтрока,
			"ЕдиницаИзмерения",
			НСтр("ru='Укажите единицу измерения в строке %1.';uk='Укажите единицу измерения в строке %1.'"),
			Отказ);
			...
			ТекСтрока,
			"Количество",
			НСтр("ru='Укажите количество в строке %1.';uk='Укажите количество в строке %1.'"),
			Отказ);
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковТовары.ЗапасыПринятые Цикл
			...
			ТекСтрока,
			"ВидОперации",
			НСтр("ru='Укажите вид операции в строке %1.';uk='Укажите вид операции в строке %1.'"),
			Отказ);
			...
			ТекСтрока,
			"Контрагент",
			НСтр("ru='Укажите поставщика в строке %1.';uk='Укажите поставщика в строке %1.'"),
			Отказ);
			...
			ТекСтрока,
			"Договор",
			НСтр("ru='Укажите договор в строке %1.';uk='Укажите договор в строке %1.'"),
			Отказ);
			...
			ТекСтрока,
			"Партия",
			НСтр("ru='Укажите партию в строке %1.';uk='Укажите партию в строке %1.'"),
			Отказ);
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковТовары.ЗапасыПереданные Цикл
			...
			ТекСтрока,
			"Контрагент",
			НСтр("ru='Укажите покупателя в строке %1.';uk='Укажите покупателя в строке %1.'"),
			Отказ);
	...
	ИначеЕсли ТекущийНомерСтраницы = 3 Тогда
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковРасчетыСКонтрагентами.РасчетыСПоставщиками Цикл
			...
			ТекСтрока,
			"СуммаВал",
			НСтр("ru='Укажите сумму в строке %1.';uk='Укажите сумму в строке %1.'"),
			Отказ);
			...
			ТекСтрока,
			"Сумма",
			НСтр("ru='Укажите сумму в валюте учета в строке %1.';uk='Укажите сумму в валюте учета в строке %1.'"),
			Отказ);
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковЗарплата.РасчетыСПерсоналом Цикл
			...
			ТекСтрока,
			"СтруктурнаяЕдиница",
			НСтр("ru='Укажите подразделение в строке %1.';uk='Укажите подразделение в строке %1.'"),
			Отказ);
			...
			ТекСтрока,
			"Сотрудник",
			НСтр("ru='Укажите сотрудника в строке %1.';uk='Укажите сотрудника в строке %1.'"),
			Отказ);
			...
			ТекСтрока,
			"ПериодРегистрации",
			НСтр("ru='Укажите период в строке %1.';uk='Укажите период в строке %1.'"),
			Отказ);
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковНалоги.РасчетыПоНалогам Цикл
			...
			ТекСтрока,
			"ВидНалога",
			НСтр("ru='Укажите вид налога в строке %1.';uk='Укажите вид налога в строке %1.'"),
			Отказ);
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковЭквайринг.РасчетыПоЭквайрингу Цикл
			...
			НСтр("ru='Укажите экваринговый терминал в строке %1.';uk='Укажите экваринговый терминал в строке %1.'"),
			Отказ);
	...
	ИначеЕсли ТекущийНомерСтраницы = 4 Тогда
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковИмущество.ВнеоборотныеАктивы Цикл
			...
			ТекСтрока,
			"ВнеоборотныйАктив",
			НСтр("ru='Укажите карточку имущества в строке %1.';uk='Укажите карточку имущества в строке %1.'"),
			Отказ);
			...
			НСтр("ru='Укажите текущее состояние в строке %1.';uk='Укажите текущее состояние в строке %1.'"),
			Отказ);
			...
			ТекСтрока,
			"СчетЗатрат",
			НСтр("ru='Укажите счет затрат в строке %1.';uk='Укажите счет затрат в строке %1.'"),
			Отказ);
		...
		Для каждого ТекСтрока Из ВводНачальныхОстатковПрочее.ПрочиеРазделы Цикл
			...
			ТекСтрока,
			"ВидДвижения",
			НСтр("ru='Укажите вид движения в строке %1.';uk='Укажите вид движения в строке %1.'"),
			Отказ);
			...
			ТекСтрока,
			"Счет",
			НСтр("ru='Укажите счет в строке %1.';uk='Укажите счет в строке %1.'"),
			Отказ);
			...
			Если УчетВалютныхОпераций И ТекСтрока.Валютный Тогда
				...
				ТекСтрока,
				"СуммаВал",
				НСтр("ru='Укажите валютную сумму в строке %1.';uk='Укажите валютную сумму в строке %1.'"),
				Отказ);
...
Функция СписокВыбораВалют(Валюты)
	...
	КонецЦикла; 
	Результат.Добавить(Неопределено, НСтр("ru='Еще валюты...';uk='Еще валюты...'"));
	Возврат Результат;
...
Процедура ПолучитьСтруктуруВидовОпераций()
	...
	Если НЕ Элементы.СтраницаПринятыеТовары.Видимость И НЕ Элементы.СтраницаПереданныеТовары.Видимость Тогда
	...
	Если ТипЗнч(РезультатЗагрузки) = Тип("Структура") Тогда
		...
		ИначеЕсли РезультатЗагрузки.ОписаниеДействия = "ОбработатьПодготовленныеДанные" Тогда
			...
			Если ПолноеИмяОбъектаЗаполнения = "Документ.ВводНачальныхОстатков.ТабличнаяЧасть.Запасы" Тогда
				...
				ПоказатьПредупреждение(,Нстр("ru='Загрузка данных завершена.';uk='Завантаження даних завершено.'"));
