...
Процедура Конвертация_ПередЗагрузкойДанных(ФайлОбмена, Отказ) Экспорт
	...
	// Таблица значений для проведения документов после выгрузки
...
Процедура Конвертация_ПослеЗагрузкиДанных() Экспорт
	...
	Перем КДОбъект;
		...
		// Установка константы "ФункциональнаяОпцияУчетПоНесколькимОрганизациям". Выполняется тут,
	...
	Если Не ПустаяСтрока(ТекстСообщенияОшибки) Тогда
	    Сообщить(НСтр("ru='Ошибки при отмене проведения: ';uk='Помилки при скасуванні проведення: '")+ Символы.ПС + ТекстСообщенияОшибки);
	КонецЕсли;
...
Процедура ПКО_ПослеЗагрузки_ВводОстатковРасчетыСПоставщикамиИПокупателями(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован,
																		ИмяТипаОбъекта, ОбъектНайден, НаборЗаписей) Экспорт
	...
	Для каждого Строка Из Объект.РасчетыСПокупателями Цикл
		...
		Если НЕ ЗначениеЗаполнено(Строка.Договор) Тогда
			Строка.Договор = Строка.Контрагент.ДоговорПоУмолчанию;	
		КонецЕсли; 
...
Процедура ПКО_ПослеЗагрузки_ВводОстатковРасчетыСПерсоналом(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован,
															ИмяТипаОбъекта, ОбъектНайден, НаборЗаписей) Экспорт
															
	Для каждого Строка Из Объект.РасчетыСПерсоналом Цикл
		Строка.СтруктурнаяЕдиница = Справочники.СтруктурныеЕдиницы.ОсновноеПодразделение;
		Строка.Валюта = Константы.НациональнаяВалюта.Получить();
	КонецЦикла;
