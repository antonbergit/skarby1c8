...
Процедура ПриОткрытии(Отказ)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		Отказ = Истина;
		ПоказатьПредупреждение(, НСтр("ru='Извлечение текстов не поддерживается в Веб-клиенте.';uk='Вилучення текстів не підтримується у Веб-клієнті.'"));
		Возврат;
	КонецЕсли;
	
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ru='Извлечение текстов поддерживается только в клиенте под управлением ОС Windows.';uk='Витяг текстів підтримується тільки в клієнті під управлінням ОС Windows.'");
...
""
"Обработано файлов: %1.';uk='Завершене вилучення тексту з"
"усіх файлів з невилученим з текстом."
""
"Оброблено файлів: %1.'"),
...
Процедура ОбновлениеОбратногоОтсчета()
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='До начала извлечения текстов осталось %1 сек';uk='До початку витягу текстів залишилося %1 сек'"),
		Осталось);
...
Процедура ИзвлечениеТекстовКлиент(РазмерПорции = Неопределено)
	...
	Состояние(НСтр("ru='Начато извлечение текста';uk='Почато витяг тексту'"));
	...
	Попытка
		...
		Если МассивФайлов.Количество() = 0 Тогда
			Состояние(НСтр("ru='Нет файлов для извлечения текста';uk='Немає файлів для витягу тексту'"));
			Возврат;
		КонецЕсли;
		...
		Для Индекс = 0 По МассивФайлов.Количество() - 1 Цикл
			...
			Попытка
				...
				Состояние(НСтр("ru='Идет извлечение текста файла';uk='Іде витяг тексту файлу'"), Прогресс, ИмяСРасширением);
...
"произошла неизвестная ошибка.';uk='Під час вилучення тексту з файлу ""%1"""
...
"Обработано файлов: %1';uk='Вилучення тексту завершене."
"Оброблено файлів: %1'"),
