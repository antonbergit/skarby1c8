...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Параметры.Свойство("КодУзлаИнформационнойБазы", КодУзлаИнформационнойБазы) Тогда
		...
		Если Объект.УзелИнформационнойБазы.Пустая() Тогда
			...
			ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ru='Настройка обмена данными не найдена.';uk='Налаштування обміну даними не знайдене.'"), Отказ);
			Возврат;
	...
	Если Не ОбменДаннымиПовтИсп.ЭтоУзелУниверсальногоОбменаДанными(Объект.УзелИнформационнойБазы) Тогда
		...
		ОбменДаннымиСервер.СообщитьОбОшибке(
			НСтр("ru='Для выбранного узла выполнение обмена данными с настройкой не предусмотрено.';uk='Для обраного вузла виконання обміну даними з налаштуванням не передбачено.'"), Отказ);
		Возврат;
...
Процедура ДополнениеВыгрузкиИсторияОтборов(Команда)
	...
	Текст = НСтр("ru='Сохранить текущую настройку...';uk='Зберегти поточне налаштування...'");
...
Процедура ДополнениеВыгрузкиИсторияОтборовВыборИзМеню(Знач ВыбранныйЭлемент, Знач ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ПредставлениеНастройки)=Тип("Строка") Тогда
		...
		НСтр("ru='Восстановить настройки ""%1""?';uk='Відновити налаштування ""%1""?'"), ПредставлениеНастройки);
...
Процедура ПослеПроверкиРасширенияРаботыСФайлами(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстПредупреждения = НСтр("ru='Расширение для работы с файлами не установлено, открытие каталога не возможно.';uk='Розширення для роботи з файлами не встановлене, відкриття каталогу не можливо.'");
...
Функция Подключаемый_СтраницаНачала_ПриПереходеДалее(Отказ)
	...
	Если Объект.УзелИнформационнойБазы.Пустая() Тогда
		...
		НСтрока = НСтр("ru='Укажите узел информационной базы';uk='Вкажіть вузол інформаційної бази'");
	...
	ИначеЕсли Объект.ВидТранспортаСообщенийОбмена.Пустая() Тогда
		...
		НСтрока = НСтр("ru='Укажите вариант подключения';uk='Вкажіть варіант підключення'");
...
Процедура ЗавершениеПолученияДанныхВоВременныйКаталогНаСервере();
	...
	ШаблонСообщенияОбОшибке = НСтр("ru='Не удалось выполнить загрузку данных. Подробности см. в журнале регистрации';uk='Не вдалося виконати завантаження даних. Подробиці див в журналі реєстрації'");
...
Процедура РегистрацияДополнительноОтобранныхДанных()
	...
	Исключение
		...
		ДлительнаяОперацияЗавершена = Истина;
		ДлительнаяОперация = Ложь;
		СообщениеОбОшибке = НСтр("ru='Возникла проблема при добавлении данных к выгрузке:';uk='Виникла проблема при додаванні даних до вивантаження:'") 
...
Функция Подключаемый_СтраницаОжиданияВыгрузкиДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ru='Выгрузка данных из сообщения обмена';uk='Вивантаження даних з повідомлення обміну'");
...
Процедура ЗавершениеВыгрузкиНаСервере();
	...
	Если РезультатВыполненияМетода.Отказ
		И Не ЗначениеЗаполнено(СообщениеОбОшибке) Тогда
		СообщениеОбОшибке = НСтр("ru='Не удалось выполнить отправку данных. Подробности см. в журнале регистрации';uk='Не вдалося виконати відправку даних. Подробиці див в журналі реєстрації'");
	КонецЕсли;
...
Процедура ОткрытьФормуОтбораПоОрганизациям(Команда)
	...
	ОткрытьФорму("ПланОбмена.ОбменУправлениеНебольшойФирмойБухгалтерия.Форма.ФормаВыбораОрганизаций",
		ПараметрыФормы,
		ЭтаФорма);
...
Процедура ОчиститьОтборПоОрганизациям(Команда)
	...
	ТекстЗаголовка = НСтр("ru='Подтверждение';uk='Підтвердження'");
	ТекстВопроса   = НСтр("ru='Очистить отбор по организациям?';uk='Очистити відбір по організаціях?'");
...
Процедура СценарийОбменаИнтерактивнаяСинхронизацияЧерезВнешнееСоединениеИлиВебСервис()
	...
	ТаблицаПереходовНоваяСтрока(9, "СтраницаПодтвержденияВыгрузкиДанных", "СтраницаНавигацииПодтверждение");
...
Процедура СценарийОбменаИнтерактивнаяСинхронизацияЧерезОбычныеКаналыСвязи()
	...
	// Подтверждение выгузки данных
	ТаблицаПереходовНоваяСтрока(7, "СтраницаПодтвержденияВыгрузкиДанных", "СтраницаНавигацииПодтверждение");
...
Процедура ОбновитьОтборПоОрганизациям(АдресОбъекта="")
	...
	Если Не ОрганизацииВыбраны Тогда
		Текст = НСтр("ru='Выбрать организации ';uk='Вибрати організації'");
	ИначеЕсли ВыбраныВсеОрганизации() Тогда
		Текст = НСтр("ru='Все организации ';uk='Всі організації'");
	Иначе
		Текст = ПланыОбмена.ОбменУправлениеНебольшойФирмойБухгалтерия20.СокращенноеПредставлениеКоллекцииЗначений(МассивВыбранныхОрганизаций);
	КонецЕсли;
...
Функция ПредставлениеОтбора(Период, Отбор)
	...
	ДополнениеВыгрузкиОбъект = РеквизитФормыВЗначение("ДополнениеВыгрузки");
	ОписаниеПустогоОтбора = НСтр("ru='Все документы';uk='Всі документи'");
