...
Процедура СтруктураСведенийОДокументе(СтруктураПараметров) Экспорт
	...
	Для каждого РеквизитОбработки Из Метаданные.Обработки.ПодборПеремещение.Реквизиты Цикл
...
Функция СтруктураОбязательныхПараметров()
	...
	Возврат Новый Структура("Дата, Организация, ТипНоменклатуры, УникальныйИдентификаторФормыВладельца, СтруктурнаяЕдиницаОтправитель, ВидОперации", "Дата", "Организация", "Тип номенклатуры", "Уникальный идентификатор формы владельца", "Склад отправитель", "Вид операции");
...
Процедура ПроверитьЗаполнениеПараметров(ПараметрыПодбора, Отказ) Экспорт
	...
	Для каждого ЭлементСтруктуры Из СтруктураОбязательныхПараметров Цикл
		...
		Если НЕ ПараметрыПодбора.Свойство(ЭлементСтруктуры.Ключ, ЗначениеПараметров) Тогда
			...
			ТекстОшибки = НСтр("ru='Отсутствует обязательный параметр (%1), необходимый для открытия формы подбора номенклатуры.';uk='Відсутній обов`язковий параметр (%1), необхідний для відкриття форми підбору номенклатури.'");
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(ЗначениеПараметров) Тогда
			...
			ТекстОшибки = НСтр("ru='Неверно заполнен обязательный параметр (%1), необходимый для открытия формы подбора номенклатуры.';uk='Невірно заповнений обов`язковий параметр (%1), необхідний для відкриття форми підбору номенклатури.'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ЭлементСтруктуры.Значение);
...
Функция ПолноеИмяФормыПодбора() Экспорт
	...
	Возврат "Обработка.ПодборПеремещение.Форма.КорзинаОстатокРезервХарактеристика";
