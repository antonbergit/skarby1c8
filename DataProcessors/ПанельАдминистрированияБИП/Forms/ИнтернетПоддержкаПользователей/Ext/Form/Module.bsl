...
"Отключить Интернет-поддержку?';uk='Логін і пароль для підключення до сервісів Інтернет-підтримки користувачів будуть видалені з програми."
...
Процедура ОтобразитьСостояниеПодключенияИПП()
	...
	Если ДанныеАутентификации = Неопределено Тогда
		Элементы.ДекорацияЛогинИПП.Заголовок = ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
			НСтр("ru='Подключение к Интернет-поддержке не выполнено.';uk='Підключення до Інтернет-підтримки не виконане.'"));
...
Функция ПредставлениеРасписания(Расписание)
	...
	Если Расписание = Неопределено Тогда
		Возврат НСтр("ru='Настроить расписание';uk='Налаштувати розклад'");
	Иначе
