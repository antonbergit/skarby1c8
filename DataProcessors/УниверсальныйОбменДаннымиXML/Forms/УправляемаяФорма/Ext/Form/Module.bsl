...
Процедура УдалениеУдалить(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Удалить выбранные данные в информационной базе?';uk='Видалити обрані дані в інформаційній базі?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура УдалениеУдалитьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Состояние(НСтр("ru='Выполняется удаление данных. Пожалуйста, подождите...';uk='Виконується видалення даних. Будь ласка, почекайте...'"));
		УдалитьНаСервере();
...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Если Не ЭтоWindowsКлиент() И ПрямаяВыгрузка = 1 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Прямое подключение к информационной базе поддерживается только в клиенте под управлением ОС Windows.';uk='Пряме підключення до інформаційної бази підтримується тільки в клієнтові під управлінням ОС Windows.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьЗагрузкуНаСервере(АдресФайла, ИмяФайлаДляРасширения)
	...
	Если Объект.БезопаснаяЗагрузка Тогда
		...
		Иначе
			СообщитьПользователю(НСтр("ru='Не указан файл правил для загрузки данных.';uk='Не вказаний файл правил для завантаження даних.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
