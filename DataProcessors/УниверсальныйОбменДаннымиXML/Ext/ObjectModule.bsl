...
Функция ЗаменитьВызовыАлгоритмовКодомЭтихАлгоритмовВОбработчике(Обработчик, АлгоритмВладелец, МассивЗапрошенныхЭлементов, Знач СтрокаПрефикса = "")
	...
	ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, АлгоритмВладелец, СтрокаПрефикса, НСтр("ru='{НАЧАЛО АЛГОРИТМА}';uk='{НАЧАЛО АЛГОРИТМА}'"));
		...
		Если АлгоритмыОбработчика.Количество() <> 0 Тогда // В этой строке есть вызовы алгоритмов.
			...
			Для Каждого Алгоритм Из АлгоритмыОбработчика Цикл
				...
				Если МассивЗапрошенныхЭлементов.Найти(ВРег(Алгоритм)) <> Неопределено Тогда // Рекурсивный вызов алгоритма.
					...
					ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, Алгоритм, СтрокаПрефиксаДляВложенногоКода, НСтр("ru='{РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА}';uk='{РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА}'"));
					
					СтрокаОператора = НСтр("ru='ВызватьИсключение ""РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА: %1"";';uk='ВызватьИсключение ""РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА: %1"";'");
	...
	ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, АлгоритмВладелец, СтрокаПрефикса, НСтр("ru='{КОНЕЦ АЛГОРИТМА}';uk='{КОНЕЦ АЛГОРИТМА}'"));
...
Процедура ИнициализацияСообщений()
	...
	одСообщения.Вставить(16, НСтр("ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства источника)';uk='Помилка одержання значення властивості підлеглого об`єкта (по імені властивості джерела)'"));
	одСообщения.Вставить(17, НСтр("ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства приемника)';uk='Помилка одержання значення властивості підлеглого об`єкта (по імені властивості приймача)'"));
	...
	одСообщения.Вставить(24, НСтр("ru='Ошибка при удалении объекта';uk='Помилка при видаленні об`єкта'"));
	...
	одСообщения.Вставить(67, НСтр("ru='Ошибка получения свойства подчиненного объекта из входящих данных';uk='Помилка одержання властивості підлеглого об`єкта із вхідних даних'"));
	...
	одСообщения.Вставить(77, НСтр("ru='Не указан файл внешней обработки с подключаемыми процедурами обработчиков событий';uk='Не зазначений файл зовнішньої обробки із підключаємимі процедурами оброблювачів подій'"));
	
	одСообщения.Вставить(78, НСтр("ru='Ошибка создания внешней обработки из файла с процедурами обработчиков событий';uk='Помилка створення зовнішньої обробки з файлу із процедурами оброблювачів подій'"));
...
"%1';uk='При спробі з`єднання з COM-сервером відбулася наступна помилка:"
"%1'");
...
Функция ВыполнитьПодключениеКИБПриемнику() Экспорт
	...
	Исключение
		...
		Текст = НСтр("ru='При попытке создания обработки УниверсальныйОбменДаннымиXML произошла ошибка: %1';uk='При спробі створення обробки УниверсальныйОбменДаннымиXML відбулася помилка: %1'");
		Текст = ПодставитьПараметрыВСтроку(Текст, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		СообщитьПользователю(Текст);
		РезультатПодключения = Неопределено;
	КонецПопытки;
