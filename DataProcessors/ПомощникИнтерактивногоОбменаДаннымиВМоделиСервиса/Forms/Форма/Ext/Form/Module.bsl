...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Объект.УзелИнформационнойБазы.Пустая() Тогда
		ТекстОшибки = НСтр("ru='Настройка обмена данными не найдена.';uk='Налаштування обміну даними не знайдене.'");
...
Процедура ДополнениеВыгрузкиИсторияОтборовВыборИзМеню(Знач ВыбранныйЭлемент, Знач ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ПредставлениеНастройки)=Тип("Строка") Тогда
		...
		ТекстЗаголовка = НСтр("ru='Подтверждение';uk='Підтвердження'");
		ТекстВопроса = СтрШаблон(НСтр("ru='Восстановить настройки ""%1""?';uk='Відновити налаштування ""%1""?'"), ПредставлениеНастройки
		);
