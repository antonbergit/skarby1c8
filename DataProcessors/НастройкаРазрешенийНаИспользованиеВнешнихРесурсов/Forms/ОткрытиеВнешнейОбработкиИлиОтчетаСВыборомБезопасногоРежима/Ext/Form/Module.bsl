...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(ФайлОбработкиИмя) Или Не ЗначениеЗаполнено(ФайлОбработкиАдрес) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите файл внешнего отчета или обработки';uk='Вкажіть файл зовнішнього звіту або обробки'"), , "ФайлОбработкиАдрес");
		Отказ = Истина;
	...
	Если Не ЗначениеЗаполнено(БезопасныйРежим) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите безопасный режим для подключения внешнего модуля';uk='Вкажіть безпечний режим для підключення зовнішнього модуля'"), , "БезопасныйРежим");
		Отказ = Истина;
...
Функция ПодключитьНаСервере()
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Файл %1 не является файлом внешнего отчета или обработки';uk='Файл %1 не є файлом зовнішнього звіту або обробки'"), ФайлОбработкиИмя);
