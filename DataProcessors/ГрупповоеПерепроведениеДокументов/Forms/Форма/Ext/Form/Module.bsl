...
Процедура КнопкаВыполнить(Команда)
	...
	Если ТолькоВыбранныеОрганизации = 1 И НЕ ЗначениеЗаполнено(ТаблицаОрганизаций) Тогда
		ТекстСообщения = НСтр("ru='Не корректно заполнен список организаций.';uk='Не коректно заповнений список організацій.'");
...
"%2';uk='Виконане перепроведення документів:"
" - проведене документів: %1;"
" %2'");
	Если НеУдалосьПровести = 0 Тогда
		ТекстСообщенияОбОбшибках = НСтр("ru=' - ошибок не обнаружено.';uk=' - помилок не виявлено'");
	Иначе
		ТекстСообщенияОбОбшибках = НСтр("ru=' - не удалось провести документов: %1';uk=' - не вдалося провести документів: %1'");
...
Процедура ИнициализацияКомандДокументаНаКлиенте()
	...
	Если Результат = 1 Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ru='Перепроведение уже выполняется. Ожидайте завершения.';uk='Перепроведення вже виконується. Очікуйте завершення.'"));
		Возврат;
	ИначеЕсли Результат = 2 Тогда
		ПоказатьПредупреждение(Неопределено,НСтр("ru='Перепроведение уже выполняется другим пользователем.';uk='Перепроведення вже виконується іншим користувачем.'"));
		Возврат;
	КонецЕсли;
