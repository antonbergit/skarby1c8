...
Процедура УстановитьПодсказкуВводаСтрокиПоискаНаСервере()
	...
	ПодсказкаВвода = ?(ПолнотекстовыйПоискНастроенЧастично, НСтр("ru='Необходимо обновить индекс полнотекстового поиска...';uk='Необхідно обновити індекс повнотекстового пошуку...'"), НСтр("ru='(ALT+F3) Введите текст поиска...';uk='(ALT+F3) Введіть текст пошуку...'"));
...
КонецФункции // ЭтоМарочнаяПродукция()
...
Процедура ДобавитьНоменклатуруВКорзину()
	...
	Иначе
		...
		СтрокаКорзины.НеобходимостьВводаАкцизнойМарки = ЭтоМаркируемаяПродукция(СтрокаКорзины.Номенклатура);
...
Процедура ОтборПоАналогамНоменклатуры(Номенклатура, ТекстСообщения)
	...
	Если СписокАналоговНоменклатуры.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ru='Для номенклатуры не заведены аналоги.';uk='Для номенклатури не заведені аналоги.'");
...
Процедура УстановитьЗаголовкиСписков()
	...
	Если Элементы.ФормаОбновлятьЗаголовки.Пометка Тогда
		...
		Если Элементы.СтраницыНоменклатураХарактеристики.ТекущаяСтраница = Элементы.СтраницаХарактеристики Тогда
			...
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СписокСкладскиеОстаткиСтруктурнаяЕдиница", "Заголовок", НСтр("ru='Склад. остатки: ';uk='Склад. залишки: '") + ПредставлениеНоменклатуры);
...
Процедура ВыводитьСоветИспользоватьПредыдущийПодбор()
	...
	Если КешНастройкиПодбора.ВыводитьСоветИспользоватьПредыдущийПодбор Тогда
		...
		ЗаголовокФормы = Нстр("ru='Совет';uk='Порада'");
		ТекстСообщения = НСтр("ru='Выключить/включить использование новой формы подбора можно в настройках пользователя.';uk='Виключити/включити використання нової форми підбіру можна в налаштуваннях користувача.'");
...
Процедура ВыводитьСоветУслугиВПриходныхДокументах()
	...
	Если КешНастройкиПодбора.ВыводитьСоветУслугиВПриходныхДокументах Тогда
		...
		ЗаголовокФормы = Нстр("ru='Совет';uk='Порада'");
		ТекстСообщения = НСтр("ru='Услуги в приходных документах отражают расходы компании, поэтому выбор характеристик ограничен.';uk='Послуги в прибуткових документах відображають витрати компанії, тому вибір характеристик обмежений.'");
...
Процедура ВыводитьСоветВернутьсяКНоменклатуре()
	...
	Если КешНастройкиПодбора.ВыводитьСоветВернутьсяКНоменклатуре Тогда
		...
		ЗаголовокФормы = Нстр("ru='Совет';uk='Порада'");
		ТекстСообщения = НСтр("ru='Вернуться к списку номенклатуры можно при помощи контекстного меню или клавиши BackSpace.';uk='Повернутися до списку номенклатури можна за допомогою контекстного меню або клавіші BackSpace.'");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаПолнотекстовыйПоискЛюбыеОкончания", "Пометка", Истина);
...
Процедура ПерейтиКСпискуХарактеристикНоменклатуры(НоменклатураСсылка)
	...
	Элементы.НавигацияСписковНоменклатураХарактеристики.Заголовок = НСтр("ru='Номенклатура';uk='Номенклатура'");
...
Процедура ПерейтиКСпискуНоменклатуры()
	...
	Элементы.НавигацияСписковНоменклатураХарактеристики.Заголовок = НСтр("ru='Характеристики';uk='Характеристики'");
