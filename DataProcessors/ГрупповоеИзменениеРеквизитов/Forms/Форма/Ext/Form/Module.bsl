...
Процедура ВыполнитьИзменениеПроверкаОтбораВыполнена(РезультатВопроса = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ЕстьНастроенныеИзменения() И Объект.ТипОперации = "ВвестиЗначения" Тогда
		ТекстВопроса = НСтр("ru='Изменения не настроены. Выполнить перезапись элементов без изменений?';uk='Зміни не настроєні. Виконати перезапис елементів без "
"змін?'");
	...
	Если Не ПустаяСтрока(Пояснение) Тогда
		Если ИзменяемыеТабличныеЧасти.Количество() > 0 Тогда
			Если ЕстьОтборПоСтрокам Тогда 
				Пояснение = Пояснение + " " + НСтр("ru='в тех строках выбранных элементов, которые удовлетворяют условиям отбора';uk='у тих рядках вибраних елементів, які задовольняють умовам "
"відбору'")
			Иначе
				Пояснение = Пояснение + " " + НСтр("ru='<b>во всех строках</b> выбранных элементов';uk='<b>у всіх рядках</b> обраних елементів'")
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьВыбранныеОбъекты Тогда
		...
		Иначе
			Пояснение = НСтр("ru='Выполнить <b>перезапись</b> выбранных элементов.';uk='Виконати <b>перезапис</b> обраних елементів.'");
		КонецЕсли;
	Иначе
...
Процедура УстановитьНастройкуИзменения(Знач Настройка)
	...
	Если ЕстьЗаблокированные Тогда
		ПоказатьПредупреждение(, НСтр("ru='Некоторые реквизиты заблокированы для изменения, изменения не установлены.';uk='Деякі реквізити заблоковані для зміни, зміни не "
"встановлені.'"));
	КонецЕсли;
