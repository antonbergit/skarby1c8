...
"Продолжить?';uk='Обраний елемент позначений на видалення. "
"Продовжити?'"), 
...
Процедура ОперандыОкончаниеПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка)
	...
	Если Элемент.ТекущиеДанные.ПометкаУдаления Тогда
		Ответ = Неопределено;
		ПоказатьВопрос(Новый ОписаниеОповещения("ОперандыОкончаниеПеретаскиванияЗавершение", ЭтотОбъект), НСтр("ru='Выбранный элемент помечен на удаление';uk='Обраний елемент позначений на видалення'") + Символы.ПС + НСтр("ru='Продолжить?';uk='Продовжити?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Функция ПолучитьСтандартноеДеревоОператоров()
	...
	ГруппаОператоров = ДобавитьГруппуОператоров(Дерево, НСтр("ru='Логические операторы и константы';uk='Логічні оператори й константи'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, "<", " < ");
	ДобавитьОператор(Дерево, ГруппаОператоров, ">", " > ");
	ДобавитьОператор(Дерево, ГруппаОператоров, "<=", " <= ");
	ДобавитьОператор(Дерево, ГруппаОператоров, ">=", " >= ");
	...
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='ЛОЖЬ';uk='ЛОЖЬ'"),   " " + НСтр("ru='ЛОЖЬ';uk='ЛОЖЬ'")   + " ");
	...
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='Минимум';uk='Мінімум'"),     НСтр("ru='Мин(,)';uk='Мін(,)'"),  2);
	...
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='Предопределенное значение';uk='Визначене значення'"), НСтр("ru='ПредопределенноеЗначение()';uk='ПредопределенноеЗначение()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='Значение заполнено';uk='Значення заповнене'"), НСтр("ru='ЗначениеЗаполнено()';uk='ЗначениеЗаполнено()'"));
...
Функция ПроверитьФормулу(Формула, Операнды, Знач Поле = "", СтроковаяФормула = Ложь)
	...
	Если ЗначениеЗаполнено(Формула) Тогда
			...
			Если СтроковаяФормула Тогда
				...
				Если ОтсутствиеРазделителей > 0 Тогда
					СообщитьПользователю(
						НСтр("ru='В формуле обнаружены ошибки. Между операндами должен присутствовать оператор или разделитель';uk='У формулі виявлені помилки. Між операндами повинен бути присутнім оператор або роздільник'"),
						Поле,
						ПутьКДанным);
					Результат = Ложь;
				КонецЕсли;
		...
		Исключение
			Результат = Ложь;
			СообщитьПользователю(
				НСтр("ru='В формуле обнаружены ошибки. Проверьте формулу. Формулы должны составляться по правилам написания выражений на встроенном языке 1С:Предприятия.';uk='У формулі виявлені помилки. Перевірте формулу. Формули повинні складатися за правилами написання виражень вбудованою мовою 1С:Підприємства.'"),
				Поле,
				ПутьКДанным);
		КонецПопытки;
