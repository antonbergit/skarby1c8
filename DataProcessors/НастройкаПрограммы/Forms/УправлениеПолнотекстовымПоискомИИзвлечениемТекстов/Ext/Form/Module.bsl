...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РежимРаботы.ЭтоАдминистраторПрограммы И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ФайловыеФункции") Тогда
		Если РежимРаботы.Файловый Тогда
			...
			СписокВыбора[0].Представление = НСтр("ru='Одна или несколько рабочих станций работают под управлением ОС Linux';uk='Одна або декілька робочих станцій працюють під управлінням ОС Linux'");
		КонецЕсли;
	Иначе
...
"Пожалуйста, подождите.';uk='Іде перевірка повнотекстового індексу..."
"Будь ласка, зачекайте.'"));
	...
	Исключение
		ТекстСообщенияОбОшибке = 
			НСтр("ru='В настоящее время проверка индекса невозможна, так как выполняется его очистка или обновление.';uk='У цей час перевірка індексу неможлива, тому що виконується його очищення або відновлення.'");
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "", ИндексПроверен = Ложь)
	...
	Если РеквизитПутьКДанным = ""
		...
		Если ИспользоватьПолнотекстовыйПоиск = 1 Тогда
			...
			Если ИндексПроверен И Не ИндексСодержитКорректныеДанные Тогда
				СтатусИндекса = НСтр("ru='Требуется очистка и обновление';uk='Потрібне очищення й оновлення'");
			ИначеЕсли ИндексАктуален Тогда
				СтатусИндекса = НСтр("ru='Обновление не требуется';uk='Оновлення не потрібно'");
			Иначе
				СтатусИндекса = НСтр("ru='Требуется обновление';uk='Потрібне відновлення'");
			КонецЕсли;
		Иначе
	...
	Если Элементы.ГруппаАвтоматическоеИзвлечениеТекстов.Видимость
		...
		Иначе
			РасписаниеПредставление = НСтр("ru='Автоматическое извлечение текстов не выполняется.';uk='Автоматичне вилучення текстів не виконується.'");
		КонецЕсли;
