...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстОрганизации = НСтр("ru='Для отправки документа подключите организацию ""%1"" к сервису 1С:Бизнес-сеть.';uk='Для отправки документа подключите организацию ""%1"" к сервису 1С:Бизнес-сеть.'");
	ТекстОрганизации = СтрШаблон(ТекстОрганизации, Организация);
	Элементы.ТекстПодключенияОрганизации.Заголовок = ТекстОрганизации;
	
	ТекстКонтрагента = НСтр("ru='Контрагент ""%1"" не подключен к сервису 1С:Бизнес-сеть.';uk='Контрагент ""%1"" не подключен к сервису 1С:Бизнес-сеть.'");
	...
	ПараметрыЗадания.Вставить("МассивСсылокНаОбъект", Параметры.МассивСсылокНаОбъект);
	...
	Если Отказ Тогда
		ТекстСообщения = НСтр("ru='Ошибка запроса истории документов в сервисе 1С:Бизнес-сеть.';uk='Ошибка запроса истории документов в сервисе 1С:Бизнес-сеть.'");
	...
	Если ТипЗнч(ДанныеСервиса) = Тип("Массив") Тогда
		...
		Для каждого ЭлементМассива Из ДанныеСервиса Цикл
			...
			НоваяСтрока.Статус        = ЭлементМассива.deliveryStatus;
...
Процедура ПодключитьОрганизациюПриИзменении(Элемент)
	...
	Элементы.Отправить.Доступность = ПодключитьОрганизацию;
...
Процедура ИсторияНажатие(Элемент, СтандартнаяОбработка)
	...
	Иначе
		ФормаПросмотраЭД = ОткрытьФорму("Обработка.БизнесСеть.Форма.ИсторияОтправки",
			Новый Структура("СтруктураЭД, РежимПросмотраИстории", МассивСтруктур, Истина), , ЭтотОбъект);
	КонецЕсли;
...
Процедура ЗагрузитьПодготовленныеДанныеЭД()
	...
	Для каждого СтрокаТаблицы Из ТаблицаДанных Цикл
		...
		ШаблонНаименования = НСтр("ru='%1 %2 от %3';uk='%1 %2 от %3'");
		
		Тип = ТипыДокументов.НайтиПоЗначению(СтрокаТаблицы.ВидЭД);
		Если Тип = Неопределено Тогда
			ВызватьИсключение НСтр("ru='Не определен тип документа.';uk='Не определен тип документа.'");
		КонецЕсли;
	...
	ТекстГиперссылки = НСтр("ru='документы не найдены';uk='документы не найдены'");
	Если ТаблицаДанных.Количество() > 1 Тогда
		ТекстГиперссылки = НСтр("ru='открыть список (%1)';uk='открыть список (%1)'");
...
Процедура ИзменитьВидимостьДоступностьЭлементов()
	...
	Элементы.ГруппаПодключениеКонтрагента.Видимость = Не КонтрагентПодключен;
	...
	Если НЕ КонтрагентПодключен И ЗначениеЗаполнено(АдресПочты) Тогда
		НаправитьУведомление = Истина;
		Элементы.ЭлектронныйАдресУведомления.Доступность = Истина;
	КонецЕсли;
	
	Если СписокИстории.Количество() = 0 Тогда
		Элементы.История.Гиперссылка = Ложь;	
		Состояние = НСтр("ru='не отправлен';uk='не отправлен'");
	ИначеЕсли СписокИстории.Количество() = 1 Тогда 
		Элементы.История.Гиперссылка = Истина;	
		Состояние = НСтр("ru='отправлен';uk='отправлен'") + " " + Формат(СписокИстории[0].Дата, "ДЛФ=D");
	Иначе
		Элементы.История.Гиперссылка = Истина;	
		ШаблонСостояния = НСтр("ru='отправлено (%1)';uk='отправлено (%1)'");
...
Процедура ОтправитьДокументыВСервис()
	...
	Если Не Отказ Тогда
		ТекстОповещения	= НСтр("ru='Отправка выполнена.';uk='Отправка выполнена.'");
		ТекстПояснения	= НСтр("ru='Отправлен документ через сервис 1С:Бизнес-сеть.';uk='Отправлен документ через сервис 1С:Бизнес-сеть.'");
...
Процедура ОтправитьДокументыНаСервере(Отказ)
	...
	Если ПодключитьОрганизацию Тогда
