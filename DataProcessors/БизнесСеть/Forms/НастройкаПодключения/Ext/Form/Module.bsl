...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ИнформационнаяБазаПодключенаКСервису() Тогда
		ТекстОшибки = НСтр("ru='Информационная база не подключена к сервису 1С:Бизнес-сеть.';uk='Информационная база не подключена к сервису 1С:Бизнес-сеть.'");
...
Процедура СинхронизироватьПользователей(Команда)
	...
	Текст = НСтр("ru='Пользователи информационной базы синхронизированы с сервисом 1С:Бизнес-сеть.';uk='Пользователи информационной базы синхронизированы с сервисом 1С:Бизнес-сеть.'");
...
Процедура СинхронизироватьПодключение(Команда)
	...
	Текст = НСтр("ru='Информационная база синхронизирована с сервисом 1С:Бизнес-сеть.';uk='Информационная база синхронизирована с сервисом 1С:Бизнес-сеть.'");
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст);
...
Процедура ОбновитьИнформациюДляТехническойПоддержки()
	...
	Шаблон = НСтр("ru='Подключение к сервису 1С:Бизнес-сеть:';uk='Подключение к сервису 1С:Бизнес-сеть:'");
	Если НЕ ПустаяСтрока(ИдентификаторИнформационнойБазы) Тогда
		Шаблон = Шаблон + Символы.ПС + НСтр("ru='Ключ информационной базы: %1';uk='Ключ информационной базы: %1'");
	КонецЕсли; 
	Если НЕ ПустаяСтрока(ИдентификаторПользователя) Тогда
		Шаблон = Шаблон + Символы.ПС + НСтр("ru='Ключ пользователя: %2';uk='Ключ пользователя: %2'");
	КонецЕсли;
	
	Шаблон = Шаблон + Символы.ПС + НСтр("ru='Версия библиотеки электронных документов: %3';uk='Версия библиотеки электронных документов: %3'");
...
Процедура ОбновитьИнформациюСостояниеСинхронизацииПользователей()
	...
	Если Запрос.Выполнить().Пустой() Тогда
		...
		СостояниеСинхронизацииПользователей = НСтр("ru='Синхронизация пользователей не требуется.';uk='Синхронизация пользователей не требуется.'");
		Элементы.СостояниеСинхронизацииПользователей.ЦветТекста = ЦветаСтиля.ПоясняющийТекст;
		
	Иначе
		
		СостояниеСинхронизацииПользователей = НСтр("ru='Требуется синхронизация пользователей.';uk='Требуется синхронизация пользователей.'");
