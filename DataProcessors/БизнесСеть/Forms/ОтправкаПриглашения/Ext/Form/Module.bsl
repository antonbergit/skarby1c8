...
Процедура ОтправитьПриглашенияЧерезБизнесСеть()
	...
	Если Результат.КодСостояния = 200 Тогда
		ТекстОповещения= НСтр("ru='Отправка приглашения';uk='Отправка приглашения'");
		ТекстПояснения = НСтр("ru='Приглашение отправлено контрагенту %1';uk='Приглашение отправлено контрагенту %1'");
		...
		Закрыть();
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка отправления приглашения.';uk='Ошибка отправления приглашения.'"));
	КонецЕсли;
...
Процедура ТекстСообщенияНажатие(Элемент)
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ru='Пример текста приглашения к обмену электронными документами';uk='Пример текста приглашения к обмену электронными документами'"));
	ПараметрыФормы.Вставить("Пояснение", НСтр("ru='Скопируйте текст сообщения в буфер обмена и отправьте получателю любым удобным способом.';uk='Скопируйте текст сообщения в буфер обмена и отправьте получателю любым удобным способом.'"));
...
Процедура ОтправитьПриглашениеИзПрограммы()
	...
	ПараметрыФормы = Новый Структура;
	ТемаПисьма = НСтр("ru='Приглашение для подключения к сервису 1С:Бизнес-сеть';uk='Приглашение для подключения к сервису 1С:Бизнес-сеть'");
