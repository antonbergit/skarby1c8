...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.СтруктураЭД) <> Тип("Структура") 
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка открытия электронного документа';uk='Ошибка открытия электронного документа'"),,,, Отказ);
		Возврат;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтоВходящийДокумент Тогда
		Шаблон = НСтр("ru='%1 (электронный документ)';uk='%1 (электронный документ)'");
	...
	Иначе
		...
		Иначе
			...
			Заголовок = СтрШаблон(Шаблон, Источник.documentTitle);
		КонецЕсли;
		Если ВРЕГ(Статус) = "SENT" Тогда
			Шаблон = НСтр("ru='отправлен %1';uk='отправлен %1'");
			Состояние = СтрШаблон(Шаблон, 
				БизнесСетьКлиентСервер.ДатаИзUnixTime(Источник.sentDate));
		ИначеЕсли ВРЕГ(Статус) = "DELIVERED" Тогда
			Шаблон = НСтр("ru='отправлен %1, получен %2';uk='отправлен %1, получен %2'");
			Состояние = СтрШаблон(Шаблон, 
				БизнесСетьКлиентСервер.ДатаИзUnixTime(Источник.sentDate),
...
Процедура ВыполнитьПросмотрЭДСервер(СтруктураЭД, Отказ)
	...
	Если СтруктураЭД.ФайлАрхива Тогда
		...
		Иначе
			ВызватьИсключение НСтр("ru='Отсутствует подсистема ""Обмен с контрагентами"".';uk='Отсутствует подсистема ""Обмен с контрагентами"".'");
		КонецЕсли;
