...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Пользователи.РолиДоступны("АдминистрированиеАбонентаБизнесСеть") Тогда
		ТекстОшибки = НСтр("ru='Недостаточно прав для выполнения операции в сервисе 1С:Бизнес-сеть.';uk='Недостаточно прав для выполнения операции в сервисе 1С:Бизнес-сеть.'");
	...
	Если Не ОдиночноеПодключение И Не ИспользуетсяНесколькоОрганизаций Тогда 
		...
		Если Не ЗначениеЗаполнено(Параметры.Ссылка) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось определить организацию.';uk='Не удалось определить организацию.'"),,,, Отказ);
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
"При этом, для других участников сервиса организации будут отображаться как подключенные.';uk='Организации будут отключены от сервиса 1С:БизнесСеть в программе (информационной базе)."
...
"Обмен электронными документами и другие функции сервиса будут недоступны.';uk='Выбранные организации будут удалены в сервисе 1С:БизнесСеть."
...
"Обмен электронными документами и другие функции сервиса будут недоступны.';uk='Организация будет удалена в сервисе 1С:БизнесСеть."
"Обмен электронными документами и другие функции сервиса будут недоступны.'");
...
"При этом, для других участников сервиса организация будет отображаться как подключенная.';uk='Будет произведено отключение организации от сервиса 1С:БизнесСеть в программе (информационной базе)."
...
Процедура ОтобразитьСостояниеПодключенияИПП()
	...
	Если ЗначениеЗаполнено(Логин) Тогда
		...
		ШаблонЛогина    = НСтр("ru='Интернет-поддержка: подключена для пользователя <b>%1</b>';uk='Интернет-поддержка: подключена для пользователя <b>%1</b>'");
		ШаблонПодсказки = НСтр("ru='Подключение организаций производится с присоединением к учетной записи Интернет-поддержки <b>%1</b>.';uk='Подключение организаций производится с присоединением к учетной записи Интернет-поддержки <b>%1</b>.'");
		ШаблонЛогина = СтрШаблон(ШаблонЛогина, Логин);
		ШаблонПодсказки = СтрШаблон(ШаблонПодсказки, Логин);
	Иначе
		ШаблонЛогина    = НСтр("ru='Интернет-поддержка: требуется подключение.';uk='Интернет-поддержка: требуется подключение.'");
		ШаблонПодсказки = НСтр("ru='Требуется подключение Интернет-поддержки в программе."
"Подключение организаций производится с присоединением к учетной записи Интернет-поддержки.';uk='Требуется подключение Интернет-поддержки в программе."
...
Процедура ПодключитьПослеПроверкиАутентификации(Знач ПараметрыАутентификации, Контекст) Экспорт
	...
	Если Не Отказ Тогда
		Если ОдиночноеПодключение Тогда
			ТекстОповещения = НСтр("ru='Подключение организации успешно выполнено.';uk='Подключение организации успешно выполнено.'");
			СсылкаОповещения = ПолучитьНавигационнуюСсылку(Параметры.Ссылка);
		Иначе
			ТекстОповещения = НСтр("ru='Подключение организаций успешно выполнено.';uk='Подключение организаций успешно выполнено.'");
			СсылкаОповещения = Неопределено;
		КонецЕсли;
		ПоказатьОповещениеПользователя(НСтр("ru='1С:Бизнес-сеть';uk='1С:Бизнес-сеть'"), СсылкаОповещения, ТекстОповещения, 
...
Процедура ПроверитьСсылкиПодключения(МассивСсылок, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		Если ПустаяСтрока(Выборка.ИНН) Тогда
			ТекстОшибки = НСтр("ru='Не заполнено свойство ""ИНН"" %1 ""%2"".';uk='Не заполнено свойство ""ИНН"" %1 ""%2"".'");
...
Процедура ВыполнитьОтключениеОрганизаций(РежимУдаленияВСервисе)
	...
	Если Не Отказ Тогда
		Если РежимУдаленияВСервисе Тогда
			ТекстОповещения = НСтр("ru='Удаление организаций в сервисе успешно выполнено.';uk='Удаление организаций в сервисе успешно выполнено.'");
		Иначе
			ТекстОповещения = НСтр("ru='Отключение организаций в программе успешно выполнено.';uk='Отключение организаций в программе успешно выполнено.'");
		КонецЕсли;
		ПоказатьОповещениеПользователя(НСтр("ru='1С:Бизнес-сеть';uk='1С:Бизнес-сеть'"),, ТекстОповещения, БиблиотекаКартинок.БизнесСеть);
	КонецЕсли;
...
Процедура ВыполнитьОтключениеОрганизацииОдиночное(РежимУдаленияВСервисе)
	...
	Если Не Отказ Тогда
		Если РежимУдаленияВСервисе Тогда
			ТекстОповещения = НСтр("ru='Удаление организации в сервисе успешно выполнено.';uk='Удаление организации в сервисе успешно выполнено.'");
		Иначе
			ТекстОповещения = НСтр("ru='Отключение организации в программе успешно выполнено.';uk='Отключение организации в программе успешно выполнено.'");
		КонецЕсли;
		ПоказатьОповещениеПользователя(НСтр("ru='1С:Бизнес-сеть';uk='1С:Бизнес-сеть'"),, ТекстОповещения, БиблиотекаКартинок.БизнесСеть);
