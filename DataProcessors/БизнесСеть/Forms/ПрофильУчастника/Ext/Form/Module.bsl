...
Функция СформироватьТекстПрофиляHTML(Реквизиты)
	...
	ЗаголовокHTML = СтрЗаменить(ЗаголовокHTML, "[НадписьИНН]", НСтр("ru='ИНН';uk='ІПН'"));
	ЗаголовокHTML = СтрЗаменить(ЗаголовокHTML, "[ИНН]", Реквизиты.ИНН);
	Если ЗначениеЗаполнено(Реквизиты.КПП) Тогда
		ЗаголовокHTML = СтрЗаменить(ЗаголовокHTML, "[НадписьКПП]", НСтр("ru='КПП';uk='КПП'"));
	...
	КонецЕсли;
	ЗаголовокHTML = СтрЗаменить(ЗаголовокHTML, "[НадписьДатаПодключения]", НСтр("ru='Дата подключения:';uk='Дата подключения:'"));
	...
	Если ЗначениеЗаполнено(Реквизиты.НаименованиеЕГРН) Тогда
		Строка1 = НСтр("ru='Наименование в едином государственном реестре налогоплательщиков';uk='Наименование в едином государственном реестре налогоплательщиков'");
...
Процедура ЗаполнитьПрофиль(ПерезаполнитьДанные = Ложь)
	...
	Заголовок = НСтр("ru='%1 (профиль 1С:Бизнес-сеть)';uk='%1 (профиль 1С:Бизнес-сеть)'");
