...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтандартныеПодсистемыСервер.УстановитьУсловноеОформлениеПоляДата(ЭтаФорма, "Список.Дата", "ДатаДокумента");
...
"Продолжить выполнение операции?';uk='Выделенные документы будут удалены в сервисе 1С:Бизнес-сеть."
"Документы учета информационной базы не изменятся."
...
Процедура ОбновитьСписок()
	...
	Если Отказ ИЛИ Результат.КодСостояния <> 200 Тогда Возврат КонецЕсли;
	
	ВходящиеДокументы = Результат.Данные;
	Если ВходящиеДокументы = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Список.Очистить();
	Для каждого ЭлементМассива Из ВходящиеДокументы Цикл
...
Процедура УдалитьДокументВСервисеПослеВопроса(Результат, МассивСтрок) Экспорт
	...
	Для каждого ВыборкаПоОрганизации Из ДокументыПоОрганизациям Цикл
		...
		Если Отказ И Результат.КодСостояния <> 200 Тогда
			Возврат;
		КонецЕсли;
	...
	Если КоличествоСтрок = 1 Тогда
		ТекстОповещения	= НСтр("ru='Документ удален.';uk='Документ удален.'");
		ТекстПояснения	= НСтр("ru='Удален входящий документ в сервисе 1С:Бизнес-сеть.';uk='Удален входящий документ в сервисе 1С:Бизнес-сеть.'");
	Иначе
		ТекстОповещения	= НСтр("ru='Документы удалены (%1).';uk='Документы удалены (%1).'");
		ТекстОповещения = СтрШаблон(ТекстОповещения, КоличествоСтрок);
		ТекстПояснения	= НСтр("ru='Удалены входящие документы в сервисе 1С:Бизнес-сеть.';uk='Удалены входящие документы в сервисе 1С:Бизнес-сеть.'");
	КонецЕсли;
