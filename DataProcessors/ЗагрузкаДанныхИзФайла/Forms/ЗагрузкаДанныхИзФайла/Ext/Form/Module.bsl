...
Процедура Назад(Команда)
	...
	Если Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.ЗаполнениеТаблицыДанными Тогда
		...
		ЭтотОбъект.Заголовок = НСтр("ru='Загрузка данных в справочник';uk='Завантаження даних у довідник'");
		ОчиститьТаблицу();
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СопоставлениеЗагружаемыхДанных Тогда
		...
		Если ТипЗагрузки = "ТабличнаяЧасть" Тогда
			...
			Если Строки.Количество() > 0 Тогда
				...
				ПоказатьВопрос(Оповещение, НСтр("ru='Строки, в которых не заполнены обязательные колонки, будут пропущены.';uk='Рядки, в яких не заповнені обов`язкові колонки, будуть пропущені.'")
			...
			Закрыть(АдресЗагруженныхДанных);
		ИначеЕсли ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
			Элементы.Назад.Заголовок = НСтр("ru='< В начало_';uk='< На початок_'");
			ЗакрытьФормуИВернутьМассивСсылок();
		Иначе
...
Процедура ВыполнитьСопоставление()
	...
	ПоказатьОповещениеПользователя(НСтр("ru='Выполнено сопоставление';uk='Виконане зіставлення'"),, НСтр("ru='Сопоставлено элементов:';uk='Зіставлене елементів:'") + " " + Строка(КоличествоСопоставленныхПоКолонкам));
	ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла();
КонецПроцедуры
...
Процедура ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла()
	...
	Если Статистика.Неоднозначных > 0 Тогда 
		...
		Элементы.ОписаниеНеоднозначность.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='(неоднозначностей: %1)';uk='(неоднозначностей: %1)'"),
...
Процедура ИнициализацияВставкиИзБуфераОбмена()
	...
	Если Параметры.Свойство("ПредставлениеПоля") Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Вставка из буфера обмена (%1)';uk='Вставка из буфера обмена (%1)'"), Параметры.ПредставлениеПоля);
	Иначе
...
Процедура УстановитьВидимостьЭлементовФормы()
	ЭтотОбъект.Заголовок = ?(ПустаяСтрока(Параметры.Заголовок), НСтр("ru='Загрузка данных в справочник';uk='Завантаження даних у довідник'"), Параметры.Заголовок);
...
" (например: Microsoft Office Excel, OpenOffice Calc и др.).';uk='Вибір довідника для завантаження даних з електронних таблиць, розташованих у зовнішніх файлах"
" (наприклад: Microsoft Office Excel, OpenOffice Calc і ін.). '");
		ВидЗагрузкиДанных = 1;
	КонецЕсли;
...
Процедура СформироватьМакетПоТипуЗагрузки()
	...
	Если ТипЗагрузки = "УниверсальнаяЗагрузка" Тогда
		ЭтотОбъект.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Загрузка данных в справочник ""%1""';uk='Завантаження даних в довідник ""%1""'"), ПредставлениеСправочника(ИмяОбъектаСопоставления));
...
Процедура СопоставитьЗагружаемыеДанные()
	...
	Если ТаблицаСДаннымиПустая() Тогда
		Если ВариантЗагрузки = 0 Тогда
			ПоказатьПредупреждение(, (НСтр("ru='Для сопоставления и загрузки данных, необходимо заполнить таблицу.';uk='Для зіставлення і завантаження даних, необхідно заповнити таблицю.'")));
		Иначе
			ПоказатьПредупреждение(, (НСтр("ru='Невозможно выполнить сопоставление данных, т.к данные не были загружены в таблицу. "
"Возможно, имена колонок в файле не соответствуют колонкам в бланке.';uk='Неможливо виконати зіставлення даних, т.к дані не були завантажені в таблицю. "
"Можливо, імена колонок у файлі не відповідають колонкам у бланку.'")));
		КонецЕсли;
		ВыполнитьШагЗаполнениеТаблицыДаннымиНаКлиенте();
	...
	Если СписокНезаполненныхКолонок.Количество() > 0 Тогда
		Если СписокНезаполненныхКолонок.Количество() = 1 Тогда
			ТекстПроКолонки = НСтр("ru='Обязательная колонка""';uk='Обов`язковий стовпчик""'") + " " + СписокНезаполненныхКолонок[0]
		...
		Иначе
			ТекстПроКолонки = НСтр("ru='Обязательные колонки""';uk='Обов`язкові стовпчики""'") + " " + СтрСоединить(СписокНезаполненныхКолонок,", ")
...
Процедура ПоказатьОтчет(АдресРезультата)
	...
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("ВсеЭлементы", НСтр("ru='Все (';uk='Всі ('") + Отчет.Всего + ")");
...
"Техническая информация: %1';uk='Можлива причина: Дані, що завантажуються, некоректні."
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхКлиент()
	...
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		...
		Если Статистика.Сопоставленных > 0 Тогда
			ТекстНайдено = НСтр("ru='Из %1 введенных строк в список будут вставлены: %2.';uk='З %1 уведених рядків у список будуть вставлені: %2.'");
			...
			Если Статистика.Неоднозначных > 0 И Статистика.НеНайдено > 0 Тогда 
				ТекстНеНайдено = НСтр("ru='%3 строк будут пропущены:';uk='%3 строк будут пропущены:'") + Символы.ПС + "  - " + НСтр("ru='Нет данных в программе: %1';uk='Немає даних у програмі: %1'") 
					+ Символы.ПС + "  - " +НСтр("ru='Несколько вариантов для вставки: %2';uk='Декілька варіантів для вставки: %2'");
				ТекстНеНайдено = НСтр("ru='Будет пропущено строк: %3"
"  - Нет данных в программе: %1"
"  - Несколько вариантов для вставки: %2';uk='Буде пропущено строк: %3"
...
"  - Кілька варіантів для вставки: %2 '");
			...
			ИначеЕсли Статистика.Неоднозначных > 0 Тогда
				ТекстНеНайдено = НСтр("ru='Строки, для которых в программе имеется несколько вариантов, будут пропущены: %1';uk='Рядки, для яких у програмі є кілька варіантів, будуть пропущені: %1'");
			...
			ИначеЕсли Статистика.НеНайдено > 0 Тогда
				ТекстНеНайдено = НСтр("ru='Строки, для которых в программе нет соответствующих данных, будут пропущены: %1';uk='Рядки, для яких у програмі немає відповідних даних, будуть пропущені: %1'");
				ТекстНеНайдено = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНеНайдено, Статистика.НеНайдено);
			КонецЕсли;
			ТекстНеНайдено = ТекстНеНайдено + Символы.ПС + НСтр("ru='Для просмотра пропущенных строк и подбора данных для вставки нажмите ""Далее"".';uk='Для перегляду пропущених рядків і добору даних для вставки натисніть ""Далі"".'");
...
Процедура УстановитьОформлениеДанных(СписокКолонок = Неопределено)
	...
	Если ЗначениеЗаполнено(СписокКолонок) Тогда
		Для каждого ИмяКолонки Из СписокКолонок Цикл
			...
			ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<неоднозначность>';uk='<неоднозначність>'"));
		КонецЦикла;
	Иначе
		ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
		ПолеОформления = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
...
Функция МенеджерОбъекта(ИмяОбъектаСопоставления)
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Объект ""%1"" не найден';uk='Об`єкт ""%1"" не знайдений'"), ИмяОбъектаСопоставления);
		КонецЕсли;
...
"• Табличный документ (.mxl)';uk='Для завантаження даних необхідно, зберегти бланк в файл для заповнення в іншій програмі."
...
Процедура ПослеЗагрузкиФайлаСДаннымиВТабличныйДокумент(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Если ФоновоеЗадание.Статус = "Выполнено" Тогда
		...
		СопоставитьЗагружаемыеДанные();
	Иначе
		ВывестиСообщениеОбОшибке(НСтр("ru='Не удалось произвести загрузку данных.';uk='Не вдалося зробити завантаження даних.'"), ФоновоеЗадание.КраткоеПредставлениеОшибки);
	КонецЕсли;
...
Функция СопоставитьЗагружаемыеДанныеНаСервереУниверсальнаяЗагрузка()
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ru='Заполнение таблицы сопоставления загруженными данными из файла.';uk='Заповнення таблиці зіставлення завантаженими даними з файлу.'");
...
Процедура ПослеСопоставленияЗагружаемыхДанных(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ФоновоеЗадание.Статус = "Ошибка" Тогда
		ВывестиСообщениеОбОшибке(НСтр("ru='Не удалось произвести сопоставление данных.';uk='Не вдалося зробити зіставлення даних.'"),
...
Процедура ПослеЗаписиЗагружаемыхДанныхОтчет(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Если ФоновоеЗадание.Статус = "Ошибка" Тогда
		ВывестиСообщениеОбОшибке(НСтр("ru='Не удалось произвести запись данных.';uk='Не вдалося зробити запис даних.'"), ФоновоеЗадание.КраткоеПредставлениеОшибки);
