...
Процедура ИнформациюПоКолонкамИзРеквизитовСправочника(ПараметрыЗагрузки, ИнформацияПоКолонкам)
	...
	Для каждого Реквизит Из МетаданныеСправочника.Реквизиты Цикл
		...
		Если Реквизит.Тип.СодержитТип(Тип("Булево")) Тогда 
			ОписаниеТипаКолонки = НСтр("ru='Флаг, Да или 1 / Нет или 0';uk='Прапорець, Так або 1 / Ні або 0'");
...
Процедура СоздатьКолонкуСтандартныеРеквизиты(ИнформацияПоКолонкам, МетаданныеСправочника, ИмяКолонки, Позиция)
	...
	Если ТипДанных = Тип("Строка") Тогда 
		ПредставлениеТипа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Строка (не более %1 символов)';uk='Рядок (не більш %1 символів)'"), ОписаниеТипа.КвалификаторыСтроки.Длина);
...
Процедура ОпределитьИнформацияПоКолонкамТабличнаяЧасть(Знач ИнформацияПоКолонкам, Макет, ОбластьЗаголовокТаблицы, Знач ПараметрыЗагрузки)
	...
	Иначе
		ВызватьИсключение НСтр("ru='Не найден макет для загрузки данных из файла';uk='Не знайдений макет для завантаження даних з файлу'");
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьCSVФайлВТаблицу(ИмяФайла, ШаблонСДанными, ИнформацияПоКолонкам)
	...
	Если Строка = Неопределено Тогда 
		ТекстСообщения = НСтр("ru='Не получилось загрузить данные из этого файла. Убедитесь в корректности данных в файле.';uk='Не вийшло завантажити дані із цього файлу. Переконайтеся в коректності даних у файлі.'");
		Возврат;
	КонецЕсли;
...
"%2';uk='Не вдалося записати елемент довідника %1 з причини:"
"%2'"), 
