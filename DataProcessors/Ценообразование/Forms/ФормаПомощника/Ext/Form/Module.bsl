...
Процедура ЗаписатьНовыеЦеныНаСервере()
	...
	ОписаниеОперации	= НСтр("ru='Запись новых цен в информационную базу';uk='Запис нових цін в інформаційну базу'");
...
"Производится запись новых.';uk='"
"Зачекайте, будь ласка. "
"Проводиться запис нових.'");
...
Процедура РасчитатьДанныеКоллекцииВДлительнойОперации(ПараметрыРасчета)
	...
	ОписаниеОперации	= НСтр("ru='Изменение колонок с новыми ценами по формуле';uk='Зміна стовпчиків з новими цінами по формулі'");
...
Процедура ДобавитьНоменклатуруВДлительнойОперации(СтруктураТаблицДанных, ПараметрыКопирования)
	...
	ОписаниеОперации	= НСтр("ru='Копирование полученых строк в табличные части обработки';uk='Копіювання отриманих рядків у табличні частини обробки'");
...
Процедура ДобавитьРеквизитыИЭлементыТабличнойЧастиПоВидуЦен(ИмяТабличнойЧасти, ВыбранныйВидЦенСоответствие)
	...
	РеквизитЦенаДо		= Новый РеквизитФормы(ПрефиксРеквизитов + "_ЦенаДо",		ТипЧислоПлюс,	ПутьРеквизита, НСтр("ru='Цена до';uk='Ціна до'"));
	РеквизитДельта		= Новый РеквизитФормы(ПрефиксРеквизитов + "_Дельта",		ТипЧислоЛюбое,	ПутьРеквизита, "Δ");
	РеквизитЦенаНовая	= Новый РеквизитФормы(ПрефиксРеквизитов + "_ЦенаНовая",		ТипЧислоПлюс,	ПутьРеквизита, НСтр("ru='Цена новая';uk='Ціна нова'"));
...
Процедура ЗаписыватьНовыеЦеныПоверхУстановленныхПриИзменении(Элемент)
	...
	Если НЕ ПоказыватьХарактеристики Тогда
		...
		Элементы.УстанавливатьХарактеристикамБезЦен.Заголовок = ?(ЗаписыватьНовыеЦеныПоверхУстановленных,
			НСтр("ru='Устанавливать цены номенклатуры характеристикам';uk='Встановлювати ціни номенклатури характеристикам'"),
			НСтр("ru='Устанавливать цены номенклатуры характеристикам без цен';uk='Встановлювати ціни номенклатури характеристикам без цін'"));
...
Процедура ДекорацияПредупреждениеНажатие(Элемент)
	...
	ТекстПредупреждения = НСтр("ru='Изменение параметра невозможно из-за заполненной табличной части номенклатуры';uk='Зміна параметра неможлива через заповнену табличну частину номенклатури'");
...
Процедура ПечатьТабличнойЧасти(Команда)
	...
	ПечатнаяФорма.СинонимМакета = НСтр("ru='Табличная часть обработки формирования цен';uk='Таблична частина обробки формування цін'");
	ПечатнаяФорма.ТабличныйДокумент = ТабличныйДокумент;
	ПечатнаяФорма.ИмяФайлаПечатнойФормы = НСтр("ru='Табличная часть обработки формирования цен';uk='Таблична частина обробки формування цін'");
...
"Продолжить выполнение печати?';uk='Цінники можуть формуватися тільки за записаними цінами,"
"у той же час у форми є не збережені дані."
"Продовжити виконання друку?'");
...
Функция Подключаемый_СтраницаОдин_ПриПереходеДалее(Отказ)
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru='Необходимо указать виды цен, для которых планируется установка новых цен.';uk='Необхідно вказати види цін, для яких планується установка нових цін.'");
...
Функция Подключаемый_СтраницаЧетыре_ПриПереходеДалее(Отказ)
	...
	Иначе
		...
		ТекстОшибки = НСтр("ru='Нет видов цен (колонок прайс-листа), для которых требуется записать новые цены.';uk='Немає видів цін (колонок прайс-листа), для яких потрібно записати нові ціни.'");
