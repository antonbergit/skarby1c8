...
Процедура НовыеЦеныЗаПериод(Ошибки)
	...
	Если ЦеныРасчитаны Тогда
		...
		Элементы.ФормаУтвердитьЦены.Заголовок = НСтр("ru='Утвердить новые цены (';uk='Затвердити нові ціни ('") + Строка(ЦеныНоменклатуры.Количество()) + НСтр("ru=')';uk=')'");
...
Процедура ЗаписатьНовыеЦены()
	...
	ШаблонТекста1 = НСтр("ru='Новые цены записаны на дату ';uk='Нові ціни записані на дату'");
	ШаблонТекста2 = НСтр("ru='по виду цен ';uk='по виду цін'");
