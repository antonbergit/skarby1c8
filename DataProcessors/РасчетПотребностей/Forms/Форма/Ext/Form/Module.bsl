...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ ВОССТАНОВЛЕНИЯ И СОХРАНЕНИЯ НАСТРОЕК

&НаСервере
// Процедура восстанавливает пользовательские настройки.
//
...
&НаСервере
// Процедура сохраняет пользовательские настройки.
//
...
&НаСервере
// Процедура обновляет отборы, имеющие зависимость от ФО.
//
Процедура ОбновитьСписокВыбораСпособПополнения()
	...
	Если ДоступныЗакупки Тогда
		Если ПолучитьФункциональнуюОпцию("ПередачаСырьяВПереработку") И ПравоДоступа("ИнтерактивноеПроведение", Метаданные.Документы.ОтчетПереработчика) Тогда
			Элементы.ОтборСпособПополнения.СписокВыбора.Добавить("Закупка и переработка", НСтр("ru='Закупка и переработка';uk='Закупівля і переробка'"));
		Иначе
			Элементы.ОтборСпособПополнения.СписокВыбора.Добавить("Закупка", НСтр("ru='Закупка';uk='Закупівля'"));
		КонецЕсли;
	КонецЕсли;
	
	Если ДоступноПроизводство Тогда
		Элементы.ОтборСпособПополнения.СписокВыбора.Добавить("Производство", НСтр("ru='Производство';uk='Виробництво'"));
		Если ДоступныЗакупки Тогда
			Элементы.ОтборСпособПополнения.СписокВыбора.Добавить("Все", НСтр("ru='Все';uk='Всі'"));
		КонецЕсли;
	КонецЕсли;
...
КонецПроцедуры // ОбновитьСписокВыбораСпособПополнения()

&НаСервере
// Процедура обновляет отбор: способ пополнения.
//
...
КонецПроцедуры // ОбновитьСпособПополнения()

////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ ОБЩЕГО НАЗНАЧЕНИЯ

&НаСервере
// Процедура обработки данных и вывода графика потребностей на форму.
//
...
&НаСервере
// Процедура обновления колонок формы.
//
...
&НаСервере
// Процедура получает акутальный вид цен контрагента.
//
...
КонецФункции // ПолучитьАктуальныеВидЦенКонтрагента()

&НаСервере
// Процедура добавляет номенклатуру и характеристику.
//
Процедура ДобавитьНоменклатуруХарактеристику(ТекущаяНоменклатура, СтруктураДетализация, СтруктураРасшифровка, ДатаНачала, ДатаОкончания)
	...
	Если ТолькоДефицит И СтруктураДетализация.Дефицит.ЗначениеПоказателя > 0 
		...
		НачальныйОстаток.Номенклатура = НСтр("ru='Начальный остаток';uk='Початковий залишок'");
		
		Поступление = НоменклатураЭлементы.Добавить();
		Поступление.Номенклатура = НСтр("ru='Поступление';uk='Надходження'");
		
		Потребность = НоменклатураЭлементы.Добавить();
		Потребность.Номенклатура = НСтр("ru='Потребность';uk='Потреба'");
		...
		Иначе
			...
			МинимальныйЗапас.Номенклатура = НСтр("ru='Минимальный запас';uk='Мінімальний запас'");
			
			МаксимальныйЗапас = НоменклатураЭлементы.Добавить();
			МаксимальныйЗапас.Номенклатура = НСтр("ru='Максимальный запас';uk='Максимальний запас'");
			
		КонецЕсли;	
		
		КонечныйОстаток = НоменклатураЭлементы.Добавить();
		КонечныйОстаток.Номенклатура = НСтр("ru='Конечный остаток';uk='Кінцевий залишок'");
...
КонецПроцедуры // ДобавитьНоменклатуруХарактеристику()	

&НаСервере
// Функция проверяет заполнение детализированных данных.
//
...
КонецФункции // ПоказателиЗаполнены()

&НаСервере
// Процедура рассчитывает график движения запасов.
//
...
КонецПроцедуры // РассчитатьГрафикДвиженияЗапасов()

&НаСервере
// Процедура добавляет расшифровку по заказу.
//
...
КонецПроцедуры // ДобавитьРасшифровкуПоЗаказу()
...
КонецПроцедуры // ОбновитьРекомендацииНаСервере()

&НаСервере
// Процедура обработки данных и формирования заказов.
//
Процедура СформироватьЗаказыНаСервере()
	...
	Для каждого ПараметрыЗаказа из ТаблицаЗаказы Цикл
		...
		ТекущийЗаказ.Комментарий = НСтр("ru='Сформирован автоматически сервисом ""Расчет потребностей в запасах"".';uk='Сформований автоматично сервісом ""Розрахунки потреб у запасах"".'");
...
КонецПроцедуры // СформироватьЗаказыНаСервере()

&НаСервереБезКонтекста
// Функция возвращает результат проведения.
//
...
КонецФункции // ЗаказыПровестиНаСервере()

&НаСервереБезКонтекста
// Функция возвращает результат отмены проведения.
//
...
КонецФункции // ЗаказыОтменаПроведенияНаСервере()

&НаСервереБезКонтекста
// Функция возвращает результат пометки удаления.
//
...
КонецФункции // ЗаказыПометитьНаУдалениеНаСервере()
...
Процедура УстановитьУсловноеОформление(НачалоПериода, ОкончаниеПериода)
	...
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ЭлементОтбора.ПравоеЗначение = НСтр("ru='Поступление';uk='Надходження'");
	
	ЭлементОтбора = ГруппаЭлементовОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Запасы.Номенклатура");
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ЭлементОтбора.ПравоеЗначение = НСтр("ru='Потребность';uk='Потреба'");
...
// Процедура формирует период формирования потребности.
//
&НаКлиенте
...
КонецПроцедуры // СформироватьПериодПотребности()

// Процедура обновляет состояние заказов (проведен, записан, помечен на удаление) в ТЧ Заказы
//
&НаСервере
...
КонецПроцедуры // ОбновитьСостояниеЗаказовНаСервере()
...
&НаКлиенте
// Процедура вызывается при нажатии кнопки "Обновить" командной панели формы.
//
Процедура Обновить(Команда)
	
	Если Не ЗначениеЗаполнено(Организация) Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru='Не выбрана организация!';uk='Не обрана організація!'");
...
&НаКлиенте
// Процедура вызывается при нажатии кнопки "СформироватьЗаказы" командной панели формы.
//
...
&НаКлиенте
// Процедура вызывается при нажатии кнопки "Провести" командной панели формы.
//
...
КонецПроцедуры // ЗаказыПровести()

&НаКлиенте
// Процедура вызывается при нажатии кнопки "ОтменаПроведения" командной панели формы.
//
...
КонецПроцедуры // ЗаказыОтменаПроведения()

&НаКлиенте
// Процедура вызывается при нажатии кнопки "ПометитьНаУдаление" командной панели формы.
//
...
КонецПроцедуры // ЗаказыПометитьНаУдаление()

&НаКлиенте
// Процедура вызывается при нажатии кнопки "Перечитать" командной панели формы.
//
Процедура Перечитать(Команда)
	
	Если ЭтаФорма.Модифицированность Тогда
		
		СтрВопроса = НСтр("ru='Данные изменены. Перечитать данные?';uk='Дані змінені. Перечитати дані?'");
...
&НаКлиенте
// Процедура вызывается при нажатии кнопки "ОткрытьНоменклатуру" командной панели Запасы.
//
...
КонецПроцедуры // ЗапасыОткрытьНоменклатуру()
...
&НаКлиенте
// Процедура вызывается при нажатии кнопки "ОткрытьНоменклатуру" командной панели Рекомендации.
//
...
КонецПроцедуры // РекомендацииОткрытьНоменклатуру()
...
// Процедура - обработчик команды ОбновитьЗаказы.
//
&НаКлиенте
...
&НаКлиенте
// Процедура - обработчик события ПриИзменении поля ввода ОтборСпособПополнения.
//
...
КонецПроцедуры // ОтборСпособПополненияПриИзменении()

////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ - ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЧНОГО ПОЛЯ

&НаКлиенте
// Процедура обработчик события Выбор табличного поля Запасы.
//
...
КонецПроцедуры // ЗапасыВыбор()

////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ - ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЧНОЙ ЧАСТИ РЕКОМЕНДАЦИИ

&НаКлиенте
// Процедура - обработчик события ПередНачаломИзменения табличной части Рекомендации.
//
...
КонецПроцедуры // РекомендацииПередНачаломИзменения()

&НаКлиенте
// Процедура - обработчик события ПередНачаломДобавления табличной части Рекомендации.
//
Процедура РекомендацииПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	
	Отказ = Истина;
	
КонецПроцедуры // РекомендацииПередНачаломДобавления()

&НаКлиенте
// Процедура - обработчик события ПередУдалением табличной части Рекомендации.
//
Процедура РекомендацииПередУдалением(Элемент, Отказ)
	
	Отказ = Истина;
	
КонецПроцедуры // РекомендацииПередУдалением()

////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ - ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЧНОЙ ЧАСТИ ЗАКАЗЫ

&НаКлиенте
// Процедура - обработчик события Выбор табличной части Заказы.
//
...
КонецПроцедуры // ЗаказыВыбор()

////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ - ОБРАБОТЧИКИ СОБЫТИЙ РЕКВИЗИТОВ ТАБЛИЧНОЙ ЧАСТИ РЕКОМЕНДАЦИИ

&НаКлиенте
// Процедура - обработчик события ПриИзменении поля ввода Выбран.
//
...
КонецПроцедуры // РекомендацииВыбранПриИзменении()
...
КонецПроцедуры // РекомендацииКоличествоПриИзменении()
