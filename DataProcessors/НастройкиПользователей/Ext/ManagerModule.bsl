...
Функция НастройкиВсехФорм(ИмяПользователя)
	...
	СписокФорм.Добавить("ВнешняяОбработка.StandardEventLog.Форма.EventsJournalFilter", 
		НСтр("ru='Стандартные.Журнал регистрации, Настройка отбора событий';uk='Стандартні.Журнал реєстрації, Налаштування відбору подій'") , Ложь, БиблиотекаКартинок.Форма);
	...
	СписокФорм.Добавить("ВнешняяОбработка.StandardFullTextSearchManagement.Форма.MainForm", 
		НСтр("ru='Стандартные.Управление полнотекстовым поиском';uk='Стандартні.Керування повнотекстовим пошуком'") , Ложь, БиблиотекаКартинок.Форма);
	...
	СписокФорм.Добавить("ВнешняяОбработка.StandardDeleteMarkedObjects.Форма.Form", 
		НСтр("ru='Стандартные.Удаление помеченных объектов';uk='Стандартні.Видалення позначених об`єктів'") , Ложь, БиблиотекаКартинок.Форма);
	СписокФорм.Добавить("ВнешняяОбработка.StandardExternalDataSourceManagement.Форма.Form", 
		НСтр("ru='Стандартные.Управление внешними источниками данных';uk='Стандартні.Керування зовнішніми джерелами даних'") , Ложь, БиблиотекаКартинок.Форма);
	СписокФорм.Добавить("ВнешняяОбработка.StandardTotalsManagement.Форма.MainForm", 
		НСтр("ru='Стандартные.Управление итогами';uk='Стандартні.Керування підсумками'") , Ложь, БиблиотекаКартинок.Форма);
...
Функция ФормыОбъектовМетаданных()
	...
	ИменаСтандартныхФорм = Новый СписокЗначений;
	ЗаполнитьФормыОбъектовМетаданных(Метаданные.ХранилищаНастроек, "ХранилищеНастроек", НСтр("ru='Хранилище настроек';uk='Сховище налаштувань'"),
	...
	ИменаСтандартныхФорм.Добавить("ФормаСписка");
	ИменаСтандартныхФорм.Добавить("ФормаДляВыбора");
	ЗаполнитьФормыОбъектовМетаданных(Метаданные.Задачи, "Задача", НСтр("ru='Задача';uk='Завдання'"),
...
"Список пропущенных вариантов отчетов:';uk='Неможливо скопіювати особисті варіанти звітів."
...
Процедура ЗаполнитьСписокПрочихНастроек(Параметры)
	...
	Если Ключи.Количество() > 0 Тогда
		...
		Если Не ПустаяСтрока(Параметры.Поиск) Тогда
			Если СтрНайти(ВРег(НСтр("ru='Персональные настройки';uk='Персональні налаштування'")), ВРег(Параметры.Поиск)) = 0 Тогда
				ВыводитьНастройку = Ложь;
			КонецЕсли;
		КонецЕсли;
		
		Если ВыводитьНастройку Тогда
			Настройка = НСтр("ru='Персональные настройки';uk='Персональні налаштування'");
	...
	Если Не ПустаяСтрока(Параметры.Поиск) Тогда
		...
		Если СтрНайти(ВРег(НСтр("ru='Настройки печати табличных документов';uk='Налаштування друку табличних документів'")), ВРег(Параметры.Поиск)) = 0 Тогда
			ВыводитьНастройку = Ложь;
		КонецЕсли;
	...
	Если ЕстьНастройкиПечати
		...
		Настройка = НСтр("ru='Настройки печати табличных документов';uk='Налаштування друку табличних документів'");
	...
	Если Не ПустаяСтрока(Параметры.Поиск) Тогда
		...
		Если СтрНайти(ВРег(НСтр("ru='Прочие настройки';uk='Інші налаштування'")), ВРег(Параметры.Поиск)) = 0 Тогда
			ВыводитьНастройку = Ложь;
		КонецЕсли;
	...
	Если ПрочиеКлючи.Количество() <> 0
		И ВыводитьНастройку Тогда
		Настройка = НСтр("ru='Прочие настройки';uk='Інші налаштування'");
		Картинка = БиблиотекаКартинок.ПрочиеПользовательскиеНастройки;
		ТипНастройки = "ПрочаяНастройка";
