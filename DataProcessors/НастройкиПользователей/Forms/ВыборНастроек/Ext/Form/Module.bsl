...
Процедура ОбновитьЗаголовокСтраницы(КоличествоНастроек)
	...
	Если ВыбраннаяСтраницаНастроек = "НастройкиОтчетовСтраница" Тогда
		...
		Если КоличествоНастроекОтчетов = 0 Тогда
			ТекстЗаголовка = НСтр("ru='Настройки отчетов';uk='Налаштування звітів'");
		Иначе
			ТекстЗаголовка = НСтр("ru='Настройки отчетов (%1)';uk='Налаштування звітів (%1)'");
	...
	ИначеЕсли ВыбраннаяСтраницаНастроек = "ПрочиеНастройкиСтраница" Тогда
		...
		Если КоличествоПрочихНастроек = 0 Тогда
			ТекстЗаголовка = НСтр("ru='Прочие настройки';uk='Інші налаштування'");
		Иначе
			ТекстЗаголовка = НСтр("ru='Прочие настройки (%1)';uk='Інші налаштування (%1)'");
...
Процедура ЗагрузитьЗначенияПометок(ДеревоЗначений, ПомеченныеНастройки, ВидНастроек)
	...
	Если КоличествоПомеченных > 0 Тогда
		...
		Если ВидНастроек = "НастройкиОтчетов" Тогда
			...
			Элементы.НастройкиОтчетовСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Настройки отчетов (%1)';uk='Налаштування звітів (%1)'"), КоличествоПомеченных);
		...
		ИначеЕсли ВидНастроек = "ПрочиеНастройки" Тогда
			...
			Элементы.ПрочиеНастройкиСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Прочие настройки (%1)';uk='Інші налаштування (%1)'"), КоличествоПомеченных);
		КонецЕсли;
