...
Процедура ПереключательОчищаемыеНастройкиПриИзменении(Элемент)
	...
	Если ПереключательКомуОчищатьНастройки = "ВыбраннымПользователям"
		...
		ПоказатьПредупреждение(,НСтр("ru='Очистка отдельных настроек доступна только при выборе одного пользователя.';uk='Очищення окремих налаштувань доступні тільки при виборі одного користувача.'"));
...
"яким необхідно очистити налаштування.'"), , "Источник");
		Возврат Ложь;
	КонецЕсли;
	...
	Если ПереключательОчищаемыеНастройки = "ОтдельныеНастройки"
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Выберите настройки, которые необходимо очистить.';uk='Виберіть налаштування, які необхідно очистити.'"), , "ПереключательОчищаемыеНастройки");
		Возврат Ложь;
	КонецЕсли;
	...
	ИначеЕсли ПереключательОчищаемыеНастройки = "ОчиститьВсе" Тогда
		...
		ТекстПояснения = НСтр("ru='Очищены все настройки %1';uk='Очищено всі налаштування %1'");
