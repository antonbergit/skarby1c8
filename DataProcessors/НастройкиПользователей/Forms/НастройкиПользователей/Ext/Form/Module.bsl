...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Пользователь <> Неопределено Тогда
		...
		Заголовок = НСтр("ru='Настройки пользователя';uk='Налаштування користувача'");
...
Процедура НастройкиПередУдалением(Элемент, Отказ)
	...
	Отказ = Истина;
	ТекстВопроса = НСтр("ru='Очистить выбранные настройки?';uk='Очистити вибрані налаштування?'");
...
"Устранить проблему можно в карточке данного пользователя.';uk='В обраного користувача ""%1"" не зазначений"
"користувач інформаційної бази, переглянути його налаштування неможливо."
"Усунути проблему можна в картці даного користувача.'");
...
Процедура СкопироватьВсеНастройки(Команда)
	...
	КопируемыеНастройки.Добавить("НастройкиОтчетов", НСтр("ru='Настройки отчетов';uk='Налаштування звітів'"));
	КопируемыеНастройки.Добавить("НастройкиВнешнегоВида", НСтр("ru='Настройки внешнего вида';uk='Налаштування зовнішнього вигляду'"));
	КопируемыеНастройки.Добавить("ДанныеФорм", НСтр("ru='Данные форм';uk='Дані форм'"));
	КопируемыеНастройки.Добавить("ПерсональныеНастройки", НСтр("ru='Персональные настройки';uk='Персональні налаштування'"));
	КопируемыеНастройки.Добавить("Избранное", НСтр("ru='Избранное';uk='Вибране'"));
	КопируемыеНастройки.Добавить("НастройкиПечати", НСтр("ru='Настройки печати';uk='Налаштування друку'"));
	КопируемыеНастройки.Добавить(
		"ПрочиеПользовательскиеНастройки", НСтр("ru='Настройки дополнительных отчетов и обработок';uk='Налаштування додаткових звітів й обробок'"));
...
Процедура Очистить(Команда)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		...
		ПоказатьПредупреждение(,НСтр("ru='Необходимо выбрать настройки, которые требуется удалить.';uk='Необхідно вибрати налаштування, які потрібно видалити.'"));
		Возврат;
		
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("ОчиститьЗавершение", ЭтотОбъект, ДеревоНастроек);
	ТекстВопроса = НСтр("ru='Очистить выделенные настройки?';uk='Очистити виділені налаштування?'");
	ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
...
"Откроется окно выбора пользователей, которым необходимо очистить настройки.';uk='Очистити виділені налаштування?"
"Відкриється вікно вибору користувачів, яким необхідно очистити налаштування.'");
...
Процедура ОчиститьВсеНастройки(Команда)
	...
	ТекстВопроса = НСтр("ru='Очистить все настройки у пользователя ""%1""?';uk='Очистити всі налаштування у користувача ""%1""?'");
...
Процедура ОчиститьНастройкиОтчетовИВнешнегоВида(Команда)
	...
	ТекстВопроса = НСтр("ru='Очистить все настройки отчетов и внешнего вида у пользователя ""%1""?';uk='Очистити всі налаштування звітів і зовнішнього вигляду у користувача ""%1""?'");
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, Строка(ПользовательСсылка));
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("Очистить", НСтр("ru='Очистить';uk='Очистити'"));
	КнопкиВопроса.Добавить("Отмена", НСтр("ru='Отмена';uk='Відмінити'"));
...
"Продолжить?';uk='Зараз будуть очищені всі налаштування всіх користувачів."
"Продовжити?'");
	КнопкиВопроса = Новый СписокЗначений;
...
Функция ОбновлениеСпискаНастроек()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Обновление настроек пользователей';uk='Оновлення налаштувань користувачів'");
...
Процедура РассчитатьКоличествоНастроек()
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.НастройкиОтчетовСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Настройки отчетов (%1)';uk='Налаштування звітів (%1)'"), КоличествоНастроек);
	Иначе
		Элементы.НастройкиОтчетовСтраница.Заголовок = НСтр("ru='Настройки отчетов';uk='Налаштування звітів'");
	КонецЕсли;
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.ПрочиеНастройкиСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Прочие настройки (%1)';uk='Інші налаштування (%1)'"), КоличествоНастроек);
	Иначе
		Элементы.ПрочиеНастройкиСтраница.Заголовок = НСтр("ru='Прочие настройки';uk='Інші налаштування'");
	КонецЕсли;
	...
	Если ВРег(ИмяСобытия) = ВРег("ВыборПользователя") Тогда
		...
		Если Параметр.КопироватьВсе Тогда
			...
			Если МассивНастроек.Количество() = 7 Тогда
				ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Скопированы все настройки %1';uk='Скопійовано всі налаштування %1'"), ПояснениеКомуСкопированыНастройки);
			Иначе
			...
			Состояние(НСтр("ru='Выполняется копирование настроек';uk='Виконується копіювання налаштувань'"));
			
			Отчет = Неопределено;
			СкопироватьВсеНастройкиНаСервере(ПользовательИнформационнойБазы, ПользователиПриемник, МассивНастроек, Отчет);
			
			Если Отчет <> Неопределено Тогда
				ТекстВопроса = НСтр("ru='Не все варианты отчетов и настройки были скопированы.';uk='Не всі варіанти звітів і налаштування були скопійовані.'");
				КнопкиВопроса = Новый СписокЗначений;
			...
			ПоказатьОповещениеПользователя(
				НСтр("ru='Копирование настроек';uk='Копіювання налаштувань'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
...
Процедура ОчиститьНастройкиВсехПользователейЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ru='Очистка настроек';uk='Очищення налаштувань'"), ,
		НСтр("ru='Очищены все настройки всех пользователя';uk='Очищено всі налаштування всіх користувачів'"), БиблиотекаКартинок.Информация32);
...
Процедура ОчиститьВсеНастройкиЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ТекстПояснения = НСтр("ru='Очищены все настройки пользователя ""%1""';uk='Очищено всі налаштування користувача ""%1""'");
	ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстПояснения, ПользовательСсылка);
	ПоказатьОповещениеПользователя(НСтр("ru='Очистка настроек';uk='Очищення налаштувань'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
...
Процедура ОчиститьНастройкиОтчетовИВнешнегоВидаЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ТекстПояснения = НСтр("ru='Очищены все настройки отчетов и внешнего вида у пользователя ""%1""';uk='Очищено всі налаштування звітів і зовнішнього вигляду у користувача ""%1""'");
...
Процедура ОповеститьОбУдалении(КоличествоНастроек, НазваниеНастройки = Неопределено, КоличествоПользователей = Неопределено)
	...
	ПрописьПредмета = Формат(КоличествоНастроек, "ЧДЦ=0") + " "
		+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоНастроек,
			"Л = ru_RU", НСтр("ru='настройка,настройки,настроек,,,,,,0';uk='налаштування,налаштування,налаштувань,,,,,,0'"));
	...
	Если КоличествоПользователей <> Неопределено Тогда
		...
		Иначе
			ТекстПояснения = НСтр("ru='Очищено %1 %2';uk='Очищене %1 %2'");
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru='Очистка настроек';uk='Очищення налаштувань'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
...
Процедура СкопироватьНастройки()
	...
	Если ДеревоНастроек.ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Необходимо выбрать настройки, которые требуется скопировать.';uk='Необхідно вибрати налаштування, які потрібно скопіювати.'"));
		Возврат;
...
"Копирование настроек личных вариантов отчетов не предусмотрено.';uk='Неможливо скопіювати налаштування особистого варіанта звіту."
"Копіювання налаштувань особистих варіантів звітів не передбачене.'"));
			Возврат;
		КонецЕсли;
