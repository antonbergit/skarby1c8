...
"скопировать, выберите другого пользователя.';uk='В обраного користувача немає налаштувань, які можна було б"
...
"выберите другого пользователя.';uk='Не можна копіювати налаштування користувача самому собі,"
...
Процедура Скопировать(Команда)
	...
	Если ПользовательСсылка = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Выберите пользователя, чьи настройки необходимо скопировать.';uk='Виберіть користувача, налаштування якого необхідно скопіювати.'"), , "ПользовательСсылка");
		Возврат;
	КонецЕсли;
	
	Если КоличествоПользователей = 0 И ПереключательКомуКопироватьНастройки <> "ВсемПользователям" Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Выберите одного или несколько пользователей, которым необходимо скопировать настройки.';uk='Виберіть одного або кілька користувачів, яким необхідно скопіювати налаштування.'"), , "Приемник");
		Возврат;
	КонецЕсли;
	
	Если ПереключательКопируемыеНастройки = "СкопироватьОтдельные" И КоличествоНастроек = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Выберите настройки, которые необходимо скопировать.';uk='Виберіть налаштування, які необхідно скопіювати.'"), , "ПереключательКопируемыеНастройки");
		Возврат;
	КонецЕсли;
...
Процедура СкопироватьНастройки(ИмяКоманды)
	...
	Если ПереключательКопируемыеНастройки = "СкопироватьОтдельные" Тогда
		...
		Если Отчет = Неопределено Тогда
			...
			ПоказатьОповещениеПользователя(НСтр("ru='Копирование настроек';uk='Копіювання налаштувань'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
	...
	Иначе
		...
		Если Не НастройкиСкопированы Тогда
			...
			ТекстПредупреждения = НСтр("ru='Настройки не были скопированы, так как у пользователя ""%1"" не было сохранено ни одной настройки.';uk='Налаштування не були скопійовані, тому що в користувача ""%1"" не було збережено жодного налаштування.'");
		...
		ТекстПояснения = НСтр("ru='Скопированы все настройки %1';uk='Скопійовано всі налаштування %1'");
