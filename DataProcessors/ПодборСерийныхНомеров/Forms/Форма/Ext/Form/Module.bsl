...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Свойство("Запасы") ИЛИ НЕ ЗначениеЗаполнено(Параметры.Запасы.Номенклатура) Тогда
		ТекстСообщения = НСтр("ru='Не заполнена номенклатура!';uk='Не заповнена номенклатура!'");
	ИначеЕсли НЕ Параметры.Запасы.Номенклатура.ИспользоватьСерийныеНомера Тогда
		ТекстСообщения = НСтр("ru='Для номенклатуры не ведется учет по серийным номерам!';uk='Для номенклатури не ведеться облік за серійними номерами!'");
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("КонтрольОстатковСерийныхНомеров") Тогда
		...
		Если Параметры.Свойство("СтруктурнаяЕдиница") Тогда
			...
			Если ЗначениеЗаполнено(Параметры.СтруктурнаяЕдиница) Тогда
				Элементы.СерииОстаткиСерия.Заголовок = НСтр("ru='В наличии на ';uk='В наличии на '")+Параметры.СтруктурнаяЕдиница;
			КонецЕсли;
		Иначе
	...
	Если ЭтоМаркировкаТоваровГИСМ Тогда
		...
		Если ЭтоМаркировкаПерсонифицированнымиКиЗ
			...
			ТекстСообщения = НСтр("ru='Выбранный КиЗ предназначен для маркировки товаров с GTIN %GTINИзКиЗ%, его нельзя использовать для маркировки товара с GTIN %GTIN%.';uk='Выбранный КиЗ предназначен для маркировки товаров с GTIN %GTINИзКиЗ%, его нельзя использовать для маркировки товара с GTIN %GTIN%.'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Зарегистрированный';uk='Зарегистрированный'"));
...
Функция СохранитьВводСерийСервер()
	...
	//ТЗСерийныхНомеров.Свернуть("СерийныйНомер,НовыйНомер,КлючСвязи");
...
Процедура Завершить(Команда)
	...
	Если СохраненоУспешно Тогда
		...
		СтруктураВозврата = Новый Структура("КлючСтроки, АдресВоВременномХранилище", КлючСвязи, АдресВоВременномХранилище);
...
Функция ЗаполнитьПоДаннымШтрихкодов(ДанныеШтрикодов)
	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		ИначеЕсли НЕ (ДанныеШтрихкода.Номенклатура = Номенклатура И ДанныеШтрихкода.Характеристика = Характеристика 
			...
			СтрокаСообщения = НСтр("ru='Считанный штрихкод привязан к другой номенклатуре: %1% %2% %3% %4%';uk='Считанный штрихкод привязан к другой номенклатуре: %1% %2% %3% %4%'");
...
Процедура НастроитьРежимГИСМ()
	...
	Если РежимГИСМ Тогда
		...
		Иначе
			ТекстСообщения = НСтр("ru='К рабочему месту подключено несколько считывателей RFID. Работа одновременно с несколькими считывателями не поддерживается. Оставьте только один считываетель.';uk='К рабочему месту подключено несколько считывателей RFID. Работа одновременно с несколькими считывателями не поддерживается. Оставьте только один считываетель.'");
...
Процедура ПолучитьИнформациюПоКиЗДобавитьСерию(НомерКиЗ, ИдентификаторТекущейСтроки)
	...
	Если Не ИнтеграцияГИСМВызовСервера.ЭтоНомерКиЗ(НомерКиЗ) Тогда
		ТекстСообщения = НСтр("ru='Введенные данные не являются номером КиЗ.';uk='Введенные данные не являются номером КиЗ.'");
		НомерКиЗ = "";
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Функция ПроверитьМеткиНаНеобходимостьЗаписать()
	...
	Если ЭтоМаркировкаТоваровГИСМ Тогда
		...
		Для Каждого Строка Из СтрокиСТребованиемЗаписи Цикл
			...
			Сообщить(НСтр("ru='Необходимо записать данные в КИЗ с номером ';uk='Необходимо записать данные в КИЗ с номером '")+Строка.НомерКИЗГИСМ);
