...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Номенклатура, "ВидМаркировки") <> Перечисления.ВидыМаркировки.МаркируемаяПродукция Тогда
		ТекстИсключения = НСтр("ru='Номенклатура %Номенклатуры% не является продукцией, которую нужно маркировать для ГИСМ.';uk='Номенклатура %Номенклатуры% не является продукцией, которую нужно маркировать для ГИСМ.'");
...
Функция ОбязательныеРеквизитыЗаполнены(ВыводитьСообщения = Истина)
	...
	Если Не ЗначениеЗаполнено(НомерКиЗГИСМ) Тогда
		...
		Если ВыводитьСообщения Тогда
			ТекстСообщения = НСтр("ru='Поле ""Номер КиЗ"" не заполнено';uk='Поле ""Номер КиЗ"" не заполнено'");
	...
	ИначеЕсли Не ИнтеграцияГИСМКлиентСервер.ЭтоНомерКиЗ(НомерКиЗГИСМ) Тогда
		Отказ = Истина;
		Если ВыводитьСообщения Тогда
			ТекстСообщения = НСтр("ru='Значение в поле ""Номер КиЗ"" не является номером контрольного (идентификационного) знака ГИСМ';uk='Значение в поле ""Номер КиЗ"" не является номером контрольного (идентификационного) знака ГИСМ'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,"НомерКиЗГИСМ");
		КонецЕсли;
	КонецЕсли;
...
Процедура НастроитьФормуПоШагу(Шаг)
	...
	Если Шаг = "Шаг1" Тогда
		...
		ЭтаФорма.Заголовок = НСтр("ru='Укажите списываемую серию';uk='Укажите списываемую серию'");
	...
	Иначе
		...
		Если Не ЭтоМаркировкаПерсонифицированнымиКиЗ Тогда
			Элементы.Номер.ПодсказкаВвода = НСтр("ru='<генерируются по данным RFID-метки>';uk='<генерируются по данным RFID-метки>'");
			Элементы.Номер.Подсказка = НСтр("ru='генерируются по данным RFID-метки';uk='генерируются по данным RFID-метки'");
		Иначе
			Элементы.Номер.ПодсказкаВвода = НСтр("ru='<заполняется по данным КиЗ>';uk='<заполняется по данным КиЗ>'");
			Элементы.Номер.Подсказка = НСтр("ru='заполняется по данным КиЗ';uk='заполняется по данным КиЗ'");
		КонецЕсли;
		
		ЭтаФорма.Заголовок = НСтр("ru='Укажите новую серию';uk='Укажите новую серию'");
...
Функция ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	Если ДанныеШтрихкодов.Количество() <> 1 Тогда
		...
		ТекстСообщения = НСтр("ru='Не поддерживается одновременное распознование нескольких штрих-кодов серий. Сканируйте серии по одной.';uk='Не поддерживается одновременное распознование нескольких штрих-кодов серий. Сканируйте серии по одной.'");
