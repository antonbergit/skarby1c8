...
Процедура Отправить(Команда)
	...
	Если Элементы.АдресОтвета.Видимость Тогда 
		Если ПустаяСтрока(АдресДляОтвета) Тогда 
			ВызватьИсключение НСтр("ru='Необходимо ввести адрес электронной почты для ответа';uk='Необхідно ввести адресу електронної пошти для відповіді'");
		КонецЕсли;
		...
		Если Результат.Количество() = 0 Тогда 
			...
			ПоказатьВопрос(Оповещение, НСтр("ru='Адрес электронной почты возможно введен неверно. Отправить сообщение?';uk='Адреса електронної пошти можливо введена невірно. Відправити повідомлення?'"), РежимДиалогаВопрос.ДаНет);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	ОтправитьСообщениеСервер();
	ПоказатьОповещениеПользователя(НСтр("ru='Сообщение в службу поддержки отправлено.';uk='Повідомлення в службу підтримки відправлене.'"));
...
Процедура ПоместитьФайлСРасширениемОповещениеРазмерОповещение(Размер, ДополнительныеПараметры) Экспорт
	...
	Если Не ОбщийРазмерФайловОптимален(Размер) Тогда 
		...
		ТекстПредупреждения = НСтр("ru='Не удалось добавить файл. Размер выбранных файлов превышает предел в %1 Мб';uk='Не вдалося додати файл. Розмір обраних файлів перевищує межу в %1 Мб'");
...
Функция СоздатьЭлементыФормыДляВложенногоФайла()
	...
	Для Каждого ВыбираемыйФайл Из ВыбираемыеФайлы Цикл
		...
		КнопкаУдаленияФайла.Подсказка = НСтр("ru='Удалить файл';uk='Видалити файл'");
