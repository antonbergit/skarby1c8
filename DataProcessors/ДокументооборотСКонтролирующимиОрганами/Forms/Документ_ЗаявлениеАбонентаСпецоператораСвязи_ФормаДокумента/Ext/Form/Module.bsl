...
Процедура ОбновитьСтатус(Команда)
	...
	ИначеЕсли Объект.Статус=ПредопределенноеЗначение("Перечисление.СтатусыЗаявленияАбонентаСпецоператораСвязи.Отправлено") Тогда
		...
		ТекстВопроса = НСтр("ru='Вы уверены, что хотите проверить наличие ответа на заявление?';uk='Вы уверены, что хотите проверить наличие ответа на заявление?'");
...
Процедура ПроверитьЗаполнениеДокумента(ВыполняемоеОповещение)
	...
	Если ПустаяСтрока(Объект.ПолноеНаименование) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите полное наименование организации.';uk='Укажите полное наименование организации.'"), ,"Объект.ПолноеНаименование");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(Объект.КраткоеНаименование) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите краткое наименование организации.';uk='Укажите краткое наименование организации.'"), ,"Объект.КраткоеНаименование");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	СтрЦифры = "0123456789";
	Если ПустаяСтрока(Объект.НомерОсновнойПоставки1с) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указан регистрационный номер.';uk='Не указан регистрационный номер.'"), ,"Объект.НомерОсновнойПоставки1с");
		ИтогПроверки = Ложь;
	КонецЕсли;

	Если ПустаяСтрока(Объект.ИНН)Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите ИНН организации.';uk='Укажите ИНН организации.'"), ,"Объект.ИНН");
		ИтогПроверки = Ложь;
	ИначеЕсли КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(Объект.ИНН,10) Тогда
		...
		Если НЕ (КонтекстЭДОКлиент.ПроверитьКПП(Объект.КПП)) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите КПП организации.';uk='Укажите КПП организации.'"), ,"Объект.КПП");
			ИтогПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ (КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(Объект.ОГРН, 13)) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите ОГРН организации.';uk='Укажите ОГРН организации.'"), ,"ОГРН");
			ИтогПроверки = Ложь;
		КонецЕсли;
	...
	ИначеЕсли КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(Объект.ИНН,12) Тогда
		...
		Если (КонтекстЭДОКлиент.ПроверитьКПП(Объект.КПП)) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Указан ИНН физического лица и КПП юридического лица.';uk='Указан ИНН физического лица и КПП юридического лица.'"), ,"Объект.КПП");
			ИтогПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ (КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(Объект.ОГРН, 15, Истина)) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Некорректно заполнен ОГРН организации.';uk='Некорректно заполнен ОГРН организации.'"), ,"ОГРН");
				ИтогПроверки = Ложь;
		КонецЕсли;
		
		Объект.ТипОрганизации = Ложь;
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Некорректно заполнен ИНН организации.';uk='Некорректно заполнен ИНН организации.'"), ,"Объект.ИНН");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ КонтекстЭДОКлиент.ПроверитьРегистрационныйНомерПФР(Объект.РегНомерПФР, Истина) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Некорректно заполнен регистрационный номер в ПФР.';uk='Некорректно заполнен регистрационный номер в ПФР.'"), ,"Объект.РегНомерПФР");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(Объект.РегНомерФСС, 10, Истина) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Некорректно заполнен регистрационный номер в ФСС.';uk='Некорректно заполнен регистрационный номер в ФСС.'"), ,"Объект.РегНомерФСС");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(Объект.ДополнительныйКодФСС, 10, Истина) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Некорректно заполнен дополнительный регистрационный номер в ФСС.';uk='Некорректно заполнен дополнительный регистрационный номер в ФСС.'"), ,"Объект.ДополнительныйКодФСС");
		ИтогПроверки = Ложь;
	КонецЕсли;
...
"реквизит необязательный для заполнения.';uk='Некорректно заполнен адрес электронной почты,"
"реквизит необязательный для заполнения.'"), ,"Объект.ЭлектроннаяПочта");
	...
	Если ПустаяСтрока(Объект.ТелефонОсновной) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите основной телефон организации.';uk='Укажите основной телефон организации.'"), ,"Объект.ТелефонОсновной");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(Объект.АдресЮридический) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указан юридический адрес.';uk='Не указан юридический адрес.'"), ,"ЮрАдрес");
		ИтогПроверки = Ложь;
	Иначе
		...
		Если ИтогПроверкиАдресЮр тогда
			...
			Если Не ПустаяСтрока(СтрокаОшибкаАдреса) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Некорректно заполнен юридический адрес: %1';uk='Некорректно заполнен юридический адрес: %1'"),СтрокаОшибкаАдреса), ,"ЮрАдрес");
				ИтогПроверки = Ложь;
			КонецЕсли;
		Иначе
			ИтогПроверки = Ложь;
	...
	Если ПустаяСтрока(Объект.АдресФактический) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указан фактический адрес.';uk='Не указан фактический адрес.'"), ,"ФактАдрес");
		ИтогПроверки = Ложь;
	Иначе
		...
		Если ИтогПроверкиАдресФакт тогда
			...
			Если Не ПустаяСтрока(СтрокаОшибкаАдреса) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Некорректно заполнен фактический адрес: %1';uk='Некорректно заполнен фактический адрес: %1'"),СтрокаОшибкаАдреса), ,"ФактАдрес");
				ИтогПроверки = Ложь;
			КонецЕсли;
		Иначе
			ИтогПроверки = Ложь;
	...
	Если ПустаяСтрока(Объект.ВладелецЭЦПФамилия) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите фамилию сотрудника.';uk='Укажите фамилию сотрудника.'"), ,"Объект.ВладелецЭЦПФамилия");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(Объект.ВладелецЭЦПИмя) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите имя сотрудника.';uk='Укажите имя сотрудника.'"), ,"Объект.ВладелецЭЦПИмя");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ КонтекстЭДОКлиент.ПроверитьСНИЛС(Объект.ВладелецЭЦПСНИЛС) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Некорректно указан СНИЛС сотрудника. Не соответствует маске 999-999-999 99';uk='Некорректно указан СНИЛС сотрудника. Не соответствует маске 999-999-999 99'"), ,"Объект.ВладелецЭЦПСНИЛС");
		ИтогПроверки = Ложь;
	ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьСНИЛС(Объект.ВладелецЭЦПСНИЛС, Ложь, Истина) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Некорректно указан СНИЛС сотрудника. Не сошлось контрольное число (СНИЛС не существует)';uk='Некорректно указан СНИЛС сотрудника. Не сошлось контрольное число (СНИЛС не существует)'"), ,"Объект.ВладелецЭЦПСНИЛС");
		ИтогПроверки = Ложь;
	ИначеЕсли Объект.ВладелецЭЦПСНИЛС = "000-000-000 00" Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Предупреждение. Указан ""нулевой"" СНИЛС 000-000-000 00. Обычно это допустимо только для иностранных граждан.';uk='Предупреждение. Указан ""нулевой"" СНИЛС 000-000-000 00. Обычно это допустимо только для иностранных граждан.'"));
	КонецЕсли;
	
	// Пол
	Если НЕ ЗначениеЗаполнено(Объект.ВладелецЭЦППол) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Пол""';uk='Заполните поле ""Пол""'"), ,"Объект.ВладелецЭЦППол");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если Объект.ТипОрганизации И ПустаяСтрока(Объект.ВладелецЭЦПДолжность) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите должность сотрудника.';uk='Укажите должность сотрудника.'"), ,"Объект.ВладелецЭЦПДолжность");
		ИтогПроверки = Ложь;
	ИначеЕсли СтрДлина(Объект.ВладелецЭЦПДолжность) > 64 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Длина значения в поле Должность не должна превышать 64 символа.';uk='Длина значения в поле Должность не должна превышать 64 символа.'"), ,"Объект.ВладелецЭЦПВидДокумента");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если СтрДлина(Объект.ВладелецЭЦППодразделение) > 64 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Длина значения в поле Подразделение не должна превышать 64 символа.';uk='Длина значения в поле Подразделение не должна превышать 64 символа.'"), ,"Объект.ВладелецЭЦППодразделение");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	// Гражданство
	Если НЕ ЗначениеЗаполнено(Объект.ВладелецЭЦПГражданство) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Гражданство""';uk='Заполните поле ""Гражданство""'"), ,"Объект.ВладелецЭЦПГражданство");
		ИтогПроверки = Ложь;
	ИначеЕсли НЕ ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.КодАльфа2Заполнен(Объект.ВладелецЭЦПГражданство)
		И Объект.ВладелецЭЦПГражданство <> ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.СтраныМира.Украина")  Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните код альфа-2 у страны, указанной в качестве гражданства сотрудника-владельца ЭП';uk='Заполните код альфа-2 у страны, указанной в качестве гражданства сотрудника-владельца ЭП'"), ,"Объект.ВладелецЭЦПГражданство");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(Объект.ВладелецЭЦПВидДокумента) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите вид документа, удостоверяющего личность сотрудника.';uk='Укажите вид документа, удостоверяющего личность сотрудника.'"), ,"Объект.ВладелецЭЦПВидДокумента");
		ИтогПроверки = Ложь;
	КонецЕсли;
	...
	Если ПустаяСтрока(Объект.ВладелецЭЦПСерияДокумента) И НЕ КонтекстЭДОКлиент.ПроверитьСериюДокумента(Объект.ВладелецЭЦПВидДокумента,Объект.ВладелецЭЦПСерияДокумента) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите серию документа удостоверяющего личность сотрудника.';uk='Укажите серию документа удостоверяющего личность сотрудника.'"), ,"Объект.ВладелецЭЦПСерияДокумента");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(Объект.ВладелецЭЦПНомерДокумента) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите номер документа удостоверяющего личность сотрудника.';uk='Укажите номер документа удостоверяющего личность сотрудника.'"), ,"Объект.ВладелецЭЦПНомерДокумента");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(Объект.ВладелецЭЦПКемВыданДокумент) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите кем выдан документ удостоверяющий личность сотрудника.';uk='Укажите кем выдан документ удостоверяющий личность сотрудника.'"), ,"Объект.ВладелецЭЦПКемВыданДокумент");
		ИтогПроверки = Ложь;
	...
	Если НЕ ЗначениеЗаполнено(Объект.ВладелецЭЦПДатаВыдачиДокумента) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите дату выдачи документа удостоверяющего личность сотрудника.';uk='Укажите дату выдачи документа удостоверяющего личность сотрудника.'"), ,"Объект.ВладелецЭЦПДатаВыдачиДокумента");
		ИтогПроверки = Ложь;
	ИначеЕсли Объект.ВладелецЭЦПДатаВыдачиДокумента > КонтекстЭДОКлиент.ТекущаяДатаНаСервере() ИЛИ Год(Объект.ВладелецЭЦПДатаВыдачиДокумента) < 1900 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Указана недопустимая дата выдачи документа удостоверяющего личность.';uk='Указана недопустимая дата выдачи документа удостоверяющего личность.'"), ,"Объект.ВладелецЭЦПДатаВыдачиДокумента");
		ИтогПроверки = Ложь;
	КонецЕсли;
	...
	Если Объект.ВладелецЭЦПВидДокумента = ПаспортРФ Тогда
		...
		Если ПустаяСтрока(КодПодразделенияБезТире) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Код подразделения""';uk='Заполните поле ""Код подразделения""'"), ,"Объект.ВладелецЭЦПКодПодразделения");
			ИтогПроверки = Ложь;
		ИначеЕсли СтрДлина(Объект.ВладелецЭЦПКодПодразделения) <> 7
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Некорректно указан код подразделения. Не соответствует маске ХХХ-ХХХ, где X – любая цифра';uk='Некорректно указан код подразделения. Не соответствует маске ХХХ-ХХХ, где X – любая цифра'"), ,"Объект.ВладелецЭЦПКодПодразделения");
			ИтогПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Объект.ВладелецЭЦПДатаРождения) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Дата рождения""';uk='Заполните поле ""Дата рождения""'"), ,"Объект.ВладелецЭЦПДатаРождения");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	// Место рождения
	Если ПустаяСтрока(Объект.ВладелецЭЦПМестоРождения) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Место рождения""';uk='Заполните поле ""Место рождения""'"), ,"Объект.ВладелецЭЦПМестоРождения");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если Объект.Получатели.Количество() < 1 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите хотя бы одно направление сдачи, ФНС или ПФР.';uk='Укажите хотя бы одно направление сдачи, ФНС или ПФР.'"), ,"Объект.Получатели");
		ИтогПроверки = Ложь;
	ИначеЕсли Объект.Получатели.Количество()=1 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Среди направлений сдачи должен быть ФНС или ПФР.';uk='Среди направлений сдачи должен быть ФНС или ПФР.'"), ,"Объект.Получатели");
			ИтогПроверки = Ложь;
	КонецЕсли;
	...
	Если Объект.Получатели.НайтиСтроки(ОтборФСГС).Количество() > 0
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Для подключения направления Росстат должно быть подключено направление ФНС, необходимо добавить направление ФНС.';uk='Для подключения направления Росстат должно быть подключено направление ФНС, необходимо добавить направление ФНС.'"), ,"Объект.Получатели");
		ИтогПроверки = Ложь;
	КонецЕсли;
	...
	Если ПустаяСтрока(Объект.РегНомерПФР)
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Указано направление ПФР, но в сведениях об организации не заполнен регистрационный номер в ПФР.';uk='Указано направление ПФР, но в сведениях об организации не заполнен регистрационный номер в ПФР.'"), ,"Объект.РегНомерПФР");
		ИтогПроверки = Ложь;
	КонецЕсли;
	...
	Если ВыборКриптопровайдера = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбран криптопровайдер.';uk='Не выбран криптопровайдер.'"), ,"ВыборКриптопровайдера");
		ИтогПроверки = Ложь;
	Иначе
	...
	Если Объект.ПодатьЗаявкуНаСертификатДляФСРАР Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.КодРегионаФСРАР) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='На закладке ""Прочие контролирующие органы"" не указан код региона Росалкогольрегулирования';uk='На закладке ""Прочие контролирующие органы"" не указан код региона Росалкогольрегулирования'"), ,"Объект.КодРегионаФСРАР");
			ИтогПроверки = Ложь;
		КонецЕсли;
...
Функция ПроверкаЗаполненияУЦ()
	Если НЕ ЗначениеЗаполнено(Объект.СпецоператорСвязи) ИЛИ СпецоператорИУЦ = Неопределено тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбран Удостоверяющий центр.';uk='Не выбран Удостоверяющий центр.'"), ,"СпецоператорИУЦ");
		Возврат Истина;
	КонецЕсли;
	Возврат Ложь;
КонецФункции
...
Процедура ОтправитьЗаявлениеНаСервер()
	...
	ТекстВопроса = НСтр("ru='Нажимая <Продолжить>, вы подтверждаете, что принимаете ""Соглашение о присоединении к регламенту удостоверяющего центра"". Продолжить ?';uk='Нажимая <Продолжить>, вы подтверждаете, что принимаете ""Соглашение о присоединении к регламенту удостоверяющего центра"". Продолжить ?'");
	...
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось записать заявление по причине: %1';uk='Не удалось записать заявление по причине: %1'"),
				ОписаниеОшибки()));
		Возврат;
	КонецПопытки;
...
Процедура ПреобразоватьАдресКодРегиона(Адрес,ТипАдреса,ИтогПроверки)
	...
	Иначе
		...
		Если ПустаяСтрока(КодРегиона) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1, неверно указан регион.';uk='%1, неверно указан регион.'"),?(ТипАдреса = 0," Юридический адрес"," Фактический адрес")), ,?(ТипАдреса = 0,"ЮрАдрес","ФактАдрес"));
			ИтогПроверки = Ложь;
		Иначе
...
Функция ПроверитьОрганизацию()
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Сначала укажите организацию!';uk='Сначала укажите организацию!'"), ,"Объект.Организация");
		Булево = Истина;
	КонецЕсли;
	Возврат Булево;
КонецФункции
...
Процедура ОбновитьСтатусПослеПроверкиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		ТекстВопроса = НСтр("ru='Вы уверены, что хотите отправить заявление на регистрацию?';uk='Вы уверены, что хотите отправить заявление на регистрацию?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	КонецЕсли;
