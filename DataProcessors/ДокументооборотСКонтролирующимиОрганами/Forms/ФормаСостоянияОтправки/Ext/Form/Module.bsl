...
Процедура ПодробнаяИнформацияНажатие(Элемент)
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("ru='Подробная информация отсутствует';uk='Подробная информация отсутствует'"));
	КонецЕсли;
...
Процедура ВыгрузитьПакетДокументовДляПредставления(Команда)
	...
	Если ЗначениеЗаполнено(Отправка) Тогда
		...
		ТекстВопроса = НСтр("ru='Выгрузить пакет документов для представления по месту требования?';uk='Выгрузить пакет документов для представления по месту требования?'");
...
Процедура ВыгрузитьЦиклыОбмена()
	...
	ТекстВопроса = НСтр("ru='Выгрузить все файлы и подписи?';uk='Выгрузить все файлы и подписи?'");
...
Процедура УдалитьСтрокиТаблицыСостоянияСоСтатусомНеОтправлено(ТаблицаСостояний)
	...
	Для каждого СтрокаТаблицыСостояний Из ТаблицаСостояний Цикл
		Если СтрокаТаблицыСостояний.ТекстНадписи = НСтр("ru='Не отправлено';uk='Не відправлено'") Тогда
...
Функция ПолучитьАдресФайлаПакета(ОтправкаСсылка, ИмяФайлаПакета)
	...
	Иначе
		ИмяФайлаПакета = НСтр("ru='Подпись';uk='Підпис'") + ОтправкаСсылка.ИмяФайлаВыгрузки;
...
Функция ПолучитьАдресФайлаКвитанции(ОтправкаСсылка, ИмяФайлаКвитанции)
	...
	Иначе
		ИмяФайлаКвитанции = НСтр("ru='Квитанция';uk='Квитанція'") + ОтправкаСсылка.ИмяФайлаВыгрузки;
