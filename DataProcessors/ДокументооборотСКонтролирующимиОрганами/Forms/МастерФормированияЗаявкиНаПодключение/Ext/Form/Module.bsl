...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИспользуетсяОднаОрганизация Тогда
		...
		Если РегламентированнаяОтчетностьПереопределяемый.ЭтоЮридическоеЛицо(Организация) Тогда
			Отказ = Истина;
			Сообщить(НСтр("ru='Поддержка регламентированнной отчетности доступна только для индивидуальных предпринимателей!';uk='Поддержка регламентированнной отчетности доступна только для индивидуальных предпринимателей!'"),СтатусСообщения.Внимание);
			Возврат;
		КонецЕсли;
	Иначе
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("ru='Прервать работу помощника?';uk='Прервать работу помощника?'");
...
Процедура КоманднаяПанельМастерДалее(Команда)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.МестоХраненияЭлектроннойПодписи Тогда
		...
		Если ДоступнаЭлектроннаяПодписьВМоделиСервиса Тогда
			Если РежимРаботыСКлючами = 0 Тогда  // Не выбрали ничего
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите место хранения ключа электронной подписи';uk='Укажите место хранения ключа электронной подписи'"), ,"РежимРаботыСКлючами");
				МастерДалее = Ложь;
			ИначеЕсли РежимРаботыСКлючами = 1 Тогда // Облачная ЭП
				...
				Если СтрДлина(СтрЗаменить(ТелефонМобильныйДляАвторизации, " ", "")) <> 14 Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						НСтр("ru='Укажите номер мобильного телефона';uk='Укажите номер мобильного телефона'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),, "ТелефонМобильныйДляАвторизации");
					МастерДалее = Ложь;
				ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ТелефонМобильныйДляАвторизации, НСтр("ru='Номер мобильного телефона';uk='Номер мобильного телефона'"), "ТелефонМобильныйДляАвторизации") Тогда
					МастерДалее = Ложь;
				КонецЕсли;			
			КонецЕсли;	
		КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ИспользуемыйКриптопровайдер Тогда
		...
		Если ВыборКриптопровайдера = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите криптопровайдер';uk='Укажите криптопровайдер'"), ,"ВыборКриптопровайдера");
			МастерДалее = Ложь;
			Возврат;
		Иначе
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СоглашениеОПрисоединенииКРегламентуУЦ Тогда
		...
		Если Спецоператор = Неопределено ИЛИ Спецоператор= КонтекстЭДОКлиент.СтруктураСсылочныхДанных.Перечисления_СпецоператорыСвязи_ПустаяСсылка Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Удостоверяющий центр""';uk='Заполните поле ""Удостоверяющий центр""'"), ,"Спецоператор");
			МастерДалее = Ложь;
		КонецЕсли;
		
		// проверка принятия лицензионного соглашения
		Если НЕ СогласиеСЛицензионнымСоглашением Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Для продолжения работы необходимо принять соглашение';uk='Для продолжения работы необходимо принять соглашение'"), ,"СогласиеСЛицензионнымСоглашением");
			МастерДалее = Ложь;
		КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ГруппаДанныхДляЗаявления Тогда 
		...
		Если ПустаяСтрока(НомерОсновнойПоставки1с) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Регистрационный номер программы""';uk='Заполните поле ""Регистрационный номер программы""'"), ,"НомерОсновнойПоставки1с");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(НомерОсновнойПоставки1с, НСтр("ru='Регистрационный номер программы';uk='Реєстраційний номер програми'"), "НомерОсновнойПоставки1с") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(Организация) Тогда
			...
			КонецЕсли;
		Иначе 
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Организация, по которой будет сдаваться отчетность""';uk='Заполните поле ""Организация, по которой будет сдаваться отчетность""'"), ,"Организация");
			МастерДалее = Ложь;
			ГруппаДанныхДляЗаявления(МастерДалее);
		КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.Реквизиты Тогда 
		...
		Если ПустаяСтрока(КраткоеНаименование) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните краткое наименование';uk='Заполните краткое наименование'"), ,"ДекорацияКраткоеНаименование");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(КраткоеНаименование, НСтр("ru='Краткое наименование';uk='Коротке найменування'"), "ДекорацияКраткоеНаименование") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		
		// полное наименование
		Если ПустаяСтрока(ПолноеНаименование) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните полное наименование';uk='Заполните полное наименование'"), ,"ДекорацияПолноеНаименование");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ПолноеНаименование, НСтр("ru='Полное наименование';uk='Повне найменування'"), "ДекорацияПолноеНаименование") Тогда
			МастерДалее = Ложь;
		КонецЕсли;	
		...
		Если ЭтоЮридическоеЛицо Тогда
			...
			Если ПустаяСтрока(ИНН) Тогда 
			    ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните ИНН';uk='Заполните ИНН'"), ,"ДекорацияИНН");
				МастерДалее = Ложь;
			ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(ИНН,10) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='ИНН должен состоять из 10 цифр';uk='ИНН должен состоять из 10 цифр'"), ,"ДекорацияИНН");
				МастерДалее = Ложь;
			КонецЕсли;
			
			// КПП
			Если ПустаяСтрока(КПП) Тогда 
			    ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните КПП';uk='Заполните КПП'"), ,"ДекорацияКПП");
				МастерДалее = Ложь;
			ИначеЕсли НЕ (КонтекстЭДОКлиент.ПроверитьКПП(КПП)) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='КПП должен состоять из 9 цифр';uk='КПП должен состоять из 9 цифр'"), ,"ДекорацияКПП");
				МастерДалее = Ложь;
			КонецЕсли;
			
			// ОГРН
			Если ПустаяСтрока(ОГРН) Тогда 
			    ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните ОГРН';uk='Заполните ОГРН'"), ,"ОГРН");
				МастерДалее = Ложь;
			ИначеЕсли НЕ (КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(ОГРН,13)) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='ОГРН должен состоять из 13 цифр';uk='ОГРН должен состоять из 13 цифр'"), ,"ОГРН");
				МастерДалее = Ложь;
			КонецЕсли;
		...
		Иначе
			...
			ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(ИНН,12) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='ИНН должен состоять из 12 цифр';uk='ИНН должен состоять из 12 цифр'"), ,"ДекорацияИНН");
				МастерДалее = Ложь;
			КонецЕсли;
			
			// ОГРН
			Если ПустаяСтрока(ОГРН) Тогда 
			    ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните ОГРНИП';uk='Заполните ОГРНИП'"), ,"ОГРН");
				МастерДалее = Ложь;
			ИначеЕсли ЗначениеЗаполнено(ОГРН) И НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(ОГРН,15, Истина) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='ОГРНИП должен состоять из 15 цифр';uk='ОГРНИП должен состоять из 15 цифр'"), ,"ОГРН");
				МастерДалее = Ложь;
			КонецЕсли;
		...
		Если СдаватьВПФР Тогда		
			Если ПустаяСтрока(СтрЗаменить(РегНомерПФР,"-","")) Тогда 
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните регистрационный номер в ПФР';uk='Заполните регистрационный номер в ПФР'"), ,"ДекорацияРегНомерПФР");
				МастерДалее = Ложь;
			ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьРегистрационныйНомерПФР(РегНомерПФР, Истина) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Регистрационный номер в ПФР должен состоять из 12 цифр (ХХХ-ХХХ-ХХХХХХ)';uk='Регистрационный номер в ПФР должен состоять из 12 цифр (ХХХ-ХХХ-ХХХХХХ)'"), ,"ДекорацияРегНомерПФР");
				МастерДалее = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Если СдаватьВФСС Тогда
			...
			Если ПустаяСтрока(РегНомерФСС) Тогда 
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните регистрационный номер в ФСС';uk='Заполните регистрационный номер в ФСС'"), ,"ДекорацияРегНомерФСС");
				МастерДалее = Ложь;
			ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(РегНомерФСС, 10, Истина) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Регистрационный номер в ФСС должен состоять из 10 цифр';uk='Регистрационный номер в ФСС должен состоять из 10 цифр'"), ,"ДекорацияРегНомерФСС");
				МастерДалее = Ложь;
			КонецЕсли;
			...
			Если ПризнакОбособленногоПодразделения И ЭтоЮридическоеЛицо И СдаватьВФСС Тогда
				Если ПустаяСтрока(ДополнительныйКодФСС) Тогда 
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните дополнительный код ФСС';uk='Заполните дополнительный код ФСС'"), ,"ДополнительныйКодФСС");
					МастерДалее = Ложь;
				ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(ДополнительныйКодФСС, 10, Истина) Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Дополнительный код ФСС должен состоять из 10 цифр';uk='Дополнительный код ФСС должен состоять из 10 цифр'"), ,"ДополнительныйКодФСС");
					МастерДалее = Ложь;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если ПустаяСтрока(СтрЗаменить(АдрЮР,",","")) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Юридический адрес""';uk='Заполните поле ""Юридический адрес""'"), ,"АдресЮридический");
			МастерДалее = Ложь;
		Иначе	
			СтрокаОшибкаАдреса = КонтекстЭДОКлиент.ПроверитьАдрес(АдрЮР);
			Если НЕ ПустаяСтрока(СтрокаОшибкаАдреса) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Юридический адрес: %1';uk='Юридический адрес: %1'"),СтрокаОшибкаАдреса), ,"АдресЮридический");
				МастерДалее = Ложь;
			ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(АдрЮР, НСтр("ru='Юридический адрес';uk='Юридична адреса'"), "АдресЮридический") Тогда
				МастерДалее = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Если ПустаяСтрока(СтрЗаменить(АдрФакт,",","")) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Фактический адрес""';uk='Заполните поле ""Фактический адрес""'"), ,"АдресФактический");
			МастерДалее = Ложь;
		Иначе
			СтрокаОшибкаАдреса = КонтекстЭДОКлиент.ПроверитьАдрес(АдрФакт);
			Если НЕ ПустаяСтрока(СтрокаОшибкаАдреса) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Фактический адрес: %1';uk='Фактический адрес: %1'"),СтрокаОшибкаАдреса), ,"АдресФактический");
				МастерДалее = Ложь;
			ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(АдрФакт, НСтр("ru='Фактический адрес';uk='Фактична адреса'"), "АдресФактический") Тогда
				МастерДалее = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Если СтрДлина(СокрЛП(ТелефонОсновной)) = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Телефон организации""';uk='Заполните поле ""Телефон организации""'"), ,"ТелефонОсновной");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ТелефонОсновной, НСтр("ru='Телефон организации';uk='Телефон організації'"), "ТелефонОсновной") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если НЕ КонтекстЭДОКлиент.ЭлектроннаяПочтаВведенаКорректно(
			...
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ЭлектроннаяПочта, НСтр("ru='Электронная почта организации';uk='Электронная почта организации'"), "ЭлектроннаяПочта") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если ПодатьЗаявкуНаСертификатДляФСРАР Тогда
			...
			Если НЕ ЗначениеЗаполнено(КодРегионаФСРАР) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Регион""';uk='Заполните поле ""Регион""'"), ,"КодРегионаФСРАР");
				МастерДалее = Ложь;
			КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ВладельцыЭцп Тогда
		...
		Если ПустаяСтрока(ВладелецЭЦПФамилия) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните фамилию';uk='Заполните фамилию'"), ,"ВладелецЭЦПФамилия");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ВладелецЭЦПФамилия, НСтр("ru='Фамилия';uk='Прізвище'"), "ВладелецЭЦПФамилия") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		
		// имя
		Если ПустаяСтрока(ВладелецЭЦПИмя) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните имя';uk='Заполните имя'"), ,"ВладелецЭЦПИмя");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ВладелецЭЦПИмя, НСтр("ru='Имя';uk='Ім`я'"), "ВладелецЭЦПИмя") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		
		// отчетство
		Если КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ВладелецЭЦПОтчество, НСтр("ru='Отчество';uk='По батькові'"), "ВладелецЭЦПОтчество") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		
		// СНИЛС
		Если ПустаяСтрока(СтрЗаменить(ВладелецЭЦПСНИЛС, "-","")) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните СНИЛС';uk='Заполните СНИЛС'"), ,"ВладелецЭЦПСНИЛС");
			МастерДалее = Ложь;
		Иначе	
			Если НЕ КонтекстЭДОКлиент.ПроверитьСНИЛС(ВладелецЭЦПСНИЛС) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Некорректно указан СНИЛС сотрудника. Не соответствует маске ХХХ-ХХХ-ХХХ ХХ, где X - любая цифра';uk='Некорректно указан СНИЛС сотрудника. Не соответствует маске ХХХ-ХХХ-ХХХ ХХ, где X - любая цифра'"), ,"ВладелецЭЦПСНИЛС");
				МастерДалее = Ложь;
			ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьСНИЛС(ВладелецЭЦПСНИЛС, Ложь, Истина) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Некорректно указан СНИЛС сотрудника. Не сошлось контрольное число (СНИЛС не существует)';uk='Некорректно указан СНИЛС сотрудника. Не сошлось контрольное число (СНИЛС не существует)'"), ,"ВладелецЭЦПСНИЛС");
				МастерДалее = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(ВладелецЭЦПДатаРождения) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Дата рождения""';uk='Заполните поле ""Дата рождения""'"), ,"ВладелецЭЦПДатаРождения");
			МастерДалее = Ложь;
		КонецЕсли;
		
		// Пол
		Если НЕ ЗначениеЗаполнено(ВладелецЭЦППол) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Пол""';uk='Заполните поле ""Пол""'"), ,"ВладелецЭЦППол");
			МастерДалее = Ложь;
		КонецЕсли;
		
		// должность
		Если ЭтоЮридическоеЛицо И ПустаяСтрока(ВладелецЭЦПДолжность) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Должность""';uk='Заполните поле ""Должность""'"), ,"ВладелецЭЦПДолжность");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ВладелецЭЦПДолжность, НСтр("ru='Должность';uk='Посада'"), "ВладелецЭЦПДолжность") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		
		// Подразделение
		Если КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ВладелецЭЦППодразделение, НСтр("ru='Подразделение';uk='Підрозділ'"), "ВладелецЭЦППодразделение") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		
		// Гражданство
		Если НЕ ЗначениеЗаполнено(ВладелецЭЦПГражданство) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Гражданство""';uk='Заполните поле ""Гражданство""'"), ,"ВладелецЭЦПГражданство");
			МастерДалее = Ложь;
		ИначеЕсли НЕ ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.КодАльфа2Заполнен(ВладелецЭЦПГражданство)
			И ВладелецЭЦПГражданство <> ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.СтраныМира.Украина") Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните код альфа-2 у страны, указанной в качестве гражданства сотрудника-владельца ЭП';uk='Заполните код альфа-2 у страны, указанной в качестве гражданства сотрудника-владельца ЭП'"), ,"ВладелецЭЦПГражданство");
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если ПустаяСтрока(ВладелецЭЦПВидДокумента) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Документ, удостоверяющий личность""';uk='Заполните поле ""Документ, удостоверяющий личность""'"), ,"ВладелецЭЦПВидДокумента");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ВладелецЭЦПВидДокумента, НСтр("ru='Документ, удостоверяющий личность';uk='Документ, що засвідчує особу'"), "ВладелецЭЦПВидДокумента") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если ПустаяСтрока(ВладелецЭЦПСерияДокумента) И НЕ КонтекстЭДОКлиент.ПроверитьСериюДокумента(ВладелецЭЦПВидДокумента,ВладелецЭЦПСерияДокумента) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Серия""';uk='Заполните поле ""Серия""'"), ,"ВладелецЭЦПСерияДокумента");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ВладелецЭЦПСерияДокумента, НСтр("ru='Серия';uk='Серія'"), "ВладелецЭЦПСерияДокумента") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		
		// номер
		Если ПустаяСтрока(ВладелецЭЦПНомерДокумента) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Номер""';uk='Заполните поле ""Номер""'"), ,"ВладелецЭЦПНомерДокумента");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ВладелецЭЦПНомерДокумента, НСтр("ru='Номер';uk='Номер'"), "ВладелецЭЦПНомерДокумента") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		
		// кем выдан
		Если ПустаяСтрока(ВладелецЭЦПКемВыданДокумент) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Кем выдан""';uk='Заполните поле ""Кем выдан""'"), ,"ВладелецЭЦПКемВыданДокумент");
			МастерДалее = Ложь;
		ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(ВладелецЭЦПКемВыданДокумент, НСтр("ru='Кем выдан';uk='Ким виданий'"), "ВладелецЭЦПКемВыданДокумент") Тогда
			МастерДалее = Ложь;
		КонецЕсли;
		
		// дата выдачи
		Если НЕ ЗначениеЗаполнено(ВладелецЭЦПДатаВыдачиДокумента) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Дата выдачи""';uk='Заполните поле ""Дата выдачи""'"), ,"ВладелецЭЦПДатаВыдачиДокумента");
			МастерДалее = Ложь;
		ИначеЕсли ВладелецЭЦПДатаВыдачиДокумента > ДатаЗапускаМастера ИЛИ Год(ВладелецЭЦПДатаВыдачиДокумента) < 1900 Тогда	
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Некорректно указана дата выдачи';uk='Некорректно указана дата выдачи'"), ,"ВладелецЭЦПДатаВыдачиДокумента");
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если ВладелецЭЦПВидДокумента = ПаспортРФ Тогда
			...
			Если ПустаяСтрока(КодПодразделенияБезТире) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Код подразделения""';uk='Заполните поле ""Код подразделения""'"), ,"ВладелецЭЦПКодПодразделения");
				МастерДалее = Ложь;
			ИначеЕсли СтрДлина(ВладелецЭЦПКодПодразделения) <> 7
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Некорректно указан код подразделения. Не соответствует маске ХХХ-ХХХ, где X – любая цифра';uk='Некорректно указан код подразделения. Не соответствует маске ХХХ-ХХХ, где X – любая цифра'"), ,"ВладелецЭЦПКодПодразделения");
				МастерДалее = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Если ПустаяСтрока(ВладелецЭЦПМестоРождения) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Место рождения""';uk='Заполните поле ""Место рождения""'"), ,"ВладелецЭЦПМестоРождения");
			МастерДалее = Ложь;
		КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.Направления Тогда
		...
		Если СдаватьВФНС И ПолучателиФНС.Количество()=0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните коды ФНС';uk='Заполните коды ФНС'"), ,"ПолучателиФНС");
			МастерДалее = Ложь;
		ИначеЕсли СдаватьВФНС Тогда
			Для каждого Получатель Из ПолучателиФНС Цикл
				Если КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(Получатель.КПП, НСтр("ru='КПП';uk='КПП'"), "ПолучателиФНС") Тогда
					МастерДалее = Ложь;
					Прервать;
				КонецЕсли;
				Если КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(Получатель.КодПолучателя, НСтр("ru='Код ФНС';uk='Код ФНС'"), "ПолучателиФНС") Тогда
					МастерДалее = Ложь;
					Прервать;
				КонецЕсли;
			КонецЦикла; 
		...
		Если СдаватьВПФР Тогда
			Если ПустаяСтрока(СтрЗаменить(КодПФР,"-","")) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Код отделения ПФР""';uk='Заполните поле ""Код отделения ПФР""'"), ,"КодПФР");
				МастерДалее = Ложь;
			ИначеЕсли СтрДлина(СокрЛП(КодПФР))<> 7 Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Код отделения ПФР должен состоять из 6 цифр';uk='Код отделения ПФР должен состоять из 6 цифр'"), ,"КодПФР");
				МастерДалее = Ложь;
			ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(КодПФР, НСтр("ru='Код отделения ПФР';uk='Код отделения ПФР'"), "КодПФР") Тогда
				МастерДалее = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Если СдаватьВРосстат Тогда
			...
			Если ПолучателиФСГС.Количество() = 0 Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните коды органа Росстата';uk='Заполните коды органа Росстата'"), ,"ПолучателиФСГС");
				МастерДалее = Ложь;
			...
			ИначеЕсли ПолучателиФСГС.Количество() > 0 Тогда
				...
				Для каждого Получатель Из ПолучателиФСГС Цикл
					...
					Если ПустаяСтрока(СтрЗаменить(Получатель.КодПолучателя,"-","")) Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Код органа Росстата""';uk='Заполните поле ""Код органа Росстата""'"), ,"ПолучателиФСГС");
						МастерДалее = Ложь;
					ИначеЕсли СтрДлина(СокрЛП(Получатель.КодПолучателя))<> 5 Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Код органа Росстата должен состоять из 4 цифр';uk='Код органа Росстата должен состоять из 4 цифр'"), ,"ПолучателиФСГС");
						МастерДалее = Ложь;
					ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(Получатель.КодПолучателя, НСтр("ru='Код органа Росстата';uk='Код органа Росстата'"), "ПолучателиФСГС") Тогда
						МастерДалее = Ложь;
					КонецЕсли;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ПодключениеЭДО Тогда 
		...
		Если ПодключитьЭДО Тогда
			...
			Если НЕ ЗначениеЗаполнено(ОператорЭДО) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Оператор ЭДО""';uk='Заполните поле ""Оператор ЭДО""'"), ,"ОператорЭДО");
				МастерДалее = Ложь;
			КонецЕсли;
			
			// код ФНС
			Если ПустаяСтрока(КодНалоговогоОрганаЭДО) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Код ФНС""';uk='Заполните поле ""Код ФНС""'"), ,"КодНалоговогоОрганаЭДО");
				МастерДалее = Ложь;
			ИначеЕсли СтрДлина(СокрЛП(КодНалоговогоОрганаЭДО))<> 4 Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Код ФНС должен состоять из 4 цифр';uk='Код ФНС должен состоять из 4 цифр'"), ,"КодНалоговогоОрганаЭДО");
				МастерДалее = Ложь;
			КонецЕсли;
...
Процедура ПодтвердитьВосстановлениеНаправления(ДополнительныеПараметры)
	...
	ТекстВопроса = НСтр("ru='Восстановить значения кодов по умолчанию?';uk='Восстановить значения кодов по умолчанию?'");
...
Процедура ГруппаДанныхДляЗаявления(МастерДалее)
	...
	Если ВладелецЭЦПТип = ПредопределенноеЗначение("Перечисление.ТипыВладельцевЭЦП.Руководитель") 
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните сведения о руководителе';uk='Заполните сведения о руководителе'"), ,"ДекорацияРуководитель");
		МастерДалее = Ложь;
	КонецЕсли;
	...
	Если ВладелецЭЦПТип = ПредопределенноеЗначение("Перечисление.ТипыВладельцевЭЦП.ГлавныйБухгалтер")
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните сведения о бухгалтере';uk='Заполните сведения о бухгалтере'"), ,"ДекорацияГлБухгалтер");
		МастерДалее = Ложь;
	КонецЕсли;
	...
	Если ВладелецЭЦПТип = ПредопределенноеЗначение("Перечисление.ТипыВладельцевЭЦП.ДругойСотрудник")
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Выберите сотрудника';uk='Выберите сотрудника'"), ,"СотрудникВыбор");
		МастерДалее = Ложь;
	КонецЕсли;	
	...
	Если НЕ (СдаватьВФНС ИЛИ СдаватьВПФР ИЛИ СдаватьВФСС ИЛИ СдаватьВРосстат ИЛИ ПодатьЗаявкуНаСертификатДляФСРАР ИЛИ ПодатьЗаявкуНаПодключениеРПН ИЛИ ПодатьЗаявкуНаПодключениеФТС) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите хотя бы один контролирующий орган, в который будет сдаваться отчетность';uk='Укажите хотя бы один контролирующий орган, в который будет сдаваться отчетность'"), ,"СдаватьВФНС");
		МастерДалее = Ложь;
	ИначеЕсли НЕ СдаватьВФНС И НЕ СдаватьВПФР Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Среди контролирующих органов, в которые будет сдаваться отчетность, должены быть ФНС или ПФР';uk='Среди контролирующих органов, в которые будет сдаваться отчетность, должены быть ФНС или ПФР'"), ,"СдаватьВФНС");
		МастерДалее = Ложь;
	КонецЕсли;
	...
	Если ПолучатьСМС Тогда
		...
		Если ПустаяСтрока(ТелефонМобильныйБезРазделителей) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните мобильный телефон';uk='Заполните мобильный телефон'"), ,"ТелефонМобильный");
			МастерДалее = Ложь;
...
"+7 XXX XXX-XX-XX';uk='Мобильный телефон должен иметь формат "
...
Процедура УстановитьЗаголовокФормы()
	...
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.ДобавитьОрганизациюВЗаголовок(
		...
		КраткоеНаименование,
		// Разный заголовок!
		НСтр("ru='Подключение к 1С-Отчетности';uk='Подключение к 1С-Отчетности'"));
...
Процедура ЗаполнитьСписокВыбораСотрудника()
	...
	НаименованиеРуководителя = ?(
		...
		НСтр("ru='Руководитель';uk='Керівник'"), 
		НСтр("ru='Предприниматель';uk='Підприємець'"));
	...
	Если ПоказыватьТриТипаСотрудников Тогда
		СписокВыбора.Добавить(
			...
			НСтр("ru='Главный бухгалтер';uk='Головний бухгалтер'"));
	КонецЕсли;
		
	СписокВыбора.Добавить(
		Перечисления.ТипыВладельцевЭЦП.ДругойСотрудник, 
		НСтр("ru='Другой сотрудник:';uk='Другой сотрудник:'"));
...
Процедура ОбновитьДанныеСпецОператора()
	...
	ЭлементСсылкаНаСайтУдостоверяющегоЦентра.Заголовок  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сайт %1';uk='Сайт %1'"),Строка(Спецоператор));
...
Процедура ОбновитьДоступностьПодачиВФСРАР()
	...
	Элементы.ПодатьЗаявкуНаСертификатДляФСРАР.Подсказка = ?(ПризнакПоддержкиФСРАР, НСтр("ru='Федеральная служба по регулированию алкогольного рынка';uk='Федеральная служба по регулированию алкогольного рынка'"),
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Федеральная служба по регулированию алкогольного рынка (не поддерживается %1)';uk='Федеральная служба по регулированию алкогольного рынка (не поддерживается %1)'"), Строка(Спецоператор)));
...
Процедура ОбновитьДоступностьПодачиВРПН()
	...
	Элементы.ПодатьЗаявкуНаПодключениеРПН.Подсказка = ?(ПризнакПоддержкиРПН, НСтр("ru='Федеральная служба по надзору в сфере природопользования';uk='Федеральная служба по надзору в сфере природопользования'"),
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Федеральная служба по надзору в сфере природопользования (не поддерживается %1)';uk='Федеральная служба по надзору в сфере природопользования (не поддерживается %1)'"), Строка(Спецоператор)));
...
Процедура ОбновитьДоступностьПодачиВФТС()
	...
	Элементы.ПодатьЗаявкуНаПодключениеФТС.Подсказка = ?(ПризнакПоддержкиФТС, НСтр("ru='Федеральная таможенная служба';uk='Федеральная таможенная служба'"),
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Федеральная таможенная служба (не поддерживается %1)';uk='Федеральная таможенная служба (не поддерживается %1)'"), Строка(Спецоператор)));
...
Процедура ИзменитьОформлениеРеквизитовОрганизации()
	...
	Если ЭтоИП Тогда
		...
		Если ИПИспользуетТрудНаемныхРаботников Тогда
			Элементы.НадписьРегистрационныйНомерВФСС.Заголовок = НСтр("ru='Рег. номер в ФСС (за сотрудников):';uk='Рег. номер в ФСС (за сотрудников):'");
			Элементы.НадписьРегистрационныйНомерВПФР.Заголовок = НСтр("ru='Рег. номер в ПФР (за сотрудников):';uk='Рег. номер в ПФР (за сотрудников):'");
		Иначе
			Элементы.НадписьРегистрационныйНомерВФСС.Заголовок = НСтр("ru='Рег. номер в ФСС (за себя):';uk='Рег. номер в ФСС (за себя):'");
			Элементы.НадписьРегистрационныйНомерВПФР.Заголовок = НСтр("ru='Рег. номер в ПФР (за себя):';uk='Рег. номер в ПФР (за себя):'");
		КонецЕсли;
		
	Иначе
		
		Элементы.НадписьРегистрационныйНомерВФСС.Заголовок = НСтр("ru='Регистрационный номер в ФСС:';uk='Регистрационный номер в ФСС:'");
		Элементы.НадписьРегистрационныйНомерВПФР.Заголовок = НСтр("ru='Регистрационный номер в ПФР:';uk='Регистрационный номер в ПФР:'");
...
Процедура СформироватьИОтправитьЗаявление()
	...
	Если РежимРаботыСКлючами = 1 И Не ТелефонПодтвержден Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите и подтвердите телефон для получения временных паролей';uk='Укажите и подтвердите телефон для получения временных паролей'"),, "ТелефонМобильныйДляАвторизации");
		УправлениеФормой(ЭтаФорма);
		Возврат;
	КонецЕсли;
...
"2. Проверьте, что отключен контроль учетных записей (UAC) в Windows.';uk='Что делать?"
"1. Попробуйте отправить заявление еще раз."
"2. Проверьте, что отключен контроль учетных записей (UAC) в Windows.'");
	Если ВыборКриптопровайдера = 1 Тогда
		 Элементы.ПодсказкаПоРезультатам8.Заголовок = Элементы.ПодсказкаПоРезультатам8.Заголовок + Символы.ПС + 
		 	НСтр("ru='3. Проверьте, что VipNet зарегистрирован.';uk='3. Проверьте, что VipNet зарегистрирован.'");
	КонецЕсли;
...
Процедура ПроверитьТелефон(Команда)
	...
	Если ЗначениеЗаполнено(НомерТелефона) И СтрДлина(СтрЗаменить(НомерТелефона, " ", "")) = 14 Тогда 
		...
		УправлениеФормой(ЭтаФорма);
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите номер мобильного телефона';uk='Укажите номер мобильного телефона'"),, "ТелефонМобильныйДляАвторизации");
	КонецЕсли;
...
Процедура Подтвердить()
	...
	Если Не ЗначениеЗаполнено(ПроверочныйКод)
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Укажите код подтверждения из SMS-сообщения';uk='Укажите код подтверждения из SMS-сообщения'"),, "ПроверочныйКод");
		Возврат;	
	КонецЕсли;
	...
	Если Не ТелефонПодтвержден Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Указан неверный код подтверждения';uk='Указан неверный код подтверждения'"),, "ПроверочныйКод");
	КонецЕсли;
	УправлениеФормой(ЭтаФорма);
...
Процедура ПодтвердитьПочта()
	...
	Если Не ЗначениеЗаполнено(ПроверочныйКодПочта)
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Укажите код подтверждения из письма';uk='Укажите код подтверждения из письма'"),, "ПроверочныйКодПочта");
		Возврат;	
	КонецЕсли;
	...
	Если Не ПочтаПодтверждена Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Указан неверный код подтверждения';uk='Указан неверный код подтверждения'"),, "ПроверочныйКодПочта");
	КонецЕсли;
	УправлениеФормой(ЭтаФорма);
...
Процедура Подключаемый_ПроверитьПолученныйКод()
	...
	Попытка
		Если СтрДлина(СокрЛП(ПроверочныйКод)) = 6 Тогда
			...
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Указан неверный код подтверждения';uk='Указан неверный код подтверждения'"),,"ПроверочныйКод"); 		
			КонецЕсли;
		КонецЕсли;
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось выполнить проверку';uk='Не удалось выполнить проверку'"),,"ПроверочныйКод");
	КонецПопытки;
	...
	Попытка
		Если СтрДлина(СокрЛП(ПроверочныйКодПочта)) = 6 Тогда
			...
			Если ПочтаПодтверждена Тогда
				ТекущийЭлемент = Элементы.Далее;
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Указан неверный код подтверждения';uk='Указан неверный код подтверждения'"),,"ПроверочныйКодПочта"); 		
			КонецЕсли;
		КонецЕсли;
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось выполнить проверку';uk='Не удалось выполнить проверку'"),,"ПроверочныйКодПочта");
	КонецПопытки;
...
Процедура ПроверитьАдресЭлектроннойПочты(Команда)
	...
	Если ЗначениеЗаполнено(АдресПочты) Тогда 
		...
		Если Не КодОтправленПочта Тогда
			ПоказатьПредупреждение(, НСтр("ru='Сервис отправки электронной почты недоступен. Повторите попытку позже.';uk='Сервис отправки электронной почты недоступен. Повторите попытку позже.'"));
			Возврат;
		КонецЕсли;
		...
		УправлениеФормой(ЭтаФорма);
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите адрес электронной почты';uk='Укажите адрес электронной почты'"),, "ЭлектроннаяПочтаДляПаролей");
	КонецЕсли;
