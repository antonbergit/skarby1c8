...
Процедура ПриОткрытии(Отказ)
	...
	Для Каждого ЭлементСписка Из НеНайденныеСертификаты Цикл 
		ШаблонСообщения = НСтр("ru='Сертификат с отпечатком ""%1"" не найден в хранилище сертификатов.';uk='Сертификат с отпечатком ""%1"" не найден в хранилище сертификатов.'");
...
"Вы уверены, что хотите продолжить выбор?';uk='Среди выбранных сертификатов есть такие, срок действия которых истек."
"Вы уверены, что хотите продолжить выбор?'");
	...
	Иначе
		...
		Если ТекДанные.Просрочен Тогда
			...
			ТекстВопроса = НСтр("ru='Вы уверены, что хотите выбрать сертификат, срок действия которого истек?';uk='Вы уверены, что хотите выбрать сертификат, срок действия которого истек?'");
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		Иначе
