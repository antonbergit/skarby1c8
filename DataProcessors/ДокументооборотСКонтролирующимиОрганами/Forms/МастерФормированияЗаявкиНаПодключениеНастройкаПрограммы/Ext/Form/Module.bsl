...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ПодключитьЭДО 
		...
		ТекстПредупреждения = НСтр("ru='Закрыть мастер без подключения к сервису 1С-ЭДО?';uk='Закрыть мастер без подключения к сервису 1С-ЭДО?'");
...
Процедура Шаг1ВторичноеЗаявлениеПослеАвтонастройки()
	...
	// Вторичное заявление
	Если Результат Тогда
		СтатусКомментарий 	= НСтр("ru='Выполнено ';uk='Виконане '") + ИзмененныеРеквизиты;
		Статус 				= ПредопределенноеЗначение("Перечисление.СтатусыЗаявленияАбонентаСпецоператораСвязи.Одобрено");
		НастройкаЗавершена 	= Истина;
	Иначе
		СтатусКомментарий 	= НСтр("ru='Заявление одобрено но не удалось создать учетную запись.';uk='Заявление одобрено но не удалось создать учетную запись.'");
...
Процедура ОбработатьОшибкиАвтонастройки(ДополнительныеПараметры)
	...
	НеУдалосьПодключитьсяКСерверу = СтрНайти(ВРЕГ(ТекстОшибокДляМастераПодключенияК1СОтчетности), НСтр("ru='СЕРВЕР';uk='СЕРВЕР'"));
...
Процедура НастроитьВнешнийВидЭлементовУправления()
	...
	Если ЭтоВторичноеЗаявление Тогда
		...
		ИзмененныеРеквизиты = "";
		ДобавитьРеквизитКСтроке(НСтр("ru='изменение состава контролирующих органов';uk='изменение состава контролирующих органов'"), ИзменитьСоставКонтролирующихОрганов, ИзмененныеРеквизиты);
		ДобавитьРеквизитКСтроке(НСтр("ru='изменение сотрудника-владельца сертификата';uk='изменение сотрудника-владельца сертификата'"), ИзменитьВладельцаСертификата, ИзмененныеРеквизиты);
		ДобавитьРеквизитКСтроке(НСтр("ru='переиздание сертификата';uk='переиздание сертификата'"), ПереиздатьСертификат, ИзмененныеРеквизиты);
		ДобавитьРеквизитКСтроке(НСтр("ru='продление лицензии на 1С-Отчетность';uk='продление лицензии на 1С-Отчетность'"), ПродлитьЛицензиюНа1СОтчетность, ИзмененныеРеквизиты);
		ДобавитьРеквизитКСтроке(НСтр("ru='изменение номера мобильного телефона для SMS-уведомления';uk='изменение номера мобильного телефона для SMS-уведомления'"), ИзменитьМобильныйТелефон, ИзмененныеРеквизиты);
		ДобавитьРеквизитКСтроке(НСтр("ru='изменение реквизитов подключения к 1С-Отчетности';uk='изменение реквизитов подключения к 1С-Отчетности'"), ИзменитьРеквизитыПодключенияК1СОтчетности, ИзмененныеРеквизиты);
		...
		Заголовок =  НСтр("ru='Изменение настроек подключения к 1С-Отчетности';uk='Изменение настроек подключения к 1С-Отчетности'");
		
		// Текст заголовка при одобрении 
		ТекстЗаголовкаПриОдобрении = НСтр("ru='Ваше заявление%1одобрено';uk='Ваше заявление%1одобрено'");
		ТекстЗаголовкаПриОдобрении = СтрЗаменить(ТекстЗаголовкаПриОдобрении, "%1", 
			?(ПустаяСтрока(ИзмененныеРеквизиты), " ", НСтр("ru=' на ';uk=' на '") + ИзмененныеРеквизиты + " "));
		Элементы.ПодсказкаПоРезультатуОдобрено.Заголовок = ТекстЗаголовкаПриОдобрении;
		
		// Текст заголовка при отклонении
		ТекстЗаголовкаПриОтклонении = НСтр("ru='Ваше заявление%1отклонено';uk='Ваше заявление%1отклонено'");
		...
		Если ПереиздатьСертификат Тогда
			ТекстСообщения = НСтр("ru='Далее произойдет установка сертификатов и автоматическая настройка электронного документооборота.';uk='Далее произойдет установка сертификатов и автоматическая настройка электронного документооборота.'");
		Иначе
			ТекстСообщения = НСтр("ru='Далее произойдет автоматическая настройка электронного документооборота.';uk='Далее произойдет автоматическая настройка электронного документооборота.'");
		КонецЕсли;
...
"Новые настройки подключения к 1С-Отчетности успешно применены';uk='Поздравляем!"
"Новые настройки подключения к 1С-Отчетности успешно применены'");
		...
		// Гиперссылка
		Элементы.ОткрытьСсылкуНаСтатьюПоПодключению.Заголовок = НСтр("ru='Инструкция по изменению настроек подключения к 1С-Отчетности';uk='Инструкция по изменению настроек подключения к 1С-Отчетности'");
			
	Иначе
			
		Заголовок =  НСтр("ru='Помощник подключения к 1С-Отчетности';uk='Помощник подключения к 1С-Отчетности'");
