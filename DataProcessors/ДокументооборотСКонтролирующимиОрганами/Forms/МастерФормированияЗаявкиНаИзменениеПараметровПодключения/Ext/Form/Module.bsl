...
"Все равно отказаться от продления сертификата?';uk='Отправка отчетов станет недоступной после окончания срока действия сертификата."
...
Процедура КоманднаяПанельМастерДалее(Команда)
	...
	Если ТекущаяСтраница = Элементы.ОсновнаяПанель.ПодчиненныеЭлементы.ВыборДействия Тогда
		...
		Если НЕ ЗначениеЗаполнено(Организация) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Организация""';uk='Заполните поле ""Организация""'"), ,"Организация");
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если ИзменитьСоставКонтролирующихОрганов Тогда
			...
			Если СдаватьВФНС И ПолучателиФНС.Количество()=0 Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните коды ФНС';uk='Заполните коды ФНС'"), ,"СдаватьВФНС");
				МастерДалее = Ложь;
			КонецЕсли;
			...
			Если СдаватьВПФР Тогда
				...
				Если ПустаяСтрока(СтрЗаменить(КодПФР,"-","")) Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Код отделения ПФР""';uk='Заполните поле ""Код отделения ПФР""'"), ,"СдаватьВПФР");
					МастерДалее = Ложь;
				ИначеЕсли СтрДлина(СокрЛП(КодПФР))<> 7 Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Код отделения ПФР должен состоять из 6 цифр';uk='Код отделения ПФР должен состоять из 6 цифр'"), ,"СдаватьВПФР");
					МастерДалее = Ложь;
				ИначеЕсли СокрЛП(КодПФР) <> КодПФРВОрганизации Тогда
					ТекстСообщения = НСтр("ru='Код отделения ПФР не совпадает с кодом, указанным в организации (%1)';uk='Код отделения ПФР не совпадает с кодом, указанным в организации (%1)'");
			...
			Если СдаватьВРосстат Тогда
				...
				Если ПолучателиФСГС.Количество() = 0 Тогда
					...
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните коды органа Росстата';uk='Заполните коды органа Росстата'"), ,"СдаватьВРосстат");
					МастерДалее = Ложь;
				...
				ИначеЕсли ПолучателиФСГС.Количество() > 0 Тогда
					...
					Для каждого Получатель Из ПолучателиФСГС Цикл
						...
						Если ПустаяСтрока(СтрЗаменить(Получатель.КодПолучателя,"-","")) Тогда
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Код органа Росстата""';uk='Заполните поле ""Код органа Росстата""'"), ,"СдаватьВРосстат");
							МастерДалее = Ложь;
						ИначеЕсли СтрДлина(СокрЛП(Получатель.КодПолучателя))<> 5 Тогда
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Код органа Росстата должен состоять из 4 цифр';uk='Код органа Росстата должен состоять из 4 цифр'"), ,"СдаватьВРосстат");
							МастерДалее = Ложь;
						ИначеЕсли КонтекстЭДОКлиент.НайденыЗапрещенныеСимволы(Получатель.КодПолучателя, НСтр("ru='Код органа Росстата';uk='Код органа Росстата'"), "СдаватьВРосстат") Тогда
							МастерДалее = Ложь;
						КонецЕсли;
		...
		Если ИзменитьСоставКонтролирующихОрганов Тогда
			Если НЕ (СдаватьВФНС ИЛИ СдаватьВПФР ИЛИ СдаватьВФСС ИЛИ СдаватьВРосстат ИЛИ СдаватьВФСРАР ИЛИ СдаватьВРПН ИЛИ СдаватьВФТС) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите хотя бы один контролирующий орган, в который будет сдаваться отчетность';uk='Укажите хотя бы один контролирующий орган, в который будет сдаваться отчетность'")
					, ,"ИзменитьСоставКонтролирующихОрганов");
				МастерДалее = Ложь;
			ИначеЕсли НЕ СдаватьВФНС И НЕ СдаватьВПФР Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Среди контролирующих органов, в которые будет сдаваться отчетность, должены быть ФНС или ПФР';uk='Среди контролирующих органов, в которые будет сдаваться отчетность, должены быть ФНС или ПФР'")
					, ,"ИзменитьСоставКонтролирующихОрганов");
				МастерДалее = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Если (ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат) 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Выберите руководителя';uk='Выберите руководителя'"), ,"ДекорацияРуководитель");
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если (ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат) 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Выберите бухгалтера';uk='Выберите бухгалтера'"), ,"ДекорацияГлБухгалтер");
			МастерДалее = Ложь;
		КонецЕсли;
...
"Установите флажок ""Изменение сотрудника-владельца сертификата"" и выберите владельца сертификата';uk='Переиздание сертификата требует обязательного указания сотрудника-владельца сертификата."
"Установите флажок ""Изменение сотрудника-владельца сертификата"" и выберите владельца сертификата'");
		...
		Если (ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат) 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Выберите сотрудника-владельца сертификата';uk='Выберите сотрудника-владельца сертификата'"), ,"СотрудникВыбор");
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если НЕ ПереиздатьСертификат 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Вы не выбрали никакого действия';uk='Вы не выбрали никакого действия'"), ,"");
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если ИзменитьМобильныйТелефон Тогда
			Если ЭтоЭлектроннаяПодписьВМоделиСервиса Тогда
				...
				Если Не ТелефонИзменен И Не ЭлектроннаяПочтаИзменена И НЕ ПолучатьУведомленияИзменен Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите номер мобильного телефона';uk='Укажите номер мобильного телефона'"),, "ТелефонДляПаролей");
					МастерДалее = Ложь;
				ИначеЕсли ТелефонИзменен И Не ТелефонПодтвержден Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Подтвердите номер мобильного телефона';uk='Подтвердите номер мобильного телефона'"),, "ТелефонДляПаролей");
					МастерДалее = Ложь;
				ИначеЕсли ЭлектроннаяПочтаИзменена И Не ЭлектроннаяПочтаПодтверждена И ЗначениеЗаполнено(ЭлектроннаяПочтаДляПаролей) Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Подтвердите адрес электронной почты';uk='Подтвердите адрес электронной почты'"),, "ЭлектроннаяПочтаДляПаролей");
					МастерДалее = Ложь;
				КонецЕсли;
			Иначе
				Если ПолучатьУведомления Тогда
					...
					Если ПустаяСтрока(ТелефонМобильныйБезРазделителей) Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите номер мобильного телефона';uk='Укажите номер мобильного телефона'"),, "ТелефонМобильный");
						МастерДалее = Ложь;
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ОсновнаяПанель.ПодчиненныеЭлементы.ПодключениеЭДО Тогда
		...
		Если ПодключитьЭДО Тогда
			...
			Если ПустаяСтрока(СтрЗаменить(АдрЮР,",","")) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните юридический адрес';uk='Заполните юридический адрес'"), ,"АдресЮридический");
				МастерДалее = Ложь;
			Иначе	
...
Процедура ПроверитьНомер(Команда)
	...
	Иначе
		Сообщить(СтрШаблон(НСтр("ru='Не удалось выполнить отправку SMS на номер %1. Возможно номер указан неверно.';uk='Не удалось выполнить отправку SMS на номер %1. Возможно номер указан неверно.'"), ТелефонДляПаролей));
	КонецЕсли;
...
Процедура ПроверитьАдрес(Команда)
	...
	Иначе
		Сообщить(СтрШаблон(НСтр("ru='Не удалось выполнить отправку письма на адрес %1. Возможно адрес указан неверно.';uk='Не удалось выполнить отправку письма на адрес %1. Возможно адрес указан неверно.'"), ЭлектроннаяПочтаДляПаролей));
	КонецЕсли;
...
Процедура ПроставитьПризнакОшибкиИДоступностиДляРедактированияКонтролируемыхРеквизитов()
	...
	Если ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат  Тогда
		СодержитОшибку = НЕ ЗначениеЗаполнено(ВладелецЭЦПГражданство)
			...
			И ВладелецЭЦПГражданство <> ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.СтраныМира.Украина"));
		РеквизитРедактируется = Истина;
	КонецЕсли;
	ПроставитьПризнакОшибкиИДоступностиДляРедактирования(ИзмененныйРеквизит, СодержитОшибку, РеквизитРедактируется);
...
Процедура ПроверитьЗаполнениеЗаявления(МастерДалее)
	...
	Если НомерОсновнойПоставки1сИзменился И ИзменитьРеквизитыПодключенияК1СОтчетности Тогда
		Если ПустаяСтрока(НомерОсновнойПоставки1с)  Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните регистрационный номер программы';uk='Заполните регистрационный номер программы'"), ,"НомерОсновнойПоставки1с");
			МастерДалее = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если ПолноеНаименованиеИзменилось И ИзменитьРеквизитыПодключенияК1СОтчетности Тогда
		Если ПустаяСтрока(ПолноеНаименование) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните полное наименование организации';uk='Заполните полное наименование организации'"), ,"ДекорацияПолноеНаименование");
			МастерДалее = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если СдаватьВФСС И ИзменитьСоставКонтролирующихОрганов Тогда
		...
		Если ПризнакОбособленногоПодразделения и ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.ЭтоЮрЛицо(Организация) и СдаватьВФСС И ЕстьРеквизитДопКодФССУОрганизации() Тогда
			Если ПустаяСтрока(ДополнительныйКодФСС) Тогда 
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните дополнительный код ФСС у организации';uk='Заполните дополнительный код ФСС у организации'"), ,"ДополнительныйКодФСС");
				МастерДалее = Ложь;
			ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(ДополнительныйКодФСС, 10, Истина) Тогда
	...
	Если ТелефонОсновнойИзменился И ИзменитьРеквизитыПодключенияК1СОтчетности Тогда
		Если СтрДлина(СокрЛП(ТелефонОсновной)) = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните телефон организации';uk='Заполните телефон организации'"), ,"ТелефонОсновной");
			МастерДалее = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если СдаватьВФСРАР И ИзменитьСоставКонтролирующихОрганов Тогда
		...
		Если НЕ ЗначениеЗаполнено(КодРегионаФСРАР) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните регион в адресе организации';uk='Заполните регион в адресе организации'"), ,"КодРегионаФСРАР");
			МастерДалее = Ложь;
		КонецЕсли;
...
Процедура ПроверитьДанныеВладельцаЭП(МастерДалее)
	...
	Если ПустаяСтрока(ВладелецЭЦПФамилия) И (ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните фамилию сотрудника-владельца ЭП';uk='Заполните фамилию сотрудника-владельца ЭП'"), ,"ВладелецЭЦПФамилия");
		МастерДалее = Ложь;
	КонецЕсли;
	
	// имя
	Если ПустаяСтрока(ВладелецЭЦПИмя) И (ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните имя сотрудника-владельца ЭП';uk='Заполните имя сотрудника-владельца ЭП'"), ,"ВладелецЭЦПИмя");
		МастерДалее = Ложь;
	КонецЕсли;	
	...
	Если ВладелецЭЦПСНИЛСИзменился И ИзменитьРеквизитыПодключенияК1СОтчетности 
		...
		Если ПустаяСтрока(СтрЗаменить(ВладелецЭЦПСНИЛС, "-","")) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните СНИЛС сотрудника-владельца ЭП';uk='Заполните СНИЛС сотрудника-владельца ЭП'"), ,"ВладелецЭЦПСНИЛС");
			МастерДалее = Ложь;
		Иначе	
			Если НЕ КонтекстЭДОКлиент.ПроверитьСНИЛС(ВладелецЭЦПСНИЛС) Тогда
	...
	Если (ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат) И НЕ ЗначениеЗаполнено(ВладелецЭЦПДатаРождения) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните дату рождения сотрудника-владельца ЭП';uk='Заполните дату рождения сотрудника-владельца ЭП'"), ,"ВладелецЭЦПДатаРождения");
		МастерДалее = Ложь;
	КонецЕсли;
	
	// Пол
	Если (ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат) И НЕ ЗначениеЗаполнено(ВладелецЭЦППол) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните пол сотрудника-владельца ЭП';uk='Заполните пол сотрудника-владельца ЭП'"), ,"ВладелецЭЦППол");
		МастерДалее = Ложь;
	КонецЕсли;	
	...
	Если ВладелецЭЦПДолжностьИзменилась И ИзменитьРеквизитыПодключенияК1СОтчетности 
		ИЛИ ПереиздатьСертификат ИЛИ ИзменитьВладельцаСертификата Тогда
		Если ЭтоЮридическоеЛицо И ПустаяСтрока(ВладелецЭЦПДолжность) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните должность сотрудника-владельца ЭП';uk='Заполните должность сотрудника-владельца ЭП'"), ,"ВладелецЭЦПДолжность");
			МастерДалее = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если (ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат) И НЕ ЗначениеЗаполнено(ВладелецЭЦПГражданство) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните гражданство сотрудника-владельца ЭП';uk='Заполните гражданство сотрудника-владельца ЭП'"), ,"ВладелецЭЦПГражданство");
		МастерДалее = Ложь;
	...
	Если ИзменитьВладельцаСертификата ИЛИ ПереиздатьСертификат Тогда
		...
		Если ПустаяСтрока(ВладелецЭЦПВидДокумента) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните документ, удостоверяющий личность сотрудника-владельца ЭП';uk='Заполните документ, удостоверяющий личность сотрудника-владельца ЭП'"), ,"ВладелецЭЦПВидДокумента");
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если ПустаяСтрока(ВладелецЭЦПСерияДокумента) И НЕ КонтекстЭДОКлиент.ПроверитьСериюДокумента(ВладелецЭЦПВидДокумента, ВладелецЭЦПСерияДокумента) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните серию документа, удостоверяющего личность сотрудника-владельца ЭП';uk='Заполните серию документа, удостоверяющего личность сотрудника-владельца ЭП'"), ,"ВладелецЭЦПСерияДокумента");
			МастерДалее = Ложь;
		КонецЕсли;
		
		// номер
		Если ПустаяСтрока(ВладелецЭЦПНомерДокумента) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните номер документа, удостоверяющего личность сотрудника-владельца ЭП';uk='Заполните номер документа, удостоверяющего личность сотрудника-владельца ЭП'"), ,"ВладелецЭЦПНомерДокумента");
			МастерДалее = Ложь;
		КонецЕсли;
		
		// дата выдачи
		Если НЕ ЗначениеЗаполнено(ВладелецЭЦПДатаВыдачиДокумента) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните дату выдачи документа, удостоверяющего личность сотрудника-владельца ЭП';uk='Заполните дату выдачи документа, удостоверяющего личность сотрудника-владельца ЭП'"), ,"ВладелецЭЦПДатаВыдачиДокумента");
			МастерДалее = Ложь;
		ИначеЕсли ВладелецЭЦПДатаВыдачиДокумента > ТекущаяДатаСервер ИЛИ Год(ВладелецЭЦПДатаВыдачиДокумента) < 1900 Тогда	
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Некорректно указана дата выдачи документа, удостоверяющего личность сотрудника-владельца ЭП';uk='Некорректно указана дата выдачи документа, удостоверяющего личность сотрудника-владельца ЭП'"), ,"ВладелецЭЦПДатаВыдачиДокумента");
			МастерДалее = Ложь;
		КонецЕсли;
		
		// кем выдан
		Если ПустаяСтрока(ВладелецЭЦПКемВыданДокумент) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните поле ""Кем выдан"" документа, удостоверяющего личность сотрудника-владельца ЭП';uk='Заполните поле ""Кем выдан"" документа, удостоверяющего личность сотрудника-владельца ЭП'"), ,"ВладелецЭЦПКемВыданДокумент");
			МастерДалее = Ложь;
		КонецЕсли;
		...
		Если ВладелецЭЦПВидДокумента = ПаспортРФ Тогда
			КодПодразделенияБезТире = СтрЗаменить(ВладелецЭЦПКодПодразделения, "-","");
			Если ПустаяСтрока(КодПодразделенияБезТире) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните код подразделения, выдавшего документ, удостоверяющего личность сотрудника-владельца ЭП';uk='Заполните код подразделения, выдавшего документ, удостоверяющего личность сотрудника-владельца ЭП'"), ,"ВладелецЭЦПКодПодразделения");
				МастерДалее = Ложь;
			ИначеЕсли СтрДлина(ВладелецЭЦПКодПодразделения) <> 7
				ИЛИ НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(КодПодразделенияБезТире, 6, Истина) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Некорректно указан код подразделения, выдавшего документ, удостоверяющего личность сотрудника-владельца ЭП. Не соответствует маске ХХХ-ХХХ, где X – любая цифра';uk='Некорректно указан код подразделения, выдавшего документ, удостоверяющего личность сотрудника-владельца ЭП. Не соответствует маске ХХХ-ХХХ, где X – любая цифра'"), ,"ВладелецЭЦПКодПодразделения");
				МастерДалее = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Если ПустаяСтрока(ВладелецЭЦПМестоРождения) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заполните место рождения сотрудника-владельца ЭП';uk='Заполните место рождения сотрудника-владельца ЭП'"), ,"ВладелецЭЦПМестоРождения");
			МастерДалее = Ложь;
		КонецЕсли;
	...
	Если ТаблицаДанныхЗаявленияНаПодключение.НайтиСтроки(ПараметрыОтбора).Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не изменено ни одной настройки подключения к 1С-Отчетности';uk='Не изменено ни одной настройки подключения к 1С-Отчетности'"), ,"");
		МастерДалее = Ложь;
	КонецЕсли;
...
Процедура СформироватьТекстПредупреждения()
	...
	Если НЕ ПустаяСтрока(ИзмененныеРеквизиты) Тогда
		ИзмененныеРеквизиты = ИзмененныеРеквизиты + НСтр("ru=' организации';uk=' организации'");
	КонецЕсли;
	...
	Если ВладелецЭЦППодразделениеИзменилось ИЛИ ВладелецЭЦПДолжностьИзменилась ИЛИ ВладелецЭЦПСНИЛСИзменился Тогда
		ИзмененныеРеквизиты = ИзмененныеРеквизиты + НСтр("ru=' сотрудника-владельца ЭП';uk=' сотрудника-владельца ЭП'");
	КонецЕсли;
...
Функция ПолучитьКодыФНСПрописью(Знач Получатели)
	...
	ИначеЕсли КоличествоКодовФНС > 1 Тогда
		Результат = НСтр("ru='Коды ФНС (%1)';uk='Коды ФНС (%1)'");
...
Функция ПолучитьКодыРосстатПрописью(Знач Получатели)
	...
	ИначеЕсли КоличествоКодовФСГС > 1 Тогда
		Результат = НСтр("ru='Коды Росстата (%1)';uk='Коды Росстата (%1)'");
...
Процедура ОформитьПодсказкуДляГалкиПродлитьСертификат()
	...
	ИначеЕсли НЕ СертификатДоступен Тогда
		ТекстЗаголовка = НСтр("ru='Сертификат недоступен';uk='Сертификат недоступен'");
	ИначеЕсли ПродлитьСертификатИсходный Тогда
		...
		Если КоличествоОставшегосяВремени = Неопределено Тогда
			ТекстЗаголовка = НСтр("ru='Срок действия сертификата истек %1';uk='Срок действия сертификата истек %1'");
			ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%1", Формат(СертификатДействителенПо,"ДЛФ=DD"));
		Иначе
			ТекстЗаголовка = НСтр("ru='Срок действия сертификата истекает через %1';uk='Срок действия сертификата истекает через %1'");
			ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%1", КоличествоОставшегосяВремени);
		КонецЕсли;
	Иначе
		ТекстЗаголовка = НСтр("ru='Сертификат действует до %1';uk='Сертификат действует до %1'");
		ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%1", Формат(СертификатДействителенПо,"ДЛФ=DD"));
	КонецЕсли;
...
Процедура ОформитьПодсказкуДляГалкиПродлитьЛицензиюНа1СОтчетность()
	...
	ИначеЕсли ПродлитьЛицензиюНа1СОтчетностьИсходный Тогда
		...
		Если КоличествоОставшегосяВремени = Неопределено Тогда
			ТекстЗаголовка = НСтр("ru='Срок действия лицензии истек %1';uk='Срок действия лицензии истек %1'");
			ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%1", Формат(ЛицензияДатаОкончания,"ДЛФ=DD"));
		Иначе
			ТекстЗаголовка = НСтр("ru='Срок действия лицензии истекает через %1';uk='Срок действия лицензии истекает через %1'");
			ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%1", КоличествоОставшегосяВремени);
		КонецЕсли;
	Иначе
		ТекстЗаголовка = НСтр("ru='Лицензия действует до %1';uk='Лицензия действует до %1'");
		ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%1", Формат(ЛицензияДатаОкончания,"ДЛФ=DD"));
	КонецЕсли;
	Надпись.Заголовок = ТекстЗаголовка;
...
Процедура ОформитьЭлементыДляГалкиИзменитьСоставНалоговыхОргановШаг1(ДанныеОрганизации)
	...
	Иначе
		...
		Если КодПФРВОрганизации <> КодПФР И НЕ ПустаяСтрока(СтрЗаменить(КодПФРВОрганизации,"-","")) Тогда
			Элементы.ВзятьКодПФРИзОрганизации.Видимость = Истина;
			ТекстЗаголовка = НСтр("ru='Изменить на %1';uk='Изменить на %1'");
...
Процедура СделатьНедоступнымиНеподдерживаемыеНаправления()
	...
	Элементы.СдаватьВФСРАР.Подсказка = ?(ОператорПоддерживаетФСРАР, НСтр("ru='Федеральная служба по регулированию алкогольного рынка';uk='Федеральная служба по регулированию алкогольного рынка'"),
	...
	Элементы.СдаватьВРПН.Подсказка = ?(ОператорПоддерживаетРПН, НСтр("ru='Федеральная служба по надзору в сфере природопользования';uk='Федеральная служба по надзору в сфере природопользования'"),
	...
	Элементы.СдаватьВФТС.Подсказка = ?(ОператорПоддерживаетФТС, НСтр("ru='Федеральная таможенная служба';uk='Федеральная таможенная служба'"),
...
Процедура ОформитьПодсказкуДляГалкиИзменитьРеквизитыПодключенияК1СОтчетности()
	...
	Иначе
		ТекстЗаголовка = НСтр("ru='реквизиты не менялись';uk='реквизиты не менялись'");
	КонецЕсли;
	Надпись.Заголовок = ТекстЗаголовка;
...
Процедура УстановитьЗаголовокФормы()
	...
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.ДобавитьОрганизациюВЗаголовок(
		...
		КраткоеНаименование,
		// Разный заголовок!
		НСтр("ru='Изменение настроек подключения к 1С-Отчетности';uk='Изменение настроек подключения к 1С-Отчетности'"));
...
Процедура СформироватьИОтправитьЗаявлениеПослеВводаКодаПодтвреждения(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		ПоказатьСледующуюСтраницу();
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Изменение номера телефона не было подтверждено';uk='Изменение номера телефона не было подтверждено'"));
	КонецЕсли;
...
"отчетов и входящих сообщениях, получения временных паролей:';uk='Изменение настроек уведомлений о статусе отправки"
"отчетов и входящих сообщениях, получения временных паролей:'");
	...
	Иначе
		Элементы.ИзменитьМобильныйТелефон.Заголовок = НСтр("ru='Изменение настроек уведомлений о статусе отправки"
"отчетов и входящих сообщениях:';uk='Изменение настроек уведомлений о статусе отправки"
...
"Для изменения настроек подключения к 1С-Отчетности свяжитесь со службой поддержки оператора электронного документооборота';uk='Оператор электронного документооборота не поддерживает отправку вторичных заявлений."
...
"Вы уверены, что хотите отключить направление сдачи отчетности %1 ?';uk='Отправка отчетности в %1 станет недоступной после отключения данного направления."
...
Процедура ОпределитьПричинуПереизданияСертификата()
	...
	Если ПереиздатьСертификат = 2 Тогда
		...
		НоваяСтрока.Префикс		= НСтр("ru='изменилось';uk='изменилось'");
		...
		НоваяСтрока.Группа		= "Организация";
		НоваяСтрока.Префикс		= НСтр("ru='изменился';uk='изменился'");
		...
		НоваяСтрока.Группа		= "Организация";
		НоваяСтрока.Префикс		= НСтр("ru='изменилась';uk='изменилась'");
		...
		Если СертификатДоступен Тогда
			ИзменившийсяРеквизит = НСтр("ru='инициировано продление сертификата';uk='инициировано продление сертификата'")
		Иначе
			ИзменившийсяРеквизит = НСтр("ru='сертификат недоступен';uk='сертификат недоступен'");
		КонецЕсли;
		...
		СформироватьТекстПричиныПереизданияСертификата(Отбор, НСтр("ru='организации';uk='організації'"));
		...
		Если ИзменилсяТолькоВладелец Тогда
			Постфикс = "";
		Иначе
			Постфикс = НСтр("ru='сотрудника-владельца ЭП';uk='сотрудника-владельца ЭП'");
		КонецЕсли;
		...
		ПричиныПереизданияСертификата = НСтр("ru='Причины переиздания сертификата: ';uk='Причины переиздания сертификата: '") + ПричиныПереизданияСертификата;
		
	ИначеЕсли ПереиздатьСертификат = 1 Тогда
		
		Если СертификатДоступен Тогда
			ПричиныПереизданияСертификата = НСтр("ru='Причины переиздания сертификата: принудительное переиздание сертификата';uk='Причины переиздания сертификата: принудительное переиздание сертификата'");
		Иначе
			ПричиныПереизданияСертификата = НСтр("ru='Причины переиздания сертификата: сертификат недоступен';uk='Причины переиздания сертификата: сертификат недоступен'");
		КонецЕсли;
...
Процедура Подключаемый_ПроверитьКодПодтверждения()
	...
	Если СтрДлина(КодПодтверждения) = 6 Тогда
		...
		Если Результат Тогда
			...
			УправлениеФормой(ЭтаФорма);
		Иначе
			ОписаниеОшибки = НСтр("ru='Неверный код подтверждения';uk='Неверный код подтверждения'");
