...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ПрограммаПроверки) Тогда
		Элементы.ДекорацияПрограммаПроверки.Заголовок = "   " + НСтр("ru='Проверено';uk='Перевірено'") + ": " + ПрограммаПроверки;
	КонецЕсли;
	...
	Если РезультатПроверкиВУниверсальномФормате = 0 ИЛИ РезультатПроверкиВУниверсальномФормате = "0" Тогда
		ЭлементДекорацияЗаголовкаПротокола.Заголовок = НСтр("ru='Ошибок не обнаружено. Отчет готов к отправке.';uk='Ошибок не обнаружено. Отчет готов к отправке.'");
	...
	ИначеЕсли РезультатПроверкиВУниверсальномФормате = 4 ИЛИ РезультатПроверкиВУниверсальномФормате = "4" Тогда
		ЭлементДекорацияЗаголовкаПротокола.Заголовок = НСтр("ru='Есть предупреждения. При необходимости внесите исправления и повторите попытку.';uk='Есть предупреждения. При необходимости внесите исправления и повторите попытку.'");
...
Процедура КомандаСохранитьПротоколПроверки(Команда)
	...
	ИмяФайлаПротоколаПроверкиДляСохранения = ?(ЗначениеЗаполнено(ИмяФайлаПротокола), 
		ИмяФайлаПротокола, НСтр("ru='Протокол проверки';uk='Протокол перевірки'") + ".txt");
