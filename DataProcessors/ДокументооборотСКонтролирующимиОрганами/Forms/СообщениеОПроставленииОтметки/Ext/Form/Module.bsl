...
"%1';uk='Ошибка выгрузки вложения во временный файл:"
...
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ ЗначениеЗаполнено(УзелСвОтметка) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Некорректная структура XML файла: не обнаружен узел ""СвОтметка"".';uk='Некорректная структура XML файла: не обнаружен узел ""СвОтметка"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
