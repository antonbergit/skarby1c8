...
"%1';uk='Ошибка выгрузки сообщения о несоответствиях во временный файл:"
...
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ ЗначениеЗаполнено(УзелСвНесоответ) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Некорректная структура XML файла: не обнаружен узел ""СвНесоответ"".';uk='Некорректная структура XML файла: не обнаружен узел ""СвНесоответ"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
