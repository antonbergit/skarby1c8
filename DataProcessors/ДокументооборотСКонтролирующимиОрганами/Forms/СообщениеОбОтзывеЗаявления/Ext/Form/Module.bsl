...
"%1';uk='Ошибка выгрузки сообщения об отзыве во временный файл:"
...
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ ЗначениеЗаполнено(УзелСвОтзыв) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Некорректная структура XML файла: не обнаружен узел ""СвОтзыв"".';uk='Некорректная структура XML файла: не обнаружен узел ""СвОтзыв"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если ОбщиеСведения.Свойство("ПричЗам") Тогда
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Некорректная структура XML файла: некорректное значение атрибута ""ПричЗам"".';uk='Некорректная структура XML файла: некорректное значение атрибута ""ПричЗам"".'"),,,, Отказ);
			Возврат;	
		КонецЕсли;
	Иначе
