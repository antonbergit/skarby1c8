...
Процедура ВыгрузитьПакеты(ВидимыеЦиклы)
	...
	Если ВидимыеЦиклы.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Не выбраны циклы обмена!';uk='Не выбраны циклы обмена!'"));
	Иначе
		Если ВидимыеЦиклы.Количество() = 1 Тогда
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выгрузить цикл обмена ""%1""?';uk='Выгрузить цикл обмена ""%1""?'"), КонтекстЭДОКлиент.ПредставлениеЦиклаОбмена(ВидимыеЦиклы[0]));
		Иначе
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выгрузить выбранные %1 циклов обмена?';uk='Выгрузить выбранные %1 циклов обмена?'"), Строка(ВидимыеЦиклы.Количество()));
		КонецЕсли;
...
Функция СодержимоеОтправкиФССВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиФСС, ИмяФайлаДляКлиента)
	...
	ИначеЕсли РежимВыгрузки = 2 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ru='Протокол';uk='Протокол'") + " " + СсылкаНаОбъектОтправкиФСС["ИдентификаторОтправкиНаСервере"] + ".html";
	Иначе
...
Процедура ВыгрузитьСодержимоеОтправкиФСС(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Не выбраны отправки ФСС!';uk='Не выбраны отправки ФСС!'"));
		Возврат;
	КонецЕсли;
	...
	Если РежимВыгрузки = 0 Тогда
		...
		Если АдресВоВременномХранилище = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ru='Зашифрованный пакет отсутствует.';uk='Зашифрованный пакет отсутствует.'"));
			Возврат;
		КонецЕсли;
		ТекстЗаголовка = НСтр("ru='Выберите каталог для выгрузки отправленного пакета';uk='Выберите каталог для выгрузки отправленного пакета'");
		
	ИначеЕсли РежимВыгрузки = 1 Тогда
		
		Если АдресВоВременномХранилище = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ru='Квитанция об успехе отсутствует.';uk='Квитанция об успехе отсутствует.'"));
			Возврат;
		КонецЕсли;
		ТекстЗаголовка = НСтр("ru='Выберите каталог для выгрузки полученной квитанции';uk='Выберите каталог для выгрузки полученной квитанции'");
		
	ИначеЕсли РежимВыгрузки = 2 Тогда
		
		Если АдресВоВременномХранилище = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ru='Протокол обработки отсутствует.';uk='Протокол обработки отсутствует.'"));
			Возврат;
		КонецЕсли;
		ТекстЗаголовка = НСтр("ru='Выберите каталог для выгрузки полученного протокола обработки';uk='Выберите каталог для выгрузки полученного протокола обработки'");
...
Процедура КомандаУведомленияСоздать(Команда)
	...
	Если ЗначениеЗаполнено(ИмяДокументаУведомлениеОКонтролируемыхСделках) Тогда
		СписокВариантовВыбора.Добавить("Документы", НСтр("ru='Уведомление о контролируемых сделках';uk='Уведомление о контролируемых сделках'"));
	КонецЕсли;
	
	Если СуществуетРегламентированныйОтчетРозничныеЦеныНаТабак() Тогда
		СписокВариантовВыбора.Добавить("УведомленияОРозничныхЦенахНаТабак", НСтр("ru='Уведомления о розничных ценах на табак';uk='Уведомления о розничных ценах на табак'"));
	КонецЕсли;
	
	СписокВариантовВыбора.Добавить("РеестрыФНС", НСтр("ru='Реестры по НДС';uk='Реестры по НДС'"));
	
	Если СписокВариантовВыбора.Количество() = 0 Тогда
		ПредставлениеУведомленийОСпецрежимах = НСтр("ru='Уведомления';uk='Повідомлення'");
	Иначе
		ПредставлениеУведомленийОСпецрежимах = НСтр("ru='Прочие уведомления';uk='Прочие уведомления'");
	КонецЕсли;
	
	СписокВариантовВыбора.Добавить("ПрочиеУведомления", ПредставлениеУведомленийОСпецрежимах);
			
	СписокВариантовВыбора.Добавить("Файлы", НСтр("ru='Загрузить из файла';uk='Завантажити з файлу'"));
...
"Перейти к форме ""1С-Отчетность""?';uk='Создание реестров НДС выполняется из формы ""1С-Отчетность""."
...
Функция СодержимоеОтправкиФСРАРВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиФСРАР, ИмяФайлаДляКлиента)
	...
	ИначеЕсли РежимВыгрузки = 2 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ru='Протокол';uk='Протокол'") + " " + СсылкаНаОбъектОтправкиФСРАР["ИдентификаторОтправкиНаСервере"] + ".html";
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ВыгрузитьСодержимоеОтправкиФСРАР(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Не выбраны отправки в Росалкогольрегулирование!';uk='Не выбраны отправки в Росалкогольрегулирование!'"));
		Возврат;
	КонецЕсли;
...
Функция СодержимоеОтправкиРПНВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиРПН, ИмяФайлаДляКлиента)
	...
	ИначеЕсли РежимВыгрузки = 2 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ru='Протокол';uk='Протокол'") + " " + СсылкаНаОбъектОтправкиРПН["ИдентификаторОтправкиНаСервере"] + ".html";
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ВыгрузитьСодержимоеОтправкиРПН(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Не выбраны отправки в Росприродназдор!';uk='Не выбраны отправки в Росприродназдор!'"));
		Возврат;
	КонецЕсли;
...
Функция СодержимоеОтправкиФТСВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиФТС, ИмяФайлаДляКлиента)
	...
	Если РежимВыгрузки = 0 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ru='Подпись';uk='Підпис'") + СсылкаНаОбъектОтправкиФТС["ИмяФайлаВыгрузки"];
	ИначеЕсли РежимВыгрузки = 1 Тогда
		СсылкаНаСодержимое = СсылкаНаОбъектОтправкиФТС["Квитанция"];
		ИмяФайлаДляКлиента = НСтр("ru='Квитанция';uk='Квитанція'") + " " + СсылкаНаОбъектОтправкиФТС["ИмяФайлаВыгрузки"];
	ИначеЕсли РежимВыгрузки = 2 Тогда
		СсылкаНаСодержимое = СсылкаНаОбъектОтправкиФТС["Протокол"];
		ИмяФайлаДляКлиента = НСтр("ru='Протокол';uk='Протокол'") + " " + СсылкаНаОбъектОтправкиФТС["ИдентификаторОтправкиНаСервере"] + ".html";
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ВыгрузитьСодержимоеОтправкиФТС(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Не выбраны отправки в ФТС!';uk='Не выбраны отправки в ФТС!'"));
		Возврат;
	КонецЕсли;
	...
	Если РежимВыгрузки = 0 Тогда
		...
		Если АдресВоВременномХранилище = Неопределено Тогда
			ПоказатьПредупреждение(,НСтр("ru='Подпись отсутствует.';uk='Подпись отсутствует.'"));
			Возврат;
		КонецЕсли;
		ТекстЗаголовка = НСтр("ru='Выберите каталог для выгрузки отправленного пакета';uk='Выберите каталог для выгрузки отправленного пакета'");
	...
	ИначеЕсли РежимВыгрузки = 1 Тогда
		...
		Если АдресВоВременномХранилище = Неопределено Тогда
			ПоказатьПредупреждение(,НСтр("ru='Квитанция отсутствует.';uk='Квитанция отсутствует.'"));
			Возврат;
		КонецЕсли;
		ТекстЗаголовка = НСтр("ru='Выберите каталог для выгрузки полученной квитанции';uk='Выберите каталог для выгрузки полученной квитанции'");
...
Процедура ОсновнаяКоманднаяПанельОбменяться(Команда)
	...
	Если ВыбранныеОрганизации.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Учетная запись не выбрана.';uk='Учетная запись не выбрана.'"));
		Возврат;
	ИначеЕсли ВыбранныеОрганизации.Количество() = 1 Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Произвести обмен сообщениями по учетной записи ""%1""?';uk='Произвести обмен сообщениями по учетной записи ""%1""?'"), ВыбранныеОрганизации[0]);
	Иначе
		ТекстВопроса = НСтр("ru='Произвести обмен сообщениями по выбранным учетным записям?';uk='Произвести обмен сообщениями по выбранным учетным записям?'");
	КонецЕсли;
...
Процедура СообщитьОСменеРежимов(ПредыдущийРежим, НовыйРежим)
	...
	// отображаем сообщение для пользователя
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Режим отображения списка был переключен с ""%1"" на ""%2""';uk='Режим отображения списка был переключен с ""%1"" на ""%2""'"), ПредыдущийРежим, НовыйРежим);
