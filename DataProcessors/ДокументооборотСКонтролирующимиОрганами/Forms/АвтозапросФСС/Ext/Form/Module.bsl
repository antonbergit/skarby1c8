...
Процедура ОбновитьПанельЗапроса()
	...
	Если ЧислоПопыток = 3 Тогда
		НадписьОсталосьСекунд = НСтр("ru='Результатов отправки отчета получить не удалось.';uk='Результатов отправки отчета получить не удалось.'");
	Иначе
...
Процедура ОбновитьФорму()
	...
	Если СтатусОтправки <> ПредопределенноеЗначение("Перечисление.СтатусыОтправки.Отправлен") Тогда
		...
		ЗаголовокРезультата = НСтр("ru='Ошибка механизма отправки.';uk='Ошибка механизма отправки.'");
		
		Если СтатусОтправки = ПредопределенноеЗначение("Перечисление.СтатусыОтправки.НеПринят") Тогда
			ЦветШрифта = Новый Цвет(255, 0, 0);
			ЗаголовокРезультата = НСтр("ru='Отчет не принят: обнаружены ошибки.';uk='Отчет не принят: обнаружены ошибки.'");
		ИначеЕсли СтатусОтправки = ПредопределенноеЗначение("Перечисление.СтатусыОтправки.ПринятЕстьОшибки") Тогда
			...
			ЗаголовокРезультата = НСтр("ru='Отчет сдан: получена квитанция, обнаружены ошибки при логическом контроле.';uk='Отчет сдан: получена квитанция, обнаружены ошибки при логическом контроле.'");
		ИначеЕсли СтатусОтправки = ПредопределенноеЗначение("Перечисление.СтатусыОтправки.Сдан") Тогда
			ЦветШрифта = Новый Цвет(0, 179, 16);
			ЗаголовокРезультата = НСтр("ru='Отчет сдан: получена квитанция, ошибок не обнаружено.';uk='Отчет сдан: получена квитанция, ошибок не обнаружено.'");
		КонецЕсли;
