...
"%1';uk='Ошибка выгрузки уведомления об уточнении во временный файл:"
"%1'"),
...
"%1';uk='Ошибка чтения XML из файла уведомления об уточнении:"
"%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	...
	Если НЕ ЗначениеЗаполнено(УзелФайл) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Некорректная структура XML уведомления об уточнении: не обнаружен узел ""Файл"".';uk='Некорректная структура XML уведомления об уточнении: не обнаружен узел ""Файл"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелДокумент) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Некорректная структура XML уведомления об уточнении: не обнаружен узел ""Документ"".';uk='Некорректная структура XML уведомления об уточнении: не обнаружен узел ""Документ"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелСведУвед) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Некорректная структура XML уведомления об уточнении: не обнаружен узел ""СведУвед"".';uk='Некорректная структура XML уведомления об уточнении: не обнаружен узел ""СведУвед"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелОбщСвУвед) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Некорректная структура XML уведомления об уточнении: не обнаружен узел ""ОбщСвУвед"".';uk='Некорректная структура XML уведомления об уточнении: не обнаружен узел ""ОбщСвУвед"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелВыявлОшФайл) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Некорректная структура XML уведомления об уточнении: не обнаружен узел ""ВыявлОшФайл"".';uk='Некорректная структура XML уведомления об уточнении: не обнаружен узел ""ВыявлОшФайл"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
