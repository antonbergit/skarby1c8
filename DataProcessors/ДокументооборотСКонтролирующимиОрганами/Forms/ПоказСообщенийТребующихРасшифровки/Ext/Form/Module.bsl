...
"Расшифровать входящие сообщения? На время расшифровки ключ должен быть доступен.';uk='Имеются новые, требующие расшифровки сообщения. Их расшифровка может занять продолжительное время."
...
Процедура ЗаполнитьРеквизитыИНастройки(МассивСообщений, Отказ)
	...
	Для Каждого СтрокаРезультата Из Результат Цикл
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Всего сообщений к расшифровке: %1.';uk='Всего сообщений к расшифровке: %1.'"),
...
Процедура Расшифровать()
	...
	Если МассивОрганизаций.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Не выбраны организации.';uk='Не выбраны организации.'"));
		Возврат;
	КонецЕсли;
...
Процедура РасшифроватьТребующиеРасшифровку(Организация, ВсегоСообщений, КоличествоНеРасшифрованных, КоличествоРасшифрованных, НомерОбъекта, Счетчик, ТребующиеРасшифровки)
	...
	Если НомерОбъекта + 1 > ТребующиеРасшифровки.Количество() Тогда
		...
		Если КоличествоНеРасшифрованных = 0 Тогда 
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Расшифровано сообщений: %1 из %2.';uk='Расшифровано сообщений: %1 из %2.'"),
			Формат(КоличествоРасшифрованных, "ЧН=; ЧГ="), Формат(ВсегоСообщений, "ЧГ="));
			ТекстПояснения = НСтр("ru='Расшифровка завершена.';uk='Розшифровка завершена.'");
		...
		Иначе
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Расшифровано сообщений: %1. Не расшифровано: %2.';uk='Расшифровано сообщений: %1. Не расшифровано: %2.'"),
			Формат(КоличествоРасшифрованных, "ЧН=; ЧГ="), Формат(КоличествоНеРасшифрованных, "ЧГ="));
			
			ТекстПояснения = НСтр("ru='Расшифровка завершена с ошибками.';uk='Расшифровка завершена с ошибками.'");
...
Процедура ВывестиВТаблицу(Организация,
	...
	Если РасшифровкаЗавершена Тогда 
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1Расшифровано сообщений: %2 из %3.';uk='%1Расшифровано сообщений: %2 из %3.'"),
			?(КоличествоНеРасшифрованных = Неопределено ИЛИ КоличествоНеРасшифрованных = 0, 
				НСтр("ru='Расшифровка завершена. ';uk='Расшифровка завершена. '"), НСтр("ru='Расшифровка завершена с ошибками. ';uk='Расшифровка завершена с ошибками. '")),
	...
	Иначе 
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Расшифровывается сообщение: %1 из %2.%3';uk='Расшифровывается сообщение: %1 из %2.%3'"),
			Счетчик,
			РезультатОтбора[0].КоличествоСообщений,
			?(КоличествоНеРасшифрованных = Неопределено ИЛИ КоличествоНеРасшифрованных = 0,
				"",
				НСтр("ru=' Не расшифровано: ';uk=' Не расшифровано: '") + КоличествоНеРасшифрованных));
