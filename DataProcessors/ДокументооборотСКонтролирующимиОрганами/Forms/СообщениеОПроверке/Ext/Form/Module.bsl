...
"%1';uk='Ошибка выгрузки сообщения о проверке во временный файл:"
...
"%1';uk='Ошибка чтения XML из файла:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелФайл) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Некорректная структура XML файла: не обнаружен узел ""Файл"".';uk='Некорректная структура XML файла: не обнаружен узел ""Файл"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелДокумент) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Некорректная структура XML файла: не обнаружен узел ""Документ"".';uk='Некорректная структура XML файла: не обнаружен узел ""Документ"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УзелСвПроверка) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Некорректная структура XML файла: не обнаружен узел ""СвПроверка"".';uk='Некорректная структура XML файла: не обнаружен узел ""СвПроверка"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ОбщиеСведения.Свойство("Отметка") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Некорректная структура XML файла: не обнаружен атрибут ""Отметка"".';uk='Некорректная структура XML файла: не обнаружен атрибут ""Отметка"".'"),,,, Отказ);
		Возврат;
	Иначе
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Некорректная структура XML файла: некорректное значение атрибута ""Отметка"".';uk='Некорректная структура XML файла: некорректное значение атрибута ""Отметка"".'"),,,, Отказ);
			Возврат;
		КонецЕсли;
	КонецЕсли;
