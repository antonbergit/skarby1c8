...
Процедура КоманднаяПанельМастерДалее(Команда)
	...
	Если ТекущаяСтраница = Элементы.СтраницаВыборОрганизации Тогда
		...
		Элементы.ПодсказкаПоРезультатамШаг2.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Выберите пользователей, которые будут иметь право использовать 1С-Отчетность по %1';uk='Выберите пользователей, которые будут иметь право использовать 1С-Отчетность по %1'"),
			Организация);
			
		Элементы.ПодсказкаКонечныйРезультат.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Поздравляем!"
"Учетная запись по %1 успешно восстановлена';uk='Поздравляем!"
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаИнформацияОбУчетнойЗаписи Тогда
		...
		Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаВыборКлюча Тогда
			Если Не ЗначениеЗаполнено(КлючЭлектроннойПодписи) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Укажите ключ электронной подписи';uk='Укажите ключ электронной подписи'"), ,"КлючЭлектроннойПодписи");
				МастерДалее = Ложь;
			КонецЕсли;
		...
		ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаВыборСпецоператораСвязи Тогда
			Если Не ЗначениеЗаполнено(Спецоператор) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Укажите оператора электронного документооборота';uk='Укажите оператора электронного документооборота'"), ,"Спецоператор");
				МастерДалее = Ложь;
			КонецЕсли;
			Если Не ЗначениеЗаполнено(ИдентификаторАбонента) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Укажите идентификатор абонента';uk='Укажите идентификатор абонента'"), ,"ИдентификаторАбонента");
				МастерДалее = Ложь;
			ИначеЕсли СтрДлина(ИдентификаторАбонента) <> 36 И СтрДлина(ИдентификаторАбонента) <> 39 Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Указан некорректный идентификатор абонента';uk='Указан некорректный идентификатор абонента'"), ,"ИдентификаторАбонента");
				МастерДалее = Ложь;
			КонецЕсли;
...
Функция ПолучитьКлючиЭлектроннойПодписиВМоделиСервисаНаСервере()
	...
	НаименованиеЗадания = НСтр("ru='Получение ключей электронной подписи в модели сервиса';uk='Получение ключей электронной подписи в модели сервиса'");
...
Процедура ЗагрузитьПодготовленныеДанныеНаСервере()
	...
	Если ЕстьДействующиеКлючи Тогда
		...
		Для Каждого СтрокаТаблицы Из ТаблицаКлючей Цикл
			ПредставлениеКлюча = НСтр("ru='%1 (%2 - %3)';uk='%1 (%2 - %3)'");
...
"Для их использования необходимо обратиться в службу поддержки.';uk='Для %1 не найдены ключи электронной подписи для приложения с идентификатором %2, но найдены для %3."
