...
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ ЗначениеЗаполнено(УзелСвНалОргПост) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Некорректная структура XML квитанции: не обнаружен узел ""СвНалОргПост"".';uk='Некорректная структура XML квитанции: не обнаружен узел ""СвНалОргПост"".'"),,,, Отказ);
		Возврат;
	КонецЕсли;
