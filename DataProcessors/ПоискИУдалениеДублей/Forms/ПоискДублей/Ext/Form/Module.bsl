...
"Введіть умови відбору та порівняння і натисніть ""Знайти дублі"".'");
	...
	ОтображениеСостояния.Видимость = Истина;
	ОтображениеСостояния.Текст = НСтр("ru='Удаление дублей...';uk='Видалення дублів...'");
	ОтображениеСостояния.Картинка = Элементы.ДлительнаяОперация48.Картинка;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновоеУдалениеДублей" Тогда
		ТекстВопроса = НСтр("ru='Прервать удаление дублей и закрыть форму?';uk='Перервати видалення дублів і закрити форму?'");
	КонецЕсли;
...
Процедура ОбновитьМестаИспользованияКандидата(Знач ИдентификаторСтроки)
	...
	Если ДанныеСтроки.ПолучитьРодителя() = Неопределено Тогда
		...
		Элементы.ОписаниеТекущейГруппыДублей.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для элемента ""%1"" найдено дублей: %2';uk='Для елемента ""%1"" знайдено дублів: %2'"),
...
Процедура ОбновитьМестаИспользованияНеобработанныхДубли(Знач СтрокаДанных)
	...
	Если ДанныеСтроки.ПолучитьРодителя() = Неопределено Тогда
		...
		Элементы.ОписаниеТекущейГруппыДублей1.Заголовок = НСтр("ru='Для просмотра причин выберите проблемный элемент-дубль.';uk='Для перегляду причин виберіть проблемний елемент-дубль.'");
...
Процедура ОбновитьОписаниеСостоянияНайденныхДублей(Форма)
	...
	Форма.ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Найдено дублей: %2 (среди элементов: %1). Все отмеченные элементы будут помечены на удаление и заменены во всех местах использования на оригиналы (отмечены стрелкой).';uk='Знайдене дублів: %2 ( серед елементів: %1). Усі відзначені елементи будуть позначені на видалення й замінені у всіх місцях використання на оригінали (відзначені стрілкою).'"),
...
Процедура ИнициализироватьСценарийМастера()
	...
	КнопкиМастера = КнопкиМастера();
	КнопкиМастера.Назад.Видимость = Ложь;
	КнопкиМастера.Далее.Заголовок = НСтр("ru='Удалить дубли >';uk='Видалити дублі >'");
	КнопкиМастера.Далее.Подсказка = НСтр("ru='Удалить дубли';uk='Видалити дублі'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ru='Закрыть';uk='Закрити'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ru='Отказаться от поиска и замены дублей';uk='Відмовитися від пошуку і заміни дублів'");
	...
	КнопкиМастера.Далее.Видимость = Ложь;
	КнопкиМастера.Отмена.Заголовок = НСтр("ru='Прервать';uk='Перервати'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ru='Прервать удаление дублей';uk='Перервати видалення дублів'");
	...
	КнопкиМастера = КнопкиМастера();
	КнопкиМастера.Назад.Видимость = Ложь;
	КнопкиМастера.Далее.Заголовок = НСтр("ru='Повторить удаление >';uk='Повторити видалення >'");
	КнопкиМастера.Далее.Подсказка = НСтр("ru='Удалить дубли';uk='Видалити дублі'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ru='Закрыть';uk='Закрити'");
...
Процедура ШагПоискНеВыполнялсяПриАктивации(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	Для Каждого Правило Из ПравилаПоиска Цикл
		...
		ИначеЕсли Правило.Правило = "Подобно" Тогда
			Сравнение = Правило.ПредставлениеРеквизита + " " + НСтр("ru='совпадает по похожим словам';uk='збігається по схожих словах'");
		Иначе
			Продолжить;
		КонецЕсли;
...
Функция ЗапуститьФоновоеЗадание()
	...
	Если НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновыйПоискДублей" Тогда
		...
		НаименованиеМетода = НСтр("ru='Поиск и удаление дублей: Поиск дублей';uk='Пошук і видалення дублів: Пошук дублів'");
	...
	ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновоеУдалениеДублей" Тогда
		...
		НаименованиеМетода = НСтр("ru='Поиск и удаление дублей: Удаление дублей';uk='Пошук і видалення дублів: Видалення дублів'");
...
Процедура ПослеЗавершенияФоновогоЗадания(Задание, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Если НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновыйПоискДублей" Тогда
			Кратко = НСтр("ru='При поиске дублей возникла ошибка:';uk='При пошуку дублів виникла помилка:'");
		ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновоеУдалениеДублей" Тогда
			Кратко = НСтр("ru='При удалении дублей возникла ошибка:';uk='При видаленні дублів виникла помилка:'");
		КонецЕсли;
...
Функция ЗаполнитьРезультатыПоискаДублей(Знач АдресРезультата)
	...
	Если ВсегоНайденоДублей = ЭлементыДерева.Количество() Тогда
		ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(Элементы.Информация16.Картинка, " ",
			НСтр("ru='Не обнаружено дублей по указанным условиям.';uk='Не виявлено дублів по зазначених умовах.'"));
	Иначе
		ОбновитьОписаниеСостоянияНайденныхДублей(ЭтотОбъект);
	КонецЕсли;
