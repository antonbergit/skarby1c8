...
Процедура ОтменитьСоответствие(Команда)
	...
	ПоказатьВопрос(ОповещениеПриЗавершении, НСтр("ru='Отменить связь выбранных организаций?';uk='Отменить связь выбранных организаций?'"), РежимДиалогаВопрос.ОКОтмена,, КодВозвратаДиалога.ОК);
...
Процедура СоздатьКонтрагентов(Команда)
	...
	ПоказатьВопрос(ОповещениеПриЗавершении, НСтр("ru='Создать контрагентов по выделенным строкам организаций ЕГАИС?';uk='Создать контрагентов по выделенным строкам организаций ЕГАИС?'"), РежимДиалогаВопрос.ОКОтмена);
...
Процедура Подключаемый_СоздатьКонтрагентов()
	...
	Если СозданныеКонтрагенты.Количество() = 1 Тогда
		ТекстСообщения = НСтр("ru='Создан контрагент %Наименование%.';uk='Создан контрагент %Наименование%.'");
	...
	Иначе
		ТекстСообщения = НСтр("ru='Создано %Количество% контрагентов.';uk='Создано %Количество% контрагентов.'");
...
Функция СоздатьКонтрагентовНаСервере(АдресПараметровСозданияКонтрагентов)
	...
	Для каждого ОрганизацияЕГАИС Из ПараметрыСозданияКонтрагентов.МассивЭлементов Цикл
		...
		КонтрагентОбъект.СтранаРегистрации = Справочники.СтраныМира.Украина;
...
Функция ПроверитьВозможностьУстановкиСоответствия(ОрганизацияЕГАИС, Контрагент, ТекстОшибки)
	...
	Если Контрагент.ЭтоГруппа Тогда
		ТекстОшибки = НСтр("ru='Связка организации ЕГАИС с группой контрагентов невозможна.';uk='Связка организации ЕГАИС с группой контрагентов невозможна.'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ТекстОшибки = НСтр("ru='Выбранная организация уже имеет связку.';uk='Выбранная организация уже имеет связку.'");
		Возврат Ложь;
	КонецЕсли;
...
Функция ПроверитьВозможностьСозданияКонтрагентов(МассивОрганизацийЕГАИС, ТекстОшибки)
	...
	Если РезультатЗапроса.Пустой() Тогда
		Если МассивОрганизацийЕГАИС.Количество() > 1 Тогда
			ТекстОшибки = НСтр("ru='Все выбранные организации уже имеют связку.';uk='Все выбранные организации уже имеют связку.'");
		Иначе
			ТекстОшибки = НСтр("ru='Выбранная организация уже имеет связку.';uk='Выбранная организация уже имеет связку.'");
		КонецЕсли;
