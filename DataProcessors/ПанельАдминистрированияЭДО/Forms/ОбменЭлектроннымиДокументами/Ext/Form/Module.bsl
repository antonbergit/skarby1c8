...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЕстьОбменСКонтрагентами Тогда
		Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
			...
			Элементы.ГруппаОткрытьПомощникПодключенияКСервисуЭДО.Видимость = Ложь;
...
Процедура ОткрытьПомощникПодключенияКПрямомуОбмену(Команда)
	...
	МодульОбработки = ОбщегоНазначенияКлиент.ОбщийМодуль("ОбменСКонтрагентамиКлиент");
	МодульОбработки.ПомощникПодключенияКПрямомуОбмену();
...
Процедура ИзменитьОповещениеЭДО(ВключитьОповещенияЭДО = Ложь)
	...
	Если ВключитьОповещенияЭДО Тогда
		Если ОповещатьОСобытияхЭДО() Тогда
			ОбменСКонтрагентамиКлиент.ИзменитьОповещенияЭДО(Истина);
		КонецЕсли;
		
	Иначе
		ОбменСКонтрагентамиКлиент.ИзменитьОповещенияЭДО();
...
Процедура ПолучитьТекстЗаголовкаИРасписанияРегламентнойНастройки(ТекстРасписания, ИдентификаторЗадания)
	...
	Если Использование Тогда
		ПрефиксРасписания = НСтр("ru='Расписание:%1%2';uk='Расписание:%1%2'");
		
	Иначе
		ПрефиксРасписания = НСтр("ru='Расписание (НЕ АКТИВНО):%1%2';uk='Расписание (НЕ АКТИВНО):%1%2'");
	КонецЕсли;
