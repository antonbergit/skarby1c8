...
Процедура Конвертация_ПередЗагрузкойДанных(ФайлОбмена, Отказ) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	СостоянияЗаказовПокупателей.Ссылка КАК СостояниеЗаказов
	|ИЗ
	|	Справочник.СостоянияЗаказовПокупателей КАК СостоянияЗаказовПокупателей
	|ГДЕ
	|	СостоянияЗаказовПокупателей.УдалитьСтатусЗаказа = ЗНАЧЕНИЕ(Перечисление.УдалитьСтатусыЗаказов.ВРаботе)";
	...
	|ГДЕ
	|	СостоянияЗаказовПоставщикам.УдалитьСтатусЗаказа = ЗНАЧЕНИЕ(Перечисление.УдалитьСтатусыЗаказов.ВРаботе)";
...
Процедура Конвертация_ПослеЗагрузкиДанных() Экспорт
	...
	// Установка корректного типа номенклатуры "Расход".
	// Расход уже не используется в УНФ
	...
	//		|НаправлениеДеятельности, Склад, Спецификация, СпособПополнения,
	...
	//	);
	//	
	//	НоваяНоменклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Расход;
	...
	//	НоменклатураОбъект.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Расход;
...
Процедура ПКО_ПослеЗагрузки_ВерсииФайлов(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
                                           ИмяТипаОбъекта, ОбъектНайден, НаборЗаписей) Экспорт
...
Процедура ПКО_ПослеЗагрузки_Файлы(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
                                           ИмяТипаОбъекта, ОбъектНайден, НаборЗаписей) Экспорт
...
Процедура ПКО_ПослеЗагрузки_КассыККМ(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Если НЕ ЗначениеЗаполнено(Объект.ВалютаДенежныхСредств) Тогда
		// Объект.ВалютаДенежныхСредств = Справочники.Валюты.НациональнаяВалюта;
...
Процедура ПКО_ПослеЗагрузки_Организации(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	|ГДЕ
	|	СтавкиНДС.Ссылка = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ОсновнаяСтавкаНДС)";
...
Процедура ПКО_ПослеЗагрузки_ВводОстатковАвансыПоЗаказамПолученныеОтКлиентов(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Для каждого Строка Из Объект.РасчетыСПокупателями Цикл
		Если НЕ ЗначениеЗаполнено(Строка.Документ) Тогда
			...
			СтрокаКомментарий = Нстр("ru='Сформирован автоматически документом Ввод начальных остатков № %Номер% от %Дата%';uk='Сформований автоматично документом Введення початкових залишків № %Номер% від %Дата%'");
		...
		Строка.СтавкаНДС = Объект.Организация.СтавкаНДСПоУмолчанию;
	КонецЦикла;
...
Процедура ПКО_ПослеЗагрузки_ЗаказПокупателя(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Если НЕ ЗначениеЗаполнено(Объект.СостояниеЗаказа) Тогда
		Если ЗначениеЗаполнено(Параметры.СостояниеЗаказовПокупателей) Тогда
			Объект.СостояниеЗаказа = Параметры.СостояниеЗаказовПокупателей;
		Иначе
			Объект.СостояниеЗаказа = Справочники.СостоянияЗаказовПокупателей.Открыт;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПКО_ПослеЗагрузки_Событие(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Если ЗначениеЗаполнено(ПараметрыОбъекта.Получить("Контрагент")) Тогда
		СтрокаТЧ = Объект.Участники.Добавить();
		СтрокаТЧ.Контакт = ПараметрыОбъекта.Получить("Контрагент");
	КонецЕсли;
...
Процедура ПКО_ПослеЗагрузки_УчетныеЗаписиЭлектроннойПочты(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
                                           ИмяТипаОбъекта, ОбъектНайден, НаборЗаписей) Экспорт

	ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(Объект.Ссылка, ПараметрыОбъекта["Пароль"]);
	ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(Объект.Ссылка, ПараметрыОбъекта["ПарольSMTP"], "ПарольSMTP");
