...
Процедура КодГородаПриИзменении(Элемент)
	...
	Если (КодСтраны = "+7" ИЛИ КодСтраны = "8") И СтрНачинаетсяС(КодГорода, "9") И СтрДлина(КодГорода) <> 3 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Кода мобильных телефонов начинающиеся на цифру 9 имеют фиксированную длину в 3 цифры, например - 916.';uk='Коди мобільних телефонів, що починаються на цифру 9 мають фіксовану довжину в 3 цифри, наприклад - 916.'"),, "КодГорода");
	КонецЕсли;
