...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если АдресТолькоУкраинский  Тогда
		...
		Если Страна <> СтранаУкраина Тогда
			...
			КонецЕсли;
			ТекстПредупрежденияПриОткрытии = НСтр("ru='Адрес введен некорректно: допускается ввод только украинских адресов. Значение поля ""Страна"" было изменено на ""Украина"", необходимо проверить остальные поля.';uk='Адреса введена некоректно: допускається введення тільки українських адресів. Значення поля ""Країна"" було змінено на ""Україна"", необхідно перевірити інші поля.'"); 
...
Процедура ИндексОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ИначеЕсли ВыбранноеЗначение.Отказ Тогда
		Если ВыбранноеЗначение.Свойство("Индекс") И НЕ ПустаяСтрока(ВыбранноеЗначение.Индекс) 
			...
			ТекстВопроса = НСтр("ru='Индекс %1 не найден в адресном классификаторе. Возможные причины:';uk='Індекс %1 не знайдений в адресному класифікаторі. Можливі причини:'") + Символы.ПС;
			ТекстВопроса = ТекстВопроса + НСтр("ru='  - индекс относится к региону, по которому в программе отсутствуют адресные сведения;';uk='  - індекс відноситься до регіону, по якім у програмі відсутні адресні відомості;'") + Символы.ПС;
			ТекстВопроса = ТекстВопроса + НСтр("ru='  - адресные сведения, загруженные в программу, устарели;';uk='  - адресні відомості, завантажені в програму, застаріли;'") + Символы.ПС;
			ТекстВопроса = ТекстВопроса + НСтр("ru='  - индекс введен некорректно.';uk='  - індекс уведений некоректно.'");
			ТекстВопроса = СтрШаблон(ТекстВопроса, Формат(ВыбранноеЗначение.Индекс, "ЧРГ=' '; ЧГ=0"));
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Обновить классификатор';uk='Оновити класифікатор'"));
			Кнопки.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru='Отмена';uk='Відмінити'"));
			ПоказатьВопрос(Оповещение, ТекстВопроса, Кнопки);
		Иначе
			Если НЕ ПодборПоИндексуДоступен Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru='Индекс %1 не найден в адресном классификаторе';uk='Індекс %1 не знайдений в адресному класифікаторі'"),
...
Процедура ЗаполнитьАдресПоДаннымИндекса(Контекст, Знач ДанныеИндекса)
	...
	// Предполагаем, что адрес все еще украинский.
	...
	Если ТипЗначения = Тип("Структура") Тогда
		...
		Иначе 
			Если НаселенныйПунктДетально <> Неопределено Тогда
				НовыеДанныеПоНаселенномуПунктуДетально = НаселенныйПунктДетальноПоИдентификатору(ИдентификаторНаселенногоПункта);
				...
				КонецЦикла;
			Иначе
				НаселенныйПунктДетально = НаселенныйПунктДетальноПоИдентификатору(ИдентификаторНаселенногоПункта);
			КонецЕсли;
		КонецЕсли;
...
"Початкова адреса буде переміщена в коментар."
"Продовжити?'");
	Иначе
...
Процедура ВвестиАдресВСвободнойФормеЗавершение(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	...
	Если РазрешитьВводАдресаВСвободнойФорме Тогда
		Комментарий = ?(НЕ ПустаяСтрока(Комментарий), Комментарий + Символы.ПС, "")
			+ НСтр("ru='Исходный адрес:';uk='Початкова адреса:'") + " " + ПредставлениеАдреса;
		ПодключитьОбработчикОжидания("УстановитьПиктограммуКомментария", 0.1, Истина);
	КонецЕсли;
...
Функция РезультатВыбора(Контекст, ВозвращатьСписокЗначений = Ложь)
	...
	ИначеЕсли Контекст.Страна = Контекст.СтранаУкраина И ПустаяСтрока(XDTOИнформация.Представление) Тогда
		ДанныеВыбора = "";
...
Процедура ОбработкаИзмененияСтраныКлиент()
	...
	ОпределениеРежимаВводаАдреса();
	ЭтоУкраинскийАдрес = Страна = СтранаУкраина;
	
	Элементы.Индекс.Видимость = ЭтоУкраинскийАдрес;
	Элементы.ТипАдреса.ТекущаяСтраница = ?(ЭтоУкраинскийАдрес, Элементы.УкраинскийАдрес, Элементы.ИностранныйАдрес);
	
	// Проверять, вводить в свободной форме и искать по индексу можем только украинские адреса
	Элементы.ПроверитьЗаполнениеАдреса.Доступность = ЭтоУкраинскийАдрес;
	
	Элементы.ВвестиАдресВСвободнойФорме.Доступность =  ЭтоУкраинскийАдрес;
	Элементы.ВвестиАдресВСвободнойФормеВсеДействия.Доступность  = ЭтоУкраинскийАдрес;
...
Процедура ОпределениеРежимаВводаАдреса()
	...
	Если Страна = СтранаУкраина Тогда
		// УКРАИНА. Загружать можем только украинские адреса.
...
Процедура УстановитьИндексИПредставление(Контекст, XDTOКонтактная = Неопределено)
	...
	Если Контекст.Страна = Контекст.СтранаУкраина Тогда
...
Процедура УстановитьЗначениеИндекса(Контекст, XDTOКонтактная = Неопределено)
	...
	УправлениеКонтактнойИнформациейСлужебный.ПочтовыйИндексАдреса(Адрес);
...
Процедура ЗначениеРеквизитовПоКонтактнойИнформации(Контекст, РедактируемаяИнформация)
	...
	Если УправлениеКонтактнойИнформациейСлужебный.ЭтоУкраинскийАдрес(ДанныеАдреса) Тогда
		...
		КонецЕсли;
		ЗначениеРеквизитовПоКонтактнойИнформацииАдресУкраина(ДанныеАдреса, Контекст);
	Иначе
		Адрес = Строка(ДанныеАдреса.Состав);
		Контекст.НаселенныйПунктДетально = УправлениеКонтактнойИнформациейКлиентСервер.СтруктураЧастейАдресаНаселенногоПункта("БезКлассификатора");
...
&НаСервереБезКонтекста
Процедура ЗначениеРеквизитовПоКонтактнойИнформацииАдресУкраина(ДанныеАдреса, Контекст)
...
Функция КонтактнаяИнформацияПоЗначениюРеквизитов(Контекст)
	...
	Если ВРег(Контекст.Страна) <> ВРег(Контекст.СтранаУкраина.Наименование) Тогда
		Если Контекст.РазрешитьВводАдресаВСвободнойФорме Тогда
	...
	Адрес.Состав = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ПространствоИмен, "АдресУкраина"));
	...
	Если Контекст.НаселенныйПунктДетально <> Неопределено Тогда
		Для Каждого КлючЗначение Из Контекст.НаселенныйПунктДетально Цикл
			...
			УправлениеКонтактнойИнформациейСлужебный.УстановитьXDTOРеквизитОбъекта(АдресУкраина, ПутьXPath, ЧастьПункта.Представление);
		КонецЦикла;
	КонецЕсли;
	...
	УправлениеКонтактнойИнформациейСлужебный.ЗданияИПомещенияАдреса(АдресУкраина, 
		Новый Структура("Здания, Помещения", ТаблицаЗданий, ТаблицаПомещений));
	
	// Индекс
	УправлениеКонтактнойИнформациейСлужебный.ПочтовыйИндексАдреса(АдресУкраина, Контекст.Индекс);
	
	//
	Если НЕ ПустаяСтрока( Контекст.ДополнительныйЭлемент) Тогда 
		УправлениеКонтактнойИнформациейСлужебный.ДобавитьДополнительныеЭлементыАдреса(АдресУкраина, Контекст.ДополнительныйЭлемент, 90);
	КонецЕсли;
	Если НЕ ПустаяСтрока(Контекст.ПодчиненныйЭлемент) Тогда 
		УправлениеКонтактнойИнформациейСлужебный.ДобавитьДополнительныеЭлементыАдреса(АдресУкраина, Контекст.ПодчиненныйЭлемент, 91);
	КонецЕсли;
	...
	Если Контекст.РазрешитьВводАдресаВСвободнойФорме И Контекст.ПредставлениеАдресаИзменено Тогда
		...
		Иначе
			...
			АдресУкраина.Адрес_по_документу = ВведенноеПредставление;
		КонецЕсли;
	Иначе
		//Результат.Представление = ВведенноеПредставление;
		////Результат.Представление = РасчетноеПредставление;
...
Процедура ЗаполнитьКонтекстПоXDTOАдреса(XDTOДанныеАдреса, Контекст, Уровни = Неопределено)
	...
	// Из переданного XDTO
	АдресУкраина = УправлениеКонтактнойИнформациейСлужебный.УкраинскийАдрес(XDTOДанныеАдреса);
		...
		Для Каждого КлючЗначение Из Контекст.НаселенныйПунктДетально Цикл
			...
			Если Уровни = Неопределено ИЛИ Уровни.Найти(Значение.Уровень) <> Неопределено Тогда
				Если Значение.Уровень <> 90 Тогда
					Представление = УправлениеКонтактнойИнформациейСлужебный.ПолучитьXDTOРеквизитОбъекта(АдресУкраина, Значение.ПутьXPath);
				Иначе
					Представление = УправлениеКонтактнойИнформациейСлужебный.НайтиДополнительныйЭлементАдреса(АдресУкраина);
				КонецЕсли;
...
Функция КонтекстФормыКлиент(НовыеДанные = Неопределено)
	...
	СписокРеквизитов = "
		...
		|Страна, КодСтраны, СтранаУкраина, СкрыватьНеактуальныеАдреса,
...
// Страну не трогаем, режим свободного ввода возможен только в Украине.
//
&НаСервере
...
Процедура ОбновитьДоступностьКлассификатора(Контекст)
	...
	Если ДоступностьКлассификатора.Отказ Тогда
		Контекст.ТекстСообщенияСервиса = НСтр("ru='Автоподбор и проверка адреса недоступны:';uk='Автопідбір і перевірка адреси недоступні:'") + Символы.ПС 
...
Функция НаселенныйПунктДетальноПоИдентификатору(ИдентификаторНаселенногоПункта)
	Возврат УправлениеКонтактнойИнформациейСлужебный.СписокРеквизитовНаселенныйПункт(ИдентификаторНаселенногоПункта);
КонецФункции
	...
	Если ТипЗначения = Тип("Структура") Тогда
		...
		Если МожноЗагружатьКлассификатор И РегионНеЗагружен Тогда
			...
			УправлениеКонтактнойИнформациейКлиент.ПредложениеЗагрузкиКлассификатора(
				СтрШаблон(НСтр("ru='Данные для ""%1"" не загружены.';uk='Дані для ""%1"" не завантажені.'"), ВыбранноеЗначение.Представление),
