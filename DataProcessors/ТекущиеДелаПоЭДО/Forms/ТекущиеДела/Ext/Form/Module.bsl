...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ОбменСКонтрагентамиСлужебныйВызовСервера.НемедленнаяОтправкаЭД() Тогда
		...
		Элементы.СформироватьПодписатьИОтправить.Заголовок = НСтр("ru='Утвердить и подписать';uk='Утвердить и подписать'");
		Команды.УтвердитьПодписатьИОтправить.Подсказка = НСтр("ru='Утвердить, подписать и подготовить к отправке"
"выделенные электронные документы';uk='Утвердить, подписать и подготовить к отправке"
...
"выделенные электронные документы';uk='Утвердить и подготовить к отправке"
"выделенные электронные документы'");
		КонецЕсли;
	ИначеЕсли НЕ ИспользоватьЭП Тогда
		Элементы.СформироватьПодписатьИОтправить.Заголовок = НСтр("ru='Утвердить и отправить';uk='Утвердить и отправить'");
		Команды.УтвердитьПодписатьИОтправить.Подсказка = НСтр("ru='Утвердить и отправить"
"выделенные электронные документы';uk='Утвердить и отправить"
"выделенные электронные документы'");
	КонецЕсли;
	...
	Если МассивДоступныхДокументов.Количество() > 0 Тогда
		...
		Для Каждого ТипДокумента Из МассивДоступныхДокументов Цикл
			...
			ПрикладноеИмяРеквизита = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении(
				ТипДокумента + "." + ИмяРеквизита);
			Если ЗначениеЗаполнено(ПрикладноеИмяРеквизита) Тогда
				ШаблонТекстПодзапроса = СтрЗаменить(ШаблонТекстПодзапроса, "Документ.Организация",
					ПрикладноеИмяРеквизита);
			ИначеЕсли Метаданные.Документы[ТипДокумента].Реквизиты.Найти(ИмяРеквизита) = Неопределено Тогда
				ШаблонТекстПодзапроса = СтрЗаменить(ШаблонТекстПодзапроса, "Документ.Организация", "NULL");
			КонецЕсли;
			...
			ПрикладноеИмяРеквизита = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении(
				ТипДокумента + "." + ИмяРеквизита);
			Если ЗначениеЗаполнено(ПрикладноеИмяРеквизита) Тогда
				ШаблонТекстПодзапроса = СтрЗаменить(ШаблонТекстПодзапроса, "Документ.Контрагент",
					ПрикладноеИмяРеквизита);
			ИначеЕсли Метаданные.Документы[ТипДокумента].Реквизиты.Найти(ИмяРеквизита) = Неопределено Тогда
				ШаблонТекстПодзапроса = СтрЗаменить(ШаблонТекстПодзапроса, "Документ.Контрагент", "NULL");
			КонецЕсли;
			...
			Если ПрикладноеИмяРеквизита <> Неопределено Тогда
				ШаблонТекстПодзапроса = СтрЗаменить(ШаблонТекстПодзапроса, "Документ.СуммаДокумента",
					ПрикладноеИмяРеквизита);
			ИначеЕсли Метаданные.Документы[ТипДокумента].Реквизиты.Найти(ИмяРеквизита) = Неопределено Тогда
				ШаблонТекстПодзапроса = СтрЗаменить(ШаблонТекстПодзапроса, "Документ.СуммаДокумента", "NULL");
			КонецЕсли;
			
			ТекстПодзапроса = ТекстПодзапроса + СтрЗаменить(ШаблонТекстПодзапроса, "&Документ", "Документ." + ТипДокумента);
...
Процедура УстановитьОтборПоДополнительнойИнформацииВСписке(Отбор, Значение, ИспользоватьОтбор)
	...
	ГруппаИли = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(Отбор.Элементы, НСтр("ru='Доп. информация';uk='Доп. информация'"), ТипГруппы);
...
Функция ДанныеДереваДействий(Знач ДеревоДействий, Запросы)
	...
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Обработать",         НСтр("ru='Отразить в учете';uk='Отразить в учете'"),     Массив[1]);
	...
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Исправить",          НСтр("ru='Исправить';uk='Исправить'"),            Массив[4]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Аннулировать",       НСтр("ru='Аннулировать';uk='Аннулировать'"),         Массив[5]);
	...
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Распаковать",        НСтр("ru='Распаковать';uk='Распаковать'"),          Массив[8]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Приглашения",        НСтр("ru='Приглашения';uk='Приглашения'"),          Массив[9]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "ТребуетсяПригласить",НСтр("ru='Требуется пригласить';uk='Требуется пригласить'"), Массив[10]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "ЖдемСогласия",       НСтр("ru='Ждем согласия';uk='Ждем согласия'"),        Массив[11]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "ТребуетсяСогласие",  НСтр("ru='Требуется согласие';uk='Требуется согласие'"),   Массив[12]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Входящие",           НСтр("ru='Входящие';uk='Вхідні'"),             0);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "Исходящие",          НСтр("ru='Исходящие';uk='Исходящие'"),            0);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "ПодписатьИсх",       НСтр("ru='Подписать';uk='Підписати'"),            Массив[15]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "УтвердитьИсх",       НСтр("ru='Утвердить';uk='Затвердити'"),            Массив[16]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "ИсправитьИсх",       НСтр("ru='Исправить';uk='Исправить'"),            Массив[17]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "АннулироватьИсх",    НСтр("ru='Аннулировать';uk='Аннулировать'"),         Массив[18]);
	УстановитьПредставлениеСтрокРекурсивно(Дерево, "НаКонтролеИсх",      НСтр("ru='На контроле';uk='На контролі'"),          Массив[19]);
...
Процедура УстановитьСтатусПакетов(Знач МассивЭД, Знач СтатусПакета, КоличествоИзмененных)
	...
	Для Каждого СтрокаТаблицы Из МассивЭД Цикл
		...
		Исключение
			...
			ТекстОперации  = НСтр("ru='изменение статуса пакетов ЭД';uk='изменение статуса пакетов ЭД'");
...
Процедура СформироватьТаблицуБыстрогоОтбора()
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ru='Дата с:';uk='Дата с:'");
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ru='Сумма с:';uk='Сумма с:'");
	НоваяСтрока.Тип = "Число";
	НоваяСтрока.Значение = Неопределено;
	
	НоваяСтрока = БыстрыеОтборы.Добавить();
	НоваяСтрока.Параметр = "Сумма_По";
	НоваяСтрока.ПредставлениеПараметра = НСтр("ru='Сумма по:';uk='Сумма по:'");
	НоваяСтрока.Тип = "Число";
	НоваяСтрока.Значение = Неопределено;
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ru='Направление:';uk='Направление:'");
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ru='Доп. информация:';uk='Доп. информация:'");
	НоваяСтрока.Тип = "Строка";
	НоваяСтрока.Значение = "";
	
	НоваяСтрока = БыстрыеОтборы.Добавить();
	НоваяСтрока.Параметр = "Ответственный";
	НоваяСтрока.ПредставлениеПараметра = НСтр("ru='Ответственный:';uk='Ответственный:'");
...
Процедура УстановитьСтатусОтменен(МассивЭД)
	...
	ТекстОповещения = НСтр("ru='Изменен статус пакетов на ""Отменен"": (%1)';uk='Изменен статус пакетов на ""Отменен"": (%1)'");
...
"необходимо заполнить электронную почту.';uk='Для отправки приглашения к обмену ЭД для получателя %1"
"необходимо заполнить электронную почту.'");
		...
		Если Не ЗначениеЗаполнено(СтруктураПараметровКонтрагента[ИмяРеквизитаИННКонтрагента]) Тогда
			ШаблонСообщения = НСтр("ru='Для отправки приглашения к обмену ЭД для получателя %1"
"необходимо заполнить ИНН.';uk='Для отправки приглашения к обмену ЭД для получателя %1"
...
Процедура ПоказатьСкрытьБыстрыйПоиск()
	...
	Элементы.ЗаголовокОтбора.Заголовок = НСтр("ru='Быстрый поиск';uk='Швидкий пошук'") + ?(ПоказатьБыстрыйПоиск, " " + НСтр("ru='(скрыть)';uk='(скрыть)'"),  " " + НСтр("ru='(показать)';uk='(показать)'"));
...
Процедура ОтклонитьПриглашенияОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("СоответствиеПрофилейИПараметровСертификатов", СоответствиеПрофилейИПараметровСертификатов)
		...
		ТекстЗаголовка = НСтр("ru='Отклоняются приглашения';uk='Отклоняются приглашения'");
		...
		ШаблонСообщения = НСтр("ru='Отклонено приглашений: %1';uk='Отклонено приглашений: %1'");
...
"Необходимо выполнить тест настроек ЭДО с указанными контрагентами.';uk='При отклонении приглашений возникли ошибки."
...
Процедура ПринятьПриглашенияОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("СоответствиеПрофилейИПараметровСертификатов", СоответствиеПрофилейИПараметровСертификатов)
		...
		ТекстЗаголовка = НСтр("ru='Принимаются приглашения';uk='Принимаются приглашения'");
		...
		ШаблонСообщения = НСтр("ru='Принято приглашений: %1';uk='Принято приглашений: %1'");
...
"Необходимо выполнить тест настроек ЭДО с указанными контрагентами.';uk='При принятии приглашений возникли ошибки."
"Необходимо выполнить тест настроек ЭДО с указанными контрагентами.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
	КонецЕсли;
...
Процедура ОтправитьПриглашениеОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("СоответствиеПрофилейИПараметровСертификатов", СоответствиеПрофилейИПараметровСертификатов)
		...
		ТекстЗаголовка = НСтр("ru='Отправка приглашений получателям';uk='Отправка приглашений получателям'");
		
		ОтправленоПриглашений = 0;
		ОтправитьПриглашенияСервер(ОтправленоПриглашений, СоответствиеПрофилейИПараметровСертификатов);
		
		ШаблонСообщения = НСтр("ru='Отправлено приглашений: %1';uk='Отправлено приглашений: %1'");
...
"Необходимо выполнить тест настроек ЭДО с указанными контрагентами.';uk='При отправке приглашений возникли ошибки."
"Необходимо выполнить тест настроек ЭДО с указанными контрагентами.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
	КонецЕсли;
...
Процедура ОтправитьПакетыОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	ШаблонОповещения = НСтр("ru='Отправлено пакетов: (%1).';uk='Отправлено пакетов: (%1).'");
	ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОповещения, КоличествоОтправленных);
	
	Оповестить("ОбновитьСостояниеЭД");
		
	ЗаголовокОповещения = НСтр("ru='Отправка электронных документов';uk='Отправка электронных документов'");
...
//
	...
	ОтборЭлемента.ПравоеЗначение = НСтр("ru='(';uk='('");
