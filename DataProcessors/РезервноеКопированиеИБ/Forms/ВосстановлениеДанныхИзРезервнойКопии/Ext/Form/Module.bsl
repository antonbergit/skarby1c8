...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ru='Резервное копирование доступно только в клиенте под управлением ОС Windows.';uk='Резервне копіювання доступно тільки в клієнтові під управлінням ОС Windows.'");
...
"Подготовка к восстановлению данных из резервной копии отменена. Информационная база разблокирована.';uk='%1."
"Підготовка до оновлення даних з резервної копії скасована. Інформаційна база розблокована.'"),
