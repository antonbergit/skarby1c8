...
Процедура ПриЗакрытии()
	...
	РазблокироватьВсеЗаписи(ЭтотОбъект);
	Оповестить("ДатыЗапретаИзмененияПриЗакрытии");
	...
	Иначе
		...
		Если Контекст.РазрешитьИзменениеДанныхДоДатыЗапрета Тогда
			...
			Если Контекст.НачалоДня > СрокРазрешения Тогда
				ШаблонНадписи = НСтр("ru='Срок возможности изменения данных с %2 по %3 истек %1';uk='Строк можливості зміни даних з %2 по %3 минув %1'");
			Иначе
				Если РассчитатьДатуЗапрета Тогда
...
"После %1 будет запрещено изменение данных по %3';uk='По %1 можлива зміна даних з %2 по %3"
"Після %1 буде заборонена зміна даних по %3'")
					+ Символы.ПС;
			КонецЕсли;
			ТекстНадписи = Символы.ПС + СтрШаблон(ШаблонНадписи,
		...
		Контекст.Элементы.ПояснениеНепроизвольнойДаты.Заголовок =
			СтрШаблон(НСтр("ru='Запрещено изменение данных по %1';uk='Заборонена зміна даних до %1'"),
...
Функция ТекстВопросаУдалитьВсеДатыЗапретаКромеДатДляВсехПользователей()
	...
	Возврат СтрШаблон(НСтр("ru='Удалить все даты запрета, кроме установленных %1?';uk='Видалити усі дати заборони, крім встановлених %1?'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстСообщенияЗначениеДляВсехПользователейНеИзменяется()
	...
	Возврат СтрШаблон(НСтр("ru='Значение %1 не изменяется.';uk='Значення %1 не змінюється.'"),
		ТекстПредставленияДляВсехПользователей(ЭтотОбъект));
...
Функция ТекстСообщенияКомментарийДляВсехПользователейНеИзменяется()
	...
	Возврат СтрШаблон(НСтр("ru='Комментарий %1 не изменяется.';uk='Коментар %1 не змінюється.'"),
		ТекстПредставленияДляВсехПользователей(ЭтотОбъект));
...
"тому буде збережена установлення дати заборони ""%2"".'"),
...
Функция ТекстСообщенияДляУдаленияВыделитеОднуСтроку()
	...
	Возврат НСтр("ru='Для удаления выделите одну строку.';uk='Для видалення виділіть один рядок.'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляВсехПользователей()
	...
	Возврат СтрШаблон(НСтр("ru='Удалить даты запрета %1?';uk='Видалити дати заборони %1?'"),
...
Функция ТекстСообщенияДляВсехПользователейДатыЗапретаНеУстановлены()
	...
	Возврат СтрШаблон(НСтр("ru='%1 даты запрета не установлены.';uk='%1 дати заборони не встановлені.'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстСообщенияДляВсехПользователейРазделыВсегдаПоказываются()
	...
	Возврат СтрШаблон(НСтр("ru='%1 разделы всегда показываются.';uk='%1 розділи завжди показуються.'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстСообщенияДляВсехПользователейОбщаяДатаВсегдаПоказывается()
	...
	Возврат СтрШаблон(НСтр("ru='%1 <Общая дата> всегда показывается.';uk='%1 <Загальна дата> завжди показується.'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстСообщенияНастройкиСНевыбраннымиПользователямиНеСохранены(Контекст)
	...
	Если Контекст.Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Возврат НСтр("ru='Настройки с невыбранными информационными базами не сохранены.';uk='Налаштування з невибраними інформаційними базами не збережені.'");
	КонецЕсли;
	
	Возврат НСтр("ru='Настройки с невыбранными пользователями не сохранены.';uk='Налаштування з невибранними користувачами не збережені.'");
...
Функция ТекстСообщенияНастройкиСНевыбраннымиОбъектамиНеСохранены()
	...
	Возврат НСтр("ru='Настройки с невыбранными объектами не сохранены.';uk='Налаштування з невибраними об`єктами не збережені.'");
...
Функция ТекстСообщенияНастройкиСНезаполненнымиДатамиЗапретаДляОбъектовНеСохранены()
	...
	Возврат НСтр("ru='Настройки с незаполненными датами запрета для объектов не сохранены.';uk='Налаштування з незаповненими датами заборони для об`єктів не збережені.'");
...
Функция ТекстСообщенияПользователиБезНастроекДатЗапретаНеСохранены()
	...
	Возврат НСтр("ru='Пользователи без настроек дат запрета не сохранены.';uk='Користувачі без налаштувань дат заборони не збережені.'");
...
Функция ТекстСообщенияНастройкиСНезаполненнымиДатамиЗапретаДляРазделовНеСохранены()
	...
	Возврат НСтр("ru='Настройки с незаполненными датами запрета для разделов не сохранены.';uk='Налаштування з незаповненими датами заборони для розділів не збережені.'");
