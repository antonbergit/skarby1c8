...
Процедура ВыполнитьЗакрытиеМесяцаВДлительнойОперации(СтруктураПараметровФоновогоЗадания)
	...
	НСтр("ru='Выполняется закрытие месяца';uk='Виконується закриття місяця'")
	);
...
Процедура ПредупредитьОАктивномФоновомЗадании(Отказ = Истина)
	...
	Отказ = Истина;
	ТекстПредупреждения = НСтр("ru='Дождитесь окончания рабочего процесса (рекомендуется) либо прервите его самостоятельно.';uk='Дочекайтеся закінчення робочого процесу (рекомендується) або перервіть його самостійно.'");
	...
	Для Сч = 1 По 12 Цикл
		...
		Если НЕ ЗначениеЗаполнено(ЭтаФорма["ТекстОшибкаРаспределениеЗатрат" + Сч]) Тогда
			Если Элементы.Найти("РаспределениеЗатратКартинка" + Сч).Картинка = Элементы.Зеленый.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРаспределениеЗатрат" + Сч] = НСтр("ru='Распределение затрат выполнено успешно!';uk='Розподіл витрат виконаний успішно!'");
			...
			ИначеЕсли Элементы.Найти("РаспределениеЗатратКартинка" + Сч).Картинка = Элементы.ЗеленыйНеТребуется.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРаспределениеЗатрат" + Сч] = НСтр("ru='Распределение затрат не требуется.';uk='Розподіл витрат не потрібно.'");
			ИначеЕсли Элементы.Найти("РаспределениеЗатратКартинка" + Сч).Картинка = Элементы.Серый.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРаспределениеЗатрат" + Сч] = НСтр("ru='Распределение затрат не производилось.';uk='Розподіл витрат не проводився.'");
			ИначеЕсли Элементы.Найти("РаспределениеЗатратКартинка" + Сч).Картинка = Элементы.Красный.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРаспределениеЗатрат" + Сч] = НСтр("ru='Требуется выполнить распределение затрат.';uk='Потрібно виконати розподіл витрат.'");
			КонецЕсли;
		Иначе
		...
		Если НЕ ЗначениеЗаполнено(ЭтаФорма["ТекстОшибкаРасчетПрямыхЗатрат" + Сч]) Тогда
			Если Элементы.Найти("РасчетПрямыхЗатратКартинка" + Сч).Картинка = Элементы.Зеленый.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетПрямыхЗатрат" + Сч] = НСтр("ru='Расчет прямых затрат выполнен успешно!';uk='Розрахунок прямих витрат виконаний успішно!'");
				МесяцЗакрыт = Истина; // месяц закрыт, если распределение выполнено успешно
			ИначеЕсли Элементы.Найти("РасчетПрямыхЗатратКартинка" + Сч).Картинка = Элементы.ЗеленыйНеТребуется.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетПрямыхЗатрат" + Сч] = НСтр("ru='Расчет прямых затрат не требуется.';uk='Розрахунок прямих витрат не потрібен.'");
			ИначеЕсли Элементы.Найти("РасчетПрямыхЗатратКартинка" + Сч).Картинка = Элементы.Серый.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетПрямыхЗатрат" + Сч] = НСтр("ru='Расчет прямых затрат не производился.';uk='Розрахунок прямих витрат не проводився.'");
			ИначеЕсли Элементы.Найти("РасчетПрямыхЗатратКартинка" + Сч).Картинка = Элементы.Красный.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетПрямыхЗатрат" + Сч] = НСтр("ru='Требуется выполнить расчет прямых затрат.';uk='Потрібно виконати розрахунки прямих витрат.'");
			КонецЕсли;
		Иначе
			МесяцЗакрыт = Истина; // месяц закрыт, если есть ошибки
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ЭтаФорма["ТекстОшибкаРасчетФактическойСебестоимости" + Сч]) Тогда
			Если Элементы.Найти("РасчетФактическойСебестоимостиКартинка" + Сч).Картинка = Элементы.Зеленый.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетФактическойСебестоимости" + Сч] = НСтр("ru='Расчет фактической себестоимости выполнен успешно!';uk='Розрахунок фактичної собівартості виконаний успішно!'");
				МесяцЗакрыт = Истина; // месяц закрыт, если распределение выполнено успешно
			ИначеЕсли Элементы.Найти("РасчетФактическойСебестоимостиКартинка" + Сч).Картинка = Элементы.ЗеленыйНеТребуется.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетФактическойСебестоимости" + Сч] = НСтр("ru='Расчет фактической себестоимости не требуется.';uk='Розрахунок фактичної собівартості не потрібен.'");
			ИначеЕсли Элементы.Найти("РасчетФактическойСебестоимостиКартинка" + Сч).Картинка = Элементы.Серый.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетФактическойСебестоимости" + Сч] = НСтр("ru='Расчет фактической себестоимости не производился.';uk='Розрахунок фактичної собівартості не проводився.'");
			ИначеЕсли Элементы.Найти("РасчетФактическойСебестоимостиКартинка" + Сч).Картинка = Элементы.Красный.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетФактическойСебестоимости" + Сч] = НСтр("ru='Требуется выполнить расчет фактической себестоимости.';uk='Потрібно виконати розрахунки фактичної собівартості.'");
			КонецЕсли;
		Иначе
			МесяцЗакрыт = Истина; // месяц закрыт, если есть ошибки
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ЭтаФорма["ТекстОшибкаРасчетФинансовогоРезультата" + Сч]) Тогда
			Если Элементы.Найти("РасчетФинансовогоРезультатаКартинка" + Сч).Картинка = Элементы.Зеленый.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетФинансовогоРезультата" + Сч] = НСтр("ru='Расчет финансового результата выполнен успешно!';uk='Розрахунок фінансового результату виконаний успішно!'");
				МесяцЗакрыт = Истина; // месяц закрыт, если распределение выполнено успешно
			ИначеЕсли Элементы.Найти("РасчетФинансовогоРезультатаКартинка" + Сч).Картинка = Элементы.ЗеленыйНеТребуется.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетФинансовогоРезультата" + Сч] = НСтр("ru='Расчет финансового результата не требуется.';uk='Розрахунок фінансового результату не потрібен.'");
			ИначеЕсли Элементы.Найти("РасчетФинансовогоРезультатаКартинка" + Сч).Картинка = Элементы.Серый.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетФинансовогоРезультата" + Сч] = НСтр("ru='Расчет финансового результата не производился.';uk='Розрахунок фінансового результату не проводився.'");
			ИначеЕсли Элементы.Найти("РасчетФинансовогоРезультатаКартинка" + Сч).Картинка = Элементы.Красный.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетФинансовогоРезультата" + Сч] = НСтр("ru='Требуется выполнить расчет финансового результата.';uk='Потрібно виконати розрахунки фінансового результату.'");
			КонецЕсли;
		Иначе
			МесяцЗакрыт = Истина; // месяц закрыт, если есть ошибки
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ЭтаФорма["ТекстОшибкаРасчетКурсовыхРазниц" + Сч]) Тогда
			Если Элементы.Найти("РасчетКурсовыхРазницКартинка" + Сч).Картинка = Элементы.Зеленый.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетКурсовыхРазниц" + Сч] = НСтр("ru='Расчет курсовых разниц выполнен успешно!';uk='Розрахунок курсових різниць виконаний успішно!'");
				МесяцЗакрыт = Истина; // месяц закрыт, если распределение выполнено успешно
			ИначеЕсли Элементы.Найти("РасчетКурсовыхРазницКартинка" + Сч).Картинка = Элементы.ЗеленыйНеТребуется.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетКурсовыхРазниц" + Сч] = НСтр("ru='Расчет курсовых разниц не требуется.';uk='Розрахунок курсових різниць не потрібен.'");
			ИначеЕсли Элементы.Найти("РасчетКурсовыхРазницКартинка" + Сч).Картинка = Элементы.Серый.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетКурсовыхРазниц" + Сч] = НСтр("ru='Расчет курсовых разниц не производился.';uk='Розрахунок курсових різниць не проводився.'");
			ИначеЕсли Элементы.Найти("РасчетКурсовыхРазницКартинка" + Сч).Картинка = Элементы.Красный.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетКурсовыхРазниц" + Сч] = НСтр("ru='Требуется выполнить расчет курсовых разниц.';uk='Потрібно виконати розрахунки курсових різниць.'");
			КонецЕсли;
		Иначе
			МесяцЗакрыт = Истина; // месяц закрыт, если есть ошибки
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ЭтаФорма["ТекстОшибкаРасчетСебестоимостиВРознице" + Сч]) Тогда
			Если Элементы.Найти("РасчетСебестоимостиВРозницеКартинка" + Сч).Картинка = Элементы.Зеленый.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетСебестоимостиВРознице" + Сч] = НСтр("ru='Расчет себестоимости в рознице (суммовой учет) выполнен успешно!';uk='Розрахунок собівартості в роздробі (сумовий облік) виконаний успішно!'");
				МесяцЗакрыт = Истина; // месяц закрыт, если распределение выполнено успешно
			ИначеЕсли Элементы.Найти("РасчетСебестоимостиВРозницеКартинка" + Сч).Картинка = Элементы.ЗеленыйНеТребуется.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетСебестоимостиВРознице" + Сч] = НСтр("ru='Расчет себестоимости в рознице (суммовой учет) не требуется.';uk='Розрахунок собівартості в роздробі (сумовий облік) не потрібен.'");
			ИначеЕсли Элементы.Найти("РасчетСебестоимостиВРозницеКартинка" + Сч).Картинка = Элементы.Серый.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетСебестоимостиВРознице" + Сч] = НСтр("ru='Расчет себестоимости в рознице (суммовой учет) не производился.';uk='Розрахунок собівартості в роздробі (сумовий облік) не проводився.'");
			ИначеЕсли Элементы.Найти("РасчетСебестоимостиВРозницеКартинка" + Сч).Картинка = Элементы.Красный.Картинка Тогда
				ЭтаФорма["ТекстОшибкаРасчетСебестоимостиВРознице" + Сч] = НСтр("ru='Требуется выполнить расчет себестоимости в рознице (суммовой учет).';uk='Потрібно виконати розрахунки собівартості в роздробі (сумовий облік).'");
			КонецЕсли;
		Иначе
			МесяцЗакрыт = Истина; // месяц закрыт, если есть ошибки
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ЭтаФорма["ТекстОшибкаНачислениеАмортизации" + Сч]) Тогда
			Если Элементы.Найти("НачислениеАмортизацииКартинка" + Сч).Картинка = Элементы.Зеленый.Картинка Тогда
				ЭтаФорма["ТекстОшибкаНачислениеАмортизации" + Сч] = НСтр("ru='Начисление амортизации выполнено успешно!';uk='Нарахування амортизації виконане успішно!'");
				МесяцЗакрыт = Истина; // месяц закрыт, если распределение выполнено успешно
			ИначеЕсли Элементы.Найти("НачислениеАмортизацииКартинка" + Сч).Картинка = Элементы.ЗеленыйНеТребуется.Картинка Тогда
				ЭтаФорма["ТекстОшибкаНачислениеАмортизации" + Сч] = НСтр("ru='Начисление амортизации не требуется.';uk='Нарахування амортизації не потрібно.'");
			ИначеЕсли Элементы.Найти("НачислениеАмортизацииКартинка" + Сч).Картинка = Элементы.Серый.Картинка Тогда
				ЭтаФорма["ТекстОшибкаНачислениеАмортизации" + Сч] = НСтр("ru='Начисление амортизации не производилось.';uk='Нарахування амортизації не проводилося.'");
			ИначеЕсли Элементы.Найти("НачислениеАмортизацииКартинка" + Сч).Картинка = Элементы.Красный.Картинка Тогда
				ЭтаФорма["ТекстОшибкаНачислениеАмортизации" + Сч] = НСтр("ru='Требуется выполнить начисление амортизации.';uk='Потрібно виконати нарахування амортизації.'");
			КонецЕсли;
		Иначе
			МесяцЗакрыт = Истина; // месяц закрыт, если есть ошибки
		КонецЕсли;
		...
		Если Элементы.Найти("РаспределениеЗатратКартинка" + Сч).Картинка = Элементы.ЗеленыйНеТребуется.Картинка
			...
			Элементы.Найти("ДекорацияВыполнятьЗакрытиеНеТребуется" + Сч).Заголовок = НСтр("ru='Закрытие месяца не требуется, т.к. нет данных для расчета.';uk='Закриття місяця не потрібно, тому що немає даних для розрахунків.'");
		Иначе
...
Процедура ВыполнитьЗакрытиеМесяца(Команда)
	...
	Если КонецМесяца(Дата(ТекГод, ТекМесяц, 1)) <= КонецДня(ДатаЗапретаРедактирования) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Нельзя закрыть месяц, т.к. он относится к запрещенному для редактирования периоду!';uk='Не можна закрити місяць, тому що він відноситься до забороненого для редагування періоду!'"));
		Возврат;
	КонецЕсли;
