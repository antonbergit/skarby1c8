	...
	ИначеЕсли ИмяСобытия = "Позиционирование в списке отчетов" И ТипЗнч(Параметр) = Тип("Структура") Тогда
		Если ТипЗнч(Параметр.Ссылка) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			...
			Если Элементы.ЖурналОтчетов.ТекущаяСтрока <> Параметр.Ссылка Тогда // позиционирование не произошло => мешает отбор
				ПоказатьПредупреждение(,НСтр("ru='Введенный отчет не отображен в журнале, так как не соответствует отбору.';uk='Введенный отчет не отображен в журнале, так как не соответствует отбору.'"));
			КонецЕсли;
		КонецЕсли;
...
Процедура ОткрытьФормуОтчетаНаКлиенте(Отчет)
	...
	Если РезультатОткрытияФормыНаСервере = "Недостаточно прав" Тогда
		...
		ПоказатьПредупреждение(,НСтр("ru='Недостаточно прав.';uk='Недостаточно прав.'"));
		ФормаОткрытаУспешно = Ложь;
				
	ИначеЕсли РезультатОткрытияФормыНаСервере = "Отчет не найден" Тогда
		
		ПоказатьПредупреждение(,НСтр("ru='Отчет не найден.';uk='Отчет не найден.'"));
		ФормаОткрытаУспешно = Ложь;
				
	ИначеЕсли РезультатОткрытияФормыНаСервере = "Не удалось открыть отчет" Тогда
				
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не удалось открыть отчет.';uk='Не удалось открыть отчет.'");
		Сообщение.Сообщить();
...
Процедура ОткрытьФормуПодробнееОбОтчете(Команда)
	...
	Если Элементы.ДеревоОтчетов.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ru='Выберите отчет.';uk='Выберите отчет.'"));
		Возврат;
	КонецЕсли;
	
	Если Элементы.ДеревоОтчетов.ТекущиеДанные.ЭтоГруппа = Истина Тогда
		ПоказатьПредупреждение(,НСтр("ru='Функция недоступна для группы отчетов.';uk='Функция недоступна для группы отчетов.'"));
		Возврат;
	КонецЕсли;
		...
		Если ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
			...
			Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
				Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 Вывод формы на предпросмотр перед печатью прерван.';uk='%1 Вывод формы на предпросмотр перед печатью прерван.'"), ОписаниеДохИно.ТекстСообщения);
					Сообщение.Сообщить();
				КонецЕсли;
				Возврат;
			КонецЕсли;
...
Процедура Выгрузить(Команда)
	...
	Если РегламентированнаяОтчетностьКлиент.ВебБраузер() = "MSIE" Тогда
		Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета = "РСВ-1 ПФР"
			...
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Для выгрузки (проверки выгрузки) в браузере ""Internet Explorer"" необходимо открыть форму регламентированного отчета ""%1""';uk='Для выгрузки (проверки выгрузки) в браузере ""Internet Explorer"" необходимо открыть форму регламентированного отчета ""%1""'"),
	...
	Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета = "4-ФСС" Тогда
		Описание4_ФСС = ОписаниеРегОтчета(Элементы.ЖурналОтчетов.ТекущаяСтрока);
		Если Описание4_ФСС.ВыбраннаяФорма = "ФормаОтчета2016Кв3" Тогда
			ПоказатьПредупреждение(, НСтр("ru='Выгрузка, проверка выгрузки, отправка в электронном виде формы в редакции приказа ФСС РФ от 04.07.2016 № 260 будут реализованы в будущих версиях программы.';uk='Выгрузка, проверка выгрузки, отправка в электронном виде формы в редакции приказа ФСС РФ от 04.07.2016 № 260 будут реализованы в будущих версиях программы.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
"Необходимо сначала осуществить выгрузку из формы отчета. Выгрузка прервана.'"), СтрокаДанные.ПредставлениеПериода);
		...
		Если СтрокаДанные.НаименованиеОтчета = "Статистика: Форма П-1 Приложение 3" Тогда 
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выгрузка отчета ""Статистика: форма П-1 (приложение 3) за %1"" производится из формы П-1 за соответствующий период.';uk='Выгрузка отчета ""Статистика: форма П-1 (приложение 3) за %1"" производится из формы П-1 за соответствующий период.'"), СтрокаДанные.ПредставлениеПериода);
			Сообщение.Сообщить();
			СтандартнаяОбработка = Ложь;
...
Процедура ВыгрузитьПослеПодключенияРасширенияРаботыСФайлами(Подключено, ДополнительныеПараметры) Экспорт
	...
	НСтр("ru='%1';uk='%1'"), ДополнительныеПараметры.ТекстВопроса), РежимДиалогаВопрос.ДаНет);
...
Процедура Сохранить(Команда)
	...
	Если Элементы.ЖурналВыгрузки.ТекущаяСтрока = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ru='Не выбран документ.';uk='Не выбран документ.'"));
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		ПоказатьПредупреждение(,НСтр("ru='В выбранном документе нет текстов для сохранения в файл.';uk='В выбранном документе нет текстов для сохранения в файл.'"));
...
Функция КалендарьЗаполнен()
	...
	Если График.Количество() = 0 Тогда		
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст =НСтр("ru='Не заполнен производственный календарь.';uk='Не заповнений виробничий календар.'");
		Сообщение.Сообщить();
		Возврат Ложь;	
	КонецЕсли;
	...
	Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета = "Доходы, выплаченные иностранным организациям" Тогда
		...
		Если ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2016Кв1"
			...
			Если НЕ ОписаниеДохИно.ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении Тогда
				Если ОписаниеДохИно.Свойство("ТекстСообщения") Тогда
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 Вывод формы на предпросмотр перед печатью с двухмерным штрихкодом PDF417 прерван.';uk='%1 Вывод формы на предпросмотр перед печатью с двухмерным штрихкодом PDF417 прерван.'"), ОписаниеДохИно.ТекстСообщения);
					Сообщение.Сообщить();
				КонецЕсли;
				Возврат;
			КонецЕсли;
...
Процедура ОтправкиФТСВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли КолонкаВыбора = "ОтправкиФТСПервичноеСообщение" Тогда
		...
		ПолучитьФайл(Адрес, НСтр("ru='Подпись';uk='Підпис'") + ИмяФайлаВыгрузки);
