...
Процедура ПодробнееОбОтчетеНажатие(Элемент)
	...
	Если НЕ ЗначениеЗаполнено(ТекДеревоОтчетов.ТекущиеДанные.Ссылка) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Для выбранного отчета вывод подробной информации не предусмотрен.';uk='Для выбранного отчета вывод подробной информации не предусмотрен.'"));
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли ДанныеОтчета.ПолноеИмя = "Документ.РегламентированныйОтчет" Тогда
		...
		ИначеЕсли ТипЗнч(РезультатОткрытияФормыНаСервере) = Тип("Структура") Тогда
			...
			Если ИсточникОтчета = "РегламентированныйОтчетРСВ1" Тогда
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Для формирования формы РСВ-1 используйте календарь налогов и отчетности!';uk='Для формирования формы РСВ-1 используйте календарь налогов и отчетности!'");
				Сообщение.Сообщить();
