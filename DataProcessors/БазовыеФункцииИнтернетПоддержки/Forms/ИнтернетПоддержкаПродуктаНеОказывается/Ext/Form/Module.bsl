...
Процедура НадписьСодержанияКонтентаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "TechSupport" Тогда
		...
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			...
			НСтр("ru='При подключении Интернет-поддержки отображается сообщение ""Интернет-поддержка продукта не оказывается"".';uk='При підключенні Інтернет-підтримки відображається повідомлення ""Інтернет-підтримка по продукту не надається"".'"),
			,
			,
