...
"Факс: %7';uk='Не виходить ввести інформацію про організацію при реєстрації програмного продукту."
""
"Назва організації: %1"
"Тип діяльності: %2"
"ІПН: %3"
...
"Ответственный сотрудник: %5';uk='Адреса: %1"
"Місце покупки: %2"
"Дата покупки: %3"
"Число робочих місць: %4"
...
Функция ЗаполнениеПолейКорректно()
	...
	Если ПустаяСтрока(Ответственный) Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ru='Поле ""Ответственный сотрудник"" не заполнено.';uk='Поле ""Відповідальний працівник"" не заповнено.'"),
			"Ответственный",
			Отказ);
	КонецЕсли;
...
Процедура СформироватьПредставлениеАдреса()
	...
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Дом, НСтр("ru='д.';uk='буд.'") + " ");
