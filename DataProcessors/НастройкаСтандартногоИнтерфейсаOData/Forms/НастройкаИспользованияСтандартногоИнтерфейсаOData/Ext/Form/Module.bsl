...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДоступа("АдминистрированиеДанных", Метаданные) Тогда
		ВызватьИсключение НСтр("ru='Недостаточно прав для настройки автоматического REST-сервиса';uk='Недостатньо прав для налаштування автоматичного REST-Сервісу'");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если СоздатьПользователяСтандартногоИнтерфейсаOData Тогда
		...
		Если Не ЗначениеЗаполнено(ИмяПользователя) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указано имя пользователя';uk='Не зазначене ім`я користувача'"), , "ИмяПользователя");
			Отказ = Истина;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Пароль) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указан пароль';uk='Не зазначений пароль'"), , "Пароль");
			Отказ = Истина;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ПодтверждениеПароля) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указано подтверждение пароля';uk='Не зазначене підтвердження пароля'"), , "ПодтверждениеПароля");
			Отказ = Истина;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Пароль) И ЗначениеЗаполнено(ПодтверждениеПароля) И Пароль <> ПодтверждениеПароля Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Подтверждение пароля не совпадает с паролем';uk='Підтвердження пароля не збігається з паролем'"), , "ПодтверждениеПароля");
			Отказ = Истина;
		КонецЕсли;
	...
	Если Строки.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбрано ни одного объекта, к которому разрешен доступ через автоматический REST-сервис';uk='Не обране жодного об`єкта, до якого дозволений доступ через автоматичний REST-Сервіс'"), , "ОбъектыМетаданных");
		Отказ = Истина;
	КонецЕсли;
