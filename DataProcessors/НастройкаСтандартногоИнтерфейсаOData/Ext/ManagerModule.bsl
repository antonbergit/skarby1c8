...
Процедура ПроверитьСоставРолиOData() Экспорт
	...
	Если ИзбыточныеПрава.Количество() > 0 Тогда
		ТекстОшибки = Символы.НПП + НСтр("ru='Следующие права избыточно включены в состав роли:';uk='Наступні права надлишково включені до складу ролі:'") + Символы.ПС + Символы.ВК + 
			ПредставлениеИзбыточныхИлиНедостающихПрав(ИзбыточныеПрава, 2);
		Ошибки.Добавить(ТекстОшибки);
	КонецЕсли;
	
	Если НедостающиеПрава.Количество() > 0 Тогда
		ТекстОшибки = Символы.НПП + НСтр("ru='Следующие права должны быть включены в состав роли:';uk='Наступні права повинні бути включені до складу ролі:'") + Символы.ПС + Символы.ВК + 
	...
	Если Ошибки.Количество() > 0 Тогда
		...
		ТекстИсключения = СтрШаблон(НСтр("ru='Обнаружены ошибки в составе прав роли %1:';uk='Виявлені помилки в складі прав ролі %1:'"),
			РольДляСтандартногоИнтерфейсаOData().Имя);
...
Процедура ПроверитьСоставРолиODataПриОбновлении() Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru='Обработчик не должен использоваться при выключенном разделении по областям данных';uk='Обробник не повинен використовуватися при виключеному поділі по областях даних'");
	КонецЕсли;
