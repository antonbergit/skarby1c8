...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ru='Резервное копирование поддерживается только в клиенте под управлением ОС Windows.';uk='Резервне копіювання підтримується тільки в клієнтові під управлінням ОС Windows.'");
...
Процедура ПутьККаталогуАрхивовНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогОткрытияФайла.Заголовок= НСтр("ru='Выберите каталог для сохранения резервных копий';uk='Виберіть каталог для збереження резервних копій'");
