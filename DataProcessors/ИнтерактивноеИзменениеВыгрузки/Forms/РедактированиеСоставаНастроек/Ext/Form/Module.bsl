...
Процедура ВариантыНастроекПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса   = НСтр("ru='Удалить настройку ""%1""?';uk='Видалити налаштування ""%1""?'");
...
Процедура СохранитьНастройку(Команда)
	...
	Если ПустаяСтрока(ПредставлениеТекущейНастройки) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не заполнено имя для текущей настройки.';uk='Не заповнене ім`я для поточного налаштування.'"), , "ПредставлениеТекущейНастройки");
		Возврат;
	КонецЕсли;
		
	Если ВариантыНастроек.НайтиПоЗначению(ПредставлениеТекущейНастройки)<>Неопределено Тогда
		ТекстЗаголовка = НСтр("ru='Подтверждение';uk='Підтвердження'");
		ТекстВопроса   = НСтр("ru='Перезаписать существующую настройку ""%1""?';uk='Перезаписати існуюче налаштування ""%1""?'");
