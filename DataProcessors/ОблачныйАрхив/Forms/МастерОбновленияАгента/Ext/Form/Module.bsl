...
#Область ДекорацияИнформацияОНовомАгентеРезервногоКопированияТекст
	...
	МассивПодстрок.Добавить(НСтр("ru='Версия дистрибутива:';uk='Версия дистрибутива:'"));
	...
	МассивПодстрок.Добавить(НСтр("ru='Размер дистрибутива:';uk='Розмір дистрибутива:'"));
	МассивПодстрок.Добавить(" ");
	МассивПодстрок.Добавить(
		Новый ФорматированнаяСтрока(
	...
	Если НЕ ПустаяСтрока(ЭтотОбъект.ТекстЧтоНовогоВВерсии) Тогда
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
					НСтр("ru='Новое в версии.';uk='Новое в версии.'"),
					,
	...
	Если НЕ ПустаяСтрока(ЭтотОбъект.ТекстПорядокОбновления) Тогда
		МассивПодстрок.Добавить(НСтр("ru='Перед установкой обновления необходимо ознакомиться с';uk='Перед установкой обновления необходимо ознакомиться с'"));
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
					НСтр("ru='особенностями перехода';uk='особенностями перехода'"),
					,
					ЦветаСтиля.ЦветГиперссылки,
					,
					"backup1C:BackupAgentUpdate_HowToUpdate")); // Идентификатор.
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(НСтр("ru='на эту версию Агента резервного копирования.';uk='на эту версию Агента резервного копирования.'"));
		МассивПодстрок.Добавить(Символы.ПС);
		МассивПодстрок.Добавить(Символы.ПС);
	КонецЕсли;

	Если ЭтотОбъект.АгентКопированияУстановлен Тогда
		МассивПодстрок.Добавить(НСтр("ru='Установлен Агент резервного копирования версии:';uk='Установлен Агент резервного копирования версии:'"));
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
					ИнтернетПоддержкаПользователейКлиентСервер.ПользовательскоеПредставлениеНомераВерсии(
		...
		МассивПодстрок.Добавить(Символы.ПС);
	Иначе
		МассивПодстрок.Добавить(НСтр("ru='Агент резервного копирования не установлен';uk='Агент резервного копирования не установлен'"));
		МассивПодстрок.Добавить(Символы.ПС);
		МассивПодстрок.Добавить(Символы.ПС);
	КонецЕсли;
...
"';uk='Для обновления Агента резервного копирования необходимо обладать правами администратора Windows."
""
"Во время обновления произошли ошибки."
"Информация об ошибках сохранена в журнал регистрации."
"'"),
					Новый ФорматированнаяСтрока(
						НСтр("ru='Журнал регистрации.';uk='Журнал регистрации.'"),
						,
						ЦветаСтиля.ЦветГиперссылки,
						,
	...
	ИначеЕсли ИмяТекущейСтраницы = "СтраницаОбновлениеАгентаРезервногоКопирования" Тогда
		Если НаправлениеПерехода = "Далее" Тогда
			...
			Элементы.ДекорацияДлительнаяОперацияТекст.Заголовок =
				НСтр("ru='Загрузка и обновление Агента резервного копирования.';uk='Загрузка и обновление Агента резервного копирования.'");
...
#Область Окончание
		...
		Если ЕстьОшибки = Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ru='Есть ошибки';uk=' Є помилки'"));
