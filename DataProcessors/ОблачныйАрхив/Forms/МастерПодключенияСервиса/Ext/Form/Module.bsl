...
#Область ПолучениеПараметров
	...
	Элементы.ДекорацияВерсияИРазмерАгентаТекст.Заголовок =
		Новый ФорматированнаяСтрока(
			НСтр("ru='Версия дистрибутива:';uk='Версия дистрибутива:'"),
			" ",
			Новый ФорматированнаяСтрока(
			...
			Символы.ПС,
			НСтр("ru='Размер дистрибутива:';uk='Розмір дистрибутива:'"),
			" ",
			Новый ФорматированнаяСтрока(
				Формат(ЭтотОбъект.РазмерДистрибутиваБайт, "ЧЦ=10; ЧДЦ=2; ЧС=6; ЧРД=,; ЧРГ=' '; ЧН=0,00; ЧГ=3,0") + " " + НСтр("ru='МБайт';uk='МБайт'"),
				Новый Шрифт(Элементы.ДекорацияВерсияИРазмерАгентаТекст.Шрифт,,,Истина))); // Жирный.
			...
			Если (ЭтотОбъект.АгентКопированияУстановлен) И (ЭтотОбъект.ТребуетсяОбновление = 0) Тогда
				Если (ЭтотОбъект.АгентКопированияАктивированНаТекущийЛогин = Истина) Тогда
					Элементы.ДекорацияУстановкаАгентаРезервногоКопированияЗавершенаУспешноТекст.Заголовок =
						Новый ФорматированнаяСтрока(
							СтрШаблон(
								НСтр("ru='Установлена версия Агента резервного копирования: %1';uk='Установлена версия Агента резервного копирования: %1'"),
							...
							Символы.ПС,
							Символы.ПС,
							НСтр("ru='Для настройки расписания автоматического резервного копирования необходимо запустить 1С:Предприятие на компьютере, на котором установлен Агент резервного копирования.';uk='Для настройки расписания автоматического резервного копирования необходимо запустить 1С:Предприятие на компьютере, на котором установлен Агент резервного копирования.'"),
							Символы.ПС,
							Символы.ПС,
							НСтр("ru='Просматривать список созданных резервных копий можно с любого компьютера.';uk='Просматривать список созданных резервных копий можно с любого компьютера.'"));
				...
				Иначе
					...
					Иначе
						Элементы.ДекорацияУстановкаАгентаРезервногоКопированияЗавершенаСОшибкамиТекст.Заголовок =
							Новый ФорматированнаяСтрока(
							НСтр("ru='Недостаточно свободных лицензий для активации Агента резервного копирования.';uk='Недостаточно свободных лицензий для активации Агента резервного копирования.'"),
							Символы.ПС,
							Символы.ПС,
							НСтр("ru='Купить дополнительные лицензии на использование Агента резервного копирования на этом компьютере можно в';uk='Купить дополнительные лицензии на использование Агента резервного копирования на этом компьютере можно в'"),
							" ",
							Новый ФорматированнаяСтрока(
								НСтр("ru='личном кабинете';uk='личном кабинете'"),
								,
...
"Для этого из';uk='Также в личном кабинете можно освободить лицензии с других компьютеров."
"Для этого из'"),
							" ",
							Новый ФорматированнаяСтрока(
								НСтр("ru='личного кабинета';uk='личного кабинета'"),
								,
								ЭтотОбъект.ЦветГиперссылки,
								,
								"backup1C:OpenWebPersonalAccount_DeactivateAgent"),
							" ",
							НСтр("ru='необходимо выбрать неиспользуемого Агента резервного копирования и нажать кнопку Освободить.';uk='необходимо выбрать неиспользуемого Агента резервного копирования и нажать кнопку Освободить.'"),
							Символы.ПС);
						ПереключитьсяНаСтраницуНаСервере("СтраницаУстановкаАгентаРезервногоКопированияЗавершенаСОшибками");
					КонецЕсли;
				КонецЕсли;
...
Процедура КаталогНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Если Не ВебКлиент Тогда
		...
		Если Элемент.Имя = "КаталогЛогов" Тогда
			ДиалогВыбора.Заголовок = НСтр("ru='Выбор каталога для сохранения лог-файлов';uk='Выбор каталога для сохранения лог-файлов'");
			ДиалогВыбора.Каталог = ЭтотОбъект.КаталогЛогов;
		ИначеЕсли Элемент.Имя = "КаталогСтатистики" Тогда
			ДиалогВыбора.Заголовок = НСтр("ru='Выбор каталога для сохранения файлов статистики';uk='Выбор каталога для сохранения файлов статистики'");
			ДиалогВыбора.Каталог = ЭтотОбъект.КаталогСтатистики;
		ИначеЕсли Элемент.Имя = "КаталогРабочий" Тогда
			ДиалогВыбора.Заголовок = НСтр("ru='Выбор каталога для сохранения рабочих и временных файлов';uk='Выбор каталога для сохранения рабочих и временных файлов'");
...
Процедура ЗагрузитьСтатистику(СрокЖизниСтатистики=300, СписокШагов = "")
	...
	МассивШагов = Новый Массив;
		...
		ШагСбораДанных = ОблачныйАрхивКлиентСервер.ПолучитьСтруктуруШагаСбораДанных();
			...
			ШагСбораДанных.ОписаниеШага          = НСтр("ru='Сбор информации об использовании облачного хранилища';uk='Сбор информации об использовании облачного хранилища'");
		...
		ШагСбораДанных = ОблачныйАрхивКлиентСервер.ПолучитьСтруктуруШагаСбораДанных();
			...
			ШагСбораДанных.ОписаниеШага          = НСтр("ru='Сбор информации об актуальных версиях Агентов резервного копирования';uk='Сбор информации об актуальных версиях Агентов резервного копирования'");
			ШагСбораДанных.СрокУстареванияСекунд = СрокЖизниСтатистики; // Обновлять только если данные были собраны > 5 минут назад.
		Если (ПустаяСтрока(СписокШагов))
				ИЛИ (НЕ ПустаяСтрока(СписокШагов) И СтрНайти(СписокШагов, ШагСбораДанных.КодШага)) Тогда
			МассивШагов.Добавить(ШагСбораДанных);
...
#Область СтраницаПодключениеКВебИТС
		...
		лкДалее.Заголовок = НСтр("ru='Подключиться';uk='Подключиться'");
		лкДалее.КнопкаПоУмолчанию = Истина;

		лкЗакрыть.Заголовок = НСтр("ru='Отмена';uk='Відмінити'");
...
#Область СтраницаУстановкаАгентаРезервногоКопирования
		...
		лкДалее.Заголовок = НСтр("ru='Установить';uk='Встановити'");
		лкДалее.КнопкаПоУмолчанию = Истина;
...
#Область СтраницаУстановкаАгентаРезервногоКопированияЗавершенаСОшибками
		...
		лкЗакрыть.Заголовок = НСтр("ru='Закрыть';uk='Закрити'");
...
#Область СтраницаУстановкаАгентаРезервногоКопированияЗавершенаУспешно
		...
		лкДалее.Заголовок = НСтр("ru='Настроить';uk='Налаштувати'");
		лкДалее.КнопкаПоУмолчанию = Истина;
...
"Проверка правильности логина и пароля.';uk='Подключение к сервисам интернет-поддержки."
"Проверка правильности логина и пароля.'");
	...
	ИначеЕсли ИмяТекущейСтраницы = "СтраницаУстановкаАгентаРезервногоКопирования" Тогда
		Если НаправлениеПерехода = "Далее" Тогда
			// Всегда переключаться на страницу длительных операций.
			Элементы.ДекорацияДлительнаяОперацияТекст.Заголовок =
				НСтр("ru='Проверка установки Агента резервного копирования.';uk='Проверка установки Агента резервного копирования.'");
			ПереключитьсяНаСтраницуНаКлиенте("СтраницаДлительнаяОперация");
...
"Проверьте правильность логина и пароля и повторите попытку.';uk='Произошла ошибка подключения к веб-сервисам."
...
Процедура ВыполнитьПереходНаКлиенте(ИмяТекущейСтраницы, НаправлениеПерехода)
	...
	ИначеЕсли ИмяТекущейСтраницы = "СтраницаПодключениеКВебИТС" Тогда
		Если НаправлениеПерехода = "Далее" Тогда
			// Всегда переключаться на страницу длительных операций.
			Элементы.ДекорацияДлительнаяОперацияТекст.Заголовок =
				НСтр("ru='Подключение к сервисам интернет-поддержки.';uk='Подключение к сервисам интернет-поддержки.'");
			ПереключитьсяНаСтраницуНаКлиенте("СтраницаДлительнаяОперация");
...
"Для этого из'"),
	...
	ИначеЕсли ИмяТекущейСтраницы = "СтраницаУстановкаАгентаРезервногоКопирования" Тогда
		Если НаправлениеПерехода = "Далее" Тогда
			// Всегда переключаться на страницу длительных операций.
			Элементы.ДекорацияДлительнаяОперацияТекст.Заголовок =
				НСтр("ru='Загрузка и установка Агента резервного копирования.';uk='Загрузка и установка Агента резервного копирования.'");
			ПереключитьсяНаСтраницуНаКлиенте("СтраницаДлительнаяОперация");
...
#Область ЗагрузкаФайлаИнсталляции
		...
		ОбновитьНадписиНаСервере(НСтр("ru='Загрузка файла установки агента резервного копирования.';uk='Загрузка файла установки агента резервного копирования.'"));
			...
			Если ФайлУстановки.Размер() = ЭтотОбъект.РазмерДистрибутиваБайт Тогда
				...
				Если СовпадаетКонтрольнаяСумма(ЭтотОбъект.ИмяФайлаУстановки, ЭтотОбъект.КонтрольнаяСумма) Тогда
					...
					ОписаниеРезультата = Новый ФорматированнаяСтрока(
						ОписаниеРезультата,
						Символы.ПС,
						НСтр("ru='Агент резервного копирования успешно загружен';uk='Агент резервного копирования успешно загружен'"));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		Если НеобходимаЗагрузка = Истина Тогда
				...
				Если ФайлУстановки.Существует() Тогда
					...
					Если ФайлУстановки.Размер() = ЭтотОбъект.РазмерДистрибутиваБайт Тогда
						...
						Если НЕ СовпадаетКонтрольнаяСумма(ЭтотОбъект.ИмяФайлаУстановки, ЭтотОбъект.КонтрольнаяСумма) Тогда
							...
							ОписаниеРезультата = Новый ФорматированнаяСтрока(
								ОписаниеРезультата,
								Символы.ПС,
								НСтр("ru='Файл дистрибутива повреждён (контрольная сумма загруженного дистрибутива отличается от указанной на сайте)';uk='Файл дистрибутива повреждён (контрольная сумма загруженного дистрибутива отличается от указанной на сайте)'"));
						Иначе
							ОписаниеРезультата = Новый ФорматированнаяСтрока(
								ОписаниеРезультата,
								Символы.ПС,
					...
					Иначе
						...
						ОписаниеРезультата = Новый ФорматированнаяСтрока(
							ОписаниеРезультата,
							Символы.ПС,
							НСтр("ru='Файл дистрибутива повреждён (размер загруженного дистрибутива отличается от указанного на сайте)';uk='Файл дистрибутива повреждён (размер загруженного дистрибутива отличается от указанного на сайте)'"));
					КонецЕсли;
				Иначе
					ЕстьОшибки = Истина;
					ОписаниеРезультата = Новый ФорматированнаяСтрока(
						ОписаниеРезультата,
						Символы.ПС,
						НСтр("ru='Файл дистрибутива отсутствует во временном каталоге.';uk='Файл дистрибутива отсутствует во временном каталоге.'"));
				КонецЕсли;
			Иначе // Не удалось загрузить...
				...
				ОписаниеРезультата = Новый ФорматированнаяСтрока(
					ОписаниеРезультата,
					Символы.ПС,
					НСтр("ru='Файл дистрибутива не загружен. Произошли непредвиденные ошибки.';uk='Файл дистрибутива не загружен. Произошли непредвиденные ошибки.'"));
			КонецЕсли;
		КонецЕсли;
...
#Область ЗапускИнсталляции
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ru='Запуск установки агента резервного копирования.';uk='Запуск установки агента резервного копирования.'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ru='Агент резервного копирования успешно установлен';uk='Агент резервного копирования успешно установлен'"));
...
#Область СборИнформации_ИнформацияОКлиенте
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ru='Сбор данных для агента резервного копирования (анализ состояния компьютера).';uk='Сбор данных для агента резервного копирования (анализ состояния компьютера).'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ru='Все операции с агентом на компьютере завершены.';uk='Все операции с агентом на компьютере завершены.'"));
...
#Область СозданиеФайловНастроек
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ru='Создание настроек по-умолчанию для агента резервного копирования.';uk='Создание настроек по-умолчанию для агента резервного копирования.'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ru='Успешно сохранены настройки для агента резервного копирования';uk='Успешно сохранены настройки для агента резервного копирования'"));
...
#Область АктивацияАгентаКопирования
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ru='Активация агента резервного копирования (получение ключей, первый запуск).';uk='Активация агента резервного копирования (получение ключей, первый запуск).'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ru='Активация Агента резервного копирования: успешно получены коды запуска, завершён первый запуск';uk='Активация Агента резервного копирования: успешно получены коды запуска, завершён первый запуск'"));
			...
			Если (НЕ ПустаяСтрока(РезультатАктивацииАгента.ЛогинАктивации))
				...
				Попытка
					СтрокаКоманды =
						...
						+ "BackupAgent.exe"
						+ """"
						+ " activate_account -login=" + РезультатАктивацииАгента.ЛогинАктивации + " -password=" + РезультатАктивацииАгента.ПарольАктивации;
				...
				Исключение
					...
					ОписаниеРезультата = Новый ФорматированнаяСтрока(
						ОписаниеРезультата,
						Символы.ПС,
						НСтр("ru='Не удалось активировать аккаунт.';uk='Не удалось активировать аккаунт.'"));
				КонецПопытки;
				...
				Если КодВозврата = Неопределено Тогда
					ОписаниеРезультата = Новый ФорматированнаяСтрока(
						ОписаниеРезультата,
						Символы.ПС,
						НСтр("ru='Не удалось запустить Агент резервного копирования для активации аккаунта.';uk='Не удалось запустить Агент резервного копирования для активации аккаунта.'"));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
#Область СборИнформации_ИнформацияОКлиенте_АктивацииАгентовКопирования
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ru='Сбор данных для агента резервного копирования (анализ состояния компьютера, состояние активации).';uk='Сбор данных для агента резервного копирования (анализ состояния компьютера, состояние активации).'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ru='Проверка состояния активации завершена.';uk='Проверка состояния активации завершена.'"));
...
#Область Окончание
		...
		ОбновитьНадписиНаСервере(НСтр("ru='Всё готово';uk='Всё готово'"));
