...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru='Загрузка файла резервной копии';uk='Загрузка файла резервной копии'"));
...
"DBName: %3"
"UIDName: %4"
"Размер файла: %5';uk='Попытка загрузки файла с нулевым размером. Загрузка отменена."
"Логин: %1"
"UIDFile: %2"
"DBName: %3"
"UIDName: %4"
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName)';uk='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName)'"));
...
"%1"
"по причине:"
"%2';uk='Ошибка создания структуры рабочего каталога:"
"%1"
"по причине:"
"%2'"),
				лкКаталогВременныхФайлов,
...
"%1"
"Описание результата:"
"%2';uk='Завершено создание каталога"
"%1"
"Описание результата:"
"%2'"),
			лкКаталогВременныхФайлов,
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName_UIDFile
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile)';uk='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile)'"));
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName_UIDFile_BackUp
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\BackUp)';uk='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\BackUp)'"));
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName_UIDFile_Downloads
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\Downloads)';uk='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\Downloads)'"));
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName_UIDFile_Scripts
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\Scripts)';uk='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\Scripts)'"));
...
#Область ПолучениеКлючаАвторизации
	Если ПустаяСтрока(ЭтотОбъект.authKey) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ru='Получение ключа авторизации (authKey) из приватных веб-сервисов';uk='Получение ключа авторизации (authKey) из приватных веб-сервисов'"));
...
"Код результата: %1"
"Описание результата:"
"%2';uk='Завершено получение ключа авторизации."
"Код результата: %1"
"Описание результата:"
"%2'"),
				ВложенныйКонтекстВыполненияПолученияКлючаАвторизации.КодРезультата,
...
"DBName: %3"
"UIDName: %4"
"Размер файла: %5';uk='Завершена инициализация (ПриСозданииНаСервере):"
"Логин: %1"
"UIDFile: %2"
"DBName: %3"
"UIDName: %4"
...
"Вначале отмените загрузку.';uk='Нельзя закрывать форму, пока происходит загрузка файла."
...
Процедура ПриОткрытии(Отказ)
	...
	#Если НЕ ВебКлиент Тогда
		...
		ЭтотОбъект.Заголовок = СтрШаблон(
			НСтр("ru='Загрузка резервной копии базы %1';uk='Загрузка резервной копии базы %1'"),
			ЭтотОбъект.DBName);

		// Текущее состояние загрузки.
		ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ru='Проверка загруженного файла...';uk='Проверка загруженного файла...'");
		...
		Иначе
			...
			// Текущее состояние загрузки.
			ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ru='Загрузка файла...';uk='Завантаження файлу...'");
			ЭтотОбъект.ОбновитьОтображениеДанных();
			Элементы.ДекорацияОшибкаТекст.Подсказка = "";
			Элементы.ГруппаСтраницыСостояниеЗагрузки.ТекущаяСтраница = Элементы.ГруппаСостояние;
...
Процедура ПутьДругогоКаталогаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Если Не ВебКлиент Тогда
		...
		ДиалогВыбора.Заголовок = НСтр("ru='Выбор каталога восстановления резервной копии';uk='Выбор каталога восстановления резервной копии'");
...
Процедура КомандаВосстановить(Команда)
	...
	Если НЕ ПустаяСтрока(Результат) Тогда
		...
		Если Каталог.Существует() Тогда
			...
			Иначе
				ТекстСообщения = НСтр("ru='Необходимо указать имя каталога, а не имя файла.';uk='Необходимо указать имя каталога, а не имя файла.'");
		...
		Иначе
			ТекстСообщения = СтрШаблон(
				НСтр("ru='Каталога %1 не существует. проверьте правильность написания имени каталога.';uk='Каталога %1 не существует. проверьте правильность написания имени каталога.'"),
				Результат);
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = ТекстСообщения;
			Сообщение.Поле  = "ПутьДругогоКаталога";
		...
		КонецЕсли;
	Иначе
		ТекстСообщения = НСтр("ru='Необходимо выбрать каталог восстановления резервной копии.';uk='Необходимо выбрать каталог восстановления резервной копии.'");
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = ТекстСообщения;
		Сообщение.Поле  = "ПутьДругогоКаталога";
		Сообщение.УстановитьДанные(Объект);
...
Процедура КомандаОтменаЗагрузки(Команда)
	...
	Если ЭтотОбъект.ИдентификаторСостоянияЗагрузкиФайла = "ЗагрузкаФайла" Тогда // Идентификатор: ЗагрузкаФайла, ОтменаЗагрузки, ФайлЗагружен, ОшибкиЗагрузки, РаспаковкаФайла, ФайлРаспакован, ОшибкиРаспаковки.
		...
		ПоказатьВопрос(
			...
			НСтр("ru='Отменить загрузку?';uk='Отменить загрузку?'"), // ТекстВопроса
			РежимДиалогаВопрос.ДаНет, // Кнопки
			10, // Таймаут
			КодВозвратаДиалога.Нет, // КнопкаПоУмолчанию
			НСтр("ru='Подтверждение';uk='Підтвердження'"), // Заголовок
...
Процедура КомандаДалее(Команда)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаВыборКаталога Тогда
		...
		Если ПустаяСтрока(ЭтотОбъект.ПутьДругогоКаталога) Тогда
			ЕстьОшибки = Истина;
			ТекстСообщения = НСтр("ru='Необходимо выбрать каталог для восстановления';uk='Необходимо выбрать каталог для восстановления'");
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = ТекстСообщения;
			Сообщение.Поле  = "ПутьДругогоКаталога";
			Сообщение.УстановитьДанные(Объект);
		...
		Иначе
			...
			Иначе
				Элементы.ДекорацияВыборКаталогаОшибкаТекст.Заголовок =
					СтрШаблон(
						НСтр("ru='Каталог %1 не существует.';uk='Каталог %1 не существует.'"),
...
#Область СтраницаЗагрузкаФайла
		...
		лкДалее.Заголовок       = НСтр("ru='Загрузить';uk='Завантажити'");
		...
		ИначеЕсли Форма.ИдентификаторСостоянияЗагрузкиФайла = "ФайлЗагружен" Тогда
			...
			лкДалее.Заголовок   = НСтр("ru='Распаковать';uk='Распаковать'");
		...
		ИначеЕсли Форма.ИдентификаторСостоянияЗагрузкиФайла = "ОшибкиЗагрузки" Тогда
			лкДалее.Заголовок   = НСтр("ru='Загрузить заново';uk='Загрузить заново'");
			лкДалее.Доступность    = Истина;
			лкЗакрыть.Доступность  = Истина;
...
#Область СтраницаВыборКаталога
		...
		Форма.Заголовок = НСтр("ru='Выберите каталог для разархивации';uk='Выберите каталог для разархивации'");

		лкОтмена.Видимость    = Ложь;
		лкНазад.Видимость     = Ложь;
		лкДалее.Видимость     = Истина;
		лкДалее.Доступность   = Истина;
		лкДалее.Заголовок     = НСтр("ru='Разархивировать';uk='Разархивировать'");
		лкЗакрыть.Видимость   = Истина;
		лкЗакрыть.Доступность = Истина;
...
#Область СтраницаОтключениеСеансов
		...
		Форма.Заголовок = НСтр("ru='Отключение активных сеансов';uk='Отключение активных сеансов'");
		...
		Если Форма.КоличествоАктивныхСеансов = 0 Тогда
			лкДалее.Заголовок = НСтр("ru='Разархивировать';uk='Разархивировать'");
		Иначе
			лкДалее.Заголовок = НСтр("ru='Обновить';uk='Оновити'");
		КонецЕсли;
...
#Область СтраницаУспешноеЗавершение
		...
		Форма.Заголовок = НСтр("ru='Данные успешно восстановлены из резервной копии';uk='Данные успешно восстановлены из резервной копии'");
		...
		лкЗакрыть.Заголовок = НСтр("ru='Готово';uk='Готово'");
	...
	#Если НЕ ВебКлиент Тогда
		...
		Попытка
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ru='Соединение с сервером для загрузки файла резервной копии';uk='Соединение с сервером для загрузки файла резервной копии'"));
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
				ШагВыполнения,
				0,
				НСтр("ru='Успешное подключение к серверу для загрузки файла резервной копии';uk='Успешное подключение к серверу для загрузки файла резервной копии'"),
				Неопределено);
...
"%1';uk='Ошибка подключения к серверу:"
"%1'"),
					ПодробноеПредставлениеОшибки),
				Неопределено);
			ЭтотОбъект.КонтекстВыполнения.ЛогШаговВыполнения.Добавить(ШагВыполнения);
		...
		Иначе
			...
			// Текущее состояние загрузки.
			ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ru='Ошибка подключения к http-сервисам.';uk='Ошибка подключения к http-сервисам.'");
			ЭтотОбъект.ОбновитьОтображениеДанных();
			Элементы.ДекорацияОшибкаТекст.Подсказка =
				СтрШаблон(
					НСтр("ru='Ошибка подключения к http-сервисам:"
"%1';uk='Ошибка подключения к http-сервисам:"
"%1'"),
					ПодробноеПредставлениеОшибки);
			Элементы.ГруппаСтраницыСостояниеЗагрузки.ТекущаяСтраница = Элементы.ГруппаОшибка;
		КонецЕсли;
	...
	#Если НЕ ВебКлиент Тогда
		...
		Если ЭтотОбъект.ИдентификаторСостоянияЗагрузкиФайла = "ЗагрузкаФайла" Тогда // Идентификатор: ЗагрузкаФайла, ОтменаЗагрузки, ФайлЗагружен, ОшибкиЗагрузки, РаспаковкаФайла, ФайлРаспакован, ОшибкиРаспаковки.
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ru='Загрузка части файла резервной копии';uk='Загрузка части файла резервной копии'"));
			КодРезультата = 0;
			ОписаниеРезультата = "";
...
"%1"
"';uk='Произошла ошибка загрузки части файла:"
"%1"
"'"),
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				КонецПопытки;
...
"%1"
"%2"
"';uk='Неправильный код возврата (в HTTP ответе):"
"%1"
"%2"
"'"),
...
"';uk='Процесс загрузки был слишком длительный, ключ авторизации просрочился и будет получен заново."
"'");
					Иначе
						ОписаниеРезультата = ОписаниеРезультата
							+ СтрШаблон(
								НСтр("ru='Код возврата (errorCode) вернул ошибку: %1"
"';uk='Код возврата (errorCode) вернул ошибку: %1"
"'"),
								КодВозврата);
					КонецЕсли;
...
"';uk='Успешно загружена часть файла."
"Имя временного файла: %1"
"Размер файла (байт): %2"
"Контрольная сумма (md5): %3"
"'"),
...
"';uk='Не совпадает ожидаемая (%1) и реальная (%2) контрольная сумма (md5) части файла."
"'"),
										ТекущаяСтрока.КонтрольнаяСумма,
...
"';uk='Не совпадает ожидаемый (%1) и реальный (%2) размер части файла."
"'"),
									ТекущаяСтрока.Размер,
...
"';uk='Не найден временный файл (%1). Возможно, он не был загружен."
"'"),
								ТекущаяСтрока.ИмяВременногоФайла);
					КонецЕсли;
				КонецЕсли;
			...
			Если ЭтотОбъект.КоличествоСбойныхЧастей = 0 Тогда
				ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = СтрШаблон(
					НСтр("ru='Загружено %1 частей из %2 (%3 из %4).';uk='Загружено %1 частей из %2 (%3 из %4).'"),
...
"Во время загрузки были сбои: %5';uk='Загружено %1 частей из %2 (%3 из %4)."
"Во время загрузки были сбои: %5'"),
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ru='Завершение загрузки файла резервной копии';uk='Завершение загрузки файла резервной копии'"));
			КодРезультата = 0;
			ОписаниеРезультата = "";
			...
			Если МассивНеЗагруженных.Количество() = 0 Тогда
				...
				ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ru='Объединение частей файла в один архив...';uk='Объединение частей файла в один архив...'");
				ЭтотОбъект.ОбновитьОтображениеДанных();
				...
				ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
					ШагВыполнения,
					0,
					НСтр("ru='Загрузка файла резервной копии успешно завершена';uk='Загрузка файла резервной копии успешно завершена'"),
					Неопределено);
				ЭтотОбъект.КонтекстВыполнения.ЛогШаговВыполнения.Добавить(ШагВыполнения);
			...
			Иначе // Есть незагруженные части файлов
				...
				Если ЕстьЧтоЗагружать = Ложь Тогда
					...
					ЭтотОбъект.ИдентификаторСостоянияЗагрузкиФайла = "ОшибкиЗагрузки"; // Идентификатор: ЗагрузкаФайла, ОтменаЗагрузки, ФайлЗагружен, ОшибкиЗагрузки, РаспаковкаФайла, ФайлРаспакован, ОшибкиРаспаковки.
					// Текущее состояние загрузки.
					ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ru='Остались незагруженные части файла.';uk='Остались незагруженные части файла.'");
					Элементы.ДекорацияОшибкаТекст.Подсказка =
						НСтр("ru='Остались незагруженные части файла,"
"по которым не осталось попыток загрузки."
"Загрузка завершена с ошибками.';uk='Остались незагруженные части файла,"
"по которым не осталось попыток загрузки."
"Загрузка завершена с ошибками.'");
					...
					ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
						ШагВыполнения,
						1002,
						НСтр("ru='Загрузка файла резервной копии не завершена из-за ошибок';uk='Загрузка файла резервной копии не завершена из-за ошибок'"),
						Неопределено);
					ЭтотОбъект.КонтекстВыполнения.ЛогШаговВыполнения.Добавить(ШагВыполнения);
	...
	#Если НЕ ВебКлиент Тогда
		...
		Элементы.ДекорацияУспешноеЗавершениеТекст.Заголовок =
			Новый ФорматированнаяСтрока(
				НСтр("ru='Информационная база из резервной копии успешно восстановлена в каталог';uk='Информационная база из резервной копии успешно восстановлена в каталог'"),
				Символы.ПС,
				Новый ФорматированнаяСтрока(
					ЭтотОбъект.ПутьДругогоКаталога,
					,
		...
		// Текущее состояние загрузки.
		ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ru='Очистка каталога перед распаковкой файла...';uk='Очистка каталога перед распаковкой файла...'");
		Элементы.ДекорацияВыборКаталогаОшибкаТекст.Заголовок = "";
		Элементы.ГруппаВыборКаталогаПодвал.ТекущаяСтраница = Элементы.ГруппаВыборКаталогаПодвалПредупреждения;
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ru='Очистка каталога-получателя перед разархивацией загруженного файла резервной копии';uk='Очистка каталога-получателя перед разархивацией загруженного файла резервной копии'"));
		...
		Если ЕстьОшибкиУдаления = Истина Тогда
			...
			// Текущее состояние загрузки.
			ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ru='Ошибка разархивации файла.';uk='Ошибка разархивации файла.'");
			ЭтотОбъект.ОбновитьОтображениеДанных();
...
"%1';uk='Информация об ошибке:"
"%1'"),
					ПодробноеПредставлениеОшибки);
			Элементы.ДекорацияВыборКаталогаОшибкаТекст.Заголовок =
					НСтр("ru='Ошибка очистки каталога."
"Возможно, запущена информационная база, связанная с этим каталогом.';uk='Ошибка очистки каталога."
...
"Ошибка:"
"%4"
"';uk='Ошибки очистки каталога-приемника перед распаковкой резервной копии."
"Путь к архиватору: %1"
"Путь к загруженному файлу: %2"
"Путь для очистки: %3"
"Ошибка:"
...
"Путь к загруженному файлу: %2"
"Путь для очистки: %3"
"';uk='Успешная очистка каталога-приемника."
"Путь к архиватору: %1"
"Путь к загруженному файлу: %2"
"Путь для очистки: %3"
"'"),
			...
			// Текущее состояние загрузки.
			ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ru='Распаковка файла...';uk='Распаковка файла...'");
			...
			Попытка
				...
				ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
					...
					НСтр("ru='Разархивация загруженного файла резервной копии';uk='Разархивация загруженного файла резервной копии'"));
				...
				Иначе
					ЕстьОшибки = Истина;
					ПодробноеПредставлениеОшибки = НСтр("ru='Код возврата программы разархивации отличается от 0.';uk='Код возврата программы разархивации отличается от 0.'");
				КонецЕсли;
...
"';uk='Успешная разархивация файла резервной копии."
"Путь к архиватору: %1"
"Путь к загруженному файлу: %2"
"Размер загруженного файла: %3"
"Код возврата программы: %4"
...
"Ошибка:"
"%7"
"';uk='Ошибка разархивации файла резервной копии."
"Путь к архиватору: %1"
"Путь к загруженному файлу: %2"
"Размер загруженного файла: %3"
"Код возврата программы: %4"
...
"%1';uk='Ошибка разархивации файла:"
"%1'"),
						ПодробноеПредставлениеОшибки);
				УправлениеФормой(ЭтотОбъект);
			КонецЕсли;
...
Процедура УдалитьВременныеФайлы()
	...
	ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ru='Удаление временных файлов...';uk='Удаление временных файлов...'");
	ЭтотОбъект.ОбновитьОтображениеДанных();
	УдалитьФайлы(
		ЭтотОбъект.КаталогРабочий
			+ ПолучитьРазделительПутиКлиента()
...
"%4"
"';uk='Загрузка файла резервной копии, итоговый отчет."
"Время начала (мс): %1"
"Время окончания (мс): %2"
"Длительность (мс): %3"
"Результат выполнения:"
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		...
		НСтр("ru='Облачный архив. Веб-сервисы. Загрузка файла резервной копии';uk='Облачный архив. Веб-сервисы. Загрузка файла резервной копии'"), // ИмяСобытия
...
Процедура ПослеВопросаПодтвержденияОтменыЗагрузки(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ru='Отмена загрузки файла резервной копии';uk='Отмена загрузки файла резервной копии'"));
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
			ШагВыполнения,
			0,
			НСтр("ru='Отменена загрузка файла резервной копии';uk='Отменена загрузка файла резервной копии'"),
			Неопределено);
		ЭтотОбъект.КонтекстВыполнения.ЛогШаговВыполнения.Добавить(ШагВыполнения);
