...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		ЭтотОбъект.ПарольАдминистратораОС = " ";
		Элементы.ПарольАдминистратораОС.ПодсказкаВвода = НСтр("ru='Введите пароль';uk='Введите пароль'");
	КонецЕсли;
	...
	Элементы.ИмяИБ.Подсказка = СтрШаблон(НСтр("ru='Идентификатор ИБ: %1';uk='Идентификатор ИБ: %1'"), ЭтотОбъект.ИдентификаторИБ);
	...
	МассивПодстрок.Добавить(
		Новый ФорматированнаяСтрока(
			НСтр("ru='Журнал работы';uk='Журнал работы'"),,ЦветаСтиля.ЦветГиперссылки,,"backup1C:BackupAgentEventLog"));
	МассивПодстрок.Добавить(" ");
	МассивПодстрок.Добавить(НСтр("ru='Агента резервного копирования.';uk='Агента резервного копирования.'"));
	МассивПодстрок.Добавить(Символы.ПС);
	МассивПодстрок.Добавить(Символы.ПС);
	Если ПроверкаНеобходимостиОбновленияАгента.ТребуетсяУстановка = Истина Тогда
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("ru='На этом компьютере не установлен Агент резервного копирования.';uk='На этом компьютере не установлен Агент резервного копирования.'"),
				Новый Шрифт(Элементы.ДекорацияЖурналРаботыРезервногоКопированияИВерсияАгента.Шрифт,,,Истина),
				ЦветаСтиля.ЦветОсобогоТекста));
		МассивПодстрок.Добавить(Символы.ПС);
	Иначе
		МассивПодстрок.Добавить(НСтр("ru='Установленная версия Агента резервного копирования:';uk='Установленная версия Агента резервного копирования:'"));
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
		...
		МассивПодстрок.Добавить(Символы.ПС);
	КонецЕсли;
	МассивПодстрок.Добавить(НСтр("ru='Установленная версия подсистемы Облачный архив:';uk='Установленная версия подсистемы Облачный архив:'"));
	МассивПодстрок.Добавить(" ");
	МассивПодстрок.Добавить(
		Новый ФорматированнаяСтрока(
			ИнтернетПоддержкаПользователейКлиентСервер.ПользовательскоеПредставлениеНомераВерсии(
	...
	ИначеЕсли ПроверкаНеобходимостиОбновленияАгента.ТребуетсяОбновление = 1 Тогда // Доступно обновление.
		МассивПодстрок.Добавить(НСтр("ru='Доступна новая версия Агента резервного копирования:';uk='Доступна новая версия Агента резервного копирования:'"));
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
				ИнтернетПоддержкаПользователейКлиентСервер.ПользовательскоеПредставлениеНомераВерсии(
		...
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("ru='Обновить';uk='Оновити'"),,ЦветаСтиля.ЦветГиперссылки,,"backup1C:UpdateAgentOnThisPC"));
		МассивПодстрок.Добавить(".");
		МассивПодстрок.Добавить(Символы.ПС);
		МассивПодстрок.Добавить(Символы.ПС);
	ИначеЕсли ПроверкаНеобходимостиОбновленияАгента.ТребуетсяОбновление = 2 Тогда // Рекомендуется обновление.
		МассивПодстрок.Добавить(НСтр("ru='Рекомендуется обновить Агента резервного копирования на версию:';uk='Рекомендуется обновить Агента резервного копирования на версию:'"));
		МассивПодстрок.Добавить(" ");
	...
	ИначеЕсли ПроверкаНеобходимостиОбновленияАгента.ТребуетсяОбновление = 3 Тогда // Необходимо обновление.
		МассивПодстрок.Добавить(НСтр("ru='Необходимо обновить Агента резервного копирования на версию:';uk='Необходимо обновить Агента резервного копирования на версию:'"));
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(Новый ФорматированнаяСтрока(
			ИнтернетПоддержкаПользователейКлиентСервер.ПользовательскоеПредставлениеНомераВерсии(
				ПроверкаНеобходимостиОбновленияАгента.ВерсияНовейшегоАгентаКопирования,
			...
			Если (НЕ ПустаяСтрока(ЭтотОбъект.ЛогинАдминистратораОС)) Тогда
				Если (НЕ ПустаяСтрока(ЭтотОбъект.ПарольАдминистратораОС)) Тогда
					...
					СтрокаКоманды = 
						...
						+ "BackupAgent.exe"
						+ """"
						+ " set_local_user -login=" + ЭтотОбъект.ЛогинАдминистратораОС
						+ " -password=" + ЭтотОбъект.ПарольАдминистратораОС;
					ЗапуститьПриложение(СтрокаКоманды, ЭтотОбъект.КаталогУстановкиАгентаКопирования, Истина, КодВозврата);
					Если КодВозврата <> 0 Тогда
						ТекстСообщения = НСтр("ru='Неверно указан логин или пароль';uk='Неверно указан логин или пароль'");
				...
				Иначе
					...
					ТекстСообщения = НСтр("ru='Не заполнен пароль';uk='Не заполнен пароль'");
					Сообщение = Новый СообщениеПользователю();
					Сообщение.Текст = ТекстСообщения;
...
Процедура УправлениеФормой(Форма)
	...
	Если Форма.РасписаниеВключено = Истина Тогда
		МассивПодстрок = Новый Массив;
		МассивПодстрок.Добавить(НСтр("ru='Выполнять автоматическое резервное копирование';uk='Виконувати автоматичне резервне копіювання'"));
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
	...
	Иначе
		...
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("ru='по расписанию';uk='по расписанию'"), // Содержимое
				, // Шрифт
				Форма.ЦветГиперссылки, // ЦветТекста
				, // ЦветФона
				"backup1C:EditBackupScheduler")); // Ссылка
