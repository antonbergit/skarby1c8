...
#Область СборДанных
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Сбор данных о настройках';uk='Сбор данных о настройках'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
#Область ПроверкаВозможностиСозданияРезервнойКопии_НастройкиКлиентскогоКомпьютера
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Информация о клиентском компьютере';uk='Информация о клиентском компьютере'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
"%3"
"%4"
"%5';uk='На этом компьютере (идентификатор: %1) невозможно работать с Агентом резервного копирования:"
"%2"
"%3"
"%4"
"%5'"),
					...
					?(ИнформацияОКлиенте.АгентКопированияУстановлен = Истина,
						" - " + НСтр("ru='Агент резервного копирования установлен';uk='Агент резервного копирования установлен'"),
						" - " + НСтр("ru='Агент резервного копирования НЕ установлен';uk='Агент резервного копирования НЕ установлен'")),
					?(ИнформацияОКлиенте.АгентКопированияАктивирован_ЕстьФайлыСертификатов = Истина,
						" - " + НСтр("ru='Файлы сертификатов загружены';uk='Файлы сертификатов загружены'"),
						" - " + НСтр("ru='Файлы сертификатов НЕ загружены (агент не активирован)';uk='Файлы сертификатов НЕ загружены (агент не активирован)'")),
					?(АктивацииАгентовКопирования.АгентКопированияАктивирован_ДанныеВебСервисов = Истина,
						" - " + НСтр("ru='Агент резервного копирования активирован на текущий логин';uk='Агент резервного копирования активирован на текущий логин'"),
						" - " + НСтр("ru='Агент резервного копирования НЕ активирован на текущий логин';uk='Агент резервного копирования НЕ активирован на текущий логин'")),
					?(АктивацииАгентовКопирования.АгентКопированияАктивированНаДругойЛогин = Истина,
						" - " + НСтр("ru='Агент резервного копирования активирован на другой логин';uk='Агент резервного копирования активирован на другой логин'"),
						" - " + НСтр("ru='Агент резервного копирования НЕ активирован на другой логин';uk='Агент резервного копирования НЕ активирован на другой логин'")));
		КонецЕсли;
...
#Область ПроверкаВозможностиСозданияРезервнойКопии_НастройкиКлиентскогоКомпьютера
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Проверка свободного места в облачном хранилище';uk='Проверка свободного места в облачном хранилище'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
"Доступно %4 байт (%5 Мбайт, %6 Гбайт)';uk='Состояние облачного хранилища:"
"Куплено: %1 байт (%2 Мбайт, %3 Гбайт)"
"Доступно %4 байт (%5 Мбайт, %6 Гбайт)'"),
		...
		Если БайтДоступно < 10*1024*1024 Тогда
			...
			ОписаниеРезультата = ОписаниеРезультата + Символы.ПС + Символы.ПС
				+ НСтр("ru='В облачном хранилище недостаточно места для создания резервной копии (менее 10 МБайт).';uk='В облачном хранилище недостаточно места для создания резервной копии (менее 10 МБайт).'");
		КонецЕсли;

		Элементы.ДекорацияСтатистика.Заголовок = СтрШаблон(
			НСтр("ru='Всего доступно %1 из %2 Мбайт (%3)';uk='Всего доступно %1 из %2 Мбайт (%3)'"),
...
#Область ПолучениеОбщихНастроек
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Чтение общих настроек';uk='Чтение общих настроек'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
#Область ЧтениеНастроекАгентаРезервногоКопирования
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Чтение настроек агента резервного копирования';uk='Чтение настроек агента резервного копирования'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
"Рабочий каталог: %1"
"Идентификатор ИБ: %2"
"Имя файла настроек: %3';uk='Чтение настроек Агента резервного копирования:"
"Рабочий каталог: %1"
"Идентификатор ИБ: %2"
...
#Область ЧтениеСпискаРезервныхКопий
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Анализ списка резервных копий';uk='Анализ списка резервных копий'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
		...
		Иначе
			ТекстЗаголовка =
				СтрШаблон(
					НСтр("ru='В последний раз резервное копирование проводилось: %1';uk='Востаннє резервне копіювання проводилося: %1'"),
...
"Идентификатор ИБ: %1"
"Дата самой последней резервной копии: %2';uk='Анализ списка резервных копий:"
"Идентификатор ИБ: %1"
...
"%4"
"';uk='Создание резервной копии, инициализация формы, завершение."
"Время начала (мс): %1"
"Время окончания (мс): %2"
"Длительность (мс): %3"
"Результат выполнения:"
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Сервис и регламент';uk='БИП:ОблачныйАрхив.Сервис и регламент'"), // Имя события
		НСтр("ru='Облачный архив. Сервис и регламент. Резервное копирование. Инициализация';uk='Облачный архив. Сервис и регламент. Резервное копирование. Инициализация'"), // ИмяСобытия
...
"работающих в программе.';uk='Производится отключение сеансов других пользователей,"
"работающих в программе.'"),
			Символы.ПС,
			Символы.ПС,
			Новый ФорматированнаяСтрока(
				СтрШаблон(
					НСтр("ru='Активные сеансы: %1';uk='Активные сеансы: %1'"),
...
"которые не могут быть завершены принудительно.';uk='Имеются активные сеансы работы с программой,"
...
#Область СтраницаПредупреждениеОДлительнойОперации
		...
		лкДалее.Заголовок = НСтр("ru='Создать резервную копию';uk='Створити резервну копію'");
...
Процедура НачатьРезервноеКопирование()
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Запуск резервного копирования, установка блокировки';uk='Запуск резервного копирования, установка блокировки'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Запуск резервного копирования, изменение настроек для Агента резервного копирования';uk='Запуск резервного копирования, изменение настроек для Агента резервного копирования'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Запуск резервного копирования, подготовка к выходу из программы';uk='Запуск резервного копирования, подготовка к выходу из программы'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
"Результат выполнения:"
"%4"
"';uk='Запуск резервного копирования."
"Время начала (мс): %1"
"Время окончания (мс): %2"
"Длительность (мс): %3"
"Результат выполнения:"
...
"'"),
	...
	ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Сервис и регламент';uk='БИП:ОблачныйАрхив.Сервис и регламент'"), // Имя события
		НСтр("ru='Облачный архив. Сервис и регламент. Резервное копирование. Завершение';uk='Облачный архив. Сервис и регламент. Резервное копирование. Завершение'"), // ИмяСобытия
...
Процедура УстановитьБлокировкуСеансовНа2Минуты()
	...
	Блокировка.Сообщение = СоединенияИБ.СформироватьСообщениеБлокировки(
		НСтр("ru='Выполняется резервное копирование. Вход в информационную базу заблокирован на 2 минуты.';uk='Виконується резервне копіювання. Вхід в інформаційну базу заблокований на 2 хвилини.'"),
		КодРазрешения);
...
#Область ЧтениеНастроек
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Запуск резервного копирования, изменение настроек для Агента резервного копирования, чтение настроек';uk='Запуск резервного копирования, изменение настроек для Агента резервного копирования, чтение настроек'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
"%1';uk='Созданы настройки по-умолчанию:"
"%1'"),
...
#Область ИзменениеНастроек
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Запуск резервного копирования, изменение настроек для Агента резервного копирования, добавление настроек для немедленного копирования';uk='Запуск резервного копирования, изменение настроек для Агента резервного копирования, добавление настроек для немедленного копирования'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
#Область ЗаписьНастроек
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Запуск резервного копирования, изменение настроек для Агента резервного копирования, запись файла настроек';uk='Запуск резервного копирования, изменение настроек для Агента резервного копирования, запись файла настроек'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
#Область РеинициализацияАгентаРезервногоКопирования
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Запуск резервного копирования, изменение настроек для Агента резервного копирования, реинициализация агента резервного копирования';uk='Запуск резервного копирования, изменение настроек для Агента резервного копирования, реинициализация агента резервного копирования'"));
	КодРезультата = 0;
	ОписаниеРезультата = "";
...
"Код возврата: %2';uk='Команда реинициализации: %1"
"Код возврата: %2'"),
		СтрокаКоманды,
		КодРезультата);
...
Процедура ЗагрузитьСтатистику(СрокЖизниСтатистики = 300)
	...
	МассивШагов = Новый Массив;
		...
		ШагСбораДанных = ОблачныйАрхивКлиентСервер.ПолучитьСтруктуруШагаСбораДанных();
			...
			ШагСбораДанных.ОписаниеШага          = НСтр("ru='Получение списка резервных копий';uk='Получение списка резервных копий'");
			ШагСбораДанных.СрокУстареванияСекунд = СрокЖизниСтатистики; // Обновлять только если данные были собраны > 5 минут назад.
		МассивШагов.Добавить(ШагСбораДанных);
