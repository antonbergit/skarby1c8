...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоПродолжениеНастройкиВПодчиненномУзлеРИБ Тогда
		...
		Элементы.ГруппаПереключателяВыбораРежимаРаботыПомощника.Заголовок = НСтр("ru='Продолжение настройки синхронизации данных с главным узлом';uk='Продовження налаштування синхронізації даних з головним вузлом'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если ОбщегоНазначенияКлиент.ПредлагатьСоздаватьРезервныеКопии() Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Перед настройкой синхронизации рекомендуется сделать <a href = %1 >резервную копию данных</a>.';uk='Перед налаштуванням синхронізації рекомендується зробити <a href = %1 >резервну копію даних</a>.'"),
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы Тогда
		Если ДлительнаяОперация Тогда
			ТекстПредупреждения = НСтр("ru='Настройка синхронизации данных будет прервана.';uk='Налаштування синхронізації даних буде перервано.'");
		КонецЕсли;
		
		Если ЭтоПродолжениеНастройкиВПодчиненномУзлеРИБ Тогда
			ТекстПредупреждения = НСтр("ru='Настройка подчиненного узла распределенной информационной базы не выполнена.';uk='Налаштування підпорядкованого вузла розподіленої інформаційної бази не виконане.'");
		КонецЕсли;
...
"Работа помощника не может быть завершена.';uk='Виконується налаштування синхронізації даних."
...
"Отказаться от настройки и использовать значения по умолчанию?';uk='Виконується налаштування підлеглого вузла розподіленої інформаційної бази."
"Відмовитися від налаштування й використати значення по умовчанню?'");
	...
	ТекстПредупреждения = НСтр("ru='Отменить настройку синхронизации и выйти из помощника?';uk='Скасувати налаштування синхронізації й вийти з помічника?'");
...
Процедура ИмяФайлаНастроекОбменаДаннымиДляЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НастройкиДиалога.Вставить("Фильтр", НСтр("ru='Файл настроек синхронизации данных (*.xml)';uk='Файл налаштувань синхронізації даних (*.xml)'") + "|*.xml" );
...
Процедура СохранитьФайлНастроекОбменаДанными(Команда)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='При сохранении файла настроек синхронизации данных возникли ошибки.';uk='При збереженні файлу налаштувань синхронізації даних виникли помилки.'"));
...
Процедура ПослеПроверкиРасширенияРаботыСФайлами(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Диалог.Заголовок      = НСтр("ru='Укажите имя файла настроек синхронизации данных';uk='Вкажіть ім`я файлу налаштувань синхронізації даних'");
		...
		Иначе
			...
			Если ЭтоОшибкаРегистрацииКомпоненты Тогда
				...
				ТекстРегистрацияКомпоненты = Символы.ПС
					+ НСтр("ru='Возможно, необходимо зарегистрировать COM-компоненту comcntr.dll';uk='Можливо, необхідно зареєструвати COM-компоненту comcntr.dll'")
					+ ?(ЭтоФайловаяИБ, ".", НСтр("ru=', после чего выполнить перезапуск сервера 1С:Предприятие.';uk=', після чого виконати перезапуск сервера 1С:Підприємство.'"))
			КонецЕсли;
...
Процедура УстановитьВидимостьНаСервере()
	...
	Если Объект.ЭтоНастройкаРаспределеннойИнформационнойБазы Тогда
		...
		Элементы.ГруппаПереключателяВыбораРежимаРаботыПомощника.Заголовок = НСтр("ru='Создание начального образа подчиненного узла РИБ';uk='Створення початкового образа підлеглого вузла РІБ'");
...
Функция ПолучитьОписаниеНастроекТранспортаОбмена()
	...
	Если ПустаяСтрока(Результат) Тогда
		...
		Результат = НСтр("ru='Настройки подключения не заданы.';uk='Налаштування підключення не задані.'");
...
Процедура ПроверитьПодключение(ВидТранспорта)
	...
	Если Не Отказ Тогда
		...
		Если ВидТранспорта = ПредопределенноеЗначение("Перечисление.ВидыТранспортаСообщенийОбмена.FILE") Тогда
			ТекстПредупреждения = НСтр("ru='Запись данных в указанный каталог разрешена.';uk='Запис даних у зазначений каталог дозволений.'");
		Иначе
...
Процедура ПроверитьКорректностьПараметровПодключенияWS(Отказ)
	...
	Если ПустаяСтрока(Объект.WSURLВебСервиса) Тогда
		...
		НСтрока = НСтр("ru='Укажите адрес приложения в Интернете.';uk='Вкажіть адресу додатка в Інтернеті.'");
	...
	ИначеЕсли ПустаяСтрока(Объект.WSИмяПользователя) Тогда
		...
		НСтрока = НСтр("ru='Укажите имя пользователя.';uk='Вкажіть ім`я користувача.'");
	...
	ИначеЕсли ПустаяСтрока(Объект.WSПароль) Тогда
		...
		НСтрока = НСтр("ru='Укажите пароль пользователя.';uk='Вкажіть пароль користувача.'");
...
Процедура ПроверитьЗаполнениеРеквизитовВФорме(Отказ, ИмяФормыДляПроверки, ПараметрыФормы, ИмяРеквизитаФормы)
	...
	Если Не ФормаНастройки.ПроверитьЗаполнение() Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Необходимо задать обязательные настройки.';uk='Необхідно задати обов`язкові налаштування.'"),,, ИмяРеквизитаФормы, Отказ);
...
Процедура ЗавершениеПервогоЭтапаНастройкиОбменаЧерезОбычныеКаналыСвязи(Отказ)
	...
	Если Не Объект.ЭтоНастройкаРаспределеннойИнформационнойБазы
		...
		НСтрока = НСтр("ru='Сохраните файл с настройками для другой программы';uk='Збережіть файл із налаштуваннями для іншої програми'");
	...
	Иначе
		...
		Если ВыполнитьОбменДаннымиСейчас Тогда
			...
			ПараметрыЗадания.НаименованиеЗадания  = НСтр("ru='Выгрузка данных';uk='Вивантаження даних'");
...
Процедура ЗавершениеВыгрузкиДанных()
	...
	ИначеЕсли РезультатВыполненияФоновогоЗадания.Статус = "Отменено" Тогда
		СообщениеОбОшибке = НСтр("ru='Действие отменено пользователем.';uk='Дія відмінена користувачем.'");
	Иначе
		
		РезультатВыполнения = ПолучитьРезультатВыполненияФоновогоЗаданияНаСервере();
		
		Если РезультатВыполнения.Отказ Тогда
			СообщениеОбОшибке = НСтр("ru='Во время отправки данных возникли ошибки(см. журнал регистрации).';uk=' Під час відправлення даних виникли помилки(див. журнал реєстрації).'");
		КонецЕсли;
...
Процедура ЗавершениеВторогоЭтапаНастройкиОбменаЧерезОбычныеКаналыСвязи(Отказ, Знач ОткрыватьПослеЗакрытияТекущей)
	...
	Состояние(НСтр("ru='Выполняется создание настройки синхронизации данных';uk='Виконується створення налаштування синхронізації даних'"));
	
	НастроитьНовыйОбменДаннымиНаСервере(Отказ, НастройкаОтборовНаУзле, ЗначенияПоУмолчаниюНаУзле);
	
	Если Отказ Тогда
		
		ПоказатьПредупреждение(, НСтр("ru='При создании настройки синхронизации данных возникли ошибки.';uk='При створенні налаштування синхронізації даних виникли помилки.'"));
		Возврат;
	КонецЕсли;
...
Функция НадписьДалееFTP()
	...
	Возврат НСтр("ru='Нажмите кнопку ""Далее"" для настройки подключения через FTP-ресурс.';uk='Натисніть кнопку ""Далі"" для налаштування підключення через FTP-ресурс.'");
...
Функция НадписьДалееEMAIL()
	...
	Возврат НСтр("ru='Нажмите кнопку ""Далее"" для настройки подключения по почте.';uk='Натисніть кнопку ""Далі"" для налаштування підключення поштою.'");
...
Функция НадписьДалееНастройки()
	...
	Возврат НСтр("ru='Нажмите кнопку ""Далее"" для настройки дополнительных параметров синхронизации данных.';uk='Натисніть кнопку ""Далі"" для налаштування додаткових параметрів синхронізації даних.'");
...
Функция Подключаемый_СтраницаМастераНастройкаПараметров_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.НаименованиеЭтойБазы) Тогда
		...
		НСтрока = НСтр("ru='Укажите наименование этой программы.';uk='Вкажіть найменування цієї програми.'");
	...
	Если ПустаяСтрока(Объект.НаименованиеВторойБазы) Тогда
		...
		НСтрока = НСтр("ru='Укажите наименование другой программы.';uk='Вкажіть найменування іншої програми.'");
	...
	Если ПустаяСтрока(Объект.ПрефиксИнформационнойБазыПриемника) Тогда
		...
		НСтрока = НСтр("ru='Укажите существующий или желаемый префикс второй информационной базы.';uk='Вкажіть існуючий або бажаний префікс другої інформаційної бази.'");
...
Функция Подключаемый_СтраницаМастераНастройкаПараметровДляВнешнегоСоединенияПервойБазы_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.НаименованиеЭтойБазы) Тогда
		...
		НСтрока = НСтр("ru='Укажите наименование информационной базы.';uk='Вкажіть найменування інформаційної бази.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.НаименованиеЭтойБазы",, Отказ);
		Возврат Неопределено;
	КонецЕсли;
...
Функция Подключаемый_СтраницаМастераНачало_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.ИмяФайлаНастроекСинхронизацииДанныхДляЗагрузки) Тогда
		ТекстОшибки = НСтр("ru='Выберите файл с настройками синхронизации данных';uk='Виберіть файл із налаштуваннями синхронізації даних'");
	...
	ТекстПредупреждения = НСтр("ru='Для передачи файла настроек синхронизации данных необходимо установить расширение для веб-клиента 1С:Предприятие.';uk='Для передачі файлу налаштувань синхронізації даних необхідно встановити розширення для веб-клієнта 1С:Підприємство.'");
...
Процедура ЗавершениеВыбораФайлаНастроекОбменаДаннымиДляЗагрузки(Знач РезультатПомещенияФайлов, Знач ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ТекстОшибки) И ПустаяСтрока(АдресПомещенногоФайла) Тогда
		ТекстОшибки = НСтр("ru='Ошибка передачи файла настроек синхронизации данных на сервер';uk='Помилка передачі файлу налаштувань синхронізації даних на сервер'");
	КонецЕсли;
	...
	Если ПустаяСтрока(ТекстОшибки) Тогда
		...
		Если ОшибкаЗагрузкиПараметровМастера Тогда
			ТекстОшибки = НСтр("ru='Указан неправильный файл настроек синхронизации данных. Укажите корректный файл.';uk='Зазначено неправильний файл налаштувань синхронізації даних. Укажіть коректний файл.'");
		Иначе
...
"Налаштування для цієї інформаційної бази:"
"========================================================"
"%2%3Префікс інформаційної бази: %4"
""
"Налаштування для додатку, розташованого в сервісі:"
"========================================================"
...
"Для решения проблем воспользуйтесь журналом регистрации.';uk='Виникли помилки на етапі створення налаштувань синхронізації даних."
...
Функция Подключаемый_СтраницаМастераОжиданиеСинхронизацииСправочниковЗагрузка_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ru='Загрузка справочников из сообщения обмена';uk='Завантаження довідників із повідомлення обміну'");
...
"Для решения проблем воспользуйтесь журналом регистрации.';uk='Виникли помилки на етапі збереження налаштувань."
"Для вирішення проблем скористайтеся журналом реєстрації.'"));
...
Процедура ПроверитьЗаданиеНастроекДляПервойБазы(Отказ, ТипСоединения = "ВебСервис")
	...
	Если ПустаяСтрока(Объект.НаименованиеВторойБазы) Тогда
		...
		НСтрока = НСтр("ru='Укажите наименование приложения в Интернете.';uk='Вкажіть найменування додатка в Інтернеті.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.НаименованиеВторойБазы",, Отказ);
...
Процедура ПроверитьЗаданиеНастроекДляВторойБазы(Отказ, ТипСоединения)
	...
	Если ПустаяСтрока(Объект.НаименованиеВторойБазы) Тогда
		...
		НСтрока = НСтр("ru='Укажите наименование программы.';uk='Вкажіть найменування програми.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.НаименованиеВторойБазы",, Отказ);
...
Процедура СтраницаМастераОжиданиеПроверкиПодключенияВнешнегоСоединения_ОбработкаДлительнойОперацииНаСервере(Отказ)
	...
	Если Объект.COMВариантРаботыИнформационнойБазы = 0 Тогда
		...
		Если ПустаяСтрока(Объект.COMКаталогИнформационнойБазы) Тогда
			...
			НСтрока = НСтр("ru='Укажите каталог информационной базы.';uk='Вкажіть каталог інформаційної бази.'");
	...
	Иначе
		...
		Если ПустаяСтрока(Объект.COMИмяСервера1СПредприятия) Тогда
			...
			НСтрока = НСтр("ru='Укажите имя кластера серверов.';uk='Вкажіть ім`я кластера серверів.'");
		...
		ИначеЕсли ПустаяСтрока(Объект.COMИмяИнформационнойБазыНаСервере1СПредприятия) Тогда
			...
			НСтрока = НСтр("ru='Укажите имя информационной базы.';uk='Вкажіть ім`я інформаційної бази.'");
	...
	Если НРег(СтрокаСоединенияИнформационнойБазы()) = НРег(ВнешнееСоединение.СтрокаСоединенияИнформационнойБазы()) Тогда
		...
		Сообщение = НСтр("ru='Заданы настройки подключения к этой информационной базе.';uk='Задано налаштування підключення до цієї інформаційної бази.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение,,,, Отказ);
		Возврат;
