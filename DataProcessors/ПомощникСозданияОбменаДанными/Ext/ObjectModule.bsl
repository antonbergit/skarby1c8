...
Процедура ВыполнитьДействияПоНастройкеНовогоОбменаДаннымиЧерезВебСервисВДвухБазах(Отказ,
	...
	Попытка
		...
		Если Отказ Тогда
			ВызватьИсключение НСтр("ru='При создании настройки обмена во второй информационной базе возникли ошибки.';uk='При створенні налаштування обміну в другій інформаційній базі виникли помилки.'");
		КонецЕсли;
		...
		Если Отказ Тогда
			ВызватьИсключение(НСтр("ru='Возникла ошибка при создании настройки синхронизации данных.';uk='Виникла помилка при створенні налаштування синхронізації даних.'"));
		КонецЕсли;
...
Процедура ВыполнитьДействияПоНастройкеНовогоОбменаДаннымиЧерезВнешнееСоединение(Отказ,
	...
	Попытка
		...
		Если Отказ Тогда
			Сообщение = НСтр("ru='При создании настройки обмена во второй информационной базе возникли ошибки: %1';uk='При створенні налаштування обміну в другій інформаційній базі виникли помилки: %1'");
...
Процедура ОбновитьНастройкиОбменаДанными(Отказ,
	...
	Попытка
		...
		Если Отказ Тогда
			ВызватьИсключение(НСтр("ru='Возникла ошибка при обновлении настройки синхронизации данных.';uk='Виникла помилка при оновленні налаштування синхронізації даних.'"));
		КонецЕсли;
...
Процедура ВнешнееСоединениеВыполнитьДействияПоНастройкеНовогоОбменаДанными(Отказ, 
	...
	Попытка
		...
		Если Отказ Тогда
			ВызватьИсключение(НСтр("ru='При создании настройки синхронизации данных возникли ошибки.';uk='При створенні налаштування синхронізації даних виникли помилки.'"));
		КонецЕсли;
...
"Можливо, інформаційна база не є підлеглим вузлом у РІБ.'");
		КонецЕсли;
...
Процедура ВыполнитьЗагрузкуПараметровМастера(Отказ, СтрокаXML) Экспорт
	...
	Если СтруктураНастроек.Свойство("ИмяПланаОбмена")
		...
		ПолеСтрокаСообщенияОбОшибке = НСтр("ru='Файл содержит настройки обмена для другой информационной базы.';uk='Файл містить налаштування обміну для іншої інформаційної бази.'");
...
"Настройку обмена требуется начать заново.';uk='На першому етапі налаштування обміну був неправильно зазначений префікс другої інформаційної бази."
"Налаштування обміну потрібно почати заново.'");
		//
		ОбменДаннымиСервер.СообщитьОбОшибке(СтрокаСообщенияОбОшибке(), Отказ);
