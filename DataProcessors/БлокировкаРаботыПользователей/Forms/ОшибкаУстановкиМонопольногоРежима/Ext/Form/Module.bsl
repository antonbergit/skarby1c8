...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если УдалениеПомеченныхОбъектов Тогда
		Заголовок = НСтр("ru='Не удалось выполнить удаление помеченных объектов';uk='Не вдалося виконати видалення позначених об`єктів'");
		Элементы.ТекстСообщенияОбОшибке.Заголовок = НСтр("ru='Невозможно выполнить удаление помеченных объектов, т.к. в программе работают другие пользователи:';uk='Неможливо виконати видалення позначених об`єктів, тому що в програмі працюють інші користувачі:'");
	КонецЕсли;
...
Процедура ТаймАутЗавершенияРаботыПользователей()
	...
	Если ПродолжительностьЗавершенияРаботыПользователей >= 3 Тогда
		...
		Элементы.ТекстСообщенияОбОшибке.Заголовок = НСтр("ru='Невозможно выполнить обновление версии программы, т.к. не удалось завершить работу пользователей:';uk='Неможливо виконати відновлення версії програми, тому що не вдалося завершити роботу користувачів:'");
...
Процедура УстановитьБлокировкуФайловойБазы()
	...
	Если УдалениеПомеченныхОбъектов Тогда
		...
		Объект.СообщениеДляПользователей = НСтр("ru='Программа заблокирована для удаления помеченных объектов.';uk='Програма заблокована для видалення позначених об`єктів.'");
	Иначе
		Объект.НачалоДействияБлокировки = ТекущаяДатаСеанса() + 2*60;
		Объект.ОкончаниеДействияБлокировки = Объект.НачалоДействияБлокировки + 5*60;
		Объект.СообщениеДляПользователей = НСтр("ru='Программа заблокирована для выполнения обновления.';uk='Програма заблокована для виконання відновлення.'");
	КонецЕсли;
