...
Процедура ЗаполнитьГруппыКатегории()
	...
	ИначеЕсли КоличествоГрупп = 0 Тогда
	    ОтборКатегорииГруппыПереключатель = Перечисления.ВидыОтборовНоменклатуры.КатегорииНоменклатуры;
		Элементы.ОтборКатегорииГруппыПереключатель.Видимость = Ложь;
		Элементы.ГруппаВыбратьНоменклатуру.Заголовок = НСтр("ru='Выберите категории номенклатуры для переноса на сайт';uk='Виберіть категорії номенклатури для перенесення на сайт'");
	...
	ИначеЕсли КоличествоКатегорий = 0 Тогда
		...
		Элементы.ОтборКатегорииГруппыПереключатель.Видимость = Ложь;
		Элементы.ГруппаВыбратьНоменклатуру.Заголовок = НСтр("ru='Выберите группы номенклатуры для переноса на сайт';uk='Виберіть групи номенклатури для перенесення на сайт'");
...
Процедура ЗаполнитьДеревоГрупп()
	...
	СтрокаВсеГруппы.Представление= НСтр("ru='<Все группы>';uk='<Всі групи>'");
	СтрокаВсеГруппы.Флаг = Истина;
	
	СтрокаВсеГруппы = Дерево.Строки.Добавить();
	СтрокаВсеГруппы.Значение	 = Справочники.Номенклатура.ПустаяСсылка();
	СтрокаВсеГруппы.Представление= НСтр("ru='<Нет группы>';uk='<Без групи>'");
	СтрокаВсеГруппы.Флаг = Истина;
...
Процедура ЗаполнитьДеревоКатегорий()
	...
	СтрокаВсеГруппы.Представление= НСтр("ru='<Все категории>';uk='<Всі категорії>'");
	СтрокаВсеГруппы.Флаг = Истина;
...
Процедура ПометкаИерархииПриИзменении()
	...
	Если ТекущиеДанные.Представление = НСтр("ru='<Все группы>';uk='<Всі групи>'") Тогда
		//Все элементы дерева
...
Функция СоздатьСайтВФоне()
	...
	Заголовок = НСтр("ru='Создание сайта';uk='Створення сайту'");
...
Функция СоздатьСайтВФонеПродолжить(ПараметрыСозданияСайта)
	...
	Заголовок = НСтр("ru='Заполнение сайта';uk='Заповнення сайту'");
...
Функция ПолучитьТаблицуКаталогов()
	...
	Если НЕ ВсеГруппы Тогда
		...
		Для Каждого ЭлементСЗ Из СписокГрупп Цикл
			...
			Если ЭлементСЗ.Представление = НСтр("ru='<Все группы>';uk='<Всі групи>'") Тогда
...
Процедура СоздатьСайт(Команда)
	...
	Если ТипСайта=0 Тогда
		Сообщить(НСтр("ru='Выберите тип сайта!';uk='Виберіть тип сайту!'"));
		Возврат;
	КонецЕсли;
	
	Если НЕ ПринимаюСоглашение Тогда
		Сообщить(НСтр("ru='Чтобы создать сайт, нужно принять лицензионное соглашение!';uk='Для того, щоб створити сайт, потрібно прийняти ліцензійну угоду!'"));
		Возврат;
	КонецЕсли;
	
	Если СокрЛП(EmailРегистрацииСайта)="" Тогда
		Сообщить(НСтр("ru='Укажите e-mail, на который будет зарегистрирован сайт!';uk='Вкажіть e-mail, на який буде зареєстрований сайт!'"));
		Возврат;
	КонецЕсли;
	...
	Если НЕ ИмяСайтаСвободно Тогда
		Сообщить(НСтр("ru='Предложенное имя сайта занято, выберите другое!';uk='Запропоноване ім`я сайту зайняте, виберіть інше!'"));
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьСозданныйСайт(ДанныеСозданияСайтаСтруктура)
	...
	ПоказатьОповещениеПользователя(,,НСтр("ru='Создан сайт: ';uk='Створено сайт:'")+ДанныеСозданияСайтаСтруктура.АдресСайта,БиблиотекаКартинок.Информация32);
	ОбновитьИнтерфейс();
	Закрыть();
...
Функция ПолучитьДанныеЗапросом(HTTPМетод, ТелоЗапроса, АдресЗапроса, ЗаголовкиЗапроса, 
	...
	Исключение
		...
		Если СтрНайти(ТекстОшибки, "Couldn't resolve host name")<>0 Тогда
		    ТекстОшибки = НСтр("ru='Ошибка: Не могу подключиться к интернету!';uk='Помилка: Не можу підключитися до інтернету!'");
		КонецЕсли;
		Возврат "";
	КонецПопытки;
...
Функция ПроверитьИмяСайтаТекст(Текст)
	...
	Пока н <= СтрДлина(Текст) Цикл
		...
		Если РазрешенныеСимволыИмениСайта.Получить(НРег(ПроверСимвол)) = Неопределено Тогда
			НадписьИмяСайтаДоступно = НСтр("ru='Разрешены латинские буквы и цифры';uk='Дозволені латинські букви і цифри'");
			Возврат Ложь;
		Иначе
			н=н+1;
		КонецЕсли;
...
Функция ОбновитьНадписьИмяСайтаДоступно(ИмяСайтаПроверено, ИмяСайтаСвободно)
	...
	Если НЕ ИмяСайтаПроверено Тогда
		Возврат НСтр("ru='Проверяется ...';uk='Перевіряється ...'");
	ИначеЕсли ИмяСайтаСвободно Тогда
		Возврат НСтр("ru='Имя свободно!';uk='Ім`я вільне!'");
	Иначе
		Возврат НСтр("ru='Имя занято!';uk='Ім`я зайняте!'");
	КонецЕсли;
