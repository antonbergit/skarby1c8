...
Процедура СоздатьУзелОбменаССайтом(ВхПараметры, АдресСайта, IDсайта, Пароль="") Экспорт
	...
	Если Пароль<>"" Тогда
		ПланОбменаСайт.Пароль					= Пароль;
	КонецЕсли;
	ПланОбменаСайт.Код							= IDсайта;
	ПланОбменаСайт.Наименование					= НСтр("ru='Обмен товарами и заказами с ';uk='Обмін товарами й замовленнями з '") + АдресСайта;
...
Процедура ЗапуститьОбменССайтом(КодУзлаОбмена)
	...
	ОбменССайтом.ВыполнитьОбмен(УзелОбмена, НСтр("ru='Обмен полный при создании сайта';uk='Обмін повний при створенні сайту'"), Ложь);
