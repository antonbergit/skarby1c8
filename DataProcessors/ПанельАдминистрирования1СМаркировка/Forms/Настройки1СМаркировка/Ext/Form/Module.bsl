...
Процедура ПроверитьВерсиюНаСервере()
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось получить версию сервера ГИСМ, возможно, требуется обновление конфигурации.';uk='Не удалось получить версию сервера ГИСМ, возможно, требуется обновление конфигурации.'"));
	КонецЕсли;
...
Процедура ПолучитьТекстЗаголовкаИРасписанияРегламентнойНастройки(Задание, ТекстРасписания, РасписаниеАктивно) Экспорт
	...
	Если Задание = Неопределено Тогда
		...
		ТекстРасписания = НСтр("ru='<Расписание не задано>';uk='<Расписание не задано>'");
		
	Иначе
		
		Если Задание.Использование Тогда
			РасписаниеАктивно = Истина;
			ТекстРасписания = СтрШаблон("Расписание: %1", Строка(Задание.Расписание));
		Иначе
			ТекстРасписания = СтрШаблон("Расписание (НЕ АКТИВНО): %1", Строка(Задание.Расписание));
		КонецЕсли;
...
Процедура ОбновитьВерсии()
	...
	Если Не ВерсияАктуальна Тогда
		ТекстСообщенияОВерсии = СтрШаблон(НСтр("ru='Текущая версия конфигурации не поддерживает актуальный формат обмена с ГИСМ (%1)';uk='Текущая версия конфигурации не поддерживает актуальный формат обмена с ГИСМ (%1)'"), ВерсииСхемОбмена.Сервер);
	Иначе
		ТекстСообщенияОВерсии = СтрШаблон(НСтр("ru='Обмен с ГИСМ выполняется с использованием актуального формата обмена (%1)';uk='Обмен с ГИСМ выполняется с использованием актуального формата обмена (%1)'"), ВерсииСхемОбмена.Сервер);
