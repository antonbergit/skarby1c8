...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.АдресВХранилище) Тогда
		...
		Если ЗначениеЗаполнено(СтруктураДанных.СтруктураДействий) Тогда
			...
			Иначе
				ВызватьИсключение НСтр("ru='Режим печати не установлен';uk='Режим друку не встановлений'");;
			КонецЕсли;
	...
	Если ТекущиеДанные.Выбран Тогда
		...
		Если Объект.Режим = "ПечатьЦенниковИЭтикеток" Тогда
			...
			Если (ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение.Текст = НСтр("ru='Не заполнено количество ценников';uk='Не заповнена кількість цінників'");
			...
			ИначеЕсли (НЕ ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Не выбран шаблон ценника';uk='Не обраний шаблон цінника'");
			...
			ИначеЕсли (НЕ ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Не заполнено количество этикеток';uk='Не заповнена кількість етикеток'");
			...
			ИначеЕсли (НЕ ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Не выбран шаблон этикетки';uk='Не обраний шаблон етикетки'");
			...
			ИначеЕсли (   ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Не заполнено количество ценников и(или) этикеток';uk='Не заповнена кількість цінників і(або) етикеток'");
				Сообщение.Поле = "Объект.Товары["+ТекущаяСтрока+"].КоличествоЦенников";
				Сообщение.Сообщить();
			...
			ИначеЕсли (НЕ ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Не выбраны шаблоны ценников и(или) этикеток';uk='Не обрані шаблони цінників і(або) етикеток'");
				Сообщение.Поле = "Объект.Товары["+ТекущаяСтрока+"].ШаблонЦенника";
				Сообщение.Сообщить();
			...
			ИначеЕсли (   ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Не заполнено количество ценников и(или) шаблон этикетки';uk='Не заповнена кількість цінників і(або) шаблон етикетки'");
				Сообщение.Поле = "Объект.Товары["+ТекущаяСтрока+"].КоличествоЦенников";
				Сообщение.Сообщить();
			...
			ИначеЕсли (НЕ ЗначениеЗаполнено(ТекущиеДанные.ШаблонЦенника)
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Не заполнено количество этикеток и(или) шаблон ценника';uk='Не заповнена кількість етикеток і(або) шаблон цінника'");
				Сообщение.Поле = "Объект.Товары["+ТекущаяСтрока+"].КоличествоЭтикеток";
				Сообщение.Сообщить();
...
Процедура ЗаполнитьТаблицуЗапасов(Команда)
	...
	ТекстВопроса = НСтр("ru='При перезаполнении все введенные вручную данные будут потеряны, продолжить?';uk='При перезаповненні всі введені вручну дані будуть загублені, продовжити?'");
...
Процедура Печать(Команда)
	...
	Если НЕ НеобходимостьПечати.ЕстьВыбранныеСтроки Тогда
		...
		ПоказатьПредупреждение(Неопределено,НСтр("ru='Не выбрано ни одного товара';uk='Не обране жодного товару'"));
		Возврат;
	...
	Если ЭтаФорма.ПроверитьЗаполнение() Тогда
		...
		ПараметрыПечати.Вставить("ЗаголовокФормы", НСтр("ru='Печать этикеток и ценников';uk='Друк етикеток і цінників'"));
...
Процедура УстановитьКоличествоЦенников(Команда)
	...
	ПоказатьВводЧисла(Новый ОписаниеОповещения("УстановитьКоличествоЦенниковЗавершение", ЭтотОбъект, Новый Структура("ВыбранноеЗначение", ВыбранноеЗначение)), ВыбранноеЗначение, НСтр("ru='Введите количество ценников';uk='Введіть кількість цінників'"), 10, 2);
...
Процедура УстановитьКоличествоЦенниковЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если (Результат <> Неопределено) Тогда
		...
		Текст = НСтр("ru='Установлено количество ценников %Количество%.';uk='Встановлена кількість цінників %Количество%.'");
...
Процедура УстановитьКоличествоЭтикетокЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если (Результат <> Неопределено) Тогда
		...
		Текст = НСтр("ru='Установлено количество этикеток %Количество%.';uk='Встановлена кількість етикеток %Количество%.'");
		Текст = СтрЗаменить(Текст, "%Количество%", ВыбранноеЗначение);
...
Процедура УстановитьШаблонЦенниковЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если (Результат <> Неопределено) Тогда
		...
		Текст = НСтр("ru='Установлен шаблон ""%Шаблон%"".';uk='Встановлений шаблон ""%Шаблон%"".'");
...
Функция ПоказатьОповещениеПользователюОВозможнойОшибке(Текст, Количество, КоличествоВсего)
	...
	Если Количество < КоличествоВсего Тогда
		...
		Текст = Текст
		        + ?(Текст <> "", Символы.ПС, "")
		        + НСтр("ru='Отметка выбора установлена для %КоличествоВыбранных% строк из %КоличествоВсего%.';uk='Відмітка вибору встановлена для %КоличествоВыбранных% рядків з %КоличествоВсего%.'");
	
		Текст = Текст
		        + Символы.ПС
		        + НСтр("ru='Проверьте: не заполнены либо количество, либо не задан шаблон ценника (этикетки). В этом случае отметка выбора для строк не может быть установлена.';uk='Перевірте: не заповнені або кількість, або не заданий шаблон цінника (етикетки). У цьому випадку оцінка вибору для рядків не може бути встановлена.'");
