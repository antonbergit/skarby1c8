...
Процедура ОбработкаГруппыПолей(Поле, Отказ)
	...
	Если НЕ ПустаяСтрока(Поле.ИмяГруппыПолей) Тогда
		...
		Отказ = Истина;
		ПоказатьПредупреждение(, Нстр("ru='Выбор групп не предусмотрен...';uk='Вибір груп не передбачений...'"));
...
"Рекомендуется разделить загрузку на несколько итераций.';uk='Велика кількість додаткових реквізитів в завантаженні істотно сповільнює процес."
"Рекомендується розділити завантаження на кілька ітерацій. '");
			
			ТекстЗаголовка = СтрШаблон(НСтр("ru='Выбрано %1 реквизита';uk='Обрано %1 реквізиту'"), МаксимумДополнительныхРеквизитов);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ЗаголовокКолонки", ЗаголовокКолонки) Тогда
		...
		ЭтаФорма.Заголовок = НСтр("ru='Выбор поля';uk='Вибір поля'") + ?(ПустаяСтрока(ЗаголовокКолонки), "", ": " + СокрЛП(ЗаголовокКолонки));
	...
	КэшЗначений.Вставить("ПолеВозвратаСписок", НСтр("ru='Назад к списку реквизитов';uk='Назад до списку реквізитів'"));
