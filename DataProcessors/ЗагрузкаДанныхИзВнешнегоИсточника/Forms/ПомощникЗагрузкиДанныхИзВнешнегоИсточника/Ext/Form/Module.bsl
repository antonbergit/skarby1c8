...
Процедура ЗаполнитьДанныеФайлаОбразцаЗагрузки(ДанныеФайла, Расширение)
	...
	НаименованиеФайлаБезРасширения = Нстр("ru='ОбразецЗагрузкиДанных';uk='ОбразецЗагрузкиДанных'");
...
Процедура ОткрытьОбразец(Расширение)
	...
	Если Расширение = "mxl" Тогда
		...
		Если ТипЗнч(ТабличныйДокумент) = Тип("ТабличныйДокумент") Тогда
			...
			ПечатнаяФорма.СинонимМакета = НСтр("ru='Образец подготовленных данных в формате mxl';uk='Зразок підготовлених даних у форматі mxl'");
			ПечатнаяФорма.ТабличныйДокумент = ТабличныйДокумент;
			ПечатнаяФорма.ИмяФайлаПечатнойФормы = НСтр("ru='Образец подготовленных данных в формате mxl';uk='Зразок підготовлених даних у форматі mxl'");
...
Процедура ЗаполнитьСписокВыбораФильтраТаблицыСопоставления(ЭтоЗагрузкаТабличнойЧасти)
	...
	Если Параметры.НастройкиЗагрузкиДанных.ЭтоЗагрузкаТабличнойЧасти Тогда
		...
		Элементы.ФильтрТаблицаСопоставления.СписокВыбора.Вставить(1, Нстр("ru='ОтборОшибокНет';uk='ВідбірПомилокНемає'"),	Нстр("ru='Данные, готовые к загрузке';uk='Дані, готові до завантаження'"));
		Элементы.ФильтрТаблицаСопоставления.СписокВыбора.Вставить(2, Нстр("ru='ОтборОшибки';uk='ВідбірПомилки'"), 	Нстр("ru='Данные, которые загрузить невозможно';uk='Дані, які завантажити неможливо'"));
		
	Иначе
		
		Элементы.ФильтрТаблицаСопоставления.СписокВыбора.Вставить(1, Нстр("ru='Сопоставленные';uk='Зіставлені'"), Нстр("ru='Данные, которые удалось сопоставить';uk='Дані, які вдалося зіставити'"));
		Элементы.ФильтрТаблицаСопоставления.СписокВыбора.Вставить(1, Нстр("ru='БудутСозданы';uk='БудутьСтворені'"), Нстр("ru='Данные, которым не найдено соответствие в программе';uk='Дані, яким не знайдена відповідність у програмі'"));
		Элементы.ФильтрТаблицаСопоставления.СписокВыбора.Вставить(1, Нстр("ru='Несогласованные';uk='Неузгоджені'"), Нстр("ru='Данные, которые содержат ошибку (заполнены не полностью)';uk='Дані, які містять помилку (заповнені не повністю)'"));
...
Процедура УстановитьТекстЗаголовкаДекорацииНесопоставленныеСтроки()
	...
	Если СоздаватьЕслиНеСопоставлено Тогда
		...
		ТекстЗаголовка = ?(Параметры.НастройкиЗагрузкиДанных.ЭтоЗагрузкаСправочника, 
			Нстр("ru='будет создано новых элементов:';uk='буде створено нових елементів:'"), 
			Нстр("ru='будет создано новых записей:';uk='буде створено нових записів:'"));
		
	Иначе
		
		ТекстЗаголовка = Нстр("ru='будет пропущено строк:';uk='буде пропущено рядків:'");
...
Процедура УстановитьТекстЗаголовкаДекорацииСопоставленыхОбъектов()
	...
	ТекстЗаголовока = ?(ОбновлятьСуществующие,
		Нстр("ru='из них сопоставлены и будут обновлены';uk='з них зіставлені й будуть оновлені'"),
		Нстр("ru='из них сопоставлены';uk='з них зіставлені'"));
...
Процедура УстановитьТекстЗаголовкаДекорацииОбщееЗначение()
	...
	Если Параметры.НастройкиЗагрузкиДанных.Свойство("ОбщееЗначение")
		...
		Если Параметры.НастройкиЗагрузкиДанных.ПолноеИмяОбъектаЗаполнения = "Справочник.Контрагенты"
			...
			Элементы.ОбщееЗначениеСправочник.Заголовок = Нстр("ru='< ';uk='<'") + Строка(Параметры.НастройкиЗагрузкиДанных.ОбщееЗначение) + Нстр("ru=' >';uk=' >'");
			
		ИначеЕсли Параметры.НастройкиЗагрузкиДанных.ПолноеИмяОбъектаЗаполнения = "РегистрСведений.ЦеныНоменклатуры" Тогда
			
			Элементы.ОбщееЗначениеРС.Заголовок = Нстр("ru='< ';uk='<'") + Строка(Параметры.НастройкиЗагрузкиДанных.ОбщееЗначение) + Нстр("ru=' >';uk=' >'");
...
"При проверке заполнения табличного документа ';uk='Если найдено %1 пустых строк, обработка следующих строк не выполняется."
"При проверке заполнения табличного документа '");
	...
	ТекстЖирный = НСтр("ru='обнаружены следующие ошибки:';uk='виявлені наступні помилки:'");
...
Процедура ЗагрузитьФайлСДаннымиВТабличныйДокументНаСервере(ПерейтиДалее)
	...
	Описание = НСтр("ru='Подсистема ЗагрузкаДанныхИзВнешнегоИсточника: Выполнение серверного метода загрузка данных из файла';uk='Підсистема ЗагрузкаДанныхИзВнешнегоИсточника: Виконання серверного методу завантаження даних з файлу'");
...
Процедура УстановитьТекстДекорацииКоличестваСтрок()
	...
	Если ФильтрТаблицаСопоставления = "БезОтбора" Тогда 
		...
		НовыйЗаголовок = Нстр("ru='Всего строк в таблице .......... %1';uk='Всього рядків у таблиці .......... %1'");
		ЗначениеПараметра = КоличествоСтрокТаблицы;
		
	ИначеЕсли ФильтрТаблицаСопоставления = "ОтборОшибокНет" Тогда 
		
		НовыйЗаголовок = Нстр("ru='Строк с данными, которые возможно загрузить в приложение .......... %1';uk='Рядків з даними, які можливо завантажити в додаток .......... %1'");
		ЗначениеПараметра = КоличествоСтрокБезОшибок;
		
	ИначеЕсли ФильтрТаблицаСопоставления = "ОтборОшибки" Тогда 
		
		НовыйЗаголовок = Нстр("ru='Строки, содержащие ошибки и препятствующие загрузке данных .......... %1';uk='Рядки, що містять помилки й перешкоджають завантаженню даних .......... %1'");
	...
	ИначеЕсли ФильтрТаблицаСопоставления = "Сопоставленные" Тогда 
		...
		Если ОбновлятьСуществующие Тогда
			...
			НовыйЗаголовок = Нстр("ru='Данные, которые соответствуют элементам программы и будут обновлены .......... %1';uk='Дані, які відповідають елементам програми й будуть оновлені .......... %1'");
			
		Иначе
			
			НовыйЗаголовок = Нстр("ru='Данные, которые соответствуют элементам программы .......... %1';uk='Дані, які відповідають елементам програми .......... %1'");
	...
	ИначеЕсли ФильтрТаблицаСопоставления = "БудутСозданы" Тогда 
		...
		НовыйЗаголовок = Нстр("ru='Данные, которые не удалось сопоставить .......... %1';uk='Дані, які не вдалося зіставити .......... %1'");
		ЗначениеПараметра = НесопоставленоДанных;
		
	ИначеЕсли ФильтрТаблицаСопоставления = "Несогласованные" Тогда 
		
		НовыйЗаголовок = Нстр("ru='Строки, которые содержат ошибку либо заполнены не полностью .......... %1';uk='Рядки, які містять помилку або заповнені не повністю .......... %1'");
...
Процедура ВыбратьВнешнийФайлОбработкаЗавершение(Результат, Адрес, ИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Истина Тогда
		...
		Иначе
			...
			ТекстПредупреждения = НСтр("ru='Импорт данных из файлов данного типа не поддерживается.';uk='Імпорт даних з файлів даного типу не підтримується.'");
