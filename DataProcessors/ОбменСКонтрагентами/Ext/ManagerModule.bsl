...
// Используюется для быстрого обмена файлами (через файл, 1С:Бизнес-сеть).
//
// Параметры:
...
Процедура ПодготовитьДанныеДляЗаполненияДокументов(Параметры, АдресХранилища) Экспорт
	...
	Для Каждого СсылкаНаОбъект Из МассивСсылокНаОбъект Цикл
		...
		НастройкиОбмена.Вставить("ВерсияФормата", ОбменСКонтрагентамиСлужебный.ВерсияСхемыCML2());
	...
	Для Каждого СтруктураОбмена Из МассивСтруктурОбмена Цикл
		...
		Если СтруктураОбмена.СтруктураЭД.ДокументыОснования.Количество() <> 1 Тогда
			ВызватьИсключение НСтр("ru='Ошибка формирования XML-файла';uk='Ошибка формирования XML-файла'");
		КонецЕсли;
	...
	Иначе
		...
		Если ОтправкаЧерезБС Тогда
			ШаблонОшибки = НСтр("ru='Произошла ошибка при отправке документа %1.';uk='Произошла ошибка при отправке документа %1.'");
...
"Подробности см. Журнал Регистрации';uk='Произошла ошибка при формировании пакета однократной сделки."
	...
	Если Не ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		Исключение
			...
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				НСтр("ru='Формирование пакета ЭД - однократная сделка';uk='Формирование пакета ЭД - однократная сделка'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
				ТекстСообщения);
		КонецПопытки;
...
Функция БыстрыйОбменИмяСохраняемогоФайла(Основание)
	...
	Если ЗначениеЗаполнено(Ссылка) И НЕ ЗначениеЗаполнено(НаименованиеФайла) Тогда
		...
		Иначе
			...
			ШаблонФайла = НСтр("ru='%1 %2 от %3';uk='%1 %2 от %3'");
...
"Подробности см. в журнале регистрации.';uk='Не удалось сформировать табличный документ."
