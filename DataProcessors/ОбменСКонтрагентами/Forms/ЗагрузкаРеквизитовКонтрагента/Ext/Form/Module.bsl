...
Процедура Загрузить(Команда)
	...
	Если ЗаполненыОбязательныеРеквизиты() Тогда
		Если ЗначениеЗаполнено(Контрагент) Тогда
			ТекстВопроса = НСтр("ru='Контрагент существует. Перезаполнить реквизиты?';uk='Контрагент существует. Перезаполнить реквизиты?'");
...
Процедура РазобратьФайлНаСервере();
		...
		Если ЗначениеXDTO <> Неопределено Тогда
			Для Каждого ТекСв Из ЗначениеXDTO.РасчетныйСчет Цикл
				...
				Если НЕ ТекСв.БанкКорреспондент=Неопределено Тогда
					МФОБанкаДляРасчетов                       = ТекСв.БанкКорреспондент.МФО;
	...
	Если СвойствоЭД <> Неопределено Тогда
		...
		Если ЗначениеXDTO <> Неопределено Тогда
			...
			КонецЕсли;
			СвойствоЕДРПОУ = ЗначениеXDTO.Свойства().Получить("ЕДРПОУ");
			Если СвойствоЕДРПОУ <> Неопределено Тогда
				ЕДРПОУ = ЗначениеXDTO.Получить(СвойствоЕДРПОУ);
			КонецЕсли;
...
Функция ЗаполненыОбязательныеРеквизиты()
	...
	Если НЕ ЗначениеЗаполнено(Наименование) Тогда
		...
		ТекстСообщения = НСтр("ru='Не заполнено <Наименование>, загрузка не возможна.';uk='Не заполнено <Наименование>, загрузка не возможна.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ТекстСообщения = НСтр("ru='Проверьте правильность указания <Файла> с реквизитами контрагента.';uk='Проверьте правильность указания <Файла> с реквизитами контрагента.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыКонтрагента()
	...
	СтруктураРеквизитов.Вставить("ЕДРПОУ",                 ЕДРПОУ);
...
Процедура ОпределитьКонтрагента()
	...
	|ГДЕ
	|	Контрагенты.ИНН = &ИНН";
	Запрос.УстановитьПараметр("ИНН", Сред(ИНН, 1, СтрНайти(ИНН, "/") - 1));
