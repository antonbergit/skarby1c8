...
"Вы действительно хотите закрыть форму?';uk='В списке выбора есть невыгруженные документы."
...
Процедура БыстрыеОтборыЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИначеЕсли ВРег(Элементы.БыстрыеОтборы.ТекущиеДанные.Параметр) = ВРег("СостояниеВерсииЭД") Тогда
		...
		СостоянияЭД.ПоказатьОтметкуЭлементов(ВыборЭлементаЗавершить,"Состояния ЭД");
...
Процедура СформироватьТаблицуБыстрогоОтбора()
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ru='Состояние ЭД:';uk='Состояние ЭД:'");
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ru='Только отраженные в учете:';uk='Только отраженные в учете:'");
...
Процедура ДобавитьВВыбранныеДокументы(МассивСтрок)
	...
	ШаблонСообщения = НСтр("ru='Для документа ""%1"" не завершен электронный документооборот.';uk='Для документа ""%1"" не завершен электронный документооборот.'");
	ТекстСообщения = "";
	...
	Если ВерсияВызова <> 1 И МножественныйВыбор = Ложь И ТаблицаВыбранныхДокументов.Количество() > 0 Тогда
		Если ЗначениеЗаполнено(ТекстСообщения) Тогда
			ТекстВопроса = ТекстСообщения + Символы.ПС + НСтр("ru='Продолжить выгрузку?';uk='Продолжить выгрузку?'");
...
Функция СформироватьФайлВыгрузки(Знач ТаблицаДокументов)
	...
	Пока Результат.СледующийПоЗначениюПоля("Организация") Цикл
		...
		Если Файлы.Количество() = 0 Тогда
			ТекстСообщения = НСтр("ru='Не удалось выгрузить документы по Организации ""%1"".';uk='Не удалось выгрузить документы по Организации ""%1"".'");
...
Функция ФайлОписанияВыгрузки(Организация, ТЗОписи, АдресКаталога)
	...
	Попытка
		...
		ОбменСКонтрагентамиВнутренний.ЗаполнитьСвойствоXDTO(Файл, "ДатаВыгрузки", Формат(ТекущаяДатаСеанса(), "ДФ=dd.MM.yyyy"), Истина, ТекстОшибки);
	...
	Исключение
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru='Формирование выгрузки ЭД в 1с-Отчетность';uk='Формирование выгрузки ЭД в 1с-Отчетность'"),
...
#Если НЕ ВебКлиент Тогда
				...
				ДиалогВыбора.Заголовок = НСтр("ru='Выберите каталог для сохранения файла (файлов) выгрузки';uk='Выберите каталог для сохранения файла (файлов) выгрузки'");
...
Процедура Выгрузить()
	...
	Если ТаблицаВыбранныхДокументов.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru='Для формирования выгрузки необходимо выбрать хотя бы один документ.';uk='Для формирования выгрузки необходимо выбрать хотя бы один документ.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	Иначе
...
"Продолжить выгрузку?';uk='В списке выбранных документов, присутствуют документы вида ""%1"","
"с незаполненными реквизитами документов-оснований (номер, дата)."
"Продолжить выгрузку?'");
