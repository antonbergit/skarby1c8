...
"расширение работы с криптографией.';uk='Для работы с ЭП необходимо установить"
...
Функция ЕстьДокументыНаПодпись() 
	...
	Если ПроверочныеДанные.КоличествоДокументов = 0 Тогда
		ТекстПредупреждения = НСтр("ru='По данному сертификату нет документов на подпись';uk='По данному сертификату нет документов на подпись'");
...
Процедура ПерейтиНаСтраницу(КДетализации)
	...
	Если КДетализации Тогда
		...
		Заголовок = НСтр("ru='Документы на подпись по сертификату';uk='Документы на подпись по сертификату'")+ ": " + СертификатПодписи;
	Иначе
		Элементы.Страницы.ТекущаяСтраница = Элементы.Страницы.ПодчиненныеЭлементы.СтраницаСводки;
		Заголовок = НСтр("ru='Документы на подпись';uk='Документы на подпись'");
	КонецЕсли;
	
	Элементы.Подписать.Заголовок = НСтр("ru='Подписать и отправить отмеченные';uk='Подписать и отправить отмеченные'");
...
"или не настроены правила подписи документов.';uk='Нет сертификатов подписи для пользователя"
