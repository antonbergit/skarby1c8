...
Процедура ВыгрузитьРеквизиты(Команда)
	...
	Если СпособВыгрузки = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезЭлектроннуюПочту") Тогда
		Если НЕ ЗначениеЗаполнено(АдресВыгрузки) Тогда
			ТекстСообщения = НСтр("ru='Не выбрана учетная запись электронной почты.';uk='Не выбрана учетная запись электронной почты.'");
...
Процедура ВыгрузитьРеквизитыОрганизации(АдресХранилища, УникальныйИдентификатор)
	...
	Попытка
		...
		ОбменСКонтрагентамиВнутренний.ЗаполнитьСвойствоXDTO(КонтрагентXDTO, "ИД", ИНН, Истина, ТекстОшибки);
		...
		ОбменСКонтрагентамиВнутренний.ЗаполнитьСвойствоXDTO(РеквизитыКонтрагентаXDTO, "ЕДРПОУ", ЕДРПОУ, , ТекстОшибки);
		...
		Если БанковскиеРеквизиты.Количество() > 0 Тогда
			...
			Для Каждого Счет Из БанковскиеРеквизиты Цикл
				...
				ОбменСКонтрагентамиВнутренний.ЗаполнитьСвойствоXDTO(БанкXDTO, "МФО", Счет.МФО, Истина, ТекстОшибки);
				ОбменСКонтрагентамиВнутренний.ЗаполнитьСвойствоXDTO(
				...
				Если ЗначениеЗаполнено(Счет.БанкДляРасчетов) Тогда
					БанкXDTO = ОбменСКонтрагентамиВнутренний.ПолучитьОбъектТипаCML("Банк", "4.02");
					ОбменСКонтрагентамиВнутренний.ЗаполнитьСвойствоXDTO(БанкXDTO, "МФО", Счет.БанкДляРасчетовМФО, Истина, ТекстОшибки);
					ОбменСКонтрагентамиВнутренний.ЗаполнитьСвойствоXDTO(
		...
		Иначе
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru='Формирование ЭД';uk='Формирование ЭД'"),
																						ТекстОшибки,
																						ТекстОшибки);
		КонецЕсли;
...
Функция ПолучитьСтруктуруРеквизитов()
	...
	СтруктураВозврата.Вставить("ЕДРПОУ");
...
Процедура ОбновитьФорму()
	...
	Если ТипУчастника = ИндивидуальныйПредприниматель Тогда
		...
		НадписьСвидетельство = НСтр("ru='Свидетельство №%1 от %2';uk='Свидетельство №%1 от %2'");
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ПравоеЗначение = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо;
