...
Процедура ОчиститьОтборПоВидамДокументов(Команда)
	...
	ТекстЗаголовка = НСтр("ru='Подтверждение';uk='Підтвердження'");
	ТекстВопроса   = НСтр("ru='Очистить отбор по видам документов?';uk='Очистити відбір по видах документів?'");
	Ответ = Неопределено;
...
Процедура ОбновитьНаименованиеКомандФормы()
	...
	Иначе
		...
		НовыйЗаголовокВидовДокументов = НСтр("ru='Выбрать виды документов ';uk='Вибрати види документів'");
...
Функция СформироватьОписаниеПравилОтправкиУНФ()
	...
	ТекстОписания = НСтр("ru='Правила отправки данных из программы ""Управление небольшой фирмой"" :';uk='Правила відправлення даних з програми ""Управління невеликою фірмою"":'") + Символы.ПС;
	
	ТекстОписания = ТекстОписания + НСтр("ru='Вся нормативно-справочная информация автоматически регистрируется к отправке;';uk='Уся нормативно-довідкова інформація автоматично реєструється до відправлення;'");
	...
	Иначе
		...
		Если ИспользоватьОтборПоОрганизациямУНФ Тогда
			...
			ПредставлениеКоллекции = ПланыОбмена.ОбменУправлениеНебольшойФирмойБухгалтерия20.СокращенноеПредставлениеКоллекцииЗначений(КоллекцияЗначений);
...
Функция СформироватьОписаниеПравилОтправкиБП()
	...
	ТекстОписания = НСтр("ru='Правила отправки данных из программы ""Бухгалтерия для Украины"" :';uk='Правила відправлення даних з програми ""Бухгалтерія для України"":'") + Символы.ПС;
	
	ТекстОписания = ТекстОписания + НСтр("ru='Вся нормативно-справочная информация автоматически регистрируется к отправке;';uk='Уся нормативно-довідкова інформація автоматично реєструється до відправлення;'");
	
	Если ПравилаОтправкиДокументовБП = "НеСинхронизировать" Тогда
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru='Документы отправляться не будут;';uk='Документи відправлятися не будуть;'");
	Иначе
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru='Все документы автоматически регистрируются к отправке:';uk='Всі документи автоматично реєструються до відправлення:'");
