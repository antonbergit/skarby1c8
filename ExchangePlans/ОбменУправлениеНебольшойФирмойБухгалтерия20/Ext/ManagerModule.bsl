...
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	...
	Возврат НСтр("ru='Создать обмен с конфигурацией ""Бухгалтерия для Украины, редакция 2.0""';uk='Створити обмін з конфігурацією ""Бухгалтерія для України, редакція 2.0""'");
...
Функция КраткаяИнформацияПоОбмену(ИдентификаторНастройки) Экспорт
	...
	ПоясняющийТекст = НСтр("ru='	Позволяет синхронизировать данные между приложениями 1С:Управление небольшой фирмой для Украины, ред. 1.6 и Бухгалтерия для Украины, ред. 2.0."
"Из приложения Управление небольшой фирмой для Украины в приложение Бухгалтерия для Украины переносятся справочники и все необходимые документы, а "
"из приложения Бухгалтерия для Украины в приложение Управление небольшой фирмой для Украины - справочники и документы учета денежных средств.';uk=' Дозволяє синхронізувати дані між додатками 1С:Управління невеликою фірмою для України, ред. 1.6 і 1С:Бухгалтерія для України , ред. 1.2."
"З додатку Управління невеликою фірмою для України в додаток Бухгалтерія для України переносяться довідники та всі необхідні документи, а"
"з додатка Бухгалтерія для України в додаток Управління невеликою фірмою для України - довідники та документи обліку коштів.'");
...
Функция ИмяОбработкиВыгрузки() Экспорт
	...
	Возврат "ОбработчикиВыгрузкиВБухгалтерияДляУкраины20";
...
Функция ИмяОбработкиЗагрузки() Экспорт
	...
	Возврат "ОбработчикиЗагрузкиИзБухгалтерияДляУкраины20";
