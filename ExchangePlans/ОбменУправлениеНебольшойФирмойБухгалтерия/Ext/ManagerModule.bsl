...
Функция НастройкаОтборовНаУзле(ВерсияКорреспондента, ИмяФормы, ИдентификаторНастройки) Экспорт
	...
	СтруктураНастроек.Вставить("ЗагружатьДокументыИзБухгалтерии", Ложь);
...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле, ВерсияКорреспондента, ИдентификаторНастройки) Экспорт
	...
	Если НастройкаОтборовНаУзле.РучнойОбмен Тогда
		...
		Если Не НастройкаОтборовНаУзле.ЗагружатьДокументыИзБухгалтерии Тогда
			ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru='Документы приложения ""Бухгалтерия для Украины"" в синхронизации не участвуют;';uk='Документи додатку ""Бухгалтерія для України"" у синхронізації не беруть участь;'");
		КонецЕсли;
...
Функция ОписаниеЗначенийПоУмолчанию(ЗначенияПоУмолчаниюНаУзле, ВерсияКорреспондента, ИдентификаторНастройки) Экспорт
	...
	Если ЗначениеЗаполнено(ЗначенияПоУмолчаниюНаУзле.ВидНалога) Тогда
		НСтрока = НСтр("ru='Вид налога: %1';uk='Вид податку: %1'");
	...
	Если ЗначениеЗаполнено(ЗначенияПоУмолчаниюНаУзле.СтатьяРасход) Тогда
		НСтрока = НСтр("ru='Статья ДДС (расход): %2';uk='Стаття РГК (витрата): %2'");
	...
	Если ЗначениеЗаполнено(ЗначенияПоУмолчаниюНаУзле.СтатьяПриход) Тогда
		НСтрока = НСтр("ru='Статья ДДС (приход): %3';uk='Стаття РГК (прихід): %3'");
	...
	Если ЗначениеЗаполнено(ЗначенияПоУмолчаниюНаУзле.Касса) Тогда
		НСтрока = НСтр("ru='Касса: %4';uk='Каса: %4'");
	...
	Если ЗначениеЗаполнено(ЗначенияПоУмолчаниюНаУзле.КассаККМ) Тогда
		НСтрока = НСтр("ru='Касса ККМ: %5';uk='Каса ККМ: %5'");
...
"%3"
"%4"
"%5';uk='При завантаженні реквізити документів будуть автоматично дозаповнені значеннями:"
"%1"
"%2"
"%3"
"%4"
...
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	...
	Возврат НСтр("ru='Создать обмен с конфигурацией ""Бухгалтерия для Украины, редакция 1.2""';uk='Створити обмін з конфігурацією ""Бухгалтерія для України, редакція 1.2""'");
...
Функция КраткаяИнформацияПоОбмену(ИдентификаторНастройки) Экспорт
	...
	ПоясняющийТекст = НСтр("ru='	Позволяет синхронизировать данные между приложениями 1С:Управление небольшой фирмой для Украины, ред. 1.6 и 1С:Бухгалтерия для Украины, ред. 1.2."
...
Процедура ИнициализироватьРежимВыгрузкиПриНеобходимости() Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	БП.Ссылка
	                      |ИЗ
	                      |	ПланОбмена.ОбменУправлениеНебольшойФирмойБухгалтерия КАК БП
	                      |ГДЕ
	                      |	БП.Ссылка <> &ЭтотУзел
	                      |	И ВЫБОР
	...
	Запрос.УстановитьПараметр("ЭтотУзел", ПланыОбмена.ОбменУправлениеНебольшойФирмойБухгалтерия.ЭтотУзел());
