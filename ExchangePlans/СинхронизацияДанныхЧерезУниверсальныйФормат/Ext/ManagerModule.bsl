...
"и ""Бухгалтерия для Украины, редакция 2.0"".  В синхронизации участвуют документы и нормативно-справочной информация.';uk='Це налаштування дозволить синхронізувати дані між програмами ""1С:Управління невеликою фірмою, редакція 1.0"","
"і ""Бухгалтерія для України, редакція 2.0"". В синхронізації беруть участь документи і нормативно-довідкова інформація.'");
	Иначе
...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле, ВерсияКорреспондента, ИдентификаторНастройки = "") Экспорт
	...
	ТекстОписания = НСтр("ru='Вся нормативно-справочная информация автоматически регистрируется к отправке;';uk='Уся нормативно-довідкова інформація автоматично реєструється до відправлення;'");
	
	Если НастройкаОтборовНаУзле.РучнойОбмен Тогда
		
		ТекстОписания = НСтр("ru='Пользователь самостоятельно отбирает и регистрирует документы к отправке;';uk='Користувач самостійно відбирає й реєструє документи до відправлення;'");
		
	Иначе
		
		Если НастройкаОтборовНаУзле.ИспользоватьОтборПоВидамДокументов Тогда
			ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru='Документы автоматически регистрируются к отправке:';uk='Документи автоматично реєструються до відправлення:'");
		Иначе
		...
		Если ЗначениеЗаполнено(НастройкаОтборовНаУзле.ДатаНачалаВыгрузкиДокументов) Тогда
			ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru='начиная с %ДатаНачала%';uk='починаючи з %ДатаНачала%'");
		...
		Если НастройкаОтборовНаУзле.ИспользоватьОтборПоОрганизациям Тогда
			...
			ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru='с отбором по организациям: %ПредставлениеКоллекции%';uk='с відбором по організаціях: %ПредставлениеКоллекции%'");
		...
		Если НастройкаОтборовНаУзле.ИспользоватьОтборПоВидамДокументов Тогда
			...
			ПредставлениеКоллекции = СокращенноеПредставлениеКоллекцииЗначений(КоллекцияЗначений);
			ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru='с отбором по видам документов: %ПредставлениеКоллекции%';uk='с відбором по видах документів: %ПредставлениеКоллекции%'");
			ТекстОписания = СтрЗаменить(ТекстОписания, "%ПредставлениеКоллекции%", ПредставлениеКоллекции);
		КонецЕсли;
	...
	Если НастройкаОтборовНаУзле.АвтоматическиЗачитыватьАвансы Тогда
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru='Авансы автоматически зачитываются при переносе документов в ""Бухгалтерию предприятия""';uk='Аванси автоматично зачитуються при перенесенні документів в ""Бухгалтерію підприємства""'");
		ТекстОписания = СтрЗаменить(ТекстОписания, "%ПредставлениеКоллекции%", ПредставлениеКоллекции);
	КонецЕсли;
...
Функция ОписаниеЗначенийПоУмолчаниюБазыКорреспондента(ЗначенияПоУмолчаниюНаУзле, ВерсияКорреспондента, ИдентификаторНастройки = "") Экспорт
	...
	Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		ТекстОписания = ТекстОписания + Символы.ПС + Символы.ПС + НСтр("ru='Услуга по комиссионному вознаграждению для подстановки в документ Отчет комитенту по умолчанию: %Значение%';uk='Послуга по комісійній винагороді для підстановки в документ Звіт комітенту за замовчуванням: %Значение%'");
		...
		ТекстОписания = ТекстОписания + Символы.ПС + Символы.ПС + НСтр("ru='Способ отражения расходов для заполнения документа Передача материалов в эксплуатацию: %Значение%';uk='Спосіб відображення витрат для заповнення документа Передача матеріалів в експлуатацію: %Значение%'");
...
Процедура ОпределитьНастройки(Настройки, ИдентификаторНастройки = "") Экспорт
	...
	Настройки.ПутьКФайлуКомплектаПравилВКаталогеШаблонов = "\1c\smallbusinessukr";
	...
	Если ИдентификаторНастройки = "ОбменУниверсальный" Тогда
		...
		Настройки.Вставить("ЗаголовокПомощникаСозданияОбмена",               НСтр("ru='Настройка синхронизации данных через универсальный формат';uk='Налаштування синхронізації даних через універсальний формат'"));
	...
	ИначеЕсли ИдентификаторНастройки = "ОбменБП20" Тогда
		Настройки.Вставить("ЗаголовокКомандыДляСозданияНовогоОбменаДанными", НСтр("ru='Бухгалтерия для Украины, редакция 2.0';uk='Бухгалтерія для України, редакція 2.0'"));
		Настройки.Вставить("ЗаголовокПомощникаСозданияОбмена",               НСтр("ru='Настройка синхронизации с программой ""Бухгалтерия для Украины, редакция. 2.0""';uk='Налаштування синхронізації з програмою ""Бухгалтерія для України, редакція. 2.0""'"));
		Настройки.Вставить("ЗаголовокУзлаПланаОбмена",                       НСтр("ru='Синхронизация с программой ""Бухгалтерия для Украины, редакция 2.0""';uk='Синхронізація з програмою ""Бухгалтерія для України, редакція 2.0""'"));
		Настройки.Вставить("НаименованиеКонфигурацииКорреспондента",         НСтр("ru='Бухгалтерия для Украины, редакция 2.0';uk='Бухгалтерія для України, редакція 2.0'"));
	Иначе
...
Функция ИмяКонфигурацииИсточника() Экспорт
	...
	Возврат "УправлениеНебольшойФирмойДляУкраины";
...
Процедура ЗарегистрироватьСвязанныеДокументы(Выгрузка, Объект, ПРО, Получатели) Экспорт
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.НалоговаяНакладная") Тогда
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.РегистрацияВходящегоНалоговогоДокумента") Тогда
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.Приложение1КНалоговойНакладной") Тогда
		
		Если ЗначениеЗаполнено(Объект.НалоговаяНакладная) Тогда
			НоваяСтрока = СвязанныеДокументы.Добавить();
			НоваяСтрока.Документ = Объект.НалоговаяНакладная;
		КонецЕсли;
		
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.Приложение2КНалоговойНакладной") Тогда
...
Функция ПереходНаСинхронизациюЧерезУниверсальныйФорматВнешнееСоединение(ПараметрыНастройкиСинхронизацииДанных) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ВариантНастройки) Тогда
		...
		ТекстСообщения = НСтр("ru='Не указан вариант настройки синхронизации в базе-корреспонденте';uk='Не вказаний варіант налаштування синхронізації в базі-кореспонденті'");
	...
	Если НЕ ЗначениеЗаполнено(ИмяПланаОбменаСтаройНастройки) Тогда
		ПараметрыНастройкиСинхронизацииДанных.Ошибка = Истина;
		НСтрока = НСтр("ru='Передан неизвестный вариант настройки синхронизации: %1.';uk='Передано невідомий варіант налаштування синхронізації: %1.'");
	...
	Если Метаданные.ПланыОбмена.Найти(ИмяПланаОбменаСтаройНастройки) = Неопределено Тогда
		ПараметрыНастройкиСинхронизацииДанных.Ошибка = Истина;
		НСтрока = НСтр("ru='Не обнаружен план обмена, соответствующий варианту настройки: %1.';uk='Не знайдено план обміну, який відповідає варіанту налаштування: %1.'");
		ПараметрыНастройкиСинхронизацииДанных.СообщениеОбОшибке = 
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтрока, ВариантНастройки);
		Возврат Неопределено;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(НастройкаСинхронизацииДанных) Тогда
		ПараметрыНастройкиСинхронизацииДанных.Ошибка = Истина;
		НСтрока = НСтр("ru='Не найден узел корреспондента для плана обмена: %1, код: %2.';uk='Не вдалося знайти вузол кореспондента для плану обміну: %1, код: %2.'");
		ПараметрыНастройкиСинхронизацииДанных.СообщениеОбОшибке = 
...
Функция ИмяПланаОбменаСтаройНастройки(ВариантНастройки) Экспорт
	Если СокрЛП(ВариантНастройки) = "ОбменБП20" Тогда
		Возврат "ОбменУправлениеНебольшойФирмойБухгалтерия20";
	КонецЕсли;
	Возврат "";
КонецФункции
...
Функция ВыполнитьПереходНаСинхронизациюДанныхЧерезУниверсальныйФормат(НастройкаСинхронизацииДанных) Экспорт
	...
	Сообщение = НСтр("ru='Создание новой настройки синхронизации данных';uk='Створення нового налаштування синхронізації даних'");
	ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);
	...
	ВариантНастройки = "";
	Если ИмяПланаОбменаСтаройНастройки = "ОбменУправлениеНебольшойФирмойБухгалтерия20" Тогда
		ВариантНастройки = "ОбменБП20";
	КонецЕсли;
	...
	ИмяСобытияЖурналРегистрации = НСтр("ru='Переход на синхронизацию данных через универсальный формат';uk='Перехід на синхронізацію даних через універсальний формат'");
	Сообщение = НСтр("ru='Заполнение настроек транспорта сообщений';uk='Заповнення налаштувань транспорту повідомлень'");
	ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);
	...
	ИмяСобытияЖурналРегистрации = НСтр("ru='Переход на синхронизацию данных через универсальный формат';uk='Перехід на синхронізацію даних через універсальний формат'");
	Сообщение = НСтр("ru='Заполнение настроек запрета изменения данных';uk='Заповнення налаштувань заборони зміни даних'");
	ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);
	...
	ИмяСобытияЖурналРегистрации = НСтр("ru='Переход на синхронизацию данных через универсальный формат';uk='Перехід на синхронізацію даних через універсальний формат'");
	Сообщение = НСтр("ru='Заполнение настроек главного узла';uk='Заповнення налаштувань головного вузла'");
	ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);
	...
	Если НЕ НастройкаСинхронизацииДанных.ПометкаУдаления Тогда
		Сообщение = НСтр("ru='Пометка на удаление старой настройки';uk='Позначка на видалення старого налаштування'");
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);
...
Процедура ОбработатьСоответствияОбъектовИнформационныхБаз(СтарыйУзел, НовыйУзел)
	...
	Если Найти(НРЕГ(Метаданные.Имя), "бухгалтерия") > 0 Тогда
		...
		Если НЕ РезультатЗапроса[1].Пустой() Тогда
			Сообщение = НСтр("ru='Обнаружены записи, в которых одной ссылке на источник соответствуют несколько приемников';uk='Виявлено записи, в яких одному посиланню на джерело відповідають кілька приймачів'") 
						+ Символы.ПС + НСтр("ru='Эти записи обработаны не будут.';uk='Ці записи оброблені не будуть.'");
			ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Ошибка, , , Сообщение);
	...
	Сообщение = НСтр("ru='Соответствие объектов информационных баз очищен';uk='Відповідність об`єктів інформаційних баз очищено'");
	ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Информация, , , Сообщение);
	...
	Если НастройкиТранспортаОбменаСтарогоУзла.Количество()>0 Тогда
		...
		Для Каждого НастройкаУзла Из НастройкиТранспортаОбменаНовогоУзла Цикл
			...
			Если МассивДоступныеТранспортыСообщений.Найти(ВидТранспортаПоУмолчанию) = Неопределено Тогда
				...
				ИмяСобытияЖурналРегистрации = НСтр("ru='Переход на синхронизацию данных через универсальный формат. Заполнение настроек транспорта сообщений';uk='Перехід на синхронізацію даних через універсальний формат. Заповнення налаштувань транспорту повідомлень'");
				НСтрока = НСтр("ru='Используемое ранее подключение по умолчанию недоступно для синхронизации данных через универсальный формат (%1)';uk='Використовуване раніше підключення за замовчуванням недоступне для синхронізації даних через універсальний формат (%1)'");
				...
				Если ЗначениеЗаполнено(НастройкаУзла.ВидТранспортаСообщенийОбменаПоУмолчанию) Тогда
					НСтрока = НСтр("ru='Взамен назначено подключение по умолчанию: %1. Необходимо проверить корректность настроек транспорта сообщений';uk='Натомість призначено підключення за замовчуванням: %1. Необхідно перевірити коректність налаштувань транспорту повідомлень'");
					Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтрока, 
																НастройкаУзла.ВидТранспортаСообщенийОбменаПоУмолчанию);

				Иначе
					Сообщение = НСтр("ru='Подключение по умолчанию очищено. Необходимо выполнить настройку транспорта сообщений вручную';uk='Підключення за замовчуванням очищено. Необхідно виконати налаштування транспорту повідомлень вручну'");
				КонецЕсли;
				ЗаписьЖурналаРегистрации(ИмяСобытияЖурналРегистрации, УровеньЖурналаРегистрации.Ошибка, , , Сообщение);
			КонецЕсли;
		КонецЦикла;
