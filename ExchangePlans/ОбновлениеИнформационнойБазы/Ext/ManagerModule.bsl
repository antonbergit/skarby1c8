...
Функция УзелПоОчереди(Очередь) Экспорт
	...
	Если ТипЗнч(Очередь) <> Тип("Число")  
		Или Очередь = 0 Тогда
		ТекстИсключения = НСтр("ru='Не возможно получить узел плана обмена ОбновлениеИнформационнойБазы, т.к. не передан номер очереди.';uk='Не можливо отримати вузол плану обміну ОбновлениеИнформационнойБазы, оскільки не переданий номер черги.'");
