...
Функция ИмяФайлаНастроекДляПриемника() Экспорт
	...
	Возврат НСтр("ru='Настройки обмена УНФ-РТ';uk='Налаштування обміну УНФ-РТ'");
...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле, ВерсияКорреспондента, ИдентификаторНастройки) Экспорт
	...
	ИначеЕсли НастройкаОтборовНаУзле.РежимВыгрузкиСправочников = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПриНеобходимости Тогда
		...
		ПредставлениеНормативноСправочнойИнформации = НСтр("ru='Отправлять нормативно-справочную информацию, которая используется в отправляемых документах.';uk='Відправляти нормативно-довідкову інформацію, яка використовується в відправлених документах.'");
	...
	Если (НастройкаОтборовНаУзле.РежимВыгрузкиДокументов = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПоУсловию)
		...
		Если НастройкаОтборовНаУзле.ИспользоватьОтборПоСкладам Тогда
			...
			ПредставлениеСкладов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Только по складам: %1.';uk='Тільки по складах: %1.'"),
				СтрокаПредставленияОтбора);
		Иначе
			ПредставлениеСкладов = НСтр("ru='По всем складам.';uk='По всіх складах.'");
		КонецЕсли;
		
		ПредставлениеОграниченийОтправкиДанных = НСтр("ru='Отправлять данные:"
"[ПредставлениеОрганизаций]"
"[ПредставлениеСкладов]';uk='Відправляти дані:"
"[ПредставлениеОрганизаций]"
"[ПредставлениеСкладов]'");
...
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	...
	Возврат НСтр("ru='Создать обмен с конфигурацией ""Розница для Украины, ред. 2.0';uk='Створити обмін з конфігурацією ""Роздріб для України, ред. 2.0'");
...
"Синхронизация является двухсторонней и позволяет иметь актуальные данные в каждой из информационных баз.';uk='Дозволяє синхронізувати дані між програмами 1С:Роздріб і 1С:Управління невеликою фірмою,"
"Синхронізація є двосторонньою і дозволяє мати актуальні дані в кожній з інформаційних баз. '");
...
Функция ОписаниеОграниченийПередачиДанныхБазыКорреспондента(НастройкаОтборовНаУзле, ВерсияКорреспондента, ИдентификаторНастройки) Экспорт
	...
	Возврат (
	НСтр("ru='Выгружать документы и справочную информацию:';uk='Вивантажувати документи та довідкову інформацію:'")
		+ Символы.ПС
