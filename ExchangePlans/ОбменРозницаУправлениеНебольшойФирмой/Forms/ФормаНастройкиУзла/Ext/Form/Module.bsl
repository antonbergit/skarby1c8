...
Процедура ОткрытьСписокВыбранныхСкладовНажатие(Элемент)
	...
	ПараметрыФормы.Вставить("ЗаголовокФормыВыбора",                   НСтр("ru='Выберите склады для отбора:';uk='Виберіть склади для відбору:'"));
	ПараметрыФормы.Вставить("МассивВыбранныхЗначений",                СформироватьМассивВыбранныхЗначений(ПараметрыФормы));
	ПараметрыФормы.Вставить("ПараметрыВнешнегоСоединения",            Неопределено);
	ПараметрыФормы.Вставить("КоллекцияФильтров",                      КоллекцияФильтров);
...
Процедура ОткрытьСписокВыбранныхВидовЦенНажатие(Элемент)
	...
	ПараметрыФормы.Вставить("ЗаголовокФормыВыбора",                   НСтр("ru='Выберите виды цен для отбора:';uk='Виберіть види цін для відбору:'"));
	ПараметрыФормы.Вставить("МассивВыбранныхЗначений",                СформироватьМассивВыбранныхЗначений(ПараметрыФормы));
	ПараметрыФормы.Вставить("ПараметрыВнешнегоСоединения",            Неопределено);
	ПараметрыФормы.Вставить("КоллекцияФильтров",                      КоллекцияФильтров);
...
Процедура ОбновитьНаименованиеКомандФормы()
	...
	Иначе
		НовыйЗаголовок = НСтр("ru='Выбрать организации';uk='Вибрати організації'");
	КонецЕсли;
	...
	Если ЭтаФорма.Склады.Количество() > 0 Тогда
		...
		НовыйЗаголовок = СтрСоединить(ВыбранныеЭлементы,", ");
	Иначе
		НовыйЗаголовок = НСтр("ru='Выбрать магазины';uk='Вибрати магазини'");
	КонецЕсли;
	...
	Если ЭтаФорма.ВидыЦен.Количество() > 0 Тогда
		...
		НовыйЗаголовок = СтрСоединить(ВыбранныеЭлементы,", ");
	Иначе
		НовыйЗаголовок = НСтр("ru='Выбрать виды цен';uk='Вибрати види цін'");
	КонецЕсли;
