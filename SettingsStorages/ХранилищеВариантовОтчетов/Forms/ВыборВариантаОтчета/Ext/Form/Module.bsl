...
"Выбрать этот варианта отчета?';uk='Вибраний варіант звіту позначений на видалення."
...
Процедура ОткрытьВариантДляИзменения()
	...
	Если Не ПравоИзмененияВарианта(Вариант, ПолныеПраваНаВарианты) Тогда
		ТекстПредупреждения = НСтр("ru='Недостаточно прав для изменения варианта ""%1"".';uk='Недостатньо прав для зміни варіанта ""%1"".'");
	...
	Для Каждого СтрокаТаблицы Из ТаблицаВариантов Цикл
		...
		Если НаборСтрокДерева = Неопределено Тогда
			...
			ИначеЕсли СтрокаТаблицы.НомерГруппы = 3 Тогда
				ГруппаДерева.Наименование = НСтр("ru='Помеченные на удаление';uk='Позначені на видалення'");
