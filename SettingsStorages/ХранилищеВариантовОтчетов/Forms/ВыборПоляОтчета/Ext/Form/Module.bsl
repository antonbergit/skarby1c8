...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Режим = "Отборы" Или Режим = "ВыбранныеПоля" Или Режим = "Сортировка" Или Режим = "ПоляГруппировки" Тогда
		ИмяТаблицы = Режим;
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Некорректное значение параметра ""Режим"": ""%1"".';uk='Некоректне значення параметра ""Режим"": ""%1"".'"), Строка(Режим));
	КонецЕсли;
...
Процедура ВыбратьИЗакрыть()
	...
	Если ТипЗнч(ДоступноеПолеКД) = Тип("ДоступноеПолеКомпоновкиДанных")
		...
		Если ДоступноеПолеКД.Папка Тогда
			ПоказатьПредупреждение(, НСтр("ru='Выберите элемент';uk='Виберіть елемент'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
