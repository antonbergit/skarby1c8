...
"но она не записана."
""
"Записать?';uk='Введено наименование настройки (%1),"
"но она не записана."
""
"Записать?'"),
			...
			Если ЭтаФорма.ТекущийБуферОбмена.Количество() > 0 Тогда
				Если НЕ ПустаяСтрока(ЭтаФорма.КлючНастройки) Тогда
					Если ЭтаФорма.СписокКлючейНастроек.НайтиПоЗначению(ЭтаФорма.КлючНастройки) = Неопределено Тогда
						...
						СписокКнопок.Добавить("ЗаписатьИЗакрыть", НСтр("ru='Записать и закрыть';uk='Записати і закрити'"));
						СписокКнопок.Добавить("Закрыть", НСтр("ru='Закрыть без записи';uk='Закрыть без записи'"));
						СписокКнопок.Добавить("Отмена", НСтр("ru='Не закрывать';uk='Не закривати'"));
						ПоказатьВопрос(ОписаниеОповещенияОЗакрытии, ТекстВопроса, СписокКнопок, 0, "ЗаписатьИЗакрыть", НСтр("ru='Текущее значение буфера обмена не сохранено';uk='Текущее значение буфера обмена не сохранено'"));
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура КомандаВыгрузитьСписокБуферовОбмена(Команда)
	...
	Диалог.Заголовок = НСтр("ru='Выберите файл, куда записать список буферов обмена';uk='Выберите файл, куда записать список буферов обмена'");
	...
	Если Диалог.Выбрать() Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ru='Данные выгружены.';uk='Данные выгружены.'"),
			,
			,
...
Процедура КомандаЗагрузитьСписокБуферовОбмена(Команда)
	...
	Диалог.Заголовок = НСтр("ru='Выберите файл с сохраненными ранее буферами обмена';uk='Выберите файл с сохраненными ранее буферами обмена'");
	Диалог.МножественныйВыбор = Ложь;
	Диалог.Расширение = "xml";
	Диалог.Фильтр = НСтр("ru='Файл xml (*.xml)|*.xml';uk='Файл xml (*.xml)|*.xml'");
	Если Диалог.Выбрать() Тогда
		КомандаЗагрузитьСписокБуферовОбменаНаСервере(Диалог.ПолноеИмяФайла);
		ПоказатьОповещениеПользователя(
			НСтр("ru='Данные загружены.';uk='Данные загружены.'"),
			,
			,
...
"Вначале скопируйте в него строки данных.';uk='Пустой буфер обмена."
"Нельзя сохранить пустой буфер обмена."
...
Функция ПолучитьОписаниеВажности(Важность)
	...
	Если Важность = 1 Тогда
		Возврат НСтр("ru='Очень важная';uk='Дуже важлива'");
	ИначеЕсли Важность = 2 Тогда
		Возврат НСтр("ru='Важная';uk='Важлива'");
	Иначе
		Возврат НСтр("ru='Обычная';uk='Звичайна'");
	КонецЕсли;
...
Процедура КомандаВыгрузитьСписокБуферовОбменаНаСервере(ИмяФайла)
	...
	ЗаписьХМЛ.ОткрытьФайл(
		...
		ЗаписьХМЛ.ЗаписатьКомментарий(
			СтрШаблон(
				НСтр("ru='Выгрузка данных буферов обмена для %1, %2';uk='Выгрузка данных буферов обмена для %1, %2'"),
				ЭтаФорма.КлючОбъекта,
