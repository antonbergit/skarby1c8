...
Процедура ОбработкаЗагрузки(КлючОбъекта, КлючНастроек, Настройки, ОписаниеНастроек, ИмяПользователяИБ)
	...
	Если НЕ (ОбработкаНовостейПовтИсп.ЕстьРолиЧтенияНовостей()) Тогда
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Хранилище настроек новостей';uk='БІП:Новини.Сховище налаштувань новин'"), // ИмяСобытия
			НСтр("ru='Новости. Хранилище настроек новостей. ОбработкаЗагрузки. Нарушение прав доступа';uk='Новини. Сховище налаштувань новин. ОбработкаЗагрузки. Порушення прав доступу'"), // ИдентификаторШага
...
Процедура ОбработкаПолученияОписания(КлючОбъекта, КлючНастроек, ОписаниеНастроек, ИмяПользователяИБ)
	...
	Если НЕ (ОбработкаНовостейПовтИсп.ЕстьРолиЧтенияНовостей()) Тогда
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Хранилище настроек новостей';uk='БІП:Новини.Сховище налаштувань новин'"), // ИмяСобытия
			НСтр("ru='Новости. Хранилище настроек новостей. ОбработкаПолученияОписания. Нарушение прав доступа';uk='Новини. Сховище налаштувань новин. ОбработкаПолученияОписания. Порушення прав доступу'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
...
Процедура ОбработкаСохранения(КлючОбъекта, КлючНастроек, Настройки, ОписаниеНастроек, ИмяПользователяИБ)
	...
	Если НЕ (ОбработкаНовостейПовтИсп.ЕстьРолиЧтенияНовостей()) Тогда
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Хранилище настроек новостей';uk='БІП:Новини.Сховище налаштувань новин'"), // ИмяСобытия
			НСтр("ru='Новости. Хранилище настроек новостей. ОбработкаСохранения. Нарушение прав доступа';uk='Новини. Сховище налаштувань новин. ОбработкаСохранения. Порушення прав доступу'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
	...
	ИначеЕсли КлючОбъекта = "ОтборыПоЛентамНовостейНастроенныеАдминистратором" Тогда
		...
		Иначе
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Хранилище настроек новостей';uk='БІП:Новини.Сховище налаштувань новин'"), // ИмяСобытия
				НСтр("ru='Новости. Хранилище настроек новостей. ОбработкаСохранения. Ошибка';uk='Новини. Сховище налаштувань новин. ОбработкаСохранения. Помилка'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				НСтр("ru='Ошибка при проверке набора записей регистра ОтборыПоЛентамНовостейОбщие (ключ объекта ОтборыПоЛентамНовостейНастроенныеАдминистратором) - возможно, не заполнены какие-то поля.';uk='Помилка при перевірці набору записів регістру ОтборыПоЛентамНовостейОбщие (ключ об`єкта ОтборыПоЛентамНовостейНастроенныеАдминистратором) - можливо, не заповнені якісь поля.'"), // Комментарий
				ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
		КонецЕсли;
	...
	ИначеЕсли КлючОбъекта = "ОтборыПоЛентамНовостейПользовательские" Тогда
		...
		Иначе
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				...
				, // ОбъектМетаданных
				, // Данные
				НСтр("ru='Ошибка при проверке набора записей регистра ОтборыПоЛентамНовостейПользовательские (ключ объекта ОтборыПоЛентамНовостейПользовательские) - возможно, не заполнены какие-то поля.';uk='Помилка при перевірці набору записів регістру ОтборыПоЛентамНовостейПользовательские (ключ об`єкта ОтборыПоЛентамНовостейПользовательские) - можливо, не заповнені якісь поля.'"), // Комментарий
				ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
		КонецЕсли;
	...
	ИначеЕсли КлючОбъекта = "ОтключенныеЛентыНовостей" Тогда
		...
		Иначе
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				...
				, // ОбъектМетаданных
				, // Данные
				НСтр("ru='Ошибка при проверке набора записей регистра ОтключенныеЛентыНовостей (ключ объекта ОтключенныеЛентыНовостей) - возможно, не заполнены какие-то поля.';uk='Помилка при перевірці набору записів регістру ОтключенныеЛентыНовостей (ключ об`єкта ОтключенныеЛентыНовостей) - можливо, не заповнені якісь поля.'"), // Комментарий
				ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
		КонецЕсли;
...
Процедура ОбработкаУстановкиОписания(КлючОбъекта, КлючНастроек, ОписаниеНастроек, ИмяПользователяИБ)
	...
	Если НЕ (ОбработкаНовостейПовтИсп.ЕстьРолиЧтенияНовостей()) Тогда
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Хранилище настроек новостей';uk='БІП:Новини.Сховище налаштувань новин'"), // ИмяСобытия
			НСтр("ru='Новости. Хранилище настроек новостей. ОбработкаУстановкиОписания. Нарушение прав доступа';uk='Новини. Сховище налаштувань новин. ОбработкаУстановкиОписания. Порушення прав доступу'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
