...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтаФорма.Заголовок = СтрШаблон(
		НСтр("ru='Настройка отборов для ленты новостей %1';uk='Настройка отборов для ленты новостей %1'"),
		...
		Если (Элементы.ОтборыПоЛентамНовостейПользовательские.ТекущиеДанные.РазрешеноНастраиватьПользователям = Истина) 
			...
			Если Элементы.ОтборыПоЛентамНовостейПользовательские.ТекущиеДанные.СписокЗначенийГлобальныхОтборов.Количество() > 0 Тогда
				ПоказатьОповещениеПользователя(
					НСтр("ru='Часть отборов не удалена';uk='Частина відборів не видалена'"),
					, // Навигационная ссылка
					НСтр("ru='Отборы, настроенные администратором, удалить нельзя.';uk='Відбори, налаштовані адміністратором, видалити не можна.'"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
					...
					Если (НайденнаяСтрока.СписокЗначенийОтборов.Количество() = 0) Тогда
						Если (НайденнаяСтрока.СписокЗначенийГлобальныхОтборов.Количество() = 0) Тогда
							Если НайденнаяСтрока.ОтбиратьПоКатегории Тогда
								НайденнаяСтрока.СписокЗначенийОтборовПредставление = НСтр("ru='Введите значения отборов';uk='Введите значения отборов'");
							Иначе
								НайденнаяСтрока.СписокЗначенийОтборовПредставление = ""; // Вывести все значения
							КонецЕсли;
						Иначе
							НайденнаяСтрока.СписокЗначенийОтборовПредставление = НСтр("ru='Настроено администратором:';uk='Настроено администратором:'") + " ";
	...
	Иначе
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			Если (НайденнаяСтрока.ЗаполняетсяАвтоматически = Истина) Тогда
				НайденнаяСтрока.ПредставлениеДоступностиОтбора = НСтр("ru='Заполняется автоматически, настройка запрещена';uk='Заповнюється автоматично, налаштування заборонене'");
			Иначе
				Если (НайденнаяСтрока.РазрешеноНастраиватьПользователям = Истина) Тогда
					Если НайденнаяСтрока.ОтбиратьПоКатегории = Истина Тогда
						НайденнаяСтрока.ПредставлениеДоступностиОтбора = НСтр("ru='Я хочу настроить отбор';uk='Я прагну налаштувати відбір'");
					Иначе
				...
				Иначе
					Если НайденнаяСтрока.СписокЗначенийГлобальныхОтборов.Количество() > 0 Тогда
						...
						НайденнаяСтрока.ПредставлениеДоступностиОтбора = НСтр("ru='Отбор уже настроен администратором, настройка запрещена';uk='Відбір вже налаштован адміністратором, налаштування заборонене'");
					Иначе
						// Если администратор просто запретил редактировать, не вводя доп.ограничений - будут получены все новости.
						НайденнаяСтрока.ПредставлениеДоступностиОтбора = НСтр("ru='Эта настройка отбора запрещена администратором';uk='Це налаштування відбору заборонене адміністратором'");
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
