...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтаФорма.СписокНовостей_ИнтервалАвтообновления % (60*24*7) = 0 Тогда // 1 раз в неделю
		ЭтаФорма.СписокНовостей_ИнтервалАвтообновления_Представление = НСтр("ru='Список новостей обновляется еженедельно';uk='Список новин оновлюється щотижня'");
	ИначеЕсли ЭтаФорма.СписокНовостей_ИнтервалАвтообновления % (60*24) = 0 Тогда // 1 раз в день
		ЭтаФорма.СписокНовостей_ИнтервалАвтообновления_Представление = НСтр("ru='Список новостей обновляется ежедневно';uk='Список новин оновлюється щодня'");
	ИначеЕсли ЭтаФорма.СписокНовостей_ИнтервалАвтообновления % 60 = 0 Тогда // 1 раз в час
		ЭтаФорма.СписокНовостей_ИнтервалАвтообновления_Представление = НСтр("ru='Список новостей обновляется ежечасно';uk='Список новин оновлюється щогодини'");
	Иначе
		ЭтаФорма.СписокНовостей_ИнтервалАвтообновления_Представление =
			СтрШаблон(
				НСтр("ru='Список новостей обновляется каждые %1 минут';uk='Список новостей обновляется каждые %1 минут'"),
	...
	Если ЭтаФорма.ТекущийПользователь <> ПользователиКлиентСервер.ТекущийПользователь() Тогда
		ЭтаФорма.Заголовок =
			СтрШаблон(
				НСтр("ru='Настройки отображения новостей (%1)';uk='Настройки отображения новостей (%1)'"),
	...
	Если ТипЗнч(ПараметрыФормы) = ТипСтруктура Тогда
		...
		Иначе
			ПоказатьПредупреждение(
				...
				НСтр("ru='Вначале включите получение новостей по этой ленте.';uk='Спочатку включить одержання новин по цій стрічці.'"),
				0,
...
"';uk='Нет отборов, разрешенных для настройки пользователем"
"'"),
							СтрШаблон(
								НСтр("ru='Отборы, настроенные пользователем (можно изменить):"
"%1"
"';uk='Отборы, настроенные пользователем (можно изменить):"
"%1"
"'"),
								ПредставлениеПользовательскихОтборов));
...
"%1"
"';uk='Отборы, настроенные администратором (нельзя изменить):"
"%1"
"'"),
								ПредставлениеАдминистративныхОтборов));
...
"%1"
"';uk='Автоматически заполняемые отборы (нельзя изменить):"
"%1"
"'"),
								ПредставлениеАвтоматическиЗаполняемыхОтборов));

				Если ПустаяСтрока(ПредставлениеВсехОтборовПодробно) Тогда
					ПредставлениеВсехОтборовКратко   = НСтр("ru='Отборы не настроены';uk='Відбори не настроєні'");
					ПредставлениеВсехОтборовПодробно = НСтр("ru='Отборы не настроены';uk='Відбори не настроєні'");
				Иначе
...
"%1"
"';uk='По этой ленте нельзя отключить получение новостей (так настроено администратором)"
"%1"
"'"),
						ПредставлениеВсехОтборовПодробно);
			КонецЕсли;
