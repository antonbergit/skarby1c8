...
Процедура МенеджерыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("Представление",							НСтр("ru='менеджеры';uk='менеджери'"));
...
Процедура ДиаграммаНовыеОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ВРег(Расшифровка) = ВРег("Новые") Тогда
		ПризнакРасшифровки = НСтр("ru='Новые';uk='Нові'");
		ЗаголовокФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Анализ базы контрагентов (расшифровка %1)';uk='Аналіз бази контрагентів (розшифрування %1)'"),
			НСтр("ru='новых';uk='нових'"));
	ИначеЕсли ВРег(Расшифровка) = ВРег("Существующие") Тогда
		ПризнакРасшифровки = НСтр("ru='Существующие';uk='Існуючі'");
		ЗаголовокФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Анализ базы контрагентов (расшифровка %1)';uk='Аналіз бази контрагентів (розшифрування %1)'"),
			НСтр("ru='существующих';uk='існуючих'"));
	КонецЕсли;
	
	ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 контрагенты за период: с %2 по %3%4%5%6';uk='%1 контрагенти за період: з %2 по %3%4%5%6'"),
		ПризнакРасшифровки,
...
Процедура ДиаграммаНовыеПоПериодамОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ИначеЕсли ВРег(ПараметрПериодичность) = ВРег(НСтр("ru='Неделя';uk='Тиждень'")) Тогда
		ПредставлениеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='неделю с %1 по %2';uk='тиждень із %1 по %2'"),
			Формат(Расшифровка, "ДФ=dd.MM.yyyy"),
			Формат(КонецНедели(Расшифровка), "ДФ=dd.MM.yyyy"));
	ИначеЕсли ВРег(ПараметрПериодичность) = ВРег(НСтр("ru='Месяц';uk='Місяць'")) Тогда
		ПредставлениеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='месяц с %1 по %2';uk='місяць із %1 по %2'"),
			Формат(Расшифровка, "ДФ=dd.MM.yyyy"),
			Формат(КонецМесяца(Расшифровка), "ДФ=dd.MM.yyyy"));
	КонецЕсли;
	
	ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Новые контрагенты за %1 %2%3%4';uk='Нові контрагенти за %1 %2%3%4'"),
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ru='Анализ базы контрагентов (расшифровка новых за период)';uk='Аналіз бази контрагентів (розшифрування нових за період)'"));
	ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
...
Процедура ДиаграммаПоМенеджерамОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Контрагенты менеджера: %1';uk='Контрагенти менеджера: %1'"),
		?(ТипЗнч(Расшифровка) = Тип("Массив"), "<сводно>", Расшифровка),
		?(ЗначениеЗаполнено(ОтборТег), ", с тегом: " + ОтборТег, ""),
		?(ЗначениеЗаполнено(ОтборСегмент), ", в сегменте: " + ОтборСегмент, ""));
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ru='Анализ базы контрагентов (расшифровка по менеджерам)';uk='Аналіз бази контрагентів (розшифрування по менеджерах)'"));
	ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
...
Процедура ОбновитьДиаграммуНовые()
	...
	Если ЗначениеЗаполнено(ОтборПериодЗначение.ДатаНачала) И ЗначениеЗаполнено(ОтборПериодЗначение.ДатаОкончания) Тогда
		Элементы.ЗаголовокНовые.Заголовок = НСтр("ru='Новых за: ';uk='Нових за:'") + Формат(ОтборПериодЗначение.ДатаНачала, "ДФ=dd.MM.yy") + " - " + Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yy");
	Иначе
		Элементы.ЗаголовокНовые.Заголовок = НСтр("ru='Новых за: (период не указан)';uk='Нових за: (період не зазначений)'");
	КонецЕсли;
	...
	Подсказка = НСтр("ru='Создано новых контрагентов за период: ';uk='Створене нових контрагентів за період:'") + Новых;
	...
	Подсказка = НСтр("ru='Существующих контрагентов: ';uk='Існуючих контрагентів:'") + Существующих;
...
Процедура ОбновитьДиаграммуНовыеПоПериодам()
	...
	ПараметрПериодичностьПредставление = Строка(ПараметрПериодичность);	
		
    Если ПараметрПериодичность = Перечисления.Периодичность.Неделя Тогда
		ПараметрПериодичностьПредставление = "Неделя";
	ИначеЕсли ПараметрПериодичность = Перечисления.Периодичность.Месяц Тогда
		ПараметрПериодичностьПредставление = "Месяц";
	ИначеЕсли ПараметрПериодичность = Перечисления.Периодичность.Год Тогда
		ПараметрПериодичностьПредставление = "Год";
	ИначеЕсли ПараметрПериодичность = Перечисления.Периодичность.Полугодие Тогда
		ПараметрПериодичностьПредставление = "Полугодие";
	КонецЕсли; 	
		
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "НЕДЕЛЯ", ВРег(ПараметрПериодичностьПредставление));
	
	Если ЗначениеЗаполнено(ОтборТег) Или ЗначениеЗаполнено(ОтборСегмент) Или ОтборМенеджеры.Количество() > 0 Тогда
	...
	Пока ВыборкаДатаСоздания.Следующий() Цикл
		...
		Подсказка = НСтр("ru='Создано ';uk='Створене'") + ВыборкаДатаСоздания.Новый + НСтр("ru=' контрагентов ';uk=' контрагентів'") + Текст;
...
Процедура ОбновитьДиаграммуПоМенеджерам()
	...
	Пока ВыборкаОтветственные.Следующий() Цикл
		...
		Если ПорядковыйНомерСерии < МаксимумСерийДиаграммы Тогда
			...
			ПредставлениеМенеджера = ?(ЗначениеЗаполнено(ВыборкаОтветственные.Ответственный), ВыборкаОтветственные.ОтветственныйПредставление, НСтр("ru='<Менеджер не указан>';uk='<Менеджер не зазначен>'"));
			Серия = ДиаграммаПоМенеджерам.Серии.Добавить(ПредставлениеМенеджера);
			Подсказка = ПредставлениеМенеджера + НСтр("ru=', контрагентов: ';uk=', контрагентів:'") + ВыборкаОтветственные.Контрагенты;
		...
		Иначе
			...
			ПредставлениеМенеджера = НСтр("ru='<Остальные>';uk='<Інші>'");
			...
			Подсказка = ПредставлениеМенеджера + НСтр("ru=', контрагентов: ';uk=', контрагентів:'") + (СводноеЗначение.Значение + ВыборкаОтветственные.Контрагенты);
			...
			ТекстПодробно = ?(ЗначениеЗаполнено(ВыборкаОтветственные.Ответственный), ВыборкаОтветственные.ОтветственныйПредставление, НСтр("ru='<Менеджер не указан>';uk='<Менеджер не зазначен>'"))
				+ НСтр("ru=', контрагентов: ';uk=', контрагентів:'") + ВыборкаОтветственные.Контрагенты;
