...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если Область.Имя = "ОКВЭД" Тогда
		...
		Если НЕ ПустаяСтрока(Область.Значение) И НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(Область.Значение) Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='На титульном листе задано некорректное значение кода вида деятельности по ОКВЭД!%1Допустимы значения, соответствующие следующим шаблонам:%2""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра).';uk='На титульному аркуші задано некоректне значення коду виду діяльності за КВЕД!%1Допустимі значення, відповідні наступним шаблонам:%2""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - будь-яка цифра).'"), Символы.ПС, Символы.ПС);
			Сообщение.Сообщить();
...
Процедура ПроверитьКС_2(СтруктураРезультата)
	...
	ОписаниеКС = НСтр("ru='Значение базовой доходности на единицу физического показателя (строка 040 Раздела 2) должно соответствовать значению из Справочника ""Коды видов предпринимательской деятельности и значения базовой доходности на единицу физического показателя""';uk='Значение базовой доходности на единицу физического показателя (строка 040 Раздела 2) должно соответствовать значению из Справочника ""Коды видов предпринимательской деятельности и значения базовой доходности на единицу физического показателя""'");
	
	Нарушение      = НСтр("ru='Неправильное применение значений базовой доходности на единицу физического показателя';uk='Неправильное применение значений базовой доходности на единицу физического показателя'");
	ОписаниеОшибки = НСтр("ru=''");
...
Процедура ПроверитьКС_4(СтруктураРезультата)
	...
	ОписаниеКС   = НСтр("ru='Сумма исчисленного единого налога на вмененный доход (строка 110 Раздела 2) должна быть равна произведению налоговой базы (строка 100 Раздела 2) и налоговой ставки 15% по одноименному ОКТМО.';uk='Сумма исчисленного единого налога на вмененный доход (строка 110 Раздела 2) должна быть равна произведению налоговой базы (строка 100 Раздела 2) и налоговой ставки 15% по одноименному ОКТМО.'");
