...
Процедура ОбработатьДополнительнуюСрокуДаты(ИмяОбласти)
	...
	Если НомерГиперссылки < МногострочнаяЧасть.Количество() Тогда
		...
		ТекстВопроса = НСтр("ru='Удалить выбранную дату?';uk='Удалить выбранную дату?'");
	...
	Если НЕ ВПрограммеИзмененаДатаПодписи Тогда
		...
		Если НЕ СтруктураРеквизитовФормы.ИП Тогда
			...
			Если РегистрацииВНО.Количество() = 0 Тогда
				...
				КодНО = "";
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Не найдена регистрация организации в налоговом органе с цифрами на 5-6 позициях в КПП организации ""35"" или ""77""';uk='Не найдена регистрация организации в налоговом органе с цифрами на 5-6 позициях в КПП организации ""35"" или ""77""'");
				Сообщение.Сообщить();
			...
			Иначе
				...
				Иначе
					...
					КодНО = "";
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = НСтр("ru='Найдено более 1-й регистрации организации в налоговом органе с цифрами на 5-6 позициях в КПП организации ""35"" или ""77"". Укажите код налогового органа на титульном листе.';uk='Найдено более 1-й регистрации организации в налоговом органе с цифрами на 5-6 позициях в КПП организации ""35"" или ""77"". Укажите код налогового органа на титульном листе.'");
					Сообщение.Сообщить();
	...
	Для Каждого Лист Из Форма.ТаблицаСтраницыРаздела2 Цикл
		...
		Если Не ЗначениеЗаполнено(ДанныеТаблПоле.Значение.П000020003000) Тогда
			...
			Если Форма.СтруктураРеквизитовФормы.СообщатьОНезаполненномКодеПоОКТМОВРазделе2 Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Не заполнен код ОКТМО на странице Раздела 2.';uk='Не заполнен код ОКТМО на странице Раздела 2.'");
				Сообщение.Сообщить();
			КонецЕсли;
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если СтрНайти(Область.Имя, "П000010001003") > 0 ИЛИ Область.Имя = "П000020003000" Тогда
		...
		Если НЕ (ДлинаОКТМО = 0 ИЛИ ДлинаОКТМО = 8 ИЛИ ДлинаОКТМО = 11) Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Код по ОКТМО должен состоять из 8 или 11 цифр.';uk='Код по ОКТМО должен состоять из 8 или 11 цифр.'");
			Сообщение.Сообщить();
	...
	Если Область.Имя = "ОКВЭД" Тогда
		...
		Если НЕ ПустаяСтрока(Область.Значение) И НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(Область.Значение) Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='На титульном листе задано некорректное значение кода вида деятельности по ОКВЭД.%1Допустимы значения, соответствующие следующим шаблонам:%2""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра).';uk='На титульном листе задано некорректное значение кода вида деятельности по ОКВЭД.%1Допустимы значения, соответствующие следующим шаблонам:%2""XX"", ""XX.X"", ""XX.XX"", ""XX.XX.X"", ""XX.XX.XX"" (X - любая цифра).'"), Символы.ПС, Символы.ПС);
			Сообщение.Сообщить();
	...
	Если Область.Имя = "П000020010503" Тогда
		...
		Если НЕ (Область.Значение >= 7.5 И Область.Значение <= 15) Тогда
			...
			ПоказатьПредупреждение(,НСтр("ru='Ставка единого налога может быть установлена в пределах от 7.5 до 15 процентов.';uk='Ставка единого налога может быть установлена в пределах от 7.5 до 15 процентов.'"));
...
Процедура ЗаполнитьАвтоЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	Если НЕ СтруктураРеквизитовФормы.ИП Тогда
		Если НЕ ЗначениеЗаполнено(КПП) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Для заполнения отчета необходимо значение КПП налогоплательщика. Внесите исправление в справочник организаций и обновите сведения в декларации (меню ""Еще"" - ""Обновить"").';uk='Для заполнения отчета необходимо значение КПП налогоплательщика. Внесите исправление в справочник организаций и обновите сведения в декларации (меню ""Еще"" - ""Обновить"").'");
			Сообщение.Сообщить();
			СтатусПроверки = Ложь;
		ИначеЕсли СтрДлина(КПП) <> 9 Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Задан КПП налогоплательщика, не состоящий из 9 символов. Внесите исправление в справочник организаций и обновите сведения в декларации (меню ""Еще"" - ""Обновить"").';uk='Задан КПП налогоплательщика, не состоящий из 9 символов. Внесите исправление в справочник организаций и обновите сведения в декларации (меню ""Еще"" - ""Обновить"").'");
			Сообщение.Сообщить();
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(КодНО) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не задан код налогового органа, в который представляется декларация.';uk='Не задан код налогового органа, в который представляется декларация.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(КодНО) <> 4 ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(КодНО) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Задан код налогового органа, не состоящий из 4 цифр.';uk='Задан код налогового органа, не состоящий из 4 цифр.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если НЕ СтатусПроверки Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Заполнение отчета прервано.';uk='Заполнение отчета прервано.'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Для налогоплательщика по %1 не найдена соответствующая запись в справочнике ""Регистрации в налоговом органе"". Заполнение декларации прервано.';uk='Для налогоплательщика по %1 не найдена соответствующая запись в справочнике ""Регистрации в налоговом органе"". Заполнение декларации прервано.'"),
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Если НЕ РегламентированнаяОтчетность.ПринтерДоступен() Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Перед формированием печатных форм необходимо определить в системе принтер и задать его в качестве используемого по умолчанию.';uk='Перед формированием печатных форм необходимо определить в системе принтер и задать его в качестве используемого по умолчанию.'");
		Сообщение.Сообщить();
...
Процедура ПроверитьКС_1(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru='Проверка раздела 1 (КС 1)';uk='Проверка раздела 1 (КС 1)'");
...
"всех Разделов 2 (по ОКТМО = %1) и суммы налога, исчисленного по всем ОКТМО (строка 010 Раздела 3)';uk='Сумма единого налога на вмененный доход, подлежащая уплате за налоговый период (строка 020 Раздел1 по ОКТМО = %1)"
"должна быть равна произведению суммы налога, подлежащей уплате в бюджет (строка 040 Раздела 3) и результата деления суммы строк 110"
"всех Разделов 2 (по ОКТМО = %1) и суммы налога, исчисленного по всем ОКТМО (строка 010 Раздела 3)'");
	
	Нарушение      = НСтр("ru='Арифметическая ошибка';uk='Арифметическая ошибка'");
...
Процедура ПроверитьКС_2(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru='Проверка раздела 2 (КС 2)';uk='Проверка раздела 2 (КС 2)'");
	ОписаниеКС = НСтр("ru='Код вида предпринимательской деятельности (строка 010 Раздела 2) должен соответствовать значению из справочника ""Коды видов предпринимательской деятельности и значения базовой доходности на единицу физического показателя""';uk='Код вида предпринимательской деятельности (строка 010 Раздела 2) должен соответствовать значению из справочника ""Коды видов предпринимательской деятельности и значения базовой доходности на единицу физического показателя""'");
	
	Нарушение      = НСтр("ru='Статья 346.27 НК РФ';uk='Статья 346.27 НК РФ'");
	ОписаниеОшибки = НСтр("ru='Неправомерное применение системы налогообложения в виде ЕНВД';uk='Неправомерное применение системы налогообложения в виде ЕНВД'");
...
Процедура ПроверитьКС_1_3(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru='Проверка раздела 2 (КС 1.3)';uk='Проверка раздела 2 (КС 1.3)'");
	ОписаниеКС   = НСтр("ru='Налоговая база в 1-ом месяце отчетного периода (строка 070 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 070 графы 2, строки 070 графы 3 деленое на количество календарных дней в 1-ом месяце отчетного периода';uk='Налоговая база в 1-ом месяце отчетного периода (строка 070 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 070 графы 2, строки 070 графы 3 деленое на количество календарных дней в 1-ом месяце отчетного периода'");
	
	Нарушение      = НСтр("ru='Пункт 10 статьи 346.29 НК РФ';uk='Пункт 10 статьи 346.29 НК РФ'");
	ОписаниеОшибки = НСтр("ru='Неправильное исчисление налоговой базы за данный месяц';uk='Неправильное исчисление налоговой базы за данный месяц'");
...
Процедура ПроверитьКС_1_4(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru='Проверка раздела 2 (КС 1.4)';uk='Проверка раздела 2 (КС 1.4)'");
	ОписаниеКС   = НСтр("ru='Налоговая база во 2-ом месяце отчетного периода (строка 080 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 080 графы 2, строки 080 графы 3 деленое на количество календарных дней во 2-ом месяце отчетного периода';uk='Налоговая база во 2-ом месяце отчетного периода (строка 080 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 080 графы 2, строки 080 графы 3 деленое на количество календарных дней во 2-ом месяце отчетного периода'");
...
Процедура ПроверитьКС_1_5(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru='Проверка раздела 2 (КС 1.5)';uk='Проверка раздела 2 (КС 1.5)'");
	ОписаниеКС   = НСтр("ru='Налоговая база в 3-ем месяце отчетного периода (строка 090 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 090 графы 2, строки 090 графы 3 деленое на количество календарных дней в 3-ем месяце отчетного периода';uk='Налоговая база в 3-ем месяце отчетного периода (строка 090 графа 4 Раздела 2) должна быть равна произведению значений строк 040, 050, 060, строки 090 графы 2, строки 090 графы 3 деленое на количество календарных дней в 3-ем месяце отчетного периода'");
...
Процедура ПроверитьКС_3(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru='Проверка раздела 2 (КС 3)';uk='Проверка раздела 2 (КС 3)'");
	ОписаниеКС   = НСтр("ru='Сумма налоговой базы по ЕНВД (строка 100 Раздела 2) должна быть равна сумме значений по строкам 070, 080, 090 графы 4 Раздела 2 по одноименному ОКТМО (строка 030)';uk='Сумма налоговой базы по ЕНВД (строка 100 Раздела 2) должна быть равна сумме значений по строкам 070, 080, 090 графы 4 Раздела 2 по одноименному ОКТМО (строка 030)'");
	
	Нарушение      = НСтр("ru='п.п. 2, 4 ст.346.29 НК РФ';uk='п.п. 2, 4 ст.346.29 НК РФ'");
	ОписаниеОшибки = НСтр("ru='Неправильно исчислена налоговая база за налоговый период.';uk='Неправильно исчислена налоговая база за налоговый период.'");
...
Процедура ПроверитьКС_4(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru='Проверка раздела 2 (КС 4)';uk='Проверка раздела 2 (КС 4)'");
	ОписаниеКС   = НСтр("ru='Сумма исчисленного единого налога на вмененный доход (строка 110 Раздела 2) должна быть равна произведению налоговой базы (строка 100 Раздела 2) и налоговой ставки по одноименному ОКТМО (строка 105 Раздела 2) деленному на 100';uk='Сумма исчисленного единого налога на вмененный доход (строка 110 Раздела 2) должна быть равна произведению налоговой базы (строка 100 Раздела 2) и налоговой ставки по одноименному ОКТМО (строка 105 Раздела 2) деленному на 100'");
	
	Нарушение      = НСтр("ru='п.п. 2, 4 ст.346.29 и ст.246.31 НК РФ';uk='п.п. 2, 4 ст.346.29 и ст.246.31 НК РФ'");
	ОписаниеОшибки = НСтр("ru='Неправильное исчисление суммы ЕНВД за налоговый период';uk='Неправильное исчисление суммы ЕНВД за налоговый период'");
...
Процедура ПроверитьКС_5(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru='Проверка раздела 3 (КС 5)';uk='Проверка раздела 3 (КС 5)'");
	ОписаниеКС   = НСтр("ru='Сумма исчисленного единого налога на вмененный доход (строка 010 Раздел 3) должна быть равна сумме строк 110 всех заполненных Разделов 2';uk='Сумма исчисленного единого налога на вмененный доход (строка 010 Раздел 3) должна быть равна сумме строк 110 всех заполненных Разделов 2'");
...
Процедура ПроверитьКС_6_1(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru='Проверка раздела 3 (КС 6.1)';uk='Проверка раздела 3 (КС 6.1)'");
	ОписаниеКС   = НСтр("ru='Сумма единого налога на вмененный доход, подлежащий к уплате (строка 040 Раздела 3) должна быть равна разнице значений по строке 010 Раздела 3 и по строке 020 Раздела 3, если результат такого вычитания больше или равен 50% строки 010 Раздела 3 и строка 005 Раздела 3 = ""1""';uk='Сумма единого налога на вмененный доход, подлежащий к уплате (строка 040 Раздела 3) должна быть равна разнице значений по строке 010 Раздела 3 и по строке 020 Раздела 3, если результат такого вычитания больше или равен 50% строки 010 Раздела 3 и строка 005 Раздела 3 = ""1""'");
...
Процедура ПроверитьКС_6_2(СтруктураРезультата)
	...
	ТекстФормулы = НСтр("ru='Проверка раздела 3 (КС 6.2)';uk='Проверка раздела 3 (КС 6.2)'");
	ОписаниеКС   = НСтр("ru='Сумма единого налога на вмененный доход, подлежащий к уплате (строка 040 Раздела 3) должна быть равна разнице значений по строкам 010 Раздела 3 и по строке 030 Раздела 3, если результат такого вычитания больше или равен нулю и строка 005 Раздела 3 = ""2""';uk='Сумма единого налога на вмененный доход, подлежащий к уплате (строка 040 Раздела 3) должна быть равна разнице значений по строкам 010 Раздела 3 и по строке 030 Раздела 3, если результат такого вычитания больше или равен нулю и строка 005 Раздела 3 = ""2""'");
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	Если УсловныйКодПоказателя = "КодОКТМО" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru='Код по ОКТМО (строка 010 раздела 1)';uk='Код по ОКТМО (строка 010 раздела 1)'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр010" Тогда
		Результат.Вставить("Имя", "П000030001003");
		Результат.Вставить("Представление", НСтр("ru='Сумма исчисленного за налоговый период единого налога на вмененный доход';uk='Сумма исчисленного за налоговый период единого налога на вмененный доход'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р1Стр020" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru='Сумма единого налога на вмененный доход, подлежащая уплате за налоговый период (строка 020 раздела 1)';uk='Сумма единого налога на вмененный доход, подлежащая уплате за налоговый период (строка 020 раздела 1)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр040" Тогда
		Результат.Вставить("Имя", "П000030004003");
		Результат.Вставить("Представление", НСтр("ru='Общая сумма единого налога на вмененный доход, подлежащая уплате в бюджет за налоговый период';uk='Общая сумма единого налога на вмененный доход, подлежащая уплате в бюджет за налоговый период'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр110" Тогда
		Результат.Вставить("Имя", "П000020011003");
		Результат.Вставить("Представление", НСтр("ru='Сумма исчисленного единого налога на вмененный доход за налоговый период';uk='Сумма исчисленного единого налога на вмененный доход за налоговый период'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр100" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru='Налоговая база всего';uk='Налоговая база всего'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр070" Тогда
		Результат.Вставить("Имя", "П000020007004");
		Результат.Вставить("Представление", НСтр("ru='Налоговая база в 1 месяце квартала';uk='Налоговая база в 1 месяце квартала'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр080" Тогда
		Результат.Вставить("Имя", "П000020008004");
		Результат.Вставить("Представление", НСтр("ru='Налоговая база во 2 месяце квартала';uk='Налоговая база во 2 месяце квартала'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр090" Тогда
		Результат.Вставить("Имя", "П000020009004");
		Результат.Вставить("Представление", НСтр("ru='Налоговая база в 3 месяце квартала';uk='Налоговая база в 3 месяце квартала'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр020" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru='Сумма страховых взносов, на которые может быть уменьшена сумма исчисленного за налоговый период налога';uk='Сумма страховых взносов, на которые может быть уменьшена сумма исчисленного за налоговый период налога'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр030" Тогда
		Результат.Вставить("Имя", "П000030003003");
		Результат.Вставить("Представление", НСтр("ru='Сумма стаховых взносов, уплаченных индивидуальным предпринимателем в Пенсионный фонд Российской Федерации и в Федеральный фонд обязательного медицинского страхования в фиксированном размере';uk='Сумма стаховых взносов, уплаченных индивидуальным предпринимателем в Пенсионный фонд Российской Федерации и в Федеральный фонд обязательного медицинского страхования в фиксированном размере'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр005" Тогда
		Результат.Вставить("Имя", "П000030000503");
		Результат.Вставить("Представление", НСтр("ru='Признак налогоплательщика';uk='Ознака платника податків'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр040" Тогда
		Результат.Вставить("Имя", "П000020004003");
		Результат.Вставить("Представление", НСтр("ru='Базовая доходность на единицу физического показателя в месяц';uk='Базовая доходность на единицу физического показателя в месяц'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр010" Тогда
		Результат.Вставить("Имя", "П000020001000");
		Результат.Вставить("Представление", НСтр("ru='Код вида предпринимательской деятельности';uk='Код виду підприємницької діяльності'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр050" Тогда
		Результат.Вставить("Имя", "П000020005003");
		Результат.Вставить("Представление", НСтр("ru='Корректирующий коэффициент К1';uk='Корректирующий коэффициент К1'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр060" Тогда
		Результат.Вставить("Имя", "П000020006003");
		Результат.Вставить("Представление", НСтр("ru='Корректирующий коэффициент К2';uk='Корректирующий коэффициент К2'"));
		Результат.Вставить("ИзменяемаяРазмерность", Ложь);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр070Гр02" Тогда
		Результат.Вставить("Имя", "П000020007002");
		Результат.Вставить("Представление", НСтр("ru='Величина физического показателя';uk='Величина физического показателя'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр070Гр03" Тогда
		Результат.Вставить("Имя", "П000020007003");
		Результат.Вставить("Представление", НСтр("ru='Количество календарных дней';uk='Количество календарных дней'"));
