...
Процедура ВывестиНаПечатьРазделы12(СтруктураПараметров, ОбщиеРеквизитыРазделов, СтрПоля)
	...
	Для НомЛиста = 1 По КолЛистовРазд12 Цикл
		...
		ТабличныйДокумент.Вывести(Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы,
			...
			СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел1"));
		...
		ТабличныйДокумент.Вывести(Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы,
			СтрНайти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы,
			СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел2"));
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СпискиВыбора2016Кв1");
...
Процедура ВыгрузитьОтчет() Экспорт
	...
	Если НЕ ЗначениеЗаполнено(СтруктураРеквизитовФормы.Организация) Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Организация не выбрана! Выгрузка невозможна.';uk='Организация не выбрана! Выгрузка невозможна.'");
		Сообщение.Сообщить();
	...
	Если ИмяРаздела = "Титульный" Тогда
		...
		Если Область.Имя = "ОКТМО" Тогда
			...
			Если ЗначениеЗаполнено(ОКТМО) И НЕ (СтрДлина(ОКТМО) = 8 ИЛИ СтрДлина(ОКТМО) = 11) Тогда
				Область.Очистить();
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Длина кода по ОКТМО должна быть равна 8 или 11. Значение кода по ОКТМО очищено!';uk='Длина кода по ОКТМО должна быть равна 8 или 11. Значение кода по ОКТМО очищено!'");
				Сообщение.Сообщить();
			Иначе
...
Процедура ЗаполнитьАвтоЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	Если НЕ СтруктураРеквизитовФормы.ИП Тогда
		Если НЕ ЗначениеЗаполнено(КПП) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Для заполнения отчета необходимо значение КПП налогового агента. Внесите исправление в справочник организаций и обновите сведения в расчете (меню ""Еще"" - ""Обновить"").';uk='Для заполнения отчета необходимо значение КПП налогового агента. Внесите исправление в справочник организаций и обновите сведения в расчете (меню ""Еще"" - ""Обновить"").'");
			Сообщение.Сообщить();
			СтатусПроверки = Ложь;
		ИначеЕсли СтрДлина(КПП) <> 9 Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Задан КПП налогового агента, не состоящий из 9 символов. Внесите исправление в справочник организаций и обновите сведения в расчете (меню ""Еще"" - ""Обновить"").';uk='Задан КПП налогового агента, не состоящий из 9 символов. Внесите исправление в справочник организаций и обновите сведения в расчете (меню ""Еще"" - ""Обновить"").'");
			Сообщение.Сообщить();
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(КодНО) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не задан код налогового органа, в который представляется расчет.';uk='Не задан код налогового органа, в который представляется расчет.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	...
	Если НЕ ЗначениеЗаполнено(ОКТМО) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не задан код по ОКТМО.';uk='Не задан код по ОКТМО.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(ОКТМО) <> 11 И СтрДлина(ОКТМО) <> 8 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Задан некорректный код по ОКТМО.';uk='Задан некорректный код по ОКТМО.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Иначе
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Для налогового агента по значениям%1 кода налогового органа и кода по ОКТМО не найдена соответствующая запись в справочнике ""Регистрации в налоговом органе"". Заполнение отчета прервано.';uk='Для налогового агента по значениям%1 кода налогового органа и кода по ОКТМО не найдена соответствующая запись в справочнике ""Регистрации в налоговом органе"". Заполнение отчета прервано.'"),
...
Процедура ПроверитьКС_1_1(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru='Раздел 1 (КС 1.1)';uk='Раздел 1 (КС 1.1)'");
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12 Тогда
		ОписаниеКС = НСтр("ru='Дата представления Расчета должна быть не позднее 1 апреля года, следующего за истекшим налоговым периодом';uk='Дата представления Расчета должна быть не позднее 1 апреля года, следующего за истекшим налоговым периодом'");
	Иначе
		ОписаниеКС = НСтр("ru='Дата представления Расчета должна быть не позднее последнего дня месяца, следующего за отчетным периодом';uk='Дата представления Расчета должна быть не позднее последнего дня месяца, следующего за отчетным периодом'");
	КонецЕсли;
	Нарушение = НСтр("ru='п. 1.2 ст. 126, п. 2 ст. 230 НК РФ';uk='п. 1.2 ст. 126, п. 2 ст. 230 НК РФ'");
	ОписаниеОшибки  = НСтр("ru='Возможно непредставление в установленный НК РФ срок Расчета';uk='Возможно непредставление в установленный НК РФ срок Расчета'");
...
Процедура ПроверитьКС_1_2(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru='Раздел 1 (КС 1.2)';uk='Раздел 1 (КС 1.2)'");
	ОписаниеКС      = НСтр("ru='Сумма начисленного дохода (строка 020 Раздела 1) должна быть больше или равна сумме налоговых вычетов (строка 030 Раздела 1)';uk='Сумма начисленного дохода (строка 020 Раздела 1) должна быть больше или равна сумме налоговых вычетов (строка 030 Раздела 1)'");
	Нарушение = НСтр("ru='ст. 126.1, ст. 210, ст. 23 НК РФ, ст. 24 НК РФ';uk='ст. 126.1, ст. 210, ст. 23 НК РФ, ст. 24 НК РФ'");
	ОписаниеОшибки  = НСтр("ru='Завышена сумма налоговых вычетов';uk='Завышена сумма налоговых вычетов'");
...
Процедура ПроверитьКС_1_4(СтруктураРезультата)
	...
	ТекстФормулы    = НСтр("ru='Раздел 1 (КС 1.4)';uk='Раздел 1 (КС 1.4)'");
	ОписаниеКС      = НСтр("ru='Сумма исчисленного налога (строка 040 Раздела 1) должна быть больше или равна сумме фиксированного авансового платежа (строка 050 Раздел 1)';uk='Сумма исчисленного налога (строка 040 Раздела 1) должна быть больше или равна сумме фиксированного авансового платежа (строка 050 Раздел 1)'");
	Нарушение = НСтр("ru='ст. 126.1, ст. 227.1. ст. 23 НК РФ, ст. 24 НК РФ';uk='ст. 126.1, ст. 227.1. ст. 23 НК РФ, ст. 24 НК РФ'");
	ОписаниеОшибки  = НСтр("ru='Завышена сумма фиксированного авансового платежа';uk='Завышена сумма фиксированного авансового платежа'");
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	Если УсловныйКодПоказателя = "ТитульныйДатаПодписи" Тогда
		...
		Результат.Вставить("Представление", Нстр("ru='Дата подписи';uk='Дата підпису'"));
...
Функция ПредставлениеПоказателя(ИмяПоказателя)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru='Графа %1 строка %2 %3';uk='Графа %1 строка %2 %3'"), ПодстрокаГрафы,
