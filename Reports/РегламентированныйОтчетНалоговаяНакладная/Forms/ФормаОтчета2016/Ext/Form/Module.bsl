...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("R01G7", "RXXXXG7");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("R01G8", "RXXXXG8");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("R01G9", "RXXXXG9");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("R01G10","RXXXXG10");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("R01G109","RXXXXG109");
	
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("R01G11","RXXXXG7, RXXXXG8, RXXXXG9, RXXXXG10, RXXXXG109");
	
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("R03G11","R03G7, R03G109, RXXXXG7, RXXXXG8, RXXXXG9, RXXXXG10, RXXXXG109");

	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("R04G7", "RXXXXG7, R03G7");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("R04G8", "RXXXXG8");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("R04G9", "RXXXXG9");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("R04G109", "RXXXXG109, R03G109");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("R04G10","RXXXXG10");
	
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("R04G11","RXXXXG7, RXXXXG8, RXXXXG9,  RXXXXG109, RXXXXG10, R02G11, R03G7, R03G109");
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ИНН                   = Сведения.ИННЮЛ;
	НомерСвидетельства    = Сведения.СвидСН;
	...
	ЗаполнитьПоказательНаСервере("C_DOC_CNT",  ПолучитьСледующийНомерДокументаВПериоде(Организация, Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), Форма, "СлужебныеДанные");
...
Процедура АктивизироватьЯчейку(Ячейка) Экспорт
	...
	Если ТипЗнч(Ячейка) = Тип("Структура") Тогда
		...
		Если Ячейка.Свойство("ИмяЯчейки") Тогда
			...
			ТекИмяЯчейки = СтрЗаменить(ТекИмяЯчейки, "RXXXXG105_2S", "RXXXXG1052S");
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Для каждого СтрокаТЧ Из ДанныеТаблицы Цикл
		...
		Для каждого Колонка Из ДанныеТаблицы.Колонки Цикл
			...
			Если ДанныеМногострочки.Колонки.Найти("RXXXX" + ИмяXSD) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			СтрокаМногострочки["RXXXX" + ИмяXSD] = СтрокаТЧ[ИмяКолонки];
...
Функция ПолучитьДанныеФормыНаСервере();
	...
	Для каждого ИдМногострочногоРаздела Из мСтруктураМногострочныхРазделов Цикл
		...
		Для каждого Строка Из Таблица Цикл
			...
			СтруктураДанных.Вставить("RXXXXG105_2S_" + НомерСтроки, Строка.RXXXXG1052S);
...
Функция СоздатьСтруктуруСоСхемамиВыгрузкиНаСервере()
	...
	Объект = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].Создать();
