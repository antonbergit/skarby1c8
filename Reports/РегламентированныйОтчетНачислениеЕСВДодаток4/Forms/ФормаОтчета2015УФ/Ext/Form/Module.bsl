...
Процедура ЗаполнитьПоказателиЗначениямиПоУмолчанию(НаимРаздела)
	...
	Если НаимРаздела = "ПереченьТаблиц" Тогда
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ЗаполнитьПоказательНаСервере("НазваниеОрганизации", Организация.НаименованиеПолное, Форма, "ПереченьТаблиц");
	
	АдресОрганизации      = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОбОрганизации,    "ЮридическийАдрес,",  Ложь);
	ТекТелефоныОрганизации = УправлениеНебольшойФирмойСервер.ОписаниеОрганизации(СведенияОбОрганизации,    "Телефоны,",          Ложь);
	ТекТелефоныОрганизации = УправлениеНебольшойФирмойСервер.ПолучитьТелефонДляНалоговойНакладной(ТекТелефоныОрганизации);
	
	ЗаполнитьПоказательНаСервере("Адрес", АдресОрганизации, Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("Телефон", ТекТелефоныОрганизации, Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("МестонахождениеОрганизации", АдресОрганизации, Форма, "ПереченьТаблиц");
	ЗаполнитьПоказательНаСервере("ТелефонОрганизации", ТекТелефоныОрганизации, Форма, "ПереченьТаблиц");
	
    КодЕДРПОУ = СведенияОбОрганизации.ЕДРПОУ;
	
	Для Инд = 1 По 10 Цикл
		ТекущееЗначениеИНН = Сред(КодЕДРПОУ, Инд, 1);
		
		ЗаполнитьПоказательНаСервере("ЕДРПОУ" + Строка(инд), Строка(ТекущееЗначениеИНН), Форма, "Таблица1");
		ЗаполнитьПоказательНаСервере("ЕДРПОУ" + Строка(инд), Строка(ТекущееЗначениеИНН), Форма, "ПереченьТаблиц");
	КонецЦикла;
	
	Мес = Формат(Месяц(Форма.СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета), "ЧЦ=2; ЧВН=; ЧГ=0");
	Год = Формат(Год(Форма.СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета), "ЧЦ=4; ЧГ=0");
	
	деньПодписи = Формат(День(Форма.ДатаПодписи), "ЧЦ=2; ЧВН=; ЧГ=0");
	месПодписи = Формат(Месяц(Форма.ДатаПодписи), "ЧЦ=2; ЧВН=; ЧГ=0");
	годПодписи = Формат(Год(Форма.ДатаПодписи), "ЧЦ=4; ЧГ=0");
	
	Для Инд = 1 По 2 Цикл
		ТекущееЗначениеМес = Сред(Мес, Инд, 1);
		
		ЗаполнитьПоказательНаСервере("Мес" + Строка(инд), Строка(ТекущееЗначениеМес), Форма, "Таблица1");
		ЗаполнитьПоказательНаСервере("Мес" + Строка(инд), Строка(ТекущееЗначениеМес), Форма, "ПереченьТаблиц");
	КонецЦикла;
	Для Инд = 1 По 4 Цикл
		ТекущееЗначениеГод = Сред(Год, Инд, 1);
		
		ЗаполнитьПоказательНаСервере("Год" + Строка(инд), Строка(ТекущееЗначениеГод), Форма, "Таблица1");
		ЗаполнитьПоказательНаСервере("Год" + Строка(инд), Строка(ТекущееЗначениеГод), Форма, "ПереченьТаблиц");
	КонецЦикла;
	
	ЗаполнитьПоказательНаСервере("Месяц", Мес, Форма, "ПереченьТаблиц");
	ЗаполнитьПоказательНаСервере("Год", Сред(Формат(Год,"ЧГ=0"), 3), Форма, "ПереченьТаблиц");
	
	ЗаполнитьПоказательНаСервере("ПериодОтчетаПриложение", ФОрмат(Форма.СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета,"Л=uk_UA; ДФ=ММММ") + " місяць " + ФОрмат(Форма.СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета,"Л=uk_UA; ДФ=yyyy") +" року", Форма, "Таблица1");
	
	ЗаполнитьПоказательНаСервере("НазваниеОрганизации", Организация.НаименованиеПолное, Форма, "Таблица1");
	
	Для Инд = 1 По 2 Цикл
		ТекущееЗначениеДень = Сред(деньПодписи, Инд, 1);
		ТекущееЗначениеМесяц = Сред(месПодписи, Инд, 1);
		
		ЗаполнитьПоказательНаСервере("деньПодписи" + Строка(инд), Строка(ТекущееЗначениеДень), Форма, "Таблица1");
		ЗаполнитьПоказательНаСервере("месПодписи" + Строка(инд), Строка(ТекущееЗначениеМесяц), Форма, "Таблица1");
	КонецЦикла;
	Для Инд = 1 По 4 Цикл
		ТекущееЗначениеГод = Сред(годПодписи, Инд, 1);
		
		ЗаполнитьПоказательНаСервере("годПодписи" + Строка(инд), Строка(ТекущееЗначениеГод), Форма, "Таблица1");
	КонецЦикла;
	
	Для Инд = 1 По 10 Цикл
		ТекущееЗначениеИННРук = Сред(СведенияОбОрганизации.ИННРук, Инд, 1);
		ТекущееЗначениеИННБух = Сред(СведенияОбОрганизации.ИННБух, Инд, 1);
		
		ЗаполнитьПоказательНаСервере("РуководительИНН" + Строка(инд), Строка(ТекущееЗначениеИННРук), Форма, "Таблица1");
		ЗаполнитьПоказательНаСервере("БухгалтерИНН" + Строка(инд), Строка(ТекущееЗначениеИННБух), Форма, "Таблица1");
	КонецЦикла;
	
	РуководительФИО = СведенияОбОрганизации.ФИОРук;
	БухгалтерФИО = СведенияОбОрганизации.ФИОБух + "          ";
	
	ЗаполнитьПоказательНаСервере("РуководительФИО", РуководительФИО, Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("БухгалтерФИО", БухгалтерФИО, Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("БухгалтерФИО_", БухгалтерФИО, Форма, "Таблица1");
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	ЗаполнитьПоказательНаСервере("П1_4", ПолучитьПоказательНаСервере("П1_4_1", Форма, "Таблица1"), Форма, "Таблица1");
	
	ЗаполнитьПоказательНаСервере("П1", ПолучитьПоказательНаСервере("П1_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П1_2", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П1_3", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П1_4", Форма, "Таблица1"), Форма, "Таблица1");
	
	П2_1_1 = ПолучитьПоказательНаСервере("П2_1_1", Форма, "Таблица1");
	
	ЗаполнитьПоказательНаСервере("П2_1", ПолучитьПоказательНаСервере("П2_1_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П2_1_2", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П2_1_3", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П2_1_4", Форма, "Таблица1")
									 + ПолучитьПоказательНаСервере("П2_1_5", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П2_1_6", Форма, "Таблица1"), Форма, "Таблица1");
									 
	ЗаполнитьПоказательНаСервере("П2_2", ПолучитьПоказательНаСервере("П2_2_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П2_2_2", Форма, "Таблица1"), Форма, "Таблица1");
	
	ЗаполнитьПоказательНаСервере("П2_3", ПолучитьПоказательНаСервере("П2_3_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П2_3_2", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П2_3_3", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П2_3_4", Форма, "Таблица1"), Форма, "Таблица1");
	
	ЗаполнитьПоказательНаСервере("П2", ПолучитьПоказательНаСервере("П2_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П2_2", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П2_3", Форма, "Таблица1"), Форма, "Таблица1");
	...
	П3_1_1 = П2_1_1 * Ставка1/100;
	
	ЗаполнитьПоказательНаСервере("П3_1_1", П3_1_1, Форма, "Таблица1");
	
	ЗаполнитьПоказательНаСервере("П3_1_2", ПолучитьПоказательНаСервере("П2_1_2", Форма, "Таблица1") * 0.363, Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П3_1_3", ПолучитьПоказательНаСервере("П2_1_3", Форма, "Таблица1") * 0.4596, Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П3_1_4", ПолучитьПоказательНаСервере("П2_1_4", Форма, "Таблица1") * 0.0841, Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П3_1_5", ПолучитьПоказательНаСервере("П2_1_5", Форма, "Таблица1") * 0.053, Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П3_1_6", ПолучитьПоказательНаСервере("П2_1_6", Форма, "Таблица1") * 0.055, Форма, "Таблица1");
	
	ЗаполнитьПоказательНаСервере("П3_1", ПолучитьПоказательНаСервере("П3_1_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П3_1_2", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П3_1_3", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П3_1_4", Форма, "Таблица1")
									 + ПолучитьПоказательНаСервере("П3_1_5", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П3_1_6", Форма, "Таблица1"), Форма, "Таблица1");
									 
	ЗаполнитьПоказательНаСервере("П3_2_1", ПолучитьПоказательНаСервере("П2_2_1", Форма, "Таблица1") * 0.347, Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П3_2_2", ПолучитьПоказательНаСервере("П2_2_2", Форма, "Таблица1") * 0.053, Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П3_2", ПолучитьПоказательНаСервере("П3_2_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П3_2_2", Форма, "Таблица1"), Форма, "Таблица1");
									 
	ЗаполнитьПоказательНаСервере("П3_3_1", ПолучитьПоказательНаСервере("П2_3_1", Форма, "Таблица1") * 0.332, Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П3_3_2", ПолучитьПоказательНаСервере("П2_3_2", Форма, "Таблица1") * 0.0841, Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П3_3_3", ПолучитьПоказательНаСервере("П2_3_3", Форма, "Таблица1") * 0.053, Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П3_3_4", ПолучитьПоказательНаСервере("П2_3_4", Форма, "Таблица1") * 0.055, Форма, "Таблица1");
	
	ЗаполнитьПоказательНаСервере("П3_3", ПолучитьПоказательНаСервере("П3_3_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П3_3_2", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П3_3_3", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П3_3_4", Форма, "Таблица1"), Форма, "Таблица1");
									 
	ЗаполнитьПоказательНаСервере("П3", ПолучитьПоказательНаСервере("П3_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П3_2", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П3_3", Форма, "Таблица1"), Форма, "Таблица1");
									 
	ЗаполнитьПоказательНаСервере("П4_1", ПолучитьПоказательНаСервере("П4_1_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П4_1_2", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П4_1_3", Форма, "Таблица1"), Форма, "Таблица1");
									 
	ЗаполнитьПоказательНаСервере("П4", ПолучитьПоказательНаСервере("П4_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П4_2", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П4_3", Форма, "Таблица1"), Форма, "Таблица1");
									 
	ЗаполнитьПоказательНаСервере("П5_1", ПолучитьПоказательНаСервере("П4_1_1", Форма, "Таблица1") * 0.036, Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П5_2", ПолучитьПоказательНаСервере("П4_1_2", Форма, "Таблица1") * 0.0285, Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П5_3", ПолучитьПоказательНаСервере("П4_1_3", Форма, "Таблица1") * 0.061, Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П5_4", ПолучитьПоказательНаСервере("П4_2", Форма, "Таблица1") * 0.026, Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П5_5", ПолучитьПоказательНаСервере("П4_3", Форма, "Таблица1") * 0.02, Форма, "Таблица1");
	
	ЗаполнитьПоказательНаСервере("П5", ПолучитьПоказательНаСервере("П5_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П5_2", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П5_3", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П5_4", Форма, "Таблица1")
									 + ПолучитьПоказательНаСервере("П5_5", Форма, "Таблица1"), Форма, "Таблица1");
	
	ЗаполнитьПоказательНаСервере("П6_1", ПолучитьПоказательНаСервере("П6_1_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_1_2", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П6_1_3", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_1_4", Форма, "Таблица1")
									 + ПолучитьПоказательНаСервере("П6_1_5", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_1_6", Форма, "Таблица1")
									 + ПолучитьПоказательНаСервере("П6_1_7", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_1_8", Форма, "Таблица1"), Форма, "Таблица1");
									 
	ЗаполнитьПоказательНаСервере("П6_3", ПолучитьПоказательНаСервере("П6_3_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_3_2", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П6_3_3", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_3_4", Форма, "Таблица1")
									 + ПолучитьПоказательНаСервере("П6_3_5", Форма, "Таблица1"), Форма, "Таблица1");
									 
	ЗаполнитьПоказательНаСервере("П6", ПолучитьПоказательНаСервере("П6_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_2", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П6_3", Форма, "Таблица1"), Форма, "Таблица1");
									 
	ЗаполнитьПоказательНаСервере("П7_1", ПолучитьПоказательНаСервере("П7_1_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П7_1_2", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П7_1_3", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П7_1_4", Форма, "Таблица1")
									 + ПолучитьПоказательНаСервере("П7_1_5", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П7_1_6", Форма, "Таблица1")
									 + ПолучитьПоказательНаСервере("П7_1_7", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П7_1_8", Форма, "Таблица1"), Форма, "Таблица1");
									 
	ЗаполнитьПоказательНаСервере("П7_3", ПолучитьПоказательНаСервере("П7_3_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П7_3_2", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П7_3_3", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П7_3_4", Форма, "Таблица1")
									 + ПолучитьПоказательНаСервере("П7_3_5", Форма, "Таблица1"), Форма, "Таблица1");
									 
	ЗаполнитьПоказательНаСервере("П7", ПолучитьПоказательНаСервере("П7_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П7_2", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П7_3", Форма, "Таблица1"), Форма, "Таблица1");
									 
	ЗаполнитьПоказательНаСервере("П8_1", ПолучитьПоказательНаСервере("П3_1_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_1_1", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П7_1_1", Форма, "Таблица1"), Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П8_2", ПолучитьПоказательНаСервере("П3_1_2", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_1_2", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П7_1_2", Форма, "Таблица1"), Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П8_3", ПолучитьПоказательНаСервере("П3_1_3", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_1_3", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П7_1_3", Форма, "Таблица1"), Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П8_4", ПолучитьПоказательНаСервере("П3_1_4", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_1_4", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П7_1_4", Форма, "Таблица1"), Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П8_5", ПолучитьПоказательНаСервере("П3_1_5", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_1_5", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П7_1_5", Форма, "Таблица1"), Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П8_6", ПолучитьПоказательНаСервере("П3_1_6", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_1_6", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П7_1_6", Форма, "Таблица1"), Форма, "Таблица1");
									 
	ЗаполнитьПоказательНаСервере("П8_7", ПолучитьПоказательНаСервере("П3_2", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_1_7", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П7_1_7", Форма, "Таблица1"), Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П8_8", ПолучитьПоказательНаСервере("П3_3", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_1_8", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П7_1_8", Форма, "Таблица1"), Форма, "Таблица1");
									 
	ЗаполнитьПоказательНаСервере("П8_9", ПолучитьПоказательНаСервере("П5_1", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_3_1", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П7_3_1", Форма, "Таблица1"), Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П8_10", ПолучитьПоказательНаСервере("П5_2", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_3_2", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П7_3_2", Форма, "Таблица1"), Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П8_11", ПолучитьПоказательНаСервере("П5_3", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_3_3", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П7_3_3", Форма, "Таблица1"), Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П8_12", ПолучитьПоказательНаСервере("П5_4", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_3_4", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П7_3_4", Форма, "Таблица1"), Форма, "Таблица1");
	ЗаполнитьПоказательНаСервере("П8_13", ПолучитьПоказательНаСервере("П5_5", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П6_3_5", Форма, "Таблица1")
	                                 + ПолучитьПоказательНаСервере("П7_3_5", Форма, "Таблица1"), Форма, "Таблица1");
									 
	ЗаполнитьПоказательНаСервере("П8_14", ПолучитьПоказательНаСервере("П6_2", Форма, "Таблица1") - ПолучитьПоказательНаСервере("П7_2", Форма, "Таблица1"), Форма, "Таблица1");
	
	ЗаполнитьПоказательНаСервере("П8", ПолучитьПоказательНаСервере("П3", Форма, "Таблица1") + ПолучитьПоказательНаСервере("П5", Форма, "Таблица1")
	                                 - ПолучитьПоказательНаСервере("П7", Форма, "Таблица1"), Форма, "Таблица1");
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	Если ИмяЯчейки = "Начальная" Тогда
		ТабличныйДокумент.Области.Начальная.Значение = "Х";
		ТабличныйДокумент.Области.Отменяющая.Значение = "";
		Если НаименованиеТекЛиста <> "Таблица6" Тогда
			ТабличныйДокумент.Области.Дополнительная.Значение = "";
		КонецЕсли; 
	ИначеЕсли ИмяЯчейки = "Отменяющая" Тогда
		ТабличныйДокумент.Области.Начальная.Значение = "";
		ТабличныйДокумент.Области.Отменяющая.Значение = "Х";
		Если НаименованиеТекЛиста <> "Таблица6" Тогда
			ТабличныйДокумент.Области.Дополнительная.Значение = "";
		КонецЕсли; 
	ИначеЕсли ИмяЯчейки = "Дополнительная" Тогда
		ТабличныйДокумент.Области.Начальная.Значение = "";
		ТабличныйДокумент.Области.Отменяющая.Значение = "";
		ТабличныйДокумент.Области.Дополнительная.Значение = "Х";
	ИначеЕсли  ВРЕГ(Лев(ИмяЯчейки, 6)) = "ЕДРПОУ" Тогда
		Если НаименованиеТекЛиста = "ПереченьТаблиц" Тогда
			Значение = ПолучитьПоказательНаСервере("ЕДРПОУ", ЭтаФорма, "ПереченьТаблиц");
			ОписаниеОповещения = Новый ОписаниеОповещения("ВводПоказателяВПолеТабличногоДокументЗавершение",
														   ЭтаФорма, Новый Структура("ИмяЯчейки, ИмяСтраницы,
																					 ...
																					 "ЕДРПОУ",  "ПереченьТаблиц,Таблица1",
																					 "ЕДРПОУ", "ПереченьТаблиц,Таблица1",
																					 10, Ложь, Неопределено));
			
			ПоказатьВводЧисла(ОписаниеОповещения,Значение, НСтр("ru='Введите значение';uk='Введіть значення'"), 10,0);
		КонецЕсли;
	ИначеЕсли  ВРЕГ(Лев(ИмяЯчейки, 10)) = "ЛИКВЕДРПОУ" Тогда
		Если НаименованиеТекЛиста = "ПереченьТаблиц" Тогда
			Значение = ПолучитьПоказательНаСервере("ЛиквЕДРПОУ", ЭтаФорма, "ПереченьТаблиц");
			ОписаниеОповещения = Новый ОписаниеОповещения("ВводПоказателяВПолеТабличногоДокументЗавершение",
														   ЭтаФорма, Новый Структура("ИмяЯчейки, ИмяСтраницы,
																					 |ИмяЯчейкиКвадратов, ИмяСтраницыКвадратов, 
																					 |Размерность, ДополнитьЗначение, НольКакНоль", 
																					 "ЛиквЕДРПОУ",  "ПереченьТаблиц,Таблица1",
																					 "ЛиквЕДРПОУ", "ПереченьТаблиц,Таблица1",
																					 10, Ложь, Неопределено));
			
			ПоказатьВводЧисла(ОписаниеОповещения,Значение, НСтр("ru='Введите значение';uk='Введіть значення'"), 10,0);
		КонецЕсли;
	ИначеЕсли  ВРЕГ(ИмяЯчейки) = ВРег("НужныИзмененияВПерсонификацииДа") Тогда
		ТабличныйДокумент.Области.OPERADD.Значение = 1;
	ИначеЕсли ВРЕГ(ИмяЯчейки) = Врег("НужныИзмененияВПерсонификацииНет") Тогда
		ТабличныйДокумент.Области.OPERADD.Значение = 0;
	ИначеЕсли ВРЕГ(Лев(ИмяЯчейки, 11)) = Врег("Коэффициент") Тогда
		
		Если ВРЕГ(Прав(ИмяЯчейки,2)) = "ДА" Тогда
			ТабличныйДокумент.Области.КоэффициентДа.Значение = "Х";
			ТабличныйДокумент.Области.КоэффициентНет.Значение = "";
		ИначеЕсли ВРЕГ(Прав(ИмяЯчейки,2)) = "ЕТ" Тогда
			
			ТабличныйДокумент.Области.КоэффициентДа.Значение = "";
			ТабличныйДокумент.Области.КоэффициентНет.Значение = "Х";
			ТабличныйДокумент.Области.СредняяЗП2014.Значение = 0;
			ТабличныйДокумент.Области.СреднееЗЛ2014.Значение = 0;
			
	        ЗаполнитьПоказательНаСервере("Коэффициент", 0, ЭтаФорма, "СлужебныеДанные");
		ИначеЕсли ЗначениеЗаполнено(ТабличныйДокумент.Области.КоэффициентДа.Значение) Тогда
			Коэффициент = ПолучитьПоказательНаСервере("Коэффициент", ЭтаФорма, "СлужебныеДанные");	
			
			ОписаниеОповещения = Новый ОписаниеОповещения("ВводПоказателяВПолеТабличногоДокументЗавершение",
														   ЭтаФорма, Новый Структура("ИмяЯчейки, ИмяСтраницы,
																					 |ИмяЯчейкиКвадратов, ИмяСтраницыКвадратов, 
																					 |Размерность, ДополнитьЗначение, НольКакНоль", 
																					 "Коэффициент",  "Таблица1",
																					 "Коэффициент", "Таблица1",
																					 4, Ложь, Неопределено));
			
			ПоказатьВводЧисла(ОписаниеОповещения,Коэффициент, НСтр("ru='Введите коэффициент (от 0,400 до 0,999)';uk='Введіть коеффіцієнт (від 0,400 до 0,999)'"), 5,3);
		КонецЕсли; 
	КонецЕсли;	
...
Процедура ВводПоказателяВПолеТабличногоДокументЗавершение(Значение, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Значение) = Тип("Число") И ИмяЯчейки <> "Коэффициент" Тогда
		Значение = Формат(Значение, "ЧГ=0");
	КонецЕсли;
	
	Если ИмяЯчейки = "Коэффициент" Тогда
		Если Значение > 0.999
			ИЛИ Значение < 0.4 Тогда
			Значение = "";
		Иначе
			Значение = "0" + Сред(Значение, 3);
		КонецЕсли;
		Для Инд = 1 По 4 Цикл
			ТекущееЗначение = Сред(Значение, Инд, 1);
			
			ЗаполнитьПоказательНаСервере("Коэффициент" + Строка(инд), Строка(ТекущееЗначение), ЭтаФорма, "Таблица1");
		КонецЦикла;
	Иначе
...
Процедура ВыгрузитьЛистВDBF(СтруктураПараметровЛиста)
	...
	ИначеЕсли ИмяСтраницы = "Таблица6" Тогда
		...
		Для каждого СтрокаГруппы Из СтруктураГруппы Цикл
			...
			Если ЗначениеЗаполнено(СтрокаГруппы["T2RXXXXG111D"]) Тогда
				докЭкспорт.PAY_MNTH 	= Месяц(СтрокаГруппы["T2RXXXXG111D"]);
				докЭкспорт.PAY_YEAR 	= Год(СтрокаГруппы["T2RXXXXG111D"]);
			КонецЕсли;
	...
	ИначеЕсли ИмяСтраницы = "Таблица7" Тогда
		...
		Для каждого СтрокаГруппы Из СтруктураГруппы Цикл
			...
			докЭкспорт.START_DT	= День(СтрокаГруппы["T3RXXXXG10"]);
			докЭкспорт.STOP_DT	= День(СтрокаГруппы["T3RXXXXG11"]);
			
			докЭкспорт.HH		= СтрокаГруппы["T3RXXXXG131"];
			докЭкспорт.MM		= СтрокаГруппы["T3RXXXXG132"];
			докЭкспорт.SEAZON	= СтрокаГруппы["T3RXXXXG17"];
			докЭкспорт.NORMA	= СтрокаГруппы["T3RXXXXG14"];
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	// 1.1
	П1_1 = ПолучитьОборотВзаиморасчетыПоВзносамВФонды(Справочники.ВидыНачисленийИУдержаний.ЕСВ_работник);
	УстановитьЗначениеПоказателя("П1_1", П1_1);
	
	// 2.1.1
	П2_1_1 = ПолучитьОборотВзаиморасчетыПоВзносамВФонды(Справочники.ВидыНачисленийИУдержаний.ЕСВ_ФОТ);
	УстановитьЗначениеПоказателя("П2_1_1", П2_1_1);
	
	// 2.1.4
	П2_1_4 = ПолучитьОборотВзаиморасчетыПоВзносамВФонды(Справочники.ВидыНачисленийИУдержаний.ЕСВ_ФОТ_инв);
	УстановитьЗначениеПоказателя("П2_1_4", П2_1_4);
	
	// 4.1.1
	УстановитьЗначениеПоказателя("П4_1_1", П1_1);
...
Процедура СформироватьНачисленияВзносыТаблица6(СписокРаботниковСоСпецСтажем)
	...
	Пока ВыборкаТаблица6.Следующий() Цикл
		...
		Строка["T2RXXXXG111D"]  = НачалоМесяца(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	...
	ЗаполнитьПоказательНаСервере("СуммаНачисленоВсего", СтруктураГруппы.Итог("T2RXXXXG17"), ЭтаФорма, "Таблица6");
	ЗаполнитьПоказательНаСервере("СуммаНачисленоМаксБазаВсего", СтруктураГруппы.Итог("T2RXXXXG18"), ЭтаФорма, "Таблица6");
	ЗаполнитьПоказательНаСервере("СуммаРазницыВсего", СтруктураГруппы.Итог("T2RXXXXG19"), ЭтаФорма, "Таблица6");
	ЗаполнитьПоказательНаСервере("СуммаВзносовУдержаноВсего", СтруктураГруппы.Итог("T2RXXXXG20"), ЭтаФорма, "Таблица6");
	ЗаполнитьПоказательНаСервере("СуммаВзносовНачисленоВсего", СтруктураГруппы.Итог("T2RXXXXG21"), ЭтаФорма, "Таблица6");
