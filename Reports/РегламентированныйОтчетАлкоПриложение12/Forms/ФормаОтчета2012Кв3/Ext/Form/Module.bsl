...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если ЭтоКППМестаДеятельности Тогда
		...
		Иначе
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='КПП должен состоять из 9 цифр.';uk='КПП должен состоять из 9 цифр.'");
			Сообщение.Сообщить();
			Область.Значение = "";
		КонецЕсли;
	...
	Если Результат <> Неопределено Тогда
		...
		Если ТипЗнч(Результат) = Тип("Структура") Тогда
			...
			Если Результат.ВведеноВСвободнойФорме 
			    ...
			    Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Российские адреса нельзя вводить в свободной форме!';uk='Российские адреса нельзя вводить в свободной форме!'");
				Сообщение.Сообщить();
