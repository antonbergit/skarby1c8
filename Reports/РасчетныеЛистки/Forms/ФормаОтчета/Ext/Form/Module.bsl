...
Процедура СформироватьВыполнить()
	...
	Если НЕ ЗначениеЗаполнено(ПериодРегистрации) Тогда
		ТекстСообщения = НСтр("ru='Не выбран период регистрации!';uk='Не обраний період реєстрації!'");
	...
	Если НЕ Константы.ФункциональнаяУчетВалютныхОпераций.Получить() И НЕ ЗначениеЗаполнено(Валюта) Тогда
		ТекстСообщения = НСтр("ru='Не выбрана валюта!';uk='Не обрана валюта!'");
	...
	Если РезультатЗапроса[0].Пустой() Тогда
		...
		Сообщение.Текст = НСтр("ru='Нет данных для формирования отчета!';uk='Немає даних для формування звіту!'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
    ...
    ОбластьОтбивка.Параметры.ТекстОтбивки = Формат(ПериодРегистрации , "Л="+КодЯзыкаПечать+"; ДФ='ММММ гггг'")+НСтр("ru=' г. ';uk=' р. '",КодЯзыкаПечать);
	ТабличныйДокумент.Вывести(ОбластьОтбивка);

    ОбластьОтбивка.Параметры.ТекстОтбивки = НСтр("ru='Организация: ';uk='Організація: '", КодЯзыкаПечать) + Организация;
	ТабличныйДокумент.Вывести(ОбластьОтбивка);

	ВыборкаПодразделение = РезультатЗапроса[0].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, "Подразделение");
	Пока ВыборкаПодразделение.Следующий() Цикл

        ОбластьОтбивка.Параметры.ТекстОтбивки = НСтр("ru='Подразделение: ';uk='Підрозділ: '", КодЯзыкаПечать) + ВыборкаПодразделение.Подразделение;
		ТабличныйДокумент.Вывести(ОбластьОтбивка);
		...
		Пока ВыборкаФизлицо.Следующий() Цикл
			...
			ОбластьШапка.Параметры.Заголовок = НСтр("ru='Расчетный листок за: ';uk='Розрахунковий листок за: '", КодЯзыкаПечать) + Формат(ПериодРегистрации , "Л="+КодЯзыкаПечать+"; ДФ='ММММ гггг'")+НСтр("ru=' г. ';uk=' р. '",КодЯзыкаПечать);
			...
			Пока ВыборкаДетали.Следующий() Цикл
				...
				Если ВыборкаДетали.Удержание = NULL Тогда
					...
					Иначе
						...
						ОбластьДетали.Параметры.ПериодНачисление = "" + День(ВыборкаДетали.ДатаНачала) + "-" + День(ВыборкаДетали.ДатаОкончания) + " " + Формат(ВыборкаДетали.ДатаОкончания , "Л="+КодЯзыкаПечать+"; ДФ='МММ гг'");
				...
				Иначе
					...
					Если ПоследнееУдержание < ПоследнееНачисление Тогда
						...
						ТабличныйДокумент.Область(ПоследнееУдержание + 1, 11, ПоследнееУдержание + 1, 12).Текст = "" + День(ВыборкаДетали.ДатаНачала) + "-" + День(ВыборкаДетали.ДатаОкончания) + " " + Формат(ВыборкаДетали.ДатаОкончания , "Л="+КодЯзыкаПечать+"; ДФ='МММ гг'");
					...
					Иначе
						...
						ОбластьДетали.Параметры.ПериодУдержание = "" + День(ВыборкаДетали.ДатаНачала) + "-" + День(ВыборкаДетали.ДатаОкончания) + " " + Формат(ВыборкаДетали.ДатаОкончания , "Л="+КодЯзыкаПечать+"; ДФ='МММ гг'");
			...
			Пока ВыборкаВыплатСотрудника.НАйтиСледующий(СтруктураПоискиПоВыборкей) Цикл
				...
				ОбластьДеталиВыплаты.Параметры.ТекстВыплаты = "" + ВыборкаВыплатСотрудника.ПредставлениеДокумента + ?(ВыборкаВыплатСотрудника.ПризнакАванса, "(аванс) №", " №") + 
																СокрЛП(ВыборкаВыплатСотрудника.Номер) + НСтр("ru=' от ';uk=' від '", КодЯзыкаПечать) + Формат(ВыборкаВыплатСотрудника.Дата, "ДФ=dd.MM.yyyy");
				ОбластьДеталиВыплаты.Параметры.ПериодВыплат		= "" + День(ПериодРегистрации) + "-" + День(КонецМесяца(ПериодРегистрации)) + " " + Формат(КонецМесяца(ПериодРегистрации) , "Л="+КодЯзыкаПечать+"; ДФ='МММ гг'");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтображениеПериодаРегистрации 	= Формат(ПериодРегистрации, "Л="+Локализация.ОпределитьКодЯзыкаДляФормат(КодЯзыка)+";ДФ='MMMM yyyy'");
