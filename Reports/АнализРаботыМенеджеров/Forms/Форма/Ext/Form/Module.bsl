...
Процедура ОсновнаяДиаграммаОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ГруппаПоказателей = "Выручка" Тогда
		...
		ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выручка от продаж по менеджеру: %1, за период: с %2 по %3';uk='Виторг від продажів по менеджерові: %1, за період: з %2 по %3'"),
			?(ЗначениеЗаполнено(Расшифровка.Точка), Расшифровка.Точка, НСтр("ru='<Менеджер не указан>';uk='<Менеджер не зазначен>'")),
			Формат(ОтборПериодЗначение.ДатаНачала, "ДФ=dd.MM.yyyy"),
			Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yyyy"));
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ru='Анализ работы менеджеров (расшифровка продаж)';uk='Аналіз роботи менеджерів (розшифрування продажів)'"));
	...
	ИначеЕсли ГруппаПоказателей = "Заказы" Тогда
		...
		ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Заказы по менеджеру: %1, за период: с %2 по %3 %4';uk='Замовлення по менеджері: %1, за період: з %2 по %3 %4'"),
			?(ЗначениеЗаполнено(Расшифровка.Точка), Расшифровка.Точка, НСтр("ru='<Менеджер не указан>';uk='<Менеджер не зазначен>'")),
			Формат(ОтборПериодЗначение.ДатаНачала, "ДФ=dd.MM.yyyy"),
			Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yyyy"),
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ru='Анализ работы менеджеров (расшифровка заказов)';uk='Аналіз роботи менеджерів (розшифрування замовлень)'"));
		ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
	...
	ИначеЕсли ГруппаПоказателей = "Задолженность" Тогда
		...
		ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Задолженность клиентов менеджера: %1, по состоянию на %2';uk='Заборгованість клієнтів менеджера: %1, за станом на %2'"),
			?(ЗначениеЗаполнено(Расшифровка.Точка), Расшифровка.Точка, НСтр("ru='<Менеджер не указан>';uk='<Менеджер не зазначен>'")),
			Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yyyy"));
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ru='Анализ работы менеджеров (расшифровка задолженности)';uk='Аналіз роботи менеджерів (розшифрування заборгованості)'"));
		ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
	...
	ИначеЕсли ГруппаПоказателей = "События" Тогда
		...
		ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='События по менеджеру: %1, за период: с %2 по %3 %4';uk='Події по менеджері: %1, за період: з %2 по %3 %4'"),
			?(ЗначениеЗаполнено(Расшифровка.Точка), Расшифровка.Точка, НСтр("ru='<Менеджер не указан>';uk='<Менеджер не зазначен>'")),
			Формат(ОтборПериодЗначение.ДатаНачала, "ДФ=dd.MM.yyyy"),
			Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yyyy"),
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ru='Анализ работы менеджеров (расшифровка событий)';uk='Аналіз роботи менеджерів (розшифрування подій)'"));
		ПараметрыФормы.Вставить("ПредставлениеОтборов", Новый ФорматированнаяСтрока(ТекстОтборов, Новый Шрифт(,,Истина)));
...
Процедура ЛегендаДиаграммыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ГруппаПоказателей = "Выручка" Тогда
		...
		ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выручка от продаж по менеджеру: %1, за период: с %2 по %3';uk='Виторг від продажів по менеджерові: %1, за період: з %2 по %3'"),
			?(ЗначениеЗаполнено(Точка), Точка, НСтр("ru='<Менеджер не указан>';uk='<Менеджер не зазначен>'")),
			Формат(ОтборПериодЗначение.ДатаНачала, "ДФ=dd.MM.yyyy"),
			Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yyyy"));
...
Процедура ОбновитьПоказателиВыручка()
	...
	Пока ВыборкаОтветственный.Следующий() Цикл
		...
		ПредставлениеОтветственного = ?(ЗначениеЗаполнено(ВыборкаОтветственный.Ответственный), ВыборкаОтветственный.ПредставлениеОтветственного, НСтр("ru='<Менеджер не указан>';uk='<Менеджер не зазначен>'"));
		...
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', выручка: ';uk=', виторг:'") + ВыборкаОтветственный.Выручка;
		ОсновнаяДиаграмма.УстановитьЗначение(Точка, Серия_Выручка, ВыборкаОтветственный.Выручка, Расшифровка, Подсказка);
		
		Расшифровка = Новый Структура("Точка", ВыборкаОтветственный.Ответственный);
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', себестоимость: ';uk=', собівартість:'") + ВыборкаОтветственный.Себестоимость;
		ОсновнаяДиаграмма.УстановитьЗначение(Точка, Серия_Себестоимость, ВыборкаОтветственный.Себестоимость, Расшифровка, Подсказка);
		
		Расшифровка = Новый Структура("Точка", ВыборкаОтветственный.Ответственный);
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', валовая прибыль: ';uk=', валовий прибуток:'") + ВыборкаОтветственный.ВаловаяПрибыль;
		...
		Расшифровка = Новый Структура("Точка", ВыборкаОтветственный.Ответственный);
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', средняя выручка по заказу: ';uk=', середній виторг за замовленням:'") + ВыборкаОтветственный.СредняяВыручка;
...
Процедура ОбновитьПоказателиЗаказы()
	...
	Пока ВыборкаОтветственный.Следующий() Цикл
		...
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', заказов всего: ';uk=', замовлень усього:'") + ВыборкаОтветственный.ЗаказыВсего;
		...
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', заказов новых клиентов: ';uk=', замовлень нових клієнтів:'") + ВыборкаОтветственный.Первичные;
		...
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', оплаченных заказов: ';uk=', сплачених замовлень: '") + ВыборкаОтветственный.Оплаченные;
		...
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', частично оплаченных заказов: ';uk=', частково сплачених замовлень:'") + ВыборкаОтветственный.ЧастичноОплаченные;
		...
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', неоплаченных заказов: ';uk=', несплачених замовлень: '") + ВыборкаОтветственный.Неоплаченные;
...
Процедура ОбновитьПоказателиЗадолженность()
	...
	Пока Выборка.Следующий() Цикл
		...
		ПредставлениеОтветственного = ?(ЗначениеЗаполнено(Выборка.Ответственный), Выборка.ПредставлениеОтветственного, НСтр("ru='<Менеджер не указан>';uk='<Менеджер не зазначен>'"));
		...
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', задолженность: ';uk=', заборгованість:'") + Выборка.Задолженность;
		ОсновнаяДиаграмма.УстановитьЗначение(Точка, Серия_Задолженность, Выборка.Задолженность, Расшифровка, Подсказка);
		
		Расшифровка = Новый Структура("Точка", Выборка.Ответственный);
		Подсказка = ПредставлениеОтветственного + НСтр("ru=', просроченная задолженность: ';uk=', прострочена заборгованість:'") + Выборка.ПросроченнаяЗадолженность;
...
Процедура ОбновитьПоказателиСобытия()
	...
	Пока ВыборкаОтветственный.Следующий() Цикл
		...
		Пока ВыборкаСостояние.Следующий() Цикл
			...
			Подсказка = ПредставлениеОтветственного + НСтр("ru=', событий в состоянии: ';uk=', подій у стані:'") + ВыборкаСостояние.ПредставлениеСостояния + " = " + КоличествоСобытий;
