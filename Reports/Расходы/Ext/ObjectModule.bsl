...
Процедура ПриОпределенииНастроекОтчета(НастройкиОтчета, НастройкиВариантов) Экспорт
	...
	НастройкиВариантов["Расходы"].Теги = НСтр("ru='Главное,Расходы,Траты,Хозяйственная операция';uk='Головне,Витрати,Страти,Господарська операція'");
...
Процедура ОбработатьРезультат(ТабличныйДокумент)
	...
	Для НомерСтроки = 1 По ТабличныйДокумент.ВысотаТаблицы Цикл
		Для НомерКолонки = 1 По ТабличныйДокумент.ШиринаТаблицы Цикл
			Если ТабличныйДокумент.Область(НомерСтроки, НомерКолонки).Текст = НСтр("ru='Итого';uk='Разом'") Тогда
				ТабличныйДокумент.Область(НомерСтроки, НомерКолонки).Текст = "";
			КонецЕсли;
			Если ТабличныйДокумент.Область(НомерСтроки, НомерКолонки).Текст = НСтр("ru='Сумма расходов.% в группировке';uk='Сума витрат.% в угрупованні'") Тогда
				ТабличныйДокумент.Область(НомерСтроки, НомерКолонки).Текст = "%";
	...
	Для НомерКолонки = 1 По ТабличныйДокумент.ШиринаТаблицы Цикл
		Если ТабличныйДокумент.Область(ТабличныйДокумент.ВысотаТаблицы, НомерКолонки).Текст = НСтр("ru='Итого';uk='Разом'") Тогда
