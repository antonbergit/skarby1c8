...
Процедура ПриОпределенииНастроекОтчета(НастройкиОтчета, НастройкиВариантов) Экспорт
	...
	НастройкиВариантов["Траты"].Теги = НСтр("ru='Главное,Расходы,Траты,Хозяйственная операция';uk='Головне,Витрати,Страти,Господарська операція'");
...
Процедура ОбработатьРезультат(ТабличныйДокумент)
	...
	Для НомерСтроки = 1 По ТабличныйДокумент.ВысотаТаблицы Цикл
		Для НомерКолонки = 1 По ТабличныйДокумент.ШиринаТаблицы Цикл
			...
			КонецЕсли;
			Если ТабличныйДокумент.Область(НомерСтроки, НомерКолонки).Текст = НСтр("ru='Сумма.% в группировке';uk='Сума.% в угрупованні'") Тогда
				ТабличныйДокумент.Область(НомерСтроки, НомерКолонки).Текст = "%";
