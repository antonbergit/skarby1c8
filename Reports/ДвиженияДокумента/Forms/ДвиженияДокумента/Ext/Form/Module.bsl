...
Процедура ДобавитьПериодВСписокПолей(ТаблицаПолей, СписокПолей)
	...
	ТаблицаПолей.Колонки.Добавить("Период", , НСтр("ru='Период';uk='Період'"));
...
Процедура ОбработатьВыводДанныхПоМассиву(СписокПолей, ТаблицаРесурсов, ТаблицаИзмерений, ТаблицаРеквизитов, ТаблицаВидДвижений = Неопределено, Знач ИмяРегистра, СинонимРегистра)
	...
	Если Отчет.СпособВыводаОтчета = Перечисления.СпособВыводаОтчета.ПоГоризонтали Тогда
		...
		Если ТаблицаВидДвижений <> Неопределено Тогда
			ОбластьЗаголовокЯчейки.Параметры.ЗаголовокКолонки = НСтр("ru='Вид движения';uk='Вид руху'");
...
Процедура ВывестиЖурналПроводок()
	...
	ОбластьЗаголовок.Параметры.СинонимРегистра = НСтр("ru='Регистр бухгалтерии ""Управленческий""';uk='Регістр бухгалтерії "" Управлінський ""'");
	ТабличныйДокумент.Вывести(ОбластьЗаголовок);
	ТабличныйДокумент.НачатьГруппуСтрок();
...
Процедура СформироватьОтчет()
	...
	Для Каждого СвойстваОбъекта Из ДвиженияДокумента Цикл
		...
		СинонимРегистра = НСтр("ru='Регистр ';uk='Регістр '") + НРег(ОпределитьСинонимВидаРегистра(ВидРегистра)) + " """ + СвойстваОбъекта.Синоним + """";
...
&НаСервере
// Локализация.Функция возвращает синоним вида регистра.
//
Функция ОпределитьСинонимВидаРегистра(ВидРегистра)
