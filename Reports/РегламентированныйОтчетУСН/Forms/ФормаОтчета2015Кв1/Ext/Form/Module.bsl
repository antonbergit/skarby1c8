...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если ИмяПоказателя = "П002110010203" Тогда
		...
		Если НЕ (ЗначПоказателя = "1" ИЛИ ЗначПоказателя = "2") Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В разделе 2.1.1 задано некорректное значение признака налогоплательщика!%1Допустимы значения 1 или 2';uk='В разделе 2.1.1 задано некорректное значение признака налогоплательщика!%1Допустимы значения 1 или 2'"), Символы.ПС);
			Сообщение.Сообщить();
