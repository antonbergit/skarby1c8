...
Процедура ВыгрузитьЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	Если НЕ СтруктураРеквизитовФормы.ПерсУчетВедетсяВоВнешнейПрограмме Тогда
		...
		Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.ВнешниеСведенияРаздела25) Тогда
			...
			ПоказатьПредупреждение(, НСтр("ru='Выгрузка невозможна!';uk='Выгрузка невозможна!'"));
	...
	Иначе
		...
		Если КоличествоЗагруженныхФайлов = 1 Тогда
			...
			Если Прав(ВыбИмяФайла, 4) <> ".pcb" Тогда
				...
				ПоказатьПредупреждение(, НСтр("ru='Необходимо выбрать файл описания пакета сведений персонифицированного учета с расширением ""pcb""!';uk='Необходимо выбрать файл описания пакета сведений персонифицированного учета с расширением ""pcb""!'"));
			...
			Иначе
				...
				НСтр("ru='Файл описания пакета сведений персонифицированного учета %1 успешно импортирован.';uk='Файл описания пакета сведений персонифицированного учета %1 успешно импортирован.'"), ВыбИмяФайла);
				Сообщение.Сообщить();
...
Процедура ПомещениеФайловРаздела6(Результат, АдресФайла, ВыбИмяФайла, ДопПараметры) Экспорт
	...
	Если Не Результат Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Импорт файлов пачек прерван!';uk='Импорт файлов пачек прерван!'"));
	...
	Иначе
		...
		Если КоличествоЗагруженныхФайлов > 0 Тогда
			...
			Если МассивВыбИменФайлов.Найти(ВыбИмяФайла) <> Неопределено Тогда
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Файл пачки %1 ранее уже был импортирован!';uk='Файл пачки %1 ранее уже был импортирован!'"), ВыбИмяФайла);
				Сообщение.Сообщить();
			...
			Иначе
				...
				Если НЕ ВыбранФайлПачки Тогда
					...
					Если КоличествоЗагруженныхФайлов > 0 Тогда
						...
						Сообщение = Новый СообщениеПользователю;
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Файл %1 не предназначен для импорта!';uk='Файл %1 не предназначен для импорта!'"), ВыбИмяФайла);
						Сообщение.Сообщить();
				...
				Иначе
					...
					Исключение
						...
						Сообщение = Новый СообщениеПользователю;
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Не удалость импортировать файл пачки %1!';uk='Не удалость импортировать файл пачки %1!'"), ВыбИмяФайла);
						Сообщение.Сообщить();
					...
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Файл пачки %1 успешно импортирован.';uk='Файл пачки %1 успешно импортирован.'"), ВыбИмяФайла);
					Сообщение.Сообщить();
...
Процедура ДиалогПриПодключенномРасширенииРаботыСФайлами()
	...
	Диалог.Фильтр = НСтр("ru='Файл описания пакета выгрузки сведений персонифицированного учета';uk='Файл описания пакета выгрузки сведений персонифицированного учета'") + "(*.pcb)|*.pcb";
...
Процедура ДиалогПриПодключенномРасширенииРаботыСФайламиПослеОтображенияДиалогаВыбораФайлаПослеПомещенияФайлов(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Файл описания пакета сведений персонифицированного учета %1 успешно импортирован.';uk='Файл описания пакета сведений персонифицированного учета %1 успешно импортирован.'"),
	...
	Для ИндексМассива = 0 По МассивИменФайлов.Количество() - 1 Цикл
		...
		Исключение
			...
			НСтр("ru='Не удалость импортировать файл пачки %1!';uk='Не удалость импортировать файл пачки %1!'"), ИмяФайлаПачки));
			Возврат;
		КонецПопытки;
	...
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = НСтр("ru='Файлы пачек:';uk='Файлы пачек:'");
	Сообщение.Сообщить();
	...
	Для ИндексМассива = 0 По МассивИменФайлов.Количество() - 1 Цикл
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='%1';uk='%1'"), ИмяФайлаПачки);
		Сообщение.Сообщить();
		
	КонецЦикла;
	
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = НСтр("ru='успешно импортированы.';uk='успешно импортированы.'");
	Сообщение.Сообщить();
...
Процедура РазобратьФайлСведенийРаздела25ПриПодключенномРасширенииРаботыСФайлами(ПомещенныеФайлы, Каталог)
	...
	Если Файл_РегистрационныйНомерПФР <> Отчет_РегистрационныйНомерПФР
	 ...
	 ИЛИ Файл_КодОтчетногоПериода <> Отчет_КодОтчетногоПериода Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Сведения файла описания пакета сведений персонифицированного учета не соответствуют экземпляру отчета!';uk='Сведения файла описания пакета сведений персонифицированного учета не соответствуют экземпляру отчета!'");
		Сообщение.Сообщить();
...
Процедура СообщитьОбОшибкеФорматаИмпорта()
	...
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = НСтр("ru='Нарушен формат файла описания пакета сведений персонифицированного учета!';uk='Нарушен формат файла описания пакета сведений персонифицированного учета!'");
	Сообщение.Сообщить();
...
Процедура ПроверитьВыгрузкуЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	Если НЕ СтруктураРеквизитовФормы.ПерсУчетВедетсяВоВнешнейПрограмме Тогда
		...
		Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.ВнешниеСведенияРаздела25) Тогда
			...
			ПоказатьПредупреждение(, НСтр("ru='Проверка выгрузки невозможна!';uk='Проверка выгрузки невозможна!'"));
...
Процедура Печать(ВидПечати, НеИзФормыОтчета = Ложь) Экспорт
	...
	Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='%1. Формируется печатная форма...';uk='%1. Формується друкована форма...'"), ЭтаФорма.Заголовок), , , БиблиотекаКартинок.Печать);
...
"в системе принтер и задать его в качестве используемого по умолчанию.';uk='Перед формированием печатных форм необходимо определить"
		...
		Если Форма.СтруктураРеквизитовФормы.мСчетчикСтраниц <> Истина Тогда
			...
			Для Каждого Эл Из Форма.СтруктураРеквизитовФормы.мПечатныеФормы Цикл
				Если НЕ УдалосьПроставитьНомерСтраницы(Эл, НомерЛиста) Тогда
					...
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Не удалось проставить номер страницы для %1';uk='Не удалось проставить номер страницы для %1'"), Эл.Представление);
					Сообщение.Сообщить();
		...
		Если ЭтаФорма.СтруктураРеквизитовФормы.мСчетчикСтраниц <> Истина Тогда
			...
			Для Ном = 0 По ЭтаФорма.СтруктураРеквизитовФормы.мПечатныеФормы.Количество() - 1 Цикл
				...
				Если НЕ УдалосьПроставитьНомерСтраницы(Эл, НомерЛиста) Тогда
					...
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось проставить номер страницы для %1';uk='Не удалось проставить номер страницы для %1'"), Эл.Представление);
					Сообщение.Сообщить();
...
Процедура ПроверитьЗаполнениеРаздел21(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	Для Каждого ЛистРаздела21 Из Раздел21 Цикл
		...
		Если ПустаяСтрока(КодТарифа) Тогда
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='На странице %1 раздела 2 не задан код тарифа.';uk='На странице %1 раздела 2 не задан код тарифа.'"), НомерЛиста);
		...
		ИначеЕсли РазрешенныеКодыТарифов.Найти(КодТарифа) = Неопределено Тогда
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='На странице %1 раздела 2 неверно задан код тарифа.';uk='На странице %1 раздела 2 неверно задан код тарифа.'"), НомерЛиста);
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
				ЭтаФорма, ТекстОписанияОшибки, "Раздел21", "П000210001001", НомерЛиста);
			СтатусПроверки = Ложь;
		Иначе
	...
	ТекстОписанияОшибки = НСтр("ru='При заполненном разделе 2 с кодом тарифа 06 должен быть заполнен раздел 3.1.';uk='При заполненном разделе 2 с кодом тарифа 06 должен быть заполнен раздел 3.1.'");
	...
	ТекстОписанияОшибки = НСтр("ru='При заполненном разделе 2 с кодом тарифа 07 должен быть заполнен раздел 3.2.';uk='При заполненном разделе 2 с кодом тарифа 07 должен быть заполнен раздел 3.2.'");
	...
	ТекстОписанияОшибки = НСтр("ru='При заполненном разделе 2 с кодом тарифа 12 должен быть заполнен раздел 3.3.';uk='При заполненном разделе 2 с кодом тарифа 12 должен быть заполнен раздел 3.3.'");
...
Процедура ПроверитьЗаполнениеРаздел24(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
		...
		Если НЕ ЗначениеЗаполнено(КодОснования) Тогда
			...
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='На странице %1 раздела 2.4 не указан код основания.';uk='На странице %1 раздела 2.4 не указан код основания.'"), НомерСтраницы);
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
		...
		Если НЕ ЗначениеЗаполнено(Основание) Тогда
			...
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='На странице %1 раздела 2.4 не указано основание заполнения раздела.';uk='На странице %1 раздела 2.4 не указано основание заполнения раздела.'"), НомерСтраницы);
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
	...
	Для Каждого Кандидат Из ПроверкаДубликатов Цикл
		...
		Если Кандидат.Счетчик > 1 И ЗначениеЗаполнено(Кандидат.Код) И ЗначениеЗаполнено(Кандидат.Основание) Тогда
			...
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Обнаружены дублирующиеся сочетания кода основания и основания заполнения раздела на страницах %1 в разделе 2.4.';uk='Обнаружены дублирующиеся сочетания кода основания и основания заполнения раздела на страницах %1 в разделе 2.4.'"),
				СписокСтраницДубликатов);
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
...
Процедура ПроверитьЗаполнениеРаздел31(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
		...
		Если ДанныеРаздела[ИмяПоказателя341] = 0 Тогда
			Если ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя342])
			 ИЛИ ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя343]) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указана сумма доходов в графе %1 строки 341 раздела 3.1.';uk='Не указана сумма доходов в графе %1 строки 341 раздела 3.1.'"), Графа);
	...
	Если НЕ ЗначениеЗаполнено(ДанныеРаздела.П000340034503) Тогда
		ТекстОписанияОшибки = НСтр("ru='Не указана дата записи в реестре аккредитованных организаций в строке 345 раздела 3.1.';uk='Не указана дата записи в реестре аккредитованных организаций в строке 345 раздела 3.1.'");
	...
	Если ПустаяСтрока(ДанныеРаздела.П000340034504) Тогда
		ТекстОписанияОшибки = НСтр("ru='Не указан номер записи в реестре аккредитованных организаций в строке 345 раздела 3.1.';uk='Не указан номер записи в реестре аккредитованных организаций в строке 345 раздела 3.1.'");
	...
	Если ДанныеРаздела[ИмяПоказателя361] = 0 Тогда
		Если ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя362])
		 ИЛИ ЗначениеЗаполнено(ДанныеРаздела[ИмяПоказателя363]) Тогда
			ТекстОписанияОшибки = НСтр("ru='Не указана сумма доходов в графе 3 строки 361 раздела 3.2.';uk='Не указана сумма доходов в графе 3 строки 361 раздела 3.2.'");
		...
		Если ДанныеРаздела[КонтрольныйПоказатель371] = 0 Тогда
			Если ЗначениеЗаполнено(ДанныеРаздела[КонтрольныйПоказатель372])
			 ...
			 ИЛИ ЗначениеЗаполнено(ДанныеРаздела[КонтрольныйПоказатель375]) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указана сумма доходов в графе %1 строки 371 раздела 3.3.';uk='Не указана сумма доходов в графе %1 строки 371 раздела 3.3.'"), Графа);
	...
	Пока ДанныеРаздела.Свойство("П000400000102_" + Формат(НомСтроки, "ЧГ=")) Цикл
		...
		Если СтрокаЗаполнена Тогда
			...
			Если НЕ ЗначениеЗаполнено(ОснованиеДляДоначисленияСВ) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указано основание для перерасчета страховых взносов в строке %1 раздела 4.';uk='Не указано основание для перерасчета страховых взносов в строке %1 раздела 4.'"),
					НомерСтроки);
			...
			Если ЗначениеЗаполнено(ОПСПоДопТарифу21)
				...
				Иначе
					НачТекста = "Не";
				КонецЕсли;
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 указан код основания для применения части 2.1 статьи 58.3 Федерального закона от 24 июля 2009 г. № 212-ФЗ в строке %2 раздела 4.';uk='%1 указан код основания для применения части 2.1 статьи 58.3 Федерального закона от 24 июля 2009 г. № 212-ФЗ в строке %2 раздела 4.'"),
			...
			Если НЕ ЗначениеЗаполнено(ГодНачисления) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указан год, за который выявлены и доначислены страховые взносы в строке %1 раздела 4.';uk='Не указан год, за который выявлены и доначислены страховые взносы в строке %1 раздела 4.'"),
				НомерСтроки);
			...
			Если НЕ ЗначениеЗаполнено(МесяцНачисления) Тогда
				ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указан месяц, за который выявлены и доначислены страховые взносы в строке %1 раздела 4.';uk='Не указан месяц, за который выявлены и доначислены страховые взносы в строке %1 раздела 4.'"),
				НомерСтроки);
...
Процедура ПроверитьЗаполнениеРаздел5(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	ТекстОписанияОшибки = НСтр("ru='В разделе 5 не заполнены сведения об обучающихся.';uk='В разделе 5 не заполнены сведения об обучающихся.'");
	...
	ТекстОписанияОшибки = НСтр("ru='В разделе 5 не указаны сведения из реестра молодежных и детских общественных объединений,"
"пользующихся государственной поддержкой.';uk='В разделе 5 не указаны сведения из реестра молодежных и детских общественных объединений,"
...
Процедура ПроверитьЗаполнениеТитульный(ПараметрыВыгрузки, ДеревоВыгрузки, СтатусПроверки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ИНН)
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ru='Не задан ИНН организации.';uk='Не задан ИНН организации.'"), "Титульный", "ИНН");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(ПараметрыВыгрузки.ИНН) <> 0 И СтрДлина(ПараметрыВыгрузки.ИНН) <> 10
		И СтрДлина(ПараметрыВыгрузки.ИНН) <> 12 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ru='Неверно задан ИНН организации.';uk='Неверно задан ИНН организации.'"), "Титульный", "ИНН");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ПараметрыВыгрузки.Свойство("КПП") И НЕ ПустаяСтрока(ПараметрыВыгрузки.КПП)
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ru='Неверно задан КПП организации.';uk='Неверно задан КПП организации.'"), "Титульный", "КПП");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если (НЕ ПараметрыВыгрузки.ЭтоПБОЮЛ И ПустаяСтрока(ПараметрыВыгрузки.НаименованиеОрганизации))
	 ИЛИ (ПараметрыВыгрузки.ЭтоПБОЮЛ И ПустаяСтрока(ПараметрыВыгрузки.ФамилияИП)) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ru='Не задан налогоплательщик.';uk='Не задан налогоплательщик.'"), "Титульный", "ОргНазв");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если НЕ РегНомерПФРСоответствуетТребованиям(ПараметрыВыгрузки.РегистрационныйНомерПФР) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			НСтр("ru='Значение рег. номера в ПФР не соответствует шаблону:""XXX-XXX-XXXXXX"" (X - любая цифра).';uk='Значение рег. номера в ПФР не соответствует шаблону:""XXX-XXX-XXXXXX"" (X - любая цифра).'"),
			"Титульный", "НомРег");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	ТекстОписанияОшибки = НСтр("ru='Не указана причина уточнения.';uk='Не указана причина уточнения.'");
	...
	Если ПустаяСтрока(СокрЛП(СтрЗаменить(ПараметрыВыгрузки.КодПоОКВЭД, ".", "")))
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ru='Некорректное значение кода вида деятельности по ОКВЭД.';uk='Некорректное значение кода вида деятельности по ОКВЭД.'"), "Титульный", "ОКВЭД");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	ТекстОписанияОшибки = НСтр("ru='Не указана среднесписочная численность.';uk='Не указана среднесписочная численность.'");
	...
	ТекстОписанияОшибки = НСтр("ru='Не указано количество застрахованных лиц.';uk='Не указано количество застрахованных лиц.'");
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ЛицоПодтверждающееСведения) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ru='Не указан тип подписанта.';uk='Не указан тип подписанта.'"), "Титульный", "ПрПодп");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрНайти("1,2,3", ПараметрыВыгрузки.ЛицоПодтверждающееСведения) = 0 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ru='Неверно указан тип подписанта.';uk='Неверно указан тип подписанта.'"), "Титульный", "ПрПодп");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ФамилияПодписанта) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ru='Не указана фамилия подписанта.';uk='Не указана фамилия подписанта.'"), "Титульный", "ОргПодписантФамилия");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыВыгрузки.ИмяПодписанта) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
			ЭтаФорма, НСтр("ru='Не указано имя подписанта.';uk='Не указано имя подписанта.'"), "Титульный", "ОргПодписантФамилия");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ПараметрыВыгрузки.ЛицоПодтверждающееСведения = "2" ИЛИ ПараметрыВыгрузки.ЛицоПодтверждающееСведения = "3" Тогда
		Если ПустаяСтрока(ПараметрыВыгрузки.НаименованиеУдостоверяющего) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
				ЭтаФорма, НСтр("ru='Не указано наименование документа, подтверждающего полномочия представителя.';uk='Не указано наименование документа, подтверждающего полномочия представителя.'"),
				"Титульный", "ДокУпПред");
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если ПустаяСтрока(ПараметрыВыгрузки.ДатаВыдачи) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
				ЭтаФорма, НСтр("ru='Не указана дата выдачи документа, подтверждающего полномочия представителя.';uk='Не указана дата выдачи документа, подтверждающего полномочия представителя.'"),
				"Титульный", "ДокУпПред");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
