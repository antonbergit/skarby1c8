	...
	Если ТипЗнч(НастройкиОтчета.РазрешеноМенятьВарианты) <> Тип("Булево") Тогда
		...
		Если НастройкиОтчета.РазрешеноМенятьВарианты // Команды не отключены параметрами открытия формы.
			...
			Если Отключенные.Найти(НастройкиОтчета.ПредопределенныйСсылка) <> Неопределено Тогда // Вариант отключен.
				Текст = НСтр("ru='Контекстный вариант ""%1"" отчета ""%2"" открыт без контекста.';uk='Контекстний варіант ""%1"" звіту ""%2"" відкритий без контексту.'");
...
Процедура РедактироватьСхему(Команда)
	...
	#Иначе
		...
		ПоказатьПредупреждение(,(НСтр("ru='Для того чтобы редактировать схему компоновки, необходимо запустить приложение в режиме толстого клиента.';uk='Для того щоб редагувати схему компонування, необхідно запустити додаток в режимі товстого клієнта.'")));
...
Процедура Подключаемый_ЗагрузитьВариантОтчета(Команда)
	...
	Если Найденные.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Вариант отчета не найден.';uk='Варіант звіту не знайдений.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьВариант(КлючВарианта)
	...
	ОтображениеСостояния.Текст     = НСтр("ru='Выбран другой вариант отчета. Нажмите ""Сформировать"" для получения отчета.';uk='Обраний інший варіант звіту. Натисніть ""Сформувати"" для одержання звіту.'");
КонецПроцедуры
...
Процедура ПослеИзмененияКлючевыхСостояний(ПараметрыЗаполнения)
	Если ПараметрыЗаполнения.ИмяСобытия <> "ПослеФормирования" Тогда
		...
		ИначеЕсли ПараметрыЗаполнения.ВариантМодифицирован
			...
			ОтображениеСостояния = Элементы.ОтчетТабличныйДокумент.ОтображениеСостояния;
			ОтображениеСостояния.Видимость = Истина;
			ОтображениеСостояния.Текст     = НСтр("ru='Изменились настройки. Нажмите ""Сформировать"" для получения отчета.';uk='Змінилися налаштування. Натисніть ""Сформувати"" для одержання звіту.'");
