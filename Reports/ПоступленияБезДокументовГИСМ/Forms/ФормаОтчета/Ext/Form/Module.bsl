...
Функция НастройкиОтчетаКорректны()
	...
	Если Не Отчет.ПоступленияЗаказанныхКиЗ И Не Отчет.ПоступленияМаркированныхТоваров Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указан ни один из типов документов поступления для анализа';uk='Не указан ни один из типов документов поступления для анализа'"), ,"ПоступленияЗаказанныхКиЗ", "Отчет");
...
Процедура ПоказатьДиалогОтправкиПоЭлектроннойПочте()
	...
	ФормаЗаголовок = СтрЗаменить(НСтр("ru='Отправка отчета ""%1"" по почте';uk='Відправлення звіту ""%1"" поштою'"), "%1", Заголовок);
