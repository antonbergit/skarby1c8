...
Процедура ВывестиДанныеОПоступлениях(Макет,
	...
	Если ИмяНабораДанных = "КиЗ" Тогда
		ЗаголовокНабораДанных = НСтр("ru='Поступления КиЗ, по которым нет уведомлений о выпуске КиЗ';uk='Поступления КиЗ, по которым нет уведомлений о выпуске КиЗ'");
	...
	ИначеЕсли ИмяНабораДанных = "МаркированныеТовары" Тогда
		ЗаголовокНабораДанных = НСтр("ru='Поступления маркированных товаров, по которым нет уведомлений о поступлении';uk='Поступления маркированных товаров, по которым нет уведомлений о поступлении'");
	...
	Если Не ЗначениеЗаполнено(ОтборКонтрагент) Тогда
		...
		Область.Параметры.ИмяГруппировки = НСтр("ru='Контрагент';uk='Контрагент'");
		Результат.Вывести(Область);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ОтборОрганизация) Тогда
		Область = Макет.ПолучитьОбласть("ШапкаГруппировка");
		Область.Параметры.ИмяГруппировки = НСтр("ru='Организация';uk='Організація'");
		Результат.Вывести(Область);
	КонецЕсли;
		...
		Пока ВыборкаОрганизации.Следующий() Цикл
			...
			Пока ВыборкаДетали.Следующий() Цикл
				...
				ИначеЕсли ВыборкаДетали.КоличествоВДокументахПоступления > 1 Тогда
					...
					Область.Параметры.КоличествоВДокументахПоступления = СтрШаблон(НСтр("ru='Документов поступления - %1';uk='Документов поступления - %1'"),
				...
				ИначеЕсли ВыборкаДетали.КоличествоВДокументахГИСМ > 1 Тогда
					...
					Область.Параметры.КоличествоВДокументахГИСМ      = СтрШаблон(НСтр("ru='Документов уведомлений - %1';uk='Документов уведомлений - %1'"),
