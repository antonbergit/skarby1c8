...
Процедура ОбновитьНастройкиНаФорме(НастройкиОтчета, НастройкиСКД, Форма) Экспорт
	...
	Стр.Заголовок = НСтр("ru='Режим';uk='Режим'");
...
Процедура УстановитьТегиВариантов(НастройкиВариантов)
	...
	НастройкиВариантов["АктСверкиКонтекст"].Теги = НСТР("ru='Покупатели,Контрагенты';uk='Покупці,Контрагенти'");
...
Процедура ДобавитьОписанияСвязанныхПолей(НастройкиВариантов)
	...
	ОтчетыУНФ.ДобавитьОписаниеПривязки(СтруктураВарианта.СвязанныеПоля, "Контрагент", "Документ.РасходСоСчета",, Истина, Истина);
	ОтчетыУНФ.ДобавитьОписаниеПривязки(СтруктураВарианта.СвязанныеПоля, "Контрагент", "Документ.ОтчетОПереработке",, Истина, Истина);
	ОтчетыУНФ.ДобавитьОписаниеПривязки(СтруктураВарианта.СвязанныеПоля, "Контрагент", "Документ.ПоступлениеВКассу",, Истина, Истина);
	...
	ОтчетыУНФ.ДобавитьОписаниеПривязки(СтруктураВарианта.СвязанныеПоля, "Контрагент", "Документ.РасходИзКассы",, Истина, Истина);
	ОтчетыУНФ.ДобавитьОписаниеПривязки(СтруктураВарианта.СвязанныеПоля, "Контрагент", "Документ.ЗаказПоставщику",, Истина, Истина);
	ОтчетыУНФ.ДобавитьОписаниеПривязки(СтруктураВарианта.СвязанныеПоля, "Контрагент", "Документ.ПоступлениеНаСчет",, Истина, Истина);
...
Функция Сформировать(Период, Контрагент, Договор)
	...
	Если НЕ ЗначениеЗаполнено(Период.ДатаНачала) И НЕ ЗначениеЗаполнено(КонецПериода) Тогда
		ОбластьЗаголовок.Параметры.Период = НСтр("ru='весь период работы';uk='увесь період роботи'");
	Иначе
	...
	Если ЗначениеЗаполнено(Период.ДатаНачала) И ВыборкаИтоги.Количество()>0 И НЕ ВыборкаИтоги.СуммаНачальныйОстаток=0 Тогда
		...
		НСтр("ru='Наш долг перед контрагентом на %1';uk='Наш борг перед контрагентом на %1'"), НСтр("ru='Долг контрагента перед нами на %1';uk='Борг контрагента перед нами на %1'"));
	...
	Если ВыборкаИтоги.Количество()>0 Тогда
		ПредставлениеПериода = ?(ЗначениеЗаполнено(КонецПериода), Формат(КонецПериода, "ДЛФ=D"), НСтр("ru='текущий момент';uk='теперішній момент'"));
		Если ВыборкаИтоги.СуммаКонечныйОстаток=0 Тогда
			ШаблонДолг = НСтр("ru='На %1 долги и авансы отсутствуют';uk='На %1 борги й аванси відсутні'");
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		ОбластьСтрока.Параметры.ЭквПриход = ?(ЗначениеЗаполнено(СуммаПриход) И ВыборкаДокументы.Эквивалент, НСтр("ru='(экв)';uk='(екв)'"), "");
		ОбластьСтрока.Параметры.ЭквРасход = ?(ЗначениеЗаполнено(СуммаРасход) И ВыборкаДокументы.Эквивалент, НСтр("ru='(экв)';uk='(екв)'"), "");
