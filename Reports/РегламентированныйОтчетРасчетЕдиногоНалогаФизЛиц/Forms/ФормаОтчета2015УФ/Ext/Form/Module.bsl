...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	КодКВЭД               = Сведения.КВЭД;
	ВЭД    			  	  = Сведения.ОснВидДеят;
	...
	Если НЕ Форма.СтруктураРеквизитовФормы.ПолучениеДанныхДляЗвит1С = Истина Тогда
		...
		Если НЕ ЗаполнитьТаблицуКВЭДДаннымиПредыдущейДекларации(Форма) Тогда
			...
			Если ТекРаздел = "ДекларацияЕН" Тогда
				...
				Форма.ТабличныйДокумент.Области["T1RXXXXG1S_1"].Значение = КодКВЭД;
				Форма.ТабличныйДокумент.Области["T1RXXXXG2S_1"].Значение = ВЭД;
				
			Иначе	
				
				СтрокаКвед = Форма.T1R[0];
				СтрокаКвед.T1RXXXXG1S = КодКВЭД;
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	ОтборПоОрганизации = Новый Структура("Организация", СтруктураРеквизитовФормы.Организация);
	
	ОграничениеСуммыДоходаЕдинщика = РегистрыСведений.СистемыНалогообложенияОрганизаций.СрезПоследних(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, ОтборПоОрганизации);
	...
	ЗаполнитьАвансовыеПлатежи();
	
	ЗаполнитьРазделПоГруппе(Перечисления.ГруппыПлательщиковЕдиногоНалога.ПерваяГруппа);
	ЗаполнитьРазделПоГруппе(Перечисления.ГруппыПлательщиковЕдиногоНалога.ВтораяГруппа);
	ЗаполнитьРазделПоГруппе(Перечисления.ГруппыПлательщиковЕдиногоНалога.ТретьяГруппа);
...
&НаСервере
Процедура ЗаполнитьАвансовыеПлатежи()
	
	Запрос = Новый Запрос;
	Запрос.Текст =  
	"ВЫБРАТЬ
	|	НАЧАЛОПЕРИОДА(ПлатежныйДокумент.Дата, МЕСЯЦ) КАК Период,
	|	СУММА(ПлатежныйДокумент.СуммаДокумента) КАК Сумма
	|ПОМЕСТИТЬ Перечисления
	|ИЗ
	|	Документ.РасходСоСчета КАК ПлатежныйДокумент
	|ГДЕ
	...
	|   И ПлатежныйДокумент.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРасходСоСчета.Налоги)
	|
	|СГРУППИРОВАТЬ ПО
	|	НАЧАЛОПЕРИОДА(ПлатежныйДокумент.Дата, МЕСЯЦ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Перечисления.Период КАК ПериодПеречисления,
	|	МАКСИМУМ(ДанныеПоОрганизации.Период) КАК ПериодУчетнойПолитики
	|ПОМЕСТИТЬ УчетнаяПолитика
	|ИЗ
	|	Перечисления КАК Перечисления
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СистемыНалогообложенияОрганизаций КАК ДанныеПоОрганизации
	|		ПО (ДанныеПоОрганизации.Период <= Перечисления.Период)
	|			И (ДанныеПоОрганизации.Организация = &Организация)
	|
	|СГРУППИРОВАТЬ ПО
	|	Перечисления.Период
	|;
	...
	|	ДанныеПоОрганизации.ГруппаПлательщикаЕдиногоНалога КАК Группа
	|ИЗ
	|	Перечисления КАК Перечисления
	|		ЛЕВОЕ СОЕДИНЕНИЕ УчетнаяПолитика КАК УчетнаяПолитика
	|		ПО Перечисления.Период = УчетнаяПолитика.ПериодПеречисления
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СистемыНалогообложенияОрганизаций КАК ДанныеПоОрганизации
	|		ПО (ДанныеПоОрганизации.Организация = &Организация)
	|			И (ДанныеПоОрганизации.Период = УчетнаяПолитика.ПериодУчетнойПолитики)
	|ГДЕ
	|	(ДанныеПоОрганизации.ГруппаПлательщикаЕдиногоНалога = ЗНАЧЕНИЕ(Перечисление.ГруппыПлательщиковЕдиногоНалога.ПерваяГруппа)
	|			ИЛИ ДанныеПоОрганизации.ГруппаПлательщикаЕдиногоНалога = ЗНАЧЕНИЕ(Перечисление.ГруппыПлательщиковЕдиногоНалога.ВтораяГруппа))
	|";
...
&НаСервере
Процедура ЗаполнитьРазделПоГруппе(Группа)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	Продажи КАК Продажи
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СистемыНалогообложенияОрганизаций КАК УчетнаяПолитикаОрганизаций
	               ...
	               |ВЫБРАТЬ
	               |	ВЫБОР
	               |		КОГДА УчетнаяПолитикаОрганизаций.СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.Общая)
	               |			ТОГДА ИСТИНА
	               |		ИНАЧЕ ЛОЖЬ
