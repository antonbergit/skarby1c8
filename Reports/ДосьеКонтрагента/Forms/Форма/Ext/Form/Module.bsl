...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей") Тогда
		ТекстИсключения = НСтр("ru='Формирование отчета ""Досье контрагента"" в данной конфигурации невозможно.';uk='Формирование отчета ""Досье контрагента"" в данной конфигурации невозможно.'");
...
Процедура СформироватьОтчет(Команда)
	...
	Если НЕ ЗначениеЗаполнено(СтрокаПоиска) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Поле ""ИНН или наименование контрагента"" не заполнено';uk='Поле ""ИНН или наименование контрагента"" не заполнено'"), , "СтрокаПоиска");
		Возврат;
	КонецЕсли;
...
Процедура УправлениеФормойНаСервере()
	...
	Если ЗначениеЗаполнено(НаименованиеКонтрагента) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Досье контрагента: %1';uk='Досье контрагента: %1'"),
	...
	ИначеЕсли ЗначениеЗаполнено(Контрагент) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			Контрагент);
	Иначе
		Заголовок = НСтр("ru='Досье контрагента';uk='Досье контрагента'");
	КонецЕсли;
	Элементы.ДекорацияРазделДанныеГосРеестров.Заголовок = ?(ЭтоЮридическоеЛицо, 
		НСтр("ru='ЕГРЮЛ';uk='ЕГРЮЛ'"), 
		НСтр("ru='ЕГРИП';uk='ЕГРИП'"));
...
Процедура СформироватьОтчетНаКлиенте(ТекстПоиска)
	...
	Если ПоискПоИНН Тогда
		...
		Если СтрДлина(ТекстПоиска) <> 10 
			...
			ТекстОшибки = НСтр("ru='Для поиска по ИНН необходимо ввести 10 или 12 цифр.';uk='Для поиска по ИНН необходимо ввести 10 или 12 цифр.'");
	...
	Иначе // Поиск по наименованию
		...
		ПараметрыФормы.Вставить("Заголовок",    НСтр("ru='Поиск контрагента';uk='Поиск контрагента'"));
...
Функция СформироватьОтчетНаСервере()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Формирование отчета: Досье контрагента: %1';uk='Формирование отчета: Досье контрагента: %1'"), 
		ИННПоиска);
...
"Подключиться сейчас?';uk='Для формирования ""Досье контрагента"""
"необходимо подключиться к интернет-поддержке пользователей."
"Подключиться сейчас?'");
