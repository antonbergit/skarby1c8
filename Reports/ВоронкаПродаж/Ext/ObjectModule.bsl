...
Процедура ПриОпределенииНастроекОтчета(НастройкиОтчета, НастройкиВариантов) Экспорт
	...
	НастройкиВариантов["ВоронкаПродаж"].Теги = НСтр("ru='CRM';uk='CRM'");
...
Процедура ОбновитьНастройкиНаФорме(НастройкиОтчета, НастройкиСКД, Форма) Экспорт
	...
	Форма.Элементы.ГруппаПараметрыЭлементы.Заголовок = НСтр("ru='Вариант воронки';uk='Варіант воронки'");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодсистемуРаботы") Тогда
		СтрВидОперации = ДобавитьПолеНастройки(Форма.ПоляНастроек, "Фильтр", "ВидОперации", Новый ОписаниеТипов("Булево"), НСтр("ru='Вид операции';uk='Вид операції'"), "Флажок");
		СтрВидОперации.Создан = Истина;
		СтрВидОперации.ИмяГруппы = ГруппаФильтрВидОперацииВидЗаказа.Имя;
		СтрВидОперации.Поле = "ВидОперации";
		
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрЗаказыПокупателей", НСтр("ru='Заказы покупателей';uk='Замовлення покупців'"), СтрВидОперации.ТипЗначения, СтрВидОперации, ИменаРеквизитов);
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрЗаказНаряды", НСтр("ru='Заказ-наряды';uk='Замовлення-наряди'"), СтрВидОперации.ТипЗначения, СтрВидОперации, ИменаРеквизитов);
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказовПокупателей") Или
		...
		СтрВидЗаказа = ДобавитьПолеНастройки(Форма.ПоляНастроек, "Фильтр", "ВидЗаказа", Новый ОписаниеТипов, НСтр("ru='Вид заказа';uk='Вид замовлення'"), "Поле");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказовПокупателей") Тогда
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрВидЗаказаПокупателя", НСтр("ru='Вид заказа покупателя';uk='Вид заказа покупателя'"), Новый ОписаниеТипов("СправочникСсылка.ВидыЗаказовПокупателей"), СтрВидЗаказа, ИменаРеквизитов);
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказНарядов") Тогда
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрВидЗаказНаряда", НСтр("ru='Вид заказ-наряда';uk='Вид заказ-наряда'"), Новый ОписаниеТипов("СправочникСсылка.ВидыЗаказНарядов"), СтрВидЗаказа, ИменаРеквизитов);
	КонецЕсли;
	
	Если Не МенееДвухПокупателейВСправочникеКонтрагенты() Тогда
		СтрКонтрагент = ДобавитьПолеНастройки(Форма.ПоляНастроек, "Фильтр", "Контрагент", Новый ОписаниеТипов("СправочникСсылка.Контрагенты"), НСтр("ru='Покупатель';uk='Покупець'"), "Поле");
		СтрКонтрагент.НестандартныйОбработчик = Ложь;
		СтрКонтрагент.Создан = Истина;
		СтрКонтрагент.Поле = "Контрагент";
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрПокупатель", НСтр("ru='Покупатель';uk='Покупець'"), СтрКонтрагент.ТипЗначения, СтрКонтрагент, ИменаРеквизитов);
	КонецЕсли;
	
	Если Не МенееДвухЭлементовВСправочникеСотрудники() Тогда
		СтрОтветственный = ДобавитьПолеНастройки(Форма.ПоляНастроек, "Фильтр", "Ответственный", Новый ОписаниеТипов("СправочникСсылка.Сотрудники"), НСтр("ru='Менеджер';uk='Менеджер'"), "Поле");
		СтрОтветственный.НестандартныйОбработчик = Ложь;
		СтрОтветственный.Создан = Истина;
		СтрОтветственный.Поле = "Ответственный";
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрМенеджер", НСтр("ru='Менеджер';uk='Менеджер'"), СтрОтветственный.ТипЗначения, СтрОтветственный, ИменаРеквизитов);
	КонецЕсли;
	
	Если Не МенееДвухПодразделенийВСправочникеСтруктурныеЕдиницы() Тогда
		СтрСтруктурнаяЕдиницаПродажи = ДобавитьПолеНастройки(Форма.ПоляНастроек, "Фильтр", "СтруктурнаяЕдиницаПродажи", Новый ОписаниеТипов("СправочникСсылка.СтруктурныеЕдиницы"), НСтр("ru='Подразделение';uk='Підрозділ'"), "Поле");
		...
		ДобавитьОписаниеРеквизита(ДобавляемыеРеквизиты, "ФильтрПодразделение", НСтр("ru='Подразделение';uk='Підрозділ'"), СтрСтруктурнаяЕдиницаПродажи.ТипЗначения, СтрСтруктурнаяЕдиницаПродажи, ИменаРеквизитов);
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказовПокупателей") Тогда
		ДобавитьПолеВвода("ФильтрВидЗаказаПокупателя", НСтр("ru='Вид заказа покупателя';uk='Вид заказа покупателя'"), СтрВидЗаказа, Форма);
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказНарядов") Тогда
		ДобавитьПолеВвода("ФильтрВидЗаказНаряда", НСтр("ru='Вид заказ-наряда';uk='Вид заказ-наряда'"), СтрВидЗаказа, Форма);
	КонецЕсли;
...
Функция ТекстСообщенияТребуетсяФильтр(ВариантВоронки, ПолеДляОбязательногоФильтра, НастройкиКД)
	...
	Если ВариантВоронки = ВариантВоронкиПоПокупателям() И МенееДвухПокупателейВСправочникеКонтрагенты() Тогда
		Возврат НСтр("ru='Для формирования воронки по покупателям требуется, чтобы в справочнике ""Контрагенты"" было не менее двух покупателей.';uk='Для формування воронки по покупцях потрібно, щоб в довіднику ""Контрагенти"" було не менше двох покупців.'");
	КонецЕсли;
	
	Если ВариантВоронки = ВариантВоронкиПоМенеджерам() И МенееДвухЭлементовВСправочникеСотрудники() Тогда
		Возврат НСтр("ru='Для формирования воронки по менеджерам требуется, чтобы в справочнике ""Сотрудники"" было не менее двух элементов.';uk='Для формування воронки по менеджерах потрібно, щоб в довіднику ""Співробітники"" було не менше двох елементів.'");
	КонецЕсли;
	
	Если ВариантВоронки = ВариантВоронкиПоПодразделениям() И МенееДвухПодразделенийВСправочникеСтруктурныеЕдиницы() Тогда
		Возврат НСтр("ru='Для формирования воронки по подразделениям требуется, чтобы в справочнике ""Подразделения"" было не менее двух элементов.';uk='Для формування воронки по підрозділах потрібно, щоб в довіднику ""Підрозділи"" було не менше двох елементів.'");
	КонецЕсли;
	
	Возврат СтрШаблон(НСтр("ru='Для формирования воронки %1 установите фильтр %2.';uk='Для формування воронки %1 встановіть фільтр %2.'"),
...
Функция ПредставлениеГруппировки(ВариантВоронки, Выборка)
	...
	Если ВариантВоронки = ВариантВоронкиПоПокупателям() Тогда
		ЗаголовокПоля = НСтр("ru='Покупатель';uk='Покупець'");
	КонецЕсли;
	
	Если ВариантВоронки = ВариантВоронкиПоМенеджерам() Тогда
		ЗаголовокПоля = НСтр("ru='Менеджер';uk='Менеджер'");
	КонецЕсли;
	
	Если ВариантВоронки = ВариантВоронкиПоПодразделениям() Тогда
		ЗаголовокПоля = НСтр("ru='Подразделение';uk='Підрозділ'");
	КонецЕсли;
...
Функция ПредставлениеВидЗаказа(ВидЗаказа)
	...
	Если ТипЗнч(ВидЗаказа) = Тип("СправочникСсылка.ВидыЗаказовПокупателей") Тогда
		...
		Иначе
			Возврат НСтр("ru='Заказы покупателей';uk='Замовлення покупців'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ТипЗнч(ВидЗаказа) = Тип("СправочникСсылка.ВидыЗаказНарядов") Тогда
		Если ПолучитьФункциональнуюОпцию("ИспользоватьВидыЗаказНарядов") Тогда
			Возврат  СтрШаблон("%1: %2", ТипЗнч(ВидЗаказа), ВидЗаказа);
		Иначе
			Возврат НСтр("ru='Заказ-наряды';uk='Замовлення-наряди'");
		КонецЕсли;
	КонецЕсли;
