...
Процедура УстановаитьПараметрыЗаполнитьИСохранитьОтчетЗвит1С(ОтчетЗвит1С, СоответствиеОтчета, НастройкаФормыОтчета, ДанныеДляСохраненияРасшифровкиОтчета, ОткрыватьОтчет)
	...
	ТекФорма = ПолучитьФорму(РегламентированнаяОтчетностьВызовСервера.ВариантОткрытияОтчета(ИмяОтчета) + "." + ИмяОтчета + ".Форма." + ИмяФормыОтчета, НастройкаФормыОтчета, ЭтаФорма, Новый УникальныйИдентификатор);
	...
	Если РезультатЗапроса.Пустой() Тогда
		Если ВыводитьСообщенияОбОшибках Тогда
			...
			ТекстВопроса = "На "+Формат(ТекущаяДата(), "ДФ=dd.MM.yyyy")+
			               НСтр("ru=' нет организаций использующих ""1С:Звіт""."
"Необходимо указать дату начала использования ""1С:Звіт"" в карточке организации.';uk=' немає організацій, які використовують ""1С:Звіт""."
...
"Повтортие подключение к 1С:Звит';uk='Установка зовнішньої компоненти для роботи з ""1С:Звіт"" виконана успішно!"
...
Функция ЗапуситьЗвит1С(НаПереднийПлан = Неопределено, Организация = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		КодПоЕДРПОУ        = СведенияОбОрганизации.ЕДРПОУ;
		НаименованиеПолное = СведенияОбОрганизации.НаимЮЛПол;
...
Функция ПолучитьИнформациюОбОрганизациях()
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл			
		МассивОрганизаций.Добавить(Новый Структура("Имя, Код", Выборка.НаименованиеПолное, РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(Выборка.Ссылка, ТекущаяДата(), СписокПоказателей).ЕДРПОУ));
	КонецЦикла;
...
Функция ОткрытьФормуПроверкиВходящихНалоговыхДокументов() Экспорт
	...
	Если ЭтоВнешнийОтчет() Тогда
		...
		Возврат ОткрытьФорму("ВнешнийОтчет.РегламентированныйОтчетМенеджерЗвит1С.Форма.ФормаВыгрузкиИПроверкиНалоговыхДокументов", Новый Структура("мЭтоИсходящиеНалоговыеДокументы", Ложь), , 1);
		
	Иначе
		
		Возврат ОткрытьФорму("Отчет.РегламентированныйОтчетМенеджерЗвит1С.Форма.ФормаВыгрузкиИПроверкиНалоговыхДокументов", Новый Структура("мЭтоИсходящиеНалоговыеДокументы", Ложь), , 1);
...
Функция ОткрытьДокументВЗвит1С(ДокументСсылка) Экспорт
	...
	Если Не ЗапуситьЗвит1С(Неопределено, ПолучитьОрганизациюДокумента(ДокументСсылка)) Тогда
		Возврат Ложь;
	КонецЕсли;
