...
Процедура ОбновитьСписок(Команда)
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		ПоказатьПредупреждение(,Нстр("ru='Укажите организацию!';uk='Вкажіть організацію!'"));
		Возврат;
	КонецЕсли;
	
	Если    НачПериода = '00010101'
		ИЛИ НачПериода = '00010101' Тогда
		ПоказатьПредупреждение(,Нстр("ru='Укажите период выписки!';uk='Вкажіть період виписки!'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьСписокНаКлиенте()
	...
	КодОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(Организация, КонПериода, СписокПоказателей).ЕДРПОУ;
...
Процедура ВнестиДанныеЗвит1С(ТаблицаДокЗвит1С, КодОрганизации, ИННКонтрагента, ФлОшибки)
	...
	Если  ЗначениеЗаполнено(НоваяСтрока.ДатаДокумента)
		...
		Если НЕ Месяц(НоваяСтрока.ДатаРегистрации) = Месяц(НоваяСтрока.ДатаДокумента) Тогда
			НоваяСтрока.ТипОформленияДатыРегистрации = 2;
		Иначе	
			НоваяСтрока.ТипОформленияДатыРегистрации = 1;
		КонецЕсли;
...
Процедура ЗагрузитьДокументНаСервере(Ссылка, Флаг, Организация, Контрагент, ТипДокумента, Номер, ДатаДокумента, ДатаРегистрации, DOC_ID, XML)
	...
	Если Не ЗначениеЗаполнено(Ссылка) Тогда
		...
		ЗаполнениеОбъектов.ЗаполнитьДокумент(ЗКП, Новый Структура);
