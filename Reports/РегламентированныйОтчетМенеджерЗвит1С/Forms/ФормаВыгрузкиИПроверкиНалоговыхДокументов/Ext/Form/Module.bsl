...
Процедура ЗаполнитьНачальныеНастройки()
	...
	РегламентированнаяОтчетность.ЗаполнитьСписокВыбораСпецРежимаНалогообложения(Элементы.СпецРежимНалогообложения.СписокВыбора);
	...
	// фактически полный номер с номером спец. режима
...
Процедура ЗаполнитьЕДРПОУОрганизации()
	...
	Сведения.Свойство("ЕДРПОУ", Организация_ЕДРПОУ);
...
Процедура ОткрытьФормуВыбораНалоговогоДокумента(ДляПодбора = Ложь) 
	...
	Если мЭтоИсходящиеНалоговыеДокументы Тогда
		...
		СписокТиповДокументов.ПоказатьВыборЭлемента(Новый ОписаниеОповещения("ОткрытьФормуВыбораНалоговогоДокументаЗавершение", ЭтотОбъект, Новый Структура("ДляПодбора", ДляПодбора)), НСтр("ru='Тип документа';uk='Тип документу'"));
        Возврат;			
	Иначе
...
Процедура ОткрытьФормуДокументаВ1С(Документ, ТекущаяСтрока)
	...
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.НалоговаяНакладная") Тогда
		...
		Форма = ОткрытьФорму("Документ.НалоговаяНакладная.ФормаОбъекта", ПараметрыВыбораДокумента);
		
	ИначеЕсли  ТипЗнч(Документ) = Тип("ДокументСсылка.Приложение2КНалоговойНакладной") Тогда
		
		Форма = ОткрытьФорму("Документ.Приложение2КНалоговойНакладной.ФормаОбъекта", ПараметрыВыбораДокумента);
		
	ИначеЕсли  ТипЗнч(Документ) = Тип("ДокументСсылка.РегистрацияВходящегоНалоговогоДокумента") Тогда
		
		Форма = ОткрытьФорму("Документ.РегистрацияВходящегоНалоговогоДокумента.ФормаОбъекта", ПараметрыВыбораДокумента);
...
Процедура ТаблицаДокументовЗаполнитьРеквизитыДокументов1С(МассивДокументов = Неопределено)
	...
	РеквизитыДокументов1С = ПолучитьРеквизитыДокументов1С(МассивДокументов, мЭтоИсходящиеНалоговыеДокументы);
...
Функция ПолучитьРеквизитыДокументов1С(МассивДокументов, ЭтоИсходящиеНалоговыеДокументы)
	...
	Если ЭтоИсходящиеНалоговыеДокументы Тогда
		...
		|		ВЫБОР КОГДА Документ.Кратность = 0 ТОГДА Документ.СуммаНДСДокумента ИНАЧЕ Документ.СуммаНДСДокумента * Документ.Курс / Документ.Кратность КОНЕЦ КАК СуммаНДСДокумента,
...
Функция ОпределитьПорядковыйНомерИсходящейНН(СтрокаТаблицы)
	...
	// подярковый номер это номер без слеша и номеров спецрежима, фактически номер в реестре НН
	НомерПорядковый 			= "";
	// полный номер, может включать слеш и коды спец.режима
	...
	Если ЗначениеЗаполнено(КодСпецРежима) Тогда
		НомерВходящегоДокумента	= НомерВходящегоДокумента + "/" + КодСпецРежима; 
	КонецЕсли;
...
Процедура ТаблицаДокументовЗафиксироватьСостояниеРегистрацииВ1С()
	...
	Для каждого СтрокаТаблицыДокументов Из МассивСтрокДокументов Цикл
		...
		ИзменитьДокументНаСервере(ДокументСсылка, мЭтоИсходящиеНалоговыеДокументы, ПараметрРегистрации, БылиОшибки);
...
&НаСервереБезКонтекста
Процедура ИзменитьДокументНаСервере(ДокументСсылка, ЭтоИсходящиеНалоговыеДокументы, ЗначениеРеквизита, ФлагОшибки)
...
Функция ВыгрузитьДокументВЗвит1С(СтрокаТаблицыДокументов) 
	...
	Попытка
		...
		ТекФорма = ПолучитьФорму(РегламентированнаяОтчетностьВызовСервера.ВариантОткрытияОтчета(ИмяОтчета) + "." + ИмяОтчета + ".Форма." + мВыбраннаяФорма, ПараметрыФормы, , Организация);
...
Функция ПолучитьКодыКонтрагентаНаСервере(Контрагент)
	...
	Возврат ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Контрагент, "КодПоЕДРПОУ");	
