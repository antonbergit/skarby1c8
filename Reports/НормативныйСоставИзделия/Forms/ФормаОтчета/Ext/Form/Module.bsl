...
Процедура ВывестиСоставИзделия(ТаблицаСостава, Макет, ОформлениеСтроки)
	...
	Если ТаблицаСостава.Количество() < 2 Тогда
		...
		Сообщение.Текст = НСтр("ru='Не заполнен нормативный состав изделия';uk='Не заповнений нормативний склад виробу'");
		Сообщение.Сообщить();
...
Процедура СформироватьОтчет(Номенклатура, Характеристика, Спецификация)
	...
	Если НЕ ЗначениеЗаполнено(Номенклатура) Тогда
		...
		ТекстСообщения = НСтр("ru='Поле Номенклатура не заполнено';uk='Поле Номенклатура не заповнене'");
	...
	Если НЕ ЗначениеЗаполнено(Спецификация) Тогда
		...
		ТекстСообщения = НСтр("ru='Поле Спецификация не заполнено';uk='Поле Специфікація не заповнене'");
	...
	ОбластьМакета.Параметры.Заголовок = СтрШаблон(НСтр("ru='Нормативный состав изделия на %1"
"Изделие: %2%3, спецификация ""%4"""
"Количество: %5 %6, стоимость: %7 %8"
"';uk='Нормативный состав изделия на %1"
"Изделие: %2%3, спецификация ""%4"""
"Количество: %5 %6, стоимость: %7 %8"
"'"),
	...
	Если Константы.ФункциональнаяОпцияИспользоватьТехоперации.Получить()
		И ДанныеДляОтчета.ТаблицаОпераций.Количество() > 0 Тогда
