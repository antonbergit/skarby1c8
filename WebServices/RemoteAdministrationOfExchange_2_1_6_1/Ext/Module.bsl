...
Функция ПолучитьПланыОбменаКонфигурации()
	...
	Для Каждого ИмяПланаОбмена Из ОбменДаннымиВМоделиСервисаПовтИсп.ПланыОбменаСинхронизацииДанных() Цикл
		...
		Если ПустаяСтрока(ExchangeFeature.ExchangeRole) Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru='Не задано возвращаемое значение функции ИмяКонфигурацииИсточника() в модуле менеджера плана обмена %1';uk='Не задано значення, що повертається, функції ИмяКонфигурацииИсточника() у модулі менеджера плану обміну %1'"),
				ИмяПланаОбмена
			);
		КонецЕсли;
