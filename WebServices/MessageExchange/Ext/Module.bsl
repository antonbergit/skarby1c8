...
Функция ДоставитьСообщения(КодОтправителя, ПотокХранилище)
	...
	Если Отправитель.Пустая() Тогда
		...
		ВызватьИсключение НСтр("ru='Заданы неправильные настройки подключения к конечной точке.';uk='Задані неправильні налаштування підключення до кінцевої точки'");
...
Функция ПроверитьПодключениеНаСторонеОтправителя(КодОтправителя)
	...
	Если ОбменСообщениямиВнутренний.КодЭтогоУзла() <> КодОтправителя Тогда
		...
		ВызватьИсключение НСтр("ru='Настройки подключения базы получателя указывают на другого отправителя.';uk='Налаштування підключення бази одержувача вказують на іншого відправника.'");
