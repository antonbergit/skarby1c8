...
Функция ПолучитьСсылкуПоИдентификатору(МенеджерОбъекта, Знач СтрокаGUID)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСТр("ru='GetPicture: не удалось получить объект по идентификатору.';uk='GetPicture: не вдалося одержати об`єкт по ідентифікатору.'"), УровеньЖурналаРегистрации.Ошибка,,, ОписаниеОшибки.Описание);
		ВызватьИсключение;
	КонецПопытки;
...
Функция ПолучитьКлассификаторИКаталог(ДатаИзменения, КодГруппы)
	...
	ОбменССайтом.ДобавитьГруппыКлассификатораXDTO(КлассификаторXDTO, Параметры.ДеревоГрупп.Строки, Неопределено, ПакетCML);
	ОбменССайтом.ДобавитьСвойстваНоменклатурыВКлассификаторXDTO(КлассификаторXDTO, ПакетCML, Параметры.ВыборкаСвойствНоменклатурыДляКлассификатора);
...
Процедура ПодготовитьДанныеДляВыгрузкиОстатовИЦен(Параметры)
	...
	Параметры.Вставить("ВыборкаВидовЦен", МассивРезультатовЗапроса[13].Выбрать());
	
	Параметры.Вставить("ДеревоСвойствХарактеристик", 
		МассивРезультатовЗапроса[14].Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкам));
...
Функция ЗагрузитьЗаказы(ДанныеЗаказовXDTO)
	...
	Если Не ОбменССайтом.ЗагрузитьЗаказы(ДанныеЗаказовXDTO, СтруктураСтатистики, Параметры, ОписаниеОшибки) Тогда 
		...
		ОбменССайтом.ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ru='Не удалось обработать документы, загруженные с сервера.';uk='Не вдалося оновити документи, завантажені із сервера.'"));
		ЕстьОшибки = Истина;
		
	КонецЕсли;
	
	ЗаписатьРезультатВыполненияОперацииВЖурналРегистрации(НСтр("ru='ОбменССайтом.ЗагрузкаЗаказов';uk='ОбменССайтом.ЗагрузкаЗаказов'"), ОписаниеОшибки, ЕстьОшибки);
...
Функция ПолучитьДвоичныеДанныеФайла(ДанныеФайла)
	...
	Если ФайлВХранилище Тогда
		...
		Если ДвоичныеДанныеФайла = Неопределено Тогда
			...
			ОписаниеОшибки = ИнформацияОбОшибке();
			ЗаписьЖурналаРегистрации(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСТр("ru='GetPicture: не удалось получить данные файла %1 номенклатуры %2.';uk='GetPicture: не вдалося одержати дані файлу %1 номенклатури %2.'"),
				ДанныеФайла.Файл,
