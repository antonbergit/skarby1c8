	...
	Если НЕ РольДоступна(Метаданные.Роли.ПолныеПрава)
	   И НЕ(РольДоступна(Метаданные.Роли.ДобавлениеИзменениеПодсистемыПродажи) // Профиль Базовые права.
		...
		ВызватьИсключение(
		  ...
		  + Пользователь
		  + НСтр("ru='"" нет прав на синхронизацию данных с мобильным приложением 1С:Управление небольшой фирмой. Необходимо включить профили прав доступа Базовые права и Деньги.';uk='"" немає прав на синхронізацію даних з мобільним додатком 1С:Управління невеликою фірмою. Необхідно включити профілі прав доступу Базові права й Гроші.'")
		);
...
Функция ДобавитьПользователя(ПочтаПользователя, ПарольПользователяСервиса, ПрофильПользователя)
	...
	Возврат НСтр("ru='Пользователю успешно отправлено приглашение на электронный адрес.';uk='Користувачеві успішно відправлено запрошення на електронну адресу.'");
...
Функция ОбновитьДанныеПользователя(ПочтаПользователя, ПарольПользователяСервиса, ПрофильПользователя, ВходРазрешен)
	...
	Если НЕ ЗначениеЗаполнено(Пользователь) Тогда
		Возврат НСтр("ru='Пользователь не найден.';uk='Користувач не знайдений.'");
	КонецЕсли;
	...
	Возврат НСтр("ru='Параметры пользователя успешно изменены.';uk='Параметри користувача успішно змінені.'");
...
Функция ОтправитьПакетОбменаВСервис(ДанныеМобильногоПриложения, КодМобильногоКомпьютера, НаименованиеМобильногоКомпьютера, НомерОтправленного, НомерПринятого, ПериодВыгрузки, ВерсияМобильногоПриложения)
	...
	Если УзелОбмена.Пустая() Тогда
		...
		НовыйУзел.НомерОтправленного = НомерОтправленного;
		НовыйУзел.НомерПринятого = НомерПринятого;
		НовыйУзел.ДатаПоследнейСинхронизации = ТекущаяДата();
	...
	Иначе
		...
		Узел.ДатаПоследнейСинхронизации = ТекущаяДата();
		Узел.Записать();
...
"Ошибка: %2';uk='Зазначено неправильну адресу електронної пошти: %1"
"Помилка: %2'");
...
Процедура ПроверитьВозможностьДобавленияновогоУзла()
	...
	Если МобильноеПриложениеВызовСервера.НужноОграничитьФункциональность() Тогда
		...
		КонецЦикла;
		Если Сч >= 4 Тогда
			ВызватьИсключение(НСтр("ru='Ваш тарифный план не позволяет подключать больше 3-х мобильных приложений.';uk='Ваш тарифний план не дозволяє підключати більше 3-х мобільних додатків.'"));
		КонецЕсли;
	КонецЕсли;
