...
"в связи с чем не поддерживает работу методов web-сервиса.';uk='Інформаційна база, що підключається, є файловою,"
"у зв`язку з чим не підтримує роботу методів web-сервісу.'");
	...
	Если ПланыОбмена[ИмяПланаОбмена].НайтиПоКоду(КодУзлаПланаОбмена).Пустая() Тогда
		СообщениеОбОшибке = НСтр("en='Presetting not foung. Please contact with application administrator';ru='Заданный узел плана обмена не найден. Обратитесь к администратору приложения.';uk='Заданий вузол плану обміну не знайдений. Зверніться до адміністратора програми.'");
		Возврат Ложь;
	КонецЕсли;
...
Функция ПодготовитьДанныеКПолучению(ИмяПланаОбмена, КодУзлаПланаОбмена, РазмерЧастиФайла, ИдентификаторФоновогоЗадания, СообщениеОбОшибке)
	...
	ФоновоеЗаданиеЗагрузкиДанных = ФоновыеЗадания.Выполнить("ОбменДаннымиСлужебный.ПодготовитьДанныеДляВыгрузкиИзИнформационнойБазы", 
		МассивПараметров, 
		Строка(Новый УникальныйИдентификатор()),
		НСтр("ru='Подготовка к выгрузке данных из информационной базы через web-сервис ""Enterprise Data Exchange""';uk='Підготовка до вивантаження даних з інформаційної бази через web-сервис ""Enterprise Data Exchange""'"));
	ИдентификаторФоновогоЗадания = Строка(ФоновоеЗаданиеЗагрузкиДанных.УникальныйИдентификатор);
