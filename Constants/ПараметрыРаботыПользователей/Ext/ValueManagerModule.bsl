...
"указанная в назначении %2.';uk='У метаданих не знайдено роль ""%1"","
"зазначену в призначенні %2.'"),
...
"%2, %3.';uk='Роль ""%1"" вказана більш ніж в одному призначенні:"
"%2, %3.'"),
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Или ПроверитьВсе Тогда
		...
		ПроверитьСоставПравРолей(НедоступныеПрава, Роли, ТекстОшибки,
			НСтр("ru='При проверке ролей для пользователей приложения найдены ошибки:';uk='При перевірці ролей для користувачів додатка знайдені помилки:'"), СписокОшибок, НеразделенныеДанные);
	КонецЕсли;
	...
	Для Каждого ОписаниеРоли Из ОписаниеРолей Цикл
		...
		Для Каждого СвойстваДанных Из НеразделенныеДанные Цикл
			...
			Если ПравоДоступа("Изменение", ОбъектМетаданных, Роль) Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Роль ""%1"" содержит право Изменение неразделенного объекта %2.';uk='Роль ""%1"" містить право Зміна нерозділеного об`єкту %2.'"),
			...
			Если ПравоДоступа("Добавление", ОбъектМетаданных, Роль) Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Роль ""%1"" содержит право Добавление неразделенного объекта %2.';uk='Роль ""%1"" містить право Додавання нерозділеного об`єкту %2.'"),
					Роль, ОбъектМетаданных.ПолноеИмя());
				Если СписокОшибок = Неопределено Тогда
					ТекстОшибки = ТекстОшибки + Символы.ПС + ОписаниеОшибки;
				Иначе
			...
			Если ПравоДоступа("Удаление", ОбъектМетаданных, Роль) Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Роль ""%1"" содержит право Удаление неразделенного объекта %2.';uk='Роль ""%1"" містить право Видалення нерозділеного об`єкту %2.'"),
					Роль, ОбъектМетаданных.ПолноеИмя());
				Если СписокОшибок = Неопределено Тогда
					ТекстОшибки = ТекстОшибки + Символы.ПС + ОписаниеОшибки;
				Иначе
