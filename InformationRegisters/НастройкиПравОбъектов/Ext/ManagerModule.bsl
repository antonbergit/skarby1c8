	...
	Для каждого ВозможноеПраво Из ВозможныеПрава Цикл
		...
		Если ОбъектМетаданныхВладельца = Неопределено Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не найден владелец прав ""%1"".';uk='Не знайдений власник прав ""%1"".'"),
...
"не зазначений в обумовленому типі ""Власник налаштувань прав"".'"),
					Строка(ТипСсылки));
			КонецЕсли;
...
"не зазначений в обумовленому типі ""Значення доступу"","
"але використовується для заповнення наборів значень доступу,"
"тому що зазначений в одній з підписок на подію:"
"- ЗаписатьЗависимыеНаборыЗначенийДоступа*,"
"- ЗаписатьНаборыЗначенийДоступа*."
"Потрібно вказати тип в обумовленому типі ""Значення доступу"""
"для коректного заповнення регістру НаборыЗначенийДоступа.'"),
					Строка(ТипСсылки));
			КонецЕсли;
...
"але виявлений в описі виду доступу ""%2"".'"),
			...
			Если ТипыПодпискиОбновитьГруппыВладельцевНастроекПрав.Получить(ТипОбъекта) = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Тип владельца прав ""%1"""
"не указан в определяемом типе ""Владелец настроек прав объект"".';uk='Тип власника прав ""%1"""
"не зазначений в обумовленому типі ""Власник налаштувань прав об`єкт"".'"),
					Строка(ТипОбъекта));
			КонецЕсли;
...
"для права ""%2"" в таблицях для зміни вказаний символ ""*""."
"Однак символ ""*"" вже зазначено в таблицях для зміни права "
"""%3"".'")
				КонецЕсли;
