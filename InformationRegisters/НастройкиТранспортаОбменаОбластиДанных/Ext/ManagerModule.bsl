...
Функция АбсолютныйКаталогОбменаИнформацией(Знач Корреспондент) Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru='Не заданы настройки подключения для корреспондента ""%1"".';uk='Не задані налаштування підключення для кореспондента ""%1"".'"), Строка(Корреспондент));
	КонецЕсли;
...
Функция НастройкиТранспортаWS(Знач Корреспондент) Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Не заданы настройки подключения веб-сервиса для корреспондента %1.';uk='Не задані налаштування підключення веб-сервісу для кореспондента %1.'"),
