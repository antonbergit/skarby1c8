...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СуммаНаДату = НСтр("ru='текущую дату';uk='поточну дату'");
...
Процедура ПриОткрытии(Отказ)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Вычеты по НДФЛ на детей (Итого: %1)';uk='Відрахування по ПДФО на дітей (Разом: %1)'"),
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Модифицированность И НЕ БылиНажатыКнопкиЗакрытия Тогда
		...
		спсОтветов.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Сохранить';uk='Зберегти'"));
		спсОтветов.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Нет';uk='Ні'"));
		спсОтветов.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru='Отмена';uk='Відмінити'"));
		
		оп = Новый ОписаниеОповещения("ОповещениеПриЗакрытии", ЭтотОбъект);
		
		ТекстВопроса = НСтр("ru='Даные были изменены. Сохранить изменения?';uk='Даные были изменены. Сохранить изменения?'");
...
Процедура ДатаРожденияРебенкаПриИзменении(Элемент)
	...
	Если СтрокаВТаблице.Номер <> ТекущийНомерСтроки Тогда
		...
		СпсОтветов = Новый СписокЗначений;
		СпсОтветов.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Скорректировать';uk='Скорегувати'"));
		СпсОтветов.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Оставить как есть';uk='Залишити як є'"));
...
"Скорректировать кода вычетов в строках согласно новому порядку?';uk='У таблиці змінився порядок рядків."
"Скорегувати коди відрахувань у рядках згідно з новим порядком?'"), СпсОтветов);
		Возврат;
	КонецЕсли;
...
Процедура ТаблицаВычетовНаДетейПриАктивизацииСтроки(Элемент)
	...
	Если Текданные<> Неопределено Тогда
		...
		Иначе
			...
			Строка.Представление = ?(ЗначениеЗаполнено(Строка.КодВычета), Нстр("ru='Код ';uk='Код '")+СокрЛП(Строка.КодВычета), "")
...
Процедура КодВычетаПриИзменении(Элемент)
	...
	Если ПустаяСтрока(ТекДанные.НаименованиеВычета) Тогда
		ТекДанные.НаименованиеВычета = Нстр("ru='<<Вычет не выбран>>';uk='<<Відрахування не обране>>'");
	КонецЕсли;
...
Процедура СуммаНаДатуНажатие(Элемент, СтандартнаяОбработка)
	...
	Если СуммаНаДату = НСтр("ru='текущую дату';uk='поточну дату'") Тогда
	...
	ПоказатьВводДаты(оп,АктуальнаяДата, НСтр("ru='Выберите дату для расчета суммы вычета';uk='Виберіть дату для розрахунків суми відрахування'"));
...
Процедура ЗаполнитьДанныеПоВычету()
	...
	Для Каждого Строка Из ТаблицаВычетовНаДетей Цикл
		...
		Если ПустаяСтрока(Строка.НаименованиеВычета) Тогда
			Строка.НаименованиеВычета = Нстр("ru='<<Вычет не выбран>>';uk='<<Відрахування не обране>>'");
		КонецЕсли;
		Строка.Представление = ?(ЗначениеЗаполнено(Строка.КодВычета), Нстр("ru='Код ';uk='Код '")+СОКРЛП(Строка.КодВычета), "")
			+?(Строка.СуммаВычета> 0 , ", "+Формат(Строка.СуммаВычета,"ЧЦ=10; ЧДЦ=0; ЧРГ=' '; ЧН=0")+ " руб. ", "");
	КонецЦикла;
...
Функция ВыполнитьПроверку(ОписаниеОшибок)
	...
	НомерСтроки = 1;
	Для Каждого Строка Из ТаблицаВычетовНаДетей Цикл
		ТекстОшибки = ?(ЗначениеЗаполнено(Строка.Период),"", НСтр("ru='Дата заявления;';uk='Дата заяви;'"))
			+ ?(ЗначениеЗаполнено(Строка.ДатаРожденияРебенка),"", НСтр("ru='Дата рождения ребенка;';uk='Дата народження дитини;'"))
			+ ?(ЗначениеЗаполнено(Строка.КодВычета),"", НСтр("ru='Код вычета;';uk='Код відрахування;'"));
		Если ТекстОшибки <> "" Тогда
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В строке №%1 не заполнены следующие поля: ';uk='У рядку №%1 не заповнені наступні поля:'"), НомерСтроки) +
				ТекстОшибки));
		КонецЕсли;
		НомерСтроки = НомерСтроки+1;
	КонецЦикла;
...
Функция КодВычетаПоПереключателям()
	...
	Если ПустаяСтрока(НаименованиеВычета) Тогда
		НаименованиеВычета = Нстр("ru='<<Вычет не выбран>>';uk='<<Відрахування не обране>>'");
	КонецЕсли;
