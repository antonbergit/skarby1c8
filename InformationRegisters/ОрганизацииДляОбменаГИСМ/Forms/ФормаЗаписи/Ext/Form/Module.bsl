...
Процедура ПолучитьGLNПоИННКППГИСМ(Команда)
	...
	Если Не ЗначениеЗаполнено(РеквизитыОрганизации.ИНН) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнено ИНН организации';uk='Не заполнено ИНН организации'"), Запись.Организация, "Запись.Организация");
		Возврат;
	КонецЕсли;
