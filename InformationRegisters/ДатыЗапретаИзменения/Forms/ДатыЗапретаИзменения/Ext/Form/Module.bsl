...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		...
		Элементы.ИспользованиеДатЗапретаВыключеноНадпись.Заголовок =
			НСтр("ru='Даты запрета загрузки данных прошлых периодов из других программ отключены в настройках.';uk='Дати заборони завантаження даних минулих періодів з інших програм відключені в налаштуваннях.'");
	...
	Иначе
		...
		Элементы.ИспользованиеДатЗапретаВыключеноНадпись.Заголовок =
			НСтр("ru='Даты запрета ввода и редактирования данных прошлых периодов отключены в настройках программы.';uk='Дати заборони введення і редагування даних минулих періодів відключені в налаштуваннях програми.'");
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстВопроса = НСтр("ru='Заданные настройки дат запрета будут автоматически скорректированы.';uk='Встановлені налаштування дат заборони будуть автоматично скориговані.'") 
...
Процедура УстановкаДатыЗапретаПриИзменении(Элемент)
	...
	Если ТекущаяУстановкаДатыЗапрета = "ПоПользователям"
	   И ВыбранноеЗначение = "ДляВсехПользователей" Тогда
		...
		ТекстВопроса = НСтр("ru='Отключить все даты запрета, кроме установленных для всех пользователей?';uk='Відключити усі дати заборони, окрім встановлених для усіх користувачів?'");
	...
	ИначеЕсли (ТекущаяУстановкаДатыЗапрета = "ПоПользователям" Или ТекущаяУстановкаДатыЗапрета = "ДляВсехПользователей")
		...
		ТекстВопроса = НСтр("ru='Отключить все установленные даты запрета?';uk='Відключити усі встановлені дати заборони?'")
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Представление)
	   И Не ТекущиеДанные.БезДатыЗапрета Тогда
		...
		Если ДополнительныеПараметры.ДатыЗапретаДляВсехПользователей Тогда
			ТекстВопроса = НСтр("ru='Отключить даты запрета для всех пользователей?';uk='Відключити дати заборони для усіх користувачів?'");
		Иначе
...
"Обновите данные формы (клавиша F5).';uk='""%1"" вже є в списку користувачів."
"Оновіть дані форми (F5).'"), ВыбранноеЗначение));
			Возврат;
		КонецЕсли;
	...
	Иначе
		ПоказатьПредупреждение(, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='""%1"" уже есть в списке пользователей.';uk='""%1"" вже є в списку користувачів.'"), ВыбранноеЗначение));
	КонецЕсли;
	...
	Если ТекущиеДанные.ЭтоРаздел Тогда
		Если ЗначениеЗаполнено(ТекущиеДанные.Раздел) Тогда
			Если ТекущиеДанные.ПолучитьЭлементы().Количество() > 0 Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Отключить все настроенные даты запрета для раздела ""%1"" и его объектов?';uk='Відключити усі налагоджені дати заборони для розділу ""%1"" і його об`єктів?'"), ТекущиеДанные.Раздел);
			Иначе
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Отключить дату запрета для раздела ""%1""?';uk='Відключити дату заборони для розділу ""%1""?'"), ТекущиеДанные.Раздел);
			КонецЕсли;
		Иначе
			ТекстВопроса = НСтр("ru='Отключить общую дату запрета для всех разделов?';uk='Відключити загальну дату заборони для усіх розділів?'");
		КонецЕсли;
	Иначе
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Отключить дату запрета для объекта ""%1""?';uk='Відключити дату заборони для об`єкту ""%1""?'"), ТекущиеДанные.Объект);
	КонецЕсли;
		...
		Если ДатаЗапретаУстановлена(ТекущиеДанные, ТекущийПользователь)
		 Или ЭлементыРаздела.Количество() > 0 Тогда
			...
			ПоказатьВопрос(
				...
				ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		Иначе
			ТекстСообщения = НСтр("ru='Для удаления даты запрета по конкретному объекту необходимо выбрать интересующий объект в одном из разделов.';uk='Для видалення дати заборони по конкретному об`єкту необхідно вибрати об`єкт, що цікавить, в одному з розділів.'");
...
"Обновите данные формы (клавиша F5).';uk='""%1"" вже є в списку об`єктів."
"Оновіть дані форми (F5).'"), ВыбранноеЗначение));
				Возврат;
			Иначе
				ОбновитьЗначенияПрочитанныхСвойств(
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Формат",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ДП='%1'", НСтр("ru='Без запрета';uk='Без запрета'")));
...
Процедура ПрочитатьДанныеПользователя(Контекст, ТекущийСпособУказания = Неопределено, Данные = Неопределено)
	...
	ИначеЕсли Контекст.УстановкаДатыЗапрета = "ПоПользователям" Тогда
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			Контекст.Элементы.ТекущийПользовательПредставление.Заголовок = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Настройка для ""%1"":';uk='Налаштування для ""%1"":'"), 
	...
	Если Контекст.ПоказыватьРазделыТекущегоПользователя Тогда
		Если Контекст.ВсеРазделыБезОбъектов Тогда
			...
			Контекст.Элементы.ДатыЗапретаПолноеПредставление.Заголовок = НСтр("ru='Раздел';uk='Розділ'");
			Контекст.Элементы.ДатыЗапрета.Отображение = ОтображениеТаблицы.Список;
			
		Иначе
			Контекст.Элементы.ДатыЗапретаПолноеПредставление.Заголовок = НСтр("ru='Раздел, объект';uk='Розділ об`єкт'");
...
Процедура УстановитьВидимость()
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Если УстановкаДатыЗапрета = "НетЗапрета" Тогда
			РасширеннаяПодсказка = НСтр("ru='Разрешена загрузка из других программ любых данных прошлых периодов.';uk='Дозволено завантаження з інших програм будь-яких цих минулих періодів.'");
		...
		Иначе
			РасширеннаяПодсказка = НСтр("ru='Персональная настройка дат запрета загрузки данных прошлых периодов из других программ для выбранных пользователей.';uk='Персональне налаштування дат заборони завантаження даних минулих періодів з інших програм для вибраних користувачів.'");
		КонецЕсли;
	Иначе
		Если УстановкаДатыЗапрета = "НетЗапрета" Тогда
			РасширеннаяПодсказка = НСтр("ru='Разрешены ввод и редактирование любых данных прошлых периодов.';uk='Дозволено введення і редагування будь-яких цих минулих періодів.'");
		ИначеЕсли УстановкаДатыЗапрета = "ДляВсехПользователей" Тогда
		...
		Иначе
			РасширеннаяПодсказка = НСтр("ru='Персональная настройка дат запрета ввода и редактирования данных прошлых периодов для выбранных пользователей.';uk='Персональне налаштування дат заборони введення і редагування даних минулих періодів для вибраних користувачів.'");
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		...
		ПараметрыФормы.Вставить("Объект", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Выбранные строки (%1)';uk='Обрані рядки (%1)'"), ВыделенныеСтроки.Количество()));
	КонецЕсли;
...
Функция ТекстПредставленияОбщейДаты()
	...
	Возврат "<" + НСтр("ru='Общая дата для всех разделов';uk='Загальна дата для усіх розділів'") + ">";
...
"поэтому для всех пользователей будет действовать более простая настройка ""%1"".';uk='Ні для одного розділу не були введені дати заборони, "
"тому для усіх користувачів діятиме більш простіше налаштування ""%1"".'"),
				СпособУказанияВБазеДанных);
		Иначе
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ни для одного раздела не были введены даты запрета, "
"поэтому для ""%1"" будет действовать более простая настройка ""%2"".';uk='Ні для одного розділу не були введені дати заборони, "
"тому для ""%1"" діятиме більш простіше налаштування ""%2"".'"),
...
"поэтому для всех пользователей будет действовать более простая настройка ""%1"".';uk='Ні для одного об`єкту не були введені дати заборони, "
"тому для усіх користувачів діятиме більш простіше налаштування ""%1"".'"),
				СпособУказанияВБазеДанных);
		Иначе
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ни для одного объекта не были введены даты запрета, "
"поэтому для ""%1"" будет действовать более простая настройка ""%2"".';uk='Ні для одного об`єкту не були введені дати заборони, "
"тому для ""%1"" діятиме більш простіше налаштування ""%2"".'"),
				ТекущийПользователь, СпособУказанияВБазеДанных);
		КонецЕсли;
	КонецЕсли;
...
Функция ТекстСообщенияВВыбранномРазделеДатыЗапретаДляОбъектовНеУстанавливаются(Раздел)
	...
	Возврат ?(Раздел <> РазделПустаяСсылка, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='В разделе ""%1"" не предусмотрена установка дат запрета для отдельных объектов.';uk='У розділі ""%1"" не передбачене встановлення дат заборони для окремих об`єктів.'"), Раздел),
