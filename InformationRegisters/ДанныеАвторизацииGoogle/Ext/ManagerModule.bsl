...
Процедура ЗаписатьТокенЗапроса(Знач Идентификатор, Знач ТокенЗапроса) Экспорт
	...
	НСтр("ru='Значение, переданное в параметре state.id, не является уникальным идентификатором.';uk='Значення, передане в параметрі state.id, не є унікальним ідентифікатором.'"));
	...
	Если Выборка.Следующий() И Выборка.КоличествоЗаписей > 1000 Тогда
		ВызватьИсключение НСтр("ru='Превышено количество попыток авторизации.';uk='Перевищена кількість спроб авторизації.'");
	КонецЕсли;
