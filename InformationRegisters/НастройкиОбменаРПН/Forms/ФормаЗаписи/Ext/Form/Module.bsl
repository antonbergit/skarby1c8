...
Процедура ОбменНапрямуюПриИзменении(Элемент)
	...
	Если Запись.ОбменНапрямую Тогда
		ПоказатьПредупреждение(, НСтр("ru='Внимание! Обмен напрямую с Росприроднадзором рекомендуется использовать только при экстренной необходимости и невозможности отправки через оператора электронного документооборота.';uk='Внимание! Обмен напрямую с Росприроднадзором рекомендуется использовать только при экстренной необходимости и невозможности отправки через оператора электронного документооборота.'"));
	КонецЕсли;
