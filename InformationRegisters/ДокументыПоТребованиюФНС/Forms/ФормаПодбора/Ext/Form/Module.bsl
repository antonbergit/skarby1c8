...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ПрограммноеЗакрытие <> Истина Тогда
		...
		Если МассивОтобранныхЗаписей.Количество() > 0 Тогда
			...
			ТекстПредупреждения = НСтр("ru='Есть отобранные документы. Вы действительно хотите закрыть форму?';uk='Есть отобранные документы. Вы действительно хотите закрыть форму?'");
...
Процедура СписокПередУдалением(Элемент, Отказ)
	...
	Если ЗначениеЗаполнено(СканированныйДокумент) Тогда
		...
		ТекстВопроса = НСтр("ru='Удалить данные, подготовленные к отправке?';uk='Удалить данные, подготовленные к отправке?'");
		
		Если ЗначениеЗаполнено(Источник) Тогда
			
			ТекстВопроса = ТекстВопроса + НСтр("ru=' Документ-источник при этом сохранится.';uk=' Документ-источник при этом сохранится.'");	
	...
	ИначеЕсли Элемент.ТекущиеДанные.ЕстьЭлектронныйДокумент Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Данные  документа-источника и связанные с ним данные электронного документооборота с контрагентом нельзя удалить из списка';uk='Данные  документа-источника и связанные с ним данные электронного документооборота с контрагентом нельзя удалить из списка'"));	
		Возврат;
		
	Иначе
		
		ПоказатьПредупреждение(, НСтр("ru='Данные документа-источника нельзя удалить из списка';uk='Данные документа-источника нельзя удалить из списка'"));	
		Возврат;
...
Процедура УстановитьОтборыДляРежимаВыбора()
	...
	Если ЗначениеЗаполнено(Параметры.ОтборВидДокументаФНС) Тогда
		...
		Если Параметры.ОтборВидДокументаФНС = Перечисления.ВидыПредставляемыхДокументов.ДобавочныйЛистГрузовойТаможеннойДекларации Тогда
