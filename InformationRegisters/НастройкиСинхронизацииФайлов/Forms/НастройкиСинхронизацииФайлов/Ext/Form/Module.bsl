...
Процедура СинхронизацияЭлемента(Команда)
	...
	Если Не СтрокаДерева.ВозможностьДетализации Тогда
		ТекстСообщения = НСтр("ru='Добавление настройки возможно только для иерархических справочников';uk='Додавання налаштування можливо тільки для ієрархічних довідників'");
	...
	ПараметрыФормыВыбора.Вставить("ЗаголовокФормыПодбора", НСтр("ru='Подбор элементов настроек';uk='Підбір елементів налаштувань'"));
...
Процедура СоздатьСинхронизациюФайлов(Команда)
	...
	СтрокаДерева = Элементы.ДеревоОбъектовМетаданных.ТекущиеДанные;
	Если Не СтрокаДерева.ВозможностьДетализации Тогда
		ТекстСообщения = НСтр("ru='Добавление настройки возможно только для справочников';uk='Додавання налаштування можливо тільки для довідників'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
