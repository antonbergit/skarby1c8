...
Процедура СнятьСМониторинга(Команда)
	...
	ПоказатьВопрос(
		...
		НСтр("ru='Снять выбранных контрагентов с мониторинга?';uk='Снять выбранных контрагентов с мониторинга?'"),
...
Процедура ПодборКонтрагентовДляМониторинга(Команда)
	...
	Иначе
		СправочникиКонтрагенты.ПоказатьВыборЭлемента(
			...
			НСтр("ru='Выбор справочника контрагентов';uk='Выбор справочника контрагентов'"));
	КонецЕсли;
...
Процедура ОткрытьФормуПодбора(ОписаниеСправочника)
	...
	Иначе
		...
		ПараметрыФормы.Вставить("РежимВыбора", Истина); // УНФ
...
Процедура ПриОтветеНаВопросОСнятииСМониторинга(КодВозврата, ДополнительныеПараметры) Экспорт
	...
	Если КодВозврата = КодВозвратаДиалога.Да Тогда
		...
		Если УдаленоЗаписей = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru='Среди выбранных контрагентов нет поставленных вручную.';uk='Среди выбранных контрагентов нет поставленных вручную.'"));
		КонецЕсли;
	КонецЕсли;
