	...
	Если Поле.Имя = "ПротоколГиперссылкаРасшифровки" Тогда
		...
		Если Элементы.Протокол.ТекущиеДанные.ПолучитьРодителя() = Неопределено Тогда
			...
			Если КлючПротокола = Неопределено Тогда
				ПоказатьПредупреждение(,НСтр("ru='Повторная проверка не выполнена из-за внутренней ошибки';uk='Повторна перевірка не виконана через внутрішню помилку'"));
				Возврат;
			КонецЕсли;
...
Процедура ЗаполнитьПротокол(ДеревоРезультата, ДанныеПротокола)
	...
	Для Каждого Строка Из ДеревоРезультата.Строки Цикл
		...
		Если Строка.РезультатКонтроля > 0 Тогда
			...
			Если Строка.Родитель <> Неопределено Тогда
				...
				Если Строка.Расшифровка.ТипРасшифровки = "НоваяЗаписьРегистраСведений" Тогда
					СтрокаПротокола.ГиперссылкаРасшифровки = НСтр("ru='Создать';uk='Створити'");
				Иначе
					СтрокаПротокола.ГиперссылкаРасшифровки = НСтр("ru='Показать';uk='Показати'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если Строка.Родитель = Неопределено Тогда
			...
			СтрокаПротокола.ГиперссылкаРасшифровки = НСтр("ru='Повторить проверку';uk='Повторити перевірку'");
...
Процедура СформироватьЗаголовок()
	...
	Заголовок = НСтр("ru='Протокол проверки данных от ';uk='Протокол перевірки даних від'") + Запись.ДатаПроверки;
