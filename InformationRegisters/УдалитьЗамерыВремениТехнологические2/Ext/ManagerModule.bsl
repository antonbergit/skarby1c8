...
Функция ПолучитьПоследниеКомментарии() Экспорт
	...
	|	(
	|		ВЫБРАТЬ ПЕРВЫЕ 1000
	|			УдалитьЗамерыВремени3.Комментарий КАК Комментарий
	|		ИЗ
	|			РегистрСведений.УдалитьЗамерыВремени3 КАК УдалитьЗамерыВремени3
	|		УПОРЯДОЧИТЬ ПО
	|			УдалитьЗамерыВремени3.ДатаЗаписи УБЫВ
	|	) КАК Выборка
	|СГРУППИРОВАТЬ ПО
	|	Комментарий
	|УПОРЯДОЧИТЬ ПО
...
Функция ПолучитьТопAPDEX(ДатаНачала, ДатаОкончания, ПериодАгрегации, Количество) Экспорт
	...
	Если Количество > 0 Тогда
		...
		|	Выборка
		|ИЗ
		|	РегистрСведений.УдалитьЗамерыВремениТехнологические2 КАК Замеры
