...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого ТекущаяЗапись Из ЭтотОбъект Цикл
		...
		Если ВРег(ТекущаяЗапись.КатегорияНовостей.Код) = ВРег("ProductVersion")
			...
			// Версия ОТ.
			ТекстСообщения = НСтр("ru='Формат значения версии ОТ (%ВерсияОТ%) должен быть 99.99.999.9999 %ПодробныйТекстОшибки%.';uk='Формат значення версії ОТ (%ВерсияОТ%) повинен бути 99.99.999.9999 %ПодробныйТекстОшибки%.'");
