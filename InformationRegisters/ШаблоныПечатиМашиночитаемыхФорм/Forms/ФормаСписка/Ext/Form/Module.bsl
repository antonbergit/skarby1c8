...
Процедура ДобавитьШаблоныНаВебКлиентеПоместитьФайлЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ОбъектФайл.Расширение = ".tif" Тогда
		ПоказатьПредупреждение(,НСтр("ru='Выбранный файл не является шаблоном печати машиночитаемой формы.';uk='Выбранный файл не является шаблоном печати машиночитаемой формы.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаписатьШаблоныВВебКлиенте(АдресФайлаВоВременномХранилище, КоличествоФайлов, ОбъектФайл, РежимОдногоФайла)
	...
	Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Загрузка шаблона ""%1""...';uk='Загрузка шаблона ""%1""...'"), ОбъектФайл.Имя), , , БиблиотекаКартинок.ЗагрузкаФайла);
	...
	Если НЕ ЗагрузитьШаблоныМашиночитаемыхФорм(АдресФайлаВоВременномХранилище, ОбъектФайл.Имя, КоличествоНезагруженныхШаблонов) Тогда
		...
		ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось загрузить шаблон ""%1"":%2';uk='Не удалось загрузить шаблон ""%1"":%2'"), ОбъектФайл.Имя, ОписаниеОшибки()));
...
Процедура ЗаписатьШаблоныНаТонкомКлиенте(КоличествоФайлов, ОбъектыФайл, РежимОдногоФайла)
		...
		Если ОбъектФайл.Существует() Тогда
			...
			Исключение
				Если РежимОдногоФайла Тогда
					...
					Возврат;
				Иначе
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось загрузить шаблон ""%1"":%2';uk='Не удалось загрузить шаблон ""%1"":%2'"), ОбъектФайл.Имя, Символы.ПС + Символы.ПС + ОписаниеОшибки());
	...
	Состояние(НСтр("ru='Запись шаблонов...';uk='Запись шаблонов...'"), , , БиблиотекаКартинок.Записать);
...
Процедура ПоказатьПредупрежденияПослеЗагрузкиШаблонов(КоличествоНезагруженныхШаблонов, КоличествоФайлов, РежимОдногоФайла)
	...
	Если КоличествоНезагруженныхШаблонов = КоличествоФайлов Тогда
		ПоказатьПредупреждение(,НСтр("ru='Не удалось загрузить ни один из указанных шаблонов.';uk='Не удалось загрузить ни один из указанных шаблонов.'"));
	Иначе
		Если РежимОдногоФайла Тогда
...
Процедура ЗагрузитьШаблоны(АдресВременногоХранилища, КоличествоНезагруженныхШаблонов)
	...
	Для Каждого ШаблонМашиночитаемойФормы Из ШаблоныМашиночитаемыхФорм Цикл
		...
		Исключение
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось записать шаблон ""%1"":%2';uk='Не удалось записать шаблон ""%1"":%2'"), ШаблонМашиночитаемойФормы.ИмяФайлаШаблона, Символы.ПС + ОписаниеОшибки());
...
Процедура Выгрузить(Команда)
	...
	Если ТекСтроки.Количество() <> 0 Тогда
		...
		#КонецЕсли
	Иначе
		ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выберите шаблоны, которые следует сохранить на диск, и повторите попытку.%1Для множественного выбора используйте клавишу Ctrl.';uk='Выберите шаблоны, которые следует сохранить на диск, и повторите попытку.%1Для множественного выбора используйте клавишу Ctrl.'"), Символы.ПС));
	КонецЕсли;
...
Процедура ПоказатьПредупреждениПослеВыгрузкиШаблонов(Знач КоличествоОшибок, Знач ТекСтроки)
	...
	Если ТекСтроки.Количество() = 1 Тогда
		Если КоличествоОшибок = 0 Тогда
			ПоказатьПредупреждение(,НСтр("ru='Шаблон успешно выгружен.';uk='Шаблон успешно выгружен.'"));
		КонецЕсли;
	Иначе
		Если КоличествоОшибок = 0 Тогда
			ПоказатьПредупреждение(,НСтр("ru='Шаблоны успешно выгружены.';uk='Шаблоны успешно выгружены.'"));
		Иначе
			ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Во время сохранения произошли ошибки. Не удалось сохранить шаблонов: %1.';uk='Во время сохранения произошли ошибки. Не удалось сохранить шаблонов: %1.'"), КоличествоОшибок));
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьШаблоны(ТекСтроки, КоличествоОшибок, ВозвращатьВСтруктуреСсылкуНаШаблонВоВременномХранилище = Ложь)
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Во время получения шаблона %1 возникла ошибка:%2';uk='Во время получения шаблона %1 возникла ошибка:%2'"), Выборка.ИмяФайлаШаблона, Символы.ПС + ОписаниеОшибки());
...
Процедура УдалитьШаблоныНаКлиенте()
	...
	Иначе
		...
		ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выберите шаблоны, которые следует удалить, и повторите попытку.%1Для множественного выбора используйте клавишу Ctrl.';uk='Выберите шаблоны, которые следует удалить, и повторите попытку.%1Для множественного выбора используйте клавишу Ctrl.'"), Символы.ПС));
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	#Если НЕ ВебКлиент Тогда
		...
		ТекстВопроса = НСтр("ru='Шаблон будет выгружен на диск и открыт. Продолжить?';uk='Шаблон будет выгружен на диск и открыт. Продолжить?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Процедура СписокВыборЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если НЕ АдресВременногоХранилища = Неопределено Тогда
		...
		Исключение
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось выгрузить шаблон во временный файл на диске:%1';uk='Не удалось выгрузить шаблон во временный файл на диске:%1'"), Символы.ПС + ОписаниеОшибки());
...
Функция ПолучитьШаблонНаСервере(ВыбраннаяСтрока)
	...
	Если Выборка.Следующий() Тогда
		...
		Исключение
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось получить шаблон из хранилища:%1';uk='Не удалось получить шаблон из хранилища:%1'"), Символы.ПС + ОписаниеОшибки());
