...
Процедура ОбновитьРодителейВладельца(ВладелецНастроекПрав, ЕстьИзменения = Ложь, ОбновитьИерархию = Ложь, ОбъектыСИзменениями = Неопределено) Экспорт
	...
	ЗаголовокОшибки =
		НСтр("ru='Ошибка при обновлении иерархии владельцев прав по значениям доступа.';uk='Помилка при оновленні ієрархії власників прав за значеннями "
"доступу.'")
		+ Символы.ПС
		+ Символы.ПС;
...
"не налаштовано використання налаштувань прав об`єктів.'"),
			Строка(ТипВладельца));
	КонецЕсли;
