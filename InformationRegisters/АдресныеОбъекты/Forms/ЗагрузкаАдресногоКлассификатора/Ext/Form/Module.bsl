...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Если КоличествоОбновлений > 0 Тогда
			...
			ИнформацияОбОбновление = НСтр("ru='Доступно обновление от';uk='Доступно обновление от'") + " " + ДатаОбновления;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Текст = НСтр("ru='Прервать загрузку адресного классификатора?';uk='Прервать загрузку адресного классификатора?'");
...
Процедура АдресЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	АдресныйКлассификаторКлиент.ВыбратьКаталог(ЭтотОбъект, "АдресЗагрузки", 
		НСтр("ru='Каталог с файлами адресного классификатора';uk='Каталог с файлами адресного классификатора'"),
...
"необходимо установить расширение для работы с файлами.';uk='Для загрузки адресного классификатора из папки"
"необходимо установить расширение для работы с файлами.'");
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю( НСтр("ru='Не указан вариант загрузки классификатора.';uk='Не указан вариант загрузки классификатора.'") );
	КонецЕсли;
...
Процедура ПослеПроверкиДоступностиОбновленияКлиент(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Выполнено" Тогда
		...
		Если Даты.ДатаОбновления > Даты.ДатаСамойСтаройЗагрузки Тогда
			...
			ИнформацияОбОбновление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Доступно обновление от %1';uk='Доступно обновление от %1'"), ДатаОбновленияСтрокой);
			Элементы.ИнформацияОбОбновление.Видимость = НЕ Элементы.АвторизацияНаСайтеПоддержкиПользователей.Видимость;
		КонецЕсли;
	КонецЕсли;
...
Функция ЗапуститьПроверкуДоступностиОбновления(УникальныйИдентификатор)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Проверка обновления для адресного классификатора';uk='Проверка обновления для адресного классификатора'");
...
Процедура ОбновитьИнтерфейсПоКоличествуЗагружаемых()
	...
	// Страница загрузки
	ТекстОписанияЗагрузки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Загружаются данные по выбранным регионам (%1)';uk='Загружаются данные по выбранным регионам (%1)'"), ВыбраноРегионовДляЗагрузки);
...
Процедура ЗагрузитьКлассификаторССайтаАутентификация(Знач Аутентификация, Знач КодыРегионов)
	...
	Элементы.ШагиЗагрузки.ТекущаяСтраница = Элементы.ОжиданиеЗагрузки;
	ТекстСостоянияЗагрузки = НСтр("ru='Загрузка файлов с сайта поддержки пользователей...';uk='Загрузка файлов с сайта поддержки пользователей...'");
...
Процедура ПослеФоновойЗагрузкиССайта(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Ошибка" Тогда
		Если СтрНайти(Задание.КраткоеПредставлениеОшибки, "404 Not Found") > 0 ИЛИ СтрНайти(Задание.КраткоеПредставлениеОшибки, "401 Unauthorized") > 0 Тогда
			ТекстОшибки = НСтр("ru='Не удается загрузить адресные сведения.';uk='Не удается загрузить адресные сведения.'");
			ТекстОшибки = ТекстОшибки + НСтр("ru='Возможные причины:';uk='Возможные причины:'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ru='• Некорректно введен или не введен логин и пароль;';uk='• Некорректно введен или не введен логин и пароль;'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ru='• Нет подключения к Интернету;';uk='• Нет подключения к Интернету;'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ru='• На сайте ведутся технические работы. Попробуйте повторить загрузку позднее.';uk='• На сайте ведутся технические работы. Попробуйте повторить загрузку позднее.'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ru='• Брандмауэр или другое промежуточное ПО (антивирусы и т.п.) блокируют попытки программы подключиться к Интернету;';uk='• Брандмауэр или другое промежуточное ПО (антивирусы и т.п.) блокируют попытки программы подключиться к Интернету;'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ru='• Подключение к Интернету выполняется через прокси-сервер, но его параметры не заданы в программе.';uk='• Подключение к Интернету выполняется через прокси-сервер, но его параметры не заданы в программе.'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ru='Техническая информация:';uk='Технічна інформація:'") + " " + Символы.ПС;
			ТекстОшибки = ТекстОшибки + СтрПолучитьСтроку(Задание.КраткоеПредставлениеОшибки, 1);
		Иначе
			ТекстОшибки = НСтр("ru='Вероятно в данный момент на сайте ведутся технические работы. Попробуйте повторить загрузку позднее.';uk='Вероятно в данный момент на сайте ведутся технические работы. Попробуйте повторить загрузку позднее.'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ru='Техническая информация:';uk='Технічна інформація:'") + " " + Символы.ПС;
...
Процедура ЗавершениеЗагрузки()
	...
	// Для сброса признака АдресныйКлассификаторУстарел в параметрах работы клиента.
		...
		ТекстОписанияЗагрузки = НСтр("ru='Адресный классификатор успешно загружен.';uk='Адресный классификатор успешно загружен.'");
		...
		Если ЕстьУстаревшиеАдресныеСведенияКЛАДРДляУдаления() Тогда
			...
			ТекстВопроса = НСтр("ru='Найдены устаревшие сведения о регионах в формате КЛАДР, которые не используются в программе.';uk='Найдены устаревшие сведения о регионах в формате КЛАДР, которые не используются в программе.'");
			ТекстВопроса = ТекстВопроса + Символы.ПС + НСтр("ru='Если загруженных регионов много, то процесс очистки может занять продолжительное время.';uk='Если загруженных регионов много, то процесс очистки может занять продолжительное время.'");
			ТекстВопроса = ТекстВопроса + Символы.ПС + НСтр("ru='Очистить?';uk='Очистить?'");
...
Функция ЗапуститьФоновуюЗагрузкуССайтаНаСервере(КодыРегионов, Аутентификация, УникальныйИдентификатор)
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Загрузка адресного классификатора с сайта';uk='Загрузка адресного классификатора с сайта'");
...
Процедура ЗапуститьФоновуюЗагрузкуИзКаталогаКлиента(Знач ПараметрыЗагрузки)
	...
	Если ПараметрыЗагрузки = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТекстСостоянияЗагрузки = НСтр("ru='Передача файлов на сервер приложения...';uk='Передача файлов на сервер приложения...'");
...
Процедура ПослеФоновойЗагрузкиИзКаталога(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Ошибка" Тогда
		ТекстОшибки = НСтр("ru='Не удается загрузить адресные сведения из файлов.';uk='Не удается загрузить адресные сведения из файлов.'");
		ТекстОшибки = ТекстОшибки + НСтр("ru='Необходимо сохранить файлы с сайта «1С» http://its.1c.ru/download/fias на диск, а затем загрузить в программу.';uk='Необходимо сохранить файлы с сайта «1С» http://its.1c.ru/download/fias на диск, а затем загрузить в программу.'") + Символы.ПС;
		ТекстОшибки = ТекстОшибки + НСтр("ru='Техническая информация:';uk='Технічна інформація:'") + Символы.ПС + Задание.КраткоеПредставлениеОшибки;
		ВывестиСообщениеОбОшибке(ТекстОшибки);
...
Функция ЗапуститьФоновуюЗагрузкуНаСервере(Знач КодыРегионов, Знач ОписаниеФайловЗагрузки, Знач Режим, Знач УникальныйИдентификатор)
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Загрузка адресного классификатора';uk='Загрузка адресного классификатора'");
...
Процедура ОчиститьУстаревшиеАдресныеСведенияКЛАДР()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Очистка адресного классификатора в формате КЛАДР';uk='Очистка адресного классификатора в формате КЛАДР'");
	...
	Если ФоновоеЗадание.Статус = "Ошибка" Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Очистка адресного классификатора в формате КЛАДР';uk='Очистка адресного классификатора в формате КЛАДР'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Предупреждение,
