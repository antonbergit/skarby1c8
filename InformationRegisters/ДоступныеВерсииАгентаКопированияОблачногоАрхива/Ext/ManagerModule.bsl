...
Процедура ЗагрузитьСтандартныеЗначения() Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		СтрШаблон(
			НСтр("ru='Загрузка стандартных значений (%1)';uk='Загрузка стандартных значений (%1)'"),
			ИмяСвойства));
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		...
		СтрШаблон(
			НСтр("ru='Облачный архив. Сервис и регламент. Загрузка стандартных значений. %1';uk='Облачный архив. Сервис и регламент. Загрузка стандартных значений. %1'"),
...
Процедура ЗагрузитьСтандартноеЗначение(НаборЗаписей, ТекущаяЗаписьХДТО, ОбъектМетаданных, ИмяСвойства, КонтекстВыполнения)
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Загрузка записи регистра сведений';uk='Загрузка записи регистра сведений'"));
...
"%1';uk='Не удалось добавить запись в регистр сведений по причине:"
"%1'"),
