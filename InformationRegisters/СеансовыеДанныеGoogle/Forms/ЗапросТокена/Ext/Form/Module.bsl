...
Процедура ОбработатьВводТокенаЗапроса(Команда)
	...
	ПоказатьОповещениеПользователя(НСтр("ru='Выполняется обработка токена запроса';uk='Виконується обробка токена запиту'"));
...
Процедура ОбработатьСобытиеБраузерДокументСформирован()
	...
	Если СтруктураЗаголовка.Статус=СтатусОтказ() Тогда
		...
		НСтр("ru='Ошибка при получении токена доступа';uk='Помилка при одержанні токена доступу'"),,
		КодОшибки,
...
Функция ТокенЗапросаНаСервере(ПараметрСеансаАвторизации)
	...
	Возврат РегистрыСведений.ДанныеАвторизацииGoogle.ТокенЗапроса(ПараметрСеансаАвторизации);
...
Процедура ОбработатьПолучениеТокенаЗапроса()
	...
	ПоказатьОповещениеПользователя(
	НСтр("ru='Токен запроса получен, выполняется обмен на токен доступа...';uk='Токен запиту отриманий, виконується обмін на токен доступу...'"),,,
	...
	Иначе
		ПоказатьОповещениеПользователя(
		НСтр("ru='Токен доступа получен успешно';uk='Токен доступу отриманий успішно'"),,,
...
Функция СтруктураЗаголовка(Знач ЗаголовокВебСтраницы)
	...
	Результат.Параметры = УправлениеНебольшойФирмойКлиентСервер.ПолучитьПараметрыИзСтроки(КомпонентыЗаголовка[1], "&");
	Возврат Результат;
