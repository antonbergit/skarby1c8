...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ТребуетсяПроверкаНаДубли Тогда
		...
		Если НЕ Запрос.Выполнить().Пустой() Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(
					НСтр("ru='Настройка обмена для организации %1 и склада %2 уже существует';uk='Настройка обмена для организации %1 и склада %2 уже существует'"),
					Запись.Организация,
	...
	Если Не ЗначениеЗаполнено(ИНН) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не заполнено поле ""ИНН""';uk='Не заполнено поле ""ИНН""'"),, "ИНН",, Отказ);
	КонецЕсли;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если ИзмененФорматОбмена Тогда
		ПоказатьВопрос(
			...
			НСтр("ru='Изменился формат обмена с ЕГАИС. Выгрузить информацию об изменении формата в УТМ?';uk='Изменился формат обмена с ЕГАИС. Выгрузить информацию об изменении формата в УТМ?'"),
...
Процедура ПриИзмененииВариантаПодключения()
	...
	Если ОбменНаСервере = 0 Тогда
		Подсказка = НСтр("ru='Подключение к УТМ будет осуществляться с компьютера пользователя';uk='Подключение к УТМ будет осуществляться с компьютера пользователя'");
	ИначеЕсли ОбменНаСервере = 1 Тогда
		Подсказка = НСтр("ru='Подключение к УТМ будет осуществляться с компьютера, на котором установлен сервер 1С:Предприятия';uk='Подключение к УТМ будет осуществляться с компьютера, на котором установлен сервер 1С:Предприятия'");
	КонецЕсли;
...
Процедура УстановитьНадписьВерсия()
	...
	Если НЕ Запись.ОбработкаОбслуживания.Пустая() Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияОбработки, ИнтеграцияЕГАИСКлиентСервер.ВерсияПодсистемыЕГАИС()) < 0 Тогда
			...
			Элементы.ОбработкаОбслуживанияВерсия.Подсказка = НСтр("ru='Используется устаревшая версия обработки';uk='Используется устаревшая версия обработки'");
		Иначе
			Элементы.ОбработкаОбслуживанияВерсия.ЦветТекста = Новый Цвет(0, 128, 0);
			Элементы.ОбработкаОбслуживанияВерсия.Подсказка = НСтр("ru='Используемая версия актуальна';uk='Используемая версия актуальна'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыгрузкаИнформацииОФорматеОбмена_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ПоказатьПредупреждение(, НСтр("ru='Информация об изменении формата обмена успешно выгружена в УТМ.';uk='Информация об изменении формата обмена успешно выгружена в УТМ.'"));
