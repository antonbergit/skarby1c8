...
Процедура НовыйШтрихкод(Команда)
	...
	Если ИспользоватьОбменСПодключаемымОборудованиемOffline Тогда
		...
		ПоказатьВводЧисла(Новый ОписаниеОповещения("НовыйШтрихкодЗавершение", ЭтотОбъект, Новый Структура("ПрефиксВесовогоТовара", ПрефиксВесовогоТовара)), ПрефиксВесовогоТовара, НСтр("ru='Если товар весовой, то введите префикс весового товара или нажмите кнопку Отмена';uk='Якщо товар ваговий, то введіть префікс вагового товару або натисніть кнопку Скасування'"), 1, 0);
	Иначе
	...
	Если Выборка.Следующий() // Штрихкод уже записан в БД
	   И Запись.ИсходныйКлючЗаписи.Штрихкод <> Запись.Штрихкод Тогда
		...
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Номенклатура%", """" + Выборка.НоменклатураПредставление + """"
		                ...
		                + ?(ЗначениеЗаполнено(Выборка.Партия), " """ + НСтр("ru='с партией';uk='с партією'") + " " + Выборка.ПартияПредставление + """", ""));
