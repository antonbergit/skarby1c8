...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Для каждого ТекущаяЗапись Из ЭтотОбъект Цикл
		...
		Если ВРег(ТекущаяЗапись.КатегорияНовостей.Код) = ВРег("ProductVersion")
				ИЛИ ВРег(ТекущаяЗапись.КатегорияНовостей.Код) = ВРег("PlatformVersion") Тогда
			ТекстСообщения = НСтр("ru='В простых категориях не должно быть категории с кодом %ТекущаяЗапись.КатегорияНовостей.Код%';uk='У простих категоріях не повинне бути категорії з кодом %ТекущаяЗапись.КатегорияНовостей.Код%'");
