...
Процедура ОтборПриИзменении(Элемент)
	...
	Состояние(НСтр("ru='Выполняется обновление таблицы товаров...';uk='Виконується відновлення таблиці товарів...'"));
...
Процедура ЗарегистрироватьИзменения(Команда)
	...
	Если МассивКодов.Количество() > 0 Тогда
		...
		Иначе
			ПоказатьПредупреждение(Неопределено, НСтр("ru='В процессе регистрации изменений произошла ошибка:';uk='У процесі реєстрації змін відбулася помилка:'") + " " + ОписаниеОшибки);
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Не выбраны строки для регистрации изменений';uk='Не обрані рядки для реєстрації змін'"));
	КонецЕсли;
...
Процедура УдалитьРегистрациюИзмененийДляВыделенныхСтрок(Команда)
	...
	Если МассивКодов.Количество() > 0 Тогда
		...
		Если Результат Тогда
			...
			Оповестить("Запись_КодыТоваровПодключаемогоОборудования", Новый Структура, Неопределено);
		Иначе
			ПоказатьПредупреждение(Неопределено, НСтр("ru='В процессе удаления регистрации изменений произошла ошибка:';uk='У процесі видалення реєстрації змін відбулася помилка:'") + " " + ОписаниеОшибки);
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Не выбраны строки для удаления регистрации изменений';uk='Не обрані рядки для видалення реєстрації змін'"));
	КонецЕсли;
