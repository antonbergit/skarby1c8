...
Процедура ПриСозданииНаСервере(Форма, Представление, ИмяОсновногоРеквизитаФормы = "Объект") Экспорт
	...
	РеквизитИзмененоПредставление = Новый РеквизитФормы("ИзмененоПредставление", Новый ОписаниеТипов("Булево"), , НСтр("ru='Изменено представление';uk='Змінено представлення'"));	
...
Функция СтруктураСклоненияЧерезЗапросКСервису(Представление, ПоказыватьСообщения) 
	...
	Если Результат <> Неопределено Тогда
		...
		Если Не ПустаяСтрока(СообщениеОбОшибке) Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Вызов веб-сервиса склонения';uk='Виклик веб-сервісу схиляння'", КодОсновногоЯзыка), УровеньЖурналаРегистрации.Предупреждение, , ,СообщениеОбОшибке);
		КонецЕсли;	
		
	Иначе
		
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
		ЗаписьЖурналаРегистрации(НСтр("ru='Вызов веб-сервиса склонения';uk='Виклик веб-сервісу схиляння'", КодОсновногоЯзыка), УровеньЖурналаРегистрации.Ошибка, , ,СообщениеОбОшибке);
		
		Если ПоказыватьСообщения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка при вызове сервиса склонения. Обратитесь к администратору.';uk='Помилка при виклику сервісу відміни. Зверніться до адміністратора.'") + Символы.ПС + НСтр("ru='Техническая информация:';uk='Технічна інформація:'") + Символы.ПС + СообщениеОбОшибке);
		КонецЕсли;
