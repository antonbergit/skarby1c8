...
"Сохранить файл на компьютер и открыть его каталог?';uk='Каталог файлу не існує. Можливо, на цьому комп`ютері файл ""%1"" ще не відкривався."
"Зберегти файл на комп`ютер і відкрити його каталог?'"),
				Файл.Имя);
...
Процедура ПоместитьПрисоединенныйФайл(Оповещение, ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Контекст.Вставить("ЗаголовокОшибки",
		НСтр("ru='Не удалось поместить файл с компьютера в хранилище файлов по причине:';uk='Не вдалося помістити файл із комп`ютера в сховище файлів через:'") + Символы.ПС);
...
Процедура ПоместитьПрисоединенныйФайлПослеПодключенияРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	Если Не РасширениеПодключено Тогда
		...
		Результат.Вставить("ОписаниеОшибки", Контекст.ЗаголовокОшибки
			+ НСтр("ru='В обозреватель интернет не установлено расширение для работы с файлами.';uk='В оглядач інтернет не встановлене розширення для роботи з файлами.'"));
...
Процедура ПоместитьПрисоединенныйФайлПослеПолученияРабочегоКаталога(Результат, Контекст) Экспорт
	...
	Если Контекст.ПолноеИмяФайла <> Контекст.ПолноеИмяПомещаемогоФайла Тогда
		...
		Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки
			+ НСтр("ru='Создание каталога не выполнено по причине:';uk='Створення каталогу не виконане через:'"));
		...
		Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки
			+ НСтр("ru='Изменение свойства файла ""Только просмотр"" не выполнено по причине:';uk='Зміна властивості файлу ""Тільки перегляд"" не виконане через:'"));
		ДействияСФайлом.Добавить(Действие);
		...
		Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки
			+ НСтр("ru='Копирование файла не выполнено по причине:';uk='Копіювання файлу не виконане через:'"));
		ДействияСФайлом.Добавить(Действие);
	...
	Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки
		+ НСтр("ru='Получение свойств файла не выполнено по причине:';uk='Одержання властивостей файлу не виконане через:'"));
	ДействияСФайлом.Добавить(Действие);
...
Процедура ПолучитьПрисоединенныйФайл(Оповещение, ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Контекст.Вставить("ЗаголовокОшибки",
		НСтр("ru='Не удалось получить файл на компьютер из хранилища файлов по причине:';uk='Не вдалося одержати файл на комп`ютер зі сховища файлів через:'") + Символы.ПС);
	...
	Если Контекст.ДляРедактирования
	   И Контекст.ДанныеФайла.Редактирует <> ПользователиКлиентСервер.АвторизованныйПользователь() Тогда
		...
		Результат.Вставить("ОписаниеОшибки", Контекст.ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Файл уже редактирует пользователь %1.';uk='Файл вже редагує користувач %1.'"), Строка(Контекст.ДанныеФайла.Редактирует)));
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Результат);
		Возврат;
	КонецЕсли;
...
Процедура ПолучитьПрисоединенныйФайлПослеПолученияРабочегоКаталога(Результат, Контекст) Экспорт
	...
	Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки
		+ НСтр("ru='Установка свойств файла не выполнено по причине:';uk='Установка властивостей файлу не виконане через:'"));
	ДействияСФайлом.Добавить(Действие);
...
Процедура ПолучитьРасшифрованныеДанные(ОбработчикРезультата, Знач ПрисоединенныйФайл, Знач ДанныеФайла, Знач ИдентификаторФормы) Экспорт
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция",              НСтр("ru='Расшифровка файла';uk='Розшифрування файлу'"));
	ОписаниеДанных.Вставить("ЗаголовокДанных",       НСтр("ru='Файл';uk='Файл'"));
	ОписаниеДанных.Вставить("Данные",                ДанныеФайла.СсылкаНаДвоичныеДанныеФайла);
	ОписаниеДанных.Вставить("Представление",         ДополнительныеПараметры.ПрисоединенныйФайл);
