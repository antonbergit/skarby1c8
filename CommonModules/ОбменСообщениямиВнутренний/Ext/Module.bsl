        ...
        Попытка
            ...
            Попытка
                ...
                Если Выборка.КонечнаяТочкаЗаблокирована Тогда
                    ОбъектСообщения.Заблокировано = Истина;
                    ВызватьИсключение НСтр("ru='Попытка обработки сообщения, полученного от заблокированной конечной точки.';uk='Спроба обробки повідомлення, отриманого від заблокованої "
"кінцевої точки.'");
                КонецЕсли;
...
Процедура УстановитьВедущуюКонечнуюТочкуНаСторонеПолучателя(КодЭтойКонечнойТочки, КодВедущейКонечнойТочки) Экспорт
	...
	Если ПланыОбмена.ОбменСообщениями.НайтиПоКоду(КодЭтойКонечнойТочки) <> ЭтотУзел() Тогда
		СтрокаСообщенияОбОшибке = НСтр("ru='Заданы неверные параметры подключения к конечной точке. Параметры подключения указывают на другую конечную точку.';uk='Задані невірні параметри підключення до кінцевої точки. "
"Параметри підключення вказують на іншу кінцеву точку.'");
