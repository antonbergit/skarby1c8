...
"</DIV><BR>';uk='<DIV STYLE=""DISPLAY: BLOCK; WIDTH: 100%; MARGIN-BOTTOM: 5px; %1"">"
"<DIV STYLE=""WIDTH: 9em; FLOAT: LEFT; COLOR: gray;"">%2</DIV>"
"<DIV STYLE=""WIDTH: 12em; FLOAT: LEFT; COLOR: gray;"">%3</DIV>"
"<DIV STYLE=""MARGIN-LEFT: 21em;"">%4</DIV>"
"</DIV><BR>'"),
		...
		Если НачалоДня(ТекущаяДатаСеанса())=НачалоДня(Выборка.МоментВремени) Тогда
			МоментВремениСтрокой = НСтр("ru='Сегодня в ';uk='Сьогодні в '")+Формат(Выборка.МоментВремени, НСтр("ru='ДФ=ЧЧ:мм';uk='ДФ=ЧЧ:мм'"));
		ИначеЕсли НачалоДня(НачалоДня(ТекущаяДатаСеанса())-1)=НачалоДня(Выборка.МоментВремени) Тогда
			МоментВремениСтрокой = НСтр("ru='Вчера в ';uk='Вчора в '")+Формат(Выборка.МоментВремени, НСтр("ru='ДФ=ЧЧ:мм';uk='ДФ=ЧЧ:мм'"));
		ИначеЕсли Год(Выборка.МоментВремени)<>Год(ТекущаяДатаСеанса()) Тогда 
			МоментВремениСтрокой = Формат(Выборка.МоментВремени, НСтр("ru=""ДФ='дд МММ гггг ЧЧ:мм'"";uk=""ДФ='дд МММ гггг ЧЧ:мм'"""))
		Иначе
			МоментВремениСтрокой = Формат(Выборка.МоментВремени, НСтр("ru=""ДФ='дд МММ ЧЧ:мм'"";uk=""ДФ='дд МММ ЧЧ:мм'"""))
		КонецЕсли; 
		Результат = Результат+СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='<DIV STYLE=""DISPLAY: BLOCK; WIDTH: 100%; MARGIN-BOTTOM: 5px; %1"">"
"<DIV STYLE=""WIDTH: 9em; FLOAT: LEFT; COLOR: gray;"">%2</DIV>"
...
"</DIV><BR>'"),
		...
		//?(ПредПользователь=Выборка.Пользователь, "-", HTMLСсылка(Выборка.Пользователь)),
		HTMLСсылка(Выборка.Пользователь),
		ТекстКомментария);
		Результат = Результат+Символы.ПС;
		//ПредПользователь = Выборка.Пользователь;
...
Процедура ДобавитьСообщение(
	...
	Если НЕ Системный Тогда
		...
		Для каждого СсылкаУпоминание Из МассивСсылокУпоминаний Цикл
			КомментарийУпоминание = НСтр("ru='Упоминание в: ';uk='Згадування в:'")+HTMLСсылка(Ссылка);
...
Процедура ОбсужденияПриЗаписи(Источник, Отказ) Экспорт
	...
	Если Источник.ДополнительныеСвойства.Свойство("ЭтоНовый") Тогда
		Если ОбщегоНазначения.ЭтоДокумент(Источник.Метаданные()) Тогда
			Сообщение = ?(Источник.Проведен, НСтр("ru='Создан и проведен';uk='Створений і проведений'"), НСтр("ru='Создан';uk='Створений'"));
		Иначе
			Сообщение = НСтр("ru='Создан';uk='Створений'");
		КонецЕсли; 
...
Функция ОбсужденияСравнитьОбъектыСправочник(Источник)
	...
	СтруктураРеквизитов = РеквизитыДляПроверкиИзмененийСправочник();
	ОписаниеИзменений = "";
	Сообщение = "";
	...
	Если НовыйОбъект.Метаданные().ТабличныеЧасти.Найти("КонтактнаяИнформация")<>Неопределено Тогда
		СтараяТЧ = СтарыйОбъект.КонтактнаяИнформация;
...
Функция ОбсужденияСравнитьОбъектыДокумент(Источник, РежимЗаписи, РежимПроведения)
	...
	СтруктураРеквизитов = РеквизитыДляПроверкиИзмененийДокумент();
	ОписаниеИзменений	= ОписаниеИзмененийРеквизитов(СтруктураРеквизитов, СтарыйОбъект, НовыйОбъект, ОписаниеИзменений);

	// ТЧ ТоварыИУслуги
	Если НовыйОбъект.Метаданные().ТабличныеЧасти.Найти("Запасы")<>Неопределено Тогда
	
		СтараяТЧ = СтарыйОбъект.Запасы;
		НоваяТЧ = НовыйОбъект.Запасы;
		
		Если СтараяТЧ.Количество()>0 И ЕстьРеквизитОбъекта("Всего", СтараяТЧ[0])
			И НЕ СтараяТЧ.Итог("Всего")=НоваяТЧ.Итог("Всего")
				Тогда
			ОписаниеИзменений = ОписаниеИзменений + ?(ПустаяСтрока(ОписаниеИзменений), "", Символы.ПС)+
			НСтр("ru='Изменена табличная часть';uk='Змінена таблична частина'");
		ИначеЕсли СтараяТЧ.Количество()>0 И ЕстьРеквизитОбъекта("Сумма", СтараяТЧ[0])
			И НЕ СтараяТЧ.Итог("Сумма")=НоваяТЧ.Итог("Сумма")
				Тогда
			ОписаниеИзменений = ОписаниеИзменений + ?(ПустаяСтрока(ОписаниеИзменений), "", Символы.ПС)+
			НСтр("ru='Изменена табличная часть';uk='Змінена таблична частина'");
		КонецЕсли;
	...
	Если НовыйОбъект.Метаданные().ТабличныеЧасти.Найти("РасшифровкаПлатежа")<>Неопределено Тогда
		
		СтараяТЧ = СтарыйОбъект.РасшифровкаПлатежа;
	...
	Если ОписаниеИзменений="" И Источник.ДополнительныеСвойства.Свойство("Модифицированность") И Источник.ДополнительныеСвойства.Модифицированность=Истина Тогда
		...
		ОписаниеИзменений=НСтр("ru='Внесены изменения';uk='Внесені зміни'")
	КонецЕсли;
...
Процедура ДобавитьЗаписьУпоминаниеКонтрагента(СтарыйОбъект=Неопределено, НовыйОбъект)
	...
	Если СтарыйОбъект=Неопределено  Тогда
		...
		Сообщение = НСтр("ru='Добавлен в : ';uk='Доданий в :'")+HTMLСсылка(НовыйОбъект.Ссылка);
		ДобавитьСообщение(
			Сообщение, 
	...
	ИначеЕсли СтарыйОбъект.Контрагент<>НовыйОбъект.Контрагент Тогда
		...
		Сообщение = НСтр("ru='Удален из : ';uk='Видалений з :'")+HTMLСсылка(СтарыйОбъект.Ссылка);
		ДобавитьСообщение(
			Сообщение, 
...
Процедура ДобавитьЗаписьВводНаОсновании(Источник)
	...
	Сообщение = НСтр("ru='Использован в: ';uk='Використаний в:'")+HTMLСсылка(Источник.Ссылка);
...
Функция РеквизитыДляПроверкиИзмененийДокумент()
...
Функция РеквизитыДляПроверкиИзмененийСправочник()
...
Процедура ДобавитьТекстСтандартныхСообщений(Сообщение, СтарыйОбъект, НовыйОбъект, РежимЗаписи, РежимПроведения)
	...
	Если НЕ СтарыйОбъект.Проведен И РежимЗаписи=РежимЗаписиДокумента.Проведение Тогда
		Сообщение = НСтр("ru='Проведен';uk='Проведений'");
	КонецЕсли; 
	
	Если СтарыйОбъект.Проведен И РежимЗаписи=РежимЗаписиДокумента.ОтменаПроведения Тогда
		Сообщение = НСтр("ru='Проведение отменено';uk='Проведення скасоване'");
	КонецЕсли; 
	
	Если НЕ СтарыйОбъект.ПометкаУдаления И НовыйОбъект.ПометкаУдаления Тогда
		Сообщение = НСтр("ru='Помечен на удаление';uk='Позначений на видалення'");
	КонецЕсли; 
	
	Если СтарыйОбъект.ПометкаУдаления И НЕ НовыйОбъект.ПометкаУдаления Тогда
		Сообщение = НСтр("ru='Снята пометка на удаление';uk='Знята позначка на видалення'");
	КонецЕсли;
...
Функция ОписаниеСравненияМассивов(МассивСтарыхЗначений, МассивНовыхЗначений)
	...
	Если СписокДобавленных.Количество()>0 И СписокУдаленных.Количество()>0 Тогда
		Результат = НСтр("ru='Удалены: ';uk='Видалені:'")+Строка(СписокУдаленных)+", "+НСтр("ru='Добавлены: ';uk='Додані:'")+Строка(СписокДобавленных);;
		Возврат Результат;
	КонецЕсли; 
	
	Если СписокУдаленных.Количество()>0 Тогда
		Результат = НСтр("ru='Удалены: ';uk='Видалені:'")+Строка(СписокУдаленных);
	КонецЕсли;
	
	Если СписокДобавленных.Количество()>0 Тогда
		Результат = НСтр("ru='Добавлены: ';uk='Додані:'")+Строка(СписокДобавленных);
	КонецЕсли;
...
Функция HTMLСсылка(Ссылка)
	...
	СсылкаСтрокой = СтрЗаменить(СсылкаСтрокой, НСтр("ru=' (не проведен)';uk=' (не проведений)'"), "");
	СсылкаСтрокой = СтрЗаменить(СсылкаСтрокой, НСтр("ru=' (удален)';uk=' (видалений)'"), "");
...
Функция ПолучитьСсылкуИзНавигационной(НС) Экспорт
	...
	Исключение
		Возврат НСтр("ru='Ошибка преобразования: ';uk='Ошибка преобразования: '")+НС;
	КонецПопытки; 
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	ГруппаДел	= НСтр("ru='Напоминания';uk='Нагадування'");
	...
	Дело.Важное			= Ложь;
	Дело.Представление	= НСтр("ru='Мои напоминания';uk='Мої нагадування'");
...
Процедура СоставПолейЗаполненияДляРабочегоПроцесса(ТаблицаОписанияПолей, знач ТипДействия) Экспорт
	...
	НовоеОписание.Заголовок					= НСтр("ru='Кому напомнить';uk='Кому напомнить'");
	...
	НовоеОписание.Заголовок					= НСтр("ru='Дата напоминания';uk='Дата нагадування'");
