...
"%1';uk='Ошибка создания менеджер криптографии (%2, %3, %4) по причине:"
"%1'"),
			СписокОшибок,
			ВходящийКонтекст.ДоступныеКриптопровайдеры[ВходящийКонтекст.ТекущийКриптопровайдер].Имя, 
			ВходящийКонтекст.ДоступныеКриптопровайдеры[ВходящийКонтекст.ТекущийКриптопровайдер].Путь, 
...
Процедура СоздатьМенеджерКриптографииПослеПолученияСпискаКриптопровайдеров(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Иначе
			СписокОшибок = НСтр("ru='Менеджер криптографии успешно инициализирован с указанными параметрами криптопровайдера, но криптопровайдер не найден в системе.';uk='Менеджер криптографии успешно инициализирован с указанными параметрами криптопровайдера, но криптопровайдер не найден в системе.'");		
...
Процедура ПоказатьСертификатПослеПоиска(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Иначе
			...
			СписокСертификатов.ПоказатьВыборЭлемента(Оповещение, НСтр("ru='Выберите сертификат для детального просмотра';uk='Выберите сертификат для детального просмотра'"));
		КонецЕсли;
	Иначе
...
Процедура ОтобразитьОдинСертификатВПолеВвода(ДополнительныеПараметры)
	...
	Если ТипЗнчСертификат <> Тип("Структура") И ТипЗнчСертификат <> Тип("Соответствие") Тогда
		...
		Иначе
			...
			Результат.Вставить("ОписаниеОшибки", 	НСтр("ru='Не установлена компонента работы с криптографией';uk='Не установлена компонента работы с криптографией'"));
...
Процедура ОтобразитьОдинСертификатВПолеВводаПослеПолученияСвойствСертификата(Результат, ВходящиеПараметры) Экспорт
	...
	Иначе
		...
		ПолеВвода.РасширеннаяПодсказка.Заголовок	= Новый ФорматированнаяСтрока(
			НСтр("ru='Сертификат не зарегистрирован в системном хранилище сертификатов';uk='Сертификат не зарегистрирован в системном хранилище сертификатов'"),,
			Новый Цвет(255, 0, 0));
...
Процедура ОтобразитьОдинСертификатВПолеВводаПоСвойствамСертификата(Знач СвойстваСертифката, ВходящиеПараметры)
	...
	Если ТекДата > СвойстваСертифката.ДействителенПо Тогда
		...
		Подсказка = Новый ФорматированнаяСтрока(
			НСтр("ru='Срок действия сертификата истек';uk='Срок действия сертификата истек'"),,
			Новый Цвет(255, 0, 0));
	...
	ИначеЕсли ТекДата < СвойстваСертифката.ДействителенС Тогда
		...
		Подсказка = Новый ФорматированнаяСтрока(
			НСтр("ru='Срок действия сертификата еще не наступил';uk='Срок действия сертификата еще не наступил'"),,
			Новый Цвет(255, 0, 0));
...
Процедура ОтобразитьНесколькоСертификатовВПолеВводаПослеПолученияСвойствСертификатов(Результат, ВходящиеПараметры) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Если ОшибкиЕсть Тогда
			ПолеВвода.ЦветТекста = Новый Цвет(255, 0, 0);
			Подсказка = Новый ФорматированнаяСтрока(
				НСтр("ru='Один или несколько сертификатов недействительны';uk='Один или несколько сертификатов недействительны'"),,
				Новый Цвет(255, 0, 0));
			ПолеВвода.РасширеннаяПодсказка.Заголовок = Подсказка;
			ПолеВвода.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСнизу;
...
"не зарегистрированы в системном хранилище сертификатов';uk='Обратите внимание, один или несколько сертификатов "
...
Функция ПредставлениеЧислаСертификатов(ЧислоСертификатов)
	...
	Если ЧислоСертификатов = 0 Тогда
		Результат = НСтр("ru='Сертификат не выбран';uk='Сертификат не выбран'");
	ИначеЕсли ЧислоСертификатов = 1 Тогда
		Результат = НСтр("ru='1 сертификат';uk='1 сертификат'");
	ИначеЕсли ЧислоСертификатов >= 2 И ЧислоСертификатов <= 4 Тогда
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1  сертификата';uk='%1  сертификата'"), ЧислоСертификатов);
	Иначе // считаем, что больше 20 сертификатов не будет, иначе следует дописать
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1  сертификатов';uk='%1  сертификатов'"), ЧислоСертификатов);
	КонецЕсли;
...
Функция ОписаниеОшибкиНеУдалосьПодключитьВнешнююКомпонентуКриптографии()
	...
	Возврат НСтр("ru='Не удалось подключить внешнюю компоненту для работы с криптографией.';uk='Не удалось подключить внешнюю компоненту для работы с криптографией.'");

КонецФункции

Функция ОписаниеОшибкиНеУдалосьСоздатьМенеджерКриптографии()
	
	Возврат НСтр("ru='Не удалось создать объект для работы с криптографией. В системе не обнаружен криптопровайдер.';uk='Не удалось создать объект для работы с криптографией. В системе не обнаружен криптопровайдер.'");
	
КонецФункции

Функция ОписаниеОшибкиОбщийТекстПриОшибкахВМенеджереКриптографии()
	
	Возврат НСтр("ru='Ошибка внешней компоненты для работы с криптографией.';uk='Ошибка внешней компоненты для работы с криптографией.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьСформироватьЗапросНаСертификат()
	
	Возврат НСтр("ru='Криптографическая ошибка: не удалось сформировать запрос на сертификат для контролирующих органов.';uk='Криптографическая ошибка: не удалось сформировать запрос на сертификат для контролирующих органов.'");
...
"Возможно выбран не верный закрытый ключ.';uk='Не удалось привязать сертификат к закрытому ключу."
...
Функция ОписаниеОшибкиНеУдалосьУстановитьЛичныйСертификат()
	...
	Возврат НСтр("ru='Не удалось установить личный сертификат.';uk='Не вдалося встановити особистий сертифікат.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьВыгрузитьСертификат()
	
	Возврат НСтр("ru='Не удалось выгрузить сертификат.';uk='Не удалось выгрузить сертификат.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьИмпортироватьСертификат()
	
	Возврат НСтр("ru='Не удалось зарегистрировать сертификат в системном хранилище.';uk='Не удалось зарегистрировать сертификат в системном хранилище.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьПодписатьФайл()

	Возврат НСтр("ru='Не удалось подписать файл.';uk='Не удалось подписать файл.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьПодписатьДанные()
	
	Возврат НСтр("ru='Не удалось подписать данные.';uk='Не удалось подписать данные.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьПроверитьПодпись()

	Возврат НСтр("ru='Не удалось проверить подпись.';uk='Не удалось проверить подпись.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьЗашифроватьФайл()

	Возврат НСтр("ru='Не удалось зашифровать файл.';uk='Не удалось зашифровать файл.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьПодписатьИЗашифроватьФайл()

	Возврат НСтр("ru='Не удалось подписать и зашифровать файл.';uk='Не удалось подписать и зашифровать файл.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьРасшифроватьФайл()

	Возврат НСтр("ru='Не удалось расшифровать файл.';uk='Не удалось расшифровать файл.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьПолучитьСвойстваПодписи()

	Возврат НСтр("ru='Не удалось получить свойства подписи.';uk='Не удалось получить свойства подписи.'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьПолучитьХешФайла()
	
	Возврат НСтр("ru='Не удалось получить хеш файла.';uk='Не удалось получить хеш файла.'");
...
Функция ОписаниеОшибкиНеУдалосьНайтиСертификат(Сертификат = Неопределено)
	...
	ОписаниеОшибки = НСтр("ru='Не удалось найти сертификат <%1>  в системном хранилище.';uk='Не удалось найти сертификат <%1>  в системном хранилище.'");
...
Функция ОписаниеОшибкиНеВведенПарольДоступаКЗакрытомуКлючу()
	...
	Возврат НСтр("ru='Операция прервана: требуется ввести пароль доступа к закрытому ключу!';uk='Операция прервана: требуется ввести пароль доступа к закрытому ключу!'");
	
КонецФункции

Функция ОписаниеОшибкиНеУдалосьОткрытьСертификат()

	Возврат НСтр("ru='Не удалось открыть сертификат.';uk='Не удалось открыть сертификат.'");
	
КонецФункции

Функция ОписаниеОшибкиОшибкаАвторизации()
	
	Возврат НСтр("ru='Ошибка авторизации на сервере криптографии.';uk='Ошибка авторизации на сервере криптографии.'");
...
Функция ПолучитьСписокХранилищСертификатов(ХранилищеСертификатов)
	...
	ИначеЕсли ТипЗнч(ХранилищеСертификатов) = Тип("Структура") Тогда
		...
		КонецЕсли;
	Иначе
		ВызватьИсключение(НСтр("ru='Параметр <ХранилищеСертификатов> должен иметь тип Структура, Массив или Неопределено.';uk='Параметр <ХранилищеСертификатов> должен иметь тип Структура, Массив или Неопределено.'"));
	КонецЕсли;
...
"%2"
"по причине:"
"%3';uk='Ошибка при вызове ""%1"" менеджера криптографии."
"по причине:"
"%2"
"по причине:"
"%3'"),
	...
	Иначе
		Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при вызове ""%1"" менеджера криптографии."
"по причине:"
"%2';uk='Ошибка при вызове ""%1"" менеджера криптографии."
"по причине:"
"%2'"),
...
Процедура МенеджерКриптографииПриОшибкеПолученияЗначенияСвойстваСписокОшибок(ИнформацияОбОшибке, СтандартнаяОбработка, ВходящийКонтекст) Экспорт
	...
	ОписаниеОшибки = НСтр("ru='Ошибка чтения свойства ""СписокОшибок"" менеджера криптографии.';uk='Ошибка чтения свойства ""СписокОшибок"" менеджера криптографии.'");
...
Функция ИмяСобытия()
	...
	Возврат НСтр("ru='Электронный документооборот с контролирующими органами. Менеджер криптографии';uk='Электронный документооборот с контролирующими органами. Менеджер криптографии'");
...
Функция ЗаполнитьПараметрыКриптографии(ДоступныеКриптопровайдеры, ОписаниеОшибки)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоLinuxКлиент() Тогда
		...
		Если НЕ ЗначениеЗаполнено(ПутьМодуляКриптографии) Тогда
			ОписаниеОшибки = НСтр("ru='Не указан путь модуля криптографии в настройках программы.';uk='Не указан путь модуля криптографии в настройках программы.'");
			Возврат Ложь;
		КонецЕсли;            
...
Функция ОписаниеОшибкиНеУказаныКлючевыеПараметрыСертификатаДляПоиска()
	...
	Возврат НСтр("ru='Не указаны ключевые параметры сертификата для поиска.';uk='Не указаны ключевые параметры сертификата для поиска.'");
