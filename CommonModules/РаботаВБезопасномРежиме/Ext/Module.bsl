...
Процедура ВыполнитьМетодОбъекта(Знач Объект, Знач ИмяМетода, Знач Параметры = Неопределено) Экспорт
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Некорректное значение параметра ИмяМетода (%1) в РаботаВБезопасномРежиме.ВыполнитьМетодОбъекта';uk='Некоректне значення параметра ИмяМетода (%1) в РаботаВБезопасномРежиме.ВыполнитьМетодОбъекта'"), ИмяМетода);
	КонецПопытки;
...
Процедура ПроверитьИмяМетодаКонфигурации(Знач ИмяМетода) Экспорт
	...
	Если ЧастиИмени.Количество() <> 2 И ЧастиИмени.Количество() <> 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неправильный формат параметра ИмяМетода (передано значение: ""%1"") в РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации';uk='Неправильний формат параметра ИмяМетода (передано значення: ""%1"") в РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации'"), ИмяМетода);
	КонецЕсли;
...
"Не найден общий модуль ""%2"".';uk='Неправильний формат параметра ИмяМетода (передано значення: ""%1"") в РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации:"
"Не знайдений загальний модуль ""%2"".'"),
	...
	Если ЧастиИмени.Количество() = 3 Тогда
		...
		Если Менеджер = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Неправильный формат параметра ИмяМетода (передано значение: ""%1"") в РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации:"
"Не найден менеджер объекта ""%2"".';uk='Неправильний формат параметра ИмяМетода (передано значення: ""%1"") в РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации:"
"Не знайдений менеджер об`єкту ""%2"".'"),
				ИмяМетода,
				ПолноеИмяОбъекта);
		КонецЕсли;
	КонецЕсли;
	...
	Исключение
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неправильный формат параметра ИмяМетода (передано значение: ""%1"") в РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации:"
"Имя метода ""%2"" не соответствует требованиям образования имен процедур и функций.';uk='Неправильний формат параметра ИмяМетода (передано значення: ""%1"") в РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации:"
"Ім`я методу ""%2"" не відповідає вимогам утворення імен процедур і функцій.'"),
			ИмяМетода,
			ИмяМетодаОбъекта);
	КонецПопытки;
...
""
"Для восстановления работоспособности программы требуется отключить использование профиля безопасности через консоль кластера и заново настроить профили безопасности с помощью интерфейса конфигурации (соответствующие команды находятся в разделе настроек программы).';uk='Неможливе виконання оброблювачів установки параметрів сеансу через: профіль безпеки %1 відсутній у кластері серверів 1С:Підприємства, або для нього заборонене використання в якості профілю безпеки безпечного режиму."
""
"Для відновлення працездатності програми потрібно відключити використання профілю безпеки через консоль кластера й заново налаштувати профілі безпеки за допомогою інтерфейсу конфігурації (відповідні команди перебувають у розділі налаштувань програми).'"),
...
""
"Для восстановления работоспособности программы требуется отключить использование профиля безопасности через консоль кластера и заново настроить профили безопасности с помощью интерфейса конфигурации (соответствующие команды находятся в разделе настроек программы).';uk='Неможливе виконання оброблювачів установки параметрів сеансу через: профіль безпеки %1 не містить дозволу на установку привілейованого режиму. Можливо, він був відредагований через консоль кластера."
""
"Для відновлення працездатності програми потрібно відключити використання профілю безпеки через консоль кластера й заново налаштувати профілі безпеки за допомогою інтерфейсу конфігурації (відповідні команди перебувають у розділі налаштувань програми).'"),
				ПрофильИнформационнойБазы);
...
"--------------------------------------------------------------------------------------------"
""
"Запуск программы будет невозможен.';uk='При установці параметрів сеансу відбулася помилка:"
""
"--------------------------------------------------------------------------------------------"
"%1"
"--------------------------------------------------------------------------------------------"
...
Функция МенеджерОбъектаПоИмени(Имя)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось получить менеджер для объекта ""%1""';uk='Не вдалося одержати менеджер для об`єкта ""%1""'"), Имя);
