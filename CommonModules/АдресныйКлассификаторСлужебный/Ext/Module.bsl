...
Функция ОписаниеПоследнейЗагрузки(ИдентификаторРегиона = Неопределено) Экспорт
	...
	Иначе
		...
		Результат.Представление        = НСтр("ru='Адресный классификатор еще не загружался.';uk='Адресный классификатор еще не загружался.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если ДнейРазницы = 0 Тогда
		Представление = НСтр("ru='Адресный классификатор был загружен сегодня.';uk='Адресный классификатор был загружен сегодня.'");
		
	ИначеЕсли ДнейРазницы = 1 Тогда
		Представление = НСтр("ru='Адресный классификатор был загружен вчера.';uk='Адресный классификатор был загружен вчера.'");
			
	ИначеЕсли ДнейРазницы = 2 Тогда
		Представление = НСтр("ru='Адресный классификатор был загружен позавчера.';uk='Адресный классификатор был загружен позавчера.'");
			
	ИначеЕсли ДнейРазницы > 2 Тогда
		Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Адресный классификатор был загружен %1 назад.';uk='Адресный классификатор был загружен %1 назад.'"),
...
Функция АдресаДляИнтерактивногоВыбора(Родитель, Уровень, ДополнительныеПараметры) Экспорт
	...
	Если Уровень = 1 Тогда
		Результат.Заголовок = НСтр("ru='Выберите регион';uk='Выберите регион'");
	ИначеЕсли Уровень = 2 Тогда
		Результат.Заголовок = НСтр("ru='Выберите округ';uk='Выберите округ'");
	ИначеЕсли Уровень = 3 Тогда
		Результат.Заголовок = НСтр("ru='Выберите район';uk='Выберите район'");
	ИначеЕсли Уровень = 4 Тогда
		Результат.Заголовок = НСтр("ru='Выберите город';uk='Выберите город'");
	ИначеЕсли Уровень = 5 Тогда
		Результат.Заголовок = НСтр("ru='Выберите внутригородской район';uk='Выберите внутригородской район'");
	ИначеЕсли Уровень = 6 Тогда
		Результат.Заголовок = НСтр("ru='Выберите населенный пункт';uk='Выберите населенный пункт'");
	ИначеЕсли Уровень = 7 Тогда
		Результат.Заголовок = НСтр("ru='Выберите улицу';uk='Выберите улицу'");
	ИначеЕсли Уровень = 90 Тогда
		Результат.Заголовок = НСтр("ru='Выберите дополнительный элемент';uk='Выберите дополнительный элемент'");
	ИначеЕсли Уровень = 91 Тогда
		Результат.Заголовок = НСтр("ru='Выберите подчиненный элемент';uk='Выберите подчиненный элемент'");
	ИначеЕсли Уровень = -1 Тогда
		Результат.Заголовок = НСтр("ru='Выберите ориентир';uk='Выберите ориентир'");
	КонецЕсли;
...
Функция ЕстьСписокДомов(ИдентификаторАдресногоОбъекта) Экспорт
	...
	ИначеЕсли Источник = "Сервис1С" Тогда
		...
		Порция = Сервис.Autocomplete(ИдентификаторАдресногоОбъекта, 0, НСтр("ru='ДОМ=';uk='ДОМ='"), 1, КодЯзыка, Метаданные.Имя);
...
Процедура ЗагрузитьКлассификаторАдресов(КодыСубъектовРФ, ОписаниеФайлов, ОповещатьОПрогрессе = Истина) Экспорт
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		Если ОповещатьОПрогрессе Тогда
			ТекстОсталосьЗагрузить = ?(ВсегоСубъектов - НомерПоПорядку > 0, " (" + НСтр("ru='осталось';uk='осталось'")
				+ " " + Формат(ВсегоСубъектов - НомерПоПорядку, "ЧН=") + ")", "");
			ИнформацияОЗагрузке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Загрузка региона ""%1 - %2""';uk='Загрузка региона ""%1 - %2""'"),
...
"%1 по причине: %2';uk='Не удалось удалить временный файл"
	...
	Иначе
		...
		ИначеЕсли Уровень = 91 Тогда
			...
			ТекстЗапроса = ТекстЗапроса + "
				...
				|";
		Иначе
			ВызватьИсключение НСтр("ru='Запрос некорректного уровня адресного объекта';uk='Запрос некорректного уровня адресного объекта'");
		КонецЕсли;
...
Процедура ОпределитьИдентификаторыАдресаСервис1С(Результат, АдресДляПроверки)
	...
	Иначе
		ВызватьИсключение НСтр("ru='Сведения о контактной информации были повреждены или некорректно заполнены';uk='Сведения о контактной информации были повреждены или некорректно заполнены'");
	КонецЕсли;
...
Функция АнализОдногоАдресаПоКлассификатору(Адрес, Уровни)
	...
	Если ТипЗнч(Адрес) <> Тип("ОбъектXDTO") Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Адрес", НСтр("ru='Невозможно проверить адрес из-за некорректных сведений. Проверьте правильность заполнения адреса.';uk='Невозможно проверить адрес из-за некорректных сведений. Проверьте правильность заполнения адреса.'"));
		Возврат Результат;
	КонецЕсли;
	...
	Если ПустаяСтрока(СубъектРФ.Наименование) Тогда
		// Нет субъекта
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", НСтр("ru='Не указан субъект РФ адреса';uk='Не указан субъект РФ адреса'") );
		Возврат Результат;
	ИначеЕсли Уровни.Найти(1) = Неопределено Тогда
		// В уровнях должно быть указано всегда.
		ВызватьИсключение НСтр("ru='В уровнях для проверки адреса не указан уровень субъекта РФ.';uk='В уровнях для проверки адреса не указан уровень субъекта РФ.'");
	КонецЕсли;
	...
	Если РегионЗагруженВАдресныеСведения = Неопределено Тогда 
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Регион ""%1"" не существует';uk='Регион ""%1"" не существует'"), Адрес.СубъектРФ));
		Результат.АдресПроверен = Истина;
		Возврат Результат;
	ИначеЕсли НЕ РегионЗагруженВАдресныеСведения Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", НСтр("ru='Отсутствуют адресные сведения по региону';uk='Отсутствуют адресные сведения по региону'") + ": " + Адрес.СубъектРФ + ".");
	...
	Если РезультатЗапроса.Пустой() Тогда
		// Нет региона
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Субъект РФ (регион) ""%1"" не найден в адресном классификаторе';uk='Субъект РФ (регион) ""%1"" не найден в адресном классификаторе'"), СубъектРФ.Наименование + " " + СубъектРФ.Сокращение));
		Возврат Результат;
	...
	Для Каждого Запись Из ЗаписиДомов Цикл;
		...
		Если ПроверятьОкруг И Не Запись.ОкругНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Округ", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Округ ""%1"" отсутствует в адресном классификаторе';uk='Округ ""%1"" отсутствует в адресном классификаторе'"), Округ.Наименование + " " + Округ.Сокращение));
		КонецЕсли;
		...
		Если ПроверятьРайон И Не Запись.РайонНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СвРайМО/Район", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Район ""%1"" отсутствует в адресном классификаторе';uk='Район ""%1"" отсутствует в адресном классификаторе'"), Район.Наименование + " " + Район.Сокращение));
		КонецЕсли;
		
		Если ПроверятьГород И Не Запись.ГородНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Город", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Город ""%1"" отсутствует в адресном классификаторе';uk='Город ""%1"" отсутствует в адресном классификаторе'"), Город.Наименование + " " + Город.Сокращение));
		КонецЕсли;
		...
		Если ПроверятьВнутригородскойРайон И Не Запись.ВнутригородскойРайонНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "ВнутригРайон", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Внутригородской район ""%1"" отсутствует в адресном классификаторе';uk='Внутригородской район ""%1"" отсутствует в адресном классификаторе'"),
		...
		Если ПроверятьНаселенныйПункт И Не Запись.НаселенныйПунктНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "НаселПункт", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Населенный пункт ""%1"" отсутствует в адресном классификаторе';uk='Населенный пункт ""%1"" отсутствует в адресном классификаторе'"),
		...
		Если ПроверятьДополнительный И Не Запись.ДополнительныйНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Дополнительный", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Дополнительная территория ""%1""отсутствует в адресном классификаторе';uk='Дополнительная территория ""%1""отсутствует в адресном классификаторе'"),
		...
		Если ПроверятьПодчиненный И Не Запись.ПодчиненныйНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Подчиненный", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Элемент дополнительной территории ""%1"" отсутствует в адресном классификаторе';uk='Элемент дополнительной территории ""%1"" отсутствует в адресном классификаторе'"),
		...
		Если ИсторияОбъекта.Количество() > 0 Тогда 
			Для каждого СтрокаОбъект Из ИсторияОбъекта Цикл
				ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Улица", 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Улица ""%1"" была переименована в ""%2""';uk='Улица ""%1"" была переименована в ""%2""'"), СтрокаОбъект.Значение, СтрокаОбъект.Представление));
			КонецЦикла;
		Иначе
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Улица", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Улица ""%1"" отсутствует в адресном классификаторе';uk='Улица ""%1"" отсутствует в адресном классификаторе'"), Улица.Наименование + " " + Улица.Сокращение));
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьЗаписиДомов И Не ЕстьВарианты Тогда
		...
		Если ЗначениеЗаполнено(ОписаниеЗданияПоСоответствиюФИАС) Тогда
			...
			Если НеточныеСовпадения.Количество() = 1 Тогда
				Подсказка = ", " + НСтр("ru='но есть';uk='но есть'") + """" + НеточныеСовпадения[0]+ """";
			ИначеЕсли НеточныеСовпадения.Количество() > 1 Тогда
				...
				Если НеточныеСовпадения.Количество() = 2 Тогда
					Подсказка = НСтр("ru='но есть ""%1"" или ""%2""';uk='но есть ""%1"" или ""%2""'");
					ВтороеЗначение =  НеточныеСовпадения[1];
				Иначе
					Подсказка = НСтр("ru='но есть ""%1"" и другие варианты(%2)';uk='но есть ""%1"" и другие варианты(%2)'");
			...
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Подчиненный", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='""%1"" отсутствует в адресном классификаторе';uk='""%1"" отсутствует в адресном классификаторе'"), ОписаниеЗданияПоСоответствиюФИАС(ЗданияИПомещения)) + Подсказка);
		КонецЕсли;
	...
	Если НЕ ИндексКорректный И ЗначениеЗаполнено(Индекс) И ОшибкиПроверки.Количество() = 0 Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Индекс", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Индекс ""%1"" в адресном классификаторе не соответствует введенному адресу';uk='Индекс ""%1"" в адресном классификаторе не соответствует введенному адресу'"), Индекс));
	КонецЕсли;
...
Функция ТипыОбъектовАдресацииАдресаРФ()
	...
	Результат.Добавить(СтрокаОбъектаАдресации("1060", НСтр("ru='Строение';uk='Будова'"),   2, 2, 1));
...
Функция ПараметрыАутентификацииНаСайте() Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru='Сервис интернет-поддержки пользователей не подключен.';uk='Сервис интернет-поддержки пользователей не подключен.'");
	КонецЕсли;
...
Функция СтруктураОписанияОшибкиПоставщика(Описание = Неопределено, ИнформацияОбОшибке = Неопределено)
	...
	Если ИнформацияОбОшибке = Неопределено Тогда
		Описание.ПодробноеПредставлениеОшибки = НСтр("ru='Информация об ошибке отсутствует';uk='Информация об ошибке отсутствует'");
		Возврат Описание;
	КонецЕсли;
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Описание = НСтр("ru='Загрузка обновлений адресного классификатора.';uk='Загрузка обновлений адресного классификатора.'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
			...
			Дело.Представление  = НСтр("ru='Адресный классификатор устарел';uk='Адресный классификатор устарел'");
			Дело.Количество     = 0;
			Дело.Подсказка      = НСтр("ru='Автоподбор и проверка корректности адресов временно недоступны.';uk='Автоподбор и проверка корректности адресов временно недоступны.'");
	...
	Подсказка = Подсказка + Символы.ПС + НСтр("ru='Необходимо проверить наличие обновлений.';uk='Необходимо проверить наличие обновлений.'");
	Для Каждого Раздел Из Разделы Цикл
		Дело = ТекущиеДела.Добавить();
...
Процедура ПеренестиДанныеУстаревшегоКлассификатор(КодыСубъектовРФ)
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		НомерПоПорядку = НомерПоПорядку + 1;
		ДлительныеОперации.СообщитьПрогресс(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Актуализация сведений о регионе ""%1 - %2"" (осталось %3) ...';uk='Актуализация сведений о регионе ""%1 - %2"" (осталось %3) ...'"),
...
Функция ОписаниеДиапазонаКЛАДР(Описание, ТаблицыДанных)
	...
	Если ВариантыТипов.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка поиска типа диапазона для %1';uk='Ошибка поиска типа диапазона для %1'"), Описание);
	КонецЕсли;
...
Функция ПредставлениеРегиона(Наименование, Сокращение)
	Если Сокращение = НСтр("ru='Респ';uk='Респ'") Тогда
		Возврат НСтр("ru='Республика';uk='Республика'") + " " + Наименование;
...
Функция ИдентификаторДома(СтрокаСоСпискомДомов, СведенияОДоме)
	...
	Пока Позиция <= ЧислоСтрок Цикл
		...
		Если Строка = "h" ИЛИ  Строка = "H" Тогда
			...
			ИначеЕсли НЕ ЗначениеЗаполнено(ОписаниеДома.Дом) ИЛИ ОписаниеДома.Дом = НСтр("ru='НЕТ';uk='НЕТ'") Тогда
...
Функция ИзвлечьСписокДомовСервис1С(ИдентификаторРодитель, СтрокаПоиска, ВариантыДомов)
	...
	Порция = Сервис.Autocomplete(ИдентификаторРодитель, 0, НСтр("ru='ДОМ=';uk='ДОМ='") + СтрокаПоиска, 2000, КодЯзыка, Метаданные.Имя);
	...
	Для Каждого Строка Из Список Цикл
		...
		Для каждого Дом Из МассивДомов Цикл
			...
			Если ТипАдресногоОбъекта = НСтр("ru='ДОМ';uk='БУДИНОК'")
				ИЛИ ТипАдресногоОбъекта = НСтр("ru='ВЛАДЕНИЕ';uk='ВЛАДЕНИЕ'")
				ИЛИ ТипАдресногоОбъекта = НСтр("ru='ДОМОВЛАДЕНИЕ';uk='ДОМОВЛАДЕНИЕ'") Тогда
...
Процедура ДобавитьИнформациюОДоме(ВариантыДомов, ОписаниеДома, НаименованиеЗданийИСтроений, СтрокаПоиска = "")
	...
	Если ЗначениеЗаполнено(ОписаниеДома.Корпус) Тогда
		ТекстПроСтроения = НСтр("ru='корпус';uk='корпус'") + " " + ОписаниеДома.Корпус;
...
Функция НаименованиеВладенийИСтроений()
	...
	Результат.Строения.Вставить(0, НСтр("ru='Строение';uk='Будова'"));
...
"• Подключение к Интернету выполняется через прокси-сервер, но его параметры не заданы в программе.';uk='Возможные причины:"
"• Некорректно введен или не введен логин и пароль;"
"• Нет подключения к Интернету;"
"• На веб-узле возникли неполадки;"
"• Брандмауэр или другое промежуточное ПО (антивирусы и т.п.) блокируют попытки программы подключиться к Интернету;"
...
Процедура ОчиститьКлассификаторАдресов(КодыСубъектовРФ)
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		НомерПоПорядку = НомерПоПорядку + 1;
		ДлительныеОперации.СообщитьПрогресс(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Очистка региона ""%1 - %2"" (осталось %3) ...';uk='Очистка региона ""%1 - %2"" (осталось %3) ...'"),
			СубъектРФ, АдресныйКлассификатор.НаименованиеРегионаПоКоду(СубъектРФ),
			Формат(ВсегоСубъектов - НомерПоПорядку, "ЧН=")));
...
Функция РазрешенияБезопасностиОбновления() Экспорт
	...
	Разрешения.Добавить( РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		ВРег(Адрес.Схема), Адрес.Хост, Адрес.Порт, НСтр("ru='Проверка обновления адресного классификатора.';uk='Проверка обновления адресного классификатора.'")));
	...
	Адрес = ОбщегоНазначенияКлиентСервер.СтруктураURI("http://downloads.1c.eu/tmplts/ITS/KLADR/");
	Разрешения.Добавить( РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		ВРег(Адрес.Схема), Адрес.Хост, Адрес.Порт, НСтр("ru='Загрузка данных адресного классификатора.';uk='Загрузка данных адресного классификатора.'")));
...
Функция ФайлЗагрузкиСубъектаРФ(КодСубъектаРФ, КаталогФайлов, КлючВида, ИмяОписывающегоУзла)
	...
	Если ТипXDTOЗаписи = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В файле ""%1"" не найден список записей в типе  %2';uk='В файле ""%1"" не найден список записей в типе  %2'"), Результат.ПолноеИмя, Результат.ТипXDTO);
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В файле ""%1"" не найден корневой узел %2';uk='В файле ""%1"" не найден корневой узел %2'"), Результат.ПолноеИмя, ИмяОписывающегоУзла);
	КонецЕсли;
