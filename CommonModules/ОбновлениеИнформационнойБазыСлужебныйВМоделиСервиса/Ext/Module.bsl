		...
		Если ИдентификаторБиблиотеки = Метаданные.Имя Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Если НЕ Выборка.Обновлена Тогда
					...
					ШаблонКомментария = НСтр("ru='Обновление версии конфигурации было выполнено до обновления версии библиотеки %1';uk='Оновлення версії конфігурації було виконано до оновлення "
"версії бібліотеки %1'");
	...
	Если КлючЗаписи <> Неопределено Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации() + "." 
				+ НСтр("ru='Обновление области данных';uk='Відновлення області даних'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение(НСтр("ru='Ошибка обновления области данных. Запись версий области данных заблокирована.';uk='Помилка відновлення області даних. Запис версій області даних заблокована.'"));
		КонецПопытки;
	КонецЕсли;
	Возврат КлючЗаписи;
...
"недоступен из сеансов с установленным значением разделителей модели сервиса!';uk='Виклик функції ОбновлениеИнформационнойБазыСлужебныйВМоделиСервиса.МинимальнаяВерсияОбластейДанных()"
