...
Процедура ИнициализироватьСеансовыеДанные(Данные, Пользователь, ОбластьДоступа, УчетнаяЗаписьЭлектроннойПочты = Неопределено) Экспорт
	...
	Если ОбластьДоступа = Перечисления.ОбластиДоступаGoogle.Почта
		...
		ВызватьИсключение НСтр("ru='Не указана учетная запись электронной почты.';uk='Не вказаний обліковий запис електронної пошти.'");
	КонецЕсли;
...
Функция КлючИзИдентификатора(Идентификатор, КонтекстПроверки) Экспорт
	...
	НСтр("ru='Не указан Идентификатор для преобразования его в Ключ';uk='Не зазначений Ідентифікатор для перетворення його в Ключ'"),
	КонтекстПроверки);
	
	ОбщегоНазначенияКлиентСервер.Проверить(
	ТипЗнч(Идентификатор) = Тип("Строка"),
	НСтр("ru='Идентификатор должен быть строкового типа';uk='Ідентифікатор повинен бути строкового типу'"),
	КонтекстПроверки);
...
Процедура ИнициализироватьУзелПланаОбменаДляКалендаряGoogle() Экспорт
	...
	УзелОбъект.Наименование = НСтр("ru='Этот узел';uk='Цей вузол'");
	УзелОбъект.Записать();
...
Процедура ПодключитьОбработчикиСобытияАвтоподбор(Форма) Экспорт
	
	ЗаполнитьРеквизитПоддержкаGoogle(Форма);
...
Функция ИдентификацияПриложенияGoogleКорректна(ИдентификацияПриложенияGoogle) Экспорт
	...
	Исключение
		Результат = НСтр("ru='Требуется указать данные в формате JSON';uk='Потрібно вказати дані у форматі JSON'");
		Возврат Результат;
	КонецПопытки;
...
Функция ЗагрузитьЗаголовкиСообщенийGmail(Знач УчетнаяЗапись, Знач СеансовыеДанные, Знач РежимЗагрузки, Знач Пользователь, Знач ОтборПоКонтакту = Неопределено) Экспорт
	...
	Для Каждого ТекМетаданные Из МетаданныеСообщений Цикл
		...
		СтрШаблон(НСтр("ru='%1: загружено %2 из %3';uk='%1: завантажено %2 з %3'"),
		ЗаголовокПрогресса,
...
Функция СоздатьУчетнуюЗаписьGoogle(СеансовыеДанные) Экспорт
	...
	Если Не ЗначениеЗаполнено(АдресЭлектроннойПочты) Тогда
		ВызватьИсключение НСтр("ru='Не удалось определить адрес электронной почты Google.';uk='Не вдалося визначити адресу електронної пошти Google.'");
	КонецЕсли;
	...
	Для Каждого ТекОписаниеКалендаря Из ОписанияВсехКалендарей["items"] Цикл
		...
		Пока ЗначениеЗаполнено(СтраничныйТокен) Или СтраничныйТокен= "" Цикл
			...
			Если СобытияКалендаря["error"] <> Неопределено Тогда
				ВызватьИсключение 
				СтрШаблон(
				НСтр("ru='Произошла ошибка синхронизации с Google Calendar: %1';uk='Відбулася помилка синхронізації з Google Calendar: %1'"),
...
Функция ВосстановитьДатуJSON(Свойство, Значение, Параметры) Экспорт
	...
	Исключение
		...
		НСтр("ru='Синхронизация календаря Google.';uk='Синхронізація календаря Google.'"),
		УровеньЖурналаРегистрации.Предупреждение,,,
		СтрШаблон(
		НСтр("ru='Не удалось прочитать дату JSON для свойства ""%1"": ""%2"".';uk='Не вдалося прочитати дату JSON для властивості ""%1"": ""%2"".'"), 
		Свойство,
		Значение));
...
Процедура ЗаписатьКонтактПоДаннымGoogle(ТекВхождение, Пользователь)
	...
	КонтактОбъект.Title = ТекВхождение["title"]["$t"];
...
Процедура ЗаполнитьРеквизитПоддержкаGoogle(Форма)
	
	Форма.ПоддержкаGoogle = ОписаниеПоддержкиGoogle(Форма);
	
	Форма.ПоддержкаGoogle.Вставить("ОписаниеПолейДляАвтоПодбора", ОписаниеПолейДляАвтоПодбора(Форма));
...
Функция ОписаниеПолейДляАвтоПодбора(Форма)
	...
	Для Каждого Элемент Из Форма.Элементы Цикл
		...
		Если ЗначениеЗаполнено(Элемент.ОграничениеТипа) Тогда
			ОписаниеТиповРеквизита = Элемент.ОграничениеТипа;
		Иначе
		...
		Результат[Элемент.Имя] = Новый Структура;
		Результат[Элемент.Имя].Вставить("ПутьКДанным", Элемент.ПутьКДанным);
		Результат[Элемент.Имя].Вставить("ТипЗначения", ОписаниеТиповРеквизита.Типы()[0]);
		Результат[Элемент.Имя].Вставить("ИмяФормыОбъекта", ИмяФормыОбъекта(ОписаниеТиповРеквизита.Типы()[0]));
...
Функция СсылкаНаКалендарьСотрудника(ОписаниеКалендаря, Пользователь)
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ДанныеЗаполнения.ВладелецКалендаря) Тогда
			...
			НСтр("ru='Не удалось определить владельца календаря ""%1"" для пользователя ""%2"".';uk='Не вдалося визначити власника календаря ""%1"" для користувача ""%2"".'"),
			КалендарьСотрудника,
			Пользователь);
		КонецЕсли;
...
Функция КомпонентыПакетногоОтвета(ПакетныйОтветHTTP, Менеджер)
	...
	НСтр("ru='Не обнаружен параметр boundary в ответе на пакетный запрос';uk='Не виявлений параметр boundary у відповіді на пакетний запит'"),
...
Функция ЗаписатьСобытиеПоМетаданнымGMail(МетаданныеСообщения, УчетнаяЗапись, СопоставленныеКонтакты, Пользователь)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(СобытияПоИдентификатору) Тогда
			...
			НСтр("ru='Возможно, запущено несколько заданий загрузки почты: сообщение с идентификатором %1 уже записано.';uk='Можливо, запущено кілька завдань завантаження пошти: повідомлення з ідентифікатором %1 вже записано.'"),
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Загрузка заголовков сообщений электронной почты';uk='Завантаження заголовків повідомлень електронної пошти'"),
