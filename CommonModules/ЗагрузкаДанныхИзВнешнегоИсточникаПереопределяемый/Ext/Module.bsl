...
Процедура ДобавитьУсловноеОформлениеТаблицыСопоставлений(ЭтотОбъект, ПутьРеквизита, НастройкиЗагрузкиДанных) Экспорт
	...
	ИначеЕсли НастройкиЗагрузкиДанных.ЭтоЗагрузкаСправочника Тогда
		...
		ТекстНовыйЭлемент	= Нстр("ru='<Будет создан новый элемент>';uk='<Буде створений новий елемент>'");
		ТекстПропущены		= Нстр("ru='<Данные будут пропущены>';uk='<Дані будуть пропущені>'");
	...
	ИначеЕсли НастройкиЗагрузкиДанных.ЭтоЗагрузкаРегистраСведений Тогда
		...
		ТекстУсловногоОформления = Нстр("ru='<Строка будет пропущена...>';uk='<Рядок буде пропущений...>'");
...
Функция НайтиКонтрагентаПоИНН_КодПоЕДРПОУ(ИНН, КодПоЕДРПОУ) Экспорт
	Перем Контрагент;
	...
	Если НЕ ПустаяСтрока(КодПоЕДРПОУ) Тогда
		...
		Запрос.Текст = Запрос.Текст + " И КодПоЕДРПОУ = &КодПоЕДРПОУ";
...
Процедура СопоставитьПоставщика(Поставщик, Поставщик_ВходящиеДанные) Экспорт
	...
	Для каждого ЗначениеРазделителя Из Разделители Цикл
		...
		КодПоЕДРПОУ = Сред(Поставщик_ВходящиеДанные, ПозицияРазделителя + 1);
		
		Запрос = Новый Запрос("ВЫБРАТЬ Справочник.Контрагенты.Ссылка ГДЕ НЕ ЭтоГруппа И ИНН = &ИНН И КодПоЕДРПОУ = &КодПоЕДРПОУ");
		Запрос.УстановитьПараметр("ИНН", ИНН);
		Запрос.УстановитьПараметр("КодПоЕДРПОУ", КодПоЕДРПОУ);
...
Процедура ДобавитьЗаписьСопоставленияПоИНН_КодПоЕДРПОУ(ОрганизацияЕГАИС, Контрагент)
	
	// Пробуем сами сопоставить
	Контрагент = НайтиКонтрагентаПоИНН_КодПоЕДРПОУ(ОрганизацияЕГАИС.ИНН, ОрганизацияЕГАИС.КодПоЕДРПОУ);
...
Функция ПолучитьКонтрагентовПоСоответствиюЕГАИС(Производитель, Импортер) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(Производитель) Тогда
		...
		ДобавитьЗаписьСопоставленияПоИНН_КодПоЕДРПОУ(Производитель, СопоставленныеДанные.Производитель);
	...
	ИначеЕсли ЗначениеЗаполнено(Импортер) Тогда
		...
		ДобавитьЗаписьСопоставленияПоИНН_КодПоЕДРПОУ(Импортер, СопоставленныеДанные.Импортер);
...
Процедура СопоставитьКонтрагента(Контрагент, ИНН_КодПоЕДРПОУ, КонтрагентНаименование, РасчетныйСчет) Экспорт
	
	//:::Поиск ИНН и КодПоЕДРПОУ
	Если НЕ ПустаяСтрока(ИНН_КодПоЕДРПОУ) Тогда
		...
		Для каждого ЗначениеРазделителя Из Разделители Цикл
			...
			ПозицияРазделителя = СтрНайти(ИНН_КодПоЕДРПОУ, ЗначениеРазделителя);
			Если ПозицияРазделителя = 0 Тогда 
			...
			ИНН = Лев(ИНН_КодПоЕДРПОУ, ПозицияРазделителя - 1);
			КодПоЕДРПОУ = Сред(ИНН_КодПоЕДРПОУ, ПозицияРазделителя + 1);
			
		КонецЦикла;
	
		// Если не было разделителя, то предполагаю, что это все ИНН
		Если ИНН = "" И КодПоЕДРПОУ = "" Тогда
			
			ИНН = ИНН_КодПоЕДРПОУ;
			
		КонецЕсли;
		
		Контрагент = НайтиКонтрагентаПоИНН_КодПоЕДРПОУ(ИНН, КодПоЕДРПОУ);
		Если ЗначениеЗаполнено(Контрагент) Тогда
