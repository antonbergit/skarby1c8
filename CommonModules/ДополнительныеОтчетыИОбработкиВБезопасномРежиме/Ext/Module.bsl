...
"%2';uk='Не вдалося видалити тимчасовий файл ""%1"":"
"%2'"),
			ВременныйФайл,
...
Процедура ПроверитьКорректностьИмениМетодаВнешнегоОбъекта(Знач ИмяМетода)
	...
	Для Каждого ЗапрещенныйСимвол Из ЗапрещенныеСимволы Цикл
		...
		Если СтрНайти(ИмяМетода, ЗапрещенныйСимвол) > 0 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 не является корректным именем метода для COM-объекта или объекта внешней компоненты!';uk='%1 не є коректним іменем методу для COM-об`єкта або об`єкта зовнішньої компоненти!'"), ИмяМетода);
