...
Процедура ПродолжитьВыполнениеКоманды(Контекст)
	...
	Если Контекст.ТребуетсяРаботаСФайлами Тогда
		...
		ТекстСообщения = НСтр("ru='Для продолжения необходимо установить расширение для веб-клиента 1С:Предприятие.';uk='Для продовження необхідно встановити розширення для веб-клієнта 1С:Підприємство.'");
	...
	Если Контекст.ТребуетсяПроведение Тогда
		...
		Если ИнформацияОДокументах.КоличествоНеПроведенных > 0 Тогда
			Если ИнформацияОДокументах.ЕстьПравоПроведенияХотяБыОдногоНепроведенного Тогда
				...
				Иначе
					ТекстВопроса = НСтр("ru='Для выполнения команды необходимо предварительно провести документы. Выполнить проведение документов и продолжить?';uk='Для виконання команди необхідно попередньо провести документи. Виконати проведення документів і продовжити?'");
				КонецЕсли;
