...
Функция ТикетАутентификацииНаПорталеПоддержки(ВладелецТикета) Экспорт
	...
	Если Не ЗначениеЗаполнено(ВладелецТикета) Тогда
		ВызватьИсключение НСтр("ru='Не заполнено значение параметра ""ВладелецТикета""';uk='Не заповнене значення параметра ""ВладелецТикета""'");
	КонецЕсли;
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Если НЕ ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		//	"login.1c.ru",
		//	443,
		//	НСтр("ru='Сервисы аутентификации (зона ru)';uk='Сервіси аутентифікації (зона ru)'"));
		//НовыеРазрешения.Добавить(Разрешение);
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			"login.1c.eu",
			443,
			НСтр("ru='Сервисы аутентификации (зона eu)';uk='Сервіси аутентифікації (зона eu)'"));
		НовыеРазрешения.Добавить(Разрешение);
		...
		//	443,
		//	НСтр("ru='Сервисы службы технической поддержки (зона ru)';uk='Сервіси служби технічної підтримки (зона ru)'"));
		//НовыеРазрешения.Добавить(Разрешение);
...
"%Комментарий%';uk='Ідентифікатор кроку: %ИдентификаторШага%"
"%Комментарий%'");
...
"Возможно, отсутствуют события за указанный период (%1...%2).';uk='Файл з вивантаженням подій журналу реєстрації не існує."
"Можливо, відсутні події за вказаний період (%1...%2).'"),
						Формат(ПараметрыОтбораПравильный.ДатаНачала, "ДЛФ=DT"),
						Формат(ПараметрыОтбораПравильный.ДатаОкончания, "ДЛФ=DT")));
			КонецЕсли;
...
"%1';uk='Не вдалося експортувати події журналу реєстрації в файл через:"
"%1'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция СобытиеЖурналаРегистрацииИнтернетПоддержкаПользователейОшибка()
	...
	Возврат НСтр("ru='Интернет-поддержка пользователей.Ошибка';uk='Інтернет-підтримка користувачів.Помилка'",
...
Функция СобытиеЖурналаРегистрацииИнтернетПоддержкаПользователейИнформация()
	...
	Возврат НСтр("ru='Интернет-поддержка пользователей.Информация';uk='Інтернет-підтримка користувачів.Інформація'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий         = НСтр("ru='Запускается при первом запуске, в общих данных. Установка общих данных.';uk='Запускається при першому запуску, в загальних даних. Установка загальних даних.'");
	...
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.Комментарий         = НСтр("ru='Запускается при первом запуске, для каждой области данных. Установка разделенных данных.';uk='Запускається при першому запуску, для кожної області даних. Установка розділених даних.'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.Комментарий         = НСтр("ru='Запускается при переходе на новую версию, в общих данных. Установка общих данных.';uk='Запускається при переході на нову версію, в загальних даних. Установка загальних даних.'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ru='Запускается при переходе на любую новую версию, для каждой области данных. Установка разделенных данных.';uk='Запускається при переході на будь-яку нову версію, для кожної області даних. Установка розділених даних.'");
	Обработчик.РежимВыполнения     = "Оперативно";
...
"'"),
	...
	ЗаписатьСообщениеВЖурналРегистрации(
		...
		НСтр("ru='Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПервыйЗапуск';uk='Сервіс та регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПервыйЗапуск'"), // ИдентификаторШага
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		СобытиеЖурналаРегистрацииИнтернетПоддержкаПользователейИнформация(), // ИмяСобытия
		НСтр("ru='Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПервыйЗапуск';uk='Сервіс та регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПервыйЗапуск'"), // ИдентификаторШага
		?(КодРезультатаОбщий = 0, УровеньЖурналаРегистрации.Информация, УровеньЖурналаРегистрации.Ошибка), // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполненияОбщий.ВремяОкончания - ШагВыполненияОбщий.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		СобытиеЖурналаРегистрацииИнтернетПоддержкаПользователейИнформация(), // ИмяСобытия
		НСтр("ru='Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПерейтиНаВерсию';uk='Сервіс та регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПерейтиНаВерсию'"), // ИдентификаторШага
		?(КодРезультатаОбщий = 0, УровеньЖурналаРегистрации.Информация, УровеньЖурналаРегистрации.Ошибка), // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполненияОбщий.ВремяОкончания - ШагВыполненияОбщий.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
"Необходима версия библиотеки стандартных подсистем не ниже версии 2.3.3.69';uk='Помилка впровадження бібліотеки Інтернет-підтримки користувачів."
"Необхідна версія бібліотеки стандартних підсистем не нижче версії 2.3.3.69'");
	КонецЕсли;
