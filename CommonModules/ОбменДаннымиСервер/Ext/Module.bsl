...
"й виконайте команду ""Оновити конфігурацію бази даних (F7)""."
""
...
Процедура ЗаписатьДанныеДляОбновленияВФайл(Параметры, Данные, ВидДанных, ПолноеИмяОбъекта = "") Экспорт
	...
	Если Параметры.ЗаписьИзмененийДляПодчиненногоУзлаРИБСФильтрами = Неопределено Тогда
		ТекстИсключения = НСтр("ru='В обработчике неправильно организована работа с параметрами регистрации данных к обработке.';uk='У обробнику неправильно організована робота з параметрами реєстрації даних до обробки.'");
...
Процедура РассчитатьПроцентВыгрузки(КоличествоВыгруженных, КоличествоОбъектовКВыгрузке) Экспорт
	...
	Если КоличествоОбъектовКВыгрузке = 0 Тогда
		...
		Шаблон = НСтр("ru='Обработано: %1 объектов.';uk='Оброблене: %1 об`єктів.'");
	...
	Иначе
		...
		Шаблон = НСтр("ru='Обработано: %1 из %2 объектов.';uk='Оброблене: %1 з %2 об`єктів.'");
...
Функция НастройкиПланаОбменаПоУмолчанию(ИмяПланаОбмена)
	...
	ЗаголовокФормыПомощника = НСтр("ru='Синхронизация данных с %Программа% (настройка)';uk='Синхронізація даних з %Программа% (налаштування)'");
...
Процедура ВыполнитьДействиеОбменаДляУзлаИнформационнойБазыПоВнешнемуСоединению(Отказ, УзелИнформационнойБазы,
	...
	Если ЭтоПланОбменаXDTO Тогда
		...
		Если НЕ ЗначениеЗаполнено(ПланОбменаСсылка.Code) Тогда
			...
			ТекстСообщения = НСтр("ru='Необходим переход на синхронизацию данных через универсальный формат в базе-корреспонденте.';uk='Необхідний перехід на синхронізацію даних через універсальний формат у базі-кореспондентові.'");
			...
			Если СтруктураПараметров.Ошибка Тогда
				ЕстьОшибки = Истина;
				НСтрока = НСтр("ru='Ошибка при переходе на синхронизацию данных через универсальный формат: %1. Обмен отменен.';uk='Помилка при переході на синхронізацію даних через універсальний формат: %1. Обмін скасований.'",
			...
			ИначеЕсли РезультатПерехода = Неопределено Тогда
				ЕстьОшибки = Истина;
				СтрокаСообщенияОбОшибке = НСтр("ru='Переход на синхронизацию данных через универсальный формат не выполнен';uk='Перехід на синхронізацію даних через універсальний формат не виконаний'");
			КонецЕсли;
			Если ЕстьОшибки Тогда
				...
				Возврат;
			Иначе
				Сообщение = НСтр("ru='Переход на синхронизацию данных через универсальный формат завершен успешно.';uk='Перехід на синхронізацію даних через універсальний формат завершений успішно.'");
...
Процедура ВыполнитьСтандартнуюЗагрузкуИзмененийДляУзла(
	...
	Попытка
		...
		Если РезультатВыполненияОбмена = Перечисления.РезультатыВыполненияОбмена.Ошибка Тогда
			ВызватьИсключение(НСтр("ru='В процессе получения данных возникли ошибки.';uk='У процесі отримання даних виникли помилки.'"));
		КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииУдалениеВременногоФайла() Экспорт
	...
	Возврат НСтр("ru='Обмен данными.Удаление временного файла';uk='Обмін даними.Видалення тимчасового файлу'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ВыполнитьПроверкуСтруктурыОбменаНаВалидность(СтруктураНастроекОбмена, ИспользоватьНастройкиТранспорта = Истина)
	...
	ИначеЕсли СтруктураНастроекОбмена.УзелИнформационнойБазы.ПометкаУдаления Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Узел информационной базы помечен на удаление. Обмен отменен.';uk='Вузол інформаційної бази позначений на видалення. Обмін скасовано.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
...
Функция ПолучитьШаблонИмениФайлаСообщения(ТекущийУзелПланаОбмена, УзелИнформационнойБазы, ЭтоИсходящееСообщение)
	...
	Возврат ИмяФайлаСообщенияОбмена(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелОтправитель, "Код"),
									ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелПолучатель, "Код"));
	//
КонецФункции
...
Процедура ЗарегистрироватьОшибкуПроведенияДокумента(Объект, УзелОбмена, ТекстИсключения) Экспорт
	...
	Если Не ПустаяСтрока(СокрЛП(ТекстСообщения)) Тогда
		...
		ПричинаОшибки = " " + НСтр("ru='По причине %1.';uk='Унаслідок %1.'");
...
Процедура ПроверитьОбязательныеРеквизитыФормы(Форма, Знач Реквизиты)
	...
	Если ОтсутствующиеРеквизиты.Количество() > 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Отсутствуют обязательные реквизиты формы настройки узла: %1';uk='Відсутні обов`язкові реквізити форми налаштування вузла: %1'"),
...
"Для предупреждения возможного возникновения ошибок при работе с программой рекомендуется актуализировать правила обмена из файла.';uk='Для планів обміну %1 використовуються правила обміну, завантажені з файлу."
...
Функция УстановитьВнешнееСоединениеСБазой(СтруктураНастроек) Экспорт
	...
	Если НетПолныхПрав Тогда
		Результат.ПодробноеОписаниеОшибки = НСтр("ru='Пользователю, указанному для подключения к другой программе, должны быть назначены роли ""Администратор системы"" и ""Полные права""';uk='Користувачу, вказаному для підключення до іншої програмі, "
"повинні бути призначені ролі ""Адміністратор системи"" і ""Повні "
"права""'");
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Иначе
			...
			Дело.ПараметрыФормы = Новый Структура("Заголовок,ИмяМакета",
				НСтр("ru='Установка обновления';uk='Установка відновлення'"), "ИнструкцияКакВыполнитьУстановкуОбновленияВручную");
		КонецЕсли;
		Дело.Владелец       = Раздел;
...
Функция СловарьКонтейнераВнешнегоМодуля() Экспорт
	...
	Результат.Вставить("Именительный", НСтр("ru='Настройка синхронизация данных';uk='Налаштування синхронізація даних'"));
	Результат.Вставить("Родительный", НСтр("ru='Настройки синхронизации данных';uk='Налаштування синхронізації даних'"));
...
Функция ИнтерактивноеИзменениеВыгрузки(Знач УзелИнформационнойБазы, Знач АдресХранилищаФормы, Знач ЕстьСценарийУзла=Неопределено) Экспорт
	...
	ПараметрыСценарияДополнения.ВариантБезДополнения.Вставить("Пояснение", 
		НСтр("ru='Будут отправлены только данные согласно общим настройкам.';uk='Будуть відправлені тільки дані відповідно до загальних налаштувань'")); 
	...
	ПараметрыСценарияДополнения.ВариантДополнительно.Вставить("Пояснение",
		НСтр("ru='Будут отправлены дополнительные данные по настройкам.';uk='Будуть відправлені додаткові дані по налаштуванням.'")); 
