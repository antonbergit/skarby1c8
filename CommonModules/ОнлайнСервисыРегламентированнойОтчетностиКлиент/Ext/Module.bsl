...
Процедура ПродолжитьПослеПроверкиПодключенияКИППЗавершение(Результат, ВходящийКонтекст) Экспорт
	...
	ИначеЕсли Результат.Выполнено И НЕ Результат.ВыполнениеРазрешено Тогда
		ПоказатьПредупреждение(, НСтр("ru='Нет доступа к интернет-поддержке пользователя - внешние формы не будут загружаться автоматически.';uk='Нет доступа к интернет-поддержке пользователя - внешние формы не будут загружаться автоматически.'"));
	ИначеЕсли НЕ Результат.Выполнено Тогда
		ПоказатьПредупреждение(, НСтр("ru='При проверке доступа к интернет-поддержке пользователя возникла неизвестная ошибка.';uk='При перевірці доступу до інтернет-підтримці користувача виникла невідома помилка.'"));
	КонецЕсли;
