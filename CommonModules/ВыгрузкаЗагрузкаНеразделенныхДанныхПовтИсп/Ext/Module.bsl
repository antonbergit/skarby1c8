...
"поле объекта не обнаружено!';uk='Поле %1 не може використовуватися в якості поля природнього ключа об`єкта %2:"
"поле об`єкта не виявлене!'", Метаданные.ОсновнойЯзык.КодЯзыка),
...
"значений типа ХранилищеЗначения в качестве полей естественного ключа не поддерживается!';uk='Поле %1 не може використовуватися в якості поля природнього ключа об`єкта %2: використання"
"значень типу ХранилищеЗначения в якості полів природнього ключа не підтримується!'", Метаданные.ОсновнойЯзык.КодЯзыка),
							ПолеЕстественногоКлюча,
							ТипОбщегоКлассификатора.ПолноеИмя());
...
"ВыгрузкаЗагрузкаДанныхПереопределяемый.ПриЗаполненииТиповОбщихДанныхПоддерживающихСопоставлениеСсылокПриЗагрузке()';uk='Поле %1 не може використовуватися в якості поля природнього ключа об`єкта %2:"
"у якості типу поля може використовуватися об`єкт %3, який не включений у набір"
...
"но при выгрузке была диагностирована попытка выгрузки ссылки на неразделенный объект).';uk='(в якості типу значення для об`єкта встановлено составний тип даних,"
...
"для яких можливе виконання зіставлення посилань при "
"вивантаженні та завантаженні."
...
"ВыгрузкаЗагрузкаДанныхПереопределяемый.ПриЗаполненииТиповОбщихДанныхНеТребующихСопоставлениеСсылокПриЗагрузке().';uk='Якщо коректне зіставлення посилань на нерозділені дані в ІБ, з якої вивантажені"
"дані і ІБ, в яку вони завантажуються, гарантується за допомогою інших механізмів, необхідно"
"вказати об`єкт метаданих %1 в процедурі"
