	...
	Если ФайлВременный.Существует() Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Файлы.Удаление файлов в томе при обмене';uk='Файли.Видалення файлів у томі при обміні'",
...
Процедура КриптографияПриСозданииФормыНаСервере(Форма, ЭтоФормаСписка = Истина, ТолькоКартинкаСтрок = Ложь) Экспорт
	...
	Возврат; // в локализации не добавляем до выяснения
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "2.3.3.50";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("93d3fe01-675a-4913-8325-1f97c2556f8d");
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Процедура = "ФайловыеФункцииСлужебный.ОбновитьПутьТомаLinux";
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "ФайловыеФункцииСлужебный.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ОчередьОтложеннойОбработки = 1;
	Обработчик.ЧитаемыеОбъекты = "Справочник.ТомаХраненияФайлов";
	Обработчик.ИзменяемыеОбъекты = "Справочник.ТомаХраненияФайлов";
	Обработчик.Комментарий = НСтр("ru='Удаление лишних символов из пути Linux для томов хранения файлов.';uk='Видалення зайвих символів з шляху Linux для томів зберігання файлів.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "2.3.3.66";
...
"у форматі UNC (\\servername\resource)'"),
	...
	Если ПустаяСтрока(ПолноеИмяФайловойБазы) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Укажите полное имя файловой базы (файл 1cv8.1cd)';uk='Вкажіть повне ім`я файлової бази (файл 1cv8.1cd)'"),,
			"ПолноеИмяФайловойБазыWindows");
		Результат.ДанныеПодготовлены = Ложь;
	ИначеЕсли Результат.ДанныеПодготовлены Тогда
		...
		Если ЕстьФайлыВТомах Тогда
			Если ПустаяСтрока(ПутьКАрхивуСФайламиТомов) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Укажите полное имя архива с файлами томов (файл *.zip)';uk='Вкажіть повне ім`я архіву з файлами томів (файл *.zip)'"),, 
					"ПутьКАрхивуСФайламиТомовWindows");
				Результат.ДанныеПодготовлены = Ложь;
			Иначе
...
"Заменить существующие файлы?';uk='Файли ""%1"" і ""%2"" вже існують."
...
"Заменить существующий файл?';uk='Файл ""%1"" вже існує."
...
Процедура ИзвлечьТекстИзФайлов() Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Файлы.Извлечение текста';uk='Файли.Витяг тексту'",
		     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
		,
		,
		НСтр("ru='Начато регламентное извлечения текста';uk='Почато регламентне витягу тексту'"));
...
"""%1"""
"произошла ошибка:"
"""%2"".';uk='Під час регламентного витягу тексту з файлу"
"""%1"""
"відбулася помилка:"
"""%2"".'"),
					ИмяСРасширениемФайла,
	...
	ЗаписьЖурналаРегистрации(
		...
		,
		,
		НСтр("ru='Закончено регламентное извлечение текста';uk='Закінчено регламентний витяг тексту'"));
...
Процедура ОбновитьПутьТомаLinux(Параметры) Экспорт
	...
	Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Выборка = ОбновлениеИнформационнойБазы.ВыбратьСсылкиДляОбработки(Параметры.Очередь, "Справочник.ТомаХраненияФайлов");
...
"%2';uk='Не вдалося обробити том зберігання файлів : %1 з причини:"
"%2'"), 
		...
		Если Не ОбновлениеИнформационнойБазы.ОбработкаДанныхЗавершена(Параметры.Очередь, "Справочник.ТомаХраненияФайлов") Тогда
			ОбработкаЗавершена = Ложь;
		КонецЕсли;
		
		Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Процедуре ОбновитьПутьТомаLinux не удалось обработать некоторые тома хранения файлов (пропущены): %1';uk='Процедурі ОбновитьПутьТомаLinux не вдалося обробити деякі томи зберігання файлів (пропущені): %1'"), 
			ПроблемныхОбъектов);
			ВызватьИсключение ТекстСообщения;
		Иначе
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			Метаданные.НайтиПоПолномуИмени("Справочник.ТомаХраненияФайлов"),,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Процедура ОбновитьПутьТомаLinux обработала очередную порцию томов хранения файлов: %1';uk='Процедура ОбновитьПутьТомаLinux обробила чергову порцію томів зберігання файлів: %1'"),
			ОбъектовОбработано));
		КонецЕсли;
...
Процедура ПроверитьИсключениеHTTP1С(Ответ, АдресСервера)
	Если (Ответ.КодСостояния >= 400) И (Ответ.КодСостояния <= 599) Тогда
		ШаблонОшибки = НСтр("ru='Ошибка HTTP 1С : %1 в %2';uk='Помилка HTTP 1С: %1 в %2'");
	...
	Попытка
		...
		Пока НайденныйResponse <> Неопределено Цикл
			...
			Если НайденныйHref = Неопределено Тогда
				ТекстОшибки = НСтр("ru='Ошибка ответа от сервера: не найден HREF в %1';uk='Помилка відповіді від сервера: не знайдений HREF в %1'");
			...
			Если (ТекущаяПапкаФайлов = Неопределено) И (ТаблицаФайлов.Найти(СтрокаДереваФайлов.Href, "Href") = Неопределено) Тогда
				...
				Если ПодсистемаРаботаСФайламиСуществует Тогда
					...
					Если НЕ ПроверитьВозможностьUID1C(СтрокаДереваФайлов.Href,Строка(Новый УникальныйИдентификатор), ПараметрыСинхронизации) Тогда
						ТекстСобытия = НСтр("ru='Невозможно сохранение дополнительных свойств файла, он не будет загружен: %1';uk='Неможливе збереження додаткових властивостей файлу, він не буде завантажений: %1'");
		...
		Для Каждого СтрокаТаблицы Из ТаблицаФайлов Цикл
			...
			Попытка
				...
				ИначеЕсли УдаленНаСервере И НЕ УдаленВБазе Тогда
					...
					ТекстСобытия = НСтр("ru='Удалено в базе: %1';uk='Видалено у базі: %1'");
					ЗаписатьВЖурналСобытийСинхронизацииФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСобытия, СтрокаТаблицы.Наименование), ПараметрыСинхронизации);
				...
				ИначеЕсли (ИзмененВБазе ИЛИ ИзмененНаСервере) И НЕ (УдаленВБазе ИЛИ УдаленНаСервере) Тогда
					...
					Если ИзмененНаСервере И НЕ ИзмененВБазе Тогда
						...
						СтрокаТаблицы.Обработан = Истина;
						ТекстСобытия = НСтр("ru='Обновлен объект в базе: %1';uk='Оновлений об`єкт у базі: %1'");
						ЗаписатьВЖурналСобытийСинхронизацииФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСобытия, СтрокаТаблицы.Наименование), ПараметрыСинхронизации);
...
Процедура ОсвободитьЗахваченныеФайлыУчетнойЗаписи(УчетнаяЗапись = Неопределено)
	...
	ТекстСобытия = НСтр("ru='Начало освобождения файлов, захваченных облачным сервисом.';uk='Початок звільнення файлів, захоплених хмарним сервісом.'");
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации);
	...
	ТекстСобытия = НСтр("ru='Завершено освобождения файлов, захваченных облачным сервисом';uk='Завершено звільнення файлів, захоплених хмарним сервісом'");
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации);
