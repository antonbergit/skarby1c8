...
Функция ПолучитьФайлИзИнтернет(Знач URL, Знач НастройкаСохранения, Знач НастройкаСоединения,
	...
	Если ИспользуетсяFTPПротокол Тогда
		...
		Исключение
			...
			СообщениеОбОшибке = НСтр("ru='Не удалось установить FTP-соединение с сервером %1:';uk='Не вдалося встановити FTP-з`єднання із сервером %1:'") + Символы.ПС + "%2";
...
"%2';uk='Не вдалося встановити HTTP-з`єднання із сервером %1:"
"%2'");
		...
		Иначе
			...
			Если HTTPОтвет.КодСостояния = 301 Или HTTPОтвет.КодСостояния = 302 Тогда
				...
				Если КоличествоПеренаправлений > 20 Тогда
					ВызватьИсключение НСтр("ru='Обнаружена циклическая переадресация.';uk='Виявлена циклічна переадресація.'");
				КонецЕсли;
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		СообщениеОбОшибке = НСтр("ru='Не удалось получить файл с сервера %1:';uk='Не вдалося одержати файл із сервера %1:'") + Символы.ПС + "%2";
