...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru='Обновление сведений о записанных версиях объектов. После выполнения обновления увеличится скорость записи версионируемых объектов.';uk='Оновлення відомостей про записані версії об`єктів. Після виконання оновлення збільшиться швидкість запису версіонуємих об`єктів.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "2.2.4.13";
	Обработчик.Процедура = "ВерсионированиеОбъектов.ПеренестиНастройкиВерсионирования";
	Обработчик.Комментарий = НСтр("ru='Обновление настроек версионирования.';uk='Оновлення налаштувань версіонування.'");
	...
	Если Выборка.Количество() > 0 Тогда
		Если ОбработаноЗаписей = 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Процедуре ОбновитьСведенияОВерсияхОбъектов не удалось обработать некоторые записи регистра сведений ВерсииОбъектов (пропущены): %1';uk='Процедурі ОбновитьСведенияОВерсияхОбъектов не вдалося "
"обробити деякі записи регістра відомостей ВерсииОбъектов (пропущені): %1'"), 
					Выборка.Количество());
		...
		Если Не Объект.ЭтоНовый() И (ИзмененаПроведенность И СведенияОВерсииОбъекта.НомерВерсии > 1 Или ВерсияОтличаетсяОтРанееЗаписанной(Объект)) Тогда
			...
			Если СведенияОВерсииОбъекта.НомерВерсии = 1 Тогда
				Если ОбъектВерсионируется(Объект.Ссылка, Ложь) Тогда
					...
					ПараметрыВерсии.Вставить("Комментарий", НСтр("ru='Версия создана по уже имеющемуся объекту';uk='Версія створена по вже наявному об`єкту'"));
