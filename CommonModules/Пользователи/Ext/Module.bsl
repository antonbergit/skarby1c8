...
Функция СоздатьАдминистратора(ПользовательИБ = Неопределено) Экспорт
	...
	Если Не ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение НСтр("ru='Справочник Пользователи недоступен в неразделенном режиме.';uk='Довідник Користувачі недоступний у нерозділеному режимі.'");
	КонецЕсли;
...
"Требуется назначить роль Полные права хотя бы одному пользователю.';uk='Список користувачів інформаційної бази не порожній, однак"
"не вдалося знайти жодного користувача з роллю Повні права."
""
"Імовірно, користувачі створювалися в конфігураторі."
...
""
"Вероятно, пользователи создавались в конфигураторе."
"Требуется назначить роли Полные права и Администратор системы хотя бы одному пользователю.';uk='Список користувачів інформаційної бази не порожній, однак не вдалося"
"знайти жодного користувача з ролями Повні права й Адміністратор системи."
""
"Імовірно, користувачі створювалися в конфігураторі."
"Потрібно призначити ролі Повні права й Адміністратор системи хоча б одному користувачеві.'");
			КонецЕсли;
...
"создания пользователя в справочнике требуется назначить ему роль Полные права.';uk='Неможливо створити користувача в довіднику для користувача"
"інформаційної бази ""%1"","
"тому що в нього немає ролі Повні права."
""
"Імовірно, користувач був створений у конфігураторі. Для автоматичного"
...
"назначить ему роли Полные права и Администратор системы.';uk='Неможливо створити користувача в довіднику для користувача"
"інформаційної бази ""%1"","
"тому що в нього немає ролей Повні права й Адміністратор системи."
""
"Імовірно, користувач був створений у конфігураторі."
"Для автоматичного створення користувача в довіднику потрібно"
"призначити йому ролі Повні права й Адміністратор системи.'");
			КонецЕсли;
	...
	Пользователь.ДополнительныеСвойства.Вставить("СозданиеАдминистратора",
		?(ПользовательИБ = Неопределено,
		  НСтр("ru='Выполнено создание первого администратора.';uk='Виконане створення першого адміністратора.'"),
		  ?(ПользовательСоздан,
...
"%1 произошла ошибка:"
""
"""%2"".';uk='При видаленні користувача інформаційної бази"
"%1 відбулася помилка:"
""
"""%2"".'"),
				ПользовательИБ.Имя,
...
"соответствует более одного элемента в справочнике Пользователи:';uk='Користувачеві сервісу з ідентифікатором ""%1"""
"відповідає більш одного елемента в довіднику Користувачі:'"),
						ТекущийНеоднозначныйИдентификатор);
				Иначе
					ОписаниеОшибки = ОписаниеОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
