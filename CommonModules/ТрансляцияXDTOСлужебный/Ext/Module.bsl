...
Функция ВыполнитьТрансляцию(Знач ИсходныйОбъект, Знач ОписаниеИсходнойВерсии, Знач ОписаниеРезультирующейВерсии) Экспорт
	...
	Если ЦепочкаТрансляцииИнтерфейса = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='В конфигурации не зарегистрирован обработчик трансляции из версии %1 в версию %2!';uk='У конфігурації не зареєстрований обробник трансляції з версії "
"%1 у версію %2!'"),
...
Функция ТранслироватьОбъект(Знач Объект, Знач ПравилаТрансляцииИнтерфейсов)
	...
	Если ЦепочкаТрансляцииИнтерфейса = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось определить обработчик трансляции для пакета {%1}, невозможно выполнение стандартной трансляции для обработки данного свойства';uk='Не вдалося визначити обробник трансляції для пакета {%1}, "
"неможливе виконання стандартної трансляції для обробки "
"цієї властивості'"), 
...
Функция СтандартнаяОбработка(Знач Объект, Знач ПакетРезультирующегоОбъекта, Знач ПравилаТрансляцииИнтерфейсов)
	...
	Иначе
		...
		Если ТипРезультирующегоОбъекта = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось выполнить стандартную обработку трансляции типа %1 в пакет %2: тип %1 не существует в пакете %2!';uk='Не вдалося виконати стандартну обробку трансляції типу %1 в "
"пакет %2: тип %1 не існує у пакеті %2!'"),
	...
	Для Каждого Свойство Из ТипРезультирующегоОбъекта.Свойства Цикл
		...
		Если СвойствоОригинала = Неопределено Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось выполнить стандартную обработку конвертации типа %1 в тип %2: свойство %3 не определено для типа %1!';uk='Не вдалося виконати стандартну обробку конвертації типу %1 у "
"тип %2: властивість %3 не визначено для типу %1!'"),
				"{" + ТипИсходногоОбъекта.URIПространстваИмен + "}" + ТипИсходногоОбъекта.Имя,
	...
	Для Каждого Свойство Из ТипИсходногоОбъекта.Свойства Цикл
		...
		Если ТранслируемоеСвойство = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось выполнить стандартную обработку конвертации типа %1 в тип %2: свойство %3 не определено для типа %2!';uk='Не вдалося виконати стандартну обробку конвертації типу %1 у "
"тип %2: властивість %3 не визначено для типу %2!'"),
				"{" + ТипИсходногоОбъекта.URIПространстваИмен + "}" + ТипИсходногоОбъекта.Имя,
				"{" + ТипРезультирующегоОбъекта.URIПространстваИмен + "}" + ТипРезультирующегоОбъекта.Имя,
				Свойство.ЛокальноеИмя);
		КонецЕсли;
