...
"%ИнформацияОбОшибке%';uk='Помилка одержання імені користувача ІБ (ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()) через:"
"%ИнформацияОбОшибке%'");
...
"%ИнформацияОбОшибке%';uk='Помилка одержання користувача з довідника Користувачі по його імені (%ИмяПользователяИБ%) в ІБ (ОбработкаНовостейПовтИсп.ПолучитьПользователяПоИмениПользователяИБ()) через:"
" %ИнформацияОбОшибке%'");
...
"Коротке ім`я користувача: %КраткоеИмяПользователя%"
...
Функция ПолучитьОписаниеОтборов(ЛентаНовостей, Пользователь) Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Если ствПользовательскиеОтборы.Количество() > 0 Тогда
			ПредставлениеВсехОтборовПодробно = ПредставлениеВсехОтборовПодробно + НСтр("ru='Отборы, настроенные пользователем (можно изменить):';uk='Відбори, налаштовані користувачем (можна змінити):'") + Символы.ПС;
		...
		Если ствАдминистративныеОтборы.Количество() > 0 Тогда
			ПредставлениеВсехОтборовПодробно = ПредставлениеВсехОтборовПодробно + НСтр("ru='Отборы, настроенные администратором (нельзя изменить):';uk='Відбори, налаштовані адміністратором (не можна змінити):'") + Символы.ПС;
		...
		Если ствАвтоматическиЗаполняемыеОтборы.Количество() > 0 Тогда
			ПредставлениеВсехОтборовПодробно = ПредставлениеВсехОтборовПодробно + НСтр("ru='Автоматически заполняемые отборы (нельзя изменить):';uk='Автоматично заповнювані відбори (не можна змінити):'") + Символы.ПС;
