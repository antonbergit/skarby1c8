...
// (реквизит РучноеИзменение). Связь ищем по МФО и Коррсчету (только для элементов).
...
"%2';uk='Помилка при записі банку-групи (регіону) %1."
"%2'");
					ТекстСообщения    = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
						ВыборкаБанков.РодительНаименование,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
					НомерОбласти      = ?(ОбщегоНазначенияПовтИсп.РазделениеВключено(),
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru=' в области %1';uk=' в області %1'"), ОбластьДанных),
						"");
					ИмяСобытия        = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Обновление банков%1';uk='Оновлення банків%1'"), НомерОбласти);
		...
		Попытка
			Банк.Записать();
		Исключение
			ШаблонСообщения   = НСтр("ru='Ошибка при записи банка с МФО %1"
"%2';uk='Помилка при записі банку з МФО %1"
"%2'");
			ТекстСообщения    = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
				ВыборкаБанков.Код,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ПодобратьБанкИзКлассификатора(Знач СсылкиБанков, ИгнорироватьРучноеИзменение = Ложь) Экспорт
	...
	|	ВТ_КлассификаторБанковРФ.МФО КАК Код,
	...
	|			И ВТ_КлассификаторБанковРФ.МФО = Банки.Код
	|ГДЕ
	...
	|	ВТ_КлассификаторБанковРФ КАК ВТ_КлассификаторБанковРФ
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Банки КАК Банки
	|		ПО ВТ_КлассификаторБанковРФ.МФО = Банки.Код
	|ГДЕ
	|	ВТ_КлассификаторБанковРФ.ЭтоГруппа
	|
	|ИНДЕКСИРОВАТЬ ПО
...
Процедура ВосстановитьЭлементИзОбщихДанных(Знач Форма) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Восстановление из общих данных';uk='Відновлення із загальних даних'"), 
...
//
// Параметры:
//  ПараметрПоиска		 - Строка - часть наименования банка или МФО.
// 
// Возвращаемое значение:
...
Функция ПредставлениеБанка(ЗаписьКлассификатора) Экспорт
	...
	МФО = Новый ФорматированнаяСтрока(
...
Процедура ОбработатьФлагРучногоИзменения(Знач Форма)
	...
	Если Форма.РучноеИзменение = Неопределено Тогда
		...
		Иначе
			Форма.ТекстРучногоИзменения = НСтр("ru='Элемент создан вручную. Автоматическое обновление не возможно.';uk='Елемент створений вручну. Автоматичне оновлення не можливе.'");
		КонецЕсли;
	...
	ИначеЕсли Форма.РучноеИзменение = Истина Тогда
		Форма.ТекстРучногоИзменения = НСтр("ru='Автоматическое обновление элемента отключено.';uk='Автоматичне оновлення елемента відключене.'");
		...
		КонецЕсли;
	Иначе
		Форма.ТекстРучногоИзменения = НСтр("ru='Элемент обновляется автоматически.';uk='Елемент оновляється автоматично.'");
		...
		Если НЕ ПустаяСтрока(ПараметрыОбъект.РодительКод) И НЕ ЗначениеЗаполнено(ПараметрыОбъект.Родитель) Тогда
			...
			Если НЕ ЗначениеЗаполнено(БанкОбъект.Родитель) Тогда
				ИмяСобытия = ?(ИмяСобытия = "",
					НСтр("ru='Подбор из классификатора';uk='Підбір з класифікатора'"), ИмяСобытия);
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не смогли получить родителя у элемента с МФО %1';uk='Не змогли отримати родителя у елементу з МФО %1'"), СокрЛП(ПараметрыОбъект.Код));
				ЗаписьЖурналаРегистрации(ИмяСобытия, 
...
// по текстовому представлению МФО или Коррсчет
//
Функция СсылкаПоКлассификатору(МФО, Коррсчет = "")
...
// по текстовому представлению МФО или Коррсчет
//
Функция СсылкаНаБанк(МФО, ЭтоРегион = Ложь)
