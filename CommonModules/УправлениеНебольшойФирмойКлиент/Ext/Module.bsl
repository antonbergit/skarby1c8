...
Процедура ОбновитьСостояниеДокумента(Объект, СостояниеДокумента, КартинкаСостоянияДокумента, РазрешеноПроведение) Экспорт
	...
	Если Объект.Проведен Тогда
		СостояниеДокумента = НСтр("ru='Проведен';uk='Проведений'");
		КартинкаСостоянияДокумента = 1;
	ИначеЕсли РазрешеноПроведение Тогда
		СостояниеДокумента = НСтр("ru='Не проведен';uk='Не проведений'");
		КартинкаСостоянияДокумента = 0;
	Иначе
		СостояниеДокумента = НСтр("ru='Записан';uk='Записаний'");
...
Процедура ПриИзмененииПериодаРегистрации(ПереданнаяФорма) Экспорт
	...
	Если СтрНайти(ПереданнаяФорма.ИмяФормы, "ЖурналДокументов") > 0 
		...
		ПереданнаяФорма.ОтображениеПериодаРегистрации 	= Формат(ПереданнаяФорма.ПериодРегистрации, "Л="+Локализация.ОпределитьКодЯзыкаДляФормат(КодЯзыка)+";ДФ='MMMM yyyy'");
	ИначеЕсли СтрНайти(ПереданнаяФорма.ИмяФормы, "ФормаСписка") > 0 Тогда
		...
		ПереданнаяФорма.ОтображениеПериодаРегистрации 	= Формат(ПереданнаяФорма.ОтборПериодРегистрации, "Л="+Локализация.ОпределитьКодЯзыкаДляФормат(КодЯзыка)+";ДФ='MMMM yyyy'");
	Иначе
		ПереданнаяФорма.Объект.ПериодРегистрации 		= НачалоМесяца(ПереданнаяФорма.Объект.ПериодРегистрации);
		ПереданнаяФорма.ОтображениеПериодаРегистрации 	= Формат(ПереданнаяФорма.Объект.ПериодРегистрации, "Л="+Локализация.ОпределитьКодЯзыкаДляФормат(КодЯзыка)+";ДФ='MMMM yyyy'");
	КонецЕсли;
	...
	Если ПараметрКоманды.Форма.ИмяФормы = "Справочник.ДоговорыКонтрагентов.Форма.ФормаЭлемента" Тогда
		...
		Если НЕ ВсеРедактируемыеПараметрыЗаполнены Тогда
			ПоказатьВопрос(Новый ОписаниеОповещения("ПечатьДоговорКонтрагентаВопрос", ЭтотОбъект,
			               ...
			               НСтр("ru='Не все редактируемые вручную поля заполнены, продолжить печать?';uk='Не всі поля, що редагуються вручну, заповнені, продовжити друк?'"), РежимДиалогаВопрос.ДаНет);
		Иначе
...
Процедура ЗагрузитьДанныеИзФайлаВыписки(
		...
		НеУдалятьДокументыКоторыхНетВВыписке = Ложь) Экспорт
...
Процедура ДействияПоВыборуФайла(ДополнительныеПараметры)
	...
	Диалог.Заголовок = НСтр("ru='Выберите файл для загрузки...';uk='Виберіть файл для завантаження...'");
	Диалог.Фильтр = НСтр("ru='Файлы обмена с 1С (*.txt)|*.txt|Все файлы (*.*)|*.*';uk='Файли обміну з 1С (*.txt)|*.txt|Всі файли (*.*)|*.*'");
...
Функция ПолучитьТекстФайла(ДополнительныеПараметры, ИмяФайлаИзНастройки, ОдиночныйРазделительСтрок = Ложь)
	...
	Исключение
		...
		Если ДополнительныеПараметры.Свойство("ВыполнитьДействияЕслиФайлНеПрочитан") И ДополнительныеПараметры.ВыполнитьДействияЕслиФайлНеПрочитан Тогда
			ТекстСообщения = НСтр("ru='Ошибка чтения файла %Файл%.';uk='Ошибка чтения файла %Файл%.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Файл%", ИмяФайла);
			Если ИмяФайлаИзНастройки Тогда
				ТекстСообщения = ТекстСообщения + Символы.ПС + НСтр("ru='Возможно файл, который указан в настройках обмена, не существует.';uk='Возможно файл, который указан в настройках обмена, не существует.'");
			КонецЕсли;
			ТекстСообщения = ТекстСообщения + Символы.ПС + НСтр("ru='Выберите другой файл.';uk='Выберите другой файл.'");
		...
		Иначе
			...
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Файл%", ИмяФайла);
			Если ИмяФайлаИзНастройки Тогда
				ТекстСообщения = ТекстСообщения + Символы.ПС + НСтр("ru='Возможно файл, который указан в настройках обмена, не существует. Выбрать другой файл можно в настройках (кнопка в правом верхнем углу).';uk='Возможно файл, который указан в настройках обмена, не существует. Выбрать другой файл можно в настройках (кнопка в правом верхнем углу).'");
			КонецЕсли;
...
Процедура ЗагрузитьДанныеИзФайлаВыпискиФрагмент(ДополнительныеПараметры)
	...
	Состояние(
		НСтр("ru='Выполняется чтение файла выписки...';uk='Выполняется чтение файла выписки...'"),
		,
	...
	ОткрытьФорму(
		...
		Новый Структура("АдресФайлаКОбработке, ПутьДоФайла, Организация, БанковскийСчетОрганизации, СтатьяДДСВходящий, СтатьяДДСИсходящий, ПроводитьЗагружаемые, ЗаполнятьДолгиАвтоматически, Программа, Кодировка, ВерсияФормата, АвтоматическиПодставлятьДокументы, АнализироватьИсториюВыбораЗначенийРеквизитов, НастройкиПолучены, НеУдалятьДокументыКоторыхНетВВыписке",
		ДополнительныеПараметры.АдресВХранилище, 
		ДополнительныеПараметры.ПутьДоФайла1, 
		ДополнительныеПараметры.Организация, 
		ДополнительныеПараметры.БанковскийСчет, 
		ДополнительныеПараметры.СтатьяДДСВходящий, 
		ДополнительныеПараметры.СтатьяДДСИсходящий, 
		ДополнительныеПараметры.ПроводитьЗагружаемые, 
		ДополнительныеПараметры.ЗаполнятьДолгиАвтоматически, 
		ДополнительныеПараметры.Программа, 
		ДополнительныеПараметры.Кодировка, 
		ДополнительныеПараметры.ВерсияФормата, 
		ДополнительныеПараметры.АвтоматическиПодставлятьДокументы, 
		ДополнительныеПараметры.АнализироватьИсториюВыбораЗначенийРеквизитов,
		ДополнительныеПараметры.НастройкиПолучены,
		ДополнительныеПараметры.НеУдалятьДокументыКоторыхНетВВыписке)
	);
...
Функция ПредложитьЗаполнитьРеквизитыОрганизации(ПараметрыПечати) Экспорт
	...
	ОткрытьФормуЗаполненияРеквизитовДляПечати(ПараметрыПечати, "ИНН,КодПоЕДРПОУ,БанковскийСчетПоУмолчанию,ФайлЛоготип,ЮрАдрес");
	
КонецФункции

Функция ПредложитьЗаполнитьРеквизитыОрганизацииСФаксимильнойПодписью(ПараметрыПечати) Экспорт
	
	ОткрытьФормуЗаполненияРеквизитовДляПечати(ПараметрыПечати, "ИНН,КодПоЕДРПОУ,БанковскийСчетПоУмолчанию,ФайлЛоготип,ФайлФаксимильнаяПечать,ЮрАдрес");
...
Процедура ОбработкаКомандыПечатиЦенниковИЭтикетокНаПринтереЭтикеток(ПараметрыПечати) Экспорт
	...
	Если Обработка.Запасы.НайтиСтроки(Новый Структура("Выбран", Истина)).Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Не выбрано ни одного товара';uk='Не обране жодного товару'"));
		Возврат;
	КонецЕсли;
