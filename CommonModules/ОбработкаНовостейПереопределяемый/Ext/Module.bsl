...
//   НоваяСтрока.Наименование = НСтр("ru='Как обновить конфигурацию';uk='Як обновити конфігурацію'");
//   НоваяСтрока.Ссылка = Справочники.Новости.НайтиПоРеквизиту("УИННовости", "123456"); // Новость должна присутствовать в справочнике!
//   НоваяСтрока = Форма.Новости.Добавить();
//   НоваяСтрока.Наименование = НСтр("ru='Как настроить автообновление';uk='Як настроїти автооновлення'");
...
"	Отказ: %3';uk='Устанавливаем для нового пользователя признак ОповещениеВключено = Ложь."
"Параметры:"
"	Наименование: %1"
"	Это новый пользователь: %2"
"	Отказ: %3'");
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Новости. Переопределяемый. ОбработатьЗаписьПользователя. Начало';uk='Новости. Переопределяемый. ОбработатьЗаписьПользователя. Начало'"), // ИмяСобытия
	...
	ТекстСообщения = НСтр("ru='Завершаем для нового пользователя установку признака ОповещениеВключено = Ложь.';uk='Завершаем для нового пользователя установку признака ОповещениеВключено = Ложь.'");
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Новости. Переопределяемый. ОбработатьЗаписьПользователя. Конец';uk='Новости. Переопределяемый. ОбработатьЗаписьПользователя. Конец'"), // ИмяСобытия
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
...
Функция ПолучитьЗначениеПредопределеннойКатегории(Категория, Знач ПредыдущийРезультат) Экспорт
	...
	Если ВРег(КатегорияКод) = ВРег("ProductName") Тогда // В плане видов характеристик "КатегорииНовостей" такого элемента нет
		...
		Если ВРег(Метаданные.Имя) = ВРег("УправлениеНебольшойФирмойДляУкраины") Тогда
			НовыйРезультат = "SmallBusinessUkr";
		ИначеЕсли ВРег(Метаданные.Имя) = ВРег("УправлениеНебольшойФирмойДляУкраиныБазовая") Тогда
			НовыйРезультат = "SmallBusinessUkrBase";
		ИначеЕсли ВРег(Метаданные.Имя) = ВРег("УправлениеНебольшойФирмойДляУкраиныБазоваяМикро") Тогда
			НовыйРезультат = "SmallBusinessUkrBaseMicro";
