...
Функция ПараметрыИсходящегоЗапроса(ВидДокумента) Экспорт
	...
	Иначе 
		ВызватьИсключение НСтр("ru='Для переданного вида документа параметры не предусмотрены.';uk='Для переданного вида документа параметры не предусмотрены.'");
...
Функция СформироватьHTTPЗапрос(ТранспортныйМодуль, ДанныеЗапроса) Экспорт
	...
	#Если Клиент Тогда
		Состояние(НСтр("ru='Обращение к транспортному модулю ЕГАИС...';uk='Обращение к транспортному модулю ЕГАИС...'"));
	#КонецЕсли
	...
	Исключение
		...
		#Если НЕ ТонкийКлиент Тогда
			ТекстОшибки = НСтр("ru='Ошибка формирования запроса в УТМ по адресу %1';uk='Ошибка формирования запроса в УТМ по адресу %1'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%1", ДанныеЗапроса.АдресЗапроса);
			ЗаписьЖурналаРегистрации(НСтр("ru='ЕГАИС';uk='ЄДАІС'", Метаданные.ОсновнойЯзык.КодЯзыка),
...
Функция ПрочитатьОтветМодуляЕГАИС(КодСостояния, ОтветМодуляЕГАИС, РазбиратьОтвет) Экспорт
	...
	Если КодСостояния = 504 Тогда
		Результат.ОписаниеОшибки = НСтр("ru='Нет связи с УТМ.';uk='Нет связи с УТМ.'");
	Иначе
		Если КодСостояния <> 200 Тогда
			Результат.ОписаниеОшибки = НСтр("ru='Сервер отклонил запрос:';uk='Сервер отклонил запрос:'");
		КонецЕсли;
...
Функция СравнитьВерсии(Знач СтрокаВерсии1, Знач СтрокаВерсии2) Экспорт
	...
	Если Версия1.Количество() <> 4 Тогда
		ТекстСообщения = НСтр("ru='Неправильный формат параметра СтрокаВерсии1: %1';uk='Неправильний формат параметра СтрокаВерсии1: %1'");
	...
	Если Версия2.Количество() <> 4 Тогда
		ТекстСообщения = НСтр("ru='Неправильный формат параметра СтрокаВерсии2: %1';uk='Неправильний формат параметра СтрокаВерсии2: %1'");
