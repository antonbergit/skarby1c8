...
"Некорректное значение параметра ДанныеСертификата ""%1"".';uk='Помилка при виклику процедури ОткрытьСертификат загального модуля ЭлектроннаяПодписьКлиент:"
...
Процедура СохранитьЗапросНаСертификатПослеУстановкиРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	Диалог = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Сохранение);
	Диалог.Заголовок = НСтр("ru='Выберите файл для сохранения запроса на сертификат';uk='Виберіть файл для збереження запиту на сертифікат'");
...
Процедура СохранитьПодписьПослеПолученияФайла(ПолученныеФайлы, Контекст) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ru='Электронная подпись сохранена в файл:';uk='Електронний підпис збережений у файл:'"),,
		ПолученныеФайлы[0].Имя);
...
"%1';uk='Сертифікат не знайдений на комп`ютері через:"
"%1'"),
			ПредставлениеОшибки);
	Иначе
...
Процедура ПроверитьПодписьНаКлиентеПослеПроверкиПодписи(Сертификат, Контекст) Экспорт
	...
	Если Сертификат = Неопределено Тогда
		ВыполнитьОбработкуОповещения(Контекст.Оповещение,
			НСтр("ru='Сертификат не найден в данных подписи.';uk='Сертифікат не знайдений у даних підпису.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьСертификатНаКлиентеПослеОшибкиПроверки(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	Если Контекст.ОписаниеОшибкиНаСервере <> Неопределено Тогда
		ОписаниеОшибки = Контекст.ОписаниеОшибкиНаСервере + " " + НСтр("ru='(на сервере)';uk='(на сервері)'") + Символы.ПС
			+ ОписаниеОшибки + " " + НСтр("ru='(на клиенте)';uk='(на клієнтові)'");
	КонецЕсли;
...
Процедура СоздатьМенеджерКриптографииПослеПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
	...
	ЗаголовокФормы = НСтр("ru='Требуется программа электронной подписи и шифрования';uk='Потрібна програма електронного підпису й шифрування'");
	Операция       = Контекст.Операция;
	
	Если Операция = "Подписание" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось подписать данные по причине:';uk='Не вдалося підписати дані через:'");
		
	ИначеЕсли Операция = "ПроверкаПодписи" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось проверить подпись по причине:';uk='Не вдалося перевірити підпис через:'");
		
	ИначеЕсли Операция = "Шифрование" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось зашифровать данные по причине:';uk='Не вдалося зашифрувати дані через:'");
		
	ИначеЕсли Операция = "Расшифровка" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось расшифровать данные по причине:';uk='Не вдалося розшифрувати дані через:'");
		
	ИначеЕсли Операция = "ПроверкаСертификата" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось проверить сертификат по причине:';uk='Не вдалося перевірити сертифікат через:'");
		
	ИначеЕсли Операция = "ПолучениеСертификатов" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось получить сертификаты по причине:';uk='Не вдалося одержати сертифікати через:'");
...
"Неверное значение параметра Операция ""%1"".';uk='Помилка у функції МенеджерКриптографии."
"Невірне значення параметра Операція ""%1"".'"), Операция);
	Иначе
		ЗаголовокОшибки = НСтр("ru='Не удалось выполнить операцию по причине:';uk='Не вдалося виконати операцію через:'");
	КонецЕсли;
...
"для работы с электронной подписью и шифрованием.';uk='В оглядач інтернету потрібно встановити розширення"
...
Процедура СоздатьМенеджерКриптографииПослеЦикла(Контекст)
	...
	Если СвойстваОшибки.Ошибки.Количество() = 0 Тогда
		ТекстОшибки = НСтр("ru='Не предусмотрено использование ни одной программы.';uk='Не передбачене використання жодної програми.'");
		СвойстваОшибки.Вставить("Описание", ТекстОшибки);
	...
	Иначе
		Если ЗначениеЗаполнено(Контекст.Программа) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				Контекст.Программа);
		Иначе
			ТекстОшибки = НСтр("ru='Ни одна из программ не доступна или не установлена на компьютере.';uk='Жодна із програм не доступна або не встановлена на комп`ютері.'");
		КонецЕсли;
		СвойстваОшибки.Вставить("Описание", ТекстОшибки);
	КонецЕсли;
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайлаПослеПодключенияРасширения(Подключено, Контекст) Экспорт
	...
	Диалог.Заголовок = НСтр("ru='Выберите файл сертификата (только для шифрования)';uk='Виберіть файл сертифіката (тільки для шифрування)'");
...
"указан некорректный тип параметра ОшибкаНаКлиенте.';uk='Для процедури ПоказатьОшибкуОбращенияКПрограмме"
"зазначений некоректний тип параметра ОшибкаНаКлиенте.'");
	КонецЕсли;
	
	Если ТипЗнч(ОшибкаНаСервере) <> Тип("Структура") Тогда
		ВызватьИсключение
			НСтр("ru='Для процедуры ПоказатьОшибкуОбращенияКПрограмме"
"указан некорректный тип параметра ОшибкаНаСервере.';uk='Для процедури ПоказатьОшибкуОбращенияКПрограмме"
"зазначений некоректний тип параметра ОшибкаНаСервере.'");
	КонецЕсли;
...
Процедура НачалоВыбораСертификатаПриУстановленномОтборе(Форма) Экспорт
	...
	Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
		Заголовок = НСтр("ru='Следующие разрешенные сертификаты доступны для выбора:';uk='Наступні дозволені сертифікати доступні для вибору:'");
	...
	Если ЗначениеЗаполнено(НедоступныеСертификаты) Тогда
		Если ЭлектроннаяПодписьКлиентСервер.ОбщиеНастройки().СоздаватьЭлектронныеПодписиНаСервере Тогда
			Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
				Заголовок = НСтр("ru='Следующие разрешенные сертификаты не найдены ни на компьютере, ни на сервере:';uk='Наступні дозволені сертифікати не знайдені ні на комп`ютері, ні на сервері:'");
			Иначе
				Заголовок = НСтр("ru='Ни один из следующих разрешенных сертификатов не найден ни на компьютере, ни на сервере:';uk='Жоден з наступних дозволених сертифікатів не знайдений ні на комп`ютері, ні на сервері:'");
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
				Заголовок = НСтр("ru='Следующие разрешенные сертификаты не найдены на компьютере:';uk='Наступні дозволені сертифікати не знайдені на комп`ютері:'");
			Иначе
				Заголовок = НСтр("ru='Ни один из следующих разрешенных сертификатов не найден на компьютере:';uk='Жоден з наступних дозволених сертифікатів не знайдений на комп`ютері:'");
			КонецЕсли;
		КонецЕсли;
...
Процедура ДобавитьПодписьИзФайлаПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если Контекст.ПроверитьМенеджерКриптографииНаКлиенте
	   И ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		...
		ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru='Требуется программа электронной подписи и шифрования';uk='Потрібна програма електронного підпису й шифрування'"),
...
"необходимо установить расширение для работы с файлами.';uk='Перед установкою розширення для роботи з електронним підписом і шифруванням"
	...
	Если Контекст.НаСторонеКлиента Тогда
		...
		Если ЭтоXMLDSig Тогда
			Если Контекст.Операция <> "Подписание" Тогда
				Ошибка = Новый Структура("ОписаниеОшибки",
					ЭлектроннаяПодписьСлужебныйКлиентСервер.ЗаголовокОшибкиПолученияДанных(Контекст.Операция)
					+ Символы.ПС + НСтр("ru='Внешняя компонента XMLDSig может использоваться только для подписания.';uk='Внешняя компонента XMLDSig может использоваться только для подписания.'"));
				ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
				Возврат;
			КонецЕсли;
		...
		Если Контекст.НаСторонеКлиента Тогда
			...
			Исключение
				...
				ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки",
					НСтр("ru='При записи подписи возникла ошибка:';uk='При записі підпису виникла помилка:'")
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиПодписи(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru='При записи подписи возникла ошибка:';uk='При записі підпису виникла помилка:'") + Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиЗашифрованныхДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru='При записи зашифрованных данных возникла ошибка:';uk='При записі зашифрованих даних виникла помилка:'")
			+ Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиРасшифрованныхДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru='При записи расшифрованных данных возникла ошибка:';uk='При записі розшифрованих даних виникла помилка:'")
			+ Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ПолучитьДанныеИзОписанияДанныхЗавершение(ПомещенныеФайлы, Контекст) Экспорт
	...
	Если ПомещенныеФайлы = Неопределено Или ПомещенныеФайлы.Количество() = 0 Тогда
		Результат = Новый Структура("ОписаниеОшибки",
			НСтр("ru='Передача данных отменена пользователем.';uk='Передача даних скасована користувачем.'"));
	Иначе
	...
	Если Подключено Тогда
		...
		Исключение
			...
			ЗавершитьОперациюСОшибкой(
				Контекст,
				НСтр("ru='Ошибка инициализации компоненты XMLDSig.';uk='Ошибка инициализации компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Если Выполнено Тогда
			...
			Иначе
				ЗавершитьОперациюСОшибкой(
					Контекст,
					НСтр("ru='Не установлен режим работы компоненты XMLDSig.';uk='Не установлен режим работы компоненты XMLDSig.'"));
			КонецЕсли;
...
"Перейти к установке данной компоненты?';uk='Для подписания XML необходима установка компоненты XMLDSig."
"Перейти к установке данной компоненты?'");
	...
	Иначе
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Операция невозможна. Требуется установка компоненты XMLDSig.';uk='Операция невозможна. Требуется установка компоненты XMLDSig.'"));
...
Процедура ПередУстановкойКомпоненты(РезультатВопроса, Контекст) Экспорт
	...
	Если НЕ РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Операция невозможна. Требуется установка компоненты для HTTP-запросов.';uk='Операция невозможна. Требуется установка компоненты для HTTP-запросов.'"));
...
Процедура НачатьПодписаниеСообщенияSOAP(ИнформацияМодуляКриптографии, Контекст) Экспорт
	...
	Если ТипКриптопровайдера = Неопределено Тогда
		...
		ЗавершитьОперациюСОшибкой(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось определить тип криптопровайдера %1';uk='Не удалось определить тип криптопровайдера %1'"), ИнформацияМодуляКриптографии.Имя)
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.';uk='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Подписание_ПослеВыполненияGetSignOIDFromCert(OIDАлгоритмаПодписи, Параметры, Контекст) Экспорт
	...
	Если OIDАлгоритмаПодписи = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru='При выполнении метода GetSignOIDFromCert произошла ошибка:';uk='При выполнении метода GetSignOIDFromCert произошла ошибка:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Если OIDАлгоритмаПодписи <> Контекст.ДанныеАлгоритмаПодписания.OIDАлгоритмаПодписи Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Алгоритм подписи сертификата криптографии отличается от алгоритма %1.';uk='Алгоритм подписи сертификата криптографии отличается от алгоритма %1.'"),
	...
	Иначе
		...
		Исключение
			...
			ЗавершитьОперациюСОшибкой(
				Контекст,
				НСтр("ru='Ошибка вызова метода C14N компоненты XMLDSig.';uk='Ошибка вызова метода C14N компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Подписание_ПослеВыполненияC14N_ПодписываемыйТег(КаноникализированныйТекстXML, Параметры, Контекст) Экспорт
	...
	Если КаноникализированныйТекстXML = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru='При выполнении метода C14N произошла ошибка:';uk='При выполнении метода C14N произошла ошибка:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Ошибка вызова метода Hash компоненты XMLDSig.';uk='Ошибка вызова метода Hash компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Подписание_ПослеВыполненияHash_ПодписываемыйТег(DigestValue, Параметры, Контекст) Экспорт
	...
	Если DigestValue = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru='При выполнении метода Hash произошла ошибка:';uk='При выполнении метода Hash произошла ошибка:'"), Контекст);
		Возврат;
	КонецЕсли;
...
Процедура Подписание_ПослеВыполненияC14N_SignedInfo(КаноникализированныйТекстXMLSignedInfo, Параметры, Контекст) Экспорт
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Ошибка вызова метода Sign компоненты XMLDSig.';uk='Ошибка вызова метода Sign компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Подписание_ПослеВыполненияSign(SignatureValue, Параметры, Контекст) Экспорт
	...
	Если SignatureValue = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru='При выполнении метода Sign произошла ошибка:';uk='При выполнении метода Sign произошла ошибка:'"), Контекст);
		Возврат;
	КонецЕсли;
...
Процедура Проверка_ПослеВыполненияC14N_SignedInfo(КаноникализированныйТекстXMLSignedInfo, Параметры, Контекст) Экспорт
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Ошибка вызова метода VerifySign компоненты XMLDSig.';uk='Ошибка вызова метода VerifySign компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Проверка_ПослеВыполненияVerifySign(ПодписьВерна, Параметры, Контекст) Экспорт
	...
	Если ПодписьВерна = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru='При выполнении метода VerifySign произошла ошибка:';uk='При выполнении метода VerifySign произошла ошибка:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Если Не ПодписьВерна Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Подпись не верна (SignatureValue не корректно.';uk='Подпись не верна (SignatureValue не корректно.'"));
...
Процедура Проверка_ПослеВыполненияHash_ПодписываемыйТег(HashValue, Параметры, Контекст) Экспорт
	...
	Если Не ПодписьВерна Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Подпись не верна (SignatureValue корректно, отличается DigestValue).';uk='Подпись не верна (SignatureValue корректно, отличается DigestValue).'"));
...
Процедура НачатьПолучениеТекстаОшибки(НачалоОписанияТекстаОшибки, Контекст)
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Ошибка вызова метода GetLastError компоненты XMLDSig.';uk='Ошибка вызова метода GetLastError компоненты XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
