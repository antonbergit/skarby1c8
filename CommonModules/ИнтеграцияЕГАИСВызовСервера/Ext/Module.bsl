...
Функция ПодготовитьДанныеЗапросаВУТМ(ТранспортныйМодуль, ВидДокумента, Знач ВходныеПараметры, ТекстОшибки) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ФорматОбмена) Тогда
		ТекстОшибки = НСтр("ru='Не заполнен формат обмена в настройках обмена с ЕГАИС.';uk='Не заполнен формат обмена в настройках обмена с ЕГАИС.'");
		Возврат Результат;
	КонецЕсли;
...
Функция ПодготовитьДанныеЗапросаВУТМЧерезОбработкуОбслуживания(ТранспортныйМодуль, ВидДокумента, Знач ВходныеПараметры, ТекстОшибки) Экспорт
	...
	Попытка
		...
		Если НЕ ПараметрыВыполненияКоманды.Свойство("Результат") Тогда
			ТекстОшибки = НСтр("ru='Ошибка выполнения команды.';uk='Ошибка выполнения команды.'");
		...
		КонецЕсли;
	Исключение
		ТекстОшибки = НСтр("ru='Ошибка при вызове обработки обслуживания.';uk='Ошибка при вызове обработки обслуживания.'") + Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
...
Функция ЗагрузитьДокументыИзУТМ(ТранспортныйМодуль, Знач ДополнительныеПараметры) Экспорт
	...
	Для Каждого URLДокумента Из СписокURL Цикл
		...
		Если НЕ РезультатОперации.Результат Тогда
			ТекстОшибки = НСтр("ru='Не удалось получить входящий документ %1';uk='Не удалось получить входящий документ %1'");
	...
	Для Каждого URLДокумента Из Результат.ЗагруженныеДокументы Цикл
		...
		Если НЕ РезультатОперации.Результат Тогда
			ТекстОшибки = НСтр("ru='Не удалось удалить входящий документ %1';uk='Не удалось удалить входящий документ %1'");
...
Функция ОбработатьОтветыИзУТМ(ТранспортныйМодуль, ДополнительныеПараметры) Экспорт
	...
	Если НЕ РезультатОперации.Результат Тогда
		Результат.ОписаниеОшибки = НСтр("ru='Не удалось получить список исходящих запросов.';uk='Не удалось получить список исходящих запросов.'") + Символы.ПС + РезультатОперации.ОписаниеОшибки;
		Возврат Результат;
	КонецЕсли;
	...
	Если НЕ РезультатОперации.Результат Тогда
		Результат.ОписаниеОшибки = НСтр("ru='Не удалось получить список ответов.';uk='Не удалось получить список ответов.'") + Символы.ПС + РезультатОперации.ОписаниеОшибки;
		Возврат Результат;
	КонецЕсли;
	...
	Для Каждого Ответ Из СписокURL Цикл
		...
		Если НЕ РезультатОперации.Результат Тогда
			ТекстОшибки = НСтр("ru='Не удалось получить ответ %1';uk='Не удалось получить ответ %1'");
	...
	Для Каждого URLОтвета Из Результат.ЗагруженныеДокументы Цикл
		...
		Если НЕ РезультатОперации.Результат Тогда
			ТекстОшибки = НСтр("ru='Не удалось удалить ответ %1';uk='Не удалось удалить ответ %1'");
...
Функция ОбработатьВходящиеДокументы(Знач ТранспортныйМодуль, Знач МассивДокументов, Знач ДополнительныеПараметры) Экспорт
	...
	Если НЕ ПустаяСтрока(ТранспортныйМодуль.ИмяОбработкиОбслуживания) Тогда
		...
		Попытка
			...
			Если НЕ ПараметрыВыполненияКоманды.Свойство("Результат") Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка выполнения команды.';uk='Ошибка выполнения команды.'"));
			Иначе
				Результат = ПараметрыВыполненияКоманды.Результат;
			КонецЕсли;
		Исключение
...
Функция ПроверитьВозможностьВыгрузкиДокумента(ВидДокумента, ВходныеПараметры)
	...
	Если НЕ РеквизитыДокумента.Проведен Тогда
		Результат.ОписаниеОшибки = НСтр("ru='Документ не проведен.';uk='Документ не проведен.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если ВидДокумента = Перечисления.ВидыДокументовЕГАИС.АктПодтвержденияТТН
		...
		ИначеЕсли РеквизитыДокумента.СтатусОбработки = Перечисления.СтатусыОбработкиТТНИсходящейЕГАИС.Новый
			...
			Результат.ОписаниеОшибки = НСтр("ru='Акт отказа требует только отправленный в ЕГАИС документ.';uk='Акт отказа требует только отправленный в ЕГАИС документ.'");
			Возврат Результат;
		...
		ИначеЕсли НЕ (РеквизитыДокумента.СтатусОбработки = Перечисления.СтатусыОбработкиТТНВходящейЕГАИС.ПринятИзЕГАИС
			...
			Если ТипЗнч(ВходныеПараметры.ДокументСсылка) = Тип("ДокументСсылка.ТТНИсходящаяЕГАИС") Тогда
				Результат.ОписаниеОшибки = НСтр("ru='Документ уже обработан грузополучателем. Отозвать документ нельзя.';uk='Документ уже обработан грузополучателем. Отозвать документ нельзя.'");
			Иначе
				Результат.ОписаниеОшибки = НСтр("ru='Документ не требует подтверждения.';uk='Документ не требует подтверждения.'");
			КонецЕсли;
	...
	ИначеЕсли ВидДокумента = Перечисления.ВидыДокументовЕГАИС.ПодтверждениеАктаРасхожденийТТН
		...
		ИначеЕсли НЕ (РеквизитыДокумента.СтатусОбработки = Перечисления.СтатусыОбработкиТТНИсходящейЕГАИС.ПринятАктРасхождений
			...
			Результат.ОписаниеОшибки = НСтр("ru='Для документа не был принят акт расхождений.';uk='Для документа не был принят акт расхождений.'");
			Возврат Результат;
	...
	ИначеЕсли ВидДокумента = Перечисления.ВидыДокументовЕГАИС.ЗапросНаОтменуПроведенияТТН Тогда
		...
		ИначеЕсли НЕ (РеквизитыДокумента.СтатусОбработки = Перечисления.СтатусыОбработкиТТНВходящейЕГАИС.ПереданАктПодтверждения
			...
			Результат.ОписаниеОшибки = НСтр("ru='Документ не подтвержден.';uk='Документ не подтвержден.'");
			Возврат Результат;
	...
	ИначеЕсли ВидДокумента = Перечисления.ВидыДокументовЕГАИС.ЗапросНаОтменуПроведенияАктаПостановкиНаБаланс Тогда
		...
		ИначеЕсли НЕ (РеквизитыДокумента.СтатусОбработки = Перечисления.СтатусыОбработкиАктаПостановкиНаБалансЕГАИС.ПереданВЕГАИС
			...
			Результат.ОписаниеОшибки = НСтр("ru='Документ не проведен в ЕГАИС.';uk='Документ не проведен в ЕГАИС.'");
			Возврат Результат;
	...
	ИначеЕсли ВидДокумента = Перечисления.ВидыДокументовЕГАИС.ЗапросОстатков
		...
		ИначеЕсли НЕ (РеквизитыДокумента.СтатусОбработки = Перечисления.СтатусыОбработкиОстатковЕГАИС.Новый
			...
			Результат.ОписаниеОшибки = НСтр("ru='По документу уже были получены остатки.';uk='По документу уже были получены остатки.'");
			Возврат Результат;
...
Функция ТекстОшибкиПередаваемогоДокумента()
	...
	Возврат НСтр("ru='Документ передается в ЕГАИС, дождитесь уведомления о фиксации.';uk='Документ передается в ЕГАИС, дождитесь уведомления о фиксации.'");
...
Функция ТекстОшибкиПереданногоДокумента()
	...
	Возврат НСтр("ru='Документ уже передан в ЕГАИС.';uk='Документ уже передан в ЕГАИС.'");
	...
	Если ЗначениеЗаполнено(ДокументОснование) Тогда
		...
		Если НЕ НовыйСтатус = Неопределено Тогда
			...
			Если НЕ РезультатОперации.Результат Тогда
				...
				ТекстСообщения = НСтр("ru='Не удалось изменить статус обработки документа %1.';uk='Не удалось изменить статус обработки документа %1.'");
...
Функция ЗаблокироватьТранспортныйМодульДляОбмена(ТранспортныйМодуль, ИдентификаторКлиента = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			СтрокаПоиска = НСтр("ru='Объект уже заблокирован:';uk='Объект уже заблокирован:'");
			Если НЕ ПустаяСтрока(СтрокаПоиска) Тогда
				...
				Если ПозицияСимвола > 0 Тогда
					...
					ОписаниеОшибки = НСтр("ru='Обмен с УТМ по адресу %1 уже выполняется:';uk='Обмен с УТМ по адресу %1 уже выполняется:'");
