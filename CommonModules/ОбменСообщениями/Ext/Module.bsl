	...
	ИначеЕсли ТипЗнч(Получатель) = Тип("Массив") Тогда
		...
		Для Каждого Элемент Из Получатель Цикл
			...
			Если ТипЗнч(Элемент) <> Тип("ПланОбменаСсылка.ОбменСообщениями") Тогда
				...
				ВызватьИсключение НСтр("ru='Указан неправильный получатель для метода ОбменСообщениями.ОтправитьСообщение().';uk='Зазначений неправильний одержувач для методу ОбменСообщениями.ОтправитьСообщение().'");
