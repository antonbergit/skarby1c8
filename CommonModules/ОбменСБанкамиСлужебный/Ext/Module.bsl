...
Процедура СохранитьВКБанка(Адрес, Информация) Экспорт
	...
	ВнешниеКомпоненты = ОбщиеФайлыОбменСБанками.ВнешниеКомпоненты;
	...
	ВнешниеКомпоненты.Вставить(Информация.ИмяМодуля, СтруктураДанных);
...
Функция ПараметрыВК(ПутьКФайлуИнформации) Экспорт
	...
	Исключение
		Операция = НСтр("ru='Чтение файла информации о внешней компоненте.';uk='Чтение файла информации о внешней компоненте.'");
		ТекстСообщения = НСтр("ru='При чтении информации о внешней компоненте произошла ошибка.';uk='При чтении информации о внешней компоненте произошла ошибка.'");
...
Функция ИнформацияОВКБанка(АдресВнешнейКомпоненты) Экспорт
	...
	Исключение
		Операция = НСтр("ru='Чтение файла внешнего модуля.';uk='Чтение файла внешнего модуля.'");
		ТекстСообщения = НСтр("ru='При чтении файла внешнего модуля произошла ошибка.';uk='При чтении файла внешнего модуля произошла ошибка.'");
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
	...
	Если Не НайденаИнформация Тогда
		...
		ТекстСообщения = НСтр("ru='Указанный файл не является внешним модулем';uk='Указанный файл не является внешним модулем'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
	...
	Исключение
		Операция = НСтр("ru='Распаковка файла внешнего модуля.';uk='Распаковка файла внешнего модуля.'");
		ТекстСообщения = НСтр("ru='При распаковки файла внешнего модуля произошла ошибка.';uk='При распаковки файла внешнего модуля произошла ошибка.'");
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
		ЭлектронноеВзаимодействиеСлужебный.УдалитьВременныеФайлы(ВремКаталог);
		ЭлектронноеВзаимодействиеСлужебный.УдалитьВременныеФайлы(ВремФайл);
...
Функция СохранитьСообщениеОбмена(Знач ПараметрыСообщения, НовоеСообщениеСсылка) Экспорт
	...
	Если ПараметрыСообщения.Свойство("СсылкаНаОбъект") Тогда
		МенеджерЗаписи = РегистрыСведений.СвязанныеОбъектыЭлектронноеВзаимодействие.СоздатьМенеджерЗаписи();
		МенеджерЗаписи.СсылкаНаОбъект = ПараметрыСообщения.СсылкаНаОбъект;
		МенеджерЗаписи.СообщениеОбмена = НовоеСообщениеСсылка;
		МенеджерЗаписи.Записать();
	КонецЕсли;
...
Процедура ОтправитьВБанк(Настройки, Данные, Результат, ТекстОшибки) Экспорт
	...
	Иначе
		Если ЗначениеЗаполнено(РезультатВыполнения.КодСостояния) Тогда
			Шаблон = НСтр("ru='При отправке документа в банк произошла ошибка (%1). %2';uk='При отправке документа в банк произошла ошибка (%1). %2'");
...
Процедура ОтправитьЗапросВыпискиВБанк(СтруктураПараметров, АдресХранилища) Экспорт
	...
	ВидОперации = НСтр("ru='Отправка запроса выписки в банк';uk='Отправка запроса выписки в банк'");
...
"Причина: %1';uk='Не удалось отправить запрос выписки в банк"
"Причина: %1'");
...
Процедура СохранитьПолученныйПакет(НастройкаОбмена, ИмяФайла, ДанныеВозврата) Экспорт
	...
	Исключение
		ТекстСообщения = НСтр("ru='Получены некорректные данные из банка. Для решения проблемы обратитесь в свой банк.';uk='Получены некорректные данные из банка. Для решения проблемы обратитесь в свой банк.'");
		
		ШаблонОшибкиДляЖурнала = НСтр("ru='Из банка получен файл с некорректным содержимым."
"Путь к файлу: %1"
"Текст ошибки: %2.';uk='Из банка получен файл с некорректным содержимым."
"Путь к файлу: %1"
"Текст ошибки: %2.'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтрШаблон(ШаблонОшибкиДляЖурнала, ИмяФайла, ПодробноеПредставлениеОшибки);
		
		ВидОперации = НСтр("ru='Чтение полученного из банка электронного документа.';uk='Чтение полученного из банка электронного документа.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
	...
	Если НЕ ResultBank.Error = Неопределено Тогда
		ТекстОшибки = ОбменСБанкамиСлужебныйВызовСервера.ТекстСообщенияОбОшибкеОтветаБанка(ResultBank.Error);
	...
	Если НЕ ResultBank.Success.GetPacketResponse = Неопределено Тогда
		...
		ОбменСБанкамиСлужебныйВызовСервера.РаспаковатьПакетОбменСБанками(ПакетОбменСБанками, ДанныеВозврата);
...
"%1';uk='При чтении данных, полученных из банка, произошла ошибка."
"%1'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, КраткоеПредставлениеОшибки);
		ТекстОшибки = НСтр("ru='Путь к файлу: %1"
"Подробное представление ошибки: %2';uk='Путь к файлу: %1"
"Подробное представление ошибки: %2'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтрШаблон(ТекстОшибки, ИмяФайла, ПодробноеПредставлениеОшибки);
		ВидОперации = НСтр("ru='Чтение списка новых документов';uk='Чтение списка новых документов'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			ВидОперации, ТекстОшибки, ТекстСообщения, 1, НастройкаОбмена);
		ЕстьОшибка = Истина;
		ОбъектXML.Закрыть();
	...
	Если НЕ ResultBank.Success = Неопределено Тогда
		...
		Иначе
			Операция = НСтр("ru='Чтение ответа банка.';uk='Чтение ответа банка.'");
			ТекстОшибки = НСтр("ru='Получен некорректный ответ из банка"
"Файл ответа: %1';uk='Получен некорректный ответ из банка"
"Файл ответа: %1'");
...
"%1';uk='При записи даты получения документов произошла ошибка:."
"%1'");
			ТекстСообщения = СтрШаблон(ТекстСообщения, КраткоеПредставлениеОшибки);
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВидОперации = НСтр("ru='Сохранение последней даты получения документов';uk='Сохранение последней даты получения документов'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура СоздатьПакетОбменСБанками(НастройкаОбмена, МассивСообщений, ПакетОбменСБанками) Экспорт
	...
	Исключение
		КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ru='формирование пакета ЭДО';uk='формирование пакета ЭДО'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			...
			Для Каждого СтрокаПодписи Из МассивПодписей Цикл
				...
				Иначе
					...
					Если СвойстваСертификата = Неопределено Тогда
						ВызватьИсключение НСтр("ru='Ошибка чтения свойств сертификата';uk='Ошибка чтения свойств сертификата'");
					КонецЕсли;
...
Процедура ОбработатьОтветИзБанка(ОтветБанка, ОтправленноеСообщениеОбмена, ПолученноеСообщениеОбмена = Неопределено, ЕстьОшибка = Неопределено) Экспорт
	...
	Иначе
		...
		НаименованиеВыписки = СтрЗаменить(НаименованиеЗапроса, НСтр("ru='Запрос выписки';uk='Запрос выписки'"), НСтр("ru='Выписка банка за период';uk='Выписка банка за период'"));
...
Функция СформироватьСообщенияОбмена(МассивОбъектов, ПараметрыОбмена) Экспорт
	...
	Для Каждого СтруктураОбмена Из МассивСтруктурОбмена Цикл
		...
		Если (СтруктураОбмена.СтруктураЭД.ВидЭД = Перечисления.ВидыЭДОбменСБанками.ПлатежноеПоручение
			...
			ПараметрыСообщения = Новый Структура;
			ПараметрыСообщения.Вставить("Представление", НСтр("ru='Схема данных';uk='Схема данных'"));
...
Процедура УстановитьСостояниеПриЗаписиОбъектаИБ(Источник, Отказ) Экспорт
	...
	Если НЕ Источник.ДополнительныеСвойства.Свойство("ЭтоНовыйОбъект") Тогда
		...
		Если НЕ РедактированиеРазрешено Тогда
			ТекстСообщения = НСтр("ru='Существует актуальный электронный документ. Запрещено редактирование ключевых реквизитов документа.';uk='Существует актуальный электронный документ. Запрещено редактирование ключевых реквизитов документа.'");
...
Функция НайтиСоздатьСертификатВнешнегоМодуля(ДанныеСертификата, Организация, ПрограммаБанка, Параметры) Экспорт
	...
	ШаблонНаименования = НСтр("ru='%1, до %2';uk='%1, до %2'");
...
Процедура ЗагрузитьСписокDirectBank(ЗапускИзРегламентногоЗадания = Истина) Экспорт
	...
	АдресСервера = "http://directbank.1c.ru/downloads/bankslist.mxl";
	...
	Если Результат.Статус Тогда
		...
		Исключение
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВидОперации = НСтр("ru='Чтение файла списка банков, поддерживающих обмен через сервис 1С:ДиректБанк.';uk='Чтение файла списка банков, поддерживающих обмен через сервис 1С:ДиректБанк.'");
			КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ШаблонОшибки = НСтр("ru='Ошибка выполнения операции: %1"
"%2';uk='Ошибка выполнения операции: %1"
"%2'");
	...
	ИначеЕсли ЗначениеЗаполнено(Результат.СообщениеОбОшибке) Тогда
		...
		ВидОперации = НСтр("ru='Загрузка списка банков, поддерживающих обмен DirectBank';uk='Загрузка списка банков, поддерживающих обмен DirectBank'");
		ШаблонОшибки = НСтр("ru='Выполнение операции: %1"
"%2';uk='Выполнение операции: %1"
"%2'");
		...
		Шаблон = НСтр("ru='Электронное взаимодействие.Обмен с банками';uk='Электронное взаимодействие.Обмен с банками'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура СоздатьНастройкуОбмена(СтруктураПараметров, АдресХранилища) Экспорт
	...
	Исключение
		ТекстСообщения = НСтр("ru='Ошибка создания настройки обмена: %1';uk='Ошибка создания настройки обмена: %1'");
		КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, КраткоеПредставлениеОшибки);
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ru='Создание настройки обмена';uk='Создание настройки обмена'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
//
Функция СоздатьНастройкуОбменаИзФайла(НастройкиЗаполнения, Недействительна = Ложь, МассивСертификатов = Неопределено, ЗагружатьВК = Истина) Экспорт
...
"Обратитесь в свой банк.'");
	...
	Если Не ЗначениеЗаполнено(СсылкаНаОрганизацию) Тогда
		ТекстСообщения = НСтр("ru='Не найдена организация по реквизитам: %1%2%3';uk='Не найдена организация по реквизитам: %1%2%3'");
	...
	Если НЕ ЗначениеЗаполнено(СсылкаНаБанк) Тогда
		ТекстСообщения = НСтр("ru='Не найден банк по реквизитам: %1%2';uk='Не найден банк по реквизитам: %1%2'");
...
"Наименование: %1."
"Модуль: %2."
"Версия: %3.';uk='Запрещено использование неизвестных внешних компонент:"
"Наименование: %1."
"Модуль: %2."
"Версия: %3.'");
	...
	Если ЗаписыватьОбъект Тогда
		Попытка
			Объект.Записать();
		Исключение
			ТекстСообщения = НСтр("ru='Возникла ошибка при создании настройки обмена с сервисом 1С:ДиректБанк.';uk='Возникла ошибка при создании настройки обмена с сервисом 1С:ДиректБанк.'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВидОперации = НСтр("ru='Создание настройки обмена с сервисом 1С:ДиректБанк.';uk='Создание настройки обмена с сервисом 1С:ДиректБанк.'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
// Функция - XDTOДанные файла настроек.
//
// Параметры:
...
Функция XDTOДанныеФайлаНастроек(АдресФайла) Экспорт
	...
	Попытка
		...
		ЭД.Проверить();
	Исключение
		ТекстСообщения = НСтр("ru='Возникла ошибка при чтении настроек.';uk='Возникла ошибка при чтении настроек.'");
		ВидОперации = НСтр("ru='Чтение настроек обмена с сервисом 1С:ДиректБанк из файла.';uk='Чтение настроек обмена с сервисом 1С:ДиректБанк из файла.'");
		ТекстОшибки = НСтр("ru='При загрузке настроек обмена с сервисом 1С:ДиректБанк произошла ошибка."
"Путь к файлу: %1"
"Информация об ошибке: %2';uk='При загрузке настроек обмена с сервисом 1С:ДиректБанк произошла ошибка."
"Путь к файлу: %1"
...
Функция ОпределитьНастройкиОбменаЭДПоИсточнику(Источник, ВыводитьСообщения, МассивОтпечатковСертификатов, СообщениеОбмена = Неопределено, ВидЭД = Неопределено, ФлагДействующиеНастройкиОбмена = Истина) Экспорт
	...
	Если ВыводитьСообщения И ДействующиеПараметрыОбмена.НастройкаОбменаДействует
		...
		ТекстСообщения = НСтр("ru='Данный вид электронного документа банк не принимает.';uk='Данный вид электронного документа банк не принимает.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат Неопределено;
	КонецЕсли;
...
Функция СформироватьНовыйПакетОбменСБанками(СтруктураПараметров)
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			Операция = НСтр("ru='Создание пакета обмена';uk='Создание пакета обмена'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
	...
	Исключение
		...
		Попытка
			...
			Если ДанныеСертификата = Неопределено Тогда
				ВызватьИсключение НСтр("ru='Некорректное содержимое сертификата криптографии.';uk='Некорректное содержимое сертификата криптографии.'");
			КонецЕсли;
			ТекСертификат = Новый СертификатКриптографии(ДанныеСертификата);
		Исключение
			ТекстСообщения = НСтр("ru='Возникла ошибка при чтении данных сертификата из файла.';uk='Возникла ошибка при чтении данных сертификата из файла.'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВидОперации = НСтр("ru='Чтение настроек ЭДО из файла.';uk='Чтение настроек ЭДО из файла.'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
			Возврат Неопределено;
		КонецПопытки;
...
Функция СформироватьПлатежноеТребованиеAsync(СсылкаНаДокумент, ДеревоДанных)
...
Функция СформироватьПлатежноеПоручение(СсылкаНаОбъект, НастройкиОбменаЭД)
	...
	Если НЕ ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		Если ПрограммаБанка = Перечисления.ПрограммыБанка.СбербанкОнлайн Тогда
			ЭДСформирован = СформироватьПлатежноеПоручениеСбербанк(СсылкаНаОбъект, ДеревоДокумента);
		ИначеЕсли ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен
			...
			ЭДСформирован = СформироватьПлатежноеПоручениеAsync(СсылкаНаОбъект, ДеревоДокумента);
		Иначе
			ЭДСформирован = СформироватьПлатежноеПоручениеCML(СсылкаНаОбъект, ДеревоДокумента);
		КонецЕсли;
...
"%2';uk='При формировании %1 возникли следующие ошибки:"
"%2'");
...
Функция СформироватьПлатежноеТребование(СсылкаНаОбъект, НастройкиОбменаЭД)
	...
	Если НЕ ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		ЭДСформирован = СформироватьПлатежноеТребованиеAsync(СсылкаНаОбъект, ДеревоДокумента);
...
Функция ПрочитатьФайлПоСхеме(ИмяФайла, ДеревоРазбора, НовыйЭД, ПоказыватьОшибки = Ложь)
	...
	Попытка
		...
		Если НеизвестныйЭД Тогда
			Если Пользователи.ЭтоПолноправныйПользователь( , , Ложь) Тогда
				ТекстСообщения = СтрШаблон(
					НСтр("ru='Неизвестный тип ЭД при чтении данных из файла %1.';uk='Неизвестный тип ЭД при чтении данных из файла %1.'"), ИмяФайла);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			КонецЕсли;
			Ошибка = Истина;
		КонецЕсли;
...
Процедура ПрочитатьВыпискуСбербанкXDTO(ЭД, ДеревоРазбора, НовыйЭД, Ошибка)
	...
	Если НЕ ЭД.Docs = Неопределено Тогда
		Для Каждого Документ Из ЭД.Docs.TransInfo Цикл
			...
			ДатаСписано = Дата(СтрЗаменить(Сред(Документ.carryDate, 1, 10), "-", ""));
		...
		Иначе
			...
			Если НайденнаяСтрока.ОписаниеОбъекта <> ОписаниеОбъекта Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка в данных эл.документа: на один Объект <%1>, два Описания <%2> и <%3>.';uk='Ошибка в данных эл.документа: на один Объект <%1>, два Описания <%2> и <%3>.'"), СсылкаНаОбъект,
					НайденнаяСтрока.ОписаниеОбъекта, ОписаниеОбъекта);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
				НайденнаяСтрока.ОписаниеОбъекта = ОписаниеОбъекта;
			КонецЕсли;
...
Процедура ПрочитатьЗапросВыпискиAsyncXDTO(ЭД, ДеревоРазбора, НовыйЭД, Ошибка)
	...
	ТекстБанк = " " + НСтр("ru='в банке';uk='в банке'") + " ";
	Если НЕ DataRequest.Bank.Name = Неопределено Тогда
		ТекстБанк = ТекстБанк + DataRequest.Bank.Name + ", ";
	КонецЕсли;
	ТекстБанк = ТекстБанк + НСтр("ru='БИК:';uk='БІК:'") + " " + DataRequest.Bank.BIC;
...
Функция СформироватьПлатежноеПоручениеCML(СсылкаНаДокумент, ДеревоДанных)
	...
	Попытка 
		...
		Номер = ЭлектронноеВзаимодействиеСлужебный.ЗначениеРеквизитаВДереве(ДеревоДанных, "Номер");
		ЗаполнитьСвойствоXDTO(ПлатежноеПоручениеЭО, "Номер", СокрЛП(Номер), Истина, ТекстОшибки);
		Дата = ЭлектронноеВзаимодействиеСлужебный.ЗначениеРеквизитаВДереве(ДеревоДанных, "Дата");
...
Функция СформироватьПлатежноеПоручениеAsync(СсылкаНаДокумент, ДеревоДанных)
...
Функция СформироватьПлатежноеПоручениеСбербанк(СсылкаНаДокумент, ДеревоДанных)
	...
	Попытка
		...
		ПеречислениеВБюджет = ЭлектронноеВзаимодействиеСлужебный.ЗначениеРеквизитаВДереве(ДеревоДанных, "ПлатежиВБюджет");
...
Функция ПредставлениеЭД(ВидЭД, СсылкаНаОбъект, НомерДокумента)
	...
	Иначе
		ШаблонПредставленияЭД = НСтр("ru='%1 %2 %3';uk='%1 %2 %3'");
	КонецЕсли;
...
"Необходимо создать ""Настройку обмена с сервисом 1С:ДиректБанк"" с реквизитами:';uk='Обработка %1."
"Операция не выполнена."
...
"Ошибка установки значения свойства ""%1"".';uk='Выполнение операции: Заполнение XDTO."
