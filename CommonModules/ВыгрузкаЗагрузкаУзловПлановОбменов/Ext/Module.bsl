...
"ВыгрузкаЗагрузкаУзловПлановОбменов.ПередВыгрузкойОбъекта()!';uk='Об`єкт метаданих %1 не може бути оброблений оброблювачем"
...
Процедура ПередЗагрузкойОбъекта(Контейнер, Объект, Артефакты, Отказ) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(
			НСтр("ru='Объект метаданных %1 не может быть обработан обработчиком"
"ВыгрузкаЗагрузкаУзловПлановОбменов.ПередЗагрузкойОбъекта()!';uk='Об`єкт метаданих %1 не може бути оброблений оброблювачем"
