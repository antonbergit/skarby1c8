...
Процедура ЗагрузитьИУстановитьОбновленияВФоновомРежиме(Параметры) Экспорт
	...
	Если Не ДоступноИспользованиеОбновленияПрограммы(Истина) Тогда
		ВызватьИсключение НСтр("ru='Использование обновления программы недоступно в текущем режиме работы.';uk='Використання оновлення програми недоступне в поточному режимі роботи.'");
	КонецЕсли;
	...
	Если Контекст.ОбновитьПлатформу Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Получение обновления платформы 1С:Предприятие: версия %1; URL: %2';uk='Отримання оновлення платформи 1С:Підприємство: версія %1; URL: %2'"),
		...
		Если Не ПустаяСтрока(Контекст.КаталогДистрибутиваПлатформы) Тогда
			...
			ЗаписатьИнформациюВЖурналРегистрации(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Обновление платформы 1С:Предприятие уже было загружено ранее в %1';uk='Оновлення платформи 1С:Підприємство вже було завантажене раніше в %1'"),
		...
		Иначе
			...
			Если КаталогУстановкиПлатформы <> Неопределено Тогда
				...
				ЗаписатьИнформациюВЖурналРегистрации(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Версия %1 платформы 1С:Предприятие уже установлена на компьютере.';uk='Версія %1 платформи 1С:Підприємство вже встановлена ​​на комп`ютері.'"),
						Контекст.ВерсияПлатформы));
...
"Ім`я файлу оновлення (cfu): %7.'"),
	...
	Если Контекст.ОбновитьПлатформу И Не Контекст.ОбновлениеПлатформыУстановлено Тогда
		...
		Контекст.ТекущееДействие = НСтр("ru='Установка платформы 1С:Предприятие';uk='Установка платформи 1С:Підприємство'");
...
Процедура ЗагрузитьОбновлениеПлатформы(Контекст, Параметры)
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Получение файла обновления платформы 1С:Предприятие: %1';uk='Отримання файлу оновлення платформи 1С:Підприємство: %1'"),
	...
	Если Не ПустаяСтрока(РезультатПолучения.КодОшибки) Тогда
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при получении файла дистрибутива платформы 1С:Предприятие (%1). %2';uk='Помилка при одержанні файлу дистрибутива платформи 1С:Підприємство (%1). % 2'"),
				Контекст.URLФайлаОбновленияПлатформы,
		...
		Контекст.ИнформацияОбОшибке = СообщениеЖурнала;
		Контекст.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при получении файла дистрибутива. %1';uk='Помилка при одержанні файлу дистрибутива. % 1'"),
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Файл обновления платформы 1С:Предприятие успешно получен: %1';uk='Файл оновлення платформи 1С:Підприємство успішно отриманий: %1'"),
			ПутьПолученногоФайла));
	...
	Попытка
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Извлечение дистрибутива платформы 1С:Предприятие в %1';uk='Витяг дистрибутива платформи 1С:Підприємство в %1'"),
				КаталогДистрибутива));
	...
	Исключение
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при извлечении файлов архива (%1) в каталог %2.';uk='Помилка при витягу файлів архіву (%1) у каталог %2.'"),
				ПутьПолученногоФайла,
				КаталогДистрибутива)
			+ Символы.ПС
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Дистрибутив платформы 1С:Предприятие успешно сохранен в %1';uk='Дистрибутив платформи 1С:Підприємство успішно збережений в %1'"),
			КаталогДистрибутива));
...
Процедура УстановитьОбновлениеПлатформы(Контекст, Параметры, УстановкаОтменена)
	...
	Если Не ПолучениеОбновленийПрограммыКлиентСервер.ФайлСуществует(ПутьФайлаПрограммыУстановки) Тогда
		...
		СообщениеЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Загружен некорректный дистрибутив платформы 1С:Предприятие. Отсутствует файл setup.exe (%1).';uk='Завантажений некоректний дистрибутив платформи 1С:Підприємство. Відсутній файл setup.exe (%1).'"),
			ПутьФайлаПрограммыУстановки);
		ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		
		Контекст.ИмяОшибки = "НекорректныйДистрибутивПлатформы";
		Контекст.ИнформацияОбОшибке = СообщениеЖурнала;
		Контекст.Сообщение = НСтр("ru='Загружен некорректный дистрибутив платформы 1С:Предприятие. Отсутствует файл setup.exe.';uk='Завантажений некоректний дистрибутив платформи 1С:Підприємство. Відсутній файл setup.exe.'");
		Возврат;
	...
	Попытка
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Установка новой версии платформы 1С:Предприятие (%1). %2';uk='Установка нової версії платформи 1С:Підприємство (%1). % 2'"),
				Контекст.ВерсияПлатформы,
				КомандаЗапуска));
	...
	Исключение
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при запуске программы установки платформы 1С:Предприятие (%1). %2';uk='Помилка при запуску програми установки платформи 1С:Підприємство (%1). % 2'"),
				КомандаЗапуска,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Контекст.ИнформацияОбОшибке = СообщениеЖурнала;
		Контекст.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при запуске программы установки платформы 1С:Предприятие. %1';uk='Помилка при запуску програми установки платформи 1С:Підприємство. % 1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	...
	Если КодВозврата = 0 Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(НСтр("ru='Новая версия платформы 1С:Предприятие успешно установлена.';uk='Нова версія платформи 1С:Підприємство успішно встановлена.'"));
...
"Версия %1;"
"Код возврата: %2;"
"Команда: %3';uk='Установка платформи 1С:Підприємство завершена з помилкою."
"Версія %1;"
"Код повернення: %2;"
"Команда: %3'"),
					Контекст.ВерсияПлатформы,
...
"<br />Код возврата: %1.';uk='<body>При встановленні нової версії платформи 1С:Підприємство сталася помилка."
"<br />Код повернення: %1.'");
			
			Если ПолучениеОбновленийПрограммыКлиентСервер.ФайлСуществует(ПутьФайлаПротокола) Тогда
				ШаблонСообщенияПользователю = ШаблонСообщенияПользователю + Символы.ПС
					+ НСтр("ru='<br />Техническая информация содержится в <a href=""open:debuglog"">протоколе установки</a>.';uk='<br />Технічна інформація міститься в <a href=""open:debuglog"">протоколі встановлення</a>.'");
			КонецЕсли;
...
"<br />Рекомендуется установить платформу с ручными настройками.</p>';uk='<br /><br /><p>Дана помилка пов`язана з обмеженнями системних політик безпеки."
"<br />Рекомендується встановити платформу з ручними налаштуваннями.</p>'");
			КонецЕсли;
			
			ШаблонСообщенияПользователю = ШаблонСообщенияПользователю + Символы.ПС
				+ НСтр("ru='<br /><br />При возникновении проблем напишите в <a href=""mailto:webits-info@1c.ru"">техподдержку</a>.</body>';uk='<br /><br />При виникненні проблем напишіть в <a href=""mailto:web-its@1c.ua"">техпідтримку</a>.</body>'");
