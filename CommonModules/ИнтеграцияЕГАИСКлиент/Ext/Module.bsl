...
Процедура НачатьФормированиеПодписиЧека(ОповещениеПриЗавершении, ПараметрыЧека, ТранспортныйМодуль = Неопределено, ОтображатьСообщения = Истина) Экспорт
	...
	ПараметрыЧекаЕГАИС = ПодготовитьВходящиеДанныеДляПодписиЧека(ПараметрыЧека, РезультатОперации);
	
	Если НЕ РезультатОперации.Результат Тогда
		ЗавершитьОперациюСОшибкой(ОповещениеПриЗавершении, ОтображатьСообщения, РезультатОперации.ОписаниеОшибки);
		Возврат;
	КонецЕсли;
...
Процедура УдалениеДанныхПоСпискуURL_ПослеУдаленияЭлементаСписка(Результат, Контекст) Экспорт
	...
	Если НЕ Результат.Результат Тогда
		ТекстОшибки = НСтр("ru='Не удалось удалить документ %1';uk='Не удалось удалить документ %1'");
...
Процедура ФормированиеИсходящегоЗапроса_ПослеВыбораТранспортногоМодуля(ТранспортныйМодуль, Контекст) Экспорт
	...
	Если (Контекст.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросОрганизаций")
		...
		Если Контекст.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросАлкогольнойПродукции") Тогда
			ПоказатьВводСтроки(ОписаниеОповещения,, НСтр("ru='Введите ИНН производителя или импортера алкогольной продукции';uk='Введите ИНН производителя или импортера алкогольной продукции'"), 12);
		Иначе
			ПоказатьВводСтроки(ОписаниеОповещения,, НСтр("ru='Введите ИНН производителя, импортера или собственной организации';uk='Введите ИНН производителя, импортера или собственной организации'"), 12);
		КонецЕсли;
	...
	ИначеЕсли (Контекст.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросСправкиА")
		...
		Если Контекст.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросСправкиА") Тогда
			ПоказатьВводСтроки(ОписаниеОповещения,, НСтр("ru='Введите регистрационный номер справки 1';uk='Введите регистрационный номер справки 1'"), 50);
		Иначе
			ПоказатьВводСтроки(ОписаниеОповещения,, НСтр("ru='Введите регистрационный номер справки 2';uk='Введите регистрационный номер справки 2'"), 50);
		КонецЕсли;
	...
	ИначеЕсли Контекст.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросТТН")
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ФормированиеИсходящегоЗапроса_ПослеВводаДополнительныхПараметров", ЭтотОбъект, Контекст);
		ПоказатьВводСтроки(ОписаниеОповещения,, НСтр("ru='Введите идентификатор ТТН';uk='Введите идентификатор ТТН'"), 50);
...
Процедура ФормированиеИсходящегоЗапроса_ПослеВводаДополнительныхПараметров(ПараметрЗапроса, Контекст) Экспорт
	...
	Если Контекст.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросОрганизаций")
		...
		Если ПустаяСтрока(ПараметрЗапроса) Тогда
			ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ru='Не введен ИНН.';uk='Не введен ИНН.'"));
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли Контекст.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросСправкиА")
		...
		Если ПустаяСтрока(ПараметрЗапроса) Тогда
			ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ru='Не введен регистрационный номер.';uk='Не введен регистрационный номер.'"));
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли Контекст.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросТТН") Тогда
		...
		Если ПустаяСтрока(ПараметрЗапроса) Тогда
			ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ru='Не введен идентификатор ТТН.';uk='Не введен идентификатор ТТН.'"));
			Возврат;
		КонецЕсли;
...
Процедура ЗагрузкаДокументов_ПослеВыбораТранспортногоМодуля(ТранспортныйМодуль, Контекст) Экспорт
	...
	Если НЕ ТранспортныйМодуль.ЗагружатьВходящиеДокументы Тогда
		ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ru='Загрузка документов запрещена.';uk='Загрузка документов запрещена.'"));
		Возврат;
	КонецЕсли;
...
//
Функция ПодготовитьВходящиеДанныеДляПодписиЧека(ПараметрыЧека, РезультатПодготовки = Неопределено)
	...
	Если ТаблицаТоваров.Количество() > 0 И ТаблицаТоваров[0].Количество() < 18 ИЛИ ШапкаЧека.Количество() < 21 Тогда
		РезультатПодготовки.ОписаниеОшибки = НСтр("ru='В процедуру подготовки входящих данных передана некорректная структура параметров.';uk='В процедуру подготовки входящих данных передана некорректная структура параметров.'");
...
Функция ПроверитьВходящиеДанныеПередПодписьюЧека(ТранспортныйМодуль, ПараметрыЧекаЕГАИС)
	...
	Если ПустаяСтрока(ПараметрыЧекаЕГАИС.ИНН) Тогда
		ДобавитьСтрокуСОшибкой(ТекстОшибки, НСтр("ru='Не указан ИНН организации.';uk='Не указан ИНН организации.'"), "");
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыЧекаЕГАИС.КПП) И СтрДлина(ПараметрыЧекаЕГАИС.ИНН) = 10 Тогда
		ДобавитьСтрокуСОшибкой(ТекстОшибки, НСтр("ru='Не указан КПП торгового объекта.';uk='Не указан КПП торгового объекта.'"), "");
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыЧекаЕГАИС.АдресМагазина) Тогда
		ДобавитьСтрокуСОшибкой(ТекстОшибки, НСтр("ru='Не указан адрес магазина.';uk='Не указан адрес магазина.'"), "");
	КонецЕсли;
...
"Не указан штрихкод марки алкогольной продукции.'"), СтрокаЧека.Наименование);
		...
		Если МассивКодовМарок.Найти(СокрЛП(СтрокаЧека.ШтрихкодМарки)) <> Неопределено Тогда
			ДобавитьСтрокуСОшибкой(ТекстОшибки, НСтр("ru='В чеке обнаружены дублирующиеся позиции.';uk='В чеке обнаружены дублирующиеся позиции.'"), "");
		КонецЕсли;
...
Процедура ВыбратьТранспортныйМодуль(ОповещениеВыбора, Контекст, Отбор = Неопределено)
	...
	Если СписокДоступныхМодулей.Количество() = 0 Тогда
		ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ru='Отсутствуют доступные транспортные модули.';uk='Отсутствуют доступные транспортные модули.'"));
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		СписокДляВыбора.ПоказатьВыборЭлемента(
			...
			НСтр("ru='Выберите транспортный модуль ЕГАИС';uk='Выберите транспортный модуль ЕГАИС'"));
	КонецЕсли;
