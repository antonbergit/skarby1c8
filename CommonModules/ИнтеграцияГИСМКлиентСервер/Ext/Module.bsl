...
Функция ПредставлениеПодписи(СертификатКриптографии, Сообщение, ВыводитьГиперссылки, СтруктураЦветаСтиля) Экспорт
	...
	СтрокаПодписано = Новый ФорматированнаяСтрока(НСтр("ru='Подписано';uk='Подписано'"));
	...
	Если Сообщение.ПодписьВерна Тогда
		СтрокаПодписьВерна = Новый ФорматированнаяСтрока(СтрШаблон(НСтр("ru='Подпись верна (Дата проверки: %1).';uk='Подпись верна (Дата проверки: %1).'"), Формат(Сообщение.ДатаПроверкиПодписи, НСтр("ru='ДФ=dd.MM.yyyy';uk='ДФ=dd.MM.yyyy'"))));
	Иначе
		Если ЗначениеЗаполнено(Сообщение.ДатаПроверкиПодписи) Тогда
			СтрокаПодписьВерна = Новый ФорматированнаяСтрока(
				СтрШаблон(НСтр("ru='Подпись не верна (Дата проверки: %1).';uk='Подпись не верна (Дата проверки: %1).'"), Формат(Сообщение.ДатаПроверкиПодписи, НСтр("ru='ДФ=dd.MM.yyyy';uk='ДФ=dd.MM.yyyy'"))),,
				СтруктураЦветаСтиля.ЦветТекстаТребуетВниманияГИСМ);
		Иначе
			СтрокаПодписьВерна = Новый ФорматированнаяСтрока(
				НСтр("ru='Проверка подписи не выполнялась.';uk='Проверка подписи не выполнялась.'"),,
				СтруктураЦветаСтиля.ЦветТекстаТребуетВниманияГИСМ);
		КонецЕсли;
	КонецЕсли;
	
	СтрокаПроверить = Новый ФорматированнаяСтрока(НСтр("ru='Проверить';uk='Перевірити'"),,СтруктураЦветаСтиля.ЦветГиперссылкиГИСМ,,"ПроверитьПодпись");
...
Процедура ДобавитьТекстОшибкиНеЗаполненGLNОрганизации(СообщениеXML, GLN, Организация) Экспорт
	...
	Если Не ЗначениеЗаполнено(GLN) Тогда
		ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru='Не заполнен GLN организации %1.';uk='Не заполнен GLN организации %1.'"), Организация));
	Иначе
		ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru='Указан не корректный GLN организации %1.';uk='Указан не корректный GLN организации %1.'"), Организация));
	КонецЕсли;
...
Процедура ДобавитьТекстОшибкиУказанНеКорректныйКППОрганизации(СообщениеXML, КПП, Организация) Экспорт
	...
	ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru='Указан не корректный КПП организации %1.';uk='Указан не корректный КПП организации %1.'"), Организация));
...
Процедура ДобавитьТекстОшибкиНеЗаполненоПолноеНаименованиеКонтрагента(СообщениеXML, Контрагент) Экспорт
	...
	ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru='Не заполнено полное наименование контрагента %1.';uk='Не заполнено полное наименование контрагента %1.'"), Контрагент));
...
Процедура ДобавитьТекстОшибкиНеЗаполненGLNПартнера(СообщениеXML, GLN, Партнер) Экспорт
	...
	Если Не ЗначениеЗаполнено(GLN) Тогда
		ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru='Не заполнен GLN партнера %1.';uk='Не заполнен GLN партнера %1.'"), Партнер));
	Иначе
		ИнтеграцияГИСМКлиентСервер.ДобавитьТекстОшибки(СообщениеXML, СтрШаблон(НСтр("ru='Указан не корректный GLN партнера %1.';uk='Указан не корректный GLN партнера %1.'"), Партнер));
	КонецЕсли;
	...
	Исключение
		...
		#Если НЕ ТонкийКлиент Тогда
			ЗаписьЖурналаРегистрации(
				...
				НСтр("ru='Ошибка отправки POST-запроса к серверу ГИСМ 195.149.110.235';uk='Ошибка отправки POST-запроса к серверу ГИСМ 195.149.110.235'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		#КонецЕсли
	КонецПопытки;
