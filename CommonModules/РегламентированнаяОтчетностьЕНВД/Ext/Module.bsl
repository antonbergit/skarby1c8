...
Функция СформироватьДекларациюПоЕНВД(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	Если СобытиеКалендаря = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Формирование декларации по ЕНВД невозможно без соответствующего события календаря';uk='Формування декларації по ЄП неможливо без відповідної події календаря'");
	КонецЕсли;
