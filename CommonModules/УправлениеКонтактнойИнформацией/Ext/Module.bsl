...
"   • в открывшейся форме ""Настройка формы"" в меню Еще выбрать пункт ""Установить стандартные настройки"".';uk='Також можна встановити стандартні налаштування форми:"
"   • в меню Ще вибрати пункт Змінити форму...;"
"   • у формі, що відкрилася, ""Налаштування форми"" в меню Ще вибрати пункт ""Встановити стандартні налаштування"".'");
...
//          АдресТолькоУкраинский - Булево - Адрес должен быть только Украинским. По умолчанию ИСТИНА.
...
Функция РегионАдресаКонтактнойИнформации(Знач XMLСтрока) Экспорт
	...
	Адрес = XDTOАдрес.Состав;
	Если Адрес = Неопределено Или Адрес.Тип() <> ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
		ВызватьИсключение НСтр("ru='Невозможно определить субъекта Украины, ожидается адрес.';uk='Неможливо визначити суб`єкта України, очікується адреса.'");
	КонецЕсли;
	
	АдресУкраина = УправлениеКонтактнойИнформациейСлужебный.УкраинскийАдрес(Адрес);
	Возврат ?(АдресУкраина = Неопределено, "", СокрЛП(АдресУкраина.СубъектРФ));
...
Функция ГородАдресаКонтактнойИнформации(Знач XMLСтрока) Экспорт
	...
	АдресУкраина = УправлениеКонтактнойИнформациейСлужебный.УкраинскийАдрес(Адрес);
	Возврат ?(АдресУкраина = Неопределено, "", СокрЛП(АдресУкраина.Город));
...
//        ** ТипКвартиры      - Строка - см. ТипыОбъектовАдресацииАдресаРФ().
//        ** Квартира         - Строка - текстовое представление квартиры (только для адресов РФ).
//        ** АдресРФ          - Булево - Если Истина, то адрес украинский.
...
Функция АдресВФорматеКЛАДР(Знач Данные) Экспорт
	...
	Если Результат.Свойство("Страна") 
		И СтрСравнить(Результат.Страна, Справочники.СтраныМира.Украина.Наименование) = 0 Тогда
...
//                * АдресТолькоУкраинский        - Булево - Истина, если возможен ввод только украинских адресов.
//                * ПроверятьКорректность        - Булево - Истина, если требуется запрещать пользователю записывать
//                                                          некорректный адрес (только если АдресТолькоУкраинский = Истина).
//                * ПроверятьПоФИАС              - Булево - Истина, если требуется проверка адреса по ФИАС, иначе
//                                                          проверка по КЛАДР (только если АдресТолькоУкраинский = Истина
//                                                          и ПроверятьКорректность = Истина). Неактуально для Украины.
...
//                                                          вводе (только если АдресТолькоУкраинский = Истина).
...
Функция ПараметрыВидаКонтактнойИнформации(Тип = Неопределено) Экспорт
	...
	Если УстанавливаемыйТип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
		...
		НастройкиПроверки.Вставить("АдресТолькоУкраинский", Ложь);
...
Функция ПолеВвода(Форма, Родитель, ИнформацияОВидеКИ, ИмяРеквизита, ИмяЭлементаДляРазмещения,ЭтоНовыйВидКИ = Ложь, ОбязательноеЗаполнение = Ложь)
	...
	Если ИнформацияОВидеКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
		...
		Кнопка = Форма.Элементы.Добавить(ИмяКоманды,Тип("КнопкаФормы"), Элемент.КонтекстноеМеню);
		Кнопка.Заголовок = НСтр("ru='Адрес на Яндекс.Картах';uk='Адреса на Яндекс.Картах'");
		Команда = Форма.Команды.Добавить(ИмяКоманды);
		Команда.Картинка = БиблиотекаКартинок.ЯндексКарты;
		Команда.Подсказка = НСтр("ru='Показывает адрес на картах Яндекс.Карты';uk='Показує адресу на картах Яндекс.Карти'");
		Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
		Кнопка.ИмяКоманды = ИмяКоманды;
...
Процедура ЗаполнитьРеквизитыТабличнойЧастиДляАдреса(СтрокаТабличнойЧасти, Источник)
	...
	Если ЭтоАдрес И Адрес.Состав <> Неопределено Тогда 
		...
		Если АдресУкраина <> Неопределено Тогда
			// Украинский адрес
			СтрокаТабличнойЧасти.Регион = АдресУкраина.СубъектРФ;
			СтрокаТабличнойЧасти.Город  = АдресУкраина.Город;
		КонецЕсли;
...
Функция ОшибкиЗаполненияАдреса(Источник, ВидИнформации, ИмяРеквизита = "", ПолеРеквизита = "")
	...
	Если НЕ УправлениеКонтактнойИнформациейСлужебный.ЭтоУкраинскийАдрес(Источник) Тогда
		Возврат 0;
	КонецЕсли;
	ПространствоИмен = УправлениеКонтактнойИнформациейКлиентСервер.ПространствоИмен();
	...
	Если СоздаватьДействиеДляТипа[Тип] = Истина Тогда
		...
		ИначеЕсли Тип = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда
			Если Форма.ПараметрыКонтактнойИнформации[ИмяЭлементаДляРазмещения].МожноОтправлятьSMS Тогда
					Элемент.Заголовок = НСтр("ru='Позвонить или отправить SMS';uk='Подзвонити або відправити SMS'");
					Команда.Подсказка = НСтр("ru='Позвонить или отправить SMS';uk='Подзвонити або відправити SMS'");
					Команда.Картинка = БиблиотекаКартинок.ПозвонитьИлиОтправитьSMS;
			Иначе
				Элемент.Заголовок = НСтр("ru='Позвонить';uk='Зателефонувати'");
				Команда.Подсказка = НСтр("ru='Позвонить по телефону';uk='Зателефонувати'");
