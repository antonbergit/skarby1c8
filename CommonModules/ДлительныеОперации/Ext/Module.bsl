...
"не могут одновременно принимать значение Истина.';uk='Параметри тривалої операції ""%1"" і ""%2"""
"не можуть одночасно набувати значення Істина.'");
...
Функция ОперацияВыполнена(Знач ИдентификаторЗадания, Знач ИсключениеПриОшибке = Ложь, Знач ВыводитьПрогрессВыполнения = Ложь, 
	...
	Если Задание = Неопределено Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru='Длительные операции';uk='Тривалі операції'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , НСтр("ru='Фоновое задание не найдено:';uk='Фонове завдання не знайдене:'") + " " + Строка(ИдентификаторЗадания));
		Если ИсключениеПриОшибке Тогда
			ВызватьИсключение(НСтр("ru='Не удалось выполнить данную операцию.';uk='Не вдалося виконати цю операцію.'"));
		КонецЕсли;
		Результат.Статус = "Ошибка";
		Возврат Результат;
	КонецЕсли;
	...
	Если Задание.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно 
		...
		Если ИсключениеПриОшибке Тогда
			...
			Иначе
				ТекстСообщения = НСтр("ru='Не удалось выполнить данную операцию.';uk='Не вдалося виконати цю операцію.'");
			КонецЕсли;
