	...
	Если НЕ ЭтоДекларация
		...
		ИначеЕсли Форма.РеквизитыПроверкиКонтрагентов.Свойство("ЭтоДекларацияПоНДС") Тогда
			...
			Если ДополнительныеПараметры.Свойство("ЭтоЗапускПроверкиКомандой") Тогда
				ПоказатьПредупреждение(, НСтр("ru='Для проверки контрагентов необходимо заполнить отчет.';uk='Для проверки контрагентов необходимо заполнить отчет.'"));
			КонецЕсли;
...
Процедура ПоказатьРезультатПроверкиСразу(Форма)
	...
	ИначеЕсли НЕ Форма.РеквизитыПроверкиКонтрагентов.ЕстьДоступКВебСервисуФНС Тогда
		ПоказатьПредупреждение(, НСтр("ru='Проверка контрагентов сервисом ФНС не выполнена из-за ошибки подключения к сервису';uk='Проверка контрагентов сервисом ФНС не выполнена из-за ошибки подключения к сервису'"));
	ИначеЕсли Форма.РеквизитыПроверкиКонтрагентов.КоличествоНекорректныхКонтрагентов = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Нет ошибок проверки контрагентов сервисом ФНС';uk='Нет ошибок проверки контрагентов сервисом ФНС'"));
	КонецЕсли;
