...
Процедура ДобавитьКомандыСервиса(
	...
	Исключение
		...
		Если ТипЗнч(КонтекстОбработчика) = Тип("Структура") Тогда
			...
			КонтекстОбработчика.ДействиеПриОшибкеДляКлиента = "ОтобразитьФормуОтсутствуетПодключение";
			КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
				НСтр("ru='Ошибка при получении данных с сервера Интернет-поддержки.';uk='Помилка при одержанні даних із сервера Інтернет-підтримки.'");
		КонецЕсли;
...
Функция ДопПараметрыОтправкиСообщения(Знач ДопПараметры) Экспорт
	...
	Вложения.Добавить(
		Новый Структура(
			"Представление, Текст",
			НСтр("ru='Техническая информация.txt';uk='Технічна інформація.txt'"),
...
"%msgtxt"
""
"Реєстраційний номер програмного продукту: <Вкажіть реєст. номер>;"
...
Функция ТекстВложенияТехническаяИнформация(ВидПриложения, ДопПараметры)
	...
	Если ИмяПрограммы = "Unknown" Тогда
		ИмяПрограммы = НСтр("ru='<Не заполнено>';uk='<Не заповнене>'");
	КонецЕсли;
...
"Вид приложения: %8;"
"Режим: %9;';uk='Технічна інформація про програму:"
"Назва програми: %1"
"Ім`я програми: %2;"
"Версія програми: %3;"
"Постачальник: %4;"
"Версія Платформи 1С:Підприємство: %5;"
...
"Права для подключения Интернет-поддержки: %3;';uk='Повні права: %1;"
...
"Регистрационный номер программного продукта: %3;';uk='Логін для підключення Інтернет-підтримки : %1;"
"Пароль для підключення Інтернет-підтримки : %2;"
"Реєстраційний номер програмного продукту : %3;'"),
			ДопПараметры.Логин,
			?(ДопПараметры.ПарольЗаполнен, НСтр("ru='заполнен';uk='заповнений'"), НСтр("ru='не заполнен';uk='не заповнений'")),
...
"	Доменная зона: %3';uk='Настройки соединения с серверами Интернет-поддержки:"
"	Таймаут: %1;"
"	Устанавливать соединение на сервере 1С:Предприятие: %2;"
"	Доменная зона: %3'"),
