...
Функция ПолучитьКБКУплаты(Организация, Дата = Неопределено, ВидВзаиморасчетов = Неопределено) Экспорт
	...
	Если ВидВзаиморасчетов = Неопределено 
		...
		Если ИсчисленМинимальныйНалог(Организация, ДатаСреза) Тогда
			
			Если Число(Год) >= 2011 Тогда 
				Возврат "18210501050011000110";
			Иначе
				Возврат "18210501030011000110";
			КонецЕсли;
...
Функция ПолучитьТекстНазначенияПлатежаНалогаПоСтраховымВзносам(ДатаДокумента, ВидВзаиморасчетов) Экспорт
	...
	Если ВидВзаиморасчетов = Справочники.ВидыНалогов.ПФРНакопительная Тогда
		Если ДатаДокумента > '20130101' Тогда
			Возврат Нстр("ru='Страховые взносы в фиксированном размере на выплату накопительной части трудовой пенсии.';uk='Страхові внески у фіксованому розмірі на виплату накопичувальної частини трудової пенсії.'");
		Иначе
			Возврат Нстр("ru='Страховые взносы исходя из стоимости страхового года на выплату накопительной части трудовой пенсии.';uk='Страхові внески виходячи з вартості страхового року на виплату накопичувальної частини трудової пенсії.'");
		КонецЕсли;
	ИначеЕсли ВидВзаиморасчетов = Справочники.ВидыНалогов.ПФРСтраховая
		...
		Если ДатаДокумента > '20130101' Тогда
			Возврат Нстр("ru='Страховые взносы в фиксированном размере на выплату страховой части трудовой пенсии.';uk='Страхові внески у фіксованому розмірі на виплату страхової частини трудової пенсії.'");
		Иначе
			Возврат Нстр("ru='Страховые взносы исходя из стоимости страхового года на выплату страховой части трудовой пенсии.';uk='Страхові внески виходячи з вартості страхового року на виплату страхової частини трудової пенсії.'");
		КонецЕсли;
	ИначеЕсли ВидВзаиморасчетов = Справочники.ВидыНалогов.ТФОМС Тогда
		Возврат Нстр("ru='Страховые взносы на ОМС, зачисляемые в бюджет ТФОМС.';uk='Страхові внески на ОМС, зачислювані в бюджет ТФОМС.'");
	ИначеЕсли ВидВзаиморасчетов = Справочники.ВидыНалогов.ФФОМС Тогда
		Возврат Нстр("ru='Страховые взносы на ОМС, зачисляемые в бюджет ФФОМС.';uk='Страхові внески на ОМС, зачислювані в бюджет ФФОМС.'");
	КонецЕсли;
...
Функция СоздатьБезналичноеСписаниеПоВзаиморасчетамСБюджетом(ДокументВзаиморасчетовСБюджетом, БанкСчет, Патент = Неопределено) Экспорт
	...
	Если Патент <> Неопределено Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			ДокументОбъект.НазначениеПлатежа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Уплата налога, взимаемого в связи с применением патентной системы налогообложения, зачисляемого в бюджеты %1, № %2,  НДС не облагается';uk='Сплата податку, що стягується у зв`язку із застосуванням патентної системи оподаткування, що зараховується до бюджетів %1, № %2,  ПДВ не обкладається'"),
	...
	Если ДокументФормирования <> Неопределено Тогда
		...
		Пока ВыборкаЗаписейУСН.Следующий() Цикл
			...
			Если ТипЗнч(ВыборкаЗаписейУСН.ПервичныйДокумент) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах") Тогда
				НоваяСтрокаУСН.Содержание = НСтр("ru='Поступление розничной выручки';uk='Надходження роздрібного виторгу'");
			Иначе
				Если ВыборкаЗаписейУСН.ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ОтПокупателя Тогда
					НоваяСтрокаУСН.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Поступление наличных денежных средств от покупателя ""%1""';uk='Надходження наявних коштів від покупця ""%1""'"), СокрЛП(ВыборкаЗаписейУСН.Контрагент));
				ИначеЕсли ВыборкаЗаписейУСН.ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.Прочее 
					...
					НоваяСтрокаУСН.Содержание = НСтр("ru='Поступление наличных денежных средств по прочим операциям';uk='Надходження наявних коштів по іншим операціям'");
				ИначеЕсли ВыборкаЗаписейУСН.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПокупателя Тогда
					НоваяСтрокаУСН.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Поступление безналичных денежных средств от покупателя ""%1""';uk='Надходження безготівкових коштів від покупця ""%1""'"), СокрЛП(ВыборкаЗаписейУСН.Контрагент));
				ИначеЕсли ВыборкаЗаписейУСН.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.Прочее
					...
					НоваяСтрокаУСН.Содержание = НСтр("ru='Поступление безналичных денежных средств по прочим операциям';uk='Надходження безготівкових коштів по іншим операціям'");
				ИначеЕсли ВыборкаЗаписейУСН.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам Тогда
					НоваяСтрокаУСН.Содержание = НСтр("ru='Поступление от розничных продаж, оплаченной безналичным способом';uk='Поступление от розничных продаж, оплаченной безналичным способом'");
				ИначеЕсли ВыборкаЗаписейУСН.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Покупателю Тогда
					НоваяСтрокаУСН.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Возврат наличных денежных средств покупателю ""%1""';uk='Повернення наявних коштів покупцеві ""%1""'"), СокрЛП(ВыборкаЗаписейУСН.Контрагент));
				ИначеЕсли ВыборкаЗаписейУСН.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Покупателю Тогда
					НоваяСтрокаУСН.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Возврат безналичных денежных средств покупателю ""%1""';uk='Повернення безготівкових коштів покупцеві ""%1""'"), СокрЛП(ВыборкаЗаписейУСН.Контрагент));
				ИначеЕсли ВыборкаЗаписейУСН.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ВозвратОплатыНаПлатежныеКарты Тогда
					НоваяСтрокаУСН.Содержание = НСтр("ru='Возврат розничному покупателю безналичных денежных средств';uk='Возврат розничному покупателю безналичных денежных средств'");
				КонецЕсли;
			...
			Если ВыборкаЗаписейУСН.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам И 
				...
				НоваяСтрокаУСНКомиисия.Содержание = НСтр("ru='Комиссия банка за операции эквайринга';uk='Комиссия банка за операции эквайринга'");
			КонецЕсли;
			
		КонецЦикла;
		
		Результат.ДополнительнаяИнформация = НСтр("ru='Сформировано записей: ';uk='Сформоване записів:'") + Строка(ДокументФормирования.ЗаписиКУДиР.Количество());
...
Процедура ВыполнитьФормированиеПоРасходамНаОплатуТруда(Организация, ПериодФормирования, Результат, МенеджерВТ)
	...
	Запрос.УстановитьПараметр("ОкончаниеПериода", КонецДня(ПериодФормирования));
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("СодержаниеСтроки", НСтр("ru='Расходы на оплату труда';uk='Витрати на оплату праці'"));
...
Процедура ВыполнитьФормированиеПоПрочимРасходам(Организация, ПериодФормирования, Результат, МенеджерВТ)
	...
	Для Каждого Строка Из РезультатЗапроса Цикл
		...
		Если Строка.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Налоги Тогда
			Строка.Содержание = НСтр("ru='Расход наличных денежных средств на уплату налогов';uk='Витрата наявних грошових коштів на сплату податків'");
		ИначеЕсли Строка.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Прочее 
			ИЛИ Строка.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.ПрочиеРасчеты Тогда
			Строка.Содержание =  НСтр("ru='Расход наличных денежных средств по прочим операциям';uk='Витрата наявних грошових коштів по іншим операціям'");
		ИначеЕсли Строка.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Налоги Тогда
			Строка.Содержание = НСтр("ru='Расход безналичных денежных средств на уплату налогов';uk='Витрата безготівкових грошових коштів на сплату податків'");
		ИначеЕсли Строка.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Прочее
			ИЛИ Строка.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.ПрочиеРасчеты Тогда
			Строка.Содержание = НСтр("ru='Расход безналичных денежных средств по прочим операциям';uk='Витрата безготівкових коштів по іншим операціям'");
		ИначеЕсли Строка.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.Налоги Тогда
			Строка.Содержание = НСтр("ru='Возврат излишне уплаченных налогов';uk='Возврат излишне уплаченных налогов'");
		КонецЕсли;
	КонецЦикла;
...
Функция ВыполнитьРасчетЕдиногоНалога(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено, ФиксироватьСтатусСобытия=Истина) Экспорт
	...
	Если ТаблицаПоНалогу.Количество() = 0 Тогда
		...
		СтруктураРасчетаЕдиногоНалога.ДополнительнаяИнформация.Описание = НСтр("ru='Нет данных для расчета налога';uk='Немає даних для розрахунків податку'");
	...
	Иначе
		//СтруктураРасчетаЕдиногоНалога.СуммаРасходовЗаГодЕН = СтруктураРасчетаЕдиногоНалога.СуммаРасходовЗаГодЕН + СтруктураРасчетаЕдиногоНалога.СуммаСтраховыхВзносовЕН;
	...
	Если СуммаМинимальногоНалога > СуммаЕдиногоНалога Тогда
		...
		СтруктураРасчетаЕдиногоНалога.ДополнительнаяИнформация.Описание = НСтр("ru='Рассчитан минимальный налог по ставке 1%';uk='Розрахований мінімальний податок по ставці 1%'");
		
	Иначе
		
		СтруктураРасчетаЕдиногоНалога.СуммаЕН = СуммаЕдиногоНалога;
		СтруктураРасчетаЕдиногоНалога.ДополнительнаяИнформация.Описание = НСтр("ru='Рассчитан налог за год';uk='Розрахований податок за рік'");
...
Функция ВыполнитьРасчетАвансовогоПлатежа(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	Если ТаблицаПоНалогу.Количество() = 0 Тогда
		...
		СтруктураРасчетаАвансовогоПлатежа.ДополнительнаяИнформация.Описание = НСтр("ru='Нет данных для расчета налога';uk='Немає даних для розрахунків податку'");
	...
	СтруктураРасчетаАвансовогоПлатежа.ДополнительнаяИнформация.Описание = НСтр("ru='Рассчитан авансовый платеж';uk='Розрахований авансовий платіж'");
...
Функция СформироватьДекларациюПоУСН(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	Если СобытиеКалендаря = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Формирование декларации по УСН невозможно без соответствующего события календаря';uk='Формування декларації по ССО неможливо без відповідної події календаря'");
	КонецЕсли;
...
Функция ВыполнитьРасчетВзносовВПФРПриДоходахСвыше300тр(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	ВременнаяТаблица.Колонки.Добавить("КОАТУУ");
	...
	Если СистемаНалогообложения.ПлательщикЕНВД Тогда
		Запрос.Текст =
			"ВЫБРАТЬ
			|	ПоказателиЕНВДПоказателиЕНВД.ВидДеятельностиЕНВД.КодПоОКАТО КАК КодКОАТУУ,
		...
		Пока Выборка.Следующий() Цикл
			...
			Строка.КОАТУУ = Выборка.КодКОАТУУ;
...
Функция СформироватьСведенияОСреднесписочнойЧисленности(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	Если СобытиеКалендаря = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Формирование сведений о среднесписочной численности невозможно без соответствующего события календаря';uk='Формування відомостей про середньооблікову чисельність неможливо без відповідної події календаря'");
	КонецЕсли;
...
Функция СформироватьФорму1Предприниматель(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	Если СобытиеКалендаря = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Формирование формы №1-предприниматель невозможно без соответствующего события календаря';uk='Формирование формы №1-предприниматель невозможно без соответствующего события календаря'");
	КонецЕсли;
...
Функция СформироватьФорму6НДФЛ(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	Если СобытиеКалендаря = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Формирование формы 6-НДФЛ невозможно без соответствующего события календаря';uk='Формирование формы 6-НДФЛ невозможно без соответствующего события календаря'");
	КонецЕсли;
...
Функция СформироватьДекларацию12(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	Если СобытиеКалендаря = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Формирование декларация об объеме розничной продажи пива и пивных напитков (Приложение №12) невозможно без соответствующего события календаря';uk='Формирование декларация об объеме розничной продажи пива и пивных напитков (Приложение №12) невозможно без соответствующего события календаря'");
	КонецЕсли;
...
Функция СформироватьСЗВМ(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	Если СобытиеКалендаря = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Формирование сведений о застрахованных лицах (СЗВ-М) невозможно без соответствующего события календаря';uk='Формирование сведений о застрахованных лицах (СЗВ-М) невозможно без соответствующего события календаря'");
	КонецЕсли;
...
Процедура ЗарегистрироватьВзаиморасчетыСБюджетомТорговыйСбор(ТаблицаПоТорговомуСбору, СтруктураРасчетаТорговогоСбора)
	...
	|	ВзаиморасчетыСБюджетом.КодПоОКАТО КАК КодКОАТУУ,
	...
	Для Каждого Строка Из ТаблицаПоТорговомуСбору Цикл
		...
		Если Найденные.Количество() > 1 Тогда
			ЗаписьЖурналаРегистрации(
				НСтр("ru='ЕНВД.Внутренняя логика';uk='ЄП.Внутрішня логіка'"),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				НСтр("ru='Нарушение логики работы алгоритма поиска документов взаиморасчетов по ЕНВД';uk='Порушення логіки роботи алгоритму пошуку документів взаєморозрахунків по ЄП'"));
		КонецЕсли;
	...
	Для Каждого Строка Из Найденные Цикл
		...
		НовСтр.КодКОАТУУ = Строка.КодКОАТУУ;
...
Процедура СформироватьСправкуНДФЛВНалоговыйОрган(Организация, СобытиеКалендаря)
	...
	Если Выборка.Следующий() Тогда
		...
		докСправка.КОАТУУ_ЕДРПОУ = РегламентированнаяОтчетностьКлиентСервер.ДополнитьСтроку(Выборка.КодКОАТУУ, 11) + "/" + РегламентированнаяОтчетностьКлиентСервер.ДополнитьСтроку(Выборка.КодПоЕДРПОУ, 9);
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("КОАТУУ_ЕДРПОУ", докСправка.КОАТУУ_ЕДРПОУ);
	...
	|			И (СправкаПоНДФЛВНалоговыйОрганСотрудникиОрганизации.Ссылка.КОАТУУ_ЕДРПОУ = &КОАТУУ_ЕДРПОУ)
	...
	КонецЦикла;
	СтруктураДанных = СправкиПоНДФЛ.ДанныеОДоходах(СотрудникиОрганизации.Выгрузить(), докСправка.НалоговыйПериод, Организация, Организация, докСправка.КОАТУУ_ЕДРПОУ, докСправка.Дата);
...
Процедура СформироватьКомплектОтчетностиВПФР(Организация, ПериодРасчета, СобытиеКалендаря) Экспорт
	
	
	//////////////////////////////////////////////////////////////////////////
	// Создаем комплект отчетности для отправки
	ОтчетныйПериод = НачалоКвартала(ПериодРасчета);
	...
	Если Комплект = Неопределено Тогда
		Справочники.КомплектыОтчетностиПерсучета.НовыйКомплектСведений(Организация, ОтчетныйПериод, РСВСсылка, Истина);
	Иначе
		КомплектОбъект = Комплект.ПолучитьОбъект();
		Справочники.КомплектыОтчетностиПерсучета.ЗаполнитьКомплектСведений(КомплектОбъект, РСВСсылка, Ложь);
	КонецЕсли;
...
Функция ПолучитьДокументРегламентированнойОтчетностиПоСобытиюКалендаря(Организация,СобытиеКалендаря) Экспорт
	...
	Если ИмяОтчета = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Не найдено соответствие задачи календаря регламентированному отчету';uk='Не знайдена відповідність завдання календаря регламентованому звіту'");
	КонецЕсли;
...
Функция СоздатьБезналичноеСписание(ДанныеДокумента, БанкСчет) Экспорт
	...
	|			ТОГДА ВЫБОР
	|					КОГДА Организации.БанковскийСчетПоУмолчанию.ВариантУказанияКодПоЕДРПОУ = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияКодПоЕДРПОУ.ВоВсехПлатежныхПоручениях)
	|						ТОГДА Организации.КодПоЕДРПОУ
	|					ИНАЧЕ """"
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ВложенныйЗапрос.БанковскийСчет.ВариантУказанияКодПоЕДРПОУ = ЗНАЧЕНИЕ(Перечисление.ВариантыУказанияКодПоЕДРПОУ.ВоВсехПлатежныхПоручениях)
	|					ТОГДА Организации.КодПоЕДРПОУ
	|				ИНАЧЕ """"
	|			КОНЕЦ
	...
	|		Организации.КодКОАТУУ КАК КодКОАТУУ,
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Если Выборка.ВидНалога = Справочники.ВидыНалогов.ЕНВД Тогда
			ДокументОбъект.КодКОАТУУ = Выборка.КодОКТМОДокумента;
		Иначе
			Если Год(ДокументОбъект.Дата) > 2013 Тогда
				ДокументОбъект.КодКОАТУУ = Выборка.КодПоОКТМО;
			Иначе
				ДокументОбъект.КодКОАТУУ = Выборка.КодКОАТУУ;
			КонецЕсли;
		КонецЕсли;
		...
		ИначеЕсли Выборка.ВидНалога = Справочники.ВидыНалогов.ФСССотрудники
			ИЛИ Выборка.ВидНалога = Справочники.ВидыНалогов.ФССТравматизмСотрудники Тогда
			ДокументОбъект.ПоказательТипа = "0";
		...
		ИначеЕсли Выборка.ВидНалога = Справочники.ВидыНалогов.ПФРНакопительная
			...
			ИЛИ Выборка.ВидНалога = Справочники.ВидыНалогов.ФОМССотрудники Тогда
	...
	Если ДокументФормирования <> Неопределено Тогда
		...
		Пока ВыборкаЗаписейУСН.Следующий() Цикл
			...
			Если ТипЗнч(ВыборкаЗаписейУСН.ПервичныйДокумент) = Тип("ДокументСсылка.АвансовыйОтчет") Тогда
				НоваяСтрокаУСН.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Расходы на оплату товаров, услуг из подотчетных денежных средств';uk='Витрати на оплату товарів, послуг з підзвітних коштів'"), СокрЛП(ВыборкаЗаписейУСН.Контрагент));
			Иначе
				Если ВыборкаЗаписейУСН.ВидОперации = Перечисления.ВидыОперацийРасходИзКассы.Поставщику Тогда
					НоваяСтрокаУСН.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Расход наличных денежных средств на оплату поставщику ""%1""';uk='Витрата наявних грошових коштів на оплату постачальникові ""%1""'"), СокрЛП(ВыборкаЗаписейУСН.Контрагент));
				ИначеЕсли ВыборкаЗаписейУСН.ВидОперации = Перечисления.ВидыОперацийРасходСоСчета.Поставщику Тогда
					НоваяСтрокаУСН.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Расход безналичных денежных средств на оплату поставщику ""%1""';uk='Витрата безготівкових грошових коштів на оплату постачальникові ""%1""'"), СокрЛП(ВыборкаЗаписейУСН.Контрагент));
				ИначеЕсли ВыборкаЗаписейУСН.ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ОтПоставщика Тогда
					НоваяСтрокаУСН.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Возврат наличных денежных средств от поставщика ""%1""';uk='Повернення наявних коштів від постачальника ""%1""'"), СокрЛП(ВыборкаЗаписейУСН.Контрагент));
				ИначеЕсли ВыборкаЗаписейУСН.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПоставщика Тогда
					НоваяСтрокаУСН.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Возврат безналичных денежных средств от поставщика ""%1""';uk='Повернення безготівкових коштів від постачальника ""%1""'"), СокрЛП(ВыборкаЗаписейУСН.Контрагент));
				КонецЕсли;
			КонецЕсли;
			...
			Если ВыборкаЗаписейУСН.СуммаНДС > 0 Тогда
				...
				НоваяСтрокаУСННДС.Содержание = НоваяСтрокаУСННДС.Содержание+ НСтр("ru=' (НДС)';uk=' (НДС)'");
			КонецЕсли;
...
Функция ПолучитьДанныеСобытия(Событие, РеквизитыСобытия) Экспорт
	...
	Если ТипЗнч(Событие) <> Тип("СправочникСсылка.КалендарьПодготовкиОтчетности")
		...
		ВызватьИсключение (НСтр("ru='Неверные параметры метода ПолучитьДанныеСобытия';uk='Невірні параметри методу ПолучитьДанныеСобытия'"));
	КонецЕсли;
...
Функция ПолучитьДанныеДокументаОтчетности(ДокументОтчетности, РеквизитыДокумента) Экспорт
	...
	Если ТипЗнч(ДокументОтчетности) <> Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
		ВызватьИсключение (НСтр("ru='Неверные параметры метода ПолучитьДанныеДокументаОтчетности';uk='Невірні параметри методу ПолучитьДанныеДокументаОтчетности'"));
	КонецЕсли;
...
Процедура ПроверитьРегистрациюВИФНС(Организация, Отказ, РегистрацияВНалоговомОргане = Неопределено) Экспорт
	...
	Иначе
		...
		Если Результат.Пустой() Тогда
			...
			РегистрацияВНалоговомОргане.Наименование = НСтр("ru='Автозапись';uk='Автозапис'");
	...
	РегистрацияВНалоговомОргане.КодКОАТУУ   = Организация.КодКОАТУУ;
	РегистрацияВНалоговомОргане.КодПоОКТМО  = Организация.КодПоОКТМО;
	РегистрацияВНалоговомОргане.КодПоЕДРПОУ = Организация.КодПоЕДРПОУ;
...
Функция ВыполнитьРасчетНалогаПоПатенту(Организация, ПериодРасчета, СобытиеКалендаря = Неопределено) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		КонецЕсли;
		СтруктураРасчетаПатент.КОАТУУВзаиморасчетов = Выборка.КодПоОКТМО;
...
Процедура ЗарегистрироватьВзаиморасчетыСБюджетомПоПатенту(СтруктураРасчета) Экспорт
	...
	СтрокаСНалогом.КодКОАТУУ  = СтруктураРасчета.КОАТУУВзаиморасчетов;
...
Процедура ВыполнитьФормированияЗаписейПатента(Патент) Экспорт
	...
	Для Каждого Строка Из РезультатЗапроса Цикл
		...
		Если ТипЗнч(Строка.ПервичныйДокумент) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах") Тогда
			Строка.Содержание = НСтр("ru='Поступление розничной выручки';uk='Надходження роздрібного виторгу'");
		Иначе
			Если Строка.ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ОтПокупателя Тогда
				Строка.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Поступление наличных денежных средств от покупателя ""%1""';uk='Надходження наявних коштів від покупця ""%1""'"), СокрЛП(Строка.Контрагент));
			ИначеЕсли Строка.ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.Прочее 
				...
				Строка.Содержание = НСтр("ru='Поступление наличных денежных средств по прочим операциям';uk='Надходження наявних коштів по іншим операціям'");
			ИначеЕсли Строка.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ОтПокупателя Тогда
				Строка.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Поступление безналичных денежных средств от покупателя ""%1""';uk='Надходження безготівкових коштів від покупця ""%1""'"), СокрЛП(Строка.Контрагент));
			ИначеЕсли Строка.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.Прочее
				...
				Строка.Содержание = НСтр("ru='Поступление безналичных денежных средств по прочим операциям';uk='Надходження безготівкових коштів по іншим операціям'");
			ИначеЕсли Строка.ВидОперации = Перечисления.ВидыОперацийПоступлениеНаСчет.ПоступлениеОплатыПоКартам Тогда
				Строка.Содержание = НСтр("ru='Поступление от розничных продаж, оплаченной безналичным способом';uk='Поступление от розничных продаж, оплаченной безналичным способом'");
			КонецЕсли;
...
Процедура РасчетПоказателей_6НДФЛ_2016Кв1(НачалоПериода, ОкончаниеПериода, Организация, РегистрацияВНалоговомОргане, ОписаниеТаблиц, ПодробныеДанные = Ложь)
	...
	Если ОписаниеТаблиц.Свойство("Раздел1Налоги") Или ОписаниеТаблиц.Свойство("Раздел2") Тогда
		...
		"ВЫБРАТЬ
		|	13 КАК П000010001001,
		|	НДФЛРасчетыНалоговыхАгентовСБюджетом.Сумма КАК П000010004001,
		|	0 КАК П000010008001,
		|	0 КАК П000010004501,
		|	0 КАК П000010005001,
		|	НДФЛРасчетыНалоговыхАгентовСБюджетом.Сумма КАК П000010007001,
		|	0 КАК П000010009001
		|ИЗ
