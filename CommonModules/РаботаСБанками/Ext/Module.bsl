...
// Получает данные из справочника КлассификаторБанковРФ по значениям МФО и корреспондентского счета банка.
// 
// Параметры:
//  МФО          - Строка - банковский идентификационный код.
//  КоррСчет     - Строка - корреспондентский счет банка.
//  ЗаписьОБанке - СправочникСсылка, Строка - (возвращаемый) найденный банк.
Процедура ПолучитьДанныеКлассификатораРФ(МФО = "", КоррСчет = "", ЗаписьОБанке = "") Экспорт
	Если Не ПустаяСтрока(МФО) Тогда
		ЗаписьОБанке = Справочники.КлассификаторБанковРФ.НайтиПоКоду(МФО);
...
Функция ПояснениеНедействительногоБанка(Банк) Экспорт
	...
	|	КлассификаторБанковРФ.Код КАК МФО
	|ИЗ
	...
	Если Выборка.Следующий() Тогда
		НовыеРеквизитыБанка = Новый Структура("Ссылка, МФО", Выборка.Ссылка, Выборка.МФО);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Банк) И ЗначениеЗаполнено(НовыеРеквизитыБанка) Тогда
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='МФО банка изменился на <a href = ""%1"">%2</a>';uk='МФО банку змінився на <a href = ""%1"">%2</a>'"),
			ПолучитьНавигационнуюСсылку(НовыеРеквизитыБанка.Ссылка), НовыеРеквизитыБанка.МФО);
	Иначе
		Результат = НСтр("ru='Деятельность банка прекращена';uk='Діяльність банку припинена'");
	КонецЕсли;
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ru='Классификатор банков устарел';uk='Класифікатор банків застарів'");
		Дело.Подсказка      = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Последнее обновление %1 назад';uk='Останнє оновлення %1 назад'"), Результат.ВеличинаПросрочкиСтрокой);
...
Функция Разрешения()
	...
	Порт = Неопределено;
	Описание = НСтр("ru='Загрузка классификатора банков из интернета.';uk='Завантаження класифікатора банків з інтернету.'");
...
Процедура ДополнитьТекстСообщения(ПараметрыЗагрузкиКлассификатора)
	...
	Если ПустаяСтрока(ТекстСообщения) Тогда
		Если Загружено = 0 И Обновлено = 0 Тогда
			ТекстСообщения = НСтр("ru='Загрузка завершена. Изменений в классификаторе нет.';uk='Загрузка завершена. Змін в класифікаторі немає.'");
		Иначе
			ТекстСообщения = НСтр("ru='Загрузка классификатора банков РФ выполнена успешно.';uk='Завантаження класифікатора банків РФ виконано успішно.'");
		КонецЕсли;
	КонецЕсли;
	
	Если Загружено > 0 Тогда
		ТекстСообщения = ТекстСообщения + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Загружено новых: %1.';uk='Завантажено нових: %1.'"), Загружено);
	КонецЕсли;
	
	Если Обновлено > 0 Тогда
		ТекстСообщения = ТекстСообщения + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Обновлено записей: %1.';uk='Оновлено записів: %1.'"), Обновлено);
	КонецЕсли;
...
// Получает, сортирует, записывает данные классификатора МФО РФ с сайта РБК.
// 
// Параметры:
//	ПараметрыЗагрузкиКлассификатора - Соответствие:
//	Загружено						- Число	 - Количество новых записей классификатора.
...
Процедура ПолучитьДанныеРБК(ПараметрыЗагрузкиКлассификатора, АдресХранилища = "") Экспорт
	...
	Если ПараметрыПолученияФайловРБК["ЕстьОбновлениеНаСервере"] Тогда
		...
		Исключение
			ТекстСообщения = НСтр("ru='Возникли проблемы с файлом классификатора банков, полученным с сайта РБК.';uk='Виникли проблеми з файлом класифікатора банків, отриманим з сайту РБК.'");
		...
		ПутьКФайлуМФОРБК = ИмяФайлаМФОСВИФТРБК(ВременныйКаталог);
		ФайлМФОРБК = Новый Файл(ИмяФайлаМФОСВИФТРБК(ВременныйКаталог));
		Если Не ФайлМФОРБК.Существует() Тогда
			ПутьКФайлуМФОРБК = ИмяФайлаМФОРБК(ВременныйКаталог);
		КонецЕсли;
		
		ФайлМФОРБК = Новый Файл(ПутьКФайлуМФОРБК);
		Если Не ФайлМФОРБК.Существует() Тогда
			ТекстСообщения = НСтр("ru='Возникли проблемы с файлом классификатора банков, полученным с сайта РБК. "
"Архив не содержит информацию - классификатор банков.';uk='Виникли проблеми з файлом класифікатора банків, отриманим з сайту РБК."
"Архів не містить інформацію - класифікатор банків. '");
			ПараметрыЗагрузкиКлассификатора.Вставить("ТекстСообщения", ТекстСообщения);
			Если НЕ ПустаяСтрока(АдресХранилища) Тогда
				ПоместитьВоВременноеХранилище(ПараметрыЗагрузкиКлассификатора, АдресХранилища);
			КонецЕсли;
		...
		Если Не ФайлРегионовРБК.Существует() Тогда
			ТекстСообщения = НСтр("ru='Возникли проблемы с файлом классификатора банков, полученным с сайта РБК. "
"Архив не содержит информацию о регионах.';uk='Виникли проблеми з файлом класифікатора банків, отриманим з сайту РБК."
"Архів не містить інформацію про регіони. '");
			ПараметрыЗагрузкиКлассификатора.Вставить("ТекстСообщения", ТекстСообщения);
			Если НЕ ПустаяСтрока(АдресХранилища) Тогда
				ПоместитьВоВременноеХранилище(ПараметрыЗагрузкиКлассификатора, АдресХранилища);
			КонецЕсли;
		...
		Если Не ФайлНедействующихБанков.Существует() Тогда
			ТекстСообщения = НСтр("ru='Возникли проблемы с файлом классификатора банков, полученным с сайта РБК. "
"Архив не содержит информацию о недействующих банках.';uk='Виникли проблеми з файлом класифікатора банків, отриманим з сайту РБК."
"Архів не містить інформацію про недіючі банки. '");
			ПараметрыЗагрузкиКлассификатора.Вставить("ТекстСообщения", ТекстСообщения);
			Если НЕ ПустаяСтрока(АдресХранилища) Тогда
				ПоместитьВоВременноеХранилище(ПараметрыЗагрузкиКлассификатора, АдресХранилища);
			КонецЕсли;
		...
		ПараметрыЗагрузкиФайловРБК.Вставить("ПутьКФайлуМФОРБК", ПутьКФайлуМФОРБК);
...
Процедура ЗагрузитьКлассификаторБанков() Экспорт
	...
	Если ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда
		ЗаписьЖурналаРегистрации(ИмяСобытияВЖурналеРегистрации(), УровеньСобытия, , , НСтр("ru='Загрузка в подчиненном узле РИБ не предусмотрена';uk='Завантаження в підпорядкованому вузлі РІБ не передбачене'"));
		Возврат;
	КонецЕсли;
...
// Сортирует, записывает данные классификатора МФО РФ с сайта РБК.
// 
// Параметры:
//	ПараметрыЗагрузкиФайловРБК - Соответствие:
//  ПутьКФайлуМФОРБК		   - Строка - путь к файлу с данными классификатора, размещенному во временном каталоге.
...
Процедура ЗагрузитьДанныеРБК(ПараметрыЗагрузкиФайловРБК)
	...
	ТекстМФОРБК		   = Новый ЧтениеТекста(ПараметрыЗагрузкиФайловРБК["ПутьКФайлуМФОРБК"], "windows-1251");
	СтрокаТекстаМФОРБК = ТекстМФОРБК.ПрочитатьСтроку();
	...
	Пока СтрокаТекстаМФОРБК <> Неопределено Цикл
		
		Строка = СтрокаТекстаМФОРБК;
	...
	ДействующиеБанки = ДействующиеБанкиИзФайла(ПараметрыЗагрузкиФайловРБК["ПутьКФайлуМФОРБК"]);
...
Процедура ПолучитьДанныеРБКИзИнтернета(ПараметрыПолученияФайловРБК)
	...
	Адрес          = "bnk";
	ФайлМФО        = "bnk.zip"; 
	ФайлРБКНаВебСервере  = "http://" + СерверИсточник + "/" + Адрес+ "/" + ФайлМФО;
	ВременныйФайлРБК = ПараметрыПолученияФайловРБК["ВременныйКаталог"]+ "\" + ФайлМФО;
		...
		Иначе
			Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
				ДополнительноеСообщение = Символы.ПС
					+ НСтр("ru='Возможно неточные или неправильные настройки подключения к Интернету.';uk='Можливо неточні або неправильні налаштування підключення до Інтернету.'");
			Иначе
				ДополнительноеСообщение = Символы.ПС
					+ НСтр("ru='Возможно неточные или неправильные настройки подключения к Интернету на сервере 1С:Предприятие.';uk='Можливо неточні або неправильні налаштування підключення до Інтернету на сервері 1С:Підприємство.'");
			КонецЕсли;
...
Функция ПолучитьСтруктуруПолейБанка(Знач Строка, Регионы)
	...
	СписокПолей.Добавить("МФО");
	СписокПолей.Добавить("КоррСчет");
	СписокПолей.Добавить("СВИФТМФО");
	...
	Если СтрДлина(Результат.МФО) <> 9 Тогда
		Возврат "";
	КонецЕсли;
	...
	КодРегиона = Сред(Результат.МФО, 3, 2);
	Регион = Регионы[КодРегиона];
	Если Регион = Неопределено Тогда
		Регион = НСтр("ru='Другие территории';uk='Інші території'");
		КодРегиона = "";
	КонецЕсли;
...
Функция ЗагрузитьДанныеИзФайлаРБК(ИмяФайла) Экспорт
	...
	Если ФайлыКлассификатораПолучены(ПапкаСИзвлеченнымиФайлами) Тогда
		...
		ФайлМФОСВИФТ = Новый Файл(ИмяФайлаМФОСВИФТРБК(ПапкаСИзвлеченнымиФайлами));
		Если ФайлМФОСВИФТ.Существует() Тогда
			Параметры.Вставить("ПутьКФайлуМФОРБК", ИмяФайлаМФОСВИФТРБК(ПапкаСИзвлеченнымиФайлами));
		Иначе
			Параметры.Вставить("ПутьКФайлуМФОРБК", ИмяФайлаМФОРБК(ПапкаСИзвлеченнымиФайлами));
		КонецЕсли;
...
Функция ФайлыКлассификатораПолучены(ПапкаСФайлами)
	...
	ИменаФайловДляПроверки.Добавить(ИмяФайлаМФОРБК(ПапкаСФайлами));
	...
	Для Каждого ИмяФайла Из ИменаФайловДляПроверки Цикл
		...
		Если Не Файл.Существует() Тогда
			ЗаписатьОшибкуВЖурналРегистрации(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не найден файл %1';uk='Не знайдений файл %1'"), ИмяФайла));
			Результат = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция ИмяСобытияВЖурналеРегистрации()
	...
	Возврат НСтр("ru='Загрузка классификатора банков.РБК';uk='Завантаження класифікатора банків.РБК'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция НедействующиеБанкиИзФайла(ПутьКФайлу)
	...
	Результат.Колонки.Добавить("МФО", Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(9)));
...
Функция ОтметитьНедействующиеБанки(ДействующиеБанки, НедействующиеБанки)
	...
	|		ПО НедействующиеБанки.МФО = КлассификаторБанковРФ.Код
	|ГДЕ
...
// Сортирует, записывает данные классификатора МФО РФ с диска ИТС.
// 
// Параметры:
//	ПараметрыЗагрузкиФайловИТС		 - Соответствие:
//	ПодготовкаИТСАдресДвоичныхДанных - ВременноеХранилище - Обработка подготовки данных МФО ИТС.
//  ДанныеИТСАдресДвоичныхДанных	 - ВременноеХранилище - Файл данных МФО ИТС.
//	Загружено						 - Число		      - Количество новых записей классификатора.
//	Обновлено						 - Число			  - Количество обновленных записей классификатора.
//	ТекстСообщения					 - Строка			  - тест сообщения о результатах загрузки.
//	ЗагрузкаВыполнена                - Булево             - флаг успешного завершения загрузки данных классификатора.
...
Процедура ЗагрузитьДанныеДискИТС(ПараметрыЗагрузкиФайловИТС) Экспорт
	...
	ТаблицаКлассификатора.Колонки.Добавить("МФО");
...
Процедура ЗаписатьЭлементКлассификатораБанковРФ(ПараметрыЗагрузкиДанных)
	...
	ЗаписываемыйЭлементСправочникаКлассификаторБанковРФ = 
		Справочники.КлассификаторБанковРФ.НайтиПоКоду(СтруктураБанк.МФО);
	...
	Если КлассификаторБанковОбъект.Код <> СтруктураБанк.МФО Тогда
		КлассификаторБанковОбъект.Код = СтруктураБанк.МФО;
	КонецЕсли;
