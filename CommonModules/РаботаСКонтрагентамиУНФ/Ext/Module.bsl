...
Функция ЗаголовокНадписиВзаиморасчетов(Контрагент, СуммаДолга = 0) Экспорт
	...
	Если Сумма < 0 Тогда
		КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Мы должны';uk='Ми повинні'") + " ", КрупныйШрифт));
		Сумма = -Сумма;
	Иначе
		КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Должен нам';uk='Повинен нам'") + " ", КрупныйШрифт));
	КонецЕсли;
