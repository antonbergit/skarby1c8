...
Функция СообщениеОЗапретах(Запреты,
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		Если ПоискЗапретовЗагрузки Тогда
			...
			Иначе
				Шаблон = НСтр("ru='%1 в запрещенном периоде невозможно заменить загружаемыми данными.';uk='%1 у забороненому періоді неможливо замінити даними, що завантажуються.'");
			КонецЕсли;
		Иначе
			Если НоваяВерсия Тогда
				Шаблон = НСтр("ru='%1 невозможно поместить в запрещенный период.';uk='%1 неможливо помістити в заборонений період.'");
			Иначе
				Шаблон = НСтр("ru='%1 в запрещенном периоде невозможно изменить.';uk='%1 у забороненому періоді неможливо змінити.'");
			КонецЕсли;
		КонецЕсли;
