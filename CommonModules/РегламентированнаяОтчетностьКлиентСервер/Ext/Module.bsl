...
Процедура СообщитьОбОшибке(Знач ТекстСообщения, Отказ, Заголовок = "", Знач Статус = Неопределено, ВызыватьИсключение = Истина) Экспорт
	...
	#Если Сервер Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Регламентированная отчетность';uk='Регламентована звітність'"), УровеньЖурналаРегистрации.Ошибка, , , Заголовок + Символы.ПС + ТекстСообщения);
	...
	#Иначе
		...
		Если ЗначениеЗаполнено(Заголовок) Тогда
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1';uk='%1'"), Заголовок);
...
Функция ПолучитьСтавкуНалога(Период) Экспорт
	...
	Возврат РегламентированнаяОтчетностьВызовСервера.ПолучитьСтавкуНалога(Период);
