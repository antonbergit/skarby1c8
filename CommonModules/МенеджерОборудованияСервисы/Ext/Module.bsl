...
Функция ПолучитьНастройки(ИДУстройства) Экспорт
	...
	ТекстСообщения = ПолучитьТекстXMLНастроек(СтруктураНастроек, ДанныеУстройства.ВерсияФорматаОбмена);
...
Функция ПолучитьПрайсЛист(ИДУстройства) Экспорт
	...
	ТекстСообщения = ПолучитьТекстXMLПрайсЛиста(СтруктураПрайсЛиста, ДанныеУстройства.ВерсияФорматаОбмена);
...
Функция ЗагрузитьДокумент(ИДУстройства, ТипДокумента, XMLТекст) Экспорт
	...
	ЗаполнитьСтруктуруДокумента(СтруктураДокумента, ТипДокумента, XMLТекст, ДанныеУстройства.ВерсияФорматаОбмена);
...
Процедура ЗаполнитьСтруктуруДокумента(СтруктураДокумента, ТипДокумента, XMLТекст, ВерсияФорматаОбмена) Экспорт
	...
	Если ТипДокумента = "SalesReport" Тогда
		...
		Исключение
			ТекстСообщения = НСтр("ru='При чтении файла-отчета произошла ошибка. Формат отчета не соответствует версии формата обмена.';uk='При чтении файла-отчета произошла ошибка. Формат отчета не соответствует версии формата обмена.'") 
		...
		Если ВерсияФорматаФайла <> ВерсияФорматаОбмена Тогда
			ТекстСообщения = НСтр("ru='При чтении файла-отчета произошла ошибка. Формат отчета не соответствует версии формата обмена.';uk='При чтении файла-отчета произошла ошибка. Формат отчета не соответствует версии формата обмена.'") + Символы.ПС + ОписаниеОшибки();
			СтруктураДокумента.Отказ = Истина;
			СтруктураДокумента.СообщениеОбОшибке = ТекстСообщения;
			Возврат;
		КонецЕсли;
...
Процедура ПометитьОбработкуДокумента(Отказ, ПолноеИмяФайла, ТипДокумента, Сообщение, ВерсияФорматаОбмена) Экспорт
	...
	Если ЗначениеЗаполнено(Данные.Обработан) Тогда
		Отказ = Истина;
		Сообщение = НСтр("ru='Отчет был обработан ранее. Пометка не производилась.';uk='Звіт був оброблений раніше. Позначка не проводилася.'");
	Иначе
...
Процедура СформироватьОчередьСообщенийОбмена(ПодключаемоеОборудование, ИдентификаторПередачи) Экспорт
	...
	Если МассивПакетов.Количество() = 0 Тогда
		...
		XMLТекстСообщения = ПолучитьТекстXMLПрайсЛиста(СтруктураПрайсЛиста, ДанныеУстройства.ВерсияФорматаОбмена);
	...
	Иначе
		...
		Для Каждого Пакет Из МассивПакетов Цикл
			...
			XMLТекстСообщения = ПолучитьТекстXMLПрайсЛиста(Пакет, ДанныеУстройства.ВерсияФорматаОбмена);
...
Процедура ДобавитьПакетДанныхВОчередьСообщенийОбмена(ПодключаемоеОборудование, ИдентификаторПередачи, ТекстСообщения, ПорядковыйНомер = 1)
	...
	Если НаборЗаписей.Количество() > 0 Тогда
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Обмен с подключаемым оборудованием. Добавление сообщения в очередь сообщений обмена';uk='Обмен с подключаемым оборудованием. Добавление сообщения в очередь сообщений обмена'"),
			УровеньЖурналаРегистрации.Ошибка,
			,
			ПодключаемоеОборудование,
			НСтр("ru = 'Очередь сообщений обмена уже содержит сообщение с номером" + " " + ПорядковыйНомер + ".'"));
