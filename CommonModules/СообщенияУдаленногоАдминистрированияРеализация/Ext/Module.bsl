	...
	Попытка
		...
		Если МенеджерЗаписи.Выбран() Тогда
			Если МенеджерЗаписи.Статус = Перечисления.СтатусыОбластейДанных.Удалена Тогда
				ШаблонСообщения = НСтр("ru='Область данных %1 удалена';uk='Область даних %1 видалена'");
			...
			ИначеЕсли МенеджерЗаписи.Статус = Перечисления.СтатусыОбластейДанных.КУдалению Тогда
				ШаблонСообщения = НСтр("ru='Область данных %1 в процессе удаления';uk='Область даних %1 в процесі видалення'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, КодОбластиДанных);
				ВызватьИсключение(ТекстСообщения);
	...
	Попытка
		...
		Для каждого ОписаниеПользователя Из Параметры.UsersList.Item Цикл
			...
			ПользовательОбъект.ДополнительныеСвойства.Вставить("СозданиеАдминистратора",
				НСтр("ru='Создание администратора области данных из менеджера сервиса.';uk='Створення адміністратора області даних з менеджера сервісу.'"));
...
Функция ПолучитьПользователяИБПоПользователюОбластиДанных(Знач ПользовательОбластиДанных)
	...
	Если ПользовательИБ = Неопределено Тогда
		ШаблонСообщения = НСтр("ru='Для пользователя области данных с идентификатором %1 не существует пользователя информационной базы';uk='Для користувача області даних з ідентифікатором %1 не "
"існує користувача інформаційної бази'");
	...
	Если ПолучитьЧасовойПоясИнформационнойБазы() <> ЧасовойПояс Тогда
		...
		Иначе
			...
			Исключение
				...
				ШаблонСообщения = НСтр("ru='Не удалось заблокировать область данных для установки часового пояса ""%1""';uk='Не вдалося заблокувати область даних для установки годинного пояса ""%1""'");
