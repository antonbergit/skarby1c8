...
Процедура ПроверитьКорректностьПерсонифицованностиЗаказываемыхКиЗ(Объект, ИмяТабличнойЧасти, Отказ) Экспорт
	...
	Если Выборка.Количество() > 1 Тогда
		...
		ТекстОшибки = НСтр("ru='Нельзя заказывать в одном документе персонифицированные и неперсонифицированные КиЗ.';uk='Нельзя заказывать в одном документе персонифицированные и неперсонифицированные КиЗ.'");
...
Функция GTINКиЗ(Номенклатура, Характеристика) Экспорт
	...
	Иначе
		...
		Если РеквизитыНоменклатуры.ИспользоватьХарактеристики Тогда
			ТекстИсключения = НСтр("ru='Невозможно определить GTIN КиЗ, т.к. не передана характеристика.';uk='Невозможно определить GTIN КиЗ, т.к. не передана характеристика.'");
...
Процедура ЗаполнитьЗаявкуНаВыпускКиЗНаОснованииДругогоДокумента(ЗаявкаОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если Не РегистрыСведений.СтатусыЗаявокНаВыпускКиЗГИСМ.ЭтоСтатусНеАктуальнойЗаявки(РеквизитыЗаказа.СтатусЗаявкиНаВыпускКиЗ) Тогда
		...
		ТекстОшибки = НСтр("ru='Для %Документ% уже существует актуальная заявка на выпуск КиЗ - %Заявка%.';uk='Для %Документ% уже существует актуальная заявка на выпуск КиЗ - %Заявка%.'");
	...
	Если РезультатЗапроса[1].Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='В %Документ% отсутствуют контрольные (идентификационные) знаки.';uk='В %Документ% отсутствуют контрольные (идентификационные) знаки.'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДанныеЗаполнения);
...
Процедура ЗаполнитьУведомлениеОСписанииКиЗНаОснованииВнутреннегоПотребления(УведомлениеОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если Не РегистрыСведений.СтатусыИнформированияГИСМ.ЭтоСтатусНеАктуальногоУведомления(РеквизитыОснования.Статус) Тогда
		...
		ТекстОшибки = НСтр("ru='Для %Документ% уже существует актуальное уведомление об списании КиЗ - %Уведомление%.';uk='Для %Документ% уже существует актуальное уведомление об списании КиЗ - %Уведомление%.'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДанныеЗаполнения);
	...
	Если РезультатЗапроса[1].Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='В %Документ% отсутствуют товары, маркированные контрольными (идентификационными) знаками и контрольные (идентификационные) знаки.';uk='В %Документ% отсутствуют товары, маркированные контрольными (идентификационными) знаками и контрольные (идентификационные) знаки.'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДанныеЗаполнения);
...
Процедура ЗаполнитьУведомлениеОбОтгрузкеГИСМНаОснованииРеализации(УведомлениеОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если Не РегистрыСведений.СтатусыУведомленийОбОтгрузкеМаркированныхТоваровГИСМ.ЭтоСтатусНеАктуальногоУведомления(РеквизитыОснования.Статус) Тогда
		...
		ТекстОшибки = НСтр("ru='Для %Документ% уже существует актуальное уведомление об отгрузке ГИСМ - %Уведомление%.';uk='Для %Документ% уже существует актуальное уведомление об отгрузке ГИСМ - %Уведомление%.'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДанныеЗаполнения);
	...
	Если РезультатЗапроса[5].Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='В %Документ% отсутствуют товары, маркированные контрольными (идентификационными) знаками.';uk='В %Документ% отсутствуют товары, маркированные контрольными (идентификационными) знаками.'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДанныеЗаполнения);
...
Процедура СформироватьТекстЗаявкаНаВыпускКиЗ(Форма) Экспорт
	...
	Если ПравоДобавления Тогда
		ТекстНадписи =  НСтр("ru='Создать заявку на выпуск КиЗ';uk='Создать заявку на выпуск КиЗ'");
		ИмяКоманды   = "СоздатьЗаявкуНаВыпускКиЗ";
	Иначе
		ТекстНадписи =  НСтр("ru='Заявка на выпуск КиЗ не создана';uk='Заявка на выпуск КиЗ не создана'");
		ИмяКоманды   = "";
	КонецЕсли;
	...
	Если Выборка.Следующий() Тогда
		...
		ТекстНадписи = СтрШаблон(НСтр("ru='Заявка на выпуск КиЗ: %1';uk='Заявка на выпуск КиЗ: %1'"), Выборка.СостояниеЗаявкиНаВыпускКиЗ);
...
Процедура СформироватьТекстУведомлениеОбИмпортеВвозеИзЕАЭС(Форма) Экспорт
	...
	Если ВидИмпорта = "ПоступлениеПоИмпорту" Тогда
		...
		Если ПравоДобавленияУведомлениеОбИпморте Тогда
			ТекстНадписи = НСтр("ru='Создать уведомление об импорте ГИСМ';uk='Создать уведомление об импорте ГИСМ'");
			ИмяКоманды   = "СоздатьУведомлениеГИСМ";
		Иначе
			ТекстНадписи = НСтр("ru='Уведомление об импорте ГИСМ не созадано';uk='Уведомление об импорте ГИСМ не созадано'");
			ИмяКоманды   = "";
		КонецЕсли;
	ИначеЕсли ВидИмпорта = "ПоступлениеИзЕАЭС" Тогда
		...
		Если ПравоДобавленияУведомлениеОВвозе Тогда
			ТекстНадписи = НСтр("ru='Создать уведомление о ввозе из ЕАЭС ГИСМ';uk='Создать уведомление о ввозе из ЕАЭС ГИСМ'");
			ИмяКоманды   = "СоздатьУведомлениеГИСМЕАЭС";
		Иначе
			ТекстНадписи = НСтр("ru='Уведомление о ввозе из ЕАЭС ГИСМ не созадано';uk='Уведомление о ввозе из ЕАЭС ГИСМ не созадано'");
			ИмяКоманды   = "";
		КонецЕсли;
	Иначе 
		ВызватьИсключение (НСтр("ru='Ошибочный вид импорта';uk='Ошибочный вид импорта'"));
	КонецЕсли;
	...
	Если Выборка.Следующий() Тогда
		...
		Если ВидИмпорта = "ПоступлениеПоИмпорту" Тогда
			ТекстНадписи = СтрШаблон(НСтр("ru='Уведомление об импорте ГИСМ: %1';uk='Уведомление об импорте ГИСМ: %1'"), Выборка.Статус);
		ИначеЕсли ВидИмпорта = "ПоступлениеИзЕАЭС" Тогда
			ТекстНадписи = СтрШаблон(НСтр("ru='Уведомление о ввозе из ЕАЭС ГИСМ: %1';uk='Уведомление о ввозе из ЕАЭС ГИСМ: %1'"), Выборка.Статус);
		КонецЕсли;
...
Процедура СформироватьТекстУведомлениеОСписанииКиЗ(Форма) Экспорт
	...
	Если ПравоДобавления Тогда
		ТекстНадписи =  НСтр("ru='Создать уведомление о списании КиЗ ГИСМ';uk='Создать уведомление о списании КиЗ ГИСМ'");
		ИмяКоманды   = "СоздатьУведомлениеГИСМ";
	Иначе
		ТекстНадписи = НСтр("ru='Уведомление о списании КиЗ ГИСМ не создано';uk='Уведомление о списании КиЗ ГИСМ не создано'");
		ИмяКоманды   = "";
	КонецЕсли;
	...
	Если Выборка.Следующий() Тогда
		...
		ТекстНадписи  = СтрШаблон(НСтр("ru='Уведомление о списании КиЗ ГИСМ: %1';uk='Уведомление о списании КиЗ ГИСМ: %1'"), Выборка.Статус);
		ИмяКоманды    = "ОткрытьПротоколОбмена";
...
Процедура СформироватьТекстУведомлениеОбОтгрузкеМаркированнойПродукции(Форма) Экспорт
	...
	Если ПравоДобавления Тогда
		ТекстНадписи =  НСтр("ru='Создать уведомление об отгрузке ГИСМ';uk='Создать уведомление об отгрузке ГИСМ'");
		ИмяКоманды   = "СоздатьУведомлениеГИСМ";
	Иначе
		ТекстНадписи = НСтр("ru='Уведомление об отгрузке ГИСМ не создано';uk='Уведомление об отгрузке ГИСМ не создано'");
		ИмяКоманды   = "";
	КонецЕсли;
	...
	Если Выборка.Следующий() Тогда
		...
		ТекстНадписи = СтрШаблон(НСтр("ru='Уведомление об отгрузке ГИСМ: %1';uk='Уведомление об отгрузке ГИСМ: %1'"), Выборка.Статус);
		ИмяКоманды   = "ОткрытьПротоколОбмена";
...
Процедура ИнициализироватьДанныеДокументаМаркировкиТоваров(ДокументСсылкаМаркировкиТоваров,СтруктураДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("ПоступлениеРасходов", НСтр("ru='Прочие расходы';uk='Інші витрати'"));
