...
Функция ЭтоЗаполнениеПоШаблону(ДанныеЗаполнения)
...
Процедура ВызватьОбработчикПередЗаполнением(СтратегияЗаполнения, ДокументОбъект, ДанныеЗаполнения)
	...
	Если ТипЗнч(СтратегияЗаполнения) <> Тип("Соответствие") Тогда
		ВызватьИсключение НСтр("ru='Некорректный тип параметра ""ОбработчикЗаполнения"": ожидается Строка или Соответствие.';uk='Некоректний тип параметра ""ОбработчикЗаполнения"": очікується Рядок або Відповідність.'");
	КонецЕсли;
...
Процедура ДополнитьЗначениямиИзНастроек(ДанныеЗаполнения, ДокументОбъект)
	...
	ДополнитьОсновноеМестоСоставления(ЗначенияИзНастроек, ДокументОбъект);
...
Процедура ДополнитьОснованиеПечати(ЗначенияИзНастроек, ДокументОбъект)
	...
	Если ЗначениеЗаполнено(ДокументОбъект.Договор) Тогда
		...
		"ОснованиеПечати",
		СтрШаблон(
		НСтр("ru='Договор: %1';uk='Договір: %1'"),
...
Процедура ДополнитьОсновноеМестоСоставления(ЗначенияИзНастроек, ДокументОбъект)
	
	Если ОтсутствуетНезаполненныйРеквизит("МестоСоставленияДокумента", ДокументОбъект) Тогда
		Возврат;
	КонецЕсли;
	
	ОсновноеМесто = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(
	Пользователи.ТекущийПользователь(),
	"ОсновноеМестоСоставленияРасходныхДокументов");
	
	Если ЗначениеЗаполнено(ОсновноеМесто) Тогда
		ЗначенияИзНастроек.Вставить("МестоСоставленияДокумента", ОсновноеМесто);
	КонецЕсли;
...
Функция ПолучитьСостояниеЗаказаПокупателя(знач ВидЗаказа = Неопределено) Экспорт
	...
	Если ИспользуютсяВидыЗаказов И Не ЗначениеЗаполнено(ВидЗаказа) Тогда
		ВызватьИсключение НСтр("ru='Не указан вид заказа для заполнения состояния заказа покупателя';uk='Не указан вид заказа для заполнения состояния заказа покупателя'");
	КонецЕсли;
...
Функция ПолучитьСостояниеЗаказНаряда(знач ВидЗаказа = Неопределено) Экспорт
	...
	Если ИспользуютсяВидыЗаказов И Не ЗначениеЗаполнено(ВидЗаказа) Тогда
		ВызватьИсключение НСтр("ru='Не указан вид заказа для заполнения состояния заказ-наряда';uk='Не указан вид заказа для заполнения состояния заказ-наряда'");
	КонецЕсли;
