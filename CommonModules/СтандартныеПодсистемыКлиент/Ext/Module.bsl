...
Процедура ПриНачалеРаботыСистемы(Знач ОповещениеЗавершения = Неопределено, НепрерывноеВыполнение = Истина) Экспорт
	...
	//Юра 26 11 2025
	Если ОбщегоНазначенияВызовСервера.нРольдоступна("АвтозапускКлиентБанкПриват24") Тогда
		нФорма = ПолучитьФорму("Обработка.МодульОбменаПриват24вБО.Форма.ФормаУправляемая");
		нФорма.Открыть();
	КонецЕсли;
	СкрытьРабочийСтолПриНачалеРаботыСистемы(Ложь, Ложь);
	//Юра 26 11 2025
...
"Необхідно звернутися до адміністратора для відновлення версії платформи 1С:Підприємство.'");
		КонецЕсли;
	Иначе
		Если ПараметрыКлиента.ЕстьДоступДляОбновленияВерсииПлатформы Тогда
			ТекстСообщения = 
...
"В противном случае некоторые возможности программы будут недоступны или будут работать некорректно.';uk='Рекомендується завершити роботу програми й звернутися до адміністратора для відновлення версії платформи 1С:Підприємства."
"У противному випадку деякі можливості програми будуть недоступні або будуть працювати некоректно.'");
		КонецЕсли;
	КонецЕсли;
...
" объем памяти до %1 Гб.';uk='Рекомендується збільшити"
" обсяг пам`яті до %1 Гб.'"), РекомендуемыйОбъем);
