...
Процедура СоздатьЗаписьТипа(Знач СловарьТипов, Знач Описание) Экспорт
	...
	Если ПустаяСтрока(Описание.ТипИнцидента) Тогда
		ВызватьИсключение НСтр("ru='ТипИнцидента не указан';uk='ТипІнцидента не зазначений'");
	КонецЕсли;
...
Процедура ОткрытьИнцидент(Знач ТипИнцидента, Знач КодИнцидента, Знач ТекстСообщения, Знач Асинхронно = Ложь) Экспорт
	...
	Если ПустаяСтрока(ТипИнцидента) Тогда	
		ВызватьИсключение НСтр("ru='Не указан тип инцидента';uk='Не зазначений тип інциденту'"); 
	КонецЕсли;
	Если ПустаяСтрока(КодИнцидента) Тогда	
		ВызватьИсключение НСтр("ru='Не указан код инцидента';uk='Не зазначений код інциденту'"); 
	КонецЕсли;
	Если ПустаяСтрока(ТекстСообщения) Тогда	
		ВызватьИсключение НСтр("ru='Не указано сообщение инцидента';uk='Не зазначене повідомлення інциденту'"); 
	КонецЕсли;
...
Процедура ОтослатьСтатистику(Знач ИдентификаторСчетчика, Знач ЗначениеСчетчика, Знач Данные = неопределено, Знач ДатаСчетчика = неопределено) Экспорт
	...
	Если Результат <> "OK" Тогда
		ЗаписьЖурналаРегистрации(
			...
			НСтр("ru='Ошибка при передаче счетчика в ЦКК';uk='Помилка при передачі лічильника в ЦКК'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	КонецЕсли;
...
Процедура ВыполнитьЗапрос(КомандаЗапроса, Данные)
	...
	Если Ответ.КодСостояния<>200 Тогда
		ЗаписьЖурналаРегистрации(
			...
			УровеньЖурналаРегистрации.Ошибка,,, 
			СтрШаблон(
			НСтр("ru='Ошибка (код %3) команды %1 сервиса инцидентов ЦКК: %2';uk='Помилка (код %3) команди %1 сервісу інцидентів ЦКК: %2'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Процедура ОтослатьПоказателиДлиныОчередейЗаданий() 
	...
	Иначе
		ВызватьИсключение НСтр("ru='Невозможно определить имя кластера и информационной базы из строки подключения';uk='Неможливо визначити ім`я кластера і інформаційної бази з рядка підключення'");
	КонецЕсли;
