...
Функция СобытиеЖурналаРегистрации(Объект)
	...
	Если Объект = Метаданные.РегламентныеЗадания.ОбновлениеИнформацииОНаправленияхСдачиОтчетности Тогда
		Возврат НСтр("ru='Обновление информации о направлениях сдачи отчетности';uk='Обновление информации о направлениях сдачи отчетности'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ОтправкаОтчетности Тогда
		Возврат НСтр("ru='Отправка отчетности';uk='Отправка отчетности'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ПолучениеРезультатовОтправкиОтчетности Тогда
		Возврат НСтр("ru='Получение результатов отправки отчетности';uk='Получение результатов отправки отчетности'");
	КонецЕсли;
...
Функция ПолучитьКомментарийКСобытиюНачалаРегламентногоЗадания(Объект)
	...
	Если Объект = Метаданные.РегламентныеЗадания.ОбновлениеИнформацииОНаправленияхСдачиОтчетности Тогда
		Возврат НСтр("ru='Начато обновление информации о направлениях сдачи отчетности';uk='Начато обновление информации о направлениях сдачи отчетности'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ОтправкаОтчетности Тогда
		Возврат НСтр("ru='Начата отправка отчетности';uk='Начата отправка отчетности'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ПолучениеРезультатовОтправкиОтчетности Тогда
		Возврат НСтр("ru='Начато получение результатов отправки отчетности';uk='Начато получение результатов отправки отчетности'");
	КонецЕсли;
...
Функция ПолучитьКомментарийКСобытиюОкончанияРегламентногоЗадания(Объект)
	...
	Если Объект = Метаданные.РегламентныеЗадания.ОбновлениеИнформацииОНаправленияхСдачиОтчетности Тогда
		Возврат НСтр("ru='Завершено обновление информации о направлениях сдачи отчетности';uk='Завершено обновление информации о направлениях сдачи отчетности'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ОтправкаОтчетности Тогда
		Возврат НСтр("ru='Завершена отправка отчетности';uk='Завершена отправка отчетности'");
	ИначеЕсли Объект = Метаданные.РегламентныеЗадания.ПолучениеРезультатовОтправкиОтчетности Тогда
		Возврат НСтр("ru='Завершено получение результатов отправки отчетности';uk='Завершено получение результатов отправки отчетности'");
	КонецЕсли;
...
Функция СсылкаПоСтрокеСтатуса(СтрокаСтатуса, ОписаниеОшибки = "")
	...
	Иначе
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось определить статус сдачи отчетности по ответу сервера (%1)';uk='Не удалось определить статус сдачи отчетности по ответу сервера (%1)'"), ТекстИзФайла);
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПроверитьПередРегистрациейЗаявкиНаОтправкуОтчетности(Отчет, КонтролирующийОрган = "ФНС", ТекстВопроса = "") Экспорт
	...
	Если НЕ НаправлениеСдачиОтчетностиОткрыто(Организация, ТипНаправления, КодНаправления) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Направление сдачи отчетности по организации закрыто. Отправка невозможна.';uk='Направление сдачи отчетности по организации закрыто. Отправка невозможна.'"));
		Возврат Ложь;
	КонецЕсли;
...
""
"Вы уверены, что хотите отправить отчет заново?';uk='Вы уже отправляли аналогичный отчет с аналогичным признаком корректировки,"
"за тот же период, по той же организации и в ту же инспекцию."
""
...
"Вы уверены, что хотите отправить отчет заново?';uk='Вы уже отправляли данный отчет."
"Вы уверены, что хотите отправить отчет заново?'");
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Функция ЗарегистрироватьОтправкуОтчетностиВСправочнике(ДанныеПакета, ИмяФайлаПакета, Отчет) Экспорт
	...
	Если ТипНаправления = Перечисления.ТипыНаправленийСдачиОтчетности.ФНС Тогда
		...
		ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(КодНаправления) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Код инспекции-получателя отчета должен состоять строго из четырех цифр.';uk='Код инспекции-получателя отчета должен состоять строго из четырех цифр.'"));
			Возврат Ложь;
		КонецЕсли;
	ИначеЕсли ТипНаправления = Перечисления.ТипыНаправленийСдачиОтчетности.ФСГС Тогда
		...
		ИЛИ Сред(КодНаправления, 3, 1) <> "-" Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Код органа ФСГС, в который представляется отчетность организации (задается в справочнике ""Организации""), должен удовлетворять шаблону XX-XX, где X - любая цифра.';uk='Код органа ФСГС, в который представляется отчетность организации (задается в справочнике ""Организации""), должен удовлетворять шаблону XX-XX, где X - любая цифра.'"));
			Возврат Ложь;
		КонецЕсли;
	Иначе
		...
		ИЛИ Сред(КодНаправления, 4, 1) <> "-" Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Код органа ПФР, в который представляется отчетность организации (задается в справочнике ""Организации""), должен удовлетворять шаблону XXX-XXX, где X - любая цифра.';uk='Код органа ПФР, в который представляется отчетность организации (задается в справочнике ""Организации""), должен удовлетворять шаблону XXX-XXX, где X - любая цифра.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если МетаданныеОтчета = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Неизвестный вид отчета.';uk='Неизвестный вид отчета.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ВидОтчета) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В справочнике ""Регламентированные отчеты"" не найдено определение отчета вида %1.';uk='В справочнике ""Регламентированные отчеты"" не найдено определение отчета вида %1.'"), ИсточникОтчета));
		Возврат Ложь;
	КонецЕсли;
...
""
"%1';uk='Не удалось запланировать отправку отчета:"
""
...
Функция ПолучитьСведенияИзУправляющегоПриложения()
	...
	Если НЕ ЗначениеЗаполнено(ВебАдрес) Тогда
		ЗаписьЖурналаРегистрации(СобытиеЖурнала,
								...
								Метаданные.РегламентныеЗадания.ОбновлениеИнформацииОНаправленияхСдачиОтчетности,
								,
								НСтр("ru='Не задан адрес веб-сервиса для получения сведений о направлениях сдачи отчетности.';uk='Не задан адрес веб-сервиса для получения сведений о направлениях сдачи отчетности.'"));
		Возврат Неопределено;
	КонецЕсли;
...
Функция СформироватьФайлПакета(
	...
	Для Инд = 0 По КороткиеИменаФайловВыгрузки.Количество() - 1 Цикл
		...
		Иначе
			Если НЕ ЗаписатьТекстВФайл(ЭлТекстВыгрузки, ПолноеИмяФайлаВыгрузки, ЭлКодировкаТекстаВыгрузки) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось запланировать отправку отчета: не удалось записать временный файл с отчетностью.';uk='Не удалось запланировать отправку отчета: не удалось записать временный файл с отчетностью.'"));
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
	...
	Если КонтролирующийОрган = "ФНС" Тогда
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось запланировать отправку отчета: отсутствуют сведения о доверенности.';uk='Не удалось запланировать отправку отчета: отсутствуют сведения о доверенности.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
"не удалось записать файл пакета с отчетностью.';uk='Не удалось запланировать отправку отчета:"
...
Функция ИзменитьФайлВыгрузкиПриОтправкеЧерезУполномоченногоПредставителя(ТекстВыгрузки, ИмяФайлаВыгрузки, Доверенность, КодНО, ДатаПериода = '00010101')
	...
	Если нрег(РасширениеФайла) = ".xml" Тогда // формат xml - меняем ИдФайл и имя файла
		...
		Если КомпонентыИмени.Количество() < 5 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Невозможно изменить реквизиты отправителя в имени файла отчета: некорректное исходное имя файла выгрузки.';uk='Невозможно изменить реквизиты отправителя в имени файла отчета: некорректное исходное имя файла выгрузки.'"));
			Возврат Ложь;
		КонецЕсли;
...
Функция ВыгрузитьСведенияОДоверенностиПоНаправлениюОтчета(Отчет, ДанныеДоверенности, ИмяФайлаДоверенности, Доверенность = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(РегистрацияВИФНС) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось запланировать отправку отчета:"
"не найдена запись в справочнике ""Регистрации в налоговом органе"", соответствующая направлению отправки (%1).';uk='Не удалось запланировать отправку отчета:"
"не найдена запись в справочнике ""Регистрации в налоговом органе"", соответствующая направлению отправки (%1).'"), КодИФНС));
	...
	Если ФайлДоверенности = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось запланировать отправку отчета:"
"не удалось выгрузить сведения о доверенности налогоплательщика.';uk='Не удалось запланировать отправку отчета:"
