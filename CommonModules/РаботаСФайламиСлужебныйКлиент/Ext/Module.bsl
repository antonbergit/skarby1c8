...
Процедура ОбновитьИзФайлаНаДиске(ОбработчикРезультата, ДанныеФайла, ИдентификаторФормы)
	...
	Если ЗначениеЗаполнено(РасширениеДляЗашифрованныхФайлов) Тогда
		Фильтр = НСтр("ru='Файл (*.%1)|*.%1|Зашифрованный файл (*.%2)|*.%2|Все файлы (*.*)|*.*';uk='Файл (*.% 1)|*.% 1|Зашифрований файл (*.% 2)|*.% 2|Всі файли (*.*)|*.*'");
...
"Расшифруйте файл и зашифруйте заново.';uk='У зашифрованого файлу не зазначені сертифікати."
...
"из рабочего каталога?';uk='Видалити файл ""%1"""
"з робочого каталогу?'");
...
"%2'"),
					...
					ТекстСообщения = ТекстСообщения + Символы.ВК + Символы.ВК
						+ НСтр("ru='Измените имя файла на более короткое.';uk='Змінить ім`я файлу на більш коротке.'");
...
"Код файлу в робочому каталозі: %3."
""
"В інформаційній базі змінить робочий каталог однієї з папок."
...
"""%2"".'"),
	...
	Если СтрДлина(ПараметрыВыполнения.ДанныеФайла.РабочийКаталогВладельца) > ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути - 5 Тогда
		ТекстСообщения = ТекстСообщения + Символы.ВК + Символы.ВК
			+ НСтр("ru='Измените имена папок или перенесите текущую папку в другую папку.';uk='Змінить імена папок або перенесіть поточну папку в іншу папку.'");
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, ТекстСообщения, Ложь);
		Возврат;
	КонецЕсли;
...
"""%1""."
"Можливо він зайнятий іншим додатком."
""
"Повторити операцію?'"),
...
"Возможно он занят другим приложением."
""
"Повторить операцию?';uk='Не вдалося видалити файл"
"""%1""."
"Можливо він зайнятий іншим додатком."
""
"Повторити операцію?'"),
...
Процедура ИмпортФайловПослеЦиклаПродолжение(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.МассивСтруктурВсехФайлов.Количество() > 1 Тогда
		...
		ТекстСостояния = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Импорт файлов завершен. Импортировано файлов: %1';uk='Імпорт файлів завершений. Імпортовано файлів: %1'"), Строка(ПараметрыВыполнения.МассивСтруктурВсехФайлов.Количество()) );
