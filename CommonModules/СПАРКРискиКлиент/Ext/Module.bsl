...
Процедура ПоказатьЗначенияИндексовКонтрагента(ИндексыСПАРКРиски, ВладелецФормы = Неопределено) Экспорт
	...
	Если ИндексыСПАРКРиски = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru='Нет информации для отображения.';uk='Нет информации для отображения.'"));
		Возврат;
	ИначеЕсли ИндексыСПАРКРиски.ПодлежитПроверке <> Истина Тогда
		ПоказатьПредупреждение(, НСтр("ru='Выбранный контрагент не подлежит проверке в сервисе 1СПАРК Риски.';uk='Выбранный контрагент не подлежит проверке в сервисе 1СПАРК Риски.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьПодключаемуюКоманду(Команда, Форма, Источник) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("ДанныеФормыСтруктура") Тогда
		...
		Если КонтрагентСсылка.Пустая() Или Форма.Модифицированность Тогда
...
"Данные будут записаны.';uk='Данные еще не записаны."
"Переход к ""Справки"" возможен только после записи данных."
...
"Данные будут записаны.'");
			...
			ОписаниеКоманды = Новый Структура("Команда, Форма, Источник", Команда, Форма, Источник);
	...
	ИначеЕсли Команда.Имя = "СПАРКРиски_ВключитьМониторингСобытий" Тогда
		...
		СПАРКРискиВызовСервера.ВключитьОтключитьМониторингСобытийКонтрагента(Источник.Ссылка, ВключитьМониторинг);
...
Процедура ПодключаемаяКомандаПриОтветеНаВопросОЗаписи(КодВозврата, ОписаниеКоманды) Экспорт
	...
	ВыполнитьПодключаемуюКоманду(ОписаниеКоманды.Команда, ОписаниеКоманды.Форма, ОписаниеКоманды.Источник);
