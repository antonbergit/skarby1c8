	...
	Если ЗначениеЗаполнено(GTIN)
		...
		Если ОбрабатываемаяСтрока.ЗаполненRFIDTID Тогда
			...
			Иначе
				...
				ТекстСообщения = НСтр("ru='При маркировке непесонифицированными КиЗ номер серии должен быть сгенирирован по TID RFID-метки. При генерации произошла ошибка: %ТекстОшибки%. Обратитесь к администратору.';uk='При маркировке непесонифицированными КиЗ номер серии должен быть сгенирирован по TID RFID-метки. При генерации произошла ошибка: %ТекстОшибки%. Обратитесь к администратору.'");
