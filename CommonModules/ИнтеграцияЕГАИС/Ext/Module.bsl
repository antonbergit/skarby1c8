...
Функция СправочникВидовАлкогольнойПродукции() Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru='Определено более одного справочника видов алкогольной продукции.';uk='Определено более одного справочника видов алкогольной продукции.'");
	КонецЕсли;
...
Функция ПреобразоватьВXDTOОрганизацию_v2(Организация, ОрганизацияРФ, ТекстОшибки)
	...
	Если РеквизитыОрганизации.ТипОрганизации.Пустая() Тогда
		ТекстОшибки = НСтр("ru='Для организации %1 не заполнен тип.';uk='Для организации %1 не заполнен тип.'");
	...
	Если ОрганизацияРФ И РеквизитыОрганизации.ТипОрганизации <> Перечисления.ТипыОрганизацийЕГАИС.ЮридическоеЛицоРФ
		...
		ТекстОшибки = НСтр("ru='Организация %1 не зарегистрирована на территории РФ.';uk='Организация %1 не зарегистрирована на территории РФ.'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Организация);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Функция ПреобразоватьВXDTOАктПодтвержденияТТН(ВидДокумента, ДокументСсылка, ТекстОшибки)
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ТТНИсходящаяЕГАИС") Тогда
		ДанныеДляВыгрузки = Документы.ТТНИсходящаяЕГАИС.ИнициализироватьДанныеДокументаДляВыгрузки(ДокументСсылка, ВидДокумента);
	Иначе
		ТекстОшибки = НСтр("ru='Для документа %1 не предусмотрена выгрузка акта подтверждения.';uk='Для документа %1 не предусмотрена выгрузка акта подтверждения.'");
...
Функция ОбработатьВходящиеДокументы(МассивДокументов, ДополнительныеПараметры) Экспорт
	...
	Для Каждого СтруктураДокумента Из МассивДокументов Цикл
		...
		Исключение
			ТекстОшибки = НСтр("ru='Не удалось прочитать входящий документ %1';uk='Не удалось прочитать входящий документ %1'");
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='ЕГАИС';uk='ЄДАІС'", Метаданные.ОсновнойЯзык.КодЯзыка),
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.ActInventoryInformBReg = Неопределено
			...
			Если РезультатОперации.Результат Тогда
				РезультатОбработкиДокумента.КомментарийЕГАИС = ?(РезультатОперации.ПолученОтказ, РезультатОперации.ОписаниеОшибки, НСтр("ru='Справки зарегистрированы.';uk='Справки зарегистрированы.'"));
			КонецЕсли;
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.WayBillAct = Неопределено
			...
			Если РезультатОперации.Результат Тогда
				РезультатОбработкиДокумента.КомментарийЕГАИС = ?(РезультатОперации.ПолученОтказ, РезультатОперации.ОписаниеОшибки, НСтр("ru='Загружен акт подтверждения ТТН.';uk='Загружен акт подтверждения ТТН.'"));
			КонецЕсли;
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.ConfirmTicket = Неопределено Тогда
			...
			Если РезультатОперации.Результат Тогда
				РезультатОбработкиДокумента.КомментарийЕГАИС = ?(РезультатОперации.ПолученОтказ, РезультатОперации.ОписаниеОшибки, НСтр("ru='Обработано подтверждение акта расхождений.';uk='Обработано подтверждение акта расхождений.'"));
			КонецЕсли;
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.Ticket = Неопределено Тогда
			...
			Иначе
				РезультатОбработкиДокумента.КомментарийЕГАИС = НСтр("ru='Не удалось определить формат ответа на запрос %1';uk='Не удалось определить формат ответа на запрос %1'");
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.ReplyClient = Неопределено
			...
			Если СписокОрганизацийXDTO.Количество() = 0 Тогда
				РезультатОбработкиДокумента.ПолученОтказ = Истина;
				РезультатОбработкиДокумента.КомментарийЕГАИС = НСтр("ru='Отсутствуют данные для загрузки.';uk='Отсутствуют данные для загрузки.'");
			Иначе
				РезультатОбработкиДокумента.КомментарийЕГАИС = НСтр("ru='Загрузка завершена."
"Загружено элементов: %1.';uk='Загрузка завершена."
...
"Загружено элементов: %1.'");
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.ReplyFormA = Неопределено
			...
			РезультатОбработкиДокумента.Результат = ПустаяСтрока(ТекстОшибки);
			РезультатОбработкиДокумента.ОписаниеОшибки = ТекстОшибки;
			РезультатОбработкиДокумента.КомментарийЕГАИС = НСтр("ru='Загружена справка 1.';uk='Загружена справка 1.'");
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.ReplyFormB = Неопределено
			...
			РезультатОбработкиДокумента.Результат = ПустаяСтрока(ТекстОшибки);
			РезультатОбработкиДокумента.ОписаниеОшибки = ТекстОшибки;
			РезультатОбработкиДокумента.КомментарийЕГАИС = НСтр("ru='Загружена справка 2.';uk='Загружена справка 2.'");
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.ReplyRests = Неопределено
			...
			Если НЕ ДанныеЗапроса = Неопределено И ЗначениеЗаполнено(ДанныеЗапроса.ДокументОснование) Тогда
				...
				РезультатОбработкиДокумента.ДокументОснование = ДанныеЗапроса.ДокументОснование;
			Иначе
				ТекстОшибки = НСтр("ru='Не найден исходящий документ запроса остатков.';uk='Не найден исходящий документ запроса остатков.'");
			КонецЕсли;
			
			РезультатОбработкиДокумента.Результат = ПустаяСтрока(ТекстОшибки);
			РезультатОбработкиДокумента.ОписаниеОшибки = ТекстОшибки;
			РезультатОбработкиДокумента.КомментарийЕГАИС = НСтр("ru='Загружены остатки по складу.';uk='Загружены остатки по складу.'");
		...
		ИначеЕсли НЕ ОбъектXDTO.Document.ReplyRestsShop_v2 = Неопределено Тогда
			...
			РезультатОбработкиДокумента.Результат = ПустаяСтрока(ТекстОшибки);
			РезультатОбработкиДокумента.ОписаниеОшибки = ТекстОшибки;
			РезультатОбработкиДокумента.КомментарийЕГАИС = НСтр("ru='Загружены остатки по торговому залу.';uk='Загружены остатки по торговому залу.'");
			РезультатОбработкиДокумента.ВидДокумента = Перечисления.ВидыДокументовЕГАИС.ОтветОстаткиВТорговомЗале;
			
			Результат.Вставить(СтруктураДокумента.АдресЗапроса, РезультатОбработкиДокумента);
			
		Иначе
			РезультатОбработкиДокумента.ОписаниеОшибки = НСтр("ru='Не удалось определить формат входящего документа %1';uk='Не удалось определить формат входящего документа %1'");
...
Функция ЗарегистрироватьСправкиПоАктуПостановкиНаБаланс(КвитанцияXDTO, ДополнительныеПараметры)
	...
	Если НЕ ЗначениеЗаполнено(КвитанцияXDTO.Header.Identity) Тогда
		Результат.ОписаниеОшибки = НСтр("ru='Не заполнен идентификатор акта.';uk='Не заполнен идентификатор акта.'");
	...
	Исключение
		Результат.ОписаниеОшибки = НСтр("ru='Некорректный идентификатор акта %1.';uk='Некорректный идентификатор акта %1.'") + Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	...
	Если АктОбъект = Неопределено Тогда
		Результат.ОписаниеОшибки = НСтр("ru='Неизвестный идентификатор акта %1.';uk='Неизвестный идентификатор акта %1.'");
		Результат.ОписаниеОшибки = СтрЗаменить(Результат.ОписаниеОшибки, "%1", КвитанцияXDTO.Header.Identity);
		Результат.ПолученОтказ = Истина;
		Возврат Результат;
	КонецЕсли;
...
Функция ЗагрузитьПодтверждениеАктаРасхожденийТТН(КвитанцияXDTO, ДополнительныеПараметры)
	...
	Если НЕ ЗначениеЗаполнено(КвитанцияXDTO.Header.WBRegId) Тогда
		Результат.ОписаниеОшибки = НСтр("ru='Не заполнен идентификатор ТТН.';uk='Не заполнен идентификатор ТТН.'");
		Результат.ПолученОтказ = Истина;
		Возврат Результат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ТТН) Тогда
		Результат.ОписаниеОшибки = НСтр("ru='Не найдена входящая ТТН с идентификатором %1.';uk='Не найдена входящая ТТН с идентификатором %1.'");
...
Функция ЗагрузитьАктПодтвержденияТТН(АктXDTO, ДополнительныеПараметры)
	...
	Если НЕ ЗначениеЗаполнено(ТТН) Тогда
		Результат.ОписаниеОшибки = НСтр("ru='Не найдена исходящая ТТН с идентификатором %1.';uk='Не найдена исходящая ТТН с идентификатором %1.'");
...
Функция ПредставлениеОшибкиXDTO(ПредставлениеОшибки, ЧтениеXML, Глубина = Неопределено)
	...
	Если СтрНайти(ПредставлениеОшибки, НСтр("ru='Ошибка проверки данных XDTO';uk='Ошибка проверки данных XDTO'")) = 0 Тогда
	...
	Если СтрокаТаблицы <> Неопределено Тогда
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента Тогда
			Если НЕ ЗначениеЗаполнено(ЗначениеПоля) Тогда
				ПредставлениеОшибки = НСтр("ru='Не заполнено значение поля ""%1"" (%2)';uk='Не заполнено значение поля ""%1"" (%2)'");
			...
			Иначе
				ПредставлениеОшибки = НСтр("ru='Некорректное значение ""%1"" поля ""%2"" (%3)';uk='Некорректное значение ""%1"" поля ""%2"" (%3)'");
		...
		Иначе
			...
			Пока ИндексСтроки > 0 Цикл
				...
				Если ПредыдущееПоле.Обязательное Тогда
					ПредставлениеОшибки = НСтр("ru='Отсутствует обязательное поле ""%1"" (%2)';uk='Отсутствует обязательное поле ""%1"" (%2)'");
...
Процедура СформироватьИдентификаторОбъектаРИБПередЗаписью(Источник, Отказ) Экспорт
	...
	Иначе
		...
		Если ПредыдущийКод <> Источник[ИмяРеквизита] Тогда
			Отказ = Истина;
			СтрокаСообщения = НСтр("ru='Изменение реквизита %1 для существующих элементов запрещено.';uk='Изменение реквизита %1 для существующих элементов запрещено.'");
...
Функция ЗаписатьОбъект(Объект, ТекстОшибки, РежимЗаписи = Неопределено)
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ТекстОшибки = НСтр("ru='Ошибка записи %1.';uk='Ошибка записи %1.'");
		
		ПредставлениеОбъекта = "";
		Если Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(Объект.Ссылка)) Тогда
			ПредставлениеОбъекта = ПредставлениеОбъекта + НСтр("ru='документа';uk='документа'");
		ИначеЕсли Справочники.ТипВсеСсылки().СодержитТип(ТипЗнч(Объект.Ссылка)) Тогда
			ПредставлениеОбъекта = ПредставлениеОбъекта + НСтр("ru='элемента справочника';uk='элемента справочника'");
		КонецЕсли;
