...
"Проверьте свойство ""Хранилище вариантов"" в свойствах отчета.';uk='Помилка одержання опису звіту ""%1"": він не підключений до підсистеми ""%2""."
...
"Поиск отчетов временно недоступен.';uk='Повне оновлення індексу пошуку звітів, передбачених у "
"програмі."
...
"Поиск отчетов временно недоступен.';uk='Повне оновлення індексу пошуку звітів, збережених "
"користувачами."
"Пошук звітів тимчасово недоступний.'");
	...
	Для Каждого ПолноеИмяВарианта Из КлючиВариантов Цикл
		...
		Пока Истина Цикл
			...
			Исключение
				...
				ОшибкаПоВарианту(
					Неопределено,
					НСтр("ru='В процессе выборки пользовательских настроек отчетов из системного хранилища возникла ошибка:';uk='У процесі вибірки користувацьких настроювань звітів із системного сховища виникла помилка:'")
					+ Символы.ПС
...
"Поиск отчетов временно недоступен.';uk='Оновлення індексу пошуку звітів, передбачених у програмі."
"Пошук звітів тимчасово недоступний.'");
	КонецЕсли;
...
"в пользовательских отчетах до 2 шт.';uk='Зменшує кількість швидких налаштувань"
...
Функция ПодключитьОтчетИЗагрузитьНастройки(Параметры) Экспорт
	...
	Если НадоПрочитатьНастройки Тогда
		...
		Если ВариантНастроекКД = Неопределено Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Вариант ""%1"" (ключ ""%2"") не найден в схеме отчета ""%3"".';uk='Вариант ""%1"" (ключ ""%2"") не найден в схеме отчета ""%3"".'"),
...
Функция ПодключитьОтчетОбъект(СсылкаОтчета, ПолучатьМетаданные)
	...
	Если СсылкаОтчета = Неопределено Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='В методе ""%1"" не указан параметр ""%2"".';uk='У методі ""%1"" не зазначений параметр ""%2"".'"),
	...
	Если ТипЗнч(Результат.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных")
		...
		Если ПолучатьМетаданные Тогда
			...
			Если Результат.Метаданные = Неопределено Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Отчет ""%1"" не найден в программе';uk='Звіт ""%1"" не знайдений в програмі'"),
					Результат.Имя);
				Возврат Результат;
			КонецЕсли;
		КонецЕсли;
...
Функция ПараметрыОткрытия(ВариантСсылка) Экспорт
	...
	Иначе
		...
		Если Не Выборка.Следующий() Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Недостаточно прав для открытия варианта ""%1"".';uk='Недостатньо прав для відкриття варіанта ""%1"".'"), Строка(ВариантСсылка));
		КонецЕсли;
...
"Возможно, было отключено расширение конфигурации с этим отчетом.';uk='Не вдалося відкрити звіт ""%1""."
...
Процедура СобратьСтатистикуКонфигурации() Экспорт
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УдалитьЦентрМониторинга") Тогда
		Возврат;
	КонецЕсли;
...
Функция СформироватьОтчет(Знач Параметры, Знач ПроверятьЗаполнение, Знач ПолучатьФлажокПустой) Экспорт
	...
	// Гунько 2021-04-14
	Если НЕ ОтчетОбъект.Метаданные().Реквизиты.Найти("ЭтоОтчетРасходы") = Неопределено И ОтчетОбъект.ЭтоОтчетРасходы Тогда
		Результат.ТабличныйДокумент.ПоказатьУровеньГруппировокСтрок(0);
	КонецЕсли;
	// ...Гунько 2021-04-14
		...
		Для Каждого СтрокаТаблицы Из Найденные Цикл
			...
			ИначеЕсли Не НайденВПрограмме Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<Отчет ""%1"" не найден в программе>';uk='<Звіт ""%1"" не знайдено в програмі>'"),
					ИмяОтчета);
			ИначеЕсли Не ДоступенПоПравам Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<Недостаточно прав для работы с отчетом ""%1"">';uk='<Недостатньо прав для роботи зі звітом ""%1"">'"),
					ИмяОтчета);
			ИначеЕсли Не ДоступенПоОпциям Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<Отчет ""%1"" отключен в настройках программы>';uk='<Звіт ""%1"" відключений в налаштуваннях програми>'"),
					ИмяОтчета);
			Иначе
	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл
		...
		Если СтрокаОтчет.ИспользуетСКД Тогда
			...
			Если ВариантыНастроек = Неопределено Тогда
				...
				Исключение
					ТекстОшибки = НСтр("ru='Не удалось прочитать список вариантов отчета ""%1"" из модуля менеджера:';uk='Не вдалося прочитати список варіантів звіту ""%1"" з модуля "
"менеджера:'");
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ОтчетМетаданные.Имя);
					ТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			...
			Если ВариантыНастроек <> Неопределено Тогда
				Для Каждого ВариантНастроекКД Из ВариантыНастроек Цикл
					...
					Если РазрешеноИндексирование И ТипЗнч(ВариантНастроекКД) = Тип("ВариантНастроекКомпоновкиДанных") Тогда
						...
						Исключение
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Не удалось прочитать настройки варианта ""%1"" отчета ""%2"":';uk='Не вдалося прочитати налаштування варіанту ""%1"" звіту ""%2"":'"),
...
"так же числится как старое имя ""%4"" (актуальное имя ""%5"").';uk='Помилка реєстрації змін імені варіанту звіту ""%1"":"
"Актуальне ім`я варіанту ""%2"" (старе ім`я ""%3"")"
"так само числиться як старе ім`я ""%4"" (актуальне ім`я ""%5"").'"),
...
"варианта отчета ""%4"" (актуальное имя ""%5"").';uk='Помилка реєстрації змін імені варіанту звіту ""%1"":"
"Старе ім`я варіанту ""%2"" (актуальне ім`я ""%3"")"
"так само числиться як старе ім`я "
...
Процедура ЗаполнитьСсылкиПредопределенных() Экспорт
	ПредставлениеПроцедуры = НСтр("ru='Заполнить ссылки предопределенных вариантов отчетов';uk='Заповнити посилання визначених варіантів звітів'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура СократитьКоличествоБыстрыхНастроек(ВходящиеПараметры = Неопределено) Экспорт
	ПредставлениеПроцедуры = НСтр("ru='Сокращение количества быстрых настроек в отчетах';uk='Скорочення кількості швидких налаштувань у звітах'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Для Каждого СтрокаТаблицы Из ТаблицаОтчетов Цикл
		...
		Если ИнформацияОбОшибке <> Неопределено Тогда // Возникла проблема.
			...
			+ Символы.ПС + НСтр("ru='При уменьшении количества быстрых настроек пользовательского возникла ошибка:';uk='При зменшенні кількості швидких налаштувань користувацького виникла помилка:'")
	...
	Если Ошибок > 0 И НомерПопытки <= 30 Тогда
		...
		ТекстОшибки = ПредставлениеПроцедуры + ":"
			+ Символы.ПС + НСтр("ru='Не удалось уменьшить количество быстрых настроек %1 отчетов.';uk='Не вдалося зменшити кількість швидких налаштувань %1 звітів.'");
...
Функция ОтложенноеОбновлениеДанных(Режим, Полное)
	...
	Кэш.Вставить("Уточнение", НРег(ОтчетыКлиентСервер.ИмяВПредставление(Режим)));
	Кэш.Уточнение = Кэш.Уточнение + ", " + ?(Полное, НСтр("ru='полное';uk='повне'"), НСтр("ru='по изменениям';uk='по змінах'"));
...
Процедура ОбновитьНастройкиПредопределенных(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Обновление настроек предопределенных (%1)';uk='Оновлення налаштувань визначених (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ПометитьНаУдалениеВариантыУдаленныхОтчетов(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Удаление вариантов удаленных отчетов (%1)';uk='Видалення варіантів видалених звітів (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗарегистрироватьЗамерыВариантаКОбновлению(Кэш, Знач СтарыйКлюч, Знач АктуальныйКлюч, Знач АктуальноеНаименование)
	...
	ОбновлениеЗамера.АктуальноеНаименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Отчет ""%1"" (настройки)';uk='Звіт ""%1"" (налаштування)'"), АктуальноеНаименование);
КонецПроцедуры
...
Функция СписокГлобальныхКлючевыхОпераций()
	...
	СписокОпераций.Добавить("ПанельОтчетов.Поиск",          НСтр("ru='Панель отчетов (поиск в режиме просмотра)';uk='Панель звітів (пошук у режимі перегляду)'"));
	СписокОпераций.Добавить("ПанельОтчетов.РежимНастройки", НСтр("ru='Панель отчетов (режим настройки)';uk='Панель звітів (режим налаштування)'"));
...
Процедура ЗаписатьПараметрыВариантовОтчетов(Кэш)
	...
	КонецЕсли;
	ПредставлениеПроцедуры = НСтр("ru='Запись неразделенного кэша в константу';uk='Запис нерозділеного кэшу в константу'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗафиксироватьТекущуюВерсиюРасширений(Кэш)
	...
	ПредставлениеПроцедуры = НСтр("ru='Запись регистра версий расширений';uk='Запис регістру версій розширень'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
"Наименования полей или Наименования параметров и отборов.';uk='Для варіанта ""%1"" звіту ""%2"" не заповнені налаштування пошуку:"
"Найменування полів або Найменування параметрів і відборів.'"),
			ВариантОбъект.КлючВарианта,
			ВариантОбъект.Отчет);
		Если ЭтоПредопределенный Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС
				+ НСтр("ru='Подробнее - см. процедуру ""НастроитьВариантыОтчетов"" модуля ""ВариантыОтчетовПереопределяемый"".';uk='Докладніше - див. процедуру ""НастроитьВариантыОтчетов"" модуля ""ВариантыОтчетовПереопределяемый"".'");
	...
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		Если ТипЗнч(ВариантОбъект) = Тип("СправочникОбъект.ПредопределенныеВариантыОтчетов")
			...
			Текст = НСтр("ru='Не удалось прочитать настройки предопределенного варианта отчета ""%1"".';uk='Не вдалося прочитати налаштування визначеного варіанту звіту ""%1"".'");
...
Функция УменьшитьКоличествоБыстрыхНастроек(ВариантОбъект, ОтчетОбъект)
	...
	НастройкиКД = ВариантОбъект.Настройки.Получить();
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		ТекстОшибки = НСтр("ru='Обнаружены пустые настройки пользовательского варианта ""%1"" отчета ""%2"".';uk='Виявлені порожні налаштування користувацького варіанта ""%1"" звіту ""%2"".'");
		ОшибкаПоВарианту(ВариантОбъект.Ссылка, ТекстОшибки, ВариантОбъект.КлючВарианта, ВариантОбъект.Отчет);
	...
	Если ОтчетыСНастройками.Найти(ОтчетСсылка) <> Неопределено Тогда
		Если ОтчетОбъект = Неопределено Тогда
			...
			Если Подключение.Успех Тогда
				ОтчетОбъект = Подключение.Объект;
			Иначе
				Текст = НСтр("ru='Не удалось получить настройки отчета ""%1"":';uk='Не вдалося одержати налаштування звіту ""%1"":'") + Символы.ПС + Подключение.ТекстОшибки;
...
Функция СообщениеДанныеЕщеОбновляются() Экспорт
	Возврат НСтр("ru='Отчет может содержать некорректные данные, так как не завершен переход на новую версию программы. Если отчет долгое время недоступен, необходимо обратиться к администратору.';uk='Звіт може містити некоректні дані, оскільки не завершений перехід на нову версію програми. Якщо звіт довгий час недоступний, необхідно звернутися до адміністратора.'");
КонецФункции
