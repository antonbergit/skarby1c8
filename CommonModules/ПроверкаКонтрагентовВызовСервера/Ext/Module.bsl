...
Процедура ПроверитьКонтрагентаПриИзменении(ПараметрыЗапуска) Экспорт  
	...
	Попытка
		...
		ФоновыеЗадания.Выполнить("ПроверкаКонтрагентов.ПроверитьКонтрагентаФоновоеЗадание", 
			Параметры, ПараметрыЗапуска.ИНН + " " + ПараметрыЗапуска.КПП, НСтр("ru='Проверка контрагента';uk='Проверка контрагента'"));
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Проверка контрагентов.Запуск проверки при изменении реквизитов контрагентов';uk='Проверка контрагентов.Запуск проверки при изменении реквизитов контрагентов'", КодОсновногоЯзыка),
...
Функция РезультатПроверкиПараметровДоступа() Экспорт
	...
	Если ПроверкаКонтрагентов.ЕстьДоступКВебСервисуФНС() Тогда
		...
		ТекстПредупреждения = НСтр("ru='Проверка доступа к веб-сервису успешно пройдена';uk='Проверка доступа к веб-сервису успешно пройдена'");
		
	Иначе
		
		ПредупреждениеПроОтсутствиеДоступа = Новый ФорматированнаяСтрока(НСтр("ru='Доступ к веб-сервису отсутствует';uk='Доступ к веб-сервису отсутствует'"));
		
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПолучениеФайловИзИнтернета") Тогда
			ПереходКНастройкамПроксиСервера = Новый ФорматированнаяСтрока(
				НСтр("ru='Настроить параметры прокси-сервера';uk='Налаштувати параметри проксі-сервера'"),,,, "e1cib/command/ОбщаяКоманда.ПереходКНастройкамДоступаКВебСервису");
		Иначе
