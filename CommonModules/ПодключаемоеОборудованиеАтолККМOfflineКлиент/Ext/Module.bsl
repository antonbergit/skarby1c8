...
Функция ЗаполнитьТранзакцию(Строка, Транзакция, ФорматОбмена, ТекстОшибки)
	...
	Попытка
		...
		Для Индекс = 8 По КоличествоПолей Цикл
			...
			ПолеОшибки = Нстр("ru='Поле №%1% (%1%)';uk='Поле №%1% (%1%)'");
...
Функция РазобратьРасширенныйОтчет(ТекстовыйДокумент, ФорматОбмена, ВыходныеПараметры)
	...
	ИначеЕсли Строка = "@" Тогда
		ТекстОшибки = НСтр("ru='Операция прервана. Отчет уже был загружен!';uk='Операція перервана. Звіт вже був завантажений!'");
		Результат = Ложь;
	Иначе
...
Функция РазобратьОтчет(ТекстовыйДокумент, Разделитель, ВыходныеПараметры)
	...
	Если ТекущаяСтрока = "#" Тогда
		...
		Пока Не ПустаяСтрока(ТекущаяСтрока) Цикл
			...
			Попытка
				...
				ПолеОшибки = Нстр("ru='Номер документа (6)';uk='Номер документа (6)'");
				НомерДокумента     = Число(НомерДокументаСтр);
				ПолеОшибки = Нстр("ru='Номер смены (14)';uk='Номер зміни (14)'");
				НомерСмены         = Число(НомерСменыСтр);
			Исключение
				ТекстОшибки = НСтр("ru='Неверный формат файла. Невозможно преобразовать к числу поле:';uk='Невірний формат файлу. Неможливо перетворити до числа поле:'") + Символы.НПП + ПолеОшибки;
				Результат = Ложь;
				Прервать;
			КонецПопытки;
			...
			Если ТипТранзакции =  1 Тогда
				...
				ТекстОшибки = НСтр("ru='Регистрация продаж без учета кода товара не допускается';uk='Реєстрація продажів без обліку коду товару не допускається'");
				Результат = Ложь;
				Прервать;
			ИначеЕсли ТипТранзакции = 11 Тогда
				...
				Попытка
					...
					ПолеОшибки = Нстр("ru='Сумма продажи (16)';uk='Сума продажу (16)'");
	...
	ИначеЕсли ТекущаяСтрока = "@" Тогда
		...
		Результат = Ложь;
	Иначе
		ТекстОшибки = НСтр("ru='Неверный формат данных или данные отсутствуют.';uk='Невірний формат даних або дані відсутні.'");
		Результат = Ложь;
	КонецЕсли;
