...
//	Возврат Результат;
	...
	Если ПустаяСтрока(Текст) Тогда
		Сообщить("Не заполнен текст сообщения!", СтатусСообщения.ОченьВажное);
	КонецЕсли;	
	
	// подключение к базе MySQL
	Соединение = Новый COMОбъект("ADODB.Connection");
	СтрокаСоединение = "DRIVER={MySQL ODBC 5.1 Driver};SERVER=77.120.116.10;DataBase=users;UID=" + Логин +";PWD=" + Пароль; //{MySQL ODBC 3.51 Driver}
	Попытка
   ...
   Если НомераПолучателей.Количество() = 0 Или ПустаяСтрока(Текст) Тогда
	...
	//КонецЕсли;
		...
		//Сообщить(ИндивидуальныйТекстСообщения);
		//Сообщить("-------------------------");
        Соединение.Execute("insert into " + Логин +"(number,sign,message) values('" + НомераПолучателей + "','" + Пароль +"','"+ Текст + "')"); 
...
"%1 (код ошибки: %2)';uk='Не удалось получить статус доставки SMS (smsid: ""%3""):"
...
Функция Разрешения() Экспорт
	...
	Порт = Неопределено;
	Описание = НСтр("ru='Отправка SMS через ""СМС-Услуги"".';uk='Отправка SMS через ""СМС-Услуги"".'");
