...
//
Процедура ТестНастроекСПроверкойСертификата(Сертификат, ВладелецФормы, ОбработкаЗавершения = Неопределено,
	ПроверитьАвторизацию = Ложь, ПроверкаПриВыборе = Ложь, БезПодтверждения = Ложь) Экспорт
	...
	ЗаголовокФормы = НСтр("ru='Проверка сертификата %1';uk='Проверка сертификата %1'");
...
Процедура ОбработатьОтклонениеАннулированиеЭД(СсылкаНаЭД, ПараметрыЭД, ОтклонитьАннулирование = Ложь) Экспорт
	...
	Если ПараметрыЭД.Отклонить Тогда
		...
		Если ОтклонитьАннулирование Тогда
			Заголовок = НСтр("ru='Укажите причины отклонения предложения об аннулировании';uk='Укажите причины отклонения предложения об аннулировании'");
		Иначе
	...
	Иначе
		...
		Заголовок = НСтр("ru='Укажите причины аннулирования документа';uk='Укажите причины аннулирования документа'");
	КонецЕсли;
	...
	Если ВходящаяСтруктура.Свойство("СтруктСтруктурМассивовЭДИСертификатов", СтруктСтруктурМассивовЭДИСертификатов)
		...
		Для Каждого Элемент Из СтруктСтруктурМассивовЭДИСертификатов Цикл
			...
			Если Структура.Свойство("СоответствиеЭдИДд", СоответствиеЭдИДд)
				...
				Иначе
					ПредставлениеЭД = НСтр("ru='Электронные документы (%1)';uk='Электронные документы (%1)'");
...
Процедура ОтправитьПолучитьЭДВыполнить(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекстСообщения = НСтр("ru='Выполняется получение информации о приглашениях. Подождите...';uk='Выполняется получение информации о приглашениях. Подождите...'");
	Состояние(НСтр("ru='Получение.';uk='Получение.'"), , ТекстСообщения);
	...
	ТекстСообщения = НСтр("ru='Выполняется отправка и получение пакетов электронных документов. Подождите...';uk='Выполняется отправка и получение пакетов электронных документов. Подождите...'");
	Состояние(НСтр("ru='Отправка и получение.';uk='Отправка и получение.'"), , ТекстСообщения);
	...
	ТекстСообщения = НСтр("ru='Выполняется распаковка пакетов электронных документов. Подождите...';uk='Выполняется распаковка пакетов электронных документов. Подождите...'");
	Состояние(НСтр("ru='Распаковка.';uk='Распаковка.'"), , ТекстСообщения);
...
Процедура ПолучитьМаркерПродолжить(Отпечатки, ДополнительныеПараметры) Экспорт
	...
	Если СвойстваСертификата.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ru='Нет доступных сертификатов. Выполнение операции %1 прервано.';uk='Нет доступных сертификатов. Выполнение операции %1 прервано.'");
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция",   НСтр("ru='Аутентификация в сервисе ЭДО';uk='Аутентификация в сервисе ЭДО'"));
	ОписаниеДанных.Вставить("ОтборСертификатов",   МассивСертификатов);
...
Процедура РасшифроватьЭлектронныеДокументыВнутреннийЦиклНачало(Контекст)
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция",            НСтр("ru='Расшифровка электронного документа';uk='Расшифровка электронного документа'"));
...
Процедура РасшифроватьЭлектронныеДокументыВнутреннийЦиклПослеРасшифровкиЭД(РезультатВыполнения, Контекст) Экспорт
	...
	Если Не РезультатВыполнения.Успех Тогда
		Контекст.СтруктураПакетаЭД.Вставить("ОписаниеОшибки", НСтр("ru='Не удалось расшифровать электронный документ из пакета.';uk='Не удалось расшифровать электронный документ из пакета.'"));
		ОбработатьПакетыЭДЦиклНачало(Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ПрочитатьСертификатыПодписейПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		Контекст.СтруктураПакетаЭД.Вставить("ОписаниеОшибки", НСтр("ru='Не удалось получить МенеджерКриптографии.';uk='Не удалось получить МенеджерКриптографии.'"));
		ОбработатьПакетыЭДЦиклНачало(Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ПрочитатьСертификатыПодписейВнутреннийЦиклПослеОшибкиВыгрузкиСертификата(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	ШаблонСообщения = НСтр("ru='Выгрузка сертификата: %1';uk='Выгрузка сертификата: %1'");
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ИнформацияОбОшибке);
...
Процедура ПрочитатьСертификатыПодписейВнутреннийЦиклПослеВыгрузкиСертификата(ДанныеСертификата, Контекст) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Контекст.Подпись.АдресЭД) Тогда
		Контекст.СтруктураПакетаЭД.Вставить("ОписаниеОшибки", НСтр("ru='Не удалось найти исходные данные для проверки ответной подписи.';uk='Не удалось найти исходные данные для проверки ответной подписи.'"));
		ОбработатьПакетыЭДЦиклНачало(Контекст);
		Возврат;
	КонецЕсли;
...
"%2.'");
	...
	ШаблонОповещения = НСтр("ru='Распаковано пакетов: %1.';uk='Распаковано пакетов: %1.'");
		...
		Если СоответствиеДанныхПакетов.Количество() > Параметры.ИндексПеребораПакетов Тогда
			...
			Если Параметры.ИндексПеребораДанных + 1 < МассивДанныхЭД.Количество() Тогда
				...
				Исключение
					...
					ТекстСообщения = НСтр("ru='При шифровании данных произошла ошибка: %1';uk='При шифровании данных произошла ошибка: %1'");
...
"установлен ответственный ""%Ответственный%""';uk='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных эл.документов"
"установлен ответственный ""%Ответственный%""'");
		...
		ТекстЗаголовка = НСтр("ru='Ответственный ""%Ответственный%"" установлен';uk='Ответственный ""%Ответственный%"" установлен'");
		ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%Ответственный%", Ответственный);
		ПоказатьОповещениеПользователя(ТекстЗаголовка, , ТекстСообщения, БиблиотекаКартинок.Информация32);
		
	Иначе
		
		ТекстСообщения = НСтр("ru='Ответственный ""%Ответственный%"" не установлен ни для одного эл.документа.';uk='Ответственный ""%Ответственный%"" не установлен ни для одного эл.документа.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Ответственный%", Ответственный);
		ТекстЗаголовка = НСтр("ru='Ответственный ""%Ответственный%"" не установлен';uk='Ответственный ""%Ответственный%"" не установлен'");
		ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка, "%Ответственный%", Ответственный);
		ПоказатьОповещениеПользователя(ТекстЗаголовка,, ТекстСообщения, БиблиотекаКартинок.Информация32);
...
Процедура СформироватьЭД(ПараметрКоманды, ПараметрыВыполненияКоманды) Экспорт
	...
	Если ПараметрКоманды.Количество() > 1 Тогда
		...
		ТекстВопроса = НСтр("ru='Создать электронные документы для выделенных элементов?';uk='Создать электронные документы для выделенных элементов?'");
...
"Если электронных документов нет, они будут созданы автоматически.';uk='Отправить электронные документы выделенных элементов?"
