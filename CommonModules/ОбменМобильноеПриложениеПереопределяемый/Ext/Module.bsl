...
Функция ЭтоВерсияСоСтарымОбменом(УзелОбмена) Экспорт
	...
	Если ЗначениеЗаполнено(УзелОбмена.ВерсияМобильногоПриложения) Тогда
		Возврат ОбщегоНазначенияКлиентСервер.СравнитьВерсииБезНомераСборки(ВерсияМобильногоПриложенияСНовымОбменом(), УзелОбмена.ВерсияМобильногоПриложения) > 0;
	Иначе
		Возврат Истина;
	КонецЕсли;
...
Функция ПолучитьОбъектXDTO(Данные, КоличествоОбъектов)
	...
	ИначеЕсли ТипЗнч(Данные) = Тип("СправочникОбъект.Номенклатура")
		...
		Если ЗначениеЗаполнено(Данные.Ссылка.ФайлКартинки) Тогда
			ПередаваемыйОбъект.Image = ПолучитьКартинку(Данные.Ссылка);
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Данные) = Тип("ДокументОбъект.ПоступлениеВКассу") Тогда
		...
		Если ЗначениеЗаполнено(Данные.ДокументОснование) Тогда
			Попытка
				...
				ИначеЕсли ТипЗнч(Данные.ДокументОснование) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
	...
	ИначеЕсли ТипЗнч(Данные) = Тип("ДокументОбъект.РасходИзКассы") Тогда
...
//
Процедура ЗаписатьОстаткиВОчередьСообщенийОбмена(УзелОбмена, НомерСообщенияОчереди) Экспорт
...
// Записывает роли.
//
Процедура ЗаписатьРолиВОчередьСообщенийОбмена(УзелОбмена, НомерСообщенияОчереди) Экспорт
...
// Сведения о компании.
//
Процедура ЗаписатьСведенияОКомпанииВОчередьСообщенийОбмена(УзелОбмена, НомерСообщенияОчереди) Экспорт
...
Процедура ЗаписатьСведенияОНастрокахНалогооблаженияВОчередьСообщенийОбмена(УзелОбмена, НомерСообщенияОчереди) Экспорт
...
Процедура ЗаполнитьОсновныеРеквизитыДокумента(Объект, ОбъектXDTO, НужноЗаписыватьОбъект)
	...
	Если Объект.Дата <> ОбъектXDTO.Date Тогда
		...
		НужноЗаписыватьОбъект = Истина;
	КонецЕсли;
	Если Объект.СуммаДокумента <> ОбъектXDTO.Total Тогда
...
Функция НайтиСоздатьПоступлениеВКассу(УзелОбмена, ОбъектXDTO, ДокументыДляОтложенногоПроведения, ЭтоНовыйОбмен)
	...
	ДокументОснование = НайтиСоздатьРасходнаяНакладная(УзелОбмена, ОбъектXDTO.Invoice, ДокументыДляОтложенногоПроведения, ЭтоНовыйОбмен);
	...
	Если Объект.ДокументОснование <> ДокументОснование Тогда
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.Ссылка <> ПланыОбмена.МобильноеПриложение.ЭтотУзел() Тогда
			Если ТипЗнч(Источник) = Тип("РегистрСведенийНаборЗаписей.ЦеныНоменклатуры") Тогда
				Если Источник.Отбор.ВидЦен.Значение = ВидЦенПродажиПоУмолчанию Тогда
					МассивУзловДляРегистрации.Добавить(Выборка.Ссылка);
				КонецЕсли;
