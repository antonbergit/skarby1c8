...
Процедура ПолучитьУникальныйИдентификаторАбонента(
	...
	Если ТипЗнч(СертификатЭП) <> Тип("СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования")
		...
		ВызватьИсключение НСтр("ru='Некорректное значение параметра ""СертификатЭП"".';uk='Некорректное значение параметра ""СертификатЭП"".'");
	КонецЕсли;
...
Процедура ЗарегистрироватьСертификатАбонента(
	...
	ИначеЕсли ТипЗнч(ИдентификаторАбонента) <> Тип("Строка") Или ПустаяСтрока(ИдентификаторАбонента) Тогда
		ВызватьИсключение НСтр("ru='Некорректное значение параметра ""ИдентификаторАбонента"".';uk='Некорректное значение параметра ""ИдентификаторАбонента"".'");
	КонецЕсли;
...
Процедура СтартоватьМеханизмРаботыСОператоромЭДО(
	...
	Если Не ЗначениеЗаполнено(СертификатЭП) Тогда
		...
		ТекстОшибки = НСтр("ru='""Сертификат абонента""';uk='""Сертификат абонента""'");
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		...
		Если НЕ ПустаяСтрока(ТекстОшибки) Тогда
			ТекстОшибки = ТекстОшибки + " " + НСтр("ru='и';uk='і'") + " ";
		КонецЕсли;
		
		ТекстОшибки = ТекстОшибки + НСтр("ru='""Организация""';uk='""Организация""'");
		КоличествоОшибок = КоличествоОшибок + 1;
	...
	Если КоличествоОшибок > 0 Тогда
		...
		Если КоличествоОшибок = 1 Тогда
			ДопТекст = НСтр("ru='Пожалуйста, заполните поле';uk='Пожалуйста, заполните поле'") + " ";
		Иначе
			ДопТекст = НСтр("ru='Пожалуйста, заполните поля';uk='Пожалуйста, заполните поля'") + " ";
		КонецЕсли;
		...
		Если ВариантБизнесПроцесса = "taxcomGetID" Тогда
			ТекстСообщения = ТекстСообщения
				+ " " + НСтр("ru='до получения уникального идентификатора участника обмена ЭД';uk='до получения уникального идентификатора участника обмена ЭД'");
		ИначеЕсли ВариантБизнесПроцесса = "taxcomPrivat" Тогда
			ТекстСообщения = ТекстСообщения + " " + НСтр("ru='до перехода в личный кабинет';uk='до перехода в личный кабинет'");
		КонецЕсли;
...
"Продолжить?';uk='Получение уникального идентификатора участника обмена ЭД будет доступно"
"после подключения к сервису Интернет-поддержки пользователей и проверки"
"подлинности владельца указанного в соглашении сертификата абонента."
"Продолжить?'");
		Иначе
...
"после подключения к сервису Интернет-поддержки пользователей и проверки"
"подлинности владельца указанного в соглашении сертификата абонента."
"Продолжить?';uk='Добавление нового сертификата в соглашение будет выполнено"
"после подключения к сервису Интернет-поддержки пользователей и проверки"
"подлинности владельца указанного в соглашении сертификата абонента."
"Продолжить?'");
		КонецЕсли;
...
"после подключения к сервису Интернет-поддержки пользователей и проверки"
"подлинности владельца указанного в соглашении сертификата абонента."
"Продолжить?';uk='Вход в личный кабинет участника обмена ЭД будет доступен"
"после подключения к сервису Интернет-поддержки пользователей и проверки"
"подлинности владельца указанного в соглашении сертификата абонента."
"Продолжить?'");
...
"Указан неверный вариант бизнес-процесса (%1).';uk='Ошибка встраивания механизма Интернет-поддержки пользователей."
...
"Подробнее см. в журнале регистрации.';uk='Ошибка при проверке пароля сертификата."
"Подробнее см. в журнале регистрации.'");
	...
	Если ПараметрыДляРасшифрования.markerED = Неопределено Тогда
		...
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru='Ошибка при проверке подлинности владельца сертификата. Отсутствует маркер аутентификации (markerED)';uk='Ошибка при проверке подлинности владельца сертификата. Отсутствует маркер аутентификации (markerED)'"));
...
"%1';uk='Ошибка при проверке подлинности владельца сертификата. Не удалось получить двоичные данные маркера (markerED)."
"%1'"),
	...
	Иначе
		...
		ИнтернетПоддержкаПользователейКлиент.ЗавершитьБизнесПроцесс(КонтекстВзаимодействия);
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru='Ошибка при проверке подлинности владельца сертификата. Не указан сертификат (IDCertificateED, IDCertificateED_Dop)';uk='Ошибка при проверке подлинности владельца сертификата. Не указан сертификат (IDCertificateED, IDCertificateED_Dop)'"));
		ПоказатьПредупреждение(, СообщениеОбОшибкеДляПользователя);
		Возврат;
	...
	ОписаниеДанных.Вставить("Операция", НСтр("ru='Проверка пароля сертификата';uk='Проверка пароля сертификата'"));
...
Процедура ПоказатьПричинуОтклоненияЗаявкиЭДО(КонтекстВзаимодействия) Экспорт
	...
	Если НЕ ПустаяСтрока(ОписаниеПричины.КодОшибки) Тогда
		ТекстСообщения = НСтр("ru='Код ошибки: %1';uk='Код помилки: %1'");
	...
	Если ПустаяСтрока(ОписаниеПричины.ТекстОшибки) Тогда
		ТекстСообщения = НСтр("ru='Неизвестная ошибка. Обратитесь в службу техподдержки.';uk='Неизвестная ошибка. Обратитесь в службу техподдержки.'");
	КонецЕсли;
...
""
"- отпечаток сертификата: %1';uk='Параметры участника обмена ЭД:"
""
...
"- описание ошибки: %2';uk='- код ошибки: %1,"
"- описание ошибки: %2'");
