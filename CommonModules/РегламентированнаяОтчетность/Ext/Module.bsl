...
// Возвращает таблицу, содержащую регламентированные отчеты, ограниченные переданным отбором.
//
// Параметры:
//   Отбор - Массив - массив имен отчетов.
//
// Возвращаемое значение:
//   ТаблицаЗначений - регламентированные отчеты, ограниченные переданным отбором.
//
Функция ТаблицаРегламентированныеОтчетыПоИмени(Отбор) Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	  ...
	  |	Справочник.РегламентированныеОтчеты КАК РегламентированныеОтчеты
	  |ГДЕ
	  |	РегламентированныеОтчеты.Наименование В(&Отбор)
	  |	И НЕ РегламентированныеОтчеты.ЭтоГруппа");
...
Функция СвойстваРегламентированногоОтчета(Ссылка) Экспорт
	...
	ПараметрыОтчета.Вставить("ПредставлениеВида",        Неопределено);
...
Функция КоличествоОбъектовНеЗаписанныхВРегистр(ПараметрыЗапуска) Экспорт
	...
	НаименованиеЗадания = НСтр("ru='Проверка полноты данных журнала регламентированной отчетности';uk='Проверка полноты данных журнала регламентированной отчетности'");
...
Функция ДобавлениеОбъектовОтсутствующихВРегистре(ПараметрыЗапуска) Экспорт
	...
	НаименованиеЗадания = НСтр("ru='Заполнение данных журнала регламентированной отчетности';uk='Заполнение данных журнала регламентированной отчетности'");
...
Процедура ВыполнитьДобавлениеВРегистр(Параметры, АдресХранилища) Экспорт
	...
	Исключение
		...
		НСтр("ru='Заполнение данных журнала регламентированной отчетности';uk='Заполнение данных журнала регламентированной отчетности'"), 
...
Процедура ОбработатьСобытие1СОтчетности(ИмяСобытия, СсылкаНаИсточник = Неопределено, ДопПараметрСтруктура = Неопределено) Экспорт
	...
	Если ИмяСобытия = НСтр("ru='Регламентированный отчет. Создание';uk='Регламентированный отчет. Создание'") Тогда
		
		СтатистикаБРОКлиентСервер.ДобавитьСобытие(ИмяСобытия + ". " + КомментарийКСозданиюРеглОтчета(СсылкаНаИсточник));
		
	ИначеЕсли ИмяСобытия = НСтр("ru='Регламентированный отчет. Выгрузка';uk='Регламентированный отчет. Выгрузка'") Тогда
		
		СтатистикаБРОКлиентСервер.ДобавитьСобытие(ИмяСобытия + ". " + КомментарийКВыгрузкеРеглОтчета(СсылкаНаИсточник));
		
	ИначеЕсли ИмяСобытия = НСтр("ru='Регламентированный отчет. Проверка выгрузки';uk='Регламентированный отчет. Проверка выгрузки'") Тогда
...
Функция ВывестиСведенияСлужебнойЧасти30(Текст, Организация = Неопределено, Знач ДатаДок = Неопределено, Знач ОргИНН = Неопределено, Знач ОргКПП = Неопределено, Знач КодИнсп = Неопределено, ТипИнф = "ОТЧЕТНОСТЬНП", ВерсияФормата = "3.00") Экспорт
	...
	Если ПустаяСтрока(ОргИНН) Тогда
		...
		Сообщение.Текст = НСтр("ru='Невозможно определить ИНН организации! Выгрузка прервана!';uk='Невозможно определить ИНН организации! Выгрузка прервана!'");
	...
	Если ПустаяСтрока(КодИНСП) Тогда
		...
		Сообщение.Текст = НСтр("ru='Невозможно определить код налоговой инспекции! Выгрузка прервана!';uk='Невозможно определить код налоговой инспекции! Выгрузка прервана!'");
...
Функция ВывестиСведенияОбОтправителе30(Текст, Организация = Неопределено, Знач ОргНазв = Неопределено, Знач ОргТелефон = Неопределено, Знач ОргБухгалтер = Неопределено, ВыгрузкаДекларацииПоАлкоголю = Ложь) Экспорт
	...
	Если ПустаяСтрока(ИНН) Тогда
		...
		Сообщение.Текст = НСтр("ru='Невозможно определить вид налогоплательщика! Выгрузка прервана!';uk='Невозможно определить вид налогоплательщика! Выгрузка прервана!'");
	...
	Если ПустаяСтрока(ОргНазв) Тогда
		...
		Сообщение.Текст = НСтр("ru='Невозможно определить полное название организации! Выгрузка прервана!';uk='Невозможно определить полное название организации! Выгрузка прервана!'");
	...
	Если КатегорияНП = 1 И ПустаяСтрока(ОргБухгалтер) Тогда
		...
		Сообщение.Текст = НСтр("ru='Невозможно определить ФИО главного бухгалтера организации! Выгрузка прервана!';uk='Невозможно определить ФИО главного бухгалтера организации! Выгрузка прервана!'");
	...
	Иначе
		...
		Если ПустаяСтрока(ФИООтпрФЛ) Тогда
			...
			Сообщение.Текст = НСтр("ru='Невозможно определить ФИО главного бухгалтера и ФИО индивидуального предпринимателя! Выгрузка прервана!';uk='Невозможно определить ФИО главного бухгалтера и ФИО индивидуального предпринимателя! Выгрузка прервана!'");
	...
	Для Каждого Стр Из ТП Цикл
		...
		Если БылаЦифра Тогда
			...
			Исключение
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибки преобразования номера из окончания: %1 === %2';uk='Помилки перетворення номеру з закінчення: %1 === %2'"), Корень, Номер);
...
Функция ПолучитьПоКодамРегистрациюВИФНС(Организация, КодНО, КПП = Неопределено) Экспорт
	...
	Если КПП <> Неопределено Тогда
		ТекстЗапроса = ТекстЗапроса + " И РегистрацииВНалоговомОргане.КодПоЕДРПОУ = &КодПоЕДРПОУ";
		Запрос.УстановитьПараметр("КодПоЕДРПОУ", КПП);
	КонецЕсли;
...
Процедура ПереносДанныхЭДОПриОбновлении10158() Экспорт
	...
	ТекстСообщения = НСтр("ru='Обновление данных информационной базы по ЭДО с КО';uk='Обновление данных информационной базы по ЭДО с КО'");
...
Процедура ОбновитьРеквизитВидОбменаСКонтролирующимиОрганамиСправочникаОрганизации() Экспорт
    ...
    Пока Выборка.Следующий() Цикл
        ...
        Попытка
            ...
            Объект.Записать();
        Исключение
            ТекстСообщения = НСтр("ru='Электронный документооборот с контролирующими органами';uk='Электронный документооборот с контролирующими органами'");
...
Функция ВозможноВосстановитьДанныеРеглОтчетаСозданногоВОбычномПриложении(ОбъектРегОтчет, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если СокрЛП(ОбъектРегОтчет.НаименованиеОтчета) = "Доходы, выплаченные иностранным организациям" Тогда
		...
		Если СокрЛП(ОбъектРегОтчет.ВыбраннаяФорма) = "ФормаОтчета2016Кв1" Тогда
			...
			Описание = НСтр("ru='Отчет сохранен в конфигурации предыдущей версии с неуникальными значениями показателя ""Уникальный номер"" в Разделе 3.1 и(или) неуникальными сочетаниями значений показателей ""Уникальный номер"" и ""Порядковый номер"" в Разделе 3.2.';uk='Отчет сохранен в конфигурации предыдущей версии с неуникальными значениями показателя ""Уникальный номер"" в Разделе 3.1 и(или) неуникальными сочетаниями значений показателей ""Уникальный номер"" и ""Порядковый номер"" в Разделе 3.2.'");
			...
			Описание = НСтр("ru='В отчете, сохраненном в конфигурации предыдущей версии, не соблюдена последовательность в нумерации значений показателя ""Уникальный номер"" в Разделе 3.1 и(или) сочетаний значений показателей ""Уникальный номер"" и ""Порядковый номер"" в Разделе 3.2 и(или) Разделе 3.3.';uk='В отчете, сохраненном в конфигурации предыдущей версии, не соблюдена последовательность в нумерации значений показателя ""Уникальный номер"" в Разделе 3.1 и(или) сочетаний значений показателей ""Уникальный номер"" и ""Порядковый номер"" в Разделе 3.2 и(или) Разделе 3.3.'");
...
// Формирует автоматически в фоне формы 6-НДФЛ для выбранных налоговых органов.
//
// Параметры:
//   Организация - СправочникСсылка.Организации - ссылка на организацию.
//   ДатаНачалаПериодаОтчета - Дата - дата начала периода отчета.
//   ДатаКонцаПериодаОтчета - Дата - дата конца периода отчета.
//   НалоговыеОрганы - СписокЗначений - список налоговых органов.
//   АдресВоВременномХранилищеРезультатаАвтоформирования - Строка - адрес во временном хранилище результата автоформирования.
//
Процедура СформироватьАвтоматическиРеглОтчет6НДФЛВФоне(Организация, ДатаНачалаПериодаОтчета, ДатаКонцаПериодаОтчета,
													   НалоговыеОрганы, АдресВоВременномХранилищеРезультатаАвтоформирования) Экспорт
	...
	Для Каждого НалоговыйОрган Из НалоговыеОрганы Цикл
		...
		ИнтерфейсыВзаимодействияБРО.СформироватьАвтоматическиРеглОтчет("РегламентированныйОтчет6НДФЛ",
						  											   Организация,
	...
	ПоместитьВоВременноеХранилище(ПараметрыАвтоФормирования.РезультатАвтоформирования, АдресВоВременномХранилищеРезультатаАвтоформирования);
	
КонецПроцедуры

#КонецОбласти

#Область РеглОтчет1СЗвит

Функция	ПроверитьМенеджер1С(ИсточникОтчета, ПараметрыВнешнихРегламентированныхОтчетов, ВыводитьСообщенияОбОшибках) Экспорт
	
	Отчет = РегламентированнаяОтчетность.ФормаРеглОтчета(ИсточникОтчета, , , , ПараметрыВнешнихРегламентированныхОтчетов);
...
Процедура ЗаполнитьСписокВыбораСпецРежимаНалогообложения(СписокВыбора) Экспорт
    ...
    СписокВыбора.Добавить(2, "2-сільськогосподарські підприємства, які застосовують спеціальний режим оподаткування 
	                         |діяльності у сфері сільського та лісового господарства, а також рибальства відповідно 
							 |до статті 209 Кодексу");
							  
	СписокВыбора.Добавить(3, "3-сільськогосподарські підприємства усіх форм власності, які відповідають критеріям,
	                         |визначеним статтею 209 Кодексу, але які не обрали спеціальний режим оподаткування
							 |діяльності у сфері сільського, лісового господарства та рибальства та реалізують молоко,
							 |худобу, птицю, вовну власного виробництва, а також молочні продукти, молочну сировину
							 |та м'ясопродукти, вироблені у власних переробних цехах");
								
	СписокВыбора.Добавить(4, "4-(до 01.01.2015) переробні підприємства усіх форм власності, які згідно з пунктом 1
	                         |підрозділу 2 розділу XX Кодексу за реалізовані ними молоко, молочну сировину та
							 |молочні продукти, м'ясо та м'ясопродукти, іншу продукцію переробних тварин, закуплених
							 |у живій вазі (шкури, субпродукти, м'ясо-кісткове борошно), у повному обсязі спрямовують
							 |суми податку на додану вартість до спеціального фонду державного бюджету");
...
Процедура РазбитьПечатнуюФорму(ПечФорма, СтруктураПараметровЛиста, ИмяФормы = "", мПечатныеФормы)
	...
	Если СтруктураПараметровЛиста.Свойство("АвтоМасштаб") Тогда
		АвтоМасштаб = СтруктураПараметровЛиста.УчитыватьАвтомасштаб;	
	КонецЕсли;
