...
"Зміна параметра %2 завдань з встановленим шаблоном "
"заборонена.'");
	...
	Попытка
		...
		Если Не ОбщегоНазначения.СсылкаСуществует(Идентификатор) Тогда
			ШаблонСообщения = НСтр("ru='Задание с идентификатором %1 к изменению не найдено. Область данных: %2';uk='Завдання з ідентифікатором %1 до зміни не знайдене. Область "
"даних: %2'");
...
"Видалення завдань з встановленим шаблоном заборонене.'");
...
Функция ПолучитьТекстИсключенияДублированиеЗаданийСОдинаковымКлючом() Экспорт
	...
	Возврат НСтр("ru=""Дублирование заданий с одинаковым значения поля 'Ключ' не допустимо."";uk=""Дублювання завдань з однаковим значенням поля 'Ключ' не "
"припустимо.""");
...
"Отсутствует свойство %2.';uk='Передано неприпустимий опис відбору в колекції опису відбору "
"%1."
"Відсутня властивість %2.'");
				...
				Если ВидыСравнения.Получить(ОписаниеОтбора.ВидСравнения) = Неопределено Тогда
					ШаблонСообщения = НСтр("ru='Передан недопустимый вид сравнения в описании отбора в коллекции описания отбора %1';uk='Переданий некоректний вид порівняння в описі відбору в колекції "
"опису відбору %1'");
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, КлючИЗначение.Ключ);
					ВызватьИсключение(ТекстСообщения);
				КонецЕсли;
...
Процедура ПроверитьНаличиеРегистрацииОбработчикаЗадания(Знач ИмяМетода)
	...
	Если ОчередьЗаданийСлужебныйПовтИсп.СоответствиеИменМетодовПсевдонимам().Получить(ВРег(ИмяМетода)) = Неопределено Тогда
		ШаблонСообщения = НСтр("ru='Не зарегистрирован псевдоним метода %1 для использования в качестве обработчика задания очереди.';uk='Не зареєстрований псевдонім методу %1 для використання в "
"якості обробника завдання черги.'");
...
Функция ОписаниеЗаданияПоИдентификатору(Знач Идентификатор)
	...
	Если Не ЗначениеЗаполнено(Идентификатор) ИЛИ Не ОбщегоНазначения.СсылкаСуществует(Идентификатор) Тогда
		ШаблонСообщения = НСтр("ru='Передано недопустимое значение %1 параметра задания Идентификатор';uk='Передано неприпустиме значення %1 параметра завдання "
"Ідентифікатор'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Идентификатор);
		ВызватьИсключение(ТекстСообщения);
	КонецЕсли;
