...
Процедура ОбменСКонтрагентамиВладелецПриЗаписи(Источник, Отказ) Экспорт
	...
	Если Не Источник.ДополнительныеСвойства.Свойство("ЭтоНовыйОбъект") Тогда
		...
		Если НЕ РедактированиеРазрешено Тогда
			ТекстСообщения = НСтр("ru='Существует электронный документ. Изменение ключевых реквизитов документа запрещено.';uk='Существует электронный документ. Изменение ключевых реквизитов документа запрещено.'");
...
Процедура ЗаполнитьЗначениеРеквизитаОрганизацияПриОдноФирменномУчете(Источник, ДанныеЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если НЕ ИспользуетсяНесколькоОрганизаций И ОбщегоНазначения.ЕстьРеквизитОбъекта("Организация", Источник.Метаданные())
		...
		Если Не ЗначениеЗаполнено(Источник.Организация) Тогда
			ВызватьИсключение НСтр("ru='Не удалось определить организацию.';uk='Не удалось определить организацию.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура РегламентноеПолучениеНовыхЭД() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		ВызватьИсключение
			НСтр("ru='Работа регламентного задания в режиме сервиса не предусмотрена.';uk='Работа регламентного задания в режиме сервиса не предусмотрена.'");
	КонецЕсли;
	...
	Текст = НСтр("ru='Начато регламентное задание по получению новых эл.документов.';uk='Начато регламентное задание по получению новых эл.документов.'");
...
"Получено пакетов: %1. Распаковано: %2.';uk='Закончено регламентное задание."
"Получено пакетов: %1. Распаковано: %2.'");
	...
	Исключение
		...
		Операция = НСтр("ru='Регламентное получение электронных документов';uk='Регламентное получение электронных документов'");
		
		ШаблонСообщения = НСтр("ru='При процессе выполнении произошла ошибка: %1';uk='При процессе выполнении произошла ошибка: %1'");
...
Процедура РегламентнаяОтправкаОформленныхЭД() Экспорт
	...
	Текст = НСтр("ru='Начато регламентное задание по отправке оформленных эл.документов.';uk='Начато регламентное задание по отправке оформленных эл.документов.'");
	...
	Попытка
		...
		Текст = НСтр("ru='Закончено регламентное задание. Отправлено пакетов: %КоличествоПакетов%.';uk='Закончено регламентное задание. Отправлено пакетов: %КоличествоПакетов%.'");
...
"%ДополнительноеОписание%';uk='Во время регламентной отправки оформленных эл.документов произошла ошибка."
"Дополнительное описание:"
...
Процедура ПроверитьНаличиеНовыхЭД() Экспорт
	...
	ВидОперации = НСтр("ru='Получение информации о новых электронных документах';uk='Получение информации о новых электронных документах'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Ошибка выполнения команды сервиса.';uk='Ошибка выполнения команды сервиса.'");
		...
		Если Не Ответ.КодСостояния = 200 Тогда
			...
			ШаблонСообщения = НСтр("ru='Код ответа сервиса %1';uk='Код ответа сервиса %1'");
...
"%1';uk='Тест. Проверка установки FTP соединения."
"%1'");
	Иначе
		ШаблонСообщения = "%1";
	КонецЕсли;
...
"%1';uk='Проверка каталога исходящих документов."
"%1'");
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, РезультатТеста);
	ТекстСообщения = НСтр("ru='Тест обмена по профилю %1.';uk='Тест обмена по профилю %1.'") + " " + ТекстСообщения;
...
"%1';uk='Проверка записи и чтения файлов в каталоге исходящих документов."
"%1'");
...
"%1';uk='Проверка каталога входящих документов."
"%1'");
	Попытка
...
"%1';uk='Проверка записи и чтения файлов в каталоге входящих документов."
"%1'");
		ПроверитьФайл(ШаблонСообщения, FTPСоединение, ТекстОшибки);
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			РезультатТеста = ТекстОшибки;
...
Функция СформироватьНовыйПакетЭД(СтруктураПараметров) Экспорт
	...
	Исключение
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru='создание нового ПакетаЭД';uk='создание нового ПакетаЭД'"),
																					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
																					ТекстСообщения);
	КонецПопытки;
...
Процедура ОбновитьСтатусыДокументовПакетаЭД(ПакетЭД, НовыйСтатусПакетаЭД, ДатаИзменения) Экспорт
	...
	Если ПакетЭД.ЭлектронныеДокументы.Количество() = 0 Тогда
		...
		ШаблонОшибки = НСтр("ru='Документ %1 не был отправлен. Подробнее см. Журнал регистрации';uk='Документ %1 не был отправлен. Подробнее см. Журнал регистрации'");
...
"Не заполнена табличная часть ""ОбменСКонтрагентами""';uk='Не корректно заполнен документ %1."
"Не заполнена табличная часть ""ОбменСКонтрагентами""'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки,
					ПакетЭД );
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			НСтр("ru='Отправка пакета ЭД и обновление статусов ЭД';uk='Отправка пакета ЭД и обновление статусов ЭД'"),
			ТекстОшибки,
			ТекстСообщения);
	...
	Попытка
		...
		Если ТипЗнч(ЭД.ОбъектВладелец)=Тип("ДокументСсылка.УдалитьПроизвольныйЭД")
	...
	Исключение
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			НСтр("ru='смена статуса пакета ЭД и обновление статусов ЭД';uk='смена статуса пакета ЭД и обновление статусов ЭД'"), ТекстОшибки, ТекстСообщения);
	КонецПопытки;
...
Функция ПолучитьПредставлениеЭД(СсылкаНаФайлЭД) Экспорт
	...
	Если ТипЗнч(СсылкаНаФайлЭД) = Тип("СправочникСсылка.ЭДПрисоединенныеФайлы") Тогда
		...
		|ВидЭД, ДатаДокументаОтправителя, НомерДокументаОтправителя");
	...
	Иначе
		...
		Если РеквизитыЭД.ВидЭД = Перечисления.ВидыЭД.КаталогТоваров ИЛИ РеквизитыЭД.ВидЭД = Перечисления.ВидыЭД.ПрайсЛист Тогда
			Представление = "" + РеквизитыЭД.ВидЭД + ТекстДаты + ТекстВерсии;
		ИначеЕсли РеквизитыЭД.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ДОПУПД
			ИЛИ РеквизитыЭД.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ДИСУКД Тогда
			
			Представление = НСтр("ru='Информация продавца №';uk='Информация продавца №'") + " " + РеквизитыЭД.НомерДокументаОтправителя + ТекстДаты;
		Иначе
			Представление = "" + РеквизитыЭД.ВидЭД + " № " + РеквизитыЭД.НомерДокументаОтправителя + ТекстДаты
			+ ТекстВерсии;
		КонецЕсли;
...
Функция ОпределитьПредставлениеЭД(ВидЭД, СтруктураПараметров) Экспорт
	...
	Если ВидЭД <> Перечисления.ВидыЭД.СчетФактура И ВидЭД <> Перечисления.ВидыЭД.ИзвещениеОПолучении
		...
		ШаблонТекстаВерсии = НСтр("ru='(версия %1)';uk='(версия %1)'");
	...
	Если СтруктураПараметров.Свойство("ТипЭД", ТипЭД)
		...
		И ТипЭД <> Перечисления.ТипыЭлементовВерсииЭД.ПервичныйЭД Тогда
		...
		Если ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУПД
			...
			ТекстВидЭД = НСтр("ru='Информация покупателя';uk='Информация покупателя'");
		...
		ИначеЕсли ЭтоИзвещениеОПолучении(ТипЭД) Тогда
			ТекстВидЭД = НСтр("ru='Извещение о получении';uk='Извещение о получении'");
		ИначеЕсли ЭтоПодтверждение(ТипЭД) Тогда
			ТекстВидЭД = НСтр("ru='Подтверждение';uk='Підтвердження'");
			
			Если ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ПДП Тогда
				ТекстВидЭД = ТекстВидЭД + " " + НСтр("ru='даты получения';uk='даты получения'");
			Иначе
				ТекстВидЭД = ТекстВидЭД + " " + НСтр("ru='даты отправки';uk='даты отправки'");
			КонецЕсли;
			
		ИначеЕсли ЭтоУведомлениеОбУточнении(ТипЭД) Тогда
			ТекстВидЭД = НСтр("ru='Уведомление об уточнении';uk='Уведомление об уточнении'");
	...
	Иначе
		...
		Если ВидЭД = Перечисления.ВидыЭД.ТОРГ12 Тогда
			ТекстВидЭД = НСтр("ru='Товарная накладная (ТОРГ-12)';uk='Товарная накладная (ТОРГ-12)'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ТОРГ12Продавец Тогда
			...
			Если ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ДОПУПД Тогда
				ТекстВидЭД = НСтр("ru='Информация продавца';uk='Информация продавца'");
			КонецЕсли;
		...
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ЗаказТовара Тогда
			ТекстВидЭД = НСтр("ru='Заказ товара';uk='Заказ товара'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ОтветНаЗаказ Тогда
			ТекстВидЭД = НСтр("ru='Ответ на заказ';uk='Ответ на заказ'");
		...
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ОтчетОПродажахКомиссионногоТовара Тогда
			ТекстВидЭД = НСтр("ru='Отчет о продажах комиссионного товара';uk='Отчет о продажах комиссионного товара'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ОтчетОСписанииКомиссионногоТовара Тогда
			ТекстВидЭД = НСтр("ru='Отчет о списании комиссионного товара';uk='Отчет о списании комиссионного товара'");
		...
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ВозвратТоваровМеждуОрганизациями Тогда
			ТекстВидЭД = НСтр("ru='Возврат Интеркампани';uk='Возврат Интеркампани'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ПроизвольныйЭД Тогда
			ТекстВидЭД = НСтр("ru='Произвольный электронный документ';uk='Произвольный электронный документ'");
		КонецЕсли;
	КонецЕсли;
...
"Документ не обработан, так как содержит невалидные подписи.';uk='Обработка электронного документа %1."
...
"%1';uk='Ошибка при получении сообщения с сервера электронной почты."
"%1'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, ТекстОшибки);
			
			ТекстСообщения = НСтр("ru='Ошибка при получении новых эл.документов.';uk='Ошибка при получении новых эл.документов.'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				НСтр("ru='Получение новых эл.документов';uk='Получение новых эл.документов'"), ТекстОшибки, ТекстСообщения);
			Продолжить;
		КонецПопытки;
		...
		Для Каждого Сообщение Из НаборСообщений Цикл
			...
			Если СтрНайти(Сообщение.Тема, НСтр("ru='Подтверждение получения пакета электронных документов.';uk='Подтверждение получения пакета электронных документов.'")) Тогда
			...
			Для Каждого КлючЗначение Из СтруктураОбязательныхПолей Цикл
				...
				Если СтруктураЭД.Свойство(КлючЗначение.Ключ, Значение) Тогда
					Если НЕ ЗначениеЗаполнено(Значение) Тогда
						Текст = НСтр("ru='Значение параметра <%1> не заполнено.';uk='Значение параметра <%1> не заполнено.'");
					...
					КонецЕсли;
				Иначе
					Текст = НСтр("ru='Не передан обязательный параметр <%1>.';uk='Не передан обязательный параметр <%1>.'");
					Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, КлючЗначение.Ключ);
					ТекстОшибки = ?(ЗначениеЗаполнено(ТекстОшибки), ТекстОшибки + Символы.ПС + Текст, Текст);
				КонецЕсли;
			КонецЦикла;
...
"%1';uk='Ошибка заполнения доп.свойств электронного документа."
"%1'");
...
//    * ФайлРедактируетТекущийПользователь - Булево - признак редактирования текушим пользователем.
// 
...
"ИНН: %3"
"КПП: %4"
"Статус: %5.';uk='Обмен по профилю настроек ЭДО %1."
"В базе не найден контрагент:"
"Наименование: %2"
"ИНН: %3"
"КПП: %4"
...
"ИНН: %5"
"КПП: %6"
"Статус: %7.';uk='Обмен по профилю настроек ЭДО %1."
"%2 в приглашении для контрагента:"
"Наименование: %3"
"Адрес ЭП: %4"
"ИНН: %5"
...
"Статус: %7.'");
			...
			Если НовыеПриглашения.СтатусУчастника = Перечисления.СтатусыУчастниковОбменаЭД.Присоединен Тогда
				...
				Если Выборка.Следующий() Тогда
					...
					НастройкаЭДО.Комментарий           = НСтр("ru='##Автоматически снят статус ""присоединен"". Уже существует настройка ЭДО по выбранному контрагенту.';uk='##Автоматически снят статус ""присоединен"". Уже существует настройка ЭДО по выбранному контрагенту.'");
				КонецЕсли;
			...
			ТекстОписанияОшибки = "";
			Если Ошибка Тогда
				ШаблонОписанияОшибки = НСтр("ru='%1. Повторите отправку.';uk='%1. Повторите отправку.'");
		...
		Если ДопИнформацияПоЭД.Свойство("СсылкаНаДвоичныеДанныеФайла")
			...
			Если СтрНайти(ДопИнформацияПоЭД.Расширение, "zip") > 0 Тогда
				...
				Исключение
					...
					КонецЕсли;
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru='Распаковка пакета ЭД';uk='Распаковка пакета ЭД'"),
...
Функция НастройкиОбменаЭД(СсылкаНаФайлЭД) Экспорт
	...
	Если ЗначениеЗаполнено(СсылкаНаФайлЭД) Тогда
		...
		РеквизитыЭД = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РеквизитыФайла.ВладелецФайла,
				"ВидЭД, НастройкаЭДО, ПрофильНастроекЭДО, ВерсияРегламентаЭДО, ТребуетсяПодтверждение");
		...
		Если ЗначениеЗаполнено(РеквизитыЭД) Тогда
			...
			НастройкиОбмена.Вставить("ТребуетсяИзвещениеОПолучении",Истина);
			...
			|	ВЫБОР
			|		КОГДА &ИспользуетсяЭП
			|			ТОГДА Соглашение.ТребуетсяПодтверждение
			|		ИНАЧЕ ЛОЖЬ
			|	КОНЕЦ КАК ТребуетсяПодтверждение,
			|	Соглашение.ТребуетсяИзвещениеОПолучении КАК ТребуетсяИзвещениеОПолучении,
...
"Имя файла: %2';uk='В имени файла не должно быть следующих символов: %1"
"Имя файла: %2'");
...
Функция ПолучитьДопустимыйСтатусЭД(НовыйСтатус, СсылкаНаЭД) Экспорт
	...
	Если ЗначениеЗаполнено(СсылкаНаЭД) Тогда
		...
		ИначеЕсли НовыйСтатус = Перечисления.СтатусыЭД.ПолученоПредложениеОбАннулировании Тогда
			...
			Если ЗначениеЗаполнено(ВторойТитулДокумента(СсылкаНаЭД)) Тогда
			...
			Иначе
				...
				Если НастройкиОбмена.ВерсияРегламентаЭДО = Перечисления.ВерсииРегламентаОбмена1С.Версия20 Тогда
					...
					Если НЕ ЭтоОтветныйТитул(НастройкиОбмена.ТипЭлементаВерсииЭД) Тогда
						...
						Если НастройкиОбмена.ТребуетсяИзвещениеОПолучении Тогда
							МассивСтатусов.Добавить(Перечисления.СтатусыЭД.Доставлен);
						КонецЕсли;
					КонецЕсли;
				Иначе
				...
				Если НастройкиОбмена.ВерсияФорматаПакета = Перечисления.ВерсииФорматаПакетаЭД.Версия30 Тогда
					...
					Если НЕ ЭтоОтветныйТитул(НастройкиОбмена.ТипЭлементаВерсииЭД) Тогда
						...
						Если НастройкиОбмена.ИспользоватьПодпись
							...
							МассивСтатусов.Добавить(Перечисления.СтатусыЭД.ПолученоПодтверждение);
						ИначеЕсли ЭтоТитулФНС(НастройкиОбмена.ВидЭД) Тогда
							МассивСтатусов.Добавить(Перечисления.СтатусыЭД.ПолученоПодтверждение)
						КонецЕсли;
	...
	ИначеЕсли НастройкиОбмена.Направление = Перечисления.НаправленияЭД.Входящий Тогда
		...
		Если НЕ (ЭтоИзвещениеОПолучении(НастройкиОбмена.ТипЭлементаВерсииЭД)
			...
			Если НастройкиОбмена.СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском
				...
				Иначе
					МассивСтатусов.Добавить(Перечисления.СтатусыЭД.Утвержден);
				Если НЕ ЭтоОтветныйТитул(НастройкиОбмена.ТипЭлементаВерсииЭД)
	...
	Для Каждого СтруктураОбмена Из МассивСтруктурОбмена Цикл
		...
		Если СтруктураОбмена.Свойство("МассивФайлов") И СтруктураОбмена.МассивФайлов.Количество() > 0 Тогда
			...
			Если Не АдресАрхива = Неопределено Тогда
				...
				ПараметрыФайла.Вставить("Автор", Пользователи.АвторизованныйПользователь());
				ПараметрыФайла.Вставить("ВладелецФайлов", НовыйЭлектронныйДокумент); 
				ПараметрыФайла.Вставить("ИмяБезРасширения", НСтр("ru='Дополнительные файлы';uk='Дополнительные файлы'"));
...
Функция ЗагрузитьСтрокуXMLВДеревоЗначений(СтрокаXML, ОписаниеОшибки = Неопределено, ЧтениеXML = Неопределено, Знач ТекущийУзел = Неопределено) Экспорт
	...
	Если ПерваяИтерация Тогда
		...
		Исключение
			ОписаниеОшибки = НСтр("ru='Ошибка разбора XML:';uk='Ошибка разбора XML:'") + " " + ИнформацияОбОшибке().Описание + ".";
			Возврат Неопределено;
		КонецПопытки;
	КонецЕсли;
...
Функция БилетНаСайтПоддержки(Аутентификация, ПоказыватьОшибки = Истина) Экспорт
	...
	Исключение
		...
		Если СтрНайти(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке), "IncorrectLoginOrPasswordApiException") > 0 Тогда
			КраткоеПредставлениеОшибки = НСтр("ru='Некорректный логин или пароль пользователя.';uk='Некорректный логин или пароль пользователя.'");
		КонецЕсли;
...
"Проверить наличие ИТС по регномеру;"
...
"Добавлить в исключения антивируса\брандмауэра платформы 1С.';uk='Не удалось подключиться к пользовательскому сайту по причине:"
"%1"
"Необходимо:"
"Проверить верность введения логина\пароля;"
"Проверить наличие ИТС по регномеру;"
...
"Проверить актуальность платформы;"
"Проверить авторизацию на сайте интернет-поддержки;"
"Добавлить в исключения антивируса\брандмауэра платформы 1С.'"),
...
//  ЗашифрованныеДвоичныеДанныеОператораЭДО - ДвоичныеДанные - данные для оператора ЭДО
...
Процедура ОтправитьРегистрационныйПакет1СЭДО(Знач Файл, ПараметрыАутентификации, ЕстьОшибки) Экспорт
	...
	Исключение
		...
		ШаблонЗаголовкаСообщения = НСтр("ru='При отправке заявления о регистрации: %1 на сервисе 1С-ЭДО возникла ошибка:';uk='При отправке заявления о регистрации: %1 на сервисе 1С-ЭДО возникла ошибка:'");
...
"Проверить актуальность платформы;"
"Проверить авторизацию на сайте интернет-поддержки;"
"Добавить в исключения антивируса\брандмауэра платформы 1С.';uk='%1"
"Необходимо:"
"Проверить верность введения логина\пароля;"
"Проверить наличие ИТС по регномеру;"
"Проверить окончание срока действия договора ИТС на сайте;"
...
Функция ПолучитьФайлОтветаОтСервераЭДО(АдресРесурса, ПараметрЗапросаОтветаОтСервера = Неопределено) Экспорт
	...
	Исключение
		...
		ТекстЗаголовкаСообщения = НСтр("ru='При работе с сервисом 1С-ЭДО возникла ошибка:';uk='При работе с сервисом 1С-ЭДО возникла ошибка:'");
...
Процедура ПроверитьКонтрагентаФоновоеЗадание(Параметры) Экспорт
	...
	Исключение
		...
		Операция = НСтр("ru='Проверка подключения контрагента к 1С-ЭДО из формы контрагента.';uk='Проверка подключения контрагента к 1С-ЭДО из формы контрагента.'");
...
Процедура ПроверитьНепроверенныхКонтрагентов(ЭтоОбновлениеИБВМоделиСервиса, Параметры = Неопределено) Экспорт
	...
	ИмяРеквизитаИННКонтрагента = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("ИННКонтрагента");
	ИмяРеквизитаКППКонтрагента = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("ЕДРПОУКонтрагента");
...
Процедура СохранитьРезультатПроверкиКонтрагентаПриЗаписи(КонтрагентОбъект, Отказ) Экспорт 
	...
	ИмяРеквизитаИННКонтрагента = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("ИННКонтрагента");
	ИмяРеквизитаЕДРПОУКонтрагента = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("ЕДРПОУКонтрагента");
	...
	Запрос.УстановитьПараметр("КПП", КонтрагентОбъект[ИмяРеквизитаЕДРПОУКонтрагента]);
	...
	ДанныеКонтрагента.Вставить("КПП",			КонтрагентОбъект[ИмяРеквизитаЕДРПОУКонтрагента]);
...
Процедура ОставитьРазрешенныхКонтрагентов(ДанныеКонтрагентов)
	...
	Если НЕ РазделениеВключено
		...
		Запрос.Текст = 
			...
			|	ДанныеКонтрагентов.КодПоЕДРПОУ,
			...
			|	ВсеДанныеКонтрагентов.КодПоЕДРПОУ,
...
Процедура ПолучитьРезультатПроверкиВебСервисом(ДанныеКонтрагентов, Отбор)
	...
	Для НомерПорции = 1 По КоличествоЗапросов Цикл 
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ru='При проверке состояния контрагентов на сервисе 1С-ЭДО возникла ошибка:';uk='При проверке состояния контрагентов на сервисе 1С-ЭДО возникла ошибка:'");
...
Процедура СохранитьРезультатыПроверкиКонтрагентаВРегистр(ДанныеКонтрагента)
	...
	Исключение
		...
		Операция = НСтр("ru='Запись результатов проверки подключения контрагентов к 1С-ЭДО в регистр.';uk='Запись результатов проверки подключения контрагентов к 1С-ЭДО в регистр.'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ПодробныйТекстОшибки);
	КонецПопытки;
...
Функция УведомленияПодключены(ИдентификаторОрганизации, Соединение = Неопределено) Экспорт
	...
	ВидОперации = НСтр("ru='Получение информации о состоянии уведомлений';uk='Получение информации о состоянии уведомлений'");
...
Функция ЭтоПакетПроизвольногоЭД(МассивДокументов)
	...
	Для Каждого Документ Из МассивДокументов Цикл
		Если ТипЗнч(Документ.ВладелецФайла) = Тип("ДокументСсылка.УдалитьПроизвольныйЭД")
...
Процедура СформироватьХМЛФайл(ТекЭлемент, МассивСтруктурВозврата, ПараметрыОбмена, ДопПараметры)
	...
	Для Каждого ОбъектДляВыгрузки Из МассивОбъектовДляВыгрузки Цикл
		...
		Если ЗначениеЗаполнено(Результат) Тогда
			...
			Результат.СтруктураЭД.Вставить("ТребуетсяПодтверждение", ?(НастройкиОбмена.Свойство("ТребуетсяПодтверждение"),
...
Процедура ОбработатьПодтверждениеПолучения(Сообщение, ЭтоФайл = Ложь)
	...
	Иначе
		Если СтрНайти(Сообщение.Тема, НСтр("ru='Подтверждение получения пакета электронных документов';uk='Подтверждение получения пакета электронных документов'")) > 0 Тогда
...
Процедура ОтправитьПодтверждениеПоПакету(ПакетЭД, РесурсОтправителя, АдресОтправителя, ТипОтправки)
	...
	Если ЗначениеЗаполнено(ВыборкаПрисоединенныхФайлов) И ВыборкаПрисоединенныхФайлов.Следующий() Тогда
		Если ТипОтправки = Перечисления.СпособыОбменаЭД.ЧерезЭлектроннуюПочту Тогда
			...
			ПараметрыОтправки.Вставить("Тема",     НСтр("ru='Подтверждение получения пакета электронных документов:';uk='Подтверждение получения пакета электронных документов:'") + " " + ИмяПакетаЭД);
...
"%1';uk='Ошибка при отправке сообщения на сервер электронной почты."
"%1'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, ТекстОшибки);
				
				ТекстСообщения = НСтр("ru='Ошибка при отправке эл.документов.';uk='Ошибка при отправке эл.документов.'");
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru='Отправка эл.документов';uk='Отправка эл.документов'"),
				ТекстОшибки,
				ТекстСообщения);
			КонецПопытки;
...
Функция НеобходимоПолучить(Сообщение, СтруктураПараметров)
	...
	Если Сообщение.Вложения.Количество() = 1 Тогда 
		...
		Если СтрНайти(Сообщение.Тема, НСтр("ru='Подтверждение';uk='Підтвердження'")) Тогда
			Возврат Истина;
		КонецЕсли;
		
		Если СтрНайти(Сообщение.Тема, НСтр("ru='Обмен эл.документами:';uk='Обмен эл.документами:'")) Тогда
...
Функция НеобходимоПолучитьДвоичныеДанные(ДвоичныеДанные, ИмяФайла, СтруктураПараметров)
	...
	Если НЕ ФайлКарточки = Неопределено Тогда
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ru='Возникла ошибка при чтении данных из файла %1: %2.';uk='Возникла ошибка при чтении данных из файла %1: %2.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
				ФайлКарточки.ПолноеИмя, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru='Чтение ЭД.';uk='Чтение ЭД.'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
				ТекстСообщения);
		КонецПопытки;
...
Процедура ОбновитьСтатусыЭД(МассивДокументов, ПризнакПодписи)
	...
	Для Каждого ЭлектронныйДокумент Из МассивДокументов Цикл
		...
		Исключение
			...
			ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru='подготовка к отправке ЭД';uk='подготовка к отправке ЭД'"),
																						ТекстОшибки,
																						ТекстСообщения);
		КонецПопытки;
	КонецЦикла;
...
Процедура ЗаписатьДатуПолучения(ЭД, ДатаИзменения)
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru='запись даты получения ЭД';uk='запись даты получения ЭД'"),
																					ТекстОшибки,
																					ТекстСообщения);
	КонецПопытки;
...
Процедура ЗаписатьДатуОтправки(ЭД, ДатаИзменения)
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru='запись даты отправки ЭД';uk='запись даты отправки ЭД'"),
																					ТекстОшибки,
																					ТекстСообщения);
	КонецПопытки;
...
Процедура ПроверитьФайл(ШаблонСообщения, FTPСоединение, ТекстОшибки)
	...
	ТестоваяСтрока = НСтр("ru='Тестовая строка 1С: Предприятие';uk='Тестовая строка 1С: Предприятие'");
...
Функция СоздатьПакетЭД(РеквизитыДокумента, МассивЭД, ТекстСообщения = "")
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ru='формирование пакета ЭД';uk='формирование пакета ЭД'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"и наличие доступа к каталогу временных файлов.';uk='Проверьте поддержку русского языка в региональных настройках ОС для non-Unicode programs"
"и наличие доступа к каталогу временных файлов.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru='запись файла архива на диск';uk='запись файла архива на диск'"),
																					ТекстОшибки,
																					ТекстСообщения);
...
Функция ЭтоТитулФНС(ДанныеЭД) Экспорт
	...
	Если РеквизитыЭД.ВидЭД = Перечисления.ВидыЭД.ТОРГ12Продавец
		...
		Или РеквизитыЭД.ВидЭД = Перечисления.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель Тогда
...
"Необходимо создать ""Настройку ЭДО"" с реквизитами:';uk='Обработка %1."
"Операция не выполнена."
		...
		Если Порядок > 0 Тогда
			...
			Если ЭтоСчетФактураВыданный Тогда
				Если НРег(ТекПараметр.Ключ) = НРег("ЭтоСчетФактураВыданный") Тогда
					СтрокаПараметров.Ключ = НСтр("ru='Способ обмена';uk='Способ обмена'");
					СтрокаПараметров.Значение = НСтр("ru='через оператора ЭДО';uk='через оператора ЭДО'");
				ИначеЕсли НРег(ТекПараметр.Ключ) = НРег("Контрагент") Тогда
...
Процедура ПолучитьТекстНастроекОбменаПоСоглашению(ТекстЗапроса)
	...
	Если Не ЗначениеЗаполнено(ТекстЗапроса) Тогда
		...
		|	ТЧ_Соглашения.ТребуетсяИзвещениеОПолучении КАК ТребуетсяИзвещениеОПолучении,
		...
		|	ВТ_ТЧ_Соглашения.ТребуетсяИзвещениеОПолучении КАК ТребуетсяИзвещениеОПолучении,
...
Функция ЕстьДоступКВебСервису1CЭДО(ВыводитьСообщения = Ложь) Экспорт
	...
	Исключение
		...
		ШаблонСообщения = НСтр("ru='%1"
"Сервис 1С-ЭДО не доступен. Попробуйте чуть позже.';uk='%1"
...
Функция ВерсияСхемыФНС_2016()
	Возврат НСтр("ru='ФНС 5.01 (с 2016г.)';uk='ФНС 5.01 (с 2016г.)'");
КонецФункции
...
Функция ПрисоединенныйФайл(ВладелецФайла, ТипЭлементаВерсииЭД = Неопределено,СообщатьОбОшибке = Ложь) Экспорт
	...
	ИначеЕсли СообщатьОбОшибке Тогда
		...
		ТекстОшибки = НСтр("ru='Не обнаружен присоединенный файл документа %1';uk='Не обнаружен присоединенный файл документа %1'");
...
Функция СписокОперацийВидаЭД(ВидЭД, ДобавлятьПредопределенныеСпособы = Ложь, Префикс = "") Экспорт 
	...
	Если ДобавлятьПредопределенныеСпособы Тогда
		Если СписокОпераций.Количество() = 0 Тогда
			СписокОпераций.Добавить("Автоматически", НСтр("ru='Автоматически';uk='Автоматично'"),Истина);
		КонецЕсли;
		
		СписокОпераций.Добавить("Вручную", НСтр("ru='Вручную в форме электронного документа';uk='Вручную в форме электронного документа'"));
	КонецЕсли;
	...
	Если СоответствиеВладельцевИЭД.Количество() > 0 Тогда
		...
		Для Каждого Элемент Из СоответствиеВладельцевИЭД Цикл
			...
			ИначеЕсли ЗначениеЗаполнено(НастройкиОбмена) Тогда
				...
				Если НастройкиОбмена.ВерсияФормата = "ФНС 5.01 (УПД:Первичный документ)" Тогда
...
Процедура ПредварительноеЗаполнениеДерева(ЭД, ДеревоОбъект, ЭтоАктуальныйЭД, НастройкиОбмена,
	...
	Если ЗначениеЗаполнено(НастройкиОбмена) И НастройкиОбмена.Свойство("СоглашениеЭД") Тогда
		...
		Иначе
			...
			СвойстваЭД = ЭД;
			СвойстваЭД.Вставить("ТребуетсяИзвещение", НастройкиОбмена.ТребуетсяИзвещениеОПолучении);
...
Процедура ЗаполнитьСтрокуДерева(СтрокаДерева, Выборка,ДатаАктуальногоЭД)
	...
	СтруктураПараметров = Новый Структура;
	Если Выборка.ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ДОПУПД
		ИЛИ Выборка.ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ДИСУКД
		ИЛИ Выборка.ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ДОП
		ИЛИ Выборка.ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ПервичныйЭД Тогда
...
Функция ПолучитьОжидаемоеДействиеКонтрагента(Строка,ВладелецФайла)
	...
	Если Строка.ВидЭД = Перечисления.ВидыЭД.ИзвещениеОПолучении
		...
		Если Строка.ВидЭД = Перечисления.ВидыЭД.Подтверждение И НЕ ЗначениеЗаполнено(Строка.Ссылка)
			...
			ВозвращаемоеЗначение = НСтр("ru='Подтверждения оператора ЭДО';uk='Подтверждения оператора ЭДО'");
		ИначеЕсли Строка.НаправлениеЭД = Перечисления.НаправленияЭД.Входящий
			...
			ВозвращаемоеЗначение = НСтр("ru='Извещения о получении';uk='Извещения о получении'");
		КонецЕсли;
		
	Иначе
		Если Строка.СтатусЭД = Перечисления.СтатусыЭД.ПереданОператору Тогда
			
			ВозвращаемоеЗначение = НСтр("ru='Подтверждения отправки';uk='Подтверждения отправки'");
		...
		ИначеЕсли Строка.СтатусЭД = Перечисления.СтатусыЭД.Отправлен Тогда
			...
			ВозвращаемоеЗначение = НСтр("ru='Подтверждения о доставке';uk='Подтверждения о доставке'");
		КонецЕсли;
	КонецЕсли;
...
Процедура СоздатьРеквизитыИЭлементыДляПечатныхФорм(Форма, ДеревоЗначений)  Экспорт 
	...
	Для Каждого СтрокаТаблицы Из ТаблицаЭД Цикл
		...
		Если СтрокаТаблицы.ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ПервичныйЭД Тогда
