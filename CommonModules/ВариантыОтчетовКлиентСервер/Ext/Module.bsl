...
Функция ТекстОшибки(Номер, Реквизит1 = Неопределено, Реквизит2 = Неопределено, Реквизит3 = Неопределено) Экспорт
	Если Номер = 1 Или Номер = 2 Тогда
		Если Номер = 1 Тогда
			ТекстОшибки = НСтр("ru='Отчет ""%1"" подключен к основной форме, но не подключен к форме настроек.';uk='Звіт ""%1"" підключений до основної форми, але не підключений до форми налаштувань.'");
		Иначе
			ТекстОшибки = НСтр("ru='Отчет ""%1"" не подключен к основной форме, но подключен к форме настроек.';uk='Звіт ""%1"" не підключений до основної форми, але підключений до форми налаштувань.'");
		КонецЕсли;
