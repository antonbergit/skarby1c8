...
"Для версії 8.2 необхідно використовувати реліз 8.2.19 (або більш"
"новий)."
"Для версії 8.3 необхідно використовувати реліз 8.3.4 (або більш новий).'");
	...
	Если Не ВыгрузкаВАрхивеСовместимаСТекущейКонфигурацией(ИнформацияОВыгрузке) Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Невозможно загрузить данные из файла, т.к. файл был выгружен из другой конфигурации (файл выгружен из конфигурации %1 и не может быть загружен в конфигурацию %2)';uk='Неможливо завантажити дані з файлу, т. я. файл був вивантажений з іншої конфігурації (файл вивантажений з конфігурації %1 і не може "
"бути завантажений в конфігурацію %2)'"),
...
Функция XMLТипСсылки(Знач Значение) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Ошибка при определении XMLТипа ссылки для объекта %1: объект не является ссылочным!';uk='Помилка при визначенні XMLТипу посилання для об`єкту %1: об`єкт не "
"є посилальним!'"),
