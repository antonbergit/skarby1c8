	...
	Для Каждого ЭлементСценария Из СценарийПримененияРазрешений Цикл
		...
		Если ЭлементСценария.Операция = ВидыОпераций.Создание Тогда
			...
			Если АдминистрированиеКластераКлиентСервер.ПрофильБезопасностиСуществует(ПараметрыАдминистрированияКластера, ЭлементСценария.Профиль) Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Профиль безопасности %1 уже присутствует в кластере серверов. Настройки в профиле безопасности будут замещены...';uk='Профіль безпеки %1 вже є присутнім у кластері серверів. Налаштування в профілі безпеки будуть заміщені...'"), ЭлементСценария.Профиль));
		...
		ИначеЕсли ЭлементСценария.Операция = ВидыОпераций.Удаление Тогда
			...
			Иначе
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Профиль безопасности %1 отсутствует в кластере серверов. Возможно, профиль безопасности был удален ранее...';uk='Профіль безпеки %1 відсутній у кластері серверів. Можливо, профіль безпеки був видалений раніше...'"), ЭлементСценария.Профиль));
...
Процедура ПослеЗавершенияЗапросаРазрешенийНаИспользованиеВнешнихРесурсов(Результат, Состояние) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru='Настройка разрешений';uk='Налаштування дозволів'"),,
			НСтр("ru='Внесены изменения в настройки профилей безопасности в кластере серверов.';uk='Внесено зміни в налаштування профілів безпеки в кластері серверів.'"));
