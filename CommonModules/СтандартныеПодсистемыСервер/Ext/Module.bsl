...
// Возвращает признак, является ли конфигурация микро.
//
// Возвращаемое значение:
//   Булево   - Истина, если конфигурация - микро.
//
Функция ЭтоМикроВерсияКонфигурации() Экспорт
	
	Возврат СтрНайти(ВРег(Метаданные.Имя), "МИКРО") > 0;
...
Процедура ПриДобавленииОбработчиковСлужебныхСобытий(КлиентскиеОбработчики, СерверныеОбработчики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УдалитьЦентрМониторинга") Тогда
...
"после обновления параметров работы программы.';uk='Неприпустиме звернення до неоновлених параметрів роботи програми"
"(наприклад, до деяких параметрів сеансу)."
""
"Необхідно перенести виклик прикладного коду"
...
"- Версия конфигурации не предусматривает обновление без установки монопольного режима.';uk='Неможливо виконати відновлення інформаційної бази:"
"- Неможливо встановити монопольний режим"
"- Версія конфігурації не передбачає відновлення без установки монопольного режиму.'");
							КонецЕсли;
			...
			Если ЗначениеЗаполнено(СписокКритичныхИзменений) Тогда
				...
				ИмяСобытия = НСтр("ru='Идентификаторы объектов метаданных.Требуется загрузить критичные изменения';uk='Ідентифікатори об`єктів метаданих.Потрібно завантажити критичні зміни'",
...
"%1';uk='Інформаційна база не може бути оновлена через проблему в головному вузлі:"
"- головний вузол був некоректно оновлений (можливо не був збільшений номер версії конфігурації,"
"  через що не заповнився довідник Ідентифікатори об`єктів метаданих);"
"- або були скасовані до вивантаження пріоритетні дані (елементи"
"  довідника Ідентифікатори об`єктів метаданих)."
""
"Необхідно заново виконати оновлення головного вузла, зареєструвати до вивантаження"
"пріоритетні дані й повторити синхронізацію даних:"
"- у головному вузлі запустіть програму з параметром /C ЗапуститьОбновлениеИнформационнойБазы;"
"%1'");
...
" (сначала в главном узле, затем в этой информационной базе после перезапуска).';uk='- потім повторіть синхронізацію даних із цією інформаційною базою"
" (спочатку в головному вузлі, потім у цій інформаційній базі після перезапуску).'"));
				Иначе
					// Настройка подчиненного узла РИБ при первом запуске.
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
						НСтр("ru='- затем повторите создание подчиненного узла.';uk='- потім повторіть створення підлеглого вузла.'"));
				КонецЕсли;
...
"  выполнились процедуры обновления данных информационной базы.';uk='Для розроблювача: можливо потрібно оновити допоміжні дані,"
"які впливають на роботу програми. Для виконання оновлення можна:"
"- скористатися зовнішньою обробкою"
"  ""Інструменти розроблювача: Оновлення допоміжних даних"","
...
Процедура ЗапретитьПометкуУдаленияПредопределенныхЭлементовПередЗаписью(Источник, Отказ) Экспорт
	...
	Если Источник.ЭтоНовый() Тогда
		ВызватьИсключение
			НСтр("ru='Недопустимо создавать предопределенный элемент помеченный на удаление.';uk='Неприпустимо створювати наперед визначений елемент позначений на видалення.'");
	Иначе
...
"""%1"".';uk='Неприпустимо позначати на видалення наперед визначений елемент:"
"""%1"".'"),
...
"""%1"".';uk='Неприпустимо пов`язувати з ім`ям наперед визначеного елемента, позначений на видалення:"
"""%1"".'"),
				Строка(Источник.Ссылка));
		КонецЕсли;
	КонецЕсли;
...
"""%1"".';uk='Неприпустимо видаляти наперед визначений елемент"
"""%1"".'"),
		Строка(Источник.Ссылка));
...
"Необхідно використовувати варіант мови ""%2"".'"),
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, МинимальнаяВерсияПлатформы) < 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для запуска необходима версия платформы 1С:Предприятие %1 или выше.';uk='Для запуску необхідна версія платформи 1С:Підприємство %1 або вище.'"), МинимальнаяВерсияПлатформы);
	КонецЕсли;
...
"версии не ниже 8.3.8 или ""Не использовать"".';uk='Режим сумісності конфігурації з 1С:Підприємством версії %1 не підтримується."
"Для запуску встановіть в конфігурації режим сумісності з 1С:Підприємством"
"версії не нижче 8.3.8 або ""Не використовувати"".'"),
			НедоступныйРежим);
	КонецЕсли;
...
"Правильный формат, например: ""1.2.3.45"".';uk='Не правильно заповнена властивість конфігурації Версія: ""%1""."
