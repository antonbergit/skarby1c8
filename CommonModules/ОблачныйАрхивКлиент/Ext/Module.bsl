...
Процедура ОбработкаНавигационнойСсылки(Форма, Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка, Параметры) Экспорт
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "backup1C:OpenWebPersonalAccount_Backups" Тогда
		...
		ИнтернетПоддержкаПользователейКлиент.ОткрытьВебСтраницу(
			...
			НСтр("ru='Резервные копии сервиса Облачный архив';uk='Резервные копии сервиса Облачный архив'"),
			Параметры.Логин,
			Параметры.Пароль);
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "backup1C:OpenWebPersonalAccount_DeactivateAgent" Тогда
		...
		ИнтернетПоддержкаПользователейКлиент.ОткрытьВебСтраницу(
			...
			НСтр("ru='Деактивация Агентов резервного копирования сервиса Облачный архив';uk='Деактивация Агентов резервного копирования сервиса Облачный архив'"),
			Параметры.Логин,
			Параметры.Пароль);
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "backup1C:OpenWebPersonalAccount_BuyAgent" Тогда
		...
		ИнтернетПоддержкаПользователейКлиент.ОткрытьВебСтраницу(
			...
			НСтр("ru='Личный кабинет клиента сервиса Облачный архив';uk='Личный кабинет клиента сервиса Облачный архив'"),
			Параметры.Логин,
			Параметры.Пароль);
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "backup1C:TechSupport_BackupsList" Тогда
		...
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ru='Облачный архив: список резервных копий';uk='Облачный архив: список резервных копий'"), // Тема
			"", // Тело
			"backup", // Получатель
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "backup1C:TechSupport_ConnectionMaster_Login" Тогда
		...
		МассивВложений = ПодготовитьСписокВложенийДляТехподдержки(НавигационнаяСсылкаФорматированнойСтроки);
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ru='Облачный архив: мастер подключения (логин)';uk='Облачный архив: мастер подключения (логин)'"), // Тема
			"", // Сообщение
			"backup", // Получатель
			МассивВложений); // Вложения
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "backup1C:TechSupport_ConnectionMaster_BackupAgentInstall" Тогда
		...
		МассивВложений = ПодготовитьСписокВложенийДляТехподдержки(НавигационнаяСсылкаФорматированнойСтроки);
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ru='Облачный архив: мастер подключения (установка Агента резервного копирования)';uk='Облачный архив: мастер подключения (установка Агента резервного копирования)'"), // Тема
			"", // Сообщение
			"backup", // Получатель
			МассивВложений); // Вложения
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "backup1C:TechSupport_ConnectionMaster_BackupAgentInstall_Error" Тогда
		...
		МассивВложений = ПодготовитьСписокВложенийДляТехподдержки(НавигационнаяСсылкаФорматированнойСтроки);
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ru='Облачный архив: мастер подключения (прочие ошибки)';uk='Облачный архив: мастер подключения (прочие ошибки)'"), // Тема
			"", // Сообщение
			"backup", // Получатель
			МассивВложений); // Вложения
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "backup1C:TechSupport_BackupCreate_Error" Тогда
		...
		МассивВложений = ПодготовитьСписокВложенийДляТехподдержки(НавигационнаяСсылкаФорматированнойСтроки);
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ru='Облачный архив: создание резервной копии';uk='Облачный архив: создание резервной копии'"), // Тема
			"", // Сообщение
			"backup", // Получатель
			МассивВложений); // Вложения
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "backup1C:TechSupport_Settings" Тогда
		...
		МассивВложений = ПодготовитьСписокВложенийДляТехподдержки(НавигационнаяСсылкаФорматированнойСтроки);
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ru='Облачный архив: настройки';uk='Облачный архив: настройки'"), // Тема
			"", // Сообщение
			"backup", // Получатель
			МассивВложений); // Вложения
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "backup1C:TechSupport_BackupAgentUpdateMaster_Update" Тогда
		...
		МассивВложений = ПодготовитьСписокВложенийДляТехподдержки(НавигационнаяСсылкаФорматированнойСтроки);
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ru='Облачный архив: мастер обновления Агента резервного копирования (обновление)';uk='Облачный архив: мастер обновления Агента резервного копирования (обновление)'"), // Тема
			"", // Сообщение
			"backup", // Получатель
			МассивВложений); // Вложения
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "backup1C:TechSupport_BackupAgentUpdateMaster_Update_Error" Тогда
		...
		МассивВложений = ПодготовитьСписокВложенийДляТехподдержки(НавигационнаяСсылкаФорматированнойСтроки);
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ru='Облачный архив: мастер обновления Агента резервного копирования (прочие ошибки)';uk='Облачный архив: мастер обновления Агента резервного копирования (прочие ошибки)'"), // Тема
			"", // Сообщение
			"backup", // Получатель
			МассивВложений); // Вложения
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "backup1C:BackupAgentUpdate_WhatIsNew" Тогда
		...
		Если Параметры.Свойство("ТекстЧтоНовогоВВерсии")
			...
			ПараметрыОткрытия = Новый Структура("Заголовок, ТекстХТМЛ",
				НСтр("ru='Что нового в версии';uk='Что нового в версии'"),
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "backup1C:BackupAgentUpdate_HowToUpdate" Тогда
		...
		Если Параметры.Свойство("ТекстПорядокОбновления")
			...
			ПараметрыОткрытия = Новый Структура("Заголовок, ТекстХТМЛ",
				НСтр("ru='Порядок обновления';uk='Порядок обновления'"),
...
"Сбор данных превысил допустимое время.';uk='Произошли ошибки при подключении к подсистеме Облачный архив:"
...
"%1';uk='Произошли ошибки при подключении к подсистеме Облачный архив:"
"%1'"),
	...
	Если ОблачныйАрхивКлиентПовтИсп.РазрешенаРаботаСОблачнымАрхивом() Тогда
		...
		Если ТипЗнч(лкРасписаниеСозданияРезервныхКопий) = ТипСписокЗначений Тогда
			...
			Если СписокНапоминаний.Количество() > 0 Тогда
				...
				Кнопки.Добавить("ВыйтиИзПрограммы", НСтр("ru='Выйти из программы';uk='Выйти из программы'"));
				Кнопки.Добавить("НапомнитьПозже", НСтр("ru='Напомнить позже';uk='Нагадати пізніше'"));
				...
				Если НачалоМинуты(СписокНапоминаний[0].Значение) - НачалоМинуты(ВремяНачала) <= 1*60 Тогда
					КогдаНачнетсяКопирование = НСтр("ru='1 минуту';uk='1 минуту'");
				...
				ИначеЕсли НачалоМинуты(СписокНапоминаний[0].Значение) - НачалоМинуты(ВремяНачала) <= 20*60 Тогда
					КогдаНачнетсяКопирование = СтрШаблон(
						НСтр("ru='%1 минут';uk='%1 минут'"),
						(НачалоМинуты(СписокНапоминаний[0].Значение) - НачалоМинуты(ВремяНачала)) / 60);
				Иначе
					КогдаНачнетсяКопирование = СтрШаблон(
						НСтр("ru='%1 мин';uk=' %1 хв'"),
						(НачалоМинуты(СписокНапоминаний[0].Значение) - НачалоМинуты(ВремяНачала)) / 60);
				КонецЕсли;
...
""
"Для выполнения резервного копирования надо выйти из программы.';uk='Согласно расписанию 1С:Облачного архива, резервное копирование должно быть выполнено через %1."
""
"Для выполнения резервного копирования надо выйти из программы.'"),
				...
				ПоказатьВопрос(
					...
					НСтр("ru='Резервное копирование 1С:Облачный архив по расписанию';uk='Резервное копирование 1С:Облачный архив по расписанию'"), // Заголовок.
...
Функция ПодготовитьСписокВложенийДляТехподдержки(НавигационнаяСсылка)
	...
	Если ВложитьАрхивЛоговИспользования = Истина Тогда
		...
		Если СписокФайлов.Количество() > 0 Тогда
			...
			СписокФайловЛоговИспользования = СтрШаблон(
				НСтр("ru='Найдено %1 файлов логов работы Агента резервного копирования.';uk='Найдено %1 файлов логов работы Агента резервного копирования.'"),
				ВсегоФайлов);
			Для Каждого ТекущийФайл Из СписокФайлов Цикл
				Результат.Добавить(Новый Структура("Представление, ИмяФайла",
					СтрШаблон(
						НСтр("ru='Результат работы Агента резервного копирования от %1.txt';uk='Результат работы Агента резервного копирования от %1.txt'"),
	...
	Если ВложитьАрхивЛоговУстановки = Истина Тогда
		...
		Если СписокФайлов.Количество() > 0 Тогда
			...
			СписокФайловЛоговУстановки = СтрШаблон(
				НСтр("ru='Найдено %1 файлов логов установки.';uk='Найдено %1 файлов логов установки.'"),
				ВсегоФайлов);
			Для Каждого ТекущийФайл Из СписокФайлов Цикл
				Результат.Добавить(Новый Структура("Представление, ИмяФайла",
					СтрШаблон(
						НСтр("ru='Результат установки Агента резервного копирования от %1.txt';uk='Результат установки Агента резервного копирования от %1.txt'"),
						Формат(ТекущийФайл.Значение, "ДФ='yyyyMMdd_HHmm'")),
					ТекущийФайл.Представление));
				ВсегоФайлов = ВсегоФайлов - 1;
				Если ВсегоФайлов <= 0 Тогда
	...
	Если ВложитьРасписаниеАгентаРезервногоКопирования = Истина Тогда
		ТекстРасписания = НСтр("ru='Агент резервного копирования не запускался.';uk='Агент резервного копирования не запускался.'");
			...
			Если ИнформацияОКлиенте.ЭтоАдминистраторWindows = Истина Тогда
				Если НЕ ПустаяСтрока(лкКаталогУстановкиАгентаКопирования) Тогда
					...
					Попытка
						...
						ТекстРасписания = НСтр("ru='Результат проверки расписания:';uk='Результат проверки расписания:'")
							+ Символы.ПС
...
"%1"
"';uk='Результат проверки расписания (ошибка):"
"%1"
"'"),
...
"но у пользователя нет прав администратора Windows для просмотра расписания.';uk='Агент резервного копирования версии %1 установлен на этом компьютере (%2),"
"но у пользователя нет прав администратора Windows для просмотра расписания.'"),
		...
		Иначе
			ТекстРасписания = СтрШаблон(
				НСтр("ru='Агент резервного копирования не установлен на этом компьютере (%1).';uk='Агент резервного копирования не установлен на этом компьютере (%1).'"),
				ИмяКомпьютера());
		КонецЕсли;
		Результат.Добавить(
			Новый Структура("Представление, Текст",
				НСтр("ru='Расписание Агента резервного копирования.txt';uk='Расписание Агента резервного копирования.txt'"),
				ТекстРасписания));
	КонецЕсли;
		...
		Если ПустаяСтрока(РезультатВыгрузкиЖурналаРегистрации.ТекстОшибки) Тогда
			Результат.Добавить(
				Новый Структура("Представление, Адрес",
					НСтр("ru='Журнал регистрации (все события).txt';uk='Журнал регистрации (все события).txt'"),
					РезультатВыгрузкиЖурналаРегистрации.АдресВременногоХранилищаФайла));
		Иначе
			Результат.Добавить(
				Новый Структура("Представление, Текст",
					НСтр("ru='Журнал регистрации (ошибка).txt';uk='Журнал регистрации (ошибка).txt'"),
					СтрШаблон(
						НСтр("ru='Ошибка выгрузки журнала регистрации:"
"%1';uk='Ошибка выгрузки журнала регистрации:"
"%1'"),
...
"'"),
		...
		?(ВложитьАрхивЛоговИспользования = Истина, СписокФайловЛоговИспользования, НСтр("ru='Не вложены';uk='Не вложены'")), // Файлы логов работы.
		?(ВложитьАрхивЛоговУстановки = Истина, СписокФайловЛоговУстановки, НСтр("ru='Не вложены';uk='Не вложены'")), // Файлы логов установки.
		?(ВложитьРасписаниеАгентаРезервногоКопирования = Истина, ТекстРасписания, НСтр("ru='Не вложено';uk='Не вложено'"))); // Текст расписания.

	Результат.Добавить(Новый Структура("Представление, Текст",
		НСтр("ru='Параметры работы Облачного архива';uk='Параметры работы Облачного архива'"),
