...
"Рекомендується забороняти виконання регламентних завдань на "
"час оновлення.'");
	КонецЕсли;
...
"Выполнение прервано.';uk='Регламентне завдання недоступне по функціональних опціях"
...
Функция ИнтервалВремениСтрокой(ВремяНачала, ВремяОкончания = Неопределено) Экспорт
	...
	ИначеЕсли ВремяНачала > ВремяОкончания Тогда
		ВызватьИсключение НСтр("ru='Дата окончания интервала не может быть меньше даты начала.';uk='Дата закінчення інтервалу не може бути менше дати початку.'");
	КонецЕсли;
	...
	Если ВеличинаИнтервалаВДнях > 365 Тогда
		ОписаниеИнтервала = НСтр("ru='более года';uk='більш року'");
	ИначеЕсли ВеличинаИнтервалаВДнях > 31 Тогда
		ОписаниеИнтервала = НСтр("ru='более месяца';uk='більш місяця'");
	...
	Иначе
		ОписаниеИнтервала = НСтр("ru='менее одного дня';uk='менш одного дня'");
	КонецЕсли;
...
Функция Просклонять(Знач ФИО, Падеж, Результат, Пол = Неопределено) Экспорт
	...
	ВызватьИсключение НСтр("ru='Для вызова ОбщегоНазначения.Просклонять необходима подсистема «Склонение представлений объектов».';uk='Для виклику ОбщегоНазначения.Просклонять необхідна підсистема «Схиляння представленнь об`єктів».'");
...
Функция ОбъектПоПолномуИмени(ПолноеИмя) Экспорт
	...
	Если МассивСтрок.Количество() >= 2 Тогда
		...
		Имя = МассивСтрок[1];
	Иначе
		ВызватьИсключение СтрЗаменить(НСтр("ru='Некорректное полное имя отчета или обработки ""%1"".';uk='Некоректне повне ім`я звіту або обробки ""%1"".'"), "%1", ПолноеИмя);
	КонецЕсли;
...
"Описание ошибки: %1';uk='Кореспондент не підтримує версионирование інтерфейсів підсистем."
"Опис помилки: %1'");
...
"параметр должен содержать ссылку; передано значение: %3 (тип %4).';uk='Неприпустиме значення параметра %1 в %2. "
"параметр повинен містити посилання; передане значення: %3 (тип %4).'"),
...
Функция РазметитьМестаИспользования(Знач ПараметрыВыполнения, Знач Ссылка, Знач ЦелеваяСсылка, Знач ТаблицаПоиска)
	...
	Для Каждого МестоИспользования Из Результат.МестаИспользования Цикл
		...
		Иначе
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Замена ссылок в ""%1"" не поддерживается.';uk='Заміна посилань в ""%1"" не підтримується.'"), Информация.ПолноеИмя);
...
"%2';uk='Не вдалося заблокувати об`єкт ""%1"":"
"%2'"),
					ПредставлениеДанных,
	...
	Попытка
		...
		Для Каждого Ссылка Из Удаляемые Цикл
			...
			Попытка
				...
				КонецЕсли;
			Исключение
				ТекстОшибки = НСтр("ru='Ошибка удаления';uk='Помилка видалення'")
					+ Символы.ПС
...
Функция СобытиеЖурналаРегистрацииЗаменыСсылок()
	Возврат НСтр("ru='Поиск и удаление ссылок';uk='Пошук і видалення посилань'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции
...
Процедура ЗарегистрироватьОшибкуВТаблицу(Результат, Дубль, Оригинал, Данные, Информация, ТипОшибки, ИнформацияОбОшибке)
	...
	ИначеЕсли ТипОшибки = "БлокировкаДляУдаленияДубля" Тогда
		НовыйШаблон = НСтр("ru='Не удалось начать удаление: %2';uk='Не вдалося почати видалення: %2'");
		Ошибка.ТипОшибки = "ОшибкаБлокировки";
	ИначеЕсли ТипОшибки = "УдалитьНаборДубля" Тогда
		НовыйШаблон = НСтр("ru='Не удалось очистить сведения о дубле в %1: %2';uk='Не вдалося очистити відомості про дубль в %1: %2'");
		Ошибка.ТипОшибки = "ОшибкаЗаписи";
	ИначеЕсли ТипОшибки = "ЗаписатьНаборОригинала" Тогда
		НовыйШаблон = НСтр("ru='Не удалось обновить сведения в %1: %2';uk='Не вдалося оновити відомості в %1: %2'");
		Ошибка.ТипОшибки = "ОшибкаЗаписи";
	Иначе
