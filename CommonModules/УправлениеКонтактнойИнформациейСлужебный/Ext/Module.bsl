...
Функция СведенияОбАдресеВВидеСтруктуры(Адрес, ДополнительныеПараметры) Экспорт
	...
	Если НЕ Формат9Запятых Тогда
		...
		Если XDTOТип = ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
			...
			АдресУкраина = Состав.Состав;
			
		ИначеЕсли XDTOТип = ФабрикаXDTO.Тип(ПространствоИмен, "АдресУкраина") Тогда
			АдресУкраина = Состав;
		Иначе
			Возврат Результат;
		КонецЕсли;
		
		Если ТипЗнч(АдресУкраина) = Тип("Строка") Тогда
			Если СтрЧислоВхождений(АдресУкраина, ",") = 9 Тогда
				ЭлементыАдреса = СтруктураАдресаПоФормату9Запятых(АдресУкраина);
				Формат9Запятых = Истина;
			Иначе
		...
		Иначе
			ЭлементыАдреса.Вставить("Страна", НаименованиеСтраны);
			ЭлементыАдреса.Вставить("Индекс", ПочтовыйИндексАдреса(АдресУкраина));
			ЭлементыАдреса.Вставить("Регион", АдресУкраина.СубъектРФ);
			ЭлементыАдреса.Вставить("Район", РайонАдреса(АдресУкраина));
			ЭлементыАдреса.Вставить("Город", АдресУкраина.Город);
			ЭлементыАдреса.Вставить("НаселенныйПункт", АдресУкраина.НаселПункт);
			ЭлементыАдреса.Вставить("Улица", АдресУкраина.Улица);
			ЭлементыАдреса.Вставить("Округ", АдресУкраина.Округ);
			ЭлементыАдреса.Вставить("ВнутригРайон", АдресУкраина.ВнутригРайон);
			ЭлементыАдреса.Вставить("Дом","");
			ЭлементыАдреса.Вставить("Корпус", "");
			ЭлементыАдреса.Вставить("Квартира", "");
			ЭлементыАдреса.Вставить("ДополнительнаяТерритория", НайтиДополнительныйЭлементАдреса(АдресУкраина).Значение);
			ЭлементыАдреса.Вставить("ЭлементДополнительнойТерритории", 
				ДополнительныйЭлементАдреса(АдресУкраина, УправлениеКонтактнойИнформациейКлиентСервер.XPathДополнительногоОбъектаАдресации(91)));
		КонецЕсли;
	КонецЕсли;
	
	СоответствиеСокращений = Новый Соответствие;
	
	ЭтоРоссийскийАдрес = ?(СтрСравнить(НаименованиеСтраны, Справочники.СтраныМира.Украина.Наименование) = 0, Истина, Ложь);
	...
	Если НЕ Формат9Запятых Тогда
		...
		ЗданияИПомещения = ЗданияИПомещенияАдреса(АдресУкраина);
...
"До завершения обработки данные документов будут отображаться некорректно.';uk='Заповнення значення поля ""Інше"" у видів контактної інформації."
"До завершення обробки дані документів будуть відображатися некоректно.'");
...
Функция НаселенныеПунктыПоПредставлению(Знач Текст) Экспорт
	...
	ПространствоИмен = УправлениеКонтактнойИнформациейКлиентСервер.ПространствоИмен();
	АдресУкраина = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ПространствоИмен, "АдресУкраина"));
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
		...
		Если АдресПоУровням <> Неопределено Тогда
			ПоместитьАдресВXDTO(АдресУкраина, АдресПоУровням);
	...
	Иначе
		...
		Если ПозицияСубъектРФ > 0 Тогда
			...
			УстановитьСвойствоПоXPath(АдресУкраина, "СубъектРФ", Регион);
	...
	Если НЕ ПустаяСтрока(НаселПункт) Тогда
		Если ЗначениеЗаполнено(АдресУкраина.НаселПункт) Тогда
			НаселПункт = НаселПункт + ", " + АдресУкраина.НаселПункт;
		КонецЕсли;
		УстановитьСвойствоПоXPath(АдресУкраина, "НаселПункт", НаселПункт);
	КонецЕсли;
	
	Возврат АдресУкраина;
...
Функция ПроверитьАдресВXML(АдресВXML, ПараметрыПроверкиАдреса = Неопределено) Экспорт
	...
	Если ТипЗнч(ПараметрыПроверкиАдреса) = Тип("СправочникСсылка.ВидыКонтактнойИнформации") Тогда
		...
		АдресТолькоУкраинский = ПараметрыПроверки.АдресТолькоУкраинский;
	КонецЕсли;
	
	ПараметрыПроверки.Вставить("АдресТолькоУкраинский ", АдресТолькоУкраинский);
...
Функция ДоступныеДляКопированияАдреса(Знач ЗначенияПолейДляАнализа, Знач ВидАдреса) Экспорт
	...
	АдресТолькоУкраинский = ВидАдреса.АдресТолькоУкраинский;
	...
	Для Каждого Адрес Из ЗначенияПолейДляАнализа Цикл
		...
		Иначе
			Если АдресТолькоУкраинский Тогда
				...
				Если Не ЭтоУкраинскийАдрес(XDTOАдрес) Тогда
					ДопустимыйИсточник = Ложь;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаполнитьСписокАвтоподбораИзТаблицы(Результат, ТабличныеДанные, ЭтоПодборУлицы)
	...
	Для Каждого Строка Из ТабличныеДанные Цикл
		...
		Если Строка.Неактуален Тогда
			...
			Предупреждение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Адрес ""%1"" неактуален.';uk='Адреса ""%1"" неактуальний.'"), Строка.Представление);
		Иначе
...
"т.к. обязательное поле тип не заполнено.';uk='Відомості про вид контактної інформації %1 були пошкоджені або некоректно заповнені,"
"оскільки обов`язкове поле тип не заповнено.'");
...
Функция СформироватьАдресПоПредставлению(Представление)
	...
	НаименованиеУкраины = СокрЛП(Справочники.СтраныМира.Украина.Наименование);
	...
	Если СтрокаСтраны = Неопределено Тогда
		Адрес.Страна = НаименованиеУкраины;
	Иначе
	...
	Если Адрес.Страна = НаименованиеУкраины Тогда
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
			ПространствоИмен = УправлениеКонтактнойИнформациейКлиентСервер.ПространствоИмен();
			АдресУкраина = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ПространствоИмен, "АдресУкраина"));
			МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
			...
			Если АдресПоУровням.Найти(0, "Уровень") <> Неопределено Тогда
				...
				АдресУкраина.Адрес_По_Документу = Представление;
			КонецЕсли;
			Адрес.Состав = АдресУкраина;
		КонецЕсли;
	Иначе
...
Процедура ПоместитьАдресВXDTO(АдресУкраина, АдресПоУровням)
	...
	Для каждого ЭлементАдреса Из АдресПоУровням Цикл
		...
		ИначеЕсли ЭлементАдреса.Уровень = 90 Тогда
			ДобавитьДополнительныеЭлементыАдреса(АдресУкраина, ЭлементАдреса.Значение, 90);
		ИначеЕсли ЭлементАдреса.Уровень = 91 Тогда
			ДобавитьДополнительныеЭлементыАдреса(АдресУкраина, ЭлементАдреса.Значение, 91);
		ИначеЕсли ЭлементАдреса.Уровень = -3 Тогда
			Продолжить;
		ИначеЕсли ЭлементАдреса.Уровень = -1 Тогда
			ПочтовыйИндексАдреса(АдресУкраина, ЭлементАдреса.Наименование);
		Иначе
		...
		Если ЭлементАдреса.Уровень > 0 И ЭлементАдреса.Уровень < 90 Тогда
			УстановитьСвойствоПоXPath(АдресУкраина, Путь, ЭлементАдреса.Значение);
		КонецЕсли;
		
	КонецЦикла;
	
	ЗданияИПомещения = Новый Структура("Здания, Помещения", ТаблицаЗданийПомещений, ТаблицаЗданийПомещений);
	ЗданияИПомещенияАдреса(АдресУкраина, ЗданияИПомещения);
...
Функция АдресВведенВСвободнойФорме(XDTOИнформация, НовоеЗначение = Неопределено) Экспорт
	...
	Если Не ЭтоУкраинскийАдрес(ОбъектXDTO) Тогда
		// Не поддерживаем
		Возврат Ложь;
	КонецЕсли;
	
	АдресУкраина = ОбъектXDTO.Состав.Состав;
	Если ТипЗнч(НовоеЗначение) <> Тип("Булево") Тогда
		// Читаем
		Возврат Не ПустаяСтрока(АдресУкраина.Адрес_по_документу);
	КонецЕсли;
		
	// Устанавливаем
	Если НовоеЗначение Тогда
		АдресУкраина.Адрес_по_документу = ОбъектXDTO.Представление;
	Иначе
		АдресУкраина.Сбросить("Адрес_по_документу");
	КонецЕсли;
...
//
Функция ЭтоУкраинскийАдрес(XDTOАдрес) Экспорт
	Возврат УкраинскийАдрес(XDTOАдрес) <> Неопределено;
КонецФункции
...
//
Функция УкраинскийАдрес(ОбъектИнформации) Экспорт
	Результат = Неопределено;
	...
	Если ТипЗнч(ОбъектИнформации) = ТипXDTO Тогда
		...
		Если ТипЗнч(Адрес) = ТипXDTO И Адрес.Тип() = ФабрикаXDTO.Тип(ПространствоИмен, "АдресУкраина") Тогда
			Результат = Адрес;
		КонецЕсли;
	КонецЕсли;
...
Функция ЗначениеДополнительныхЭлементов(Знач XDTOАдрес)
	...
	АдресУкраина = УкраинскийАдрес(XDTOАдрес);
	Если АдресУкраина = Неопределено Тогда
		Возврат Результат;
	КонецЕсли;
	
	
	ДополнительныйЭлементАдреса = НайтиДополнительныйЭлементАдреса(АдресУкраина).Значение;

	Результат.ДополнительныйЭлемент = ДополнительныйЭлементАдреса;
	Результат.ПодчиненныйЭлемент = ДополнительныйЭлементАдреса(АдресУкраина, УправлениеКонтактнойИнформациейКлиентСервер.XPathДополнительногоОбъектаАдресации(91));
...
Функция ПочтовыйИндексАдреса(XDTOАдрес, НовоеЗначение = Неопределено) Экспорт
	...
	Если НовоеЗначение = Неопределено Тогда
		// Чтение
		Результат = АдресУкраина.Получить( УправлениеКонтактнойИнформациейКлиентСервер.XPathПочтовогоИндекса() );
	...
	ЗаписьИндекса = АдресУкраина.Получить(УправлениеКонтактнойИнформациейКлиентСервер.XPathПочтовогоИндекса());
	Если ЗаписьИндекса = Неопределено Тогда
		ЗаписьИндекса = АдресУкраина.ДопАдрЭл.Добавить(ФабрикаXDTO.Создать(XDTOАдрес.ДопАдрЭл.ВладеющееСвойство.Тип));
...
Функция ДополнительныйЭлементАдреса(XDTOАдрес, XPathЭлемента)
	...
	Результат = АдресУкраина.Получить(XPathЭлемента);
	Если Результат <> Неопределено Тогда
...
//
Функция НайтиДополнительныйЭлементАдреса(АдресУкраина) Экспорт
	...
	Для каждого ВариантДополнительныйЭлементАдреса Из ВариантыДополнительногоЭлементаАдреса Цикл
		...
		ДополнительныйЭлементАдреса = ДополнительныйЭлементАдреса(АдресУкраина, XPath);
...
Процедура ДобавитьДополнительныеЭлементыАдреса(XDTOАдрес, НовоеЗначение, Уровень) Экспорт
	...
	ЗначениеПоля = АдресУкраина.Получить(XPathЭлемента);
	Если ЗначениеПоля = Неопределено Тогда
		ЗначениеПоля = АдресУкраина.ДопАдрЭл.Добавить(ФабрикаXDTO.Создать(XDTOАдрес.ДопАдрЭл.ВладеющееСвойство.Тип));
...
Функция РайонАдреса(XDTOАдрес, НовоеЗначение = Неопределено)
	...
	Если НовоеЗначение = Неопределено Тогда
		...
		Если XDTOТип = ФабрикаXDTO.Тип(ПространствоИмен, "АдресУкраина") Тогда
			АдресУкраина = XDTOАдрес;
		Иначе
			АдресУкраина = XDTOАдрес.Состав;
		КонецЕсли;
		
		Если ТипЗнч(АдресУкраина) = Тип("ОбъектXDTO") Тогда
			Возврат ПолучитьXDTOРеквизитОбъекта(АдресУкраина, УправлениеКонтактнойИнформациейКлиентСервер.XPathРайона());
		КонецЕсли;
...
Функция ЗданияИПомещенияАдреса(XDTOАдрес, НовоеЗначение = Неопределено) Экспорт
	...
	// Чтение
	Для Каждого ДопЭлемент Из АдресУкраина.ДопАдрЭл Цикл
...
Функция ПредставлениеНаселенногоПункта(ОбъектАдреса) Экспорт
	...
	АдресУкраина = УкраинскийАдрес(ОбъектАдреса);
	Если АдресУкраина = Неопределено Тогда
		Возврат "";
	КонецЕсли;
	
	Если АдресУкраина.СвРайМО = Неопределено Тогда
		Район = "";
	ИначеЕсли АдресУкраина.СвРайМО.Район <> Неопределено Тогда
		Район = АдресУкраина.СвРайМО.Район;
	ИначеЕсли АдресУкраина.СвРайМО.СвМО <> Неопределено Тогда
		Районы = Новый Массив(АдресУкраина.СвРайМО.СвМО.МунОбр2, АдресУкраина.СвРайМО.СвМО.МунОбр1);
	...
	Адрес = Новый Массив;
	Адрес.Добавить(АдресУкраина.НаселПункт);
	Адрес.Добавить(АдресУкраина.ВнутригРайон);
	Адрес.Добавить(АдресУкраина.Город);
	Адрес.Добавить(Район);
	Адрес.Добавить(АдресУкраина.Округ);
	Адрес.Добавить(АдресУкраина.СубъектРФ);
...
Функция ПредставлениеУлицы(ОбъектАдреса) Экспорт
	...
	Адрес = Новый Массив;
	Адрес.Добавить(АдресУкраина.Улица);
	
	ДополнительныйЭлементАдреса = ДополнительныйЭлементАдреса(АдресУкраина, УправлениеКонтактнойИнформациейКлиентСервер.XPathДополнительногоОбъектаАдресации(90, "СНТ"));
	Если ДополнительныйЭлементАдреса = Неопределено Тогда
		ДополнительныйЭлементАдреса = ДополнительныйЭлементАдреса(АдресУкраина, УправлениеКонтактнойИнформациейКлиентСервер.XPathДополнительногоОбъектаАдресации(90, "ГСК"));
		Если ДополнительныйЭлементАдреса = Неопределено Тогда
			ДополнительныйЭлементАдреса = ДополнительныйЭлементАдреса(АдресУкраина, УправлениеКонтактнойИнформациейКлиентСервер.XPathДополнительногоОбъектаАдресации(90, "ТЕР"));
			Если ДополнительныйЭлементАдреса = Неопределено Тогда
				ДополнительныйЭлементАдреса = ДополнительныйЭлементАдреса(АдресУкраина, УправлениеКонтактнойИнформациейКлиентСервер.XPathДополнительногоОбъектаАдресации(90));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	Адрес.Добавить(ДополнительныйЭлементАдреса);
	Адрес.Добавить(ДополнительныйЭлементАдреса(АдресУкраина, УправлениеКонтактнойИнформациейКлиентСервер.XPathДополнительногоОбъектаАдресации(91)));
...
// 1) Страна, если надо.
// 2) Улица, здания, помещения.
// 3) Населенный пункт, Индекс.
//
Функция ПредставлениеАдреса(Знач XDTOАдрес, Знач ВидИнформации) Экспорт
	...
	Если ЭтоУкраинскийАдрес(АдресУкраина) Тогда
		...
		Данные = ЗданияИПомещенияАдреса(АдресУкраина);
		...
		Адрес = Новый Массив;
		Адрес.Добавить(Представление);
		Адрес.Добавить(АдресУкраина.НаселПункт);
		Адрес.Добавить(ДополнительныйЭлементАдреса(АдресУкраина, УправлениеКонтактнойИнформациейКлиентСервер.XPathПочтовогоИндекса()));
	...
	Иначе
		...
		Представление = СокрЛП(АдресУкраина);
	КонецЕсли;
...
Функция ОшибкиЗаполненияАдресаXDTO(XDTOАдрес, ВидИнформации, РезультатГруппами = Ложь) Экспорт
	...
	Иначе
		...
		АдресУкраина = ?(Адрес = Неопределено, Неопределено, Адрес.Состав);
	КонецЕсли;
	...
	ПространствоИмен = УправлениеКонтактнойИнформациейКлиентСервер.ПространствоИмен();
	Если ТипЗнч(АдресУкраина) <> Тип("ОбъектXDTO") Или АдресУкраина.Тип() <> ФабрикаXDTO.Тип(ПространствоИмен, "АдресУкраина") Тогда
	...
	Если Не XDTOКонтактнаяИнформацияЗаполнена(АдресУкраина) Тогда
		// Адрес пуст и не должен быть заполнен - считаем его корректным.
		Возврат ?(РезультатГруппами, Новый Массив, Новый СписокЗначений);
	КонецЕсли;
	
	ВсеОшибки = ОбщиеГруппыОшибокЗаполненияАдреса(АдресУкраина, ФлагиПроверки);
...
Функция ОбщиеГруппыОшибокЗаполненияАдреса(Знач ДанныеАдреса, Знач ВидИнформации)
	...
	Если ТипЗнч(ДанныеАдреса) = Тип("ОбъектXDTO") Тогда
		АдресУкраина = ДанныеАдреса;
		
	Иначе
		XTDOКонтактная = АдресXMLВXDTO(ДанныеАдреса);
		Адрес = XTDOКонтактная.Состав;
		Если Не ЭтоУкраинскийАдрес(Адрес) Тогда
			Возврат Результат;
		КонецЕсли;
		АдресУкраина = Адрес.Состав;
		
		// С) совпадение представления и набора данных.
		Представление = ПредставлениеАдреса(АдресУкраина, ВидИнформации);
		Если XTDOКонтактная.Представление <> Представление Тогда
			...
			ДобавитьОшибкуЗаполненияАдреса(Результат[0], "",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Представление адреса для вида контактной информации ""%1"" отличается от данных в адресе.';uk='Представлення адреси для виду контактної інформації ""%1"" відрізняється від даних в адресі.'"),
	...
	Индекс = ПочтовыйИндексАдреса(АдресУкраина);
	...
	ЗданияПомещения = ЗданияИПомещенияАдреса(АдресУкраина);
	Если ИсключатьПроверкуДомаВАдресе(АдресУкраина) Тогда
	...
	// 3) Район, Город, НаселенныйПункт, Улица должны:
	//      - иметь сокращение.
	...
	// Район
	Район = РайонАдреса(АдресУкраина);
	Если Не ПустаяСтрока(Район) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(Район)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указано сокращение в названии района ""%1"".';uk='Не зазначене скорочення в назві району ""%1"".'"), Район), НСтр("ru='Район';uk='Район'"));
		КонецЕсли;
		Если СтрДлина(Район) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Название района ""%1"" должно быть короче 50 символов.';uk='Назва району ""%1"" повинне бути коротше 50 символів.'"), Район), НСтр("ru='Район';uk='Район'"));
		КонецЕсли;
	...
	Если Не ПустаяСтрока(Город) Тогда
		...
		Если СтрДлина(Город) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Название города ""%1"" должно быть короче 50 символов.';uk='Назва міста ""%1"" повинне бути коротше 50 символів.'"), Город), НСтр("ru='Город';uk='Місто'"));
		КонецЕсли;
	...
	// Населенный пункт
	НаселенныйПункт = АдресУкраина.НаселПункт;
	Если Не ПустаяСтрока(НаселенныйПункт) Тогда
		...
		Если СтрДлина(НаселенныйПункт) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Название населенного пункта ""%1"" должно быть короче 50 символов.';uk='Назва населеного пункту ""%1"" повинне бути коротше 50 символів.'"), НаселенныйПункт),
				НСтр("ru='Населенный пункт';uk='Населений пункт'"));
		КонецЕсли;
	...
	Если Не ПустаяСтрока(Улица) Тогда
		...
		Если СтрДлина(Район) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Название улицы ""%1"" должно быть короче 50 символов.';uk='Назва вулиці ""%1"" повинне бути коротше 50 символів.'"), Улица), НСтр("ru='Улица';uk='Вулиця'"));
		КонецЕсли;
	...
	ДополнительныйЭлемент = НайтиДополнительныйЭлементАдреса(АдресУкраина).Значение;
	Если ЗначениеЗаполнено(ДополнительныйЭлемент) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(ДополнительныйЭлемент)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указано сокращение у дополнительного элемента ""%1"".';uk='Не зазначене скорочення в додаткового елемента ""%1"".'"), ДополнительныйЭлемент),
				НСтр("ru='Улица';uk='Вулиця'"));
		КонецЕсли;
		Если СтрДлина(Район) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Название дополнительного элемента ""%1"" должно быть короче 50 символов.';uk='Назва додаткового елемента ""%1"" повинне бути коротше 50 символів.'"), ДополнительныйЭлемент),
		...
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(ДополнительныйЭлемент, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В названии дополнительного элемента ""%1"" есть не кириллические символы.';uk='У назві додаткового елемента ""%1"" є не кириличні символи.'"), ДополнительныйЭлемент),
				НСтр("ru='ДополнительныйЭлемент';uk='ДополнительныйЭлемент'"));
		КонецЕсли;
	КонецЕсли;
	
	// 4) Индекс - если есть, то 5 цифр.
	Если Не ПустаяСтрока(Индекс) Тогда
		Поле = УправлениеКонтактнойИнформациейКлиентСервер.XPathПочтовогоИндекса();
		Если СтрДлина(Индекс) <> 5 Или Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Индекс) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
	...
	// 6) Улица не может быть пустой если НаселенныйПункт пустой.
	Если Не ИсключатьПроверкуУлицыВАдресе(АдресУкраина) Тогда
...
//
Функция ИсключатьПроверкуДомаВАдресе(Знач АдресУкраина)
	Результат = Ложь;
...
// Локальные исключения при проверке адреса.
//
Функция ИсключатьПроверкуУлицыВАдресе(Знач АдресУкраина)
	Результат = Ложь;
	
	// Дополнительные элементы адреса могут быть без улиц.
	ДополнительныеЭлементы = ЗначениеДополнительныхЭлементов(АдресУкраина);
...
Функция ПроверитьПараметрыВидаКонтактнойИнформации(ВидКонтактнойИнформации) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ВидКонтактнойИнформации.Тип) Тогда
		...
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не заполонено обязательное поле тип у вида контактной информации ""%1"".';uk='Не заповнено обов`язкове поле тип у виду контактної інформації ""%1"".'"),
	...
	Если ВидКонтактнойИнформации.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
		...
		Если НЕ ВидКонтактнойИнформации.АдресТолькоУкраинский
...
" Проверка корректности адреса доступна только для российских адресов';uk='Некоректно заповнені налаштування перевірки адреси у виду контактної інформації %1."
...
" Не допускается возможность ввода нескольких значений контактной информации при включенной истории хранения изменений.';uk='Некоректно заповнені налаштування адреси у виду контактної інформації %1."
...
Функция АдресВСтарыйСписокПолей(XDTOАдрес, РасширенныйСоставПолей = Истина)
	...
	ДобавитьЗначение(Список, "Индекс", ПочтовыйИндексАдреса(АдресУкраина) );
	
	ДобавитьЗначение(Список, "Регион", АдресУкраина.СубъектРФ);
	ДобавитьЗначение(Список, "КодРегиона", КодРегиона(АдресУкраина.СубъектРФ) );
	Если РасширенныйСоставПолей Тогда
		ДобавитьЗначение(Список, "РегионСокращение", УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(АдресУкраина.СубъектРФ));
	КонецЕсли;
	...
	ДобавитьЗначение(Список, "Город", АдресУкраина.Город);
	Если РасширенныйСоставПолей Тогда
		ДобавитьЗначение(Список, "ГородСокращение", УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(АдресУкраина.Город));
	КонецЕсли;
	
	// преобразование ФИАС в КЛАДР
	НаселПункт = АдресУкраина.НаселПункт;
	Улица = АдресУкраина.Улица;
	ДополнительныйЭлементАдреса = НайтиДополнительныйЭлементАдреса(АдресУкраина).Значение;
	ПодчиненныйЭлементЭлементАдреса = ДополнительныйЭлементАдреса(АдресУкраина, 
...
Функция ДесериализацияАдресаОбщая(Знач ЗначенияПолей, Знач Представление, Знач ОжидаемыйТип = Неопределено)
	...
	// Общий состав
	Адрес = Результат.Состав;
	Адрес.Страна = НазваниеУкраины; // Страна по умолчанию Украина
	...
	Для Каждого ЭлементСписка Из СписокПолей Цикл
		...
		Если ИмяПоля="ИНДЕКС" Тогда
			ЭлементИндекс = СоздатьДопАдрЭлемента(АдресУкраина);
		...
		ИначеЕсли ИмяПоля = "КОДРЕГИОНА" Тогда
			АдресУкраина.СубъектРФ = РегионКода(ЭлементСписка.Значение);
			
		ИначеЕсли ИмяПоля = "РЕГИОН" Тогда
			АдресУкраина.СубъектРФ = ЭлементСписка.Значение;
			
		ИначеЕсли ИмяПоля = "РАЙОН" Тогда
			Если АдресУкраина.СвРайМО = Неопределено Тогда
				АдресУкраина.СвРайМО = ФабрикаXDTO.Создать( АдресУкраина.Тип().Свойства.Получить("СвРайМО").Тип )
			КонецЕсли;
			АдресУкраина.СвРайМО.Район = ЭлементСписка.Значение;
			
		ИначеЕсли ИмяПоля = "ГОРОД" Тогда
			АдресУкраина.Город = ЭлементСписка.Значение;
			
		ИначеЕсли ИмяПоля = "НАСЕЛЕННЫЙПУНКТ" Тогда
			АдресУкраина.НаселПункт = ЭлементСписка.Значение;
			
		ИначеЕсли ИмяПоля = "УЛИЦА" Тогда
			АдресУкраина.Улица = ЭлементСписка.Значение;
			
		ИначеЕсли ИмяПоля = "ТИПДОМА" Тогда
			Если ЭлементДом = Неопределено Тогда
				ЭлементДом = СоздатьНомерДопАдрЭлемента(АдресУкраина);
			КонецЕсли;
		...
		ИначеЕсли ИмяПоля = "ТИПКОРПУСА" Тогда
			Если ЭлементКорпус = Неопределено Тогда
				ЭлементКорпус = СоздатьНомерДопАдрЭлемента(АдресУкраина);
			КонецЕсли;
		...
		ИначеЕсли ИмяПоля = "ТИПКВАРТИРЫ" Тогда
			Если ЭлементКвартира = Неопределено Тогда
				ЭлементКвартира = СоздатьНомерДопАдрЭлемента(АдресУкраина);
			КонецЕсли;
	...
	Адрес.Состав = ?(АдресУкраинский, АдресУкраина, Результат.Представление);
...
Функция ЕстьЗаполненныеСвойстваXDTOКонтактнойИнформации(Знач Владелец)
	...
	ИначеЕсли ТипВладельца = ФабрикаXDTO.Тип(ПространствоИмен, "АдресУкраина") Тогда
...
Функция СоздатьНомерДопАдрЭлемента(АдресУкраина)
	ДопАдрЭл = СоздатьДопАдрЭлемента(АдресУкраина);
	ДопАдрЭл.Номер = ФабрикаXDTO.Создать(ДопАдрЭл.Тип().Свойства.Получить("Номер").Тип);
	Возврат ДопАдрЭл.Номер;
КонецФункции

Функция СоздатьДопАдрЭлемента(АдресУкраина)
	СвойствоДопАдрЭлемента = АдресУкраина.ДопАдрЭл.ВладеющееСвойство;
	ДопАдрЭлемента = ФабрикаXDTO.Создать(СвойствоДопАдрЭлемента.Тип);
	АдресУкраина.ДопАдрЭл.Добавить(ДопАдрЭлемента);
	Возврат ДопАдрЭлемента;
КонецФункции

Функция СвРайМО(АдресУкраина)
	Если АдресУкраина.СвРайМО <> Неопределено Тогда
		Возврат АдресУкраина.СвРайМО;
	КонецЕсли;
	
	АдресУкраина.СвРайМО = ФабрикаXDTO.Создать( АдресУкраина.Свойства().Получить("СвРайМО").Тип );
	Возврат АдресУкраина.СвРайМО;
КонецФункции
...
Процедура УстановитьXDTOРеквизитОбъекта(ОбъектXDTO, ПутьXPath, Значение) Экспорт
	...
	Если Объект <> Неопределено Тогда
		...
		Иначе
			...
			ЗначениеПоля.Значение = Значение.Значение;
		КонецЕсли;
