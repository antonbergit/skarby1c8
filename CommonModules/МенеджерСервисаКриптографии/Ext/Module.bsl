...
Функция ЗавершитьИзменениеНастроекПолученияВременныхПаролей(ИдентификаторЗаявления, Код1, Код2) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Код2) Тогда
		ПараметрыЗапроса.Вставить("only_confirm", Истина);
	КонецЕсли;
...
Функция СоединениеССерверомИнтернета(ПараметрыСоединения)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Электронная подпись в модели сервиса.Соединение с сервером интернета';uk='Электронная подпись в модели сервиса.Соединение с сервером интернета'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ВызватьHTTPМетод(HTTPМетод, URL, ПараметрыЗапроса, СоответствиеПолейОтвета, Заголовки = Неопределено)
	...
	Иначе
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Электронная подпись в модели сервиса.Менеджер сервиса криптографии';uk='Электронная подпись в модели сервиса.Менеджер сервиса криптографии'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
