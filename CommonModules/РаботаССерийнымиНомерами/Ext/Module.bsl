...
Процедура ЗаполнитьТЧСерийныеНомераПоКлючуСвязи(ДокументОбъект, ДанныеЗаполнения, ИмяТЧЗапасы="Запасы", 
	...
	//Если НЕ ОбщегоНазначения.НаличиеТабличнойЧастиУДокумента(ОбъектМетаданныхИмя,ИмяТЧСерийныеНомера) Тогда
	//	Возврат;
	//КонецЕсли;
...
Процедура ПроверкаЗаполненияСерийныхНомеров(Отказ, Знач Запасы, Знач СерийныеНомера, Знач вхФорма, ИмяПоляКлючСвязи="КлючСвязи") Экспорт
		...
		Для каждого СтрокаЗапасы Из Запасы Цикл
			Если СтрокаЗапасы.Номенклатура.ИспользоватьСерийныеНомера Тогда
				...
				Если ОтборСерийныеНомера.Количество() <> СтрокаЗапасыКоличество Тогда
					...
					ТекстСообщения = НСтр("ru='Число серийных номеров табличной части %ИмяТЧ% отличается от количества единиц в строке %Номер%.';uk='Число серийных номеров табличной части %ИмяТЧ% отличается от количества единиц в строке %Номер%.'");
					ТекстСообщения = ТекстСообщения + НСтр("ru=' Серийных номеров - %ЧислоНомеров%, нужно %КоличествоВСтроке%';uk=' Серійних номерів - %ЧислоНомеров%, потрібно %КоличествоВСтроке%'");
...
Процедура ВыполнитьКонтрольДублей(Отказ, Знач Запасы, Знач СерийныеНомера, Знач вхФорма, ИмяПоляКлючСвязи)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
			ТекстСообщения = НСтр("ru='Серийный номер ""%СерийныйНомер%"", указанный в строке %НомерСтроки%, указан повторно.';uk='Серийный номер ""%СерийныйНомер%"", указанный в строке %НомерСтроки%, указан повторно.'"
			);
	...
	Если ИспользоватьСерийныеНомераОстатки() = Истина Тогда
		...
		Если Объект.Номенклатура.ИспользоватьСерийныеНомера Тогда
			...
			Если Объект.СерийныеНомера.Количество() <> ОбъектКоличество Тогда
				ТекстСообщения = НСтр("ru='Число серийных номеров отличается от количества в документе.';uk='Число серийных номеров отличается от количества в документе.'");
				ТекстСообщения = ТекстСообщения + НСтр("ru=' Серийных номеров - %ЧислоНомеров%, нужно %КоличествоВСтроке%';uk=' Серійних номерів - %ЧислоНомеров%, потрібно %КоличествоВСтроке%'");
