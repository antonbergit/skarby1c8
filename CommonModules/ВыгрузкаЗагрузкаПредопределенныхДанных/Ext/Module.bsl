...
"т.к. не требуется обеспечивать сопоставление ссылок на его предопределенные элементы!';uk='Об`єкт метаданих %1 не може бути оброблений оброблювачем"
"ВыгрузкаЗагрузкаПредопределенныхДанных.ПередВыгрузкойОбъекта(),"
"тому що не потрібно забезпечувати зіставлення посилань на його визначені елементи!'", Метаданные.ОсновнойЯзык.КодЯзыка),
...
"ВыгрузкаЗагрузкаПредопределенныхДанных.ПередВыгрузкойОбъекта(),"
"т. я. не може містити передобумовлених елементів!'", Метаданные.ОсновнойЯзык.КодЯзыка),
			Объект.Метаданные().ПолноеИмя()
		);
		...
		Если ЗначениеЗаполнено(СтрокаТаблицыИсходныхСсылок.ИмяПредопределенныхДанных) Тогда
			...
			Если Не РезультатЗапроса.Пустой() Тогда
				...
				Иначе
					...
					ВызватьИсключение СтрШаблон(
						НСтр("ru='Обнаружено дублирование предопределенных элементов %1 в таблице %2!';uk='Виявлене дублювання передобумовлених елементів %1 у таблиці "
