...
Процедура ПриНажатииНаКнопкуОтправкиВКонтролирующийОрган(
		...
		Знач ОрганизацияОтчета = Неопределено) Экспорт
...
Процедура ОткрытьФормуМастераЗаявленияНаПодключениеПослеПолученияКонтекстаЭДО(Результат, ДополнительныеПараметры) Экспорт
	...
	Если КонтекстЭДОКлиент = Неопределено Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Подключение к электронному документообороту невозможно: недостаточно прав.';uk='Подключение к электронному документообороту невозможно: недостаточно прав.'"));
...
Процедура ОткрытьФормуСотрудникаНаРеквизите(ОповещениеОЗавершении, ФормаВладелец, Сотрудник, ЗначениеРеквизита, ИмяРеквизита) Экспорт
	...
	Если СтандартнаяОбработка Тогда
		...
		ИначеЕсли ИмяРеквизита = "АдресРегистрации" Тогда
			...
			Параметры.Вставить("Заголовок",               НСтр("ru='Ввод адреса';uk='Введення адреси'"));
		...
		ИначеЕсли ИмяРеквизита = "Телефон" Тогда
			Параметры = Новый Структура;
			Параметры.Вставить("Заголовок",               НСтр("ru='Ввод телефона';uk='Введення телефону'"));
			Параметры.Вставить("Представление", 		   ЗначениеРеквизита);
...
"Отправка пояснения невозможна, внесите исправления в учетные данные и сформируйте корректирующую декларацию.';uk='Все строки, в которых указан ответ, содержат изменения в сумме начисленного НДС."
...
"Отправка пояснения невозможна.';uk='Нет ни одной строки, в которой указан ответ."
"Отправка пояснения невозможна.'");
		КонецЕсли;
		ПоказатьПредупреждение( , ТекстПредупреждения, , НСтр("ru='Невозможно отправить пояснение';uk='Невозможно отправить пояснение'"));
		Возврат;
	КонецЕсли;
	...
	Если СведенияОЗаполнении.ЕстьИзмененияПоНДС Тогда
		МассивСообщений.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Внимание!';uk='Увага!'"), , Новый Цвет(225, 40, 40)));
...
"Строки, содержащие изменения по начисленному НДС, в пояснение включены не будут.';uk='"
"Изменилась сумма начисленного НДС. "
"Внесите исправления в учетные данные и сформируйте корректирующую декларацию."
...
"Уточнение по остальным строкам требования можно будет отправить позже в другом пояснении.';uk='В ФНС будут отправлены только те строки пояснения, по которым указан ответ."
"Уточнение по остальным строкам требования можно будет отправить позже в другом пояснении.'")));
	...
	Если МассивСообщений.Количество() > 0 Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, Новый ФорматированнаяСтрока(МассивСообщений), Кнопки, , , НСтр("ru='Отправка пояснения';uk='Отправка пояснения'"));
	Иначе
...
Процедура ПроверитьВладельцаПриОткрытииПрисоединенныхФайловИзСпискаОтчетов(ПараметрыПроверки) Экспорт
	...
	Если ТипЗнч(ПараметрыПроверки.Владелец) = Тип("СправочникСсылка.МакетыПенсионныхДел") Тогда
		...
		ПараметрыПроверки.ТекстПредупреждения = НСтр("ru='Присоединение файлов к ""Макету пенсионного дела"" должно выполняться из основной формы.';uk='Присоединение файлов к ""Макету пенсионного дела"" должно выполняться из основной формы.'");
	КонецЕсли;
