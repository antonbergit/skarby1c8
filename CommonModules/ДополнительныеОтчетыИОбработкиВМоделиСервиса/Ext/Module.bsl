...
Процедура УстановитьПоставляемуюОбработкуВОбластьДанных(Знач ОписаниеИнсталляции, Знач БыстрыйДоступ, Знач Задания, Знач Разделы, Знач СправочникиИДокументы, Знач НастройкиРасположенияКоманд, Знач ВариантыДополнительногоОтчета, Знач Ответственный) Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Поставляемые дополнительные отчеты и обработки.Инициирована установка поставляемой обработки в область данных';uk='Додаткові звіти та обробки, які поставляються.Ініційована "
"установка обробки, яка поставляється, в область даних'",
	...
	Попытка
		...
		Если ОбщегоНазначения.СсылкаСуществует(ПоставляемаяОбработка) Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Установка в область данных';uk='Додаткові звіти та обробки, які поставляються.Установка в "
"область даних'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Установка в область данных отложена';uk='Додаткові звіти та обробки, які поставляються.Установка в "
"область даних відкладена'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				Строка(ОписаниеИнсталляции.Идентификатор),
...
Процедура УдалитьПоставляемуюОбработкуИзОбластиДанных(Знач ПоставляемаяОбработка, Знач ИдентификаторИспользуемойОбработки) Экспорт
	...
	Попытка
		...
		Попытка
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Удаление из области данных';uk='Додаткові звіти та обробки, які поставляються.Видалення з "
"області даних'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Ошибка удаления из область данных';uk='Додаткові звіти та обробки, які поставляються.Помилка видалення з області даних'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ОбработатьОшибкуУстановкиДополнительнойОбработкиВОбластьДанных(Знач ПоставляемаяОбработка, Знач ИдентификаторИнсталляции, Знач ТекстИсключения) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Ошибка установки в область данных';uk='Додаткові звіти та обробки, які поставляються.Помилка установки "
"в область даних'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,
		,
	...
	Если ЭтоПоставляемаяОбработка(Источник.Ссылка) Тогда
		...
		Для Каждого КонтролируемыйРеквизит Из КонтролируемыеРеквизиты Цикл
			...
			Если ИсходныйРеквизит <> РезультирующийРеквизит Тогда
				...
				ВызватьИсключение СтрШаблон(НСтр("ru='Недопустимая попытка изменения значения реквизита %1 для дополнительной обработки %2, полученной из каталога дополнительных обработок менеджера сервиса!';uk='Неприпустима спроба зміни значення реквізиту %1 для "
"додаткової обробки %2, отриманої з каталогу додаткових "
"обробок менеджера сервісу!'"), 
...
Процедура ПриСозданииВнешнейОбработки(Знач Ссылка, СтандартнаяОбработка, Результат) Экспорт
	...
	Если Не СтандартнаяОбработка Тогда
		...
		Если ИмяОбработки = Неопределено Тогда
			ВызватьИсключение НСтр("ru='Запрошено создание объекта несуществующей дополнительной обработки!';uk='Запитано створення об`єкта неіснуючої додаткової "
"обробки!'");
		КонецЕсли;
...
Процедура ПриОпределенииПсевдонимовОбработчиков(СоответствиеИменПсевдонимам) Экспорт
	...
	ИмяЗадания = "РаспределенноеВыполнениеКоманд.ВыполнитьКомандуДополнительнойОбработки";
	СоответствиеИменПсевдонимам.Вставить(ИмяЗадания, Неопределено);
...
Процедура АктуализацияНастроекИспользуемойОбработки(Знач Ссылка) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Дополнительная обработка с идентификатором %1 не является поставляемой!';uk='Додаткова обробка з ідентифікатором %1 не є такою, що "
"поставляється!'"),
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	Если Дескриптор.DataType = ИдентификаторВидаПоставляемыхДанных() Тогда
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки отменена';uk='Додаткові звіти та обробки, які поставляються.Завантаження "
"обробки, яка поставляється, скасоване'", 
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				,
...
Процедура ЗапроситьОбновлениеДополнительныхОтчетовИОбработок() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru='Запрос обновлений дополнительных отчетов и обработок';uk='Запит відновлень додаткових звітів і обробок'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
...
Процедура ОбработатьПоставляемыеДополнительныеОтчетыИОбработки(Дескриптор, ПутьКФайлу)
	...
	Если ПоставляемаяОбработка.ВерсияGUID <> ОписаниеПоставляемойОбработки.Версия Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки завершена';uk='Додаткові звіти та обробки, які поставляються.Завантаження "
"обробки, яка поставляється, завершене'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
		...
		Для Каждого ИнсталляцияОбработки Из ИспользуемыеОбработки Цикл
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Запланирована актуализация настроек поставляемой обработки';uk='Додаткові звіти та обробки, які поставляються.Запланована актуалізація налаштувань обробки, яка поставляється'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				ПоставляемаяОбработка.Ссылка,
		...
		Для Каждого ЭлементОчереди Из ОчередьИнсталляций Цикл
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Поставляемые дополнительные отчеты и обработки.Запланирована отложенная установка поставляемой обработки в область данных';uk='Додаткові звіти та обробки, які поставляються.Запланована "
"відкладена установка обробки, яка поставляється, в область даних'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				ПоставляемаяОбработка.Ссылка,
...
Процедура ПроверкаВозможностиВыполненияПоставляемойОбработки(Знач ИспользуемаяОбработка)
	...
	Если ПараметрыПубликацииИспользуемойОбработки.Публикация = Перечисления.ВариантыПубликацииДополнительныхОтчетовИОбработок.Отключена Тогда
		ВызватьИсключение НСтр("ru='Использование дополнительной обработки в вашем приложении запрещено! Обратитесь за помощью к пользователю, обладающего правами администратора в данном приложении.';uk='Використання додаткової обробки у вашому додатку "
"заборонено! Зверніться за допомогою до користувача, який має "
"права адміністратора в даному додатку.'");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПоставляемаяОбработка) Тогда
		...
		Если ПараметрыПубликацииИспользуемойОбработки.Версия <> ПараметрыПубликацииПоставляемойОбработки.Версия Тогда
			ВызватьИсключение НСтр("ru='Использование дополнительной обработки временно недоступно по причине выполнения ее обновления. Данный процесс может занять несколько минут. Приносим извинения на доставленные неудобства.';uk='Використання додаткової обробки тимчасово недоступне через виконання її оновлення. Даний процес може зайняти "
"кілька хвилин. Приносимо вибачення на доставлені незручності.'");
		КонецЕсли;
