...
Процедура ВыполнитьДействиеСценарияОбменаДаннымиВПервойИнформационнойБазе(ИндексСтрокиСценария, СценарийОбменаДанными) Экспорт
	...
	Если СтрокаСценария.НомерИнформационнойБазы = 1 Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неизвестное действие ""%1"" при обмене данными между областями данных.';uk='Невідома дія ""%1"" при обміні даними між областями "
"даних.'"),
...
Функция СформироватьНаименованиеПредопределенногоУзла() Экспорт
	...
	Возврат ?(ПустаяСтрока(ИмяПриложения), НСтр("ru='Приложение в Интернете';uk='Додаток в Інтернеті'"), ИмяПриложения);
КонецФункции
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	Если Дескриптор.DataType = ИдентификаторВидаПоставляемыхДанных() Тогда
		...
		Иначе
			...
			ТекстСообщения = НСтр("ru='Поставляемые правила обмена не подходят для текущей конфигурации и предназначены для плана обмена %1 конфигурации %2 версии %3';uk='Правила обміну, що поставляються, не підходять для поточної конфігурації й призначені для плану обміну %1 конфігурації %2 версії %3'");
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые правила обмена данными.Загрузка поставляемых правил отменена';uk='Правила обміну даними, що поставляються.Завантаження правил, що поставляються, скасоване'",
...
Функция СобытиеЖурналаРегистрацииНастройкаСинхронизацииДанных() Экспорт
	...
	Возврат НСтр("ru='Обмен данными в модели сервиса.Настройка синхронизации данных';uk='Обмін даними в моделі сервісу. Налаштування синхронізації даних'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииСессииОбменаСообщениямиСистемы()
	...
	Возврат НСтр("ru='Обмен данными в модели сервиса.Сессии обмена сообщениями системы';uk='Обмін даними в моделі сервісу.Сесії обміну повідомленнями "
"системи'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
