...
Процедура ПодключитьКорреспондента(Сообщение, Отправитель)
	...
	Если ЭтаКонечнаяТочка.Пустая()
		...
		ПредставлениеОшибки = НСтр("ru='Конечная точка не соответствует ожидаемой. Код ожидаемой конечной точки %1. Код текущей конечной точки %2.';uk='Кінцева точка не відповідає очікуваній. Код очікуваної кінцевої "
"точки %1. Код поточної кінцевої точки %2.'");
	...
	Если Корреспондент.Пустая() Тогда // Подключаем конечную точку корреспондента
		...
		Если Отказ Тогда // Отправляем сообщение в менеджер сервиса об ошибке
			...
			ПредставлениеОшибки = НСтр("ru='Ошибка подключения конечной точки корреспондента обмена. Код конечной точки корреспондента обмена %1.';uk='Помилка підключення кінцевої точки кореспондента обміну. Код "
"кінцевої точки кореспондента обміну %1.'");
			ПредставлениеОшибки = СтрШаблон(ПредставлениеОшибки,
				Тело.RecipientId);
...
"Налаштування підключення веб-сервісу не відповідають очікуваним."
...
"Код конечной точки корреспондента обмена %2';uk='Помилка оновлення параметрів підключення цієї кінцевої точки і "
"кінцевої точки кореспондента обміну."
