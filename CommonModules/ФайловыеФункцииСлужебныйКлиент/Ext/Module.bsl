...
"Google Chrome и Mozilla Firefox не поддерживается.';uk='Виконання команди ""%1"" в браузерах"
...
"%1';uk='Не вдалося одержати робочий каталог даних користувача через:"
...
"відкрити існуючий без відновлення?'"));
		Иначе
...
Процедура ОбработатьФайлПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Если ТипЗнч(ПолученныеФайлы) <> Тип("Массив") Или ПолученныеФайлы.Количество() = 0 Тогда
		ОбработатьФайлПослеОшибки(НСтр("ru='Получение файла было отменено.';uk='Одержання файлу було скасовано.'"), , Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьФайлПослеПомещенияФайлов(ПомещенныеФайлы, Контекст) Экспорт
	...
	Если ТипЗнч(ПомещенныеФайлы) <> Тип("Массив") Или ПомещенныеФайлы.Количество() = 0 Тогда
		ОбработатьФайлПослеОшибки(НСтр("ru='Помещение файла было отменено.';uk='Розміщення файлу було скасовано.'"), , Контекст);
		Возврат;
	КонецЕсли;
