...
Процедура ЗагрузитьКурсы() Экспорт
	...
	Если Дескрипторы.Descriptor.Количество() < 1 Тогда
		ВызватьИсключение(НСтр("ru='В менеджере сервиса отсутствуют данные вида ""КурсыВалют""';uk='У менеджерові сервісу відсутні дані виду ""КурсыВалют""'"));
	КонецЕсли;
...
Процедура ОбработатьПоставляемыеКурсыЗаДень(Знач Дескриптор, Знач ПутьКФайлу)
	...
	Если ДатаКурсов = "" Тогда
		ВызватьИсключение НСтр("ru='Данные вида ""КурсыВалютЗаДень"" не содержат характеристики ""Дата"". Обновление курсов невозможно.';uk='Дані виду ""КурсыВалютЗаДень"" не містять характеристики ""Дата"". Оновлення курсів неможливо.'"); 
	КонецЕсли;
