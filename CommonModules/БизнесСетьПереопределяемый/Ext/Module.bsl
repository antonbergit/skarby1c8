...
Процедура СоздатьКонтрагентаПоРеквизитам(СтруктураРеквизитов, Отказ) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Запись нового элемента справочника Контрагенты';uk='Запись нового элемента справочника Контрагенты'"),
...
"Подробности см. в Журнале регистрации.';uk='Ошибка при записи нового элемента справочника Контрагенты"
...
Процедура ВыполнитьКонтрольРеквизитовДокументов(МассивСсылокНаОбъект, ТекстСообщения, Отказ) Экспорт
	...
	Для каждого Ссылка Из МассивСсылокНаОбъект Цикл
		...
		ИначеЕсли ИмяМетаданных <> Ссылка.Метаданные().Имя Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("ru='Операция невозможна для разных видов документов';uk='Операция невозможна для разных видов документов'");
			Возврат;
		КонецЕсли;
	КонецЦикла;
	...
	Если Выборка.Количество() > 1 Тогда
		Отказ = Истина;
		Шаблон = НСтр("ru='Операция невозможна. Отличаются реквизиты документов (%1)';uk='Операция невозможна. Отличаются реквизиты документов (%1)'");
