...
"%2';uk='Користувач ""%1"" доданий в групу доступу Адміністратори з причини:"
"%2'"),
		Администратор,
		Уточнение);
	...
	Попытка
		...
		Если Объект.Пользователи.Найти(Администратор, "Пользователь") = Неопределено Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Управление доступом.Автоматическое изменение группы доступа Администраторы';uk='Управління доступом.Автоматична зміна групи доступу Адміністратори'",
...
""
"Неверный параметр ОписаниеОбъектаМетаданных.';uk='Помилка у функції ПустаяСсылкаОбъектаМетаданных"
...
Процедура ЗаменитьУдаленныеРолиВПрофиляхГруппДоступа() Экспорт
	...
	МассивРолей = Новый Массив;
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УдалитьАдресныйКлассификатор") Тогда
...
Функция СвойстваПрофиляОткрытиеВнешнихОтчетовИОбработок()
	...
	Результат.Наименование = НСтр("ru='Открытие внешних отчетов и обработок';uk='Відкриття зовнішніх звітів і обробок'", Метаданные.ОсновнойЯзык.КодЯзыка);
	Результат.Комментарий  = НСтр("ru='Предоставляет право открытия внешних отчетов и обработок из меню ""Файл - Открыть""."
"Состав ролей профиля не рекомендуется изменять.';uk='Надає право відкриття зовнішніх звітів і обробок з меню ""Файл - Відкрити""."
...
"интерфейса настройки прав доступа.';uk='Дана операція можлива тільки для спрощеного"
"інтерфейсу налаштування прав доступу. '");
	КонецЕсли;
...
"Допустимые типы: Структура, Неопределено.';uk='Некоректний тип набору властивостей ""%1""."
"Припустимі типи: Структура, Невизначено.'"),
	...
	Для каждого КлючИЗначение Из Параметры Цикл
		Если Не ВсеПараметры.Свойство(КлючИЗначение.Ключ) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Указан несуществующий параметр %1';uk='Зазначений неіснуючий параметр %1'"),
				КлючИЗначение.Ключ);
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ОбязательныеПараметры) Тогда
		...
		Для каждого КлючИЗначение Из ОбязательныеПараметры Цикл
			Если Не Параметры.Свойство(КлючИЗначение.Ключ) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не указан обязательный параметр %1';uk='Не зазначений обов`язковий параметр %1'"),
					КлючИЗначение.Ключ);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
