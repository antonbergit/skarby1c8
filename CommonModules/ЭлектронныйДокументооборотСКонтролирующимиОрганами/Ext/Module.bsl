...
Функция НовыйОбъектДвоичныеДанные(ТихийРежим = Истина)
	...
	Исключение
		...
		Если НЕ ТихийРежим Тогда
			ТекстСообщения = НСтр("ru='Не удалось создать объект для работы с двоичными данными.';uk='Не удалось создать объект для работы с двоичными данными.'");
...
Функция ПодключитьВК(ПутьВК, ТихийРежим = Истина)
	...
	Если НЕ КодВозврата И НЕ ТихийРежим Тогда
		ТекстСообщения = НСтр("ru='Не удалось подключить внешнюю компоненту для работы с криптографией.';uk='Не удалось подключить внешнюю компоненту для работы с криптографией.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Функция ЗаголовокГиперссылкиЗаявления(ФормаОрганизации, Организация, Отказ, СтандартнаяОбработка) Экспорт
	...
	ЗаголовокПервичногоЗаявления = НСтр("ru='Заявление на подключение к 1С-Отчетности';uk='Заявление на подключение к 1С-Отчетности'");
	ЗаголовокВторичногоЗаявления = НСтр("ru='Заявление на изменение реквизитов подключения к 1С-Отчетности или замену сертификата';uk='Заявление на изменение реквизитов подключения к 1С-Отчетности или замену сертификата'");
...
Процедура СоздатьЗаписьВРегистреЖурналОтправокВКонтролирующиеОрганы(ЗначенияЗаполнения) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Электронный документооборот с контролирующими органами. Журнал отправок';uk='Электронный документооборот с контролирующими органами. Журнал отправок'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Процедура ОпределитьСтатусИСостояниеСдачиОтчетности(Ссылка, Организация, ВидКонтролирующегоОргана, СостояниеСдачиОтчетности, СтатусОтправки) Экспорт
	...
	Если ОбъектОтправляетсяВКонтролирующиеОрганы(Ссылка) Тогда
		...
		Если ТекущееСостояние = Неопределено ИЛИ ТекущееСостояние.ТекущийЭтапОтправки = Неопределено Тогда
			СтатусОтправки 				= НСтр("ru='Не отправлено';uk='Не відправлено'");
...
Процедура ЗаполнитьРегистрДокументыПоТребованиюФНС(Параметры) Экспорт
	...
	Пока ОбработаноОбъектов < 1000 И Не Параметры.ОбработкаЗавершена Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Электронный документооборот с контролирующими органами. Документы по требованию ФНС';uk='Электронный документооборот с контролирующими органами. Документы по требованию ФНС'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция СвойстваСканированногоДокументаДляРегистраДокументыПоТребованиюФНС(Ссылка) 
	...
	ИННКодПоЕДРПОУОрганизации = Новый Структура; //требуется для формирования представления контрагентов (из списка участников нужно убрать организацию)
	...
	Иначе
		...
		СведенияОбОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(Организация, , "ИННЮЛ, КодПоЕДРПОУЮЛ");	
		ИНН = СведенияОбОрганизации.ИННЮЛ; 
		КодПоЕДРПОУ = СведенияОбОрганизации.КодПоЕДРПОУЮЛ; 
		
	КонецЕсли; 
	
	ИННКодПоЕДРПОУОрганизации.Вставить("ИНН", ИНН);
	ИННКодПоЕДРПОУОрганизации.Вставить("КодПоЕДРПОУ", КодПоЕДРПОУ);
	...
	Запрос.Текст = 
		...
		|	СканированныеДокументыДляПередачиВЭлектронномВидеРеквизитыУчастников.ЮрЛицоКодПоЕДРПОУ,
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.ЯвляетсяЮрЛицом Тогда
			Если ИННКодПоЕДРПОУОрганизации.ИНН <> ВыборкаДетальныеЗаписи.ЮрЛицоИНН И ИННКодПоЕДРПОУОрганизации.КодПоЕДРПОУ <> ВыборкаДетальныеЗаписи.ЮрЛицоКодПоЕДРПОУ Тогда
				Контрагент = ВыборкаДетальныеЗаписи.ЮрЛицоНаименование;	
			КонецЕсли; 
		Иначе
			Если ИННКодПоЕДРПОУОрганизации.ИНН <> ВыборкаДетальныеЗаписи.ФизЛицоИНН Тогда
...
Функция РеквизитыСправочникаКонтрагентовДоступны() Экспорт
	...
	ИЛИ СправочникКонтрагенты.Реквизиты.Найти("КодПоЕДРПОУ") = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ОбменССерверомСоздатьСервис(
	...
	Исключение
		...
		Если ВыводитьСообщения Тогда
			...
			ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.СообщитьИлиПередатьВМастерОшибку(НСтр("ru='Ошибка соединения с сервисом регистрации!';uk='Ошибка соединения с сервисом регистрации!'"), 
							ЭтоВызовИзМастера, 
							ТекстОшибокДляМастера, 
							Истина);
		КонецЕсли;
...
Функция ОбновитьСвойстваЦиклаОбмена(ЦиклОбмена, Предмет = Неопределено) Экспорт
	...
	Если ОбъектЦиклОбмена.Модифицированность() Тогда
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось обновить свойства родительского цикла обмена %1%2';uk='Не удалось обновить свойства родительского цикла обмена %1%2'"),
...
Функция ПолучитьГруппуНовогоСобытияДокументооборотаСКО(Запись)
	...
	ЗавершенныеСостояния = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(
		"Не сдано;Не принято в обработку;Сдано, требует уточнения;Сдано;Не принято;Принято;Доставлено;Получен комментарий", ";");
...
Функция ПолучитьНадписьВремяОбновления(ДатаПоследнегоОбновления) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДатаПоследнегоОбновления) Тогда
		Возврат НСтр("ru='Обновлено только что';uk='Обновлено только что'");
	КонецЕсли;
	...
	ИначеЕсли ВременнаяРазница < 15 * 60 Тогда
		НадписьВремяОбновления = НСтр("ru='Обновлено только что';uk='Обновлено только что'");
	ИначеЕсли ВременнаяРазница < 30 * 60 Тогда
		НадписьВремяОбновления = НСтр("ru='Обновлено 15 минут назад';uk='Обновлено 15 минут назад'");		
	ИначеЕсли ВременнаяРазница < 60 * 60 Тогда
		НадписьВремяОбновления = НСтр("ru='Обновлено полчаса назад';uk='Обновлено полчаса назад'");		
	ИначеЕсли ВременнаяРазница < 2 * 60 * 60 Тогда
		НадписьВремяОбновления = НСтр("ru='Обновлено час назад';uk='Обновлено час назад'");		
	ИначеЕсли ВременнаяРазница < 24 * 60 * 60 Тогда
		НадписьВремяОбновления = НСтр("ru='Обновлено несколько часов назад';uk='Обновлено несколько часов назад'");		
	ИначеЕсли ВременнаяРазница < 48 * 60 * 60 Тогда
		НадписьВремяОбновления = НСтр("ru='Обновлено вчера';uk='Обновлено вчера'");		
	Иначе
		НадписьВремяОбновления = НСтр("ru='Обновлено несколько дней назад';uk='Обновлено несколько дней назад'");
	КонецЕсли;
...
Функция НадписьНовыхНет(Группа)
	...
	Если Группа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ПолученныеСообщения
		...
		Возврат НСтр("ru='Новых нет';uk='Новых нет'");
	ИначеЕсли Группа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.НезавершенныеОтправки Тогда
		Возврат НСтр("ru='Незавершенных нет';uk='Незавершенных нет'");
	КонецЕсли;
...
Функция ЗаголовокНедавноПросмотренных(Группа)
	...
	Если Группа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ПолученныеСообщения Тогда
		Возврат НСтр("ru='Недавно полученные сообщения';uk='Недавно полученные сообщения'");
	ИначеЕсли Группа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ОбработанныеЗапросы Тогда
		Возврат НСтр("ru='Недавно обработанные запросы';uk='Недавно обработанные запросы'");
	ИначеЕсли Группа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ЗавершенныеОтправки Тогда
		Возврат НСтр("ru='Недавно завершенные отправки';uk='Недавно завершенные отправки'");
	ИначеЕсли Группа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.НезавершенныеОтправки Тогда
		Возврат "";
	КонецЕсли;
...
Процедура СконвертироватьРеквизитыЗаявленияПо1СОтчетности() Экспорт
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ЗаявлениеАбонентаСпецоператораСвязи.УдалитьВладелецЭЦППол <> ЗНАЧЕНИЕ(Перечисление.УдалитьПолФизическихЛиц.ПустаяСсылка)
