...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	ПараметрыПредупреждения.ТекстПредупреждения = НСтр("ru='Не выполнено резервное копирование при завершении работы.';uk='Не виконане резервне копіювання при завершенні роботи.'");
...
"Выполнить резервное копирование сейчас?';uk='Все готово для виконання резервного копіювання за розкладом."
...
Процедура ОповеститьПользователяОРезервномКопировании(ВариантОповещения)
	...
	ИначеЕсли ВариантОповещения = "ЕщеНеНастроено" Тогда
		...
		ТекстПояснения = НСтр("ru='Рекомендуется настроить резервное копирование информационной базы.';uk='Рекомендується налаштувати резервне копіювання інформаційної бази.'"); 
		ПоказатьОповещениеПользователя(НСтр("ru='Резервное копирование';uk='Резервне копіювання'"),
