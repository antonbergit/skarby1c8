...
"доступно только для сеансов с установленным разделением данных!';uk='Використання функції РасширенияВМоделиСервиса.ПоставляемоеРасширение()"
"доступно тільки для сеансів зі встановленим розділенням даних!'");
...
"доступно только сеансов с установленным разделением данных!';uk='Використання функції РасширенияВМоделиСервиса.ИспользуемоеРасширение()"
"доступно тільки для сеансів зі встановленим розділенням даних!'");
...
Процедура УстановитьПоставляемоеРасширениеВОбластьДанных(Знач ОписаниеИнсталляции, Знач Ответственный) Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Поставляемые расширения.Инициирована установка поставляемого расширения в область данных';uk='Розширення, що поставляються.Ініційована установка розширення, що поставляється, в область даних'",
	...
	Попытка
		...
		Если ОбщегоНазначения.СсылкаСуществует(ПоставляемоеРасширение) Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Установка в область данных';uk='Розширення, що поставляються.Установка в область даних'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Установка в область данных отложена';uk='Розширення, що поставляються.Установка в область даних відкладена'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				Строка(ОписаниеИнсталляции.Идентификатор),
...
Процедура УдалитьПоставляемоеРасширениеИзОбластиДанных(Знач ПоставляемоеРасширение) Экспорт
	...
	Попытка
		...
		Попытка
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Удаление из области данных';uk='Розширення, що поставляються.Видалення з області даних'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				ПоставляемоеРасширение,
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Ошибка удаления из области данных';uk='Розширення, що поставляються.Помилка видалення з області даних'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ОбработатьОшибкуУстановкиРасширенияВОбластьДанных(Знач ПоставляемоеРасширение, Знач ИдентификаторИнсталляции, Знач ТекстИсключения) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Ошибка установки в область данных';uk='Розширення, що поставляються.Помилка установки в область даних'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,,
		ПоставляемоеРасширение,
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	Если Дескриптор.DataType = ИдентификаторВидаПоставляемыхДанных() Тогда
		...
		Если НЕ Загружать Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Поставляемые расширения.Загрузка поставляемого расширения отменена';uk='Розширення, що поставляються.Завантаження розширення, що поставляється, скасоване'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,,,
				НСтр("ru='Поставляемое расширение несовместимо с данной конфигурацией';uk='Розширення, що поставляється, несумісне з цією конфігурацією'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемогоРасширения.Совместимость);
...
Процедура ЗапроситьОбновлениеРасширений() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Запрос обновлений расширений';uk='Запит оновлень розширень'"), УровеньЖурналаРегистрации.Предупреждение, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ОбработатьПоставляемыеРасширения(Дескриптор, ПутьКФайлу)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Загрузка поставляемого расширения';uk='Розширення, що поставляються.Завантаження розширення, що поставляється'", 
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,,,
		НСтр("ru='Инициирована загрузка поставляемого расширения';uk='Ініційовано завантаження розширення, що поставляється'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемогоРасширения.Манифест);
	...
	Если ПоставляемоеРасширение.ИдентификаторВерсии <> ОписаниеПоставляемогоРасширения.Версия Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Загрузка поставляемого расширения завершена';uk='Розширення, що поставляються.Завантаження розширення, що поставляється, завершене'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,,
		...
		Для каждого ИнсталляцияРасширения из ИспользуемыеРасширения Цикл
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Запланирована актуализация используемого расширения';uk='Розширення, що поставляються.Запланована актуалізація використовуваного розширення'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,,
				ПоставляемоеРасширение.Ссылка,
		...
		Для каждого ЭлементОчереди из ОчередьИнсталляций Цикл
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Поставляемые расширения.Запланирована отложенная установка поставляемого расширения в область данных';uk='Розширення, що поставляються.Запланована відкладена установка розширення, що поставляється, в область даних'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,,
				ПоставляемоеРасширение.Ссылка,
