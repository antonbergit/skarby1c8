...
"%1';uk='Не вдалося підключити обробник запиту налаштувань клієнта ліцензування."
"%1'"),
...
Процедура СтартоватьМеханизмРаботыСОператоромЭДО(
	...
	Если НЕ ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Подключение1СТакском") Тогда
		...
		ВызватьИсключение НСтр("ru='Ошибка встраивания. Отсутствует подсистема ""Подключение 1С-Такском"".';uk='Помилка вбудовування. Відсутня підсистема ""Підключення 1С-Такском"".'");
...
Процедура ОткрытьВебСтраницуСДополнительнымиПараметрами(
	...
	Если ПараметрыОткрытия.ЭтоПолноправныйПользователь = Неопределено Тогда
		ПараметрыОткрытия.ЭтоПолноправныйПользователь =
			СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ЭтоПолноправныйПользователь;
	КонецЕсли;
	...
	Если НеобходимаАвторизации И ПараметрыОткрытия.ЭтоПолноправныйПользователь Тогда
		...
		Тикет = "";
		Состояние(, , НСтр("ru='Переход на Портал 1С:ИТС';uk='Перехід на Портал 1С:ІТС'"));
...
Процедура ОбработкаКомандСервиса(
	...
	Исключение
		...
		КонтекстОбработчика.ПолноеОписаниеОшибки = НСтр("ru='Произошло необрабатываемое исключение:';uk='Відбулося необроблюване виключення:'")
			+ " " + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура ПриОтправкеДанныхСообщенияВТехПоддержку(РезультатОтправки, ПараметрыСообщения)
	...
	Если Результат Тогда
		...
		ПараметрыОткрытия = Новый Структура;
		ПараметрыОткрытия.Вставить("ЗаголовокОкна", НСтр("ru='Отправка сообщения в службу технической поддержки';uk='Відправка повідомлення в службу технічної підтримки'"));
...
Функция ВидПриложения()
	...
	Возврат НСтр("ru='Толстый клиент, управляемое приложение';uk='Товстий клієнт, керований додаток'");
...
Процедура ДобавитьКомандыСервиса(
	...
	Исключение
		...
		Если ТипЗнч(КонтекстОбработчика) = Тип("Структура") Тогда
			...
			КонтекстОбработчика.ДействияПриОшибкеДляСервера.Добавить("СоздатьЗаписьВЖурналеРегистрации");
			КонтекстОбработчика.ДействиеПриОшибкеДляКлиента = "ОтобразитьФормуОтсутствуетПодключение";
			КонтекстОбработчика.ПользовательскоеОписаниеОшибки = НСтр("ru='Ошибка при получении данных с сервера Интернет-поддержки.';uk='Помилка при одержанні даних із сервера Інтернет-підтримки.'");
		КонецЕсли;
...
"Продолжить?';uk='Підключення користувача з логіном %1 до Інтернет-підтримки буде"
"припинено. Для нового підключення треба наново ввести логін і пароль."
"Продовжити?'");
