...
Функция ОтправитьSMS(НомераПолучателей, Текст, ИмяОтправителя, Логин, Пароль) Экспорт
	...
	Если ОтветСервера["error_no"] = 0 Тогда
		...
		Если Не ПустаяСтрока(Результат.ОписаниеОшибки) Тогда
			Результат.ОписаниеОшибки = НСтр("ru='Не удалось отправить SMS';uk='Не удалось отправить SMS'") + ":" + СокрП(Результат.ОписаниеОшибки);
		КонецЕсли;
	Иначе
...
Функция ОписанияОшибок()
	...
	Результат.Вставить(1, НСтр("ru='Неверный API-ключ.';uk='Неверный API-ключ.'"));
	Результат.Вставить(2, НСтр("ru='Неизвестная команда.';uk='Неизвестная команда.'"));
	Результат.Вставить(3, НСтр("ru='Пользователь с указанным ID кабинета не найден.';uk='Пользователь с указанным ID кабинета не найден.'"));
	Результат.Вставить(4, НСтр("ru='Пустой список телефонов для отправки сообщений.';uk='Пустой список телефонов для отправки сообщений.'"));
	Результат.Вставить(5, НСтр("ru='Не указан текст сообщения.';uk='Не указан текст сообщения.'"));
	Результат.Вставить(6, НСтр("ru='Не удалось отправить сообщение на указанный номер.';uk='Не удалось отправить сообщение на указанный номер.'"));
	Результат.Вставить(7, НСтр("ru='Не указан отправитель по приоритетному трафику.';uk='Не указан отправитель по приоритетному трафику.'"));
	Результат.Вставить(8, НСтр("ru='Некорректный отправитель, допускается только латиница и цифры.';uk='Некорректный отправитель, допускается только латиница и цифры.'"));
	Результат.Вставить(9, НСтр("ru='Пустой список идентификаторов сообщений для получения статусов.';uk='Пустой список идентификаторов сообщений для получения статусов.'"));
	Результат.Вставить(10, НСтр("ru='Не найдено сообщение с таким идентификатором.';uk='Не найдено сообщение с таким идентификатором.'"));
	Результат.Вставить(11, НСтр("ru='Не удалось оплатить рассылку, проверьте баланс.';uk='Не удалось оплатить рассылку, проверьте баланс.'"));
...
Функция ОписаниеОшибкиПолученияСтатуса(КодОшибки)
	...
	Если ТекстСообщения = Неопределено Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Статус сообщения не получен (код ошибки: %1).';uk='Статус сообщения не получен (код ошибки: %1).'"), КодОшибки);
	КонецЕсли;
	Возврат ТекстСообщения;
КонецФункции
...
Функция Разрешения() Экспорт
	...
	Адрес = "gsm-inform.ru";
	Порт = Неопределено;
	Описание = НСтр("ru='Отправка SMS через ""GSM-INFORM"".';uk='Отправка SMS через ""GSM-INFORM"".'");
