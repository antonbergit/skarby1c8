	...
	Если СтрНайти(РежимОчистки, "ОчиститьТекущийРаздел") > 0 Тогда
		...
		Если Многостраничность Тогда
			...
			Если КолВоСтраницВРазделе > 1 Тогда
				ТекстВопроса = НСтр("ru='Внимание! Будут удалены все страницы текущего раздела.';uk='Увага! Будуть видалені всі сторінки поточного розділу.'") + Символы.ПС
				  + НСтр("ru='Продолжить?';uk='Продовжити?'");
			Иначе
				Если СведенияОФЛ Тогда 
					// Очищаем лист со сведениями о физических лицах.
					ТекстВопроса = НСтр("ru='Внимание! Будут очищены сведения о физическом лице.';uk='Увага! Будуть очищені відомості про фізичну особу.'") + Символы.ПС
					  + НСтр("ru='Продолжить?';uk='Продовжити?'");
				Иначе
					ТекстВопроса = НСтр("ru='Внимание! Будет ';uk='Увага! Буде '") + ОчищенИлиОчищено + " " + ПредставлениеРазделаОтчета + "." + Символы.ПС
					  + НСтр("ru='Продолжить?';uk='Продовжити?'");
				КонецЕсли;
			КонецЕсли;
		Иначе
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1';uk='%1'"), ТекстВопроса);
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("Форма", Форма);
...
Процедура Очистить(Форма, ВыполняемоеОповещение = Неопределено, ЗадаватьВопросПередОчисткой = Истина, Знач ТекстВопроса = "") Экспорт
	...
	Если ЗадаватьВопросПередОчисткой Тогда
		...
		Если ПустаяСтрока(ТекстВопроса) Тогда
			ТекстВопроса = НСтр("ru='Внимание! Будут очищены все разделы отчета.';uk='Увага! Будуть очищені всі розділи звіту.'") + Символы.ПС
				 + НСтр("ru='Продолжить?';uk='Продовжити?'");
		КонецЕсли;
...
"иначе измененные данные будут утеряны.';uk='Дані звіту були змінені."
"Перед завершенням роботи рекомендується записати звіт,"
...
"Это связано с тем, что индивидуальные предприниматели не обязаны сдавать бухгалтерскую отчетность (пп. 5 п. 1 ст. 23 НК РФ).';uk='Отчет не может быть выгружен."
"Выгрузить этот отчет могут только организации."
"Для индивидуальных предпринимателей выгрузка не предусмотрена форматом ФНС."
...
"Это связано с тем, что индивидуальные предприниматели не обязаны сдавать бухгалтерскую отчетность (пп. 5 п. 1 ст. 23 НК РФ).';uk='Отчет не может быть отправлен."
"Отправить этот отчет могут только организации."
"Для индивидуальных предпринимателей отправка не предусмотрена форматом ФНС."
"Это связано с тем, что индивидуальные предприниматели не обязаны сдавать бухгалтерскую отчетность (пп. 5 п. 1 ст. 23 НК РФ).'"));
...
Процедура ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВТонкомКлиентеПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Элемент Из ДистрибутивКомпонентыПечатиМашиночитаемыхФорм Цикл
		...
		Исключение
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось выгрузить шаблон машиночитаемой формы ""%1"" во временный каталог!%2';uk='Не удалось выгрузить шаблон машиночитаемой формы ""%1"" во временный каталог!%2'"), Элемент.Представление, Символы.ПС + ОписаниеОшибки());
			Сообщение.Сообщить();
		КонецПопытки;
	КонецЦикла;
	...
	Если ТаксДокПринт = Неопределено Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Для формирования машиночитаемой формы необходимо установить в системе вспомогательную компоненту.%1Установить компоненту сейчас?';uk='Для формирования машиночитаемой формы необходимо установить в системе вспомогательную компоненту.%1Установить компоненту сейчас?'"), Символы.ПС);
...
Процедура ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВТонкомКлиентеУстановитьКомпонентуЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Если НЕ УстановитьКомпонентуПечатиМашиночитаемыхФорм(Истина, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм, ВременныйКаталог) Тогда
			ПоказатьПредупреждение(,НСтр("ru='Формирование машиночитаемой формы прервано: в системе не установлена вспомогательная компонента.';uk='Формирование машиночитаемой формы прервано: в системе не установлена вспомогательная компонента.'"));
			Возврат;
		Иначе
			ТаксДокПринт = СоздатьОбъектДляФормированияМашичитаемойФормы();
...
Процедура ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВВебКлиентеПодключитьРасширениеРаботыСФайламиПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ВебБраузер)
		...
		ПоказатьПредупреждение(,НСтр("ru='Внимание! Печать бланка формы с двухмерным штрихкодом PDF417 в Веб-клиенте поддерживается только в браузере Internet Explorer.';uk='Внимание! Печать бланка формы с двухмерным штрихкодом PDF417 в Веб-клиенте поддерживается только в браузере Internet Explorer.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВывестиМашиночитаемуюФорму(Форма, ИмяКнопки) Экспорт
	...
	Если Форма.ИмяФормы = "Отчет.РегламентированныйОтчетЕУНД.Форма.ФормаОтчета2007Кв3" Тогда
		...
		Если НЕ ЗначениеЗаполнено(НаименованиеНО) Тогда
			ТекстПредупреждения = НСтр("ru='На титульном листе не указано наименование налогового органа, в который представляется декларация."
"Внесите исправление в справочник организаций и обновите сведения в декларации (меню ""Еще"" - ""Обновить"").';uk='На титульном листе не указано наименование налогового органа, в который представляется декларация."
...
Функция УстановитьКомпонентуПечатиМашиночитаемыхФорм(ЖдатьЗавершения, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм, ВремКаталог = "") Экспорт
	...
	Исключение
		...
		ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось сохранить дистрибутив компоненты печати машиночитаемых форм во временный файл!%1';uk='Не удалось сохранить дистрибутив компоненты печати машиночитаемых форм во временный файл!%1'"), Символы.ПС + ОписаниеОшибки()));
		Возврат Ложь;
	КонецПопытки;
	...
	#Если ВебКлиент Тогда
		Если НЕ РезультатПолученияФайлов Тогда
			...
			ПоказатьПредупреждение(,НСтр("ru='Установка компоненты отменена.';uk='Установка компоненты отменена.'"));
	...
	Исключение
		УдалитьВременныйФайлДистрибутива(ВремФайлДистрибутива);
		ПоказатьПредупреждение(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='При попытке установки компоненты печати машиночитаемых форм произошла ошибка!%1';uk='При попытке установки компоненты печати машиночитаемых форм произошла ошибка!%1'"), Символы.ПС + ОписаниеОшибки()));
		Возврат Ложь;
	КонецПопытки;
...
Процедура ОбработатьОшибкуМодуляПечатиЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сейчас будет запущена программа установки компоненты печати.%1Возможно, в ходе установки потребуется завершение сеанса работы ""1С:Предприятие"".%2Продолжить?';uk='Сейчас будет запущена программа установки компоненты печати.%1Возможно, в ходе установки потребуется завершение сеанса работы ""1С:Предприятие"".%2Продолжить?'"), Символы.ПС, Символы.ПС);
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Функция ВыгрузитьОтчеты(СписокСсылокНаРеглОтчеты, КаталогСохранения = Неопределено, СообщенияОбОшибках = Неопределено, ВВебКлиентеСохранятьФайлВыгрузкиИнтерактивно = Истина, ВыгружатьСОшибками = Ложь, ФайлВыгрузкиВоВременномХранилище = Неопределено, УникальныйИДФормы = Неопределено) Экспорт
	...
	Попытка
		...
		Если РезультатПроверки = 1 Тогда
			...
			ПоказатьПредупреждение(,НСтр("ru='У выбранных отчетов различаются реквизиты ""Организация"". Пакетная выгрузка невозможна!';uk='У выбранных отчетов различаются реквизиты ""Организация"". Пакетная выгрузка невозможна!'"));
			
		ИначеЕсли РезультатПроверки = 2 Тогда
			
			ПоказатьПредупреждение(,НСтр("ru='У выбранных отчетов различаются даты окончания периодов составления. Пакетная выгрузка невозможна!';uk='У выбранных отчетов различаются даты окончания периодов составления. Пакетная выгрузка невозможна!'"));
...
"Вывести форму для просмотра соотношений?';uk='В отчете не обнаружены ошибки по соотношениям показателей."
...
Процедура ПриНажатииНаКнопкуОтправкиЧерезПредставителяВРегламентированномОтчете(Форма, КонтролирующийОрган = "ФНС") Экспорт
	...
	Если Форма.Модифицированность Тогда
		...
		Иначе // все, кроме документа РегламентированныйОтчет
			ПоказатьПредупреждение(,НСтр("ru='Необходимо записать отчет перед отправкой.';uk='Необходимо записать отчет перед отправкой.'"));
		КонецЕсли;
	Иначе 
		ВыполнитьОбработкуОповещения(ОписаниеОповещения);
	КонецЕсли;
...
Процедура ОткрытьФормуПрисоединенныеФайлы(Результат, ДопПараметры) Экспорт
	...
	Если ТипЗнч(ДопПараметры.ВладелецФайла)
		...
		ТекстПредупреждения = НСтр("ru='Присоединение файлов к электронным представлениям регламентированных отчетов не предусмотрено';uk='Присоединение файлов к электронным представлениям регламентированных отчетов не предусмотрено'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		...
		Если ТипЗнч(ДопПараметры.ВладелецФайла) <> Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			...
			Если РегламентированнаяОтчетностьВызовСервера.ВозможноПрисоединениеФайловКОбъекту(
				...
				Если НЕ ПараметрыПроверки.СтандартнаяОбработка Тогда
					...
					Иначе
						ТекстПредупреждения = НСтр("ru='Присоединение файлов к отчетам данного вида из формы списка отчетов не предусмотрено';uk='Присоединение файлов к отчетам данного вида из формы списка отчетов не предусмотрено'");
					КонецЕсли;
			...
			Иначе
				...
				ТекстПредупреждения = НСтр("ru='Присоединение файлов к отчетам данного вида не предусмотрено';uk='Присоединение файлов к отчетам данного вида не предусмотрено'");
				ПоказатьПредупреждение(, ТекстПредупреждения);
...
Процедура ОткрытьИсториюОтправкиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если КонтекстЭДОКлиент = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ru='Недостаточно прав для использования модуля документооборота!';uk='Недостаточно прав для использования модуля документооборота!'"));
		Возврат;
	КонецЕсли;
	...
	Если Источник = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ru='Объект не записан.';uk='Объект не записан.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьОшибкуМодуляПечати(ОписаниеОшибки, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм, ВремКаталог)
	...
	ИначеЕсли КодОшибки = "0x8004003C" Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Отсутствует шаблон для формирования машиночитаемой формы!%1Возможно, необходимо зарегистрировать в информационной базе внешний шаблон.%2Показать список зарегистрированных шаблонов?';uk='Отсутствует шаблон для формирования машиночитаемой формы!%1Возможно, необходимо зарегистрировать в информационной базе внешний шаблон.%2Показать список зарегистрированных шаблонов?'"),
			Символы.ПС, Символы.ПС);
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , , НСтр("ru='Ошибка при формировании машиночитаемой формы';uk='Ошибка при формировании машиночитаемой формы'"));
		Возврат;
	...
	ПоказатьВопрос(ОписаниеОповещения, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1';uk='%1'"), ТекстПредупреждения),
				РежимДиалогаВопрос.ДаНет,
				,
				,
				НСтр("ru='Ошибка при формировании машиночитаемой формы';uk='Ошибка при формировании машиночитаемой формы'"));
...
"Открыть справочную информацию с описанием особенностей настроек веб-браузера?';uk='Формирование бланка формы прервано. Не удалось подключить вспомогательную"
"компоненту для формирования бланков форм с двухмерным штрихкодом PDF417."
"Возможно, браузер не настроен в соответствии с особенностями настройки веб-браузера."
