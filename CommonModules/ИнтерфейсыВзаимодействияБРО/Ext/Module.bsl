...
Функция ПоляКоторыеНеВсегдаТребуетсяЗаполнять(ИмяРеглОтчета, ИмяФормы, Период) Экспорт
	...
	Если ИмяРеглОтчета = "РегламентированныйОтчетСтатистикаФормаП3" 
		И (ИмяФормы = "ФормаОтчета2013Кв1"
			...
			ИЛИ ИмяФормы = "ФормаОтчета2016Кв1") Тогда
	...
	ИначеЕсли ИмяРеглОтчета = "РегламентированныйОтчетСтатистикаФормаП1"
			И (ИмяФормы = "ФормаОтчета2010Кв1" Или ИмяФормы = "ФормаОтчета2014Кв1" Или ИмяФормы = "ФормаОтчета2015Кв1" Или ИмяФормы = "ФормаОтчета2016Кв1") Тогда
...
Процедура ОбновитьДанныеРеглОтчета(ИсточникОтчета, Организация, ДатаНачала, ДатаОкончания, СсылкаНаСохрРеглОтчет) Экспорт
	...
	Если ИмяФормыОтчета = Неопределено Тогда
		...
		Сообщение.Текст = НСтр("ru='Не удалось обновить данные отчета!';uk='Не удалось обновить данные отчета!'");
		Сообщение.Сообщить();
...
Функция СформироватьАвтоматическиРеглОтчет(ИсточникОтчета, Организация, ДатаНачала, ДатаОкончания, КорректирующийОтчет = Ложь,
	...
	Если ИмяФормыОтчета = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не удалось сформировать отчет!';uk='Не удалось сформировать отчет!'");
		Сообщение.Сообщить();
...
Процедура ЗаполнитьСведенияОПодписантах(Организация, РазделыОтчета, ДатаПодписи)
	...
	КодНО = СокрЛП(РазделыОтчета.Титульный.Данные.НалоговыйОрган);
	КПП = СокрЛП(РазделыОтчета.Титульный.Данные.КПП);
