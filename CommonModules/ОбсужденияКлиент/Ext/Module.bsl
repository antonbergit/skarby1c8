...
Процедура ДобавитьСообщение(Комментарий, Ссылка, Форма) Экспорт
	...
	Если (Ссылка.Пустая() Или Форма.Модифицированность) Тогда
		ТекстВопроса = НСтр("ru='Чтобы оставлять комментарии, нужно записать изменения. Продолжить?';uk='Щоб залишати коментарі, потрібно записати зміни. Продовжити?'");
	...
	Если ИмяСобытия=ИмяСобытияЗаписьПрисоединенногоФайла() Тогда
		Если Параметр.Свойство("ВладелецФайла") И Параметр.ВладелецФайла = Ссылка Тогда
			...
			Если Параметр.Свойство("ЭтоНовый") И Параметр.ЭтоНовый=Истина Тогда
				ОбсужденияВызовСервера.ДобавитьСообщение(НСтр("ru='Присоединен файл ';uk='Приєднаний файл'")+ТекстИсточник, Ссылка, Истина);
			Иначе
				ОбсужденияВызовСервера.ДобавитьСообщение(НСтр("ru='Записан файл ';uk='Записаний файл'")+ТекстИсточник, Ссылка,Истина);
			КонецЕсли;
