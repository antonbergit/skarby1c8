...
""
"Ссылка на файл: ""%1"".';uk='Двоічні дані файлу відсутні в регістрі ПрисоединенныеФайлы"
""
"Посилання на файл: ""%1"".'"),
...
Функция ПолучитьДанныеФайла(Знач ПрисоединенныйФайл,
	...
	ОбщегоНазначенияКлиентСервер.Проверить(ФайлОбъект <> Неопределено, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не найден присоединенный файл ""%1"" (%2)';uk='Не знайдений приєднаний файл ""%1"" (%2)'"),
...
Функция ДобавитьПрисоединенныйФайл(ПараметрыФайла,
	...
	Если НоваяСсылкаНаФайл = Неопределено Тогда
		ИмяСправочника = ПрисоединенныеФайлыСлужебный.ИмяСправочникаХраненияФайлов(ВладелецФайлов, "", ЗаголовокОшибки,
			НСтр("ru='В этом случае параметр ""НоваяСсылкаНаФайл"" должен быть указан.';uk='У цьому випадку параметр ""НоваяСсылкаНаФайл"" повинен бути зазначений.'"));
	...
	Если Не Источник.ЭтоНовый() Тогда
		...
		Если ИзмененаПометкаУдаления И ЗначениеЗаполнено(Источник.Редактирует) Тогда
			...
			Если Источник.Редактирует = Пользователи.АвторизованныйПользователь() Тогда
				...
				ТекстОшибки = НСтр("ru='Действие недоступно, так как файл ""%1"" занят для редактирования.';uk='Дія недоступна, тому що файл ""%1"" зайнятий для редагування.'");
...
"пользователем %2.';uk='Дія недоступна, тому що файл ""%1"" зайнятий для редагування"
"користувачем %2.'");
...
Процедура ПометитьНаУдалениеПрисоединенныеФайлы(Знач Источник, ИмяСправочника = Неопределено)
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru='Ошибка при пометке на удаление присоединенных файлов.';uk='Помилка при позначенні на видалення приєднаних файлів.'")
			+ Символы.ПС
			+ ПредставлениеОшибки;
	КонецПопытки;
