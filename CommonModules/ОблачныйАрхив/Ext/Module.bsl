...
Процедура СобратьДанныеПоОблачномуАрхиву(ДополнительныеСвойства, АдресХранилища = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru='Сбор данных в фоне';uk='Сбор данных в фоне'"));
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
			НСтр("ru='Получение массива шагов';uk='Получение массива шагов'"));
		...
		Если МассивШагов.Количество() > 0 Тогда
			...
			ОписаниеРезультатаВложенный = СтрШаблон(НСтр("ru='Получено %1 шагов для выполнения';uk='Получено %1 шагов для выполнения'"), МассивШагов.Количество());
		Иначе
		...
		Для Каждого ТекущийШаг Из МассивШагов Цикл
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				ШагВыполненияВложенный,
				"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
				СтрШаблон(НСтр("ru='Выполнение шага: %1';uk='Выполнение шага: %1'"),ТекущийШаг.ОписаниеШага));
...
#Область ИнформацияОКлиенте
				...
				Иначе
					КодРезультатаВложенный      = 0;
					ОписаниеРезультатаВложенный = НСтр("ru='Собранные ранее данные еще не устарели';uk='Собранные ранее данные еще не устарели'");
				КонецЕсли;
...
"%3"
"';uk='Произошла ошибка %1 сбора данных на шаге %2:"
"%3"
"'"),
						КодРезультатаВложенный,
						ТекущийШаг.КодШага,
...
"';uk='Сбор данных шага %1 успешно завершен"
"'"),
						ТекущийШаг.КодШага);
			КонецЕсли;
			...
			ДлительныеОперации.СообщитьПрогресс(
				(НомерШага / МассивШагов.Количество()) * 100,
				СтрШаблон(НСтр("ru='Выполнение шага: %1. Завершено.';uk='Выполнение шага: %1. Завершено.'"), ТекущийШаг.ОписаниеШага),
				Новый Структура("КодСостояния, ОписаниеРезультата",
					КодРезультатаВложенный,
					Неопределено));
	...
	ДлительныеОперации.СообщитьПрогресс(
		101, // > 100%
		НСтр("ru='СобратьДанныеПоОблачномуАрхиву. Завершено.';uk='СобратьДанныеПоОблачномуАрхиву. Завершено.'"),
		Новый Структура("КодСостояния, ОписаниеРезультата",
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Все обновления Облачного архива';uk='БИП:ОблачныйАрхив.Все обновления Облачного архива'"), // Имя события
		СтрШаблон(
			НСтр("ru='Облачный архив. Все обновления Облачного архива. %1';uk='Облачный архив. Все обновления Облачного архива. %1'"),
...
Функция ДоступнаАктивацияАгентаКопированияНаКомпьютере(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		ШагВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		СтрШаблон(НСтр("ru='Вызов процедуры / функции (%1)';uk='Вызов процедуры / функции (%1)'"), НаименованиеПроцедурыФункции));
...
"'"),
		...
		ШагВыполнения.ВремяНачала);
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Все обновления Облачного архива';uk='БИП:ОблачныйАрхив.Все обновления Облачного архива'"), // ИмяСобытия
		СтрШаблон(
			НСтр("ru='Облачный архив. Все обновления Облачного архива. %1. Начало';uk='Облачный архив. Все обновления Облачного архива. %1. Начало'"),
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			ШагВыполненияВложенный,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
			СтрШаблон(НСтр("ru='Загрузка информации из приватного веб-сервиса (%1)';uk='Загрузка информации из приватного веб-сервиса (%1)'"), "isMachineIDAvailable"));
			...
			Если (ТипЗнч(Результат) <> ТипОбъектXDTO) // Неправильный тип
				...
				КонтекстВыполненияВложенный.ОписаниеРезультата = СтрШаблон(
					НСтр("ru='Веб-сервис вернул значение неправильного типа: %1';uk='Веб-сервис вернул значение неправильного типа: %1'"),
					ТипЗнч(Результат));
				ЗаполнитьЗначенияСвойств(КонтекстВыполнения, КонтекстВыполненияВложенный, "КодРезультата, ОписаниеРезультата"); // Установить код ошибки для самого верхнего уровня Контекста выполнения.
				ЕстьОшибки = Истина;
			ИначеЕсли (ТипЗнч(Результат) = ТипОбъектXDTO)
				...
				КонтекстВыполненияВложенный.ОписаниеРезультата = СтрШаблон(
					НСтр("ru='Веб-сервис вернул ошибку с кодом: %1';uk='Веб-сервис вернул ошибку с кодом: %1'"),
					Результат.errorCode);
				ЗаполнитьЗначенияСвойств(КонтекстВыполнения, КонтекстВыполненияВложенный, "КодРезультата, ОписаниеРезультата"); // Установить код ошибки для самого верхнего уровня Контекста выполнения.
				ЕстьОшибки = Истина;
			КонецЕсли;
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			СтрШаблон(НСтр("ru='Отключение от веб-сервиса (%1)';uk='Отключение от веб-сервиса (%1)'"), "logout"));
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Все обновления Облачного архива';uk='БИП:ОблачныйАрхив.Все обновления Облачного архива'"), // ИмяСобытия
		СтрШаблон(
			НСтр("ru='Облачный архив. Все обновления Облачного архива. %1. Конец';uk='Облачный архив. Все обновления Облачного архива. %1. Конец'"),
			НаименованиеПроцедурыФункции), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
...
"'"),
	...
	Если (ДатаПредыдущегоЗапуска + 30) <= ТекущаяДатаДляРегистров Тогда // Запускалось предыдущий раз более 30 секунд назад.
		...
		Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				СтрШаблон(НСтр("ru='Загрузка информации из приватного веб-сервиса (%1)';uk='Загрузка информации из приватного веб-сервиса (%1)'"), "updateAccountStats"));
			...
			Если ЕстьОшибки <> Истина Тогда
				...
				ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
					...
					НСтр("ru='Сохранение информации в ИБ';uk='Сохранение информации в ИБ'"));
	...
	Иначе
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
			...
			380,
			СтрШаблон(
				НСтр("ru='Обновление не осуществлялось: предыдущее обновление было менее 30 секунд назад (было %1, сейчас %2)';uk='Обновление не осуществлялось: предыдущее обновление было менее 30 секунд назад (было %1, сейчас %2)'"),
	...
	Иначе // Облачный архив
		...
		МассивШагов = Новый Массив;
			...
			ШагСбораДанных = ОблачныйАрхивКлиентСервер.ПолучитьСтруктуруШагаСбораДанных();
				...
				ШагСбораДанных.ОписаниеШага          = НСтр("ru='Сбор информации о клиентском компьютере';uk='Сбор информации о клиентском компьютере'");
			...
			ШагСбораДанных = ОблачныйАрхивКлиентСервер.ПолучитьСтруктуруШагаСбораДанных();
				...
				ШагСбораДанных.ОписаниеШага          = НСтр("ru='Проверка активации Агента резервного копирования на этом компьютере';uk='Проверка активации Агента резервного копирования на этом компьютере'");
				ШагСбораДанных.СрокУстареванияСекунд = 5*60; // Обновлять только если данные были собраны > 5 минут назад.
			МассивШагов.Добавить(ШагСбораДанных);
		...
		ФоновоеЗадание = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			НСтр("ru='Сбор данных для Облачного архива';uk='Сбор данных для Облачного архива'"),
			Ложь);
...
Процедура ПанельАдминистрированияБСП_ПереключитьНаПравильнуюСтраницу(Форма, СобратьДанныеОКлиенте = Ложь) Экспорт
	...
	ИначеЕсли Форма.СпособРезервногоКопирования = "1С:Облачный архив" Тогда
		...
		Если ИнформацияОКлиенте.ЭтоАдминистраторWindows Тогда
			...
			КонецЕсли;
		Иначе
			Элементы.ДекорацияОблачныйАрхивОшибки.Заголовок = НСтр("ru='Для подключения, выполнения резервного копирования и настройки Агента резервного копирования необходимы права администратора Windows на этом компьютере.';uk='Для подключения, выполнения резервного копирования и настройки Агента резервного копирования необходимы права администратора Windows на этом компьютере.'");
	...
	Если ОблачныйАрхивПовтИсп.РазрешенаРаботаСОблачнымАрхивом() Тогда
		...
		Если НЕ МодульТекущиеДелаСервер.ДелоОтключено(ИдентификаторДела) Тогда
			...
			Если Результат.ТребуетсяУстановка = Истина Тогда
				...
				Дело.Важное         = Истина;
				Дело.Представление  = НСтр("ru='Для работы 1С:Облачный архив необходимо установить агента резервного копирования';uk='Для работы 1С:Облачный архив необходимо установить агента резервного копирования'");
			...
			ИначеЕсли Результат.ТребуетсяОбновление = 1 Тогда // Доступно обновление.
				...
				Дело.Представление  = СтрШаблон(
					НСтр("ru='Доступно обновление %1 агента резервного копирования';uk='Доступно обновление %1 агента резервного копирования'"),
			...
			ИначеЕсли Результат.ТребуетсяОбновление = 2 Тогда // Рекомендуется обновление.
				...
				Дело.Важное         = Ложь;
				Дело.Представление  = СтрШаблон(
					НСтр("ru='Рекомендуется обновление агента резервного копирования до версии %1';uk='Рекомендуется обновление агента резервного копирования до версии %1'"),
					ИнтернетПоддержкаПользователейКлиентСервер.ПользовательскоеПредставлениеНомераВерсии(
						Результат.ВерсияНовейшегоАгентаКопирования,
						Ложь));
				Дело.Форма          = "Обработка.ОблачныйАрхив.Форма.МастерОбновленияАгента";
			...
			ИначеЕсли Результат.ТребуетсяОбновление = 3 Тогда // Необходимо обновление.
				...
				Дело.Важное         = Истина;
				Дело.Представление  = СтрШаблон(
					НСтр("ru='Необходимо обновить агента резервного копирования до версии %1';uk='Необходимо обновить агента резервного копирования до версии %1'"),
					ИнтернетПоддержкаПользователейКлиентСервер.ПользовательскоеПредставлениеНомераВерсии(
						Результат.ВерсияНовейшегоАгентаКопирования,
						Ложь));
				Дело.Форма          = "Обработка.ОблачныйАрхив.Форма.МастерОбновленияАгента";
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		443,
		НСтр("ru='Веб-сервисы авторизации в 1С';uk='Веб-сервисы авторизации в 1С'"));
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		443,
		НСтр("ru='Веб-сервисы работы с облачным архивом (публичные и приватные)';uk='Веб-сервисы работы с облачным архивом (публичные и приватные)'"));
	НовыеРазрешения.Добавить(Разрешение);
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		443,
		НСтр("ru='Веб-сервисы информации об агенте копирования';uk='Веб-сервисы информации об агенте копирования'"));
	НовыеРазрешения.Добавить(Разрешение);
...
Процедура ВыгрузитьФайлВОблако(ИмяФайла, ОтсрочкаСтартаСекунд = 30) Экспорт
	...
	МассивШагов = Новый Массив;
		...
		ШагСбораДанных = ОблачныйАрхивКлиентСервер.ПолучитьСтруктуруШагаСбораДанных();
			...
			ШагСбораДанных.ОписаниеШага          = НСтр("ru='Чтение настроек Агента резервного копирования';uk='Чтение настроек Агента резервного копирования'");
			ШагСбораДанных.СрокУстареванияСекунд = СрокЖизниСтатистики; // Обновлять только если данные были собраны > 5 минут назад.
		МассивШагов.Добавить(ШагСбораДанных);
...
Функция ПолучитьПроксиВебСервисаАвторизации1С(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		ШагВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("ru='Подключение к прокси веб-сервиса (авторизация 1С)';uk='Подключение к прокси веб-сервиса (авторизация 1С)'"));
...
"%1';uk='Неудачное подключение к веб-сервису (авторизация 1С) по причине:"
"%1'"),
			лкПодробноеПредставлениеОшибки);
		// Запись в журнал регистрации.
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:ОблачныйАрхив.Веб-сервисы';uk='БИП:ОблачныйАрхив.Веб-сервисы'"), // Имя события
			НСтр("ru='Облачный архив. Веб-сервисы. Подключение к серверу';uk='Облачный архив. Веб-сервисы. Подключение к серверу'"), // ИмяСобытия
...
Функция ПолучитьПроксиВебСервисаБэкап1СПубличный(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		ШагВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("ru='Подключение к прокси веб-сервиса (публичный backup.1c.ru)';uk='Подключение к прокси веб-сервиса (публичный backup.1c.ru)'"));
...
"%1';uk='Неудачное подключение к веб-сервису (публичный, backup.1c.ru) по причине:"
"%1'"),
			лкПодробноеПредставлениеОшибки);
		// Запись в журнал регистрации.
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
...
"'"),
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			ШагВыполненияВложенный,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции + ".ЗагрузкаДанных",
			СтрШаблон(НСтр("ru='Загрузка информации из публичного веб-сервиса (%1)';uk='Загрузка информации из публичного веб-сервиса (%1)'"), "getBackupAgentInfo"));
...
Функция ПолучитьПроксиВебСервисаБэкап1СПриватный(КонтекстВыполнения = Неопределено) Экспорт
	...
	Если (ТипЗнч(ПроксиЛогин1С) = Тип("WSПрокси")) И (КонтекстВыполнения.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ru='Получение тикета';uk='Получение тикета'"));
		...
		Если ЕстьОшибки <> Истина Тогда
			...
			Если (ТипЗнч(ПроксиПубличныйБэкап1С) = Тип("WSПрокси")) И (КонтекстВыполнения.КодРезультата = 0) Тогда
				...
				ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
					...
					СтрШаблон(НСтр("ru='Загрузка информации из публичного веб-сервиса (%1)';uk='Загрузка информации из публичного веб-сервиса (%1)'"), "login"));
				...
				Если ЕстьОшибки <> Истина Тогда
					...
					ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
						ШагВыполнения,
						"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
						НСтр("ru='Подключение к прокси веб-сервиса (приватный backup.1c.ru)';uk='Подключение к прокси веб-сервиса (приватный backup.1c.ru)'"));
...
"%1';uk='Неудачное подключение к веб-сервису (приватный, backup.1c.ru) по причине:"
"%1'"),
							лкПодробноеПредставлениеОшибки);
						// Запись в журнал регистрации.
						ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
...
Процедура ЗагрузитьИнформациюСтатусаАккаунта(КонтекстВыполнения = Неопределено)
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			ШагВыполненияВложенный,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции + ".ЗагрузкаДанных",
			СтрШаблон(НСтр("ru='Загрузка информации из приватного веб-сервиса (%1)';uk='Загрузка информации из приватного веб-сервиса (%1)'"), "getAccountInfo"));
...
"'"),
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			ШагВыполненияВложенный,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции + ".ЗагрузкаДанных",
			СтрШаблон(НСтр("ru='Загрузка информации из приватного веб-сервиса (%1)';uk='Загрузка информации из приватного веб-сервиса (%1)'"), "getAccountStats"));
...
"'"),
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			СтрШаблон(НСтр("ru='Загрузка информации из приватного веб-сервиса (%1)';uk='Загрузка информации из приватного веб-сервиса (%1)'"), "activateAccount"));
...
"'"),
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			ШагВыполненияВложенный,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции + ".ЗагрузкаДанных",
			СтрШаблон(НСтр("ru='Загрузка информации из приватного веб-сервиса (%1)';uk='Загрузка информации из приватного веб-сервиса (%1)'"), "getFileList"));
...
Процедура УдалитьРезервнуюКопию(КонтекстВыполнения) Экспорт
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ru='Удаление файла с сервера';uk='Удаление файла с сервера'"));
			...
			Попытка
				...
				ОписаниеРезультата = СтрШаблон(
					НСтр("ru='Удаление файла с UIN %1 выполнено успешно';uk='Удаление файла с UIN %1 выполнено успешно'"),
					КонтекстВыполнения.ИдентификаторФайла);
			Исключение
				Результат = -1;
				ИнформацияОбОшибке = ИнформацияОбОшибке();
			...
			ИначеЕсли (ТипЗнч(Результат) = ТипЧисло) Тогда
				Если Результат = 0 Тогда
					// Файл удален
					ОписаниеРезультата = НСтр("ru='Файл резервной копии с идентификатором %1 успешно удален с сервера';uk='Файл резервной копии с идентификатором %1 успешно удален с сервера'");
					ОписаниеРезультата = СтрШаблон(
						НСтр("ru='Файл резервной копии с идентификатором %1 успешно удален с сервера.';uk='Файл резервной копии с идентификатором %1 успешно удален с сервера.'"),
						КонтекстВыполнения.ИдентификаторФайла);
				Иначе // Ошибка
					ОписаниеРезультата = СтрШаблон(
						НСтр("ru='Произошла ошибка удаления файла резервной копии с сервера:"
"Код ошибки: %1"
"Идентификатор файла: %2';uk='Произошла ошибка удаления файла резервной копии с сервера:"
"Код ошибки: %1"
...
"Результат выполнения:"
"%4"
"';uk='Удаление файла резервной копии с сервера, итоговый отчет."
"Время начала (мс): %1"
"Время окончания (мс): %2"
"Длительность (мс): %3"
"Результат выполнения:"
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Веб-сервисы';uk='БИП:ОблачныйАрхив.Веб-сервисы'"), // Имя события
		НСтр("ru='Облачный архив. Веб-сервисы. Удаление файла резервной копии';uk='Облачный архив. Веб-сервисы. Удаление файла резервной копии'"), // ИмяСобытия
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
...
#Область ПолучениеСпискаОСиВерсий
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Получение списка ОС и версий ОС';uk='Получение списка ОС и версий ОС'"));
...
"%1';uk='Получен список ОС и версий:"
"%1'"),
...
#Область ПреобразованиеСпискаОСиВерсий
	...
	Если ТаблицаВсехОСиВерсий <> Неопределено Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ru='Преобразование списка ОС и версий ОС';uk='Преобразование списка ОС и версий ОС'"));
...
"%1';uk='Преобразован список ОС и версий:"
"%1'"),
			ТелоОтправки);
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
			ШагВыполнения,
...
#Область ЗагрузкаДанных
	...
	Если ТаблицаВсехОСиВерсий <> Неопределено Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			ШагВыполнения,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции + ".ЗагрузкаДанных",
			НСтр("ru='Загрузка данных с серверов 1С';uk='Загрузка данных с серверов 1С'"));
...
"%2"
"Результат:"
"%3';uk='Получены данные для версий Агента резервного копирования."
"Код состояния: %1"
"Заголовки:"
"%2"
"Результат:"
...
#Область ПарсингПолученныхДанных
	...
	Если ЕстьОшибки = Ложь Тогда
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ru='Парсинг загруженных данных';uk='Парсинг загруженных данных'"));
			...
			Если (ЗагруженныеДанные.Свойство("backupAgentInfoList"))
				...
				ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
					...
					НСтр("ru='Парсинг загруженных данных, загрузка файлов описаний';uk='Парсинг загруженных данных, загрузка файлов описаний'"));
...
"%1';uk='Список URL файлов описаний:"
"%1'"),
				...
				ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
					...
					НСтр("ru='Парсинг загруженных данных, сохранение данных в ИБ';uk='Парсинг загруженных данных, сохранение данных в ИБ'"));
...
"';uk='Для строки %1 не заполнены обязательные поля: %2"
"'"),
...
"%1"
"';uk='Обработаны данные для версий Агента резервного копирования:"
"%1"
"'"),
...
Процедура ВсеОбновленияОблачногоАрхива() Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		ШагВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("ru='Выполнение регламентных заданий подсистемы Облачный архив';uk='Выполнение регламентных заданий подсистемы Облачный архив'"));
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Все обновления Облачного архива';uk='БИП:ОблачныйАрхив.Все обновления Облачного архива'"), // ИмяСобытия
		НСтр("ru='Облачный архив. Все обновления Облачного архива. Начало';uk='Облачный архив. Все обновления Облачного архива. Начало'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
	...
	Если МонопольныйРежим() Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:ОблачныйАрхив.Все обновления Облачного архива';uk='БИП:ОблачныйАрхив.Все обновления Облачного архива'"), // ИмяСобытия
			НСтр("ru='Облачный архив. Все обновления Облачного архива. Конец';uk='Облачный архив. Все обновления Облачного архива. Конец'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияОблачногоАрхива, // ОбъектМетаданных
			, // Данные
...
#Область ЗагрузкаИнформацииОбАктуальныхВерсияхАгентаКопирования
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				СтрШаблон(НСтр("ru='1. %1';uk='1. %1'"), ВидОбновления));
...
#Область ЗагрузкаИнформацииОбАктивированныхАгентахКопирования
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				СтрШаблон(НСтр("ru='2. %1';uk='2. %1'"), ВидОбновления));
...
#Область ЗагрузкаСпискаРезервныхКопий
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				СтрШаблон(НСтр("ru='3. %1';uk='3. %1'"), ВидОбновления));
...
#Область ЗагрузкаИнформацииОСостоянииИспользованияОблачногоАрхива
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				СтрШаблон(НСтр("ru='4. %1';uk='4. %1'"), ВидОбновления));
...
"По шагам:"
"%4"
"';uk='Завершение выполнения регламентного задания"
"Время начала (мс): %1"
"Время окончания (мс): %2"
"Длительность (мс): %3"
""
...
Процедура ИзменитьИспользованиеРегламентныхЗаданий(Использовать) Экспорт
	...
	РегламентныеЗаданияОблачнногоАрхива = РегламентныеЗадания.ПолучитьРегламентныеЗадания(Новый Структура("Метаданные", Метаданные_РегламентныеЗадания_ВсеОбновленияОблачногоАрхива));

	Если Использовать = Истина Тогда
		// Добавить, если нет.
		Найдено = Ложь;
		Для Каждого ТекущееРегламентноеЗадание Из РегламентныеЗаданияОблачнногоАрхива Цикл
			Найдено = Истина;
			Прервать;
		КонецЦикла;
		...
		ТекущееРегламентноеЗадание.Наименование                             = НСтр("ru='Все обновления Облачного архива';uk='Все обновления Облачного архива'");
...
"в параметре Использовать передано значение неправильного типа: %1';uk='ОблачныйАрхив.ИзменитьИспользованиеРегламентныхЗаданий:"
"в параметре Использовать передано значение неправильного типа: %1'"),
			ТипЗнч(Использовать));
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:ОблачныйАрхив.Сервис и регламент';uk='БИП:ОблачныйАрхив.Сервис и регламент'"), // ИмяСобытия
			НСтр("ru='Облачный архив. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий';uk='Облачный архив. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"%2"
"';uk='Описание времени некорректное: %1"
"Произошла ошибка преобразования:"
"%2"
"'"),
					ТекстВремя,
...
#Область ОсновноеОбновление
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ru='Основное обновление';uk='Основное обновление'"));
...
#Область ОсновноеОбновление_ЗагрузитьВсеСтандартныеЗначения
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ru='Основное обновление, загрузка стандартных значений';uk='Основное обновление, загрузка стандартных значений'"));
...
#Область ПереопределяемоеОбновление
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ru='Переопределяемое обновление';uk='Переопределяемое обновление'"));
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Сервис и регламент';uk='БИП:ОблачныйАрхив.Сервис и регламент'"), // ИмяСобытия
		НСтр("ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПервыйЗапуск';uk='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПервыйЗапуск'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Сервис и регламент';uk='БИП:ОблачныйАрхив.Сервис и регламент'"), // ИмяСобытия
		НСтр("ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПервыйЗапуск';uk='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПервыйЗапуск'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Сервис и регламент';uk='БИП:ОблачныйАрхив.Сервис и регламент'"), // ИмяСобытия
		НСтр("ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПерейтиНаВерсию';uk='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПерейтиНаВерсию'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
#Область ОсновноеОбновление_ИзменитьИспользованиеРегламентныхЗаданий
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ru='Основное обновление, установка расписания';uk='Основное обновление, установка расписания'"));
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Сервис и регламент';uk='БИП:ОблачныйАрхив.Сервис и регламент'"), // ИмяСобытия
		НСтр("ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПерейтиНаВерсию';uk='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПерейтиНаВерсию'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
Функция СобратьДанные_ИнформацияОКлиенте(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		ШагВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("ru='Обновление информации о клиенте';uk='Обновление информации о клиенте'"));
...
#Область Шаг_ПолучениеДанныхИзРеестраWindows
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		...
		НСтр("ru='Получение информации об установленном агенте копирования и параметрах компьютера (из реестра Windows)';uk='Получение информации об установленном агенте копирования и параметрах компьютера (из реестра Windows)'"));
...
"%1"
"';uk='Ошибка получения параметров агента копирования (тип операционной системы Windows) (через объекты OLE, переменные окружения) по причине:"
"%1"
"'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
"';uk='Ошибка получения параметров агента копирования (ТипОС). Причины были указаны выше."
"'");
		ОписаниеРезультата = ОписаниеРезультата + ОписаниеРезультатаШага;
	КонецЕсли;
#КонецОбласти
...
"Тип платформы: %1"
"Тип ОС: %2';uk='Не получен каталог установки и версия агента, т.к. операционная система не относится к Windows."
"Тип платформы: %1"
"Тип ОС: %2'"),
...
"';uk='Данные успешно получены из реестра Windows."
"'");
		ОписаниеРезультата = ОписаниеРезультата + ОписаниеРезультатаШага;
	КонецЕсли;
#КонецОбласти
...
"%1"
"';uk='Ошибка получения параметров агента копирования из реестра Windows (через объекты OLE) по причине:"
"%1"
"'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			ОписаниеРезультата = ОписаниеРезультата + ОписаниеРезультатаШага;
...
"%1"
"';uk='Ошибка получения параметров агента копирования из реестра Windows (через reg.exe) по причине:"
"%1"
"'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			ОписаниеРезультата = ОписаниеРезультата + ОписаниеРезультатаШага;
...
"%1"
"';uk='Ошибка удаления временного файла для получения значения из реестра (через reg.exe) по причине:"
"%1"
"'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				ОписаниеРезультата = ОписаниеРезультата + ОписаниеРезультатаШага;
...
"%1"
"';uk='Ошибка получения параметров агента копирования из реестра Windows (через BackupAgent.exe) по причине:"
"%1"
"'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				ОписаниеРезультата = ОписаниеРезультата + ОписаниеРезультатаШага;
...
"';uk='Ошибка получения параметров агента копирования (ИдентификаторКомпьютера). Причины были указаны выше."
"'");
			ОписаниеРезультата = ОписаниеРезультата + ОписаниеРезультатаШага;
		КонецЕсли;
...
#Область Шаг_ПроверкаДанныхОбАгенте_Ключи
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ru='Проверка наличия файлов программы и файлов ключей';uk='Проверка наличия файлов программы и файлов ключей'"));
...
"';uk='Файл программы %1 отсутствует"
"'"),
							ТекущийФайл);
				Иначе
					ОписаниеРезультатаШага = ОписаниеРезультатаШага
...
"'"),
			...
			Если ЕстьОшибки = Истина Тогда
				...
				ОписаниеРезультатаШага = ОписаниеРезультатаШага
					+ НСтр("ru='Отсутствуют файлы, необходимые для работы Агента резервного копирования';uk='Отсутствуют файлы, необходимые для работы Агента резервного копирования'");
				КонтекстВыполнения.КодРезультата = КонтекстВыполненияВложенный.КодРезультата; // Установить параметры корневого контекста выполнения
		...
		Иначе
			...
			ОписаниеРезультатаШага = ОписаниеРезультатаШага
				+ СтрШаблон(
					НСтр("ru='Отсутствует каталог установки Агента резервного копирования (%1)';uk='Отсутствует каталог установки Агента резервного копирования (%1)'"),
...
Функция СобратьДанные_НастройкиАгентаКопированияКлиент(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
		ШагВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("ru='Обновление информации о настройках агента копирования';uk='Обновление информации о настройках агента копирования'"));
...
#Область Шаг_ПроверкаДанныхОбАгенте_Настройки_Каталоги
		...
		ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
			...
			НСтр("ru='Получение настроек Агента резервного копирования';uk='Получение настроек Агента резервного копирования'"));
		...
		Если Каталог.Существует() Тогда
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
				...
				0,
				СтрШаблон(
					НСтр("ru='Каталог настроек Агента копирования обнаружен: %1';uk='Каталог настроек Агента копирования обнаружен: %1'"),
					ИмяКаталогаНастроек),
				Неопределено);
			КонтекстВыполненияВложенный.ЛогШаговВыполнения.Добавить(ШагВыполненияВложенный);
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ru='Проверка каталогов для работы Агента резервного копирования';uk='Проверка каталогов для работы Агента резервного копирования'"));
				...
				КаталогиДляПроверки.Добавить(ИмяКаталогаЛогов, НСтр("ru='Каталог логов';uk='Каталог логов'"));
				КаталогиДляПроверки.Добавить(ИмяКаталогаСтатистики, НСтр("ru='Каталог статистики';uk='Каталог статистики'"));
				КаталогиДляПроверки.Добавить(ИмяКаталогаРабочий, НСтр("ru='Рабочий каталог';uk='Робочий каталог'"));
...
"';uk='%1 (%2) отсутствовал, но был успешно создан."
"'"),
									ТекущийКаталог.Представление,
									ТекущийКаталог.Значение);
						Исключение
...
"%3"
"';uk='%1 (%2) отсутствовал, создать его не удалось по причине:"
"%3"
"'"),
									ТекущийКаталог.Представление,
									ТекущийКаталог.Значение,
...
#Область Шаг_ПроверкаДанныхОбАгенте_Настройки_Расписание
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьНачалоШагаВыполнения(
				...
				НСтр("ru='Загрузка настроек этой ИБ из рабочего каталога';uk='Загрузка настроек этой ИБ из рабочего каталога'"));
...
"';uk='Найден файл настроек Агента резервного копирования для ИБ (%1). Производится считывание настроек."
"'"),
								ИмяФайлаНастроекИБ);
...
"';uk='Настройка расписания BackupNow пропущена."
"'");
									КонецЕсли;
	...
	#Область Шаг_ПроверкаДанныхОбАгенте_Настройки_Расписание_Ежедневно
									...
									Если РасписаниеНайдено = Ложь Тогда
										Если СтруктураРасписания.Элементы.Свойство("Time") Тогда
											ВложенноеОписаниеРезультата = ВложенноеОписаниеРезультата
												+ НСтр("ru='Найдена настройка расписания Time.';uk='Найдена настройка расписания Time.'")
												+ Символы.ПС;
...
"';uk='Найдена настройка расписания BackupDaily."
"'");
...
"';uk='Найдена настройка расписания BackupDaily (начало, окончание, периодичность)."
"'");
...
"';uk='Найдена настройка расписания BackupDaily (ежедневно)."
"'");
...
"';uk='Найдена настройка расписания BackupWeekly."
"'");
...
"';uk='Найдена настройка расписания BackupMonthly."
"'");
...
"';uk='Найдена настройка расписания BackupMonthly (время, дни месяца)."
"'");
												ТекстВремя = ТекущийЭлементРасписания.Атрибуты.time;
												Время = ПолучитьВремяИзТекста(ТекстВремя, ВложенноеОписаниеРезультата);
...
"';uk='Найдена настройка расписания BackupMonthly (время, дни недели)."
"'");
													ТекстВремя = ТекущийЭлементРасписания.Атрибуты.time;
													Время = ПолучитьВремяИзТекста(ТекстВремя, ВложенноеОписаниеРезультата);
...
"';uk='Файл настроек Агента резервного копирования для ИБ НЕ найден. Будет настроено расписание по-умолчанию."
"'");
					КонецЕсли;
				Иначе
...
"';uk='Настройки расписания сгенерированы по-умолчанию."
"'");
				Иначе
...
#КонецОбласти
		...
		Иначе
			...
			ОблачныйАрхивКлиентСервер.ЗарегистрироватьКонецШагаВыполнения(
				...
				124,
				СтрШаблон(
					НСтр("ru='Отсутствует каталог настроек установленного Агента резервного копирования (%1). Возможно, он был удален вручную.';uk='Отсутствует каталог настроек установленного Агента резервного копирования (%1). Возможно, он был удален вручную.'"),
