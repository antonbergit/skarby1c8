...
"Обратитесь к администратору.';uk='Недостаточно прав для настройки прямого обмена с банком."
...
Процедура ПолучитьВыпискуБанка(НастройкаОбмена, ДатаНачала, Знач ДатаОкончания, Владелец, НомерСчета = Неопределено, ОткрыватьФормуУточненияПериода = Ложь) Экспорт
	...
	Если ДатаНачала > ТекущаяДатаСеанса Тогда
		СообщениеТекст = НСтр("ru='Дата начала периода запроса не должна быть позднее текущей даты.';uk='Дата начала периода запроса не должна быть позднее текущей даты.'");
	...
	Если ДатаОкончания > КонецДня(ТекущаяДатаСеанса) Тогда
		СообщениеТекст = НСтр("ru='Дата конца периода запроса не должна быть позднее текущей даты.';uk='Дата конца периода запроса не должна быть позднее текущей даты.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеТекст, , "КонецПериода");
		Возврат;
	КонецЕсли;
	
	Если ДатаНачала > ДатаОкончания Тогда
		СообщениеТекст = НСтр("ru='Дата начала периода запроса не должна быть больше позднее даты конца периода.';uk='Дата начала периода запроса не должна быть больше позднее даты конца периода.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеТекст, , "НачалоПериода");
		Возврат;
	КонецЕсли;
	...
	Если РеквизитыНастройкиОбмена.Недействительна Тогда
		ТекстСообщения = НСтр("ru='Настройка обмена недействительна, операция невозможна';uk='Настройка обмена недействительна, операция невозможна'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли РеквизитыНастройкиОбмена.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.СбербанкОнлайн") Тогда
		...
		Если МинимальнаяДата > ДатаНачала Тогда
			Если ОткрыватьФормуУточненияПериода Тогда
				...
				Возврат;
			Иначе
				СообщениеТекст = НСтр("ru='Период запроса выписки не должен превышать 14 дней.';uk='Период запроса выписки не должен превышать 14 дней.'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеТекст, , "КонецПериода");
				Возврат;
			КонецЕсли;
		КонецЕсли;
...
"Загрузить выписки из базы или получить новые из банка?';uk='В базе уже есть выписки банка за указанный период."
"Загрузить выписки из базы или получить новые из банка?'");
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить(Истина, НСтр("ru='Загрузить из базы';uk='Загрузить из базы'"));
			Кнопки.Добавить(Ложь, НСтр("ru='Получить из банка';uk='Получить из банка'"));
			Заголовок = НСтр("ru='Получение выписки';uk='Получение выписки'");
...
Процедура ОткрытьАктуальныйЭД(ПараметрКоманды, Источник = Неопределено, ПараметрыОткрытия = Неопределено) Экспорт
	...
	Для Каждого ТекЭл Из МассивСсылок Цикл
		...
		Иначе
			ТекстШаблона = НСтр("ru='%1. Актуальный электронный документ не найден.';uk='%1. Актуальный электронный документ не найден.'");
