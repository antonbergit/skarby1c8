...
Процедура ВыполнитьМетодКонфигурации(ИмяМетода, Параметры = Неопределено)
	...
	Попытка
		...
		Если ТранзакцияАктивна() Тогда
			...
			ШаблонСообщения = НСтр("ru='По завершении выполнения обработчика %1 не была закрыта транзакция';uk='По завершенні виконання обробника %1 не була закрита "
"транзакція'");
		...
		Если Не(РазделительУстановлен) И ОбщегоНазначения.ИспользованиеРазделителяСеанса() Тогда
			...
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='По завершении выполнения обработчика %1 не было выключено разделение сеанса!';uk='По завершенні виконання обробника %1 не було вимкнено "
"розподіл сеансу!'"),
				ИмяМетода);
...
"%3';uk='Встановлення черги з ім`ям методу %1 в області %2 не заплановано через:"
"%3 '");
			...
			ЗаписьЖурналаРегистрации(
			    НСтр("ru='Очередь заданий.Планирование';uk='Черга завдань.Планування'",
			    ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			    УровеньЖурналаРегистрации.Ошибка,,,
				ТекстСообщения);
