...
"Повторно подключите учетную запись Gmail.';uk='Не заповнений токен доступу для відправки поштового повідомлення."
"Повторно підключіть обліковий запис Gmail.'");
		КонецЕсли;
...
Процедура ЗагрузитьПочтуПоКомандеПользователя(Параметры, АдресХранилища) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Загрузка заголовков сообщений электронной почты';uk='Завантаження заголовків повідомлень електронної пошти'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗагрузитьСодержаниеСообщения(Параметры, АдресХранилища) Экспорт
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(Параметры.УчетнаяЗапись) Тогда
			ВызватьИсключение НСтр("ru='Не указана учетная запись для задания ""ЗагрузитьСодержаниеСообщения()"".';uk='Не вказаний обліковий запис для завдання ""ЗагрузитьСодержаниеСообщения()"".'");
		КонецЕсли;
...
"Повторно подключите учетную запись Gmail.';uk='Не заповнений токен доступу для завдання завантаження змісту повідомлення."
"Повторно підключіть обліковий запис Gmail.'");
		КонецЕсли;
	...
	Исключение
		...
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(НСтр("ru='Загрузка содержания сообщения электронной почты';uk='Завантаження вмісту повідомлення електронної пошти'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,,,
		ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		СообщитьОбОшибкеДлительногоЗадания(ИнформацияОбОшибке);
...
Функция ЗаголовокПрогресса(Знач УчетнаяЗапись, Знач ОтборПоКонтакту) Экспорт
	...
	Если ЗначениеЗаполнено(ОтборПоКонтакту) Тогда
		Возврат СтрШаблон(НСтр("ru='Загружается переписка с ""%1"" из ""%2""';uk='Завантажується листування з ""%1"" з ""%2""'"), ОтборПоКонтакту, УчетнаяЗапись);
	Иначе
		Возврат СтрШаблон(НСтр("ru='Получение почты ""%1""';uk='Отримання пошти ""%1""'"), УчетнаяЗапись);
	КонецЕсли;
...
"Повторно подключите учетную запись Gmail.';uk='Не заповнений токен доступу для завдання завантаження заголовків повідомлень."
"Повторно підключіть обліковий запис Gmail.'");
	КонецЕсли;
...
Функция ПротоколИнтернетПочтыIMAP(УчетнаяЗапись)
	...
	ВызватьИсключение НСтр("ru='Подключение к серверу входящей почты может быть выполнено только по протоколу IMAP.';uk='Підключення до сервера вхідної пошти може бути виконане тільки по протоколу IMAP.'");
