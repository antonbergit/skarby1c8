...
Функция РазделитьВерсию(Знач ВерсияСтрокой, Разделитель = ".", ЛюбаяВерсия = "*") Экспорт
	...
	Если НЕ ФорматСоблюден Тогда
		...
		Описание = НСтр("ru='Версия """"{Значение}"""" не соответствует формату Редакция.Подредакция.Версия.Сборка.';uk='Версія """"{Значение}"""" не відповідає формату Редакція.Подредакція.Версія.Сборка.'");
