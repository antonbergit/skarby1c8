...
Процедура ПодтвердитьПолучениеДляДокументовИзСписка(ДинамическийСписок) Экспорт
	...
	Иначе
		...
		ПоказатьПредупреждение(, НСтр("ru='В указанных строках нет документов, для которых требуется подтверждение поступления КиЗ.';uk='В указанных строках нет документов, для которых требуется подтверждение поступления КиЗ.'"));
...
Процедура ПолучитьGLNПоИННКПП(Форма) Экспорт
	...
	Если Не ЗначениеЗаполнено(Контрагент) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнено поле ""Контрагент""';uk='Не заполнено поле ""Контрагент""'"), Форма.Объект.Ссылка, "Объект.Контрагент");
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Данные.ИНН) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнен ИНН контрагента';uk='Не заполнен ИНН контрагента'"), Форма.Объект.Ссылка, "Объект.Контрагент");
		Возврат;
	КонецЕсли;
	...
	Если Данные = Неопределено Тогда
		...
		Если Результат.ИнформацияОВерсиях <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(Результат.ИнформацияОВерсиях.ТекстСообщения) Тогда
				...
				Иначе
					ПоказатьОповещениеПользователя(Результат.ИнформацияОВерсиях.ТекстСообщения,,НСтр("ru='Версия обмена обновлена';uk='Версия обмена обновлена'"), БиблиотекаКартинок.Информация32);
				КонецЕсли;
			КонецЕсли;
...
Процедура Подписать(Сообщения, Организация, ОповещениеПриЗавершении) Экспорт
	...
	Если КоличествоСообщений = 1 Тогда
		ЗаголовокДанных = НСтр("ru='Сообщение';uk='Повідомлення'");
	Иначе
		ЗаголовокДанных = НСтр("ru='Сообщения';uk='Повідомлення'");
	КонецЕсли;
	...
	ОписаниеДанных.Вставить("Операция",            НСтр("ru='Обмен с ГИСМ';uk='Обмін з ДІСМ'"));
...
"электронной подписи, связанных с организацией %2, необходимых для обмена с ГИСМ.';uk='В информационной базе для пользователя %1 не найдено зарегистрированных сертификатов"
"электронной подписи, связанных с организацией %2, необходимых для обмена с ГИСМ.'"),
	...
	ОписаниеДанных.Вставить("ПредставлениеНабора", СтрШаблон(НСтр("ru='Показать (%1)';uk='Показать (%1)'"), КоличествоСообщений));
...
Функция СообщенияПоОрганизациям(Сообщения) Экспорт
	...
	Для Каждого Сообщение Из Сообщения Цикл
		...
		Если ЗначениеЗаполнено(Сообщение.ТекстОшибки) Тогда
			ЗаголовокСообщения = СтрШаблон(НСтр("ru='Сообщение ""%1"" не подготовлено к отправке в ГИСМ:';uk='Сообщение ""%1"" не подготовлено к отправке в ГИСМ:'"), Сообщение.Описание) + Символы.ПС;
...
Процедура ПодготовитьСообщениеКПередаче(ДокументСсылка, Операция) Экспорт
	...
	Если Сообщения = Неопределено И Операция <> Неопределено Тогда
		ПоказатьПредупреждение(,СтрШаблон(НСтр("ru='По операции ""%1"" передача данных в ГИСМ не требуется';uk='По операции ""%1"" передача данных в ГИСМ не требуется'"), Операция));
	КонецЕсли;
	Если Сообщения = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПодготовитьСообщениеКПередаче_ПриЗавершенииОперацииПодписи(Сообщения, ДополнительныеПараметры) Экспорт
	...
	Для Каждого ЭлементДанных Из Результат Цикл
		...
		Если Не Выполнено Тогда
			...
			ТекстЗаголовка = НСтр("ru='При подготовке сообщения к передаче в ГИСМ возникла ошибка:';uk='При подготовке сообщения к передаче в ГИСМ возникла ошибка:'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстЗаголовка + Символы.ПС + ЭлементДанных.ТекстОшибки);
			
		Иначе
			
			ТекстЗаголовка = НСтр("ru='Сообщение подготовлено к передаче в ГИСМ';uk='Сообщение подготовлено к передаче в ГИСМ'");
			Если Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаДанных") Тогда
				ТекстСообщения = НСтр("ru='Для документа %Документ% подготовлено к отправке сообщение передачи данных';uk='Для документа %Документ% подготовлено к отправке сообщение передачи данных'");
			ИначеЕсли Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаЗакрытияЗаявки") Тогда
				ТекстСообщения = НСтр("ru='Для документа %Документ% подготовлено к отправке сообщение о закрытии заявки';uk='Для документа %Документ% подготовлено к отправке сообщение о закрытии заявки'");
			ИначеЕсли Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаПодтверждения") Тогда
				ТекстСообщения = НСтр("ru='Для документа %Документ% подготовлено к отправке подтверждение о поступлении товаров';uk='Для документа %Документ% подготовлено к отправке подтверждение о поступлении товаров'");
			КонецЕсли;
...
Процедура ОповеститьПользователяОВыполненнойОперации(СписокДокументов, КоличествоОбработанных, КоличествоВсего, Операция) Экспорт
	...
	ТекстЗаголовка = НСтр("ru='Подготовка сообщения к передаче в ГИСМ';uk='Подготовка сообщения к передаче в ГИСМ'");
	...
	Если КоличествоОбработанных > 0 Тогда
		...
		Если Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаЗакрытияЗаявки") Тогда
			ТекстСообщения = НСтр("ru='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов закрытие заявки подготовлено к отправке';uk='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов закрытие заявки подготовлено к отправке'");
		ИначеЕсли Операция = ПредопределенноеЗначение("Перечисление.ОперацииОбменаГИСМ.ПередачаПодтверждения") Тогда
			ТекстСообщения = НСтр("ru='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов подтверждение о поступлении подготовлено к отправке';uk='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов подтверждение о поступлении подготовлено к отправке'");
		Иначе
			ТекстСообщения = НСтр("ru='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов данные подготовлены к отправке';uk='Для %КоличествоОбработанных% из %КоличествоВсего% выделенных в списке документов данные подготовлены к отправке'");
		КонецЕсли;
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru='Подготовлено сообщений: 0.';uk='Подготовлено сообщений: 0.'");
...
Процедура ОповеститьПользователяОРезультатахВыполненияОбмена(Изменения) Экспорт
	...
	ТекстЗаголовка = НСтр("ru='Выполнение обмена с ГИСМ';uk='Выполнение обмена с ГИСМ'");
	...
	Если СтатусыДокументов.Количество() = 0 Тогда
		...
		ТекстСообщения = СтрШаблон(НСтр("ru='Документов с измененным статусом: %1.';uk='Документов с измененным статусом: %1.'"), СтатусыДокументов.Количество());
	...
	ИначеЕсли СтатусыДокументов.Количество() = 1 Тогда
		...
		Для Каждого КлючИЗначение Из СтатусыДокументов Цикл
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru='Для документа %1 изменен статус ГИСМ: %2.';uk='Для документа %1 изменен статус ГИСМ: %2.'"),
				КлючИЗначение.Ключ,
...
"Проверьте, что для сертификатов электронной подписи указаны организации с заполненным GLN.';uk='В информационной базе для пользователя %1 не найдено зарегистрированных сертификатов"
"электронной подписи, необходимых для обмена с ГИСМ."
