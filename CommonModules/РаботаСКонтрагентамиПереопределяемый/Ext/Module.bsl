...
Процедура ОпределитьЯвляетсяЛиКонтрагентИностранным(ДанныеКонтрагента, ЯвляетсяИностранным) Экспорт
	...
	ЯвляетсяИностранным = СтранаРегистрации <> Справочники.СтраныМира.Украина;
...
Процедура ДополнитьРеквизитыФормыКонтрагент(Форма, НовыеРеквизиты) Экспорт
	...
	Если Форма.ИмяФормы = "Справочник.Контрагенты.Форма.ФормаЭлемента" Тогда
		...
		НовыеРеквизиты.ЭтоИностранныйКонтрагент	= (Форма.Объект.ВидКонтрагента = Перечисления.ВидыКонтрагентов.ЮридическоеЛицо
												И Форма.Объект.СтранаРегистрации <> Справочники.СтраныМира.Украина);
...
Процедура ЗаполнитьОписаниеДанныхПрограммы(ТаблицаОписания) Экспорт
	...
	НоваяСтрока.Заголовок  = НСтр("ru='Наименование';uk='Найменування'");
	
	НоваяСтрока = ТаблицаОписания.Добавить();
	НоваяСтрока.ИмяДанных  = "Реквизиты";
	НоваяСтрока.ЭтоТаблица = Ложь;
	НоваяСтрока.Заголовок  = НСтр("ru='Реквизиты';uk='Реквізити'");
	
	НоваяСтрока = ТаблицаОписания.Добавить();
	НоваяСтрока.ИмяДанных  = "ЮридическийАдрес";
	НоваяСтрока.ЭтоТаблица = Ложь;
	НоваяСтрока.Заголовок  = НСтр("ru='Юридический адрес';uk='Юридична адреса'");
	
	НоваяСтрока = ТаблицаОписания.Добавить();
	НоваяСтрока.ИмяДанных  = "Телефон";
	НоваяСтрока.ЭтоТаблица = Ложь;
	НоваяСтрока.Заголовок  = НСтр("ru='Телефон';uk='Телефон'");
	
	НоваяСтрока = ТаблицаОписания.Добавить();
	НоваяСтрока.ИмяДанных  = "Email";
	НоваяСтрока.ЭтоТаблица = Ложь;
	НоваяСтрока.Заголовок  = НСтр("ru='E-mail';uk='E-mail'");
	
	НоваяСтрока = ТаблицаОписания.Добавить();
	НоваяСтрока.ИмяДанных  = "КонтактныеЛица";
	НоваяСтрока.ЭтоТаблица = Истина;
	НоваяСтрока.Заголовок  = НСтр("ru='Контактные лица';uk='Контактні особи'");
	
	НоваяСтрока = ТаблицаОписания.Добавить();
	НоваяСтрока.ИмяДанных  = "БанковскиеСчета";
	НоваяСтрока.ЭтоТаблица = Истина;
	НоваяСтрока.Заголовок  = НСтр("ru='Банковские счета';uk='Банківські рахунки'");
	
	НоваяСтрока = ТаблицаОписания.Добавить();
	НоваяСтрока.ИмяДанных  = "Родитель";
	НоваяСтрока.ЭтоТаблица = Ложь;
	НоваяСтрока.Заголовок  = НСтр("ru='Группа';uk='Група'");
...
#Область ЗаполнениеРеквизитовКонтрагента
	...
	СтруктураРеквизиты.Представление = СтрШаблон(НСтр("ru='ИНН %1';uk='ИНН %1'"), Реквизиты.ИНН);
	...
	Если Реквизиты.ВидКонтрагента = Перечисления.ВидыКонтрагентов.ЮридическоеЛицо Или Реквизиты.ВидКонтрагента = Перечисления.ВидыКонтрагентов.ГосударственныйОрган Тогда
		...
		СтруктураРеквизиты.Описание = СтрШаблон(НСтр("ru='КПП %1';uk='КПП %1'"), Реквизиты.КПП);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.КоррСчет) Тогда
			Представление = СтрШаблон(
				НСтр("ru='р/с %1, БИК %2, корр. счет %3';uk='р/с %1, БИК %2, корр. счет %3'"),
				Выборка.НомерСчета,
				Выборка.БИК,
				Выборка.КоррСчет);
		Иначе
			Представление = СтрШаблон(
				НСтр("ru='р/с %1, БИК %2';uk='р/с %1, БИК %2'"),
				Выборка.НомерСчета,
				Выборка.БИК);
			КонецЕсли;
...
Процедура ЗаполнитьИдентификаторКонфигурации(ИдентификаторКонфигурации) Экспорт
	...
	ИдентификаторКонфигурации = "SmallBusinessUkr";
