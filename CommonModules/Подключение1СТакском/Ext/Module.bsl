...
Процедура ПриСозданииКонтекстаВзаимодействия(Контекст) Экспорт
	...
	Если МестоЗапуска = "taxcomGetID" Тогда
		Контекст.Вставить("СообщениеОНедоступностиДействия",
			НСтр("ru='Получение уникального идентификатора абонента Такском недоступно для этой конфигурации.';uk='Получение уникального идентификатора абонента Такском недоступно для этой конфигурации.'"));
	ИначеЕсли МестоЗапуска = "taxcomPrivat" Тогда
		Контекст.Вставить("СообщениеОНедоступностиДействия",
			НСтр("ru='Работа с личным кабинетом абонента Такском недоступна для этой конфигурации.';uk='Работа с личным кабинетом абонента Такском недоступна для этой конфигурации.'"));
	КонецЕсли;
...
Процедура ПодготовитьСертификатЭПКОтправке(КСКонтекст, КонтекстОбработчика)
	...
	Если СертификатЭП = Неопределено Тогда
		...
		КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
			НСтр("ru='Не удалось получить двоичные данные сертификата. Сертификат не обнаружен в списке параметров.';uk='Не удалось получить двоичные данные сертификата. Сертификат не обнаружен в списке параметров.'");
	...
	Если ДвоичныеДанныеСертификата = Неопределено Тогда
		...
		СообщениеДляЖурналаРегистрации = НСтр("ru='Ошибка при получении двоичных данных сертификата. Двоичные данные сертификата отсутствуют, либо сертификат был удален.';uk='Ошибка при получении двоичных данных сертификата. Двоичные данные сертификата отсутствуют, либо сертификат был удален.'");
		...
		КонтекстОбработчика.ДействияПриОшибкеДляСервера.Добавить("ПрерватьБизнесПроцесс");
		КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
			НСтр("ru='Ошибка при получении данных сертификата. Подробнее см. в журнале регистрации.';uk='Ошибка при получении данных сертификата. Подробнее см. в журнале регистрации.'");
		КонтекстОбработчика.ДействиеПриОшибкеДляКлиента = "ПоказатьСообщение";
		Возврат;
...
Процедура ПодготовитьДанныеОбОрганизации(КСКонтекст, КонтекстОбработчика)
	...
	Исключение
		...
		КонтекстОбработчика.ПолноеОписаниеОшибки =
			НСтр("ru='Не удалось получить регистрационные данные организации.';uk='Не удалось получить регистрационные данные организации.'")
...
"Подробнее см. в журнале регистрации';uk='Ошибка при получении данных организации."
