...
Процедура УстановитьViPNetCSP(ОповещениеОЗавершении, ВладелецФормы) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ru='Установка ViPNet CSP возможно только на операционных системах семейства Windows.';uk='Установка ViPNet CSP можлива тільки на операційних системах сімейства Windows.'");
		ПоказатьПредупреждение(, ОписаниеОшибки,, НСтр("ru='Установка ViPNet CSP';uk='Установка ViPNet CSP'"));
	...
	Если Результат.Выполнено Тогда
		Если Результат.СистемнаяИнформация.ВерсияОС = "10.0" Тогда // Windows 10, Windows Server 2016
			ОписаниеОшибки = СтрШаблон(
								НСтр("ru='Работа ViPNet CSP на операционной системе %1 не поддерживается.';uk='Робота ViPNet CSP на операційній системі %1 не підтримується.'"),
		...
		Если Не ЗначениеЗаполнено(ПараметрыДистрибутива) Тогда
			ОписаниеОшибки = НСтр("ru='Сервис временно недоступен. Обратитесь в службу поддержки или повторите попытку позже.';uk='Сервіс тимчасово недоступний. Зверніться в службу підтримки або повторіть спробу пізніше.'");
			ПоказатьПредупреждение(, ОписаниеОшибки);
			РезультатВыполнения = Новый Структура;
			РезультатВыполнения.Вставить("Выполнено", Ложь);
			РезультатВыполнения.Вставить("ОписаниеОшибки", ОписаниеОшибки);
		...
		Параметры = Новый Структура("ПоясняющийТекст", НСтр("ru='Выполняется загрузка программы...';uk='Виконується завантаження програми...'"));
...
Процедура УстановитьViPNetCSPПослеПолученияПодтвержденияНаУстановку(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		РезультатВыполнения = Новый Структура;
		РезультатВыполнения.Вставить("Выполнено", Ложь);
		РезультатВыполнения.Вставить("ОписаниеОшибки", НСтр("ru='Пользователь отклонил установку ViPNet CSP';uk='Користувач відхилив установку ViPNet CSP'"));
...
Процедура УстановитьViPNetCSPПослеСохраненияСкрипта(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		РезультатВыполнения = Новый Структура;
		РезультатВыполнения.Вставить("Выполнено", Ложь);
		РезультатВыполнения.Вставить("ОписаниеОшибки", НСтр("ru='Ошибка при установке ViPNet CSP';uk='Помилка при установці ViPNet CSP'"));
...
Процедура УстановитьViPNetCSPПослеЗавершенияУстановки(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		Если ЗначениеЗаполнено(Результат.КодВозврата) Тогда
			РезультатВыполнения = Новый Структура;
			РезультатВыполнения.Вставить("Выполнено", Ложь);
			РезультатВыполнения.Вставить("ОписаниеОшибки", НСтр("ru='Установка ViPNet CSP не выполнена';uk='Установка ViPNet CSP не виконана'"));
