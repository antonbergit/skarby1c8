		...
		Для Каждого ТекФайл Из Файлы Цикл
			...
			#Иначе
				...
				Исключение
					...
					СоздатьСообщениеОбОшибке(ВыходныеПараметры, НСтр("ru='При чтении файла произошла ошибка.';uk='При читанні файлу відбулася помилка.'"));
					Возврат Ложь;
				КонецПопытки;
...
Процедура ОповещениеПоискФайловЗагрузкаОтчета(НайденныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если НайденныеФайлы.Количество()= 0 Тогда
		СоздатьСообщениеОбОшибке(ВыходныеПараметры, НСтр("ru='Файл отчета не найден';uk='Файл звіту не знайдений'"));
		Отказ = Истина;
	КонецЕсли;
...
Процедура ЗаписатьПрайсЛист(ИмяФайла, СтруктураПрайсЛиста, Результат, ВыходныеПараметры, ВерсияФорматаОбмена)
 	...
 	ТекстXML = МенеджерОборудованияСервисы.ПолучитьТекстXMLПрайсЛиста(СтруктураПрайсЛиста, ВерсияФорматаОбмена);
...
Функция ВыгрузитьНастройки(Параметры, ВыходныеПараметры, СтруктураНастроек, ВерсияФорматаОбмена)
	...
	ТекстXML = МенеджерОборудованияСервисы.ПолучитьТекстXMLНастроек(СтруктураНастроек, ВерсияФорматаОбмена);
...
Функция ОтчетЗагружен(Параметры, ВыходныеПараметры, ВерсияФорматаОбмена)
	...
	#Иначе
		...
		МенеджерОборудованияСервисы.ПометитьОбработкуДокумента(Отказ, Отчет.ПолноеИмя, "SalesReports", Сообщение, ВерсияФорматаОбмена);
...
Функция ТестУстройства(Параметры, ВыходныеПараметры)
	...
	Если ПустаяСтрока(ВремПараметр) Тогда
		Результат = Ложь;
		ТекстОшибкиОбщий = НСтр("ru='Каталог выгрузки не указан';uk='Каталог вивантаження не зазначений'");
	КонецЕсли;
	...
	Если ПустаяСтрока(ВремПараметр) Тогда
		...
		ТекстОшибкиОбщий = ТекстОшибкиОбщий + НСтр("ru='Имя файла прайс-листа не указано';uk='Ім`я файлу прайс-листа не зазначене'") 
	КонецЕсли;
	...
	Если ПустаяСтрока(ВремПараметр) Тогда
		Результат = Ложь;
		ТекстОшибкиОбщий = ТекстОшибкиОбщий + ?(ПустаяСтрока(ТекстОшибкиОбщий), "", Символы.ПС); 
		ТекстОшибкиОбщий = ТекстОшибкиОбщий + НСтр("ru='Каталог загрузки не указан';uk='Каталог завантаження не зазначений'") 
	КонецЕсли;
	...
	Если ПустаяСтрока(ВремПараметр) Тогда
		Результат = Ложь;
		ТекстОшибкиОбщий = ТекстОшибкиОбщий + ?(ПустаяСтрока(ТекстОшибкиОбщий), "", Символы.ПС); 
		ТекстОшибкиОбщий = ТекстОшибкиОбщий + НСтр("ru='Имя файла отчета не указано';uk='Ім`я файлу звіту не зазначене'") 
	КонецЕсли;
	...
	Если ВремПараметр=Неопределено Тогда
		Результат = Ложь;
		ТекстОшибкиОбщий = ТекстОшибкиОбщий + ?(ПустаяСтрока(ТекстОшибкиОбщий), "", Символы.ПС); 
		ТекстОшибкиОбщий = ТекстОшибкиОбщий + НСтр("ru='Не указано количество элементов в пакете';uk='Не зазначена кількість елементів у пакеті'") 
	КонецЕсли;
	...
	Если Результат Тогда
		Если Лев(Параметры.ИмяЗагружаемогоФайла, СтрДлина(Параметры.ИмяФайлаПрайсЛиста)) = Параметры.ИмяФайлаПрайсЛиста Тогда
			Результат = Ложь;
			ТекстОшибкиОбщий = ТекстОшибкиОбщий + ?(ПустаяСтрока(ТекстОшибкиОбщий), "", Символы.ПС); 
			ТекстОшибкиОбщий = ТекстОшибкиОбщий + НСтр("ru='Имена выгружаемого и загружаемого файлов не должны пересекаться.';uk='Імена файлів, що вивантажується й завантажується, не повинні перетинатися.'"); 
		КонецЕсли;
	КонецЕсли;
...
Функция ОбработатьФайлОтчет(Отказ, СодержимоеОтчета, ВыходныеПараметры, ВерсияФорматаОбмена)
	...
	МенеджерОборудованияСервисы.ЗаполнитьСтруктуруДокумента(СтруктураДокумента, "SalesReport", СодержимоеОтчета, ВерсияФорматаОбмена);
...
Процедура ЗаписатьТекстовыйДокумент(ИмяФайла, Содержимое, ВыходныеПараметры, Отказ)
	...
	Попытка
		ТекстовыйДокумент.Записать(ИмяФайла, КодировкаТекста.UTF8);
	Исключение
		ТекстСообщения =  НСтр("ru='При попытке записи файла %ИмяФайла% произошла ошибка';uk='При спробі запису файлу %ИмяФайла% відбулася помилка'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяФайла%", ИмяФайла);
