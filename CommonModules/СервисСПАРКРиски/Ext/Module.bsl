...
Процедура ЗагрузитьИндексыКонтрагентовВФоне(Параметры, АдресРезультата) Экспорт
	...
	ДлительныеОперации.СообщитьПрогресс(
		100,
		НСтр("ru='СервисСПАРКРиски.ЗагрузитьИндексыКонтрагентовВФоне завершено';uk='СервисСПАРКРиски.ЗагрузитьИндексыКонтрагентовВФоне завершено'"),
...
#Область ПроверкаПодключенияИнтернетПоддержки
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru='Выполнение ЗагрузитьИндексыКонтрагентов, ПередВызовомОперацииСервиса';uk='Выполнение ЗагрузитьИндексыКонтрагентов, ПередВызовомОперацииСервиса'"));
		...
		Если Не Результат.ВидОшибки.Пустая() Тогда
			...
			ОписаниеРезультата = СтрШаблон(
				НСтр("ru='Ошибка выполнения: %1';uk='Ошибка выполнения: %1'"),
				Результат.ВидОшибки);
...
#Область СозданиеТаблицыЗначений
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru='Выполнение ЗагрузитьИндексыКонтрагентов, СозданиеТаблицыЗначений';uk='Выполнение ЗагрузитьИндексыКонтрагентов, СозданиеТаблицыЗначений'"));
...
#Область ДополнительныеПараметрыВызоваОперацииСервиса
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru='Выполнение ЗагрузитьИндексыКонтрагентов, ДополнительныеПараметрыВызоваОперацииСервиса';uk='Выполнение ЗагрузитьИндексыКонтрагентов, ДополнительныеПараметрыВызоваОперацииСервиса'"));
...
#Область ВызовВебСервиса
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru='Выполнение ЗагрузитьИндексыКонтрагентов, ВызовВебСервиса';uk='Выполнение ЗагрузитьИндексыКонтрагентов, ВызовВебСервиса'"));

	ОписаниеРезультата = СтрШаблон(
		НСтр("ru='Вызов метода веб-сервиса для %1 записей (пачками).';uk='Вызов метода веб-сервиса для %1 записей (пачками).'"),
	...
	Для Каждого ТекущийКонтрагент Из МассивОписанийКонтрагентов Цикл
		...
		Если РазмерЗапроса >= 100 Тогда
			...
			ОписаниеРезультата = ОписаниеРезультата
				+ Символы.ПС
				+ СтрШаблон(
					НСтр("ru='Обработка пачки %4 заняла %1 мс (%2 - %3)';uk='Обработка пачки %4 заняла %1 мс (%2 - %3)'"),
					ТКонец - ТНачало,
					ТКонец,
					ТНачало,
					НомерПачки);
	...
	Если РазмерЗапроса > 0 Тогда
		...
		ОписаниеРезультата = ОписаниеРезультата
			+ Символы.ПС
			+ СтрШаблон(
				НСтр("ru='Обработка пачки %4 (последней) заняла %1 мс (%2 - %3)';uk='Обработка пачки %4 (последней) заняла %1 мс (%2 - %3)'"),
				ТКонец - ТНачало,
				ТКонец,
				ТНачало,
				НомерПачки);
...
#Область ЗаполнениеГрадациямиИндексовИРеквизитами
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru='Выполнение ЗагрузитьИндексыКонтрагентов, ЗаполнениеГрадациямиИндексовИРеквизитами';uk='Выполнение ЗагрузитьИндексыКонтрагентов, ЗаполнениеГрадациямиИндексовИРеквизитами'"));

	ОписаниеРезультата = СтрШаблон(
		НСтр("ru='Обработка результата веб-сервиса для %1 записей.';uk='Обработка результата веб-сервиса для %1 записей.'"),
		МассивОписанийКонтрагентов.Количество());
	...
	ОписаниеРезультата = ОписаниеРезультата
		+ Символы.ПС
		+ СтрШаблон(
			НСтр("ru='Подготовка массива контрагентов из %4 элементов заняла %1 мс (%2 - %3)';uk='Подготовка массива контрагентов из %4 элементов заняла %1 мс (%2 - %3)'"),
			ТКонец - ТНачало,
			ТКонец,
			ТНачало,
	...
	ОписаниеРезультата = ОписаниеРезультата
		+ Символы.ПС
		+ СтрШаблон(
			НСтр("ru='Получение реквизитов контрагентов для массива контрагентов из %4 элементов заняло %1 мс (%2 - %3)';uk='Получение реквизитов контрагентов для массива контрагентов из %4 элементов заняло %1 мс (%2 - %3)'"),
			ТКонец - ТНачало,
			ТКонец,
			ТНачало,
			МассивКонтрагентов.Количество());
	...
	ОписаниеРезультата = ОписаниеРезультата
		+ Символы.ПС
		+ СтрШаблон(
			НСтр("ru='Заполнение результата веб-сервиса (%4 элементов) реквизитами контрагентов и градациями индексов заняло %1 мс (%2 - %3)';uk='Заполнение результата веб-сервиса (%4 элементов) реквизитами контрагентов и градациями индексов заняло %1 мс (%2 - %3)'"),
			ТКонец - ТНачало,
			ТКонец,
			ТНачало,
...
#Область ДозаполнениеРезультатаИСохранениеВКэше
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru='Выполнение ЗагрузитьИндексыКонтрагентов, ДозаполнениеРезультатаИСохранениеВКэше';uk='Выполнение ЗагрузитьИндексыКонтрагентов, ДозаполнениеРезультатаИСохранениеВКэше'"));

	ОписаниеРезультата = НСтр("ru='Дозаполнение результата веб-сервиса.';uk='Дозаполнение результата веб-сервиса.'");
		...
		Если (ТипЗнч(Результат.ЗначенияИндексов) = ТипТаблицаЗначений) И (Результат.ЗначенияИндексов.Количество() > 0) Тогда
			...
			Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
				...
				ОписаниеРезультата = ОписаниеРезультата
					+ Символы.ПС
					+ СтрШаблон(
						НСтр("ru='Идентификаторы типов событий, подготовка списка из %4 в %5 элементов заняла %1 мс (%2 - %3)';uk='Идентификаторы типов событий, подготовка списка из %4 в %5 элементов заняла %1 мс (%2 - %3)'"),
						ТКонец - ТНачало,
						ТКонец,
						ТНачало,
				...
				ОписаниеРезультата = ОписаниеРезультата
					+ Символы.ПС
					+ СтрШаблон(
						НСтр("ru='Идентификаторы типов событий, загрузка списка новых событий из %4 элементов заняла %1 мс (%2 - %3)';uk='Идентификаторы типов событий, загрузка списка новых событий из %4 элементов заняла %1 мс (%2 - %3)'"),
						ТКонец - ТНачало,
						ТКонец,
						ТНачало,
			...
			ОписаниеРезультата = ОписаниеРезультата
				+ Символы.ПС
				+ СтрШаблон(
					НСтр("ru='Заполнение списка событий в результате из %4 элементов заняла %1 мс (%2 - %3)';uk='Заполнение списка событий в результате из %4 элементов заняла %1 мс (%2 - %3)'"),
					ТКонец - ТНачало,
					ТКонец,
					ТНачало,
					ЛокальныйЗначенияИндексов.Количество());
			...
			ОписаниеРезультата = ОписаниеРезультата
				+ Символы.ПС
				+ СтрШаблон(
					НСтр("ru='Сохранение результата из %4 элементов в кэше заняла %1 мс (%2 - %3)';uk='Сохранение результата из %4 элементов в кэше заняла %1 мс (%2 - %3)'"),
					ТКонец - ТНачало,
					ТКонец,
					ТНачало,
					МассивОписанийКонтрагентов.Количество());
...
"';uk='СервисСПАРКРиски.ЗагрузитьИндексыКонтрагентов, результат."
"Длительность (мс): %1"
"Результат выполнения:"
"%2"
""
...
"'"),
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru='СПАРК Риски. Отладка. ЗагрузитьИндексыКонтрагентов';uk='СПАРК Риски. Отладка. ЗагрузитьИндексыКонтрагентов'"), // ИдентификаторШага
...
Функция НачатьПодготовкуСправок(СписокИНН) Экспорт
	...
	Если Не Пользователи.РолиДоступны("ЗапросНовойСправкиСПАРКРиски", , Ложь) Тогда
		...
		Результат.СообщениеОбОшибке  = НСтр("ru='Недостаточно прав для запроса новой справки 1СПАРК Риски.';uk='Недостаточно прав для запроса новой справки 1СПАРК Риски.'");
...
"Некорректное значение параметра СписокИНН. Количество элементов списка превышает 100.';uk='Исключительная ситуация при вызове метода СПАРК.ВключитьМониторингСобытийПоКонтрагентам()."
...
"Обратитесь к администратору.';uk='Недостаточно прав для работы с сервисом 1СПАРК Риски."
"Обратитесь к администратору.'");
		Результат.ИнформацияОбОшибке = НСтр("ru='Недостаточно прав для работы с сервисом 1СПАРК Риски.';uk='Недостаточно прав для работы с сервисом 1СПАРК Риски.'");
	КонецЕсли;
	...
	Если ЗаполнитьДанныеАутентификации И Результат.ВидОшибки.Пустая() Тогда
		...
		Если ДанныеАутентификации = Неопределено Тогда
			...
			Результат.СообщениеОбОшибке  = НСтр("ru='Интернет-поддержка пользователей не подключена.';uk='Интернет-поддержка пользователей не подключена.'");
			Результат.ИнформацияОбОшибке = Результат.СообщениеОбОшибке;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаписатьДанныеАутентификации(ЗаписьДанныхСообщения, Логин, Пароль)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		ЗаписьДанныхСообщения.ЗаписатьЗначение("005d413a-f6ff-4a5f-8bd0-28d9eb765373");
	Иначе
...
Функция ВызватьОперациюСостояниеПодготовкиСправок(
...
Функция НовыйТелоЗапросаReportPdfCheck(
	...
	ЗаписатьДанныеАутентификации(ЗаписьДанныхСообщения, Логин, Пароль);
...
Функция ВызватьОперациюЗагрузитьФайлСправки(
	ИдентификаторСправки,
	Результат,
	ДанныеАутентификации)
...
Функция НовыйТелоЗапросаReportPdfDownload(ИдентификаторСправки, Логин, Пароль)
...
Функция НовыйТелоЗапросаMonitoringQueryStatus(ИдентификаторЗаявки, Логин, Пароль, ДополнительныеПараметрыЗапроса)
			...
			ИначеЕсли ИмяСвойства = "results" Тогда
				...
				ИначеЕсли ЧтениеОтвета.ТипТекущегоЗначения <> ТипЗначенияJSON.НачалоМассива Тогда
					ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Некорректное значение свойства results. Ожидается тип значения Массив; Фактический тип значения: %1.';uk='Некорректное значение свойства results. Ожидается тип значения Массив; Фактический тип значения: %1.'"),
...
Функция ВызватьОперациюСобытияМониторинга(
	СписокИНН,
	НачальныйИндекс,
	КонечныйИндекс,
	Результат,
...
Процедура ПрочитатьСобытияМониторинга(ЧтениеОтвета, Результат, ИНН)
	...
	ИначеЕсли ЧтениеОтвета.ТипТекущегоЗначения <> ТипЗначенияJSON.НачалоМассива Тогда
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Некорректное значение свойства results[].events. Ожидается тип значения Массив; Фактический тип значения: %1.';uk='Некорректное значение свойства results[].events. Ожидается тип значения Массив; Фактический тип значения: %1.'"),
			Строка(ЧтениеОтвета.ТипТекущегоЗначения));
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
...
Процедура ПроверитьДоступностьСервиса(РезультатОперации)
	...
	ИначеЕсли РезультатПроверки.ИмяОшибки = "ServerError" Тогда
		...
		РезультатОперации.СообщениеОбОшибке =
			НСтр("ru='Сервис временно недоступен.';uk='Сервис временно недоступен.'")
			+ Символы.ПС
	...
	Иначе
		...
		РезультатОперации.СообщениеОбОшибке =
			НСтр("ru='Не удалось подключиться к сервису.';uk='Не удалось подключиться к сервису.'")
			+ Символы.ПС
			+ РезультатПроверки.СообщениеОбОшибке;
		
	КонецЕсли;
	
	РезультатОперации.ИнформацияОбОшибке = НСтр("ru='Не удалось проверить доступность сервиса.';uk='Не удалось проверить доступность сервиса.'")
...
Процедура ПриОшибкеВызоваОперацииСервиса(РезультатЗагрузкиСодержимого, URLОперации, Результат)
	...
	ШапкаИнформацииОбОшибке =
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при вызове операции %1 сервиса СПАРК.';uk='Ошибка при вызове операции %1 сервиса СПАРК.'"),
			URLОперации);
	...
	Если ЭтоОбрабатываемыйКодОшибкиСервиса(РезультатЗагрузкиСодержимого.КодСостояния) Тогда
		...
		Исключение
			ПодвалСообщенияЖурналаРегистрации =
				НСтр("ru='Не удалось обработать описание ошибки.';uk='Не удалось обработать описание ошибки.'")
	...
	Если РезультатЗагрузкиСодержимого.КодСостояния = 402 Тогда
		...
		Если ПустаяСтрока(Результат.СообщениеОбОшибке) Тогда
			Результат.СообщениеОбОшибке = НСтр("ru='Требуется оплата сервиса.';uk='Требуется оплата сервиса.'");
		КонецЕсли;
	...
	ИначеЕсли РезультатЗагрузкиСодержимого.КодСостояния = 403 Тогда
		...
		Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
			...
			Результат.ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ИспользованиеЗапрещено;
			Результат.СообщениеОбОшибке = НСтр("ru='Использование сервиса 1СПАРК Риски недоступно при работе в модели сервиса.';uk='Использование сервиса 1СПАРК Риски недоступно при работе в модели сервиса.'");
			
		Иначе
			
			Результат.ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ИнтернетПоддержкаНеПодключена;
			Если ПустаяСтрока(Результат.СообщениеОбОшибке) Тогда
				Результат.СообщениеОбОшибке = НСтр("ru='Неверный логин или пароль пользователя Интернет-поддержки.';uk='Неверный логин или пароль пользователя Интернет-поддержки.'");
			КонецЕсли;
	...
	ИначеЕсли РезультатЗагрузкиСодержимого.КодСостояния = 404 Тогда
		...
		Если ПустаяСтрока(Результат.СообщениеОбОшибке) Тогда
			Результат.СообщениеОбОшибке = НСтр("ru='Запрошенные данные не найдены.';uk='Запрошенные данные не найдены.'");
		КонецЕсли;
	...
	ИначеЕсли РезультатЗагрузкиСодержимого.КодСостояния = 429 Тогда
		...
		Если ПустаяСтрока(Результат.СообщениеОбОшибке) Тогда
			Результат.СообщениеОбОшибке = НСтр("ru='Превышено количество попыток аутентификации.';uk='Перевищена кількість спроб аутентифікації.'");
		КонецЕсли;
	...
	ИначеЕсли РезультатЗагрузкиСодержимого.КодОшибки = "ConnectError" Тогда
		...
		Результат.СообщениеОбОшибке = НСтр("ru='Не удалось подключиться к сервису.';uk='Не удалось подключиться к сервису.'")
	...
	ИначеЕсли РезультатЗагрузкиСодержимого.КодОшибки = "ClientError" Тогда
		...
		Результат.ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ОшибкаПодключения;
		Результат.СообщениеОбОшибке = НСтр("ru='Не удалось получить данные из сервиса СПАРК.';uk='Не удалось получить данные из сервиса СПАРК.'")
			+ Символы.ПС
	...
	ИначеЕсли РезультатЗагрузкиСодержимого.КодОшибки = "ServerError" Тогда
		...
		Результат.СообщениеОбОшибке = НСтр("ru='В сервисе возникли неполадки.';uk='В сервисе возникли неполадки.'")
			+ Символы.ПС
			+ ?(ПустаяСтрока(Результат.СообщениеОбОшибке),
				РезультатЗагрузкиСодержимого.СообщениеОбОшибке,
				Результат.СообщениеОбОшибке);
	...
	Иначе
		...
		Результат.СообщениеОбОшибке = НСтр("ru='Ошибка при получении данных из сервиса СПАРК.';uk='Ошибка при получении данных из сервиса СПАРК.'")
			+ Символы.ПС + РезультатЗагрузкиСодержимого.СообщениеОбОшибке;
...
"Тело ответа: %3';uk='Ошибка при вызове операции %1 сервиса СПАРК."
"Ошибка при обработке ответа сервиса. %2"
"Тело ответа: %3'"),
		URLОперации,
		ПодробноеПредставлениеОшибки(ИнформацияОбОшибке),
	...
	Результат.ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ВнутренняяОшибкаСервиса;
	Результат.СообщениеОбОшибке = НСтр("ru='Некорректный ответ сервиса.';uk='Некорректный ответ сервиса.'");
...
Функция ЗначениеСвойстваJSON(ЧтениеОбъектаJSON, ЗначениеПоУмолчанию = Неопределено) Экспорт
	...
	Иначе
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось прочитать значение свойства %1. Некорректный тип значения свойства (%2).';uk='Не удалось прочитать значение свойства %1. Некорректный тип значения свойства (%2).'"),
			ИмяСвойства,
