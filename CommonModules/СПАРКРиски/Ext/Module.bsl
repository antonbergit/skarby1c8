...
Функция ИспользованиеРазрешено(ДополнительныеПрава = Неопределено) Экспорт
	...
	Возврат СПАРКРискиПовтИсп.ИспользованиеРазрешено(ДополнительныеПрава);
...
//
Процедура ДобавитьПодключаемыеКомандыКонтрагента(Форма, Источник, ЭлементПодменю) Экспорт
	...
	Для Каждого ТекущаяКоманда Из Команды Цикл
		...
		Если ТекущаяКоманда.Имя = "СПАРКРиски_ВключитьМониторингСобытий" Тогда
			...
			НовыйЭлемент.Пометка = СПАРКРиски.МониторингСобытийВключен(Источник.Ссылка, Истина);
		КонецЕсли;
			...
			Если СПАРКРискиКлиентСервер.ИННСоответствуетТребованиям(КонтрагентИНН, ТекстОшибки) Тогда
				...
				Если ТипЗнч(ПризнакЗагрузкиДанных) = ТипУникальныйИдентификатор Тогда // Задание было запущено ранее.
					...
					Иначе
						...
						Результат.Вставить("ТекстОшибки",             НСтр("ru='Не найдено фоновое задание';uk='Не найдено фоновое задание'"));
						Результат.Вставить("СостояниеВыводаДанных",   Перечисления.СостоянияВыводаИндексовСПАРКРиски.ВКэшеНетДанных);
				...
				Иначе
					...
					Если (ПризнакЗагрузкиДанных = Истина) Тогда
						...
						Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
							ОписаниеКонтрагента.ОбластьДанных = ОбщегоНазначения.ЗначениеРазделителяСеанса();
						КонецЕсли;
						Если ТипЗнч(Контрагент) = ТипСтрока Тогда
						...
						ПараметрыЗапускаПроцедуры = Новый Структура("МассивОписанийКонтрагентов", МассивОписанийКонтрагентов);
						ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(ИдентификаторФормы);
						ПараметрыВыполнения.Вставить("НаименованиеФоновогоЗадания",
							СтрШаблон(
								НСтр("ru='Загрузка индексов СПАРК Риски для контрагента %1 с ИНН %2';uk='Загрузка индексов СПАРК Риски для контрагента %1 с ИНН %2'"),
								Контрагент,
								КонтрагентИНН));
				...
				ИначеЕсли РезультатЗапускаФоновогоЗадания.Статус = "Выполнено" Тогда
					...
					Иначе
						Результат.Вставить("ВидОшибки",                Перечисления.ВидыОшибокСПАРКРиски.НеизвестнаяОшибка);
						Результат.Вставить("ТекстОшибки",              НСтр("ru='Ошибка получения индексов СПАРК Риски';uk='Ошибка получения индексов СПАРК Риски'"));
				...
				ИначеЕсли РезультатЗапускаФоновогоЗадания.Статус = "Отменено" Тогда
					Результат.Вставить("ТекстОшибки",                  НСтр("ru='Выполнение фонового задания отменено администратором';uk='Выполнение фонового задания отменено администратором'"));
					Результат.Вставить("СостояниеВыводаДанных",        Перечисления.СостоянияВыводаИндексовСПАРКРиски.ПустаяСсылка());
...
Процедура ПослеЗаписиНаСервере(Форма, ТекущийОбъект, ПараметрыЗаписи) Экспорт
	...
	Если ТипЗнч(ПараметрыЗаписи) = ТипСтруктура
		И ПараметрыЗаписи.Свойство("ВключитьМониторингСобытий1СПАРКРиски")
		И ПараметрыЗаписи.ВключитьМониторингСобытий1СПАРКРиски = Истина Тогда
		// Включить события мониторинга по контрагенту
		УстановитьПривилегированныйРежим(Истина);
		ВключитьОтключитьМониторингСобытий(ТекущийОбъект.Ссылка, Истина);
	...
	Если ТипЗнч(Форма.ИндексыСПАРКРиски) = ТипСтруктура
			И ИспользованиеРазрешено() Тогда
		Если Форма.ИндексыСПАРКРиски.СостояниеВыводаДанных = Перечисления.СостоянияВыводаИндексовСПАРКРиски.ПолученоИзФоновогоЗадания Тогда
			Форма.ИндексыСПАРКРиски.Вставить("Контрагент", ТекущийОбъект.Ссылка);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий         = НСтр("ru='1СПАРК Риски. Начальное заполнение.';uk='1СПАРК Риски. Начальное заполнение.'");
	...
	Обработчик.Комментарий         = НСтр("ru='1СПАРК Риски. Переход на новую версию';uk='1СПАРК Риски. Переход на новую версию'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ru='1СПАРК Риски. Переход на версию 2.1.7.10';uk='1СПАРК Риски. Переход на версию 2.1.7.10'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ru='1СПАРК Риски. Переход на версию 2.1.7.11';uk='1СПАРК Риски. Переход на версию 2.1.7.11'");
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	//	"assessment.1c.ru",
	//	443,
	//	НСтр("ru='Сервис 1СПАРК Риски (зона ru)';uk='Сервис 1СПАРК Риски (зона ru)'"));
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		"assessment.1c.eu",
		443,
		НСтр("ru='Сервис 1СПАРК Риски (зона eu)';uk='Сервис 1СПАРК Риски (зона eu)'"));
	НовыеРазрешения.Добавить(Разрешение);
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	ИдентификаторДела = "СобытияМониторингаСПАРКРиски";
	...
	НаименованиеДела =
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='1СПАРК Риски. Критичных событий мониторинга %1';uk='1СПАРК Риски. Критичных событий мониторинга %1'"),
			КоличествоСобытий);
...
"общего модуля СПАРКРискиПереопределяемый.';uk='Не удалось определить свойства справочника Контрагенты."
"Не заполнена реализация метода ПриОпределенииСвойствСправочниковКонтрагентов()"
...
"ПриОпределенииСвойствСправочниковКонтрагентов() общего модуля СПАРКРискиПереопределяемый.';uk='Неизвестный тип справочника контрагентов (%1). Описание справочника отсутствует в методе"
...
Функция ДанныеАутентификацииПользователяИнтернетПоддержки() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Возврат Новый Структура("Логин, Пароль", "1cfresh", "1cfresh");
...
Функция ИмяСобытияЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru='1СПАРК Риски';uk='1СПАРК Риски'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ПодключаемыеКомандыФормыКонтрагента(ЭтоФормаОбъекта)
	...
	Если ЭтоФормаОбъекта И ИспользованиеРазрешено("ПостановкаНаМониторинг;") Тогда
		...
		СтрКоманды.Синоним = НСтр("ru='Включить мониторинг событий';uk='Включить мониторинг событий'");
	КонецЕсли;
	
	СтрКоманды = Результат.Добавить();
	СтрКоманды.Имя     = "СПАРКРиски_ОткрытьСписокСправокКонтрагента";
	СтрКоманды.Синоним = НСтр("ru='Справки';uk='Довідки'");
	
	СтрКоманды = Результат.Добавить();
	СтрКоманды.Имя     = "СПАРКРиски_СобытияМониторингаПоКонтрагенту";
	СтрКоманды.Синоним = НСтр("ru='События мониторинга';uk='События мониторинга'");
...
Процедура УстановитьСостояниеСправки(СправкаСсылка, Состояние) Экспорт
	...
	Если Не Пользователи.РолиДоступны("ЧтениеДанныхСПАРКРиски") Тогда
		ВызватьИсключение НСтр("ru='Недостаточно прав.';uk='Недостаточно прав.'");
	КонецЕсли;
...
Процедура ВсеОбновленияСПАРКРискиОбщее() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru='Выполнение ВсеОбновленияСПАРКРискиОбщее';uk='Выполнение ВсеОбновленияСПАРКРискиОбщее'"));
		...
		Если МонопольныйРежим() Тогда
			...
			ОписаниеРезультата = НСтр("ru='Информационная база запущена в монопольном режиме.';uk='Информационная база запущена в монопольном режиме.'");
			ЕстьОшибки = Истина;
...
#Область Шаг_ЗагрузкаИндексовСПАРК
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru='Выполнение ВсеОбновленияСПАРКРискиОбщее, Загрузка индексов СПАРК';uk='Выполнение ВсеОбновленияСПАРКРискиОбщее, Загрузка индексов СПАРК'"));
				...
				Если ДатаСледующегоОбновления <= ТекущаяДатаСеансаДляПроверки Тогда
					...
					Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
						...
						// На текущий момент по всем областям данных индексы загружаются с единым логином и паролем,
						//  поэтому можно не разбивать процесс получения индексов по каждой области данных отдельно,
						//  а получать все вместе, сгруппированное по ИНН.
...
"Дата планируемого запуска: %3';uk='Время выполнения шага %1 еще не наступило."
"Текущая дата сеанса: %2"
...
#Область ПостановкаНаМониторинг
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru='Выполнение ВсеОбновленияСПАРКРискиОбщее, постановка контрагентов на мониторинг';uk='Выполнение ВсеОбновленияСПАРКРискиОбщее, постановка контрагентов на мониторинг'"));
...
"  Розділення включено: %7"
"  Доступне використання розділених даних: %8"
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		...
		НСтр("ru='СПАРК Риски. Сервис и регламент. Все обновления СПАРК Риски (общее)';uk='СПАРК Риски. Сервис и регламент. Все обновления СПАРК Риски (общее)'"), // ИдентификаторШага
...
Процедура ВсеОбновленияСПАРКРискиРазделенное() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru='Выполнение ВсеОбновленияСПАРКРискиРазделенное';uk='Выполнение ВсеОбновленияСПАРКРискиРазделенное'"));
...
"'"),
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		ИмяСобытияЖурналаРегистрации(), // ИмяСобытия
		НСтр("ru='СПАРК Риски. Сервис и регламент. Все обновления СПАРК Риски (разделенное)';uk='СПАРК Риски. Сервис и регламент. Все обновления СПАРК Риски (разделенное)'"), // ИдентификаторШага
		?(КодРезультатаОбщий = 0, УровеньЖурналаРегистрации.Информация, УровеньЖурналаРегистрации.Ошибка), // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполненияОбщий.ВремяОкончания - ШагВыполненияОбщий.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
"Не определено значение поля ПараметрыЗаполнения.ЗапросСвойствКонтрагентов.';uk='Не заполнена реализация переопределяемого метода"
"СПАРКРискиПереопределяемый.ПараметрыНачальногоЗаполненияДанных1СПАРКРиски()."
