...
Процедура ОбработатьВнРеглОтчет(Дескриптор, ПутьКФайлу)
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru='Не удалось распаковать архив с поставляемыми данными.';uk='Не удалось распаковать архив с поставляемыми данными.'");
	...
	Если НЕ Выборка.Следующий() Тогда
		ТекстОшибки = НСтр("ru='Для внешнего отчета не найден соответствующий элемент.';uk='Для внешнего отчета не найден соответствующий элемент.'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ВнешнийОтчетИспользовать Тогда 
		...
		ТекстДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Внешний отчет ""%1"" вер. ""%2"" отключен.';uk='Внешний отчет ""%1"" вер. ""%2"" отключен.'"), Имя, СвойстваМанифеста["КраткаяВерсияВнешнегоОтчета"]);
	...
	Если СвойстваКонфигурации["ИДКонфигурации"] <> СвойстваОтчета["ИДКонфигурации"] Тогда
		ТекстОшибки = НСтр("ru='Не пройдена проверка на соответствие идентификатора конфигурации в отчете идентификатору конфигурации в текущем подключенном отчете';uk='Не пройдена проверка на соответствие идентификатора конфигурации в отчете идентификатору конфигурации в текущем подключенном отчете'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
	...
	Если ИспользуетсяВнешнийОтчет Тогда
		Если СвойстваОтчета["ВерсияКонфигурации"] <> СвойстваКонфигурации["ВерсияКонфигурации"] Тогда
			ТекстОшибки = НСтр("ru='Не пройдена проверка на соответствие версии в отчете версии в текущем подключенном отчете';uk='Не пройдена проверка на соответствие версии в отчете версии в текущем подключенном отчете'");
			ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если СвойстваОтчета["ИсточникОтчета"] <> СвойстваКонфигурации["ИсточникОтчета"] Тогда
		ТекстОшибки = НСтр("ru='Не пройдена проверка на имени метаданных отчета имени метаданных текущего подключенного отчета';uk='Не пройдена проверка на имени метаданных отчета имени метаданных текущего подключенного отчета'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
...
"Описание:"
"""%1""';uk='Получение объекта отчета по ссылке из выборки."
"Описание:"
...
Функция ИмяСобытияВЖурналеРегистрации()
	...
	Возврат НСтр("ru='Поставляемые данные.Загрузка внешних регламентированных отчетов';uk='Поставляемые данные.Загрузка внешних регламентированных отчетов'");
