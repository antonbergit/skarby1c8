...
Процедура ЗаполнитьДокументПослеСопоставления(Результат, ДополнительныеПараметры) Экспорт
	...
	ЗагрузитьДокументВИБ(ДополнительныеПараметры.Контекст, Истина, Ложь, ВыполнялосьСопоставление);
...
Процедура ОтправитьЧерезБизнесСетьПродолжить(ПараметрКоманды, ПараметрыВыполненияКоманды) Экспорт
	...
	ИначеЕсли Не ВозможнаОтправка Тогда
		ТекстВопроса = НСтр("ru='Организация ""%1"" не подключена к сервису 1С:Бизнес-сеть. Подключить сейчас?';uk='Организация ""%1"" не подключена к сервису 1С:Бизнес-сеть. Подключить сейчас?'");
...
"Направить приглашение подключения?';uk='Контрагент ""%1"" не подключен к сервису 1С:Бизнес-сеть."
"Направить приглашение подключения?'");
			...
			КонецЕсли;
		Иначе
			ПоказатьПредупреждение(Неопределено, НСтр("ru='Участник не зарегистрирован в сервисе 1С:Бизнес-сеть.';uk='Участник не зарегистрирован в сервисе 1С:Бизнес-сеть.'"));
		КонецЕсли;
		Возврат;
	Иначе
		Возврат;
...
Процедура ЗагрузитьДокументВИБ(Контекст, СопоставлятьНоменклатуруПередЗаполнениемДокумента, Отказ, ОбновитьСтруктуруРазбора = Ложь)
	...
	ТекстОповещения	= НСтр("ru='Загрузка выполнена.';uk='Загрузка выполнена.'");
	ТекстПояснения	= НСтр("ru='Загружен документ через сервис 1С:Бизнес-сеть.';uk='Загружен документ через сервис 1С:Бизнес-сеть.'");
