...
Функция ПолучитьСерийныйНомерПоTID(Знач TID, Знач EPC = Неопределено) Экспорт
	...
	ОписаниеОшибки = НСтр("ru='Ошибка генерации серийного номера по TID.';uk='Ошибка генерации серийного номера по TID.'");
	...
	Если ПризнакG2 <> "E2" Тогда // Фиксированное значение "E2" признак того что тип соответствует EPC Class1Gen2. 
		РезультатВыполнения.ОписаниеОшибки = НСтр("ru='Указанный TID не соответствует типу EPC Class1Gen2.';uk='Указанный TID не соответствует типу EPC Class1Gen2.'");
	...
	ИначеЕсли Производитель = "003" Или Производитель = "803" Тогда // Чипы Alien Technology  
		...
		Если ПустаяСтрока(EPC) Тогда
			РезультатВыполнения.ОписаниеОшибки = НСтр("ru='Для генерация серийного номера для чипов ""Alien Technology"" необходимо указание EPC .';uk='Для генерация серийного номера для чипов ""Alien Technology"" необходимо указание EPC .'");
			Возврат РезультатВыполнения;
		КонецЕсли;
		
		СтруктураEPC = ДекодированиеДанныхSGTIN(EPC);
		Если Не СтруктураEPC.Результат Тогда
			РезультатВыполнения.ОписаниеОшибки = НСтр("ru='Ошибка декодирования EPC для чипов ""Alien Technology"".';uk='Ошибка декодирования EPC для чипов ""Alien Technology"".'");
			Возврат РезультатВыполнения;
		КонецЕсли;
	...
	Иначе
		...
		РезультатВыполнения.ОписаниеОшибки = НСтр("ru='Генерация серийного номера для чипа производителя указанного в TID не поддерживается.';uk='Генерация серийного номера для чипа производителя указанного в TID не поддерживается.'");
		Возврат РезультатВыполнения;
