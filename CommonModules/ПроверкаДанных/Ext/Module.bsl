...
Функция ВыполнитьПроизвольнуюПроверку(Организация, ПараметрыКонтроля, ДатаНачала, ДатаОкончания, ОграниченияПроверки = Неопределено, СохранятьПротокол = Истина, ПолныйРезультат = Ложь) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Проверка данных.Ошибка проверки данных';uk='Перевірка даних.Помилка перевірки даних'"),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='При выполнении проверки данных для области данных «%1» по организации «%2» произошла ошибка:"
"%3';uk='При виконанні перевірки даних для області даних «%1» по організації «%2» відбулася помилка:"
"%3'"),
...
Функция ВыполнитьПовторнуюПроверкуПараметраКонтроля(ПараметрКонтроля, Организация, СобытиеКалендаря = Неопределено, ДатаНачала, ДатаОкончания, ОргиниальноеДеревоРезультата) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Проверка данных.Ошибка повторной проверки данных';uk='Перевірка даних.Помилка повторної перевірки даних'"),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='При выполнении повторной проверки данных для области данных «%1» по организации «%2» произошла ошибка:"
"%3';uk='При виконанні повторної перевірки даних для області даних «%1» по організації «%2» відбулася помилка:"
"%3'"),
				ОбщегоНазначения.ЗначениеРазделителяСеанса(),
				Организация,
				ОписаниеОшибки()),
...
Процедура ВыполнитьПроверкуСведенийПоОрганизации(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока.Описание = НСтр("ru='Реквизиты Налогоплательщика';uk='Реквізити Платника податків'");
	...
	Если Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		...
		Если ПустаяСтрока(ФИОФизическихЛиц.Фамилия) И (ОграниченияПроверки.Найти("Фамилия")<>Неопределено ИЛИ ОграниченияПроверки.Найти("Наименование")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("ФИОФизическихЛиц.Фамилия",НСтр("ru='Не указана фамилия';uk='Не зазначене прізвище'"));
		КонецЕсли;
		...
		Если ПустаяСтрока(ФИОФизическихЛиц.Имя) И (ОграниченияПроверки.Найти("Имя")<>Неопределено ИЛИ ОграниченияПроверки.Найти("Наименование")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("ФИОФизическихЛиц.Имя",НСтр("ru='Не указано имя';uk='Не зазначене ім`я'"));
		КонецЕсли;
		...
		Если ПустаяСтрока(ФИОФизическихЛиц.Отчество) И (ОграниченияПроверки.Найти("Отчество")<>Неопределено ИЛИ ОграниченияПроверки.Найти("Наименование")<>Неопределено) Тогда
			ПоляСПредупреждениями.Вставить("ФИОФизическихЛиц.Отчество",НСтр("ru='Не указано отчество';uk='Не зазначене по батькові'"));
		КонецЕсли;
		
		Если ПустаяСтрока(Организация.НаименованиеПолное) И (ОграниченияПроверки.Найти("НаименованиеПолное")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.НаименованиеПолное",НСтр("ru='Не указано юридическое название предпринимателя';uk='Не зазначена юридична назва підприємця'"));
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ФизЛицо.Пол) И (ОграниченияПроверки.Найти("Пол")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("ФизическоеЛицо.Пол",НСтр("ru='Не указан пол';uk='Не зазначена стать'"));
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ФизЛицо.ДатаРождения) И (ОграниченияПроверки.Найти("ДатаРождения")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("ФизическоеЛицо.ДатаРождения",НСтр("ru='Не указана дата рождения';uk='Не зазначена дата народження'"));
		КонецЕсли;
		
		Если ПустаяСтрока(Организация.ИНН) И (ОграниченияПроверки.Найти("ИНН")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.ИНН",НСтр("ru='Не указан ИНН';uk='Не зазначений ІПН'"));
		ИначеЕсли НЕ РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(СокрЛП(Организация.ИНН), Ложь, "") И (ОграниченияПроверки.Найти("ИНН")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.ИНН",НСтр("ru='Некорректное значение ИНН';uk='Некоректне значення ІПН'"));
		КонецЕсли;
		
		Если ПустаяСтрока(Организация.ОГРН) И (ОграниченияПроверки.Найти("ОГРНИП")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.ОГРН",НСтр("ru='Не указан ОГРН';uk='Не зазначений ОГРН'"));
		ИначеЕсли НЕ РегламентированныеДанныеКлиентСервер.ОГРНСоответствуетТребованиям(СокрЛП(Организация.ОГРН), Ложь, "") И (ОграниченияПроверки.Найти("ОГРНИП")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.ОГРН",НСтр("ru='Некорректное значение ОГРНИП';uk='Некоректне значення ОДРНІП'"));
		КонецЕсли;
		
		Если ПустаяСтрока(Организация.КодПоОКПО) И (ОграниченияПроверки.Найти("КодПоОКПО")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.КодПоОКПО",НСтр("ru='Не указан Код по ОКПО';uk='Не зазначений Код по ЄДРПОУ'"));
		КонецЕсли;
		...
		Если ПустаяСтрока(Организация.РегистрационныйНомерПФР_ИП) И (ОграниченияПроверки.Найти("РегистрационныйНомерПФР_ИП")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.РегистрационныйНомерПФР_ИП",НСтр("ru='Не указан Регистрационный номер в ПФР (на себя)';uk='Не зазначений Реєстраційний номер у ПФР (на себе)'"));
		ИначеЕсли СтрДлина(СокрЛП(Организация.РегистрационныйНомерПФР_ИП))<> 14 И (ОграниченияПроверки.Найти("РегистрационныйНомерПФР_ИП")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.РегистрационныйНомерПФР_ИП",НСтр("ru='Некорректное значение Регистрационного номера в ПФР (на себя)';uk='Некоректне значення Реєстраційного номера в ПФР (на себе)'"));
		КонецЕсли;
		
	Иначе
		
		Если ПустаяСтрока(Организация.Наименование) И (ОграниченияПроверки.Найти("Наименование")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.Наименование",НСтр("ru='Не указано представление организации в программе';uk='Не зазначене представлення організації в програмі'"));
		КонецЕсли;
		
		Если ПустаяСтрока(Организация.НаименованиеПолное) И (ОграниченияПроверки.Найти("НаименованиеПолное")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.НаименованиеПолное",НСтр("ru='Не указано юридическое название организации';uk='Не зазначена юридична назва організації'"));
		КонецЕсли;
		...
		Если ПустаяСтрока(Организация.КПП) И (ОграниченияПроверки.Найти("КПП")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.КПП",НСтр("ru='Не указан КПП';uk='Не зазначений КПП'"));
		ИначеЕсли НЕ РегламентированныеДанныеКлиентСервер.КППСоответствуетТребованиям(СокрЛП(Организация.КПП), "") И (ОграниченияПроверки.Найти("КПП")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.КПП",НСтр("ru='Некорректное значение КПП';uk='Некоректне значення КПП'"));
		КонецЕсли;
		...
		ИначеЕсли НЕ РегламентированныеДанныеКлиентСервер.ОГРНСоответствуетТребованиям(СокрЛП(Организация.ОГРН), Истина, "") И (ОграниченияПроверки.Найти("ОГРН")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.ОГРН",НСтр("ru='Некорректное значение ОГРН';uk='Некоректне значення ОДРН'"));
		КонецЕсли;
		...
		ИначеЕсли НЕ РегламентированныеДанныеКлиентСервер.КодПоОКПОСоответствуетТребованиям(СокрЛП(Организация.КодПоОКПО), Истина, "") И (ОграниченияПроверки.Найти("КодПоОКПО")<>Неопределено) Тогда
			ПоляСОшибками.Вставить("Объект.КодПоОКПО",НСтр("ru='Некорректное значение кода по ОКПО';uk='Некоректне значення коду по ЄДРПОУ'"));
		КонецЕсли;
	...
	Если ПустаяСтрока(Организация.НаименованиеПлательщикаПриПеречисленииНалогов) И (ОграниченияПроверки.Найти("НаименованиеПлательщикаПриПеречисленииНалогов")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.НаименованиеПлательщикаПриПеречисленииНалогов",НСтр("ru='Не указано наименование организации при оплате налогов в бюджет';uk='Не зазначене найменування організації при оплаті податків у бюджет'"));
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Организация.КассаПоУмолчанию) И (ОграниченияПроверки.Найти("КассаПоУмолчанию")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.КассаПоУмолчанию",НСтр("ru='Не указана основная касса';uk='Не зазначена основна каса'"));
	КонецЕсли;
	...
	Если ПустаяСтрока(Организация.СвидетельствоСерияНомер) И (ОграниченияПроверки.Найти("СвидетельствоСерияНомер")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.СвидетельствоСерияНомер",НСтр("ru='Не указан номер свидетельства';uk='Не зазначений номер свідоцтва'"));
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Организация.СвидетельствоДатаВыдачи) И (ОграниченияПроверки.Найти("СвидетельствоДатаВыдачи")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.СвидетельствоДатаВыдачи",НСтр("ru='Не указан дата выдачи свидетельства';uk='Не зазначений дата видачі свідчення'"));
	КонецЕсли;
	
	Если ПустаяСтрока(Организация.КодПоОКАТО) И (ОграниченияПроверки.Найти("КодПоОКАТО")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.КодПоОКАТО",НСтр("ru='Не указан Код по ОКАТО';uk='Не зазначений Код по КОАТУУ'"));
	КонецЕсли;
	
	Если ПустаяСтрока(Организация.КодПоОКТМО) И (ОграниченияПроверки.Найти("КодПоОКТМО")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.КодПоОКТМО",НСтр("ru='Не указан Код по ОКТМО';uk='Не зазначений Код по ЗКТМУ'"));
	КонецЕсли;
	...
	Если (ОграниченияПроверки.Найти("КодНалоговогоОрганаПолучателя")<>Неопределено) Тогда
		Если ПустаяСтрока(Организация.КодНалоговогоОрганаПолучателя) Тогда
			ПоляСОшибками.Вставить("Объект.КодНалоговогоОрганаПолучателя",НСтр("ru='Не указан Код ИФНС';uk='Не зазначений Код ІФНС'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если (ОграниченияПроверки.Найти("НаименованиеИФНС")<>Неопределено) Тогда
		...
		Если Не ЗначениеЗаполнено(ГосОрган.Ссылка) Тогда
			ПоляСОшибками.Вставить("ПлатежныеРеквизитыФНСПредставление",НСтр("ru='Не указано наименование налогового органа';uk='Не зазначене найменування податкового органа'"));
		КонецЕсли;
	КонецЕсли;
	
	Если ПустаяСтрока(Организация.КодОКОПФ) И (ОграниченияПроверки.Найти("КодОКОПФ")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.КодОКОПФ",НСтр("ru='Не указан Код ОКОПФ';uk='Не зазначений Код ЗКОПФ'"));
	КонецЕсли;
	
	Если ПустаяСтрока(Организация.КодОКФС) И (ОграниченияПроверки.Найти("КодОКФС")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.КодОКФС",НСтр("ru='Не указан Код ОКФС';uk='Не зазначений Код ЗКФВ'"));
	КонецЕсли;
	
	Если ПустаяСтрока(Организация.КодОКВЭД) И (ОграниченияПроверки.Найти("КодОКВЭД")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.КодОКВЭД",НСтр("ru='Не указан Код ОКВЭД';uk='Не указан Код ОКВЭД'"));
	ИначеЕсли НЕ ПроверкаДанныхКлиентСервер.КодОКВЭДСоответствуетТребованиям(Организация.КодОКВЭД) И (ОграниченияПроверки.Найти("КодОКВЭД")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.КодОКВЭД",НСтр("ru='Некорректное значение кода вида деятельности по ОКВЭД';uk='Некорректное значение кода вида деятельности по ОКВЭД'"));
	КонецЕсли;
	
	Если ПустаяСтрока(Организация.СтраховойНомерПФР) И (ОграниченияПроверки.Найти("СтраховойНомерПФР")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.СтраховойНомерПФР",НСтр("ru='Не указан страховой номер ПФР';uk='Не зазначений страховий номер ПФР'"));
	ИначеЕсли НЕ РегламентированныеДанныеКлиентСервер.СтраховойНомерПФРСоответствуетТребованиям(СокрЛП(Организация.СтраховойНомерПФР),"") И (ОграниченияПроверки.Найти("СтраховойНомерПФР")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.СтраховойНомерПФР",НСтр("ru='Некорректное значение страхового номера ПФР';uk='Некоректне значення страхового номера ПФР'"));
	КонецЕсли;
	...
	Если ПустаяСтрока(Организация.РегистрационныйНомерПФР) И (ОграниченияПроверки.Найти("РегистрационныйНомерПФР")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.РегистрационныйНомерПФР",НСтр("ru='Не указан Регистрационный номер в ПФР в качестве работодателя';uk='Не зазначений Реєстраційний номер у ПФР у якості роботодавця'"));
	ИначеЕсли НЕ ПроверкаДанныхКлиентСервер.РегНомерПФРСоответствуетТребованиям(СокрЛП(Организация.РегистрационныйНомерПФР)) И (ОграниченияПроверки.Найти("РегистрационныйНомерПФР")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.РегистрационныйНомерПФР",НСтр("ru='Значение рег. номера в ПФР не соответствует шаблону:""XXX-XXX-XXXXXX"" (X - любая цифра)!';uk='Значення рег. номера в ПФР не відповідає шаблону:""XXX-XXX-XXXXXX"" (X - будь-яка цифра)!'"));
	КонецЕсли;
	
	Если ПустаяСтрока(Организация.КодОрганаПФР) И (ОграниченияПроверки.Найти("КодОрганаПФР")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.КодОрганаПФР",НСтр("ru='Не указан Код органа ПФР';uk='Не зазначений Код органа ПФР'"));
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Организация.ДатаРегистрации) И (ОграниченияПроверки.Найти("ДатаРегистрации")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.ДатаРегистрации",НСтр("ru='Не указана дата регистрации';uk='Не зазначена дата реєстрації'"));
	КонецЕсли;
	
	
	Если Не ЗначениеЗаполнено(Организация.КодОрганаФСС) И (ОграниченияПроверки.Найти("КодОрганаФСС")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.КодОрганаФСС",НСтр("ru='Не указан код органа ФСС';uk='Не зазначений код органа ФСС'"));
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Организация.ТарифНаТравматизм) И (ОграниченияПроверки.Найти("ТарифНаТравматизм")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.ТарифНаТравматизм",НСтр("ru='Не указан тариф на травматизм';uk='Не зазначений тариф на травматизм'"));
	КонецЕсли;
	...
	Если ПустаяСтрока(Организация.РегистрационныйНомерФСС) И (ОграниченияПроверки.Найти("РегистрационныйНомерФСС")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.РегистрационныйНомерФСС",НСтр("ru='Не указан Регистрационный номер в ФСС';uk='Не зазначений Реєстраційний номер у ФСС'"));
	ИначеЕсли СтрДлина(СокрЛП(Организация.РегистрационныйНомерФСС))<> 10 И (ОграниченияПроверки.Найти("РегистрационныйНомерФСС")<>Неопределено) Тогда
		ПоляСОшибками.Вставить("Объект.РегистрационныйНомерФСС",НСтр("ru='Некорректное значение Регистрационного номера в ФСС';uk='Некоректне значення Реєстраційного номера у ФСС'"));
	КонецЕсли;
	
	Если (ОграниченияПроверки.Найти("ЮрАдрес")<>Неопределено) Тогда
		Если Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
			ТекстОшибки = НСтр("ru='Не указан юридический адрес (Адрес по прописке)';uk='Не зазначена юридична адреса (Адреса по прописці)'")
		Иначе
			ТекстОшибки = НСтр("ru='Не указан юридический адрес';uk='Не зазначена юридична адреса'");
		КонецЕсли;
	...
	Если (ОграниченияПроверки.Найти("Телефон")<>Неопределено) Тогда
		...
		Если Строки.Количество() = 0 Тогда
			ПоляСОшибками.Вставить("ПредставлениеКИ_0",НСтр("ru='Не указан контактный телефон';uk='Не зазначений контактний телефон'"));
		Иначе
			Если ПустаяСтрока(Строки[0].Представление) Тогда
				ПоляСОшибками.Вставить("ПредставлениеКИ_0",НСтр("ru='Не указан контактный телефон';uk='Не зазначений контактний телефон'"));
			КонецЕсли;
	...
	Если (ОграниченияПроверки.Найти("EmailОрганизации")<>Неопределено) Тогда
		...
		Если Строки.Количество() = 0 Тогда
			ПоляСОшибками.Вставить("ПредставлениеКИ_1",НСтр("ru='Не указана e-mail';uk='Не указана e-mail'"));
		Иначе
			Если ПустаяСтрока(Строки[0].Представление) Тогда
				ПоляСОшибками.Вставить("ПредставлениеКИ_1",НСтр("ru='Не указан e-mail';uk='Не указан e-mail'"));
			КонецЕсли;
		КонецЕсли;
	...
	Если ПоляСОшибками.Количество() > 0 Тогда
		...
		Строка.Описание = НСтр("ru='Не заполнены необходимые реквизиты';uk='Не заповнені необхідні реквізити'");
	...
	Если ПоляСПредупреждениями.Количество() > 0 Тогда
		...
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ru='Есть проблемы с некоторыми полями';uk='Є проблеми з деякими полями'");
	...
	Если ПоляСОшибками.Количество() = 0 И ПоляСПредупреждениями.Количество() = 0 Тогда
		...
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ru='Основные реквизиты заполнены';uk='Основні реквізити заповнені'");
	...
	Если Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
		Если (ОграниченияПроверки.Найти("Руководитель")<>Неопределено) Тогда
			...
			Если Выборка.Следующий() Тогда
				...
				Если Не ЗначениеЗаполнено(ФизЛицо) Тогда
					...
					ОшибочныеПоля.Вставить("Объект.ФизЛицо",НСтр("ru='Не указано физическое лицо';uk='Не зазначена фізична особа'"));
				...
				Иначе
					...
					Если ДанныеФЛ.Количество() > 0 Тогда
						...
						Если ПустаяСтрока(ФИО.Фамилия) ИЛИ ПустаяСтрока(ФИО.Имя) Тогда
							Строка = КорневаяСтрока.Строки.Добавить();
							Строка.Описание = НСтр("ru='Не указано ФИО руководителя - физического лица';uk='Не зазначене ПІБ керівника - фізичної особи'");
							Строка.РезультатКонтроля = 2;
							Строка.ПараметрКонтроля = "СведенияОбОрганизации";
							...
							ОшибочныеПоля = Новый Соответствие;
							ОшибочныеПоля.Вставить("Объект.Наименование",НСтр("ru='Не указано ФИО';uk='Не зазначене ПІБ'"));
							Строка.Расшифровка.ПараметрыРасшифровки = ОшибочныеПоля;
						Иначе
							Строка = КорневаяСтрока.Строки.Добавить();
							Строка.Описание = НСтр("ru='Руководитель организации установлен';uk='Керівник організації встановлений'");
							Строка.РезультатКонтроля = 0;
							Строка.ПараметрКонтроля = "СведенияОбОрганизации";
						КонецЕсли;
					Иначе
						Строка = КорневаяСтрока.Строки.Добавить();
						Строка.Описание = НСтр("ru='Не указано ФИО физического лица';uk='Не зазначене ПІБ фізичної особи'");
						Строка.РезультатКонтроля = 2;
						Строка.ПараметрКонтроля = "СведенияОбОрганизации";
			...
			Иначе
				Строка = КорневаяСтрока.Строки.Добавить();
				Строка.Описание = НСтр("ru='Не задан руководитель организации';uk='Не заданий керівник організації'");
				Строка.РезультатКонтроля = 2;
				Строка.ПараметрКонтроля = "СведенияОбОрганизации";
		...
		Если (ОграниченияПроверки.Найти("ГлавныйБухгалтер")<>Неопределено) Тогда
			...
			Если Выборка.Следующий() Тогда
				...
				Иначе
					...
					Если ДанныеФЛ.Количество() > 0 Тогда
						...
						Если ПустаяСтрока(ФИО.Фамилия) ИЛИ ПустаяСтрока(ФИО.Имя) Тогда
							Строка = КорневаяСтрока.Строки.Добавить();
							Строка.Описание = НСтр("ru='Не указано ФИО главного бухгалтера - физического лица';uk='Не зазначене ПІБ головного бухгалтера - фізичної особи'");
							Строка.РезультатКонтроля = 2;
							Строка.ПараметрКонтроля = "СведенияОбОрганизации";
						...
						Иначе
							Строка = КорневаяСтрока.Строки.Добавить();
							Строка.Описание = НСтр("ru='Главный бухгалтер организации установлен';uk='Головний бухгалтер організації встановлений'");
							Строка.РезультатКонтроля = 0;
							Строка.ПараметрКонтроля = "СведенияОбОрганизации";
						КонецЕсли;
					Иначе
			...
			Иначе
				...
				Строка = КорневаяСтрока.Строки.Добавить();
				Строка.Описание = НСтр("ru='Не задан главный бухгалтер организации';uk='Не заданий головний бухгалтер організації'");
				Строка.РезультатКонтроля = 2;
				Строка.ПараметрКонтроля = "СведенияОбОрганизации";
...
Процедура ВыполнитьПроверкуПараметровУчета(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ru='Настройки параметров учета';uk='Налаштування параметрів обліку'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Если НЕ РегламентированнаяОтчетностьУСН.ПараметрыУСНЗаполнены(Организация, ДатаПроверки) Тогда
		...
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ru='Параметры учета не заполнены';uk='Параметри обліку не заповнені'");
		Строка.РезультатКонтроля = 2;
...
Процедура ВыполнитьПроверкуБанковскихСчетов(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ru='Банковские счета';uk='Банківські рахунки'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Если ЗначениеЗаполнено(ОсновнойБанковскийСчет) Тогда
		Строка.Описание = НСтр("ru='Основной банковский счет задан';uk='Основний банківський рахунок заданий'");
		Строка.РезультатКонтроля = 0;
		Строка.ПараметрКонтроля = "БанковскиеСчета";
	Иначе
		ПоляСПредупреждениями = Новый Соответствие;
		ПоляСПредупреждениями.Вставить("Объект.БанковскийСчетПоУмолчанию", НСтр("ru='Не задан основной банковский счет';uk='Не заданий основний банківський рахунок'"));
		
		Строка.Описание = НСтр("ru='Основной банковский счет не задан';uk='Основний банківський рахунок не заданий'");
		Строка.РезультатКонтроля = 1;
		Строка.ПараметрКонтроля = "БанковскиеСчета";
		Строка.Расшифровка = ПолучитьСтруктуруРасшифровкиРезультата();
		Строка.Расшифровка.ТипРасшифровки = "Объектная";
	...
	Если Результат[1].Пустой() Тогда
		...
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ru='Нет банковских счетов';uk='Немає банківських рахунків'");
		Строка.РезультатКонтроля = 1;
		Строка.ПараметрКонтроля = "БанковскиеСчета";
		Строка.Расшифровка = ПолучитьСтруктуруРасшифровкиРезультата();
	...
	ИначеЕсли Результат[0].Пустой() Тогда
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ru='Даты открытия банковских счетов заданы';uk='Дати відкриття банківських рахунків задані'");
		Строка.РезультатКонтроля = 0;
		Строка.ПараметрКонтроля = "БанковскиеСчета";
	Иначе
		Пока Выборка.Следующий() Цикл
			...
			ПоляСПредупреждениями = Новый Соответствие;
			ПоляСПредупреждениями.Вставить("Объект.ДатаОткрытия", НСтр("ru='Не указана дата открытия счета «';uk='Не зазначена дата відкриття рахунку «'")+Выборка.ПредставлениеБанкСчета+"»");
			
			Строка.Описание = НСтр("ru='Не указана дата открытия счета «';uk='Не зазначена дата відкриття рахунку «'")+ Выборка.ПредставлениеБанкСчета + "»";
			Строка.РезультатКонтроля = 1;
			Строка.ПараметрКонтроля = "БанковскиеСчета";
			Строка.Расшифровка = ПолучитьСтруктуруРасшифровкиРезультата();
			Строка.Расшифровка.ТипРасшифровки = "Объектная";
...
Процедура ВыполнитьПроверкуНачальныхОстатков(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ru='Начальные остатки';uk='Початкові залишки'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Если Не ЗначениеЗаполнено(ДатаРегистрации) Или (ДатаРегистрации >= ГодСдачиОтчетности) Тогда
		...
		Строка.Описание = НСтр("ru='Начальные остатки заполнены';uk='Початкові залишки заповнені'");
		Строка.РезультатКонтроля = 0;
	...
	Иначе
		Если МассивРезультатов[1].Пустой() Тогда
			...
			Если ВыборкаВНО.Количество() = 0 Тогда
				...
				Строка.Описание = НСтр("ru='Начальные остатки не заполнены';uk='Початкові залишки не заповнені'");
				Строка.РезультатКонтроля = 1;
				Строка.ПараметрКонтроля = "НачальныеОстатки";
				Строка.Расшифровка = ПолучитьСтруктуруРасшифровкиРезультата();
				Строка.Расшифровка.ТипРасшифровки = "Объектная";
...
Процедура ВыполнитьПроверкуКонтрольПроведенных(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ru='Контроль непроведенных документов';uk='Контроль непроведених документів'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Если Результат.Пустой() Тогда
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ru='Непроведенных документов нет';uk='Непроведених документів немає'");
		Строка.РезультатКонтроля = 0;
	...
	Иначе
		...
		Если ТаблицаРезультата.Количество() = 11 Тогда
			Строка = КорневаяСтрока.Строки.Добавить();
			Строка.Описание = НСтр("ru='Есть непроведенные документы. Эти документы игнорируются отчетностью. Если они не нужны, то пометьте их на удаление';uk='Є непроведені документи. Ці документи ігноруються звітністю. Якщо вони не потрібні, то позначте їх на видалення'");
			Строка.РезультатКонтроля = 1;
			Строка.ПараметрКонтроля = "КонтрольПроведенных";
			Строка.Расшифровка = ПолучитьСтруктуруРасшифровкиРезультата();
			Строка.Расшифровка.ТипРасшифровки = "Списочная";
		...
		Иначе
			...
			Для Каждого СтрокаРезультата Из ТаблицаРезультата Цикл
				...
				Строка.Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Документ «%1» не проведен и не помечен на удаление';uk='Документ «%1» не проведений і не позначений на видалення'"),
...
Процедура ВыполнитьПроверкуНаличияПоказателейЕНВД(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ru='Показатели ЕНВД';uk='Показники ЄП'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Если Результат.Пустой() Тогда
		...
		ПоляСОшибкам.Вставить("Команда.НастроитьЕНВД",НСтр("ru='Нет актуальных видов деятельности по ЕНВД';uk='Немає актуальних видів діяльності по ЄП'"));
		
		Строка = КорневаяСтрока.Строки.Добавить();
		
		Строка.Описание = НСтр("ru='Нет актуальных видов деятельности по ЕНВД';uk='Немає актуальних видів діяльності по ЄП'");
	...
	Иначе
		...
		КорневаяСтрока.РезультатКонтроля = 0;
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ru='Показатели ЕНВД заполнены';uk='Показники ЄП заповнені'");
		Строка.РезультатКонтроля = 0;
		Строка.ПараметрКонтроля = "НаличиеПоказателейЕНВД";
...
Процедура ВыполнитьПроверкуЗаполненияПоказателейЕНВД(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	Для Каждого Строка Из ДокументЕНВД.ПоказателиЕНВД Цикл
		...
		Если Строка.ЗначениеФизическогоПоказателя1 = 0 И Строка.ВыработкаДней1 > 0 Тогда
			ПоляСОшибками.Вставить("Объект.ПоказателиЕНВД["+(Строка.НомерСтроки-1)+"].ЗначениеФизическогоПоказателя1", НСтр("ru='Нет физ. показателя';uk='Немає фіз. показника'"));
		КонецЕсли;
		
		Если Строка.ЗначениеФизическогоПоказателя2 = 0  И Строка.ВыработкаДней2 > 0 Тогда
			ПоляСОшибками.Вставить("Объект.ПоказателиЕНВД["+(Строка.НомерСтроки-1)+"].ЗначениеФизическогоПоказателя2", НСтр("ru='Нет физ. показателя';uk='Немає фіз. показника'"));
		КонецЕсли;
		
		Если Строка.ЗначениеФизическогоПоказателя3 = 0  И Строка.ВыработкаДней3 > 0 Тогда
			ПоляСОшибками.Вставить("Объект.ПоказателиЕНВД["+(Строка.НомерСтроки-1)+"].ЗначениеФизическогоПоказателя3", НСтр("ru='Нет физ. показателя';uk='Немає фіз. показника'"));
		КонецЕсли;
	...
	Если ПоляСОшибками.Количество() > -0 Тогда
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ru='Не заполнены некоторые показатели по ЕНВД';uk='Не заповнені деякі показники по ЄП'");
		Строка.РезультатКонтроля = 2;
	...
	Иначе
		КорневаяСтрока.РезультатКонтроля = 0;
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ru='Показатели ЕНВД заполнены корректно';uk='Показники ЄП заповнені коректно'");
		Строка.РезультатКонтроля = 0;
		Строка.ПараметрКонтроля = "ЗаполнениеПоказателейЕНВД";
	КонецЕсли;
...
Процедура КорректностьЗаполненияВидовДеятельностиЕНВД(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ru='Корректность видов деятельности ЕНВД';uk='Коректність видів діяльності ЄП'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Если Результат.Пустой() Тогда
		...
		КорневаяСтрока.РезультатКонтроля = 0;
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ru='Виды деятельности ЕНВД заполнены корректно';uk='Види діяльності ЄП заповнені коректно'");
		Строка.РезультатКонтроля = 0;
	...
	Иначе
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.Код010203 И Выборка.КоличествоОкун = 0 И Год(ДатаНачала) < 2015 Тогда
				ПоляСОшибкам.Вставить("СсылкаЗаполненияОКУН",НСтр("ru='Для кодов видов деятельности 01, 02, 03 необходимы коды ОКУН';uk='Для кодів видів діяльності 01, 02, 03 необхідні коди ЗКПН'"));
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(Выборка.КодНалоговогоОрганаПолучателя) Тогда
				ПоляСОшибкам.Вставить("Объект.КодНалоговогоОрганаПолучателя",НСтр("ru='Не заполнен код налогового органа получателя';uk='Не заповнений код податкового органа одержувача'"));
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(Выборка.СтавкаЕНВД) Тогда
				ПоляСОшибкам.Вставить("Объект.СтавкаЕНВД",НСтр("ru='Не заполнена ставка ЕНВД';uk='Не заполнена ставка ЕНВД'"));
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(Выборка.КодПоОКАТО) Тогда
				ПоляСОшибкам.Вставить("Объект.КодПоОКАТО",НСтр("ru='Не заполнен код по ОКТМО';uk='Не заповнений код по ЗКТМУ'"));
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(Выборка.АдресОсуществленияДеятельности) Тогда
				ПоляСОшибкам.Вставить("КонтактнаяИнформацияПоле0",НСтр("ru='Не заполнено место осуществления деятельности';uk='Не заповнене місце здійснення діяльності'"));
			КонецЕсли;
			...
			Если ПоляСОшибкам.Количество() > 0 Тогда
				...
				Строка.Описание = НСтр("ru='У вида деятельности «';uk='У виду діяльності «'") + Выборка.Ссылка + НСтр("ru='» не заполнены реквизиты';uk='» не заповнені реквізити'");
			...
			Если ПоляСПредупреждениями.Количество() > 0 Тогда
				...
				Строка.Описание = НСтр("ru='У вида деятельности «';uk='У виду діяльності «'") + Выборка.Ссылка + НСтр("ru='» есть проблемы с некоторыми полями';uk='» є проблеми з деякими полями'");
...
Процедура ВыполнитьПроверкуСведенийПоСотрудникам(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ru='Реквизиты сотрудников';uk='Реквізити співробітників'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Пока ВыборкаСотрудники.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ФизЛицо) Тогда
			Если ПустаяСтрока(ВыборкаСотрудники.Фамилия) И (ОграниченияПроверки.Найти("Фамилия")<>Неопределено) Тогда
				ПоляСОшибками.Вставить("Наименование",НСтр("ru='Не указана фамилия';uk='Не зазначене прізвище'"));
			КонецЕсли;
			
			Если ПустаяСтрока(ВыборкаСотрудники.Имя) И (ОграниченияПроверки.Найти("Имя")<>Неопределено) Тогда
				ПоляСОшибками.Вставить("Наименование",НСтр("ru='Не указано имя';uk='Не зазначене ім`я'"));
			КонецЕсли;
			
			Если ПустаяСтрока(ВыборкаСотрудники.Отчество) И (ОграниченияПроверки.Найти("Отчество")<>Неопределено) Тогда
				ПоляСПредупреждениями.Вставить("Наименование",НСтр("ru='Не указано отчество';uk='Не зазначене по батькові'"));
			КонецЕсли;
			
			Если ПустаяСтрока(ФизЛицо.ИНН) И (ОграниченияПроверки.Найти("ИНН")<>Неопределено) Тогда
				ПоляСОшибками.Вставить("ФизическоеЛицо.ИНН",НСтр("ru='Не указан ИНН';uk='Не зазначений ІПН'"));
			ИначеЕсли НЕ РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(СокрЛП(ФизЛицо.ИНН), Ложь, "") И (ОграниченияПроверки.Найти("ИНН")<>Неопределено) Тогда
				ПоляСОшибками.Вставить("ФизическоеЛицо.ИНН",НСтр("ru='Некорректное значение ИНН';uk='Некоректне значення ІПН'"));
			КонецЕсли;
			...
			Если ПустаяСтрока(ФизЛицо.СтраховойНомерПФР) И (ОграниченияПроверки.Найти("СтраховойНомерПФР")<>Неопределено) Тогда
				ПоляСОшибками.Вставить("ФизическоеЛицо.СтраховойНомерПФР",НСтр("ru='Не указан страховой номер ПФР';uk='Не зазначений страховий номер ПФР'"));
			ИначеЕсли Не РегламентированныеДанныеКлиентСервер.СтраховойНомерПФРСоответствуетТребованиям(СокрЛП(ФизЛицо.СтраховойНомерПФР),"") И (ОграниченияПроверки.Найти("СтраховойНомерПФР")<>Неопределено) Тогда
				ПоляСОшибками.Вставить("ФизическоеЛицо.СтраховойНомерПФР",НСтр("ru='Некорректное значение Страхового номера в ПФР';uk='Некоректне значення Страхового номера в ПФР'"));
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(ФизЛицо.Гражданство) И (ОграниченияПроверки.Найти("Гражданство")<>Неопределено) Тогда
				ПоляСОшибками.Вставить("ФизическоеЛицо.Гражданство",НСтр("ru='Не указано гражданство';uk='Не зазначене громадянство'"));
			КонецЕсли;
			...
			Если (ОграниченияПроверки.Найти("АдресПоПрописке")<>Неопределено) Тогда
				...
				Если Строки.Количество() = 0 Тогда
					ПоляСОшибками.Вставить("ПредставлениеКИ_0",НСтр("ru='Не указан адрес по прописке';uk='Не зазначена адреса по прописці'"));
				Иначе
					Если ПустаяСтрока(Строки[0].Представление) Тогда
						ПоляСОшибками.Вставить("ПредставлениеКИ_0",НСтр("ru='Не указан адрес по прописке';uk='Не зазначена адреса по прописці'"));
					КонецЕсли;
			...
			Если (ОграниченияПроверки.Найти("Индекс")<>Неопределено) Тогда
				...
				Строки = ФизЛицо.КонтактнаяИнформация.НайтиСтроки(Новый Структура("Вид", Справочники.ВидыКонтактнойИнформации.АдресФизЛицаПоПрописке));
				Если Строки.Количество() = 0 Тогда
					ПоляСОшибками.Вставить("ПредставлениеКИ_0",НСтр("ru='Не указан индекс адреса по прописке';uk='Не зазначений індекс адреси по прописці'"));
				Иначе
					...
					ИначеЕсли СтрДлина(СокрЛП(СтруктураАдреса.Индекс))<> 6 Тогда
						ПоляСОшибками.Вставить("ПредставлениеКИ_0",НСтр("ru='Некорректно заполнен индекс адреса по прописке';uk='Некоректно заповнений індекс адреси по прописці'"));
					КонецЕсли;
				КонецЕсли;
			...
			Если (ОграниченияПроверки.Найти("Телефон")<>Неопределено) Тогда
				...
				Если Строки.Количество() = 0 Тогда
					ПоляСОшибками.Вставить("Физлицо",НСтр("ru='Не указан контактный телефон';uk='Не зазначений контактний телефон'"));
				Иначе
					Если ПустаяСтрока(Строки[0].Представление) Тогда
						ПоляСОшибками.Вставить("Физлицо",НСтр("ru='Не указан контактный телефон';uk='Не зазначений контактний телефон'"));
					КонецЕсли;
			...
			Если (ОграниченияПроверки.Найти("УдостоверениеЛичности")<>Неопределено) Тогда
				...
				Если ДокументУдЛичности = Неопределено Тогда
					ПоляСОшибками.Вставить("КлючДокументФизЛица.Серия", НСтр("ru='Не указан документ, удостоверяющий личность';uk='Не зазначений документ, що засвідчує особистість'"));
				Иначе
					
					Если ПустаяСтрока(ДокументУдЛичности.Серия) Тогда
						ПоляСОшибками.Вставить("КлючДокументФизЛица.Серия", НСтр("ru='Не указана серия документа, удостоверяющего личность';uk='Не зазначена серія документа, що засвідчує особистість'"));
					КонецЕсли;
					
					Если ПустаяСтрока(ДокументУдЛичности.Номер) Тогда
						ПоляСОшибками.Вставить("КлючДокументФизЛица.Номер", НСтр("ru='Не указан номер документа, удостоверяющего личность';uk='Не зазначений номер документа, що засвідчує особистість'"));
					КонецЕсли;
		...
		Если ПоляСОшибками.Количество() > 0 Тогда
			Строка = КорневаяСтрока.Строки.Добавить();
			Строка.Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='У сотрудника «%1» не заполнены необходимые реквизиты';uk='У співробітника «%1» не заповнені необхідні реквізити'"),
		...
		Если ПоляСПредупреждениями.Количество() > 0 Тогда
			Строка = КорневаяСтрока.Строки.Добавить();
			Строка.Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='У сотрудника «%1» есть проблемы с некоторыми полями';uk='У співробітника «%1» є проблеми з деякими полями'"),
				ВыборкаСотрудники.СотрудникПредставление);
		...
		Если ПоляСОшибками.Количество() = 0 И ПоляСПредупреждениями.Количество() = 0 Тогда
			Строка = КорневаяСтрока.Строки.Добавить();
			Строка.Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Основные реквизиты по сотруднику «%1» заполнены';uk='Основні реквізити по співробітникові «%1» заповнені'"),
				ВыборкаСотрудники.СотрудникПредставление);
			Строка.РезультатКонтроля = 0;
			Строка.ПараметрКонтроля = "СведенияПоСотрудникам";
...
Процедура ВыполнитьПроверкуТарифаСтраховыхВзносов(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ru='Настройки тарифа страховых взносов';uk='Налаштування тарифу страхових внесків'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Если НЕ РегламентированнаяОтчетностьУСН.ТарифыСтраховыхВзносовЗаполнены(Организация, ДатаПроверки) Тогда
		...
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ru='Тарифы страховых взносов не заполнены';uk='Тарифи страхових внесків не заповнені'");
		Строка.РезультатКонтроля = 2;
...
Процедура ВыполнитьПроверкуТорговыхТочек(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ru='Торговые точки';uk='Торговые точки'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Если Результат.Пустой() Тогда
		...
		ПоляСОшибкам = Новый Соответствие;
		ПоляСОшибкам.Вставить("Команда.ТорговыеТочки",НСтр("ru='Нет актуальных торговых точек';uk='Нет актуальных торговых точек'"));
		
		Строка = КорневаяСтрока.Строки.Добавить();
		
		Строка.Описание = НСтр("ru='Нет актуальных торговых точек';uk='Нет актуальных торговых точек'");
	...
	Иначе
		...
		КорневаяСтрока.РезультатКонтроля = 0;
		Строка = КорневаяСтрока.Строки.Добавить();
		Строка.Описание = НСтр("ru='Показатели торговых точек заполнены';uk='Показатели торговых точек заполнены'");
		Строка.РезультатКонтроля = 0;
		Строка.ПараметрКонтроля = "СведенияПоТорговымТочкам";
...
Процедура ВыполнитьПроверкуТорговыхТочекРозничнойПродажиАлкоголя(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания, ОграниченияПроверки=Неопределено) Экспорт
	...
	КорневаяСтрока = ДеревоРезультата.Строки.Добавить();
	КорневаяСтрока.Описание = НСтр("ru='Торговые точки розничной продажи алкоголя';uk='Торговые точки розничной продажи алкоголя'");
	КорневаяСтрока.РезультатКонтроля = 0;
	...
	Если Результат.Пустой() Тогда
		...
		Строка.Описание = НСтр("ru='Нет торговых точек, подключенных к ЕГАИС';uk='Нет торговых точек, подключенных к ЕГАИС'");
	...
	Иначе
		...
		Пока Выборка.Следующий() Цикл
			Если Не ЗначениеЗаполнено(Выборка.Представление) Тогда
				ЕстьОшибка = Истина;
				ПоляСОшибками = Новый Соответствие;
				ПоляСОшибками.Вставить("ПредставлениеКИ_1", НСтр("ru='Не указан фактический адрес';uk='Не указан фактический адрес'"));
				
				Строка = КорневаяСтрока.Строки.Добавить();
				Строка.Описание = НСтр("ru='Не указан адрес торговой точки «';uk='Не указан адрес торговой точки «'")+ Выборка.Наименование + "»";
				Строка.РезультатКонтроля = 2;
				Строка.ПараметрКонтроля = "ТорговыеТочкиРозничнойПродажиАлкоголя";
				Строка.Расшифровка = ПолучитьСтруктуруРасшифровкиРезультата();
				Строка.Расшифровка.ТипРасшифровки = "Объектная";
		...
		Если Не ЕстьОшибка Тогда
			КорневаяСтрока.РезультатКонтроля = 0;
			Строка = КорневаяСтрока.Строки.Добавить();
			Строка.Описание = НСтр("ru='Торговые точки розничной продажи алкоголя заполнены';uk='Торговые точки розничной продажи алкоголя заполнены'");
			Строка.РезультатКонтроля = 0;
			Строка.ПараметрКонтроля = "ТорговыеТочкиРозничнойПродажиАлкоголя";
		КонецЕсли;
	КонецЕсли;
