...
Функция СообщениеЗапросаGLN(Организация, ИНН, КПП) Экспорт
	...
	Если ЗначениеЗаполнено(КПП) Тогда
		СообщениеXML.Описание = СтрШаблон(НСтр("ru='Получение GLN по ИНН %1, КПП %2';uk='Получение GLN по ИНН %1, КПП %2'"), ИНН, КПП);
	Иначе
		СообщениеXML.Описание = СтрШаблон(НСтр("ru='Получение GLN по ИНН %1';uk='Получение GLN по ИНН %1'"), ИНН);
	КонецЕсли;
...
Функция СообщенияКПередачеЗапросыСписковВходящихДокументовИКвитанцийXML() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		СообщениеXML = ИнтеграцияГИСМКлиентСервер.СтруктураСообщенияXML();
		СообщениеXML.Описание = СтрШаблон(НСтр("ru='Получение входящих документов организации %1';uk='Получение входящих документов организации %1'"), Выборка.Организация);
...
Функция СообщенияКПередачеПолучениеКвитанций(GLN) Экспорт
	...
	Пока ДанныеДляПолученияКвитанцииИзОчереди.Следующий() Цикл
		...
		СообщениеXML.Описание = СтрШаблон(
			НСтр("ru='Получение квитанции по операции %1 документа %2 по организации %3';uk='Получение квитанции по операции %1 документа %2 по организации %3'"),
	...
	Для Каждого Сообщение Из Сообщения Цикл
		...
		Если Категории.ПолучениеСпискаВходящихДокументов.Получить(Сообщение.Операция) <> Неопределено Тогда
			...
			Для Каждого ДанныеДляПолученияДокумента Из РезультатПередачиИОбработки.РезультатОбработкиОтвета.Документы Цикл
				...
				СообщениеXML = ИнтеграцияГИСМКлиентСервер.СтруктураСообщенияXML();
				СообщениеXML.Описание = СтрШаблон(НСтр("ru='Получение документа ""%1"" по организации %2';uk='Получение документа ""%1"" по организации %2'"), ТипДокумента.Описание, Сообщение.Организация);
...
Функция ПередатьДанные(ИсходящееСообщение, СертификатыДляПодписанияНаСервере = Неопределено) Экспорт
	...
	Если ОбновитьСостояниеПодтверждения Тогда
		...
		Исключение
			...
			ВозвращаемоеЗначение.РезультатОбработкиОтвета.ТекстОшибки = СтрШаблон(НСтр("ru='Не удалось заблокировать документ %1.';uk='Не удалось заблокировать документ %1.'"), ДокументСсылка);
			Возврат ВозвращаемоеЗначение;
		КонецПопытки;
...
"Текст ошибки: %2';uk='При передаче сообщения по документу %1 возникла ошибка:"
"Текст ошибки: %2'"),
				ДокументСсылка,
...
"%1"
"По причине:"
"%2';uk='Данные не переданы в ГИСМ:"
"%1"
"По причине:"
...
"Текст ошибки: %2';uk='При загрузке квитанции о фиксации по документу %1 возникла ошибка:"
"Текст ошибки: %2'"),
				ДокументСсылка,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
"Переданные данные: %1"
"По причине:"
"%2';uk='Квитанция о фиксации не получена:"
"Переданные данные: %1"
"По причине:"
"%2'"), ОписаниеОперации, РезультатОтправкиЗапроса.ТекстОшибки));
		Иначе
...
Функция ЗапросGLNПоИННКППXML(СообщениеXML, ИНН, КПП = Неопределено) Экспорт
	...
	Исключение
		СообщениеXML.ТекстОшибки = НСтр("ru='Указан не верный ИНН.';uk='Указан не верный ИНН.'");
	КонецПопытки;
	...
	Если ЗначениеЗаполнено(КПП) Тогда
		...
		Исключение
			СообщениеXML.ТекстОшибки = НСтр("ru='Указан не верный КПП.';uk='Указан не верный КПП.'");
		КонецПопытки;
	КонецЕсли;
...
Функция ЗапросКвитанцииОФиксацииПоСсылкеXML(Ссылка, Операция) Экспорт
	...
	ВозвращаемоеЗначение.Вставить("Описание", СтрШаблон(НСтр("ru='Запрос квитанции о фиксации документа %1';uk='Запрос квитанции о фиксации документа %1'"), Ссылка));
	ВозвращаемоеЗначение.Вставить("СообщенияXML", Новый Массив);
...
"Текст ошибки: %2';uk='При подготовке к передаче сообщения по документу %1 возникла ошибка:"
"Текст ошибки: %2'"),
			Реквизиты.Документ,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ДобавитьЗаписьВПротоколОбмена(КонвертSOAP, Реквизиты) Экспорт
	...
	Если Реквизиты.ТипСообщения = Перечисления.ТипыСообщенийГИСМ.Исходящее Тогда
		ЗаголовокОшибки = СтрШаблон(НСтр("ru='Ошибка при проверке электронной подписи отправляемого в ГИСМ сообщения %1:';uk='Ошибка при проверке электронной подписи отправляемого в ГИСМ сообщения %1:'"), ПрисоединенныйФайл);
	ИначеЕсли Реквизиты.ТипСообщения = Перечисления.ТипыСообщенийГИСМ.Входящее Тогда
		ЗаголовокОшибки = СтрШаблон(НСтр("ru='Ошибка при проверке электронной подписи загруженного из ГИСМ сообщения %1:';uk='Ошибка при проверке электронной подписи загруженного из ГИСМ сообщения %1:'"), ПрисоединенныйФайл);
	КонецЕсли;
	...
	Для Каждого СвойствоИсточника Из ОбъектXDTOИсточник.Свойства() Цикл
		...
		Если ТипЗнч(ЗначениеСвойства) = Тип("ОбъектXDTO") Тогда
			...
			Если ТипЗнч(СвойствоПриемника.Тип) = Тип("ТипОбъектаXDTO") Тогда
				...
				ОбъектXDTOПриемник[ИмяСвойства] = ИнтеграцияГИСМ.ОбъектXDTOПоИмениСвойства(ОбъектXDTOПриемник, ИмяСвойства, Версия);
				
				Если ТипЗнч(ОбъектXDTOПриемник[ИмяСвойства]) = Тип("ОбъектXDTO") Тогда
					ПреобразоватьПроизвольныйОбъектXDTOВОбъектXDTO(ЗначениеСвойства, ОбъектXDTOПриемник[ИмяСвойства], Версия);
				КонецЕсли;
...
"Текст ошибки: %1';uk='При разборе XML ответа на запрос получения квитанции из ГИСМ возникла ошибка:"
...
"Дополнительная информация: Код ошибки: %1. Текст ошибки: %2.';uk='Квитанция получена. Ошибка: Неверный запрос."
...
"%1.';uk='При получения квитанции из ГИСМ произошел системный сбой:"
"%1.'"),
					ОписаниеОшибки);
...
"Дополнительная информация: %1.';uk='Квитанция получена. Ошибка: Системный сбой."
				...
				ИначеЕсли ОбъектXDTO.result.reject <> Неопределено Тогда
					...
					Если ОбъектXDTO.result.reject.denied_action <> Неопределено Тогда
						...
						Если ОписаниеТипаДокумента <> Неопределено Тогда
							ОписаниеОшибки = СтрШаблон(НСтр("ru='Недостаточно прав на выполнение операции ""%1""';uk='Недостаточно прав на выполнение операции ""%1""'"), ОписаниеТипаДокумента.Описание);
						Иначе
							ОписаниеОшибки = СтрШаблон(НСтр("ru='Недостаточно прав на выполнение операции (action id: %1)';uk='Недостаточно прав на выполнение операции (action id: %1)'"), ТипДокумента);
						КонецЕсли;
...
"Дополнительная информация: %1.';uk='Квитанция получена. Ошибка: Отказ на проводку запроса."
"Дополнительная информация: %1.'"),
						ОписаниеОшибки);
...
"Дополнительная информация: %1.';uk='Запрос ожидает рассмотрения и утверждения."
"Дополнительная информация: %1.'"),
						ОписаниеОшибки);
...
"Текст ошибки: %1';uk='При разборе объекта XDTO ответа на запрос получения квитанции из ГИСМ возникла ошибка:"
"Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
...
"Текст ошибки: %1';uk='При разборе XML ответа на запрос передачи данных в ГИСМ возникла ошибка:"
"Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
...
"Код ошибки: %1. Текст ошибки: %2.';uk='При передаче данных в ГИСМ возникла ошибка"
...
"Дополнительная информация: %1.';uk='При передаче данных в ГИСМ произошел системный сбой."
"Дополнительная информация: %1.'"),
					ОписаниеОшибки);
...
"Текст ошибки: %1';uk='При разборе объекта XDTO ответа на запрос передачи данных в ГИСМ возникла ошибка:"
"Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
...
"Текст ошибки: %1';uk='При разборе XML ответа на запрос получения документа из ГИСМ возникла ошибка:"
"Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
	...
	Попытка
		...
		Если ОбъектXDTO.item_body = Неопределено Тогда
			...
			Если ДанныеДляПолученияДокумента = Неопределено Тогда
				...
				ДанныеДляПолученияДокумента.Вставить("action_id", НСтр("ru='<не прочитано>';uk='<не прочитано>'"));
				ДанныеДляПолученияДокумента.Вставить("output_id", НСтр("ru='<не прочитано>';uk='<не прочитано>'"));
				
				НаименованиеДокумента = НСтр("ru='<не прочитано>';uk='<не прочитано>'");
...
"Код ошибки: %1. Текст ошибки: %2.';uk='При получении документа %5 (action_id=%3, output_id=%4) из ГИСМ возникла ошибка:"
"Код ошибки: %1. Текст ошибки: %2.'"),
					КодОшибки,
					ОписаниеОшибки,
...
"Дополнительная информация: %1.';uk='При получении документа %4 (action_id=%2, output_id=%3) из ГИСМ произошел системный сбой."
"Дополнительная информация: %1.'"),
					ОписаниеОшибки,
					ДанныеДляПолученияДокумента.action_id,
					ДанныеДляПолученияДокумента.output_id,
...
"Текст ошибки: %1';uk='При разборе объекта XDTO ответа на запрос получения документа из ГИСМ возникла ошибка:"
"Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
...
"Текст ошибки: %1';uk='При разборе XML ответа на запрос получения списка новых документов из ГИСМ возникла ошибка:"
"Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
...
"Код ошибки: %1. Текст ошибки: %2.';uk='При получении списка новых документов из ГИСМ возникла ошибка:"
"Код ошибки: %1. Текст ошибки: %2.'"),
					КодОшибки,
					ОписаниеОшибки);
...
"Дополнительная информация: %1.';uk='При получении списка новых документов из ГИСМ произошел системный сбой."
"Дополнительная информация: %1.'"),
					ОписаниеОшибки);
...
"Текст ошибки: %1';uk='При разборе объекта XDTO ответа на запрос получения списка новых документов из ГИСМ возникла ошибка:"
"Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
...
"Текст ошибки: %1';uk='При разборе XML ответа на запрос получения данных реестра из ГИСМ возникла ошибка:"
"Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
...
"Код ошибки: %1. Текст ошибки: %2.';uk='При получении данных реестра из ГИСМ возникла ошибка:"
"Код ошибки: %1. Текст ошибки: %2.'"),
					КодОшибки,
					ОписаниеОшибки);
...
"Дополнительная информация: %1.';uk='При получении данных реестра из ГИСМ произошел системный сбой."
"Дополнительная информация: %1.'"),
					ОписаниеОшибки);
...
"Текст ошибки: %1';uk='При разборе объекта XDTO ответа на запрос получения данных реестра из ГИСМ возникла ошибка:"
"Текст ошибки: %1'"), ИнформацияОбОшибке(), ВозвращаемоеЗначение);
...
Функция ОтправитьЗапрос(ТекстПодписанногоИсходящегоСообщенияXML, ВариантЗапроса) Экспорт
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(РезультатОтправки.КодСостояния) Тогда
			...
			ЗаголовокОшибки = НСтр("ru='HTTP-запрос не отправлен.';uk='HTTP-запрос не отправлен.'");
		Иначе
			ТекстСообщенияXMLОтправлен = Истина;
			ЗаголовокОшибки = СтрШаблон(НСтр("ru='Код состояния HTTP: %1.';uk='Код состояния HTTP: %1.'"), РезультатОтправки.КодСостояния);
		КонецЕсли;
