...
"ВыгрузкаЗагрузкаСовместноРазделенныхДанных.ЗаполнитьКэшСовместноРазделенныхОбъектов()';uk='Об`єкт метаданих %1 не може бути обробником ВыгрузкаСовместноРазделенныхДанных.ПередВыгрузкойОбъекта(),"
"оскільки відсутній в кеші спільно-розділених об`єктів."
"Якщо після редагування структури метаданих конфігурації кеш не оновлювався - необхідно виконувати"
"оновлення кешу спільно-розділених об`єктів за допомогою виклику методу"
...
"Если после редактирования структуры метаданных конфигурации кэш не обновлялся - необходимо выполнять"
"обновление кэша совместно-разделенных объектов с помощью вызова метода"
"ВыгрузкаЗагрузкаСовместноРазделенныхДанных.ЗаполнитьКэшСовместноРазделенныхОбъектов()';uk='Об`єкт метаданих %1 не може бути обробником ВыгрузкаСовместноРазделенныхДанных.ПередЗагрузкойОбъекта(),"
"оскільки відсутній в кеші спільно-розділених об`єктів."
"Якщо після редагування структури метаданих конфігурації кеш не оновлювався - необхідно виконувати"
"оновлення кешу спільно-розділених об`єктів за допомогою виклику методу"
"ВыгрузкаЗагрузкаСовместноРазделенныхДанных.ЗаполнитьКэшСовместноРазделенныхОбъектов()'", Метаданные.ОсновнойЯзык.КодЯзыка),
