...
Процедура УдалитьВнешнююКомпонентуИзКэш(ИмяМодуля) Экспорт
	...
	Если ПараметрыОбменаСБанками <> Неопределено Тогда
		...
		Если ВнешниеКомпоненты <> Неопределено Тогда
			ВнешниеКомпоненты.Удалить(ИмяМодуля);
		КонецЕсли;
	КонецЕсли;
...
Функция ПодключеннаяВнешняяКомпонентаБанка(ИмяВК) Экспорт
	...
	Иначе
		ВнешниеКомпоненты = ПараметрыПодсистемыОбменСБанками.Получить("ВнешниеКомпоненты");
		Если ЗначениеЗаполнено(ВнешниеКомпоненты) Тогда
			Возврат ВнешниеКомпоненты.Получить(ИмяВК);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗакэшироватьПодключаемыйМодуль(ИмяВК, ПодключаемыйМодуль) Экспорт
	...
	ВнешниеКомпоненты = ПараметрыПодсистемыОбменСБанками.Получить("ВнешниеКомпоненты");
	Если ВнешниеКомпоненты = Неопределено Тогда
		ВнешниеКомпоненты = Новый Соответствие;
	КонецЕсли;
	ВнешниеКомпоненты.Вставить(ИмяВК, ПодключаемыйМодуль);
	ПараметрыПодсистемыОбменСБанками.Вставить("ВнешниеКомпоненты", ВнешниеКомпоненты);
...
Процедура ЗавершитьТестПодписиСертификатаСбербанк(ПодписьВерна, Параметры) Экспорт
	...
	Если ПодписьВерна = Неопределено Тогда
		Параметры.Контекст.ОписаниеОшибки = НСтр("ru='При проверке подписи произошла ошибка';uk='При проверке подписи произошла ошибка'");
	ИначеЕсли НЕ ПодписьВерна Тогда
		Параметры.Контекст.ОписаниеОшибки = НСтр("ru='Установленная подпись неверна';uk='Установленная подпись неверна'");
	КонецЕсли;
...
Процедура ПослеПолученияСпискаБизнесСистемСТокенаСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВызова <> 0 Тогда
		ТекстСообщения = НСтр("ru='Произошла ошибка при получении списка бизнес систем.';uk='Произошла ошибка при получении списка бизнес систем.'");
		ТекстОшибки = НСтр("ru='Внешняя компонента VpnKey-TLS при получении списка бизнес систем вернула код ошибки %1';uk='Внешняя компонента VpnKey-TLS при получении списка бизнес систем вернула код ошибки %1'");
		ТекстОшибки = СтрШаблон(ТекстОшибки, РезультатВызова);
		Операция = НСтр("ru='Получение списка бизнес систем.';uk='Получение списка бизнес систем.'");
...
Процедура ОбработатьОшибкуПолученияСпискаБизнесСистемНаТокенеСбербанк(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	Операция = НСтр("ru='Получение списка бизнес систем на аппаратном устройстве.';uk='Получение списка бизнес систем на аппаратном устройстве.'");
	ТекстСообщения = НСтр("ru='Не удалось получить список бизнес систем с аппаратного устройства';uk='Не удалось получить список бизнес систем с аппаратного устройства'");
...
Процедура ПроверитьУстановкуПодписиПослеАутентификацииНаТокенеСбербанк(Успех, Параметры) Экспорт
	...
	Если Не Успех Тогда
		ОписаниеОшибки = НСтр("ru='Не удалось аутентифицироваться на банковском ключе';uk='Не удалось аутентифицироваться на банковском ключе'");
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка получения банковских сертификатов."
"Код ошибки: ДО-%1"
"%2'");
		...
		Операция = НСтр("ru='Получение банковских сертификатов';uk='Отримання банківських сертифікатів'");
	...
	Если СертификатыНаУстройстве.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru='На банковском ключе отсутствуют сертификаты.';uk='На банківському ключі відсутні сертифікати.'");
	...
	ОткрытьФорму("Обработка.ОбменСБанками.Форма.ВыборСертификатаДляДобавления", ПараметрыФормы, , ,
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка установки PIN-кода."
"Код ошибки: ДО-%1"
"%2'");
		...
		Операция = НСтр("ru='Установка PIN-кода';uk='Установка PIN-кода'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
...
Процедура ПолучитьВыпискуЧерезДополнительнуюОбработку(ВнешнийПодключаемыйМодуль, Параметры) Экспорт
	...
	Если ВнешнийПодключаемыйМодуль <> Неопределено Тогда
		...
		Если Устройства <> Неопределено И Устройства.Количество() > 0 Тогда
			...
			КонецЦикла
		Иначе
			ТекстСообщения = НСтр("ru='Для выполнения операции необходимо подключить банковский ключ к компьютеру';uk='Для выполнения операции необходимо подключить банковский ключ к компьютеру'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат
		КонецЕсли;
		...
		ВидОперации = НСтр("ru='Аутентификация на ресурсе банка';uk='Аутентификация на ресурсе банка'");
		Если УстановленПарольСертификата Тогда
		...
		Иначе
			Если Соответствие.Количество() > 0 Тогда
				Если НЕ ЕстьСертификатССохраненнымПаролем(Соответствие, ВнешнийПодключаемыйМодуль) Тогда
					Оповещение = Новый ОписаниеОповещения(
				...
				КонецЕсли;
			Иначе
				ТекстСообщения = НСтр("ru='К компьютеру подключен не подходящий банковский ключ';uk='К компьютеру подключен не подходящий банковский ключ'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
				Возврат;
			КонецЕсли;
		КонецЕсли;
		...
		Если Устройства <> Неопределено И Устройства.Количество() > 0 Тогда
			...
			Если НЕ ПарольУстановлен Тогда
				...
				Иначе
					...
					ПолучитьПарольКСертификату(ОООЗ, Соответствие, НСтр("ru='Аутентификация на ресурсе банка';uk='Аутентификация на ресурсе банка'"));
			...
			КонецЕсли;
		Иначе
			ТекстСообщения = НСтр("ru='К компьютеру не подключен банковский ключ для отправки данных по настройке обмена: %1';uk='К компьютеру не подключен банковский ключ для отправки данных по настройке обмена: %1'");
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка проверки подписи."
"Код ошибки: ДО-%1"
"%2'");
					ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
					ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
					Операция = НСтр("ru='Проверка подписи';uk='Перевірка підпису'");
					ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция УстановитьПарольСертификатаЧерезДополнительнуюОбработку(ВнешнийПодключаемыйМодуль, СертификатXML, Пароль, Идентификатор = Неопределено) Экспорт
	...
	Если Не УстановленПарольСертификатаЧерезДополнительнуюОбработку(ВнешнийПодключаемыйМодуль, СертификатXML) Тогда
		Попытка
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка установки пароля сертификата."
"Код ошибки: ДО-%1"
"%2'");
			ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
			ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
			Операция = НСтр("ru='Установка пароля сертификата';uk='Установка пароля сертификата'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка проверки установленного PIN-кода."
"Код ошибки: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ru='Проверка наличия установленного PIN-кода';uk='Проверка наличия установленного PIN-кода'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		Возврат Ложь;
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка отправки данных."
"Код ошибки: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ru='Отправка данных';uk='Відправлення даних'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
	КонецПопытки;
...
Процедура НачатьТестНастройкиОбменаЧерезДополнительнуюОбработку(Параметры) Экспорт
	...
	РезультатТеста = НСтр("ru='Пройден успешно.';uk='Пройден успешно.'");
	...
	Если ВнешнийПодключаемыйМодуль = Неопределено Тогда
		ТекстСообщения = НСтр("ru='Тест не пройден.';uk='Тест не пройден.'");
	...
	Для Каждого Элемент Из ДоступныеСертификаты Цикл
		...
		ТекстСообщения = НСтр("ru='Проверка сертификата: %1';uk='Проверка сертификата: %1'");
		...
		ПолучитьПарольКСертификату(Оповещение, Соответствие, НСтр("ru='Аутентификация на ресурсе банка';uk='Аутентификация на ресурсе банка'"));
		Возврат;
	КонецЦикла;
	
	Если ДоступныеСертификаты.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru='Проверка проведена не полностью, т.к. в настройке обмена отсутствуют сертификаты подписи';uk='Проверка проведена не полностью, т.к. в настройке обмена отсутствуют сертификаты подписи'");
		СообщитьПользователю(ТекстСообщения, ИдентификаторНазначения);
		Возврат;
	КонецЕсли;
	
	// Блок проверки отправки тестового запроса.
	ОписаниеТеста = НСтр("ru='Тест. Отправка тестового запроса.';uk='Тест. Отправка тестового запроса.'");
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка отправки тестового запроса."
"Код ошибки: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ru='Отправка тестового запроса';uk='Відправка тестового запиту'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка при поиске подключенных аппаратных устройств."
"Код ошибки: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ru='Поиск аппаратных устройств.';uk='Поиск аппаратных устройств.'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
							Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		Возврат Неопределено;
...
"Убедитесь, что устройство подключено к компьютеру и повторите операцию';uk='Не найдено ни одного аппаратного устройства."
		...
		Если НЕ ПодключаемыйМодуль.УстановитьСоединение(СертификатXML) Тогда
			...
			Если ТребуетсяРасширеннаяАутентификация Тогда
				Если ПараметрыРасширеннойАутентификации.Способы.Количество() = 0 Тогда
					ВызватьИсключение НСтр("ru='Не определены способы расширенной аутентификации.';uk='Не визначені способи розширеної аутентифікації.'");
				КонецЕсли;
				Если НЕ ПараметрыРасширеннойАутентификации.Способы.Свойство("SMS") Тогда
					ВызватьИсключение НСтр("ru='Расширенная аутентификация по SMS не поддерживается.';uk='Розширена аутентифікація по SMS не підтримується.'");
				КонецЕсли;
			...
			Иначе
				ВызватьИсключение НСтр("ru='Ошибка установки соединения.';uk='Помилка установки з`єднання.'");
			КонецЕсли;
		Иначе
			ВыполнитьОбработкуОповещения(Оповещение, Истина);
		КонецЕсли;
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка установки соединения."
"Код ошибки: ДО-%1"
"%2'");
		ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ru='Установка соединения';uk='Установка з`єднання'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, НастройкаОбмена);
		ВыполнитьОбработкуОповещения(Оповещение, Ложь);
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка проверки необходимости ввода PIN-кода."
"Код ошибки: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ru='Проверка необходимости ввода PIN-кода';uk='Проверка необходимости ввода PIN-кода'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		Ошибка = Истина;
...
"Причина: %1';uk='Электронная подпись неверна."
"Причина: %1'");
...
"Проверьте настройки криптографии или обратитесь к администратору.';uk='Не удалось проверить подписи электронного документа."
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка получения данных сертификата."
"Код ошибки: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ru='Получение данных сертификата';uk='Получение данных сертификата'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		Возврат Неопределено;
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка получения схемы данных."
"Код ошибки: ДО-%1"
"%2'");
				ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
				ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
				Операция = НСтр("ru='Получение схемы данных';uk='Получение схемы данных'");
				ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
					Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, НастройкаОбмена);
				ПрерватьПодписание = Истина;
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка подписания документов."
"Код ошибки: ДО-%1"
"%2'");
				ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
				ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
				Операция = НСтр("ru='Подписание документов';uk='Подписание документов'");
				ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
									Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, НастройкаОбмена);
				ПрерватьПодписание = Истина;
...
"Тест прерван.';uk='Не введен пароль для сертификата."
"Тест прерван.'");
		СообщитьПользователю(ТекстСообщения, ИдентификаторНазначения);
		Возврат;
	КонецЕсли;
	...
	ОписаниеТеста = НСтр("ru='Тест. Чтение данных сертификата.';uk='Тест. Чтение данных сертификата.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
	ДанныеСертификата = ДанныеСертификатаЧерезДополнительнуюОбработку(ВнешнийПодключаемыйМодуль, СертификатXML);
	Если ДанныеСертификата = Неопределено Тогда
		Возврат;
	...
	ОписаниеТеста = НСтр("ru='Тест. Проверка наличия PIN-кода на аппаратном устройстве.';uk='Тест. Проверка наличия PIN-кода на аппаратном устройстве.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
	ЕстьОшибка = Ложь;
	ТребуетсяПИН = ТребуетсяУстановкаPINКодаХранилищаЧерезДополнительнуюОбработку(
	...
	Если ТребуетсяПИН Тогда
		ОписаниеТеста = НСтр("ru='Тест. Установка PIN-кода на аппаратном устройстве.';uk='Тест. Установка PIN-кода на аппаратном устройстве.'");
		СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
		ПараметрыФормы = Новый Структура;
...
Процедура ПродолжитьТестНастройкиОбменаЧерезДополнительнуюОбработкуПослеАутентификации(АутентификацияВыполнена, Параметры) Экспорт
	...
	ОписаниеТеста = НСтр("ru='Тест. Установка подписи.';uk='Тест. Установка подписи.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка установки подписи."
"Код ошибки: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ru='Установка подписи';uk='Установка підпису'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		Возврат;
	...
	ОписаниеТеста = НСтр("ru='Тест. Проверка подписи.';uk='Тест. Проверка подписи.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
	Попытка
...
"%2'");
	...
	Если НЕ ПодписьВалидна Тогда
		СообщитьПользователю(НСтр("ru='Подпись не валидна';uk='Подпись не валидна'"), ИдентификаторНазначения);
		Возврат;
	КонецЕсли;
	СообщитьПользователю(РезультатТеста, ИдентификаторНазначения);
...
Процедура НачатьПроцессПолученияВыпискиЧерезВК(НастройкаОбмена, ДатаНачала, ДатаОкончания, НомерСчета, Владелец) Экспорт
	...
	Если НастройкиОбмена.Подписывать Тогда
		Если НЕ ЗначениеЗаполнено(НастройкиОбмена.СертификатОрганизацииДляПодписи)
			...
			ТекстСообщения = НСтр("ru='Не найден подходящий сертификат для подписи документа Запрос выписки';uk='Не найден подходящий сертификат для подписи документа Запрос выписки'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
		...
		Иначе
			...
			ВидОперации = НСтр("ru='Подписание электронных документов';uk='Подписание электронных документов'");
...
Процедура ПослеОшибкиВключенияЖурналированияВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ru='При включении журналирования произошла ошибка.';uk='При включении журналирования произошла ошибка.'");
	ВидОперации = НСтр("ru='Включение журналирования.';uk='Включение журналирования.'");
...
Процедура ПослеПопыткиПодключенияВнешнейКомпоненты(Результат, Параметры) Экспорт
	...
	Если Результат Тогда
		...
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка подключения внешнего модуля банка.';uk='Ошибка подключения внешнего модуля банка.'");
			Операция = НСтр("ru='Подключение внешней компоненты банка';uk='Подключение внешней компоненты банка'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		КонецПопытки;
...
Процедура ПослеПодключенияВнешнейКомпоненты(Подключено, Параметры) Экспорт
	...
	Если Подключено Тогда
		Попытка
			...
			ЗакэшироватьПодключаемыйМодуль(Параметры.НазваниеВК, ПодключаемыйМодуль);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка подключения внешней компоненты банка.';uk='Ошибка подключения внешней компоненты банка.'");
			Операция = НСтр("ru='Подключение внешнего модуля банка';uk='Подключение внешнего модуля банка'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		КонецПопытки;
...
Функция ТекстСообщенияСбербанк(Тикет)
	...
	Если Тикет = "00000000-0000-0000-0000-000000000000" Тогда
		ТекстСообщения = НСтр("ru='Сервер банка временно недоступен. Повторите попытку или обратитесь в свой банк.';uk='Сервер банка временно недоступен. Повторите попытку или обратитесь в свой банк.'");
...
"Код: %1"
"Повторите сеанс связи или обратитесь в свой банк.';uk='Сервер банка вернул неизвестную ошибку."
"Код: %1"
...
"Необходимо проверить работу TLS VPN Key.';uk='Не найдена бизнес система на банковском ключе."
"Необходимо проверить работу TLS VPN Key.'");
		Операция = НСтр("ru='Поиск бизнес системы на электронном ключе.';uk='Поиск бизнес системы на электронном ключе.'");
		ШаблонОшибки = НСтр("ru='На электронном ключе не найдена бизнес-система %1."
"Содержимое списка возврата: %2';uk='На электронном ключе не найдена бизнес-система %1."
...
"%1';uk='При аутентификации на аппаратном устройстве произошла ошибка."
"%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ru='Предъявление кода доступа на аппаратном устройстве';uk='Предъявление кода доступа на аппаратном устройстве'");
...
Процедура ПолучитьИнформациюОТокенеСбербанк(Оповещение, ПодключаемыйМодуль)
	...
	Исключение
		Операция = НСтр("ru='Получение информации об аппаратном устройстве.';uk='Получение информации об аппаратном устройстве.'");
		КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ШаблонСообщения = НСтр("ru='При получении информации об аппаратном устройстве произошла ошибка:"
"%1';uk='При получении информации об аппаратном устройстве произошла ошибка:"
"%1'");
...
"Внешний модуль VpnKey-TLS вернул код ошибки %1';uk='При получении информации об аппаратном устройстве произошла ошибка."
...
"%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ru='Получение информации об аппаратном устройстве.';uk='Получение информации об аппаратном устройстве.'");
...
Процедура ОбработатьРезультатАсинхроннойПодписиСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВызова <> 0 Тогда
		Операция = НСтр("ru='Подписание данных';uk='Підписання даних'");
		ТекстОшибки = НСтр("ru='При подписании электронного документа произошла ошибка"
"Внешний модуль VpnKey-TLS при подписании вернул код ошибки %1';uk='При подписании электронного документа произошла ошибка"
"Внешний модуль VpnKey-TLS при подписании вернул код ошибки %1'");
	...
	Иначе // сенсорный или экранный
		...
		ОткрытьФорму("Обработка.ОбменСБанками.Форма.ОжиданиеПодписиСбербанк", ПараметрыФормы, , ,
			ВариантОткрытияОкна.ОтдельноеОкно, , ДополнительныеПараметры.ОповещениеПослеПодписи);
	КонецЕсли;
...
"%1';uk='При подписи данных произошла ошибка."
"%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ru='Подпись данных.';uk='Подпись данных.'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, , 1);
	ОчиститьДанныеАвторизацииСбербанк();
	Результат = Новый Структура;
	Результат.Вставить("Успех", Ложь);
...
Процедура ПослеПолученияДанныхСертификатаСТокенаСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВызова <> 0 Тогда
		ТекстСообщения = НСтр("ru='При получении данных сертификата произошла ошибка.';uk='При получении данных сертификата произошла ошибка.'");
		ТекстОшибки = НСтр("ru='Внешний модуль VpnKey-TLS при получении сертификата вернул код ошибки';uk='Внешний модуль VpnKey-TLS при получении сертификата вернул код ошибки'") + РезультатВызова;
		Операция = НСтр("ru='Получение сертификата криптографии.';uk='Получение сертификата криптографии.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
...
"%1';uk='При получении сертификата с аппаратного устройства произошла ошибка."
"%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ru='Получение данных сертификата с аппаратного устройства.';uk='Получение данных сертификата с аппаратного устройства.'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, ТекстСообщения, 1);
	ОчиститьДанныеАвторизацииСбербанк();
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеПолученияДанныхСертификата, Неопределено);
...
Процедура ПроверитьУстановкуПодписиПолученияСертификатовСТокенаСбербанк(Результат, Параметры) Экспорт
	...
	Для Каждого КлючЗначение Из СоответствиеСертификатов Цикл
		...
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка чтения сертификата.';uk='Ошибка чтения сертификата.'");
		...
		Если СтруктураСертификата.Отпечаток = РеквизитыПроверяемогоСертификата.Отпечаток Тогда
			...
			Представление = НСтр("ru='Тестовая строка';uk='Тестовая строка'");
	...
	ТекстСообщения = НСтр("ru='Сертификат не найден на аппаратном устройстве.';uk='Сертификат не найден на аппаратном устройстве.'");
...
"Внешний модуль VpnKey-TLS при получении списка доступных сертификатов вернул код ошибки: %1';uk='При получении списка сертификатов на банковском ключе произошла ошибка."
"Внешний модуль VpnKey-TLS при получении списка доступных сертификатов вернул код ошибки: %1'");
		...
		Операция = НСтр("ru='Подписание электронного документа.';uk='Подписание электронного документа.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, , 1);
		ОчиститьДанныеАвторизацииСбербанк();
		Результат = Новый Структура("Успех, ТекстОшибки", Ложь, ТекстОшибки);
	...
	Если СоответствиеСертификатов.Количество() Тогда
		...
		Возврат;
	Иначе
		ТекстСообщения = НСтр("ru='Не найден ни один сертификат на аппаратном устройстве.';uk='Не найден ни один сертификат на аппаратном устройстве.'");
		Результат = Новый Структура("Успех, ТекстОшибки", Ложь, ТекстСообщения);
	КонецЕсли;
...
Процедура ОбработатьОшибкуПолученияСпискаИдентификаторовСертификатов(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	Операция = НСтр("ru='Получение списка сертификатов с аппаратного устройства.';uk='Получение списка сертификатов с аппаратного устройства.'");
	ТекстСообщения = НСтр("ru='Не удалось получить информацию о сертификатах с аппаратного устройства.';uk='Не удалось получить информацию о сертификатах с аппаратного устройства.'");
	ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, , 1);
	ОчиститьДанныеАвторизацииСбербанк();
	Результат = Новый Структура("Успех, ТекстОшибки", Ложь, ТекстСообщения);
...
Процедура ПодобратьСертификатПослеПолученияСпискаСТокенаСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Элемент Из СоответствиеСертификатов Цикл
		...
		Исключение
			...
			Операция = НСтр("ru='Чтение данных сертификата.';uk='Чтение данных сертификата.'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"- Указана неверная учетная запись. Выберите правильную учетную запись при входе на аппаратное устройство.';uk='Не найден подходящий сертификат подписи."
"Возможные причины:"
"- В настройке обмена через сервис 1С:ДиректБанк указан сертификат другой организации. Загрузите нужный сертификат с аппаратного устройства."
"- Вставлено другое аппаратное устройство. Проверьте, какое устройство подключено к компьютеру."
"- Указана неверная учетная запись. Выберите правильную учетную запись при входе на аппаратное устройство.'");
	...
	ЗаголовокФормыВыбора = НСтр("ru='Выберите сертификат подписи';uk='Выберите сертификат подписи'");
...
"%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ru='Предъявление данных аутентификации на аппаратном устройстве.';uk='Предъявление данных аутентификации на аппаратном устройстве.'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, ТекстСообщения, 1);
	ОчиститьДанныеАвторизацииСбербанк();
...
"Внешний модуль VpnKey-TLS при подписании вернул код ошибки %1'");
	...
	ИначеЕсли Состояние = "FAILED" ИЛИ Состояние = "REJECTED" Тогда
		Операция = НСтр("ru='Подписание данных';uk='Підписання даних'");
		ТекстОшибки = НСтр("ru='При подписании электронного документа произошла ошибка"
"Код состояния: %1';uk='При подписании электронного документа произошла ошибка"
"Код состояния: %1'");
...
"%1';uk='При получении данных подписи произошла ошибка:"
"%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ru='Получение данных подписи';uk='Получение данных подписи'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, , 1);
	ОчиститьДанныеАвторизацииСбербанк();
...
"Удалите программу ""VpnKey-TLS для 1С:Предприятия 8"", перезагрузите компьютер и повторите операцию.';uk='На компьютере уже установлен внешний модуль другой версии."
...
"Загрузите новый внешний модуль или перезапустите программу.';uk='Отличаются версии используемого и загруженного внешнего модуля."
...
Процедура ПодписатьТестовыйЗапросПослеОпределенияСертификатаСбербанк(Результат, Параметры) Экспорт
	...
	ПодключаемыйМодуль = ПодключеннаяВнешняяКомпонентаБанка("VPNKeyTLS");
	Представление = НСтр("ru='Тестовый запрос';uk='Тестовий запит'");
...
Процедура ОтправитьТестовыйЗапросПослеПроверкиПодписиСбербанк(ПодписьВерна, Параметры) Экспорт
	...
	Если Не ПодписьВерна Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Установленная подпись неверна.';uk='Встановлений підпис невірний.'"));
		Результат = Новый Структура("Успех", Ложь);
		ВыполнитьОбработкуОповещения(Параметры.ОповещениеПослеТестаНастройки, Результат);
		Возврат;
	КонецЕсли;
...
Процедура ПослеОтправкиЗапросаВСбербанк(РезультатВызова, ПараметрыВызова, Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(КодОшибки) ИЛИ ЗначениеЗаполнено(ТекстОшибки) Тогда
		ВидОперации = НСтр("ru='Отправка запроса в банк';uk='Отправка запроса в банк'");
		ТекстСообщения = НСтр("ru='Нет связи с сервером банка."
"Повторите операцию позже или обратитесь в техподдержку банка';uk='Нет связи с сервером банка."
...
"Код ошибки: %1"
"Текст ошибки: %2';uk='Произошла ошибка сетевого взаимодействия."
"Код ошибки: %1"
"Текст ошибки: %2'");
	...
	Если РезультатВызова = "<!--REQUESTID_DUBLIC-->" Тогда
		ТекстОшибки = НСтр("ru='Документ с таким идентификатором есть в информационной базе банка. Повторная отправка невозможна.';uk='Документ с таким идентификатором есть в информационной базе банка. Повторная отправка невозможна.'");
		Результат = Новый Структура("Успех, ТекстОшибки", Ложь, ТекстОшибки);
		ВыполнитьОбработкуОповещения(Параметры.Оповещение, Результат);
		Возврат;
	КонецЕсли;
...
"%1';uk='При отправке запроса в банк произошла ошибка."
"%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ru='Отправка запроса в банк.';uk='Отправка запроса в банк.'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, , 1);
...
"%2';uk='Произошла ошибка при отправке документа %1:"
"%2'");
...
Процедура ПослеПолученияДокументаСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(КодОшибки) ИЛИ ЗначениеЗаполнено(ТекстОшибки) Тогда
		ВидОперации = НСтр("ru='Получение документов из банка.';uk='Получение документов из банка.'");
		ШаблонОшибки = НСтр("ru='Произошла ошибка сетевого взаимодействия."
"Код ошибки: %1."
"Текст ошибки: %2';uk='Произошла ошибка сетевого взаимодействия."
"Код ошибки: %1."
...
"%1';uk='При получении документов из банка произошла ошибка."
"%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ru='Получение документов из банка.';uk='Получение документов из банка.'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, , 1);
...
Процедура ОтправитьЗапросВыпискиСбербанк(Оповещение, НастройкаОбмена, ЗапросВыписки) Экспорт
	...
	ВидОперации = НСтр("ru='Запрос выписки банка';uk='Запрос выписки банка'");
...
Процедура ПослеУстановкиКаналаСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если НЕ РезультатВызова = 0 Тогда
		Операция = НСтр("ru='Установка виртуального канала';uk='Установка виртуального канала'");
		ТекстСообщения = НСтр("ru='Не удалось установить связь с сервером."
"Необходимо проверить работу TLS VPN Key.';uk='Не удалось установить связь с сервером."
"Необходимо проверить работу TLS VPN Key.'");
		ТекстОшибки = НСтр("ru='Компонента AddIn.Bicrypt при установке виртуального канала вернула код ошибки';uk='Компонента AddIn.Bicrypt при установке виртуального канала вернула код ошибки'")
							+ " " + РезультатВызова;
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
		ОчиститьДанныеАвторизацииСбербанк();
	КонецЕсли;
...
"%1';uk='При установке канала на аппаратном устройстве произошла ошибка."
"%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ru='Установка канала на аппаратном устройстве.';uk='Установка канала на аппаратном устройстве.'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, ТекстСообщения, 1);
	ОчиститьДанныеАвторизацииСбербанк();
...
"%1';uk='При получении версии внешнего модуля банка произошла ошибка:"
"%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ru='Получение версии внешней компоненты банка';uk='Получение версии внешней компоненты банка'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, ТекстСообщения, 1);
...
"Убедитесь, что устройство подключено к компьютеру.';uk='Ошибка при запуске клиента аппаратного устройства."
"Убедитесь, что устройство подключено к компьютеру.'");
		ТекстОшибки = НСтр("ru='Внешний модуль VpnKey-TLS при запуске Start.exe вернул код ошибки %1';uk='Внешний модуль VpnKey-TLS при запуске Start.exe вернул код ошибки %1'");
		ТекстОшибки = СтрШаблон(ТекстОшибки, РезультатВызова);
		Операция = НСтр("ru='Запуск клиента аппаратного устройства.';uk='Запуск клиента аппаратного устройства.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
		ОчиститьДанныеАвторизацииСбербанк();
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеАутентификацииНаТокене, Ложь);
		Возврат;
...
"%1';uk='При запуске программы банка произошла ошибка:"
"%1'");
	ТекстСообщения = СтрШаблон(ШаблонОшибки, КраткаяИнформация);
	ВидОперации = НСтр("ru='Запуск программы Start.exe';uk='Запуск программы Start.exe'");
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, ТекстСообщения, 1);
	ОчиститьДанныеАвторизацииСбербанк();
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеАутентификацииНаТокене, Ложь);
...
Процедура ОбработатьРезультатАутентификацииНаТокенеСбербанка(НастройкаОбмена, РезультатАвторизации, ОповещениеПослеАутентификацииНаТокене)
	...
	Если Не (РезультатАвторизации = 0) Тогда
		Если РезультатАвторизации = 28 Тогда
			ШаблонСообщения = НСтр("ru='PIN%1 заблокирован';uk='PIN%1 заблокирован'");
		...
		ИначеЕсли РезультатАвторизации = 27 Тогда
			ШаблонСообщения = НСтр("ru='PUK%1 заблокирован';uk='PUK%1 заблокирован'");
			ТекстСообщения = СтрШаблон(
								ШаблонСообщения, ЗначениеИзКэшаСбербанк("НомерКонтейнера"));
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ИначеЕсли РезультатАвторизации = 25 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Пользователь заблокирован';uk='Пользователь заблокирован'"));
		ИначеЕсли РезультатАвторизации = 29 ИЛИ РезультатАвторизации = 30 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Неверные данные аутентификации';uk='Неверные данные аутентификации'"));
		Иначе
			ОчиститьСообщения();
			ТекстСообщения = НСтр("ru='Не удалось выполнить аутентификацию на аппаратном устройстве."
"Необходимо выполнить его перезапуск';uk='Не удалось выполнить аутентификацию на аппаратном устройстве."
"Необходимо выполнить его перезапуск'");
			ТекстОшибки = НСтр("ru='Внешний модуль VpnKey-TLS при авторизации на аппаратном устройстве вернул код ошибки: %1';uk='Внешний модуль VpnKey-TLS при авторизации на аппаратном устройстве вернул код ошибки: %1'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, РезультатАвторизации);
			Операция = НСтр("ru='Авторизация на токене';uk='Авторизация на токене'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура ПослеПолученияОтпечатковВыполнитьОбмен(ОтпечаткиСертификатов, Параметры) Экспорт
	...
	Если МассивСертификатов.Количество() Тогда
		...
		ОписаниеДанных.Вставить("Операция", НСтр("ru='Аутентификация на сервере банка';uk='Аутентификация на сервере банка'"));
...
"Проверьте настройки прямого обмена с банком или обратитесь к администратору';uk='Нет доступных сертификатов для аутентификации на сервере банка."
...
"Установите сертификаты в личное хранилище сертификатов операционной системы.';uk='На компьютере не найдены сертификаты для аутентификации на сервере банка."
...
Процедура ВыполнитьОбменСБанками(Результат, Параметры) Экспорт
	...
	Если НЕ Параметры.Свойство("НастройкиОбмена", НастройкиОбмена)
		...
		ЗаголовокОповещения = НСтр("ru='Обмен с сервисом 1С:ДиректБанк';uk='Обмен с сервисом 1С:ДиректБанк'");
		ШаблонОповещения = НСтр("ru='Отправлено пакетов: (%1), получено пакетов: (%2).';uk='Отправлено пакетов: (%1), получено пакетов: (%2).'");
...
Процедура ВыполнитьОбменСБанком(Параметры)
	...
	ТекстСообщения = НСтр("ru='Отправка пакетов электронных документов в банк. Подождите...';uk='Отправка пакетов электронных документов в банк. Подождите...'");
	Состояние(НСтр("ru='Отправка.';uk='Отправка.'"), , ТекстСообщения);
...
"Операция прервана.';uk='Настройка обмена недействительна."
"Операция прервана.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ВыполнитьОбменСБанками(Неопределено, Параметры);
	КонецЕсли;
...
Процедура ВыполнитьСинхронизациюПослеОпределенияСертификатаСбербанк(Результат, Параметры) Экспорт
	...
	Если ОтправлятьЗапросНовыхДокументов Тогда
		...
		ПодключаемыйМодуль = ПодключеннаяВнешняяКомпонентаБанка("VPNKeyTLS");
		Представление = НСтр("ru='Запрос новых документов';uk='Запрос новых документов'");
		Параметры.Вставить("ИдентификаторЗапроса", ИдентификаторЗапроса);
		Параметры.Вставить("ИдентификаторОрганизации", ИдентификаторОрганизации);
...
Процедура ПослеПолученияОтпечатковПроверитьСвязь(ОтпечаткиСертификатов, Параметры) Экспорт
	...
	Если НастройкиОбмена.Подписывать Тогда
		Если НЕ ЗначениеЗаполнено(НастройкиОбмена.СертификатОрганизацииДляПодписи)
			ИЛИ НЕ НастройкиОбмена.СертификатДоступен Тогда
			ТекстСообщения = НСтр("ru='Не найден подходящий сертификат для подписи документа Запрос-зонд';uk='Не найден подходящий сертификат для подписи документа Запрос-зонд'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			СтруктураРезультата = Новый Структура("Успех", Ложь);
			ВыполнитьОбработкуОповещения(Параметры.ОбработчикПослеТестаНастройки, СтруктураРезультата);
			Возврат;
		...
		Операция = НСтр("ru='Подписание электронного документа';uk='Підписання електронного документу'");
		ОписаниеДанных = Новый Структура;
...
Процедура ПолучитьМаркерБанкаПоЛогинуИПаролю(Обработчик, АдресСервера, ИдентификаторОрганизации, ДанныеАутентификации, ВерсияAPI, НастройкаОбмена = Неопределено, ПробнаяОперация = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеSMSАвторизации) Тогда
		...
		Параметры.Вставить("ВерсияAPI", ИдентификаторСессииБанка);
...
Процедура ОтправитьИПолучитьДокументыВБанк(ДанныеАутентификации, Параметры)
	...
	ЗаголовокОповещения = НСтр("ru='1С:ДиректБанк';uk='1С:ДиректБанк'");
	ПоказатьОповещениеПользователя(ЗаголовокОповещения, , ТекстОповещения);
...
Процедура ВыполнитьДействияПослеОтправки(Параметры)
	...
	ТекстЗаголовка = НСтр("ru='1С:ДиректБанк';uk='1С:ДиректБанк'");
...
Процедура ПолучитьДанныеАутентификации(НастройкаОбмена, ОписаниеОповещенияОЗакрытии, ЭтоТест = Ложь) Экспорт
	...
	ПараметрыФормы = Новый Структура();
	ПараметрыФормы.Вставить("ВидОперации", НСтр("ru='Аутентификация на сервере банка';uk='Аутентификация на сервере банка'"));
...
Процедура ОбработатьОшибкуПолученияСертификатаИзПодписи(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	Операция = НСтр("ru='Получение сертификата из подписи';uk='Получение сертификата из подписи'");
	ТекстСообщения = НСтр("ru='Не удалось получить сертификат из подписи';uk='Не удалось получить сертификат из подписи'");
	ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
...
Процедура ОбработатьОшибкуВыгрузкиСертификата(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	Операция = НСтр("ru='Выгрузка сертификата подписи';uk='Выгрузка сертификата подписи'");
	ТекстСообщения = НСтр("ru='Не удалось выгрузить сертификат полученной подписи';uk='Не удалось выгрузить сертификат полученной подписи'");
	ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения, 1);
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработчикПослеПолученияСертификатов, Неопределено);
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка инициализации дополнительной обработки."
"Код ошибки: ДО-%1"
"%2'");
			ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
			ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
			Операция = НСтр("ru='Инициализация внешней обработки';uk='Ініціалізація зовнішньої обробки'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
			Возврат Ложь;
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка завершения инициализации."
"Код ошибки: ДО-%1"
"%2'");
				ДеталиОшибки = ПодключаемыйМодуль.ДеталиОшибки();
				ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
				Операция = НСтр("ru='Завершение инициализации внешней обработки';uk='Завершення ініціалізації зовнішньої обробки'");
				ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
					Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
				Возврат Ложь;
...
Процедура ВыполнитьДействияПослеОтправкиПЭДЗавершить(Результат, ДопПараметры)
	...
	Если ДопПараметры.Свойство("СообщенияОбмена") Тогда
		...
		Для Каждого Владелец Из МассивВладельцев Цикл
			Оповестить("ОбновитьСостояниеЭД", , Владелец);
		КонецЦикла;
	КонецЕсли;
...
Процедура ОбработатьОтклонениеЭД(СообщениеОбмена, ОписаниеОповещения) Экспорт
	...
	Заголовок = НСтр("ru='Укажите причины отклонения документа';uk='Укажите причины отклонения документа'");
...
Процедура ОбработатьОтклонениеЭДЗавершить(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат <> Неопределено Тогда
		ТекстСообщения = НСтр("ru='Причина не указана, действие отменено.';uk='Причина не указана, действие отменено.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
"%1 "
"произошла ошибка: %2';uk='При проверке подписи электронного документа:"
"%1 "
"произошла ошибка: %2'");
		ПредставлениеЭД = ОбменСБанкамиСлужебныйВызовСервера.ПредставлениеСообщенияОбмена(
			ДополнительныеПараметры.СообщениеОбмена);
		ТекстСообщения = СтрШаблон(ТекстСообщения, ПредставлениеЭД, Результат);
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
		...
		Для Каждого Элемент Из СертификатыСообщенийОбмена Цикл
			...
			Если Структура.Свойство("СоответствиеСообщенийОбменаИДД", СоответствиеСообщенийОбменаИДД)
				...
				Если СоответствиеСообщенийОбменаИДД.Количество() = 1 Тогда
					Операция = НСтр("ru='Подписание электронного документа';uk='Підписання електронного документу'");
				Иначе
					Операция = НСтр("ru='Подписание электронных документов';uk='Подписание электронных документов'");
				КонецЕсли;
				...
				Иначе
					ПредставлениеСообщенияОбмена = НСтр("ru='Электронные документы (%1)';uk='Электронные документы (%1)'");
...
Процедура ТестНастройкиОбменаЧерезДополнительнуюОбработку(НастройкаОбмена, ИдентификаторНазначения)
	...
	ОписаниеТеста = НСтр("ru='Тест. Проверка наличия внешней обработки для обмена с банком.';uk='Тест. Проверка наличия внешней обработки для обмена с банком.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
	ВнешняяОбработкаПодключена = ОбменСБанкамиСлужебныйВызовСервера.ПодключитьАктуальнуюВнешнююОбработку(
	...
	ОписаниеТеста = НСтр("ru='Тест. Инициализация служебного интерфейса.';uk='Тест. Инициализация служебного интерфейса.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
...
Процедура ПродолжитьТестНастройкиОбменаЧерезДополнительнуюОбработку(Параметры)
	...
	ОписаниеТеста = НСтр("ru='Тест. Аутентификация на ресурсе банка.';uk='Тест. Аутентификация на ресурсе банка.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
	ВидОперации = НСтр("ru='Аутентификация на ресурсе банка';uk='Аутентификация на ресурсе банка'");
	Попытка
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка авторизации на ресурсе банка."
"Код ошибки: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ru='Аутентификация на ресурсе банка';uk='Аутентификация на ресурсе банка'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, НастройкаОбмена);
		Возврат;
	...
	ОписаниеТеста = НСтр("ru='Тест. Установка соединения с банком.';uk='Тест. Установка соединения с банком.'");
	СообщитьПользователю(ОписаниеТеста, ИдентификаторНазначения);
...
Процедура ПодписатьЗапросыВыписок(Результат, ПараметрыОбработки)
	...
	ПредставлениеНабора = НСтр("ru='Запросы выписок (%1)';uk='Запросы выписок (%1)'");
...
Процедура ПослеПолученияВерсииВК(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если НЕ РезультатВызова = ДополнительныеПараметры.ОжидаемаяВерсия Тогда
		#Если ВебКлиент Тогда
			ТекстСообщения = НСтр("ru='На компьютере уже установлен внешний модуль другой версии."
"Удалите расширение для браузера, перезагрузите компьютер и повторите операцию.';uk='На компьютере уже установлен внешний модуль другой версии."
"Удалите расширение для браузера, перезагрузите компьютер и повторите операцию.'");
		#Иначе
			ТекстСообщения = НСтр("ru='Отличаются версии используемого и загруженного внешнего модуля."
"Перезапустите программу.';uk='Отличаются версии используемого и загруженного внешнего модуля."
...
Процедура ОбработатьОшибкуПолученияВерсииВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ВидОперации = НСтр("ru='Получение версии внешней компоненты.';uk='Получение версии внешней компоненты.'");
	ТекстСообщения = НСтр("ru='Произошла ошибка при получении версии внешнего модуля.';uk='Произошла ошибка при получении версии внешнего модуля.'");
...
Процедура ПолучитьПарольАутентификацииПослеОпределенияСертификата(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ВидОперации = НСтр("ru='Аутентификация на сервере банка';uk='Аутентификация на сервере банка'") + " "; // Для запроса пароля сертификата
...
Процедура ОбработатьОшибкуПроверкиНеобходимостиУстановкиПароляВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ru='При аутентификации произошла ошибка.';uk='При аутентификации произошла ошибка.'");
	ВидОперации = НСтр("ru='Проверка необходимости установки пароля.';uk='Проверка необходимости установки пароля.'");
...
Процедура УстановитьСоединениеПоСертификатуЧерезВК(Оповещение, ПодключаемыйМодуль, НастройкаОбмена, СертификатСсылка, Пароль)
	...
	АутентифицироватьсяНаЭлектронномКлючеЧерезВК(
		ОповещениеПослеАутентификации, ПодключаемыйМодуль, РеквизитыСертификата.ДанныеСертификата, Пароль);
...
Процедура ОбработатьОшибкуПолученияХранилищКлючейЭП(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ВидОперации = НСтр("ru='Получение информации об аппаратных устройствах.';uk='Получение информации об аппаратных устройствах.'");
	ТекстСообщения = НСтр("ru='При поиске аппаратных устройств, подключенных к компьютеру, произошла ошибка.';uk='При поиске аппаратных устройств, подключенных к компьютеру, произошла ошибка.'");
...
Процедура ПродолжитьОтправкуПлатежныхДокументовПоНастройкамиПослеПолученияНовыхДокументовЧерезВК(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат.Результат) = Тип("Строка") Тогда
		ВидОперации = НСтр("ru='Получение новых документов из банка.';uk='Получение новых документов из банка.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
// Производит процесс аутентификации на электронном ключе (асинхронно).
//
// Параметры:
//  Оповещение - ОписаниеОповещения - Содержит описание процедуры, которая будет вызвана после завершения вызова метода со следующими параметрами:
//                * Результат - Булево - Истина, если аутентификация выполнена
//                            - Строка - текст ошибки
//                * ДополнительныеПараметры - значение, которое было указано при создании объекта ОписаниеОповещения.
//  ПодключаемыйМодуль - AddIn - внешняя компонента банка;
//  СертификатBase64 - Строка - данные сертификата подписи;
//  Пароль - Строка - пароль закрытой части сертификата.
//
Процедура ВызватьАутентификациюНаЭлектронномКлючеВК(Оповещение, ПодключаемыйМодуль, СертификатBase64, Пароль)
...
Процедура АутентифицироватьсяНаЭлектронномКлючеЧерезВК(Оповещение, ПодключаемыйМодуль, СертификатBase64, Пароль, ЭтоТест = Ложь)
...
Процедура ВыполнитьАутентификациюПослеПроверкиНеобходимостиУстановкиПароляЧерезВК(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда // Требуется установить пароль сертификата
		ВызватьАутентификациюНаЭлектронномКлючеВК(ДополнительныеПараметры.ОповещениеПослеАутентификации,
			...
			ДополнительныеПараметры.Пароль);
	Иначе
...
Процедура АутентифицироватьсяНаТокенеПослеПодключенияВК(Результат, ДополнительныеПараметры) Экспорт
	...
	РеквизитыСертификата = Новый Структура("ДанныеСертификата");
	ОбменСБанкамиСлужебныйВызовСервера.ПолучитьЗначенияРеквизитовСертификата(
		ДополнительныеПараметры.СертификатСсылка, РеквизитыСертификата);
	ДополнительныеПараметры.Вставить("СертификатBase64", РеквизитыСертификата.ДанныеСертификата);
	
	Оповещение = Новый ОписаниеОповещения(
		"ПроверитьНеобходимостьДополненияЭДПослеАутентификацииЧерезВК", ЭтотОбъект, ДополнительныеПараметры);
	
	АутентифицироватьсяНаЭлектронномКлючеЧерезВК(Оповещение, Результат, ДополнительныеПараметры.СертификатBase64,
		ДополнительныеПараметры.Пароль, ДополнительныеПараметры.ЭтоТест);
...
Процедура ОбработатьОшибкуПроверкиДополненияЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ru='При подписании документа произошла ошибка.';uk='При подписании документа произошла ошибка.'");
	ВидОперации = НСтр("ru='Проверка дополнения электронного документа.';uk='Проверка дополнения электронного документа.'");
...
Процедура ОбработатьОшибкуДополненияЭДЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ru='При дополнении документа произошла ошибка.';uk='При дополнении документа произошла ошибка.'");
	ВидОперации = НСтр("ru='Дополнение электронного документа.';uk='Дополнение электронного документа.'");
...
Процедура ПослеПолученияДанныхСертификатовСКлючаВК(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДоступныеСертификаты.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru='В настройке обмена с сервисом 1С:ДиректБанк нет ни одного доступного сертификата.';uk='В настройке обмена с сервисом 1С:ДиректБанк нет ни одного доступного сертификата.'");
	...
	Если ДанныеВыбора.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru='На банковском ключе не найдены сертификаты, указанные в настройке обмена с банком.';uk='На банковском ключе не найдены сертификаты, указанные в настройке обмена с банком.'");
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеТестаНастройки, ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Если НастройкиОбмена.Подписывать Тогда
		ВидОперации = НСтр("ru='Подписание электронного документа.';uk='Подписание электронного документа.'");
		Оповещение = Новый ОписаниеОповещения(
...
Процедура АутентификацияПослеПолученияПароляЧерезВК(ДанныеВыбора, ДополнительныеПараметры) Экспорт
	...
	ВызватьАутентификациюНаЭлектронномКлючеВК(Оповещение, ДополнительныеПараметры.ПодключаемыйМодуль,
		СвойстваСертификата.СертификатBase64, ДанныеВыбора.ПарольПользователя);
...
Процедура ОбработатьОшибкуОтправкиПакетаЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ru='При отправке данных в банк произошла ошибка.';uk='При отправке данных в банк произошла ошибка.'");
	ВидОперации = НСтр("ru='Отправка пакетов электронных документов.';uk='Отправка пакетов электронных документов.'");
...
Процедура ПослеОтправкиПакетаЧерезВК(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Исключение
		...
		ВидОперации = НСтр("ru='Отправка документа в банк';uk='Отправка документа в банк'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура ОбработатьОшибкуПолученияСпискаПакетов(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ru='Ошибка получения списка пакетов документов из банка.';uk='Ошибка получения списка пакетов документов из банка.'");
	ВидОперации = НСтр("ru='Получение списка пакетов.';uk='Получение списка пакетов.'");
...
Процедура СохранитьПакетПослеПолученияЧерезВК(Результат, ДополнительныеПараметры) Экспорт
	...
	ДанныеВозврата.Вставить("ПараметрОповещения", Новый Соответствие);
	...
	Оповестить("ОбновитьСостояниеЭД", ДанныеВозврата.ПараметрОповещения);

	Если ДанныеВозврата.Свойство("ЕстьОшибка") Тогда
		СтруктураВозврата = Новый Структура;
...
Процедура ОбработатьОшибкуПолученияПакетаВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ru='Ошибка получения пакета документов из банка.';uk='Ошибка получения пакета документов из банка.'");
	ВидОперации = НСтр("ru='Получение пакета из банка.';uk='Получение пакета из банка.'");
...
Процедура ОбработатьОшибкуПроверкиПодписиЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ru='При проверке подписи документа произошла ошибка.';uk='При проверке подписи документа произошла ошибка.'");
	ВидОперации = НСтр("ru='Проверка подписи электронного документа.';uk='Проверка подписи электронного документа.'");
	
	Оповещение = Новый ОписаниеОповещения("ПроверитьОчереднуюПодписьСообщенияОбменаЧерезВК", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ОбработатьОшибкуПодготовкиПодписиЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ru='При подготовки подписи электронного документа произошла ошибка.';uk='При подготовки подписи электронного документа произошла ошибка.'");
	ВидОперации = НСтр("ru='Подготовка подписи электронного документа.';uk='Подготовка подписи электронного документа.'");
...
Процедура ОбработатьОшибкуПодписанияЧерезВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ru='При подписании электронного документа произошла ошибка.';uk='При подписании электронного документа произошла ошибка.'");
	ВидОперации = НСтр("ru='Подписание электронного документа.';uk='Подписание электронного документа.'");
	ПолучитьИнформациюОбОшибкеВК(ДополнительныеПараметры.ОповещениеПослеПодписанияЭД,
		ДополнительныеПараметры.ПодключаемыйМодуль, ВидОперации, ТекстОшибки);
...
Процедура ОбработатьОшибкуРасширеннойАутентификации(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ru='Ошибка аутентификации по SMS.';uk='Ошибка аутентификации по SMS.'");
	ВидОперации = НСтр("ru='Отправка одноразового пароля из SMS.';uk='Отправка одноразового пароля из SMS.'");
...
Процедура ОбработатьОшибкуОтправкиПароляПоSMSВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ru='Ошибка отправки одноразового пароля по SMS.';uk='Ошибка отправки одноразового пароля по SMS.'");
	ВидОперации = НСтр("ru='Отправка одноразового пароля в SMS.';uk='Отправка одноразового пароля в SMS.'");
	ПолучитьИнформациюОбОшибкеВК(ДополнительныеПараметры.ОповещениеПослеУстановкиСоединения,
		ДополнительныеПараметры.ПодключаемыйМодуль, ВидОперации, ТекстСообщения);
...
Процедура ОбработатьОшибкуУстановкиСоединенияВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ru='При установке соединения с банком произошла ошибка.';uk='При установке соединения с банком произошла ошибка.'");
	ВидОперации = НСтр("ru='Установка соединения с банком.';uk='Установка соединения с банком.'");
...
Процедура ОбработатьОшибкуДополненияДанныхКлючаВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ru='При работе с электронным ключем произошла ошибка.';uk='При работе с электронным ключем произошла ошибка.'");
	ВидОперации = НСтр("ru='Дополнение данных ключа.';uk='Дополнение данных ключа.'");
...
Процедура ОбработатьОшибкуУстановкиПароляВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстОшибки = НСтр("ru='Ошибка установки пароля.';uk='Ошибка установки пароля.'");
	ВидОперации = НСтр("ru='Установка пароля ключа.';uk='Установка пароля ключа.'");
...
Процедура ВыбратьКлючПослеПолученияХранилищКлючейЭП(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Количество() > 1 Тогда
		...
		ЗаголовокВыбора = НСтр("ru='Выберите электронный ключ:';uk='Виберіть електронний ключ:'");
...
Процедура ОбработатьОшибкуПроверкиНеобходимостиПИНВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ВидОперации = НСтр("ru='Проверка необходимости установки ПИН-кода.';uk='Проверка необходимости установки ПИН-кода.'");
	ТекстСообщения = НСтр("ru='При проверке необходимости ПИН-кода произошла ошибка.';uk='При проверке необходимости ПИН-кода произошла ошибка.'");
...
Процедура ОбработатьОшибкуУстановкиПинВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ВидОперации = НСтр("ru='Установка PIN кода.';uk='Установка PIN кода.'");
	ТекстСообщения = НСтр("ru='При установки PIN кода произошла ошибка.';uk='При установки PIN кода произошла ошибка.'");
...
Процедура ВыбратьКлючВК(Обработчик, ПодключаемыйМодуль, ДанныеСертификатов)
	...
	ОткрытьФорму("Обработка.ОбменСБанками.Форма.ВыборСертификатаДляДобавления", ПараметрыФормы, ЭтотОбъект, ,
		ВариантОткрытияОкна.ОтдельноеОкно, , Обработчик);
...
Процедура ОбработатьОшибкуПолученияДанныхСертификатаВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ВидОперации = НСтр("ru='Получение данных ключа ЭП.';uk='Получение данных ключа ЭП.'");
	ТекстСообщения = НСтр("ru='Произошла ошибка при получении данных ключа ЭП.';uk='Произошла ошибка при получении данных ключа ЭП.'");
...
Процедура ПослеПолученияКлючейЭПВК(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если НЕ МассивДвоичныхДанныхСертификатов.Количество() Тогда
		ТекстСообщения = НСтр("ru='Отсутствуют сертификаты на банковском ключе';uk='Відсутні сертифікати на банківському ключі'");
...
Процедура ОбработатьОшибкуПолученияКлючейВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ВидОперации = НСтр("ru='Получение ключей';uk='Получение ключей'");
	ТекстСообщения = НСтр("ru='Произошла ошибка при получении ключей.';uk='Произошла ошибка при получении ключей.'");
...
Процедура ОбработатьОшибкуИнициализацииВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	ТекстСообщения = НСтр("ru='При инициализации внешнего модуля произошла ошибка.';uk='При инициализации внешнего модуля произошла ошибка.'");
	ВидОперации = НСтр("ru='Инициализация внешней компоненты.';uk='Инициализация внешней компоненты.'");
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка SMS аутентификации."
"Код ошибки: ДО-%1"
"%2'");
		ДеталиОшибки = ДополнительныеПараметры.ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ru='SMS аутентификация';uk='SMS аутентификация'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеОтправкиОдноразовогоПароля, Ложь);
...
"Код ошибки: ДО-%1"
"%2';uk='Ошибка запуска аутентификации по SMS."
"Код ошибки: ДО-%1"
"%2'");
		ДеталиОшибки = ВнешнийПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(
...
"Код ошибки: ДО-%1"
"%2';uk='При загрузке сертификата произошла ошибка."
"Код ошибки: ДО-%1"
"%2'");
		ДеталиОшибки = ДополнительныеПараметры.ПодключаемыйМодуль.ДеталиОшибки();
		ТекстСообщения = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
		Операция = НСтр("ru='Получение дополнительных данных сертификата';uk='Отримання додаткових даних сертифікату'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеПолученияСертификата, Неопределено);
