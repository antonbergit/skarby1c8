...
"расширение для работы с файлами не подключено.';uk='Не удалось открыть файл %1, т.к."
...
Процедура ОткрытьФайлПослеПолученияФайлов(ПолученныеФайлы, ВходящийКонтекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ru='Не удалось открыть файл.';uk='Не удалось открыть файл.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ОписаниеОшибки);
	КонецЕсли;
...
"Возможно файл уже открыт.';uk='Не удалось открыть файл %1."
...
"т.к. расширение для работы с файлами не подключено.';uk='Не удалось запустить приложение,"
...
Процедура СохранитьФайлы(Знач СохраняемыеФайлы, КаталогСохранения, ВыводитьСообщения) Экспорт
	...
	Если ТипЗнч(СохраняемыеФайлы) = Тип("Массив") И СохраняемыеФайлы.Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Отсутствуют файлы для сохранения';uk='Отсутствуют файлы для сохранения'"));
		Возврат;
	КонецЕсли;
...
Процедура СохранитьФайлыПослеПодключенияРасширенияРаботыСФайлами(Подключено, ВходящийКонтекст) Экспорт
	...
	Если Подключено Тогда
		...
		Если ТребуетсяПоказатьДиалогВыбораКаталога Тогда
			...
			Диалог.Заголовок = НСтр("ru='Выберите каталог для сохранения';uk='Выберите каталог для сохранения'");
...
"%1!"
"%2';uk='Не удалось выгрузить на диск файл "
"%1!"
...
"Выберите другой каталог сохранения файлов.';uk='Длина пути одного или нескольких файлов превышает 260 символов."
"Файлы не могут быть сохранены."
...
"Продолжить сохранение с заменой файлов?';uk='В выбранном каталоге присутствуют файлы с такими же именами."
...
Процедура СохранитьФайлыПослеПолученияФайлов(ПолученныеФайлы, ВходящийКонтекст) Экспорт
	...
	Если ВходящийКонтекст.ВыводитьСообщения Тогда
		...
		Если КоличествоСохраняемыхФайлов <> 0 Тогда
			...
			СообщениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выгружено файлов: %1.';uk='Выгружено файлов: %1.'"), КоличествоСохраненныхФайлов);
			...
			Для Каждого ОписаниеФайла Из ОписанияФайлов Цикл
				Если ОписаниеФайла.Размер > ВходящийКонтекст.Параметры.МаксимальныйРазмерФайла Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						СтрШаблон(НСтр("ru='Размер файла ""%1"" (%2) не соответствует требованиям: максимальный размер файла - %3.';uk='Размер файла ""%1"" (%2) не соответствует требованиям: максимальный размер файла - %3.'"),
							ОписаниеФайла.Имя,
		...
		Если ВходящийКонтекст.Параметры.Свойство("ДопустимыеТипыФайлов")
			...
			Для Каждого ОписаниеФайла Из ОписанияФайлов Цикл
				Если ДопустимыеТипыФайлов.Найти(ОписаниеФайла.Расширение) = Неопределено Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						СтрШаблон(НСтр("ru='Тип файла ""%1"" не соответствует требованиям: допустимые типы файлов - ""%2"".';uk='Тип файла ""%1"" не соответствует требованиям: допустимые типы файлов - ""%2"".'"),
...
Функция ОписаниеОшибкиНеПолученКаталогВременныхФайлов()
	...
	Возврат НСтр("ru='Не удалось получить каталог временных файлов на клиенте.';uk='Не удалось получить каталог временных файлов на клиенте.'");

КонецФункции

Функция ОписаниеОшибкиНеУдалосьСоздатьКаталог()
	
	Возврат НСтр("ru='Не удалось создать каталог на клиенте.';uk='Не удалось создать каталог на клиенте.'");
...
"Дальнейшее выполнение операции невозможно.';uk='Расширение для работы с файлами не подключено. "
...
Функция ОписаниеОшибкиНеУдалосьСоздатьОбъектДвоичныеДанные()
	...
	Возврат НСтр("ru='Не удалось создать объект для работы с двоичными данными.';uk='Не удалось создать объект для работы с двоичными данными.'");
...
Функция ОписаниеОшибкиОшибкаЧтенияФайла(ИмяФайла)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Ошибка чтения файла %1.';uk='Ошибка чтения файла %1.'"), ИмяФайла);
...
Функция ОписаниеОшибкиОшибкаСохраненияФайла(ИмяФайла)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Во время сохранения файла %1 возникла ошибка.';uk='Во время сохранения файла %1 возникла ошибка.'"), ИмяФайла);	
...
"%1"
"по причине:"
"%2';uk='Не удалось удалить файл или каталог"
"%1"
"по причине:"
"%2'");
...
Функция ОписаниеОшибкиФайлНеСуществует(ИмяФайла)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Ошибка обращения к файлу %1 - файл не существует.';uk='Ошибка обращения к файлу %1 - файл не существует.'"), ИмяФайла);	
...
"%2';uk='Ошибка при вызове ""%1"" объекта для работы с файлами."
"%2'"),
...
Функция ИмяСобытия()
	...
	Возврат НСтр("ru='Электронный документооборот с контролирующими органами. Операции с файлами';uk='Электронный документооборот с контролирующими органами. Операции с файлами'");
...
Процедура ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(ИмяМетода, Контекст, ВозможноПродолжениеБезУстановки)
	...
	ТекстСообщения = НСтр("ru='Для работы подсистемы документооборота с контролирующими органами необходимо установить расширение работы с файлами.';uk='Для работы подсистемы документооборота с контролирующими органами необходимо установить расширение работы с файлами.'");
