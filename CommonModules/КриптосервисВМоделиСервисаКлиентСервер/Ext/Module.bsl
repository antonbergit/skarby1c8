...
Функция ЗамаскироватьНомерТелефона(Знач НомерТелефона) Экспорт
	...
	Если МассивСимволов.Количество() = 12 Тогда
		...
		КонецЦикла;
	Иначе
		ВызватьИсключение(НСтр("ru='Ошибка преобразования номера телефона.';uk='Ошибка преобразования номера телефона.'"));
	КонецЕсли;
...
Функция НомерТелефона(Знач НомерТелефона) Экспорт 
	...
	Иначе
		ВызватьИсключение(НСтр("ru='Ошибка преобразования номера телефона к формату +79999999999.';uk='Ошибка преобразования номера телефона к формату +79999999999.'"));
	КонецЕсли;
