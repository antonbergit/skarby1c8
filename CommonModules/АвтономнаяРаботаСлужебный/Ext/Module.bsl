...
Функция СобытиеЖурналаРегистрацииУдалениеАвтономногоРабочегоМеста() Экспорт
	...
	Возврат НСтр("ru='Автономная работа.Удаление автономного рабочего места';uk='Автономна робота.Видалення автономного робочого місця'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура СинхронизироватьДанныеСПриложениемВИнтернете() Экспорт
	...
	Если Отказ Тогда
		ВызватьИсключение НСтр("ru='В процессе синхронизации данных с приложением в Интернете возникли ошибки (см. журнал регистрации).';uk='У процесі синхронізації даних з додатком в Інтернеті "
"виникли помилки (див. журнал реєстрації).'");
	КонецЕсли;
...
"Можливо, довідник користувачів не включений до складу плану "
"обміну автономної роботи.'");
		КонецЕсли;
		...
		Пользователь.ДополнительныеСвойства.Вставить("СозданиеАдминистратора",
			НСтр("ru='Начальная настройка автономного рабочего места.';uk='Початкове налаштування автономного робочого місця.'"));
...
"Работа с автономным рабочим место невозможна.';uk='В автономне робоче місце не були передані налаштування."
