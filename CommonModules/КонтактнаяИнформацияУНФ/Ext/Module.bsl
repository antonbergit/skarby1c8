...
//
Процедура ПриСозданииПриЧтенииНаСервере(Форма, ВладелецКИ = Неопределено, ШиринаПоляВида = 8) Экспорт
	
	Если ВладелецКИ = Неопределено Тогда
		ВладелецКИ = Форма.Объект.Ссылка;
	КонецЕсли;
	...
	ЗаполнитьТаблицуКонтактнойИнформации(Форма, ВладелецКИ);
	...
	Для Каждого ДанныеКИ Из Форма.КонтактнаяИнформация Цикл
		...
		Если ДанныеКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
			...
			ДобавитьКомандуКонтекстногоМеню(Форма,
				...
				НСтр("ru='Адрес на Яндекс.Картах';uk='Адреса на Яндекс.Картах'"),
				НСтр("ru='Показывает адрес на картах Яндекс.Карты';uk='Показує адресу на картах Яндекс.Карти'"),
				ПолеПредставление
			);
			ДобавитьКомандуКонтекстногоМеню(Форма,
				"КонтекстноеМенюКартаGoogle_" + ИндексКИ,
				БиблиотекаКартинок.GoogleMaps,
				НСтр("ru='Адрес на Google Maps';uk='Адреса на Google Maps'"),
				НСтр("ru='Показывает адрес на карте Google Maps';uk='Показує адресу на карті Google Maps'"),
				ПолеПредставление
			);
...
Процедура Контрагенты_УстановитьСвойстваВидов()
	...
	ПараметрыВида.ЗапретитьРедактированиеПользователем	= Ложь;
	ПараметрыВида.Используется							= Истина;
	ПараметрыВида.НастройкиПроверки.АдресТолькоУкраинский			= Ложь;
...
Процедура ФизическиеЛица_УстановитьСвойстваВидов()
	...
	ПараметрыВида.Используется							= Истина;
	ПараметрыВида.НастройкиПроверки.АдресТолькоУкраинский			= Истина;
	ПараметрыВида.НастройкиПроверки.ПроверятьКорректность			= Ложь;
	ПараметрыВида.НастройкиПроверки.СкрыватьНеактуальныеАдреса		= Ложь;
	...
	ПараметрыВида = УправлениеКонтактнойИнформацией.ПараметрыВидаКонтактнойИнформации("Адрес");
	ПараметрыВида.Вид = Справочники.ВидыКонтактнойИнформации.АдресФизЛицаЗаПределамиУкраины;
	ПараметрыВида.Порядок = 5;
	ПараметрыВида.МожноИзменятьСпособРедактирования		= Ложь;
	ПараметрыВида.РедактированиеТолькоВДиалоге			= Истина;
	ПараметрыВида.ОбязательноеЗаполнение				= Ложь;
...
Процедура ИнициализироватьФорму(Форма)
	...
	ГруппаЗначенийКИ.Заголовок = НСтр("ru='Значения контактной информации';uk='Значення контактної інформації'");
	...
	Команда.Заголовок = НСтр("ru='+ телефон, адрес';uk='+ телефон, адреса'");
