...
#Если ВебКлиент Тогда
	ТекстУточнения = Символы.ПС + НСтр("ru='При работе через веб, требуется браузер Internet Explorer под управлением операционной системы Windows.';uk='При роботі через веб, потрібен браузер Internet Explorer під управлінням операційної системи Windows.'");
#Иначе		
	ТекстУточнения = "";	
#КонецЕсли
...
"Технические подробности: %2';uk='Для виведення друкованих форм в форматі OpenOffice.org Writer потрібно, щоб на комп`ютері був встановлений пакет OpenOffice.org. %1"
