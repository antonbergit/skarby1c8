...
Процедура СформироватьНовыйИдентификаторВидаЦен(Идентификатор, ВидЦенНаименование, ВидЦенВладелец = "") Экспорт
	...
	Если ПустаяСтрока(Идентификатор) Тогда
		...
		Идентификатор = НСтр("ru='ИдентификаторВидаЦен';uk='ИдентификаторВидаЦен'");
...
Процедура ПроверитьФормулу(Ошибки, Формула) Экспорт
	...
	Если СтрЧислоВхождений(ТекстФормулы, ЦенообразованиеФормулыСервер.СтрокаНачалаОперанда()) <> СтрЧислоВхождений(ТекстФормулы, ЦенообразованиеФормулыСервер.СтрокаКонцаОперанда()) Тогда
		...
		ТекстОшибки = НСтр("ru='Количество открытых операндов не равно количеству закрытых.';uk='Кількість відкритих операндів не дорівнює кількості закритих.'");
	...
	Если СтрЧислоВхождений(ТекстФормулы, "(") <> СтрЧислоВхождений(ТекстФормулы, ")") Тогда
		...
		ТекстОшибки = НСтр("ru='Количество открытых скобок не равно количеству закрытых.';uk='Кількість відкритих дужок не дорівнює кількості закритих.'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Формула", ТекстОшибки, "");
...
"Проверьте правильность написания формулы.';uk='Не розпізнаний операнд %1."
"Перевірте правильність написання формули.'");
...
"Подробное описание: ';uk='При розрахунках виникли помилки. Перевірте правильність написання формули."
"Докладний опис: '") + Символы.ПС + РасчетныеДанные.ТекстОшибки;
