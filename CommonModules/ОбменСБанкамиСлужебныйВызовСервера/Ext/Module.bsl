...
Функция ПроверитьВыполнениеФоновогоЗаданияНаСервере(ФоновоеЗаданиеИдентификатор, ИдентификаторФормы) Экспорт
	...
	Исключение
		Задание.Завершено = Истина;
		Задание.Успешно   = Ложь;
		Текст = НСтр("ru='При выполнении операции произошла ошибка.';uk='При виконанні операції сталася помилка.'");
		ВидОперации = НСтр("ru='Фоновое задание на сервере';uk='Фонове завдання на сервері'");
...
"%3%4';uk='Получена ошибка из банка (%1). Код ошибки %2."
"%3%4'");
...
Функция ДвоичныеДанныеПрисоединенногоФайла(Знач СсылкаНаОбъект) Экспорт
	...
	Если НЕ МассивФайлов.Количество() Тогда
		ТекстОшибки = НСтр("ru='Не обнаружен присоединенный файл объекта: %1';uk='Не обнаружен присоединенный файл объекта: %1'");
		ТекстОшибки = СтрШаблон(ТекстОшибки, СсылкаНаОбъект);
		Операция = НСтр("ru='Получение двоичных данных присоединенного файла';uk='Получение двоичных данных присоединенного файла'");
...
"Причина отклонения: %1';uk='Запрос отклонен."
...
Функция ВерсияАктуальнойВК(ИмяВнешнегоМодуля) Экспорт
	...
	Если НЕ ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		НаименованиеЗадания = НСтр("ru='1С:ДиректБанк: Обновление внешней компоненты (%1).';uk='1С:ДиректБанк: Обновление внешней компоненты (%1).'");
...
Функция СкачатьВКНаСервере(Знач УникальныйИдентификатор, Знач URL) Экспорт
	...
	Попытка
		НазваниеЗадания = НСтр("ru='1С:ДиректБанк: Скачивание внешней компоненты с сервера банка';uk='1С:ДиректБанк: Скачивание внешней компоненты с сервера банка'");
	...
	Исключение
		Задание.Завершено = Истина;
		Задание.Успешно = Ложь;
		Текст = НСтр("ru='Не удалось скачать внешний модуль с сервера банка.';uk='Не удалось скачать внешний модуль с сервера банка.'");
		ВидОперации = НСтр("ru='Скачивание внешней компоненты с сервера банка';uk='Скачивание внешней компоненты с сервера банка'");
...
Функция ИнформацияОВКБанка(АдресВнешнейКомпоненты) Экспорт
	...
	Если Не НайденаИнформация Тогда
		Операция = НСтр("ru='Чтение информации о файле внешнего модуля.';uk='Чтение информации о файле внешнего модуля.'");
		ТекстОшибки = НСтр("ru='В архиве внешней компоненты отсутствует файл INFO.XML';uk='В архиве внешней компоненты отсутствует файл INFO.XML'");
		ТекстСообщения = НСтр("ru='При подключении внешнего модуля произошла ошибка."
"Загрузите актуальную версию внешнего модуля.';uk='При подключении внешнего модуля произошла ошибка."
...
"не найден ни один из доступных сертификатов ЭП.';uk='В настройке обмена: %1"
		...
		Если ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ЗначениеФункциональнойОпции("ИспользоватьОбменСБанками") Тогда
			...
			Если ДействующиеПараметрыОбмена.НастройкаОбменаДействует
				...
				Если Не ЗначениеЗаполнено(ТекстСостоянияЭД) Тогда
					Гиперссылка = Ложь;
					ТекстСостоянияЭД = НСтр("ru='ЭДО не начат';uk='ЭДО не начат'");
				КонецЕсли;
			Иначе
				Если ЗначениеЗаполнено(ТекстСостоянияЭД) Тогда
					ШаблонСостоянияЭД = НСтр("ru='%1 (настройка обмена недействительна)';uk='%1 (настройка обмена недействительна)'");
				...
				Иначе
					Если ДействующиеПараметрыОбмена.НастройкаОбменаДействует
						...
						ТекстСостоянияЭД = НСтр("ru='Данный вид электронного документа банк не принимает';uk='Данный вид электронного документа банк не принимает'");
					Иначе
						ТекстСостоянияЭД = НСтр("ru='Прямой обмен с банком не подключен';uk='Прямой обмен с банком не подключен'");
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		Иначе
			ТекстСостоянияЭД = НСтр("ru='Прямой обмен с банками отключен';uk='Прямой обмен с банками отключен'");
		КонецЕсли;
	КонецЕсли;
...
Функция ОбъектПривязки(СообщениеОбмена) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	СвязанныеОбъектыЭлектронноеВзаимодействие.СсылкаНаОбъект
	|ИЗ
	|	РегистрСведений.СвязанныеОбъектыЭлектронноеВзаимодействие КАК СвязанныеОбъектыЭлектронноеВзаимодействие
	|ГДЕ
	|	СвязанныеОбъектыЭлектронноеВзаимодействие.СообщениеОбмена = &СообщениеОбмена";
	Запрос.УстановитьПараметр("СообщениеОбмена", СообщениеОбмена);
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат Выборка.СсылкаНаОбъект;
...
Процедура ИзменитьСообщениеОбмена(СообщениеОбмена, Знач Реквизиты) Экспорт
	...
	Исключение
		ВидОперации = НСтр("ru='Изменение параметров сообщения';uk='Изменение параметров сообщения'");
		ТекстОшибки = НСтр("ru='Не удалось заблокировать сообщение обмена: %1"
"Причина: %2';uk='Не удалось заблокировать сообщение обмена: %1"
"Причина: %2'");
	...
	Если Реквизиты.Свойство("УдалитьЭП") Тогда
		...
		Исключение
			ВидОперации = НСтр("ru='Удаление электронных подписей';uk='Удаление электронных подписей'");
			ТекстОшибки = НСтр("ru='Не удалось заблокировать присоединенный файл: %1"
"Причина: %2';uk='Не удалось заблокировать присоединенный файл: %1"
"Причина: %2'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстОшибки = СтрШаблон(
...
"Причина: %2"
"Операция не выполнена.';uk='Не удалось удалить подписи присоединенного файла: %1"
"Причина: %2"
"Операция не выполнена.'");
			ТекстСообщения = СтрШаблон(
...
"Операция не выполнена.';uk='Не обнаружены служебные данные электронного документа."
"Операция не выполнена.'");
			ТекстОшибки = НСтр("ru='Не обнаружены служебные данные для сообщения обмена:"
"%1';uk='Не обнаружены служебные данные для сообщения обмена:"
"%1'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, СообщениеОбмена);
			ВидОперации = НСтр("ru='Получение служебных данных электронного документа';uk='Получение служебных данных электронного документа'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
	...
	СтруктураВозврата = Новый Структура;
	СтруктураВозврата.Вставить("ДанныеЭД", ДвоичныеДанныеПрисоединенногоФайла(СообщениеОбмена));
...
Процедура ДействияПослеПодписанияЭДНаСервере(МассивСообщенийОбменаКОбновлениюСтатуса) Экспорт
	...
	Для Каждого СообщениеОбмена Из МассивСообщенийОбменаКОбновлениюСтатуса Цикл
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ru='Ошибка подписи электронного документа в сообщении обмена %1';uk='Ошибка подписи электронного документа в сообщении обмена %1'");
			ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%1", СообщениеОбмена);
			Операция = НСтр("ru='Установка подписи ЭП';uk='Установка подписи ЭП'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"%1';uk='При сохранении результата проверки электронных подписей произошла ошибка:"
"%1'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, КраткоеПредставлениеОшибки);

		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВидОперации = НСтр("ru='Сохранение результата проверки электронных подписей';uk='Сохранение результата проверки электронных подписей'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"Документ не обработан, так как содержит невалидные подписи.';uk='Обработка сообщения обмена %1."
...
Процедура ДобавитьПодпись(Знач СообщениеОбмена, Знач Подпись, Знач Сертификат, Знач ДанныеПроверкиПодписи = Неопределено) Экспорт
	...
	Исключение
		...
		ШаблонСообщения = НСтр("ru='Ошибка сохранения подписи электронного документа %1';uk='Ошибка сохранения подписи электронного документа %1'");
		ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%1", ПрисоединенныйФайл);
		ВидОперации = НСтр("ru='Сохранение данных электронной подписи';uk='Сохранение данных электронной подписи'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Процедура ОбновитьСтатусыДокументовПакетаЭДО(Знач ПакетОбменСБанками, Знач НовыйСтатусПакетаЭД, Знач СтруктураПараметров, СообщенияОбмена = Неопределено) Экспорт
	...
	Исключение
		...
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ru='Смена статуса пакета ЭДО и обновление статусов ЭД';uk='Смена статуса пакета ЭДО и обновление статусов ЭД'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция ГотовыеКРаспаковкеПакетыЭД(Знач ПакетыЭД) Экспорт
	...
	Пока ВыборкаПакеты.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(ВыборкаПакеты.Ссылка) Тогда
			Операция = НСтр("ru='Распаковка пакета';uk='Распаковка пакета'");
			ТекстСообщения = НСтр("ru='Нет присоединенного файла для пакета: %1';uk='Нет присоединенного файла для пакета: %1'");
		...
		Если ВыборкаПакеты.Статус <> Перечисления.СтатусыПакетовЭД.КРаспаковке Тогда
			...
			ШаблонСообщения = НСтр("ru='Статус пакета %1 отличен от значения ""К распаковке"".';uk='Статус пакета %1 отличен от значения ""К распаковке"".'");
	...
	Для Каждого ПакетОбменСБанками Из МассивПакетовЭДО Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(ТекстОшибки) Тогда
				ОтменитьТранзакцию();
				ШаблонВидаОперации = НСтр("ru='Отправка пакета в банк по настройке: %1';uk='Отправка пакета в банк по настройке: %1'");
...
"%2.';uk='Ошибка отправки пакета по настройке: %1"
"%2.'");
			КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстСообщения = СтрШаблон(
				ШаблонСообщения, НастройкаОбмена, КраткоеПредставлениеОшибки);
			ШаблонВидаОперации = НСтр("ru='Отправка пакета по настройке: %1';uk='Отправка пакета по настройке: %1'");
			ВидОперации = СтрШаблон(ШаблонВидаОперации, НастройкаОбмена);
...
Функция СтруктураСертификата(Знач ДвоичныеДанныеСертификата) Экспорт
	...
	Исключение
		ВидОперации = НСтр("ru='Загрузка сертификата с банковского ключа';uk='Загрузка сертификата с банковского ключа'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТестСообщения = НСтр("ru='Не удалось прочитать файл сертификата, операция прервана.';uk='Не удалось прочитать файл сертификата, операция прервана.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"Проверьте настройки обмена или обратитесь к администратору.';uk='Сертификат подписи недоступен для текущего пользователя."
...
"Получите новый сертификат';uk='Закончился срок действия сертификата: %1"
...
//   Структура Или Неопределено - параметры обмена или Неопределено, если параметры не найдены
...
Функция ПрисоединенныйФайл(СообщениеОбмена) Экспорт
	...
	Иначе
		Операция = НСтр("ru='Получение присоединенного файла';uk='Получение присоединенного файла'");
		ТекстОшибки = НСтр("ru='Не обнаружен присоединенный файл сообщения %1';uk='Не обнаружен присоединенный файл сообщения %1'");
		ТекстОшибки = СтрШаблон(ТекстОшибки, СообщениеОбмена);
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция МаркерБанкаПоСертификату(АдресСервера, ИдентификаторОрганизации, ДвоичныеДанныеСертификата, ВерсияФормата = Неопределено, ПробнаяОперация = Ложь, НастройкаОбмена = Неопределено) Экспорт
	...
	ВидОперации = НСтр("ru='Аутентификация на сервере банка';uk='Аутентификация на сервере банка'");
...
"Код ошибки: %1."
"%2';uk='Ошибка аутентификации на сервере банка."
"Код ошибки: %1."
"%2'");
...
"Проверьте настройки обмена с сервисом 1С:ДиректБанк или обратитесь в техподдержку банка.';uk='Не удалось пройти процесс аутентификации. Получен некорректный ответ банка."
...
"Полученный файл: %1"
"Подробная информация об ошибке: %2';uk='Не удалось прочитать файл, полученный из банка."
"Полученный файл: %1"
"Подробная информация об ошибке: %2'");
	...
	Если НЕ ResultBank.Error = Неопределено Тогда
		Если Не ПробнаяОперация Тогда
			ТекстСообщенияОПолученнойОшибке = ТекстСообщенияОбОшибкеОтветаБанка(ResultBank.Error);
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
//         ** Значение - Массив - подписи электронного документа:
//               *** ДвоичныеДанные - двоичные данные электронной подписи.
//     * МассивОповещений - Массив - используется только в зарплатном проекте.
//
Процедура РаспаковатьПакетОбменСБанками(ПакетОбменСБанками, ДанныеВозврата) Экспорт
	
	НастройкаОбмена = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПакетОбменСБанками, "НастройкаОбмена");
	
	ДвоичныеДанныеФайла = ДвоичныеДанныеПрисоединенногоФайла(ПакетОбменСБанками);
...
"%2';uk='Возникла ошибка при разборе пакета %1."
"%2'");
			КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстСообщения = СтрШаблон(
				ШаблонСообщения, ПакетОбменСБанками, КраткоеПредставлениеОшибки);
		Иначе
			ТекстСообщения = НСтр("ru='Файл, полученный из банка невозможно прочитать так как он содержит ошибку';uk='Файл, полученный из банка невозможно прочитать так как он содержит ошибку'");
		КонецЕсли;
		ПодробнаяИнформация = НСтр("ru='Ошибка при распаковке пакета %1"
"Подробное представление ошибки:"
"%2"
"Файл пакета: %3';uk='Ошибка при распаковке пакета %1"
"Подробное представление ошибки:"
"%2"
"Файл пакета: %3'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		...
		ВидОперации = НСтр("ru='Чтение ЭД.';uk='Чтение ЭД.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
"Подробное представление ошибки:"
"%2"
"Файл с ошибкой: %3';uk='Ошибка при распаковке пакета %1"
"Подробное представление ошибки:"
"%2"
"Файл с ошибкой: %3'");
					ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			...
			Если Document.dockind = "02" Тогда // Извещение о состоянии электронного документа
				...
				Попытка
					...
					Фабрика = ОбменСБанкамиСлужебный.ФабрикаAsyncXDTO(StatusDocNotice.formatVersion);
					Если Фабрика = Неопределено Тогда
						ВызватьИсключение НСтр("ru='Неизвестная версия формата файла.';uk='Неизвестная версия формата файла.'");
					КонецЕсли;
					ИзвещениеОСостоянииТип = ОбменСБанкамиСлужебный.ТипЗначенияCML(Фабрика, ПространствоИмен, "StatusDocNotice");
					ОбъектXML.ОткрытьФайл(ФайлВложение);
...
"Файл пакета: %3'");
				...
				Если ЗначениеЗаполнено(СообщениеЭДО) Тогда
					...
					НазваниеДокумента = НСтр("ru='Извещение о состоянии электронного документа';uk='Извещение о состоянии электронного документа'");
					...
					Если Не ОбменСБанкамиСлужебный.СохранитьСообщениеОбмена(СтруктураРеквизитов, НовоеСообщениеОбмена) Тогда 
						ДанныеВозврата.Вставить("ЕстьОшибка", Истина);
						Продолжить;
					КонецЕсли;
					...
					Если ЭтоОтветНаЗапросСостояния И ЗначениеЗаполнено(СообщениеЗапрос) Тогда
						...
						ИзменитьСообщениеОбмена(СообщениеЗапрос, РеквизитыЗапроса);
					КонецЕсли;
					...
					Если НЕ StatusDocNotice.Result.Status = Неопределено Тогда
						...
						Если ЗначениеЗаполнено(НовыйСтатус) И РеквизитыРодителя.ДатаИзмененияВнешнегоСтатуса < ДатаИзмененияСтатуса Тогда
							...
							ИзменитьСообщениеОбмена(СообщениеЭДО, РеквизитыДляИзменения);
						КонецЕсли;
					ИначеЕсли НЕ StatusDocNotice.Result.Error = Неопределено Тогда
						...
						КонецЕсли;
					КонецЕсли;
					ДокументУчета = ДокументУчета(СообщениеЭДО);
					...
					КонецЕсли;
				Иначе
					ВидОперации = НСтр("ru='Чтение извещения о состоянии электронного документа';uk='Чтение извещения о состоянии электронного документа'");
					ПодробнаяИнформация = НСтр("ru='Ошибка при распаковке пакета %1"
"Не найден электронный документ по идентификатору: %2';uk='Ошибка при распаковке пакета %1"
"Не найден электронный документ по идентификатору: %2'");
			...
			ИначеЕсли Document.dockind = "01" Тогда // Извещение о состоянии пакета
				...
				Попытка
					...
					Фабрика = ОбменСБанкамиСлужебный.ФабрикаAsyncXDTO(StatusPacketNotice.formatVersion);
					Если Фабрика = Неопределено Тогда
						ВызватьИсключение НСтр("ru='Неизвестная версия формата файла.';uk='Неизвестная версия формата файла.'");
					КонецЕсли;
					ИзвещениеОСостоянииПакетаТип = ОбменСБанкамиСлужебный.ТипЗначенияCML(
						Фабрика, ПространствоИмен, "StatusPacketNotice");
					ОбъектXML.ОткрытьФайл(ФайлВложение);
...
"Файл с ошибкой: %3'");
				...
				Если НЕ StatusPacketNotice.Result.Error = Неопределено Тогда
					...
					Если ЗначениеЗаполнено(ОтправленныйПакетЭД) Тогда
						...
						СообщенияОбмена = Новый Массив;
						ОбновитьСтатусыДокументовПакетаЭДО(
						...
						Для Каждого СообщениеОбмена Из СообщенияОбмена Цикл
							ДокументУчета = ДокументУчета(СообщениеОбмена);
							Если ЗначениеЗаполнено(ДокументУчета) Тогда
			...
			ИначеЕсли Document.dockind = "15" Тогда // Выписка банка
				...
				Попытка
					...
					Фабрика = ОбменСБанкамиСлужебный.ФабрикаAsyncXDTO(Statement.formatVersion);
					Если Фабрика = Неопределено Тогда
						ВызватьИсключение НСтр("ru='Неизвестная версия формата файла.';uk='Неизвестная версия формата файла.'");
					КонецЕсли;
					ВыпискаТип = ОбменСБанкамиСлужебный.ТипЗначенияCML(Фабрика, ПространствоИмен, "Statement");
					ОбъектXML.ОткрытьФайл(ФайлВложение);
...
"Файл с ошибкой: %3'");
				...
				Если ЗначениеЗаполнено(ИдентификаторЗапроса) Тогда
					...
					Если НЕ ЗначениеЗаполнено(СообщениеЗапрос) Тогда
						ВидОперации = НСтр("ru='Распаковка пакета';uk='Распаковка пакета'");
						ПодробнаяИнформация = НСтр("ru='Ошибка при распаковке пакета %1"
"Не найден  запрос выписки по идентификатору: %2';uk='Ошибка при распаковке пакета %1"
"Не найден  запрос выписки по идентификатору: %2'");
				...
				Если ЗначениеЗаполнено(СообщениеЗапрос) Тогда
					...
					ИзменитьСообщениеОбмена(СообщениеЗапрос, СтруктураПараметров);
				КонецЕсли;
				...
				Если ЗначениеЗаполнено(СообщениеЗапрос) Тогда
					...
					НазваниеДокумента = СтрЗаменить(
						НазваниеЗапроса, НСтр("ru='Запрос выписки';uk='Запрос выписки'"), НСтр("ru='Выписка банка за период';uk='Выписка банка за период'"));
				Иначе
					НазваниеДокумента = НСтр("ru='Выписка банка от %1';uk='Выписка банка от %1'");
			...
			ИначеЕсли Document.dockind = "06" Тогда // настройки обмена
				...
				ОбменСБанкамиСлужебный.СоздатьНастройкуОбменаИзФайла(АдресХранилища);
			ИначеЕсли Document.dockind = "20" Тогда // Подтверждение открытия счетов по зарплатному проекту
				...
				НазваниеДокумента = НСтр("ru='Подтверждение открытия счетов по зарплатному проекту';uk='Подтверждение открытия счетов по зарплатному проекту'");
				...
				Если ЗначениеЗаполнено(ИдентификаторЭДВладельца) Тогда
					...
					Если ЗначениеЗаполнено(СообщениеРодитель) Тогда
						...
						ДокументУчета = ДокументУчета(СообщениеРодитель);
						Если ЗначениеЗаполнено(ДокументУчета) Тогда
				...
				Если НЕ ДанныеВозврата.Свойство("МассивОповещений") Тогда
					МассивОповещений = Новый Массив;
					ДанныеВозврата.Вставить("МассивОповещений", МассивОповещений);
				КонецЕсли;
			...
			ИначеЕсли Document.dockind = "22" Тогда // Подтверждение зачисления денежных средств на счета сотрудников
				...
				НазваниеДокумента = НСтр("ru='Подтверждение зачисления денежных средств на счета сотрудников';uk='Подтверждение зачисления денежных средств на счета сотрудников'");
...
"Код ошибки: %1."
"%2';uk='Ошибка отправки данных аутентификации на сервер банка."
"Код ошибки: %1."
"%2'");
			ТекстОшибки = СтрШаблон(Шаблон, Результат.КодСостояния, Результат.СообщениеОбОшибке);
		Иначе
			ТекстОшибки = Результат.СообщениеОбОшибке;
		КонецЕсли;
		ОписаниеОперации = НСтр("ru='Аутентификация на сервере банка';uk='Аутентификация на сервере банка'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
	...
	Попытка
		...
		Если НЕ ResultBank.Success = Неопределено Тогда
			...
			Иначе
				ТекстСообщения = НСтр("ru='Получен некорректный ответ банковского сервиса';uk='Получен некорректный ответ банковского сервиса'");
				ТекстОшибки = НСтр("ru='Ответ банка содержит некорректные данные."
"Файл ответа банка: %1';uk='Ответ банка содержит некорректные данные."
"Файл ответа банка: %1'");
		...
		ЭлектронноеВзаимодействиеСлужебный.УдалитьВременныеФайлы(ИмяФайлаРезультата);
	Исключение
		ТекстСообщения = НСтр("ru='Ошибка чтения ответа банка.';uk='Ошибка чтения ответа банка.'");
		ОписаниеОперации = НСтр("ru='Аутентификация на сервере банка';uk='Аутентификация на сервере банка'");
		ШаблонОшибки = НСтр("ru='При чтении ответа банка произошла ошибка."
"Файл ответа: %1"
"Ошибка: %2';uk='При чтении ответа банка произошла ошибка."
"Файл ответа: %1"
"Ошибка: %2'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура ПолучитьЭДИзБанкаАсинхронныйОбмен(Знач НастройкаОбмена, Знач Параметры, ДанныеВозврата) Экспорт
	...
	Иначе
		Шаблон = НСтр("ru='Ошибка аутентификации на сервисе по адресу (%1)';uk='Ошибка аутентификации на сервисе по адресу (%1)'");
...
"Код ошибки: %1."
"%2';uk='При получении списка новых документов из банка произошла ошибка."
"Код ошибки: %1."
"%2'");
				ТекстОшибки = СтрШаблон(Шаблон, Результат.КодСостояния, Результат.СообщениеОбОшибке);
			КонецЕсли;
		Иначе
			ТекстОшибки = Результат.СообщениеОбОшибке;
		КонецЕсли;
		ВидОперации = НСтр("ru='Получение списка новых пакетов из банка.';uk='Получение списка новых пакетов из банка.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
	...
	Если МассивИдентификаторов.Количество() = 0 Тогда
		ДанныеВозврата.КолПолученныхПакетов = 0;
		Возврат;
	КонецЕсли;
...
"Код ошибки: %1."
"%2';uk='При получении нового документа из банка произошла ошибка."
"Код ошибки: %1."
"%2'");
				ТекстОшибки = СтрШаблон(Шаблон, Результат.КодСостояния, Результат.СообщениеОбОшибке);
			Иначе
...
"Код ошибки: %1."
"%2';uk='Ошибка отправки одноразового пароля."
"Код ошибки: %1."
"%2'");
			ТекстОшибки = СтрШаблон(Шаблон, Результат.КодСостояния, Результат.СообщениеОбОшибке);
		Иначе
...
Процедура СформироватьЗапросЗонд(Знач НастройкаОбмена, Знач МассивОтпечатковСертификатов, НовоеСообщениеОбмена, НастройкиОбмена) Экспорт
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ru='Формирование ЭД';uk='Формирование ЭД'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
	...
	НазваниеДокумента = НСтр("ru='Запрос-зонд';uk='Запрос-зонд'");
...
Процедура ОбработатьОтветБанкаПослеОтправкиДокументовЧерезВнешнююОбработку(Знач ПакетыЭДО, Знач ДанныеОтвета, ОтправленныеДокументы) Экспорт
	...
	Для Каждого Ответ Из ДанныеОтвета Цикл
		...
		Если Не ЗначениеЗаполнено(СообщениеОбмена) Тогда
			ТекстСообщения = НСтр("ru='Получены некорректные данные из банка.';uk='Получены некорректные данные из банка.'");
			ТекстОшибки = НСтр("ru='Не найдено исходное сообщение обмена по идентификатору: %1.';uk='Не найдено исходное сообщение обмена по идентификатору: %1.'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, Ответ.Ключ);
			ВидОперации = НСтр("ru='Чтение данных ответа банка';uk='Чтение данных ответа банка'");
...
Процедура СохранитьСхемыДанных(Знач НастройкаОбмена, Знач МассивСообщений, Знач МассивСхемДанных) Экспорт
	...
	Для Индекс = 0 По КоличествоСообщений - 1 Цикл
		...
		СтруктураРеквизитов.Вставить("НастройкаОбмена", НастройкаОбмена);
		СтруктураРеквизитов.Вставить("Направление", Перечисления.НаправленияЭД.Входящий);
		СтруктураРеквизитов.Вставить("Представление", НСтр("ru='Схема данных';uk='Схема данных'"));
...
Процедура ОбработатьИзвещениеСОшибкойСбербанк(Знач Ответ, Знач ИдентификаторСобытия, ТекстСообщения) Экспорт
	...
	Исключение
		ЧтениеXML.Закрыть();
		ТекстСообщения = НСтр("ru='Возникла ошибка при чтении полученных данных из банка';uk='Возникла ошибка при чтении полученных данных из банка'");
		ТекстОшибки = НСтр("ru='Возникла ошибка при чтении полученных данных из банка"
"Идентификатор события: %1"
"Текст ошибки: %2';uk='Возникла ошибка при чтении полученных данных из банка"
"Идентификатор события: %1"
"Текст ошибки: %2'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтрШаблон(
			ТекстОшибки, ИдентификаторСобытия, ПодробноеПредставлениеОшибки);
		ВидОперации = НСтр("ru='Чтение полученных данных из банка';uk='Чтение полученных данных из банка'");
...
"Код: %1."
"Тип: %2"
"Описание: %3';uk='Получена ошибка из банка."
"Код: %1."
"Тип: %2"
"Описание: %3'");
	Если ТипЗнч(ЭД.Errors.Error) = Тип("СписокXDTO") Тогда
		Для Каждого Error Из ЭД.Errors.Error Цикл
			...
			ВидОперации = НСтр("ru='Отправка данных в банк';uk='Отправка данных в банк'");
...
Процедура УдалитьИдентификаторЗапроса(Знач НастройкаОбмена, Знач Идентификатор) Экспорт
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ru='Удаление идентификатора запроса банка';uk='Удаление идентификатора запроса банка'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1, НастройкаОбмена);
	КонецПопытки;
...
Функция ТекстЗапросаНочнойВыписки(НастройкаОбмена, ИдентификаторЗапроса, ИдентификаторОрганизации, ЭП, СертификатПодписи) Экспорт
	...
	Попытка
		...
		Если ДанныеСертификата = Неопределено Тогда
			ВызватьИсключение НСтр("ru='Не найден сертификат установленной подписи';uk='Не найден сертификат установленной подписи'");
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			ВызватьИсключение НСтр("ru='Ошибка формирования запроса ночной выписки';uk='Ошибка формирования запроса ночной выписки'");
		КонецЕсли;
...
"Идентификатор события: %1';uk='Возникла ошибка при чтении полученных данных из банка"
"Идентификатор события: %1'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, ИдентификаторСобытия);
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			НСтр("ru='Чтение полученных данных из банка';uk='Чтение полученных данных из банка'"), ТекстОшибки, ТекстСообщения, 1, НастройкаОбмена);
		ЕстьОшибка = Истина;
		Возврат;
	КонецПопытки;
...
Функция ПакетXMLСбербанка(Знач СообщениеОбмена, Знач НастройкаОбмена, ИдентификаторЗапроса, ИдентификаторОрганизации) Экспорт
	...
	Попытка
		...
		ИначеЕсли ЭД.Тип() = ОбменСБанкамиСлужебный.ТипЗначенияCML(ФабрикаXDTO, ПространствоИмен, "StmtReqType") Тогда
			...
			Request.Проверить();
		Иначе
			ВызватьИсключение НСтр("ru='Неизвестный формат файла';uk='Неизвестный формат файла'");
		КонецЕсли;
		ОбъектXML.Закрыть();
	...
	Исключение
		ОбъектXML.Закрыть();
		ТекстОшибки = НСтр("ru='Возникла ошибка при формировании пакета данных банка';uk='Возникла ошибка при формировании пакета данных банка'");
...
Функция МассивЗапросовСостоянийОбработкиДокументов(НастройкаОбмена, ВидЭД) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			Операция = НСтр("ru='Формирование запроса статуса электронного документа';uk='Формирование запроса статуса электронного документа'");
			ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция ЗапросВыпискиСбербанк(Знач НастройкаОбмена, Знач ДатаНачала, Знач ДатаОкончания, Знач НомерСчета, ФормироватьПринудительно = Ложь, ГотовыеВыписки = Неопределено) Экспорт
	...
	СформироватьЗапросВыпискиСбербанк(
		НастройкаОбмена, ДатаНачала, ДатаОкончания, МассивБанковскихСчетов, ИдентификаторЗапроса, ФайлЗапроса, ЕстьОшибка);
	...
	ИмяФайла = НСтр("ru='Запрос выписки за период с %1 по %2';uk='Запрос выписки за период с %1 по %2'");
	ИмяФайла = СтрШаблон(
		...
		Если ПЭД = Неопределено Тогда
			...
			Если Выборка.ПрограммаБанка = Перечисления.ПрограммыБанка.ОбменЧерезДопОбработку Тогда
				...
				Если Не ЗначениеЗаполнено(СлужебноеСообщениеБанка) Тогда
					ТекстСообщения = НСтр("ru='Невозможно сформировать пакет банка';uk='Невозможно сформировать пакет банка'");
					ВидОперации = НСтр("ru='Формирование пакета ЭД';uk='Формирование пакета ЭД'");
					ТекстОшибки = НСтр("ru='Не найдены дополнительные данные для электронного документа';uk='Не найдены дополнительные данные для электронного документа'");
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция ДанныеДляАутентификацииПоСертификату(ДанныеСертификата, ВерсияФормата)
	...
	Операция = НСтр("ru='Формирование электронного документа.';uk='Формирование электронного документа.'");
...
Функция МассивЗапросовВыписки(НастройкаОбмена, Организация, Банк, ДатаНачала, ДатаОкончания, МассивБанковскихСчетов)
	...
	ШаблонСообщения = НСтр("ru='Запрос выписки с %1 по %2';uk='Запрос выписки с %1 по %2'");
...
Функция МассивЗапросовВыпискиAsync(НастройкаОбмена, ИдентификаторОрганизации, ДатаНачала, ДатаОкончания, МассивБанковскихСчетов)
	...
	Для Каждого НомерСчета Из МассивБанковскихСчетов Цикл
		...
		Шаблон = НСтр("ru='Запрос выписки с %1 по %2';uk='Запрос выписки с %1 по %2'");
...
Функция ДанныеДляОтправкиВБанк(Ссылки, ПрограммаБанка)
	...
	Пока ВыборкаНастройкиОбмена.Следующий() Цикл
		...
		Иначе
			ШаблонСообщения = НСтр("ru='Не указаны сертификаты в настройке обмена с банком %1';uk='Не указаны сертификаты в настройке обмена с банком %1'");
			ТекстСообщения = СтрШаблон(
...
Процедура СохранитьВыпискуСбербанка(ЭД, НастройкаОбмена, МассивВыписок, СообщениеВладелец = Неопределено)
	...
	Для Каждого ВыпискаXDTO Из ЭД.Statements.Statement Цикл
		...
		ПредставлениеДокумента = НСтр("ru='Выписка банка за период с';uk='Выписка банка за период с'") + " " + Формат(ДатаНачала, "ДЛФ=D")
			+ " " + НСтр("ru='по';uk='по'") + " " + Формат(ДатаОкончания, "ДЛФ=D");
	...
	Для Каждого Тикет Из ЭД.Tickets.Ticket Цикл
		...
		Если Выборка.Следующий() Тогда
			...
			Если НЕ ЗначениеЗаполнено(Выборка.Квитанция) Тогда
				НазваниеЭД = НСтр("ru='Квитанция от';uk='Квитанция от'")+ " " + Формат(ТекущаяДатаСеанса(), "ДЛФ=DDT");
			...
			Иначе
				...
				Представление = НСтр("ru='Квитанция от';uk='Квитанция от'") + " " + Формат(ТекущаяДатаСеанса(), "ДЛФ=DDT");
...
Процедура СохранитьШтампыБанка(ВыпискаБанка)
	...
	ДвоичныеДанныеЭД = ДвоичныеДанныеПрисоединенногоФайла(ВыпискаБанка);
	...
	Для Каждого СтрокаТЧ Из СтрокиТЧ Цикл
		...
		Если ЗначениеЗаполнено(ДатаСписано) И ЗначениеЗаполнено(ШтампБанкаСтатус) И ЗначениеЗаполнено(ИдПлатежа) Тогда
			...
			Если ЗначениеЗаполнено(ПлатежныйДокумент) Тогда
				...
				ИзменитьСообщениеОбмена(ПлатежныйДокумент, СтруктураИзменения);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла
...
//
Процедура СформироватьЗапросВыпискиСбербанк(НастройкаОбмена, ДатаНачала, ДатаОкончания, МассивСчетов, ИдентификаторЗапроса, ФайлЗапроса, ЕстьОшибка)
...
"%1.';uk='При формировании электронного документа произошла ошибка."
"%1.'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
	...
	Исключение
		ТекстСообщения = НСтр("ru='При формировании электронного документа произошла ошибка."
"Обратитесь к администратору.';uk='При формировании электронного документа произошла ошибка."
...
Процедура УдалитьНедоступныеДляФормированияЭДОбъекты(МассивСсылок)
	...
	Пока Результат.Следующий() Цикл
		...
		ШаблонСообщения = НСтр("ru='Для документа %1 уже есть актуальный электронный документ.';uk='Для документа %1 уже есть актуальный электронный документ.'");
...
"Не требуется выполнения действий с электронным документом.';uk='Обработка %1."
...
Процедура УстановитьСтатусПакета(Пакет, СтатусПакета)
...
Функция ОтправитьЗапросЗондНаСервере(Знач ПараметрыЗапроса, АдресХранилища, УникальныйИдентификатор, ИдентификаторЗадания, МассивСообщений) Экспорт
	...
	НаименованиеЗадания = НСтр("ru='Отправка тестового запроса в банк';uk='Отправка тестового запроса в банк'");
...
Функция ПолучитьИзвещениеОСостоянииЭДАсинхронноНаСервере(Знач ПараметрыЗапроса, АдресХранилища, УникальныйИдентификатор, ИдентификаторЗадания, МассивСообщений = Неопределено) Экспорт
	...
	НаименованиеЗадания = НСтр("ru='Получение извещения о состоянии электронного документа';uk='Получение извещения о состоянии электронного документа'");
	ПараметрыВыполнения = Новый Массив;
	ПараметрыВыполнения.Добавить(ПараметрыЗапроса);
	ПараметрыВыполнения.Добавить(АдресХранилища);
...
Процедура ПодключитьВнешнююОбработку(Знач АдресФайла, ИмяОбъекта) Экспорт
	...
	Исключение
		ВидОперации = НСтр("ru='Подключение внешней обработки';uk='Подключение внешней обработки'");
		ТекстСообщения = НСтр("ru='При подключении внешнего модуля произошла ошибка';uk='При подключении внешнего модуля произошла ошибка'");
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ТекстОшибки, ТекстСообщения, 1);
	КонецПопытки;
...
Функция ПодключитьАктуальнуюВнешнююОбработку(Знач ИмяВнешнегоМодуля, АдресФайла = "") Экспорт
	...
	#Иначе
		Попытка
			ВнешниеОбработки.Подключить(АдресФайла, , Ложь);
			ОбработкаПодключена = Истина;
		Исключение
			ВидОперации = НСтр("ru='Подключение внешней обработки';uk='Подключение внешней обработки'");
			ТекстСообщения = НСтр("ru='При подключении внешнего модуля произошла ошибка';uk='При подключении внешнего модуля произошла ошибка'");
...
Функция СведенияОВнешнейОбработке(ИмяВнешнегоМодуля) Экспорт
	...
	Попытка
		...
		Возврат СтруктураВозврата;
	Исключение
		Операция = НСтр("ru='Создание объекта внешней обработки.';uk='Создание объекта внешней обработки.'");
		ТекстСообщения = НСтр("ru='Не удалось подключить указанный внешний модуль';uk='Не вдалося підключити вказаний зовнішній модуль'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
