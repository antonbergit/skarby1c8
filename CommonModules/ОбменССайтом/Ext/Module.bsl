...
Функция ВыполнитьТестовоеПодключениеКСайту(НастройкиПодключения, ТекстСообщения) Экспорт
	...
	//ПараметрыПодключения.Вставить("Пользователь", НастройкиПодключения.ИмяПользователя);
	...
	Если Соединение = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ru='Ошибка при установке соединения с сайтом.';uk='Помилка при установці з`єднання з сайтом.'") + Символы.ПС + ОписаниеОшибки;
		Возврат Ложь;
	...
	Если Успешно Тогда
		...
		ТекстСообщения = НСтр("ru='Соединение с сайтом успешно установлено.';uk='З`єднання з сайтом успішно встановлене.'");
...
Процедура ЗаданиеВыполнитьОбмен(КодУзлаОбмена) Экспорт
	...
	Если УзелОбмена.ПометкаУдаления Тогда
		...
		ЗаписьЖурналаРегистрации("Обмен с сайтами",
			...
			УзелОбмена.Метаданные(),
			УзелОбмена,
			НСтр("ru='Настройка обмена помечена на удаление. Обмен отменен.';uk='Налаштування обміну позначене на видалення. Обмін скасований.'"));
	...
	Если Не ВыполнитьТестовоеПодключениеКСайту(УзелОбмена, ТекстСообщения) Тогда
		...
		ЗаписьЖурналаРегистрации("Обмен с сайтами",
			...
			УзелОбмена.Метаданные(),
			УзелОбмена,
			ТекстСообщения + НСтр("ru=' Обмен отменен.';uk=' Обмін скасовано.'"));
...
Функция ВыгрузитьКаталогПредложенияНаСайт(Параметры, СтрокаТаблицыИнформации)
	...
	Если Успешно Тогда 
		...
		СтрокаТаблицыИнформации.Описание = 
			...
			+ ТекущаяДата() + НСтр("ru=' Товары успешно выгружены на сайт.';uk=' Товари успішно вивантажені на сайт.'")
			+ ?(ПустаяСтрока(ОписаниеОшибки), "", Символы.ПС + НСтр("ru='Дополнительная информация о выгрузке:';uk='Додаткова інформація про вивантаження:'") + Символы.ПС + ОписаниеОшибки);
		
	Иначе
		
		СтрокаТаблицыИнформации.Описание = 
			СтрокаТаблицыИнформации.Описание + Символы.ПС
			+ ТекущаяДата() + НСтр("ru=' Выгрузка на сайт завершилась с ошибками.';uk=' Вивантаження на сайт завершилося з помилками.'") + Символы.ПС + ОписаниеОшибки;
...
Процедура ПодготовитьДанныеДляВыгрузки(Параметры)
	...
	Параметры.Вставить("ВыборкаЦен", МассивРезультатовЗапроса[10].Выбрать());
	Параметры.Вставить("ВыборкаСкладов", МассивРезультатовЗапроса[11].Выбрать());
	
	ТаблицаОстатковПоСкладам = МассивРезультатовЗапроса[12].Выгрузить();
	...
	Параметры.Вставить("ВыборкаВидовЦен", МассивРезультатовЗапроса[14].Выбрать());
	
	Параметры.Вставить("ДеревоСвойствХарактеристик", 
		МассивРезультатовЗапроса[15].Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкам));
	
	Параметры.Вставить("ВыборкаСвойствНоменклатурыДляКлассификатора", 
		МассивРезультатовЗапроса[20].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам));
	
	РезультатЗапросаСвойствНоменклатуры = МассивРезультатовЗапроса[21];
	...
	РезультатЗапросаДанныхОрганизацииВладельцаКаталога = МассивРезультатовЗапроса[24];
	...
	Параметры.Вставить("ВыборкаНоменклатуры", МассивРезультатовЗапроса[25].Выбрать());
	Параметры.Вставить("ДеревоГрупп", МассивРезультатовЗапроса[27].Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкамСИерархией));
	
	Если Параметры.ВыгружатьКартинки Тогда
		
		РезультатЗапросаФайлов = МассивРезультатовЗапроса[28];
...
Функция ВыгрузитьКлассификаторИКаталог(Параметры, ИмяФайлаКаталога, ПакетCML)
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(СтруктураРезультата.ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось записать XML-файл классификатора товаров на диск: ';uk='Не вдалося записати XML-файл класифікатора товарів на диск:'")
...
Функция ПодготовитьДанныеXDTOКлассификаторИКаталог(Параметры, ПакетCML)
	...
	ДобавитьГруппыКлассификатораXDTO(КлассификаторXDTO, Параметры.ДеревоГрупп.Строки, Параметры.СтрокаТаблицыКаталогов.Группы, ПакетCML);
	
	ДобавитьСвойстваНоменклатурыВКлассификаторXDTO(КлассификаторXDTO, ПакетCML, Параметры.ВыборкаСвойствНоменклатурыДляКлассификатора);
	...
	//Возврат Новый Структура("ФабрикаXDTO, КоммерческаяИнформацияXDTO", ФабрикаXDTO, КоммерческаяИнформацияXDTO);
...
Процедура ДобавитьГруппыКлассификатораXDTO(КлассификаторXDTO, ДеревоГрупп, СписокГрупп, ПакетCML) Экспорт
	...
	ЗаполнитьГруппыКлассификатораXDTO(ГруппыКлассификатораXDTO, ДеревоГрупп, СписокГрупп, ПакетCML);
...
//
Функция ПолучитьГруппыНоменклатурыXDTO(ДеревоГрупп, СписокГрупп, ПакетCML, ТекущийРодитель = Неопределено)
	...
	Для Каждого СтрокаДерева Из ДеревоГрупп Цикл
		...
		Если СтрокаДерева.Строки.Количество() > 0 Тогда
			...
			ГруппыОбъектXDTO = ПолучитьГруппыНоменклатурыXDTO(СтрокаДерева.Строки, СписокГрупп, ПакетCML, СтрокаДерева.Номенклатура);
...
Функция ВыгрузитьФайл(ДанныеФайла, Параметры, ОписаниеОшибки) Экспорт
		...
		Если ДвоичныеДанныеФайла = Неопределено Тогда
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки, 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось получить данные файла %1 номенклатуры %2';uk='Не вдалося одержати дані файлу %1 номенклатури %2'"),
					ДанныеФайла.Файл,
	...
	Иначе
		...
		Исключение
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки, 
				ОписаниеИсключительнойОшибки(НСтр("ru='Выгрузка файла номенклатуры: ';uk='Вивантаження файлу номенклатури:'")
					+ Параметры.ВыборкаНоменклатуры.Номенклатура));
...
Функция ВыгрузитьПакетПредложений(Параметры, ИмяФайлаПрайса, ПакетCML)
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(СтруктураРезультата.ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось записать XML-файл пакета предложений на диск: ';uk='Не вдалося записати XML-файл пакета пропозицій на диск:'")
...
Процедура ПодготовитьИтоговуюИнформациюПоВыгрузкеТоваров(ТаблицаКаталогов, СтрокаТаблицыИнформации, ВыгруженоОбъектов)
	...
	СтрокаТаблицыИнформации.Описание = 
		СтрокаТаблицыИнформации.Описание + Символы.ПС
		+ ТекущаяДата() + НСтр("ru=' Завершено формирование файлов выгрузки товаров';uk=' Завершене формування файлів вивантаження товарів'");
	...
	СтрокаТаблицыИнформации.Описание = 
		СтрокаТаблицыИнформации.Описание + Символы.ПС
		+ НСтр("ru='Выгружено товаров: ';uk='Вивантажено товарів:'") + СтруктураРезультата.ВыгруженоТоваров + Символы.ПС
		+ НСтр("ru='предложений: ';uk='пропозицій:'") + СтруктураРезультата.ВыгруженоПредложений + Символы.ПС
		+ НСтр("ru='картинок: ';uk='картинок:'") + СтруктураРезультата.ВыгруженоКартинок + Символы.ПС;
	
	Для Каждого Каталог Из ТаблицаКаталогов Цикл
		
		СтрокаТаблицыИнформации.Описание = 
			СтрокаТаблицыИнформации.Описание + Символы.ПС
			+ НСтр("ru='В том числе для каталога ';uk='У тому числі для каталогу'") + Каталог.Каталог + ":" + Символы.ПС 
			+ НСтр("ru='товаров: ';uk='товарів:'") + Каталог.СтруктураРезультата.ВыгруженоТоваров + Символы.ПС
			+ НСтр("ru='предложений: ';uk='пропозицій:'") + Каталог.СтруктураРезультата.ВыгруженоПредложений + Символы.ПС
			+ НСтр("ru='картинок: ';uk='картинок:'") + Каталог.СтруктураРезультата.ВыгруженоКартинок + Символы.ПС;
		...
		СтрокаТаблицыИнформации.Описание = 
			СтрокаТаблицыИнформации.Описание + Символы.ПС
			+ НСтр("ru='Ошибки в процессе выгрузки каталога ';uk='Помилки в процесі вивантаження каталогу'") + Каталог.Каталог + ":"
...
Процедура ЗафиксироватьЗавершениеВыгрузкиНоменклатуры(СтрокаТаблицыИнформации, Результат)
	...
	СтрокаТаблицыИнформации.Описание = СтрокаТаблицыИнформации.Описание + Символы.ПС
		+ ДатаОкончания + " " + НСтр("ru='Завершена выгрузка товаров';uk='Завершене вивантаження товарів'");
...
Функция ЗагрузитьЗаказыССайта(Параметры, СтруктураСтатистики, ОписаниеОшибки)
	...
	Если ПустаяСтрока(СтрокаXML) Тогда
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ru='Не удалось прочитать данные, загруженные с сервера.';uk='Не вдалося прочитати дані, завантажені із сервера.'"));
		Возврат Ложь;
	...
	Если НЕ ЗагрузитьЗаказы(СтрокаXML, СтруктураСтатистики, Параметры, ОписаниеОшибки) Тогда 
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, 
			НСтр("ru='Не удалось обработать документы, загруженные с сервера.';uk='Не вдалося оновити документи, завантажені із сервера.'"));
...
Функция ЗагрузитьЗаказыИзФайла(Параметры, СтруктураСтатистики, ОписаниеОшибки)
	...
	Если НЕ Файл.Существует()
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ru='Файл загрузки не найден';uk='Файл завантаження не знайдений'") + ": " + Параметры.ФайлЗагрузки);
		Возврат Ложь;
	...
	Если МассивОтмененныхЗаказов.Количество() > 0 Тогда
		...
		Для Каждого ДокументСсылка Из МассивОтмененныхЗаказов Цикл
			...
			Исключение
				...
				ДобавитьОписаниеОшибки(ОписаниеОшибки,
					ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось отменить заказ: ';uk='Не вдалося скасувати замовлення:'")
						+ ДокументСсылка));
...
Функция СоздатьОбновитьЗаказы(ДокументыXDTO, РанееЗагруженныеЗаказы, СтруктураСтатистики, Параметры, ОписаниеОшибки)
	...
	Для каждого ДокументXDTO Из ДокументыXDTO Цикл
		...
		Если НЕ ОбъектXDTOСодержитСвойство(ДокументXDTO, "Валюта")
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ru='Ошибка в значении узла <Документ>.<Валюта> документа XML. Валюта должна быть заполнена.';uk='Помилка в значенні вузла <Документ>.<Валюта> документа XML. Валюта повинна бути заповнена.'"));
		...
		Если НайденнаСтрока <> Неопределено Тогда
			...
			Если ДокументОбъект.СостояниеЗаказа = Справочники.СостоянияЗаказовПокупателей.Завершен Тогда
				...
				ДобавитьОписаниеОшибки(ОписаниеОшибки,
					Строка(ДокументОбъект.Ссылка) + НСтр("ru=' пропущен по причине:';uk=' пропущений через:'") + Символы.ПС 
					+ НСтр("ru='Статус заказа - ""Завершен"".';uk='Статус замовлення - ""Завершений"".'"));
		...
		Если ЕстьСсылкиНаДокументыОплатыОтгрузки Тогда
			...
			Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 пропущен, т.к. существуют документы, введенные на основании заказа.';uk='%1 пропущений, тому що існують документи, уведені на підставі замовлення.'"),
		...
		Иначе
			...
			Попытка
				...
				ВыполнитьДействияПередЗаписьюЗаказа(ДокументОбъект);
				ЗаписатьДокумент(ДокументОбъект);
...
Процедура ВыполнитьДействияПередЗаписьюЗаказа(ДокументОбъект)
...
Функция ОбновитьСоздатьДополнительныеСведенияЗаказа(ЗаказПокупателя, СвойстваЗаказа, ОписаниеОшибки)
	...
	Для Каждого СвойствоЗаказа Из МассивСоздатьОбновить Цикл
		...
		Если Свойство = Неопределено Тогда
			...
			СвойствоОбъект.Комментарий = НСтр("ru='Создано автоматически при загрузке заказа с сайта';uk='Створене автоматично при завантаженні замовлення з сайту'");
...
Функция ЗаполнитьЗаказДаннымиДокументаXDTO(ДокументОбъект, ДокументXDTO, СвойстваЗаказа, Параметры, ОписаниеОшибки)
	...
	Если НЕ ИдентифицироватьКонтрагента(ДокументОбъект, ДокументXDTO, Параметры, ОписаниеОшибки) Тогда
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			НСтр("ru='Не удалось идентифицировать контрагента для заказа номер: ';uk='Не вдалося ідентифікувати контрагента для замовлення номер:'") + ДокументXDTO.Номер);
	КонецЕсли;
...
Функция ПолучитьНомерЗаказаССайта(Знач НомерНаСайте, Организация)
	...
	//ПерепрефиксацияОбъектов.ПриОпределенииФункциональнойОпцииПрефиксыОрганизаций(ФункциональнаяОпцияИспользуется);
...
// При установленном способе идентификации контрагентов "Наименование" или "ИННЕДРПОУ", если контрагент не найден, он создается.
//
// Параметры
...
Функция ИдентифицироватьКонтрагента(ДокументОбъект, ДокументXDTO, Параметры, ОписаниеОшибки)
	...
	Иначе
		...
		ЕДРПОУ = "";
		Если ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "ЕДРПОУ")
			И ТипЗнч(КонтрагентXDTO.ЕДРПОУ) = Тип("Строка") Тогда
			ЕДРПОУ = КонтрагентXDTO.ЕДРПОУ;
		КонецЕсли;
		...
		Если ПустаяСтрока(Наименование) Тогда
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки, 
				НСтр("ru='Наименование контрагента не заполнено!';uk='Найменування контрагента не заповнене!'"));
		...
		Если Параметры.СпособИдентификацииКонтрагентов = Перечисления.СпособыИдентификацииКонтрагентов.Наименование Тогда
			...
			ТекстСообщенияНайденоНесколькоКонтрагентов = НСтр("ru=' по наименованию: ';uk=' по найменуванню:'") + Наименование;
			
			Запрос.Текст = Запрос.Текст + " И Контрагенты.Наименование = &Наименование ";
			
			Запрос.УстановитьПараметр("Наименование", Наименование);
			
		ИначеЕсли Параметры.СпособИдентификацииКонтрагентов = Перечисления.СпособыИдентификацииКонтрагентов.ИННЕДРПОУ Тогда	
			
			Если ПустаяСтрока(ИНН) Тогда
				
				ДобавитьОписаниеОшибки(ОписаниеОшибки, 
					НСтр("ru='ИНН контрагента не заполнен!';uk='ІПН контрагента не заповнений!'"));
			...
			ТекстСообщенияНайденоНесколькоКонтрагентов = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru=' по ИНН: %1, ЕДРПОУ: %2';uk=' по ІПН: %1, ЄДРПОУ: %2'"),
					ИНН, ЕДРПОУ);
			
			Запрос.Текст = Запрос.Текст + " И Контрагенты.ИНН = &ИНН И Контрагенты.КодПоЕДРПОУ = &ЕДРПОУ ";
...
Процедура ЗаполнитьДанныеКонтрагентаЗаказа(КонтрагентXDTO, ИнформацияПоКонтрагенту)
	...
	Если ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "ЕДРПОУ") И СвойствоXDTOЗаполнено(КонтрагентXDTO.ЕДРПОУ) Тогда
		ДобавитьСтрокуИнформацииПоКонтрагенту("ЕДРПОУ", КонтрагентXDTO.ЕДРПОУ, ИнформацияПоКонтрагенту);
	КонецЕсли;
	...
	Если ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "РасчетныеСчета")
		...
		Для каждого РасчетныйСчетXDTO Из МассивРасчетныхСчетовXDTO Цикл
			...
			ИнформацияПоРасчетномуСчету = "Номер счета = " + НомерСчета 
				...
				+ ?(ПустаяСтрока(ДанныеБанкаXDTO.МФО), "", ", МФО = " + ДанныеБанкаXDTO.МФО)
...
Функция СоздатьКонтрагента(ДокументОбъект, КонтрагентXDTO, Параметры, ОписаниеОшибки)
	...
	Если ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "ЕДРПОУ") И СвойствоXDTOЗаполнено(КонтрагентXDTO.ЕДРПОУ) Тогда
		НовыйКонтрагент.КодПоЕДРПОУ = КонтрагентXDTO.ЕДРПОУ;
	КонецЕсли;
...
Процедура ЗаполнитьДанныеСправочниковПодчиненныхКонтрагенту(ДокументОбъект, КонтрагентОбъект, КонтрагентXDTO, ОписаниеОшибки)
	...
	Если ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "Представители") И КонтрагентXDTO.Представители <> Неопределено Тогда
		...
		Для каждого ПредставительXDTO Из МассивПредставителейXDTO Цикл 
			...
			Элемент.Комментарий = НСтр("ru='Создан автоматически при импорте заказа с сайта ';uk='Створений автоматично при імпорті замовлення з сайту'") + ТекущаяДата();
			Элемент.Записать();
	...
	Если ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "РасчетныеСчета")
		...
		Для каждого РасчетныйСчетXDTO Из МассивРасчетныхСчетовXDTO Цикл
			...
			МФО = ДанныеБанкаXDTO.МФО;
			Если НЕ СвойствоXDTOЗаполнено(МФО) Тогда
				Продолжить;
			КонецЕсли;
			...
			Запрос.УстановитьПараметр("МФО", МФО);
			...
			Если НЕ ЗначениеЗаполнено(Валюта) Тогда
				...
				ДобавитьОписаниеОшибки(ОписаниеОшибки,
					ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось определить валюту банковского счета: ';uk='Не вдалося визначити валюту банківського рахунку:'")
					+ НомерСчета));
	...
	Договор.ВидДоговораПоГК = Справочники.ВидыДоговоровПоГК.Поставка;
...
Функция НайтиСоздатьХарактеристикуНоменклатуры(ТоварXDTO, ЭтоУслуга, Номенклатура, Параметры, ОписаниеОшибки)
	...
	Попытка
		...
		Если Не ПустаяСтрока(ИдХарактеристики) Тогда
			...
			Иначе
				...
				ДобавитьОписаниеОшибки(ОписаниеОшибки,
					НСтр("ru='Характеристика номенклатуры не найдена по уникальному идентификатору: ';uk='Характеристика номенклатури не знайдена по унікальному ідентифікатору:'") + ИдХарактеристики);
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось выполнить поиск характеристики номенклатуры по уникальному идентификатору: ';uk='Не вдалося виконати пошук характеристики номенклатури по унікальному ідентифікатору:'") + ИдХарактеристики));
...
Функция НайтиСоздатьНоменклатуру(ТоварXDTO, ЭтоУслуга, Параметры, СтавкаНДС, ОписаниеОшибки)
	...
	Если ОбъектXDTOСодержитСвойство(ТоварXDTO, "Ид") И СвойствоXDTOЗаполнено(ТоварXDTO.Ид) Тогда
		...
		Попытка
			...
			Иначе
				...
				ДобавитьОписаниеОшибки(ОписаниеОшибки,
					НСтр("ru='Номенклатура не найдена по уникальному идентификатору: ';uk='Номенклатура не знайдена по унікальному ідентифікатору:'") + ИдНоменклатуры);
		...
		Исключение
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки,
				ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось выполнить поиск номенклатуры по уникальному идентификатору: ';uk='Не вдалося виконати пошук номенклатури по унікальному ідентифікатору:'") + ТоварXDTO.Ид));
...
Процедура ЗаписатьИнформациюПоЗаказамВТаблицуИнформации(ТаблицаИнформации, ДатаНачала, Действие, Успех, СтруктураСтатистики, ОписаниеОшибки);
	...
	Если Действие = Перечисления.ДействияПриОбмене.ЗагрузкаДанных Тогда
		...
		Описание = Строка(ДатаНачала) + " " + НСтр("ru='Запуск загрузки заказов';uk='Запуск завантаження замовлень'")
			+ Символы.ПС + НСтр("ru='Обработано: ';uk='Оброблене:'") + СтруктураСтатистики.ОбработаноНаЗагрузке
			+ Символы.ПС + НСтр("ru='Загружено: ';uk='Завантажене:'") + СтруктураСтатистики.Загружено.Количество();
		
		ВывестиСписокДокументовДляПротокола(Описание, СтруктураСтатистики.Загружено);
		
		Описание = Описание
			+ Символы.ПС + НСтр("ru='Пропущено: ';uk='Пропущене:'") + СтруктураСтатистики.Пропущено.Количество();
		
		ВывестиСписокДокументовДляПротокола(Описание, СтруктураСтатистики.Пропущено);
		
		Описание = Описание
			+ Символы.ПС + НСтр("ru='Обновлено: ';uk='Оновлене:'") + СтруктураСтатистики.Обновлено.Количество();
		
		ВывестиСписокДокументовДляПротокола(Описание, СтруктураСтатистики.Обновлено);
		
		Описание = Описание
			+ Символы.ПС + НСтр("ru='Создано: ';uk='Створене:'") + СтруктураСтатистики.Создано.Количество();
		...
		Описание = Описание
			...
			+ " "
			+ НСтр("ru='Завершена загрузка заказов';uk='Завершене завантаження замовлень'");
	...
	Иначе
		...
		Описание = Описание
			...
			+ СтрокаТаблицыИнформации.ДатаОкончания
			+ " "
			+ НСтр("ru='Завершена выгрузка заказов';uk='Завершене вивантаження замовлень'");
...
Функция ПолучитьСтруктуруОсновныхПараметровОбмена() Экспорт
	...
	ОсновныеПараметры.Вставить("НаименованиеНалога", НСтр("ru='НДС';uk='ПДВ'"));
...
Функция УстановитьСоединениеССервером(ПараметрыПодключения, ОписаниеОшибки)
...
Функция ВыполнитьАвторизациюДляСоединения(Соединение, ПараметрыПодключения, 
...
Функция ВыгрузитьНаСайт(Параметры,
	...
	Если СтрЧислоСтрок(ОтветСервера) <> 2 Тогда
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ru='Выгрузка на сайт не выполнена.';uk='Вивантаження на сайт не виконане.'") + Символы.ПС + НСтр("ru='Не удалось прочитать ответ сервера. Параметры обмена не получены.';uk='Не вдалося прочитати відповідь сервера. Параметри обміну не отримані.'"));
		Возврат Ложь;
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ru='Ошибка получения параметров обмена (ограничение размера файла)!';uk='Помилка одержання параметрів обміну (обмеження розміру файлу)!'")));
	...
	Для Каждого ТекФайл Из СписокФайловДляОтправки цикл
		...
		Если ОтветСервера = Неопределено Тогда
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки, 
				НСтр("ru='Не удалось получить ответ сервера. Файл не отправлен.';uk='Не вдалося одержати відповідь сервера. Файл не відправлений.'") + " (" + ТекФайл.Значение + ").");
		...
		Если СостояниеОбмена = НастройкиПодключения.ОтветСервера_АварийноеЗавершениеТекущейОперации Тогда
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки,
				НСтр("ru='Произошла ошибка на стороне сервера. Файл не отправлен.';uk='Відбулася помилка на стороні сервера. Файл не відправлений.'") + " (" + ТекФайл.Значение + ")."
		...
		ИначеЕсли СостояниеОбмена = НастройкиПодключения.ОтветСервера_УспешноеЗавершениеТекущейОперации Тогда
			...
			Если СтрЧислоСтрок(ОтветСервера) > 1 Тогда
				...
				ДобавитьОписаниеОшибки(ОписаниеОшибки,
					НСтр("ru='Получен расширенный статус успешного завершения сеанса.';uk='Отриманий розширений статус успішного завершення сеансу.'")
					+ Символы.ПС + НСтр("ru='Ответ сервера: ';uk='Відповідь сервера:'") + Символы.ПС + ОтветСервера);
		...
		Иначе
			...
			ДобавитьОписаниеОшибки(ОписаниеОшибки,
				НСтр("ru='Произошла ошибка на стороне сервера. Не получен статус завершения операции. Файл не отправлен.';uk='Відбулася помилка на стороні сервера. Не отриманий статус завершення операції. Файл не відправлений.'") + " (" + ТекФайл.Значение + ")."
				+ Символы.ПС + НСтр("ru='Ответ сервера: ';uk='Відповідь сервера:'") + Символы.ПС + ОтветСервера);
			...
			Пока ИмпортПродолжается Цикл
				...
				Если ОтветСервера = Неопределено Тогда 
					...
					ДобавитьОписаниеОшибки(ОписаниеОшибки, ТекФайл.Имя + ": "
						+ НСтр("ru='Не удалось получить текущее состояние процесса обмена. Данные обмена отправлены, но не загружены.';uk='Не вдалося одержати поточний стан процесу обміну. Дані обміну відправлені, але не завантажені.'"));
				...
				Иначе
					...
					Если СостояниеОбмена = НастройкиПодключения.ОтветСервера_АварийноеЗавершениеТекущейОперации Тогда
						...
						Успешно = Ложь;
						ДобавитьОписаниеОшибки(ОписаниеОшибки, ТекФайл.Имя + ": "
							+ НСтр("ru='Произошла ошибка на стороне сервера.';uk='Відбулася помилка на стороні сервера.'")
							+ Символы.ПС + НСтр("ru='Ответ сервера';uk='Відповідь сервера'") + ": " + Символы.ПС + ОтветСервера);
					...
					Иначе
						...
						ДобавитьОписаниеОшибки(ОписаниеОшибки, ТекФайл.Имя + ": "
							+ НСтр("ru='Произошла ошибка на стороне сервера. Получен неизвестный статус импорта.';uk='Відбулася помилка на стороні сервера. Отриманий невідомий статус імпорту.'")
							+ Символы.ПС + НСтр("ru='Ответ сервера';uk='Відповідь сервера'") + ": " + Символы.ПС + ОтветСервера);
...
Функция ОтправитьФайлНаСервер(ПолноеИмяФайла, Соединение, ПараметрыЗапроса="", Заголовки="", ОписаниеОшибки)
	...
	Если ФайлОтвета.Существует() Тогда
		...
		Иначе
			ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ru='Отправка файла на сервер: Получен пустой ответ сервера.';uk='Відправлення файлу на сервер: Отримана порожня відповідь сервера.'"));
		КонецЕсли;
		
	Иначе
		
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ru='Отправка файла на сервер: Ответ сервера не получен.';uk='Відправлення файлу на сервер: Відповідь сервера не отримана.'")); 
	...
	Исключение
		ЗаписьЖурналаРегистрации("Обмен с сайтами",
			...
			,
			НСтр("ru='Не удалось удалить временный файл';uk='Не вдалося вилучити тимчасовий файл'") + " " + ИмяФайлаОтвета);
	КонецПопытки;
...
Функция ПолучитьДанныеССервера(Соединение, ПараметрыЗапроса = "", Заголовки = "", ОписаниеОшибки)
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось получить данные с сервера. Проверьте правильность адреса сервера, порт, имя пользователя и пароль,';uk='Не вдалося одержати дані із сервера. Перевірте правильність адреси сервера, порт, ім`я користувача й пароль,'")
				+ Символы.ПС
				+ НСтр("ru='а также настройки подключения к Интернет.';uk='а також налаштування підключення до Інтернет.'")));
	...
	Если ФайлОтвета.Существует() Тогда
		...
		Иначе
			ДобавитьОписаниеОшибки(ОписаниеОшибки,
				НСтр("ru='Получение данных с сервера: Получен пустой ответ сервера.';uk='Одержання даних із сервера: Отримана порожня відповідь сервера.'"));
		КонецЕсли;
		
	Иначе
		
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			НСтр("ru='Получение данных с сервера: Ответ сервера не получен.';uk='Одержання даних із сервера: Відповідь сервера не отримана.'"));
...
Функция ПолучитьКонтрагентаXDTO(ДанныеКонтрагента, ПакетCML) Экспорт
	...
	ЕДРПОУ = ПолучитьЕДРПОУДляXDTO(ДанныеКонтрагента.Контрагент.КодПоЕДРПОУ);
	Если НЕ ПустаяСтрока(ЕДРПОУ) Тогда
		КонтрагентXDTO.ЕДРПОУ = ЕДРПОУ;
	КонецЕсли;
	
	Если ЭтоЮрЛицо Тогда
		КонтрагентXDTO.ЕДРПОУ = ДанныеКонтрагента.КодПоЕДРПОУ;
	КонецЕсли;
...
Функция ПолучитьЕДРПОУДляXDTO(ЕДРПОУ)
	
	ЗначениеЕДРПОУ = ?(СтрДлина(ЕДРПОУ) = 9, ЕДРПОУ, "");
	
	Возврат ЗначениеЕДРПОУ;
...
Функция РаспаковатьZIPАрхив(СтрокаZIP, ОписаниеОшибки) Экспорт
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось распаковать архив с заказами!';uk='Не вдалося розпакувати архів із замовленнями!'")));
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ru='Не удалось записать архив с заказами: ';uk='Не вдалося записати архів із замовленнями:'")
				+ ИмяФайла));
...
Функция ПолучитьДанныеБанкаXDTO(БанкXDTO)
	...
	МФО = "";
	Если ОбъектXDTOСодержитСвойство(БанкXDTO, "МФО") И СвойствоXDTOЗаполнено(БанкXDTO.МФО) Тогда
		МФО = СтрЗаменить(БанкXDTO.МФО, " ", "");
	КонецЕсли;
...
// 12-14 разряды номера 14-значного банковского счета должны соответствовать коду валюты.
// В остальных случаях определить валюту счета невозможно, т.к. структура счета в каждом банке своя.  
//
// Параметры:
...
Функция ПолучитьВалютуПоНомеруСчета(НомерСчета) Экспорт
	...
	Если СтрДлина(НомерСчета) = 14 Тогда
		
		КодВалюты = Прав(НомерСчета, 3);
...
Функция НадписьВсеГруппы() Экспорт
	Возврат "(" + НСтр("ru='Все';uk='Всі'") + ")";
КонецФункции

Функция НадписьНетГруппы() Экспорт
	Возврат НСтр("ru='<Нет группы>';uk='<Без групи>'");
КонецФункции
