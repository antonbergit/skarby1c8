...
Процедура ФормированиеHTTPЗапроса_ПослеПодключенияКомпоненты(Подключено, Контекст) Экспорт
	...
	Если Подключено Тогда
		...
		Исключение
			ТекстОшибки = НСтр("ru='Ошибка инициализации компоненты HTTP-запросов.';uk='Ошибка инициализации компоненты HTTP-запросов.'") + Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
...
"Перейти к установке данной компоненты?';uk='Для поддержки интеграции с ЕГАИС необходима установка компоненты HTTP-запросов."
"Перейти к установке данной компоненты?'");
	...
	Иначе
		ИнтеграцияЕГАИСКлиент.ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ru='Операция невозможна. Требуется установка компоненты для HTTP-запросов.';uk='Операция невозможна. Требуется установка компоненты для HTTP-запросов.'"));
	КонецЕсли;
...
Процедура ФормированиеHTTPЗапроса_ПередУстановкойКомпоненты(РезультатВопроса, Контекст) Экспорт
	...
	Если НЕ РезультатВопроса = КодВозвратаДиалога.Да Тогда
		ИнтеграцияЕГАИСКлиент.ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ru='Не установлена компонента HTTP-запросов.';uk='Не установлена компонента HTTP-запросов.'"));
	Иначе
...
Процедура ФормированиеHTTPЗапроса_ПослеУстановкиПараметровПрокси(РезультатВыполнения, Параметры, Контекст) Экспорт
	...
	Если НЕ РезультатВыполнения Тогда
		ИнтеграцияЕГАИСКлиент.ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ru='Ошибка установки настроек прокси-сервера.';uk='Ошибка установки настроек прокси-сервера.'"));
		Возврат;
	КонецЕсли;
...
Процедура ФормированиеHTTPЗапроса_ПослеДобавленияЗаголовка(РезультатВыполнения, Параметры, Контекст) Экспорт
	...
	Если НЕ РезультатВыполнения Тогда
		ИнтеграцияЕГАИСКлиент.ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ru='Ошибка добавления заголовка запроса.';uk='Ошибка добавления заголовка запроса.'"));
		Возврат;
	КонецЕсли;
...
Процедура ФормированиеHTTPЗапроса_ПослеДобавленияЗаголовков(Контекст)
	...
	Иначе
		ИнтеграцияЕГАИСКлиент.ЗавершитьОперациюСОшибкой(Контекст.ОповещениеПриЗавершении, Контекст.ОтображатьСообщения, НСтр("ru='Неверный тип HTTP-запроса.';uk='Неверный тип HTTP-запроса.'"));
	КонецЕсли;
...
Процедура ФормированиеHTTPЗапроса_Завершение(КодСостояния, Параметры, Контекст) Экспорт
	...
	Если НЕ ПустаяСтрока(Результат.ОписаниеОшибки) Тогда
		ТекстОшибки = НСтр("ru='Ошибка при выполнении %1-запроса по адресу %2';uk='Ошибка при выполнении %1-запроса по адресу %2'");
