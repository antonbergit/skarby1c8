...
Процедура ОткрытьПодробноеОписаниеСинхронизации(СсылкаНаПодробноеОписание) Экспорт
	...
	Если ЗначениеЗаполнено(СсылкаНаПодробноеОписание) Тогда
...
Процедура ОбработчикВыбораФайловогоКаталога(Объект, Знач ИмяСвойства, СтандартнаяОбработка = Ложь, Знач ПараметрыДиалога = Неопределено, ОповещениеЗавершения = Неопределено) Экспорт
	...
	ТекстПредупреждения = НСтр("ru='Для данной операции необходимо установить расширение для веб-клиента 1С:Предприятие.';uk='Для цієї операції необхідно встановити розширення для "
"веб-клієнта 1С:Підприємство.'");
...
Процедура ПередатьФайлыНаСервер(ОповещениеЗавершения, Знач ИменаФайлов, Знач ИдентификаторФормы = Неопределено, Знач ТекстПредупреждения = Неопределено) Экспорт
	...
	Если ТекстПредупреждения = Неопределено Тогда
		...
		Иначе
			ТекстПредупреждения = НСтр("ru='Для передачи файлов на сервер необходимо установить расширение для веб-клиента 1С:Предприятие.';uk='Для передачі файлів на сервер необхідно встановити розширення "
"для веб-клієнта 1С:Підприємство.'");
		КонецЕсли;
	КонецЕсли;
...
"с помощью параметра запуска конфигуратора /ResetMasterNode.';uk='Відключення інформаційної бази від головного вузла можна виконати"
"за допомогою параметра запуску конфігуратора /ResetMasterNode.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
	Иначе
		ТекстВопроса = НСтр("ru='Удалить настройку синхронизации данных?';uk='Видалити налаштування синхронізації даних?'");
