...
Функция НалоговыйОрганПоАдресу(Знач ЗначенияПолейАдреса) Экспорт
	...
	Если Прокси <> Неопределено Тогда
		...
		Исключение
			...
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Адрес %1:';uk='Адрес %1:'"), ЗначенияПолейАдреса)
	...
	Если РезультатыПроверки.Item.Количество()>0 Тогда
		...
		Если ОшибкиПроверки.Количество() > 0 Тогда
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Адрес %1: %2';uk='Адрес %1: %2'"), ЗначенияПолейАдреса, ОшибкиПроверки[0].Text);
		...
		Иначе
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Адрес %1: ошибка разбора адреса';uk='Адрес %1: ошибка разбора адреса'"), ЗначенияПолейАдреса);
		КонецЕсли;
...
Процедура ОбработатьОшибкуСервиса(ОписаниеОшибки, СтруктураРеквизитов)
	...
	ИмяСобытия = НСтр("ru='Сервис данных ФИАС.';uk='Сервис данных ФИАС.'", КодОсновногоЯзыка) + " " + ТекстСобытия;
