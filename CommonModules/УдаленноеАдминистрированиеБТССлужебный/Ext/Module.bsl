...
Процедура ЗавершитьСеансыОбластиДанных(Знач НомераСеансов, Знач ПарольПользователя) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru='Текущая версия управляющего приложения не поддерживает завершение сеанса из приложений. Необходимо обновить управляющее приложение.';uk='Поточна версія керуючого додатка не підтримує завершення сеансу з додатків. Необхідно оновити керуючий додаток.'");
...
Процедура ОтправитьЗапросыНаИспользованиеВнешнихРесурсов(Знач СериализацияЗапросов) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru='Текущая версия управляющего приложения не поддерживает запросы разрешений на использование внешних ресурсов. Необходимо обновить управляющее приложение.';uk='Поточна версія керуючого додатку не підтримує запити дозволів на використання зовнішніх ресурсів. Необхідно оновити керуючий додаток.'");
