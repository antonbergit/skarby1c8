...
Функция ЗаголовокСозданияРезервнойКопии(Параметры) Экспорт
	...
	Если Параметры.ВосстанавливатьИнформационнуюБазу Тогда
		Результат = Результат + " " + НСтр("ru='и выполнять откат при нештатной ситуации';uk='і виконувати відкіт при позаштатній ситуації'");
	Иначе
		Результат = Результат + " " + НСтр("ru='и не выполнять откат при нештатной ситуации';uk='і не виконувати відкіт при позаштатній ситуації'");
	КонецЕсли;
...
Функция ВозможнаУстановкаОбновления(Параметры, ПараметрыАдминистрирования)
	...
	Если ЭтоФайловаяБаза И Параметры.СоздаватьРезервнуюКопию = 2 Тогда
		...
		Если Не Файл.Существует() Или Не Файл.ЭтоКаталог() Тогда
			ПоказатьПредупреждение(, НСтр("ru='Укажите существующий каталог для сохранения резервной копии ИБ.';uk='Вкажіть існуючий каталог для збереження резервної копії ІБ.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	ИначеЕсли Параметры.РежимОбновления = 2 Тогда
		...
		Если Параметры.ВыслатьОтчетНаПочту
			...
			ПоказатьПредупреждение(, НСтр("ru='Укажите допустимый адрес электронной почты.';uk='Вкажіть припустиму адресу електронної пошти.'"));
			Возврат Ложь;
		КонецЕсли;
		...
		Если Не ЗапланироватьОбновлениеКонфигурации(Параметры, ПараметрыАдминистрирования) Тогда
			ПоказатьПредупреждение(, НСтр("ru='Не удалось запланировать обновление конфигурации. Сведения об ошибке сохранены в журнал регистрации.';uk='Не вдалося запланувати оновлення конфігурації. Відомості про помилку збережені в журнал реєстрації.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Функция УдалитьЗадачуПланировщикаScheduledJob(КодЗадачи)
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Ошибка при удалении задачи планировщика: %1';uk='Помилка при видаленні завдання планувальника: %1'");
...
Функция СоздатьЗадачуПланировщикаScheduledJob(Знач ИмяФайлаПрограммы, Знач ДатаВремя)
	...
	Исключение
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании задачи планировщика: %1';uk='Помилка при створенні завдання планувальника: %1'"), ОписаниеОшибки());
	...
	Если КодОшибки <> 0 Тогда // Коды ошибок: http://msdn2.microsoft.com/en-us/library/aa389389(VS.85).aspx.
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании задачи планировщика: %1';uk='Помилка при створенні завдання планувальника: %1'"), КодОшибки);
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", ТекстСообщения);
		Возврат 0;
	КонецЕсли;
...
Функция СоздатьЗадачуПланировщикаScheduleService(Знач ИмяФайлаПрограммы, Знач ДатаВремя)
	...
	Попытка
		...
		ОписаниеЗадачи.RegistrationInfo.Description = НСтр("ru='Обновление конфигурации 1С:Предприятие';uk='Оновлення конфігурації 1С:Підприємство'");
...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	Если ПараметрыПриложения["СтандартныеПодсистемы.ПредлагатьОбновлениеИнформационнойБазыПриЗавершенииСеанса"] = Истина Тогда
		...
		ПараметрыПредупреждения.ТекстФлажка  = НСтр("ru='Установить обновление конфигурации';uk='Встановити оновлення конфігурації'");
		ПараметрыПредупреждения.ТекстПредупреждения  = НСтр("ru='Запланирована установка обновления';uk='Запланована установка оновлення'");
