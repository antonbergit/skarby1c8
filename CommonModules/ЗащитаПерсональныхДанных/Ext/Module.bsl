...
Процедура ДобавитьКомандуПечатиСогласияНаОбработкуПерсональныхДанных(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='Согласие на обработку ПДн...';uk='Згода на обробку ПДн...'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ru='Согласия на обработку ПДн';uk='Згоди на обробку ПДн'");
		Дело.Количество     = ИстекшиеСогласия.Количество();
		Дело.Подсказка      = НСтр("ru='Истекли согласия на обработку персональных данных (ПДн). Необходимо прекратить обработку ПДн или запросить у субъекта продление согласия на обработку.';uk='Минули згоди на обробку персональних даних (ПДн). Необхідно припинити обробку ПДн або запросити в суб`єкта продовження згоди на обробку.'");
...
Функция ОбластиПерсональныхДанных()
	...
	НоваяОбласть.Представление = НСтр("ru='Обработка ПДн';uk='Обробка ПДн'");
...
Функция ДеревоОбластейПерсональныхДанных()
	...
	Если ОбластиДанных.Количество() = 0 
		...
		ДобавитьОбластьДанныхВДерево(ДеревоОбластейДанных, ОбластиДанных, Новый Структура("Имя, Представление, Родитель", "", НСтр("ru='Персональные данные';uk='Персональні дані'")));
	КонецЕсли;
