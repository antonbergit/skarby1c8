...
Процедура ЗапускФоновогоЗадания(Получатели, Параметры) Экспорт
	...
	Если Параметры.ЭтоФормаЭлемента Тогда
		...
		КонецЕсли;
		ТекстСостояния = НСтр("ru='Выполняется рассылка отчетов';uk='Виконується розсилання звітів'");
	Иначе
		ТекстСостояния = НСтр("ru='Выполняются рассылки отчетов';uk='Виконуються розсилання звітів'");
	КонецЕсли;
...
Процедура ПослеЗавершенияФоновогоЗадания(Задание, Параметры) Экспорт
	...
	Иначе
		...
		Предупреждение.Текст = НСтр("ru='Не удалось выполнить рассылки отчетов.';uk='Не вдалося виконати розсилки звітів.'")
			+ Символы.ПС
