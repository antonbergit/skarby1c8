...
Функция КПППоКомбинацииОрганизацииИКодаИФНС(Организация, КодИФНС) Экспорт
	...
	Иначе
		...
		Иначе
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Невозможно определить КПП для организации %1 и ИФНС с кодом %2!';uk='Невозможно определить КПП для организации %1 и ИФНС с кодом %2!'"), Организация, КодИФНС);
	...
	Для Каждого МакетОтчета Из МакетыОтчета Цикл
		...
		И Лев(нрег(МакетОтчета.Имя), СтрДлина(ПрефиксИмениМакетаШаблона)) = нрег(ПрефиксИмениМакетаШаблона) Тогда
			...
			Исключение
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось получить шаблон машиночитаемой формы ""%1""!%2';uk='Не удалось получить шаблон машиночитаемой формы ""%1""!%2'"), ИмяФайлаШаблона, Символы.ПС + ОписаниеОшибки());
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Исключение
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось получить шаблон машиночитаемой формы ""%1""!%2';uk='Не удалось получить шаблон машиночитаемой формы ""%1""!%2'"), Выборка.ИмяФайлаШаблона, Символы.ПС + ОписаниеОшибки());
...
Функция ПроверитьВозможностьВыводаМашиночитаемойФормыЧерезВебСервис(АдресПараметровПроксиВоВременномХранилище = "", ПолучитьОпределенияВебСервиса = Истина, ЭтоВызовИзВебКлиента = Ложь) Экспорт
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Невозможно подключиться к веб-сервису формирования машиночитаемых бланков.';uk='Невозможно подключиться к веб-сервису формирования машиночитаемых бланков.'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
	...
	Если ПустаяСтрока(АдресТочкиПодключенияВебСервиса) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не заполнен адрес точки подключения веб-сервиса!';uk='Не заполнен адрес точки подключения веб-сервиса!'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
	...
	Исключение
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не удалось получить подключение к веб-сервису!';uk='Не удалось получить подключение к веб-сервису!'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецПопытки;
...
Функция ВывестиМашиночитаемуюФормуЧерезВебСервис(ИмяФормыОтчета, НаименованиеИФНС, ФайлВыгрузкиВоВременномХранилище, ПараметрыСформированногоОтчета, АдресПараметровПроксиВоВременномХранилище) Экспорт
	...
	Исключение
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не удалось получить подключение к веб-сервису! Проверьте адрес точки подключения, имя, пароль или доступность веб-сервиса.';uk='Не удалось получить подключение к веб-сервису! Проверьте адрес точки подключения, имя, пароль или доступность веб-сервиса.'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецПопытки;
	...
	Если НЕ ЭтоАдресВременногоХранилища(АдресФайлаВыгрузкиВоВременномХранилище) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не удалось получить файл выгрузки отчета на сервере!';uk='Не удалось получить файл выгрузки отчета на сервере!'");
		Сообщение.Сообщить();
		Возврат Ложь;
	КонецЕсли;
...
Функция ПолучитьМашиночитаемуюФормуСВебСервиса(ПараметрыСформированногоОтчета) Экспорт
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru='Не удалось получить подключение к веб-сервису! Проверьте адрес точки подключения, имя, пароль или доступность веб-сервиса.';uk='Не удалось получить подключение к веб-сервису! Проверьте адрес точки подключения, имя, пароль или доступность веб-сервиса.'");
...
Процедура ОтразитьВЖурналеОтчетов(ПараметрыОтчета) Экспорт
	...
	Исключение
		...
		Если ОшибкаУстановкиБлокировки Тогда
			Возврат;
		Иначе
			ЗаписьЖурналаРегистрации(НСтр("ru='Журнал отчетов (статусы): ошибка записи';uk='Журнал отчетов (статусы): ошибка записи'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		Если ФП = "Ложь"
		   И ДатаНачалаОП = НачалоГода(ДатаНачалаОП) Тогда
			...
			Если СтрНайти(ПредставлениеФинПериода, "1 квартал") > 0 Тогда
				...
				ПредставлениеФинПериода = СтрЗаменить(ПредставлениеФинПериода, НСтр("ru='1 квартал';uk='1 квартал'"), НСтр("ru='Январь - Март';uk='Январь - Март'"));
				
			ИначеЕсли ПредставлениеФинПериода = ПредставлениеФинПериодаГод Тогда
				
				ПредставлениеФинПериода = НСтр("ru='Январь - Декабрь ';uk='Январь - Декабрь '") + ПредставлениеФинПериодаГод;
...
Функция ПредставлениеКонтролирующегоОргана(ЗначенияЗаполнения) Экспорт
	...
	Если ЗначениеЗаполнено(ЗначенияЗаполнения.ВидКонтролирующегоОргана) Тогда
		...
		КонецЕсли;
	ИначеЕсли ТипЗнч(ЗначенияЗаполнения.Ссылка) = Тип("ДокументСсылка.УведомлениеОСпецрежимахНалогообложения") Тогда
...
Функция ПроверитьИВыборочноОбновитьШаблоныМашиночитаемыхФормНаВебСервисе(ПрефиксИмениШаблона, ШаблоныМашиночитаемыхФорм, Прокси)
	...
	Если ШаблоныМашиночитаемыхФорм.Количество() > 0 Тогда
		...
		Если Прокси.UploadTemplates(WSСписокШаблонов) <> Истина Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Не удалось обновить шаблон машиночитаемой формы на веб-сервисе';uk='Не удалось обновить шаблон машиночитаемой формы на веб-сервисе'");
			Сообщение.Сообщить();
			Возврат Ложь;
		КонецЕсли;
...
Функция ПолучитьСтавкуНалога(Период) Экспорт

	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЕСТЬNULL(НДФЛСтавкиНалоговСрезПоследних.Ставка1, 0) КАК Ставка1
	|ИЗ
	|	РегистрСведений.НДФЛСтавкиНалогов.СрезПоследних(&Период, Налог = &Налог) КАК НДФЛСтавкиНалоговСрезПоследних";
	Запрос.УстановитьПараметр("Период", Период);
	Запрос.УстановитьПараметр("Налог", Справочники.ВидыНачисленийИУдержаний.ЕСВ_ФОТ);
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Если Выборка.Ставка1 <> NULL Тогда
			
			Возврат Выборка.Ставка1;  
