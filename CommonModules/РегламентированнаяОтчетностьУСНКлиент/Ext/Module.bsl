...
Функция РассчитатьЕдиныйНалог(Организация, ДатаДокументаОбработкиСобытия, СобытиеКалендаря) Экспорт
	...
	Состояние(
		НСтр("ru='Рассчитано';uk='Розраховане'"),
		,
		НСтр("ru='Единый налог рассчитан';uk='Єдиний податок розрахований'"),
...
Функция РассчитатьАвансовыйПлатежПоУСН(Организация, ДатаДокументаОбработкиСобытия,СобытиеКалендаря) Экспорт
	...
	Состояние(
		НСтр("ru='Рассчитано';uk='Розраховане'"),
		,
		НСтр("ru='Авансовый платеж рассчитан';uk='Авансовий платіж розрахован'"),
		БиблиотекаКартинок.Рассчет32);
...
Функция СформироватьОтчетностьВФПР(Организация, ДатаДокументаОбработкиСобытия,СобытиеКалендаря) Экспорт
	...
	Если УчетСтраховыхВзносов.ОтразитьНаличиеСотрудниковПриСдачеВПФР(Организация, ДатаДокументаОбработкиСобытия,СобытиеКалендаря) Тогда
		// Формируем справку РСВ-1
		РезультатРасчета = РегламентированнаяОтчетностьУСН.СформироватьСправкуРСВ1(Организация,ДатаДокументаОбработкиСобытия, СобытиеКалендаря);
		Если Год(ДатаДокументаОбработкиСобытия)< 2013 Тогда
			ФормаОтчета = "ФормаОтчета2012Кв1";
		ИначеЕсли Год(ДатаДокументаОбработкиСобытия)< 2014 Тогда
			ФормаОтчета = "ФормаОтчета2013Кв1";
		ИначеЕсли ДатаДокументаОбработкиСобытия < Дата('20150401') Тогда
			ФормаОтчета = "ФормаОтчета2014Кв1";
		Иначе
			ФормаОтчета = "ФормаОтчета2015Кв1";
		КонецЕсли;
		...
		Параметры.Вставить("СформироватьФормуОтчетаАвтоматически", Истина);
		Параметры.Вставить("НеОтображатьПредупреждение", Истина);
		
		Форма = ПолучитьФорму("Отчет.РегламентированныйОтчетРСВ1.Форма."+ФормаОтчета, Параметры);
		...
		Форма.СтруктураРеквизитовФормы.мВариант = 0;
		Форма.ЗаполнитьСведенияОбОрганизации();
		//Форма.ОПЗаполнитьАвто();
...
Процедура ОткрытьФормуРегламентированногоОтчетаПоСобытию(Организация = Неопределено,СобытиеКалендаря) Экспорт
	...
	Если ДокументРегламентированнойОтчетности = Неопределено  Тогда
		ПоказатьПредупреждение(,НСтр("ru='Нет сохраненного отчета. Сформируйте отчет.';uk='Немає збереженого звіту. Сформуйте звіт.'"));
		Возврат;
	КонецЕсли;
...
// Выполняет регламентные процедуры, связанные с событие календаря отчетности
//
Процедура ВыполнитьРегламентныеПроцедурыПередРасчетомПоСобытию(Организация,СобытиеКалендаря) Экспорт
	
	ДатаДокументаОбработкиСобытия = РегламентированнаяОтчетностьУСН.ПолучитьДанныеСобытия(СобытиеКалендаря,"ДатаДокументаОбработкиСобытия");
	
	ВыполнитьРегламентныеПроцедурыПередРасчетом(Организация, ДатаДокументаОбработкиСобытия);
	
КонецПроцедуры

// Выполняет все регламентные процедуры на дату
//
Процедура ВыполнитьРегламентныеПроцедурыПередРасчетом(Организация,ДатаДокументаОбработкиСобытия) Экспорт
	
	//КоличествоОпераций = 2;
	//
	//Состояние(
	//	НСтр("ru='Зачет оплат';uk='Залік оплат'"),
	//	1/КоличествоОпераций*100,
	//	НСтр("ru='Проводится автозачет оплат';uk='Проводиться автозалік оплат'"),
	//	БиблиотекаКартинок.Рассчет32
	//);
	//
	//ПротоколАвтозачета = Неопределено;
	//ДенежныеСредства.ПровестиАвтозачетОплат(
	//	Организация,
	//	КонецГода(ДатаДокументаОбработкиСобытия),
	//	ПротоколАвтозачета);
	//
	//Если ПротоколАвтозачета <> Неопределено Тогда
	//	ПоказатьОповещениеПользователя(
	//		НСтр("ru='Автозачет оплат';uk='Автозачет сплат'"),
	//		ПолучитьНавигационнуюСсылку(ПротоколАвтозачета),
	//		НСтр("ru='Произведен автозачет оплат документов';uk='Зроблений автозалік оплат документів'"),
	//		БиблиотекаКартинок.Информация32);
	//КонецЕсли;
	//
	//
	//Состояние(
	//	НСтр("ru='Формирование партий';uk='Формування партій'"),
	//	2/КоличествоОпераций*100,
	//	НСтр("ru='Формируются партии товаров для определения себестоимости';uk='Формуються партії товарів для визначення собівартості'"),
	//	БиблиотекаКартинок.Рассчет32
	//);
	//
	//ДвиженияТоваровСервер.ВыполнитьФормированияДвиженийПартионногоУчета(
	//	Организация,
	//	КонецГода(ДатаДокументаОбработкиСобытия));
...
Процедура ПолучитьБанковскийСчетДляУплатыНалога(ОповещениеВыбораСчета, Организация, Форма = Неопределено, ЭлементФормы = Неопределено) Экспорт
	...
	Если СпсБанкСчетов.Количество() = 0 Тогда
		...
		СпсОтвет.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Показать счета';uk='Показати рахунки'"));
		СпсОтвет.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru='Отмена';uk='Відмінити'"));
		
		оп = Новый ОписаниеОповещения("ОповещениеВопросаПоказаСпискаСчетов", ЭтотОбъект, Новый Структура("Организация", Организация));
		ПоказатьВопрос(оп, НСтр("ru='Не найдены подходящие банковские счета';uk='Не знайдені підходящі банківські рахунки'"), СпсОтвет);
	...
	Иначе
		...
		СпсБанкСчетов.ПоказатьВыборЭлемента(оп, НСтр("ru='С какого счета провести оплату';uk='З якого рахунку провести оплату'"));
		#Иначе
...
Функция ОбработкаКомандыНалоговогоРаздела(Задача) Экспорт
	...
	Если ДанныеСобытия.ЗадачаПрименима Тогда
		...
		Если ДанныеСобытия.СобытиеКалендаря = Неопределено Тогда
			ОткрытьФорму("Справочник.ЗаписиКалендаряПодготовкиОтчетности.Форма.КалендарьНалоговИОтчетности", 
				Новый Структура("Задача", ПредопределенноеЗначение("Справочник.ЗадачиКалендаряПодготовкиОтчетности."+Задача)));
		Иначе
	...
	Иначе
		ОткрытьФорму("Справочник.ЗаписиКалендаряПодготовкиОтчетности.Форма.КалендарьНалоговИОтчетности",
				Новый Структура("Организация,Задача,Недоступность", ДанныеСобытия.Организация,  ПредопределенноеЗначение("Справочник.ЗадачиКалендаряПодготовкиОтчетности."+Задача), Истина));
	КонецЕсли;
