...
"ИмяОтчета = ""%1"", КлючВарианта = ""%2"".';uk='Не удалось определить вариант отчета по переданным параметрам:"
...
Процедура ДобавитьВариантыОформленияСумм(ВариантыОформления, Поля, НастройкиКД = Неопределено) Экспорт
	...
	КлючОсновногоВариантаОформления = НСтр("ru='С копейками';uk='З копійками'");
	...
	Для каждого ИмяПоля Из МассивПолей Цикл
		...
		КлючВариантаОформления = НСтр("ru='Без копеек';uk='Без копійок'");
		ОтчетыУНФ.ДобавитьВариантОформления(ВариантыОформления, ИмяПоля, КлючВариантаОформления, "Формат", "ЧДЦ=0");
		
		КлючВариантаОформления = НСтр("ru='В тысячах';uk='У тисячах'");
...
Процедура ДобавитьВариантыОформленияКоличества(ВариантыОформления, Поля, НастройкиКД = Неопределено) Экспорт
	...
	КлючОсновногоВариантаОформления = НСтр("ru='Не выравнивать';uk='Не вирівнювати'");
	...
	Для каждого ИмяПоля Из МассивПолей Цикл
		...
		КлючВариантаОформления = НСтр("ru='Выравнивать до 3-х знаков после запятой';uk='Вирівнювати до 3-х знаків після коми'");
...
Процедура ДобавитьВычисляемыеПоля(СхемаКомпоновкиДанных) Экспорт
	...
	Если НЕ Поле=Неопределено Тогда
		...
		Если ВычисляемоеПоле=Неопределено Тогда
			...
			ВычисляемоеПоле.Заголовок = НСтр("ru='Месяц года';uk='Месяц года'");
			ВычисляемоеПоле.ТипЗначения = Новый ОписаниеТипов("Дата", Новый КвалификаторыДаты(ЧастиДаты.Дата));
			ВычисляемоеПоле.Оформление.УстановитьЗначениеПараметра("Формат", НСтр("ru='ДФ=ММММ';uk='ДФ=ММММ'"));
		...
		Если ВычисляемоеПоле=Неопределено Тогда
			...
			ВычисляемоеПоле.Заголовок = НСтр("ru='День и месяц года';uk='День и месяц года'");
			ВычисляемоеПоле.ТипЗначения = Новый ОписаниеТипов("Дата", Новый КвалификаторыДаты(ЧастиДаты.Дата));
			ВычисляемоеПоле.Оформление.УстановитьЗначениеПараметра("Формат", НСтр("ru=""ДФ='д ММММ'"";uk=""ДФ='д ММММ'""")); 
			ВычисляемоеПоле.ОграничениеИспользования.Условие = Истина;
		КонецЕсли; 
		...
		Если ВычисляемоеПоле=Неопределено Тогда
			...
			ВычисляемоеПоле.Заголовок = НСтр("ru='Неделя года';uk='Неделя года'");
			ВычисляемоеПоле.ОграничениеИспользования.Условие = Истина;
		КонецЕсли; 
	КонецЕсли; 
...
Процедура СтандартизироватьСхему(СхемаКомпоновкиДанных) Экспорт
	...
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодСекунда", НСтр("ru='Секунда';uk='Секунда'"), НСтр("ru=""ДФ='дд.ММ.гггг ЧЧ:мм:сс'"";uk=""ДФ='дд.ММ.гггг ЧЧ:мм:сс'"""));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодМинута", НСтр("ru='Минута';uk='Минута'"), НСтр("ru=""ДФ='дд.ММ.гггг ЧЧ:мм'"";uk=""ДФ='дд.ММ.гггг ЧЧ:мм'"""));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодЧас", НСтр("ru='Час';uk='Година'"), НСтр("ru=""ДФ='дд.ММ.гггг ЧЧ:00'"";uk=""ДФ='дд.ММ.гггг ЧЧ:00'"""));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодДень", НСтр("ru='День';uk='День'"), НСтр("ru='ДФ=дд.ММ.гггг';uk='ДФ=дд.ММ.гггг'"));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодНеделя", НСтр("ru='Начало недели';uk='Начало недели'"), НСтр("ru='ДФ=дд.ММ.гггг';uk='ДФ=дд.ММ.гггг'"));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодДекада", НСтр("ru='Начало декады';uk='Начало декады'"), НСтр("ru='ДФ=дд.ММ.гггг';uk='ДФ=дд.ММ.гггг'"));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодМесяц", НСтр("ru='Месяц';uk='Місяць'"), НСтр("ru=""ДФ='МММ гггг'"";uk=""ДФ='МММ гггг'"""));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодКвартал", НСтр("ru='Квартал';uk='Квартал'"), НСтр("ru=""ДФ='к ''кв.'' гггг'"";uk=""ДФ='к ''кв.'' гггг'"""));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодПолугодие", НСтр("ru='Начало полугодия';uk='Начало полугодия'"), НСтр("ru='ДФ=дд.ММ.гггг';uk='ДФ=дд.ММ.гггг'"));
	ДобавитьСтрокуТаблицыПолей(ТабПолей, "ПериодГод", НСтр("ru='Год';uk='Рік'"), НСтр("ru='ДФ=гггг';uk='ДФ=гггг'"));
	...
	Для каждого Показатель Из СписокПоказателей Цикл
		...
		Для каждого ПолеИтога Из СхемаКомпоновкиДанных.ПоляИтога Цикл
			...
			Если НастройкиОтчета.ДополнительныеСвойства.ПоказыватьАбсолютноеИзменение И Показатель.Пометка Тогда
				...
				ПараметрыФормирования.СтруктураПолейСравнения.Вставить(ИмяПоля);
				ВычисляемоеПоле.Выражение = Показатель.Значение;
				ВычисляемоеПоле.Заголовок = НСтр("ru='Разница';uk='Різниця'");
				Ресурс = СхемаКомпоновкиДанных.ПоляИтога.Добавить();
				Ресурс.ПутьКДанным = ВычисляемоеПоле.ПутьКДанным;
				Ресурс.Группировки.Очистить();
				Для каждого ИмяГруппировки Из ПолеИтога.Группировки Цикл
			...
			Если НастройкиОтчета.ДополнительныеСвойства.ПоказыватьОтносительноеИзменение И Показатель.Пометка Тогда
				...
				ПараметрыФормирования.СтруктураПолейСравнения.Вставить(ИмяПоля);
				ВычисляемоеПоле.Выражение = Показатель.Значение;
				ВычисляемоеПоле.Заголовок = НСтр("ru='%';uk='%'");
	...
	Для каждого Рисунок Из ДокументРезультат.Рисунки Цикл
		...
		Для ИндексСерии = 0 По Диаграмма.Серии.Количество() - 1 Цикл
			...
			Если НЕ ЗначениеЗаполнено(Серия.Значение) Тогда
				Серия.Значение = НСтр("ru='<Не указано>';uk='<Не указано>'")
			КонецЕсли; 
...
Функция ТаблицаПериодов(НачалоПериода, КонецПериода, ПараметрыОтчета) Экспорт
	...
	Пока ТекущаяДата <= КонецПериода Цикл
		...
		Иначе
			...
			НСтр("ru='Отсутствует обработчик параметра Периодичность = ""%1"".';uk='Отсутствует обработчик параметра Периодичность = ""%1"".'"),
			Периодичность);
		КонецЕсли;
...
"%2';uk='Ошибка при записи варианта дополнительного отчета ""%1"":"
"%2'"),
		Источник.Наименование,
		ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		
		ЗаписьЖурналаРегистрации(
		НСтр("ru='Запись вариантов дополнительного отчета';uk='Запись вариантов дополнительного отчета'"),
...
"%2';uk='Не удалось обновить дополнительный внешний отчет ""%1"" по причине:"
"%2'"), 
...
"%2';uk='Ошибка при вызове обработчика ""ПриОпределенииНастроекОтчета"" в отчете ""%1"" по причине:"
"%2'"), 
				ВыборкаОтчеты.Отчет, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ОбновитьНастройкиВарианта(Вариант, СтруктураНастроек, ЗаменятьСуществующие = Ложь)
	...
	Если (НЕ Запись.ИзмененаПользователем ИЛИ ЗаменятьСуществующие) И НастройкаОтчетаИзменена(Запись, СтруктураНастроек) Тогда
		...
		КонецЕсли; 
		Если НЕ СтруктураНастроек.Свойство("Образец") ИЛИ СтруктураНастроек.Образец=Неопределено Тогда
	...
	Если СтруктураНастроек.Свойство("СвязанныеПоля") И (ОбновитьПривязки ИЛИ ЗаменятьСуществующие) Тогда
		Для каждого ОписаниеПоля Из СтруктураНастроек.СвязанныеПоля Цикл
			...
			Исключение
				ТекстСообщения = СтрШаблон(
				НСтр("ru='Неверное описание типа связанного поля ""%1"" в модуле ""%2"", вариант ""%3""';uk='Невірний опис типу зв`язаного поля ""%1"" у модулі ""%2"", варіант ""%3""'"), 
				ОписаниеПоля.Объект,
...
Функция НастройкаОтчетаИзменена(Запись, СтруктураВарианта)
	...
	Если НЕ ТипЗнч(ДанныеХранилища)=Тип("Картинка") И НЕ СтруктураВарианта.Образец=Неопределено Тогда
		Возврат Истина;
	КонецЕсли; 
...
Процедура СоздатьФильтрПределы(Стр, ЭтаФорма, Элементы)
	...
	Для каждого Реквизит Из Стр.Реквизиты Цикл
		...
		Элемент.Вид = ВидПоляФормы.ПолеВвода;
		Элемент.Заголовок = ?(Прав(ИмяРеквизита,2)="От", НСтр("ru='от';uk='від'"), НСтр("ru='до';uk='до'"));
		Элемент.РастягиватьПоГоризонтали = Истина;
...
Процедура СоздатьФильтрТумблер(Стр, ЭтаФорма, Элементы)
	...
	Для каждого Реквизит Из Стр.Реквизиты Цикл
		...
		Если ТипЗнч(Значения)=Тип("Массив") Тогда
			Элемент.СписокВыбора.Добавить(Стр.ТипЗначения.ПривестиЗначение(Неопределено), НСтр("ru='Все';uk='Всі'"));
	...
	Для каждого Реквизит Из Стр.Реквизиты Цикл
		...
		Если Стр.Тип="Фильтр" Тогда
			...
			Если ОписаниеТиповСодержитИерархическийТип(Стр.ТипЗначения) Тогда
				...
				Кнопка.Подсказка = НСтр("ru='Используется выбор элементов';uk='Використовується вибір елементів'");
...
Процедура ДобавитьПолеВыбораПериодаПланирования(НастройкиСКД, Форма, Обязательный = Истина) Экспорт
	...
	Если Стр=Неопределено Тогда
		...
		Стр.Заголовок = НСтр("ru='Сценарий планирования';uk='Сценарій планування'");
