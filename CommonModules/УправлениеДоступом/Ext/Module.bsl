...
"У обмеженні перевірки ролі (як додаткового права)"
...
""
"Указувати потрібно тільки спеціальні види доступу"
"""ПравоЧтения"", ""ПравоИзменения"", якщо вони використовуються.'"),
				Строка.ВидДоступа);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка в функции ЕстьРоль модуля УправлениеДоступом."
"Набор значений доступа содержит неизвестный вид доступа ""%1"".';uk='Помилка у функції ЕстьРоль модуля УправлениеДоступом."
...
"%1';uk='Параметр СозданиеНового не зазначений, а автоматичне заповнення"
"з реквізиту форми ""Объект.Посилання"" недоступно через:"
...
"из реквизита формы ""Объект.Ссылка"" недоступно по причине:"
"%1';uk='Параметр ТипЗначения не зазначений, а автоматичне заповнення"
"з реквізиту форми ""Объект.Посилання"" недоступно через:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецПопытки;
	Иначе
...
"%1';uk='Параметр Реквизит не зазначений, а автоматичне заповнення"
"з реквізиту форми ""Объект.ГруппаДоступа"" недоступно через:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецПопытки;
	Иначе
...
"%2';uk='Не вдалося одержати значення реквізиту форми ""%1"", зазначеного параметрі Реквізит через:"
...
"Значение параметра Список ""%1"" не является динамическим списком.';uk='Помилка виклику процедури НастроитьОтборыДинамическогоСписка загального модуля УправлениеДоступом."
"Значення параметра Список ""%1"" не є динамічним списком.'"), Строка(Список));
	...
	Если Не ЗначениеЗаполнено(Список.ОсновнаяТаблица) Тогда
		ВызватьИсключение
			НСтр("ru='Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом."
"У переданного динамического списка не указана основная таблица.';uk='Помилка виклику процедури НастроитьОтборыДинамическогоСписка загального модуля УправлениеДоступом."
"У переданого динамічного списку не зазначена основна таблиця.'");
	КонецЕсли;
	
	Если Не Список.ПроизвольныйЗапрос Тогда
		ВызватьИсключение
			НСтр("ru='Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом."
"У переданного динамического списка не установлен флажок ""Произвольный запрос"".';uk='Помилка виклику процедури НастроитьОтборыДинамическогоСписка загального модуля УправлениеДоступом."
...
"""%1"".';uk='Помилка виклику процедури НастроитьОтборыДинамическогоСписка загального модуля УправлениеДоступом."
...
"%3'"),
	...
	Если Таблица.Количество() = 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 ""%2"""
"сформировал пустой набор значений доступа.';uk='%1 ""%2"""
...
//	ОписаниеПрофиля.Наименование  = НСтр("ru='Пользователь';uk='Користувач'", Метаданные.ОсновнойЯзык.КодЯзыка);
...
"загального модуля УправлениеДоступом."
""
"Після оновлення будуть повторюватися налаштування наступних нових імен прав:"
"%1.'"),
				ПовторяющиесяНовыеИменаПрав);
		КонецЕсли;
...
"який містить відомий вид доступу ""%2"", який не потрібно "
"вказувати."
""
