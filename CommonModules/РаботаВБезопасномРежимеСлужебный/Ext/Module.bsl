...
"некоректне ім`я макета %1!'"), ИмяМакета);
	...
	Если Макет = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось сформировать разрешение на использование внешней компоненты,"
"поставляемой в макете %1: макет %1 не обнаружен в составе конфигурации!';uk='Не вдалося сформувати дозвіл на використання зовнішньої компоненти,"
"що поставляється в макеті %1: макет %1 не виявлений у складі конфігурації!'"), ИмяМакета);
	КонецЕсли;
...
"поставляє в макеті %1: в архіві не виявлений файл MANIFEST.XML!'"), ИмяМакета);
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Работа в безопасном режиме.Не удалось удалить временный файл';uk='Робота в безпечному режимі.Не вдалося видалити тимчасовий файл'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
