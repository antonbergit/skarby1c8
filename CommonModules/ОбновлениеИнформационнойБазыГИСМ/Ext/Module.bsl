...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru='Заполнение версии схем обмена с ГИСМ.';uk='Заполнение версии схем обмена с ГИСМ.'");
	...
	Обработчик.Комментарий = НСтр("ru='Обновление версии схем обмена с ГИСМ.';uk='Обновление версии схем обмена с ГИСМ.'");
	...
	Обработчик.Комментарий = НСтр("ru='Обновление настроек обмена с ГИСМ.';uk='Обновление настроек обмена с ГИСМ.'");
	...
	Обработчик.Комментарий = НСтр("ru='Перенос GLN из справочника организаций в регистр ""Организации для обмена с ГИСМ"".';uk='Перенос GLN из справочника организаций в регистр ""Организации для обмена с ГИСМ"".'");
	...
	Обработчик.Комментарий = НСтр("ru='Заполнение ресурса GLN в регистре ""Очередь получения квитанций о фиксации ГИСМ"".';uk='Заполнение ресурса GLN в регистре ""Очередь получения квитанций о фиксации ГИСМ"".'");
	...
	Обработчик.Комментарий = НСтр("ru='Обновляет справочник ""Виды меха ГИСМ"" согласно новым данным классификатора.';uk='Обновляет справочник ""Виды меха ГИСМ"" согласно новым данным классификатора.'");
	...
	Обработчик.Комментарий = НСтр("ru='Очищает в документе ""Уведомление об импорте маркированных товаров"" виды меха не соответствующие новому классификатору.';uk='Очищает в документе ""Уведомление об импорте маркированных товаров"" виды меха не соответствующие новому классификатору.'");
...
Процедура ПередОбновлениемИнформационнойБазы() Экспорт
	...
	Если ВерсияБиблиотекаИнтеграцииГИСМ <> "0.0.0.0" Тогда
		...
		Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
			...
			Набор = РегистрыСведений.ВерсииПодсистемОбластейДанных.СоздатьНаборЗаписей();
...
Процедура ВерсииСхемОбменаГИСМ_ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось обработать константу ВерсииСхемОбменаГИСМ по причине: %Причина%';uk='Не удалось обработать константу ВерсииСхемОбменаГИСМ по причине: %Причина%'");
...
Процедура НастройкиОбменаГИСМ_ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось обработать константу НастройкиОбменаГИСМ по причине: %Причина%';uk='Не удалось обработать константу НастройкиОбменаГИСМ по причине: %Причина%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Предупреждение,
