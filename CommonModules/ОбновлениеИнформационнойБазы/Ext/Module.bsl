...
"Не завершены следующие процедуры обработки данных:';uk='Дії з об`єктом тимчасово заборонені, тому що не завершений перехід на нову версію програми."
"Не завершені такі процедури обробки даних: '");
...
Процедура ОтметитьВыполнениеОбработки(Данные, ДополнительныеПараметры = Неопределено, Очередь = Неопределено) Экспорт
	...
	Если (ТипЗнч(Данные) = Тип("Массив")
		...
		ТекстИсключения = НСтр("ru='В процедуру ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки передан пустой массив. Не возможно отметить выполнение обработки.';uk='В процедуру ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки переданий порожній масив. Неможливо відмінити виконання обробки.'");
		ВызватьИсключение ТекстИсключения;
	...
	Иначе
		Если ТипЗнч(Данные) = Тип("ОбъектМетаданных") Тогда
			ТекстИсключения = НСтр("ru='Не поддерживается отметка выполнения обработки обновления целиком объекта метаданных. Нужно отмечать обработку конкретных данных.';uk='Не підтримується відмітка виконання обробки оновлення цілком об`єкту метаданих. Треба відмічати обробку конкретних даних.'");
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
...
Функция МетаданныеИОтборПоДанным(Данные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Иначе
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистр(МетаданныеОбъекта) Тогда
			...
			КонецЕсли;
		Иначе
			ТекстИсключения = НСтр("ru='Для этого типа метаданных не поддерживается анализ в функции ОбновлениеИнформационнойБазы.МетаданныеИОтборПоДанным.';uk='Для цього типу метаданих не підтримується аналіз в функції ОбновлениеИнформационнойБазы.МетаданныеИОтборПоДанным.'");
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
...
Процедура ОтметитьКОбработке(ОсновныеПараметры, Данные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ДополнительныеПараметры.ЭтоДвижения Тогда
		...
		Если ДополнительныеПараметры.ОтметитьВсеРегистраторы Тогда
			...
			Иначе
				ТекстИсключения = НСтр("ru='Для регистрации всех регистраторов регистра необходимо в параметре ""Данные"" передать ОбъектМетаданных:Документ или ДокументСсылка.';uk='Для реєстрації всіх реєстраторів регістра необхідно в параметрі ""Дані"" передати ОбъектМетаданных: Документ або ДокументСсылка.'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
	...
	Иначе
		...
		Иначе
			Если ТипЗнч(Данные) = Тип("ОбъектМетаданных") Тогда
				ТекстИсключения = НСтр("ru='Не поддерживается регистрация к обновлению целиком объекта метаданных. Нужно обновлять конкретные данные.';uk='Не підтримується реєстрація до оновлення цілком об`єкту метаданих. Треба оновлювати конкретні дані.'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
...
Функция ЕстьДанныеДляОбработки(Очередь, ПолноеИмяМетаданныеОбъекта, Отбор = Неопределено) Экспорт
	...
	Иначе
		ТекстИсключения = НСтр("ru='Передан неправильный тип параметра ""ПолноеИмяМетаданныеОбъекта"" в функцию ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки';uk='Переданий неправильний тип параметра ""ПолноеИмяМетаданныеОбъекта"" у функцію ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;	
	...
	Для каждого ОбрабатываемыйТип Из ПолныеИменаОбрабатываемыхОбъектов Цикл 
		...
		Иначе
			ТекстИсключения = НСтр("ru='Для типа метаданных ""%МетаданныеОбъекта%"" не поддерживается проверка в функции ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки';uk='Для типу метаданих ""%МетаданныеОбъекта%"" не підтримується перевірка у функції ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки'");
...
Функция СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияСсылок(Очередь, ПолныеИменаОбъектов, МенеджерВременныхТаблиц, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Иначе	
		...
		Для Каждого ОбрабатываемыйТип Из ПолныеИменаОбъектовМассив Цикл
			...
			Иначе
				ТекстИсключения = НСтр("ru='Для типа метаданных ""%МетаданныеОбъекта%"" не поддерживается проверка в функции ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияСсылок';uk='Для типу метаданих ""%МетаданныеОбъекта%"" не підтримується перевірка у функції ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияСсылок'");
				ТекстИсключения = СтрЗаменить(ТекстИсключения, "%МетаданныеОбъекта%", Строка(МетаданныеОбъекта)); 
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
...
Процедура ДобавитьПроверкуБлокировкиДополнительныхИсточников(Очередь, ТекстЗапроса, ПолноеИмяОбъекта, ПолноеИмяРегистра, МенеджерВременныхТаблиц, ЭтоСозданиеВременнойТаблицы, ДополнительныеПараметры)
	...
	Иначе	
		Если ПолноеИмяОбъекта = Неопределено Тогда
			ТекстИсключения = НСтр("ru='Ошибка вызова функции %ИмяФункции%: не передано имя документа, но переданы дополнительные источники данных.';uk='Помилка виклику функції %ИмяФункции%: не передано ім`я документу, але передані додаткові джерела даних.'");
