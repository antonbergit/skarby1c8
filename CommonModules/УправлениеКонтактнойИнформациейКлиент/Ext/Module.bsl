...
Процедура СоздатьЭлектронноеПисьмо(Знач ЗначенияПолей, Знач Представление = "", ОжидаемыйВид = Неопределено) Экспорт
	...
	Если ЗначенияПолей = "" И ПустаяСтрока(Представление) Тогда
		ПоказатьПредупреждение(,НСтр("ru='Для отправки письма необходимо ввести адрес электронной почты.';uk='Для відправлення листа необхідно ввести адресу електронної пошти.'"));
		Возврат;
	КонецЕсли;
...
Процедура СоздатьSMS(Знач ЗначенияПолей, Знач Представление = "", ОжидаемыйВид = Неопределено, ИсточникКонтактнойИнформации = "") Экспорт
	...
	Если ТипИнформации <> ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Нельзя отправитьSMS по контактной информацию с типом ""%1""';uk='Не можна відправити SMS по контактній інформації з типом ""%1""'"), ТипИнформации);
	КонецЕсли;
...
Процедура ЗаполнитьАдрес(Знач Форма, Знач ИмяРеквизита, Знач НайденнаяСтрока, Знач Результат)
	...
	Если АдресаДляЗаполнения.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Для того чтобы не вводить один и тот же адрес несколько раз, можно скопировать в это поле значение, ранее введенное в соседнем поле.';uk='Для того щоб не вводити ту ж саму адресу кілька раз, можна скопіювати в це поле значення, раніше введене в сусідньому полі.'")
