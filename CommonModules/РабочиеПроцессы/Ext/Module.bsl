...
Процедура ОбработатьПравилаРабочихПроцессов(ДанныеКОбработке) Экспорт
	...
	ТекстСообщения = СтрШаблон(
		НСтр("ru='Старт обработки правил рабочих процессов. Количество источников к обработке: %1';uk='Старт обробки правил робочих процесів. Кількість джерел до обробки: %1'"),
...
"для источника: ""%2""';uk='Старт виконання дій за правилом робочого процесу: ""%1"","
"для джерела: ""%2""'"), Выборка.Правило, Выборка.Источник);
			ЗаписьЖурналаРегистрации(РабочиеПроцессы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Примечание,
...
"для источника: ""%2""';uk='Завершення виконання дій за правилом робочого процесу: ""%1"","
"для джерела: ""%2""'"), Выборка.Правило, Выборка.Источник);
			ЗаписьЖурналаРегистрации(РабочиеПроцессы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Примечание,
				Метаданные.Справочники.ПравилаРабочегоПроцесса, Выборка.Правило, ТекстСообщения);
	...
	ТекстСообщения = НСтр("ru='Завершение обработки правил рабочих процессов';uk='Завершення обробки правил робочих процесів'");
	ЗаписьЖурналаРегистрации(РабочиеПроцессы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Примечание,
		Метаданные.Справочники.ПравилаРабочегоПроцесса, , ТекстСообщения);
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru='Правила рабочего процесса';uk='Правила робочого процесу'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
