...
"Для корректной работы устройства необходимо задать параметры его работы.';uk='Не налаштовані параметри пристрою."
...
Функция ВыгрузитьТовары(Параметры, Товары, ЧастичнаяВыгрузка, ВыходныеПараметры, РасширеннаяВыгрузка) 
	...
	Если ЧастичнаяВыгрузка И НЕ МожноВыгружатьТовары(Параметры) Тогда
		...
		ВыходныеПараметры.Добавить(999);
		ОписаниеОшибки = НСтр("ru='Нельзя сделать выгрузку. Товары предыдущей выгрузки еще не были получены ККМ-offline';uk='Не можна зробити вивантаження. Товари попереднього вивантаження ще не були отримані ККМ-offline'");
	...
	Если Результат Тогда
		...
		Исключение
			...
			ВыходныеПараметры.Добавить(999);
			ОписаниеОшибки = НСтр("ru='Не удалось записать файл товаров по адресу: %Адрес%';uk='Не вдалося записати файл товарів за адресою: %Адреса%'");
...
Функция ЗагрузитьОтчет(Параметры, ВыходныеПараметры) 
	...
	Исключение
		ВыходныеПараметры.Добавить(999);
		ОписаниеОшибки = НСтр("ru='Не удалось прочитать файл отчета по адресу: %Адрес%';uk='Не вдалося прочитати файл звіту за адресою: %Адреса%'");
...
Функция ОтчетЗагружен(Параметры, ВыходныеПараметры) 
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='При пометке отчета %1 произошла ошибка. Обработка отчета не зафиксирована!';uk='При пометке отчета %1 произошла ошибка. Обработка отчета не зафиксирована!'");
...
Функция ОчиститьТоварыНаККМ(Параметры, ВыходныеПараметры) 
	...
	Файл.ДобавитьСтроку("##@@&&");
	Файл.ДобавитьСтроку("#");
	Файл.ДобавитьСтроку("$$$CLR {COM_GR_TOV}");
...
Функция ТестУстройства(Параметры, ВыходныеПараметры) 
	...
	Если ПустаяСтрока(ВремПараметр) Тогда
		Результат = Ложь;
		ТекстОшибкиОбщий = НСтр("ru='Файл базы товаров не указан.';uk='Файл бази товарів не зазначений.'");
	КонецЕсли;
	...
	Если ПустаяСтрока(ВремПараметр) Тогда
		...
		ТекстОшибкиОбщий = ТекстОшибкиОбщий + НСтр("ru='Файл отчета не указан.';uk='Файл звіту не зазначений.'") 
	КонецЕсли;
...
Процедура ПроверкаОбработанностиФайлаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Успешно Тогда
		...
		Если СтрДлина(Строка) = 1 И Найти(Строка,"#") > 0 Тогда
			...
			ОписаниеОповещения.ДополнительныеПараметры.ВыходныеПараметры.Добавить(НСтр("ru='Нельзя сделать выгрузку. Товары предыдущей выгрузки еще не были получены ККМ-offline';uk='Не можна зробити вивантаження. Товари попереднього вивантаження ще не були отримані ККМ-offline'"));
...
Процедура ВыгрузкаНастроекЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Исключение
			ТекстОшибки = НСтр("ru='Не удалось записать файл товаров по адресу: %Адрес%';uk='Не вдалося записати файл товарів за адресою: %Адреса%'");
...
Процедура ПроверкаСуществованияФайлаПриЗагрузкеОтчетаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ВыходныеПараметры.Добавить(999);
		ОписаниеОшибки = НСтр("ru='Файл %Адрес% не существует!';uk='Файл %Адрес% не существует!'");
		ВыходныеПараметры.Добавить(СтрЗаменить(ОписаниеОшибки, "%Адрес%", ИмяФайла));
...
Функция ЗаполнитьТранзакцию(Строка, Транзакция, ТекстОшибки)
	...
	Попытка
		...
		ПолеОшибки = Нстр("ru='Номер транзакции (1)';uk='Номер транзакції (1)'");
		
		// Номер транзакции.
		Транзакция.Вставить("Номер", Число(СтрПолучитьСтроку(Строка, 1)));
		
		ПолеОшибки = Нстр("ru='Дата и время транзакции (2,3)';uk='Дата и время транзакции (2,3)'");
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru='Неверный формат файла. Невозможно распознать поле: %1%';uk='Неверный формат файла. Невозможно распознать поле: %1%'");
...
Функция УдалитьТранзакциюИзЧека(Смена, Транзакция, ТекстОшибки)
	...
	Иначе
		...
		Если НЕ Результат Тогда
			ТекстОшибки = НСтр("ru='Операция прервана. Ошибка при загрузке транзакции №%1%!';uk='Операция прервана. Ошибка при загрузке транзакции №%1%!'");
...
Функция ЗакрытьЧек(Смена, НомерЧека, ТекстОшибки)
	...
	Если ТекущийЧек = Неопределено Тогда
		ТекстОшибки = НСтр("ru='Неверный формат файла. Невозможно найти чек №%1%';uk='Неверный формат файла. Невозможно найти чек №%1%'");
...
Функция РазобратьРасширенныйОтчет(ТекстовыйДокумент, ВыходныеПараметры)
	...
	ИначеЕсли Строка = "@" Тогда
		ТекстОшибки = НСтр("ru='Операция прервана! Отчет уже был загружен.';uk='Операция прервана! Отчет уже был загружен.'");
		Результат = Ложь;
	Иначе
		ТекстОшибки = НСтр("ru='Загрузка невозможна! Неизвестный формат отчета.';uk='Загрузка невозможна! Неизвестный формат отчета.'");
		Результат = Ложь;
	КонецЕсли;
	...
	Если Строка = "#" Тогда
		...
		Пока Истина Цикл
			...
			Попытка
				...
				ПолеОшибки = Нстр("ru='Дата транзакции (2,3)';uk='Дата транзакції (2,3)'");
				...
				ПолеОшибки = Нстр("ru='Номер ККМ (5)';uk='Номер ККМ (5)'");
				НомерККМ        = Число(НомерККМСтр);
				ПолеОшибки = Нстр("ru='Номер чека (6)';uk='Номер чека (6)'");
				НомерЧека       = Число(НомерЧекаСтр);
			Исключение
				ВыходныеПараметры.Добавить(999);
				ВыходныеПараметры.Добавить(НСтр("ru='Неверный формат файла. Невозможно распознать поле:';uk='Невірний формат файлу. Неможливо розпізнати поле:'") + Символы.НПП + ПолеОшибки);
				Результат = Ложь;
				Прервать;
			КонецПопытки;
			...
			Если ТипТранзакции = 1 ИЛИ ТипТранзакции = 11 Тогда
				...
				Попытка
					...
					ПолеОшибки = Нстр("ru='Код товара (8)';uk='Код товару (8)'");
					...
					ПолеОшибки = Нстр("ru='Секция (9)';uk='Секція (9)'");
					Секция     = Число(СекцияСтр);
					ПолеОшибки = Нстр("ru='Цена товара (10)';uk='Ціна товару (10)'");
					Цена       = Число(ЦенаСтр);
					ПолеОшибки = Нстр("ru='Количество товара (11)';uk='Кількість товару (11)'");
					Количество = Число(КоличествоСтр);
					ПолеОшибки = Нстр("ru='Сумма (12)';uk='Сума (12)'");
			...
			ИначеЕсли ТипТранзакции = 2 Тогда
				ВыходныеПараметры.Добавить(999);
				ВыходныеПараметры.Добавить(НСтр("ru='Ошибка сторно';uk='Помилка сторно'"));
				Результат = Ложь;
				Прервать;
			...
			ИначеЕсли ТипТранзакции = 12 Тогда
				...
				Исключение
					ВыходныеПараметры.Добавить(999);
					ВыходныеПараметры.Добавить(НСтр("ru='Неверный формат файла. Невозможно преобразовать к числу поле:';uk='Невірний формат файлу. Неможливо перетворити до числа поле:'") + Символы.НПП + ПолеОшибки);
					Результат = Ложь;
					Прервать;
				КонецПопытки;
			...
			ИначеЕсли ТипТранзакции = 3 Или ТипТранзакции = 4 Тогда
				ВыходныеПараметры.Добавить(999);
				ВыходныеПараметры.Добавить(НСтр("ru='Ошибка возврата';uk='Помилка повернення'"));
				Результат = Ложь;
				Прервать;
			...
			ИначеЕсли ТипТранзакции = 15 Или ТипТранзакции = 17 Тогда
				...
				Попытка
					...
					ПолеОшибки = Нстр("ru='Секция (9)';uk='Секція (9)'");
					Секция     = Число(СекцияСтр);
					ПолеОшибки = Нстр("ru='Скидка (11)';uk='Знижка (11)'");
				...
				Иначе
					ВыходныеПараметры.Добавить(999);
					ВыходныеПараметры.Добавить(НСтр("ru='Ошибка итоговой скидки на позицию';uk='Помилка підсумкової знижки на позицію'"));
					Результат = Ложь;
					Прервать;
				КонецЕсли;
			...
			ИначеЕсли ТипТранзакции = 16 Или ТипТранзакции = 18 Тогда
				...
				Попытка
					...
					ПолеОшибки = Нстр("ru='Секция (9)';uk='Секція (9)'");
					Секция     = Число(СекцияСтр);
					ПолеОшибки = Нстр("ru='Надбавка (11)';uk='Надбавка (11)'");
				...
				Иначе
					ВыходныеПараметры.Добавить(999);
					ВыходныеПараметры.Добавить(НСтр("ru='Ошибка итоговой надбавки на позицию';uk='Помилка підсумкової надбавки на позицію'"));
					Результат = Ложь;
					Прервать;
				КонецЕсли;
			...
			ИначеЕсли ТипТранзакции = 80 Тогда
				...
				Попытка
					ПолеОшибки = Нстр("ru='Транзакция позиции (11)';uk='Транзакція позиції (11)'");
					ТранзакцияПозиции      = Число(ТранзакцияПозицииСтр);
					ПолеОшибки = Нстр("ru='Номер возвращаемого чека (12)';uk='Номер чека, що вертається (12)'");
				...
				Иначе
					ВыходныеПараметры.Добавить(999);
					ВыходныеПараметры.Добавить(НСтр("ru='Ошибка возврата по номеру чека';uk='Помилка повернення по номеру чека'"));
					Результат = Ложь;
					Прервать;
				КонецЕсли;
			...
			ИначеЕсли ТипТранзакции = 35 Тогда
				...
				Попытка
					ПолеОшибки  = Нстр("ru='Сумма скидки (11)';uk='Сума знижки (11)'");
			...
			ИначеЕсли ТипТранзакции = 36 Тогда
				...
				Попытка
					ПолеОшибки    = Нстр("ru='Сумма надбавки (11)';uk='Сума надбавки (11)'");
			...
			ИначеЕсли ТипТранзакции = 37 Тогда
				...
				Попытка
					ПолеОшибки = Нстр("ru='Процент скидки (11)';uk='Відсоток знижки (11)'");
			...
			ИначеЕсли ТипТранзакции = 38 Тогда
				...
				Попытка
					ПолеОшибки = Нстр("ru='Процент надбавки (11)';uk='Відсоток надбавки (11)'");
			...
			Иначе
				ОписаниеОшибки = НСтр("ru='Обнаружена неизвестная транзакция: %ТипТранзакции%. Данные по транзакции не были загружены!';uk='Виявлена невідома транзакція: %ТипТранзакции%. Дані по транзакції не були завантажені!'");
		...
		Если НеизвестнаяТранзакция Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не все данные были загружены из отчета. Обратитесь к администратору системы!';uk='Не всі дані були завантажені зі звіту. Зверніться до адміністратора системи!'"));
		КонецЕсли;
		
	ИначеЕсли Строка = "@" Тогда
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(НСтр("ru='Операция прервана. Отчет уже был загружен!';uk='Операція перервана. Звіт вже був завантажений!'"));
		Результат = Ложь;
	Иначе
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(НСтр("ru='Неверный формат данных или данные отсутствуют.';uk='Невірний формат даних або дані відсутні.'"));
		Результат = Ложь;
	КонецЕсли;
...
Функция СформироватьФайлНастроек(СтруктураНастроек, ТекстовыйФайл, ТекстОшибки)
	...
	Если Налоги.Количество() = 0 Тогда
		ТекстОшибки = НСтр("ru='Выгрузка настроек невозможна. Список ставок налога пуст!';uk='Выгрузка настроек невозможна. Список ставок налога пуст!'");
		Результат = Ложь;
	Иначе
		...
		Если КомбинацииНалогов.Количество() = 0 Тогда
			...
			Если Налоги.Количество() > 1 Тогда
				ТекстОшибки = НСтр("ru='Выгрузка настроек невозможна. На товары начисляется несколько налогов, необходимо заполнить комбинации налогов!';uk='Выгрузка настроек невозможна. На товары начисляется несколько налогов, необходимо заполнить комбинации налогов!'");
				Результат = Ложь;
			Иначе
...
Функция ДополнитьФайлОчисткойНастроек(ТекстовыйФайл, ТекстОшибки)
	...
	Если ТекстовыйФайл.КоличествоСтрок()>0 Тогда
		...
		Строка3 = ТекстовыйФайл.ПолучитьСтроку(3);
		
		Если Найти(Строка3, "{NAL} {GR_NL}")<>0 Тогда
			Возврат Истина;
		Иначе
			Строка3 = Строка3 + " {NAL} {GR_NL}";
		КонецЕсли;
		
		ТекстовыйФайл.ЗаменитьСтроку(3, Строка3);
...
Процедура ДополнитьФайл(Результат, Файл, Параметры, БлокДополнительныхАтрибутов = Неопределено, ВыходныеПараметры, Знач ДанныеДляЗаполнения = Неопределено)
	...
	Если Уровень = "Номенклатура" Тогда
		...
		Иначе
			...
			ИначеЕсли ТипЗнч(Источник.Штрихкод)=Тип("Массив") И Источник.Штрихкод.Количество()=0 Тогда
				Штрихкоды.Добавить("");
			КонецЕсли;
