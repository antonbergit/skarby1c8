...
Процедура ОповеститьОНовыхДокументахЭДО() Экспорт
	...
	НавигационнаяСсылка = НавигационнаяСсылкаТекущиеДелаЭДО();
		ЗаголовокОповещения = НСтр("ru='Сервис ЭДО';uk='Сервис ЭДО'");
		ТекстОповещения = НСтр("ru='Получить новые документы';uk='Получить новые документы'");
...
"Записать и провести?';uk='записать и провести."
"Записать и провести?'"), НСтр("ru='записать."
"Записать?';uk='записать."
"Записать?'"));
				
				ШаблонСообщения = НСтр("ru='Документ изменен. Для формирования электронного документа его необходимо %1';uk='Документ изменен. Для формирования электронного документа его необходимо %1'");
				...
				ПоказатьВопрос( Обработчик, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, , КодВозвратаДиалога.Отмена,
					НСтр("ru='Документ изменен.';uk='Документ изменен.'"));
...
Процедура ЗаписатьВФорме(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.ОК Тогда
		...
		Если Проведен Тогда
			...
			Исключение
				ПоказатьПредупреждение(, НСтр("ru='Операция не выполнена.';uk='Операция не выполнена.'"));
				Отказ = Истина;
			КонецПопытки;
		Иначе
...
Процедура ОткрытьАктуальныйЭД(ПараметрКоманды, Источник = Неопределено, ПараметрыОткрытия = Неопределено) Экспорт
	...
	Для Каждого ТекЭл Из МассивСсылок Цикл
		...
		Иначе
			ТекстШаблона = НСтр("ru='%1. Электронный документ не найден.';uk='%1. Электронный документ не найден.'");
...
Процедура ЗакрытьПринудительноЭДО(МассивВладельцевЭД) Экспорт
	...
	ПричинаЗакрытия = "";
	ПоказатьВводСтроки(Обработчик, ПричинаЗакрытия, НСтр("ru='Укажите причину закрытия документооборота';uk='Укажите причину закрытия документооборота'"),,Истина);
...
"Операция возможна только для непроведенных документов.';uk='Перезаполнение документа %1."
"Операция возможна только для непроведенных документов.'");
	...
	Если СоответствиеВладельцевИЭД.Количество() = 0 Тогда
		Для Каждого ТекущийДокумент Из МассивСсылок Цикл
			Шаблон = НСтр("ru='Электронный документ для %1 не найден';uk='Электронный документ для %1 не найден'");
	...
	Если МассивИзмененныхВладельцев.Количество() > 0 Тогда
		...
		Если МассивИзмененныхВладельцев.Количество() = 1 Тогда
			ТекстСостоянияВывод = НСтр("ru='Документ перезаполнен.';uk='Документ перезаполнен.'");
		Иначе
			ТекстСостоянияВывод = НСтр("ru='Документы перезаполнены (%1).';uk='Документы перезаполнены (%1).'");
...
"Выберите файл с расширением ""zip"" или ""xml"".';uk='Не корректный формат файла."
...
Процедура ЗакрытьПринудительноРезультатВводаСтроки(ПричинаЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПричинаЗакрытия) Тогда
		...
		ТекстСообщения = НСтр("ru='Для закрытия документооборота по выбранным ЭД необходимо указать причину закрытия.';uk='Для закрытия документооборота по выбранным ЭД необходимо указать причину закрытия.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	...
	ТекстОповещения = НСтр("ru='Изменено состояние ЭД документов на ""Закрыт принудительно"": (%1)';uk='Изменено состояние ЭД документов на ""Закрыт принудительно"": (%1)'");
	ТекстОповещения = СтрЗаменить(ТекстОповещения, "%1", КоличествоОбработанныхЭД);
	ПоказатьОповещениеПользователя(НСтр("ru='Обработка документов';uk='Обработка документов'"), , ТекстОповещения);
...
"Обратитесь к администратору.';uk='Недостаточно прав для настройки ЭДО с контрагентом."
...
Процедура ОбновитьКартинкуЭДО(Форма, КартинкаЭДО, МестоРазмещенияКоманд)
	...
	Если Не КартинкаЭДО = БиблиотекаКартинок.ЭмблемаСервиса1СЭДО Тогда
		ПодменюЭДО.Подсказка = НСтр("ru='В сервисе 1С-ЭДО есть новые документы';uk='В сервисе 1С-ЭДО есть новые документы'");
	Иначе
		ПодменюЭДО.Подсказка = НСтр("ru='Команды ЭДО';uk='Команды ЭДО'");
	КонецЕсли;
