...
Процедура ПодготовитьДанныеОтчетаПоШаблону(КонтекстФормы, ДанныеШаблонаОтчета, Отказ = Ложь) Экспорт 
	...
	Исключение
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось получить текст XML-шаблона:%1';uk='Не удалось получить текст XML-шаблона:%1'"), Символы.ПС + ОписаниеОшибки());
		Сообщение.Сообщить();
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете(ПараметрыДляПроверки) Экспорт 
	...
	Если ПараметрыДляПроверки.Свойство("ПредидущийПериод") Тогда 
		...
		Если ЗначениеЗаполнено(Док) Тогда 
			ВыгрузкаПредидущегоПериода = ТекстВыгрузкиОтчетаСтатистики(Док);
	...
	Если ПараметрыДляПроверки.Свойство("НеобходимоПолучениеВыгрузкиТекущегоПериода") Тогда 
		АдресВоВремХранилище = ТекстВыгрузкиОтчетаСтатистики(ПараметрыДляПроверки.СтруктураРеквизитовФормы.мСохраненныйДок);
	...
	Для каждого КонтрольноеСоотношение Из ПараметрыДляПроверки.ТаблицаКонтрольныхСоотношений Цикл
		...
		СтруктураОшибок.Вставить("Описание" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1';uk='%1'"), ""));
		СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1';uk='%1'"), ВерсияШаблона));
		...
		СтруктураОшибок.Вставить("ТекстовоеПредставлениеФормулы" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1';uk='%1'"), Описание));
		Если ПараметрыДляПроверки.КвоОшибокКС > 0 Тогда
			Стр1 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1';uk='%1'"), "Не удалось вычислить условие! Соотношение не используется.");
			Стр2 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1';uk='%1'"), ?(ОбязательностьКонтроля, "", "Контролировать не обязательно"));
...
Функция ПолучитьШаблон(ИмяФормы, ИмяМакетаШаблона) Экспорт 
	...
	ДД = Отчеты[Сред(Лев(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(ИмяМакетаШаблона);
...
Функция ТекстВыгрузкиОтчетаСтатистики(мСохраненныйДок)
