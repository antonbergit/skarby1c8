...
Процедура ВыполнитьКомандуСервиса(ИмяКоманды, ПараметрыКоманды, Результат, Отказ) Экспорт
	...
	ПараметрыСоединения = Новый Структура(СтандартныеПодсистемыСервер.ПараметрыКлиентаНаСервере().Получить("БизнесСетьПараметрыСоединения"));
	...
	Исключение
		ТекстОшибки = нСтр("ru='Отсутствует соединение с сервером';uk='Отсутствует соединение с сервером'");
	...
	// Соединение по внешнему временному ключу, может использоваться для отладки.
	КлючСоединения = "";
	Если Не ПараметрыКоманды.Свойство("ОтладочныйТокен", КлючСоединения) Тогда
	...
	Попытка
		...
		Иначе
			ВызватьИсключение НСтр("ru='Ошибка выполнения команды сервиса.';uk='Ошибка выполнения команды сервиса.'");
		КонецЕсли; 
	Исключение
		ТекстСообщения = НСтр("ru='Ошибка выполнения команды сервиса:';uk='Ошибка выполнения команды сервиса:'") + " " + ИнформацияОбОшибке().Описание;
	...
	ТекстСообщения = "";
	Если Результат.КодСостояния = 403 ИЛИ Результат.КодСостояния = 400
		ИЛИ (Результат.КодСостояния = 404 И ИмяКоманды = "СписокВходящихДокументов") Тогда
		ТекстСообщения = НСтр("ru='Ошибка выполнения команды сервиса (%1).';uk='Ошибка выполнения команды сервиса (%1).'");
	...
	Если Отказ И Не ПараметрыКоманды.Свойство("БлокироватьСообщенияОбОшибках") Тогда
		Если ТекстСообщения = "" Тогда
			ТекстСообщения = НСтр("ru='Ошибка выполнения команды в сервисе 1С:Бизнес-сеть.';uk='Ошибка выполнения команды в сервисе 1С:Бизнес-сеть.'");
		КонецЕсли;
		ПодробныйТекстОшибки = ТекстСообщения + Символы.ПС + СтрШаблон(НСтр("ru='Адрес: %1.';uk='Адрес: %1.'"), ПараметрыВызова.Адрес);
		Если ТипЗнч(ОбъектJSON) = Тип("Структура") Тогда
			...
			Если Не ПустаяСтрока(ТекстОшибкиСервиса) Тогда
				ПодробныйТекстОшибки = ПодробныйТекстОшибки	+ Символы.ПС + СтрШаблон(НСтр("ru='Подробности: %1.';uk='Подробности: %1.'"), ТекстОшибкиСервиса);
			КонецЕсли; 
			
		КонецЕсли;
		ВидОперации = СтрШаблон(НСтр("ru='Команда ""%1""';uk='Команда ""%1""'"), ИмяКоманды);
...
Функция ВидыДокументовСервиса() Экспорт
	...
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.АктИсполнитель,    НСтр("ru='Акт выполненных работ';uk='Акт виконаних робіт'"));
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.АктНаПередачуПрав, НСтр("ru='Акт передачи прав';uk='Акт передачи прав'"));
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.ОтветНаЗаказ,      НСтр("ru='Заказ покупателя';uk='Замовлення покупця'"));
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.ЗаказТовара,       НСтр("ru='Заказ поставщику';uk='Замовлення постачальникові'"));
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель,
		НСтр("ru='Корректировка стоимости';uk='Корректировка стоимости'"));
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.ТОРГ12Продавец,    НСтр("ru='Товарная накладная';uk='Товарная накладная'"));
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.ОтчетОПродажахКомиссионногоТовара,
		НСтр("ru='Отчет о продажах комиссионного товара';uk='Отчет о продажах комиссионного товара'"));
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.ОтчетОСписанииКомиссионногоТовара,
		НСтр("ru='Отчет о списании комиссионного товара';uk='Отчет о списании комиссионного товара'"));
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.ПрайсЛист,         НСтр("ru='Прайс-лист';uk='Прайс-лист'"));
	СписокВидовЭД.Добавить(Перечисления.ВидыЭД.СчетНаОплату,      НСтр("ru='Счет на оплату';uk='Рахунок на оплату'"));
...
Функция ОрганизацияПодключена(Организация) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	Организации1СБизнесСеть.Организация
	|ИЗ
...
Процедура ПодключитьОрганизации(МассивОрганизаций, Отказ, ПараметрыКоманды, ТребуетсяОбновитьИнтерфейс) Экспорт
	...
	|	РегистрСведений.Организации1СБизнесСеть КАК Организации1СБизнесСеть
	|ГДЕ
	|	Организации1СБизнесСеть.Организация = &Ссылка_Справочник_Организации";
	...
	Если ДанныеСервиса = Неопределено ИЛИ ДанныеСервиса = Ложь ИЛИ (ТекущаяИБПодключена = Ложь И ДанныеСервиса = Истина) Тогда
	...
	Иначе
		...
		Если ТипЗнч(ДанныеСервиса) = Тип("Массив") Тогда
			Если ДанныеСервиса.Количество() = 0 Тогда
				Отказ = Истина;
				ВызватьИсключение НСтр("ru='Абонент сервиса 1С:Бизнес-сеть не найден. Требуется повторное подключение организации к сервису.';uk='Абонент сервиса 1С:Бизнес-сеть не найден. Требуется повторное подключение организации к сервису.'");
			Иначе
	...
	Если ТекущийАбонент = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка получения администратора абонента.';uk='Ошибка получения администратора абонента.'"),,,,Отказ);
		Возврат;
	КонецЕсли;
	...
	Для каждого ОрганизацияМассива Из МассивОрганизаций Цикл
		...
		Если Результат.КодСостояния = 200 И ТипЗнч(ДанныеСервиса) = Тип("Массив") И ДанныеСервиса[0] = Истина Тогда
			...
			Если Результат.КодСостояния <> 200 Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ru='Ошибка подключения организации ""%1"".';uk='Ошибка подключения организации ""%1"".'");
...
"свяжитесь со службой технической поддержки.';uk='Возможно, организация уже подключена под другой учетной записью интернет-поддержки."
"Если Вы являетесь представителем организации, для подключения организации в программе"
"свяжитесь со службой технической поддержки.'");
				ВидОперации = НСтр("ru='Подключение организации';uk='Подключение организации'");
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробныйТекстОшибки, ТекстСообщения, 5, ОрганизацияМассива);
			КонецЕсли;
		КонецЕсли;
...
Процедура ОтключитьОрганизации(МассивОрганизаций, РежимОтключенияВСервисе, Отказ, ТребуетсяОбновитьИнтерфейс) Экспорт
	...
	ПроверитьСсылкиОтключения(МассивОрганизаций, Отказ);
...
//
Процедура СформироватьПараметрыВызоваКомандыСервиса(ИмяКоманды, ПараметрыКоманды, ПараметрыВызова, ПараметрыСоединения, Отказ)
	...
	Если ИмяКоманды = "ПолучениеКлюча" Тогда
		...
		ИначеЕсли ПараметрыКоманды.Права = "bn_user" Тогда 
			...
			Иначе
				ТекстСообщения = НСтр("ru='Текущий пользователь не зарегистрирован в сервисе 1С:Бизнес-сеть';uk='Текущий пользователь не зарегистрирован в сервисе 1С:Бизнес-сеть'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,,, Отказ);
			КонецЕсли;
	...
	ИначеЕсли ИмяКоманды = "ПолучитьРеквизитыУчастника" Тогда
		...
		Иначе
			ПараметрыОрганизации = Новый Структура("ИНН", ПараметрыКоманды.ИНН);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ПараметрыОрганизации.ИНН) Тогда
			Если ПараметрыКоманды.Свойство("Ссылка") И ЗначениеЗаполнено(ПараметрыКоманды.Ссылка) Тогда
				ТестСообщения = НСтр("ru='Не заполнено свойство ""ИНН"" для ""%1"".';uk='Не заполнено свойство ""ИНН"" для ""%1"".'");
				ТестСообщения = СтрШаблон(ТестСообщения, ПараметрыКоманды.Ссылка);
			Иначе
				ТестСообщения = НСтр("ru='Не заполнено свойство ""ИНН"".';uk='Не заполнено свойство ""ИНН"".'");
			КонецЕсли;
	...
	ИначеЕсли ИмяКоманды = "СписокПользователейАбонента" Тогда
		...
		ПараметрыВызова.Адрес = "api/core/v1/subscribers/" + Формат(ПараметрыКоманды.Абонент, "ЧГ=") + "/subscriberMembers/";
		ПараметрыВызова.Метод = "get";
		ПараметрыВызова.Права = "bn_user";
		
	ИначеЕсли ИмяКоманды = "ПодключениеОрганизации" Тогда
		
		ПараметрыВызова.Адрес = "api/core/v1/subscribers/" + Формат(ПараметрыКоманды.Абонент, "ЧГ=") + "/organizations";
		ПараметрыВызова.Метод = "post";
	...
	ИначеЕсли ИмяКоманды = "ПроверкаОрганизации" Тогда
		...
		СтруктураТела = Новый Структура("inn", 
	...
	ИначеЕсли ИмяКоманды = "СписокПользователейОрганизации" Тогда
		...
		ПараметрыАбонента = ЭлектронноеВзаимодействиеПереопределяемый.ПолучитьДанныеЮрФизЛица(ПараметрыКоманды.Организация);
		ПараметрыВызова.Адрес = "api/core/v1/organizations/"
			+ БизнесСетьКлиентСервер.ФорматИдентификатора(ПараметрыАбонента.ИНН) + "/organizationMembers";
		ПараметрыВызова.Метод = "get";
		ПараметрыВызова.Права = "bn_user";
	...
	ИначеЕсли ИмяКоманды = "ОтправкаПриглашения" Тогда
		...
		Если Не ЗначениеЗаполнено(ПараметрыКоманды.ЭлектроннаяПочта) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указана адрес электронной почты контрагента';uk='Не указана адрес электронной почты контрагента'")
				+ " " + ПараметрыКоманды.Контрагент,,,, Отказ);
			Возврат;
		КонецЕсли;
		ПараметрыВызова.Адрес = "api/core/v1/organizations/"
			+ БизнесСетьКлиентСервер.ФорматИдентификатора(ПараметрыКонтрагента.ИНН) + "/invitation"
	...
	ИначеЕсли ИмяКоманды = "ПроверкаСтатусаДокумента" Тогда
		...
		ПараметрыВызова.Адрес = "api/core/v1/documents/" + Формат(ПараметрыКоманды.Номер, "ЧГ=") + "/status";
		ПараметрыВызова.Метод = "get";
		ПараметрыВызова.Права = "bn_user";
	...
	ИначеЕсли ИмяКоманды = "СписокВходящихДокументов" Тогда
		...
		Если ПараметрыКоманды.Свойство("МассивОрганизаций") Тогда
			...
			РеквизитыОрганизации = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(ПараметрыКоманды.МассивОрганизаций, "ИНН");
			...
			Если ПараметрыКоманды.Свойство("ОтборКонтрагент") Тогда
				РеквизитыКонтрагента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПараметрыКоманды.ОтборКонтрагент, "ИНН, КПП");
		...
		Иначе
			...
			ПараметрыВызова.Адрес = "api/edi/v1/organizations/"
				+ БизнесСетьКлиентСервер.ФорматИдентификатора(ПараметрыАбонента.ИНН) + "/documents?fromTime="
	...
	ИначеЕсли ИмяКоманды = "ОбновитьСтатус" Тогда
		...
		Если ПараметрыКоманды.Свойство("МассивДанных") Тогда
			...
			Для каждого ЭлементМассива Из ПараметрыКоманды.МассивДанных Цикл
				...
				СтруктураСтатуса.Вставить("metaData", НСтр("ru='Дата загрузки:';uk='Дата загрузки:'") + ТекущаяДатаСеанса());
	...
	ИначеЕсли ИмяКоманды = "ПолучитьСписокКатегорийПервогоУровня" Тогда
	...
	ИначеЕсли ИмяКоманды = "ПоискКатегорий" Тогда
		...
		ПараметрыВызова.Адрес = "api/classifier/v1/categories/search?search=" + ПараметрыКоманды.СтрокаПоиска;
		ПараметрыВызова.Метод = "get";
		ПараметрыВызова.Права = "bn_user";
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка инициализации сервиса';uk='Ошибка инициализации сервиса'"),,,, Отказ);
...
Процедура ПолучитьКлюч(Соединение, ПараметрыСоединения, ПараметрыКоманды, ПараметрыВызова, СохранятьПараметрыСеанса, Отказ)
	...
	Если СтруктураКлюча = Неопределено ИЛИ ТекущаяДатаСеанса() > СтруктураКлюча.СрокЖизни Тогда
		...
		СформироватьПараметрыВызоваКомандыСервиса("ПолучениеКлюча", ПараметрыКоманды, Аутентификация, ПараметрыСоединения, Отказ);
...
Процедура ПолучитьНовыйКлюч(Соединение, ПараметрыСоединения, Аутентификация, Отказ)
	...
	Если СтруктураКлюча <> Неопределено Тогда
		...
		Попытка
			...
			HTTPОтвет = Соединение.ОтправитьДляОбработки(HTTPЗапрос);
		Исключение
			ТекстСообщения = НСтр("ru='Ошибка подключения к сервису 1С:Бизнес-сеть:';uk='Ошибка подключения к сервису 1С:Бизнес-сеть:'") + " " + ИнформацияОбОшибке().Описание;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,,, Отказ);
			ЭлектронноеВзаимодействиеСлужебный.ВыполнитьЗаписьСобытияПоЭДВЖурналРегистрации(
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), 5);
		КонецПопытки;
	...
	Если HTTPОтвет.КодСостояния <> 200 Тогда
		Отказ = Истина;
		ТекстСообщения = СтрШаблон(НСтр("ru='Ошибка аутентификации в сервисе 1С:Бизнес-сеть. Код ошибки %1.';uk='Ошибка аутентификации в сервисе 1С:Бизнес-сеть. Код ошибки %1.'"), HTTPОтвет.КодСостояния);
...
Функция СформироватьДанныеРегистрацииАбонента(ПараметрыКоманды, ПараметрыСоединения, Отказ)
	...
	Если Не Пользователи.РолиДоступны("АдминистрированиеАбонентаБизнесСеть") Тогда
		ТекстОшибки = НСтр("ru='Недостаточно прав доступа для выполнения операции в сервисе 1С:Бизнес-сеть';uk='Недостаточно прав доступа для выполнения операции в сервисе 1С:Бизнес-сеть'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,,,,Отказ);
		Возврат "";
	КонецЕсли;
	...
	|	Пользователи1СБизнесСеть.Идентификатор
	|ИЗ
	|	РегистрСведений.Пользователи1СБизнесСеть КАК Пользователи1СБизнесСеть
...
Процедура СоздатьОбъектыИБ(ПараметрыЗагрузки, Отказ)
	...
	Если НЕ МожноЗагрузитьЭДВида(ПараметрыЗагрузки.ВидЭД) Тогда
		ТекстСообщения = НСтр("ru='Не поддерживается загрузка электронных документов вида ""%1"".';uk='Не поддерживается загрузка электронных документов вида ""%1"".'");
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыЗагрузки.Контрагент) Тогда
		ТекстСообщения = НСтр("ru='Не указан контрагент.';uk='Не указан контрагент.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Отказ = Истина;
	КонецЕсли;
	...
	Если ПараметрыЗагрузки.РежимЗаполненияДокумента Тогда
		Если Не ЗначениеЗаполнено(ПараметрыЗагрузки.ДокументИБ) Тогда
			ТекстСообщения = НСтр("ru='Не указан документ для заполнения.';uk='Не указан документ для заполнения.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Отказ = Истина;
		КонецЕсли;
...
"Операция возможна только для непроведенных документов.';uk='Обработка документа %1."
...
// Поиск строки в дереве значений электронного документа.
//
// Параметры:
//   ДеревоРазбора - ДеревоЗначений - структура данных ЭД.
//   СтрокаОбъекта - СтрокаДереваЗначений - строка для поиска.
//   ИмяОбъектаПоиска - Строка - имя объекта поиска.
//
// Возвращаемое значение:
//   СтрокаДереваЗначений - строка дерева по результатам поиска.
//
...
//
Процедура СформироватьДокументИБ(ПараметрыФормирования, ДанныеФормы, ТекстСообщения, Записывать,
	...
	Если СтруктураРазбора <> Неопределено И СтруктураРазбора.Свойство("ДеревоРазбора", ДеревоРазбора)
		...
		// Если на контрагент не совпадает контрагентом в дереве разбора (найденный по реквизитам
		// из ЭД), такой контрагент в дереве заменяется на контрагент формы.
		СтрокаДерева = НайтиСтрокуВДереве(ДеревоРазбора, СтрокаОбъекта, "Контрагент");
		
		Если СтрокаДерева = Неопределено Тогда
			ВызватьИсключение НСтр("ru='Ошибка формата электронного документа.';uk='Ошибка формата электронного документа.'");
		ИначеЕсли СтрокаДерева.СсылкаНаОбъект <> ПараметрыФормирования.Контрагент Тогда
			СтрокаДерева.СсылкаНаОбъект = ПараметрыФормирования.Контрагент;
			// Замена партнера.
			Если ОбменСКонтрагентамиПовтИсп.ИспользуетсяДополнительнаяАналитикаКонтрагентовСправочникПартнеры() Тогда
				СтрокаДереваПартнер = НайтиСтрокуВДереве(ДеревоРазбора, СтрокаОбъекта,
					"Партнер");
				Если СтрокаДереваПартнер <> Неопределено Тогда
					РеквизитыПартнера = Новый Структура();
					РеквизитыПартнера.Вставить("Контрагент", ПараметрыФормирования.Контрагент);
					ИмяПрикладногоСправочника = 
						ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ИмяПрикладногоСправочника("Партнеры");
					СтрокаДереваПартнер.СсылкаНаОбъект = 
						ЭлектронноеВзаимодействиеПереопределяемый.НайтиСсылкуНаОбъект(ИмяПрикладногоСправочника,, 
							РеквизитыПартнера);
				КонецЕсли; 
			КонецЕсли;
		КонецЕсли;
		
		ДокументСсылка = ?(ПараметрыФормирования.РежимЗаполненияДокумента, ПараметрыФормирования.ДокументИБ, Неопределено);
		
		Попытка
			ДанныеОбъектаСсылка = ОбменСКонтрагентамиПереопределяемый.СохранитьДанныеОбъектаВБД(СтрокаОбъекта, ДеревоРазбора,
				ДокументСсылка, Записывать);
			ДанныеФормы = ДанныеОбъектаСсылка;
			ДокументИБСформирован = Истина;
		Исключение
			Отказ = Истина;
			ШаблонСообщения = "%1." + Символы.ПС + "%2";
			ТекстСообщения = СтрШаблон(ШаблонСообщения,
				ИнформацияОбОшибке().Описание, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	Иначе
		Отказ = Истина;
	КонецЕсли;
	...
	Если Не Отказ Тогда
		УстановитьСтатусЗагружен(ПараметрыФормирования.Идентификатор, ДанныеФормы, Отказ);	
	КонецЕсли;
...
Процедура ПроверитьСсылкиОтключения(МассивСсылок, Отказ)
	...
	|	И НЕ Организации1СБизнесСеть.Организация ЕСТЬ NULL ";
	...
	Если Результат.Пустой() Тогда
		ТекстОшибки = НСтр("ru='Команда не может быть выполнена для выделенных строк.';uk='Команда не может быть выполнена для выделенных строк.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,,,, Отказ);
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьИдентификаторыБизнесСети(ПараметрыОбновления, РежимУдаления = Ложь)
	...
	Если ПараметрыОбновления.Свойство("МассивПользователей") Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			ЭтоПустойАдминистратор = НЕ ЗначениеЗаполнено(МассивПользователей) И (НЕ ЗначениеЗаполнено(Выборка.СсылкаНаОбъект)
