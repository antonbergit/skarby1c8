	...
	Если (ПараметрыЦентраМониторинга.ВключитьЦентрМониторинга ИЛИ ПараметрыЦентраМониторинга.ЦентрОбработкиИнформацииОПрограмме) И ЭтоГлавныйУзел() Тогда 
		Если ПараметрыЦентраМониторинга.РегистрироватьДампы И ДатаЗапуска >= ПараметрыЦентраМониторинга.РегистрацияДамповСледующееФормирование Тогда
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Центр мониторинга - регистрация дампов';uk='Центр моніторингу - реєстрація дампів'",
		...
		Если ПараметрыЦентраМониторинга.РегистрироватьБизнесСтатистику И ДатаЗапуска >= ПараметрыЦентраМониторинга.БизнесСтатистикаСледующееФормирование Тогда
			...
			Исключение
				Комментарий = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Центр мониторинга - регистрация операций статистики';uk='Центр моніторингу - реєстрація операцій статистики'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
		...
		Если (ПараметрыЦентраМониторинга.РегистрироватьСтатистикуКонфигурации ИЛИ ПараметрыЦентраМониторинга.РегистрироватьНастройкуКонфигурации) И ДатаЗапуска >= ПараметрыЦентраМониторинга.СтатистикаКонфигурацииСледующееФормирование Тогда
			...
			Исключение
				Комментарий = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Центр мониторинга - собрать статистику конфигурации';uk='Центр моніторингу - зібрати статистику конфігурації'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
		...
		Если ДатаЗапуска >= ПараметрыЦентраМониторинга.ОтправкаДанныхСледующееФормирование Тогда
			...
			Исключение
				Комментарий = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Центр мониторинга - сформировать пакет для отправки';uk='Центр моніторингу - сформувати пакет для відправки'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
			...
			Исключение
				Комментарий = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Центр мониторинга - отправить данные мониторинга';uk='Центр моніторингу - відправити дані моніторингу'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
...
Процедура АгрегироватьЗамерыОперацийСтатистики(ТекущаяДата)
	...
	Если ОбработатьЗаписиДо > ГраницаАгрегирования Тогда
		...
		Исключение
			ОтменитьТранзакцию();
			Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЗаписьЖурналаРегистрации(НСтр("ru='Центр мониторинга.Агрегировать замеры операций статистики';uk='Центр моніторингу.Агрегіровать виміри операцій статистики'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
			ВызватьИсключение Ошибка;
		КонецПопытки;
	КонецЕсли;
...
Процедура УдалитьУстаревшиеДанныеОперацийСтатистики()
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ru='Центр мониторинга.Удалить устаревшие данные операций статистики';uk='Центр моніторингу.Видалити застарілі дані операцій статистики'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
			...
			Пока Выборка.Следующий() Цикл
				...
				Если РегистрыСведений.ОбластиСтатистики.СобиратьСтатистикуКонфигурации(ОбластьДанныхСтрока) Тогда
					...
					Исключение
						...
						ЗаписьЖурналаРегистрации(НСтр("ru='Центр мониторинга.Статистика конфигурации переопределяемая';uk='Центр моніторингу.Статистика конфігурації переобумовлена'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
						УровеньЖурналаРегистрации.Ошибка,
						,,
...
Процедура СформироватьПакетДляОтправки()
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ru='Центр мониторинга.Формирование пакета для отправки';uk='Центр моніторингу.Формування пакета для відправлення'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
...
Функция УдалитьПараметрыЦентраМониторинга()
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ru='Центр мониторинга.Удалить параметры центра мониторинга';uk='Центр моніторингу.Видалити параметри центру моніторингу'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
КонецФункции
...
Функция НайтиКаталогДампов(КаталогНастроек) 
	...
	Если Файл.Существует() Тогда
		Попытка
			...
			КонецЦикла;
		Исключение
			Сообщение = НСтр("ru='Ошибка чтения файла настроек технологического журнала';uk='Помилка читання файлу налаштувань технологічного журналу'");
			Сообщение = Сообщение + " """ +Файл.ПолноеИмя + """." + Символы.ПС;
			Сообщение = Сообщение + НСтр("ru='Скорее всего файл поврежден. Регистрация дампов не возможна. Удалите поврежденный файл или восстановите настройки.';uk='Швидше за все файл пошкоджений. Реєстрація дампів не можлива. Видаліть пошкоджений файл або відновіть налаштування.'");
		...
		Если ПараметрыДампов <> Неопределено Тогда
			Если НЕ ПараметрыДампов.Свойство("location") ИЛИ НЕ ПараметрыДампов.Свойство("create") ИЛИ НЕ ПараметрыДампов.Свойство("type") Тогда
				Сообщение = НСтр("ru='Ошибка секции сбора дампов в файле настроек технологического журнала';uk='Помилка секції збору дампів у файлі налаштувань технологічного журналу'");
				Сообщение = Сообщение + " """ +Файл.ПолноеИмя + """." + Символы.ПС;
				Сообщение = Сообщение + НСтр("ru='Регистрация дампов не возможна. Удалите файл или восстановите настройки.';uk='Реєстрація дампів не можлива. Видаліть файл або відновіть налаштування.'");
				ЧтениеXML.Закрыть();
	...
	ЧтениеXML.Закрыть();
		...
		Если ВРЕГ(ПервыйДочерний.ИмяУзла) = "CONFIG" Тогда
			...
			Попытка
				...
				ЗаписьXML.Закрыть();
			Исключение
				Сообщение = НСтр("ru='Ошибка записи файла настроек технологического журнала. Регистрация дампов не возможна.';uk='Помилка запису файлу налаштувань технологічного журналу. Реєстрація дампів не можлива.'");
				Сообщение = Сообщение + " """ +Файл.ПолноеИмя + """." + Символы.ПС;
...
Функция СоздатьФайлНастроекСбораДампов(ПутьКаталога)
	...
	Попытка
		...
		ЗаписьXML.Закрыть();
	Исключение
		Сообщение = НСтр("ru='Ошибка создания файла настроек технологического журнала. Регистрация дампов не возможна.';uk='Помилка створення файлу налаштувань технологічного журналу. Реєстрація дампів не можлива.'");
...
Функция ПолучитьКаталогНастроекТехнологическогоЖурнала()
	...
	Пока ПоискКаталога = Истина Цикл
		...
		Если КаталогиНастроек.Найти(ПутьКаталога) <> Неопределено Тогда
			...
			КаталогНастроек.ОписаниеОшибки = НСтр("ru='Обнаружена циклическая ссылка';uk='Виявлене циклічне посилання'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		...
		Иначе
			...
			Иначе
				...
				Иначе
					КаталогНастроек.Путь = "";
					КаталогНастроек.Существует = Ложь;
					КаталогНастроек.ОписаниеОшибки = НСтр("ru='Не найден файл конфигурации настроек в каталоге';uk='Не знайдений файл конфігурації налаштувань у каталозі'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + " " + ПутьКаталога;
...
Функция СобытиеЖурналаРегистрацииЦентрМониторингаУдалениеДампа()
	Возврат НСтр("ru='Центр мониторинга.Удаление дампа';uk='Центр моніторингу.Видалення дампа'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции
