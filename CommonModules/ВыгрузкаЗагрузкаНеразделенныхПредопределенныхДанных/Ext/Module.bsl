...
Процедура ВызватьИсключениеНеУдалосьОпределитьПоле(Знач ИмяПоля, Знач ИмяОбъекта)
	...
	ВызватьИсключение СтрШаблон(НСтр("ru='Не удалось построить запрос получения значения поля %1 объекта метаданных %2!';uk='Не вдалося побудувати запит отримання значення поля %1 об`єкту метаданих %2!'"),
		ИмяПоля, ИмяОбъекта
	);
