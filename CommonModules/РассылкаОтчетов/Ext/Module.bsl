...
Функция ПодготовитьПараметрыИВыполнитьРассылку(Рассылка, ПараметрыЖурнала = Неопределено, ПредварительныеНастройки = Неопределено) Экспорт
	...
	Если Не ПараметрыЖурнала.Свойство("ИмяСобытия") Тогда
		ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("ru='Рассылка отчетов. Запуск по требованию';uk='Розсилання звітів. Запуск на вимогу'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	КонецЕсли;
	...
	Если Не Рассылка.Подготовлена
		...
		Если Не Рассылка.Подготовлена Тогда
			Причина = Причина + Символы.ПС + НСтр("ru='Рассылка не подготовлена';uk='Розсилання не підготовлене'");
		КонецЕсли;
		Если Рассылка.ПометкаУдаления Тогда
			Причина = Причина + Символы.ПС + НСтр("ru='Рассылка помечена на удаление';uk='Розсилання позначене на видалення'");
		КонецЕсли;
...
Функция ВыполнитьРассылку(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала = Неопределено) Экспорт
	...
	ЗаписьЖурнала(ПараметрыЖурнала, ,
		НСтр("ru='Инициализирована рассылка %1, автор: %2';uk='Ініційоване розсилання %1, автор: %2'"), , 
		"'"+ Строка(Рассылка) +"'", 
	...
	Для Каждого СтрокаОтчет Из ТаблицаОтчетов Цикл
		...
		Если СтрокаОтчет.Настройки = Неопределено Тогда
			ТекстЖурнала = ТекстЖурнала + Символы.ПС + НСтр("ru='(пользовательские настройки не заданы)';uk='(користувацькі налаштування не задані)'");
		КонецЕсли;
	...
	Если РассылкаВыполнена Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, , НСтр("ru='Рассылка выполнена';uk='Розсилання виконане'"));
	Иначе
		ЗаписьЖурнала(ПараметрыЖурнала, , НСтр("ru='Рассылка не выполнена';uk='Розсилання не виконане'"));
	КонецЕсли;
	...
	ИначеЕсли Всего <= 5 Тогда
		...
		Для Каждого СтрокаТаблицы Из ТаблицаРассылок Цикл
			Если Не СтрокаТаблицы.Подготовлена Тогда
				ШаблонСообщения = НСтр("ru='Рассылка ""%1"" не подготовлена.';uk='Розсилання ""%1"" не підготовлене.'");
			ИначеЕсли Не СтрокаТаблицы.Выполнена Тогда
				ШаблонСообщения = НСтр("ru='Рассылка ""%1"" не выполнена.';uk='Розсилання ""%1"" не виконане.'");
			ИначеЕсли СтрокаТаблицы.СОшибками Тогда
				ШаблонСообщения = НСтр("ru='Рассылка ""%1"" выполнена с ошибками.';uk='Розсилання ""%1"" виконане з помилками.'");
			Иначе
				ШаблонСообщения = НСтр("ru='Рассылка ""%1"" выполнена.';uk='Розсилання ""%1"" виконане.'");
			КонецЕсли;
...
"Выполнено: %3"
"С ошибками: %4"
"Не выполнено: %5';uk='Підготовлене розсилань: %1 з %2"
"Виконане: %3"
"З помилками: %4"
"Не виконане: %5'"),
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ru='Проблемы с рассылками отчетов';uk='Проблеми з розсиланнями звітів'");
...
Процедура ФормаОтчетаДобавитьКоманды(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	КомандаСоздать.Заголовок = НСтр("ru='Создать рассылку отчетов...';uk='Створити розсилання звітів...'");
	КомандаСоздать.Подсказка = НСтр("ru='Создать новую рассылку отчетов и добавить в нее отчет с текущими настройками.';uk='Створити нове розсилання звітів і додати в нього звіт з поточними налаштуваннями.'");
	...
	КомандаПрисоединить.Заголовок = НСтр("ru='Включить в существующую рассылку отчетов...';uk='Включити в існуюче розсилання звітів...'");
	КомандаПрисоединить.Подсказка = НСтр("ru='Присоединить отчет с текущими настройками к существующей рассылке отчетов.';uk='Приєднати звіт з поточними налаштуваннями до існуючого розсилання звітів.'");
	...
	Подменю.Заголовок = НСтр("ru='Отправить рассылку';uk='Відправити розсилання'");
...
"Рекомендуется отключить все рассылки этого пользователя или сменить автора его рассылок (на вкладке ""Расписание"").';uk='У поточного користувача недостатньо прав для читання розсилок звітів."
"Рекомендується відключити усі розсилки цього користувача або змінити автора його розсилок (на вкладці ""Розклад"").'");
	КонецЕсли;
...
"Рекомендуется сменить автора рассылки (на вкладке ""Расписание"").';uk='У поточного користувача недостатньо прав для читання цієї розсилки."
...
"Рекомендуется отключить соответствующее регламентное задание или перезаписать эту рассылку.';uk='У розсилки звітів ""%1"" відключений прапорець ""Виконувати за розкладом"""
"Рекомендується відключити відповідне регламентне завдання або перезаписати цю розсилку.'"),
	...
	ПараметрыЖурнала.ИмяСобытия = НСтр("ru='Рассылка отчетов. Запуск по расписанию';uk='Розсилання звітів. Запуск за розкладом'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
" - Недостаточно прав доступа к справочнику.';uk='Не вдалося сформувати список одержувачів %1 розсилки %2 по одній з можливих причин:"
...
Функция ИнициализироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ПерсонализацияВозможна, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	Исключение
		ПараметрыОтчета.Ошибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При подключении и загрузке настроек отчета ""%1"" возникла ошибка.';uk='При підключенні і завантаженні налаштувань звіту ""%1"" виникла помилка.'"),
...
"Подробнее см. в процедуре ОпределитьИсключаемыеОтчеты модуля РассылкаОтчетовПереопределяемый.';uk='Звіт ""%1"" не підтримує розсилання."
...
Функция ВыполнитьДоставку(ПараметрыЖурнала, ПараметрыДоставки, Вложения) Экспорт
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		Если ПараметрыДоставки.ТолькоУведомить Тогда
			ШаблонСообщенияОбОшибке = НСтр("ru='Невозможно отправить уведомление о рассылке по электронной почте:';uk='Неможливо відправити повідомлення про розсилання по електронній пошті:'");
			ВложенияПисьма = Новый Соответствие;
		Иначе
			ШаблонСообщенияОбОшибке = НСтр("ru='Невозможно отправить отчет по электронной почте:';uk='Неможливо відправити звіт по електронній пошті:'");
...
Функция ПроверитьИДозаполнитьПараметрыВыполнения(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала)
	...
	Если Недоступные.Количество() > 0 Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("ru='В рассылке есть недоступные отчеты (%1):%2';uk='У розсилці є недоступні звіти (%1):%2'"), ,
	...
	Если ПараметрыДоставки.ИспользоватьПапку Тогда
		...
		Иначе
			...
			Если Не ПравоДоступа Тогда
				...
				УстановитьПривилегированныйРежим(Ложь);
				ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
					НСтр("ru='Недостаточно прав для создания файлов в папке ""%1"".';uk='Недостатньо прав для створення файлів в папці ""%1"".'"), ,
					ПредставлениеПапки);
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
	...
	Если ПараметрыДоставки.Персонализирована Тогда
		Если Не ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				НСтр("ru='Персонализированная рассылка может быть отправлена только по электронной почте';uk='Персоналізоване розсилання може бути відправлена тільки по електронній пошті'"));
			Возврат Ложь;
		КонецЕсли;
		ПараметрыДоставки.ИспользоватьПапку          = Ложь;
		ПараметрыДоставки.ИспользоватьСетевойКаталог = Ложь;
