...
Процедура Расшифровать(ОписаниеДанных, Форма = Неопределено, ОбработкаРезультата = Неопределено) Экспорт
	...
	СерверныеПараметры = Новый Структура;
	СерверныеПараметры.Вставить("Операция",            НСтр("ru='Расшифровка данных';uk='Розшифрування даних'"));
	СерверныеПараметры.Вставить("ЗаголовокДанных",     НСтр("ru='Данные';uk='Дані'"));
	СерверныеПараметры.Вставить("ОтборСертификатов");
...
Процедура ИнформироватьОПодписанииОбъекта(ПредставлениеДанных, МножествоДанных = Ложь, ИзФайла = Ложь) Экспорт
	...
	Если ИзФайла Тогда
		...
		Иначе
			ТекстСообщения = НСтр("ru='Добавлена подпись из файла:';uk='Доданий підпис із файлу:'");
		КонецЕсли;
	Иначе
		Если МножествоДанных Тогда
			ТекстСообщения = НСтр("ru='Установлены подписи:';uk='Встановлені підписи:'");
		Иначе
			ТекстСообщения = НСтр("ru='Установлена подпись:';uk='Встановлено підпис:'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ИнформироватьОШифрованииОбъекта(ПредставлениеДанных, МножествоДанных = Ложь) Экспорт
	...
	ТекстСообщения = НСтр("ru='Выполнено шифрование:';uk='Виконане шифрування:'");
...
Процедура ИнформироватьОРасшифровкеОбъекта(ПредставлениеДанных, МножествоДанных = Ложь) Экспорт
	...
	ТекстСообщения = НСтр("ru='Выполнена расшифровка:';uk='Виконане розшифрування:'");
