...
Функция ПКОПоИмени(КомпонентыОбмена, Имя) Экспорт
	...
	Если ПравилоКонвертации = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не найдено ПКО с именем %1';uk='Не знайдене ПКО з іменем %1'"), Имя);
...
Функция ОбъектXDTOИзДанныхXDTO(
	...
	Для Каждого Свойство Из ТипXDTO.Свойства Цикл
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка формирования объекта XDTO: Тип свойства <%1>. Имя свойства: <%2>.';uk='Помилка формування об`єкту XDTO: Тип властивості <%1>. Ім`я властивості: <%2>.'"), ТипСвойства, Свойство.Имя)
...
"Тип источника: %1"
"Тип приемника: %2';uk='Помилка XML-схеми. Для приймача повинне бути задано одна властивість."
"Тип джерела: %1"
"Тип приймача: %2'"),
...
Процедура ОткрытьФайлВыгрузки(КомпонентыОбмена, ИмяФайлаОбмена = "") Экспорт
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		Если ТранзакцияАктивна() Тогда
			ВызватьИсключение НСтр("ru='Блокировка на обмен данными не может быть установлена в активной транзакции.';uk='Блокування на обмін даними не може бути встановлена в активній транзакції.'");
		КонецЕсли;
...
"обновление настройки синхронизации данных. Необходимо:';uk='Одержання даних від джерела, у якім не виконане"
"оновлення налаштувань синхронізації даних. Необхідно:'")
...
"повторно выполнить синхронизацию данных в этой информационной базе.';uk='2) Виконати синхронізацію даних на стороні джерела, після цього "
"повторно виконати синхронізацію даних у цій інформаційній базі.'")
		...
		Если ЭтоОбменЧерезПланОбмена Тогда
			...
			Если МаксимальнаяОбщаяВерсия <> НомерВерсииКорреспондента Тогда
				...
				ЗаписатьВПротоколВыполнения(КомпонентыОбмена, 
				                            НСтр("ru='Изменен номер версии формата обмена.';uk='Змінений номер версії формату обміну.'"),,Ложь,,, Истина);
			КонецЕсли;
...
Функция МенеджерОбменаВерсииФормата(Знач УзелИнформационнойБазы, Знач ВерсияФормата) Экспорт
	...
	Если Результат = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не определен Менеджер конвертации для версии формата обмена <%1>.';uk='Не визначений Менеджер конвертації для версії формату обміну <%1>.'"),
			ВерсияФормата);
	КонецЕсли;
...
"Задано более одного правила конвертации для типа источника <%1>.';uk='Помилка правил конвертації визначених даних."
"Задане більш одного правила конвертації для типу джерела <%1>.'"),
				Строка(Тип));
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка правил конвертации предопределенных данных."
"Правило конвертации не определено для типа источника <%1>.';uk='Помилка правил конвертації визначених даних."
"Правило конвертації не визначене для типу джерела <%1>.'"),
			Строка(Тип));
	...
	Иначе
		...
		ИначеЕсли НайденныеПравила.Количество() > 1 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка правил конвертации предопределенных данных."
"Задано более одного правила конвертации для типа источника <%1> и типа приемника <%2>.';uk='Помилка правил конвертації визначених даних."
"Задане більш одного правила конвертації для типу джерела <%1> і типу приймача <%2>.'"),
				Строка(Тип),
				Строка(ТипXDTO));
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка правил конвертации предопределенных данных."
"Правило конвертации не определено для типа источника <%1> и типа приемника <%2>.';uk='Помилка правил конвертації визначених даних."
"Правило конвертації не визначене для типу джерела <%1> і типу приймача <%2>.'"),
			Строка(Тип),
			Строка(ТипXDTO));
...
"Значение источника: <%2>';uk='Не знайдене правило конвертації для значення визначених даних."
...
Процедура КонвертацияСвойстваXDTOВЭлементСтруктуры(Источник, Свойство, Приемник, ИмяДляСвойстваСоставногоТипа = "")
	...
	Исключение
		ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка чтения объекта XDTO, имя свойства: <%1>.';uk='Помилка читання об`єкту XDTO, ім`я властивості: <%1>.'"), ИмяСвойства)
			+ Символы.ПС + Символы.ПС + ПредставлениеОшибки;
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Процедура КонвертацияСвойстваСтруктурыОбъектаXDTO(
	...
	Исключение
		ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка конвертации свойства объекта XDTO, имя свойства: <%2>.';uk='Помилка конвертації властивості об`єкту XDTO, ім`я властивості: <%2>.'"), ЗначениеСвойства)
			+ Символы.ПС + Символы.ПС + ПредставлениеОшибки;
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
"Тип ссылки XDTO: %1"
"Описание ошибки: %2';uk='Не знайдене ПОД для типу посилання XDTO."
...
"Тип объекта XDTO: %1"
"Описание ошибки: %2';uk='Не знайдене ПОД для типу об`єкта XDTO."
"Тип об`єкта XDTO: %1"
"Опис помилки: %2'"),
...
"Объект метаданных: %3.';uk='Не знайдено ПОД для об`єкту метаданих."
"Об`єкт метаданих : %3.'"),
...
Функция ПОДПоИмени(КомпонентыОбмена, Имя)
	...
	Если ПравилоОбработки = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не найдено ПОД с именем %1';uk='Не знайдене ПОД з іменем %1'"), Имя);
...
"Имя плана обмена: %1"
"Процедура: ПолучитьВерсииФорматаОбмена(<ВерсииФорматаОбмена>)';uk='Не задані версії формату обміну."
"Ім`я плану обміну: %1"
