...
Функция ОписаниеРесурсаИнтернетПоддержки() Экспорт
	...
	Результат.Вставить("Описание", НСтр("ru='Интернет-поддержка пользователей';uk='Інтернет-підтримка користувачів'"));
	Возврат Результат;
		...
		Если Ответ.КодСостояния = 301 // 301 Moved Permanently
			...
			Если КоличествоПеренаправлений > МаксКолвоПеренаправлений Тогда
				УстановитьОписаниеОшибки(
					Результат,
					"ServerError",
					НСтр("ru='Превышено количество перенаправлений.';uk='Перевищена кількість перенапрямків.'"),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Ошибка сервера при получении файла (%1). Превышено количество перенаправлений (%2).';uk='Помилка сервера при одержанні файлу (%1). Перевищена кількість перенапрямків (%2).'"),
						URL,
			...
			Иначе
				...
				Если Location = Неопределено Тогда
					УстановитьОписаниеОшибки(
						Результат,
						"ServerError",
						НСтр("ru='Некорректное перенаправление.';uk='Некоректний перенапрямок.'"),
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Ошибка сервера (%1) при получении файла (%2). Некорректное перенаправление, отсутствует HTTP-заголовок ответа ""Location"".';uk='Помилка сервера (%1) при одержанні файлу (%2). Некоректний перенапрямок, отсутствует HTTP-заголовок відповіді ""Location"".'"),
							Ответ.КодСостояния,
							URL),
						Перенаправления);
					Возврат Результат;
				Иначе
					...
					Если ПустаяСтрока(Location) Тогда
						УстановитьОписаниеОшибки(
							...
							НСтр("ru='Некорректное перенаправление.';uk='Некоректний перенапрямок.'"),
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Ошибка сервера (%1) при получении файла (%2). Некорректное перенаправление, пустой HTTP-заголовок ответа ""Location"".';uk='Помилка сервера (%1) при одержанні файлу (%2). Некоректний перенапрямок, порожній HTTP-заголовок відповіді ""Location"".'"),
								Ответ.КодСостояния,
								URL),
							Перенаправления);
						Возврат Результат;
					...
					Если ВыполненныеПеренаправления.Получить(Location) <> Неопределено Тогда
						УстановитьОписаниеОшибки(
							Результат,
							"ServerError",
							НСтр("ru='Циклическое перенаправление.';uk='Циклічний перенапрямок.'"),
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Ошибка сервера (%1) при получении файла (%2). Циклическое перенаправление (%3).';uk='Помилка сервера (%1) при одержанні файлу (%2). Циклічний перенапрямок (%3).'"),
								Ответ.КодСостояния,
								URL,
								Location),
							Перенаправления);
	...
	Если Ответ.КодСостояния < 200 Или Ответ.КодСостояния >= 300 Тогда
		...
		Если Ответ.КодСостояния = 407 Тогда
			...
			УстановитьОписаниеОшибки(
				Результат,
				"ConnectError",
				НСтр("ru='Ошибка аутентификации на прокси-сервере.';uk='Помилка аутентифікації на проксі-сервері.'"),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка клиента (%1) при выполнении запроса к ресурсу (%2)."
"Тело ответа: %3';uk='Помилка клієнта (%1) при виконанні запиту до ресурсу (%2)."
"Тіло відповіді: %3'"),
					Ответ.КодСостояния,
					URL,
...
"Тело ответа: %3';uk='Помилка сервера при одержанні файлу (%1). Некоректна (непідтримувана) відповідь (%2)."
"Тіло відповіді: %3'"),
					URL,
					Ответ.КодСостояния,
					Лев(Ответ.ПолучитьТелоКакСтроку(), 5120)),
...
"Тело ответа: %3';uk='Помилка сервера (%1) при обробці запиту до ресурсу (%2)."
"Тіло відповіді: %3'"),
					Ответ.КодСостояния,
					URL,
					Лев(Ответ.ПолучитьТелоКакСтроку(), 5120)),
...
//
Функция ПроверитьURLДоступен(URL, НастройкиПроксиСервера = Неопределено) Экспорт
...
Функция ПользовательскоеПредставлениеИнтервалаВерсий(ВерсияОТ, ВерсияДО) Экспорт
	...
	ИначеЕсли (ВерсияОТ <> "00.00.000.0000") И (ВерсияДО = "99.99.999.9999") Тогда // От версии и выше
		Результат = СтрШаблон(
			НСтр("ru='От %1 и выше';uk='Від %1 і вище'"),
			ПользовательскоеПредставлениеНомераВерсии(ВерсияОТ, Истина));
...
"%1';uk='Помилка при отриманні xml-представлення об`єкту ХДТО:"
"%1'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:БазовыйФункционал.Отладка';uk='БІП:БазовыйФункционал.Отладка'"), // ИмяСобытия.
				НСтр("ru='БазовыйФункционал. Отладка. ПредставлениеОбъектаХДТО';uk='БазовыйФункционал. Отладка. ПредставлениеОбъектаХДТО'"), // ИдентификаторШага.
...
"%2';uk='При читанні файлу з ім`ям %1 сталася помилка:"
"%2'"),
			ПолноеИмяФайла,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
"Повторите попытку позже.';uk='Перевищена кількість спроб введення логіна і пароля."
"Спробуйте ще раз пізніше.'");
	...
	Иначе
		...
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось проверить логин и пароль в сервисе %1. %2';uk='Не вдалося перевірити логін і пароль в сервісі %1. %2'"),
				URLСервиса,
...
Функция СлужебнаяТикетАутентификации(
	...
	Если НЕ ПустаяСтрока(ПроксиСервиса.КодОшибки) Тогда
		...
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru='Ошибка при подключении к сервису аутентификации.';uk='Помилка при підключенні до сервісу аутентифікації.'")
		...
		Результат.СообщениеОбОшибке = НСтр("ru='Ошибка при подключении к сервису.';uk='Помилка при підключенні до сервісу.'")
			...
			+ Символы.ПС
			+ НСтр("ru='Подробнее см. в журнале регистрации.';uk='Детальніше див. в журналі реєстрації.'");
		Возврат Результат;
...
Функция СервисАутентификации_getTicket(
	...
	Если Не ПустаяСтрока(Результат.КодОшибки) Тогда
		...
		ИначеЕсли СтрНайти(Результат.ИнформацияОбОшибке, "TooManyFailedAttemptsApiException") > 0 Тогда
			Результат.КодОшибки = "ПревышеноКоличествоПопыток";
			Результат.ИнформацияОбОшибке =
				НСтр("ru='Превышено количество попыток аутентификации (TooManyFailedAttemptsApiException).';uk='Перевищена кількість спроб аутентифікації (TooManyFailedAttemptsApiException).'");
		...
		ИначеЕсли СтрНайти(Результат.ИнформацияОбОшибке, "TicketApiException") > 0 Тогда
			...
			Результат.ИнформацияОбОшибке = НСтр("ru='Неизвестная ошибка сервиса аутентификации (TicketApiException).';uk='Невідома помилка сервісу аутентифікації (TicketApiException).'");
...
"%1';uk='Не вдалося відправити ці повідомлення до служби технічної підтримки."
"%1'"),
...
"%1"
"Тело ответа: %2';uk='Не вдалося відправити ці повідомлення до служби технічної підтримки."
"Помилка при обробці відповіді сервісу."
"Некоректна відповідь сервісу відправки повідомлень."
"%1"
"Тіло відповіді: %2'"),
		...
		Результат.СообщениеОбОшибке =
			НСтр("ru='Не удалось отправить данные сообщения в службу технической поддержки."
"Некорректный ответ сервиса отправки сообщений.';uk='Не вдалося відправити ці повідомлення до служби технічної підтримки."
"Некоректна відповідь сервісу відправки повідомлень.'");
...
"Ответ сервиса: %1';uk='Не вдалося відправити ці повідомлення до служби технічної підтримки."
...
"Відповідь сервісу: %1'"),
		...
		Результат.СообщениеОбОшибке =
			НСтр("ru='Не удалось отправить данные сообщения в службу технической поддержки."
"Ошибка при передаче сообщения в тех. поддержку."
"Подробнее см. в журнале регистрации.';uk='Не вдалося відправити ці повідомлення до служби технічної підтримки."
"Помилка при передачі повідомлення в тех. підтримку."
...
"Подробнее см. в журнале регистрации.';uk='Помилка входу на Портал 1С:ІТС."
"Детальніше див. в журналі реєстрації.'");
			КонецЕсли;
		КонецЕсли;
...
Процедура УстановитьОписаниеОшибки(Результат, КодОшибки, СообщениеОбОшибке, ИнформацияОбОшибке, Перенаправления)
	...
	ИначеЕсли КодОшибки = "LoginError" Тогда
		ДопСообщение = НСтр("ru='Ошибка аутентификации на сервере.';uk='Помилка аутентифікації на сервері.'");
	...
	Если Перенаправления.Количество() > 0 Тогда
		Результат.ИнформацияОбОшибке = Результат.ИнформацияОбОшибке + Символы.ПС
			+ НСтр("ru='Перенаправления:';uk='Перенапрямку:'") + Символы.ПС
...
Функция НовыйОпределенияСервиса(
	...
	Если НЕ ПустаяСтрока(ОписаниеWSDL.КодОшибки) Тогда
		...
		Результат.ИнформацияОбОшибке =
			НСтр("ru='Ошибка при получении WSDL-описания.';uk='Помилка при одержанні WSDL-опису.'") + " "
	...
	Исключение
		...
		Результат.КодОшибки          = "ServerError";
		Результат.СообщениеОбОшибке  = НСтр("ru='Некорректное WSDL-описание сервиса.';uk='Некоректний WSDL-опис сервісу.'");
		Результат.ИнформацияОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция НовыйПроксиВебСервиса(
	...
	Если НайденныйПорт = Неопределено Тогда
		...
		Результат.СообщениеОбОшибке = НСтр("ru='Не найдено определение сервиса.';uk='Не знайдене визначення сервісу.'");
		Результат.ИнформацияОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при создании описания веб-сервиса %1. Определение сервиса не найдено."
"URI пространства имен: %2;"
"Имя сервиса: %3;"
"Имя точки подключения: %4';uk='Помилка при створенні опису веб-сервісу %1. Визначення сервісу не знайдене."
"URI простору імен: %2;"
"Ім`я сервісу: %3;"
...
Процедура СлужебнаяПроверитьURLДоступен(
	...
	Исключение
		...
		ИнформацияОбОшибке = СтрШаблон(
			НСтр("ru='Неизвестная ошибка при проверке доступности URL."
"%1';uk='Невідома помилка при перевірці доступності URL."
"%1'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
...
Функция ПрочитатьОтветВКонвертеSOAP(ТелоОтвета, ОписаниеСервисаИПП, ТипЗначения)
	...
	Исключение
		...
		Результат.СообщениеОбОшибке  = НСтр("ru='Неверный формат ответа.';uk='Невірний формат відповіді.'");
		Результат.ИнформацияОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Неверный формат ответа SOAP. %1';uk='Невірний формат відповіді SOAP. % 1'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Результат;
	...
	Если ЧтениеОтвета.ТипУзла = ТипУзлаXML.НачалоЭлемента
		...
		Результат.Ошибка = Истина;
		Результат.СообщениеОбОшибке  = НСтр("ru='Ошибка SOAP-сервера.';uk='Помилка SOAP-серверу.'");
		Результат.ИнформацияОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка SOAP-сервера. %1';uk='Помилка SOAP-серверу. % 1'"),
