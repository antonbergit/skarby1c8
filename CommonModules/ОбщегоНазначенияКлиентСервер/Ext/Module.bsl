	...
	Для Каждого Элемент Из Источник Цикл
		Если Заменять <> Истина И Приемник.Свойство(Элемент.Ключ) Тогда
			Если Заменять = Ложь Тогда
				Продолжить;
			Иначе
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Пересечение ключей источника и приемника: ""%1"".';uk='Перетинання ключів джерела й приймача: ""%1"".'"), Элемент.Ключ);
			КонецЕсли
		КонецЕсли;
...
Функция УстановитьВнешнееСоединениеСБазой(Параметры) Экспорт
	...
	#Иначе
		...
		КраткоеОписаниеОшибки = НСтр("ru='Прямое подключение к информационной базе доступно только на клиенте под управлением ОС Windows.';uk='Пряме підключення до інформаційної бази доступне тільки на клієнтові під управлінням ОС Windows.'");
	#КонецЕсли
