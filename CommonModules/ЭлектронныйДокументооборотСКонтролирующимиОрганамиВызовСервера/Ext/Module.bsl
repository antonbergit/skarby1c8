...
Функция ЗаголовокГиперссылкиЗаявления(Организация) Экспорт
	...
	ЗаголовокВторичногоЗаявления = НСтр("ru='Заявление на изменение реквизитов или замену сертификата';uk='Заявление на изменение реквизитов или замену сертификата'");
...
Процедура ИзменитьЗначениеВФорме1СОтчетность(Ссылка, ИмяПоля, Знач НовоеЗначение) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Не удалось изменить свойство в таблице форме 1С-Отчетности';uk='Не удалось изменить свойство в таблице форме 1С-Отчетности'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Процедура УдалитьВыборCSPИзВременныхНастроек() Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Удаление сохраненных настроек мастера подключения к 1С-Отчетности';uk='Удаление сохраненных настроек мастера подключения к 1С-Отчетности'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция СведенияОЗаполненииПоясненияКДекларацииПоНДС(Пояснение) Экспорт
	...
	|	ГДЕ
	|		ПоясненияКДекларацииПоНДСВыставленныеСчетаФактурыНеплательщиковНДС.Ссылка = &Пояснение) КАК СведенияОЗаполнении";
...
Функция НаименованиеТОГС(КодОрганаФСГС) Экспорт
	...
	Возврат НСтр("ru='<Не выбран код территориального органа>';uk='<Не выбран код территориального органа>'");
...
Функция ПолучитьДанныеСотрудника(ВидВладельцаЭЦП, ДанныеОрганизации, Сотрудник) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Электронный документооборот с контролирующими органами. Не удалось получить сведения о сотруднике';uk='Электронный документооборот с контролирующими органами. Не удалось получить сведения о сотруднике'");
	...
	Если НЕ ЗначениеЗаполнено(Гражданство) Тогда
		Гражданство = Справочники.СтраныМира.Украина;
	КонецЕсли;
