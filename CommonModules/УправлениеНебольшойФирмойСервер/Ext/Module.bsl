...
// Функция возвращает код УКТВЭД для строки товаров.
//
// Параметры:
//	Номенклатура - СправочникСсылка.Номенклатура - Номенклатура, для которой определяется код УКТВЭД
//	Характеристика - СправочникСсылка.ХарактеристикиНоменклатуры - Характеристика номенклатура, для которой определяется код УКТВЭД
//
Функция ПолучитьУКТВЭДВСтрокеТоваров(Номенклатура, Характеристика) Экспорт

	Если ЗначениеЗаполнено(Характеристика) Тогда
		
		НоменклатураГТДХарактеристики = Характеристика.НоменклатураГТД;
		
		Если Характеристика.ОтечественныйНеподакцизныйТовар Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(НоменклатураГТДХарактеристики) Тогда
			Если ТипЗнч(НоменклатураГТДХарактеристики) = Тип("СправочникСсылка.НоменклатураГТД") Тогда
				Возврат НоменклатураГТДХарактеристики.КодУКТВЭД;
			ИначеЕсли ТипЗнч(НоменклатураГТДХарактеристики) = Тип("СправочникСсылка.КлассификаторУКТВЭД") Тогда
				Возврат НоменклатураГТДХарактеристики;
			КонецЕсли; 
		Иначе
			Если ТипЗнч(Номенклатура.НоменклатураГТД) = Тип("СправочникСсылка.НоменклатураГТД") Тогда
				Возврат Номенклатура.НоменклатураГТД.КодУКТВЭД;
			ИначеЕсли ТипЗнч(Номенклатура.НоменклатураГТД) = Тип("СправочникСсылка.КлассификаторУКТВЭД") Тогда
				Возврат Номенклатура.НоменклатураГТД;
			КонецЕсли; 
		КонецЕсли;
...
// Функция возвращает номер ГТД для строки товаров.
//
// Параметры:
//	Номенклатура - СправочникСсылка.Номенклатура - Номенклатура, для которой определяется номер ГТД
//	Характеристика - СправочникСсылка.ХарактеристикиНоменклатуры - Характеристика номенклатура, для которой определяется номер ГТД
//
Функция ПолучитьНомерГТДСтрокеТоваров(Номенклатура, Характеристика) Экспорт
	...
	Если ЗначениеЗаполнено(Характеристика) Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(НоменклатураГТДХарактеристики) Тогда
			Если ТипЗнч(НоменклатураГТДХарактеристики) = Тип("СправочникСсылка.НоменклатураГТД") Тогда
				Возврат НоменклатураГТДХарактеристики.НомерГТД;
			ИначеЕсли ТипЗнч(НоменклатураГТДХарактеристики) = Тип("СправочникСсылка.КлассификаторУКТВЭД") Тогда
				Возврат НоменклатураГТДХарактеристики;
			КонецЕсли; 
		Иначе
			Если ТипЗнч(Номенклатура.НоменклатураГТД) = Тип("СправочникСсылка.НоменклатураГТД") Тогда
				Возврат Номенклатура.НоменклатураГТД.НомерГТД;
			ИначеЕсли ТипЗнч(Номенклатура.НоменклатураГТД) = Тип("СправочникСсылка.КлассификаторУКТВЭД") Тогда
				Возврат Номенклатура.НоменклатураГТД;
			КонецЕсли; 
		КонецЕсли;
...
// Выполняет движения по регистру НДСПродаж.
//
Процедура ОтразитьНДСПродаж(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	ТаблицаНДСПродаж = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаНДСПродаж;
	
	Если Отказ
	 ИЛИ ТаблицаНДСПродаж.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДвиженияНДСПродаж = Движения.НДСПродаж;
	ДвиженияНДСПродаж.Записывать = Истина;
	ДвиженияНДСПродаж.Загрузить(ТаблицаНДСПродаж);
	
КонецПроцедуры // ОтразитьВыпускПродукции()

// Выполняет движения по регистру РозничныеПродажиПодакцизныхТоваров.
//
...
Процедура УстановитьОформлениеКолонкиДата(ДинСписок) Экспорт
	...
	Для каждого ЭлементУсловногоОформления Из ДинСписок.КомпоновщикНастроек.Настройки.УсловноеОформление.Элементы Цикл
		Если ЭлементУсловногоОформления.ИдентификаторПользовательскойНастройки = "ПредустановленныйТолькоВремяПоляДата"
			ИЛИ ЭлементУсловногоОформления.Представление = НСтр("ru='Формат поля Дата (сегодня - только время)';uk='Формат поля Дата (сьогодні - тільки час)'") Тогда
	...
	ЭлементОформления.Представление = НСтр("ru='Формат поля Дата (сегодня - только время)';uk='Формат поля Дата (сьогодні - тільки час)'");
	...
	Пока ЕстьСледующаяПорция Цикл
		...
		Для СчетчикЭлементов = 0 По КонечнаяПозиция Цикл
			...
			Если Элемент.Метаданные = Метаданные.Справочники.Контрагенты Тогда
				...
				Основание = НСтр("ru='Найдено: Контрагент - ';uk='Знайдене: Контрагент -'") + Элемент.Описание;
				
			// Контактные лица
			ИначеЕсли Элемент.Метаданные = Метаданные.Справочники.КонтактныеЛица Тогда
				
				Контрагент = Элемент.Значение.Владелец;
				ШаблонОснования = НСтр("ru='Найдено: Контактное лицо ""%1"" - %2';uk='Знайдене: Контактна особа ""%1"" - %2'");
			...
			ИначеЕсли Элемент.Метаданные = Метаданные.Справочники.ФизическиеЛица Тогда
				...
				Если ТаблицаНайденныхКонтрагентов <> Неопределено Тогда
					...
					Для каждого СтрокаТаблицы Из ТаблицаНайденныхКонтрагентов Цикл
						ШаблонОснования = НСтр("ru='Найдено: Физическое лицо ""%1"" контактного лица ""%2"" - %3';uk='Знайдене: Фізична особа ""%1"" контактної особи ""%2"" - %3'");
			...
			ИначеЕсли Элемент.Метаданные = Метаданные.Справочники.ДоговорыКонтрагентов Тогда
				...
				Контрагент = Элемент.Значение.Владелец;
				ШаблонОснования =  НСтр("ru='Найдено: Договор ""%1"" - %2';uk='Знайдене: Договір ""%1"" - %2'");
				Основание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОснования, Элемент.Значение, Элемент.Описание);
			...
			ИначеЕсли Элемент.Метаданные = Метаданные.Справочники.БанковскиеСчета Тогда
				...
				Контрагент = Элемент.Значение.Владелец;
				ШаблонОснования =  НСтр("ru='Найдено: Банковский счет ""%1"" - %2';uk='Знайдене: Банківський рахунок ""%1"" - %2'");
				Основание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОснования, Элемент.Значение, Элемент.Описание);
...
// Выполняет движения по регистру КнигаДоходовРасходовПоЕдиномуНалогу.
//
Процедура ОтразитьКнигаДоходовРасходовПоЕН(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	ТаблицаКнигаДоходовРасходовПоЕН = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаКнигаДоходовРасходовПоЕН;
	
	Если Отказ
	 ИЛИ ТаблицаКнигаДоходовРасходовПоЕН.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДвиженияКнигаДоходовРасходовПоЕН = Движения.КнигаДоходовРасходовПоЕдиномуНалогу;
	ДвиженияКнигаДоходовРасходовПоЕН.Записывать = Истина;
	ДвиженияКнигаДоходовРасходовПоЕН.Загрузить(ТаблицаКнигаДоходовРасходовПоЕН);
...
Функция ПересчитатьИзВалютыВВалюту(Сумма, КурсНач, КурсКон,	КратностьНач = 1, КратностьКон = 1) Экспорт
	...
	Если КурсНач = 0 ИЛИ КурсКон = 0 ИЛИ КратностьНач = 0 ИЛИ КратностьКон = 0 Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru='Обнаружен нулевой курс валюты. Пересчет не выполнен.';uk='Виявлений нульовий курс валюти. Перерахування не виконане.'");
		Сообщение.Сообщить();
		Возврат Сумма;
	КонецЕсли;
...
Функция ПолучитьПаспортныеДанныеСтрокой(СтруктураДанных) Экспорт
	...
	Иначе
		...
		Если НЕ (НЕ ЗначениеЗаполнено(ДатаВыдачи)
			...
			СтрокаПаспортныеДанные = НСтр("ru='%ВидДокумента% Серия: %Серия%, № %Номер%, Выдан: %ДатаВыдачи% года, %КемВыдан%; № подр. %НомерПодразделения%';uk='%ВидДокумента% Серія: %Серия%, № %Номер%, Виданий: %ДатаВыдачи% року, %КемВыдан%; № підр. % НомерПодразделения%'");
...
Функция ПолучитьПредставлениеТипаСтруктурныхЕдиниц(ТипСтруктурнойЕдиницы)
	...
	Если ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Подразделение Тогда
		ПредставлениеТипаСтруктурныхЕдиниц = НСтр("ru='в подразделении';uk='в підрозділі'");
	ИначеЕсли ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Розница
		...
		ПредставлениеТипаСтруктурныхЕдиниц = НСтр("ru='на розничном складе';uk='на роздрібному складі'");
	Иначе
		ПредставлениеТипаСтруктурныхЕдиниц = НСтр("ru='на складе';uk='на складі'");
	КонецЕсли;
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыНаСкладах(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Не хватает запасов %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление%';uk='Не вистачає запасів %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление%'");
	
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"остаток %КоличествоОстаток% %ЕдиницаИзмерения%,"
"недостаточно %Количество% %ЕдиницаИзмерения%';uk='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"залишок %КоличествоОстаток% %ЕдиницаИзмерения%,"
"недостатньо %Количество% %ЕдиницаИзмерения%'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасы(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Не хватает остатка по учету запасов и затрат %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление%';uk='Не вистачає залишку по обліку запасів і витрат %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление%'");
	
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"остаток %КоличествоОстаток% %ЕдиницаИзмерения%,"
"недостаточно %КоличествоИРезерв% %ЕдиницаИзмерения%';uk='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"залишок %КоличествоОстаток% %ЕдиницаИзмерения%,"
"недостатньо %КоличествоИРезерв% %ЕдиницаИзмерения%'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыСписком(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Не хватает остатка по учету запасов и затрат';uk='Не вистачає залишку по обліку запасів і витрат'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыПереданные(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Не хватает запасов, переданных стороннему контрагенту %КонтрагентПредставление%';uk='Не вистачає запасів, переданих сторонньому контрагентові %КонтрагентПредставление%'");
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристикаПартия%,';uk='Номенклатура: %НоменклатураХарактеристикаПартия%,'");
...
"недостаточно %Количество% %ЕдиницаИзмерения%';uk='залишок %КоличествоОстаток% %ЕдиницаИзмерения%,"
"недостатньо %Количество% %ЕдиницаИзмерения%'");
...
"недостаточно %Сумма% %Валюта%';uk='Сума розрахунків:"
"залишок %СуммаОстаток% %Валюта%,"
"недостатньо %Сума% %Валюта%'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыПереданныеСписком(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Не хватает запасов, переданных стороннему контрагенту';uk='Не вистачає запасів, переданих сторонньому контрагентові'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
	
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"контрагент %КонтрагентПредставление%';uk='Номенклатура: %НоменклатураХарактеристикаПартия%,"
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыПринятые(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Не хватает запасов, поступивших от стороннего контрагента %КонтрагентПредставление%';uk='Не вистачає запасів, що надійшли від стороннього контрагента %КонтрагентПредставление%'");
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристикаПартия%,';uk='Номенклатура: %НоменклатураХарактеристикаПартия%,'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗапасыПринятыеСписком(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Не хватает запасов, поступивших от стороннего контрагента';uk='Не вистачає запасів, що надійшли від стороннього контрагента'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"контрагент %КонтрагентПредставление%';uk='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"контрагент %КонтрагентПредставление%'");
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗаказыПокупателей(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Оформлено больше, чем указано в заказе покупателя';uk='Оформлене більше, ніж зазначене в замовленні покупця'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
	
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"остаток по заказу %КоличествоОстаток% %ЕдиницаИзмерения%,"
"превышает на %Количество% %ЕдиницаИзмерения%. %ЗаказПокупателя%';uk='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"остаток по заказу %КоличествоОстаток% %ЕдиницаИзмерения%,"
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗаказыПоставщикам(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Оформлено больше, чем указано в заказе поставщику';uk='Оформлене більше, ніж зазначене в замовленні постачальникові'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
		
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"остаток по заказу %КоличествоОстаток% %ЕдиницаИзмерения%,"
"превышает на %Количество% %ЕдиницаИзмерения%"
"%ЗаказПоставщику%';uk='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"залишок по замовленню %КоличествоОстаток% %ЕдиницаИзмерения%,"
"перевищує на %Количество% %ЕдиницаИзмерения%"
"%ЗаказПоставщику%'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗаказыНаПроизводство(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Оформлено больше, чем указано в заказе на производство';uk='Оформлене більше, ніж зазначене в замовленні на виробництво'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
		
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"остаток по заказу %КоличествоОстаток% %ЕдиницаИзмерения%,"
"превышает на %Количество% %ЕдиницаИзмерения%"
"%ЗаказНаПроизводство%';uk='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"залишок на замовлення %КоличествоОстаток% %ЕдиницаИзмерения%,"
"перевищує на %Количество% %ЕдиницаИзмерения%"
"%ЗаказНаПроизводство%'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструПотребностьВЗапасах(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Оформлено больше, чем есть потребность в запасах';uk='Оформлене більше, чим є потреба в запасах'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
	
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"потребность %КоличествоОстаток% %ЕдиницаИзмерения%,"
"превышает на %Количество% %ЕдиницаИзмерения%"
"%ЗаказПокупателя%';uk='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"потреба %КоличествоОстаток% %ЕдиницаИзмерения%,"
"перевищує на %Количество% %ЕдиницаИзмерения%"
"%ЗаказПокупателя%'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструРазмещениеЗаказов(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Оформлено больше, чем размещено запасов в заказах';uk='Оформлене більше, ніж розміщене запасів у замовленнях'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
...
"превышает на %Количество% %ЕдиницаИзмерения%"
"по %ЗаказПокупателя%';uk='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"розміщена %КоличествоОстаток% %ЕдиницаИзмерения%"
"в %ИсточникОбеспечения%,"
"перевищує на %Количество% %ЕдиницаИзмерения%"
"по %ЗаказПокупателя%'");
...
"остаток %СуммаОстаток% %Валюта%,"
"недостаточно %Сумма% %Валюта%';uk='%СчетКасса%: %ПредставлениеСчетаКассы%,"
"залишок %СуммаОстаток% %Валюта%,"
"недостатньо %Сумма% %Валюта%'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструДенежныеСредстваВКассахККМ(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Не хватает денежных средств в кассе ККМ';uk='Не вистачає коштів у касі ККМ'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
	
	ШаблонСообщения = НСтр("ru='Касса ККМ: %ПредставлениеКассыККМ%,"
"остаток %СуммаОстаток% %Валюта%,"
"недостаточно %Сумма% %Валюта%';uk='Каса ККМ: %ПредставлениеКассыККМ%,"
"залишок %СуммаОстаток% %Валюта%,"
"недостатньо %Сумма% %Валюта%'"
	);
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструРасчетыСПодотчетниками(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Расчеты с подотчетным лицом станут отрицательными';uk='Розрахунки з підзвітною особою стануть негативними'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
	
	ШаблонСообщения = НСтр("ru='%ПредставлениеПодотчетногоЛица%,"
"Остаток выданных подотчетному лицу денежных средств: %ОстатокУПодотчетногоЛица% %ВалютаПредставление%';uk='%ПредставлениеПодотчетногоЛица%,"
"Залишок виданих підзвітній особі коштів: %ОстатокУПодотчетногоЛица% %ВалютаПредставление%'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструРасчетыСПоставщиками(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Нет возможности зафиксировать расчеты с поставщиками';uk='Немає можливості зафіксувати розрахунки з постачальниками'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Если ВыборкаЗаписей.ДвиженияДенежныхДокументов Тогда
			...
			Если ВыборкаЗаписей.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
				ТекстСообщения = НСтр("ru='%ПредставлениеКонтрагента% - остаток задолженности перед поставщиком по документу расчетов меньше оплаченной суммы."
"Разнесенная сумма платежа: %СуммаВалПриЗаписи% %ВалютаПредставление%."
"Остаток задолженности перед поставщиком: %СуммаОстаткаЗадолженности% %ВалютаПредставление%.';uk='%ПредставлениеКонтрагента% - залишок заборгованості перед постачальником по документу розрахунків менше оплаченої суми."
"Рознесена сума платежу: %СуммаВалПриЗаписи% %ВалютаПредставление%."
"Залишок заборгованості перед постачальником: %СуммаОстаткаЗадолженности% %ВалютаПредставление%.'"
				);
			...
			Если ВыборкаЗаписей.ТипРасчетов = Перечисления.ТипыРасчетов.Аванс Тогда
				Если ВыборкаЗаписей.СуммаНепогашенныхАвансов = 0 Тогда
					ТекстСообщения = НСтр("ru='%ПредставлениеКонтрагента% - выданные по документу авансы поставщику уже полностью зачтены в товарных документах.';uk='%ПредставлениеКонтрагента% - видані по документу аванси постачальникові вже повністю зараховані в товарних документах.'"
					);
				Иначе
					ТекстСообщения = НСтр("ru='%ПредставлениеКонтрагента% - выданные по документу авансы поставщику уже частично зачтены в товарных документах."
"Остаток незачтенных авансов: %СуммаНепогашенныхАвансов% %ВалютаПредставление%.';uk='%ПредставлениеКонтрагента% - видані по документу аванси постачальникові вже частково зараховані в товарних документах."
"Залишок незарахованих авансів: %СуммаНепогашенныхАвансов% %ВалютаПредставление%.'"
					);
		...
		Иначе
			...
			Если ВыборкаЗаписей.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
				Если ВыборкаЗаписей.СуммаНепогашеннойЗадолженности = 0 Тогда
					ТекстСообщения = НСтр("ru='%ПредставлениеКонтрагента% - задолженность перед поставщиком по документу уже полностью оплачена.';uk='%ПредставлениеКонтрагента% - заборгованість перед постачальником по документу вже повністю оплачена.'"
					);
				Иначе
					ТекстСообщения = НСтр("ru='%ПредставлениеКонтрагента% - задолженность перед поставщиком по документу уже частично оплачена."
"Остаток непогашенной суммы задолженности: %СуммаНепогашеннойЗадолженности% %ВалютаПредставление%.';uk='%ПредставлениеКонтрагента% - заборгованість перед постачальником по документу вже частково оплачена."
"Залишок непогашеної суми заборгованості: %СуммаНепогашеннойЗадолженности% %ВалютаПредставление%.'"
					);
			...
			КонецЕсли;
			Если ВыборкаЗаписей.ТипРасчетов = Перечисления.ТипыРасчетов.Аванс Тогда
				ТекстСообщения = НСтр("ru='%ПредставлениеКонтрагента% - зачитываемая сумма авансов не может быть больше остатка выданных авансов поставщику."
"Зачитываемая сумма: %СуммаВалПриЗаписи% %ВалютаПредставление%"
"Остаток выданных авансов: %СуммаВыданныхАвансов% %ВалютаПредставление%.';uk='%ПредставлениеКонтрагента% - сума авансів, що зачитуються, не може бути більше залишку виданих авансів постачальникові."
"Сума, що зачитується: %СуммаВалПриЗаписи% %ВалютаПредставление%"
"Залишок виданих авансів: %СуммаВыданныхАвансов% %ВалютаПредставление%.'"
				);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаВалПриЗаписи%", Строка(ВыборкаЗаписей.СуммаВалПриЗаписи));
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструРасчетыСПокупателями(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Нет возможности зафиксировать расчеты с покупателями';uk='Немає можливості зафіксувати розрахунки з покупцями'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Если ВыборкаЗаписей.ДвиженияДенежныхДокументов Тогда
			...
			Если ВыборкаЗаписей.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
				ТекстСообщения = НСтр("ru='%ПредставлениеКонтрагента% - остаток задолженность покупателя по документу расчетов меньше разнесенной суммы платежа."
"Разнесенная сумма платежа: %СуммаВалПриЗаписи% %ВалютаПредставление%."
"Остаток задолженности покупателя: %СуммаОстаткаЗадолженности% %ВалютаПредставление%.';uk='%ПредставлениеКонтрагента% - залишок заборгованість покупця по документу розрахунків менше рознесеної суми платежу."
"Рознесена сума платежу: %СуммаВалПриЗаписи% %ВалютаПредставление%."
"Залишок заборгованості покупця: %СуммаОстаткаЗадолженности% %ВалютаПредставление%.'"
				);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаВалПриЗаписи%", Строка(ВыборкаЗаписей.СуммаВалПриЗаписи));
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаОстаткаЗадолженности%", Строка(ВыборкаЗаписей.СуммаОстаткаЗадолженности));
			КонецЕсли;
			...
			Если ВыборкаЗаписей.ТипРасчетов = Перечисления.ТипыРасчетов.Аванс Тогда
				Если ВыборкаЗаписей.СуммаНепогашенныхАвансов = 0 Тогда
					ТекстСообщения = НСтр("ru='%ПредставлениеКонтрагента% - полученные по документу авансы от покупателя уже полностью зачтены в товарных документах.';uk='%ПредставлениеКонтрагента% - отримані по документу аванси від покупця вже повністю зараховані в товарних документах.'"
					);
				Иначе
					ТекстСообщения = НСтр("ru='%ПредставлениеКонтрагента% - полученные по документу авансы от покупателя уже частично зачтены в товарных документах."
"Остаток незачтенных авансов: %СуммаНепогашенныхАвансов% %ВалютаПредставление%.';uk='%ПредставлениеКонтрагента% - отримані по документу аванси від покупця вже частково зараховані в товарних документах."
"Залишок незарахованих авансів: %СуммаНепогашенныхАвансов% %ВалютаПредставление%.'"
					);
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаНепогашенныхАвансов%", Строка(ВыборкаЗаписей.СуммаНепогашенныхАвансов));
				КонецЕсли;
		...
		Иначе
			...
			Если ВыборкаЗаписей.ТипРасчетов = Перечисления.ТипыРасчетов.Долг Тогда
				Если ВыборкаЗаписей.СуммаНепогашеннойЗадолженности = 0 Тогда
					ТекстСообщения = НСтр("ru='%ПредставлениеКонтрагента% - задолженность покупателя по документу уже полностью оплачена.';uk='%ПредставлениеКонтрагента% - заборгованість покупця по документу вже повністю оплачена.'"
					);
				Иначе
					ТекстСообщения = НСтр("ru='%ПредставлениеКонтрагента% - задолженность покупателя по документу уже частично оплачена."
"Остаток непогашенной суммы задолженности: %СуммаНепогашеннойЗадолженности% %ВалютаПредставление%.';uk='%ПредставлениеКонтрагента% - заборгованість покупця по документу вже частково оплачена."
"Залишок непогашеної суми заборгованості: %СуммаНепогашеннойЗадолженности% %ВалютаПредставление%.'"
					);
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаНепогашеннойЗадолженности%", Строка(ВыборкаЗаписей.СуммаНепогашеннойЗадолженности));
				КонецЕсли;
			...
			Если ВыборкаЗаписей.ТипРасчетов = Перечисления.ТипыРасчетов.Аванс Тогда
				ТекстСообщения = НСтр("ru='%ПредставлениеКонтрагента% - зачитываемая сумма авансов не может быть больше остатка полученных авансов от покупателя."
"Зачитываемая сумма: %СуммаВалПриЗаписи% %ВалютаПредставление%"
"Остаток полученных авансов: %СуммаПолученныхАвансов% %ВалютаПредставление%.';uk='%ПредставлениеКонтрагента% - сума авансів, що зачитуються, не може бути більше залишку отриманих авансів від покупця."
"Сума, що зачитується: %СуммаВалПриЗаписи% %ВалютаПредставление%"
"Залишок отриманих авансів: %СуммаПолученныхАвансов% %ВалютаПредставление%.'"
				);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаВалПриЗаписи%", Строка(ВыборкаЗаписей.СуммаВалПриЗаписи));
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструВнеоборотныеАктивы(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Возможно имущество уже списано или передано';uk='Можливе майно вже списане або передане'");
	СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
		
	ШаблонСообщения = НСтр("ru='Имущество: %НоменклатураХарактеристикаПартия%,"
"остаточная стоимость имущества: %Стоимость%';uk='Майно: %НоменклатураХарактеристикаПартия%,"
"залишкова вартість майна: %Стоимость%'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструСуммовойУчетВРознице(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Задолженность розничной точки %СтруктурнаяЕдиницаПредставление% уже погашена';uk='Заборгованість роздрібної точки %СтруктурнаяЕдиницаПредставление% вже погашена'");
	
	ШаблонСообщения = НСтр("ru='Остаток задолженности (суммовой учет): %ОстатокВРознице% %ВалютаПредставление%';uk='Залишок заборгованості (сумовий облік): %ОстатокВРознице% %ВалютаПредставление%'");
...
Функция РассчитатьЗначениеПараметра(СтруктураПараметров, ПараметрРасчета, ТекстОшибки = "") Экспорт
	...
	Для каждого ПараметрЗапроса Из ПараметрРасчета.ПараметрыЗапроса Цикл
		...
		Иначе
			...
			Если СтруктураПараметров.Свойство(ПараметрЗапроса.Имя) Тогда
				
				СтрокаПериод = ПараметрРасчета.ПериодыОтбораДанных.Найти(ПараметрЗапроса.Имя, "ИмяДатаГраницы");
				Если СтрокаПериод <> Неопределено  Тогда
					
					Если СтрокаПериод.ПериодСдвиг <> 0 Тогда
						НовыйПериод = ДобавитьИнтервал(СтруктураПараметров[ПараметрЗапроса.Имя], СтрокаПериод.ПериодСмещения, СтрокаПериод.ПериодСдвиг);
						Запрос.УстановитьПараметр(ПараметрЗапроса.Имя, НовыйПериод);
					Иначе
						Запрос.УстановитьПараметр(ПараметрЗапроса.Имя, СтруктураПараметров[ПараметрЗапроса.Имя]);
					КонецЕсли;
			...
			Иначе
				...
				Сообщение = Новый СообщениеПользователю();
				Сообщение.Текст = НСтр("ru='Не задано значение параметра запроса ';uk='Не задане значення параметра запиту'") + ПараметрЗапроса.Имя + ТекстОшибки;
				Сообщение.Сообщить();
		...
		Пока ВыборкаНоменклатуры.Следующий() Цикл
			...
			Если СтрокиРекурсивногоВхождения.Количество() <> 0 Тогда
				...
				Для каждого СтрокаРекВх Из СтрокиРекурсивногоВхождения Цикл
					...
					НСтр("ru='Обнаружено рекурсивное вхождение элемента %1 в элемент %2!';uk='Обнаружено рекурсивное вхождение элемента %1 в элемент %2!'"),
...
Функция ДанныеФизЛица(Организация, ФизЛицо, ДатаСреза, ФИОКратко = Истина) Экспорт
	...
	ЗапросПоЛицам.УстановитьПараметр("Организация", УправлениеНебольшойФирмойСервер.ПолучитьОрганизацию(Организация));
...
//
Функция ПолучитьНомерНаПечать(Знач НомерДокумента, Префикс) Экспорт

	//Для обратной совместимости. Ранее в функцию передавался сам объект-документ
	Если НЕ ТипЗнч(НомерДокумента) = Тип("Строка") Тогда 
		НомерДокумента = НомерДокумента.Номер;
	КонецЕсли;
...
Функция ОписаниеОрганизации(СписокСведений, Список = "", СПрефиксом = Истина) Экспорт
	...
	Если ПустаяСтрока(Список) Тогда
		Список = "ПолноеНаименование,ИНН,КодПоЕДРПОУ,Свидетельство,ЮридическийАдрес,Телефоны,Факс,НомерСчета,Банк,МФО,КоррСчет,ПлательщикНалогаНаПрибыль";
	КонецЕсли; 
	...
	СоответствиеПараметров.Вставить("РегистрационныйНомер",		" Рег.№ ");
	...
	Для Счетчик = 1 по ЧислоПараметров Цикл
		...
		Если ПозЗапятой > 0  Тогда
			...
			Исключение
				...
				Сообщение = Новый СообщениеПользователю();
				Сообщение.Текст = НСтр("ru='Не удалось определить значение параметра организации: ';uk='Не вдалося визначити значення параметра організації: '") + ИмяПараметра;
				Сообщение.Сообщить();
...
Функция СведенияОЮрФизЛице(ЮрФизЛицо, ДатаПериода, ДляФизЛицаТолькоИнициалы = Истина, БанковскийСчет = Неопределено, КодЯзыка = "ru") Экспорт
	...
	Сведения = Новый Структура("Представление, ПолноеНаименование, КодПоЕДРПОУ, ИНН, НомерСвидетельства, Телефоны, Факс, ЮридическийАдрес, Банк, МФО, КоррСчет, ТекстКорреспондента, НомерСчета, АдресБанка, ЭлектроннаяПочта");
	Запрос   = Новый Запрос;
	Данные   = Неопределено;
	...
	|	Организации.ИНН,
	|	Организации.НомерСвидетельства,
	...
	Сведения.Вставить("КодПоЕДРПОУ", 		"");
	Сведения.Вставить("НомерСвидетельства",       		"");
	
	Если НЕ (ЮрФизЛицо.Метаданные().Реквизиты.Найти("ЮридическоеФизическоеЛицо") = Неопределено) Тогда

		Сведения.КодПоЕДРПОУ = Данные.КодПоЕДРПОУ;
		
		Если ИмяСправочника = "Организации" Тогда
			
			Сведения.НомерСвидетельства = НСтр("ru='свидетельство';uk='свідоцтво'") + " " + Данные.СвидетельствоСерияНомер + " " + НСтр("ru='от';uk='від'") + " " + Формат(Данные.СвидетельствоДатаВыдачи, "ДЛФ=D");
	...
	Если  ИмяСправочника = "Организации" Тогда
		ДанныеОНалогообложении       = РегистрыСведений.СистемыНалогообложенияОрганизаций.СрезПоследних(?(ЗначениеЗаполнено(ДатаПериода),ДатаПериода,ТекущаяДата()), Новый Структура("Организация",ЮрФизЛицо));
		
	    Если ДанныеОНалогообложении.Количество() > 0 Тогда
			Если ДанныеОНалогообложении[0].СистемаНалогообложения = Перечисления.СистемыНалогообложения.Общая Тогда
				Если ДанныеОНалогообложении[0].ПлательщикЕНВД Тогда
					ПлательщикНалогаНаПрибыль = Ложь;
				Иначе	
		...
		КонецЕсли;
	ИначеЕсли ИмяСправочника = "Контрагенты" Тогда
		ДанныеОНалогообложении       = РегистрыСведений.СистемыНалогообложенияКонтрагентов.СрезПоследних(?(ЗначениеЗаполнено(ДатаПериода),ДатаПериода,ТекущаяДата()), Новый Структура("Контрагент",ЮрФизЛицо));
	...
	Если Данные <> Неопределено Тогда
		...
		Сведения.Вставить("ПлательщикНалогаНаПрибыль",   ?(ПлательщикНалогаНаПрибыль = Истина, ?(КодЯзыка = "ru", "Является ", "Є "), ?(КодЯзыка = "ru", "Не является ", "Не є ")) 
		...
		Если ИмяСправочника = "Контрагенты" Тогда
			...
			Если ЗначениеЗаполнено(Данные.СтранаРегистрации) 
				И Данные.СтранаРегистрации<>Справочники.СтраныМира.Украина Тогда
...
"Для документа %1 печатная форма не сформирована.';uk='__________________"
"Друк універсального передатного документа доступен з 1 січня 2013. "
"Для документа %1 друкована форма не сформована.'");
...
"%1, "
"т.к. действие доступно только для заказ-нарядов!';uk='Не можна надрукувати універсальний передатний документ "
"%1, "
"тому що дія доступна тільки для замовлення-наряду!'");
...
"Для документа %1 печатная форма не сформирована.';uk='__________________"
"Для друку універсального передатного документа необхідно заповнити контрагента. "
"Для документа %1 друкована форма не сформована.'");
...
"Для документа %1 печатная форма не сформирована.';uk='__________________"
"Друк універсального коректувального документа доступен з 1 січня 2013. "
"Для документа %1 друкована форма не сформована.'");
...
"Для документа %1 печатная форма не сформирована.';uk='__________________"
"Для друку універсального коректувального документа необхідно заповнити контрагента. "
"Для документа %1 друкована форма не сформована.'");
...
Процедура УстановитьСтандартныеНастройкиПодбора(ТекущийПользователь) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ТекущийПользователь) Тогда
		...
		Нстр("ru='Неуказан пользователь, для которого устанавливаются настройки подбора по умолчанию.';uk='Незазначений користувач, для якого встановлюються налаштування підбору за замовчуванням.'")
		);
...
Процедура ЗаполнитьТемуТекстОтправки(ПараметрыОтправки, МассивОбъектов, КоллекцияПечатныхФорм)
	...
	Тема = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 %2 %3';uk='%1 %2 %3'"),
		ПредставлениеЗаголовкаДокумента,
		ПредставлениеДляКого,
		ПредставлениеОтКого
		);
...
Функция ОчиститьДанныеВБазе() Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Удаление данных';uk='Видалення даних'"), 
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструРасчетыПоЭквайрингу(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	Иначе
		ЗаголовокОшибки = НСтр("ru='Информация:';uk='Інформація:'");
		Если ТипЗнч(ДокОбъект) = Тип("ДокументОбъект.ОперацияПоПлатежнымКартам") Тогда
			ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Обнаружен отрицательный остаток в расчетах по платежным картам';uk='Виявлений негативний залишок у розрахунках по платіжних картках'");
		Иначе
			ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Есть остаток в расчетах по платежным картам';uk='Є залишок у розрахунках по платіжних картках'");
		КОнецЕсли;
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Если ВыборкаЗаписей.СуммаВалОстаток < 0 Или ВыборкаЗаписей.КомиссияВалОстаток < 0 Тогда
			Если ПоказатьСообщениеОбОшибки Тогда
				...
				ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
				ТекстЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Нет возможности зафиксировать расчеты по платежным картам';uk='Немає можливості зафіксувати розрахунки по платіжних картках'");
				СообщитьОбОшибке(ДокОбъект, ТекстЗаголовкаСообщения, , , , Отказ);
			КонецЕсли;
			
			ТекстСообщения = НСтр("ru='%ПредставлениеОперацииОплаты%"
"Разнесенная сумма платежа: %СуммаВалПриЗаписи% %ВалютаПредставление% (комиссия %КомиссияВалПриЗаписи%)."
"Остаток суммы операции оплаты: %СуммаОстаткаЗадолженности% %ВалютаПредставление% (комиссия %СуммаОстаткаКомиссии%).';uk='%ПредставлениеОперацииОплаты%"
"Рознесена сума платежу: %СуммаВалПриЗаписи% %ВалютаПредставление% (комісія %КомиссияВалПриЗаписи%)."
"Залишок суми операції оплати: %СуммаОстаткаЗадолженности% %ВалютаПредставление% (комісія %СуммаОстаткаКомиссии%).'"
			);
...
"Остаток суммы операции оплаты: %КомиссияВалОстаток% %ВалютаПредставление%.';uk='%ПредставлениеОперацииОплаты%"
"Після проведення залишок боргу банку склав: %СуммаВалОстаток% %ВалютаПредставление%."
"Залишок суми операції оплати: %КомиссияВалОстаток% %ВалютаПредставление%.'"
			);
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструКонтрольОперацийЭквайринга(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		ТекстСообщения = НСтр("ru='%ВидПлатежнойКартыПредставление% %НомерПлатежнойКартыПредставление% (%ДатаПлатежаПредставление%) - остаток суммы по карте меньше разнесенной суммы платежа."
"Разнесенная сумма платежа: %СуммаВалПриЗаписи% %ВалютаПредставление%."
"Остаток суммы по карте: %СуммаОстаткаЗадолженности% %ВалютаПредставление%.';uk='%ВидПлатежнойКартыПредставление% %НомерПлатежнойКартыПредставление% (%ДатаПлатежаПредставление%) - залишок суми по картці менше рознесеної суми платежу."
"Рознесена сума платежу: %СуммаВалПриЗаписи% %ВалютаПредставление%."
"Залишок суми по картці: %СуммаОстаткаЗадолженности% %ВалютаПредставление%.'"
		);
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаВалПриЗаписи%", Строка(-ВыборкаЗаписей.СуммаВалПриЗаписи));
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуммаОстаткаЗадолженности%", Строка(ВыборкаЗаписей.СуммаОстаткаЗадолженности));
...
Функция ОтказСнятьФункциональнаяУчетВалютныхОпераций() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ТекстСообщения = НСтр("ru='В базе заведены валюты, отличные от национальной. Изменение настройки невозможно!';uk='У базі заведені валюти, відмінні від національної. Зміна налаштувань неможлива!'");
	КонецЕсли;
...
Функция ОтказИзменитьВалютаУчета() Экспорт
	...
	Если ЕстьДвижения Тогда
		...
		ТекстСообщения = НСтр("ru='В базе есть движения по ""суммовым"" регистрам накопления! Изменение валюты учета запрещено!';uk='У базі є рухи по ""сумовим"" регістрам накопичення! Зміна валюти обліку заборонена!'");	
...
Функция ОтказСнятиеУчетПоНесколькимСкладам() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='В базе используются склады, отличные от основного! Снятие опции запрещено!';uk='У базі використовуються склади, відмінні від основного! Зняття опції заборонено!'");
...
Функция ОтказСнятьФункциональнаяОпцияУчетВРазличныхЕдиницахИзмерения() Экспорт
	...
	Пока ВыборкаЕдиницыИзмерения.Следующий() Цикл
		...
		Если ТаблицаСсылок.Количество() > 0 Тогда
			...
			ТекстОшибки = НСтр("ru='В приложении введены документы в пользовательских единицах измерения! Снятие опции запрещено!';uk='У додатку введені документи в користувацьких одиницях виміру! Зняття опції заборонено!'");
			Прервать;
...
Функция ОтказСнятьФункциональнаяОпцияПередачаЗапасовНаОтветхранение() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='Регистр накопления ""Запасы переданные"" содержит информацию о передаче на ответхранение! Снятие флага запрещено!';uk='Регістр накопичення ""Запаси передані"" містить інформацію про передачу на відповідзберігання! Зняття прапорця заборонене!'");
...
Функция ОтказСнятьФункциональнаяОпцияПриемЗапасовНаОтветхранение() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='Регистр накопления ""Запасы принятые"" содержит информацию о приеме на ответхранение! Снятие флага запрещено!';uk='Регістр накопичення ""Запаси прийняті"" містить інформацію про приймання на відповідзберігання! Зняття прапорця заборонене!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьОрдерныйСклад() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='В базе присутствуют ордерные склады! Снятие флага запрещено!';uk='У базі присутні ордерні склади! Зняття прапорця заборонено!'");
	...
	Если НЕ РезультатЗапросаЗапасыКПоступлениюНаСклады.Пустой()
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В базе присутствуют движения по ордерному складу! Снятие флага запрещено!';uk='У базі присутні рухи по ордерному складу! Зняття прапорця заборонено!'");
...
Функция ОтказСнятьФункциональнаяОпцияРезервированиеЗапасов() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='В базе содержится информация о резервах! Снятие флага запрещено!';uk='У базі міститься інформація про резерви! Зняття прапорця заборонено!'");
...
Функция ОтказСнятьФункциональнаяОпцияУчетЦенКонтрагентов() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='В базе содержатся договоры контрагентов с установлинными видами входящих цен! Снятие флага запрещено!';uk='У базі містяться договори контрагентів з встановленими видами вхідних цін! Зняття прапорця заборонено!'");
...
Функция ОтказСнятьФункциональнаяОпцияУчетПоЯчейкам() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='В базе содержатся движения по ячейкам! Снятие флага запрещено!';uk='У базі містяться рухи по комірках! Зняття прапорця заборонено!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьХарактеристики() Экспорт
	...
	Если ЕстьДвижения Тогда
		...
		ТекстОшибки = НСтр("ru='В базе есть движения по характеристикам! Снятие флага запрещено!';uk='У базі є рухи по характеристиках! Зняття прапорця заборонено!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьПартии() Экспорт
	...
	Если ЕстьДвижения Тогда
		...
		ТекстОшибки = НСтр("ru='В базе есть движения по партиям! Снятие флага запрещено!';uk='У базі є рухи по партіях! Зняття прапорця заборонено!'");
	...
	Если ПолучитьФункциональнуюОпцию("ПриемТоваровНаКомиссию") Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + 
			НСтр("ru='Включена опция ""Прием товаров на комиссию"" (раздел Продажи)! Снятие флага запрещено!';uk='Включена опція ""Приймання товарів на комісію"" (розділ Продажу)! Зняття прапорця заборонено!'");
	...
	Если ПолучитьФункциональнуюОпцию("ПереработкаДавальческогоСырья") Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + 
			НСтр("ru='Включена опция ""Переработка давальческого сырья"" (раздел Производство)! Снятие флага запрещено!';uk='Включена опція ""Переробка давальницької сировини"" (розділ Виробництво)! Зняття прапорця заборонено!'");
...
Функция ОтказСнятьФункциональнаяОпцияПередачаСырьяВПереработку() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='Регистр накопления ""Запасы переданные"" содержит информацию о передаче в переработку! Снятие флага запрещено!';uk='Регістр накопичення ""Запаси передані"" містить інформацію про передачу в переробку! Зняття прапорця заборонене!'");	
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьСкидкиНаценки() Экспорт
	...
	Пока ВыборкаВидыСкидокНаценок.Следующий() Цикл
		...
		Если ТаблицаСсылок.Количество() > 0 Тогда
			...
			ТекстОшибки = НСтр("ru='В базе используются виды скидок, наценок! Снятие опции запрещено!';uk='У базі використовуються види знижок, націнок! Зняття опції заборонено!'");
			Прервать;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В документах используются скидки и наценки! Снятие опции запрещено!';uk='У документах використовуються знижки й націнки! Зняття опції заборонено!'");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьДисконтныеКарты") Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + 
			НСтр("ru='Включена опция ""Использовать дисконтные карты""! Снятие флага запрещено!';uk='Включена опція ""Використовувати дисконтні картки""! Зняття прапорця заборонено!'");
...
Функция ОтказСнятьФункциональнаяОпцияПередачаТоваровНаКомиссию() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='Регистр накопления ""Запасы переданные"" содержит информацию о передаче на комиссию! Снятие флага запрещено!';uk='Регістр накопичення ""Запаси передані"" містить інформацію про передачу на комісію! Зняття прапорця заборонене!'");
...
Функция ОтказСнятьФункциональнаяОпцияПриемТоваровНаКомиссию() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='Регистр накопления ""Запасы принятые"" содержит информацию о приеме на комиссию! Снятие флага запрещено!';uk='Регістр накопичення ""Запаси прийняті"" містить інформацію про приймання на комісію! Зняття прапорця заборонене!'");
...
Функция ОтказСнятьФункциональнаяОпцияУчетРозничныхПродаж() Экспорт
	...
	Если ВыборкаЗапроса.Следующий()
		...
		ТекстОшибки = НСтр("ru='В базе есть движения или объекты, относящиеся к учету операций розничных продаж! Снятие флага запрещено!';uk='У базі є рухи або об`єкти, що відносяться до обліку операцій роздрібних продажів! Зняття прапорця заборонено!'");
	...
	Пока ВыборкаНаправленияДеятельности.Следующий() Цикл
		...
		Если ВыборкаНаправленияДеятельности.Ссылка <> Справочники.НаправленияДеятельности.ОсновноеНаправление
			...
			Если ТаблицаСсылок.Количество() > 0 Тогда
				...
				ТекстОшибки = НСтр("ru='В базе используются направления деятельности, отличные от основного! Снятие опции запрещено!';uk='У базі використовуються напрямки діяльності, відмінні від основного! Зняття опції заборонено!'");
				Прервать;
...
Функция ОтказСнятиеУчетПоНесколькимПодразделениям() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='В базе используются подразделения, отличные от основного! Снятие опции запрещено!';uk='У базі використовуються підрозділи, відмінні від основного! Зняття опції заборонено!'");
...
Функция ОтказСнятьФункциональнаяОпцияУчетВнеоборотныхАктивов() Экспорт
	...
	Если Отказ Тогда
		...
		ТекстОшибки = НСтр("ru='В базе присутствуют движения по внеоборотным активам! Снятие флага запрещено!';uk='У базі присутні рухи по необоротним активам! Зняття прапорця заборонено!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьПодсистемуРаботы() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='В информационной базе присутствуют документы ""Заказ - наряд""! Снятие флага ""Работы"" запрещено!';uk='В інформаційній базі присутні документи ""Замовлення - наряд""! Зняття прапорця ""Роботи"" заборонено!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьПодсистемуПроизводство() Экспорт
	...
	Если НЕ МассивРезультатов[0].Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='В информационной базе присутствуют документы ""Заказ на производство""! Снятие флага ""Производство"" запрещено!';uk='В інформаційній базі присутні документи ""Замовлення на виробництво""! Зняття прапорця ""Виробництво"" заборонено!'");
	...
	Если НЕ МассивРезультатов[1].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют документы ""Производство""! Снятие флага ""Производство"" запрещено!';uk='В інформаційній базі присутні документи ""Виробництво""! Зняття прапорця ""Виробництво"" заборонено!'");
	...
	Если НЕ МассивРезультатов[2].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют документы ""Распределение затрат""! Снятие флага ""Производство"" запрещено!';uk='В інформаційній базі присутні документи ""Розподіл витрат""! Зняття прапорця ""Виробництво"" заборонено!'");
	...
	Если НЕ МассивРезультатов[3].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют документы ""Заказ покупателя"" с видом операции ""Заказ на переработку""! Снятие флага ""Производство"" запрещено!';uk='В інформаційній базі присутні документи ""Замовлення покупця"" з видом операції ""Замовлення на переробку""! Зняття прапорця ""Виробництво"" заборонено!'");
	...
	Если НЕ МассивРезультатов[4].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют документы ""Сдельный наряд""! Снятие флага ""Производство"" запрещено!';uk='В інформаційній базі присутні документи ""Відрядний наряд""! Зняття прапорця ""Виробництво"" заборонено!'");
	...
	Если НЕ МассивРезультатов[5].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют документы ""Перемещение по ячейкам"", где структурная единица компании имеет тип Подразделение! Снятие флага ""Производство"" запрещено!';uk='В інформаційній базі присутні документи ""Переміщення по комірках"", де структурна одиниця компанії має тип Підрозділ! Зняття прапорця ""Виробництво"" заборонено!'");
	...
	Если НЕ МассивРезультатов[6].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют документы ""Перемещение запасов"", где структурная единица компании имеет тип Подразделение и/или счет затрат имеет тип Косвенные затраты! Снятие флага ""Производство"" запрещено!';uk='В інформаційній базі присутні документи ""Переміщення запасів"", де структурна одиниця компанії має тип Підрозділ і/або рахунок витрат має тип Непрямі витрати! Зняття прапорця ""Виробництво"" заборонено!'");
	...
	Если НЕ МассивРезультатов[7].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют документы ""Ввод начальных остатков"", где структурная единица компании имеет тип Подразделение  и/или счет затрат имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!';uk='В інформаційній базі присутні документи ""Введення початкових залишків"", де структурна одиниця компанії має тип Підрозділ  і/або рахунок витрат має тип Непрямі витрати або Незавершене виробництво! Зняття прапорця ""Виробництво"" заборонено!'");
	...
	Если НЕ МассивРезультатов[8].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют документы ""Принятие к учету имущества"", где счет затрат имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!';uk='В інформаційній базі присутні документи ""Прийняття до обліку майна"", де рахунок витрат має тип Непрямі витрати або Незавершене виробництво! Зняття прапорця ""Виробництво"" заборонено!'");
	...
	Если НЕ МассивРезультатов[9].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют документы ""Оприходование запасов"", где структурная единица компании имеет тип Подразделение! Снятие флага ""Производство"" запрещено!';uk='В інформаційній базі присутні документи ""Оприбуткування запасів"", де структурна одиниця компанії має тип Підрозділ! Зняття прапорця ""Виробництво"" заборонено!'");
	...
	Если НЕ МассивРезультатов[10].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют документы ""Бюджет"", где счета затрат имеют тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!';uk='В інформаційній базі присутні документи ""Бюджет"", де рахунки витрат мають тип Непрямі витрати або Незавершене виробництво! Зняття прапорця ""Виробництво"" заборонено!'");
	...
	Если НЕ МассивРезультатов[11].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют документы ""Изменение параметров имущества"", где счет затрат имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!';uk='В інформаційній базі присутні документи ""Зміна параметрів майна"", де рахунок витрат має тип Непрямі витрати або Незавершене виробництво! Зняття прапорця ""Виробництво"" заборонено!'");
	...
	Если НЕ МассивРезультатов[12].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют документы ""Начисление зарплаты"", где счет затрат имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!';uk='В інформаційній базі присутні документи ""Нарахування зарплати"", де рахунок витрат має тип Непрямі витрати або Незавершене виробництво! Зняття прапорця ""Виробництво"" заборонено!'");
	...
	Если НЕ МассивРезультатов[13].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют документы ""Начисление налогов"", где счет затрат имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!';uk='В інформаційній базі присутні документи ""Нарахування податків"", де рахунок витрат має тип Непрямі витрати або Незавершене виробництво! Зняття прапорця ""Виробництво"" заборонено!'");
	...
	Если НЕ МассивРезультатов[14].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют документы ""Операция"", где счет затрат имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!';uk='В інформаційній базі присутні документи ""Операція"", де рахунок витрат має тип Непрямі витрати або Незавершене виробництво! Зняття прапорця ""Виробництво"" заборонено!'");
	...
	Если НЕ МассивРезультатов[15].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют документы ""Прочие затраты (расходы)"", где счет затрат имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!';uk='В інформаційній базі присутні документи ""Інші витрати"", де рахунок витрат має тип Непрямі витрати або Незавершене виробництво! Зняття прапорця ""Виробництво"" заборонено!'");
	...
	Если НЕ МассивРезультатов[16].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют элементы справочника ""Номенклатура"", где счет учета затрат имеет тип Косвенные затраты, Незавершенное производство или способ пополнения запаса Производство! Снятие флага ""Производство"" запрещено!';uk='В інформаційній базі присутні елементи довідника ""Номенклатура"", де рахунок обліку витрат має тип Непрямі витрати, Незавершене виробництво або спосіб поповнення запасу Виробництво! Зняття прапорця ""Виробництво"" заборонено!'");
	...
	Если НЕ МассивРезультатов[17].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют элементы справочника ""Структурная единица"", где параметр автоперемещения (перемещение, комплектация) имеет тип Подразделение! Снятие флага ""Производство"" запрещено!';uk='В інформаційній базі присутні елементи довідника ""Структурна одиниця"", де параметр автопереміщення (переміщення, комплектація) має тип Підрозділ! Зняття прапорця ""Виробництво"" заборонено!'");
...
Функция ПроверитьДвиженияПоРегистрамПодсистемыПроизводство()
	...
	Если НЕ МассивРезультатов[0].Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='В информационной базе присутствуют движения по регистру ""Запасы"", где счет учета имеет тип Косвенные затраты или Незавершенное производство! Снятие флага ""Производство"" запрещено!';uk='В інформаційній базі присутні рухи по регістру ""Запаси"", де рахунок обліку має тип Непрямі витрати або Незавершене виробництво! Зняття прапорця ""Виробництво"" заборонено!'");
	...
	Если НЕ МассивРезультатов[1].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='Регистр накопления ""Запасы принятые"" содержит информацию о приеме в переработку! Снятие флага ""Производство"" запрещено!';uk='Регістр накопичення ""Запаси прийняті"" містить інформацію про прийом до переробки! Зняття прапорця ""Виробництво"" заборонене!'");
...
Функция ОтказСнятьФункциональнаяОпцияПереработкаДавальческогоСырья() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='Регистр накопления ""Запасы принятые"" содержит информацию о приеме в переработку! Снятие флага запрещено!';uk='Регістр накопичення ""Запаси прийняті"" містить інформацію про прийом до переробки! Зняття прапорця заборонене!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьТехоперации() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='В базе присутствует информация о загрузке рабочих центров или документы вида ""Сдельный наряд""! Снятие флага запрещено!';uk='У базі присутня інформація про завантаження робочих центрів або документи виду ""Відрядний наряд""! Зняття прапорця заборонено!'");
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + 
			НСтр("ru='В базе присутствует номенклатура с типом ""Операция""! Снятие флага запрещено!';uk='У базі присутня номенклатура з типом ""Операція""! Зняття прапорця заборонено!'");
...
Функция ПроверитьДвиженияПоРегистрамПодсистемыЗарплата() Экспорт
	...
	Если НЕ МассивРезультатов[0].Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='В информационной базе присутствуют движения по регистру ""Начисления и удержания""! Снятие флага ""Зарплата"" запрещено!';uk='В інформаційній базі присутні рухи по регістру ""Нарахування й утримання""! Зняття прапорця ""Зарплата"" заборонено!'");
	...
	Если НЕ МассивРезультатов[1].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют движения по регистру ""Расчеты с персоналом""! Снятие флага ""Зарплата"" запрещено!';uk='В інформаційній базі присутні рухи по регістру ""Розрахунки з персоналом""! Зняття прапорця ""Зарплата"" заборонено!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьПодсистемуЗарплата() Экспорт
	...
	Если НЕ МассивРезультатов[0].Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='В информационной базе присутствуют документы ""Начисление зарплаты""! Снятие флага ""Зарплата"" запрещено!';uk='В інформаційній базі присутні документи ""Нарахування зарплати""! Зняття прапорця ""Зарплата"" заборонено!'");
	...
	Если НЕ МассивРезультатов[1].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют документы ""Сдельный наряд"", которые начисляют зарплату сотрудникам! Снятие флага ""Зарплата"" запрещено!';uk='В інформаційній базі присутні документи ""Відрядний наряд"", які нараховують зарплату співробітникам! Зняття прапорця ""Зарплата"" заборонено!'");
	...
	Если НЕ МассивРезультатов[2].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют документы ""Заказ - наряд"", которые начисляют зарплату сотрудникам! Снятие флага ""Зарплата"" запрещено!';uk='В інформаційній базі присутні документи ""Замовлення - наряд"", які нараховують зарплату співробітникам! Зняття прапорця ""Зарплата"" заборонено!'");
	...
	Если НЕ МассивРезультатов[3].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) +  НСтр("ru='В информационной базе присутствуют документы ""Ввод начальных остатвков"", по разделу учета ""Расчеты с персоналом""! Снятие флага ""Зарплата"" запрещено!';uk='В інформаційній базі присутні документи ""Введення початкових залишків"", по розділу обліку ""Розрахунки з персоналом""! Зняття прапорця ""Зарплата"" заборонено!'");
	...
	Если НЕ МассивРезультатов[4].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют документы ""Расход из кассы"", с видом операции ""Зарплата по ведомости"" и/или ""Зарплата сотруднику""! Снятие флага ""Зарплата"" запрещено!';uk='В інформаційній базі присутні документи ""Витрата з каси"", з видом операції ""Зарплата по відомості"" і/або ""Зарплата співробітникові""! Зняття прапорця ""Зарплата"" заборонено!'");
	...
	Если НЕ МассивРезультатов[5].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют документы ""Расод со счета"", с видом операции ""Зарплата""! Снятие флага ""Зарплата"" запрещено!';uk='В інформаційній базі присутні документи ""Витрата з рахунку"", з видом операції ""Зарплата""! Зняття прапорця ""Зарплата"" заборонено!'");
	...
	Если НЕ МассивРезультатов[6].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе есть сотрудники с типом занятости ""Совместительство""! Снятие флага ""Зарплата"" запрещено!';uk='В інформаційній базі є співробітники з типом зайнятості ""Сумісництво""! Зняття прапорця ""Зарплата"" заборонено!'");	
	...
	Если НЕ МассивРезультатов[7].Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В информационной базе присутствуют элементы справочника ""Виды начислений и удержаний"" с типом ""Налог""! Снятие флага ""Зарплата"" запрещено!';uk='В інформаційній базі присутні елементи довідника ""Види нарахувань і втримань"" з типом ""Податок""! Зняття прапорця ""Зарплата"" заборонено!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьСовместительство() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='В базе есть сотрудники с типом занятости ""Совместительство""! Снятие флага запрещено!';uk='У базі є співробітники з типом зайнятості ""Сумісництво""! Зняття прапорця заборонено!'");
...
Функция ОтказСнятьФункциональнаяОпцияВестиУчетНалогаНаДоходыИВзносов() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='В базе присутствуют элементы справочника ""Виды начислений и удержаний"" с типом ""Налог""! Снятие флага запрещено!';uk='У базі присутні елементи довідника ""Види нарахувань і втримань"" з типом ""Податок""! Зняття прапорця заборонено!'");	
...
// 
Функция ПолучитьКалендарьПоПроизводственномуКалендарюУкраины() Экспорт
	
	ПроизводственныйКалендарьУкраины = КалендарныеГрафики.ПроизводственныйКалендарьУкраины();
	Если ПроизводственныйКалендарьУкраины = Неопределено Тогда
		
		ЗаписьЖурналаРегистрации(НСтр("ru='Не удалось заполнить данные графиков работы для организации на основании производственного календаря Украины.';uk='Не вдалося заповнити дані графіків роботи для організації на підставі виробничого календаря України.'"), 
			УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	|ГДЕ
	|	Календари.ПроизводственныйКалендарь = &ПроизводственныйКалендарьУкраины");
	
	Запрос.УстановитьПараметр("ПроизводственныйКалендарьУкраины", ПроизводственныйКалендарьУкраины);
	ВыборкаРезультатаЗапроса = Запрос.Выполнить().Выбрать();
...
КонецФункции // ПолучитьКалендарьПоПроизводственномуКалендарюУкраины()

#КонецОбласти

#Область РасчетЕСВ

// Возвращает процент ЕСВ
//
Функция ПолучитьПроцентЕСВ(Дата, Организация) Экспорт
	
	ПроцентЕСВ = 0;
	
	Данные = Новый Структура("Организация",Организация);
	ДанныеПоОрганизации = РегистрыСведений.СистемыНалогообложенияОрганизаций.СрезПоследних(Дата, Данные);
	Если ДанныеПоОрганизации.Количество() > 0 Тогда
		ПроцентЕСВ = ДанныеПоОрганизации[0].ПроцентЕСВ;
	КонецЕсли;
	
	Возврат ПроцентЕСВ;
	
КонецФункции // ПолучитьПроцентЕСВ()

Функция ПолучитьСуммуОкладМинимум(Период) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ПрожиточныйОкладМинимумСрезПоследних.Значение КАК Сумма
	|ИЗ
	|	РегистрСведений.ПрожиточныйМинимум.СрезПоследних(&Период) КАК ПрожиточныйОкладМинимумСрезПоследних";
	Запрос.УстановитьПараметр("Период", Период);
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		СуммаЕСВ = Выборка.Сумма;
	КонецЕсли;
	
	Возврат СуммаЕСВ;
	
КонецФункции // ПолучитьСуммуОкладМинимум()

Функция ПолучитьСписокДоступныхСтавок(Дата, Налог) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	НДФЛСтавкиНалоговСрезПоследних.Ставка1
	|ИЗ
	|	РегистрСведений.НДФЛСтавкиНалогов.СрезПоследних(&Дата, Налог = &Налог) КАК НДФЛСтавкиНалоговСрезПоследних";
	Запрос.УстановитьПараметр("Дата", Дата);
	Запрос.УстановитьПараметр("Налог", Налог);
	тзЗапроса = Запрос.Выполнить().Выгрузить();
	массивЗапроса = тзЗапроса.ВыгрузитьКолонку("Ставка1");
...
Функция ЕстьПроцедураСоздатьПустуюВременнуюТаблицуИзменение(ПолноеИмяРегистра)
	...
	// Гунько 2020-12-27
	РегистрыСПроцедурой.Добавить(Метаданные.РегистрыНакопления.гвРезервированиеТоваровПоСерийныхНомерах.ПолноеИмя());
	// ...Гунько 2020-12-27
...
//
Процедура СообщитьОбОшибкахПроведенияПоРегиструСерийныеНомера(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Не хватает серийных номеров %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление%';uk='Не хватает серийных номеров %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление%'");
	
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристикаПартия%,"
"серийный номер %СерийныйНомер%';uk='Номенклатура: %НоменклатураХарактеристикаПартия%,"
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструТоварыВРемонте(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='Нет в наличии %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление%';uk='Нет в наличии %ТипСтруктурнойЕдиницы% %СтруктурнаяЕдиницаПредставление%'");
	
	ШаблонСообщения = НСтр("ru='Номенклатура: %НоменклатураХарактеристика%';uk='Номенклатура: %НоменклатураХарактеристика%'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструТоварыПереданныеВРемонте(ДокОбъект, ВыборкаЗаписей, Отказ) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка:';uk='Помилка:'");
	ШаблонЗаголовкаСообщения = ЗаголовокОшибки + Символы.ПС + НСтр("ru='В сервисном центре %СервисныйЦентрПредставление% нет в наличии ';uk='В сервисном центре %СервисныйЦентрПредставление% нет в наличии '");
...
Функция ПроверитьВыводТабличногоДокумента(ТабДокумент, ВыводимыеОбласти, РезультатПриОшибке = Истина)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСТр("ru='Невозможно получить информацию о текущем принтере (возможно, в системе не установлено ни одного принтера)';uk='Невозможно получить информацию о текущем принтере (возможно, в системе не установлено ни одного принтера)'"),
...
Функция ОтказСнятьИспользоватьОбменССайтом() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ТекстСообщения = НСтр("ru='В программе настроен и используется обмен с сайтами! Отключение настройки невозможно!';uk='В программе настроен и используется обмен с сайтами! Отключение настройки невозможно!'");
	КонецЕсли;
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьСерийныеНомера() Экспорт
	...
	РезультатЗапроса = Запрос.Выполнить();
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ТекстОшибки = НСтр("ru='В базе есть остатки по серийным номерам! Снятие флага запрещено!';uk='В базе есть остатки по серийным номерам! Снятие флага запрещено!'");
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ВестиУчетМаркировкиПродукцииВГИСМ") Тогда
		ТекстОшибки = НСтр("ru='Включен учет маркировки продукции в государственной информационной системе маркировки (ГИСМ). Снятие флага запрещено.';uk='Включен учет маркировки продукции в государственной информационной системе маркировки (ГИСМ). Снятие флага запрещено.'");
	КонецЕсли;
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьДисконтныеКарты() Экспорт
	...
	Пока ВыборкаДисконтныеКарты.Следующий() Цикл
		...
		Если ТаблицаСсылок.Количество() > 0 Тогда
			...
			ТекстОшибки = НСтр("ru='В базе используются дисконтные карты! Снятие опции запрещено!';uk='У базі використовуються дисконтні картки! Зняття опції заборонено!'");
			Прервать;
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьАвтоматическиеСкидкиНаценки() Экспорт
	...
	Пока ВыборкаАвтоматическиеСкидки.Следующий() Цикл
		...
		Если ТаблицаСсылок.Количество() > 0 Тогда
			...
			ТекстОшибки = НСтр("ru='В базе используются виды автоматических скидок, наценок! Снятие опции запрещено!';uk='У базі використовуються види автоматичних знижок, націнок! Зняття опції заборонено!'");
			Прервать;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В документах используются автоматические скидки и наценки! Снятие опции запрещено!';uk='У документах використовуються автоматичні знижки й націнки! Зняття опції заборонено!'");
...
Функция ОтказСнятьФункциональнаяОпцияРазноситьОплатуОтЭквайрераПоЭквайринговымОперациям() Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("КассовыйМетодУчетаДоходовИРасходов") Тогда
		ТекстОшибки = НСтр("ru='При установленной опции ""Кассовый метод учета доходов и расходов"" снятие опции запрещено!';uk='При встановленій опції ""Касовий метод обліку доходів і витрат"" зняття опції заборонене!'");
	КонецЕсли;
...
"Выполнить анализ можно в отчете ""Расчеты по эквайрингу (сводно)"" с детализацией до регистратора. Отчет вызывается из раздела ""Деньги"".';uk='Регістр накопичення ""Розрахунки по еквайрингові"" містить інформацію по еквайрингових операціях! Зміна прапорця заборонена!"
...
"Выполнить анализ можно в отчетах ""Займы сотрудникам"" и ""Кредиты (полученные)"". Отчет вызывается из раздела ""Деньги"".';uk='Регистр накопления ""Расчеты по кредитам и займам"" содержит информацию! Изменение флага запрещено!"
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьЗаказыВРозничнойТорговле() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + НСтр("ru='В розничных документах используются заказы покупателей! Снятие опции запрещено!';uk='В розничных документах используются заказы покупателей! Снятие опции запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьРемонты() Экспорт
	...
	Если ЕстьДвижения Тогда
		...
		ТекстОшибки = НСтр("ru='В базе есть остатки по товарам в ремонте! Снятие флага запрещено!';uk='В базе есть остатки по товарам в ремонте! Снятие флага запрещено!'");
...
Функция ОтказСнятьИспользоватьНесколькоОрганизаций() Экспорт
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		ТекстОшибки = НСтр("ru='В базе есть организации помимо основной. Для выключения опции необходимо удалить все кроме основной.';uk='В базе есть организации помимо основной. Для выключения опции необходимо удалить все кроме основной.'");
	КонецЕсли;
...
Функция ОтказСнятьУчетПоКомпании() Экспорт
	...
	Для Каждого Документ Из Метаданные.Документы Цикл
		...
		Если Счетчик = ПорцияДляПроверки Тогда
			Счетчик = 0;
			Если Не Запрос.Выполнить().Пустой() Тогда
				Возврат НСтр("ru='В базе есть проведенные документы по организации, отличной от компании. Для выключения опции необходимо изменить организацию в таких документах.';uk='В базе есть проведенные документы по организации, отличной от компании. Для выключения опции необходимо изменить организацию в таких документах.'");
			КонецЕсли;
			Запрос.Текст = "";
		КонецЕсли;
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьВидыЗаказовПокупателей() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='Для выключения опции необходимо удалить все виды заказов покупателей кроме основного.';uk='Для выключения опции необходимо удалить все виды заказов покупателей кроме основного.'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьВидыЗаказНарядов() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru='Для выключения опции необходимо удалить все виды заказ-нарядов кроме основного.';uk='Для выключения опции необходимо удалить все виды заказ-нарядов кроме основного.'");
...
Процедура УстановитьАдресСервисаЦентраМониторинга(НовоеЗначение) Экспорт
	...
	Исключение
		...
		ОписаниеОшибки = НСтр("ru='Адрес сервиса';uk='Адреса сервісу'") + " "
			+ НовоеЗначение + " "
			+ НСтр("ru='не является допустимым адресом веб-сервиса для отправки отчетов об использовании программы.';uk='не є припустимою адресою веб-сервісу для відправлення звітів про використання програми.'"); 
...
Функция ПодготовкаДанныхДляПробитияЧекаККМ(ТаблицаТовары, ТаблицаАкцизы, ДокументОбъект, ДанныеЕГАИСДостаточны) Экспорт
...
Процедура ПроверитьДанныеЕГАИС(ТаблицаТоваров, ДанныеЕГАИСДостаточны, ДокументОбъект) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
		НСтр("ru='Не хватает данных для передачи в ЕГАИС:';uk='Не хватает данных для передачи в ЕГАИС:'"),
		ДокументОбъект);
	
	ДополнительныйСимвол = "";
	Пока Выборка.Следующий() Цикл
		
		ТекстТекущейОшибки = НСтр("ru='Строка №%1:';uk='Строка №%1:'");
		...
		Если Выборка.ОшибкаШтрихкода Тогда
			ТекстТекущейОшибки = ТекстТекущейОшибки + Отступ + НСтр("ru='штрихкод';uk='штрихкод'");
		КонецЕсли;
		Если Выборка.ОшибкаОбъема Тогда
			ТекстТекущейОшибки = ТекстТекущейОшибки + Отступ + НСтр("ru='объем единицы продукции';uk='объем единицы продукции'");
		КонецЕсли;
		Если Выборка.ОшибкаМарки Тогда
			ТекстТекущейОшибки = ТекстТекущейОшибки + Отступ + НСтр("ru='акцизная марка';uk='акцизная марка'");
		КонецЕсли;
		Если Выборка.ОшибкаКрепости Тогда
			ТекстТекущейОшибки = ТекстТекущейОшибки + Отступ + НСтр("ru='крепость';uk='крепость'");
		КонецЕсли;
		Если Выборка.ОшибкаКодаВидаПродукции Тогда
			ТекстТекущейОшибки = ТекстТекущейОшибки + Отступ + НСтр("ru='код вида алкогольной продукции';uk='код вида алкогольной продукции'");
		КонецЕсли;
...
Процедура УстановитьОтображаниеПодменюПечати(ГруппаФормыПечать) Экспорт
	...
	ГруппаФормыПечать.Подсказка		= НСтр("ru='Печать с предварительным просмотром';uk='Друк з попереднім переглядом'");
...
Функция КомандаПечатаетсяВСерверномКонтексте(знач ОписаниеКомандыПечати) Экспорт
	...
	Если ОписаниеКомандыПечати.Представление = НСтр("ru='Настраиваемый комплект документов';uk='Комплект документів, що налаштовується'") Тогда
		Возврат Ложь;
	КонецЕсли;
...
// Гунько 2020-12-06
Процедура гвОтразитьСдельныеНарядыПоСерийныхНомерах(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	ТаблицаСдельныеНарядыПоСерийныхНомерах = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаСдельныеНарядыПоСерийныхНомерах;
	
	Если Отказ
	 ИЛИ ТаблицаСдельныеНарядыПоСерийныхНомерах.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДвиженияСдельныеНарядыПоСерийныхНомерах = Движения.гвСдельныеНарядыПоСерийныхНомерах;
	ДвиженияСдельныеНарядыПоСерийныхНомерах.Записывать = Истина;
	ДвиженияСдельныеНарядыПоСерийныхНомерах.Загрузить(ТаблицаСдельныеНарядыПоСерийныхНомерах);
	
КонецПроцедуры // ОтразитьСдельныеНарядыПоСерийныхНомерах()

Процедура гвОтразитьРезервированиеТоваровПоСерийныхНомерах(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	ТаблицаРезервированиеТоваровПоСерийныхНомерах = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРезервированиеТоваровПоСерийныхНомерах;
	
	Если Отказ
	 ИЛИ ТаблицаРезервированиеТоваровПоСерийныхНомерах.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДвиженияРезервированиеТоваровПоСерийныхНомерах = Движения.гвРезервированиеТоваровПоСерийныхНомерах;
	ДвиженияРезервированиеТоваровПоСерийныхНомерах.Записывать = Истина;
	ДвиженияРезервированиеТоваровПоСерийныхНомерах.Загрузить(ТаблицаРезервированиеТоваровПоСерийныхНомерах);
