...
Процедура ПроверитьКонтрагентовПослеВключенияПроверкиФоновоеЗадание() Экспорт
	...
	Попытка
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Проверка контрагентов';uk='Проверка контрагентов'");
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Проверка контрагентов.Фоновое задание по обработке непроверенных контрагентов после включения';uk='Проверка контрагентов.Фоновое задание по обработке непроверенных контрагентов после включения'", КодОсновногоЯзыка),
...
Процедура УстановитьВидимостьИЗаголовокПредупрежденияПроТестовыйРежим(Элемент) Экспорт
	...
	Иначе
		...
		Элемент.Заголовок = 
			НСтр("ru='В настоящее время веб-сервис ФНС работает в тестовом режиме.';uk='В настоящее время веб-сервис ФНС работает в тестовом режиме.'");
	КонецЕсли;
...
Процедура УстановитьНадписиВПанелиОтчетаПриСозданииНаСервере(Форма) Экспорт
	...
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru='Проверка контрагентов по данным ФНС выполнена успешно';uk='Проверка контрагентов по данным ФНС выполнена успешно'"), СсылкаНаИнструкцию);
	КонецЕсли;
	
	// Есть некорректные контрагенты.
	ЭлементФормы = Форма.Элементы.Найти("ТекстПриНекорректныхКонтрагентах");
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru='Обнаружены недействующие контрагенты по данным ФНС';uk='Обнаружены недействующие контрагенты по данным ФНС'"), СсылкаНаИнструкцию);
	КонецЕсли;
	
	// Выполняется проверка
	ЭлементФормы = Форма.Элементы.Найти("ТекстПриПроверкеВПроцессеВыполнения");
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru='Выполняется проверка контрагентов согласно данным ФНС';uk='Выполняется проверка контрагентов согласно данным ФНС'"), СсылкаНаИнструкцию);
	КонецЕсли;
	
	// Нет доступа к сервису
	ЭлементФормы = Форма.Элементы.Найти("ТекстНетДоступаКСервису");
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru='Не удалось произвести проверку контрагентов: сервис ФНС временно недоступен';uk='Не удалось произвести проверку контрагентов: сервис ФНС временно недоступен'"), СсылкаНаИнструкцию);
	КонецЕсли;
	...
	Если ДанныеКонтрагентов.Количество() > 0 Тогда
		...
		Попытка
		 	ФоновоеЗадание = ФоновыеЗадания.Выполнить("ПроверкаКонтрагентов.ПроверитьКонтрагентовВДокументе", 
				Параметры, , НСтр("ru='Проверка контрагентов в документе';uk='Проверка контрагентов в документе'"));
		...
		Исключение
			...
			КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ЗаписьЖурналаРегистрации(НСтр("ru='Проверка контрагентов.Фоновое задание в документе';uk='Проверка контрагентов.Фоновое задание в документе'", КодОсновногоЯзыка),
				УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура ПроверитьКонтрагентовВОтчете(Форма, СтруктураПараметров) Экспорт
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Проверка контрагентов в отчете';uk='Проверка контрагентов в отчете'");
...
Процедура УдалитьРезультатПроверкиКонтрагентов(Контрагенты = Неопределено) Экспорт 
	...
	Исключение
		...
		НСтр("ru='Проверка контрагентов.Очистка состояний контрагентов';uk='Проверка контрагентов.Очистка состояний контрагентов'", КодОсновногоЯзыка), 
...
Процедура УдалитьРезультатПроверкиКонтрагентовПоДокументам(Документы = Неопределено) Экспорт 
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ЗаписьЖурналаРегистрации(
		НСтр("ru='Проверка контрагентов.Очистка состояний документов';uk='Проверка контрагентов.Очистка состояний документов'", КодОсновногоЯзыка), 
		УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
"До завершения выполнения существование контрагента может быть определено не для всех контрагентов';uk='Проверяет существование контрагентов по базе ФНС."
...
Процедура ПроверитьКонтрагентаФоновоеЗадание(Параметры) Экспорт 
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(НСтр("ru='Проверка контрагентов.Ошибка при проверке из карточки контрагента';uk='Проверка контрагентов.Ошибка при проверке из карточки контрагента'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция ПроксиСервиса(Таймаут = 120)
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(НСтр("ru='Проверка контрагентов.Ошибка доступа к веб-сервису';uk='Проверка контрагентов.Ошибка доступа к веб-сервису'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция ОтветСервисаФНС(Прокси, WSЗапрос)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Проверка контрагентов.NdsRequest2';uk='Проверка контрагентов.NdsRequest2'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ДобавитьКонтрагентаВЗапросКСервису(WSЗапрос, Прокси, ДанныеКонтрагента)
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ЗаписьЖурналаРегистрации(НСтр("ru='Проверка контрагентов.Формирование данных для обращения к сервису ФНС';uk='Проверка контрагентов.Формирование данных для обращения к сервису ФНС'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
	КонецПопытки; 
	...
	Для ИндексТекущегоКонтрагента = МинимальныйНомерКонтрагента По МаксимальныйНомерКонтрагента Цикл 
		...
		Если ДанныеКонтрагента.Состояние <> Перечисления.СостоянияСуществованияКонтрагента.КонтрагентСодержитОшибкиВДанных Тогда
			...
			Исключение
				...
				КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
				ЗаписьЖурналаРегистрации(
				НСтр("ru='Проверка контрагентов.Обработка ответа от веб-сервиса';uk='Проверка контрагентов.Обработка ответа от веб-сервиса'", КодОсновногоЯзыка), 
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура СохранитьРезультатыПроверкиКонтрагентаВРегистр(ДанныеКонтрагента)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Проверка контрагентов.Запись результатов проверки контрагентов в регистр';uk='Проверка контрагентов.Запись результатов проверки контрагентов в регистр'", КодОсновногоЯзыка), 
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура СохранитьРезультатыПроверкиКонтрагентовВДокументеВРегистр(СохраняемыйРезультат)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
		НСтр("ru='Проверка контрагентов.Запись результатов проверки документов в регистр';uk='Проверка контрагентов.Запись результатов проверки документов в регистр'", КодОсновногоЯзыка), 
		УровеньЖурналаРегистрации.Ошибка,,,
		ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция НовоеПолеКартинки(Форма, ТаблицаФормы, Родитель, ПолеСКонтрагентом)
	...
	НовоеПоле.Заголовок					= НСтр("ru='Есть ошибка по данным ФНС';uk='Есть ошибка по данным ФНС'");
