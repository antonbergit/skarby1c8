...
"""%1""."
""
"Обратитесь к администратору.';uk='Не вдалося встановити параметр сеансу ТекущийПользователь з причини:"
"""%1""."
""
	...
	Если ПользовательПоИдентификаторуСуществует(
		...
		Если ПриНачалеРаботыСистемы И РолиАдминистратораДоступны() Тогда
			ИнтеграцияСтандартныхПодсистем.ПриСозданииАдминистратора(НайденныйПользователь,
				НСтр("ru='При авторизации у пользователя найдены роли администратора.';uk='При авторизації у користувача знайдені ролі адміністратора.'"));
		КонецЕсли;
		Возврат РезультатУстановкиПараметровСеанса(РегистрироватьВЖурнале);
	КонецЕсли;
...
"режим конфигурирования 1С:Предприятия для этого использовать не следует.';uk='Запуск від імені користувача із правом Адміністрування неможливий,"
"тому що він не зареєстрований у списку користувачів."
""
"Для ведення списку й налаштування прав користувачів призначений список Користувачі,"
...
""
"Для ведения списка и настройки прав пользователей предназначен список Пользователи,"
"режим конфигурирования 1С:Предприятия для этого использовать не следует.';uk='Не вдалося виконати автоматичну реєстрацію адміністратора в списку з причини:"
"""%1""."
""
"Для ведення списку і налаштування прав користувачів призначений список Користувачі,"
"режим конфігурації 1С:Підприємства для цього використати не слід.'"),
			РегистрироватьВЖурнале);
	КонецПопытки;
...
""
"Для ведения списка и настройки прав пользователей предназначен список Пользователи,"
"режим конфигурирования 1С:Предприятия для этого использовать не следует.';uk='Виконано запуск від імені користувача з роллю ""Повні права"","
"який не зареєстрований в списку користувачів."
...
""
"Обратитесь к администратору.';uk='Недостатньо прав для входу в програму."
""
"Зверніться до адміністратора.'"),
		, РегистрироватьВЖурнале);
...
Функция ВозможноИзменитьПароль(Пользователь, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("ПроверятьНаличиеПользователяИБ")
	   И ПользовательИБ = Неопределено Тогда
		...
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не найдена учетная запись пользователя ""%1"".';uk='Не знайдений обліковий запис користувача ""%1"".'"), Пользователь));
		Возврат Ложь;
	КонецЕсли;
...
"Зверніться до адміністратора.'"), Пользователь));
	...
	ЧислоИПредмет = Формат(КоличествоДней, "ЧГ=") + " "
		+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоДней,
			"Л = ru_RU", НСтр("ru='день,дня,дней,,,,,,0';uk='день,дня,днів,,,,,,0'"));
	...
	Если ДополнительныеПараметры.ЭтоТекущийПользовательИБ
	   ...
	   И (ВызовИзФормыСменаПароля Или СтарыйПароль <> Неопределено) Тогда
		...
		Если Не Параметры.СтарыйПарольСовпадает Тогда
			Возврат НСтр("ru='Старый пароль указан неверно.';uk='Старий пароль зазначений невірно.'");
		КонецЕсли;
	КонецЕсли;
...
"задать настройки входа пользователей в настройках программы.';uk='%1"
""
"Обмеження встановлено в конфігураторі в меню ""Адміністрування"" пункт ""Параметри інформаційної бази ...""."
"Слід очистити мінімальну довжину і вимогу складності пароля в конфігураторі і"
...
"%2';uk='Не вдалося перемінити пароль користувача ""%1"" через:"
"%2'"),
...
"- не совпадает с именем (для входа).';uk='Надійний пароль:"
"- має не менше 7 символів;"
"- містить будь-які 3 з 4-х типів символів: заголовні"
"  букви, рядкові букви, цифри, спеціальні символи;"
"- не співпадає з ім`ям (для входу).'");
...
"Продолжить?';uk='У список користувачів програми додається перший користувач, тому йому"
...
"не найдена в метаданных.';uk='При відновленні ролей зовнішнього користувача"
"""%1"""
"роль ""%2"""
...
"т.к. у группы стоит признак ""Все пользователи заданного типа"".';uk='Користувач ""%1"" не може бути включений у групу ""%2"","
"тому що в групи встановлена ознака ""Всі користувачі заданого типу"".'"),
					Предмет, НаименованиеГруппы) + Символы.ПС;
			Иначе
				СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Пользователь ""%1"" не может быть включен в группу ""%2"","
"т.к. в состав ее участников входят только %3.';uk='Користувач ""%1"" не може бути включений у групу ""%2"","
...
"или у группы стоит признак ""Все пользователи заданного типа"".';uk='Не всі користувачі можуть бути включені в групу ""%1"","
"тому що до складу її учасників входять тільки %2"
"або в групи встановлена ознака ""Всі користувачі заданого типу"".'"),
				НаименованиеГруппы,
...
Функция ЗаголовокСообщенияАвторизацияНеВыполненаСПереводомСтроки()
	...
	Возврат НСтр("ru='Авторизация не выполнена. Работа системы будет завершена.';uk='Авторизація не виконана. Робота системи буде завершена.'")
...
""
"Обратитесь к администратору.';uk='Користувач ""%1"" не знайдений в довідниках"
...
""
"Обратитесь к администратору.';uk='Користувач ""%1"" не знайдений в довіднику ""Користувачі""."
""
"Зверніться до адміністратора.'");
	КонецЕсли;
...
Функция КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(ИнформацияОбОшибке, ШаблонОшибки = "", РегистрироватьВЖурнале = Истина)
	...
	Если РегистрироватьВЖурнале Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Пользователи.Ошибка входа в программу';uk='Користувачі.Помилка входу в програму'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
"Обратитесь к администратору.';uk='Неможливо виконати вхід із-за наявності недоступних ролей."
"Зверніться до адміністратора.'");
...
"""%2"".';uk='Спроба видалити користувачеві %1 недоступну роль"
"""%2"".'"),
...
"%2.';uk='Спроба видалити користувачеві %1 недоступні ролі:"
"%2.'"),
			ПользовательИБ.ПолноеИмя, УдаляемыеНедоступныеРоли);
	Иначе
...
"%2.';uk='Спроба призначити користувачеві %1 недоступну роль :"
"%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
...
"%2.';uk='Спроба призначити користувачеві %1 недоступні ролі :"
"%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
	Иначе
	...
	Если СтрЧислоСтрок(УдаляемыеНедоступныеРоли) = 1 И ЗначениеЗаполнено(УдаляемыеНедоступныеРоли) Тогда
		ТекстСообщенияУдаления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Пользователю ""%1"" невозможно удалить недоступную роль.';uk='Користувачеві ""%1"" неможливо видалити недоступну роль.'"),
			ПользовательИБ.ПолноеИмя);
	ИначеЕсли СтрЧислоСтрок(УдаляемыеНедоступныеРоли) > 1 Тогда
		ТекстСообщенияУдаления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Пользователю ""%1"" невозможно удалить недоступные роли.';uk='Користувачеві ""%1"" неможливо видалити недоступні ролі.'"),
			ПользовательИБ.ПолноеИмя);
	Иначе
...
"%2.';uk='Користувачеві ""%1"" неможливо призначити недоступну роль"
"%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
...
"%2.';uk='Користувачеві ""%1"" неможливо призначити недоступні ролі:"
"%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
	Иначе
...
"%1"
""
"Обратитесь к администратору.';uk='Не вдалося записати відомості про поточного користувача через:"
"%1"
""
"Зверніться до адміністратора.'");
		Если ПриЗапуске Тогда
			...
			Если РегистрироватьВЖурнале Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Пользователи.Ошибка входа в программу';uk='Користувачі.Помилка входу в програму'",
					     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
		...
		Иначе
			Если РегистрироватьВЖурнале Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Пользователи.Ошибка обновления даты последней активности';uk='Користувачі.Помилка оновлення дати останньої активності'",
					     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					Метаданные.НайтиПоТипу(ТипЗнч(ТекущийПользователь)),
					ТекущийПользователь,
...
Функция ОшибкаДлиныИлиСложностиПароля(Пароль, МинимальнаяДлинаПароля, СложныйПароль)
	...
	Если СтрДлина(Пароль) < МинимальнаяДлинаПароля Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Количество символов нового пароля должно быть не менее %1.';uk='Кількість символів нового пароля повинне бути не менш %1.'"),
...
"%1';uk='Не вдалося перевірити чи відповідає пароль вимогам складності через:"
"%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	КонецПопытки;
...
"Відновлення реквізиту виконується автоматично.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
...
"Не можна позначати на видалення користувача, якому дозволений вхід у програму.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
...
"%2';uk='Не вдалося зняти користувачеві ""%1"" ознаку"
"""Вхід у програму дозволений"" у зв`язку із закінченням терміну дії через:"
"%2'");
			Иначе
				ШаблонКомментария =
					НСтр("ru='Не удалось снять пользователю ""%1"" признак"
...
"%2';uk='Не вдалося зняти користувачеві ""%1"" ознаку"
"""Вхід у програму дозволений"" у зв`язку з відсутністю роботи"
"в програмі більш встановленого терміну через:"
"%2'");
			КонецЕсли;
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Пользователи.Ошибка автоматического запрещения входа в программу';uk='Користувачі.Помилка автоматичної заборони входу в програму'",
				     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.НайтиПоТипу(ТипЗнч(Пользователь)),
				Пользователь,
