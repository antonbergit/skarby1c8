	...
	Если Прокси <> Неопределено Тогда
		...
		Исключение
			...
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Код налогового органа %1:';uk='Код податкового органа %1:'"), РеквизитыНалоговогоОргана.Код)
...
Функция РеквизитыОтделенияФССПоКоду(Знач КодОтделенияФСС) Экспорт
	...
	Если Прокси <> Неопределено Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Код отделения ФСС %1:';uk='Код відділення ФСС %1:'"), РеквизитыФСС.Код)
				+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		КонецПопытки;
	КонецЕсли;
...
Функция РеквизитыОтделенияПФРПоКоду(Знач КодОтделенияПФР) Экспорт
	...
	Если Прокси <> Неопределено Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Код отделения ПФР %1:';uk='Код відділення ПФР %1:'"), РеквизитыПФР.Код)
				+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		КонецПопытки;
	КонецЕсли;
...
Функция ГосударственныйОрган(Знач ВидГосударственногоОргана, Знач КодГосударственногоОргана) Экспорт
	...
	|	ЕСТЬNULL(Контрагенты.БанковскийСчетПоУмолчанию.Банк.Код, """") КАК МФО,
...
Процедура ОбновитьДанныеГосударственногоОргана(Реквизиты) Экспорт
	...
	Иначе
		...
		НаименованиеРодителя = НСтр("ru='Государственные органы';uk='Державні органи'");
		Запрос = Новый Запрос();
	...
	Иначе
		...
		Иначе
			...
			БанковскийСчет.ВариантУказанияКодПоЕДРПОУ = Перечисления.ВариантыУказанияКодПоЕДРПОУ.ВоВсехПлатежныхПоручениях;
	...
	Исключение
		...
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка сохранения государственного органа %1:';uk='Помилка збереження державного органа %1:'"), Реквизиты.Код)
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
			...
			Если ЗначениеЗаполнено(Реквизиты) Тогда
				...
				Если НЕ ЗначениеЗаполнено(Реквизиты.ОписаниеОшибки) Тогда
					...
					Если Реквизиты.ИНН <> ПлатежныеРеквизиты.ИНН Тогда
						СписокРасхождений = СписокРасхождений + ?(СписокРасхождений = "", "", ", ") + НСтр("ru='ИНН';uk='ІПН'");
					КонецЕсли;
					
					Если Реквизиты.КПП <> ПлатежныеРеквизиты.КПП Тогда
						СписокРасхождений = СписокРасхождений + ?(СписокРасхождений = "", "", ", ") + НСтр("ru='КПП';uk='КПП'");
					КонецЕсли;
					
					Если Реквизиты.ПлатежныеРеквизиты.ПолучательПлатежа <> ПлатежныеРеквизиты.ПолучательПлатежа Тогда
						СписокРасхождений = СписокРасхождений + ?(СписокРасхождений = "", "", ", ") + НСтр("ru='Получатель платежа';uk='Одержувач платежу'");
					КонецЕсли;
					
					Если Реквизиты.ПлатежныеРеквизиты.РасчетныйСчет <> ПлатежныеРеквизиты.РасчетныйСчет Тогда
						СписокРасхождений = СписокРасхождений + ?(СписокРасхождений = "", "", ", ") + НСтр("ru='Расчетный счет';uk='Розрахунковий рахунок'");
					КонецЕсли;
					
					Если Реквизиты.ПлатежныеРеквизиты.БИК <> ПлатежныеРеквизиты.БИК Тогда
						СписокРасхождений = СписокРасхождений + ?(СписокРасхождений = "", "", ", ") + НСтр("ru='БИК';uk='МФО'");
					КонецЕсли;
...
"указаны верно';uk='Платіжні реквізити "
"зазначені вірно'"));
					Иначе
...
"Найдены отличия в реквизитах: %1';uk='Можливо, платіжні реквізити зазначені не вірно."
"Знайдені відмінності в реквізитах: %1'"), СписокРасхождений));
					КонецЕсли;
...
"необходимо подключиться к интернет-поддержке пользователей.';uk='Для перевірки платіжних реквізитів"
...
Функция ПредставлениеПлатежныхРеквизитовГосударственногоОргана(ГосударственныйОрган) Экспорт
	...
	Иначе
		Возврат НСтр("ru='<Не заполнено>';uk='<Не заповнене>'");
	КонецЕсли;
...
Процедура ОбработатьОшибкуСервиса(ОписаниеОшибки, СтруктураРеквизитов)
	...
	ИначеЕсли Найти(ОписаниеОшибки, "SERVER-1") > 0 Тогда
		ТекстОшибки  = НСтр("ru='Не указан код государственного органа';uk='Не зазначений код державного органа'");
	...
	ИначеЕсли Найти(ОписаниеОшибки, "SERVER-3") > 0 Тогда
		ТекстОшибки  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Данные о государственном органе с кодом %1 не найдены';uk='Дані про державний орган з кодом %1 не знайдені'"),
	...
	ИмяСобытия = НСтр("ru='Сервис данных гос_органов.';uk='Сервіс даних гос_органів.'", КодОсновногоЯзыка) + " " + ТекстСобытия;
...
Функция НаименованиеПлательщикаПолучателяПоУмолчанию(ВладелецСчета, БанковскийСчет) Экспорт
	...
	Если ЗначениеЗаполнено(СвойстваБанковскогоСчета.БанкРасчетов)
		...
		Наименование = СокрЛП(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 р/с %2 в %3 %4';uk='%1 р/р %2 у %3 %4'"),
