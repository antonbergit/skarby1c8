...
Функция НачатьЗагрузку(ОбъектДрайвера, Параметры, ПараметрыПодключения, Количество, ВыходныеПараметры)
	...
	Если Количество = 0 Тогда
		ВыходныеПараметры.Очистить();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(НСтр("ru='Отсутствуют данные для загрузки из терминала сбора данных.';uk='Відсутні дані для завантаження з термінала збору даних.'"));
		Результат = Ложь;
...
"Проверьте настройку задачи терминала.';uk='Невірний формат даних поля ""Кількість""."
"Перевірте налаштування завдання термінала.'"));
				Результат = Ложь;
			КонецПопытки;
		КонецЕсли;
	КонецЕсли;
