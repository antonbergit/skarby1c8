...
Функция ОпределитьНаличиеСотрудниковПоСобытиюКалендаря(Организация, СобытиеКалендаря) 
...
Функция ОтразитьНаличиеСотрудниковПриСдачеВПФР(Организация, ДатаДокументаОбработкиСобытия,СобытиеКалендаря) Экспорт
	...
	СтруктураОтчетностьВПФР.ИмеютсяСотрудникиВПФР = ОпределитьНаличиеСотрудниковПоСобытиюКалендаря(Организация, СобытиеКалендаря);
	...
	Возврат СтруктураОтчетностьВПФР.ИмеютсяСотрудникиВПФР;
...
Процедура ДокументыПерсУчетаПередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Если СтруктураОбъекта.ОтчетныйПериод <> Неопределено И (Не ЗначениеЗаполнено(СтруктураОбъекта.ОтчетныйПериод)) И (Не Источник.ПометкаУдаления) Тогда
		ТекстСообщения = НСтр("ru='Не заполнено значение поля отчетный период';uk='Не заповнене значення поля звітний період'");
...
Процедура ЗаполнитьАдрес(НаборЗаписейАдрес, СтрокаАдрес, СписокСтран, ФорматНеструктурированныйАдрес, ФорматИностранныйАдрес, ЗаментяБуквуЕ = Ложь) Экспорт
	...
	Если ЗаПределамиРФ = Истина Тогда
		Если АдресСписком.Страна = "РОССИЯ" Тогда
			...
			ТекстПредупреждения = Нстр("ru='сохранен как неструктурированный адрес.';uk='збережена як неструктурована адреса.'");
		Иначе
			// Иностранный адрес
			...
			Иначе
				...
				НаборЗаписейАдрес.Удалить("Индекс");
				ТекстПредупреждения = Нстр("ru=' сохранен как иностранный адрес.';uk=' збережена як іноземна адреса.'");
			КонецЕсли;
		КонецЕсли;
	Иначе
		// Российский адрес
...
Функция ПредставлениеОтчетногоПериода(Период, Кратко = Ложь) Экспорт
	...
	ИначеЕсли Месяц(Период) <= 6  Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='1 полугодие %1 г.';uk='1 півріччя %1 р.'"), Формат(Период, "ДФ=ггг"));
	Иначе
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='2 полугодие %1 г.';uk='2 півріччя %1 р.'"), Формат(Период, "ДФ=ггг"));
	КонецЕсли;
