...
"';uk='УстановитьСостояниеНовостейДляПользователей, начало работы."
"МассивПользователей:"
"	%МассивПользователей%"
"СтруктураОтборов:"
"	%СтруктураОтборов%"
...
"'");
	...
	Если (ТипЗнч(МассивПользователей) <> ТипМассив)
		...
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%МассивПользователей%", НСтр("ru='По всем';uk='По всім'"));
	Иначе
	...
	Иначе
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СтруктураОтборов%", НСтр("ru='Передано значения типа, отличного от Структуры';uk='Передано значения типа, отличного от Структуры'"));
	КонецЕсли;
	...
	Иначе
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЗначенияСостояний%", НСтр("ru='Передано значения типа, отличного от Структуры';uk='Передано значения типа, отличного от Структуры'"));
	КонецЕсли;
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс і регламент'"), // ИмяСобытия
...
"';uk='УстановитьСостояниеНовостейДляПользователей, ошибка выполнения: при работе в модели сервиса запускать получение новостей вручную можно только при отключенных разделителях."
"Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
...
"Состояния новостей можно установить только для пользователей с ролью ЧтениеНовостей или ПолныеПрава';uk='Помилка виконання: немає користувачів, для яких би слід було встановити стани новин."
"Стани новин можна встановити тільки для користувачів з роллю ЧтениеНовостей або ПолныеПрава'");
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс і регламент'"), // ИмяСобытия
				НСтр("ru='Новости. Сервис и регламент. УстановитьСостояниеНовостейДляПользователей';uk='Новини. Сервіс і регламент. УстановитьСостояниеНовостейДляПользователей'") , // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
...
"';uk='УстановитьСостояниеНовостейДляПользователей, завершение."
"Обработано пользователей: %1"
"Обработано новостей: %2"
"'"),
...
"Возможные имена ключей совпадают с наименованиями ресурсов регистра сведений СостоянияНовостей';uk='УстановитьСостояниеНовостейДляПользователей, помилка виконання: у якості значень заповнення передане значення типу, відмінного від Структура, або не заповнені налаштування заповнення (не зазначене жодного ключа структури)."
"Можливі імена ключів збігаються з найменуваннями ресурсів регістру відомостей СостоянияНовостей'");
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс і регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. УстановитьСостояниеНовостейДляПользователей';uk='Новини. Сервіс і регламент. УстановитьСостояниеНовостейДляПользователей'") , // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
...
"';uk='Не рекомендуется давать возможность чтения новостей внешнему пользователю (%ТекущийВнешнийПользователь%)"
"Подключены следующие роли:"
"	- ЧтениеНовостей         - %ЧтениеНовостей%"
"	- РедактированиеНовостей - %РедактированиеНовостей%"
"	- АдминистраторСистемы   - %АдминистраторСистемы%"
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		"news.webits.1c.ru",
		443,
		НСтр("ru='Веб-сервисы получения новостей';uk='Веб-сервіси одержання новин'"));
	...
	Пока (СоединениеУстановлено <> Истина) И (ПопыткаСоединения < НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокСоединения) Цикл
		...
		Если ГдеДвоеточие > 0 Тогда
			...
			Исключение
				...
				ТекстСообщения = СтрШаблон(
					НСтр("ru='Попытка %1 / %2."
"Ошибка определения порта из имени сервера %3 (%4).';uk='Попытка %1 / %2."
...
"Порт        = %5'"),
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
				НСтр("ru='Новости. Загрузка новостей. Соединение с сервером новостей';uk='Новини. Завантаження новин. З`єднання із сервером новин'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
#Область ЗаполнениеШаблонов_Новости_КатегорииНовостейИнтервалыВерсий
			...
			Иначе
				РезультатТекущейНовости = СтрЗаменить(
					РезультатТекущейНовости,
					"%РегистрСведений." + ИмяРегистраСведений + "%",
					Символы.Таб + НСтр("ru='Набор записей пустой.';uk='Набор записей пустой.'"));
			КонецЕсли;
...
#Область ЗаполнениеШаблонов_Новости_РассчитанныеОтборыПоНовостям_Пользовательские
			...
			Иначе
				РезультатТекущейНовости = СтрЗаменить(
					РезультатТекущейНовости,
					"%РегистрСведений." + ИмяРегистраСведений + "%",
					НСтр("ru='В модели сервиса для администратора системы нельзя получить пользовательские настройки или отборы.';uk='В модели сервиса для администратора системы нельзя получить пользовательские настройки или отборы.'"));
			КонецЕсли;
...
#Область ЗаполнениеШаблонов_Новости_ПоставляемыеДанные
			...
			Иначе
				РезультатТекущейНовости = СтрЗаменить(РезультатТекущейНовости, "%ПоставляемыеДанные%", Символы.Таб + НСтр("ru='Не найдено в поставляемых данных.';uk='Не найдено в поставляемых данных.'"));
			КонецЕсли;
...
#КонецОбласти
	...
	Иначе
		Результат = СтрЗаменить(Результат, "%ДанныеОНовостях%", НСтр("ru='Не найдено ни одной новости с таким UIN';uk='Не найдено ни одной новости с таким UIN'"));
	КонецЕсли;
...
#Область ЗаполнениеШаблонов_ЛентыНовостей
	...
	Иначе
		Результат = СтрЗаменить(Результат, "%ДанныеОЛентахНовостей%", НСтр("ru='Не найдено ни одной ленты новостей для новости с таким UIN';uk='Не найдено ни одной ленты новостей для новости с таким UIN'"));
	КонецЕсли;
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_Константы
	...
	Иначе
		ПредставлениеНастройкиНовостей = СтрШаблон(НСтр("ru='Настройки новостей необрабатываемого типа %1';uk='Настройки новостей необрабатываемого типа %1'"), ТипЗнч(НастройкиНовостей));
	КонецЕсли;
...
"РазрешенаРаботаСНовостямиЧерезИнтернет: %3';uk='Константа.НастройкиНовостей:"
"%1"
"РазрешенаРаботаСНовостями: %2"
...
"	Разделение данных:"
"		%ТекущийПользователь().РазделениеДанных%"
"';uk='Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
"Доступно использование разделенных данных: %ДоступноИспользованиеРазделенныхДанных()%"
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_ПараметрыСеанса
	...
	Иначе
		ПредставлениеПараметрыОкруженияБИП_Новости = СтрШаблон(НСтр("ru='Параметры окружения подсистемы новостей необрабатываемого типа %1';uk='Параметры окружения подсистемы новостей необрабатываемого типа %1'"), ТипЗнч(ПараметрыОкруженияБИП_Новости));
	КонецЕсли;

	РезультатДанныеОПрограммномОкружении_ПараметрыСеанса = СтрШаблон(
		НСтр("ru='Параметры окружения подсистемы новостей:"
"%1';uk='Параметры окружения подсистемы новостей:"
"%1'"),
...
"Данные компьютера: %7';uk='Имя конфигурации: %1"
"Версия конфигурации: %2"
"Версия подсистемы БИП: %3"
"Версия платформы: %4"
"Операционная система: %5"
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_РасписаниеРегламентногоЗадания
	...
	Если РегламентноеЗадание = Неопределено Тогда
		ПредставлениеРасписания = НСтр("ru='Регламентное задание не найдено';uk='Регламентное задание не найдено'");
		РегламентноеЗадание_Использование = Ложь;
		ПредставлениеРегламентноеЗадание_ПоследнееЗадание = НСтр("ru='Никогда не выполнялось';uk='Никогда не выполнялось'");
	Иначе
		...
		Если ТипЗнч(РегламентноеЗадание_ПоследнееЗадание) = ТипФоновоеЗадание Тогда
			...
			Иначе
				РегламентноеЗадание_СтруктураПоследнегоЗадания.Вставить("ПодробноеПредставлениеОшибки", НСтр("ru='Выполнено без ошибок';uk='Выполнено без ошибок'"));
			КонецЕсли;
		...
		Иначе
			ПредставлениеРегламентноеЗадание_ПоследнееЗадание = НСтр("ru='Не выполнялось';uk='Не выполнялось'");
		КонецЕсли;
	КонецЕсли;
...
"Использование: %2"
"Последнее задание:"
"%3';uk='Расписание регламентного задания:"
"%1"
"Использование: %2"
"Последнее задание:"
"%3'"),
...
"';uk='Начало выполнения регламентного задания"
"Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Усі оновлення новин'"), // ИмяСобытия
		НСтр("ru='Новости. Все обновления новостей. Начало';uk='Новини. Всі відновлення новин. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
	...
	Если МонопольныйРежим() Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Усі оновлення новин'"), // ИмяСобытия
			НСтр("ru='Новости. Все обновления новостей. Конец';uk='Новини. Всі відновлення новин. Кінець'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
...
"Время начала (мс): %ТНачало%"
"';uk='Получение и обработка новостей для лент новостей: %ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеЗначения(МассивЛентНовостей)%"
"Время начала (мс): %ТНачало%"
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Усі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Получение и обработка новостей по лентам новостей. Начало';uk='Новини. Всі відновлення новин. Одержання й обробка новин по стрічках новин. Початок'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"';uk='Завершення одержання й обробки новин для стрічок новин"
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Усі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Получение и обработка новостей по лентам новостей. Конец';uk='Новини. Всі відновлення новин. Одержання й обробка новин по стрічках новин. Кінець'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			...
			Иначе
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Усі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Получение и обработка новостей по лентам новостей. Отсутствие лент новостей для обработки';uk='Новини. Всі відновлення новин. Одержання й обробка новин по стрічках новин. Відсутність стрічок новин для обробки'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					НСтр("ru='Нет лент новостей для регламентного обновления';uk='Немає стрічок новин для регламентного відновлення'"), // Комментарий
					ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
			КонецЕсли;
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ru='1. Получение и обработка новостей из лент новостей (с обязательным заполнением измерения ЛентаНовостей)';uk='1. Одержання й обробка новин зі стрічок новин (з обов`язковим заповненням виміру ЛентаНовостей)'"),
					ТШаг,
...
"'");
					...
					ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Усі оновлення новин'"), // ИмяСобытия
						НСтр("ru='Новости. Все обновления новостей. Обновление классификаторов. Начало';uk='Новини. Всі відновлення новин. Відновлення класифікаторів. Початок'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
						Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
						, // Данные
						ТекстСообщения, // Комментарий
...
"'");
					...
					ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Усі оновлення новин'"), // ИмяСобытия
						НСтр("ru='Новости. Все обновления новостей. Обновление классификаторов. Конец';uk='Новини. Всі відновлення новин. Відновлення класифікаторів. Кінець'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
						Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
						Т1Длительность, // Данные
						ТекстСообщения, // Комментарий
			...
			Иначе
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Усі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Обновление классификаторов. Отключена работа через интернет';uk='Новини. Всі відновлення новин. Відновлення класифікаторів. Відключена робота через інтернет'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					НСтр("ru='Обновление классификаторов через интернет отключено';uk='Оновлення класифікаторів через інтернет відключене'"), // Комментарий
					ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
			КонецЕсли;
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
...
"Время начала (мс): %ТНачало%"
"';uk='Автовидалення новин, початок"
"Час початку (мс): %ТНачало%"
"'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т1Начало);
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Усі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Автоудаление новостей. Начало';uk='Новини. Всі відновлення новин. Автовидалення новин. Початок'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"';uk='Автовидалення новин, завершення."
"Результат виконання:"
"%РезультатВыполнения%"
"Час початку (мс): %ТНачало%"
"Час закінчення (мс): %ТКонец%"
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Усі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Автоудаление новостей. Конец';uk='Новини. Всі відновлення новин. Автовидалення новин. Кінець'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					Т1Длительность, // Данные
					ТекстСообщения, // Комментарий
			...
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ru='3. Автоудаление новостей';uk='3. Автовидалення новин'"),
					ТШаг,
					ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Усі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Обновление периодических свойств. Начало';uk='Новини. Всі відновлення новин. Відновлення періодичних властивостей. Початок'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Усі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Обновление периодических свойств. Конец';uk='Новини. Всі відновлення новин. Відновлення періодичних властивостей. Кінець'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					Т1Длительность, // Данные
					ТекстСообщения, // Комментарий
			...
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ru='4. Обновление периодических свойств (важность, актуальность)';uk='4. Відновлення періодичних властивостей (важливість, актуальність)'"),
					ТШаг,
					ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Усі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Обновление привязок к метаданным. Начало';uk='Новини. Всі відновлення новин. Відновлення прив`язок до метаданих. Початок'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Усі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Обновление привязок к метаданным. Конец';uk='Новини. Всі відновлення новин. Відновлення прив`язок до метаданих. Кінець'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
			...
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ru='5. Обновление привязок к метаданным (+ важность)';uk='5. Відновлення прив`язок до метаданих (+ важливість)'"),
					ТШаг,
					ТекущаяУниверсальнаяДатаВМиллисекундах()));
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Усі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Проверка версии платформы. Начало';uk='Новини. Всі відновлення новин. Перевірка версії платформи. Початок'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"'");
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Усі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Проверка версии платформы. Конец';uk='Новини. Всі відновлення новин. Перевірка версії платформи. Кінець'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					Т1Длительность, // Данные
					ТекстСообщения, // Комментарий
...
"По шагам:"
"%РезультатПоШагам%"
"';uk='Завершення виконання регламентного завдання"
"Час початку (мс): %ТНачало%"
"Час закінчення (мс): %ТКонец%"
"Тривалість (мс): %ТДлительность%"
""
...
"	Разделение данных:"
"		%ТекущийПользователь().РазделениеДанных%"
"';uk='Выполнение регламентного задания"
"Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
...
"КатегорииНовостейИнтервалыВерсийНеобработанные: %ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество()%';uk='Зібрані дані для видалення:"
"Новин (для видалення): %ДанныеДляУдаления.НовостиДляУдаления.Количество()%"
"Новин (для установки позначки видалення): %ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()%"
"КатегорииНовостейПростыеНеобработанные: %ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество()%"
"КатегорииНовостейИнтервалыВерсийНеобработанные: %ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество()%'");
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс і регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. УдалитьНовости_Ежедневно. После сбора данных для удаления';uk='Новини. Сервіс і регламент. УдалитьНовости_Ежедневно. Після збору даних для видалення'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"'");
		...
		Если НЕ Результат.Пустой() Тогда
			...
			РезультатВыполнения = НСтр("ru='Найдено %ВсегоСтрок% новостей для обновления периодических свойств';uk='Знайдене %ВсегоСтрок% новин для відновлення періодичних властивостей'");
			...
			КонецЦикла;
		Иначе
			РезультатВыполнения = НСтр("ru='Не найдено данных для обновления';uk='Не знайдене даних для оновлення'");
		КонецЕсли;
...
"'");
		...
		Если НЕ Результат.Пустой() Тогда
			Выборка = Результат.Выбрать(ОбходРезультатаЗапроса.Прямой);
			ВсегоСтрок = Выборка.Количество();
			РезультатВыполнения = НСтр("ru='Найдено %ВсегоСтрок% новостей для обновления привязок к метаданным';uk='Знайдене %ВсегоСтрок% новин для відновлення прив`язок до метаданих'");
			РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ВсегоСтрок%", ВсегоСтрок);
			Пока Выборка.Следующий() Цикл
...
"		%ТекущийПользователь().РазделениеДанных%"
"Пользователь: %ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%"
"';uk='ПолучитьИОбработатьНовостиПоЛентамНовостей, при работе в модели сервиса запускать получение новостей вручную можно только при отключенных разделителях."
"Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
...
"Время начала (мс): %ТНачало%"
"';uk='Початок завантаження й обробки новин"
"Час початку (мс): %ТНачало%"
"'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
...
"Переданий параметр із кількістю значень: %МассивЛентНовостей.Количество()%"
"Результат виконання запиту: Порожній"
"Можливо, список стрічок новин порожній, або у всіх стрічок новин встановлена ознака ЛокальнаяЛентаНовостей (що означає, що по ній не треба завантажувати новини), або у всіх стрічок новин встановлена позначка видалення."
"'");
...
"недостаточно прав для выполнения операции загрузки новостей.';uk='ПолучитьИОбработатьНовостиПоЛентамНовостей, у користувача"
"%ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%"
...
"По шагам:"
"%РезультатПоШагам%"
"';uk='Закінчення завантаження й обробки новин"
"Час початку (мс): %ТНачало%"
"Час закінчення (мс): %ТКонец%"
"Тривалість (мс): %ТДлительность%"
""
...
"		%ТекущийПользователь().РазделениеДанных%"
"Пользователь: %ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%"
"';uk='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, при работе в модели сервиса запускать обновление стандартных списков можно только при отключенных разделителях."
"Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
...
"	Разделение данных:"
"		%ТекущийПользователь().РазделениеДанных%"
"';uk='Выполнение регламентного задания (начало)"
"Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
...
"Результат выполнения:"
"	%Результат%"
"';uk='Выполнение регламентного задания (конец)"
"Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
...
"Их можно обновлять из метаданных (при обновлении конфигурации), либо вручную.';uk='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, включен режим [Работа только с локальными новостями],"
"поэтому никакие классификаторы не обновляются с серверов 1С."
...
"поэтому все механизмы новостного центра отключены.';uk='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, отключен режим [Разрешена работа с новостями],"
...
"%ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%"
"недостаточно прав для выполнения операции загрузки стандартных списков.';uk='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, у користувача"
"%ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%"
...
"		%ТекущийПользователь().РазделениеДанных%"
"Пользователь: %ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%"
"';uk='ПересчитатьНеобработанныеКатегорииНовостей, при работе в модели сервиса запускать обновление стандартных списков можно только при отключенных разделителях."
"Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
...
" - УИННовости: %ТекущаяСтрока.УИННовости%';uk='Видалений запис із регістру відомостей КатегорииНовостейПростыеНеобработанные із ключами:"
" - КатегорияНовостейСтрока: %ТекущаяСтрока.КатегорияНовостейСтрока%"
" - ЗначениеКатегорииНовостейСтрока: %ТекущаяСтрока.ЗначениеКатегорииНовостейСтрока%"
...
" - ЛентаНовостей: %ТекущаяСтрока.ЛентаНовостей%"
" - УИННовости: %ТекущаяСтрока.УИННовости%';uk='Видалений запис із регістру відомостей КатегорииНовостейИнтервалыВерсийНеобработанные із ключами:"
" - КатегорияНовостейСтрока: %ТекущаяСтрока.КатегорияНовостейСтрока%"
" - Продукт: %ТекущаяСтрока.Продукт%"
" - ВерсияОТ: %ТекущаяСтрока.ВерсияОТ%"
" - ВерсияДО: %ТекущаяСтрока.ВерсияДО%"
...
" - УИННовости: %ТекущаяСтрока.УИННовости%'");
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс і регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Выполнение завершено';uk='Новини. Сервіс і регламент. ПересчитатьНеобработанныеКатегорииНовостей. Виконання завершене'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегистрыСведений.КатегорииНовостейПростые, // ОбъектМетаданных
			, // Данные
			НСтр("ru='ПересчитатьНеобработанныеКатегорииНовостей, обработка окончена.';uk='ПересчитатьНеобработанныеКатегорииНовостей, обробка кінчена.'"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации

	Иначе
		ТекстСообщения = НСтр("ru='ПересчитатьНеобработанныеКатегорииНовостей, у пользователя"
"%ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%"
"недостаточно прав для выполнения операции обработка категорий новостей.';uk='ПересчитатьНеобработанныеКатегорииНовостей, у користувача"
"%ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%"
...
"		%ТекущийПользователь().РазделениеДанных%"
"Пользователь: %ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%"
"';uk='ПересчитатьОтборыПоНовостям_РедкоМеняющиеся, при работе в модели сервиса запускать обновление стандартных списков можно только при отключенных разделителях."
"Строка подключения: %СтрокаСоединенияИнформационнойБазы()%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация()%"
"Разделение включено: %РазделениеВключено()%"
...
"';uk='ПересчитатьОтборыПоНовостям_РедкоМеняющиеся, не заполнены значения плана видов характеристик КатегорииНовостей:"
"КатегорияВерсияПродукта: %КатегорияВерсияПродукта%"
"КатегорияВерсияПлатформы: %КатегорияВерсияПлатформы%"
"Возможно, они еще не обновились с сервера при обновлении классификаторов, а также не были заполнены в макете СтандартныеЗначения плана видов характеристик КатегорииНовостей"
"'");
...
Процедура ПроверитьИзменениеВерсииПлатформы(РезультатВыполнения = "") Экспорт
	...
	Если МонопольныйРежим() Тогда
		...
		//  то будут ошибки при всех попытках записи данных в базу.
		// Поэтому в монопольном режиме не следует выполнять никаких обновлений в подсистеме новостей.
		РезультатВыполнения = НСтр("ru='В монопольном режиме проверка версии платформы не осуществляется';uk='У монопольному режимі перевірка версії платформи не здійснюється'");
		Возврат;
	КонецЕсли;
...
""
"Версия конфигурации в этой обработке не проверяется, а пересчитывается принудительно в модуле обновления конфигурации, а также после каждого получения новостей.';uk='Збережена версія платформи: %СохраненнаяВерсияПлатформы%"
"Поточна версія платформи: %ТекущаяВерсияПлатформы%"
"%ТребуетсяОбновлениеРедкоменяющихсяОтборов%"
""
"Версія конфігурації в цій обробці не перевіряється, а перераховується примусово в модулі оновлення конфігурації, а також після кожного одержання новин.'");
	...
	УстановитьПривилегированныйРежим(Истина);
			...
			Если МенеджерЗаданийАктивен = Истина Тогда
				Если МассивВыполняемыхФоновыхЗаданий.Количество() = 0 Тогда
					...
					МассивФоновыхЗаданий.Добавить(ФоновыеЗадания.Выполнить(
						...
						"", // Ключ
						СтрШаблон(
							НСтр("ru='ОбработкаНовостей.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся в связи с изменением версии платформы с %1 на %2';uk='ОбработкаНовостей.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся в связи с изменением версии платформы с %1 на %2'"),
							лкПоследняяВерсия,
							лкТекущаяВерсия)));
					// Последняя версия платформы сохранится в "ОбработкаНовостей.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся".
				КонецЕсли;
				РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ТребуетсяОбновлениеРедкоменяющихсяОтборов%", НСтр("ru='Редко меняющиеся отборы будут пересчитаны (запущено фоновое задание)';uk='Рідко мінливі відбори будуть перелічені (запущене фонове завдання)'"));
			Иначе
				РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ТребуетсяОбновлениеРедкоменяющихсяОтборов%", НСтр("ru='Редко меняющиеся отборы будут пересчитаны (явный запуск ПересчитатьОтборыПоНовостям_РедкоМеняющиеся())';uk='Рідко мінливі відбори будуть перелічені (явний запуск ПересчитатьОтборыПоНовостям_РедкоМеняющиеся())'"));
		...
		Иначе
			РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%СохраненнаяВерсияПлатформы%", лкПоследняяВерсия);
			РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ТекущаяВерсияПлатформы%", лкТекущаяВерсия);
			РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ТребуетсяОбновлениеРедкоменяющихсяОтборов%", НСтр("ru='Редко меняющиеся отборы останутся как есть';uk='Рідко мінливі відбори залишаться як є'"));
		КонецЕсли;
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Выборка.ЛентаНовостейЧастотаОбновления = 0 Тогда
					лкЧастотаОбновления = НСтр("ru='вручную (0)';uk='вручную (0)'");
				ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 1 Тогда
					лкЧастотаОбновления = НСтр("ru='один раз в день (1)';uk='один раз в день (1)'");
				ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 2 Тогда
					лкЧастотаОбновления = НСтр("ru='каждый час (2)';uk='каждый час (2)'");
				ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 3 Тогда
					лкЧастотаОбновления = НСтр("ru='каждые 15 минут (3)';uk='каждые 15 минут (3)'");
				ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 4 Тогда
					лкЧастотаОбновления = НСтр("ru='каждую минуту (4)';uk='каждую минуту (4)'");
				Иначе
					лкЧастотаОбновления = СтрШаблон(
						НСтр("ru='неопределено (%1)';uk='неопределено (%1)'"),
						Строка(Выборка.ЛентаНовостейЧастотаОбновления));
				КонецЕсли;

				РезультатВыполнения_ЛентаНовостей = НСтр("ru='Лента новостей %ЛентаНовостей%, частота обновления ленты новостей: %ЧастотаОбновления%, дата последней успешной загрузки: %ДатаЗагрузкиПоследнихНовостей%, неудач: %КоличествоНеудачПриЗагрузкеНовостей%, последняя попытка загрузки: %ДатаПоследнейПопыткиЗагрузкиНовостей%. %Действие%';uk='Стрічка новин %ЛентаНовостей%, частота відновлення стрічки новин: %ЧастотаОбновления%, дата останнього успішного завантаження: %ДатаЗагрузкиПоследнихНовостей%, невдач: %КоличествоНеудачПриЗагрузкеНовостей%, остання спроба завантаження: %ДатаПоследнейПопыткиЗагрузкиНовостей%. % Действие%'");
				...
				Если Выборка.ЛентаНовостейАктивна = Истина Тогда
					...
					Иначе
						...
						НовостьОбъект.УстановитьПометкуУдаления(Истина);
						РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ru='Лента новостей активна, новость о неактивности будет помечена на удаление';uk='Стрічка новин активна, новина про неактивність буде позначена на видалення'"));
					КонецЕсли;
				Иначе
					...
					Иначе
						НовостьОбъект = СтруктураНовости.Ссылка.ПолучитьОбъект();
						РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ru='Лента новостей НЕактивна, новость о неактивности будет обновлена';uk='Стрічка новин НЕактивна, новина про неактивність буде оновлена'"));
					КонецЕсли;
					...
					НовостьОбъект.Подзаголовок                       = НСтр("ru='Лента новостей неактивна. Рекомендуется исключить ее из автообновления';uk='Стрічка новин неактивна. Рекомендується виключити її з оновлення'");
...
"Подробно:"
"%ИнформацияОбОшибке%"
"';uk='Відбулася помилка при створенні / оновленні новини про неактивність стрічки новин %ЛентаНовостей%"
"Докладно:"
"%ИнформацияОбОшибке%"
"'");
						...
						КонецПопытки;
					Иначе
						РезультатВыполнения_ЛентаНовостей = РезультатВыполнения_ЛентаНовостей + Символы.ПС + НСтр("ru='Отменено создание / изменение новости про отсутствие активности для ленты новостей';uk='Отменено создание / изменение новости про отсутствие активности для ленты новостей'");
					КонецЕсли;
				КонецЕсли;
...
"Стрічка новин вважається НЕактуальною, якщо по ній було більш 10 невдалих спроб одержання новин і остання вдала спроба була більш доби назад."
""
"Результат виконання:"
"%РезультатВыполнения%"
""
...
"в параметре Использовать передано значение неправильного типа: %1';uk='ОбработкаНовостей.ИзменитьИспользованиеРегламентныхЗаданий:"
"в параметре Использовать передано значение неправильного типа: %1'"),
				ТипЗнч(Использовать));
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс і регламент'"), // ИмяСобытия
				НСтр("ru='Новости. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий';uk='Новости. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
		...
		КонецЕсли;
	Иначе
		ТекстСообщения = НСтр("ru='Не найдено предопределенное регламентное задание ВсеОбновленияНовостей.';uk='Не найдено предопределенное регламентное задание ВсеОбновленияНовостей.'");
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс і регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий';uk='Новости. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
	...
	УстановитьПривилегированныйРежим(Истина);
		...
		Если (ДанныеДляУдаления.Свойство("НовостиДляУстановкиПометкиУдаления"))
			...
			Для Каждого ТекущаяНовость Из ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления Цикл
				...
				КонецЕсли;
			КонецЦикла;
			ТекстСообщения = НСтр("ru='Все устаревшие новости успешно помечены на удаление (%ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()% новостей).';uk='Всі застарілі новини успішно позначені на видалення (%ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()% новин).'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()%", ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество());
		Иначе
			ТекстСообщения = НСтр("ru='Нет устаревших новостей, которые надо пометить на удаление.';uk='Немає застарілих новин, які треба позначити на видалення.'");
		КонецЕсли;
		...
		Если (ДанныеДляУдаления.Свойство("КатегорииНовостейПростыеНеобработанные"))
			...
			Для Каждого ТекущаяЗапись Из ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные Цикл
				...
				Запись.Удалить();
			КонецЦикла;
			ТекстСообщения = НСтр("ru='Было удалено %ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество()% необработанных простых категорий.';uk='Було видалено %ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество()% неопрацьованих простих категорій.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество()%", ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество());
		Иначе
			ТекстСообщения = НСтр("ru='Простых необработанных категорий не было найдено.';uk='Простих неопрацьованих категорій не було знайдено.'");
		КонецЕсли;
		...
		Если (ДанныеДляУдаления.Свойство("КатегорииНовостейИнтервалыВерсийНеобработанные"))
			...
			Для Каждого ТекущаяЗапись Из ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные Цикл
				...
				Запись.Удалить();
			КонецЦикла;
			ТекстСообщения = НСтр("ru='Было удалено %ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество()% необработанных категорий интервалов версий.';uk='Було видалено %ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество()% неопрацьованих категорій інтервалів версій.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество()%", ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество());
		Иначе
			ТекстСообщения = НСтр("ru='Необработанных категорий интервалов версий не было найдено.';uk='Неопрацьованих категорій інтервалів версій не було знайдено.'");
		КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий         = НСтр("ru='Запускается при первом запуске, в общих данных. Установка констант, загрузка стандартных значений, сброс регистров сведений (общих данных).';uk='Запускается при первом запуске, в общих данных. Установка констант, загрузка стандартных значений, сброс регистров сведений (общих данных).'");
	...
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.Комментарий         = НСтр("ru='Запускается при первом запуске, для каждой области данных. Установка настроек пользователей (разделенных данных).';uk='Запускается при первом запуске, для каждой области данных. Установка настроек пользователей (разделенных данных).'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.Комментарий         = НСтр("ru='Запускается при переходе на новую версию, в общих данных. Установка констант, загрузка стандартных значений, сброс регистров сведений (общих данных).';uk='Запускается при переходе на новую версию, в общих данных. Установка констант, загрузка стандартных значений, сброс регистров сведений (общих данных).'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ru='Запускается при переходе на любую новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).';uk='Запускается при переходе на любую новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ru='Запускается при переходе на новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).';uk='Запускается при переходе на новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).'");
	Обработчик.РежимВыполнения     = "Оперативно";
...
"'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
		НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПервыйЗапуск. Начало';uk='Новини. Відновлення ІБ. ОбщиеДанные. ПервыйЗапуск. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
...
"%Результат%';uk='Виявлені помилки впровадження підсистеми ИнтернетПоддержкаПользователей.Новини."
"Виявлені помилки впровадження:"
"%Результат%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Результат%", Результат);
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
					НСтр("ru='Новости. Обновление ИБ. Проверка внедрения';uk='Новини. Відновлення ІБ. Перевірка впровадження'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПервыйЗапуск. Начало';uk='Новини. Відновлення ІБ. ОбщиеДанные. Переобумовлений. ПервыйЗапуск. Початок'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПервыйЗапуск. Конец';uk='Новини. Відновлення ІБ. ОбщиеДанные. Переобумовлений. ПервыйЗапуск. Кінець'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				Т1Длительность, // Данные
				ТекстСообщения, // Комментарий
	...
	Иначе
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПервыйЗапуск. Конец';uk='Новини. Відновлення ІБ. ОбщиеДанные. ПервыйЗапуск. Кінець'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
		НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПервыйЗапуск. Начало';uk='Новини. Відновлення ІБ. ОбластьДанных. ПервыйЗапуск. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
	...
	Если ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя() Тогда
		...
		ТекстСообщения = НСтр("ru='Работа с новостями не поддерживается для внешнего пользователя. Обработчик обновления пропущен.';uk='Робота з новинами не підтримується для зовнішнього користувача. Оброблювач оновлення пропущений.'");
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПервыйЗапуск. Конец';uk='Новини. Відновлення ІБ. ОбластьДанных. ПервыйЗапуск. Кінець'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПервыйЗапуск. Начало';uk='Новини. Відновлення ІБ. ОбластьДанных. Переобумовлений. ПервыйЗапуск. Початок'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПервыйЗапуск. Конец';uk='Новини. Відновлення ІБ. ОбластьДанных. Переобумовлений. ПервыйЗапуск. Кінець'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				Т1Длительность, // Данные
				ТекстСообщения, // Комментарий
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
		НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию. Начало';uk='Новини. Відновлення ІБ. ОбщиеДанные. ПерейтиНаВерсию. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПерейтиНаВерсию. Начало';uk='Новини. Відновлення ІБ. ОбщиеДанные. Переобумовлений. ПерейтиНаВерсию. Початок'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПерейтиНаВерсию. Конец';uk='Новини. Відновлення ІБ. ОбщиеДанные. Переобумовлений. ПерейтиНаВерсию. Кінець'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				Т1Длительность, // Данные
				ТекстСообщения, // Комментарий
	...
	Иначе
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию. Конец';uk='Новини. Відновлення ІБ. ОбщиеДанные. ПерейтиНаВерсию. Кінець'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"';uk='Конец процедуры обновления. Переход на версию %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()% (общие данные)"
"Разрешена работа с новостями = %лкРазрешенаРаботаСНовостями%"
"Время начала (мс): %ТНачало%"
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
		НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию_ХХХ. Начало';uk='Новини. Відновлення ІБ. ОбщиеДанные. ПерейтиНаВерсию_ХХХ. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПерейтиНаВерсию_ХХХ. Начало';uk='Новини. Відновлення ІБ. ОбщиеДанные. Переобумовлений. ПерейтиНаВерсию_ХХХ. Початок'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПерейтиНаВерсию_ХХХ. Конец';uk='Новини. Відновлення ІБ. ОбщиеДанные. Переобумовлений. ПерейтиНаВерсию_ХХХ. Кінець'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				Т1Длительность, // Данные
				ТекстСообщения, // Комментарий
	...
	Иначе
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию_ХХХ. Конец';uk='Новини. Відновлення ІБ. ОбщиеДанные. ПерейтиНаВерсию_ХХХ. Кінець'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"Время начала (мс): %ТНачало%"
"';uk='Початок процедури оновлення. Перехід на версію %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()% (область даних)"
"Час початку (мс): %ТНачало%"
"'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()%", ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки());
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
		НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию. Начало';uk='Новини. Відновлення ІБ. ОбластьДанных. ПерейтиНаВерсию. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
	...
	Если ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя() Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию. Конец';uk='Новини. Відновлення ІБ. ОбластьДанных. ПерейтиНаВерсию. Кінець'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПерейтиНаВерсию. Начало';uk='Новини. Відновлення ІБ. ОбластьДанных. Переобумовлений. ПерейтиНаВерсию. Початок'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
"Время окончания (мс): %ТКонец%"
"Длительность (мс): %ТДлительность%"
"';uk='Кінець процедури оновлення. Перехід на версію %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()% (область даних)"
"Час початку (мс): %ТНачало%"
"Час закінчення (мс): %ТКонец%"
"Тривалість (мс): %ТДлительность%"
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПерейтиНаВерсию. Конец';uk='Новини. Відновлення ІБ. ОбластьДанных. Переобумовлений. ПерейтиНаВерсию. Кінець'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				Т1Длительность, // Данные
				ТекстСообщения, // Комментарий
...
"'");
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
		НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию_ХХХ. Начало';uk='Новини. Відновлення ІБ. ОбластьДанных. ПерейтиНаВерсию_ХХХ. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения, // Комментарий
	...
	Если ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя() Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию_ХХХ. Конец';uk='Новини. Відновлення ІБ. ОбластьДанных. ПерейтиНаВерсию_ХХХ. Кінець'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПерейтиНаВерсию_ХХХ. Начало';uk='Новини. Відновлення ІБ. ОбластьДанных. Переобумовлений. ПерейтиНаВерсию_ХХХ. Початок'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
"'");
			...
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Відновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПерейтиНаВерсию_ХХХ. Конец';uk='Новини. Відновлення ІБ. ОбластьДанных. Переобумовлений. ПерейтиНаВерсию_ХХХ. Кінець'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				Т1Длительность, // Данные
				ТекстСообщения, // Комментарий
...
"%1"
""
"';uk='Ошибки настройки общего реквизита ОбластьДанныхОсновныеДанные:"
"%1"
""
"'"),
				РезультатЛокальный);
...
"%1"
""
"';uk='Ошибки настройки общего реквизита ОбластьДанныхВспомогательныеДанные:"
"%1"
""
"'"),
				РезультатЛокальный);
	...
	Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
		Результат = Результат
			+ НСтр("ru='Ошибки настройки прав для роли АдминистраторСистемы:';uk='Помилки налаштування прав для ролі АдминистраторСистемы:'")
	...
	Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
		Результат = Результат
			+ НСтр("ru='Ошибки настройки прав для роли ПолныеПрава:';uk='Помилки налаштування прав для ролі ПолныеПрава:'")
			+ Символы.ПС + РезультатЛокальный
			+ Символы.ПС + Символы.ПС;
	КонецЕсли;
...
"';uk='Не настроен идентификатор конфигурации."
"Для его настройки необходимо внести изменения в общий модуль"
"ОбработкаНовостейПереопределяемый.ПолучитьЗначениеПредопределеннойКатегории,"
"для строки условия ProductName."
"В противном случае новости для этой конфигурации приходить НЕ будут."
...
"';uk='Неправильно налаштован ідентифікатор конфігурації."
"Для його налаштування необхідно внести зміни в загальний модуль"
"ОбработкаНовостейПереопределяемый.ПолучитьЗначениеПредопределеннойКатегории,"
"для рядка умови ProductName."
"Ідентифікатор конфігурації повинен складатися з англійських букв, цифр, знака мінуса й знака підкреслення."
"'");
	КонецЕсли;

	Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
		Результат = Результат
			+ НСтр("ru='Ошибки настройки кода конфигурации:';uk='Помилки налаштування коду конфігурації:'")
			+ Символы.ПС + РезультатЛокальный
			+ Символы.ПС + Символы.ПС;
	КонецЕсли;
...
"ОбработкаНовостейПереопределяемый.ПолучитьЗначениеПредопределеннойКатегории,"
"для строки условия ProductVersion.';uk='Неправильно налаштована версія конфігурації (кількість чисел відмінно від 4)."
"У метаданих, у полі Версія повинні бути дані виду А.Б.В.Г (чотири числа, розділені крапкою)."
"Якщо з якихось причин версія в метаданих задана у відмінному від А.Б.В.Г форматі,"
"то необхідно внести зміни в загальний модуль"
"ОбработкаНовостейПереопределяемый.ПолучитьЗначениеПредопределеннойКатегории,"
...
"Список неправильных символов: %НайденныеНеправильныеСимволы%';uk='Неправильно налаштована версія конфігурації (є неправильні символи)."
"У метаданих, у полі Версія повинні бути дані виду А.Б.В.Г (чотири числа, розділені крапкою)."
"Якщо з якихось причин версія в метаданих задана у відмінному від А.Б.В.Г форматі,"
"то необхідно внести зміни в загальний модуль"
"ОбработкаНовостейПереопределяемый.ПолучитьЗначениеПредопределеннойКатегории,"
...
"Список неправильних символів: %НайденныеНеправильныеСимволы%'");
	...
	Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
		Результат = Результат
			+ НСтр("ru='Ошибки настройки версии конфигурации:';uk='Помилки налаштування версії конфігурації:'")
			+ Символы.ПС + РезультатЛокальный
			+ Символы.ПС + Символы.ПС;
	КонецЕсли;
...
"Также допустимо, что у %ПолноеИмяМетаданных% установлено использование Авто, но тогда у общего реквизита %ИмяОбщегоРеквизита% должно быть установлено Автоиспользование в %ПравильноеЗначениеИспользования%.';uk='Неправильно установлено использование для %ПолноеИмяМетаданных% в общем реквизите %ИмяОбщегоРеквизита%. Установлено: %НайденныйЭлемент.Использование%, ожидается: %ПравильноеЗначениеИспользования%."
"Также допустимо, что у %ПолноеИмяМетаданных% установлено использование Авто, но тогда у общего реквизита %ИмяОбщегоРеквизита% должно быть установлено Автоиспользование в %ПравильноеЗначениеИспользования%.'");
		...
		КонецЕсли;
	Иначе
		ТекущийРезультат = НСтр("ru='Не найден требуемый объект метаданных: %ПолноеИмяМетаданных%';uk='Не знайдений необхідний об`єкт метаданих: %ПолноеИмяМетаданных%'");
		ТекущийРезультат = СтрЗаменить(
			ТекущийРезультат,
			"%ПолноеИмяМетаданных%",
			ОбщегоНазначения.ИмяБазовогоТипаПоОбъектуМетаданных(лкОбъектМетаданных) + "." + лкОбъектМетаданных.Имя);
...
Процедура Проверка_Права(
	...
	Если ТекущееЗначениеПрава <> ПравильноеЗначениеПрава Тогда
		ТекущийРезультат = НСтр("ru='Неправильно установлено право %Право% в роли %Роль% для %ПолноеИмяМетаданных%. Установлено: %ТекущееЗначениеПрава%, ожидается: %ПравильноеЗначениеПрава%';uk='Неправильно встановлене право %Право% у ролі %Роль% для %ПолноеИмяМетаданных%. Установлене: %ТекущееЗначениеПрава%, очікується: %ПравильноеЗначениеПрава%'");
		ТекущийРезультат = СтрЗаменить(
			ТекущийРезультат,
			"%ПолноеИмяМетаданных%",
			ОбщегоНазначения.ИмяБазовогоТипаПоОбъектуМетаданных(лкОбъектМетаданных) + "." + лкОбъектМетаданных.Имя);
