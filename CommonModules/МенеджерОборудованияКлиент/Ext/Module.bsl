...
Функция ПодключитьОборудование(ИдентификаторКлиента, ТипыПО = Неопределено, ИдентификаторУстройства = Неопределено, ОписаниеОшибки = "") Экспорт
	...
	Если Не Результат Тогда
		ОписаниеОшибки = НСтр("ru='Предварительно необходимо выбрать рабочее место подключаемого оборудования текущего сеанса.';uk='Попередньо необхідно вибрати робоче місце підключаємого обладнання поточного сеансу.'");
		Возврат Ложь;
	КонецЕсли;
...
"Перевірте, що драйвер коректно встановлений і зареєстрований у системі.'");
		...
		Иначе
			Результат = ОбработчикДрайвераМодуль.ОтключитьУстройство(ПодключенноеУстройство.ОбъектДрайвера, ПодключенноеУстройство.Параметры, 
				ПодключенноеУстройство.ПараметрыПодключения, ВыходныеПараметры);
			Если НЕ Результат Тогда
				ОписаниеОшибки = НСтр("ru='При отключении устройства ""%Наименование%"" произошла ошибка: %ОписаниеОшибки% (%КодОшибки%)';uk='При відключенні обладнання ""%Наименование%"" відбулася помилка: %ОписаниеОшибки% (%КодОшибки%)'");
				ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Наименование%", ПодключенноеУстройство.Наименование);
				ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", ВыходныеПараметры[1]);
				ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%КодОшибки%", ВыходныеПараметры[0]);
			Иначе
...
Процедура НачатьВыполнениеКоманды(ОповещениеПриЗавершении, Идентификатор, Команда, ВходныеПараметры) Экспорт
	...
	Если ПодключенноеУстройство <> Неопределено Тогда
		...
		Если ОбработчикДрайвераМодуль = Неопределено Или ОбъектДрайвера = Неопределено Тогда
			// Сообщить об ошибке, что не удалось загрузить драйвер.
			ТекстОшибки = НСтр("ru='Не удалось подключить обработчик драйвера или загрузить драйвер.';uk='Не удалось подключить обработчик драйвера или загрузить драйвер.'");
			ВыходныеПараметры = Новый Массив();
			ВыходныеПараметры.Добавить(999);
...
Процедура НачатьПодключениеРасширенияРаботыСФайламиЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Не Подключено И ДополнительныеПараметры.ПредлагатьУстановку Тогда
		...
		ТекстСообщения = НСтр("ru='Для продолжении работы необходимо установить расширение для веб-клиента ""1С:Предприятие"". Установить?';uk='Для продовження роботи необхідно встановити розширення для веб-клієнта ""1С:Підприємство"". Встановити?'");
...
Процедура ПредложитьВыбратьУстройство(ОповещениеВыбора, ТипОборудования, ТекстЗаголовкаВыбора, 
	...
	Если Не ОбновитьРабочееМестоКлиента() Тогда
		ТекстСообщения = НСтр("ru='Предварительно необходимо выбрать рабочее место подключаемого оборудования текущего сеанса.';uk='Попередньо необхідно вибрати робоче місце підключаємого обладнання поточного сеансу.'");
...
Функция КодСоответствуетШаблонуМК(ДанныеДорожек, ДанныеШаблона) Экспорт
	...
	Для Итератор = 1 По 3 Цикл
		Если ДанныеШаблона["ДоступностьДорожки"+Строка(Итератор)] Тогда
			...
			Если Найти(текСтрока, ДанныеШаблона["РазделительБлоков"+Строка(Итератор)]) = 0 Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Дорожка';uk='Доріжка'") + Символы.НПП + Строка(Итератор) 
					+ ". "+НСтр("ru='Разделитель блоков карты не соответствует разделителю блоков шаблона.';uk='Роздільник блоків картки не відповідає роздільнику блоків шаблону.'"));
				ПроверкаПройдена = Ложь;
			КонецЕсли;
	...
	Если ОднаДорожкаПрисутствует Тогда 
		Возврат Истина;
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='В шаблоне не указано ни одной доступной дорожки.';uk='У шаблону не зазначене ні однієї доступної доріжки.'"));
		Возврат Ложь;
	КонецЕсли;
...
Процедура НачатьВыполнениеКоманды_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			...
			РезультатОперации.ОписаниеОшибки = НСтр("ru='Фискальное устройство не выбрано или не подключено.';uk='Фіскальний пристрій не обрано або не підключено.'");
...
Процедура НачатьПечатьФискальногоОтчета(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, 
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, ТипОборудования,
			НСтр("ru='Выберите фискальное устройство';uk='Виберіть фіскальне обладнання'"), НСтр("ru='Фискальное устройство не подключено.';uk='Фіскальний пристрій не підключено.'"), НСтр("ru='Фискальное устройство не выбрано.';uk='Фіскальний пристрій не обрано.'"));
	Иначе
...
Процедура НачатьПечатьТекста(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, 
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("НачатьВыполнениеКоманды_ВыбратьУстройствоЗавершение", ЭтотОбъект, Контекст);
		ПредложитьВыбратьУстройство(ОписаниеОповещения, ПоддерживаемыеТипыВО,
			НСтр("ru='Выберите печатающее устройство';uk='Виберіть друкувальний пристрій'"), НСтр("ru='Печатающее устройство не подключено.';uk='Друкувальний пристрій не підключений.'"), 
			НСтр("ru='Печатающее устройство не выбрано.';uk='Друкувальний пристрій не обраний.'"), НЕ ОтображатьСообщения);
	Иначе
		НачатьВыполнениеКоманды_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Контекст);
	КонецЕсли;
...
Процедура НачатьПодключенияЭквайринговогоТерминала(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройстваЭТ = Неопределено, ИдентификаторУстройстваПУ = Неопределено) Экспорт
	...
	Если Контекст.ИдентификаторУстройстваЭТ = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, "ЭквайринговыйТерминал",
			НСтр("ru='Выберите эквайринговый терминал';uk='Виберіть еквайринговий термінал'"), НСтр("ru='Эквайринговый терминал не подключен.';uk='Еквайринговий термінал не підключений.'"), НСтр("ru='Эквайринговый терминал не выбран.';uk='Еквайринговий термінал не обраний.'"), Истина);
	Иначе
...
Процедура НачатьПодключенияЭквайринговогоТерминала_ВыбратьУстройствоЗавершение(ИдентификаторУстройстваЭТ, Параметры) Экспорт
	...
	Если ИдентификаторУстройстваЭТ = Неопределено Тогда
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			...
			РезультатОперации.Результат = Ложь;
			РезультатОперации.ОписаниеОшибки = НСтр("ru='Эквайринговый терминал не выбран.';uk='Еквайринговий термінал не обраний.'");
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
		КонецЕсли;
	Иначе
...
Процедура НачатьПодключенияЭквайринговогоТерминала_ВыбратьПечатающегоУстройстваЗавершение(ИдентификаторУстройстваПУ, Параметры) Экспорт
	...
	Если ИдентификаторУстройстваПУ = Неопределено Тогда
		...
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			РезультатОперации = ПараметрыРезультатаПодключенияЭквайринговогоТерминала();
			РезультатОперации.Результат = Ложь;
			РезультатОперации.ОписаниеОшибки = НСтр("ru='Печатающее устройство не выбрано.';uk='Друкувальний пристрій не обраний.'");
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
		КонецЕсли;
	Иначе
...
Процедура НачатьПолученияВесаСЭлектронныхВесов(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, ОтображатьСообщения = Истина) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(Оповещение, "ЭлектронныеВесы",
			НСтр("ru='Выберите электронные весы';uk='Виберіть електронні ваги'"), НСтр("ru='Электронные весы не подключены.';uk='Електронні ваги не підключені.'"), НСтр("ru='Электронные весы не выбраны.';uk='Електронні ваги не обрані.'"), Истина);
	Иначе
...
Процедура НачатьВыгрузкуДанныеВВесыСПечатьюЭтикеток_ФайловоеРасширениеЗавершение(Подключено, Параметры) Экспорт
	...
	Иначе
		ТекстСообщения = НСтр("ru='Данная операция не доступна без установленного расширения для веб-клиента ""1С:Предприятие"".';uk='Дана операція не доступна без встановленого розширення для веб-клієнта ""1С:Підприємство"".'");
...
"%ОписаниеОшибки%';uk='При очищенні даних в обладнанні відбулася помилка."
"%ОписаниеОшибки%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОписаниеОшибки%", РезультатВыполнения.ВыходныеПараметры[1]);
		Если Параметры.ОтображатьСообщения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура НачатьПечатьЭтикеток_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		Если Параметры.СледующееОповещение <> Неопределено Тогда
			ТекстСообщения = НСтр("ru='Принтер этикеток не выбран.';uk='Принтер етикеток не обраний.'");
			Результат = Новый Структура("Результат, ОписаниеОшибки, ИдентификаторУстройства", Ложь, ТекстСообщения, Неопределено);
			ВыполнитьОбработкуОповещения(Параметры.СледующееОповещение, Результат);
		КонецЕсли;
		Возврат;
...
"%ОписаниеОшибки%';uk='При печати на принтере этикеток произошла ошибка."
"%ОписаниеОшибки%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатВыполнения.ВыходныеПараметры[1]);
		Если Параметры.ОтображатьСообщения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ОписаниеОшибки);
...
Процедура НачатьВыгрузкуНастроекВККМOfflineФайловоеРасширениеЗавершение(Подключено, Параметры) Экспорт
	...
	Если Результат Тогда
		...
		Состояние(НСтр("ru='Выполняется выгрузка настроек в ККМ Offline...';uk='Виконується вивантаження налаштувань у ККМ Offline...'")); 
...
Процедура НачатьОчисткуТоваровВККМOfflineЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		ТекстСообщения = НСтр("ru='Оборудование не выбрано.';uk='Обладнання не обране.'");
		Если Параметры.СледующееОповещение <> Неопределено Тогда
...
Процедура НачатьОчисткуТоваровВККМOfflineФайловоеРасширениеЗавершение(Подключено, Параметры) Экспорт
	...
	Если Результат Тогда
		...
		Состояние(НСтр("ru='Выполняется очистка товаров в ККМ Offline...';uk='Виконується очищення товарів у ККМ Offline...'"));
...
"%ОписаниеОшибки%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОписаниеОшибки%", ВыходныеПараметры[1]);
			Иначе
				ТекстСообщения = НСтр("ru='Очистка данных успешно завершена.';uk='Очищення даних успішно завершено.'");
			КонецЕсли;
...
Процедура НачатьВыгрузкуДанныеВККМOffline(ОповещениеПриВыгрузкеДанные, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено,
	...
	Иначе
		...
		Если МенеджерОборудованияВызовСервера.ПроверитьДублированиеКодов(РасширеннаяВыгрузка, ГруппыТоваров, Товары, ДублирующиесяКоды) Тогда
			...
			ТекстСообщения = НСтр("ru='Выгрузка невозможна! Дублируются следующие коды товаров: %1';uk='Выгрузка невозможна! Дублируются следующие коды товаров: %1'");
...
Процедура НачатьВыгрузкуДанныеВККМOfflineФайловоеРасширениеЗавершение(Подключено, Параметры) Экспорт
	...
	Если Результат Тогда
		...
		Состояние(НСтр("ru='Выполняется выгрузка товаров в ККМ Offline...';uk='Виконується вивантаження товарів у ККМ Offline...'"));
...
Процедура НачатьЗагрузкуОтчетаОРозничныхПродажахИзККМOfflineФайловоеРасширениеЗавершение(Подключено, Параметры) Экспорт
	...
	Если Результат Тогда
		Состояние(НСтр("ru='Выполняется загрузка товаров из ККМ Offline...';uk='Виконується завантаження товарів із ККМ Offline...'"));
...
"%ОписаниеОшибки%';uk='При загрузке данных из ККМ Offline произошла ошибка."
"%ОписаниеОшибки%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОписаниеОшибки%", ВыходныеПараметры[1]);
				ТаблицаТоваров = Неопределено;
			Иначе
				ТаблицаТоваров = ВыходныеПараметры[0];
				ТекстСообщения = НСтр("ru='Данные загружены успешно.';uk='Дані завантажені успішно.'");
			КонецЕсли;
...
Процедура НачатьОткрытиеСессииСчитывателяRFID(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено) Экспорт
	...
	ИначеЕсли  ОповещениеПриЗавершении <> Неопределено Тогда
		РезультатОперации = ПараметрыВыполненияОперацииНаОборудовании();
		РезультатОперации.Результат = Ложь;
		РезультатОперации.ОписаниеОшибки = НСтр("ru='Нет подключенных RFID считывателей.';uk='Нет подключенных RFID считывателей.'");
		ВыполнитьОбработкуОповещения(ОповещениеПриЗавершении, РезультатОперации);
	КонецЕсли;
...
Процедура НачатьОткрытиеСессииСчитывателяRFID_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = "";
	Иначе
		ОписаниеОшибки = НСтр("ru='При работе со считывателем RFID произошла ошибка: %ДополнительноеОписание%';uk='При работе со считывателем RFID произошла ошибка: %ДополнительноеОписание%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ДополнительноеОписание%", РезультатВыполнения.ВыходныеПараметры[1]);
	КонецЕсли;
...
Процедура НачатьЗаписьДанныхВМеткуRFID(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, ПараметрыЗаписи, Таймаут = 0) Экспорт 
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, "СчитывательRFID",
			НСтр("ru='Выберите RFID считыватель';uk='Выберите RFID считыватель'"), НСтр("ru='Нет подключенных RFID считывателей.';uk='Нет подключенных RFID считывателей.'"));
	Иначе
...
Процедура НачатьЗаписьДанныхВМеткуRFID_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		ОписаниеОшибки = НСтр("ru='RFID считыватель  не подключен.';uk='RFID считыватель  не подключен.'");
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			РезультатОперации = ПараметрыВыполненияОперацииНаОборудовании();
			РезультатОперации.Результат = Ложь;
			РезультатОперации.ОписаниеОшибки = ОписаниеОшибки;
...
Процедура НачатьЗаписьДанныхВМеткуRFID_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	ОписаниеОшибки = ""; 
	Если НЕ РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = НСтр("ru='При работе со считывателем RFID произошла ошибка: %ОписаниеОшибки%';uk='При работе со считывателем RFID произошла ошибка: %ОписаниеОшибки%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатВыполнения.ВыходныеПараметры[1]);
	КонецЕсли;
...
"Файл ""%Файл%"" в макете не найден.';uk='Помилка установки драйвера з дистрибутива в макеті."
...
"Необходимый файл в архиве не найден.';uk='Помилка установки драйвера з дистрибутива в архіві."
"Необхідний файл в архіві не знайдений.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки); 
...
Процедура УстановитьДрайвер(Идентификатор, ОповещениеИзДистрибутиваПриЗавершении = Неопределено, ОповещениеИзАрхиваПриЗавершении = Неопределено) Экспорт
	...
	Попытка  
		Если ДанныеДрайвера.ВСоставеКонфигурации Тогда
			...
			Если ДанныеДрайвера.ПоставляетсяДистрибутивом Тогда
			#Если ВебКлиент Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Данный драйвер не поддерживает работу в веб-клиенте.';uk='Даний драйвер не підтримує роботу у веб-клієнтові.'")); 
			#Иначе
				Если МенеджерОборудованияКлиентПовтИсп.ЭтоLinuxКлиент() Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Данный драйвер не может быть установлен и использован в среде Linux.';uk='Даний драйвер не може бути встановлений і використаний у середовищі Linux.'")); 
					Возврат;
				КонецЕсли;
...
Функция СформироватьШаблонИнфоКвитанции(Параметры, ШиринаСтроки, ПечатьШтрихкода = Истина) Экспорт
	...
	НомерКассы = ?(НЕ ПустаяСтрока(Параметры.НомерКассы), НСтр("ru='КАССА:';uk='КАССА:'") + Параметры.НомерКассы, "");
	НомерСмены = ?(НЕ ПустаяСтрока(Параметры.НомерСмены), НСтр("ru='СМЕНА:';uk='СМЕНА:'") + Параметры.НомерСмены, "");
...
Функция СформироватьТексНефискальногоЧека(ШиринаСтроки, ВходныеПараметры) Экспорт
		...
		Для ИндексМассива = 0 По ПозицииЧека.Количество() - 1 Цикл
			...
			Если ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
				...
				Если Окр(Количество * Цена, 2) > Сумма Тогда
					ТекстСтроки = ТекстСтроки + ПостроитьПоле(НСтр("ru='СКИДКА =';uk='ЗНИЖКА ='") + Символы.НПП
									+ Формат(Количество * Цена - Сумма, ФорматЧисла) + " ", ШиринаСтроки, , Ложь)  + Символы.ПС;
				ИначеЕсли Окр(Количество * Цена, 2) < Сумма Тогда
					ТекстСтроки = ТекстСтроки + ПостроитьПоле(НСтр("ru='НАДБАВКА =';uk='НАДБАВКА ='") + Символы.НПП
	...
	Если ТаблицаОплат <> Неопределено Тогда
		...
		Текст = Текст + РазделительВнут;
		Текст = Текст + ПостроитьПоле(НСтр("ru='ИТОГ =';uk='ПІДСУМОК ='"), 16, ,Ложь) + Символы.НПП + Формат(СуммаЧека, ФорматЧисла) + Символы.ПС;
		Текст = Текст + РазделительВнут;
		
		Если СуммаНаличнойОплаты > 0 Тогда
			Текст = Текст + ПостроитьПоле(НСтр("ru='НАЛИЧНЫМИ =';uk='ГОТІВКОЮ ='"), 16, ,Ложь) + Символы.НПП + Формат(СуммаНаличнойОплаты, ФорматЧисла) + Символы.ПС;
		КонецЕсли;
		Если СуммаБезналичнойОплаты1 > 0 Тогда
			Текст = Текст + ПостроитьПоле(НСтр("ru='ПЛАТ.КАРТОЙ =';uk='ПЛАТ.КАРТКОЮ ='"), 16, ,Ложь) + Символы.НПП + Формат(СуммаБезналичнойОплаты1, ФорматЧисла) + Символы.ПС;
		КонецЕсли;
		Если СуммаБезналичнойОплаты2 > 0 Тогда
			Текст = Текст + ПостроитьПоле(НСтр("ru='КРЕДИТОМ =';uk='КРЕДИТОМ ='"), 16, ,Ложь) + Символы.НПП + Формат(СуммаБезналичнойОплаты2, ФорматЧисла) + Символы.ПС;
		КонецЕсли;
		Если СуммаБезналичнойОплаты3 > 0 Тогда
			Текст = Текст + ПостроитьПоле(НСтр("ru='СЕРТИФИКАТОМ =';uk='СЕРТИФІКАТОМ ='"), 16, ,Ложь) + Символы.НПП + Формат(СуммаБезналичнойОплаты3, ФорматЧисла) + Символы.ПС;
		КонецЕсли;          
		
		Текст = Текст + РазделительВнут;
		Текст = Текст + ПостроитьПоле(НСтр("ru='СДАЧА =';uk='ЗДАЧА ='"), 16, ,Ложь) + Символы.НПП + Формат(СуммаОплаты - СуммаЧека, ФорматЧисла) + Символы.ПС;
		Текст = Текст + РазделительВнут;
		
		ИмяКассира = ?(ОбщиеПараметры.Свойство("ИмяКассира"), ОбщиеПараметры.ИмяКассира, "");
		Текст = Текст + ПостроитьПоле(Символы.НПП + НСтр("ru='КАССИР';uk='КАССИР'") + ":" + Символы.НПП + ИмяКассира, ШиринаСтроки) + Символы.ПС;                        
		Текст = Текст + ПостроитьПоле(НСтр("ru='ПОДПИСЬ';uk='ПОДПИСЬ'") + ":", ШиринаСтроки, "_") + Символы.ПС + Символы.ПС;
...
Функция СформироватьШаблонЧека(ВходныеПараметры, ДополнительныйТекст = Неопределено) Экспорт
	...
	ТипыЧеков.Вставить(0, НСтр("ru='ПРОДАЖА';uk='ПРОДАЖА'"));       
	ТипыЧеков.Вставить(1, НСтр("ru='ВОЗВРАТ';uk='ПОВЕРНЕННЯ'"));  
	ТипыЧеков.Вставить(2, НСтр("ru='ВНЕСЕНИЕ';uk='ВНЕСЕНИЕ'"));  
	ТипыЧеков.Вставить(3, НСтр("ru='ВЫЕМКА';uk='ВЫЕМКА'"));  
	ТипыЧеков.Вставить(4, НСтр("ru='ОТЧЕТ БЕЗ ГАШЕНИЯ';uk='ОТЧЕТ БЕЗ ГАШЕНИЯ'"));  
	ТипыЧеков.Вставить(5, НСтр("ru='ОТЧЕТ С ГАШЕНИЕМ';uk='ОТЧЕТ С ГАШЕНИЕМ'"));  
		...
		Для ИндексМассива = 0 По ПозицииЧека.Количество() - 1 Цикл
			...
			ИначеЕсли ПозицияЧека.Свойство("Штрихкод") Тогда
				...
				Если НЕ ОбщийМодульОборудования.ПечатьШтрихкода(ОбъектДрайвера, Параметры, ПараметрыПодключения,
					...
					Текст = НСтр("ru='<Штрихкод %ТипШтрихКода% не распечатан>';uk='<Штрихкод %ТипШтрихКода% не распечатан>'");
...
Процедура ПолучитьСодержаниеТекстовыхФайловЗавершение(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ПомещенныеФайлы = Неопределено Тогда
		...
		РезультатЧтенияФайлов.ТекстОшибки = НСтр("ru='Неизвестная ошибка при передаче файлов на сервер.';uk='Неизвестная ошибка при передаче файлов на сервер.'");
...
// Функция формирует подпись QR чека.
//
Процедура СформироватьПодписьЧека(ШиринаСтроки, Параметры, ПозицииЧека, ПечатьШтрихкода = Истина) Экспорт
	...
	СтрокаПозицииЧека = Новый Структура();
	СтрокаПозицииЧека.Вставить("ТекстоваяСтрока");
	СтрокаПозицииЧека.Вставить("Текст", РазделительВнут);
	ПозицииЧека.Добавить(СтрокаПозицииЧека);
	...
	Если ПечатьШтрихкода Тогда
		...
		СтрокаПозицииЧека.Вставить("ШтрихКод");
		СтрокаПозицииЧека.Вставить("ТипШтрихкода", "QR");
		СтрокаПозицииЧека.Вставить("ШтрихКод"    , Параметры.АдресЧека);
		ПозицииЧека.Добавить(СтрокаПозицииЧека);
	КонецЕсли;
	...
	СтрокаПозицииЧека = Новый Структура();
	СтрокаПозицииЧека.Вставить("ТекстоваяСтрока");
	СтрокаПозицииЧека.Вставить("Текст", МенеджерОборудованияКлиент.ПостроитьПолеПереносом(Параметры.АдресЧека, ШиринаСтроки));
	ПозицииЧека.Добавить(СтрокаПозицииЧека);
	...
	СтрокаПозицииЧека = Новый Структура();
	СтрокаПозицииЧека.Вставить("ТекстоваяСтрока");
	СтрокаПозицииЧека.Вставить("Текст", МенеджерОборудованияКлиент.ПостроитьПолеПереносом(Параметры.ПодписьЧека, ШиринаСтроки));
	ПозицииЧека.Добавить(СтрокаПозицииЧека);
