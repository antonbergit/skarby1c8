...
Процедура ОбновитьСтатусыЗаявленийАбонентов(
	...
	Если ЗаявленияАбонентов.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Отсутствуют отправленные заявления';uk='Отсутствуют отправленные заявления'"));
		Возврат;
	КонецЕсли;
...
Процедура СообщитьОбОтсуствииИнтернета(ДополнительныеПараметры)
	...
	ОписаниеОшибки 		= НСтр("ru='Не удалось обратиться к серверу обновлений';uk='Не удалось обратиться к серверу обновлений'");
...
Процедура СообщитьОбОтсуствииИзмененияВСтатусеИПолучитьОтветНаСледующееЗаявление(ДополнительныеПараметры)
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Статус заявления %1 от %2 не изменился';uk='Статус заявления %1 от %2 не изменился'"),
