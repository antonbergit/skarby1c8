...
Функция ОтправитьSMS(НомераПолучателей, Текст, ИмяОтправителя, Логин, Пароль) Экспорт
	...
	Если ПустаяСтрока(СтрокаПолучателей) Или ПустаяСтрока(Текст) Тогда
		Результат.ОписаниеОшибки = НСтр("ru='Неверные параметры сообщения';uk='Неверные параметры сообщения'");
		Возврат Результат;
	КонецЕсли;
	...
	Если ПустаяСтрока(ИмяФайлаОтвета) Тогда
		Результат.ОписаниеОшибки = Результат.ОписаниеОшибки + НСтр("ru='Соединение не установлено';uk='Соединение не установлено'");
		Возврат Результат;
	КонецЕсли;
...
"%1 (код ошибки: %2)';uk='Не удалось отправить SMS:"
"%1 (код ошибки: %2)'"), ОписаниеОшибкиОтправки(ОтветСервера["error_code"]), ОтветСервера["error_code"]);
			
		ЗаписьЖурналаРегистрации(НСтр("ru='Отправка SMS';uk='Відправка SMS'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
"%1 (код ошибки: %2)';uk='Не удалось получить статус доставки SMS (id: ""%3""):"
...
Функция ОписаниеОшибкиОтправки(КодОшибки)
	...
	ОписанияОшибок.Вставить(1, НСтр("ru='Ошибка в параметрах.';uk='Ошибка в параметрах.'"));
	ОписанияОшибок.Вставить(2, НСтр("ru='Неверный логин или пароль.';uk='Невірний логін або пароль.'"));
	ОписанияОшибок.Вставить(3, НСтр("ru='Недостаточно средств на счете.';uk='Недостаточно средств на счете.'"));
	ОписанияОшибок.Вставить(4, НСтр("ru='IP-адрес временно заблокирован из-за частых ошибок в запросах.Подробнее см. http://smsc.ru/faq/99';uk='IP-адрес временно заблокирован из-за частых ошибок в запросах.Подробнее см. http://smsc.ru/faq/99'"));
	ОписанияОшибок.Вставить(5, НСтр("ru='Неверный формат даты.';uk='Неверный формат даты.'"));
	ОписанияОшибок.Вставить(6, НСтр("ru='Сообщение запрещено (по тексту или по имени отправителя).';uk='Сообщение запрещено (по тексту или по имени отправителя).'"));
	ОписанияОшибок.Вставить(7, НСтр("ru='Неверный формат номера телефона.';uk='Невірний формат номера телефону.'"));
	ОписанияОшибок.Вставить(8, НСтр("ru='Сообщение на указанный номер не может быть доставлено.';uk='Сообщение на указанный номер не может быть доставлено.'"));
	ОписанияОшибок.Вставить(9, НСтр("ru='Отправка более одного одинакового запроса на передачу SMS-сообщения в течение минуты.';uk='Отправка более одного одинакового запроса на передачу SMS-сообщения в течение минуты.'"));
	
	ТекстСообщения = ОписанияОшибок[КодОшибки];
	Если ТекстСообщения = Неопределено Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Сообщение не отправлено (код ошибки: %1).';uk='Сообщение не отправлено (код ошибки: %1).'"), КодОшибки);
	КонецЕсли;
...
Функция ОписаниеОшибкиПолученияСтатуса(КодОшибки)
	...
	ОписанияОшибок.Вставить(1, НСтр("ru='Ошибка в параметрах.';uk='Ошибка в параметрах.'"));
	ОписанияОшибок.Вставить(2, НСтр("ru='Неверный логин или пароль.';uk='Невірний логін або пароль.'"));
	ОписанияОшибок.Вставить(5, НСтр("ru='Ошибка удаления сообщения.';uk='Ошибка удаления сообщения.'"));
	ОписанияОшибок.Вставить(4, НСтр("ru='IP-адрес временно заблокирован из-за частых ошибок в запросах. Подробнее см. http://smsc.ru/faq/99';uk='IP-адрес временно заблокирован из-за частых ошибок в запросах. Подробнее см. http://smsc.ru/faq/99'"));
	ОписанияОшибок.Вставить(9, НСтр("ru='Отправка более пяти запросов на получения статуса одного и того же сообщения в течение минуты.';uk='Отправка более пяти запросов на получения статуса одного и того же сообщения в течение минуты.'"));
	
	ТекстСообщения = ОписанияОшибок[КодОшибки];
	Если ТекстСообщения = Неопределено Тогда
		ТекстСообщения = НСтр("ru='Отказ выполнения операции';uk='Отказ выполнения операции'");
	КонецЕсли;
	Возврат ТекстСообщения;
КонецФункции
...
Функция ВыполнитьЗапрос(ИмяМетода, ПараметрыЗапроса)
	...
	Попытка
		Соединение = Новый HTTPСоединение("alphasms.ua", , , , ПолучениеФайловИзИнтернетаКлиентСервер.ПолучитьПрокси("http"), 60);
		Соединение.ОтправитьДляОбработки(ИмяФайлаЗапроса, "/sys/" + ИмяМетода, ИмяФайлаОтвета, Заголовок);
		Результат = ИмяФайлаОтвета;
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Отправка SMS';uk='Відправка SMS'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция Разрешения() Экспорт
	...
	Адрес = "smsc.ru";
	Порт = Неопределено;
	Описание = НСтр("ru='Отправка SMS через ""SMS-ЦЕНТР"".';uk='Отправка SMS через ""SMS-ЦЕНТР"".'");
