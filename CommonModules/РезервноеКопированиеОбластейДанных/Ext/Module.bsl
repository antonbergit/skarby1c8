...
Процедура УстановитьФлагАктивностиВОбласти(Знач ОбластьДанных = Неопределено, Знач Состояние = Истина)
	...
	Если ОбластьДанных = Неопределено Тогда
		...
		Иначе
			ВызватьИсключение НСтр("ru='При вызове процедуры УстановитьФлагАктивностиВОбласти из неразделенного сеанса параметр ОбластьДанных является обязательным!';uk='При виклику процедури УстановитьФлагАктивностиВОбласти з нерозподіленого сеансу параметр ОбластьДанных є "
"обов`язковим!'");
		КонецЕсли;
	Иначе
...
Процедура СозданиеКопий(Знач ПараметрыСоздания, Знач СостояниеСоздания) Экспорт
	...
	Попытка
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Создание резервной копии области данных.Не удалось удалить временный файл';uk='Створення резервної копії області даних.Не вдалося вилучити тимчасовий файл'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
"Номер попытки: %2"
"По причине:"
"%3';uk='При створенні резервної копії області %1 відбулася "
"невідновна помилка."
"Номер спроби: %2"
"З причини:"
"%3'");
...
"%1';uk='Помилка при отриманні налаштувань резервного копіювання:"
"%1'");
...
"%1';uk='Помилка при збереженні налаштувань резервного копіювання:"
"%1'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, СообщениеОбОшибке);
		ВызватьИсключение(ТекстСообщения);
	КонецЕсли;
