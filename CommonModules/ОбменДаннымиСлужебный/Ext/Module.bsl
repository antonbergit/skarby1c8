...
"Необходимо для загружаемого файла назначить уникальный идентификатор.';uk='Не зазначений ідентифікатор файлу, що завантажується. Подальше виконання методу неможливе."
"Необхідно для файлу, що завантажується, призначити унікальний ідентифікатор.'");
		ВызватьИсключение(СообщениеОбОшибке);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ЗагружаемаяЧастьФайла)
		И ТипЗнч(ЗагружаемаяЧастьФайла) <> Тип("ДвоичныеДанные") Тогда
		СообщениеОбОшибке = НСтр("ru='Метод не может быть выполнен, т.к. переданные данные не соответствуют типу для получения данных.';uk='Метод не може бути виконаний, тому що передані дані не відповідають типу для одержання даних.'");
		ВызватьИсключение(СообщениеОбОшибке);
	КонецЕсли;
...
Функция ВыгрузитьЧастьФайла(ИдентификаторФайла, НомерВыгружаемойЧастиФайла, СообщениеОбОшибке) Экспорт
	...
	Иначе
		СообщениеОбОшибке = НСтр("ru='Часть файла с указанным номером не найдена.';uk='Частина файлу із зазначеним номером не знайдена.'");
	КонецЕсли;
...
"""Каталог временных файлов для Linux"" и ""Каталог временных файлов для Windows"".';uk='Не знайдений жоден фрагмент сесії передачі з ідентифікатором %1."
"Необхідно переконатися, що в налаштуваннях програми задані параметри"
...
Процедура ПроверитьБлокировкуИнформационнойБазыДляОбновления() Экспорт
	...
	Если ЗначениеЗаполнено(ОбновлениеИнформационнойБазыСлужебный.ИнформационнаяБазаЗаблокированаДляОбновления()) Тогда
		...
		ВызватьИсключение НСтр("ru='Синхронизация данных временно недоступна в связи с обновлением приложения.';uk='Синхронізація даних тимчасово недоступна у зв`язку з оновленням додатку.'");
