...
Функция СформироватьОчередьСообщенийОбмена(УзелОбмена, НомерПринятого, НужнаИнициализацияУзла = Ложь, ЭтоНовыйОбмен = Ложь) Экспорт
	...
	Если ЭтоНовыйОбмен Тогда
		// Запись ролей.
		ОбменМобильноеПриложениеПереопределяемый.ЗаписатьРолиВОчередьСообщенийОбмена(УзелОбмена, НомерСообщенияОчереди);
		// Запись сведений о компании.
		ОбменМобильноеПриложениеПереопределяемый.ЗаписатьСведенияОКомпанииВОчередьСообщенийОбмена(УзелОбмена, НомерСообщенияОчереди);
	КонецЕсли;
	
	ОбменМобильноеПриложениеПереопределяемый.ЗаписатьСведенияОНастрокахНалогооблаженияВОчередьСообщенийОбмена(УзелОбмена, НомерСообщенияОчереди);
