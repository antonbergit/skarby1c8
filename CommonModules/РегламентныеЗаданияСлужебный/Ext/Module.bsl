...
"Регламентное задание ""%1"", работающее с внешними ресурсами, отключено.';uk='Додаток був переміщений."
"Регламентне завдання ""%1"", працююче із зовнішніми ресурсами, відключено.'"), РегламентноеЗадание.Синоним);
...
"Регламентное задание ""%1"" отключено.';uk='Змінився рядок з`єднання інформаційної бази."
"Можливо інформаційна база була переміщена."
"Регламентне завдання ""%1"" відключене. '"), РегламентноеЗадание.Синоним);
...
Функция НастройкиПоУмолчанию()
	...
	НастройкиПодсистемы.Вставить("РасположениеКомандыСнятияБлокировки",
		НСтр("ru='Блокировку также можно снять позднее в разделе <b>Администрирование - Поддержка и обслуживание</b>.';uk='Блокування також можна зняти пізніше в розділі <b>Адміністрування - Підтримка і обслуговування</ b>.'"));
...
////////////////////////////////////////////////////////////////////////////////
// Обработчики событий других библиотек.

// Заполняет массив типов, исключаемых из выгрузки и загрузки данных.
//
// Параметры:
//  Типы - Массив - массив элементов типа Тип.
//
...
"информационная база работать с внешними ресурсами.';uk='%1"
""
...
""
"Якщо файл відсутній в каталозі з інформаційною базою, то програма запросить адміністратора, чи повинна ця"
"інформаційна база працювати з зовнішніми ресурсами. '"), ИдентификаторБазы, Метаданные.Синоним);
...
Процедура УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Работа с внешними ресурсами заблокирована';uk='Робота із зовнішніми ресурсами заблокована'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция УстановленаБлокировкаРаботыСВнешнимиРесурсами(ЭтоВызовПередНачаломРаботыСистемы = Ложь, ПоказатьФорму = Ложь) Экспорт
	...
	Если РазделениеВключено <> ПараметрыБлокировки.РазделениеВключено Тогда
		...
		ТекстСообщения = НСтр("ru='Информационная база была перемещена из приложения в Интернете.';uk='Інформаційна база була переміщена з додатка в Інтернеті.'");
	...
	Если ЭтоФайловаяБаза <> ПараметрыБлокировки.ЭтоФайловаяБаза Тогда
		ТекстСообщения = ?(ЭтоФайловаяБаза, НСтр("ru='Информационная база была перемещена из клиент-серверного режима работы в файловый.';uk='Інформаційна база була переміщена з клієнт-серверного режиму роботи у файловий.'"),
	...
	Если ЭтоФайловаяБаза Тогда
		...
		Если ИдентификаторБазы <> ПараметрыБлокировки.ИдентификаторБазы Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Идентификатор информационной базы в файле проверки %1 не соответствует идентификатору в текущей базе.';uk='Ідентифікатор інформаційної бази в файлі перевірки %1 не відповідає ідентифікатору в поточній базі.'"), ИмяФайлаПроверки);
			УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения);
			Возврат Истина;
		КонецЕсли;
...
"Сохраненное имя компьютера: %3, текущее имя компьютера: %4';uk='Розрізняються ім`я інформаційної бази або ім`я комп`ютера."
