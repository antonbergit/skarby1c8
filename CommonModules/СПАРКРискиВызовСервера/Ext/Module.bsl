...
Функция ИспользованиеРазрешено() Экспорт
	...
	Возврат СПАРКРискиПовтИсп.ИспользованиеРазрешено();
...
Процедура ВключитьОтключитьМониторингСобытийКонтрагента(Контрагент, МониторингВключен) Экспорт
	...
	Если Не СПАРКРиски.ИспользованиеРазрешено("ПостановкаНаМониторинг;") Тогда
		ВызватьИсключение НСтр("ru='Недостаточно прав для постановки контрагентов на мониторинг.';uk='Недостаточно прав для постановки контрагентов на мониторинг.'");
	КонецЕсли;
...
Функция ПолучитьПервыйИндекс(АдресВременногоХранилища) Экспорт
	...
	Если ПустаяСтрока(АдресВременногоХранилища) Тогда
		...
		Результат.Вставить("ТекстОшибки", НСтр("ru='Вызов веб-сервиса не сохранил данные в хранилище.';uk='Вызов веб-сервиса не сохранил данные в хранилище.'"));
