...
Функция ПрочитатьЭлементXML(ЧтениеXML, ТекстСообщенияОбОшибке)
	...
	Исключение
		ТекстСообщенияОбОшибке = НСтр("ru='При чтении XML-описания макета произошла ошибка.';uk='При читанні XML-опису макета відбулася помилка.'");
		Результат = Ложь;
	КонецПопытки;
...
Функция ПолучитьОписаниеМакета(Источник, СообщениеОбОшибке, Результат) Экспорт
	...
	Пока ПрочитатьЭлементXML(ЧтениеXML, ТекстСообщенияОбОшибке) Цикл
		...
		Если ЧтениеXML.Имя="Formatting" И ЧтениеXML.ТипУзла=ТипУзлаXML.НачалоЭлемента Тогда			
			...
			Иначе				
				СообщениеОбОшибке = НСтр("ru='При чтении файла атрибута Width элемента Formatting произошла ошибка.';uk='При читанні файлу атрибута Width елемента Formatting відбулася помилка.'");
				Возврат Ложь;				
			КонецЕсли;
			...
			Иначе				
				СообщениеОбОшибке = НСтр("ru='При чтении файла атрибута Height элемента Formatting произошла ошибка.';uk='При читанні файлу атрибута Height елемента Formatting відбулася помилка.'");
				Возврат Ложь;				
			КонецЕсли;
		...
		ИначеЕсли (ЧтениеXML.Имя="Text" ИЛИ ЧтениеXML.Имя="Barcode" ИЛИ ЧтениеXML.Имя="Image" ИЛИ ЧтениеXML.Имя="UserData") И ЧтениеXML.ТипУзла=ТипУзлаXML.НачалоЭлемента Тогда
			...
			Иначе				
				СообщениеОбОшибке = НСтр("ru='При чтении файла атрибута FieldName элемента UserData произошла ошибка.';uk='При читанні файлу атрибута FieldName елемента UserData відбулася помилка.'");
				Возврат Ложь;				
			КонецЕсли;
			...
			Если ПрочитатьАтрибут(ЧтениеXML, "TypeFill", ЗначениеАтрибута) Тогда
				...
				Если ЗначениеАтрибута=Неопределено Тогда					
					СообщениеОбОшибке = НСтр("ru='При чтении файла атрибута TypeFill элемента UserData произошла ошибка.';uk='При читанні файлу атрибута TypeFill елемента UserData відбулася помилка.'");
					Возврат Ложь;					
				КонецЕсли;
				...
				Если ЗначениеАтрибута="Parameter" Тогда					
					...
					Если ПрочитатьАтрибут(ЧтениеXML, "Value", ЗначениеАтрибута) Тогда
						...
						Если ЗначениеАтрибута=Неопределено Тогда							
							СообщениеОбОшибке = НСтр("ru='При чтении файла атрибута Value элемента UserData произошла ошибка.';uk='При читанні файлу атрибута Value елемента UserData відбулася помилка.'");
							Возврат Ложь;							
						Иначе							
			...
			Если НовоеПоле.Тип<>"UserData" Тогда				
				...
				Иначе					
					СообщениеОбОшибке = НСтр("ru='При чтении файла атрибута Left элемента Text произошла ошибка.';uk='При читанні файлу атрибута Left елемента Text відбулася помилка.'");
					Возврат Ложь;					
				КонецЕсли;
				...
				Иначе					
					СообщениеОбОшибке = НСтр("ru='При чтении файла атрибута Top элемента Text произошла ошибка.';uk='При читанні файлу атрибута Top елемента Text відбулася помилка.'");
					Возврат Ложь;					
				КонецЕсли;
				...
				Иначе					
					СообщениеОбОшибке = НСтр("ru='При чтении файла атрибута Width элемента Text произошла ошибка.';uk='При читанні файлу атрибута Width елемента Text відбулася помилка.'");
					Возврат Ложь;					
				КонецЕсли;
				...
				Иначе					
					СообщениеОбОшибке = НСтр("ru='При чтении файла атрибута Height элемента Text произошла ошибка.';uk='При читанні файлу атрибута Height елемента Text відбулася помилка.'");
					Возврат Ложь;					
				КонецЕсли;
				...
				Если НовоеПоле.Тип="Text" Тогда
					...
					Иначе
						СообщениеОбОшибке = НСтр("ru='При чтении файла атрибута FontSize элемента Text произошла ошибка.';uk='При читанні файлу атрибута FontSize елемента Text відбулася помилка.'");
						Возврат Ложь;
					КонецЕсли;
				...
				ИначеЕсли НовоеПоле.Тип="Barcode" Тогда
					...
					Иначе
						СообщениеОбОшибке = НСтр("ru='При чтении файла атрибута Type элемента Barcode произошла ошибка.';uk='При читанні файлу атрибута Type елемента Barcode відбулася помилка.'");
						Возврат Ложь;
					КонецЕсли;
...
Функция СформироватьТаблицуДопРеквизитов(ДанныеЗагрузки) Экспорт
	...
	Для Каждого Позиция Из ДанныеЗагрузки  Цикл
		...
		Если Позиция[1].Количество() = 0 Тогда
			ЗаписьXML.ЗаписатьАтрибут("IsExcisable", "False");
		Иначе	
			ЗаписьXML.ЗаписатьАтрибут("IsExcisable", Строка(Формат(Позиция[1][0], "БЛ=False; БИ=True")));
		КонецЕсли; 
		ЗаписьXML.ЗаписатьКонецЭлемента();
	КонецЦикла;
	ЗаписьXML.ЗаписатьКонецЭлемента();
