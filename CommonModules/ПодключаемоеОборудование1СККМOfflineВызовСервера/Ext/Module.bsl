...
Функция ПакетыОбработаны(Выгрузка, Пакеты, ВыходныеПараметры, ВерсияФорматаОбмена) Экспорт
	...
	Для Каждого ТекПакет Из Пакеты Цикл
		...
		Исключение
			СоздатьСообщениеОбОшибке(ВыходныеПараметры, НСтр("ru='При проверке файла на обработанность произошла ошибка.';uk='При перевірці файла на обробленість відбулася помилка.'"));
			Возврат Неопределено;
		КонецПопытки;
...
Функция ОбъектXDTOВТекстXML(ОбъектXDTO, ТекстXML, ВыходныеПараметры)
	...
	Исключение
		СоздатьСообщениеОбОшибке(ВыходныеПараметры, НСтр("ru='При формировании файла произошла ошибка.';uk='При формуванні файлу відбулася помилка.'"));
		Возврат Ложь;
	КонецПопытки;
