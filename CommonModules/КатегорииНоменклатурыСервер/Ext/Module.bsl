...
Функция ДеревоКатегорий() Экспорт
	...
	Если Дерево.Строки.Количество() > 1 Тогда
		...
		СтрокаНетКатегории.Представление = НСтр("ru='<Все категории>';uk='<Все категории>'");
	...
	Для каждого ОписаниеСвойства Из Форма.СвойстваОписаниеДополнительныхРеквизитов Цикл
		...
		Иначе
			...
			Если ОписаниеСвойства.ДополнительноеЗначение Тогда
				...
				//ЗначенияВыбора = СвойстваИЗначения.НайтиСтроки(ПараметрыОтбора);
				...
				//ЗначенияВыбора = Запрос.Выполнить().Выгрузить();
	...
	Если ОписаниеСвойства.ДополнительноеЗначение Тогда
		...
		Если ЗначенияВыбора <> Неопределено Тогда
			...
			Если ЗначенияВыбора.Количество() <= 10 Тогда
				...
				//	Элемент.СписокВыбора.Добавить(Значение.Значение);
				//КонецЦикла;
			...
			Иначе
				...
				//Для каждого Значение Из ЗначенияВыбора Цикл
				//	НоваяСтрока = Форма.СвойстваЗначенияВыбора.Добавить();
				//	НоваяСтрока.ЭлементИмя = Элемент.Имя;
