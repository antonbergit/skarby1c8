...
Процедура ПередНачаломРаботыКлиента(ТребуетсяОбновлениеИнтерфейса = Ложь) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(Нстр("ru='Получение активных тарифов';uk='Получение активных тарифов'"), УровеньЖурналаРегистрации.Ошибка, , , ?(ЗначениеЗаполнено(ПодробноеОписание), ПодробноеОписание, Строка(СписокТарифиов)));
	КонецПопытки;
...
Функция ТекстОшибкиПодключения()
	...
	Возврат(НСтр("ru='Не возможно подключиться к Менеджеру сервиса.';uk='Не можливо підключитися до Менеджера сервісу.'"));
...
Функция ЕстьАктивныйТарифМобильногоУНФ(Знач ОтветСтрокой)
	...
	Если ПустаяСтрока(ОтветСтрокой) Тогда
		ВызватьИсключение(НСтр("ru='Пустой ответ Менеджера сервиса по тарифам.';uk='Пустой ответ Менеджера сервиса по тарифам.'"));
	КонецЕсли;
...
Процедура СервисныеОперацииМобильногоПриложения() Экспорт
	...
	Запрос.УстановитьПараметр("ДатаПоследнейСинхронизации", ТекущаяДата() - 86400);
