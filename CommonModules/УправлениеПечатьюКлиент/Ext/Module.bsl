...
//
Процедура ВыполнитьКомандуПечати(ИмяМенеджераПечати, ИменаМакетов, МассивОбъектов, ВладелецФормы, ПараметрыПечати = Неопределено, КоличествоЭкземпляров = 2) Экспорт
...
#КонецЕсли
	...
	Если ПечатныеФормы.Отказ Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Нет прав для вывода печатной формы на принтер, обратитесь к администратору.';uk='Немає прав для виводу друкованої форми на принтер, зверніться до адміністратора.'"));
		Возврат;
	КонецЕсли;
