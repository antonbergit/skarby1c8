...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru='Выполняет обновление информации в форме 1С-Отчетность во всех разделах, кроме Отчеты. До завершения выполнения данные в форме 1С-Отчетность могут отображаться некорректно';uk='Виконує відновлення інформації у формі 1С-Звітність у всіх розділах, крім Звіти. До завершення виконання дані у формі 1С-Звітність можуть відображатися некоректно'");
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор();
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Комментарий = НСтр("ru='Выполняет обновление наименования загруженных отчетов.';uk='Виконує відновлення найменування завантажених звітів.'");
...
"До завершения обработки рекомендуется воздержаться от формирования и отправки документов в ответ на требование о представлении документов в ФНС';uk='Перетворяться файли зображень сканованих документів, призначених для відправлення у ФПС. "
"До завершення обробки рекомендується втриматися від формування й відправлення документів у відповідь на вимогу про представлення документів у ФПС'");
...
"До завершения обработки рекомендуется воздержаться от формирования и отправки документов в ответ на требование о представлении документов в ФНС';uk='Заповнюються дані єдиного списку документів для представлення у ФПС. "
"До завершення обробки рекомендується втриматися від формування й відправлення документів у відповідь на вимогу про представлення документів у ФПС'");
...
"До завершения обработки рекомендуется воздержаться от формирования заявлений на изменение реквизитов 1С-Отчетности.';uk='Копирование сертификатов 1С-Отчетности в общее хранилище сертификатов."
...
Процедура РазнестиДублиПредопределенныхЭлементовВИБ()
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='БРО. Восстановление предопределенных элементов';uk='БРО. Восстановление предопределенных элементов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,
			...
			Если ЗначениеЗаполнено(Сертификат) Тогда
				...
				Исключение
					ЗаписьЖурналаРегистрации(
						НСтр("ru='Электронный документооборот с контролирующими органами.Обновление информационной базы.Добавление сертификата';uk='Электронный документооборот с контролирующими органами.Обновление информационной базы.Добавление сертификата'"),
						УровеньЖурналаРегистрации.Ошибка,,,
						СтрШаблон(НСтр("ru='Ошибка при добавление сертификата (отпечаток=%1)';uk='Ошибка при добавление сертификата (отпечаток=%1)'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Выборка.Отпечаток));
				КонецПопытки;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
