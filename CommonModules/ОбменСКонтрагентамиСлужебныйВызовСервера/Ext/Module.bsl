...
Процедура ОбновитьВерсиюЭД(ЭлектронныйДокумент, ПринудительнаяСменаСостоянияВерсии = Ложь, ВерсияФорматаПакета = Неопределено, 
	...
	Если НЕ ЭтоСлужебныйДокумент(ЭлектронныйДокумент)
		...
		Если (СвойстваЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Исходящий
			...
			Если (СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ОбменЗавершен
...
"организация %1, "
"контрагент %2, "
"договор %3.';uk='Не удалось определить настройку ЭДО для параметров: "
"организация %1, "
"контрагент %2, "
"договор %3.'");
	...
	Пока Выборка.Следующий() Цикл
		Если ТипЗнч(Выборка.ДокументПотомок) = Тип("ДокументСсылка.ЭлектронныйДокументВходящий") Тогда
			...
			Исключение
				...
				ВидОперации = НСтр("ru='запись документа-основания ""%1"" в документ ""%2""';uk='запись документа-основания ""%1"" в документ ""%2""'");
		...
		Для каждого ЭлектронныйДокумент Из СтруктураПакетаЭД.МассивЭД Цикл
			...
			Если ЭлектронныйДокумент.Зашифрован Тогда
				...
				Если ПарольДоступаКЗакрытомуКлючу = Неопределено Тогда
					ШаблонСообщения = НСтр("ru='Не удалось получить пароль к сертификату: %1';uk='Не удалось получить пароль к сертификату: %1'");
				...
				Исключение
					ШаблонСообщения = НСтр("ru='Расшифровка пакета ЭД: %1';uk='Расшифровка пакета ЭД: %1'");
					ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
...
"%2.';uk='При распаковке пакета %1 возникла ошибка:"
"%2.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, СтруктураПакетаЭД.ПакетЭДСсылка,
				ОписаниеОшибки);
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru='Распаковка пакета ЭД';uk='Распаковка пакета ЭД'"), ТекстСообщения,
				ТекстСообщения);
		...
		Для каждого Подпись Из СтруктураПакетаЭД.МассивЭП Цикл
			...
			Исключение
				ШаблонСообщения = НСтр("ru='Получение сертификатов из подписи: %1';uk='Получение сертификатов из подписи: %1'");
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Прервать;
			КонецПопытки;
			...
			Если НЕ ЗначениеЗаполнено(Подпись.АдресЭД) Тогда
				ОписаниеОшибки = НСтр("ru='Не удалось найти исходные данные для проверки ответной подписи.';uk='Не удалось найти исходные данные для проверки ответной подписи.'");
				Прервать;
			КонецЕсли;
	...
	Для каждого ПакетЭД Из МассивПакетовЭД Цикл
		...
		Попытка
			...
			Исключение
				...
				КонецЕсли;
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru='Распаковка архива пакета ЭД';uk='Распаковка архива пакета ЭД'"), ТекстОшибки,
					ТекстСообщения);
				...
				Для Каждого Документ Из ДокОборот.Documents.Document Цикл
					...
					Если Документ.Files.ExternalCard <> Неопределено Тогда
						...
						Если КарточкаЭД.Type <> Неопределено Тогда
							...
							Если ИмяТипаЭлектронногоДокумента = "formalizedworkresultvendor"
							...
							ИначеЕсли ИмяТипаЭлектронногоДокумента = "formalizedtradingcustomer"
								...
								Если КарточкаЭД.Description.Title <> Неопределено
									И КарточкаЭД.Description.Title = Строка(Перечисления.ВидыЭД.СоглашениеОбИзмененииСтоимостиПолучатель) Тогда
		...
		Исключение
			ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru='Распаковка пакета ЭД';uk='Распаковка пакета ЭД'"), ТекстОшибки,
				ТекстСообщения);
...
"- полученный пакет не предназначен для текущей ИБ (первичный титул был сформирован в другой базе/копии текущей базы).';uk='Для полученного ответного титула (%1) не найден исходящий ЭД."
"Возможные причины:"
"- первичный титул был удален из текущей базы;"
"- полученный пакет не предназначен для текущей ИБ (первичный титул был сформирован в другой базе/копии текущей базы).'");
									...
									// Возьмем НомерЭД из карточки транспортной информации.
						...
						Если ВладелецЭД = Неопределено Тогда
							...
							Иначе
								...
								ИначеЕсли ЗначениеЗаполнено(ЭлектронныйДокумент.ДополнительныеПараметры)
									...
									|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.УдалитьПроизвольныйЭД КАК ПроизвольныйЭД
									|		ПО ЭДПрисоединенныеФайлы.ВладелецФайла = ПроизвольныйЭД.Ссылка
									|ГДЕ
									|	ЭДПрисоединенныеФайлы.УникальныйИД = &УникальныйИДОснования
									|	И ЭДПрисоединенныеФайлы.ВладелецФайла.Контрагент = &Контрагент
									|	И ЭДПрисоединенныеФайлы.ВладелецФайла.Организация = &Организация";
							...
							Если НЕ ЭлектронныйДокумент.ПроизвольныйЭД Тогда
								...
								Если НЕ ВРег(СпособОбработки) = ВРег("Вручную") Тогда
									...
									Если СтруктураЭД.ВидЭД = Перечисления.ВидыЭД.КаталогТоваров Тогда
										...
										ДокументыУчета.Добавить(ПараметрыПакетаЭД.НастройкаЭДО);
									...
									Если Не ЗначениеЗаполнено(ДокументыУчета) Тогда
										ДокументыУчета = ОбменСКонтрагентамиВнутренний.СохранитьДанныеОбъектаВБД(СтруктураПараметров, СпособОбработки);
									КонецЕсли;
				...
				Если ЭлектронныйДокумент.ТипДанных = "СлужебныйЭД" Тогда
					...
					ИначеЕсли ЭлектронныйДокумент.КодРегламента = "Nonformalized" 
						...
						Или СтруктураФайлаЭД.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.Ошибка Тогда
					...
					Если Не ПервичныйЭДНайден Тогда
						ТекстСообщения = НСтр("ru='Не удалось найти владельца для служебного электронного документа.';uk='Не удалось найти владельца для служебного электронного документа.'");
					...
					Если ЭлектронныйДокументВладелец = Неопределено Тогда
						...
						Параметры.Вставить("ТипЭлементаВерсииЭД", Неопределено);
					...
					Если СтруктураФайлаЭД.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.Ошибка Тогда
						...
						ШаблонСообщения = НСтр("ru='Возникла ошибка при передаче электронного документа %1, получатель %2.';uk='Возникла ошибка при передаче электронного документа %1, получатель %2.'");
...
"Соглашение об обмене электронными документами - %4.';uk='При передаче электронного документа %1, возникла ошибка: %2."
"Документ информационной базы (владелец электронного документа) - %3;"
"Соглашение об обмене электронными документами - %4.'");
						...
						ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru='Передача электронного документа';uk='Передача электронного документа'"),
					...
					Иначе
						...
						Если ДобавленныйФайл = Неопределено Тогда
							...
							СтруктураОбновленияПараметров.Вставить("НомерЭД",                     ЭлектронныйДокумент.НомерЭД);
			...
			Для каждого Подпись Из СтруктураПакетаЭД.МассивЭП Цикл
				...
				Если НЕ ЗначениеЗаполнено(Подпись.СсылкаНаЭДПрисоединенныйФайл) Тогда
					ТекстСообщения = НСтр("ru='Не удалось найти электронный документ для сохранения подписи.';uk='Не удалось найти электронный документ для сохранения подписи.'");
					ВызватьИсключение ТекстСообщения;
				КонецЕсли;
...
"%2';uk='При распаковке пакетаЭД: %1 возникла ошибка:"
"%2'");
...
Процедура ДобавитьПодпись(ПрисоединенныйФайл, ДанныеПодписи) Экспорт
	...
	Исключение
		...
		ШаблонСообщения = НСтр("ru='Ошибка подписи электронного документа %1';uk='Ошибка подписи электронного документа %1'");
		ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%1", ПрисоединенныйФайл);
		ВидОперации = НСтр("ru='установка подписи ЭП';uk='установка подписи ЭП'");
		...
		Попытка
			...
			ИначеЕсли ТипОтправки =  Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском
				...
				Если КоличествоОтправленных <> КоличествоКОтправке Тогда
					ШаблонСообщения = НСтр("ru='Не удалось отправить оператору ЭДО ""%1"".';uk='Не удалось отправить оператору ЭДО ""%1"".'");
...
"%3';uk='Ошибка отправки пакета по настройке: %1, способ обмена: %2"
"%3'");
			КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
				РеквизитыПакетаЭД.НастройкаЭДО, РеквизитыПакетаЭД.СпособОбменаЭД, КраткоеПредставлениеОшибки);
			ШаблонВидаОперации = НСтр("ru='отправка пакета ЭД по настройке: %1, способ обмена %2';uk='отправка пакета ЭД по настройке: %1, способ обмена %2'");
	...
	Если ЗначениеЗаполнено(СсылкаНаВладельца) Тогда
		...
		Если НастройкаЭДСуществует(СсылкаНаВладельца, ПараметрыЭД) Тогда
			Если Не ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ЕстьПравоЧтенияЭД(Ложь) Тогда
				ШаблонСостоянияЭД = НСтр("ru='%1 «Недостаточно прав для чтения ЭДО»';uk='%1 «Недостаточно прав для чтения ЭДО»'");
			...
			Иначе
				...
				Если Не ЗначениеЗаполнено(Результат) Тогда
					Гиперссылка = Ложь;
					Результат = НСтр("ru='ЭДО не начат';uk='ЭДО не начат'");
				КонецЕсли;
				...
				Если СостояниеСоглашения = Перечисления.СостоянияСоглашенийЭД.ПроверкаТехническойСовместимости Тогда
					ШаблонСостоянияЭД = НСтр("ru='%1 (проверка технической совместимости)';uk='%1 (проверка технической совместимости)'");
					Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСостоянияЭД, Результат);
				КонецЕсли;
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(Результат) Тогда
				ШаблонСостоянияЭД = НСтр("ru='%1 (настройка ЭДО не подключена)';uk='%1 (настройка ЭДО не подключена)'");
				Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСостоянияЭД, Результат);
				Гиперссылка = Истина;
			Иначе
			...
			Если Не ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ЕстьПравоНастройкиЭДО(Ложь) Тогда
				ШаблонСостоянияЭД = НСтр("ru='%1 «Недостаточно прав для настройки ЭДО»';uk='%1 «Недостаточно прав для настройки ЭДО»'");
				Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСостоянияЭД, Результат);
				Гиперссылка = Ложь;
			КонецЕсли;
...
Процедура ЗаполнитьТекстПриглашенияКЭДО(ТекстПриглашения, ПараметрыЭД, СсылкаНаВладельца, Гиперссылка) Экспорт
	...
	Если ПараметрыЭД.ВидЭД = Перечисления.ВидыЭД.ПередачаТоваровМеждуОрганизациями
		...
		ТекстПриглашения = НСтр("ru='Нет действующей настройки ЭДО с организацией-получателем';uk='Нет действующей настройки ЭДО с организацией-получателем'");
	ИначеЕсли ЗначениеЗаполнено(ПараметрыЭД.Организация) И ЗначениеЗаполнено(ПараметрыЭД.Контрагент)
		...
		ШаблонТекстаНавигационнойСсылки = НСтр("ru='Пригласить %1 к обмену электронными документами в 1С-ЭДО';uk='Пригласить %1 к обмену электронными документами в 1С-ЭДО'");
		...
		Если Запрос.Выполнить().Пустой() Тогда
			...
			ШаблонТекстаНавигационнойСсылки = НСтр("ru='Подключить ""%1"" к ЭДО';uk='Подключить ""%1"" к ЭДО'");
			...
			Если Выборка.Следующий()
				...
				ШаблонТекстаНавигационнойСсылки = НСтр("ru='С ""%1"" возможен обмен электронными документами';uk='С ""%1"" возможен обмен электронными документами'");
				...
				Если ИмяРеализацияТоваровУслуг <> Неопределено
					...
					ШаблонТекстаНавигационнойСсылки = НСтр("ru='Обмен электронными документами с ""%1"" можно начать прямо сейчас';uk='Обмен электронными документами с ""%1"" можно начать прямо сейчас'");
				ИначеЕсли ИмяПоступлениеТоваровУслуг <> Неопределено
					...
					ШаблонТекстаНавигационнойСсылки = НСтр("ru='Получать электронные документы от ""%1"", а не вводить руками, можно прямо сейчас';uk='Получать электронные документы от ""%1"", а не вводить руками, можно прямо сейчас'");
				КонецЕсли;
...
Функция ЭтоСлужебныйДокумент(ФайлЭД) Экспорт 
	...
	Если ЗначениеЗаполнено(ТипЭлементаВерсииЭД) Тогда
		...
		ВозвращаемоеЗначение = НЕ (ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ДОПУПД
...
Процедура ОпределитьСтатусыПодписей(ЭД, ВыводитьСообщения = Ложь) Экспорт
	...
	Для каждого СтрокаЭП Из ОбъектЭД.ЭлектронныеПодписи Цикл
		...
		Исключение
			...
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВидОперации = НСтр("ru='проверка электронной подписи';uk='проверка электронной подписи'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения);
		КонецПопытки;
	КонецЦикла;
...
Функция ПолучитьДвоичныеДанныеФайла(СсылкаНаФайлЭД, СертификатПодписи) Экспорт
	...
	Если ЗначениеЗаполнено(СертификатПодписи) Тогда    
		...
		Если ЭтоСлужебныйДокумент(СсылкаНаФайлЭД) Тогда
			...
			ПолучаемДанные = ОбменСКонтрагентамиСлужебный.ЭтоОтветныйТитул(ПараметрыФайлаЭД.ТипЭлементаВерсииЭД);
		...
		Если ПолучаемДанные Тогда 
			...
			ИначеЕсли СтрНайти(ПараметрыФайлаЭД.Расширение, "zip") > 0 Тогда
				...
				Исключение
					...
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
						НСтр("ru='Извлечение ЭД CML из архива при подписи';uk='Извлечение ЭД CML из архива при подписи'"), ТекстОшибки, ТекстСообщения);
				КонецПопытки;
				АрхивЧтение.Закрыть();
			...
			Если СтрНайти(ПараметрыФайлаЭД.Расширение, "zip") > 0 Тогда
				...
				Исключение
					ТекстОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru='Формирование архива ЭД при подписи';uk='Формирование архива ЭД при подписи'"),
...
Функция ВыполнитьДействияПоЭД(Знач МассивСсылокНаОбъект,
	...
	ТипыПервичныхЭД.Добавить(Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУПД);
	ТипыПервичныхЭД.Добавить(Перечисления.ТипыЭлементовВерсииЭД.ДИСУКД);
	ТипыПервичныхЭД.Добавить(Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУКД);
	...
	ТипыПервичныхЭД.Добавить(Перечисления.ТипыЭлементовВерсииЭД.АктЗаказчик);
	ТипыПервичныхЭД.Добавить(Перечисления.ТипыЭлементовВерсииЭД.ТОРГ12Покупатель);
	ТипыПервичныхЭД.Добавить(Перечисления.ТипыЭлементовВерсииЭД.СоглашениеОбИзмененииСтоимостиПолучатель);
			...
			Если ТекСтрока.СтатусЭД = Перечисления.СтатусыЭД.Сформирован 
				...
				Исключение
					...
					Если ЗначениеЗаполнено(ТекстСообщения) Тогда
						Операция = НСтр("ru='Утверждение ЭД';uk='Утверждение ЭД'");
	...
	Если ЭлектронноеВзаимодействиеКлиентСервер.ЕстьДействие(Действия, "Подписать") Тогда
		...
		|				ТОГДА ВЫБОР
		|							КОГДА ВТ_ЭД.ТипЭлементаВерсииЭД = ЗНАЧЕНИЕ(Перечисление.ТипыЭлементовВерсииЭД.Торг12Покупатель)
		|								ТОГДА СоглашенияЭДИсходящие.ИсходящийДокумент = ЗНАЧЕНИЕ(Перечисление.ВидыЭД.Торг12Продавец)
		|							КОГДА ВТ_ЭД.ТипЭлементаВерсииЭД = ЗНАЧЕНИЕ(Перечисление.ТипыЭлементовВерсииЭД.СоглашениеОбИзмененииСтоимостиПолучатель)
		|								ТОГДА СоглашенияЭДИсходящие.ИсходящийДокумент = ЗНАЧЕНИЕ(Перечисление.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель)
		|							КОГДА ВТ_ЭД.ТипЭлементаВерсииЭД = ЗНАЧЕНИЕ(Перечисление.ТипыЭлементовВерсииЭД.АктЗаказчик)
		|								ТОГДА СоглашенияЭДИсходящие.ИсходящийДокумент = ЗНАЧЕНИЕ(Перечисление.ВидыЭД.АктИсполнитель)
		|							ИНАЧЕ ВТ_ЭД.ВидЭД = СоглашенияЭДИсходящие.ИсходящийДокумент
		|						КОНЕЦ
...
"не найден ни один из доступных сертификатов ЭП.';uk='В профиле настроек ЭДО: %1"
...
"Проверьте настройки криптографии.';uk='Ошибка подписания электронного документа."
"Проверьте настройки криптографии.'");
			...
			ИначеЕсли ДопПараметры.Свойство("ОшибкаНастройкиСертификата", ОшибкаНастройкиСертификата)
				...
				ТекстСообщения = НСтр("ru='Ошибка подписания электронного документа."
"Проверьте настройки сертификатов.';uk='Ошибка подписания электронного документа."
...
Функция НастройкиЭДОИСертификатыАвторизации(Знач ПрофилиНастроекЭДО, Знач ЭлектронныеДокументы = Неопределено,
	...
	Если ТипЗнч(СтМассивовСтруктурСертификатов) = Тип("Структура") Тогда
		...
		Если ВыполнятьКриптооперацииНаСервере И НЕ ЗначениеЗаполнено(МассивОтпечатковСервер) И ИспользоватьЭП Тогда
			...
			Исключение
				ВидОперации = НСтр("ru='Получение данных сертификатов';uk='Получение данных сертификатов'");
				ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	...
	Если ТипЗнч(СтруктураСертификата) = Тип("Структура") Тогда
		...
		ИначеЕсли СтруктураСертификата.Свойство("МаркерЗашифрованный", МаркерЗашифрованный)
			...
			Попытка
				...
				КонецЕсли;
			Исключение
				Операция = НСтр("ru='Расшифровка маркера';uk='Расшифровка маркера'");
				ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
				ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
//
// Возвращаемое значение:
//  Массив - массив структур подпивей.
//
...
Процедура УдалитьСтаруюВерсиюЭД(ЗаписьНабора)
	...
	Если ЗначениеЗаполнено(СтруктураВерсии.СсылкаНаДокумент)
		И ОбменСКонтрагентамиСлужебный.ВозможноУдалениеЭД(СтруктураВерсии.СостояниеВерсииЭД)
		Тогда
...
Функция СостояниеВерсииЭД(СсылкаНаВладельца)
	...
	Если СтруктураВерсииЭД.Свойство("СостояниеВерсииЭД") Тогда
		...
		ИначеЕсли СтруктураВерсииЭД.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.НеСформирован 
			...
			СостояниеВерсии = НСтр("ru='ЭДО не начат';uk='ЭДО не начат'");
		Иначе
		...
		Если ДобавитьПричинуЗакрытия Тогда
			...
			Причина = СтрЗаменить(НСтр("ru=', причина: %1';uk=', причина: %1'"), "%1", ?(ЗначениеЗаполнено(Комментарий), Комментарий, НСтр("ru='не указана';uk='не вказана'")));
	...
	Если ПустаяСтрока(СостояниеВерсии) И ТипЗнч(СсылкаНаВладельца) = Тип("ДокументСсылка.УдалитьПроизвольныйЭД") Тогда
...
Функция ПолучитьТекстСводногоСостоянияЭД(СсылкаНаВладельца)
	...
	Если ТаблицаДанных.Количество() > 0 Тогда
		...
		Если Строка.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ЗакрытПринудительно Тогда
			ТекстСводногоСостояния = НСтр("ru='ЭДО закрыт принудительно';uk='ЭДО закрыт принудительно'");
		Иначе
			Если ЗначениеЗаполнено(Строка.ДействияСНашейСтороны)
				...
				ТекстСНашейСтороны = НСтр("ru='с нашей стороны';uk='с нашей стороны'");
			КонецЕсли;
			Если ЗначениеЗаполнено(Строка.ДействияСоСтороныДругогоУчастника)
				...
				ТекстСоСтороныДругогоУчастника = НСтр("ru='со стороны других участников';uk='со стороны других участников'");
			КонецЕсли;
			Если ЗначениеЗаполнено(ТекстСНашейСтороны) 
				Или ЗначениеЗаполнено(ТекстСоСтороныДругогоУчастника) Тогда
				
				ТекстСводногоСостояния = НСтр("ru='Требуются действия';uk='Требуются действия'")+ " " + ТекстСНашейСтороны
			...
			ИначеЕсли ЗначениеЗаполнено(Строка.ДействияСНашейСтороны)
				...
				Если Строка.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ОбменЗавершен Тогда
					ТекстСводногоСостояния = НСтр("ru='ЭДО завершен';uk='ЭДО завершен'");
				ИначеЕсли Строка.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ОбменЗавершенСИсправлением Тогда
					ТекстСводногоСостояния = НСтр("ru='ЭДО завершен с исправлением';uk='ЭДО завершен с исправлением'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
"%2';uk='Ошибка при отправке сообщения на сервер электронной почты по профилю настроек ЭДО %1."
"%2'");
		...
		ШаблонСообщения = НСтр("ru='Ошибка при отправке эл.документов по профилю настроек ЭДО: %1, способ обмена: %2.';uk='Ошибка при отправке эл.документов по профилю настроек ЭДО: %1, способ обмена: %2.'"); 
		...
		ВидОперации = НСтр("ru='Отправка эл.документов';uk='Отправка эл.документов'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
							ВидОперации,
							ТекстОшибки,
							ТекстСообщения);
...
Функция СформироватьТемуПисьма(Отправитель, Получатель)
	...
	Возврат НСтр("ru='Обмен эл.документами:';uk='Обмен эл.документами:'")+ " " + Отправитель + ?(ЗначениеЗаполнено(Получатель), " -> " + Получатель, "");
...
Функция ОпределитьСводнуюИнформациюПоСтатусуЭД(СсылкаНаФайлЭД) Экспорт
	...
	СвойстваВладельца = "ВидЭД, ВерсияРегламентаЭДО, НастройкаЭДО, ПрофильНастроекЭДО, ТребуетсяИзвещение";
...
Функция МожноАннулироватьЭтотЭД(Знач СсылкаНаЭД) Экспорт
	...
	СвойстваЭД = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаЭД, "ВладелецФайла.ВидЭД, СтатусЭД, НаправлениеЭД");
	
	МожноАннулировать = ОправленИсходящийТитул(СвойстваЭД)
		Или ДокументооборотЗавершен(СсылкаНаЭД);
...
Функция ЭтоОтветныйДокумент(СсылкаНаФайлЭД,ЭлектронныйДокумент) 
	...
	Иначе
		...
		Результат = ЭлектронныйДокумент.ВидЭД = Перечисления.ВидыЭД.АктНаПередачуПрав;
	КонецЕсли;
...
Функция ЭтоДокументКорректировки(ДобавленныйФайл)
	...
	// ТОРГ12Продавец, ТОРГ12Покупатель или ИзвещениеОПолучении.
	...
	Если ЭтоСлужебныйДокумент(ДобавленныйФайл) Тогда
		...
		Если ДобавленныйФайл.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ТОРГ12Покупатель
			Или ДобавленныйФайл.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.АктЗаказчик Тогда
...
Функция ДокументооборотЗавершен(СсылкаНаЭД)
	...
	Иначе
		...
		|	И НЕ ЭДПрисоединенныеФайлы.ВладелецФайла.ВидЭД В (&СписокВидовИсключений)
		|	И НЕ ЭДПрисоединенныеФайлы.ПометкаУдаления
		...
		МассивВидовИсключений.Добавить(Перечисления.ВидыЭД.СчетНаОплату);
...
Функция ОправленИсходящийТитул(СвойстваЭД)
	
	Результат = Ложь;
	
	Если СвойстваЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Исходящий
		И (СвойстваЭД.ВладелецФайлаВидЭД = Перечисления.ВидыЭД.ТОРГ12Продавец
			Или СвойстваЭД.ВладелецФайлаВидЭД = Перечисления.ВидыЭД.АктИсполнитель)
		И (СвойстваЭД.СтатусЭД = Перечисления.СтатусыЭД.Отправлен
			Или СвойстваЭД.СтатусЭД = Перечисления.СтатусыЭД.Доставлен
			Или СвойстваЭД.СтатусЭД = Перечисления.СтатусыЭД.ПереданОператору) Тогда
...
Процедура ЗаполнитьДокументИБПоЭД(СсылкаНаВладельца, СсылкаНаЭД, ДокЗаполнен =Ложь, ТипДокумента = "")
	...
	Если СтруктураЭД.Загружен Тогда
		...
		Попытка
			...
			ДокументыУчета = ОбменСКонтрагентамиВнутренний.СохранитьДанныеОбъектаВБД(СтруктураПараметров, ТипДокумента);
...
"%2';uk='Не удалось заблокировать объект %1"
"%2'");
		
		ВидОперации = НСтр("ru='Сохранение результатов проверки подписей';uk='Сохранение результатов проверки подписей'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Функция ПодписатьЭДОпределеннымСертификатом(ДобавленныеФайлы,
	...
	Если КриптоСертификат <> Неопределено Тогда
		...
		Исключение
			...
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru='проверка сертификата на корректность';uk='проверка сертификата на корректность'"),
											  ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
											  ТекстСообщения);
			Для Каждого НеПодписанныйЭД Из ФайлыНаПодпись Цикл
				МассивНеПодписанныхЭД.Добавить(НеПодписанныйЭД);
		...
		Для Каждого ТекДокумент Из ФайлыНаПодпись Цикл
			...
			СтруктураДанныхПодписи = Новый Структура("Сертификат, ПарольПользователя, Комментарий", КриптоСертификат, ПараметрыСертификата.ПарольПользователя,
				НСтр("ru='Подписание электронного документа';uk='Підписання електронного документу'"));
			Попытка
...
"%2';uk='Ошибка подписи на сервере. Необходимо провести тест сертификата криптографии для: %1."
"%2'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, СертификатПодписи,
					КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
						НСтр("ru='установка подписи ЭП';uk='установка подписи ЭП'"),
						ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
						ТекстСообщения);
...
"Необходимо указать сетевой каталог для обмена.';uk='Указанный каталог %1 не может использоваться для обмена, так как он не доступен с сервера."
...
Процедура ЗаполнитьДанныеПодписантаЭД(ДанныеФайлаЭД, ПространствоИменСхемы, ПараметрыЭД, СертификатПодписи)
	...
	ИначеЕсли ОбменСКонтрагентамиСлужебный.ЭтоИзвещениеОПолучении(ПараметрыЭД.ТипЭлементаВерсииЭД)
		Или ОбменСКонтрагентамиСлужебный.ЭтоУведомлениеОбУточнении(ПараметрыЭД.ТипЭлементаВерсииЭД) Тогда
...
Процедура УстановитьОтветственногоЭД(Знач СписокОбъектов, Знач НовыйОтветственный, КоличествоОбработанныхЭД,
	...
	Исключение
		ОтменитьТранзакцию();
		ТекстОшибки = СтрШаблон(НСтр("ru='Не удалось установить ответственного электронного документа (%1). %2';uk='Не удалось установить ответственного электронного документа (%1). %2'"), 
			Выборка.Ссылка,
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция СформироватьСлужебныеЭД(МассивЭД, ВидЭД, ТекстУточнения = "") Экспорт
	...
	Для Каждого СсылкаНаЭД Из МассивЭД Цикл
		...
		Если ЭтоИзвещение Тогда
			...
			Если НЕ (ПараметрыЭД.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ПервичныйЭД
				...
				ШаблонСообщения = НСтр("ru='Для типа ЭД %1 не предусмотрено формирование Извещения о получении.';uk='Для типа ЭД %1 не предусмотрено формирование Извещения о получении.'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
		...
		ИначеЕсли СтрНайти(ДопИнформацияПоЭД.Расширение, "zip") > 0 Тогда
			...
			Исключение
				...
				Если НЕ ЭлектронноеВзаимодействиеСлужебный.ВозможноИзвлечьФайлы(АрхивЧтение, ПапкаДляРаспаковки) Тогда
					ТекстСообщения = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ПолучитьСообщениеОбОшибке("006");
				КонецЕсли;
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru='Распаковка пакета ЭД';uk='Распаковка пакета ЭД'"), ТекстОшибки, ТекстСообщения);
				ЭлектронноеВзаимодействиеСлужебный.УдалитьВременныеФайлы(ПапкаДляРаспаковки);
				Возврат "";
			КонецПопытки;
