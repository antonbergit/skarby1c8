...
"Для включения подсистемы обратитесь к администратору программы.';uk='Елемент ""%1"" не підключений, тому що підсистема ""%2"" відключена у налаштуваннях програми."
	...
	Если Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.Отчет
		...
		Исключение
			ПараметрыОтчета.ТекстОшибки = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='При подключении дополнительного отчета ""%1"" возникла ошибка:';uk='При підключенні додаткового звіту ""%1"" виникла помилка:'"), Строка(Ссылка))
...
"Обратитесь к администратору программы.';uk='Функціональна опція ""%2"" відключена в налаштуваннях програми."
		...
		Для Каждого СтрокаТаблицы Из Найденные Цикл
			Если Не ПодсистемаВключена Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<Отчет ""%1"" недоступен, т.к. дополнительные отчеты и обработки отключены в настройках программы.';uk='<Звіт ""%1"" недоступний, тому що додаткові звіти і обробки відключені в налаштуваннях програми.'"),
...
Процедура ПриПолученииПрочихНастроек(СведенияОПользователе, Настройки) Экспорт
	...
	НазваниеНастройки = НСтр("ru='Настройки быстрого доступа к дополнительным отчетам и обработкам';uk='Налаштування швидкого доступу до додаткових звітів й обробок'");
...
"Для загрузки нового объекта нажмите ""Создать"".';uk='Вид об`єкта, що завантажується (%1) не відповідає поточному (%2)."
