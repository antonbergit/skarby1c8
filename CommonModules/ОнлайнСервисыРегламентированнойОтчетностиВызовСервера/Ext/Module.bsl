	...
	Если НЕ ПустаяСтрока(СтруктураURI.ПутьНаСервере) Тогда
		...
		Если ПозицияПараметры > 0 Тогда
			...
			Если ПозицияПараметраПуть > 0 Тогда
				...
				АдресСайта = "http://downloads.1c.eu/ftp//" + ПараметрПутьНаСервере;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработатьИЗаписатьВнешнийОтчетВСправочникРО(ПараметрыВнешнейФормы, ПутьКФайлу)
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru='Не удалось распаковать архив с внешней формой отчета.';uk='Не удалось распаковать архив с внешней формой отчета.'");
		ЗаписьЖурналаРегистрации(НСтр("ru='Загрузка внешних регламентированных отчетов';uk='Загрузка внешних регламентированных отчетов'"), УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
	...
	Если СвойстваМанифеста["ИДКонфигурации"] <> СвойстваКонфигурации["ИДКонфигурации"] Тогда
		ТекстОшибки = НСтр("ru='Не пройдена проверка на соответствие идентификатора конфигурации в отчете идентификатору текущей конфигурации';uk='Не пройдена проверка на соответствие идентификатора конфигурации в отчете идентификатору текущей конфигурации'");
	...
	Если СвойстваМанифеста["ВерсияКонфигурации"] <> СвойстваКонфигурации["ВерсияКонфигурации"] Тогда
		ТекстОшибки = НСтр("ru='Не пройдена проверка на соответствие версии в отчете версии текущей конфигурации.';uk='Не пройдена проверка на соответствие версии в отчете версии текущей конфигурации.'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
...
"Описание:"
"""%1""';uk='Получение объекта отчета по ссылке."
"Описание:"
"""%1""'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ ВнешнийОтчетИспользовать Тогда 
		...
		ТекстДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Внешний отчет ""%1"" вер. ""%2"" отключен.';uk='Внешний отчет ""%1"" вер. ""%2"" отключен.'"), СвойстваМанифеста["Имя"], СвойстваМанифеста["КраткаяВерсияВнешнегоОтчета"]);
	...
	Если ИспользуетсяВнешнийОтчет Тогда
		...
		Если РезультатСравнениеВерсийХранимогоИЗагружаемогоОтчетов  = -1 Тогда // если загружаемый отчет старее хранимого
			ТекстОшибки = НСтр("ru='Не пройдена проверка на сравнение версий манифеста выбранного и хранимого отчетов';uk='Не пройдена проверка на сравнение версий манифеста выбранного и хранимого отчетов'");
	...
	Если СвойстваКонфигурации["ИДКонфигурации"] <> СвойстваОтчета["ИДКонфигурации"] Тогда
		ТекстОшибки = НСтр("ru='Не пройдена проверка на соответствие идентификатора конфигурации идентификатору конфигурации в текущем подключаемом отчете';uk='Не пройдена проверка на соответствие идентификатора конфигурации идентификатору конфигурации в текущем подключаемом отчете'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
	...
	Если ИспользуетсяВнешнийОтчет Тогда
		Если СвойстваОтчета["ВерсияКонфигурации"] <> СвойстваКонфигурации["ВерсияКонфигурации"] Тогда
			ТекстОшибки = НСтр("ru='Не пройдена проверка на соответствие версий текущего и подключаемого отчетов';uk='Не пройдена проверка на соответствие версий текущего и подключаемого отчетов'");
			ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ВРег(СвойстваОтчета["ИсточникОтчета"]) <> ВРег(СвойстваКонфигурации["ИсточникОтчета"]) Тогда
		ТекстОшибки = НСтр("ru='Не пройдена проверка на соответствие имен метаданных текущего и подключаемого отчетов';uk='Не пройдена проверка на соответствие имен метаданных текущего и подключаемого отчетов'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
	...
	Если ИспользуетсяВнешнийОтчет Тогда
		...
		Если РезультатСравнениеВерсийХранимогоИЗагружаемогоОтчетов  = -1 Тогда // если загружаемый отчет старее хранимого
			ТекстОшибки = НСтр("ru='Не пройдена проверка на сравнение версий выбранного и хранимого отчетов';uk='Не пройдена проверка на сравнение версий выбранного и хранимого отчетов'");
			ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		...
		ТекстИнформации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Внешний отчет вер. ""%1"" успешно загружен.';uk='Внешний отчет вер. ""%1"" успешно загружен.'"), СвойстваОтчета["КраткаяВерсияВнешнегоОтчета"]);
...
"Описание:"
"""%1""';uk='Запись объекта отчета с новыми данными."
"Описание:"
"""%1""'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
...
Функция ПрочитатьМанифест(КаталогРаспаковки)
	...
	Если Чтение.ТипУзла <> ТипУзлаXML.НачалоЭлемента ИЛИ Чтение.Имя <> "Info" Тогда
		ВызватьИсключение(НСтр("ru='Ошибка чтения XML. Неверный формат файла. Ожидается начало элемента Info.';uk='Ошибка чтения XML. Неверный формат файла. Ожидается начало элемента Info.'"));
	КонецЕсли;
...
Функция ФайлОтчетаИзАрхива(КаталогРаспаковки)
	...
	Если НайденныеФайлы.Количество() = 0 Тогда
		ВызватьИсключение(НСтр("ru='Ошибка получения отчета. Архив не содержит файл с расширением ""*.erf"".';uk='Ошибка получения отчета. Архив не содержит файл с расширением ""*.erf"".'"));
	Иначе
...
Процедура ЗаписатьВЖурналРегистрации(Уровень, ТекстОшибки)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Загрузка внешних регламентированных отчетов';uk='Загрузка внешних регламентированных отчетов'"), Уровень,,, ТекстОшибки);
