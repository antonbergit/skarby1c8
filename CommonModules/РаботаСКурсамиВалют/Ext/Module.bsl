...
Функция ДобавитьВалютыПоКоду(Знач Коды) Экспорт
	...
	Для каждого Код Из Коды Цикл
		...
		Если ВалютаСсылка.Пустая() Тогда
			...
			Попытка
				НоваяСтрока.ПараметрыПрописиНаУкраинском = ЗаписьОКВ.UkrNumerationItemOptions;
			Исключение
			КонецПопытки;
...
Функция СформироватьСуммуПрописью(СуммаЧислом, Валюта, ВыводитьСуммуБезКопеек = Ложь, КодЯзыка = "ru") Экспорт
	...
	Результат = ЧислоПрописью(Сумма, ФорматСтрока, ПараметрыПрописи);
...
Процедура ЗагрузитьАктуальныйКурс(ПараметрыЗагрузки = Неопределено, АдресРезультата = Неопределено) Экспорт
	...
	Иначе
		Результат = ЗагрузитьКурсыВалютПоПараметрам(ПараметрыЗагрузки.СписокВалют,
			ПараметрыЗагрузки.НачалоПериода, ПараметрыЗагрузки.КонецПериода, ПриЗагрузкеВозниклиОшибки, ПараметрыЗагрузки.ИсточникДанных, ПараметрыЗагрузки.КодДоступа, ПараметрыЗагрузки.БукваДиска);
	КонецЕсли;
	...
	Если ПриЗагрузкеВозниклиОшибки Тогда
		...
		ВызватьИсключение НСтр("ru='Загрузка курсов не выполнена. Подробности см. в журнале регистрации.';uk='Завантаження курсів не виконане. Подробиці див. в журналі реєстрації.'");
	Иначе
		ЗаписьЖурналаРегистрации(
			ИмяСобытия,
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "1.0.5.9";
	Обработчик.Процедура = "РаботаСКурсамиВалют.ОбновитьФорматХраненияПрописиНаУкраинскомЯзыке";
...
// Обработчик обновления формата хранения прописей при переходе на более новую версию БСП.
//
Процедура ОбновитьФорматХраненияПрописиНаУкраинскомЯзыке() Экспорт
	...
	Пока ВыборкаВалют.Следующий() Цикл
		...
		Род1 = НРег(Лев(СокрЛП(СтрПолучитьСтроку(СтрокаПараметров, 4)), 1));
		Род2 = НРег(Лев(СокрЛП(СтрПолучитьСтроку(СтрокаПараметров, 8)), 1));
		Объект.ПараметрыПрописиНаУкраинском = 
					  СокрЛП(СтрПолучитьСтроку(СтрокаПараметров, 1)) + ", "
					+ СокрЛП(СтрПолучитьСтроку(СтрокаПараметров, 2)) + ", "
					+ СокрЛП(СтрПолучитьСтроку(СтрокаПараметров, 3)) + ", "
					+ Род1 + ", "
...
Процедура ОбновитьСведенияОВалюте937() Экспорт
	...
	Если Не Валюта.Пустая() Тогда
		...
		Валюта.НаименованиеПолное = НСтр("ru='Боливар';uk='Болівар'");
...
Функция ЗагрузитьКурсыВалютПоПараметрам(Знач Валюты, Знач НачалоПериодаЗагрузки, Знач ОкончаниеПериодаЗагрузки, 
	ПриЗагрузкеВозниклиОшибки = Ложь, Знач ИсточникДанных = Ложь, Знач КодДоступа = "", Знач БукваДиска = "") Экспорт
...
"%3"
"Возможно, нет доступа к веб сайту с курсами валют, либо указана несуществующая валюта.';uk='Неможливо одержати файл даних з курсами валюти (%1 - %2):"
"%3"
"Можливо, немає доступу до веб сайту з курсами валют, або зазначена неіснуюча валюта.'"),
				Валюта.КодВалюты,
				Валюта.Валюта,
...
"Курсы запрещенного периода были пропущены при загрузке.';uk='Завантаження курсів валюти %1(%2) обмежено датою заборони змін %3."
