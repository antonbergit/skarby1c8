...
Процедура ПолучитьМаркерБезопасностиПослеПолучения(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		...
		Попытка
			ВызватьИсключение(НСтр("ru='Пользователь отказался от ввода пароля';uk='Пользователь отказался от ввода пароля'"));
		Исключение
...
Процедура Расшифровать(ОповещениеОЗавершении, ЗашифрованныеДанные, ТипШифрования = "CMS", ПараметрыШифрования = Неопределено) Экспорт
	...
	Попытка
		...
		Если СвойстваСообщения.Тип = "envelopedData" Тогда
			...
			КонецЦикла;			
		Иначе
			ВызватьИсключение(НСтр("ru='Недопустимое значение параметра ЗашифрованныеДанные - файл не является криптосообщением';uk='Недопустимое значение параметра ЗашифрованныеДанные - файл не является криптосообщением'"));
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Идентификаторы) Тогда
			ВызватьИсключение(НСтр("ru='В хранилище отсутствуют сертификаты для расшифровки сообщения.';uk='В хранилище отсутствуют сертификаты для расшифровки сообщения.'"));
		КонецЕсли;
	...
	Если ВходящийКонтекст.ТекущийИдентификатор < ВходящийКонтекст.Идентификаторы.Количество() Тогда
		Попытка
			...
			Если ТипЗнч(Результат) = Тип("Структура") Тогда
				...
				Иначе
					ВызватьИсключение(НСтр("ru='Неизвестный код возврата';uk='Неизвестный код возврата'"));
				КонецЕсли;
			Иначе
				РезультатВыполнения = Новый Структура;
				РезультатВыполнения.Вставить("Выполнено", Истина);
