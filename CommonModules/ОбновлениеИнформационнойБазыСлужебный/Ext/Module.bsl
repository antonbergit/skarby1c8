...
"Буде виконане відновлення інформаційної бази.'"),
	...
	Сообщение = НСтр("ru='Для обновления программы на новую версию будут выполнены обработчики: %1';uk='Для оновлення програми на нову версію будуть виконані "
"обробники: %1'");
...
"(ролі ""Адміністратор системи"" і ""Повні права""). '");
...
"ознакомиться с документацией на ИТС (http://its.1c.ru/db/bspdoc)';uk='Файл поставки 1С:Бібліотека стандартних підсистем не призначений для створення"
"інформаційних баз по шаблонові. Перед використанням необхідно"
"ознайомитися з документацією на ІТС (http://its.1c.ru/db/bspdoc)'");
...
Функция ВыполнитьИтерациюОбновления(Знач ИтерацияОбновления, Знач Параметры) Экспорт
	...
	Если ТекущаяВерсияИБ <> "0.0.0.0"
		...
		Если ВыполняемыеОбработчики = Неопределено Тогда
			...
			Иначе
				ШаблонСообщения = НСтр("ru='Не найден план обновления библиотеки %1 с версии %2 на версию %3';uk='Не знайдений план відновлення бібліотеки %1 з версії %2 на версію %3'");
			КонецЕсли;
...
""
"Подробности ошибки:"
"%1';uk='Неможливо виконати відновлення інформаційної бази:"
...
"""%1"".';uk='Не вказана процедура заповнення даних"
...
""
"Данные свойства не могут одновременно принимать значение ""Истина"".';uk='У відкладеного обробника ""%1"""
"некоректно заповнені значення властивостей:"
" - ""ЗапускатьТолькоВГлавномУзле"""
" - ""ЗапускатьИВПодчиненномУзлеРИБСФильтрами""."
""
"Ці властивості не можуть одночасно приймати значення ""Істина"".'"), 
			Обработчик.Процедура);
		ЗаписатьОшибку(ТекстОшибки);
		ТекстОшибок = ТекстОшибок + ТекстОшибки + Символы.ПС;
	КонецЕсли;
...
""
"Данное свойство не может принимать значение ""Истина"" у отложенного обработчика.';uk='У відкладеного обробника ""%1"""
"вказано неприпустиме значення властивості ""ОбщиеДанные""."
""
"Ця властивість не може приймати значення ""Істина"" у відкладеного обробника.'"), 
			Обработчик.Процедура);
		ЗаписатьОшибку(ТекстОшибки);
		ТекстОшибок = ТекстОшибок + ТекстОшибки + Символы.ПС;
	КонецЕсли;
...
"';uk='У следующих отложенных обработчиков неуникальные идентификаторы:"
"'");
...
""
"%1';uk='Наступні обробники не підтримують оновлення без установки монопольного режиму:"
""
"%1'");
...
"установлена версия выше, чем в метаданных. (%1, должна быть %2)';uk='У спільному макеті ОписаниеИзмененийСистемы для одного з розділів змін"
...
"Выполнение прервано для предотвращения зацикливания механизма обработки данных.';uk='Перевищена припустима кількість запусків процедури оновлення."
"Виконання перерване для запобігання зациклення механізму обробки даних.'");
...
"Выполнение прервано из-за явной ошибки в коде обработчика.';uk='Обробник оновлення не ініціалізував параметр ОбработкаЗавершена."
		...
		ИначеЕсли ПараметрыОбновления.ПараллельныйРежим Тогда
			...
			Если Не ЕстьОбработанныеОбъекты
				...
				Если ЧислоПопыток >= 2 Тогда
					ТекстИсключения = НСтр("ru='Произошло зацикливание процедуры обработки данных. Выполнение прервано.';uk='Сталося зациклення процедури обробки даних. Виконання перерване.'");
					ВызватьИсключение ТекстИсключения;
				Иначе
...
"заполнен список блокируемых объектов, но не задано свойство ""ПроцедураПроверки"".';uk='У відкладеного обробника оновлення ""%1"""
"заповнений список об`єктів, що блокуються, але не задана властивість ""ПроцедураПроверки"".'");
...
"""%1"""
"отложенного обработчика обновления"
"""%2"".';uk='Виконується процедура заповнення даних"
"""%1"""
"відкладеного обробника оновлення"
"""%2"".'");
...
"""%2"""
"произошла ошибка:"
"""%3"".';uk='При виклику процедури заповнення даних"
"""%1"""
"відкладеного обробника оновлення"
"""%2"""
"сталася помилка:"
