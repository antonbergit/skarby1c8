...
Процедура ОткрытьФормуНастройкиСканирования() Экспорт
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		ТекстСообщения = НСтр("ru='Сканирование поддерживается только в клиенте под управлением ОС Windows.';uk='Сканування підтримується тільки в клієнтові під управлінням ОС Windows.'");
		ПоказатьПредупреждение(, ТекстСообщения);
		Возврат;
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		ТекстСообщения = НСтр("ru='Сканирование не поддерживается в веб-клиенте.';uk='Сканування не підтримується у веб-клієнтові.'");
		ПоказатьПредупреждение(, ТекстСообщения);
		Возврат;
	КонецЕсли;
...
"Установить компоненту?';uk='Для продовження роботи необхідно встановити компоненту сканування. "
