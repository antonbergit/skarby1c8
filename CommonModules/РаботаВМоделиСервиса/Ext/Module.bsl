...
Процедура УстановитьРазделениеСеанса(Знач Использование = Неопределено, Знач ОбластьДанных = Неопределено) Экспорт
	...
	Если НЕ ОбщегоНазначенияПовтИсп.СеансЗапущенБезРазделителей() Тогда
		ВызватьИсключение(НСтр("ru='Изменить разделение сеанса возможно только из сеанса запущенного без указания разделителей';uk='Змінити поділ сеансу можливо лише з сеансу запущеного "
"без вказання роздільників'"));
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииПолучениеФайлаИзХранилища() Экспорт
	...
	Возврат НСтр("ru='Работа в модели сервиса.Получение файла из хранилища';uk='Робота в моделі сервісу.Одержання файлу зі сховища'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ПолучитьФайлИзХранилища(Знач ИдентификаторФайла, Знач ПараметрыПодключения, 
		...
		Попытка
			...
			Если СвойстваФайла.Существует() Тогда
				...
				ЗаписатьВЖурналСобытиеФайловогоХранилища(
					НСтр("ru='Извлечение';uk='Вилучення'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					ИдентификаторФайла,
	...
	Если ФайлВременный.Существует() Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Выполнение операции удаления временного файла';uk='Виконання операції видалення тимчасового файлу'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ ПрочитанZipФайл Тогда
		...
		ВызватьИсключение(НСтр("ru='При чтении zip файла произошла ошибка.';uk='При читанні zip файлу сталася помилка.'"));
...
""
"Для використання цієї конфігурації в моделі сервісу потрібно "
"впровадити бібліотеку"
...
""
"Используемая версия: %1, требуется версия не младше %2!';uk='Для використання конфігурації в моделі сервісу з поточною версією "
"БСП потрібно"
...
"Дана підсистема поставляється до складу бібліотеки технології "
"сервісу, яка повинна окремо впроваджуватися до складу конфігурації."
...
"повинні бути включені до складу підписок на події (%2), "
"що контролюють"
...
"Следующие объекты не удовлетворяют этому критерию: %2';uk='Всі об`єкти метаданих конфігурації повинні бути розділені не "
"більш ніж одним роздільником БСП (%1)."
