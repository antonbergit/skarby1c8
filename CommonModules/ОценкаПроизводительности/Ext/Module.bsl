	...
	Попытка
		...
		Для Каждого КлючеваяОперация Из КлючевыеОперации Цикл
			...
			Если НЕ РезультатЗапроса.Пустой() Тогда
				...
				КлючеваяОперацияСсылка = Выборка.Ссылка;
				КлючеваяОперацияОбъект = КлючеваяОперацияСсылка.ПолучитьОбъект();
				КлючеваяОперацияОбъект.Наименование = КлючеваяОперация.ИмяКлючевойОперацииНовое;
...
Процедура ЗафиксироватьДлительностьКлючевойОперации(
	...
	Если БезопасныйРежим() <> Ложь Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru='Работа подсистемы оценка производительности в безопасном режиме не поддерживается';uk='Робота підсистеми оцінка продуктивності у безпечному режимі не підтримується'", Метаданные.ОсновнойЯзык.КодЯзыка),
